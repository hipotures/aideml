...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    field_median = out.groupby(keys, sort=False)["LapTime_Delta"].transform("median")
    out["pace_residual_vs_field"] = out["LapTime_Delta"] - field_median

    tmp = out[seq_keys + ["LapNumber", "pace_residual_vs_field"]].copy()
    tmp["_orig_idx"] = np.arange(len(tmp))
    tmp = tmp.sort_values(seq_keys + ["LapNumber", "_orig_idx"], kind="mergesort")

    g = tmp.groupby(seq_keys, sort=False)["pace_residual_vs_field"]
    tmp["_res_lag1"] = g.shift(1)
    tmp["_res_lag3"] = g.shift(3)

    tmp["pace_residual_roll3_mean_prior"] = (
        tmp.groupby(seq_keys, sort=False)["_res_lag1"]
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=seq_keys, drop=True)
    )
    tmp["pace_residual_roll5_max_prior"] = (
        tmp.groupby(seq_keys, sort=False)["_res_lag1"]
        .rolling(5, min_periods=1)
        .max()
        .reset_index(level=seq_keys, drop=True)
    )
    tmp["pace_residual_slope3_prior"] = (tmp["_res_lag1"] - tmp["_res_lag3"]) / 2.0

    tmp = tmp.sort_values("_orig_idx", kind="mergesort")
    for c in [
        "pace_residual_roll3_mean_prior",
        "pace_residual_roll5_max_prior",
        "pace_residual_slope3_prior",
    ]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000036"],
        "research_usage_note": "Verified assigned hypothesis 000036.",
    }, sort_keys=True))
