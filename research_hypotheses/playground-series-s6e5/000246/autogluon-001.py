...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in cat_cols:
        out[col] = out[col].astype("string").fillna("NA").str.strip().str.upper()

    out["Year_str"] = out["Year"].astype("Int64").astype("string").fillna("NA")
    out["Stint_str"] = out["Stint"].astype("Int64").astype("string").fillna("NA")
    ...
    lap_bucket = (
        pd.cut(
            ...
            bins=[-np.inf, 5, 10, 15, 20, 30, 40, 50, 60, np.inf],
            labels=[
                "L00_05",
                "L06_10",
                "L11_15",
    ...
    out["LapBucket"] = lap_bucket

    out["Driver_Race_Year"] = out["Driver"] + "_" + out["Race_Year"]
    out["Driver_Compound"] = out["Driver"] + "_" + out["Compound"]
    out["Race_Year_LapBucket"] = out["Race_Year"] + "_" + out["LapBucket"]
    out["Compound_Stint"] = out["Compound"] + "_" + out["Stint_str"]

    out["RaceDriver_Order"] = (
        out.groupby(["Race_Year", "Driver"], sort=False).cumcount().astype("int16")
    )
    out["RaceDriver_ReverseOrder"] = (
        out.groupby(["Race_Year", "Driver"], sort=False)["RaceDriver_Order"]
        .transform("max")
        .sub(out["RaceDriver_Order"])
        .astype("int16")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000246"],
        "research_usage_note": "Verified assigned hypothesis 000246.",
    }, sort_keys=True))
