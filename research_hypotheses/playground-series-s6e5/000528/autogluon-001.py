...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Race_Compound"] = (
        out["Race"].astype("string") + "__x__" + out["Compound"].astype("string")
    ).astype("category")
    out["Driver_Compound"] = (
        out["Driver"].astype("string") + "__x__" + out["Compound"].astype("string")
    ).astype("category")
    out["Race_Stint"] = (
        out["Race"].astype("string") + "__x__" + out["Stint"].astype("string")
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000528.",
    }, sort_keys=True))
