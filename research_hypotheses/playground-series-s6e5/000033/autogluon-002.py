...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ordered = out.assign(_row_order=np.arange(len(out))).sort_values(
        keys + ["LapNumber", "_row_order"], kind="mergesort"
    )
    g = ordered.groupby(keys, sort=False, observed=True)
    ...
    reset = (
        prev_tyre.isna()
        | (ordered["TyreLife"] < prev_tyre)
        | (ordered["Compound"] != prev_compound)
        | (ordered["Stint"] > prev_stint)
        | (ordered["PitStop"] == 1)
    ).astype(np.int16)

    ordered["_obs_reset"] = reset
    ordered["_obs_stint_no"] = g["_obs_reset"].cumsum().astype(np.int16)

    reset_lap = ordered["LapNumber"].where(ordered["_obs_reset"].eq(1))
    ordered["_obs_start_lap"] = reset_lap.groupby(
        [ordered[k] for k in keys], sort=False, observed=True
    ).ffill()
    ordered["_obs_age"] = (
        (ordered["LapNumber"] - ordered["_obs_start_lap"] + 1)
        .clip(lower=1)
        .astype(np.float32)
    )

    prev_age = ordered.groupby(keys, sort=False, observed=True)["_obs_age"].shift(1)
    last_len_at_reset = prev_age.where(ordered["_obs_reset"].eq(1))
    ordered["_prior_obs_stint_len"] = (
        last_len_at_reset.groupby([ordered[k] for k in keys], sort=False, observed=True)
        .ffill()
        .fillna(0)
        .astype(np.float32)
    )

    denom_prior = ordered["_prior_obs_stint_len"].replace(0, np.nan)
    out.loc[ordered.index, "obs_stint_reset"] = ordered["_obs_reset"].astype(np.int8)
    out.loc[ordered.index, "obs_stint_number"] = ordered["_obs_stint_no"].astype(
        np.int16
    )
    out.loc[ordered.index, "obs_stint_age"] = ordered["_obs_age"].astype(np.float32)
    out.loc[ordered.index, "prior_obs_stint_length"] = ordered[
        "_prior_obs_stint_len"
    ].astype(np.float32)
    out.loc[ordered.index, "obs_age_vs_tyrelife"] = (
        ordered["_obs_age"] / ordered["TyreLife"].clip(lower=1)
    ).astype(np.float32)
    out.loc[ordered.index, "obs_age_vs_prior_stint"] = (
        (ordered["_obs_age"] / denom_prior)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype(np.float32)
    )
    out.loc[ordered.index, "tyrelife_vs_prior_stint"] = (
        (ordered["TyreLife"] / denom_prior)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype(np.float32)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000033"],
        "research_usage_note": "Verified assigned hypothesis 000033.",
    }, sort_keys=True))
