...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    keys = ["Race", "Driver"]
    sort_cols = ["Race", "Driver", "Year", "LapNumber"]
    ...
    stint_keys = [ordered["Race"], ordered["Driver"], ordered["_obs_stint_no"]]
    ordered["_obs_start_lap"] = ordered.groupby(stint_keys, sort=False, observed=True)[
        "LapNumber"
    ].transform("min")
    ordered["_obs_age"] = (
    ...
    stint_len = ordered.groupby(stint_keys, sort=False, observed=True)[
        "_obs_age"
    ].transform("max")
    last_len_at_reset = stint_len.where(ordered["_obs_reset"].eq(1))
    ordered["_prior_obs_stint_len"] = (
        last_len_at_reset.groupby(
            [ordered["Race"], ordered["Driver"]], sort=False, observed=True
        )
        .shift(1)
        .ffill()
        .fillna(0)
