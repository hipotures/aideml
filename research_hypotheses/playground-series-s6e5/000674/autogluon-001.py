...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["lap_x_stint"] = out["LapNumber"] * out["Stint"]
    ...
    out["laptime_per_tyre_lap"] = out["LapTime (s)"] / (out["TyreLife"] + eps)
    out["delta_per_tyre_lap"] = out["LapTime_Delta"] / (out["TyreLife"] + eps)
    out["position_pressure"] = (21 - out["Position"]) * out["RaceProgress"]
    out["position_change_abs"] = out["Position_Change"].abs()
    out["late_stint_pressure"] = out["TyreLife"] * out["RaceProgress"] * out["Stint"]
    out["pit_window_proxy"] = (
        (out["TyreLife"] >= 10).astype("int8")
        + (out["TyreLife"] >= 18).astype("int8")
        + (out["TyreLife"] >= 28).astype("int8")
        + (out["RaceProgress"] >= 0.25).astype("int8")
        + (out["RaceProgress"] >= 0.50).astype("int8")
        + (out["RaceProgress"] >= 0.75).astype("int8")
    ).astype("int8")

    dry = out["Compound"].astype(str).isin(["SOFT", "MEDIUM", "HARD"])
    wet = out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"])
    out["is_dry_compound"] = dry.astype("int8")
    out["is_wet_compound"] = wet.astype("int8")
    out["soft_tyre_age"] = (
        out["Compound"].astype(str).eq("SOFT") * out["TyreLife"]
    ).astype("float32")
    out["hard_tyre_age"] = (
        out["Compound"].astype(str).eq("HARD") * out["TyreLife"]
    ).astype("float32")
    out["wet_tyre_age"] = (wet * out["TyreLife"]).astype("float32")

    for c in ["Driver", "Race", "Compound", "query_driver_race_stint"]:
        freq = out[c].map(out[c].value_counts(dropna=False)).astype("float32")
        out[c + "_freq"] = np.log1p(freq)

    grp = out.groupby(["Driver", "Race", "Year", "Stint"], observed=True, sort=False)
    out["stint_row_count"] = grp["LapNumber"].transform("count").astype("float32")
    out["stint_lap_rank_pct"] = (
        grp["LapNumber"].rank(method="average", pct=True).astype("float32")
    )
    out["stint_tyre_rank_pct"] = (
        grp["TyreLife"].rank(method="average", pct=True).astype("float32")
    )
    out["stint_deg_rank_pct"] = (
        grp["Cumulative_Degradation"].rank(method="average", pct=True).astype("float32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000674"],
        "research_usage_note": "Verified assigned hypothesis 000674.",
    }, sort_keys=True))
