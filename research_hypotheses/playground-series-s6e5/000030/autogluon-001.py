...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_time = out["LapTime (s)"].astype("float32").replace(0, np.nan)
    tyre_life = out["TyreLife"].astype("float32")
    race_phase = out["RaceProgress"].astype("float32")

    out["lap_delta_pos"] = delta.clip(lower=0)
    out["lap_delta_abs"] = delta.abs()
    out["lap_delta_ratio"] = (
        (delta / lap_time).replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    out["lap_delta_abs_ratio"] = (
        (out["lap_delta_abs"] / lap_time).replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    ...
    expected_life = out["Compound"].map(compound_life).astype("float32").fillna(28.0)
    tyre_age_ratio = (
        (tyre_life / expected_life).replace([np.inf, -np.inf], np.nan).fillna(0)
    )

    out["compound_expected_life"] = expected_life
    out["tyre_age_to_expected_life"] = tyre_age_ratio
    out["delta_pos_x_tyre_age"] = out["lap_delta_pos"] * tyre_age_ratio
    out["delta_abs_x_tyre_age"] = out["lap_delta_abs"] * tyre_age_ratio
    out["delta_ratio_x_tyre_age"] = out["lap_delta_ratio"] * tyre_age_ratio
    out["delta_pos_x_race_phase"] = out["lap_delta_pos"] * race_phase
    out["delta_abs_ratio_x_race_phase"] = out["lap_delta_abs_ratio"] * race_phase

    lap_group = ["Year", "Race", "LapNumber"]
    lap_median = (
        out.groupby(lap_group, sort=False)["LapTime_Delta"]
        .transform("median")
        .astype("float32")
    )
    lap_std = (
    ...
    out["lap_delta_field_diff"] = (delta - lap_median).fillna(0)
    out["lap_delta_field_abs_diff"] = out["lap_delta_field_diff"].abs()
    out["lap_delta_field_z"] = (
        ((delta - lap_median) / lap_std).replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    out["field_diff_x_tyre_age"] = out["lap_delta_field_diff"] * tyre_age_ratio
    out["field_abs_diff_x_race_phase"] = out["lap_delta_field_abs_diff"] * race_phase
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000030"],
        "research_usage_note": "Verified assigned hypothesis 000030.",
    }, sort_keys=True))
