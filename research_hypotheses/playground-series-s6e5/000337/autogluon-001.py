...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = key_cols + ["LapNumber", "_orig_order"]
    out = out.sort_values(sort_cols, kind="mergesort")

    race_key = [out["Year"], out["Race"]]
    driver_key = [out["Year"], out["Race"], out["Driver"]]
    base_cols = [
        "LapTime_Delta",
        "LapTime (s)",
    ...
    for c in base_cols:
        race_med = out.groupby(race_key, sort=False)[c].transform("median")
        out[c + "_race_demean"] = out[c] - race_med

    prior_pit = out.groupby(driver_key, sort=False)["PitStop"].shift(1).fillna(0)
    out["_seg"] = prior_pit.groupby(driver_key, sort=False).cumsum()
    seg_key = [out["Year"], out["Race"], out["Driver"], out["_seg"]]

    for c in base_cols:
        g = out.groupby(seg_key, sort=False)[c]
        lag1 = g.shift(1)
        lag2 = g.shift(2)
        lag3 = g.shift(3)

        out[c + "_lag1"] = lag1
        out[c + "_d1_lag"] = lag1 - lag2
        out[c + "_d2_lag"] = lag1 - 2.0 * lag2 + lag3

        r3_mean = (
            lag1.groupby(seg_key, sort=False)
        ...
        out[c + "_roll3_mean_lag"] = r3_mean
        out[c + "_roll5_mean_lag"] = r5_mean
        out[c + "_roll3_slope_lag"] = (lag1 - lag3) / 2.0
        out[c + "_ewm3_lag"] = lag1.groupby(seg_key, sort=False).transform(
            lambda s: s.ewm(span=3, adjust=False, min_periods=1).mean()
        )
        out[c + "_shock_lag"] = (
            (lag1 - r5_mean).abs() > (2.0 * r3_std.fillna(0))
        ).astype("int8")

    out["tyre_life_x_degradation_lag"] = out["TyreLife"] * out[
        "Cumulative_Degradation_lag1"
    ].fillna(0)
    out["pace_cliff_score"] = (
        out["LapTime_Delta_d1_lag"].fillna(0)
        + out["LapTime (s)_roll3_slope_lag"].fillna(0)
        + 0.01 * out["Cumulative_Degradation_d1_lag"].fillna(0)
    )
    out["laps_since_prior_pit"] = out.groupby(seg_key, sort=False).cumcount()

    out = out.sort_values("_orig_order", kind="mergesort").drop(
        columns=["_orig_order", "_seg"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000337"],
        "research_usage_note": "Verified assigned hypothesis 000337.",
    }, sort_keys=True))
