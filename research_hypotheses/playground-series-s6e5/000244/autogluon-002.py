...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("category")
    year_race_lap = [out["Year"], out["Race"], out["LapNumber"]]
    ...
    race_progress = out["RaceProgress"].astype(float)
    pit_now = ((tyre_life <= 2.0) & (out["Stint"].astype(float) > 1.0)).astype("float32")

    lap_median = lap_time.groupby(year_race_lap, observed=True).transform("median")
    lap_std = (
        lap_time.groupby(year_race_lap, observed=True).transform("std").fillna(0.0)
    )
    out["pace_vs_lap_median"] = lap_time - lap_median
    out["pace_vs_lap_median_z"] = out["pace_vs_lap_median"] / (lap_std + eps)

    delta_median = lap_delta.groupby(year_race_lap, observed=True).transform("median")
    out["delta_vs_lap_median"] = lap_delta - delta_median
    out["positive_delta_worn"] = np.maximum(lap_delta, 0.0) * np.log1p(tyre_life)

    pit_loss_obs = (lap_time - lap_median).where(pit_now > 0.5)
    race_pit_loss = pit_loss_obs.groupby(race, observed=True).transform("median")
    global_pit_loss = pit_loss_obs.median()
    if not np.isfinite(global_pit_loss):
        global_pit_loss = out["pace_vs_lap_median"].quantile(0.95)
    race_pit_loss = race_pit_loss.fillna(global_pit_loss).clip(lower=0.0)
    out["race_est_pit_loss"] = race_pit_loss

    lap_pos_spread = lap_std.clip(lower=0.25)
    cumulative_slot_gap = (lap_pos_spread * np.sqrt(lap_num.clip(lower=1.0))).clip(
        lower=0.5
    )
    out["approx_positions_lost_if_pit"] = (race_pit_loss / cumulative_slot_gap).clip(
        0.0, 20.0
    )
    out["approx_rejoin_position"] = (
        position + out["approx_positions_lost_if_pit"]
    ).clip(1.0, 20.0)
    out["pit_loss_affordability"] = (20.0 - out["approx_rejoin_position"]) / 19.0

    slot_frac = out["approx_rejoin_position"] - np.floor(out["approx_rejoin_position"])
    out["rejoin_slot_center"] = 1.0 - (np.abs(slot_frac - 0.5) * 2.0)
    out["inside_approx_rejoin_gap"] = (
        (out["rejoin_slot_center"] > 0.55)
        & (out["approx_rejoin_position"] < 19.5)
        & (race_progress.between(0.18, 0.88))
    ).astype("int8")

    fresh_mask = (tyre_life <= 3.0) & (out["Stint"].astype(float) > 1.0)
    warmup_obs = np.maximum(lap_delta, 0.0).where(fresh_mask)
    race_warmup = warmup_obs.groupby(race, observed=True).transform("median")
    global_warmup = warmup_obs.median()
    if not np.isfinite(global_warmup):
        global_warmup = np.maximum(lap_delta, 0.0).median()
    race_warmup = race_warmup.fillna(global_warmup).clip(lower=0.0)
    out["race_fresh_tyre_warmup_penalty"] = race_warmup

    tyre_age_ratio = tyre_life / (lap_num + eps)
    out["undercut_pressure"] = (
        np.maximum(lap_delta, 0.0) * tyre_age_ratio * (1.0 + race_progress)
    )
    out["undercut_overcut_balance"] = out["undercut_pressure"] - race_warmup
    out["overcut_favored_warmup"] = (race_warmup > out["undercut_pressure"]).astype(
        "int8"
    )

    out["pit_utility_score"] = (
        out["undercut_overcut_balance"]
        + out["pit_loss_affordability"] * 5.0
        + out["inside_approx_rejoin_gap"] * 2.0
        - np.maximum(-out["pace_vs_lap_median"], 0.0)
    )

    out["compound_is_wet_or_inter"] = (
        out["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    )
    out["late_race_pit_penalty"] = (race_progress > 0.82).astype("int8") * out[
        "race_est_pit_loss"
    ]
    out["pit_window_context"] = (
        race_progress.between(0.18, 0.78).astype("int8")
        * np.log1p(tyre_life)
        * (1.0 + np.maximum(lap_delta, 0.0))
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000244"],
        "research_usage_note": "Verified assigned hypothesis 000244.",
    }, sort_keys=True))
