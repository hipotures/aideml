...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_key = [c for c in ["Race", "Year", "LapNumber"] if c in out.columns]
    race_key = [c for c in ["Race", "Year"] if c in out.columns]
    ...
    if lap_key:
        g = out.groupby(lap_key, sort=False)
        out["grid_size_lap"] = g["LapNumber"].transform("size").astype("float32")
        ...
        for c in num_cols:
            ...
            out[f"{c}_grid_mean"] = mean.astype("float32")
            out[f"{c}_grid_std"] = std.astype("float32")
            out[f"{c}_vs_grid_mean"] = (out[c] - mean).astype("float32")
            out[f"{c}_grid_range"] = (mx - mn).astype("float32")
        ...
        for c in [
            ...
            out[f"{c}_grid_rank_pct"] = (
                g[c].rank(method="average", pct=True).astype("float32")
            )

        if "PitStop" in out.columns:
            pit_sum = g["PitStop"].transform("sum")
            out["current_lap_pit_count"] = pit_sum.astype("float32")
            out["current_lap_other_pit_count"] = (pit_sum - out["PitStop"]).astype(
                "float32"
            )
            out["current_lap_pit_rate"] = (
                pit_sum / out["grid_size_lap"].clip(lower=1)
            ).astype("float32")

        if "Compound" in out.columns:
            for comp in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
                ind = (out["Compound"].astype(str) == comp).astype("float32")
                out[f"grid_compound_{comp.lower()}_count"] = (
                    ind.groupby([out[k] for k in lap_key], sort=False)
                    .transform("sum")
                    .astype("float32")
                )
                out[f"is_{comp.lower()}"] = ind.astype("int8")

    if race_key and "LapNumber" in out.columns:
        rg = out.groupby(race_key, sort=False)
        max_lap = rg["LapNumber"].transform("max").replace(0, np.nan)
        out["lap_frac_of_race_seen"] = (out["LapNumber"] / max_lap).astype("float32")

    if "TyreLife" in out.columns and "LapNumber" in out.columns:
        out["tyre_life_frac_lap"] = (
            out["TyreLife"] / out["LapNumber"].clip(lower=1)
        ).astype("float32")

    if "Position" in out.columns and "grid_size_lap" in out.columns:
        out["position_pct_grid"] = (
            out["Position"] / out["grid_size_lap"].clip(lower=1)
        ).astype("float32")

    for c in [x for x in ["Driver", "Race", "Compound"] if x in out.columns]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000390.",
    }, sort_keys=True))
