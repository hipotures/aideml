...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    race_keys = ["Year", "Race"] if "Year" in out.columns else ["Race"]
    lap_keys = race_keys + ["LapNumber"]

    out["lap_median_time"] = out.groupby(lap_keys)["LapTime (s)"].transform("median")
    out["lap_time_dev"] = out["LapTime (s)"] - out["lap_median_time"]
    out["lap_time_dev_abs"] = out["lap_time_dev"].abs()

    s = out.sort_values(lap_keys + ["Position"], kind="mergesort")
    g = s.groupby(lap_keys, sort=False)

    for k in (1, 2):
        s[f"time_dev_ahead_{k}"] = g["lap_time_dev"].shift(k)
        s[f"time_dev_behind_{k}"] = g["lap_time_dev"].shift(-k)
        s[f"tyrelife_ahead_{k}"] = g["TyreLife"].shift(k)
        s[f"tyrelife_behind_{k}"] = g["TyreLife"].shift(-k)
        s[f"pos_ahead_{k}"] = g["Position"].shift(k)
        s[f"pos_behind_{k}"] = g["Position"].shift(-k)

        s[f"tyrelife_delta_ahead_{k}"] = s["TyreLife"] - s[f"tyrelife_ahead_{k}"]
        s[f"tyrelife_delta_behind_{k}"] = s["TyreLife"] - s[f"tyrelife_behind_{k}"]
        s[f"time_dev_delta_ahead_{k}"] = s["lap_time_dev"] - s[f"time_dev_ahead_{k}"]
        s[f"time_dev_delta_behind_{k}"] = s["lap_time_dev"] - s[f"time_dev_behind_{k}"]

    s["adj_time_gap_ahead"] = (s["LapTime (s)"] - g["LapTime (s)"].shift(1)).abs()
    s["adj_time_gap_behind"] = (s["LapTime (s)"] - g["LapTime (s)"].shift(-1)).abs()
    s["local_pack_gap_sum"] = s["adj_time_gap_ahead"].fillna(0) + s[
        "adj_time_gap_behind"
    ].fillna(0)
    s["local_pack_gap_min"] = s[["adj_time_gap_ahead", "adj_time_gap_behind"]].min(
        axis=1
    )
    s["local_pack_density"] = 1.0 / (1.0 + s["local_pack_gap_sum"])
    s["close_pack_flag"] = (s["local_pack_gap_min"].fillna(99) <= 1.0).astype("int8")

    out = s.sort_index(kind="mergesort")

    anomaly = (out["LapTime (s)"] - out["lap_median_time"]).clip(lower=0)
    pit_anom = anomaly.where(out["PitStop"].eq(1))
    race_pit_loss = pit_anom.groupby([out[k] for k in race_keys]).transform("median")
    global_pit_loss = (
        float(pit_anom.median())
        if pit_anom.notna().any()
        else float(anomaly.quantile(0.9))
    )
    out["pit_loss_proxy"] = race_pit_loss.fillna(global_pit_loss).clip(
        lower=5, upper=45
    )

    race_gap = out.groupby(race_keys)["adj_time_gap_ahead"].transform("median")
    global_gap = (
        float(out["adj_time_gap_ahead"].median())
        if out["adj_time_gap_ahead"].notna().any()
        else 1.5
    )
    out["median_lap_gap_proxy"] = race_gap.fillna(global_gap).clip(
        lower=0.25, upper=5.0
    )

    lost_positions = np.rint(out["pit_loss_proxy"] / out["median_lap_gap_proxy"]).clip(
        1, 20
    )
    out["position_after_loss_proxy"] = (out["Position"] + lost_positions).clip(1, 20)
    out["pit_window_traffic_flag"] = (out["position_after_loss_proxy"] >= 11).astype(
        "int8"
    )
    out["pit_window_free_air_flag"] = (
        (out["position_after_loss_proxy"] <= 10)
        & (out["local_pack_gap_sum"].fillna(0) >= 2.5)
    ).astype("int8")
    out["undercut_pressure_proxy"] = (
        (out["tyrelife_delta_ahead_1"].fillna(0) >= 3)
        & (out["time_dev_delta_ahead_1"].fillna(0) > 0)
    ).astype("int8")
    out["overcut_pressure_proxy"] = (
        (out["tyrelife_delta_behind_1"].fillna(0) >= 3)
        & (out["time_dev_delta_behind_1"].fillna(0) < 0)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000662"],
        "research_usage_note": "Verified assigned hypothesis 000662.",
    }, sort_keys=True))
