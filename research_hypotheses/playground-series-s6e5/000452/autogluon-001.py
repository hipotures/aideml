...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cnt = g.cumcount()
    denom = cnt.replace(0, np.nan).astype(float)
    ...
    out["compound_wear_prior"] = (
        out["Compound"].map(compound_prior).astype(float).fillna(1.0)
    )
    ...
    past_lap_mean = (g["LapTime (s)"].cumsum() - lap_time) / denom
    past_delta_mean = (g["LapTime_Delta"].cumsum() - lap_delta) / denom
    past_deg_mean = (g["Cumulative_Degradation"].cumsum() - cum_deg) / denom

    pace_resid = (lap_time - past_lap_mean).fillna(0.0).clip(-15.0, 25.0)
    delta_resid = (lap_delta - past_delta_mean).fillna(0.0).clip(-10.0, 10.0)
    deg_resid = (cum_deg - past_deg_mean).fillna(0.0).clip(-100.0, 150.0)

    deg_rate = (cum_deg / tyre_life.clip(lower=1.0)).clip(-20.0, 60.0)
    life_pressure = (tyre_life * out["compound_wear_prior"]).astype(float)
    progress_pressure = (race_progress * tyre_life).astype(float)
    ...
    out["_latent_seed"] = (
        ...
        + 0.16 * pace_resid
        + 0.10 * delta_resid
        + 0.07 * progress_pressure
        + 0.03 * deg_resid
    )

    seed = out["_latent_seed"].astype(float)
    seed_csum = g["_latent_seed"].cumsum()
    seed_sq_csum = (seed**2).groupby([out[c] for c in group_cols], sort=False).cumsum()

    out["latent_wear_level"] = ((seed_csum - seed) / denom).fillna(
        0.55 * life_pressure + 0.25 * deg_rate + 0.20 * progress_pressure
    )

    seed_diff = g["_latent_seed"].diff().fillna(0.0).clip(-8.0, 8.0)
    diff_csum = seed_diff.groupby([out[c] for c in group_cols], sort=False).cumsum()
    out["latent_wear_slope"] = ((diff_csum - seed_diff) / denom).fillna(0.0)

    past_mean = (seed_csum - seed) / denom
    past_sq_mean = (seed_sq_csum - (seed**2)) / denom
    out["latent_wear_uncertainty"] = np.sqrt(
        np.maximum((past_sq_mean - past_mean.pow(2)).fillna(0.0), 0.0)
    )

    out["teacher_expected_next_pace"] = (
        out["latent_wear_level"]
        + 0.35 * out["latent_wear_slope"]
        + 0.15 * out["latent_wear_uncertainty"]
    )

    out["teacher_pace_gap"] = seed - out["teacher_expected_next_pace"]
    out["teacher_state_x_progress"] = out["latent_wear_level"] * race_progress
    out["teacher_state_x_compound"] = (
        out["latent_wear_level"] * out["compound_wear_prior"]
    )
    out["teacher_slope_x_life"] = out["latent_wear_slope"] * tyre_life

    out = out.sort_values("_row_id", kind="mergesort").drop(
        columns=["_row_id", "_latent_seed"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000452"],
        "research_usage_note": "Verified assigned hypothesis 000452.",
    }, sort_keys=True))
