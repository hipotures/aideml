...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["CompoundScore"] = (
        out["Compound"].astype(str).map(compound_score_map).astype("float32")
    )

    lap_keys = ["Year", "Race", "LapNumber"]
    lap_group = out.groupby(lap_keys, sort=False)

    out["FieldSize"] = lap_group["Driver"].transform("size").astype("float32")
    out["LapTimeFieldPct"] = (
        lap_group["LapTime (s)"].rank(method="average", pct=True).astype("float32")
    )
    out["TyreLifeFieldPct"] = (
        lap_group["TyreLife"].rank(method="average", pct=True).astype("float32")
    )
    out["DegradationFieldPct"] = (
        lap_group["Cumulative_Degradation"]
        .rank(method="average", pct=True)
        .astype("float32")
    )
    out["PositionPct"] = (out["Position"] / out["FieldSize"].clip(lower=1)).astype(
        "float32"
    )

    order = out.sort_values(lap_keys + ["Position", "Driver"], kind="mergesort").index
    ordered = out.loc[
        order,
        lap_keys
        + [
    ...
    ordered_group = ordered.groupby(lap_keys, sort=False)

    ordered["AheadLapTime"] = ordered_group["LapTime (s)"].shift(1)
    ordered["BehindLapTime"] = ordered_group["LapTime (s)"].shift(-1)
    ordered["AheadTyreLife"] = ordered_group["TyreLife"].shift(1)
    ordered["BehindTyreLife"] = ordered_group["TyreLife"].shift(-1)
    ordered["AheadDegradation"] = ordered_group["Cumulative_Degradation"].shift(1)
    ordered["BehindDegradation"] = ordered_group["Cumulative_Degradation"].shift(-1)
    ordered["AheadCompoundScore"] = ordered_group["CompoundScore"].shift(1)
    ordered["BehindCompoundScore"] = ordered_group["CompoundScore"].shift(-1)
    ...
    out.loc[order, neighbor_cols] = ordered[neighbor_cols].to_numpy()

    out["AheadLapTimeGapProxy"] = (
        (out["LapTime (s)"] - out["AheadLapTime"]).fillna(0.0).astype("float32")
    )
    out["BehindLapTimeGapProxy"] = (
        (out["BehindLapTime"] - out["LapTime (s)"]).fillna(0.0).astype("float32")
    )
    out["LocalTrafficTightness"] = (
        1.0 / (out["AheadLapTimeGapProxy"].abs() + 1.0)
        + 1.0 / (out["BehindLapTimeGapProxy"].abs() + 1.0)
    ).astype("float32")

    out["TyreLifeVsAhead"] = (
        (out["TyreLife"] - out["AheadTyreLife"]).fillna(0.0).astype("float32")
    )
    out["TyreLifeVsBehind"] = (
        (out["TyreLife"] - out["BehindTyreLife"]).fillna(0.0).astype("float32")
    )
    out["DegradationVsAhead"] = (
        (out["Cumulative_Degradation"] - out["AheadDegradation"])
        .fillna(0.0)
        .astype("float32")
    )
    out["DegradationVsBehind"] = (
        (out["Cumulative_Degradation"] - out["BehindDegradation"])
        .fillna(0.0)
        .astype("float32")
    )
    out["CompoundVsAhead"] = (
        (out["CompoundScore"] - out["AheadCompoundScore"]).fillna(0.0).astype("float32")
    )
    out["CompoundVsBehind"] = (
        (out["CompoundScore"] - out["BehindCompoundScore"])
        .fillna(0.0)
        .astype("float32")
    )

    out["MidfieldPressure"] = (
        (1.0 - (out["PositionPct"] - 0.5).abs() * 2.0).clip(lower=0.0).astype("float32")
    )
    out["FreshTyreGainProxy"] = (
        0.40 * out["TyreLifeFieldPct"]
        + 0.35 * out["DegradationFieldPct"]
        + 0.25 * out["LapTimeFieldPct"]
    ).astype("float32")
    out["TrackPositionCostProxy"] = (
        0.60 * (1.0 - out["PositionPct"]) + 0.40 * (1.0 - out["RaceProgress"])
    ).astype("float32")
    out["TrafficPenaltyProxy"] = (
        out["LocalTrafficTightness"] * out["MidfieldPressure"]
    ).astype("float32")
    out["UndercutOpportunityScore"] = (
        out["FreshTyreGainProxy"]
        - 0.35 * out["TrafficPenaltyProxy"]
        - 0.25 * out["TrackPositionCostProxy"]
    ).astype("float32")

    out["RelativePaceVsField"] = (
        out["LapTime (s)"] - lap_group["LapTime (s)"].transform("median")
    ).astype("float32")
    out["RelativeTyreLifeVsField"] = (
        out["TyreLife"] - lap_group["TyreLife"].transform("median")
    ).astype("float32")
    out["RelativeDegradationVsField"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000681"],
        "research_usage_note": "Verified assigned hypothesis 000681.",
    }, sort_keys=True))
