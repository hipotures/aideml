...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit_col = out["PitStop"].astype("float32")
    pit_lap_mean = g["PitStop"].transform("mean").astype("float32")
    race_pit_rate = rg["PitStop"].transform("mean").astype("float32")
    out["same_lap_pit_density"] = pit_lap_mean
    out["race_pit_rate"] = race_pit_rate

    pit_loss_laps = (
        out["LapTime_Delta"]
        .where(out["PitStop"].eq(1))
        .groupby([out["Year"], out["Race"]])
        .transform("median")
    )
    race_delta_median = rg["LapTime_Delta"].transform("median")
    pit_loss = (pit_loss_laps - race_delta_median).abs()
    pit_loss = pit_loss.replace([np.inf, -np.inf], np.nan).fillna(pit_loss.median())
    pit_loss = (
        pit_loss.fillna(out["LapTime_Delta"].abs().median())
