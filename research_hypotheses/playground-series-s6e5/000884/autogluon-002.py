...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rg = out.groupby(race_keys, sort=False)

    field_size = g["Position"].transform("count").astype("float32")
    out["field_size_lap"] = field_size
    out["cars_ahead"] = (out["Position"].astype("float32") - 1).clip(lower=0)
    out["cars_behind"] = (field_size - out["Position"].astype("float32")).clip(lower=0)
    out["position_frac"] = out["Position"].astype("float32") / field_size.clip(lower=1)

    out["tyrelife_rank_lap"] = (
        g["TyreLife"].rank(method="average", pct=True).astype("float32")
    )
    out["lapdelta_rank_lap"] = (
        g["LapTime_Delta"].rank(method="average", pct=True).astype("float32")
    )
    out["degradation_rank_lap"] = (
        g["Cumulative_Degradation"].rank(method="average", pct=True).astype("float32")
    )

    out["ahead_tyre_age_proxy"] = (
        out["tyrelife_rank_lap"] * out["cars_ahead"] / field_size.clip(lower=1)
    ).astype("float32")
    out["behind_fresh_risk_proxy"] = (
        (1.0 - out["tyrelife_rank_lap"]) * out["cars_behind"] / field_size.clip(lower=1)
    ).astype("float32")
    out["slow_old_tyre_pressure"] = (
        out["tyrelife_rank_lap"] * out["lapdelta_rank_lap"]
    ).astype("float32")

    pit_lap_mean = g["PitStop"].transform("mean").astype("float32")
    out["same_lap_pit_density"] = pit_lap_mean
    lap_rates = out[keys].copy()
    lap_rates["lap_pit_rate"] = pit_lap_mean.to_numpy()
    lap_rates = lap_rates.drop_duplicates(keys).sort_values(keys, kind="mergesort")
    lap_rates["race_pit_rate"] = (
        lap_rates.groupby(race_keys, sort=False)["lap_pit_rate"]
        .transform(lambda s: s.shift(1).expanding(min_periods=1).mean())
        .fillna(0.0)
    )
    out = out.merge(
        lap_rates[keys + ["race_pit_rate"]], on=keys, how="left", sort=False
    )
    race_pit_rate = out["race_pit_rate"].fillna(0.0).astype("float32")

    rg = out.groupby(race_keys, sort=False)
    race_delta_scale = rg["LapTime_Delta"].transform("std").fillna(0.0).abs()
    pit_loss = (
        (18.0 + 1.5 * race_delta_scale + 0.15 * out["LapTime_Delta"].abs())
        .clip(lower=10, upper=35)
        .astype("float32")
    )
    out["race_pit_loss_proxy"] = pit_loss

    pos_penalty = np.ceil(pit_loss / 2.5).astype("float32")
    rejoin_pos = (out["Position"].astype("float32") + pos_penalty).clip(lower=1)
    out["rejoin_position_proxy"] = rejoin_pos
    out["rejoin_position_frac"] = (
        (rejoin_pos / field_size.clip(lower=1)).clip(0, 1).astype("float32")
    )

    clean_air_gap = (field_size - rejoin_pos).clip(lower=0)
    out["clean_air_gap_after_pit"] = clean_air_gap.astype("float32")
    out["rejoin_in_traffic"] = (
        (clean_air_gap <= 2) & (rejoin_pos <= field_size + 1)
    ).astype("int8")

    undercut_reach = (
        (out["cars_ahead"] <= pos_penalty + 1).astype("float32")
        * out["tyrelife_rank_lap"].astype("float32")
        * (1.0 - out["lapdelta_rank_lap"].astype("float32") + 0.05)
    )
    out["undercut_reach_if_pit_now"] = undercut_reach.astype("float32")

    overcut_risk = (
        out["cars_behind"]
        / field_size.clip(lower=1)
        * (1.0 - out["tyrelife_rank_lap"].astype("float32"))
        * (pit_lap_mean + race_pit_rate + 0.01)
    )
    out["overcut_risk_if_stay_out"] = overcut_risk.astype("float32")

    out["pit_release_counterfactual"] = (
        out["undercut_reach_if_pit_now"]
        - out["overcut_risk_if_stay_out"]
        - out["rejoin_in_traffic"].astype("float32") * 0.25
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000884"],
        "research_usage_note": "Verified assigned hypothesis 000884.",
    }, sort_keys=True))
