...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_wet"] = (comp == "WET").astype("int8")
    out["is_intermediate"] = (comp == "INTERMEDIATE").astype("int8")
    out["is_wet_inter"] = ((out["is_wet"] == 1) | (out["is_intermediate"] == 1)).astype(
        "int8"
    )
    out["is_slick"] = (1 - out["is_wet_inter"]).astype("int8")
    out["compound_regime"] = np.where(
        out["is_wet"] == 1,
        "wet",
        np.where(out["is_intermediate"] == 1, "intermediate", "slick"),
    )
    ...
    lap_agg = (
        out.groupby(race_lap_keys, sort=False)
        .agg(
            field_wet_share=("is_wet", "mean"),
            field_inter_share=("is_intermediate", "mean"),
            field_wet_inter_share=("is_wet_inter", "mean"),
            field_slick_share=("is_slick", "mean"),
            lap_delta_median=("LapTime_Delta", "median"),
            lap_delta_std=("LapTime_Delta", "std"),
            lap_time_median=("LapTime (s)", "median"),
            lap_time_std=("LapTime (s)", "std"),
        )
        .reset_index()
        .sort_values(race_lap_keys)
    )

    for c in ["lap_delta_std", "lap_time_std"]:
        lap_agg[c] = lap_agg[c].fillna(0.0)

    shifted_cols = [
        "field_wet_share",
        "field_inter_share",
        "field_wet_inter_share",
        "field_slick_share",
        "lap_delta_median",
        "lap_delta_std",
        "lap_time_median",
    ...
    for c in shifted_cols:
        lap_agg[f"prev_{c}"] = lap_agg.groupby(race_keys, sort=False)[c].shift(1)

    lap_agg["prev_field_wet_inter_share"] = lap_agg[
        "prev_field_wet_inter_share"
    ].fillna(0.0)
    lap_agg["prev_field_slick_share"] = lap_agg["prev_field_slick_share"].fillna(1.0)
    lap_agg["prev_lap_delta_median"] = lap_agg["prev_lap_delta_median"].fillna(
        lap_agg["lap_delta_median"]
    )
    lap_agg["prev_lap_delta_std"] = lap_agg["prev_lap_delta_std"].fillna(
        lap_agg["lap_delta_std"]
    )
    lap_agg["prev_lap_time_std"] = lap_agg["prev_lap_time_std"].fillna(
        lap_agg["lap_time_std"]
    )

    lap_agg["wet_inter_share_change"] = (
        lap_agg["field_wet_inter_share"] - lap_agg["prev_field_wet_inter_share"]
    )
    lap_agg["slick_share_change"] = (
        lap_agg["field_slick_share"] - lap_agg["prev_field_slick_share"]
    )
    lap_agg["field_delta_spike"] = (
        lap_agg["lap_delta_median"] - lap_agg["prev_lap_delta_median"]
    )
    lap_agg["field_delta_dispersion_spike"] = (
        lap_agg["lap_delta_std"] - lap_agg["prev_lap_delta_std"]
    )
    lap_agg["field_laptime_dispersion_spike"] = (
        lap_agg["lap_time_std"] - lap_agg["prev_lap_time_std"]
    )

    lap_agg["slick_to_wet_wave"] = (
        (lap_agg["field_wet_inter_share"] >= 0.15)
        & (lap_agg["wet_inter_share_change"] >= 0.10)
    ).astype("int8")
    lap_agg["wet_to_slick_wave"] = (
        (lap_agg["field_slick_share"] >= 0.50)
        & (lap_agg["wet_inter_share_change"] <= -0.10)
    ).astype("int8")
    lap_agg["field_transition_wave"] = (
        lap_agg["slick_to_wet_wave"] | lap_agg["wet_to_slick_wave"]
    ).astype("int8")

    out = out.merge(lap_agg, on=race_lap_keys, how="left", sort=False)

    first_wet_lap = (
        out.loc[out["is_wet_inter"] == 1, race_keys + ["LapNumber"]]
        .groupby(race_keys, sort=False)["LapNumber"]
        .min()
        .rename("first_wet_lap_in_race")
        .reset_index()
    )
    out = out.merge(first_wet_lap, on=race_keys, how="left", sort=False)
    out["first_wet_lap_in_race"] = (
        out["first_wet_lap_in_race"].fillna(999).astype("int16")
    )
    out["race_has_any_wet_inter"] = (out["first_wet_lap_in_race"] < 999).astype("int8")
    out["at_or_after_first_wet"] = (
        out["LapNumber"] >= out["first_wet_lap_in_race"]
    ).astype("int8")
    out["laps_since_first_wet"] = np.where(
        out["race_has_any_wet_inter"] == 1,
        out["LapNumber"] - out["first_wet_lap_in_race"],
        -1,
    ).astype("int16")
    out["wet_regime_active"] = (
        (out["field_wet_inter_share"] > 0) | (out["at_or_after_first_wet"] == 1)
    ).astype("int8")

    tmp = out[driver_keys + ["LapNumber", "is_slick", "is_wet_inter"]].copy()
    tmp["_row_id"] = np.arange(len(tmp))
    tmp = tmp.sort_values(driver_keys + ["LapNumber", "_row_id"])

    tmp["prev_driver_is_wet_inter"] = (
        tmp.groupby(driver_keys, sort=False)["is_wet_inter"]
        .shift(1)
        .fillna(0)
        .astype("int8")
    )
    tmp["prev_driver_is_slick"] = (
        tmp.groupby(driver_keys, sort=False)["is_slick"]
        .shift(1)
        .fillna(0)
        .astype("int8")
    )
    tmp["driver_seen_slick_so_far"] = (
        tmp.groupby(driver_keys, sort=False)["is_slick"].cummax().astype("int8")
    )
    tmp["driver_seen_wet_inter_so_far"] = (
        tmp.groupby(driver_keys, sort=False)["is_wet_inter"].cummax().astype("int8")
    )
    tmp["driver_slick_to_wet"] = (
        (tmp["prev_driver_is_slick"] == 1) & (tmp["is_wet_inter"] == 1)
    ).astype("int8")
    tmp["driver_wet_to_slick"] = (
        (tmp["prev_driver_is_wet_inter"] == 1) & (tmp["is_slick"] == 1)
    ).astype("int8")

    tmp = tmp.sort_values("_row_id")
    for c in [
        "prev_driver_is_wet_inter",
        "prev_driver_is_slick",
        "driver_seen_slick_so_far",
        "driver_seen_wet_inter_so_far",
        "driver_slick_to_wet",
        "driver_wet_to_slick",
    ]:
        out[c] = tmp[c].to_numpy()

    out["dry_rule_pressure"] = (
        (1 - out["wet_regime_active"])
        * (1 - out["driver_seen_slick_so_far"])
        * out["RaceProgress"]
    )
    out["wet_rule_relief"] = out["wet_regime_active"] * (
        1 - out["driver_seen_slick_so_far"]
    )
    out["wet_transition_pressure"] = out["field_transition_wave"] * (
        out["TyreLife"]
        + out["Stint"]
        + np.maximum(out["Cumulative_Degradation"], 0.0) / 100.0
    )
    out["wet_transition_x_delta_spike"] = (
        out["field_wet_inter_share"] * out["field_delta_spike"]
    )
    out["wet_transition_x_dispersion"] = (
        out["field_wet_inter_share"] * out["field_laptime_dispersion_spike"]
    )
    out["wet_rule_relief_x_tyrelife"] = out["wet_rule_relief"] * out["TyreLife"]
    out["wet_rule_relief_x_pitstop"] = out["wet_rule_relief"] * out["PitStop"]
    out["driver_switch_aligned_with_wave"] = (
        (out["driver_slick_to_wet"] & out["slick_to_wet_wave"])
        | (out["driver_wet_to_slick"] & out["wet_to_slick_wave"])
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000465"],
        "research_usage_note": "Verified assigned hypothesis 000465.",
    }, sort_keys=True))
