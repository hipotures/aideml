...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ltd = out["LapTime_Delta"].astype("float32")
    lap_grp = out.groupby(lap_keys, sort=False)["LapTime_Delta"]

    lap_mean = lap_grp.transform("mean").astype("float32")
    lap_std = lap_grp.transform("std").fillna(0).astype("float32")
    lap_min = lap_grp.transform("min").astype("float32")
    lap_max = lap_grp.transform("max").astype("float32")
    lap_count = lap_grp.transform("size").astype("float32")

    out["lap_delta_mean"] = lap_mean
    out["lap_delta_std"] = lap_std
    out["lap_delta_spread"] = (lap_max - lap_min).astype("float32")
    out["lap_delta_z"] = (
        ((ltd - lap_mean) / (lap_std + 1e-3)).clip(-10, 10).astype("float32")
    )
    out["lap_delta_pct"] = lap_grp.rank(method="average", pct=True).astype("float32")
    out["lap_delta_abs_z"] = out["lap_delta_z"].abs().astype("float32")

    race_keys = ["Race", "Year"]
    race_lap_mean = (
        out.groupby(race_keys, sort=False)["LapTime_Delta"]
        .transform("mean")
        .astype("float32")
    )
    race_lap_std = (
    ...
    out["field_slowdown"] = (
        ((lap_mean - race_lap_mean) / (race_lap_std + 1e-3))
        .clip(-10, 10)
        .astype("float32")
    )
    out["field_slow_flag"] = (out["field_slowdown"] > 1.0).astype("int8")

    ordered = out[lap_keys + ["Position", "LapTime_Delta", "PitStop"]].copy()
    ordered["_idx"] = np.arange(len(out))
    ordered = ordered.sort_values(lap_keys + ["Position"], kind="mergesort")

    prev_delta = ordered.groupby(lap_keys, sort=False)["LapTime_Delta"].shift(1)
    next_delta = ordered.groupby(lap_keys, sort=False)["LapTime_Delta"].shift(-1)
    prev_pos = ordered.groupby(lap_keys, sort=False)["Position"].shift(1)
    next_pos = ordered.groupby(lap_keys, sort=False)["Position"].shift(-1)

    ordered["gap_ahead_delta"] = (ordered["LapTime_Delta"] - prev_delta).abs()
    ordered["gap_behind_delta"] = (next_delta - ordered["LapTime_Delta"]).abs()
    ordered["pos_gap_ahead"] = (ordered["Position"] - prev_pos).fillna(99)
    ordered["pos_gap_behind"] = (next_pos - ordered["Position"]).fillna(99)
    ordered["nearest_delta_gap"] = (
        ordered[["gap_ahead_delta", "gap_behind_delta"]].min(axis=1).fillna(99)
    )
    ordered["traffic_tight"] = (ordered["nearest_delta_gap"] <= 0.75).astype("int8")

    aligned = ordered.sort_values("_idx", kind="mergesort")
    out["gap_ahead_delta"] = (
        aligned["gap_ahead_delta"].fillna(99).clip(0, 99).to_numpy(dtype="float32")
    )
    out["gap_behind_delta"] = (
        aligned["gap_behind_delta"].fillna(99).clip(0, 99).to_numpy(dtype="float32")
    )
    out["pos_gap_ahead"] = (
        aligned["pos_gap_ahead"].clip(0, 99).to_numpy(dtype="float32")
    )
    out["pos_gap_behind"] = (
        aligned["pos_gap_behind"].clip(0, 99).to_numpy(dtype="float32")
    )
    out["nearest_delta_gap"] = (
        aligned["nearest_delta_gap"].clip(0, 99).to_numpy(dtype="float32")
    )
    out["traffic_tight"] = aligned["traffic_tight"].to_numpy(dtype="int8")

    lap_pit_rate = (
        out.groupby(lap_keys, sort=False)["PitStop"].transform("mean").astype("float32")
    )
    race_pit_rate = (
        out.groupby(race_keys, sort=False)["PitStop"]
        .transform("mean")
        .astype("float32")
    )
    out["lap_pit_rate"] = lap_pit_rate
    out["pit_wave_score"] = (lap_pit_rate - race_pit_rate).clip(-1, 1).astype("float32")
    out["pit_wave_flag"] = ((lap_pit_rate >= 0.12) & (lap_count >= 8)).astype("int8")

    pos = out["Position"].astype("float32")
    tyre = out["TyreLife"].astype("float32")
    out["position_band"] = pd.cut(
        pos, bins=[0, 5, 10, 15, 25], labels=False, include_lowest=True
    ).astype("int8")
    out["tyre_age_x_slowdown"] = (
        (tyre * out["field_slowdown"]).clip(-300, 300).astype("float32")
    )
    out["tyre_age_x_pit_wave"] = (
        (tyre * out["pit_wave_score"]).clip(-100, 100).astype("float32")
    )
    out["traffic_x_slowdown"] = (out["traffic_tight"] * out["field_slowdown"]).astype(
        "float32"
    )
    out["undercut_window_score"] = (
        (tyre * (1.0 / (1.0 + out["nearest_delta_gap"])) * (1.0 + pos / 20.0))
        .clip(0, 200)
        .astype("float32")
    )
    out["cheap_stop_window_score"] = (
        (
            tyre
            * (
                out["field_slowdown"].clip(lower=0)
                + out["pit_wave_score"].clip(lower=0)
            )
        )
        .clip(0, 300)
        .astype("float32")
    )
    out["position_band_x_slowdown"] = (
        out["position_band"].astype("float32") * out["field_slowdown"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000181"],
        "research_usage_note": "Verified assigned hypothesis 000181.",
    }, sort_keys=True))
