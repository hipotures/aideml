...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    band_keys = ["Year", "Race", "LapNumber", "PositionBand"]

    df["PositionBand"] = pd.cut(
        df["Position"],
        bins=[0, 5, 10, 15, 20],
        labels=["P1_5", "P6_10", "P11_15", "P16_20"],
        include_lowest=True,
        right=True,
    ).astype(str)

    pit_col = df["PitStop"].astype(float)

    df["lap_pit_share_now"] = pit_col.groupby([df[c] for c in lap_keys]).transform(
        "mean"
    )
    df["lap_pit_count_now"] = pit_col.groupby([df[c] for c in lap_keys]).transform(
        "sum"
    )
    df["lap_field_size"] = (
        df.groupby(lap_keys)["Driver"].transform("size").astype(float)
    )

    snap = (
        df[lap_keys]
        .assign(lap_pit_share_now=df["lap_pit_share_now"].values)
        .drop_duplicates(lap_keys)
        .sort_values(lap_keys)
    )
    snap["lap_pit_share_prev"] = (
        snap.groupby(["Year", "Race"])["lap_pit_share_now"].shift(1).fillna(0.0)
    )
    df = df.merge(
        snap[lap_keys + ["lap_pit_share_prev"]], on=lap_keys, how="left", sort=False
    )

    df["lap_compound_pit_share_now"] = pit_col.groupby(
        [df["Year"], df["Race"], df["LapNumber"], df["Compound"]]
    ).transform("mean")
    df["lap_band_pit_share_now"] = pit_col.groupby(
        [df[c] for c in band_keys]
    ).transform("mean")

    for col in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta"]:
        name = col.lower().replace(" ", "_").replace("(", "").replace(")", "")
        df[f"{name}_lap_pct"] = df.groupby(lap_keys)[col].rank(
            method="average", pct=True
        )
        df[f"{name}_band_pct"] = df.groupby(band_keys)[col].rank(
            method="average", pct=True
        )

    df["tyre_life_by_lap"] = df["TyreLife"] / df["LapNumber"].clip(lower=1)
    df["tyre_life_by_progress"] = df["TyreLife"] / df["RaceProgress"].clip(lower=1e-3)
    df["deg_per_tyre_lap"] = df["Cumulative_Degradation"] / df["TyreLife"].clip(lower=1)
    df["deg_per_progress"] = df["Cumulative_Degradation"] / df["RaceProgress"].clip(
        lower=1e-3
    )
    df["lap_delta_per_tyre"] = df["LapTime_Delta"] / df["TyreLife"].clip(lower=1)

    df["pressure_x_tyre"] = df["lap_pit_share_now"] * df["tyre_life_by_lap"]
    df["pressure_x_deg"] = df["lap_pit_share_now"] * df["deg_per_tyre_lap"]
    df["prev_pressure_x_tyre"] = df["lap_pit_share_prev"] * df["tyre_life_by_lap"]
    df["compound_pressure_x_tyre"] = (
        df["lap_compound_pit_share_now"] * df["tyre_life_by_lap"]
    )
    df["band_pressure_x_delta"] = df["lap_band_pit_share_now"] * df["LapTime_Delta"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000356.",
    }, sort_keys=True))
