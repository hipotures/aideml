...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_soft = compound.eq("SOFT")
    is_medium = compound.eq("MEDIUM")
    is_hard = compound.eq("HARD")

    sort_cols = ["Driver", "Race"]
    if "Year" in out.columns:
        sort_cols = ["Year"] + sort_cols
    sort_cols = sort_cols + ["LapNumber"]

    ordered = out.assign(
        _orig_order=np.arange(len(out)),
        _is_wet=is_wet.astype("int8"),
        _soft=is_soft.astype("int8"),
        _medium=is_medium.astype("int8"),
        _hard=is_hard.astype("int8"),
    ).sort_values(sort_cols + ["_orig_order"], kind="mergesort")

    group_cols = ["Driver", "Race"]
    if "Year" in out.columns:
        group_cols = ["Year"] + group_cols

    g = ordered.groupby(group_cols, sort=False, observed=True)
    ordered["_wet_used_so_far"] = g["_is_wet"].cummax()
    ordered["_soft_used_so_far"] = g["_soft"].cummax()
    ordered["_medium_used_so_far"] = g["_medium"].cummax()
    ordered["_hard_used_so_far"] = g["_hard"].cummax()
    ordered["_distinct_slicks_used_so_far"] = (
        ordered["_soft_used_so_far"]
        + ordered["_medium_used_so_far"]
        + ordered["_hard_used_so_far"]
    ).astype("int8")

    restored = ordered.sort_values("_orig_order", kind="mergesort")
    out["wet_used_so_far"] = restored["_wet_used_so_far"].to_numpy(dtype="int8")
    out["distinct_slicks_used_so_far"] = restored[
        "_distinct_slicks_used_so_far"
    ].to_numpy(dtype="int8")

    out["dry_rule_active_so_far"] = (1 - out["wet_used_so_far"]).astype("int8")
    out["mandatory_compound_used_so_far"] = (
        out["distinct_slicks_used_so_far"] >= 2
    ).astype("int8")
    out["owes_dry_change"] = (
        (out["dry_rule_active_so_far"] == 1)
        & (is_slick.to_numpy())
        & (out["distinct_slicks_used_so_far"] < 2)
    ).astype("int8")

    out["monaco_2025_special"] = (
        (out["Year"].eq(2025) if "Year" in out.columns else False)
        & race.str.contains("Monaco", case=False, na=False)
    ).astype("int8")
    out["is_testing_session"] = race.str.contains(
        "Pre-Season Testing|Testing", case=False, na=False
    ).astype("int8")

    out["slick_compound_code"] = (
        compound.map({"SOFT": 3, "MEDIUM": 2, "HARD": 1}).fillna(0).astype("int8")
    )
    out["wet_compound_code"] = (
        compound.map({"WET": 2, "INTERMEDIATE": 1}).fillna(0).astype("int8")
    )
    out["rule_exception_regime"] = (
        out["wet_used_so_far"].astype("int8")
        + 2 * out["is_testing_session"].astype("int8")
        + 4 * out["monaco_2025_special"].astype("int8")
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000354"],
        "research_usage_note": "Verified assigned hypothesis 000354.",
    }, sort_keys=True))
