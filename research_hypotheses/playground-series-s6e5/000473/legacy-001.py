...
def map_group_stat(frame, cols, values):
    stat = frame.groupby(cols, sort=False)[values].median()
...
def add_hypothesis_000473_features(frame):
    ...
    frame["LapTime_winsor"] = frame[LAPTIME_COL].clip(lo, hi)
    ...
    y = tmp["pace_resid"].astype(float).where(valid.astype(bool), 0.0)
    ...
    denom = sxx - (sx * sx / n.replace(0, np.nan))
    numer = sxy - (sx * sy / n.replace(0, np.nan))
    slope = numer / denom
    ...
    frame["deg_slope_smooth"] = (
        frame["deg_slope_smooth"].fillna(global_slope).clip(0.005, 2.5)
    )
    ...
    if len(positive_pit):
        p_lo, p_hi = positive_pit.quantile([0.05, 0.95])
        pit_excess = pit_excess.clip(max=max(float(p_hi), 10.0))
        global_pit_loss = float(positive_pit.clip(8, 80).median())
    else:
    ...
    frame["pit_loss_est"] = frame["pit_loss_est"].fillna(global_pit_loss).clip(8, 80)
    ...
    frame["stop_window_pressure"] = 1.0 / (
        1.0 + np.exp(-frame["distance_to_optimal_stint_length"].clip(-30, 30) / 2.5)
    )
