...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap = pd.to_numeric(out["LapTime (s)"], errors="coerce")
    tyre = pd.to_numeric(out["TyreLife"], errors="coerce").astype(float)
    qlo, qhi = lap.quantile([0.01, 0.99])
    if not np.isfinite(qlo) or not np.isfinite(qhi) or qhi <= qlo:
        qlo, qhi = lap.min(), lap.max()
    lap_w = lap.clip(qlo, qhi)

    race_year_med = lap_w.groupby([out["Race"], out["Year"]], sort=False).transform(
        "median"
    )
    residual = lap_w - race_year_med

    def _causal_slope(keys, min_n):
        tmp = pd.DataFrame({"_x": tyre, "_y": residual}, index=out.index)
        for k in keys:
            tmp[k] = out[k].values
        tmp["_lap"] = pd.to_numeric(out["LapNumber"], errors="coerce").values
        tmp = tmp.sort_values(keys + ["_x", "_lap"], kind="mergesort")

        g = tmp.groupby(keys, sort=False, dropna=False)
        n = g.cumcount().astype(float) + 1.0
        sx = g["_x"].cumsum()
        sy = g["_y"].cumsum()
        sx2 = (
            (tmp["_x"] * tmp["_x"])
            .groupby([tmp[k] for k in keys], sort=False, dropna=False)
            .cumsum()
        )
        sxy = (
            (tmp["_x"] * tmp["_y"])
        ...
        slope = slope.where((n >= float(min_n)) & (den > 1e-9) & np.isfinite(slope))
        return slope.reindex(out.index)

    slope_a = _causal_slope(["Race", "Year", "Compound", "Stint"], 8).where(
        lambda s: s > 0
    )
    slope_b = _causal_slope(["Race", "Year", "Compound"], 12).where(lambda s: s > 0)
    slope_c = _causal_slope(["Race", "Year"], 20).where(lambda s: s > 0)

    rc_med = slope_a.groupby([out["Race"], out["Compound"]], sort=False).transform(
        "median"
    )
    c_med = slope_a.groupby(out["Compound"], sort=False).transform("median")
    global_slope = slope_a.median()
    if not np.isfinite(global_slope) or global_slope <= 0:
        global_slope = 0.08

    deg_slope = (
        slope_a.fillna(slope_b)
        .fillna(slope_c)
        .fillna(rc_med)
        .fillna(c_med)
        .fillna(global_slope)
    )
    slope_hi = deg_slope.quantile(0.99)
    if not np.isfinite(slope_hi) or slope_hi < 0.02:
        slope_hi = 1.0
    deg_slope = deg_slope.clip(0.005, slope_hi)

    pit_excess = (
        (lap_w - race_year_med)
        .clip(5.0, 60.0)
        .where(pd.to_numeric(out["PitStop"], errors="coerce") > 0)
    )
    pit_ry = pit_excess.groupby([out["Race"], out["Year"]], sort=False).transform(
        "median"
    )
    pit_r = pit_excess.groupby(out["Race"], sort=False).transform("median")
    pit_c = pit_excess.groupby(out["Compound"], sort=False).transform("median")
    global_pit = pit_excess.median()
    if not np.isfinite(global_pit):
        global_pit = 22.0

    pit_loss = pit_ry.fillna(pit_r).fillna(pit_c).fillna(global_pit).clip(8.0, 55.0)
    opt_len = np.sqrt((2.0 * pit_loss) / deg_slope).clip(3.0, 80.0)
    dist = tyre - opt_len
    width = np.maximum(2.0, 0.12 * opt_len)
    scaled = (dist / width).clip(-10.0, 10.0)

    out["race_stop_deg_slope"] = deg_slope.astype("float32")
    out["race_pit_loss_est"] = pit_loss.astype("float32")
    out["optimal_stint_length"] = opt_len.astype("float32")
    out["distance_to_optimal_stint_length"] = dist.astype("float32")
    out["abs_distance_to_optimal_stint_length"] = np.abs(dist).astype("float32")
    out["in_optimal_stop_window"] = (np.abs(dist) <= width).astype("int8")
    out["past_optimal_stop_window"] = (dist > width).astype("int8")
    out["stop_window_pressure"] = (1.0 / (1.0 + np.exp(-scaled))).astype("float32")
    out["tyrelife_over_optimal_stint"] = (
        (tyre / opt_len.replace(0, np.nan))
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000473.",
    }, sort_keys=True))
