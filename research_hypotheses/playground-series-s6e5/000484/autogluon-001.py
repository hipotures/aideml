...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    est_total = (lap / progress).replace([np.inf, -np.inf], np.nan)
    est_total = est_total.clip(lower=lap, upper=90).fillna(lap)
    remaining = (est_total - lap).clip(lower=0)

    out["EstimatedTotalLaps"] = est_total.astype("float32")
    out["StintStartLap"] = (lap - tyre_life + 1).astype("float32")
    out["StintProgressWithinRace"] = (
        (tyre_life / est_total.clip(lower=1)).clip(0, 1.5).astype("float32")
    )
    out["ProjectedCurrentStintLength_NoStop"] = (tyre_life + remaining).astype(
        "float32"
    )
    out["ProjectedCurrentStintShare_NoStop"] = (
        (tyre_life + remaining) / est_total.clip(lower=1)
    ).astype("float32")

    compound = out["Compound"].astype(str).str.upper()
    race = out["Race"].astype(str).str.upper()
    is_wet = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    is_preseason = race.str.contains("PRE-SEASON|TEST", regex=True).astype("int8")
    dry_rule_gate = ((is_wet == 0) & (is_preseason == 0)).astype("int8")

    out["Is_WetWeather"] = is_wet
    out["Is_PreSeasonTesting"] = is_preseason
    out["MandatoryStopDebt"] = ((stint < 2) & (dry_rule_gate == 1)).astype("int8")
    out["RemainingSpecDebt"] = (
        (stint < 2) & (dry_rule_gate == 1) & (~compound.isin(["INTERMEDIATE", "WET"]))
    ).astype("int8")

    comp_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    out["Compound_StrategyCode"] = compound.map(comp_map).fillna(-1).astype("int8")

    for stops in (1, 2, 3):
        for idx in range(1, stops + 1):
            ideal = est_total * idx / (stops + 1)
            signed = lap - ideal
            absdist = signed.abs()
            prefix = f"Tpl{stops}Stop_Stop{idx}"
            out[f"{prefix}_SignedDist"] = signed.astype("float32")
            out[f"{prefix}_AbsDist"] = absdist.astype("float32")
            out[f"{prefix}_RelAbsDist"] = (absdist / est_total.clip(lower=1)).astype(
                "float32"
            )
            out[f"{prefix}_WindowScore"] = np.exp(-absdist / 5.0).astype("float32")
            out[f"{prefix}_StintInteraction"] = (signed * stint).astype("float32")
            out[f"{prefix}_WetGatedAbsDist"] = (absdist * (1 + is_wet)).astype(
                "float32"
            )
            out[f"{prefix}_DryDebtWindow"] = (
                np.exp(-absdist / 5.0) * out["MandatoryStopDebt"]
            ).astype("float32")
            out[f"{prefix}_SpecDebtSigned"] = (
                signed * out["RemainingSpecDebt"]
            ).astype("float32")
            out[f"{prefix}_CompoundInteraction"] = (
                signed * out["Compound_StrategyCode"]
            ).astype("float32")

    out["TyreLife_vs_OneStopIdeal"] = (tyre_life - est_total / 2).astype("float32")
    out["TyreLife_vs_TwoStopIdeal"] = (tyre_life - est_total / 3).astype("float32")
    out["TyreLife_vs_ThreeStopIdeal"] = (tyre_life - est_total / 4).astype("float32")
    out["CurrentStintFinishMargin"] = (
        est_total - out["StintStartLap"].astype(float) - tyre_life
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000484.",
    }, sort_keys=True))
