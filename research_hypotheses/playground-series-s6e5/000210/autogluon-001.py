...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.copy()
    X["__row_id"] = np.arange(len(X), dtype=np.int64)

    X["RacePhaseBucket"] = np.floor(
        np.clip(X["RaceProgress"].astype(float) * 8, 0, 7)
    ).astype("int8")
    X["TyreLifeClip"] = X["TyreLife"].astype(float).clip(1, 50)
    X["DegPerLap"] = X["Cumulative_Degradation"].astype(float) / X["TyreLifeClip"]
    X["PaceVsLapMedian"] = X["LapTime (s)"].astype(float) - X.groupby("LapNumber")[
        "LapTime (s)"
    ].transform("median")

    X = X.sort_values(
        ["Year", "Race", "LapNumber", "__row_id"], kind="mergesort"
    ).copy()

    def past_mean(values: pd.Series, keys, mask: pd.Series | None = None) -> pd.Series:
        if mask is None:
            masked_values = values.astype(float)
            masked_count = pd.Series(1.0, index=values.index)
        else:
            m = mask.fillna(False)
            masked_values = values.astype(float).where(m, 0.0)
            masked_count = m.astype(float)

        csum = masked_values.groupby(keys).cumsum()
        ccnt = masked_count.groupby(keys).cumsum()
        prev_sum = csum - masked_values
        prev_cnt = ccnt - masked_count
        return prev_sum.div(prev_cnt.replace(0, np.nan))

    phase_keys = [X["Year"], X["Race"], X["RacePhaseBucket"]]
    race_compound_phase_keys = [
        X["Year"],
        X["Race"],
        X["Compound"],
        X["RacePhaseBucket"],
    ...
    prior_phase_pace = past_mean(X["PaceVsLapMedian"], phase_keys)
    global_pace = float(X["PaceVsLapMedian"].median())
    X["PaceResidual"] = X["PaceVsLapMedian"] - prior_phase_pace.fillna(global_pace)

    fresh_mask = X["TyreLifeClip"] <= 2
    outlap_mask = X["PitStop"].astype(int) == 1
    stale_mask = X["TyreLifeClip"] >= 15
    stayout_mask = (X["PitStop"].astype(int) == 0) & (X["TyreLifeClip"] >= 5)

    fresh_mean = past_mean(X["PaceResidual"], race_compound_phase_keys, fresh_mask)
    outlap_mean = past_mean(X["PaceResidual"], race_compound_phase_keys, outlap_mask)
    stale_mean = past_mean(X["PaceResidual"], race_compound_phase_keys, stale_mask)
    stayout_mean = past_mean(X["PaceResidual"], race_compound_phase_keys, stayout_mask)

    fallback_fresh = past_mean(X["PaceResidual"], phase_keys, fresh_mask)
    fallback_outlap = past_mean(X["PaceResidual"], phase_keys, outlap_mask)
    fallback_stale = past_mean(X["PaceResidual"], phase_keys, stale_mask)
    fallback_stayout = past_mean(X["PaceResidual"], phase_keys, stayout_mask)

    fresh_mean = fresh_mean.fillna(fallback_fresh).fillna(0.0)
    outlap_mean = outlap_mean.fillna(fallback_outlap).fillna(0.0)
    stale_mean = stale_mean.fillna(fallback_stale).fillna(0.0)
    stayout_mean = stayout_mean.fillna(fallback_stayout).fillna(0.0)

    X["FreshTyrePremium"] = (stayout_mean - fresh_mean).astype("float32")
    X["OutLapPenalty"] = (outlap_mean - fresh_mean).astype("float32")
    X["StaleTyrePenalty"] = (stale_mean - stayout_mean).astype("float32")
    X["UndercutStrengthIndex"] = (X["FreshTyrePremium"] - X["OutLapPenalty"]).astype(
        "float32"
    )

    X["FreshTyrePremium_x_TyreLife"] = (
        X["FreshTyrePremium"] * X["TyreLifeClip"]
    ).astype("float32")
    X["StaleTyrePenalty_x_Deg"] = (
        X["StaleTyrePenalty"] * X["DegPerLap"].clip(-50, 50)
    ).astype("float32")
    X["UndercutStrength_x_Position"] = (
        X["UndercutStrengthIndex"] / X["Position"].astype(float).clip(lower=1)
    ).astype("float32")
    X["OutLapPenalty_x_Progress"] = (
        X["OutLapPenalty"] * X["RaceProgress"].astype(float)
    ).astype("float32")
    X["FreshVsStalePressure"] = (
        (X["FreshTyrePremium"] + X["StaleTyrePenalty"]) * X["TyreLifeClip"]
    ).astype("float32")
    ...
    X["CompoundPhaseKey"] = (
        ...
        + "_"
        + X["RacePhaseBucket"].astype(str)
    ).astype("category")

    X = X.sort_values("__row_id", kind="mergesort").drop(columns=["__row_id"])
    return X
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000210"],
        "research_usage_note": "Verified assigned hypothesis 000210.",
    }, sort_keys=True))
