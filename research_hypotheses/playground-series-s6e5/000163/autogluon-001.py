...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver_race_year = out["Driver"].astype(str) + "_" + race_year

    out["Race_Year"] = race_year.astype("category")
    out["Driver_Race_Year"] = driver_race_year.astype("category")

    sort_cols = ["Driver", "Race", "Year", "LapNumber"]
    tmp = out[sort_cols].copy()
    tmp["_row"] = np.arange(len(out))
    tmp = tmp.sort_values(sort_cols + ["_row"], kind="mergesort")

    g = tmp.groupby(["Driver", "Race", "Year"], sort=False, observed=True)
    tmp["_prev_lap"] = g["LapNumber"].shift(1)
    tmp["_next_lap"] = g["LapNumber"].shift(-1)
    tmp["_seq_idx"] = g.cumcount() + 1
    tmp["_seq_len"] = g["LapNumber"].transform("size")

    tmp = tmp.sort_values("_row", kind="mergesort")
    out["DriverRace_LapIndex"] = tmp["_seq_idx"].to_numpy(dtype=np.int16)
    out["DriverRace_TotalLapsSeen"] = tmp["_seq_len"].to_numpy(dtype=np.int16)
    out["HasPrevLapSameDriverRace"] = (
        out["LapNumber"].to_numpy() - tmp["_prev_lap"].to_numpy() == 1
    ).astype(np.int8)
    out["HasNextLapSameDriverRace"] = (
        tmp["_next_lap"].to_numpy() - out["LapNumber"].to_numpy() == 1
    ).astype(np.int8)
    out["LapGapFromPrevSameDriverRace"] = (
        (out["LapNumber"] - tmp["_prev_lap"]).fillna(99).clip(-99, 99).astype(np.int16)
    )
    out["LapGapToNextSameDriverRace"] = (
        (tmp["_next_lap"] - out["LapNumber"]).fillna(99).clip(-99, 99).astype(np.int16)
    )

    out["TyreLife_x_Stint"] = out["TyreLife"] * out["Stint"]
    out["TyreLife_RaceProgress"] = out["TyreLife"] * out["RaceProgress"]
    out["LapProgress_Remaining"] = 1.0 - out["RaceProgress"]
    out["Position_x_Change"] = out["Position"] * out["Position_Change"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000163"],
        "research_usage_note": "Verified assigned hypothesis 000163.",
    }, sort_keys=True))
