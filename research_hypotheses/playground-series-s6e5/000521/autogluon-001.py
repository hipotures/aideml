...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    orig_idx = np.arange(len(out))
    out["_orig_order_000521"] = orig_idx

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber"]
    out = out.sort_values(sort_cols, kind="mergesort")

    grp = out.groupby(["Year", "Race", "Driver", "Stint"], sort=False)

    pace = out["LapTime (s)"].astype(float)
    lap_base = out.groupby(["Year", "Race", "LapNumber"], sort=False)[
        "LapTime (s)"
    ].transform("median")
    compound_base = out.groupby(["Year", "Race", "Compound"], sort=False)[
        "LapTime (s)"
    ].transform("median")
    race_base = out.groupby(["Year", "Race"], sort=False)["LapTime (s)"].transform(
        "median"
    )
    expected = (
        lap_base.fillna(race_base)
        + (compound_base.fillna(race_base) - race_base.fillna(pace.median())) * 0.35
    )
    residual = (pace - expected).replace([np.inf, -np.inf], np.nan).fillna(0.0)

    prev_resid = grp[
        residual.name if residual.name in out.columns else "LapTime (s)"
    ].shift(1)
    out["_resid_000521"] = residual
    prev_resid = grp["_resid_000521"].shift(1).fillna(0.0)

    alpha = 0.35
    state = grp["_resid_000521"].transform(
        lambda s: s.shift(1).ewm(alpha=alpha, adjust=False).mean()
    )
    state = state.fillna(prev_resid).fillna(0.0)
    out["latent_deg_level_000521"] = state

    out["latent_deg_slope_000521"] = grp["latent_deg_level_000521"].diff().fillna(0.0)
    out["latent_deg_accel_000521"] = grp["latent_deg_slope_000521"].diff().fillna(0.0)

    lagged_err = (
        (out["_resid_000521"] - out["latent_deg_level_000521"])
        .groupby([out["Year"], out["Race"], out["Driver"], out["Stint"]], sort=False)
        .shift(1)
    )
    out["latent_resid_vol_000521"] = (
        lagged_err.groupby(
            [out["Year"], out["Race"], out["Driver"], out["Stint"]], sort=False
        )
        .transform(lambda s: s.rolling(5, min_periods=2).std())
        .fillna(0.0)
    )

    out["latent_changepoint_000521"] = (
        out["latent_deg_slope_000521"].abs() / (out["latent_resid_vol_000521"] + 1.0)
    ).clip(0, 25)

    prev_pit = grp["PitStop"].shift(1).fillna(0)
    out["laps_since_pit_signal_000521"] = prev_pit.groupby(
        [out["Year"], out["Race"], out["Driver"], out["Stint"]], sort=False
    ).cumsum()
    out["post_pit_recovery_000521"] = (
        out["latent_deg_level_000521"]
        - grp["latent_deg_level_000521"].transform("first")
    ) / out["TyreLife"].clip(lower=1).astype(float)

    out["latent_deg_per_tyre_life_000521"] = out["latent_deg_level_000521"] / out[
        "TyreLife"
    ].clip(lower=1).astype(float)
    out["latent_slope_x_progress_000521"] = out["latent_deg_slope_000521"] * out[
        "RaceProgress"
    ].astype(float)

    out = out.drop(columns=["_resid_000521"])
    out = out.sort_values("_orig_order_000521", kind="mergesort").drop(
        columns=["_orig_order_000521"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000521"],
        "research_usage_note": "Verified assigned hypothesis 000521.",
    }, sort_keys=True))
