...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), 1):
    ...
    tr_order = rank_tr.sort_values("_group", kind="mergesort").index.to_numpy()
    va_order = rank_va.sort_values("_group", kind="mergesort").index.to_numpy()
    ...
    test_rank_df = test[[id_col]].copy()
    test_rank_df["_group"] = test_group.values
    test_order = test_rank_df.sort_values("_group", kind="mergesort").index.to_numpy()
    ...
    fold_test_rank = np.zeros(len(test), dtype=float)
    ...
    print(f"fold {fold} rank-average ROC AUC: {fold_auc:.6f}")
