...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap_keys = ["Race", "Year", "LapNumber"]
    compound_lap_keys = ["Race", "Year", "LapNumber", "Compound"]
    ...
    out["compound_lap_query"] = (
        out["race_lap_query"].astype(str) + "_" + out["Compound"].astype(str)
    ).astype("category")
    out["race_year"] = out["race_year"].astype("category")

    g = out.groupby(race_lap_keys, observed=True, sort=False)
    gc = out.groupby(compound_lap_keys, observed=True, sort=False)

    out["race_lap_field_size"] = g["Position"].transform("count").astype("int16")
    out["compound_lap_field_size"] = gc["Position"].transform("count").astype("int16")
    ...
    rank_cols = [c for c in rank_cols if c in out.columns]

    denom = out["race_lap_field_size"].clip(lower=2) - 1
    cdenom = out["compound_lap_field_size"].clip(lower=2) - 1

    for c in rank_cols:
        r = g[c].rank(method="average", pct=False)
        out[f"{c}_race_lap_rank"] = r
        out[f"{c}_race_lap_pct_rank"] = (r - 1) / denom

        mean = g[c].transform("mean")
        std = g[c].transform("std").fillna(0)
        out[f"{c}_race_lap_centered"] = out[c] - mean
        out[f"{c}_race_lap_z"] = (out[c] - mean) / (std + 1e-6)

        cr = gc[c].rank(method="average", pct=False)
        out[f"{c}_compound_lap_pct_rank"] = (cr - 1) / cdenom

    if "TyreLife" in out.columns and "LapNumber" in out.columns:
        out["tyre_life_lap_ratio"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    if "TyreLife" in out.columns and "RaceProgress" in out.columns:
        out["tyre_life_progress_ratio"] = out["TyreLife"] / out["RaceProgress"].clip(
            lower=0.01
        )
    if "Cumulative_Degradation" in out.columns and "TyreLife" in out.columns:
        out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / out[
            "TyreLife"
        ].clip(lower=1)
    if "Position" in out.columns and "Position_Change" in out.columns:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000286"],
        "research_usage_note": "Verified assigned hypothesis 000286.",
    }, sort_keys=True))
