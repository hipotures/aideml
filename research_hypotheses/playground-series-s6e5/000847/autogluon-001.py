...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_pit_count"] = out["Stint"].astype("float32") - 1.0

    sort_cols = ["Year", "Race", "LapNumber", "Position", "_row_id"]
    out = out.sort_values(sort_cols, kind="mergesort")

    grp = out.groupby(["Year", "Race", "LapNumber"], sort=False, observed=True)

    out["_ahead_pos"] = grp["Position"].shift(1)
    out["_behind_pos"] = grp["Position"].shift(-1)

    for base_col, short_name in [
        ("TyreLife", "tyre"),
        ("LapTime_Delta", "delta"),
        ("Compound", "compound"),
        ("_pit_count", "pit"),
    ]:
        out[f"_ahead_{short_name}"] = grp[base_col].shift(1)
        out[f"_behind_{short_name}"] = grp[base_col].shift(-1)

    has_ahead = out["_ahead_pos"].notna()
    has_behind = out["_behind_pos"].notna()

    out["rival_role_indicator"] = np.select(
        [~has_ahead, ~has_behind],
        [1.0, -1.0],
        default=0.0,
    ).astype("float32")

    out["ahead_tyre_age_gap"] = (
        (out["TyreLife"] - out["_ahead_tyre"]).fillna(0).astype("float32")
    )
    out["behind_tyre_age_gap"] = (
        (out["TyreLife"] - out["_behind_tyre"]).fillna(0).astype("float32")
    )

    out["ahead_lap_delta_gap"] = (
        (out["LapTime_Delta"] - out["_ahead_delta"]).fillna(0).astype("float32")
    )
    out["behind_lap_delta_gap"] = (
        (out["LapTime_Delta"] - out["_behind_delta"]).fillna(0).astype("float32")
    )

    out["ahead_same_compound"] = (
        (out["Compound"] == out["_ahead_compound"]) & has_ahead
    ).astype("int8")
    out["behind_same_compound"] = (
        (out["Compound"] == out["_behind_compound"]) & has_behind
    ).astype("int8")

    out["ahead_pit_count_gap"] = (
        (out["_pit_count"] - out["_ahead_pit"]).fillna(0).astype("float32")
    )
    out["behind_pit_count_gap"] = (
        (out["_pit_count"] - out["_behind_pit"]).fillna(0).astype("float32")
    )

    out["ahead_position_gap"] = (
        (out["Position"] - out["_ahead_pos"]).fillna(0).astype("float32")
    )
    out["behind_position_gap"] = (
        (out["_behind_pos"] - out["Position"]).fillna(0).astype("float32")
    )

    behind_tyre_term = np.clip(out["behind_tyre_age_gap"] / 8.0, -3.0, 3.0)
    behind_pace_term = np.clip(out["behind_lap_delta_gap"] / 2.5, -3.0, 3.0)
    behind_pit_term = np.clip((-out["behind_pit_count_gap"]) / 2.0, -2.0, 2.0)

    ahead_tyre_term = np.clip((-out["ahead_tyre_age_gap"]) / 8.0, -3.0, 3.0)
    ahead_pace_term = np.clip((-out["ahead_lap_delta_gap"]) / 2.5, -3.0, 3.0)
    ahead_pit_term = np.clip(out["ahead_pit_count_gap"] / 2.0, -2.0, 2.0)

    out["undercut_threat_score"] = (
        (
            (0.45 * behind_tyre_term)
            + (0.35 * behind_pace_term)
            + (0.15 * out["behind_same_compound"])
            + (0.05 * behind_pit_term)
        )
        .where(has_behind, 0.0)
        .astype("float32")
    )

    out["overcut_exposure_score"] = (
        (
            (0.45 * ahead_tyre_term)
            + (0.35 * ahead_pace_term)
            + (0.15 * out["ahead_same_compound"])
            + (0.05 * ahead_pit_term)
        )
        .where(has_ahead, 0.0)
        .astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000847"],
        "research_usage_note": "Verified assigned hypothesis 000847.",
    }, sort_keys=True))
