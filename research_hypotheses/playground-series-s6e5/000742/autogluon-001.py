...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Compound", "Driver", "Race"]:
        if c in out.columns:
            out[c] = out[c].astype("string").fillna("missing").str.strip().str.upper()

    eps = 1e-6
    out["lap_progress_x_tyre_life"] = out["RaceProgress"] * out["TyreLife"]
    ...
    out["race_progress_sq"] = out["RaceProgress"] ** 2
    ...
    if "Compound" in out.columns:
        ...
        out["compound_ord"] = (
            out["Compound"].map(compound_map).fillna(0).astype("int16")
        )
    ...
    for c in ["Driver", "Race", "Compound"]:
        if c in out.columns:
            ...
            out[f"{c.lower()}_freq"] = out[c].map(freq).astype("float32")
    ...
    if {"Race", "Year"}.issubset(out.columns):
        ...
        freq = out["race_year"].value_counts(normalize=True, dropna=False)
        out["race_year_freq"] = out["race_year"].map(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000742"],
        "research_usage_note": "Verified assigned hypothesis 000742.",
    }, sort_keys=True))
