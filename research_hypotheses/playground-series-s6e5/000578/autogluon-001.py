...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Basic categorical normalization for coarse weather regime.
    comp = X["Compound"].astype(str).str.upper()
    X["is_wet_compound"] = comp.isin(["WET", "INTERMEDIATE"]).astype("int8")
    X["is_dry_compound"] = comp.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["is_intermediate"] = (comp == "INTERMEDIATE").astype("int8")
    X["is_wet"] = (comp == "WET").astype("int8")

    # Race-level normalization to proxy abnormal strategic regimes.
    for col in [
        "LapTime (s)",
        "LapTime_Delta",
        "Position_Change",
        ...
        grp = X.groupby(key, observed=True)[col]
        mean = grp.transform("mean")
        std = grp.transform("std").replace(0, np.nan)
        X[f"{col}_race_z"] = ((X[col] - mean) / std).fillna(0.0).astype("float32")

    # Driver-race ordering for lap-local sequential proxies.
    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    order = np.lexsort(
        (
            X["LapNumber"].to_numpy(),
            X["Driver"].astype(str).to_numpy(),
            X["Race"].astype(str).to_numpy(),
            X["Year"].to_numpy(),
        )
    )
    rev = np.empty(len(X), dtype=np.int64)
    rev[order] = np.arange(len(X))

    XS = X.iloc[order].copy()
    drv_key = ["Year", "Race", "Driver"]
    g = XS.groupby(drv_key, observed=True, sort=False)
    ...
    XS["stint_start_flag"] = (
        (XS["TyreLife"] <= 2) | (XS["Stint"] != prev_stint)
    ).astype("int8")
    XS["tyre_reset_size"] = (prev_tyre - XS["TyreLife"]).fillna(0.0).astype("float32")
    XS["lap_delta_shock"] = (
        (XS["LapTime_Delta"] - prev_lap_delta).fillna(0.0).astype("float32")
    )
    XS["pos_change_shock"] = (
        (XS["Position_Change"] - prev_pos_change).fillna(0.0).astype("float32")
    )

    # Coarse neutralization proxies: unusually slow laps, sudden field compression/overtake swings, restart-like laps.
    XS["sc_vsc_pace_proxy"] = (
        (XS["LapTime_Delta_race_z"] > 2.0) | (XS["LapTime (s)_race_z"] > 2.0)
    ).astype("int8")
    XS["field_disruption_proxy"] = (
        XS["Position_Change"].abs().ge(3)
        | XS["Position_Change_race_z"].abs().gt(2.0)
        | XS["pos_change_shock"].abs().gt(2.0)
    ).astype("int8")
    XS["restart_lap_proxy"] = (
        (
            (XS["LapNumber"] > 1)
            & (prev_lap_delta > 0)
            & (XS["LapTime_Delta"] < prev_lap_delta)
        )
        .fillna(False)
        .astype("int8")
    )

    # Laps since likely neutralization start/end, computed without future leakage.
    regime_start = (
        (XS["sc_vsc_pace_proxy"] == 1)
        & (g["sc_vsc_pace_proxy"].shift(1).fillna(0) == 0)
    ).astype("int8")
    regime_end = (
        (XS["sc_vsc_pace_proxy"] == 0)
        & (g["sc_vsc_pace_proxy"].shift(1).fillna(0) == 1)
    ).astype("int8")

    lap_in_regime = XS["sc_vsc_pace_proxy"].eq(1)
    regime_block = regime_start.groupby(
        [XS[c] for c in drv_key], observed=True
    ).cumsum()
    since_start = lap_in_regime.groupby(
        [XS[c] for c in drv_key] + [regime_block], observed=True
    ).cumsum()
    since_start = since_start.where(lap_in_regime, 0)

    clear_block = regime_end.groupby([XS[c] for c in drv_key], observed=True).cumsum()
    clear_state = XS["sc_vsc_pace_proxy"].eq(0)
    since_end = clear_state.groupby(
        [XS[c] for c in drv_key] + [clear_block], observed=True
    ).cumsum()
    since_end = since_end.where(clear_state, 0)

    XS["laps_since_sc_start_proxy"] = since_start.astype("float32")
    XS["laps_since_sc_end_proxy"] = since_end.astype("float32")

    # Simple interactions for pit economics under wet/caution regimes.
    XS["wet_tyre_life"] = (XS["is_wet_compound"] * XS["TyreLife"]).astype("float32")
    XS["wet_degradation"] = (
        XS["is_wet_compound"] * XS["Cumulative_Degradation"]
    ).astype("float32")
    XS["caution_tyre_life"] = (XS["sc_vsc_pace_proxy"] * XS["TyreLife"]).astype(
        "float32"
    )
    XS["caution_progress"] = (XS["sc_vsc_pace_proxy"] * XS["RaceProgress"]).astype(
        "float32"
    )
    XS["wet_caution_combo"] = (XS["is_wet_compound"] * XS["sc_vsc_pace_proxy"]).astype(
        "int8"
    )
    XS["late_race_caution"] = (
        (XS["RaceProgress"] >= 0.7) & (XS["sc_vsc_pace_proxy"] == 1)
    ).astype("int8")

    X = XS.iloc[rev].copy()

    # Compact progress bins to let models separate strategy windows.
    X["race_progress_bin"] = pd.cut(
        X["RaceProgress"],
        bins=[-np.inf, 0.2, 0.5, 0.8, np.inf],
        labels=["start", "mid", "late", "final"],
    ).astype(str)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000578"],
        "research_usage_note": "Verified assigned hypothesis 000578.",
    }, sort_keys=True))
