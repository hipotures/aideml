...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = ["Race_Year", "LapNumber"]
    out = out.sort_values(grp + ["Position"], kind="mergesort")

    comp_codes, _ = pd.factorize(out["Compound"].astype(str), sort=True)
    out["_compound_code"] = comp_codes.astype("int16")

    for k in (1, 2, -1, -2):
        suffix = f"pos_{k:+d}".replace("+", "p").replace("-", "m")
        shifted = out.groupby(grp, sort=False).shift(-k)
        out[f"neighbor_{suffix}_pitstop"] = shifted["PitStop"].fillna(0).astype("int8")
        out[f"neighbor_{suffix}_tyrelife"] = (
            shifted["TyreLife"].fillna(-1).astype("float32")
        )
        out[f"neighbor_{suffix}_compound"] = (
            shifted["_compound_code"].fillna(-1).astype("int16")
        )
        out[f"neighbor_{suffix}_lapdelta_diff"] = (
            (out["LapTime_Delta"] - shifted["LapTime_Delta"])
            .fillna(0)
            .astype("float32")
        )
        out[f"neighbor_{suffix}_fresh"] = (
            shifted["TyreLife"].le(3) & shifted["TyreLife"].notna()
        ).astype("int8")

    neigh_pit_cols = [
        c
        for c in out.columns
        if c.startswith("neighbor_pos_") and c.endswith("_pitstop")
    ]
    neigh_fresh_cols = [
        c for c in out.columns if c.startswith("neighbor_pos_") and c.endswith("_fresh")
    ]
    neigh_delta_cols = [
        c
        for c in out.columns
        if c.startswith("neighbor_pos_") and c.endswith("_lapdelta_diff")
    ]

    out["nearby_pitters_count"] = out[neigh_pit_cols].sum(axis=1).astype("int8")
    out["nearby_fresh_tyre_count"] = out[neigh_fresh_cols].sum(axis=1).astype("int8")
    out["same_lap_pitters_count"] = (
        out.groupby(grp, sort=False)["PitStop"].transform("sum").astype("int16")
    )
    out["same_lap_other_pitters_count"] = (
        out["same_lap_pitters_count"] - out["PitStop"]
    ).astype("int16")
    out["lapdelta_rank_same_lap"] = (
        out.groupby(grp, sort=False)["LapTime_Delta"]
        .rank(method="average")
        .astype("float32")
    )
    out["nearby_lapdelta_mean_diff"] = (
        out[neigh_delta_cols].mean(axis=1).astype("float32")
    )
    out["nearby_lapdelta_min_diff"] = (
        out[neigh_delta_cols].min(axis=1).astype("float32")
    )

    out["undercut_pressure"] = (
        out["nearby_fresh_tyre_count"] * (1 + out["same_lap_other_pitters_count"])
    ).astype("float32")
    out["pit_wave_pressure"] = (
        out["nearby_pitters_count"] + out["same_lap_other_pitters_count"]
    ).astype("float32")

    out = out.sort_index()
    out = out.drop(columns=["_compound_code"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000409"],
        "research_usage_note": "Verified assigned hypothesis 000409.",
    }, sort_keys=True))
