...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cat_cols:
        if c in out.columns:
            out[c] = out[c].astype("string").str.strip().fillna("missing")

    lap = pd.to_numeric(out["LapNumber"], errors="coerce").fillna(0)
    tyre = pd.to_numeric(out["TyreLife"], errors="coerce").fillna(0)
    ...
    out["LapBucket"] = pd.cut(
        lap,
        bins=[0, 5, 10, 15, 20, 30, 40, 55, 90],
        labels=False,
        include_lowest=True,
    ).astype("int8")
    out["TyreLifeBucket"] = pd.cut(
        tyre,
        bins=[0, 3, 8, 15, 25, 40, 90],
        labels=False,
        include_lowest=True,
    ).astype("int8")

    out["Race_Year"] = out["Race"].astype("string") + "_" + year.astype("string")
    out["Driver_Compound"] = (
    ...
    out["Compound_Stint"] = (
        out["Compound"].astype("string") + "_" + stint.astype("string")
    )
    out["Race_LapBucket"] = (
        out["Race"].astype("string") + "_" + out["LapBucket"].astype("string")
    )
    ...
    if "PitStop" in out.columns:
        y = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0).clip(0, 1)
        global_rate = float(y.mean())
        for key in keys:
            g = out.groupby(key, observed=True)["PitStop"]
            cnt = g.transform("count").astype("float32")
            sm = 20.0
            rate = (
                (g.transform("sum").astype("float32") + sm * global_rate) / (cnt + sm)
            ).astype("float32")
            out[f"{key}_eb_pitstop_rate"] = rate
            out[f"{key}_log_count"] = np.log1p(cnt).astype("float32")
            out[f"{key}_freq"] = (cnt / max(n, 1)).astype("float32")
    else:
        for key in keys:
            cnt = (
                out.groupby(key, observed=True)[key]
                .transform("count")
                .astype("float32")
            )
            out[f"{key}_log_count"] = np.log1p(cnt).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000224"],
        "research_usage_note": "Verified assigned hypothesis 000224.",
    }, sort_keys=True))
