...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g_lap = [race_col, lap_col]

    lap_median_time = out.groupby(g_lap)[lt_col].transform("median")
    lap_median_delta = out.groupby(g_lap)[delta_col].transform("median")
    lap_median_tyre = out.groupby(g_lap)[tyre_col].transform("median")
    lap_median_degr = out.groupby(g_lap)[degr_col].transform("median")
    lap_field_size = out.groupby(g_lap)[pos_col].transform("count")

    out["pace_vs_field"] = out[lt_col] - lap_median_time
    out["delta_vs_field"] = out[delta_col] - lap_median_delta
    out["tyre_age_vs_field"] = out[tyre_col] - lap_median_tyre
    out["degr_vs_field"] = out[degr_col] - lap_median_degr
    out["front_pack_flag"] = (out[pos_col] <= 10).astype("int8")
    out["race_progress_x_tyre_life"] = out["RaceProgress"] * out[tyre_col]
    out["stint_tyre_ratio"] = out[tyre_col] / (out["Stint"] + eps)

    order = out.sort_values(g_lap + [pos_col]).index
    s = out.loc[order]
    sg = s.groupby(g_lap, sort=False)

    ahead_time = sg[lt_col].shift(1)
    behind_time = sg[lt_col].shift(-1)
    ahead_delta = sg[delta_col].shift(1)
    behind_delta = sg[delta_col].shift(-1)
    ahead_tyre = sg[tyre_col].shift(1)
    behind_tyre = sg[tyre_col].shift(-1)
    ahead_degr = sg[degr_col].shift(1)
    behind_degr = sg[degr_col].shift(-1)
    ahead_pos = sg[pos_col].shift(1)
    behind_pos = sg[pos_col].shift(-1)

    tmp = pd.DataFrame(index=s.index)
    tmp["pace_loss_vs_ahead"] = (s[lt_col] - ahead_time).clip(lower=0).fillna(0)
    tmp["pace_gain_vs_behind"] = (behind_time - s[lt_col]).clip(lower=0).fillna(0)
    tmp["delta_loss_vs_ahead"] = (s[delta_col] - ahead_delta).clip(lower=0).fillna(0)
    tmp["delta_pressure_behind"] = (behind_delta - s[delta_col]).clip(lower=0).fillna(0)
    tmp["tyre_age_advantage_behind"] = (
        (s[tyre_col] - behind_tyre).clip(lower=0).fillna(0)
    )
    tmp["tyre_age_deficit_ahead"] = (s[tyre_col] - ahead_tyre).clip(lower=0).fillna(0)
    tmp["degr_pressure_vs_ahead"] = (s[degr_col] - ahead_degr).clip(lower=0).fillna(0)
    tmp["degr_buffer_vs_behind"] = (s[degr_col] - behind_degr).clip(lower=0).fillna(0)
    tmp["position_gap_ahead"] = (s[pos_col] - ahead_pos).fillna(1).clip(lower=1)
    tmp["position_gap_behind"] = (behind_pos - s[pos_col]).fillna(1).clip(lower=1)

    for c in tmp.columns:
        out.loc[tmp.index, c] = tmp[c].values

    race_time_iqr = out.groupby(race_col)[lt_col].transform(
        lambda x: x.quantile(0.75) - x.quantile(0.25)
    )
    race_delta_scale = out.groupby(race_col)[delta_col].transform(
        lambda x: x.abs().median()
    )
    pit_loss_proxy = (race_time_iqr + race_delta_scale).clip(lower=5, upper=45)
    out["pit_loss_proxy"] = pit_loss_proxy

    out["undercut_pressure"] = (
        out["pace_loss_vs_ahead"]
        + 0.35 * out["delta_loss_vs_ahead"]
        + 0.18 * out["tyre_age_advantage_behind"]
        + 0.006 * out["degr_pressure_vs_ahead"]
    )
    out["pit_loss_norm_undercut"] = out["undercut_pressure"] / (
        out["pit_loss_proxy"] + eps
    )

    rejoin_pos = (out[pos_col] + 6 - 3 * out["RaceProgress"]).clip(lower=1, upper=20)
    out["estimated_rejoin_position"] = rejoin_pos
    out["traffic_risk"] = (
        (20 - rejoin_pos) / 19
        + 0.25 * (lap_field_size - out[pos_col]).clip(lower=0) / (lap_field_size + eps)
        + 0.15 * (out["position_gap_behind"] <= 1).astype("int8")
    )
    out["clean_air_proxy"] = (
        out["position_gap_ahead"]
        + out["position_gap_behind"]
        + (out[pos_col] == 1).astype("int8")
    ) / (lap_field_size + eps)

    out["pit_window_score"] = (
        out["pit_loss_norm_undercut"]
        + 0.45 * out["clean_air_proxy"]
        - 0.35 * out["traffic_risk"]
        + 0.25 * out["front_pack_flag"]
    )
    out["old_tyre_slowdown"] = (
        out["pace_vs_field"].clip(lower=0)
        + 0.25 * out["delta_vs_field"].clip(lower=0)
        + 0.004 * out["degr_vs_field"].clip(lower=0)
    )
    out["old_tyre_slowdown_norm"] = out["old_tyre_slowdown"] / (
        out["pit_loss_proxy"] + eps
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000934"],
        "research_usage_note": "Verified assigned hypothesis 000934.",
    }, sort_keys=True))
