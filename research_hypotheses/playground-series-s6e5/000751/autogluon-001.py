...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound_str = X["Compound"].astype(str)
    wet_mask = compound_str.isin(["INTERMEDIATE", "WET"]).astype("int8")
    dry_mask = (1 - wet_mask).astype("int8")
    slick_soft = (compound_str == "SOFT").astype("int8")
    slick_medium = (compound_str == "MEDIUM").astype("int8")
    slick_hard = (compound_str == "HARD").astype("int8")
    full_wet = (compound_str == "WET").astype("int8")

    eps = 1e-6
    lap = X["LapNumber"].astype("float32")
    tyre = X["TyreLife"].astype("float32")
    prog = X["RaceProgress"].astype("float32")
    ...
    X["IsDry"] = dry_mask
    X["IsSoft"] = slick_soft
    X["IsMedium"] = slick_medium
    X["IsHard"] = slick_hard
    X["IsFullWet"] = full_wet

    X["RemainingRace"] = (1.0 - prog).astype("float32")
    X["TyreLifeFrac"] = (tyre / (lap + eps)).astype("float32")
    X["StintPressure"] = (tyre / (stint + eps)).astype("float32")
    X["DegPerLap"] = (degr / (tyre + 1.0)).astype("float32")
    X["DegPerRaceFrac"] = (degr / (prog + 0.02)).astype("float32")
    X["LapDeltaAbs"] = lap_delta.abs().astype("float32")
    X["PositionPressure"] = (pos * (1.0 - prog)).astype("float32")
    X["TyreEndgame"] = (tyre * (1.0 - prog)).astype("float32")
    X["DegEndgame"] = (degr * (1.0 - prog)).astype("float32")
    X["TrackPosLossRisk"] = ((20.0 - pos) * X["LapDeltaAbs"]).astype("float32")
    X["UndercutRisk"] = ((-pos_chg).clip(lower=0) * X["LapDeltaAbs"]).astype("float32")
    X["OvercutChance"] = (pos_chg.clip(lower=0) * (-lap_delta).clip(lower=0)).astype(
        "float32"
    )
    X["WetTyreAge"] = (wet_mask * tyre).astype("float32")
    X["DryTyreAge"] = (dry_mask * tyre).astype("float32")
    X["WetDeg"] = (wet_mask * degr).astype("float32")
    X["DryDeg"] = (dry_mask * degr).astype("float32")

    for c in ["Driver", "Race", "Compound"]:
        freq = X[c].value_counts(normalize=True)
        X[f"{c}_Freq"] = X[c].map(freq).astype("float32")

    drv_race_grp = ["Year", "Race", "Driver"]
    race_grp = ["Year", "Race"]
    drv_race_stint_grp = ["Year", "Race", "Driver", "Stint"]
    compound_stint_grp = ["Year", "Compound", "Stint"]

    drv_race_lap_med = (
        X.groupby(drv_race_grp, observed=True)["LapTime (s)"]
        .transform("median")
        .astype("float32")
    )
    drv_race_delta_med = (
        X.groupby(drv_race_grp, observed=True)["LapTime_Delta"]
        .transform("median")
        .astype("float32")
    )
    race_lap_med = (
        X.groupby(race_grp, observed=True)["LapTime (s)"]
        .transform("median")
        .astype("float32")
    )
    race_delta_med = (
        X.groupby(race_grp, observed=True)["LapTime_Delta"]
        .transform("median")
        .astype("float32")
    )
    drv_race_stint_tyre_med = (
        X.groupby(drv_race_stint_grp, observed=True)["TyreLife"]
        .transform("median")
        .astype("float32")
    )
    comp_stint_deg_med = (
        X.groupby(compound_stint_grp, observed=True)["Cumulative_Degradation"]
        .transform("median")
        .astype("float32")
    )

    X["LapTimeVsDriverRaceMed"] = (lap_time - drv_race_lap_med).astype("float32")
    X["LapDeltaVsDriverRaceMed"] = (lap_delta - drv_race_delta_med).astype("float32")
    X["LapTimeVsRaceMed"] = (lap_time - race_lap_med).astype("float32")
    X["LapDeltaVsRaceMed"] = (lap_delta - race_delta_med).astype("float32")
    X["TyreLifeVsDriverRaceStintMed"] = (tyre - drv_race_stint_tyre_med).astype(
        "float32"
    )
    X["DegVsCompoundStintMed"] = (degr - comp_stint_deg_med).astype("float32")

    disagreement_base = pd.DataFrame(
        {
            "s1": X["TyreEndgame"].astype("float32"),
            "s2": X["DegPerLap"].astype("float32"),
            "s3": X["LapDeltaAbs"].astype("float32"),
            "s4": X["PositionPressure"].astype("float32"),
        },
        index=X.index,
    )

    disagreement_z = disagreement_base.copy()
    for c in disagreement_z.columns:
        col = disagreement_z[c]
        disagreement_z[c] = ((col - col.mean()) / (col.std() + 1e-6)).astype("float32")

    X["PressureMean"] = disagreement_z.mean(axis=1).astype("float32")
    X["PressureStd"] = disagreement_z.std(axis=1).fillna(0).astype("float32")
    X["PressureRange"] = (
        disagreement_z.max(axis=1) - disagreement_z.min(axis=1)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000751.",
    }, sort_keys=True))
