...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["CompoundGrip"] = df["Compound"].map(compound_map).fillna(0.0)

    rp = df["RaceProgress"].clip(0.01, 1.0)
    total_laps_est = (df["LapNumber"] / rp).replace([np.inf, -np.inf], np.nan)
    total_laps_est = total_laps_est.fillna(df["LapNumber"]).clip(lower=df["LapNumber"])
    laps_remaining_est = (total_laps_est - df["LapNumber"]).clip(lower=0.0)

    tyre_life_norm = df["TyreLife"] / total_laps_est.clip(lower=1.0)
    stint_maturity = df["TyreLife"] / (df["Stint"] + 1.0)
    deg_per_lap = df["Cumulative_Degradation"] / df["TyreLife"].clip(lower=1.0)
    abs_lap_delta = df["LapTime_Delta"].abs()

    broad_candidate = (
        (df["TyreLife"] >= 8)
        & (laps_remaining_est >= 2)
        & (rp >= 0.08)
        & (rp <= 0.98)
        & (
            (df["Stint"] >= 2)
            | (tyre_life_norm >= 0.18)
            | (df["Cumulative_Degradation"] >= 80)
        )
    ).astype(np.int8)

    candidate_score = (
        0.30 * np.clip((df["TyreLife"] - 5.0) / 20.0, 0.0, 1.0)
        + 0.20 * np.clip((df["Stint"] - 1.0) / 4.0, 0.0, 1.0)
        + 0.20 * np.clip(df["RaceProgress"], 0.0, 1.0)
        + 0.20 * np.clip(df["Cumulative_Degradation"] / 250.0, -1.0, 1.5)
        + 0.10 * np.clip((laps_remaining_est - 2.0) / 25.0, 0.0, 1.0)
    )
    candidate_score = np.clip(candidate_score, 0.0, 1.0)

    df["PitWindowCandidate"] = broad_candidate
    df["PitWindowScore"] = candidate_score
    df["LapsRemaining_Est"] = laps_remaining_est
    df["TyreLifeNorm_Total"] = tyre_life_norm
    df["StintMaturity"] = stint_maturity
    df["Degradation_PerLap"] = deg_per_lap
    df["AbsLapTimeDelta"] = abs_lap_delta

    df["Candidate_TyreLife"] = broad_candidate * df["TyreLife"]
    df["Candidate_Degradation"] = broad_candidate * df["Cumulative_Degradation"]
    df["Candidate_LapDelta"] = broad_candidate * abs_lap_delta
    df["Candidate_PositionLoss"] = broad_candidate * (-df["Position_Change"]).clip(
        lower=0
    )
    df["Candidate_GripFade"] = broad_candidate * (df["CompoundGrip"] * df["TyreLife"])
    df["WindowPressure"] = candidate_score * deg_per_lap * (1.0 + tyre_life_norm)
    df["LateStintPressure"] = rp * df["TyreLife"] * (df["Stint"] >= 2).astype(np.int8)
    df["StopUrgency"] = (
        np.clip(df["TyreLife"] / 25.0, 0.0, 3.0)
        * np.clip(1.0 + deg_per_lap / 20.0, 0.0, 3.0)
        * np.clip(laps_remaining_est / 20.0, 0.0, 3.0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000233.",
    }, sort_keys=True))
