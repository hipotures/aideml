...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Compound", "Driver", "Race"]:
        ...
        freq = out[c].map(out[c].value_counts(normalize=True)).astype("float32")
        out[c + "_freq"] = freq
    ...
    out["Compound_ord"] = out["Compound"].map(compound_map).fillna(0).astype("float32")
    out["is_wet_regime"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["is_slick_regime"] = (
        out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    )

    rp = out["RaceProgress"].astype("float32").clip(0, 1)
    ...
    out["early_race"] = (rp <= 0.33).astype("int8")
    out["mid_race"] = ((rp > 0.33) & (rp <= 0.67)).astype("int8")
    out["late_race"] = (rp > 0.67).astype("int8")
    ...
    out["tyre_life_progress_ratio"] = (tyre / (lap + 1.0)).astype("float32")
    out["stint_lap_pressure"] = (tyre * np.log1p(stint)).astype("float32")
    out["late_tyre_pressure"] = (tyre * rp).astype("float32")
    out["position_loss_flag"] = (pos_chg > 0).astype("int8")
    out["position_gain_flag"] = (pos_chg < 0).astype("int8")
    out["front_runner"] = (pos <= 5).astype("int8")
    out["traffic_pack"] = ((pos >= 6) & (pos <= 15)).astype("int8")

    abs_delta = delta.abs()
    out["abs_laptime_delta"] = abs_delta.astype("float32")
    out["laptime_delta_per_tyre"] = (delta / (tyre + 1.0)).astype("float32")
    out["degradation_per_tyre"] = (deg / (tyre + 1.0)).astype("float32")
    out["degradation_per_lap"] = (deg / (lap + 1.0)).astype("float32")
    out["slow_lap_flag"] = (delta > 2.0).astype("int8")
    out["extreme_laptime_flag"] = (laptime > laptime.quantile(0.995)).astype("int8")

    race_deg = (
        out.groupby("Race")["Cumulative_Degradation"]
        .transform("median")
        .astype("float32")
    )
    out["race_median_degradation"] = race_deg
    out["high_deg_track"] = (race_deg > race_deg.median()).astype("int8")

    out["wet_late_regime"] = (out["is_wet_regime"] * out["late_race"]).astype("int8")
    out["slick_late_highdeg"] = (
        out["is_slick_regime"] * out["late_race"] * out["high_deg_track"]
    ).astype("int8")
    out["wet_tyre_pressure"] = (
        out["is_wet_regime"] * out["late_tyre_pressure"]
    ).astype("float32")
    out["highdeg_tyre_pressure"] = (
        out["high_deg_track"] * out["late_tyre_pressure"]
    ).astype("float32")
    out["phase_tyre_life"] = (out["race_phase"].astype("float32") * tyre).astype(
        "float32"
    )
    out["phase_degradation"] = (out["race_phase"].astype("float32") * deg).astype(
        "float32"
    )

    out["driver_race_key_freq"] = (
        (out["Driver"] + "_" + out["Race"]).map(
            (out["Driver"] + "_" + out["Race"]).value_counts(normalize=True)
        )
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000787.",
    }, sort_keys=True))
