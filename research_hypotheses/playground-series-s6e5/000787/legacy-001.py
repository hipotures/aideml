...
def build_features(train, test):
    ...
    return full.iloc[: len(train)].reset_index(drop=True), full.iloc[
        len(train) :
    ].reset_index(drop=True)


def make_regimes(df):
    wet = df["Compound"].astype(str).str.upper().isin(["WET", "INTERMEDIATE"])
    phase = np.select(
        [df["RaceProgress"].to_numpy() < 0.34, df["RaceProgress"].to_numpy() < 0.67],
        ["early", "mid"],
        default="late",
    )
    return np.where(wet.to_numpy(), "wet_", "slick_") + phase
...
stack_test = np.zeros(len(test), dtype=np.float64)
