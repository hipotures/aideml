...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    train_mask = pd.Series(np.arange(n_total) < n_train, index=out.index)

    out["Race_Year"] = out["Race"].astype(str) + "__" + out["Year"].astype(str)
    out["Driver_Year"] = out["Driver"].astype(str) + "__" + out["Year"].astype(str)
    out["Driver_Race"] = out["Driver"].astype(str) + "__" + out["Race"].astype(str)
    out["Compound_Year"] = out["Compound"].astype(str) + "__" + out["Year"].astype(str)
    out["is_preseason"] = (
        out["Race"]
        .astype(str)
        .str.contains("Pre-Season", case=False, regex=False)
        .astype("int8")
    )

    tyre_life = out["TyreLife"].astype(float).clip(lower=1.0)
    stint = out["Stint"].astype(float).clip(lower=1.0)
    lap_number = out["LapNumber"].astype(float).clip(lower=1.0)
    race_progress = out["RaceProgress"].astype(float).clip(0.0, 1.0)

    out["tyre_life_per_lap"] = tyre_life / lap_number
    out["tyre_life_per_stint"] = tyre_life / stint
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"].astype(float) / tyre_life
    )
    out["lap_delta_per_tyre_lap"] = out["LapTime_Delta"].astype(float) / tyre_life
    out["stint_progress_proxy"] = tyre_life / (lap_number + 1.0)
    out["late_race_tyre"] = tyre_life * race_progress
    out["position_pressure"] = out["Position"].astype(float) * race_progress
    out["position_change_pressure"] = (
        out["Position_Change"].astype(float) * race_progress
    )
    out["pitstop_stint_interaction"] = out["PitStop"].astype(float) * stint
    ...
    for col in cat_cols:
        ...
        train_counts = s[train_mask].value_counts(dropna=False)
        test_counts = (
            s[~train_mask].value_counts(dropna=False)
            if n_test > 0
            else pd.Series(dtype=float)
        )
        total_counts = s.value_counts(dropna=False)

        train_freq = s.map(train_counts).fillna(0.0).astype(float) / max(n_train, 1)
        test_freq = (
            s.map(test_counts).fillna(0.0).astype(float) / max(n_test, 1)
            if n_test > 0
            else 0.0
        )

        out[f"{col}_count"] = s.map(total_counts).fillna(0).astype("float32")
        out[f"{col}_train_freq"] = train_freq.astype("float32")
        out[f"{col}_test_freq"] = (
            test_freq.astype("float32") if n_test > 0 else np.float32(0.0)
        )
        if n_test > 0:
            out[f"{col}_shift_score"] = np.log(
                (test_freq + 1e-6) / (train_freq + 1e-6)
            ).astype("float32")
        else:
            out[f"{col}_shift_score"] = np.float32(0.0)
        out[f"{col}_rare_flag"] = (out[f"{col}_count"] <= 3).astype("int8")
        out[f"{col}_unseen_in_train"] = (~s.isin(train_counts.index)).astype("int8")
    ...
    for col in num_cols:
        ...
        if not np.isfinite(std) or std == 0:
            std = 1.0
        out[f"{col}_train_z"] = ((x - mean) / std).astype("float32")

        if n_test > 0:
            try:
                bins = pd.qcut(x, q=12, labels=False, duplicates="drop")
                train_bin_counts = bins[train_mask].value_counts(dropna=False)
                test_bin_counts = bins[~train_mask].value_counts(dropna=False)
                train_bin_freq = bins.map(train_bin_counts).fillna(0.0).astype(
                    float
                ) / max(n_train, 1)
                test_bin_freq = bins.map(test_bin_counts).fillna(0.0).astype(
                    float
                ) / max(n_test, 1)
                out[f"{col}_bin_shift_score"] = np.log(
                    (test_bin_freq + 1e-6) / (train_bin_freq + 1e-6)
                ).astype("float32")
            except Exception:
                out[f"{col}_bin_shift_score"] = np.float32(0.0)
        else:
            out[f"{col}_bin_shift_score"] = np.float32(0.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000443.",
    }, sort_keys=True))
