...
def query_percentile(values, query_ids):
    ...
    return s.groupby(q, sort=False).rank(pct=True).astype("float32").values
...
splits = list(LeaveOneGroupOut().split(train, y, groups))
n_splits = len(splits)
print(
...
for fold, (tr_idx, va_idx) in enumerate(splits, 1):
    ...
    ranker.fit(
        Pool(
            rank_tr[features],
            label=rank_tr["_target"].astype(int),
    ...
    va_pred_sorted = ranker.predict(
        Pool(
            rank_va[features],
            group_id=rank_va["query_id"],
            cat_features=cat_features,
        )
    )
    rank_oof_raw[rank_va.index.values] = va_pred_sorted.astype(np.float32)

    test_pred_sorted = ranker.predict(test_pool_sorted)
    test_pred = pd.Series(test_pred_sorted, index=test_sorted.index).sort_index().values
    test_rank_sum += test_pred / n_splits
...
test_rank_cv = query_percentile(test_rank_sum, test["query_id"].values)
...
full_ranker = CatBoostRanker(**{**rank_params, "iterations": 55})
full_ranker.fit(
    Pool(
...
full_rank_test_sorted = full_ranker.predict(test_pool_sorted)
full_rank_test_raw = (
