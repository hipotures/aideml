...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    query_key = (
        ...
        + X["Race"].astype(str)
        + "_L"
        + X["LapNumber"].astype(str)
    )
    race_key = X["Year"].astype(str) + "_" + X["Race"].astype(str)

    X["Year_Race"] = race_key
    X["Year_Race_LapNumber"] = query_key
    ...
    X["CompoundOrdinal"] = X["Compound"].map(compound_order).astype("float32")

    qg = X.groupby(query_cols, sort=False)
    sg = X.groupby(["Year", "Race", "Driver", "Stint"], sort=False)

    def pct_rank_within(grouped, col, asc=True):
        return (
            grouped[col]
            .rank(method="average", pct=True, ascending=asc)
            .astype("float32")
        )

    def centered_within(grouped, col):
        return (X[col] - grouped[col].transform("mean")).astype("float32")

    def zscore_within(grouped, col):
    ...
    X["query_size"] = qg["Driver"].transform("size").astype("int16")

    X["tyrelife_query_pct"] = pct_rank_within(qg, "TyreLife", asc=True)
    X["degradation_query_pct"] = pct_rank_within(qg, "Cumulative_Degradation", asc=True)
    X["lapdelta_query_pct"] = pct_rank_within(qg, "LapTime_Delta", asc=True)
    X["laptime_query_pct"] = pct_rank_within(qg, "LapTime (s)", asc=True)
    X["position_query_pct"] = pct_rank_within(qg, "Position", asc=False)
    X["position_change_query_pct"] = pct_rank_within(qg, "Position_Change", asc=True)
    X["raceprogress_query_pct"] = pct_rank_within(qg, "RaceProgress", asc=True)
    X["stint_query_pct"] = pct_rank_within(qg, "Stint", asc=True)

    X["tyrelife_query_diff"] = centered_within(qg, "TyreLife")
    X["degradation_query_diff"] = centered_within(qg, "Cumulative_Degradation")
    X["lapdelta_query_diff"] = centered_within(qg, "LapTime_Delta")
    X["laptime_query_diff"] = centered_within(qg, "LapTime (s)")
    X["position_query_diff"] = centered_within(qg, "Position")
    X["position_change_query_diff"] = centered_within(qg, "Position_Change")

    X["tyrelife_query_z"] = zscore_within(qg, "TyreLife")
    X["degradation_query_z"] = zscore_within(qg, "Cumulative_Degradation")
    X["lapdelta_query_z"] = zscore_within(qg, "LapTime_Delta")
    X["laptime_query_z"] = zscore_within(qg, "LapTime (s)")
    X["position_query_z"] = zscore_within(qg, "Position")

    X["front_gap"] = (X["Position"] - 1).astype("float32")
    X["rear_pressure"] = (X["query_size"] - X["Position"]).astype("float32")
    X["position_pressure_pct"] = (
        (X["rear_pressure"] / X["query_size"].replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )

    X["stint_progress"] = (X["TyreLife"] / X["LapNumber"].clip(lower=1)).astype(
        "float32"
    )
    X["degradation_per_lap"] = (
        X["Cumulative_Degradation"] / X["TyreLife"].clip(lower=1)
    ).astype("float32")
    X["degradation_x_tyrelife"] = (X["Cumulative_Degradation"] * X["TyreLife"]).astype(
        "float32"
    )
    X["lapdelta_x_tyrelife"] = (X["LapTime_Delta"] * X["TyreLife"]).astype("float32")
    X["lapdelta_x_degradation"] = (
        X["LapTime_Delta"] * X["Cumulative_Degradation"]
    ).astype("float32")

    X["stint_tyrelife_rank"] = (
        sg["TyreLife"].rank(method="average", pct=True).astype("float32")
    )
    X["stint_degradation_rank"] = (
        sg["Cumulative_Degradation"].rank(method="average", pct=True).astype("float32")
    )

    X["pit_window_score"] = (
        0.35 * X["tyrelife_query_pct"]
        + 0.25 * X["degradation_query_pct"]
        + 0.20 * X["lapdelta_query_pct"]
        + 0.10 * X["position_pressure_pct"]
        + 0.10 * X["stint_progress"]
    ).astype("float32")

    X["pit_window_rank"] = (
        qg["pit_window_score"].rank(method="average", pct=True).astype("float32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000749.",
    }, sort_keys=True))
