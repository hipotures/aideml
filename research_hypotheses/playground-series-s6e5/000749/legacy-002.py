...
for fold, (tr_idx, va_idx) in enumerate(splits, 1):
    ...
    test_point_sum += clf.predict_proba(test_features)[:, 1] / n_splits
    ...
    test_rank_sum += (
        pd.Series(test_pred_sorted, index=test_sorted.index).sort_index().values
        / n_splits
    )
...
full_point_test = full_point.predict_proba(test_features)[:, 1]
