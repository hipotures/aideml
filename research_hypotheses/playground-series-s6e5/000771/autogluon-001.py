...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_str = df["Race"].astype(str)
    compound_str = df["Compound"].astype(str)
    race_lower = race_str.str.lower()

    is_monaco = race_lower.str.contains("monaco", regex=False)
    monaco_2025 = ((df["Year"] == 2025) & is_monaco).astype(np.int8)

    df["Year_Race"] = df["Year"].astype(str) + "_" + race_str
    df["Monaco2025_Specialist"] = monaco_2025
    df["TyreLife_Pressure"] = df["TyreLife"] * (1.0 + df["RaceProgress"])
    df["Stint_TyreLife"] = df["Stint"] * df["TyreLife"]
    df["Position_Pressure"] = (21 - df["Position"]) * df["RaceProgress"]

    df["Monaco2025_TyrePressure"] = monaco_2025 * df["TyreLife_Pressure"]
    df["Monaco2025_StintPressure"] = (
        monaco_2025 * df["Stint"] * (1.0 + df["RaceProgress"])
    )
    df["Monaco2025_PositionPressure"] = monaco_2025 * df["Position_Pressure"]
    df["Monaco2025_LapTyrePressure"] = (
        monaco_2025 * df["TyreLife"] * df["LapNumber"] / (df["Stint"] + 1.0)
    )

    df["Compound_SOFT"] = (compound_str == "SOFT").astype(np.int8)
    df["Compound_MEDIUM"] = (compound_str == "MEDIUM").astype(np.int8)
    df["Compound_HARD"] = (compound_str == "HARD").astype(np.int8)
    df["Compound_INTERMEDIATE"] = (compound_str == "INTERMEDIATE").astype(np.int8)
    df["Compound_WET"] = (compound_str == "WET").astype(np.int8)

    for col in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        df[f"Monaco2025_{col}"] = monaco_2025 * df[f"Compound_{col}"]

    df["Monaco2025_SoftTyrePressure"] = (
        monaco_2025 * df["Compound_SOFT"] * df["TyreLife_Pressure"]
    )
    df["Monaco2025_MidfieldWindow"] = monaco_2025 * (
        (df["Position"] >= 5) & (df["Position"] <= 15)
    ).astype(np.int8)
    df["Monaco2025_LateRace"] = monaco_2025 * (df["RaceProgress"] >= 0.6).astype(
        np.int8
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000771"],
        "research_usage_note": "Verified assigned hypothesis 000771.",
    }, sort_keys=True))
