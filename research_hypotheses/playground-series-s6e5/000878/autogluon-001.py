...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_inter = (comp == "INTERMEDIATE").astype("int8")
    is_wet = (comp == "WET").astype("int8")

    race_lap_keys = ["Year", "Race", "LapNumber"]
    race_comp_keys = ["Year", "Race", "Compound"]

    lap_med = lap_time.groupby([out[k] for k in race_lap_keys]).transform("median")
    lap_q25 = lap_time.groupby([out[k] for k in race_lap_keys]).transform(
        lambda s: s.quantile(0.25)
    )
    comp_med = lap_time.groupby([out[k] for k in race_comp_keys]).transform("median")

    out["race_lap_pace_loss"] = (lap_time / lap_med.replace(0, np.nan) - 1.0).fillna(
        0.0
    )
    out["race_lap_fast_loss"] = (lap_time / lap_q25.replace(0, np.nan) - 1.0).fillna(
        0.0
    )
    out["compound_pace_loss"] = (lap_time / comp_med.replace(0, np.nan) - 1.0).fillna(
        0.0
    )

    wet_share = is_wet.groupby([out[k] for k in race_lap_keys]).transform("mean")
    inter_share = is_inter.groupby([out[k] for k in race_lap_keys]).transform("mean")
    slick_share = is_slick.groupby([out[k] for k in race_lap_keys]).transform("mean")

    out["wet_compound_share_lap"] = wet_share.astype(float)
    out["inter_compound_share_lap"] = inter_share.astype(float)
    out["slick_compound_share_lap"] = slick_share.astype(float)

    inflation = out["race_lap_fast_loss"].clip(-0.10, 1.50)
    tyre_life = out["TyreLife"].astype(float)
    race_progress = out["RaceProgress"].astype(float)
    degradation = out["Cumulative_Degradation"].astype(float)

    out["slick_to_inter_pressure"] = (
        is_slick
        * ((inflation > 0.08).astype("int8") + (inter_share > 0.10).astype("int8"))
    ).astype("int8")
    out["inter_to_slick_drying_pressure"] = (
        is_inter
        * ((inflation < 0.035).astype("int8") + (slick_share > 0.35).astype("int8"))
    ).astype("int8")
    out["wet_overstay_risk"] = (
        is_wet
        * (
            (inflation > 0.06).astype("int8")
            + (inter_share + slick_share > 0.25).astype("int8")
            + (tyre_life >= 8).astype("int8")
        )
    ).astype("int8")

    out["wet_crossover_pressure"] = (
        out["slick_to_inter_pressure"]
        + out["inter_to_slick_drying_pressure"]
        + out["wet_overstay_risk"]
    ).astype("int8")
    out["wet_regime_pace_x_tyre"] = (
        out["wet_crossover_pressure"].astype(float) * tyre_life
    )
    out["wet_regime_deg_x_progress"] = (
        out["wet_crossover_pressure"].astype(float) * degradation * race_progress
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000878"],
        "research_usage_note": "Verified assigned hypothesis 000878.",
    }, sort_keys=True))
