...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Basic tyre-state helpers to let the model modulate undercut value by compound family.
    compound_map = {"SOFT": 3, "MEDIUM": 2, "HARD": 1, "INTERMEDIATE": 0, "WET": -1}
    out["Compound_Code"] = out["Compound"].map(compound_map).fillna(0).astype("int8")
    out["Is_Wet_Compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["TyreLife_Ratio"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["Degradation_Per_Lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )

    # Same-lap rival pressure / clean-air proxies from immediate neighbors in running order.
    lap_sort = out.sort_values(
        ["Year", "Race", "LapNumber", "Position", "Driver", "_row_id"],
        kind="mergesort",
    ).copy()
    lap_keys = ["Year", "Race", "LapNumber"]

    ahead_lap = lap_sort.groupby(lap_keys, sort=False)["LapTime (s)"].shift(1)
    behind_lap = lap_sort.groupby(lap_keys, sort=False)["LapTime (s)"].shift(-1)

    lap_sort["Front_Pace_Adv"] = (
        (ahead_lap - lap_sort["LapTime (s)"]).clip(-30, 30).fillna(0.0)
    )
    lap_sort["Rear_Pace_Threat"] = (
        (lap_sort["LapTime (s)"] - behind_lap).clip(-30, 30).fillna(0.0)
    )
    lap_sort["Attacking_Rival"] = (
        (lap_sort["Position"] > 1) & (lap_sort["Front_Pace_Adv"] > 0.3)
    ).astype("int8")
    lap_sort["Covering_Rival"] = (
        (lap_sort["Position"] < 20) & (lap_sort["Rear_Pace_Threat"] > 0.3)
    ).astype("int8")
    lap_sort["Clean_Air_Proxy"] = (
        lap_sort["Front_Pace_Adv"] - lap_sort["Rear_Pace_Threat"]
    ).clip(-30, 30)
    ...
    # Build prior pit-stop summaries from observed earlier laps only.
    history = out.loc[
        :,
        ["Year", "Race", "LapNumber", "PitStop", "Position_Change", "LapTime_Delta"],
    ].copy()
    history["pit_cnt"] = (history["PitStop"] == 1).astype(np.int32)
    history["pit_gain_proxy"] = np.where(
        history["PitStop"] == 1, history["Position_Change"], 0.0
    )
    history["pit_loss_proxy"] = np.where(
        history["PitStop"] == 1,
        history["LapTime_Delta"].clip(-60, 300),
        0.0,
    )

    # Earlier-lap within-race priors.
    lap_agg = (
        history.groupby(["Year", "Race", "LapNumber"], as_index=False, sort=True)[
            ["pit_cnt", "pit_gain_proxy", "pit_loss_proxy"]
        ]
        .sum()
        .sort_values(["Year", "Race", "LapNumber"], kind="mergesort")
    )

    race_groups = lap_agg.groupby(["Year", "Race"], sort=False)
    race_key_series = [lap_agg["Year"], lap_agg["Race"]]
    race_pit_csum = race_groups["pit_cnt"].cumsum()
    race_gain_csum = race_groups["pit_gain_proxy"].cumsum()
    race_loss_csum = race_groups["pit_loss_proxy"].cumsum()
    lap_agg["prior_race_pit_cnt"] = race_pit_csum.groupby(
        race_key_series, sort=False
    ).shift(fill_value=0)
    lap_agg["prior_race_pit_gain_sum"] = race_gain_csum.groupby(
        race_key_series, sort=False
    ).shift(fill_value=0.0)
    lap_agg["prior_race_pit_loss_sum"] = race_loss_csum.groupby(
        race_key_series, sort=False
    ).shift(fill_value=0.0)

    lap_agg["Race_Prior_Pit_Gain"] = lap_agg["prior_race_pit_gain_sum"] / lap_agg[
        "prior_race_pit_cnt"
    ].replace(0, np.nan)
    lap_agg["Race_Prior_Pit_Loss"] = lap_agg["prior_race_pit_loss_sum"] / lap_agg[
        "prior_race_pit_cnt"
    ].replace(0, np.nan)

    out = out.merge(
        lap_agg[
            ["Year", "Race", "LapNumber", "Race_Prior_Pit_Gain", "Race_Prior_Pit_Loss"]
        ],
    ...
    # Circuit-level historical undercut prior from earlier years only.
    race_year_agg = (
        history.groupby(["Race", "Year"], as_index=False, sort=True)[
            ["pit_cnt", "pit_gain_proxy", "pit_loss_proxy"]
        ]
        .sum()
        .sort_values(["Race", "Year"], kind="mergesort")
    )

    circuit_groups = race_year_agg.groupby("Race", sort=False)
    circuit_pit_csum = circuit_groups["pit_cnt"].cumsum()
    circuit_gain_csum = circuit_groups["pit_gain_proxy"].cumsum()
    circuit_loss_csum = circuit_groups["pit_loss_proxy"].cumsum()
    race_year_agg["prior_circuit_pit_cnt"] = circuit_pit_csum.groupby(
        race_year_agg["Race"], sort=False
    ).shift(fill_value=0)
    race_year_agg["prior_circuit_pit_gain_sum"] = circuit_gain_csum.groupby(
        race_year_agg["Race"], sort=False
    ).shift(fill_value=0.0)
    race_year_agg["prior_circuit_pit_loss_sum"] = circuit_loss_csum.groupby(
        race_year_agg["Race"], sort=False
    ).shift(fill_value=0.0)

    race_year_agg["Circuit_Prior_Pit_Gain"] = race_year_agg[
        "prior_circuit_pit_gain_sum"
    ] / race_year_agg["prior_circuit_pit_cnt"].replace(0, np.nan)
    race_year_agg["Circuit_Prior_Pit_Loss"] = race_year_agg[
        "prior_circuit_pit_loss_sum"
    ] / race_year_agg["prior_circuit_pit_cnt"].replace(0, np.nan)
    race_year_agg["Circuit_Undercut_Score"] = (
        race_year_agg["Circuit_Prior_Pit_Gain"]
        - 0.05 * race_year_agg["Circuit_Prior_Pit_Loss"]
    )
    ...
    out = out.merge(
        race_year_agg[
            [
                "Race",
                "Year",
                "Circuit_Prior_Pit_Gain",
                "Circuit_Prior_Pit_Loss",
                "Circuit_Undercut_Score",
            ]
        ],
        on=["Race", "Year"],
        how="left",
    ...
    # Global fallback from observed pitted laps only.
    pit_mask = history["pit_cnt"] == 1
    global_gain = (
        float(history.loc[pit_mask, "Position_Change"].mean()) if pit_mask.any() else 0.0
    )
    global_loss = (
        float(history.loc[pit_mask, "LapTime_Delta"].clip(-60, 300).mean())
        if pit_mask.any()
        else 0.0
    )
    global_score = global_gain - 0.05 * global_loss

    out["Race_Prior_Pit_Gain"] = (
        out["Race_Prior_Pit_Gain"]
        .fillna(out["Circuit_Prior_Pit_Gain"])
        .fillna(global_gain)
    )
    out["Race_Prior_Pit_Loss"] = (
        out["Race_Prior_Pit_Loss"]
        .fillna(out["Circuit_Prior_Pit_Loss"])
        .fillna(global_loss)
    )
    out["Circuit_Prior_Pit_Gain"] = out["Circuit_Prior_Pit_Gain"].fillna(global_gain)
    out["Circuit_Prior_Pit_Loss"] = out["Circuit_Prior_Pit_Loss"].fillna(global_loss)
    out["Circuit_Undercut_Score"] = out["Circuit_Undercut_Score"].fillna(global_score)

    # Interactions required by the hypothesis: position, clean air, tyre age, and cover/attack context.
    out["Undercut_x_Position"] = out["Circuit_Undercut_Score"] * out["Position"]
    out["Undercut_x_TyreLife"] = out["Circuit_Undercut_Score"] * out["TyreLife"]
    out["Undercut_x_Progress"] = out["Circuit_Undercut_Score"] * out["RaceProgress"]
    out["RacePriorGain_x_TyreLife"] = out["Race_Prior_Pit_Gain"] * out["TyreLife"]
    out["RacePriorLoss_x_TyreLife"] = out["Race_Prior_Pit_Loss"] * out["TyreLife"]
    out["Undercut_x_Attack"] = out["Circuit_Undercut_Score"] * out["Attacking_Rival"]
    out["Undercut_x_Cover"] = out["Circuit_Undercut_Score"] * out["Covering_Rival"]
    out["Undercut_x_CleanAir"] = out["Circuit_Undercut_Score"] * out["Clean_Air_Proxy"]
    out["RacePriorGain_x_Attack"] = out["Race_Prior_Pit_Gain"] * out["Attacking_Rival"]
    out["RacePriorGain_x_Cover"] = out["Race_Prior_Pit_Gain"] * out["Covering_Rival"]
    out["PitLoss_x_RearThreat"] = out["Race_Prior_Pit_Loss"] * out["Rear_Pace_Threat"]
    out["PitGain_x_FrontAdv"] = out["Race_Prior_Pit_Gain"] * out["Front_Pace_Adv"]

    out["Potential_Stop_Window"] = (
        out["TyreLife_Ratio"]
        * (1.0 + 0.1 * out["Stint"])
        * (1.0 + 0.05 * out["Position"])
    )
    out["TrackWindow_x_Attack"] = (
        out["Potential_Stop_Window"] * out["Undercut_x_Attack"]
    )
    out["TrackWindow_x_Cover"] = out["Potential_Stop_Window"] * out["Undercut_x_Cover"]

    out = out.sort_values("_row_id", kind="mergesort").drop(
        columns=["_row_id"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000585.",
    }, sort_keys=True))
