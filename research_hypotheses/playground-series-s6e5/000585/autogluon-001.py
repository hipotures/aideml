...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Fixed wrapper contract: train rows come first, then Kaggle test rows.
    test_size = 188165
    train_size = max(len(out) - test_size, 0)
    out["_is_train_src"] = False
    if train_size > 0:
        out.loc[: train_size - 1, "_is_train_src"] = True
    ...
    # Build training-only pit-stop priors.
    train = out.loc[
        out["_is_train_src"],
        ["Year", "Race", "LapNumber", "PitStop", "Position_Change", "LapTime_Delta"],
    ].copy()
    train["pit_cnt"] = (train["PitStop"] == 1).astype(np.int32)
    train["pit_gain_proxy"] = np.where(
        train["PitStop"] == 1, train["Position_Change"], 0.0
    )
    train["pit_loss_proxy"] = np.where(
        train["PitStop"] == 1,
        train["LapTime_Delta"].clip(-60, 300),
        0.0,
    )

    # Earlier-lap within-race priors: only training rows contribute.
    lap_agg = (
        train.groupby(["Year", "Race", "LapNumber"], as_index=False, sort=True)[
    ...
    lap_agg["prior_race_pit_cnt"] = race_groups["pit_cnt"].cumsum().shift(fill_value=0)
    lap_agg["prior_race_pit_gain_sum"] = (
        race_groups["pit_gain_proxy"].cumsum().shift(fill_value=0.0)
    )
    lap_agg["prior_race_pit_loss_sum"] = (
        race_groups["pit_loss_proxy"].cumsum().shift(fill_value=0.0)
    )
    ...
    race_year_agg = (
        train.groupby(["Race", "Year"], as_index=False, sort=True)[
            ["pit_cnt", "pit_gain_proxy", "pit_loss_proxy"]
        ]
        .sum()
    ...
    race_year_agg["prior_circuit_pit_cnt"] = (
        circuit_groups["pit_cnt"].cumsum().shift(fill_value=0)
    )
    race_year_agg["prior_circuit_pit_gain_sum"] = (
        circuit_groups["pit_gain_proxy"].cumsum().shift(fill_value=0.0)
    )
    race_year_agg["prior_circuit_pit_loss_sum"] = (
        circuit_groups["pit_loss_proxy"].cumsum().shift(fill_value=0.0)
    )
    ...
    # Global fallback from training pitted laps only.
    pit_mask = train["pit_cnt"] == 1
    global_gain = (
        float(train.loc[pit_mask, "Position_Change"].mean()) if pit_mask.any() else 0.0
    )
    global_loss = (
        float(train.loc[pit_mask, "LapTime_Delta"].clip(-60, 300).mean())
        if pit_mask.any()
        else 0.0
    )
