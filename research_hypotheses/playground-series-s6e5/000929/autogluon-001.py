...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp_lap_keys = lap_keys + ["Compound"]
    race_comp_keys = race_keys + ["Compound"]

    X["PitStop"] = X["PitStop"].fillna(0).astype(float)
    X["TyreLife"] = X["TyreLife"].astype(float)

    lap_group = X.groupby(lap_keys, sort=False)
    lap_size = lap_group["PitStop"].transform("size").astype(float)
    lap_pit_count = lap_group["PitStop"].transform("sum").astype(float)

    other_cars = (lap_size - 1.0).clip(lower=0.0)
    X["field_pit_count_exself"] = lap_pit_count - X["PitStop"]
    X["field_pit_rate_exself"] = np.where(
        other_cars > 0,
        X["field_pit_count_exself"] / other_cars,
        0.0,
    )

    pit_tyre = X["TyreLife"].where(X["PitStop"] == 1)
    nonpit_tyre = X["TyreLife"].where(X["PitStop"] == 0)

    X["lap_pitters_tyre_median"] = pit_tyre.groupby(
        [X[k] for k in lap_keys], sort=False
    ).transform("median")
    X["lap_nonpit_tyre_min"] = nonpit_tyre.groupby(
        [X[k] for k in lap_keys], sort=False
    ).transform("min")
    X["lap_nonpit_tyre_mean"] = nonpit_tyre.groupby(
        [X[k] for k in lap_keys], sort=False
    ).transform("mean")

    comp_group = X.groupby(comp_lap_keys, sort=False)
    comp_size = comp_group["PitStop"].transform("size").astype(float)
    comp_pit_count = comp_group["PitStop"].transform("sum").astype(float)
    comp_other = (comp_size - 1.0).clip(lower=0.0)
    X["compound_pit_count_exself"] = comp_pit_count - X["PitStop"]
    X["compound_pit_rate_exself"] = np.where(
        comp_other > 0,
        X["compound_pit_count_exself"] / comp_other,
        0.0,
    )
    X["compound_share_of_field_pits"] = np.where(
        lap_pit_count > 0,
        comp_pit_count / lap_pit_count,
        0.0,
    )

    lap_level = (
        X[lap_keys + ["PitStop"]]
        .groupby(lap_keys, as_index=False, sort=False)["PitStop"]
        .sum()
        .rename(columns={"PitStop": "lap_pit_count"})
        .sort_values(race_keys + ["LapNumber"])
    )

    race_lag = lap_level.groupby(race_keys, sort=False)["lap_pit_count"]
    for lag in (1, 2, 3):
        lap_level[f"field_pit_count_lag_{lag}"] = race_lag.shift(lag)

    lap_level["race_cum_stops_before_lap"] = (
        race_lag.cumsum() - lap_level["lap_pit_count"]
    )

    X = X.merge(
        lap_level[
            lap_keys
            + [f"field_pit_count_lag_{i}" for i in (1, 2, 3)]
            + ["race_cum_stops_before_lap"]
        ],
        on=lap_keys,
        how="left",
        sort=False,
    ...
    comp_lap_level = (
        X[race_comp_keys + ["LapNumber", "PitStop"]]
        .groupby(race_comp_keys + ["LapNumber"], as_index=False, sort=False)["PitStop"]
        .sum()
        .rename(columns={"PitStop": "compound_lap_pit_count"})
        .sort_values(race_comp_keys + ["LapNumber"])
    )
    comp_lap_level["compound_cum_stops_before_lap"] = (
        comp_lap_level.groupby(race_comp_keys, sort=False)[
            "compound_lap_pit_count"
        ].cumsum()
        - comp_lap_level["compound_lap_pit_count"]
    )

    X = X.merge(
        comp_lap_level[race_comp_keys + ["LapNumber", "compound_cum_stops_before_lap"]],
        on=race_comp_keys + ["LapNumber"],
        how="left",
        sort=False,
    )

    X["field_pit_count_exself_x_tyre_life"] = (
        X["field_pit_count_exself"] * X["TyreLife"]
    )
    X["field_pit_rate_exself_x_race_progress"] = (
        X["field_pit_rate_exself"] * X["RaceProgress"]
    )
    X["compound_pit_rate_exself_x_tyre_life"] = (
        X["compound_pit_rate_exself"] * X["TyreLife"]
    )

    new_num_cols = [
        "field_pit_count_exself",
        "field_pit_rate_exself",
        "lap_pitters_tyre_median",
        "lap_nonpit_tyre_min",
        "lap_nonpit_tyre_mean",
        "compound_pit_count_exself",
        "compound_pit_rate_exself",
        "compound_share_of_field_pits",
        "field_pit_count_lag_1",
        "field_pit_count_lag_2",
        "field_pit_count_lag_3",
        "race_cum_stops_before_lap",
        "compound_cum_stops_before_lap",
        "field_pit_count_exself_x_tyre_life",
        "field_pit_rate_exself_x_race_progress",
        "compound_pit_rate_exself_x_tyre_life",
    ]
    X[new_num_cols] = X[new_num_cols].fillna(0.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000929.",
    }, sort_keys=True))
