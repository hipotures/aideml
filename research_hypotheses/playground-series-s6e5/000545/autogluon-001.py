...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_progress = out["RaceProgress"].astype("float32").clip(0.001, 1.0)
    stint = out["Stint"].astype("float32")
    position = out["Position"].astype("float32")
    compound = out["Compound"].astype("category")

    total_laps_est = (lap_number / race_progress).clip(1, 100)
    laps_remaining = (total_laps_est - lap_number).clip(0, 100)
    out["strategy_laps_remaining"] = laps_remaining.astype("float32")

    race_keys = ["Year", "Race"]
    race_median_pace = (
        out.groupby(race_keys, observed=True)["LapTime (s)"]
        .transform("median")
        .astype("float32")
    )
    rel_pace = (lap_time - race_median_pace).astype("float32")
    ...
    means = work.groupby("Compound", observed=True)[["TyreLife", "rel_pace"]].transform(
        "mean"
    )
    x = work["TyreLife"] - means["TyreLife"]
    y = work["rel_pace"] - means["rel_pace"]
    cov = (x * y).groupby(compound, observed=True).transform("mean")
    var = (x * x).groupby(compound, observed=True).transform("mean")
    slope = (
        (cov / var.replace(0, np.nan)).replace([np.inf, -np.inf], np.nan).fillna(0.08)
    )
    slope = slope.clip(0.005, 0.35).astype("float32")
    out["strategy_deg_slope"] = slope

    pit_loss = np.float32(22.0)
    opt_stint = np.sqrt((2.0 * pit_loss) / slope).clip(8, 80).astype("float32")
    out["strategy_opt_stint_len"] = opt_stint
    out["strategy_stint_over_opt"] = (tyre_life - opt_stint).astype("float32")
    out["strategy_stint_over_opt_ratio"] = (tyre_life / (opt_stint + 1.0)).astype(
        "float32"
    )

    one_more_loss = (slope * (tyre_life + 0.5)).astype("float32")
    out["strategy_wait_one_lap_loss"] = one_more_loss
    out["strategy_future_wear_cost"] = (
        (slope * laps_remaining * (tyre_life + 0.5 * laps_remaining))
        .clip(0, 500)
        .astype("float32")
    )

    fresh_life_after_stop = laps_remaining.clip(0, 80).astype("float32")
    fresh_finish_cost = (
        slope * fresh_life_after_stop * (fresh_life_after_stop + 1.0) / 2.0
    ).astype("float32")
    current_finish_cost = (
        slope * laps_remaining * (tyre_life + laps_remaining / 2.0)
    ).astype("float32")
    out["strategy_stop_now_value"] = (
        (current_finish_cost - fresh_finish_cost - pit_loss)
        .clip(-100, 300)
        .astype("float32")
    )
    out["strategy_can_finish_fresh"] = (
        fresh_life_after_stop <= opt_stint * 1.35
    ).astype("int8")
    out["strategy_current_can_finish"] = (
        (tyre_life + laps_remaining) <= opt_stint * 1.35
    ).astype("int8")

    fresh_advantage = (slope * tyre_life).clip(0, 20).astype("float32")
    traffic_penalty = ((position - 1.0) / 19.0 * 2.5).clip(0, 2.5).astype("float32")
    out["strategy_undercut_margin"] = (
        fresh_advantage - traffic_penalty - pit_loss / (laps_remaining + 1.0)
    ).astype("float32")

    out["strategy_late_race_stop_pressure"] = (
        out["strategy_stop_now_value"]
        * np.sqrt((laps_remaining + 1.0) / total_laps_est)
    ).astype("float32")
    out["strategy_stint_stop_interaction"] = (
        stint * out["strategy_stint_over_opt_ratio"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000545.",
    }, sort_keys=True))
