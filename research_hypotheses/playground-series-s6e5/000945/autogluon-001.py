...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Driver"] = (
        out["Driver"].astype("string").fillna("missing").str.upper().str.strip()
    )
    out["Race"] = out["Race"].astype("string").fillna("missing").str.strip()
    out["Compound"] = (
        out["Compound"].astype("string").fillna("missing").str.upper().str.strip()
    )
    ...
    tyre = out["TyreLife"].clip(lower=0).astype("float32")
    stint = out["Stint"].astype("float32")
    pos = out["Position"].clip(lower=1).astype("float32")
    ...
    out["estimated_laps_remaining"] = (total_laps_est - lap).astype("float32")
    out["next_lap_progress_est"] = (
        ((lap + 1.0) / total_laps_est).clip(0, 1.25).astype("float32")
    )
    out["stint_start_lap_est"] = (lap - tyre + 1.0).astype("float32")
    out["tyre_life_frac_race"] = (
        (tyre / total_laps_est).replace([np.inf, -np.inf], np.nan).astype("float32")
    )
    out["tyre_life_x_progress"] = (tyre * progress).astype("float32")
    out["stint_x_progress"] = (stint * progress).astype("float32")
    out["degradation_per_tyre_lap"] = (deg / tyre.clip(lower=1.0)).astype("float32")
    out["abs_degradation"] = deg.abs().astype("float32")
    out["lap_delta_abs"] = ltd.abs().astype("float32")
    out["lap_delta_per_tyre_lap"] = (ltd / tyre.clip(lower=1.0)).astype("float32")
    out["lap_delta_sign"] = np.sign(ltd).astype("int8")
    out["position_pressure"] = (1.0 / pos).astype("float32")
    out["front_runner"] = (out["Position"] <= 3).astype("int8")
    out["points_position"] = (out["Position"] <= 10).astype("int8")
    out["recent_position_gain"] = (out["Position_Change"] < 0).astype("int8")
    out["recent_position_loss"] = (out["Position_Change"] > 0).astype("int8")
    out["current_lap_pit"] = out["PitStop"].astype("int8")

    race_group = out.groupby(["Year", "Race"], sort=False, observed=False)
    race_laptime_med = race_group["LapTime (s)"].transform("median")
    race_delta_med = race_group["LapTime_Delta"].transform("median")
    race_deg_med = race_group["Cumulative_Degradation"].transform("median")
    out["race_laptime_centered"] = (laptime - race_laptime_med).astype("float32")
    out["race_delta_centered"] = (ltd - race_delta_med).astype("float32")
    out["race_degradation_centered"] = (deg - race_deg_med).astype("float32")

    out["compound_family"] = np.where(
        out["Compound"].isin(["WET", "INTERMEDIATE"]), "wet", "dry"
    )
    out["race_phase"] = (
        pd.cut(
            progress,
            bins=[-0.001, 0.25, 0.55, 0.80, 1.001],
    ...
    out["tyre_age_bin"] = (
        pd.cut(
            tyre,
            bins=[-0.001, 3, 8, 15, 25, 40, 1000],
            labels=["fresh", "short", "medium", "old", "very_old", "extreme"],
            include_lowest=True,
        )
        .astype("string")
        .fillna("unknown")
    ...
    year_s = out["Year"].round().astype("Int64").astype("string").fillna("missing")
    stint_s = out["Stint"].round().astype("Int64").astype("string").fillna("missing")
    out["Driver_Race"] = out["Driver"] + "__" + out["Race"]
    out["Driver_Compound"] = out["Driver"] + "__" + out["Compound"]
    out["Race_Compound"] = out["Race"] + "__" + out["Compound"]
    out["Year_Race"] = year_s + "__" + out["Race"]
    out["Stint_Compound"] = stint_s + "__" + out["Compound"]
    out["Phase_Compound"] = out["race_phase"] + "__" + out["Compound"]
    out["Family_Phase"] = (
        out["compound_family"].astype("string") + "__" + out["race_phase"]
    )
    out["Race_Phase_Compound"] = (
        out["Race"] + "__" + out["race_phase"] + "__" + out["Compound"]
    )

    out["is_wet_family"] = (out["compound_family"] == "wet").astype("int8")
    out["dry_late_or_final"] = (
        (out["compound_family"] == "dry") & out["race_phase"].isin(["late", "final"])
    ).astype("int8")
    out["wet_late_or_final"] = (
        (out["compound_family"] == "wet") & out["race_phase"].isin(["late", "final"])
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000945"],
        "research_usage_note": "Verified assigned hypothesis 000945.",
    }, sort_keys=True))
