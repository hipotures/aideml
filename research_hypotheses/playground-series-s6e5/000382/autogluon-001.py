...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    driver = out["Driver"].astype(str).str.upper().str.strip()
    ...
    team_map = {
        ...
        "2022_GAS": "AlphaTauri",
        "2022_TSU": "AlphaTauri",
        "2022_VET": "Aston Martin",
        "2022_STR": "Aston Martin",
        "2022_HUL": "Aston Martin",
        "2022_ALB": "Williams",
        "2022_LAT": "Williams",
        "2022_DEV": "Williams",
        "2022_BOT": "Alfa Romeo",
        ...
        "2023_OCO": "Alpine",
        "2023_GAS": "Alpine",
        "2023_ALO": "Aston Martin",
        "2023_STR": "Aston Martin",
        "2023_ALB": "Williams",
        "2023_SAR": "Williams",
        "2023_BOT": "Alfa Romeo",
        "2023_ZHO": "Alfa Romeo",
        ...
        "2024_GAS": "Alpine",
        "2024_DOO": "Alpine",
        "2024_ALO": "Aston Martin",
        "2024_STR": "Aston Martin",
        "2024_ALB": "Williams",
        "2024_SAR": "Williams",
        "2024_COL": "Williams",
        "2024_BOT": "Sauber",
        ...
        "2025_RUS": "Mercedes",
        "2025_ANT": "Mercedes",
        "2025_ALO": "Aston Martin",
        "2025_STR": "Aston Martin",
        "2025_GAS": "Alpine",
        "2025_DOO": "Alpine",
        "2025_COL": "Alpine",
        "2025_ALB": "Williams",
        ...
        "2025_HUL": "Sauber",
        "2025_BOR": "Sauber",
        "2025_HAD": "Racing Bulls",
        "2025_TSU": "Racing Bulls",
        "2025_LAW": "Racing Bulls",
    }

    key = year.astype(str) + "_" + driver
    mapped_team = key.map(team_map)
    out["TeamKnown"] = mapped_team.notna().astype("int8")
    out["Team"] = mapped_team.fillna("UNK_" + driver)
    ...
    out["TightPitLaneCircuit"] = out["Race"].isin(tight).astype("int8")

    lap_keys = ["Race", "Year", "Team", "LapNumber"]
    team_lap_count = (
        out.groupby(lap_keys, sort=False)["Driver"].transform("count").astype("float32")
    )
    team_pits = (
        out.groupby(lap_keys, sort=False)["PitStop"].transform("sum").astype("float32")
    )
    out["TeamCarsOnLap"] = team_lap_count
    out["TeamPitCountThisLap"] = team_pits
    out["TeammatePittedThisLap"] = (
        (team_pits - out["PitStop"].astype(float)) > 0
    ).astype("int8")

    denom = (team_lap_count - 1).replace(0, np.nan)
    for col, name in [
        ("TyreLife", "TeammateTyreLife"),
        ...
        out[name] = (
            (
                (
                    out.groupby(lap_keys, sort=False)[col]
                    .transform("sum")
                    .astype(float)
                    - s
                )
    ...
    out["TeamTyreLifeGap"] = np.where(
        out["TeammateTyreLife"] >= 0,
        (out["TyreLife"] - out["TeammateTyreLife"]).abs(),
        -1,
    ).astype("float32")
    out["TeamPositionGap"] = np.where(
        out["TeammatePosition"] >= 0,
        (out["Position"] - out["TeammatePosition"]).abs(),
        -1,
    ).astype("float32")
    out["SameTeamStint"] = (
        (out["TeammateStint"] >= 0)
        & (out["Stint"].astype(float).round() == out["TeammateStint"].round())
    ).astype("int8")
    ...
    out["SharedPitWindow"] = (
        ...
        & (out["SameTeamStint"] == 1)
        & (out["TyreLife"] >= 8)
        & (out["TeammateTyreLife"] >= 8)
        & (out["TeamTyreLifeGap"].between(0, 5))
        & (out["RaceProgress"].between(0.12, 0.95))
    ).astype("int8")

    team_lap = (
        out[lap_keys + ["PitStop"]]
        .groupby(lap_keys, sort=False)["PitStop"]
        .sum()
        .reset_index()
    )
    team_lap["LapNumber"] = team_lap["LapNumber"] + 1
    team_lap = team_lap.rename(columns={"PitStop": "_TeamPitPrevLap"})
    out = out.merge(team_lap, on=lap_keys, how="left", sort=False)
    out["_TeamPitPrevLap"] = out["_TeamPitPrevLap"].fillna(0).astype("float32")

    tmp = out[["Race", "Year", "Driver", "LapNumber", "PitStop"]].copy()
    tmp["_row"] = np.arange(len(out))
    tmp = tmp.sort_values(["Race", "Year", "Driver", "LapNumber", "_row"])
    prev_pit = (
        tmp.groupby(["Race", "Year", "Driver"], sort=False)["PitStop"]
        .shift(1)
        .fillna(0)
    )
    prev_lap = tmp.groupby(["Race", "Year", "Driver"], sort=False)["LapNumber"].shift(1)
    tmp["_OwnPitPrevLap"] = np.where((tmp["LapNumber"] - prev_lap) == 1, prev_pit, 0)
    own_prev = tmp.sort_values("_row")["_OwnPitPrevLap"].to_numpy(dtype="float32")

    out["TeammateJustStopped"] = (
        (out["_TeamPitPrevLap"].to_numpy(dtype="float32") - own_prev) > 0
    ).astype("int8")
    out["LikelyDoubleStack"] = (
        (team_pits >= 2)
        | ((out["TeammatePittedThisLap"] == 1) & (out["SharedPitWindow"] == 1))
    ).astype("int8")

    out["TightPit_TeammatePitted"] = (
        out["TightPitLaneCircuit"] * out["TeammatePittedThisLap"]
    ).astype("int8")
    out["TightPit_LikelyDoubleStack"] = (
        out["TightPitLaneCircuit"] * out["LikelyDoubleStack"]
    ).astype("int8")
    out["Position_TeamConflict"] = (
        out["Position"].astype(float)
        * (out["LikelyDoubleStack"] + out["TeammateJustStopped"])
    ).astype("float32")
    out["FrontPack_TeamConflict"] = (
        (out["Position"] <= 6)
        & ((out["LikelyDoubleStack"] == 1) | (out["TeammateJustStopped"] == 1))
    ).astype("int8")

    return out.drop(columns=["_TeamPitPrevLap"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000382"],
        "research_usage_note": "Verified assigned hypothesis 000382.",
    }, sort_keys=True))
