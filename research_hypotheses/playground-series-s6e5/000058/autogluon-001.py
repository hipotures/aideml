...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    slick = comp.isin(["SOFT", "MEDIUM", "HARD"])

    group_cols = ["Year", "Race", "Driver"]
    sort_cols = [
        c for c in group_cols + ["LapNumber", "Stint", "TyreLife"] if c in out.columns
    ]
    ordered = out.assign(_compound_norm=comp).sort_values(sort_cols, kind="mergesort")

    g = ordered.groupby(group_cols, sort=False, observed=True)
    prev_comp = g["_compound_norm"].shift(1)
    ordered["_compound_prev"] = prev_comp
    ordered["compound_same_as_prev"] = (
        (ordered["_compound_norm"] == prev_comp).fillna(False).astype("int8")
    )
    ordered["compound_switched"] = (
        prev_comp.notna() & (ordered["_compound_norm"] != prev_comp)
    ).astype("int8")

    seen_before = np.zeros(len(ordered), dtype=bool)
    codes = ordered["_compound_norm"].fillna("__MISSING__").to_numpy()
    for _, idx in ordered.groupby(
        group_cols, sort=False, observed=True
    ).indices.items():
        seen = set()
        for pos in idx:
            val = codes[pos]
            seen_before[pos] = val in seen
            seen.add(val)

    ordered["compound_seen_before"] = seen_before.astype("int8")
    ordered["compound_new_in_sequence"] = (~seen_before).astype("int8")
    ordered["compound_new_slick"] = (
        (~seen_before) & slick.loc[ordered.index].to_numpy()
    ).astype("int8")
    ordered["compound_return_to_seen"] = (
        seen_before & (ordered["_compound_norm"] != prev_comp).fillna(False).to_numpy()
    ).astype("int8")

    pit = ordered["PitStop"].fillna(0).astype("int8")
    ordered["compound_switch_with_current_pit"] = (
        ordered["compound_switched"] & pit
    ).astype("int8")
    ordered["compound_new_with_current_pit"] = (
        ordered["compound_new_in_sequence"] & pit
    ).astype("int8")
    ordered["compound_return_with_current_pit"] = (
        ordered["compound_return_to_seen"] & pit
    ).astype("int8")

    add_cols = [
        "compound_same_as_prev",
        "compound_switched",
        "compound_seen_before",
        "compound_new_in_sequence",
        "compound_new_slick",
        "compound_return_to_seen",
        "compound_switch_with_current_pit",
        "compound_new_with_current_pit",
        "compound_return_with_current_pit",
    ]
    out[add_cols] = ordered.sort_index()[add_cols]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000058"],
        "research_usage_note": "Verified assigned hypothesis 000058.",
    }, sort_keys=True))
