...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["CompoundCode"] = out["Compound"].map(comp_map).astype("float32")
    ...
    s = out.sort_values(grp_cols + ["Position", "_ord"], kind="mergesort")

    g = s.groupby(grp_cols, sort=False)
    lap_count = g["Position"].transform("count").astype("float32")
    pos_rank = g["Position"].rank(method="first", ascending=True).astype("float32")
    pace_rank = g["LapTime (s)"].rank(method="average", pct=True).astype("float32")
    delta_rank = g["LapTime_Delta"].rank(method="average", pct=True).astype("float32")
    deg_rank = (
        g["Cumulative_Degradation"].rank(method="average", pct=True).astype("float32")
    )

    lt = s["LapTime (s)"]
    pc = s["Position_Change"]
    pos = s["Position"].astype("float32")
    tyre = s["TyreLife"].astype("float32")
    remain = (1.0 - s["RaceProgress"].astype("float32")).clip(0, 1)

    ahead_lt = g["LapTime (s)"].shift(1)
    behind_lt = g["LapTime (s)"].shift(-1)
    ahead2_lt = g["LapTime (s)"].shift(2)
    behind2_lt = g["LapTime (s)"].shift(-2)
    ahead_pc = g["Position_Change"].shift(1)
    behind_pc = g["Position_Change"].shift(-1)

    local_mean_3 = pd.concat([ahead_lt, lt, behind_lt], axis=1).mean(axis=1)
    local_mean_5 = pd.concat(
        [ahead2_lt, ahead_lt, lt, behind_lt, behind2_lt], axis=1
    ).mean(axis=1)
    local_std_5 = (
        pd.concat([ahead2_lt, ahead_lt, lt, behind_lt, behind2_lt], axis=1)
        .std(axis=1)
        .fillna(0)
    )

    s["traffic_ahead_pace"] = (lt - ahead_lt).fillna(0).astype("float32")
    s["traffic_behind_pace"] = (behind_lt - lt).fillna(0).astype("float32")
    s["traffic_neighbor_spread"] = (behind_lt - ahead_lt).fillna(0).astype("float32")
    s["traffic_local_mean3_diff"] = (lt - local_mean_3).fillna(0).astype("float32")
    s["traffic_local_mean5_diff"] = (lt - local_mean_5).fillna(0).astype("float32")
    s["traffic_local_std5"] = local_std_5.astype("float32")
    s["traffic_density_pos5"] = (
        ((g["Position"].shift(-2) - g["Position"].shift(2)).abs() / 4.0)
        .fillna(lap_count / 20.0)
        .astype("float32")
    )
    s["traffic_pressure_ahead"] = (
        (pc - ahead_pc).fillna(0) + s["traffic_ahead_pace"]
    ).astype("float32")
    s["traffic_pressure_behind"] = (
        (behind_pc - pc).fillna(0) + s["traffic_behind_pace"]
    ).astype("float32")
    s["traffic_pace_rank"] = pace_rank
    s["traffic_delta_rank"] = delta_rank
    s["traffic_deg_rank"] = deg_rank
    s["traffic_position_pct"] = (pos_rank / lap_count.clip(lower=1)).astype("float32")

    clean_ahead = (ahead_lt.isna() | ((lt - ahead_lt) < -0.20)).astype("int8")
    clear_behind = (behind_lt.isna() | ((behind_lt - lt) > 0.20)).astype("int8")
    s["free_air_ahead_flag"] = clean_ahead
    s["free_air_behind_flag"] = clear_behind
    s["free_air_proxy"] = (
        clean_ahead.astype("float32")
        + clear_behind.astype("float32")
        - (local_std_5 < 0.15).astype("float32")
    ).astype("float32")

    s["traffic_window_remaining"] = remain
    s["traffic_laps_remaining_est"] = (
        remain
        * s["LapNumber"].astype("float32")
        / s["RaceProgress"].astype("float32").clip(lower=0.01)
    ).astype("float32")
    s["traffic_pace_loss_x_remaining"] = (
        s["LapTime_Delta"].astype("float32") * remain
    ).astype("float32")
    s["traffic_tyre_x_density"] = (tyre * s["traffic_density_pos5"]).astype("float32")
    s["traffic_deg_x_free_air"] = (
        s["Cumulative_Degradation"].astype("float32") * s["free_air_proxy"]
    ).astype("float32")
    s["traffic_soft_free_air"] = (
        s["IsSoftCompound"].astype("float32") * s["free_air_proxy"]
    ).astype("float32")
    s["traffic_wet_density"] = (
        s["IsWetCompound"].astype("float32") * s["traffic_density_pos5"]
    ).astype("float32")

    out = s.sort_values("_ord", kind="mergesort").drop(columns=["_ord"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000371"],
        "research_usage_note": "Verified assigned hypothesis 000371.",
    }, sort_keys=True))
