...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out[
        ["Race", "Year", "Driver", "LapNumber", "PitStop", "Compound", "RaceProgress"]
    ].copy()
    work["_idx"] = np.arange(len(work))
    work["_race_year"] = race_year
    work = work.sort_values(
        ["_race_year", "Driver", "LapNumber", "_idx"], kind="mergesort"
    )

    grp = work.groupby(["_race_year", "Driver"], sort=False)
    pit = work["PitStop"].fillna(0).astype(float)
    current_stop_count_raw = pit.groupby(
        [work["_race_year"], work["Driver"]], sort=False
    ).cumsum()
    prior_stops = (
        current_stop_count_raw.groupby([work["_race_year"], work["Driver"]], sort=False)
        .shift(1)
        .fillna(0.0)
    )
    current_stops = prior_stops + pit

    compound = work["Compound"].astype(str).str.upper().str.strip()
    slick = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype(int)
    slick_seen_now = (
        pd.get_dummies(compound)
        .reindex(columns=["SOFT", "MEDIUM", "HARD"], fill_value=0)
        .astype(int)
        .groupby([work["_race_year"], work["Driver"]], sort=False)
        .cummax()
    )
    slick_seen_prior = (
        slick_seen_now.groupby([work["_race_year"], work["Driver"]], sort=False)
        .shift(1)
        .fillna(0)
        .astype(int)
    )

    prior_slick_div = slick_seen_prior.sum(axis=1).astype(int)
    current_slick_div = slick_seen_now.sum(axis=1).astype(int)

    stop_debt_before = ((prior_stops < 1) | (prior_slick_div < 2)).astype(int)
    stop_debt_after = ((current_stops < 1) | (current_slick_div < 2)).astype(int)
    remaining_laps = (
        (1.0 - work["RaceProgress"].clip(0, 1))
        * work["LapNumber"].clip(lower=1)
        / work["RaceProgress"].clip(lower=0.01)
    ).clip(lower=0)
    laps_per_debt = remaining_laps / np.maximum(stop_debt_after, 1)

    aligned = pd.DataFrame(
        {
            "PriorPitStops_Causal": prior_stops,
            "CurrentPitStops_Causal": current_stops,
            "PriorSlickCompoundDiversity": prior_slick_div,
            "CurrentSlickCompoundDiversity": current_slick_div,
            "StopDebtBeforeCurrentLap": stop_debt_before,
            "StopDebtAfterCurrentLap": stop_debt_after,
            "LapsPerRemainingStopDebt": laps_per_debt,
            "CanFinishButStillOwesStop": (
                (remaining_laps <= 1.5) & (stop_debt_after > 0)
            ).astype(int),
            "CurrentPitRelievesStopDebt": (
                (pit > 0) & (stop_debt_before > 0) & (stop_debt_after == 0)
            ).astype(int),
            "CurrentLapIsSlick": slick,
        },
        index=work["_idx"].to_numpy(),
    ).sort_index()
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000005.",
    }, sort_keys=True))
