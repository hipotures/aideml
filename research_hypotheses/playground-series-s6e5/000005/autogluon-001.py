...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prior_stops = grp["PitStop"].cumsum().shift(1).fillna(0.0)
    prior_stops = prior_stops.where(grp.cumcount() > 0, 0.0)
    current_stops = prior_stops + pit

    slick = work["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype(int)
    comp_codes = (
        work["Compound"].map({"SOFT": 1, "MEDIUM": 2, "HARD": 4}).fillna(0).astype(int)
    )
    current_slick_mask = comp_codes.groupby(
        [work["_race_year"], work["Driver"]], sort=False
    ).cumsum()
    prior_slick_mask = current_slick_mask - comp_codes

    prior_slick_div = (
        ((prior_slick_mask & 1) > 0).astype(int)
        + ((prior_slick_mask & 2) > 0).astype(int)
        + ((prior_slick_mask & 4) > 0).astype(int)
    )
    current_slick_div = (
        ((current_slick_mask & 1) > 0).astype(int)
        + ((current_slick_mask & 2) > 0).astype(int)
        + ((current_slick_mask & 4) > 0).astype(int)
    )
