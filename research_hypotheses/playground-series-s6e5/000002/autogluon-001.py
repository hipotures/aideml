...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Driver_Race_Year"] = (
        df["Driver"].astype(str) + "_" + df["Race_Year"].astype(str)
    )

    compound = df["Compound"].astype("string")
    df["Compound_is_dry"] = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    df["Compound_is_wet"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    df["Compound_is_soft"] = (compound == "SOFT").astype("int8")
    df["Compound_is_medium"] = (compound == "MEDIUM").astype("int8")
    df["Compound_is_hard"] = (compound == "HARD").astype("int8")

    df["TyreLife_ratio"] = df["TyreLife"] / df["LapNumber"].clip(lower=1)
    df["Stint_progress_proxy"] = df["TyreLife"] / (
        df["TyreLife"] + (df["Stint"] - 1).clip(lower=0)
    )
    df["Lap_remaining_proxy"] = 1.0 - df["RaceProgress"]
    df["TyreLife_x_RaceProgress"] = df["TyreLife"] * df["RaceProgress"]
    df["Degradation_per_tyre_lap"] = df["Cumulative_Degradation"] / df["TyreLife"].clip(
        lower=1
    )
    df["Degradation_per_race_progress"] = df["Cumulative_Degradation"] / df[
        "RaceProgress"
    ].clip(lower=1e-3)

    race_cols = ["Race_Year"]
    race_stint_cols = ["Race_Year", "Stint"]
    driver_race_cols = ["Driver_Race_Year"]

    race_laptime_mean = df.groupby(race_cols)["LapTime (s)"].transform("mean")
    race_laptime_std = df.groupby(race_cols)["LapTime (s)"].transform("std").fillna(0)
    df["LapTime_race_rel"] = df["LapTime (s)"] - race_laptime_mean
    df["LapTime_race_z"] = df["LapTime_race_rel"] / race_laptime_std.replace(0, 1)

    race_delta_mean = df.groupby(race_cols)["LapTime_Delta"].transform("mean")
    df["LapTime_Delta_race_rel"] = df["LapTime_Delta"] - race_delta_mean

    race_pos_mean = df.groupby(race_cols)["Position"].transform("mean")
    df["Position_race_rel"] = df["Position"] - race_pos_mean

    race_tyre_mean = df.groupby(race_cols)["TyreLife"].transform("mean")
    race_stint_tyre_mean = df.groupby(race_stint_cols)["TyreLife"].transform("mean")
    df["TyreLife_race_rel"] = df["TyreLife"] - race_tyre_mean
    df["TyreLife_stint_rel"] = df["TyreLife"] - race_stint_tyre_mean

    driver_pos_mean = df.groupby(driver_race_cols)["Position"].transform("mean")
    driver_delta_mean = df.groupby(driver_race_cols)["LapTime_Delta"].transform("mean")
    df["Driver_position_rel"] = df["Position"] - driver_pos_mean
    df["Driver_delta_rel"] = df["LapTime_Delta"] - driver_delta_mean

    df["Position_x_Progress"] = df["Position"] * df["RaceProgress"]
    df["PositionChange_x_Progress"] = df["Position_Change"] * df["RaceProgress"]
    df["TyreLife_x_CompoundWet"] = df["TyreLife"] * df["Compound_is_wet"]
    df["Degradation_x_CompoundWet"] = (
        df["Cumulative_Degradation"] * df["Compound_is_wet"]
    )

    cat_cols = ["Race", "Driver", "Compound", "Race_Year", "Driver_Race_Year"]
    for col in cat_cols:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000002.",
    }, sort_keys=True))
