...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    obj_cols = [c for c in ["Compound", "Driver", "Race"] if c in df.columns]
    for c in obj_cols:
    ...
    if "Compound" in df.columns:
        ...
        df["is_slick_compound"] = (
            df["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
        )
        compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
        df["compound_code"] = df["Compound"].map(compound_map).fillna(-1).astype("int8")

    if "Race" in df.columns:
        df["is_testing_race"] = (
            df["Race"].str.contains("TEST", regex=False).astype("int8")
        )

    count_specs = []
    for c in ["Driver", "Race", "Compound", "Year", "Stint"]:
        if c in df.columns:
            count_specs.append((c, f"{c.lower()}_freq"))
    if {"Driver", "Race"}.issubset(df.columns):
        dr_key = df["Driver"] + "__" + df["Race"]
        df["driver_race_key"] = dr_key
        count_specs.append(("driver_race_key", "driver_race_freq"))
    if {"Driver", "Year"}.issubset(df.columns):
        dy_key = df["Driver"] + "__" + df["Year"].astype(str)
        df["driver_year_key"] = dy_key
        count_specs.append(("driver_year_key", "driver_year_freq"))
    if {"Race", "Year"}.issubset(df.columns):
        ry_key = df["Race"] + "__" + df["Year"].astype(str)
        df["race_year_key"] = ry_key
        count_specs.append(("race_year_key", "race_year_freq"))
    if {"Race", "Compound"}.issubset(df.columns):
        rc_key = df["Race"] + "__" + df["Compound"]
        df["race_compound_key"] = rc_key
        count_specs.append(("race_compound_key", "race_compound_freq"))

    n_rows = float(len(df))
    for src, dst in count_specs:
        vc = df[src].value_counts(dropna=False)
        df[dst] = df[src].map(vc).astype("float32")
        df[f"{dst}_share"] = (df[dst] / n_rows).astype("float32")
        df[f"{dst}_rare"] = (df[dst] <= 3).astype("int8")

    if {"TyreLife", "LapNumber"}.issubset(df.columns):
        df["tyrelife_over_lap"] = (df["TyreLife"] / (df["LapNumber"] + eps)).astype(
            "float32"
        )
    if {"TyreLife", "RaceProgress"}.issubset(df.columns):
        df["tyrelife_over_progress"] = (
            df["TyreLife"] / (df["RaceProgress"] + eps)
        ).astype("float32")
    if {"Cumulative_Degradation", "TyreLife"}.issubset(df.columns):
        df["degradation_per_lap"] = (
            df["Cumulative_Degradation"] / (df["TyreLife"] + eps)
        ).astype("float32")
        df["degradation_abs_per_lap"] = (
            df["Cumulative_Degradation"].abs() / (df["TyreLife"] + eps)
        ).astype("float32")
    if {"LapTime (s)", "TyreLife"}.issubset(df.columns):
        df["laptime_per_tyre_lap"] = (
            df["LapTime (s)"] / (df["TyreLife"] + eps)
        ).astype("float32")
    if {"LapTime_Delta", "LapTime (s)"}.issubset(df.columns):
        df["delta_over_laptime"] = (
            df["LapTime_Delta"] / (df["LapTime (s)"] + eps)
        ).astype("float32")
    if {"Position", "LapNumber"}.issubset(df.columns):
        df["position_over_lap"] = (df["Position"] / (df["LapNumber"] + eps)).astype(
            "float32"
        )
    if {"Position_Change", "LapNumber"}.issubset(df.columns):
        df["position_change_rate"] = (
            df["Position_Change"] / (df["LapNumber"] + eps)
        ).astype("float32")

    group_defs = []
    if {"Race", "Year", "LapNumber"}.issubset(df.columns):
        group_defs.append((["Race", "Year", "LapNumber"], "ryl"))
    if {"Race", "Year", "Stint"}.issubset(df.columns):
        group_defs.append((["Race", "Year", "Stint"], "rys"))
    if {"Race", "Year", "Compound"}.issubset(df.columns):
        group_defs.append((["Race", "Year", "Compound"], "ryc"))
    ...
    for keys, suffix in group_defs:
        grp = df.groupby(keys, sort=False)
        for col in num_targets:
            mean = grp[col].transform("mean")
            std = grp[col].transform("std").fillna(0.0)
            df[f"{col}_{suffix}_mean_diff"] = (df[col] - mean).astype("float32")
            df[f"{col}_{suffix}_z"] = ((df[col] - mean) / (std + eps)).astype("float32")

    if {"is_wet_compound", "is_testing_race"}.issubset(df.columns):
        df["shift_regime_flag"] = (
            df["is_wet_compound"] | df["is_testing_race"]
        ).astype("int8")
    if {"shift_regime_flag", "driver_freq"}.issubset(df.columns):
        df["shift_x_driver_rarity"] = (
            df["shift_regime_flag"] / (df["driver_freq"] + 1.0)
        ).astype("float32")
    if {"shift_regime_flag", "race_freq"}.issubset(df.columns):
        df["shift_x_race_rarity"] = (
            df["shift_regime_flag"] / (df["race_freq"] + 1.0)
        ).astype("float32")
    if {"is_wet_compound", "TyreLife"}.issubset(df.columns):
        df["wet_tyrelife"] = (df["is_wet_compound"] * df["TyreLife"]).astype("float32")
    if {"is_testing_race", "LapTime_Delta"}.issubset(df.columns):
        df["testing_delta"] = (df["is_testing_race"] * df["LapTime_Delta"]).astype(
            "float32"
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000253"],
        "research_usage_note": "Verified assigned hypothesis 000253.",
    }, sort_keys=True))
