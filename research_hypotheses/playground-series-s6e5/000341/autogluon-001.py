...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = out["RaceProgress"].astype(float).clip(0.005, 1.0)
    ...
    remaining_laps = (est_total_laps - lap).clip(0, 120)
    ...
    out["IsDryCompound"] = is_dry
    out["IsWetCompound"] = is_wet

    out["Positive_LapTime_Delta"] = delta.clip(lower=0).astype("float32")
    out["Positive_Degradation"] = degr.clip(lower=0).astype("float32")
    out["TyreLife_To_Remaining"] = (
        (tyre / (remaining_laps + 1.0)).clip(0, 80).astype("float32")
    )
    out["CurrentTyreFinishExcess"] = (tyre - remaining_laps).astype("float32")

    dry_rows = is_dry.astype(float)
    out["Dry_TyreLife"] = (tyre * dry_rows).astype("float32")
    out["Dry_CurrentTyreFinishExcess"] = ((tyre - remaining_laps) * dry_rows).astype(
        "float32"
    )
    out["Dry_Positive_LapTime_Delta"] = (delta.clip(lower=0) * dry_rows).astype(
        "float32"
    )

    stop_debt = np.where((is_dry == 1) & (stint <= 1) & (remaining_laps > 2), 1.0, 0.0)
    out["StopDebtAfterCurrent"] = stop_debt.astype("float32")
    out["StopDebt_RemainingPressure"] = (stop_debt / (remaining_laps + 1.0)).astype(
        "float32"
    )

    out["LateRace_TyreAge"] = (tyre * progress).astype("float32")
    out["PitRulePressure"] = (
        out["StopDebtAfterCurrent"] * out["TyreLife_To_Remaining"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000341.",
    }, sort_keys=True))
