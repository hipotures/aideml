...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["is_dry"] = X["Compound"].isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")
    ...
    race_prog = X["RaceProgress"].astype("float32")
    deg = X["Cumulative_Degradation"].astype("float32")
    ...
    safe_tyre = tyre_life.clip(lower=1.0)
    safe_lap = lap_num.clip(lower=1.0)
    laps_left_frac = (1.0 - race_prog).clip(lower=0.0)

    X["degradation_per_tyre_lap"] = deg / safe_tyre
    X["degradation_per_race_lap"] = deg / safe_lap
    X["tyre_age_vs_race"] = tyre_life / safe_lap
    X["stint_index_norm"] = stint / 8.0
    X["position_norm"] = pos / 20.0
    X["position_change_abs"] = pos_chg.abs()
    X["laps_left_frac"] = laps_left_frac

    X["wear_pressure"] = tyre_life * laps_left_frac
    X["late_stint_wear"] = tyre_life * race_prog
    X["compound_wear_load"] = X["compound_code"] * tyre_life
    X["compound_deg_load"] = X["compound_code"] * X["degradation_per_tyre_lap"]
    X["pace_shock"] = lap_delta / lap_time.clip(lower=1.0)
    X["deg_pace_interaction"] = X["degradation_per_tyre_lap"] * lap_delta
    X["position_risk"] = X["position_norm"] + X["position_change_abs"] / 20.0
    X["lost_places_pressure"] = pos_chg.clip(upper=0).abs() * laps_left_frac
    X["gained_places_buffer"] = pos_chg.clip(lower=0) * (1.0 - laps_left_frac)
    X["wet_wear_pressure"] = (
        X["is_wet"] * tyre_life * (1.0 + X["degradation_per_tyre_lap"].abs())
    )
    X["dry_attack_signal"] = (
        X["is_dry"] * X["is_soft"] * (1.0 - X["position_norm"]) * laps_left_frac
    )

    X["Driver_freq"] = X.groupby("Driver")["Driver"].transform("size").astype("float32")
    X["Race_freq"] = X.groupby("Race")["Race"].transform("size").astype("float32")
    X["Compound_freq"] = (
        X.groupby("Compound")["Compound"].transform("size").astype("float32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000388.",
    }, sort_keys=True))
