...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp_race_lap = ["Year", "Race", "LapNumber"]

    compound_median_life = df.groupby("Compound")["TyreLife"].transform("median")
    df["undercut_old_for_compound"] = df["TyreLife"] / compound_median_life.replace(
        0, np.nan
    )
    df["undercut_old_for_compound"] = (
        df["undercut_old_for_compound"].fillna(1.0).clip(0, 5)
    )

    df["undercut_positive_pace_loss"] = df["LapTime_Delta"].clip(lower=0)
    race_lap_delta_med = df.groupby(grp_race_lap)["LapTime_Delta"].transform("median")
    df["undercut_pace_vs_race_lap"] = (df["LapTime_Delta"] - race_lap_delta_med).clip(
        lower=0
    )

    df["undercut_position_pct"] = (21 - df["Position"]) / 20.0

    fresh_flag = (df["TyreLife"] <= 2).astype(float)
    pit_flag = df["PitStop"].astype(float)

    race_lap_count = df.groupby(grp_race_lap)["Driver"].transform("size").astype(float)
    fresh_sum = fresh_flag.groupby([df[c] for c in grp_race_lap]).transform("sum")
    pit_sum = pit_flag.groupby([df[c] for c in grp_race_lap]).transform("sum")

    denom = (race_lap_count - 1).replace(0, np.nan)
    df["undercut_peer_fresh_rate_loo"] = (
        ((fresh_sum - fresh_flag) / denom).fillna(0.0).clip(0, 1)
    )
    df["undercut_peer_pit_rate_loo"] = (
        ((pit_sum - pit_flag) / denom).fillna(0.0).clip(0, 1)
    )

    lap_compound_med = df.groupby(["Year", "Race", "LapNumber", "Compound"])[
        "TyreLife"
    ].transform("median")
    df["undercut_current_window_pressure"] = (
        (df["TyreLife"] / lap_compound_med.replace(0, np.nan)).fillna(1.0).clip(0, 5)
    )

    old_component = (
        0.5 * df["undercut_old_for_compound"]
        + 0.5 * df["undercut_current_window_pressure"]
    )
    df["undercut_vulnerability_score"] = (
        0.30 * old_component
        + 0.25
        * df["undercut_positive_pace_loss"]
        / (df["undercut_positive_pace_loss"].median() + 1e-6)
        + 0.15
        * df["undercut_pace_vs_race_lap"]
        / (df["undercut_pace_vs_race_lap"].median() + 1e-6)
        + 0.15 * df["undercut_position_pct"]
        + 0.10 * df["undercut_peer_fresh_rate_loo"]
        + 0.05 * df["undercut_peer_pit_rate_loo"]
    )

    score_cut = df["undercut_vulnerability_score"].median()
    df["undercut_high_vulnerability_flag"] = (
        df["undercut_vulnerability_score"] > score_cut
    ).astype(int)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000046"],
        "research_usage_note": "Verified assigned hypothesis 000046.",
    }, sort_keys=True))
