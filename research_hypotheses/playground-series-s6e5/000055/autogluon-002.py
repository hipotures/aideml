...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_soft_bool = compound.eq("SOFT").fillna(False)
    is_medium_bool = compound.eq("MEDIUM").fillna(False)
    is_hard_bool = compound.eq("HARD").fillna(False)
    is_inter_bool = compound.eq("INTERMEDIATE").fillna(False)
    is_wet_bool = compound.eq("WET").fillna(False)
    is_slick_bool = is_soft_bool | is_medium_bool | is_hard_bool
    is_soft = is_soft_bool.astype("int8")
    is_medium = is_medium_bool.astype("int8")
    is_hard = is_hard_bool.astype("int8")
    is_inter = is_inter_bool.astype("int8")
    is_wet = is_wet_bool.astype("int8")
    is_slick = is_slick_bool.astype("int8")

    out["compound_is_soft"] = is_soft
    out["compound_is_medium"] = is_medium
    out["compound_is_hard"] = is_hard
    out["compound_is_intermediate"] = is_inter
    out["compound_is_wet"] = is_wet
    out["compound_is_slick"] = is_slick
    out["compound_is_dry_state"] = is_slick

    expected_life = np.select(
        [
            is_soft_bool.to_numpy(dtype=bool),
            is_medium_bool.to_numpy(dtype=bool),
            is_hard_bool.to_numpy(dtype=bool),
            is_inter_bool.to_numpy(dtype=bool),
            is_wet_bool.to_numpy(dtype=bool),
        ],
        [18.0, 28.0, 38.0, 22.0, 18.0],
        default=28.0,
    )
    fresh_slick_life = np.array([18.0, 28.0, 38.0], dtype="float64")

    lap = out["LapNumber"].astype("float64")
    progress = out["RaceProgress"].astype("float64").clip(0.01, 1.0)
    tyre_life = out["TyreLife"].astype("float64")
    stint = out["Stint"].astype("float64")

    race_len_est = (lap / progress).clip(1.0, 90.0)
    laps_remaining = (race_len_est - lap).clip(0.0, 90.0)

    out["race_len_est"] = race_len_est
    out["laps_remaining_est"] = laps_remaining
    out["race_phase_early"] = (progress < 0.33).astype("int8")
    out["race_phase_mid"] = ((progress >= 0.33) & (progress < 0.66)).astype("int8")
    out["race_phase_late"] = (progress >= 0.66).astype("int8")
    out["race_phase_final_quarter"] = (progress >= 0.75).astype("int8")

    pressure = tyre_life / expected_life
    out["compound_expected_life"] = expected_life
    out["tyre_pressure"] = pressure
    out["tyre_pressure_sq"] = pressure * pressure
    out["tyre_life_remaining_expected"] = expected_life - tyre_life
    out["current_tyre_can_finish"] = (
        expected_life - tyre_life >= laps_remaining
    ).astype("int8")
    out["current_finish_margin"] = expected_life - tyre_life - laps_remaining
    out["current_finish_pressure"] = laps_remaining / (expected_life - tyre_life + 1.0)

    slick_margins = fresh_slick_life.reshape(1, 3) - laps_remaining.to_numpy().reshape(
        -1, 1
    )
    out["fresh_slick_option_count"] = (slick_margins >= 0.0).sum(axis=1).astype("int8")
    out["fresh_slick_best_margin"] = slick_margins.max(axis=1)
    out["fresh_slick_worst_margin"] = slick_margins.min(axis=1)
    out["fresh_slick_can_soft"] = (18.0 >= laps_remaining).astype("int8")
    out["fresh_slick_can_medium"] = (28.0 >= laps_remaining).astype("int8")
    out["fresh_slick_can_hard"] = (38.0 >= laps_remaining).astype("int8")
    out["fresh_slick_shortest_feasible_life"] = np.where(
        laps_remaining <= 18.0,
        18.0,
        np.where(
            laps_remaining <= 28.0, 28.0, np.where(laps_remaining <= 38.0, 38.0, 99.0)
        ),
    )

    out["slick_diversity_seen_count"] = (
        ((is_soft == 1) | (stint > 1)).astype("int8")
        + ((is_medium == 1) | (stint > 2)).astype("int8")
        + ((is_hard == 1) | (stint > 3)).astype("int8")
    )
    out["stint_progress_pressure"] = tyre_life / (stint + 1.0)
    out["lap_to_tyre_ratio"] = lap / (tyre_life + 1.0)

    deg = out["Cumulative_Degradation"].astype("float64")
    delta = out["LapTime_Delta"].astype("float64")
    laptime = out["LapTime (s)"].astype("float64")

    out["degradation_clip"] = deg.clip(-50.0, 300.0)
    out["degradation_per_tyre_lap"] = deg / (tyre_life + 1.0)
    out["degradation_pressure"] = out["degradation_per_tyre_lap"] * pressure
    out["lap_delta_clip"] = delta.clip(-30.0, 60.0)
    out["lap_delta_abs_clip"] = delta.abs().clip(0.0, 120.0)
    out["lap_time_clip"] = laptime.clip(60.0, 180.0)
    out["slow_lap_flag"] = (laptime > 140.0).astype("int8")
    out["large_delta_flag"] = (delta.abs() > 20.0).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000055"],
        "research_usage_note": "Verified assigned hypothesis 000055.",
    }, sort_keys=True))
