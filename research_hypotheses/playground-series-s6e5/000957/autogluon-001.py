...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_delta_abs = out["LapTime_Delta"].abs()
    lap_time = out["LapTime (s)"]
    deg_abs = out["Cumulative_Degradation"].abs()

    delta_hi = lap_delta_abs.quantile(0.985)
    laptime_hi = lap_time.quantile(0.985)
    deg_hi = deg_abs.quantile(0.985)

    transition_anomaly = (
        (lap_delta_abs >= delta_hi)
        | (lap_time >= laptime_hi)
        | ((deg_abs >= deg_hi) & (out["RaceProgress"] > 0.12))
    ).astype("int8")

    wet_gate = np.maximum(is_wet_compound, transition_anomaly).astype("int8")
    dry_gate = (1 - wet_gate).astype("int8")

    out["is_dry_compound"] = is_dry_compound
    out["is_wet_compound"] = is_wet_compound
    out["transition_anomaly"] = transition_anomaly
    out["wet_gate"] = wet_gate
    out["dry_gate"] = dry_gate

    out["tyre_life_progress"] = out["TyreLife"] * out["RaceProgress"]
    out["stint_tyre_life"] = out["Stint"] * out["TyreLife"]
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / (
        out["TyreLife"] + 1.0
    )
    out["lap_delta_per_tyre_lap"] = out["LapTime_Delta"] / (out["TyreLife"] + 1.0)
    out["lap_time_ratio_to_life"] = out["LapTime (s)"] / (out["TyreLife"] + 1.0)
    out["late_stint_pressure"] = out["RaceProgress"] * np.log1p(out["TyreLife"])
    out["position_loss_pressure"] = out["Position"] * np.maximum(
    ...
    for col in base_cols:
        out[f"dry_{col}"] = out[col] * dry_gate
        out[f"wet_{col}"] = out[col] * wet_gate

    out["wet_crossover_pressure"] = wet_gate * (
        lap_delta_abs
        + np.maximum(out["LapTime (s)"] - out["LapTime (s)"].median(), 0)
        + np.maximum(out["Cumulative_Degradation"], 0) / 10.0
    )
    out["dry_rule_pressure"] = (
        dry_gate * out["RaceProgress"] * out["Stint"] * np.log1p(out["TyreLife"])
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000957"],
        "research_usage_note": "Verified assigned hypothesis 000957.",
    }, sort_keys=True))
