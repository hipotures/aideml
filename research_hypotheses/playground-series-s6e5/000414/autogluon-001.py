...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").fillna("UNK")
    driver = out["Driver"].astype("string").fillna("UNK")
    year = out["Year"].astype("string").fillna("UNK")
    compound = out["Compound"].astype("string").fillna("UNK")

    out["Race_Year"] = race + "_" + year
    out["Race_Year_Driver"] = out["Race_Year"] + "_" + driver
    out["Race_Year_Driver_Stint"] = (
        out["Race_Year_Driver"] + "_S" + out["Stint"].astype("string")
    )
    out["Compound_Race"] = compound + "_" + race
    out["Compound_Stint"] = compound + "_S" + out["Stint"].astype("string")

    g = out.groupby("Race_Year_Driver", sort=False)
    gs = out.groupby("Race_Year_Driver_Stint", sort=False)
    ...
    for col in [
        ...
        out[f"{col}_grp_rank_pct"] = (
            g[col].rank(method="average", pct=True).astype("float32")
        )
        out[f"{col}_stint_rank_pct"] = (
            gs[col].rank(method="average", pct=True).astype("float32")
        )
        grp_mean = g[col].transform("mean").astype(float)
        grp_std = g[col].transform("std").replace(0, np.nan).astype(float)
        out[f"{col}_grp_z"] = (
            ((vals - grp_mean) / grp_std)
            .replace([np.inf, -np.inf], 0)
            .fillna(0)
            .astype("float32")
        )

    stint_max_tyre = gs["TyreLife"].transform("max").replace(0, np.nan).astype(float)
    stint_max_lap = gs["LapNumber"].transform("max").replace(0, np.nan).astype(float)
    race_max_lap = g["LapNumber"].transform("max").replace(0, np.nan).astype(float)

    out["TyreLife_frac_stint"] = (
        (out["TyreLife"].astype(float) / stint_max_tyre).fillna(0).astype("float32")
    )
    out["Lap_frac_stint"] = (
        (out["LapNumber"].astype(float) / stint_max_lap).fillna(0).astype("float32")
    )
    out["Lap_frac_driver_race"] = (
        (out["LapNumber"].astype(float) / race_max_lap).fillna(0).astype("float32")
    )
    out["Laps_left_driver_race"] = (
        (race_max_lap - out["LapNumber"].astype(float)).fillna(0).astype("float32")
    )

    out["TyreLife_x_Progress"] = (
        out["TyreLife"].astype(float) * out["RaceProgress"].astype(float)
    ).astype("float32")
    out["Degradation_per_TyreLife"] = (
        out["Cumulative_Degradation"].astype(float)
        / (out["TyreLife"].astype(float) + 1.0)
    ).astype("float32")
    out["LapDelta_per_TyreLife"] = (
        out["LapTime_Delta"].astype(float) / (out["TyreLife"].astype(float) + 1.0)
    ).astype("float32")
    out["PositionLoss_flag"] = (out["Position_Change"].astype(float) > 0).astype("int8")
    out["PositionGain_flag"] = (out["Position_Change"].astype(float) < 0).astype("int8")

    out["is_wet_compound"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["is_slick_compound"] = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["late_stint_high_tyre"] = (
        (out["TyreLife_frac_stint"] > 0.7) & (out["TyreLife"] >= 10)
    ).astype("int8")
    out["late_race"] = (out["RaceProgress"].astype(float) > 0.75).astype("int8")
    out["early_race"] = (out["RaceProgress"].astype(float) < 0.25).astype("int8")
    ...
    for col in [
        ...
        freq = out[col].map(out[col].value_counts(dropna=False)).astype("float32")
        out[f"{col}_freq"] = np.log1p(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000414"],
        "research_usage_note": "Verified assigned hypothesis 000414.",
    }, sort_keys=True))
