...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver = out["Driver"].astype("string").str.upper()
    year = out["Year"].astype(int)
    team = driver.map({})
    team = pd.Series(pd.NA, index=out.index, dtype="string")
    for y, mp in team_by_year.items():
        m = year.eq(y)
        team.loc[m] = driver.loc[m].map(mp)

    out["Team"] = team.fillna("UNKNOWN_TEAM").astype("category")
    out["team_unknown_flag"] = (
        out["Team"].astype("string").eq("UNKNOWN_TEAM").astype("int8")
    )

    group_cols = ["Year", "Race", "Team", "LapNumber"]
    lap_team_pits = out.groupby(group_cols, observed=True)["PitStop"].transform("sum")
    out["teammate_pitted_current_lap"] = (
        (lap_team_pits - out["PitStop"]).clip(lower=0).astype("int8")
    )

    prev = out[["Year", "Race", "Team", "Driver", "LapNumber", "PitStop"]].copy()
    prev["LapNumber"] = prev["LapNumber"] + 1
    prev = prev.rename(columns={"PitStop": "_team_prev_pits"})
    prev_team = prev.groupby(group_cols, observed=True)["_team_prev_pits"].sum()
    cur_key = pd.MultiIndex.from_frame(out[group_cols])
    prev_sum = (
        pd.Series(cur_key.map(prev_team), index=out.index).fillna(0).astype(float)
    )
    out["teammate_pitted_previous_lap"] = (
        (prev_sum - out["PitStop"].where(out["LapNumber"].gt(1), 0))
        .clip(lower=0)
        .astype("int8")
    )

    team_tyre_sum = out.groupby(group_cols, observed=True)["TyreLife"].transform("sum")
    team_pos_sum = out.groupby(group_cols, observed=True)["Position"].transform("sum")
    team_count = (
        out.groupby(group_cols, observed=True)["Driver"]
        .transform("count")
        .clip(lower=1)
    )

    teammate_count = (team_count - 1).replace(0, np.nan)
    teammate_tyre = (team_tyre_sum - out["TyreLife"]) / teammate_count
    teammate_pos = (team_pos_sum - out["Position"]) / teammate_count

    out["teammate_tyre_age_gap"] = (
        (out["TyreLife"] - teammate_tyre).fillna(0).astype("float32")
    )
    out["teammate_position_gap"] = (
        (out["Position"] - teammate_pos).fillna(0).astype("float32")
    )

    out["same_team_double_stack_pressure"] = (
        (out["teammate_pitted_current_lap"] > 0).astype("float32")
        * (1.0 / (1.0 + out["teammate_position_gap"].abs()))
        * (1.0 + out["RaceProgress"].astype(float))
    ).astype("float32")

    race_team_driver_lap = ["Year", "Race", "Team", "Driver", "LapNumber"]
    team_lap_stop = out.groupby(race_team_driver_lap, observed=True)[
        "PitStop"
    ].transform("max")
    out["_team_lap_stop"] = team_lap_stop
    out["team_stop_count"] = (
        out.sort_values(["Year", "Race", "Team", "LapNumber"])
        .groupby(["Year", "Race", "Team"], observed=True)["_team_lap_stop"]
        .cumsum()
        .reindex(out.index)
        .astype("float32")
    )
    out = out.drop(columns=["_team_lap_stop"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000476"],
        "research_usage_note": "Verified assigned hypothesis 000476.",
    }, sort_keys=True))
