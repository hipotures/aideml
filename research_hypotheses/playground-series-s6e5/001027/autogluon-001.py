...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = [out["Year"], race, out["LapNumber"]]
    field_med_lap = lap_time.groupby(grp, sort=False).transform("median")
    field_med_delta = lap_delta.groupby(grp, sort=False).transform("median")
    field_iqr_lap = (
        lap_time.groupby(grp, sort=False).transform(lambda s: s.quantile(0.75))
        - lap_time.groupby(grp, sort=False).transform(lambda s: s.quantile(0.25))
    ).replace(0, np.nan)

    rel_lap_gap = (lap_time - field_med_lap).fillna(0.0)
    rel_delta_gap = (lap_delta - field_med_delta).fillna(0.0)
    field_shock = (
        (rel_lap_gap / field_iqr_lap.fillna(field_iqr_lap.median()))
        .replace([np.inf, -np.inf], 0)
        .fillna(0.0)
    )

    delta_scale = lap_delta.abs().median()
    if not np.isfinite(delta_scale) or delta_scale <= 0:
        delta_scale = 1.0

    anomaly_raw = (
        0.55 * np.clip(np.abs(lap_delta) / (3.0 * delta_scale), 0, 6)
        + 0.30 * np.clip(np.abs(field_shock) / 4.0, 0, 6)
        + 0.15 * np.clip(np.abs(rel_delta_gap) / (3.0 * delta_scale), 0, 6)
    )
    out["regime_anomaly_score"] = anomaly_raw.astype("float32")
    out["regime_anomaly_lap"] = (anomaly_raw > 0.85).astype("int8")

    sigmoid = lambda x: 1.0 / (1.0 + np.exp(-np.clip(x, -30, 30)))
    out["gate_wet_sigmoid"] = sigmoid(3.0 * wet + 0.8 * anomaly_raw - 1.5).astype(
        "float32"
    )
    out["gate_dry_sigmoid"] = sigmoid(3.0 * dry - 0.6 * anomaly_raw - 1.0).astype(
        "float32"
    )
    out["gate_anomaly_sigmoid"] = sigmoid(2.2 * anomaly_raw - 1.4).astype("float32")

    out["tyre_life_x_dry"] = (tyre_life * dry).astype("float32")
    out["tyre_life_x_wet"] = (tyre_life * wet).astype("float32")
    out["degradation_x_dry"] = (degr * dry).astype("float32")
    out["degradation_x_wet"] = (degr * wet).astype("float32")
    out["lap_delta_x_anomaly"] = (lap_delta * out["gate_anomaly_sigmoid"]).astype(
        "float32"
    )
    out["race_progress_x_anomaly"] = (
        race_progress * out["gate_anomaly_sigmoid"]
    ).astype("float32")

    out["tyre_life_frac"] = (tyre_life / lap_num.clip(lower=1)).astype("float32")
    out["stint_lap_pressure"] = (tyre_life * np.log1p(stint)).astype("float32")
    out["late_race_tyre"] = (race_progress * tyre_life).astype("float32")
    out["position_loss_pressure"] = np.maximum(pos_chg, 0).astype("float32")
    out["position_gain_pressure"] = np.maximum(-pos_chg, 0).astype("float32")
    out["front_runner"] = (pos <= 5).astype("int8")
    out["traffic_zone"] = ((pos >= 6) & (pos <= 15)).astype("int8")

    out["driver_freq"] = driver.map(driver.value_counts(normalize=True)).astype(
        "float32"
    )
    out["race_freq"] = race.map(race.value_counts(normalize=True)).astype("float32")
    out["compound_freq"] = compound.map(compound.value_counts(normalize=True)).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001027"],
        "research_usage_note": "Verified assigned hypothesis 001027.",
    }, sort_keys=True))
