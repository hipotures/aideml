...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    obj_cols = ["Compound", "Driver", "Race"]
    for c in obj_cols:
        if c in out.columns:
            out[c] = out[c].astype("string").str.strip().str.upper().fillna("UNKNOWN")

    if "RaceProgress" in out.columns:
        rp = out["RaceProgress"].clip(0, 1)
        out["RaceProgress_bin"] = pd.cut(
            rp,
    ...
    if "Driver" in out.columns:
        train_drivers = set(out.loc[train_mask, "Driver"].dropna().unique())
        out["driver_seen_in_train"] = out["Driver"].isin(train_drivers).astype("int8")
        out["driver_novelty"] = (1 - out["driver_seen_in_train"]).astype("int8")
    ...
    for c in ratio_cols:
        tr = out.loc[train_mask, c]
        te = out.loc[test_mask, c]
        tr_counts = tr.value_counts(dropna=False)
        te_counts = te.value_counts(dropna=False)
        cats = tr_counts.index.union(te_counts.index)
        smooth = 20.0
        tr_rate = (tr_counts.reindex(cats, fill_value=0).astype(float) + smooth) / (
            n_train + smooth * len(cats)
        )
        te_rate = (te_counts.reindex(cats, fill_value=0).astype(float) + smooth) / (
            max(n - n_train, 1) + smooth * len(cats)
        )
        ratio = (te_rate / tr_rate).clip(0.2, 5.0)
        mapped = out[c].map(ratio).astype("float32").fillna(1.0)
        out[f"shift_ratio_{c}"] = mapped
        weights *= mapped.to_numpy(dtype="float32")

    out["shift_density_ratio"] = np.clip(weights, 0.1, 10.0).astype("float32")
    out["shift_log_density_ratio"] = np.log1p(out["shift_density_ratio"]).astype(
        "float32"
    )

    if {"TyreLife", "RaceProgress"}.issubset(out.columns):
        out["tyre_life_x_progress"] = out["TyreLife"] * out["RaceProgress"]
        out["tyre_life_per_progress"] = out["TyreLife"] / (out["RaceProgress"] + 0.02)

    if {"TyreLife", "Stint"}.issubset(out.columns):
        out["tyre_life_x_stint"] = out["TyreLife"] * out["Stint"]
        out["late_stint_old_tyre"] = (
            (out["Stint"] >= 2) & (out["TyreLife"] >= 15)
        ).astype("int8")

    if {"LapNumber", "RaceProgress"}.issubset(out.columns):
        out["estimated_total_laps"] = out["LapNumber"] / out["RaceProgress"].clip(
            lower=0.01
        )
    ...
    if {"LapTime (s)", "LapTime_Delta"}.issubset(out.columns):
        ...
        out["lap_delta_ratio"] = out["LapTime_Delta"] / out["LapTime (s)"].clip(
            lower=1.0
        )

    if {"Position", "Position_Change"}.issubset(out.columns):
        out["position_after_change"] = out["Position"] + out["Position_Change"]
        out["lost_positions_flag"] = (out["Position_Change"] > 0).astype("int8")
        out["gained_positions_flag"] = (out["Position_Change"] < 0).astype("int8")

    if {"Compound", "TyreLife"}.issubset(out.columns):
        for comp in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
            out[f"is_{comp.lower()}"] = (out["Compound"] == comp).astype("int8")
        out["soft_old_tyre"] = (
            (out["Compound"] == "SOFT") & (out["TyreLife"] >= 10)
        ).astype("int8")
        out["wet_or_intermediate"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000449.",
    }, sort_keys=True))
