...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Stint_progress"] = out["TyreLife"] / out.groupby(
        ["Year", "Race", "Driver", "Stint"], sort=False
    )["TyreLife"].transform("max").clip(lower=1)
    out["Laps_remaining"] = (
        (1.0 - out["RaceProgress"])
        * out["LapNumber"]
        / out["RaceProgress"].clip(lower=1e-3)
    )

    out["Deg_per_tyre_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["Deg_per_race_lap"] = out["Cumulative_Degradation"] / out["LapNumber"].clip(
        lower=1
    )
    out["Abs_LapTime_Delta"] = out["LapTime_Delta"].abs()
    out["PosChange_abs"] = out["Position_Change"].abs()

    race_median_laptime = out.groupby(["Year", "Race", "LapNumber"], sort=False)[
        "LapTime (s)"
    ].transform("median")
    race_median_delta = out.groupby(["Year", "Race", "LapNumber"], sort=False)[
        "LapTime_Delta"
    ].transform("median")
    out["LapTime_vs_race_median"] = out["LapTime (s)"] - race_median_laptime
    out["LapDelta_vs_race_median"] = out["LapTime_Delta"] - race_median_delta

    compound_order = {"HARD": 0, "MEDIUM": 1, "SOFT": 2, "INTERMEDIATE": 3, "WET": 4}
    out["Compound_code"] = (
        out["Compound"].astype(str).map(compound_order).fillna(-1).astype("int8")
    )
    out["Is_WetCompound"] = (
        out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]).astype("int8")
    )

    out["TyreLife_x_Deg"] = out["TyreLife"] * out["Cumulative_Degradation"]
    out["TyreLife_x_Delta"] = out["TyreLife"] * out["LapTime_Delta"]
    ...
    for c in [
        ...
        out[c + "_freq"] = out[c].astype(str).map(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000703.",
    }, sort_keys=True))
