...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prev_pit = pit_s.groupby(driver_code).shift(1).fillna(0.0).astype("float32")
    prev2_pit = pit_s.groupby(driver_code).shift(2).fillna(0.0).astype("float32")
    pit_count_prev = (pit_s.groupby(driver_code).cumsum() - pit_s).astype("float32")
    last_pit_lap = lap_s.where(pit_s > 0.5)
    last_pit_prev = (
