...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lapnum = (
        pd.to_numeric(out["LapNumber"], errors="coerce").fillna(0.0).astype("float32")
    )
    tyre_life = (
        pd.to_numeric(out["TyreLife"], errors="coerce").fillna(0.0).astype("float32")
    )
    stint = pd.to_numeric(out["Stint"], errors="coerce").fillna(0.0).astype("float32")

    race_code = out[["Race", "Year"]].groupby(["Race", "Year"], sort=False).ngroup()
    race_code = pd.Series(race_code, index=out.index)
    lap_code = (
        out[["Race", "Year", "LapNumber"]]
        .groupby(["Race", "Year", "LapNumber"], sort=False)
        .ngroup()
    )
    lap_code = pd.Series(lap_code, index=out.index)

    race_median = delta.groupby(race_code).transform("median")
    race_mad = (delta - race_median).abs().groupby(race_code).transform("median")
    race_scale = (1.4826 * race_mad).clip(lower=1.0)

    lap_median = delta.groupby(lap_code).transform("median")
    lap_mad = (delta - lap_median).abs().groupby(lap_code).transform("median")
    lap_std = delta.groupby(lap_code).transform("std").fillna(0.0)
    lap_count = delta.groupby(lap_code).transform("size").astype("float32")

    q10 = lap_code.map(delta.groupby(lap_code).quantile(0.10))
    q90 = lap_code.map(delta.groupby(lap_code).quantile(0.90))
    lap_trimmed_mean = (
        delta.clip(lower=q10, upper=q90).groupby(lap_code).transform("mean")
    )

    extreme_gap = (3.0 * race_scale).where((3.0 * race_scale) > 8.0, 8.0)
    positive_extreme = (delta > (race_median + extreme_gap)).astype("float32")
    lap_pos_shock_frac = positive_extreme.groupby(lap_code).transform("mean")

    lap_table = pd.DataFrame(
        {
            "lap_code": lap_code.to_numpy(),
            "race_code": race_code.to_numpy(),
            "LapNumber": lapnum.to_numpy(),
            "lap_median": lap_median.to_numpy(),
            "lap_pos_shock_frac": lap_pos_shock_frac.to_numpy(),
        }
    ).drop_duplicates("lap_code")
    lap_table = lap_table.sort_values(["race_code", "LapNumber"], kind="mergesort")
    lap_table["prev_lap_median"] = lap_table.groupby("race_code", sort=False)[
        "lap_median"
    ].shift(1)
    lap_table["prev_pos_frac"] = lap_table.groupby("race_code", sort=False)[
        "lap_pos_shock_frac"
    ].shift(1)
    lap_table["lap_median_change"] = (
        lap_table["lap_median"] - lap_table["prev_lap_median"]
    )
    lap_table["pos_frac_change"] = (
        lap_table["lap_pos_shock_frac"] - lap_table["prev_pos_frac"]
    )

    lap_change = lap_code.map(
        lap_table.set_index("lap_code")["lap_median_change"]
    ).fillna(0.0)
    frac_change = lap_code.map(
        lap_table.set_index("lap_code")["pos_frac_change"]
    ).fillna(0.0)

    shock_z = ((lap_median - race_median) / race_scale).clip(-8.0, 8.0)
    abrupt_z = (lap_change / race_scale).clip(-8.0, 8.0)
    dispersion_z = ((lap_mad / race_scale) - 1.0).clip(0.0, 6.0)

    score = (
        1.05 * shock_z.clip(lower=0.0)
        + 0.85 * abrupt_z.clip(lower=0.0)
        + 3.00 * lap_pos_shock_frac.clip(0.0, 1.0)
        + 0.35 * frac_change.clip(lower=0.0)
        + 0.25 * dispersion_z
        - 3.25
    ).clip(-20.0, 20.0)
    neutral_prob = pd.Series(1.0 / (1.0 + np.exp(-score)), index=out.index).astype(
        "float32"
    )
    neutral_flag = (
        (neutral_prob > 0.60) | ((shock_z > 2.25) & (lap_pos_shock_frac > 0.30))
    ).astype("float32")

    max_lap = lapnum.groupby(race_code).transform("max")
    laps_remaining = (max_lap - lapnum).clip(lower=0.0).astype("float32")

    order = out.sort_values(
        ["Race", "Year", "Driver", "LapNumber"], kind="mergesort"
    ).index
    driver_code = (
        out.loc[order, ["Race", "Year", "Driver"]]
        .groupby(["Race", "Year", "Driver"], sort=False)
        .ngroup()
    )
    driver_code = pd.Series(driver_code, index=order)
    lap_s = lapnum.loc[order]
    stint_s = stint.loc[order]
    tyre_s = tyre_life.loc[order]
    prev_stint_s = stint_s.groupby(driver_code).shift(1)
    prev_tyre_s = tyre_s.groupby(driver_code).shift(1)
    stop_s = (
        (stint_s > prev_stint_s.fillna(stint_s))
        | (tyre_s < (prev_tyre_s - 1).fillna(tyre_s))
    ).astype("float32")

    prev_pit = stop_s.groupby(driver_code).shift(1).fillna(0.0).astype("float32")
    prev2_pit = stop_s.groupby(driver_code).shift(2).fillna(0.0).astype("float32")
    pit_count_prev = (stop_s.groupby(driver_code).cumsum() - stop_s).astype("float32")
    last_pit_lap = lap_s.where(stop_s > 0.5)
    last_pit_prev = (
        last_pit_lap.groupby(driver_code).ffill().groupby(driver_code).shift(1)
    )
    laps_since_prev_pit = (
        (lap_s - last_pit_prev.fillna(0.0)).clip(lower=0.0).astype("float32")
    )

    out["RYLap_LapTimeDelta_Median"] = lap_median.astype("float32")
    out["RYLap_LapTimeDelta_TrimMean"] = lap_trimmed_mean.astype("float32")
    out["RYLap_LapTimeDelta_MAD"] = lap_mad.astype("float32")
    out["RYLap_LapTimeDelta_Std"] = lap_std.astype("float32")
    out["RYLap_FieldCount"] = lap_count
    out["RYLap_PositiveShockFrac"] = lap_pos_shock_frac.astype("float32")
    out["RYLap_MedianShockZ"] = shock_z.astype("float32")
    out["RYLap_MedianChangeZ"] = abrupt_z.astype("float32")
    out["RYLap_PositiveShockFracChange"] = frac_change.astype("float32")
    out["NeutralizationProb"] = neutral_prob
    out["NeutralizationFlag"] = neutral_flag
    out["LapsRemaining_Est"] = laps_remaining

    out.loc[order, "PrevPitStop"] = prev_pit.to_numpy()
    out.loc[order, "Prev2PitStop"] = prev2_pit.to_numpy()
    out.loc[order, "PitStopsPrev"] = pit_count_prev.to_numpy()
    out.loc[order, "LapsSincePrevPitObserved"] = laps_since_prev_pit.to_numpy()

    out["NeutralizationProb_x_LapsRemaining_Est"] = (
        neutral_prob * laps_remaining
    ).astype("float32")
    out["NeutralizationProb_x_TyreLife"] = (neutral_prob * tyre_life).astype("float32")
    out["NeutralizationProb_x_Stint"] = (neutral_prob * stint).astype("float32")
    out["NeutralizationProb_x_PrevPitStop"] = (
        neutral_prob * out["PrevPitStop"].astype("float32")
    ).astype("float32")
    out["NeutralizationProb_x_PitStopsPrev"] = (
        neutral_prob * out["PitStopsPrev"].astype("float32")
    ).astype("float32")
    out["NeutralizationProb_x_LapsSincePrevPit"] = (
        neutral_prob * out["LapsSincePrevPitObserved"].astype("float32")
    ).astype("float32")

    compound = out["Compound"].astype(str).str.upper()
    for name in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        mask = compound.eq(name).astype("float32")
        out[f"NeutralizationProb_x_Compound_{name}"] = (neutral_prob * mask).astype(
            "float32"
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000209"],
        "research_usage_note": "Verified assigned hypothesis 000209.",
    }, sort_keys=True))
