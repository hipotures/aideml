...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Risk"] = (
        out["Compound"].map(compound_risk).fillna(1.0).astype("float32")
    )

    race_len = (out["LapNumber"] / out["RaceProgress"].clip(lower=0.01)).replace(
        [np.inf, -np.inf], np.nan
    )
    race_len = (
        race_len.groupby(out["Race"])
        .transform("median")
        .fillna(race_len.median())
        .clip(20, 90)
    )
    out["EstRaceLaps"] = race_len.astype("float32")
    out["LapsRemaining"] = (race_len - out["LapNumber"]).clip(lower=0).astype("float32")
    ...
    out["TyreLife_Share"] = (tyre / race_len.clip(lower=1)).astype("float32")
    out["Stint_TyreLife"] = (stint * tyre).astype("float32")
    out["Deg_per_TyreLap"] = (deg / tyre.clip(lower=1)).astype("float32")
    out["LapDelta_abs"] = lap_delta.abs().astype("float32")
    out["LapTime_log"] = np.log1p(lap_time.clip(lower=0)).astype("float32")
    out["PitWindow_Base"] = (
        out["Compound_Risk"]
        * np.log1p(tyre.clip(lower=0))
        * (0.35 + progress)
        * (1.0 + 0.08 * stint)
    ).astype("float32")
    ...
    for h in range(1, 9):
        future_tyre = tyre + h
        future_progress = ((out["LapNumber"] + h) / race_len.clip(lower=1)).clip(
            0, 1.05
        )
        remaining_after_h = (race_len - out["LapNumber"] - h).clip(lower=0)
        out[f"H{h}_TyreLife"] = future_tyre.astype("float32")
        out[f"H{h}_RaceProgress"] = future_progress.astype("float32")
        out[f"H{h}_LapsRemaining"] = remaining_after_h.astype("float32")
        out[f"H{h}_PitWindowScore"] = (
            out["Compound_Risk"]
            * np.log1p(future_tyre.clip(lower=0))
            * (0.40 + future_progress)
            * (1.0 + 0.08 * stint)
            * np.exp(-remaining_after_h / 30.0)
        ).astype("float32")

    out["Hazard_1to8_mean"] = (
        out[[f"H{h}_PitWindowScore" for h in range(1, 9)]]
        .mean(axis=1)
        .astype("float32")
    )
    out["Hazard_1to8_max"] = (
        out[[f"H{h}_PitWindowScore" for h in range(1, 9)]].max(axis=1).astype("float32")
    )
    out["Hazard_h1_minus_mean"] = (
        out["H1_PitWindowScore"] - out["Hazard_1to8_mean"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000566"],
        "research_usage_note": "Verified assigned hypothesis 000566.",
    }, sort_keys=True))
