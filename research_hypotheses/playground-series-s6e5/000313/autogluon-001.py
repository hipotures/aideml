...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["laps_remaining"] = 1.0 - out["RaceProgress"]
    out["stint_lap_idx"] = g.cumcount() + 1

    degr_inc = g["Cumulative_Degradation"].diff().fillna(0.0)
    pos_worse = g["Position"].diff().clip(lower=0).fillna(0.0)

    out["degr_inc"] = degr_inc
    out["pos_worse"] = pos_worse
    ...
    degr_med5 = (
        degr_inc.groupby([out[c] for c in group_cols], sort=False)
        .rolling(window=5, min_periods=2)
        .median()
        .reset_index(level=list(range(len(group_cols))), drop=True)
    )
    pos_med5 = (
        pos_worse.groupby([out[c] for c in group_cols], sort=False)
        .rolling(window=5, min_periods=2)
        .median()
        .reset_index(level=list(range(len(group_cols))), drop=True)
    )

    out["lapdelta_resid_med5"] = out["LapTime_Delta"] - lap_med5
    out["degr_inc_resid_med5"] = degr_inc - degr_med5
    out["pos_worse_resid_med5"] = pos_worse - pos_med5

    out["lapdelta_slope_3"] = (out["LapTime_Delta"] - g["LapTime_Delta"].shift(2)) / 2.0
    out["degr_slope_3"] = (
        degr_inc - g["Cumulative_Degradation"].diff(3).fillna(0.0) / 3.0
    )
    out["pos_worse_slope_3"] = (
        pos_worse - g["Position"].diff(2).clip(lower=0).fillna(0.0) / 2.0
    )

    out["lapdelta_accel"] = g["lapdelta_slope_3"].diff().fillna(0.0)
    out["degr_accel"] = g["degr_slope_3"].diff().fillna(0.0)
    out["pos_worse_accel"] = g["pos_worse_slope_3"].diff().fillna(0.0)

    lap_mad5 = (
        out["lapdelta_resid_med5"]
        .abs()
    ...
    out["lapdelta_robust_z"] = out["lapdelta_resid_med5"] / (lap_mad5 + 1e-6)
    out["degr_inc_robust_z"] = out["degr_inc_resid_med5"] / (degr_mad5 + 1e-6)
    out["pos_worse_robust_z"] = out["pos_worse_resid_med5"] / (pos_mad5 + 1e-6)

    def _cusum_pos(signal, keys, drift=0.25):
        base = np.maximum(signal.fillna(0.0).to_numpy() - drift, 0.0)
        s = np.zeros(len(base), dtype=np.float32)
        prev_key = None
        run = 0.0
        key_vals = list(zip(*[keys[c].to_numpy() for c in keys.columns]))
        for i, k in enumerate(key_vals):
            if k != prev_key:
                run = 0.0
                prev_key = k
            run = max(0.0, run + base[i])
            s[i] = run
        return s

    key_df = out[group_cols]
    out["lapdelta_cusum"] = _cusum_pos(out["lapdelta_robust_z"], key_df, drift=0.35)
    out["degr_cusum"] = _cusum_pos(out["degr_inc_robust_z"], key_df, drift=0.25)
    out["pos_cusum"] = _cusum_pos(out["pos_worse_robust_z"], key_df, drift=0.20)

    raw_shift = (
        (out["lapdelta_robust_z"] > 1.5).astype(int)
        + (out["degr_inc_robust_z"] > 1.5).astype(int)
        + (out["pos_worse_robust_z"] > 1.0).astype(int)
        + (out["lapdelta_accel"] > 0.75).astype(int)
        + (out["degr_accel"] > 0.5).astype(int)
        + (out["lapdelta_cusum"] > 3.0).astype(int)
        + (out["degr_cusum"] > 2.5).astype(int)
    )

    persist = (
        raw_shift.groupby([out[c] for c in group_cols], sort=False)
        .rolling(window=3, min_periods=1)
        .sum()
        .reset_index(level=list(range(len(group_cols))), drop=True)
    )
    out["regime_shift_flag"] = ((raw_shift >= 2) & (persist >= 3)).astype(np.int8)

    shift_cum = g["regime_shift_flag"].cumsum()
    laps_at_shift = out["LapNumber"].where(out["regime_shift_flag"] == 1, np.nan)
    last_shift_lap = (
        laps_at_shift.groupby([out[c] for c in group_cols], sort=False)
        .ffill()
        .fillna(0.0)
    )
    out["laps_since_shift"] = (out["LapNumber"] - last_shift_lap).clip(lower=0.0)
    out["shift_count_in_stint"] = shift_cum

    compound_upper = out["Compound"].astype(str).str.upper()
    out["compound_soft_shift"] = (compound_upper == "SOFT").astype(np.int8) * out[
        "laps_since_shift"
    ]
    out["compound_medium_shift"] = (compound_upper == "MEDIUM").astype(np.int8) * out[
        "laps_since_shift"
    ]
    out["compound_hard_shift"] = (compound_upper == "HARD").astype(np.int8) * out[
        "laps_since_shift"
    ]
    out["compound_wet_shift"] = (compound_upper.isin(["INTERMEDIATE", "WET"])).astype(
        np.int8
    ) * out["laps_since_shift"]

    out["shift_x_laps_remaining"] = out["laps_since_shift"] * out["laps_remaining"]
    out["regime_flag_x_laps_remaining"] = (
        out["regime_shift_flag"] * out["laps_remaining"]
    )
    out["regime_flag_x_tyrelife"] = out["regime_shift_flag"] * out["TyreLife"]
    out["cliff_pressure"] = (
        0.5 * out["lapdelta_cusum"] + 0.35 * out["degr_cusum"] + 0.15 * out["pos_cusum"]
    ) * (1.0 + out["laps_remaining"])

    out = out.sort_values("_orig_idx", kind="mergesort").drop(columns=["_orig_idx"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000313"],
        "research_usage_note": "Verified assigned hypothesis 000313.",
    }, sort_keys=True))
