...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_pos_band"] = pd.cut(
        out["Position"],
        bins=[0, 5, 10, 15, 20],
        labels=["p01_05", "p06_10", "p11_15", "p16_20"],
        include_lowest=True,
    ).astype("object")
    out["_age_bucket"] = pd.cut(
        out["TyreLife"],
        bins=[0, 8, 16, 28, 45, 100],
        labels=["age_00_08", "age_09_16", "age_17_28", "age_29_45", "age_46p"],
        include_lowest=True,
    ).astype("object")
    out["_stop_proxy"] = (
    ...
    def add_decayed_count(name, group_cols):
        agg_cols = keys + [lap_col] + group_cols
        lap_events = (
            out.groupby(agg_cols, observed=True, sort=False)["_stop_proxy"]
            .sum()
            .reset_index(name="_pit_count")
            .sort_values(agg_cols)
        )
        by_cols = keys + group_cols
        s1 = (
            lap_events.groupby(by_cols, observed=True, sort=False)["_pit_count"]
            .shift(1)
            .fillna(0.0)
        )
        s2 = (
        ...
        lap_events[name] = s1 + 0.55 * s2 + 0.30 * s3
        out[name] = (
            out.merge(lap_events[agg_cols + [name]], on=agg_cols, how="left")[name]
            .fillna(0.0)
            .to_numpy()
        )

    add_decayed_count("pit_wave_decay_race", [])
    add_decayed_count("pit_wave_decay_posband", ["_pos_band"])
    add_decayed_count("pit_wave_decay_compound", ["Compound"])
    add_decayed_count("pit_wave_decay_age", ["_age_bucket"])

    pace = (
        out.groupby(keys + [lap_col], observed=True, sort=False)
        .agg(
            field_laptime_std=("LapTime (s)", "std"),
            field_delta_std=("LapTime_Delta", "std"),
            field_pit_count=("_stop_proxy", "sum"),
        )
        .reset_index()
        .sort_values(keys + [lap_col])
    )
    pace["field_laptime_std"] = pace["field_laptime_std"].fillna(0.0)
    pace["field_delta_std"] = pace["field_delta_std"].fillna(0.0)
    pace["pace_std_jump"] = (
        pace["field_laptime_std"]
        - pace.groupby(keys, observed=True, sort=False)["field_laptime_std"].shift(1)
    ).fillna(0.0)
    pace["delta_std_jump"] = (
        pace["field_delta_std"]
        - pace.groupby(keys, observed=True, sort=False)["field_delta_std"].shift(1)
    ).fillna(0.0)
    pace["prev_lap_field_pits"] = (
        pace.groupby(keys, observed=True, sort=False)["field_pit_count"]
        .shift(1)
        .fillna(0.0)
    )
    ...
    out["pit_wave_decay_near_context"] = (
        0.45 * out["pit_wave_decay_race"]
        + 0.25 * out["pit_wave_decay_posband"]
        + 0.20 * out["pit_wave_decay_compound"]
        + 0.10 * out["pit_wave_decay_age"]
    )
    out["pit_wave_x_pace_jump"] = out["pit_wave_decay_near_context"] * out[
        "pace_std_jump"
    ].clip(lower=0)
    out["pit_wave_x_delta_jump"] = out["pit_wave_decay_near_context"] * out[
        "delta_std_jump"
    ].clip(lower=0)
    out["prev_field_pits_x_tyre_life"] = out["prev_lap_field_pits"] * out["TyreLife"]

    return out.drop(columns=["_pos_band", "_age_bucket", "_stop_proxy"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000220"],
        "research_usage_note": "Verified assigned hypothesis 000220.",
    }, sort_keys=True))
