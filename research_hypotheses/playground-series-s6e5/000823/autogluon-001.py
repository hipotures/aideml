...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    n_total = len(out)
    n_test = 188165 if n_total > 188165 else 0
    ...
    progress = out["RaceProgress"].astype(float).clip(0.0, 1.0)
    tyre = out["TyreLife"].astype(float)
    stint = out["Stint"].astype(float)
    pos = out["Position"].astype(float)
    pit = out["PitStop"].astype(float).clip(0, 1)

    out["laps_remaining_est"] = (
        (lap / progress.replace(0, np.nan) - lap)
    ...
    out["tyre_distress"] = (
        (
            tyre
            / (
                out.groupby(["Race", "Compound"], observed=True)["TyreLife"]
                .transform("median")
                .replace(0, np.nan)
            )
        )
    ...
    out["degradation_distress"] = np.log1p(
        np.abs(out["Cumulative_Degradation"].astype(float))
    )
    out["lap_delta_abs"] = np.log1p(np.abs(out["LapTime_Delta"].astype(float)))
    out["pit_now_pressure"] = (
        0.45 * out["tyre_distress"]
        + 0.25 * out["degradation_distress"]
        + 0.20 * out["lap_delta_abs"]
        + 0.10 * stint
    )

    train = out.loc[train_mask, ["Race", "LapNumber", "RaceProgress", "PitStop"]].copy()
    train["next_lap"] = train["LapNumber"].astype(float) - 1
    lap_prior = train.groupby(["Race", "next_lap"], observed=True)["PitStop"].mean()
    out["next_lap_pit_burst_prior"] = (
        pd.MultiIndex.from_arrays([out["Race"], lap]).map(lap_prior).astype(float)
    )

    train["progress_bin"] = np.floor(
        train["RaceProgress"].astype(float).clip(0, 0.999) * 20
    ).astype(int)
    out_progress_bin = np.floor(progress.clip(0, 0.999) * 20).astype(int)
    progress_prior = train.groupby(["Race", "progress_bin"], observed=True)[
        "PitStop"
    ].mean()
    out["race_progress_pit_burst_prior"] = (
        pd.MultiIndex.from_arrays([out["Race"], out_progress_bin])
        .map(progress_prior)
        .astype(float)
    )

    race_prior = train.groupby("Race", observed=True)["PitStop"].mean()
    global_prior = float(train["PitStop"].mean()) if len(train) else float(pit.mean())
    out["race_pit_burst_prior"] = (
        out["Race"].map(race_prior).astype(float).fillna(global_prior)
    )
    out["next_lap_pit_burst_prior"] = (
        out["next_lap_pit_burst_prior"]
        .fillna(out["race_progress_pit_burst_prior"])
        .fillna(out["race_pit_burst_prior"])
        .fillna(global_prior)
    )
    out["race_progress_pit_burst_prior"] = (
        out["race_progress_pit_burst_prior"]
    ...
    backmarker_penalty = ((pos - 1) / 19).clip(0, 1)
    traffic_penalty = (1 - backmarker_penalty).clip(0, 1)
    wait_discount = out["next_lap_pit_burst_prior"].clip(0, 1) * (
        0.65 + 0.35 * traffic_penalty
    )

    out["pit_now_loss_proxy"] = out["pit_now_pressure"] * (1.0 + 0.30 * traffic_penalty)
    out["expected_pit_loss_if_wait_1"] = out["pit_now_loss_proxy"] * (
        1.0 - wait_discount
    ) + 0.08 * out["tyre_distress"] * out["laps_remaining_est"].clip(0, 5)
    out["wait_one_lap_option_value"] = (
        out["pit_now_loss_proxy"] - out["expected_pit_loss_if_wait_1"]
    )
    out["wait_value_x_position"] = out["wait_one_lap_option_value"] * pos
    out["wait_value_x_laps_remaining"] = (
        out["wait_one_lap_option_value"] * out["laps_remaining_est"]
    )
    out["wait_value_x_tyre_distress"] = (
        out["wait_one_lap_option_value"] * out["tyre_distress"]
    )
    out["wait_value_x_degradation"] = (
        out["wait_one_lap_option_value"] * out["degradation_distress"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000823"],
        "research_usage_note": "Verified assigned hypothesis 000823.",
    }, sort_keys=True))
