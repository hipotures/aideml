...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    group_cols = ["Race", "LapNumber"]
    eps = 1e-6

    race_q75 = out.groupby("Race")["LapTime_Delta"].transform(
        lambda s: s.quantile(0.75)
    )
    race_q90 = out.groupby("Race")["LapTime_Delta"].transform(
        lambda s: s.quantile(0.90)
    )
    slow_flag = (out["LapTime_Delta"] > race_q75).astype("float32")
    very_slow_flag = (out["LapTime_Delta"] > race_q90).astype("float32")

    g = out.groupby(group_cols, sort=False)
    n = g["LapTime_Delta"].transform("count").astype("float32")
    denom = (n - 1.0).clip(lower=1.0)

    slow_sum = (
        slow_flag.groupby([out["Race"], out["LapNumber"]])
        .transform("sum")
        .astype("float32")
    )
    very_slow_sum = (
        very_slow_flag.groupby([out["Race"], out["LapNumber"]])
        .transform("sum")
        .astype("float32")
    )
    out["rl_slow_frac_loo"] = ((slow_sum - slow_flag) / denom).astype("float32")
    out["rl_very_slow_count_loo"] = (
        (very_slow_sum - very_slow_flag).clip(lower=0).astype("float32")
    )

    med = g["LapTime_Delta"].transform("median").astype("float32")
    q75 = g["LapTime_Delta"].transform(lambda s: s.quantile(0.75)).astype("float32")
    q25 = g["LapTime_Delta"].transform(lambda s: s.quantile(0.25)).astype("float32")
    out["rl_laptime_delta_iqr"] = (q75 - q25).astype("float32")
    abs_dev = (out["LapTime_Delta"] - med).abs()
    out["rl_laptime_delta_mad"] = (
        abs_dev.groupby([out["Race"], out["LapNumber"]])
        .transform("median")
        .astype("float32")
    )
    ...
    pos_abs_sum = (
        out["Position_Change"]
        .abs()
        .groupby([out["Race"], out["LapNumber"]])
        .transform("sum")
        .astype("float32")
    )
    pos_sq_sum = (
    ...
    own_abs = out["Position_Change"].abs().astype("float32")
    loo_abs_sum = (pos_abs_sum - own_abs).clip(lower=0)
    out["rl_position_change_abs_mean_loo"] = (loo_abs_sum / denom).astype("float32")
    out["rl_field_compression_loo"] = (
        1.0 / (1.0 + out["rl_position_change_abs_mean_loo"])
    ).astype("float32")
    out["rl_position_change_std"] = np.sqrt(
        (pos_sq_sum / n) - ((pos_abs_sum / n) ** 2).clip(lower=0)
    ).astype("float32")

    abs_pos = out["Position_Change"].abs().clip(upper=5).astype("int16")
    for k in range(6):
        pk = (
            abs_pos.eq(k)
            .astype("float32")
        ...
        ent_part = -pk * np.log(pk)
        out["rl_poschg_entropy"] = (
            ent_part if k == 0 else out["rl_poschg_entropy"] + ent_part
        )
    out["rl_poschg_entropy"] = out["rl_poschg_entropy"].astype("float32")

    out["cheap_stop_proxy"] = (
        out["rl_slow_frac_loo"] * 1.5
        + out["rl_very_slow_count_loo"] / n.clip(lower=1)
        + out["rl_laptime_delta_iqr"] / (out["LapTime_Delta"].abs() + 10.0)
        + out["rl_field_compression_loo"]
        + out["rl_poschg_entropy"] / np.log(6.0)
    ).astype("float32")

    for base in [
        "cheap_stop_proxy",
        "rl_slow_frac_loo",
        "rl_field_compression_loo",
        "rl_poschg_entropy",
    ]:
        out[f"{base}_x_raceprogress"] = (out[base] * out["RaceProgress"]).astype(
            "float32"
        )
        out[f"{base}_x_tyrelife"] = (out[base] * out["TyreLife"]).astype("float32")
        out[f"{base}_x_stint"] = (out[base] * out["Stint"]).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000649"],
        "research_usage_note": "Verified assigned hypothesis 000649.",
    }, sort_keys=True))
