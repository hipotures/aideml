...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    baseline = out.groupby(["Race_Year", "LapNumber"], observed=True)[
        "LapTime (s)"
    ].transform("median")
    fallback = out.groupby("LapNumber", observed=True)["LapTime (s)"].transform(
        "median"
    )
    out["_pace_resid"] = out["LapTime (s)"] - baseline.fillna(fallback)

    sort_cols = ["Race_Year", "Driver", "Stint", "LapNumber"]
    s = out.sort_values(sort_cols, kind="mergesort").copy()
    g = s.groupby(["Race_Year", "Driver", "Stint"], observed=True, sort=False)

    s["latent_pace_ema"] = g["_pace_resid"].transform(
        lambda x: x.shift(1).ewm(span=4, adjust=False, min_periods=1).mean()
    )
    s["latent_deg_ema"] = g["Cumulative_Degradation"].transform(
        lambda x: x.shift(1).ewm(span=4, adjust=False, min_periods=1).mean()
    )
    s["latent_tyre_life_ema"] = g["TyreLife"].transform(
        lambda x: x.shift(1).ewm(span=4, adjust=False, min_periods=1).mean()
    )

    s["_deg_step"] = g["Cumulative_Degradation"].diff()
    s["latent_deg_slope"] = g["_deg_step"].transform(
        lambda x: x.shift(1).ewm(span=4, adjust=False, min_periods=1).mean()
    )
    s["_deg_accel_step"] = g["_deg_step"].diff()
    s["latent_deg_accel"] = g["_deg_accel_step"].transform(
        lambda x: x.shift(1).ewm(span=4, adjust=False, min_periods=1).mean()
    )

    s["latent_pace_vol3"] = g["_pace_resid"].transform(
        lambda x: x.shift(1).rolling(3, min_periods=2).std()
    )
    s["latent_pace_vol5"] = g["_pace_resid"].transform(
        lambda x: x.shift(1).rolling(5, min_periods=2).std()
    )
    s["latent_delta_vol3"] = g["LapTime_Delta"].transform(
        lambda x: x.shift(1).rolling(3, min_periods=2).std()
    )

    s["_past_mean5"] = g["_pace_resid"].transform(
        lambda x: x.shift(1).rolling(5, min_periods=3).mean()
    )
    s["_past_std5"] = g["_pace_resid"].transform(
        lambda x: x.shift(1).rolling(5, min_periods=3).std()
    )
    s["_prev_resid"] = g["_pace_resid"].shift(1)
    s["latent_slow_shock_prev"] = (
        s["_prev_resid"] > s["_past_mean5"] + 2.0 * s["_past_std5"]
    ).astype("int8")
    s["latent_slow_shock_recent3"] = g["latent_slow_shock_prev"].transform(
        lambda x: x.shift(1).rolling(3, min_periods=1).max()
    )

    s["latent_state_laps_seen"] = g.cumcount().astype("int16")
    s["latent_life_x_slope"] = s["latent_tyre_life_ema"] * s["latent_deg_slope"]
    s["latent_pace_x_vol"] = s["latent_pace_ema"] * s["latent_pace_vol3"]

    keep = [
        "latent_pace_ema",
        "latent_deg_ema",
        "latent_tyre_life_ema",
        "latent_deg_slope",
        "latent_deg_accel",
        "latent_pace_vol3",
        "latent_pace_vol5",
        "latent_delta_vol3",
        "latent_slow_shock_prev",
        "latent_slow_shock_recent3",
        "latent_state_laps_seen",
        "latent_life_x_slope",
        "latent_pace_x_vol",
    ]
    ...
    out = out.drop(columns=["_pace_resid"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000483.",
    }, sort_keys=True))
