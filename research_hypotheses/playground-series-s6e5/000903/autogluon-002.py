...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wet_mask = comp.isin(["WET", "INTERMEDIATE"])
    ...
    work = out[order_cols + ["PitStop", "RaceProgress", "Stint", "TyreLife"]].copy()
    work["_row_order_000903"] = np.arange(len(work))
    work = work.sort_values(order_cols + ["_row_order_000903"], kind="mergesort")
    work["_pit"] = (
        pd.to_numeric(work["PitStop"], errors="coerce").fillna(0).clip(0, 1)
    )
    g = work.groupby(["Year", "Race", "Driver"], sort=False)
    work["_stops_so_far"] = g["_pit"].cumsum()
    work["_prior_stops"] = (work["_stops_so_far"] - work["_pit"]).clip(lower=0)

    race_lower = work["Race"].astype(str).str.lower()
    is_monaco_2025 = (
        (pd.to_numeric(work["Year"], errors="coerce").fillna(0).astype(int) == 2025)
        & race_lower.str.contains("monaco", regex=False)
    )
    dry_mask = comp.iloc[work["_row_order_000903"].to_numpy()].isin(
        ["SOFT", "MEDIUM", "HARD"]
    )
    min_stops = np.where(dry_mask.to_numpy(), 1.0, 0.0)
    min_stops = np.where(is_monaco_2025.to_numpy(), 2.0, min_stops)
    progress_ord = pd.to_numeric(work["RaceProgress"], errors="coerce").fillna(0.5)
    stint_ord = pd.to_numeric(work["Stint"], errors="coerce").fillna(1.0)
    tyre_ord = pd.to_numeric(work["TyreLife"], errors="coerce").fillna(0.0)

    observed_stops = np.maximum(
        work["_stops_so_far"].to_numpy(dtype=float),
        np.maximum(0.0, stint_ord.to_numpy(dtype=float) - 1.0),
    )
    rule_debt = np.maximum(0.0, min_stops - observed_stops)
    tyre_pressure = (tyre_ord.to_numpy(dtype=float) / 28.0).clip(0.0, 2.0)
    late_pressure = progress_ord.to_numpy(dtype=float).clip(0.0, 1.0)
    expected_remaining = (
        rule_debt
        + (1.0 - late_pressure) * 0.65
        + np.maximum(0.0, tyre_pressure - 0.75) * 0.55
        + wet_mask.iloc[work["_row_order_000903"].to_numpy()].to_numpy(dtype=float)
        * 0.15
    ).clip(0.0, 3.0)

    scores = -1.35 * np.abs(expected_remaining[:, None] - np.arange(4)[None, :])
    scores -= np.array([0.0, 0.10, 0.35, 0.70], dtype="float32")
    scores = scores - scores.max(axis=1, keepdims=True)
    aux_prob_ord = np.exp(scores).astype("float32")
    aux_prob_ord = aux_prob_ord / aux_prob_ord.sum(axis=1, keepdims=True)
    aux_prob = np.empty_like(aux_prob_ord)
    aux_prob[work["_row_order_000903"].to_numpy()] = aux_prob_ord

    rule_debt_orig = np.empty(len(out), dtype="float32")
    expected_orig = np.empty(len(out), dtype="float32")
    order_idx = work["_row_order_000903"].to_numpy()
    rule_debt_orig[order_idx] = rule_debt.astype("float32")
    expected_orig[order_idx] = expected_remaining.astype("float32")
    out["latent_remaining_stops_rule_debt"] = rule_debt_orig
    out["latent_remaining_stops_expected"] = expected_orig
