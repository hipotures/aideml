...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["LapNumber"] = pd.to_numeric(out["LapNumber"], errors="coerce").astype(float)
    out["TyreLife"] = pd.to_numeric(out["TyreLife"], errors="coerce").astype(float)
    out["RaceProgress"] = (
        pd.to_numeric(out["RaceProgress"], errors="coerce").fillna(0).clip(0, 1)
    )
    out["Stint"] = pd.to_numeric(out["Stint"], errors="coerce").fillna(0).astype(float)
    out["Year"] = pd.to_numeric(out["Year"], errors="coerce").fillna(-1).astype(int)

    comp = out["Compound"].astype("string").fillna("UNKNOWN").str.upper()
    out["tpl_race_key"] = out["Race"].astype("string").fillna("UNKNOWN").astype(str)
    out["tpl_compound_key"] = comp.astype(str)
    out["tpl_wet_dry"] = np.where(comp.isin(["INTERMEDIATE", "WET"]), "wet", "dry")

    pos = pd.to_numeric(out["Position"], errors="coerce")
    out["tpl_position_bucket"] = (
        pd.cut(
            pos,
            bins=[-np.inf, 5, 10, 15, np.inf],
            labels=["top5", "p6_10", "p11_15", "p16p"],
        )
        .astype("string")
        .fillna("unknown")
        .astype(str)
    )
    out["tpl_progress_bin"] = np.minimum(
        (out["RaceProgress"] * 12).astype(int), 11
    ).astype("int16")

    all_mask = np.ones(len(out), dtype=bool)
    first_mask = out["Stint"].to_numpy() == 1
    if not first_mask.any():
        first_mask = all_mask

    def q(mask, col, quant, default):
        vals = out.loc[mask, col].dropna()
        return float(vals.quantile(quant)) if len(vals) else float(default)

    lap_default = float(out["LapNumber"].median())
    life_default = float(out["TyreLife"].median())
    g_win_start = q(first_mask, "LapNumber", 0.65, lap_default)
    g_first = q(first_mask, "LapNumber", 0.80, lap_default)
    g_win_end = q(first_mask, "LapNumber", 0.90, lap_default)
    g_life = q(all_mask, "TyreLife", 0.80, life_default)
    g_age = q(all_mask, "TyreLife", 0.50, life_default)

    def add_shrunk_quantile(mask, keys, value, quant, name, fallback, k):
        nonlocal out
        raw_col = name + "_raw_tmp"
        n_col = name + "_n_tmp"
        base = out.loc[mask, keys + [value]].dropna(subset=[value])
        if len(base):
            grp = base.groupby(keys, dropna=False, observed=True)[value]
            stats = grp.quantile(quant).rename(raw_col).reset_index()
            counts = grp.size().rename(n_col).reset_index()
            stats = stats.merge(counts, on=keys, how="left")
            out = out.merge(stats, on=keys, how="left", sort=False)
            out.index = original_index
            n = out[n_col].fillna(0).astype(float)
            raw = out[raw_col].astype(float)
            out.drop(columns=[raw_col, n_col], inplace=True)
        else:
            n = pd.Series(0.0, index=out.index)
            raw = pd.Series(np.nan, index=out.index)

        fb = (
            out[fallback].astype(float)
            if isinstance(fallback, str)
            else pd.Series(float(fallback), index=out.index)
        )
        raw = raw.fillna(fb)
        w = n / (n + float(k))
        out[name] = (w * raw + (1.0 - w) * fb).astype("float32")
        out[name + "_n"] = n.astype("float32")

    rc = ["tpl_race_key", "tpl_compound_key", "tpl_wet_dry"]
    rcy = rc + ["Year"]
    rcp = rc + ["tpl_position_bucket"]
    rcs = rc + ["Stint"]

    add_shrunk_quantile(
        first_mask, rc, "LapNumber", 0.65, "tpl_window_start_lap", g_win_start, 30
    )
    add_shrunk_quantile(
        first_mask, rc, "LapNumber", 0.80, "tpl_expected_first_stop_lap", g_first, 30
    )
    add_shrunk_quantile(
        first_mask, rc, "LapNumber", 0.90, "tpl_window_end_lap", g_win_end, 30
    )
    add_shrunk_quantile(
        first_mask,
        rcy,
        "LapNumber",
        0.80,
        "tpl_year_expected_first_stop_lap",
        "tpl_expected_first_stop_lap",
        18,
    )

    add_shrunk_quantile(
        all_mask, rc, "TyreLife", 0.80, "tpl_expected_stint_life", g_life, 40
    )
    add_shrunk_quantile(
        all_mask,
        rcp,
        "TyreLife",
        0.80,
        "tpl_pos_expected_stint_life",
        "tpl_expected_stint_life",
        25,
    )
    add_shrunk_quantile(
        all_mask,
    ...
    add_shrunk_quantile(
        ...
        "LapNumber",
        0.70,
        "tpl_stint_window_start_lap",
        "tpl_window_start_lap",
        25,
    )
    add_shrunk_quantile(
        all_mask,
        ...
        "LapNumber",
        0.90,
        "tpl_stint_window_end_lap",
        "tpl_window_end_lap",
        25,
    )

    lap = out["LapNumber"].astype(float)
    life = out["TyreLife"].astype(float)
    progress = out["RaceProgress"].astype(float)
    eps = 1e-3

    first_width = (out["tpl_window_end_lap"] - out["tpl_window_start_lap"]).clip(
        lower=1.0
    )
    stint_width = (
        out["tpl_stint_window_end_lap"] - out["tpl_stint_window_start_lap"]
    ).clip(lower=1.0)

    out["tpl_first_stop_lap_resid"] = (lap - out["tpl_expected_first_stop_lap"]).astype(
        "float32"
    )
    out["tpl_year_first_stop_lap_resid"] = (
        lap - out["tpl_year_expected_first_stop_lap"]
    ).astype("float32")
    out["tpl_dist_to_window_start"] = (lap - out["tpl_window_start_lap"]).astype(
        "float32"
    )
    out["tpl_dist_to_window_end"] = (out["tpl_window_end_lap"] - lap).astype("float32")
    out["tpl_first_window_width"] = first_width.astype("float32")
    out["tpl_first_window_pressure"] = (
        ((lap - out["tpl_window_start_lap"]) / (first_width + eps))
        .clip(-5, 5)
        .astype("float32")
    )
    out["tpl_in_first_window"] = (
        (lap >= out["tpl_window_start_lap"]) & (lap <= out["tpl_window_end_lap"])
    ).astype("int8")

    out["tpl_life_resid"] = (life - out["tpl_expected_stint_life"]).astype("float32")
    out["tpl_pos_life_resid"] = (life - out["tpl_pos_expected_stint_life"]).astype(
        "float32"
    )
    out["tpl_year_life_resid"] = (life - out["tpl_year_expected_stint_life"]).astype(
        "float32"
    )
    out["tpl_stintno_life_resid"] = (life - out["tpl_stintno_expected_life"]).astype(
        "float32"
    )
    out["tpl_age_progress_resid"] = (life - out["tpl_expected_age_at_progress"]).astype(
        "float32"
    )
    out["tpl_life_ratio"] = (
        (life / (out["tpl_expected_stint_life"] + eps))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )
    out["tpl_age_progress_ratio"] = (
        (life / (out["tpl_expected_age_at_progress"] + eps))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )

    out["tpl_stint_window_start_dist"] = (
        lap - out["tpl_stint_window_start_lap"]
    ).astype("float32")
    out["tpl_stint_window_end_dist"] = (out["tpl_stint_window_end_lap"] - lap).astype(
        "float32"
    )
    out["tpl_stint_window_width"] = stint_width.astype("float32")
    out["tpl_stint_window_pressure"] = (
        ((lap - out["tpl_stint_window_start_lap"]) / (stint_width + eps))
        .clip(-5, 5)
        .astype("float32")
    )
    out["tpl_in_stint_window"] = (
        (lap >= out["tpl_stint_window_start_lap"])
        & (lap <= out["tpl_stint_window_end_lap"])
    ).astype("int8")

    est_total_laps = (
        (lap / progress.clip(lower=0.01))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(lap_default)
        .clip(1, 120)
    )
    out["tpl_first_stop_progress_resid"] = (
        progress - out["tpl_expected_first_stop_lap"] / est_total_laps
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000312"],
        "research_usage_note": "Verified assigned hypothesis 000312.",
    }, sort_keys=True))
