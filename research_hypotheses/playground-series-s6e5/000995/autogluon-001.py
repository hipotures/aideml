...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_priors = {
        "Bahrain Grand Prix": (5, 2, 3, 5, 2),
        "Saudi Arabian Grand Prix": (3, 2, 2, 3, 3),
        "Australian Grand Prix": (3, 2, 3, 3, 3),
        "Emilia Romagna Grand Prix": (3, 3, 4, 4, 2),
        "Miami Grand Prix": (4, 3, 3, 4, 2),
        "Monaco Grand Prix": (2, 4, 5, 3, 4),
        "Spanish Grand Prix": (5, 3, 4, 5, 2),
        "Canadian Grand Prix": (3, 2, 2, 3, 3),
        "Austrian Grand Prix": (4, 2, 2, 4, 2),
        "British Grand Prix": (4, 3, 3, 4, 2),
        "Hungarian Grand Prix": (4, 4, 5, 5, 3),
        "Belgian Grand Prix": (3, 3, 2, 3, 3),
        "Dutch Grand Prix": (4, 3, 4, 4, 2),
        "Italian Grand Prix": (2, 2, 2, 2, 3),
        "Singapore Grand Prix": (4, 4, 5, 5, 3),
        "Japanese Grand Prix": (5, 3, 4, 5, 2),
        "Qatar Grand Prix": (5, 3, 3, 5, 2),
        "United States Grand Prix": (4, 3, 3, 4, 2),
        "Mexico City Grand Prix": (3, 4, 3, 3, 4),
        "Sao Paulo Grand Prix": (4, 3, 3, 4, 3),
        "Las Vegas Grand Prix": (2, 5, 2, 2, 5),
        "Abu Dhabi Grand Prix": (3, 3, 3, 3, 3),
        "Azerbaijan Grand Prix": (2, 3, 2, 2, 4),
        "French Grand Prix": (4, 2, 3, 4, 2),
        "Portuguese Grand Prix": (3, 3, 3, 3, 3),
        "Pre-Season Testing": (3, 3, 3, 3, 3),
    }

    prior_cols = [
        "race_deg_severity",
        "race_warmup_difficulty",
        "race_overtake_difficulty",
        "race_undercut_strength",
        "race_overcut_viability",
    ]

    pri = out["Race"].map(race_priors)
    defaults = pd.Series([(3, 3, 3, 3, 3)] * len(out), index=out.index)
    pri = pri.where(pri.notna(), defaults)
    prior_df = pd.DataFrame(pri.tolist(), columns=prior_cols, index=out.index).astype(
        "int8"
    )
    out = pd.concat([out, prior_df], axis=1)
    ...
    out["compound_deg_pressure"] = (
        out["Compound"].map(compound_pressure).fillna(1.0).astype("float32")
    )
    out["compound_warmup_pressure"] = (
        out["Compound"].map(warmup_compound).fillna(1.0).astype("float32")
    )
    ...
    out["track_tyre_stress"] = (
        tyre_life * out["race_deg_severity"] * out["compound_deg_pressure"]
    )
    out["undercut_window_pressure"] = (
        out["race_undercut_strength"] * out["compound_deg_pressure"] * race_progress
    )
    out["overcut_window_pressure"] = (
        out["race_overcut_viability"]
        * out["compound_warmup_pressure"]
        * (1.0 - race_progress)
    )
    out["warmup_overcut_tradeoff"] = (
        out["race_warmup_difficulty"]
        * out["race_overcut_viability"]
        * out["compound_warmup_pressure"]
    )
    out["traffic_pit_cost_proxy"] = out["race_overtake_difficulty"] * out[
        "Position"
    ].astype("float32")
    out["degradation_delta_pressure"] = (
        out["race_deg_severity"] * out["compound_deg_pressure"] * lap_delta
    )
    out["undercut_delta_signal"] = out["race_undercut_strength"] * lap_delta
    out["late_stint_undercut_pressure"] = out["track_tyre_stress"] * race_progress
    out["overcut_vs_undercut_balance"] = (
        out["race_overcut_viability"] - out["race_undercut_strength"]
    )
    out["compound_track_pit_bias"] = (
        out["race_undercut_strength"] * out["compound_deg_pressure"]
        - out["race_overcut_viability"] * out["compound_warmup_pressure"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000995"],
        "research_usage_note": "Verified assigned hypothesis 000995.",
    }, sort_keys=True))
