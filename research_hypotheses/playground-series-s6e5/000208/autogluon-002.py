...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in cat_cols:
        ...
        freq = out[col].map(out[col].value_counts(normalize=True))
    ...
    out["Compound_Aggressiveness"] = (
        out["Compound"].map(compound_age).fillna(1.0).astype("float32")
    )
    out["Is_Wet_Compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )

    est_total = out["LapNumber"] / out["RaceProgress"].clip(lower=eps)
    out["Est_Total_Laps"] = est_total.clip(1, 120).astype("float32")
    out["Laps_Remaining_Est"] = (
        (out["Est_Total_Laps"] - out["LapNumber"]).clip(lower=0).astype("float32")
    )
    out["TyreLife_RaceFrac"] = (
        out["TyreLife"] / out["Est_Total_Laps"].clip(lower=1)
    ).astype("float32")
    out["TyreLife_Remaining_Ratio"] = (
        out["TyreLife"] / (out["Laps_Remaining_Est"] + 1)
    ).astype("float32")
    out["Degradation_Per_TyreLap"] = (
        out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1)
    ).astype("float32")
    out["Abs_LapTime_Delta"] = out["LapTime_Delta"].abs().astype("float32")
    out["Position_Loss_Flag"] = (out["Position_Change"] > 0).astype("int8")
    out["Position_Gain_Flag"] = (out["Position_Change"] < 0).astype("int8")

    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    s = out.reset_index().sort_values(sort_cols + ["index"], kind="mergesort")
    g = s.groupby(["Year", "Race", "Driver"], sort=False)

    prev_stint = g["Stint"].shift(1)
    prev_tyre = g["TyreLife"].shift(1)
    stop_proxy = (
        (s["Stint"] > prev_stint.fillna(s["Stint"]))
        | (s["TyreLife"] < (prev_tyre - 1).fillna(s["TyreLife"]))
    ).astype("int8")
    stop_group = stop_proxy.groupby([s["Year"], s["Race"], s["Driver"]], sort=False)
    s["Prev_PitStop"] = stop_group.shift(1).fillna(0)
    s["Pit_Count_To_Now"] = stop_group.cumsum()
    last_pit_lap = (
        s["LapNumber"]
        .where(stop_proxy.eq(1))
        .groupby([s["Year"], s["Race"], s["Driver"]], sort=False)
        .ffill()
        .fillna(0)
    )
    s["Laps_Since_Last_Pit"] = (s["LapNumber"] - last_pit_lap).clip(lower=0)
    s["Seq_Lap_Diff"] = g["LapNumber"].diff().fillna(1).clip(lower=0, upper=5)
    s = s.set_index("index").sort_index()

    out["Prev_PitStop"] = s["Prev_PitStop"].astype("int8")
    out["Pit_Count_To_Now"] = s["Pit_Count_To_Now"].astype("int16")
    out["Laps_Since_Last_Pit"] = s["Laps_Since_Last_Pit"].astype("float32")
    out["Seq_Lap_Diff"] = s["Seq_Lap_Diff"].astype("float32")
    out["Survival_Clock"] = np.maximum(
        out["TyreLife"], out["Laps_Since_Last_Pit"]
    ).astype("float32")
    out["Survival_Clock_Remaining_Ratio"] = (
        out["Survival_Clock"] / (out["Laps_Remaining_Est"] + 1)
    ).astype("float32")

    for h in (1, 2, 3, 5, 8):
        out[f"h{h}_TyreLife"] = (out["TyreLife"] + h).astype("float32")
        out[f"h{h}_SurvivalClock"] = (out["Survival_Clock"] + h).astype("float32")
        out[f"h{h}_RaceProgress"] = (
            ((out["LapNumber"] + h) / out["Est_Total_Laps"].clip(lower=1))
            .clip(0, 1.25)
            .astype("float32")
        )
        out[f"h{h}_LapsRemaining"] = (
            (out["Laps_Remaining_Est"] - h).clip(lower=0).astype("float32")
        )
        out[f"h{h}_ClockRemainingRatio"] = (
            out[f"h{h}_SurvivalClock"] / (out[f"h{h}_LapsRemaining"] + 1)
        ).astype("float32")

    out["Driver_Race_freq"] = (
        (out["Driver"] + "_" + out["Race"])
        .map((out["Driver"] + "_" + out["Race"]).value_counts(normalize=True))
        .astype("float32")
    )
    out["Compound_Stint"] = (out["Compound"] + "_S" + out["Stint"].astype(str)).astype(
        "category"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000208.",
    }, sort_keys=True))
