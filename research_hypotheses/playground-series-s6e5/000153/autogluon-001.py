...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Driver_Race_Year"] = (
        out["Driver"].astype(str) + "_" + out["Race_Year"].astype(str)
    )

    compound_order = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    out["Compound_Code"] = out["Compound"].map(compound_order).astype("float32")
    out["Is_Wet_Compound"] = (
    ...
    for col in ["Driver", "Race", "Race_Year", "Driver_Race_Year", "Compound"]:
        freq = out[col].map(out[col].value_counts(normalize=True))
        out[col + "_freq"] = freq.astype("float32")

    g = out.groupby("Driver_Race_Year", sort=False)

    out["Lap_In_Seq"] = g.cumcount().astype("int16")
    out["Prev_PitStop"] = g["PitStop"].shift(1).fillna(0).astype("int8")
    out["Prev_LapTime"] = g["LapTime (s)"].shift(1)
    out["Prev_LapTime_Delta"] = g["LapTime_Delta"].shift(1)
    out["Prev_Position"] = g["Position"].shift(1)
    out["Prev_TyreLife"] = g["TyreLife"].shift(1)

    out["LapTime_Change"] = (
        (out["LapTime (s)"] - out["Prev_LapTime"]).fillna(0).astype("float32")
    )
    out["Position_Change_From_Prev"] = (
    ...
    out["PitStops_So_Far"] = g["PitStop"].cumsum().sub(out["PitStop"]).astype("int16")
    out["Laps_Since_Pit"] = out["TyreLife"].clip(lower=0).astype("float32")
    out["TyreLife_x_Progress"] = (out["TyreLife"] * out["RaceProgress"]).astype(
        "float32"
    )
    out["Stint_x_TyreLife"] = (out["Stint"] * out["TyreLife"]).astype("float32")
    out["Progress_Remaining"] = (1.0 - out["RaceProgress"]).astype("float32")
    ...
    for col in [
        ...
        out[col.replace(" ", "_").replace("(", "").replace(")", "") + "_roll3_mean"] = (
            shifted.groupby(out["Driver_Race_Year"], sort=False)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000153"],
        "research_usage_note": "Verified assigned hypothesis 000153.",
    }, sort_keys=True))
