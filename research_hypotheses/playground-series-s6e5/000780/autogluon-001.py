...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prior_pit = g["PitStop"].shift(1).fillna(0).astype(float)
    prior2_pit = g["PitStop"].shift(2).fillna(0).astype(float)
    prior3_pit = g["PitStop"].shift(3).fillna(0).astype(float)
    cum_pits_prior = g["PitStop"].cumsum().astype(float) - pit

    pit_lap = s["LapNumber"].where(s["PitStop"].eq(1))
    last_pit_lap = pit_lap.groupby([s[k] for k in keys], sort=False).ffill()
    last_pit_lap_prior = last_pit_lap.groupby([s[k] for k in keys], sort=False).shift(1)
    laps_since_pit = (
        (s["LapNumber"] - last_pit_lap_prior).fillna(s["LapNumber"]).clip(lower=0)
    )

    seq_lap_idx = g.cumcount().astype(float)
    lap_gap = g["LapNumber"].diff().fillna(1).clip(lower=1, upper=5)

    s["pit_prev_lap"] = prior_pit
    s["pit_prev2_lap"] = prior2_pit
    s["pit_recent_3"] = prior_pit + prior2_pit + prior3_pit
    s["cum_pits_prior"] = cum_pits_prior
    s["laps_since_pit"] = laps_since_pit.astype(float)
    s["pit_cooldown"] = np.exp(-laps_since_pit.astype(float) / 3.0)
    s["pit_refractory_flag"] = (laps_since_pit <= 2).astype(int)
    s["stint_hazard"] = np.log1p(s["TyreLife"].clip(lower=0)) * (
        1.0 - s["pit_cooldown"]
    )
    s["tyre_life_sq"] = s["TyreLife"] ** 2
    s["tyre_life_x_stint"] = s["TyreLife"] * s["Stint"]
    s["tyre_life_x_progress"] = s["TyreLife"] * s["RaceProgress"]
    s["degradation_per_tyre_lap"] = s["Cumulative_Degradation"] / (
        s["TyreLife"].abs() + 1.0
    )
    s["lap_delta_abs"] = s["LapTime_Delta"].abs()
    s["lap_delta_x_tyre"] = s["LapTime_Delta"] * s["TyreLife"]
    s["position_loss_flag"] = (s["Position_Change"] > 0).astype(int)
    s["position_gain_flag"] = (s["Position_Change"] < 0).astype(int)
    s["seq_lap_idx"] = seq_lap_idx
    s["lap_gap"] = lap_gap

    compound_age_mean = s.groupby(["Compound"], sort=False)["TyreLife"].transform(
        "mean"
    )
    race_progress_mean = s.groupby(["Race"], sort=False)["RaceProgress"].transform(
        "mean"
    )
    s["compound_tyre_age_ratio"] = s["TyreLife"] / (compound_age_mean + 1.0)
    s["race_progress_centered"] = s["RaceProgress"] - race_progress_mean
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000780"],
        "research_usage_note": "Verified assigned hypothesis 000780.",
    }, sort_keys=True))
