...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    domain = np.zeros(n_total, dtype=np.int8)
    domain[n_train:] = 1
    ...
    num_df = out[num_cols].astype("float32").copy()
    for c in num_cols:
        s = num_df[c]
        lo = s.quantile(0.001)
        hi = s.quantile(0.999)
        num_df[c] = s.clip(lo, hi)
    num_df = num_df.fillna(num_df.median())

    cat_df = out[cat_cols].astype(str).fillna("NA")
    X_cat = (
        pd.get_dummies(cat_df, columns=cat_cols, sparse=True, dtype=np.float32)
        .sparse.to_coo()
        .tocsr()
    )
    X_num = sparse.csr_matrix(num_df.to_numpy(dtype=np.float32))
    X = sparse.hstack([X_num, X_cat], format="csr")

    valid_mask = (np.arange(n_total) % 5) == 0
    ...
    clf.fit(X[train_mask], domain[train_mask])

    valid_proba = clf.predict_proba(X[valid_mask])[:, 1]
    domain_auc = roc_auc_score(domain[valid_mask], valid_proba)
    print(f"domain_auc={domain_auc:.6f}")

    p_test = clf.predict_proba(X)[:, 1]
    p_test = np.clip(p_test, 1e-4, 1 - 1e-4)

    prior_ratio = n_train / max(n_test, 1)
    density_ratio = (p_test / (1.0 - p_test)) * prior_ratio
    density_ratio = np.clip(density_ratio, 0.05, 20.0)

    out["domain_test_proba"] = p_test.astype("float32")
    out["domain_logit"] = np.log(p_test / (1.0 - p_test)).astype("float32")
    out["density_ratio_clip"] = density_ratio.astype("float32")
    out["density_ratio_log"] = np.log1p(density_ratio).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000431.",
    }, sort_keys=True))
