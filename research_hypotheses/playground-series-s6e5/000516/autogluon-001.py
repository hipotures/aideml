...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Compound" in X.columns:
        comp = X["Compound"].astype(str).fillna("UNK")
    else:
        comp = pd.Series("UNK", index=X.index)
    ...
    if "TyreLife" in X.columns:
        ...
        for t in [8, 12, 16, 20, 24, 28, 32]:
            X[f"tyrelife_ge_{t}"] = (X["TyreLife"] >= t).astype("int8")

    if "RaceProgress" in X.columns:
        X["raceprogress_bin"] = pd.cut(
            X["RaceProgress"],
            bins=[-np.inf, 0.15, 0.35, 0.55, 0.75, 0.9, np.inf],
            labels=False,
        ).astype("float32")
        for t in [0.25, 0.5, 0.7, 0.85]:
            X[f"raceprogress_ge_{str(t).replace('.', '_')}"] = (
                X["RaceProgress"] >= t
            ).astype("int8")

    if "Stint" in X.columns:
        for t in [2, 3, 4]:
            X[f"stint_ge_{t}"] = (X["Stint"] >= t).astype("int8")

    if "Cumulative_Degradation" in X.columns:
        deg = X["Cumulative_Degradation"]
        X["degradation_bin"] = pd.cut(
            deg,
            bins=[-np.inf, 0, 50, 100, 150, 250, 400, np.inf],
            labels=False,
        ).astype("float32")
        for t in [25, 50, 100, 150, 200, 300]:
            X[f"degradation_ge_{t}"] = (deg >= t).astype("int8")
        X["degradation_abs"] = deg.abs().astype("float32")

    if {"TyreLife", "Cumulative_Degradation"} <= set(X.columns):
        X["deg_per_tyre_lap"] = (
        ...
        for t in [2, 4, 6, 8]:
            X[f"deg_per_tyre_lap_ge_{t}"] = (X["deg_per_tyre_lap"] >= t).astype("int8")

    if {"TyreLife", "RaceProgress"} <= set(X.columns):
        X["pit_window_basic"] = (
            (X["TyreLife"] >= 15) & (X["RaceProgress"] >= 0.45)
        ).astype("int8")
        X["pit_window_late"] = (
            (X["TyreLife"] >= 20) & (X["RaceProgress"] >= 0.70)
        ).astype("int8")
        X["pit_window_very_late"] = (
            (X["TyreLife"] >= 25) & (X["RaceProgress"] >= 0.85)
        ).astype("int8")

    if {"TyreLife", "Stint"} <= set(X.columns):
        X["stint_tyre_pressure"] = ((X["TyreLife"] >= 12) & (X["Stint"] >= 2)).astype(
            "int8"
        )
        X["deep_stint_pressure"] = ((X["TyreLife"] >= 18) & (X["Stint"] >= 3)).astype(
            "int8"
        )

    if {"Cumulative_Degradation", "RaceProgress"} <= set(X.columns):
        X["late_deg_pressure"] = (
            (X["Cumulative_Degradation"] >= 100) & (X["RaceProgress"] >= 0.55)
        ).astype("int8")
        X["extreme_deg_late"] = (
            (X["Cumulative_Degradation"] >= 150) & (X["RaceProgress"] >= 0.75)
        ).astype("int8")

    soft_like = comp.isin(["SOFT", "INTERMEDIATE", "WET"]).astype("int8")
    hard_like = comp.isin(["HARD", "MEDIUM"]).astype("int8")
    X["compound_soft_like"] = soft_like
    X["compound_hard_like"] = hard_like

    if "TyreLife" in X.columns:
        X["soft_tyre_old"] = (soft_like & (X["TyreLife"] >= 12)).astype("int8")
        X["hard_tyre_old"] = (hard_like & (X["TyreLife"] >= 20)).astype("int8")

    if {"RaceProgress", "TyreLife"} <= set(X.columns):
        X["soft_late_window"] = (
            soft_like & (X["TyreLife"] >= 10) & (X["RaceProgress"] >= 0.5)
        ).astype("int8")
        X["hard_late_window"] = (
            hard_like & (X["TyreLife"] >= 18) & (X["RaceProgress"] >= 0.6)
        ).astype("int8")
    ...
    if rule_cols:
        X["rule_score_000516"] = X[rule_cols].sum(axis=1).astype("float32")
        X["rule_score_000516_ge_2"] = (X["rule_score_000516"] >= 2).astype("int8")
        X["rule_score_000516_ge_3"] = (X["rule_score_000516"] >= 3).astype("int8")
        X["rule_score_000516_ge_4"] = (X["rule_score_000516"] >= 4).astype("int8")

    if {"LapTime_Delta", "TyreLife"} <= set(X.columns):
        X["slow_lap_on_old_tyre"] = (
            (X["LapTime_Delta"] >= 1.0) & (X["TyreLife"] >= 12)
        ).astype("int8")

    if {"Position_Change", "RaceProgress"} <= set(X.columns):
        X["lost_track_pos_late"] = (
            (X["Position_Change"] <= -1) & (X["RaceProgress"] >= 0.5)
        ).astype("int8")

    cat_cols = [c for c in ["Compound", "Driver", "Race", "Year"] if c in X.columns]
    for c in cat_cols:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000516.",
    }, sort_keys=True))
