...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap = out.groupby([race_col, lap_col], sort=False)

    out["race_lap_median_time"] = race_lap[time_col].transform("median")
    out["race_lap_lap_time_q25"] = race_lap[time_col].transform(
        lambda s: s.quantile(0.25)
    )
    out["race_lap_lap_time_q75"] = race_lap[time_col].transform(
        lambda s: s.quantile(0.75)
    )
    out["lap_time_iqr"] = out["race_lap_lap_time_q75"] - out["race_lap_lap_time_q25"]

    out["large_position_change"] = (out["Position_Change"].abs() >= 3).astype("int8")
    out["count_large_position_changes"] = race_lap["large_position_change"].transform(
        "sum"
    )

    out["new_stint_or_pitstop_current"] = (
        (out["Stint"].astype(float) > 1) | (out["PitStop"].astype(float) > 0)
    ).astype("int8")
    out["fraction_new_stint_or_pitstop_current_lap"] = race_lap[
        "new_stint_or_pitstop_current"
    ].transform("mean")
    ...
    lap_ctx = (
        out[
            [
                race_col,
                lap_col,
                "race_lap_median_time",
                "lap_time_iqr",
                "count_large_position_changes",
                "fraction_new_stint_or_pitstop_current_lap",
            ]
        ]
        .drop_duplicates([race_col, lap_col])
        .sort_values([race_col, lap_col])
    )

    lap_ctx["race_median_time_baseline"] = lap_ctx.groupby(race_col)[
        "race_lap_median_time"
    ].transform(lambda s: s.shift(1).rolling(5, min_periods=2).median())
    race_fallback = lap_ctx.groupby(race_col)["race_lap_median_time"].transform(
        "median"
    )
    lap_ctx["race_median_time_baseline"] = lap_ctx["race_median_time_baseline"].fillna(
        race_fallback
    )
    lap_ctx["median_time_vs_race_rolling_baseline"] = (
        lap_ctx["race_lap_median_time"] - lap_ctx["race_median_time_baseline"]
    )

    lag_cols = [
        "race_lap_median_time",
        "median_time_vs_race_rolling_baseline",
        "lap_time_iqr",
        "count_large_position_changes",
        "fraction_new_stint_or_pitstop_current_lap",
    ]
    for col in lag_cols:
        lap_ctx[f"{col}_lag1"] = lap_ctx.groupby(race_col)[col].shift(1)

    fill_cols = ["median_time_vs_race_rolling_baseline"] + [
        f"{c}_lag1" for c in lag_cols
    ]
    lap_ctx[fill_cols] = lap_ctx.groupby(race_col)[fill_cols].transform(
        lambda x: x.fillna(x.median())
    )
    lap_ctx[fill_cols] = lap_ctx[fill_cols].fillna(0)
    ...
    out["driver_time_vs_race_lap_median"] = out[time_col] - out["race_lap_median_time"]
    out["race_lap_field_compression"] = out["lap_time_iqr"] / (
        out["race_lap_median_time"].abs() + 1e-6
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000618"],
        "research_usage_note": "Verified assigned hypothesis 000618.",
    }, sort_keys=True))
