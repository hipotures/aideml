...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").fillna("UNKNOWN")
    driver = out["Driver"].astype("string").fillna("UNKNOWN")
    ...
    out["compound_softness"] = (
        compound.map({"WET": 0, "INTERMEDIATE": 1, "HARD": 2, "MEDIUM": 3, "SOFT": 4})
        .fillna(2)
        .astype("int8")
    )
    ...
    out["race_phase"] = pd.cut(
        rp, bins=[-0.001, 0.333, 0.667, 1.001], labels=False
    ).astype("int8")
    out["early_race"] = (rp <= 0.333).astype("int8")
    out["mid_race"] = ((rp > 0.333) & (rp <= 0.667)).astype("int8")
    out["late_race"] = (rp > 0.667).astype("int8")
    out["regime_dry_phase"] = (
        dry.astype(int) * 3 + out["race_phase"].astype(int)
    ).astype("int8")
    out["regime_wet_phase"] = (
        wet.astype(int) * 3 + out["race_phase"].astype(int)
    ).astype("int8")
    ...
    out["tyre_life_progress"] = tyre * rp
    out["tyre_life_stint"] = tyre * stint
    out["stint_progress"] = stint * rp
    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["degradation_per_race_progress"] = degr / (rp + 0.02)
    out["abs_laptime_delta"] = lap_delta.abs()
    out["laptime_delta_per_tyre_lap"] = lap_delta / (tyre + 1.0)
    ...
    out["midfield_runner"] = ((pos > 5) & (pos <= 15)).astype("int8")
    out["backmarker"] = (pos > 15).astype("int8")

    out["late_old_tyre"] = ((rp > 0.6) & (tyre >= 15)).astype("int8")
    out["wet_old_tyre"] = ((wet == 1) & (tyre >= 10)).astype("int8")
    out["soft_old_tyre"] = ((compound == "SOFT") & (tyre >= 12)).astype("int8")
    out["hard_long_stint"] = ((compound == "HARD") & (tyre >= 25)).astype("int8")
    out["pit_window_pressure"] = (
        0.35 * rp + 0.03 * tyre + 0.08 * stint + 0.0005 * degr + 0.01 * lap_delta
    ).astype("float32")

    out["time_order_axis"] = (year - year.min()) * 1000 + lap
    out["race_year"] = (race + "_" + out["Year"].astype("string")).astype("category")
    out["driver_year"] = (driver + "_" + out["Year"].astype("string")).astype(
        "category"
    )
    out["compound_phase"] = (
        compound + "_P" + out["race_phase"].astype("string")
    ).astype("category")
    out["race_compound"] = (race + "_" + compound).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000858.",
    }, sort_keys=True))
