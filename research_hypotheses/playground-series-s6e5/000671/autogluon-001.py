...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(driver_race_key + ["LapNumber", "_row_id"]).copy()

    out["IsWetCompound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["IsSoftCompound"] = (out["Compound"] == "SOFT").astype("int8")
    out["IsHardCompound"] = (out["Compound"] == "HARD").astype("int8")

    out["PriorPitStops"] = (
        out.groupby(driver_race_key)["PitStop"].cumsum() - out["PitStop"]
    )
    out["LapsInCurrentStintProxy"] = (
        out.groupby(driver_race_key + ["Stint"]).cumcount() + 1
    )
    out["TyreLifeMinusStintLap"] = out["TyreLife"] - out["LapsInCurrentStintProxy"]

    out["DriverStint_MedianDelta"] = out.groupby(driver_race_key + ["Stint"])[
        "LapTime_Delta"
    ].transform("median")
    out["CompoundRace_MedianTyreLife"] = out.groupby(race_key + ["Compound"])[
        "TyreLife"
    ].transform("median")
    out["CompoundRace_MedianDeg"] = out.groupby(race_key + ["Compound"])[
        "Cumulative_Degradation"
    ].transform("median")

    out["TyreAgePressure"] = out["TyreLife"] - out["CompoundRace_MedianTyreLife"]
    out["DegPressure"] = out["Cumulative_Degradation"] - out["CompoundRace_MedianDeg"]
    out["LapDeltaPressure"] = out["LapTime_Delta"] - out["DriverStint_MedianDelta"]

    out["RaceLap_MedianLapTime"] = out.groupby(lap_key)["LapTime (s)"].transform(
        "median"
    )
    out["RaceLap_MedianDelta"] = out.groupby(lap_key)["LapTime_Delta"].transform(
        "median"
    )
    out["Race_MedianLapTime"] = out.groupby(race_key)[
        "RaceLap_MedianLapTime"
    ].transform("median")
    out["Race_MedianDelta"] = out.groupby(race_key)["RaceLap_MedianDelta"].transform(
        "median"
    )
    out["Race_StdLapTime"] = (
        out.groupby(race_key)["RaceLap_MedianLapTime"].transform("std").fillna(0)
    )
    out["Race_StdDelta"] = (
        out.groupby(race_key)["RaceLap_MedianDelta"].transform("std").fillna(0)
    )

    out["NeutralizationProxyActive"] = (
        (
            (out["RaceLap_MedianLapTime"] - out["Race_MedianLapTime"])
            > 2.0 * out["Race_StdLapTime"]
        )
        | (
            (out["RaceLap_MedianDelta"] - out["Race_MedianDelta"])
            > 2.0 * out["Race_StdDelta"]
        )
    ).astype("int8")

    lap_state = (
        out[lap_key + ["NeutralizationProxyActive"]]
        .drop_duplicates(lap_key)
        .sort_values(lap_key)
        .copy()
    )
    lap_state["PrevNeutralization"] = lap_state.groupby(race_key)[
        "NeutralizationProxyActive"
    ].shift(1, fill_value=0)
    lap_state["NeutralizationJustEnded"] = (
        (lap_state["PrevNeutralization"] == 1)
        & (lap_state["NeutralizationProxyActive"] == 0)
    ).astype("int8")
    lap_state["NeutralizationStarted"] = (
        (lap_state["PrevNeutralization"] == 0)
        & (lap_state["NeutralizationProxyActive"] == 1)
    ).astype("int64")
    lap_state["NeutralizationBlock"] = lap_state.groupby(race_key)[
        "NeutralizationStarted"
    ].cumsum()
    lap_state["LapsSinceNeutralizationStart"] = (
        lap_state.groupby(race_key + ["NeutralizationBlock"])
        .cumcount()
        .where(lap_state["NeutralizationProxyActive"].eq(1), 0)
        .astype("int16")
    )
    lap_state["RecentEnd1"] = lap_state.groupby(race_key)[
        "NeutralizationJustEnded"
    ].shift(1, fill_value=0)
    lap_state["RecentEnd2"] = lap_state.groupby(race_key)[
        "NeutralizationJustEnded"
    ].shift(2, fill_value=0)
    lap_state["NeutralizationRecentlyEnded"] = (
        (lap_state["NeutralizationJustEnded"] == 1)
        | (lap_state["RecentEnd1"] == 1)
        | (lap_state["RecentEnd2"] == 1)
    ).astype("int8")
    ...
    out = out.merge(
        lap_state[
            ...
            + [
                "NeutralizationJustEnded",
                "LapsSinceNeutralizationStart",
                "NeutralizationRecentlyEnded",
            ]
        ],
        on=lap_key,
        how="left",
    ...
    out["CarsOnLap"] = out.groupby(lap_key)["Driver"].transform("size")
    out["MidfieldTrafficProxy"] = (
        out["Position"].between(6, 15) & out["Position_Change"].abs().le(1)
    ).astype("int8")
    out["RejoinInTrafficProxy"] = (
        out["PriorPitStops"].ge(1)
        & out["Position"].between(8, 16)
        & out["MidfieldTrafficProxy"].eq(1)
    ).astype("int8")
    out["CheapStopWindowProxy"] = (
        out["NeutralizationProxyActive"].eq(1)
        | out["NeutralizationRecentlyEnded"].eq(1)
    ).astype("int8")
    out["UndercutPressureProxy"] = (
        out["TyreAgePressure"].gt(0)
        & out["LapDeltaPressure"].gt(0)
        & out["DegPressure"].gt(0)
        & out["MidfieldTrafficProxy"].eq(1)
    ).astype("int8")

    out["PitTrafficInteraction"] = out["PriorPitStops"] * out["MidfieldTrafficProxy"]
    out["StopWindowPressure"] = out["CheapStopWindowProxy"] * (
        out["TyreAgePressure"].clip(lower=0) + out["DegPressure"].clip(lower=0)
    )
    out["UndercutWindowScore"] = (
        out["UndercutPressureProxy"].astype("int8")
        + out["CheapStopWindowProxy"].astype("int8")
        + out["RejoinInTrafficProxy"].astype("int8")
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000671"],
        "research_usage_note": "Verified assigned hypothesis 000671.",
    }, sort_keys=True))
