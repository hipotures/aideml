...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_key = X["Year"].astype(str) + "|" + X["Race"].astype(str)
    driver_race_key = race_key + "|" + X["Driver"].astype(str)
    compound_map = {
        "SOFT": 0,
        "MEDIUM": 1,
        "HARD": 2,
    ...
    X["RaceYear"] = race_key
    X["DriverRace"] = driver_race_key

    eps = 1e-6
    tyre_life = X["TyreLife"].astype("float32")
    lap_no = X["LapNumber"].astype("float32")
    ...
    progress = X["RaceProgress"].astype("float32")

    race_max_lap = X.groupby("RaceYear")["LapNumber"].transform("max").astype("float32")
    race_med_laptime = (
        X.groupby("RaceYear")["LapTime (s)"].transform("median").astype("float32")
    )
    race_std_laptime = (
        X.groupby("RaceYear")["LapTime (s)"]
        .transform("std")
        .fillna(0)
        .astype("float32")
    )
    race_med_delta = (
        X.groupby("RaceYear")["LapTime_Delta"].transform("median").astype("float32")
    )
    race_med_tyre = (
        X.groupby("RaceYear")["TyreLife"].transform("median").astype("float32")
    )
    compound_med_laptime = (
        X.groupby(["RaceYear", "Compound"])["LapTime (s)"]
        .transform("median")
        .astype("float32")
    )
    compound_med_degr = (
        X.groupby(["RaceYear", "Compound"])["Cumulative_Degradation"]
        .transform("median")
        .astype("float32")
    )
    driver_race_med_laptime = (
        X.groupby("DriverRace")["LapTime (s)"].transform("median").astype("float32")
    )
    driver_race_med_pos = (
        X.groupby("DriverRace")["Position"].transform("median").astype("float32")
    )

    X["LapsRemaining"] = (race_max_lap - lap_no).astype("float32")
    X["TyreLifeFrac"] = (tyre_life / (race_max_lap + eps)).astype("float32")
    X["TyreLifeVsLap"] = (tyre_life / (lap_no + eps)).astype("float32")
    X["StintPressure"] = (tyre_life * (1.0 + progress)).astype("float32")
    X["DegradationPerLap"] = (degr / (tyre_life + eps)).astype("float32")
    X["DegradationPressure"] = (
        degr / (X["LapsRemaining"].astype("float32") + 1.0)
    ).astype("float32")
    X["LapDeltaPerTyreLap"] = (lap_delta / (tyre_life + eps)).astype("float32")
    X["LapDeltaXDeg"] = (lap_delta * X["DegradationPerLap"]).astype("float32")
    X["ProgressXTyreLife"] = (progress * tyre_life).astype("float32")
    X["ProgressXStint"] = (progress * stint).astype("float32")
    X["PositionPressure"] = ((21.0 - pos) * tyre_life).astype("float32")
    X["PositionChangePressure"] = (pos_chg * tyre_life).astype("float32")

    X["LapTimeRaceRel"] = (lap_time - race_med_laptime).astype("float32")
    X["LapTimeCompoundRel"] = (lap_time - compound_med_laptime).astype("float32")
    X["LapTimeDriverRaceRel"] = (lap_time - driver_race_med_laptime).astype("float32")
    X["LapDeltaRaceRel"] = (lap_delta - race_med_delta).astype("float32")
    X["TyreLifeRaceRel"] = (tyre_life - race_med_tyre).astype("float32")
    X["DegradationCompoundRel"] = (degr - compound_med_degr).astype("float32")
    X["PositionDriverRaceRel"] = (pos - driver_race_med_pos).astype("float32")

    X["LapTimeRaceZ"] = (
        (lap_time - race_med_laptime) / (race_std_laptime + eps)
    ).astype("float32")
    X["NearRaceEnd"] = (X["LapsRemaining"] <= 3).astype("int8")
    X["LongTyreRun"] = (
        tyre_life >= X.groupby(["RaceYear", "Compound"])["TyreLife"].transform("median")
    ).astype("int8")
    X["HighDegFlag"] = (
        degr
        >= X.groupby(["RaceYear", "Compound"])["Cumulative_Degradation"].transform(
            "median"
        )
    ).astype("int8")

    X["Compound_Stint"] = X["Compound"].astype(str) + "_S" + X["Stint"].astype(str)
    X["Compound_PitStop"] = X["Compound"].astype(str) + "_P" + X["PitStop"].astype(str)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000737.",
    }, sort_keys=True))
