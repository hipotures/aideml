...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out["__orig_order"] = np.arange(len(out))

    sort_cols = ["Year", "Race", "Driver", "LapNumber", "__orig_order"]
    sorted_out = out.sort_values(sort_cols, kind="mergesort")
    ...
    for col in lag_cols:
        lag_name = f"prev_{col.replace(' ', '_').replace('(', '').replace(')', '')}"
        sorted_out[lag_name] = sorted_out.groupby(group_cols, sort=False)[col].shift(1)
    ...
    for col in numeric_lags:
        safe = col.replace(" ", "_").replace("(", "").replace(")", "")
        prev_col = f"prev_{safe}"
        sorted_out[f"delta_{safe}_from_prev"] = sorted_out[col] - sorted_out[prev_col]

    sorted_out["prev_compound_same"] = (
        sorted_out["Compound"].eq(sorted_out["prev_Compound"]).astype("int8")
    )
    sorted_out.loc[sorted_out["prev_Compound"].isna(), "prev_compound_same"] = 0

    sorted_out = sorted_out.sort_values("__orig_order", kind="mergesort").drop(
        columns="__orig_order"
    )
    return sorted_out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000032"],
        "research_usage_note": "Verified assigned hypothesis 000032.",
    }, sort_keys=True))
