...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = out["RaceProgress"].astype(float).clip(lower=1e-6)
    tyre_life = out["TyreLife"].astype(float)

    est_total_laps = lap / progress
    laps_remaining_after_next = (est_total_laps - lap - 1.0).clip(lower=0.0)
    out["FreshStop_LapsRemainingAfterNext"] = laps_remaining_after_next
    ...
    margins = []
    slick_margins = []
    for compound, window in fresh_windows.items():
        margin = window - laps_remaining_after_next
        col = f"FreshStop_{compound}_FinishMargin"
        out[col] = margin
        out[f"FreshStop_{compound}_CanFinish"] = (margin >= 0).astype("int8")
        margins.append(margin)
        if compound in ("SOFT", "MEDIUM", "HARD"):
            slick_margins.append(margin)

    best_margin = np.maximum.reduce(margins)
    best_slick_margin = np.maximum.reduce(slick_margins)

    out["FreshStop_AnyFreshCanFinish"] = (best_margin >= 0).astype("int8")
    out["FreshStop_AnyFreshSlickCanFinish"] = (best_slick_margin >= 0).astype("int8")
    out["FreshStop_BestFreshMargin"] = best_margin
    out["FreshStop_BestFreshSlickMargin"] = best_slick_margin

    current_window = out["Compound"].map(fresh_windows).astype(float)
    current_margin_to_finish = current_window - tyre_life - laps_remaining_after_next
    out["FreshStop_CurrentTyreFinishMargin"] = current_margin_to_finish
    out["FreshStop_CurrentTyreUnderPressure"] = (current_margin_to_finish < 0).astype(
        "int8"
    )
    out["FreshStop_StopSolvesCurrentPressure"] = (
        (current_margin_to_finish < 0) & (best_margin >= 0)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000013"],
        "research_usage_note": "Verified assigned hypothesis 000013.",
    }, sort_keys=True))
