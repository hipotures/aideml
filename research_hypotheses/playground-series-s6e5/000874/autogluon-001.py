...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver = out["Driver"].astype("string").fillna("__missing__").str.strip()
    race = out["Race"].astype("string").fillna("__missing__").str.strip()
    compound = out["Compound"].astype("string").fillna("__missing__").str.strip()
    stint = out["Stint"].astype("string").fillna("__missing__")

    stint_num = pd.to_numeric(out["Stint"], errors="coerce")
    stint_bin = (
        pd.cut(
            stint_num,
    ...
    out["StintBin"] = stint_bin.astype("category")

    out["Driver_Race"] = (driver + "|" + race).astype("category")
    out["Driver_Compound"] = (driver + "|" + compound).astype("category")
    out["Race_Stint"] = (race + "|" + stint).astype("category")
    out["Driver_Race_StintBin"] = (driver + "|" + race + "|" + stint_bin).astype(
        "category"
    )
    ...
    order_frame = pd.DataFrame(
        {
            ...
            "Race": race.to_numpy(),
            "Driver": driver.to_numpy(),
            "LapNumber": out["LapNumber"].to_numpy(),
            "_pos": np.arange(n, dtype=np.int64),
        }
    )
    ordered_pos = order_frame.sort_values(
        ["Year", "Race", "Driver", "LapNumber", "_pos"],
        kind="mergesort",
    )["_pos"].to_numpy()

    sig_ord = pd.Series(signal[ordered_pos], dtype="float64")
    global_count = np.arange(n, dtype=np.float64)
    global_sum = np.cumsum(sig_ord.to_numpy()) - sig_ord.to_numpy()
    ...
    def add_ordered_prior(col: str, smooth: float = 30.0) -> None:
        key_ord = pd.Series(
            out[col].astype("string").iloc[ordered_pos].to_numpy(), dtype="string"
        )
        grp = sig_ord.groupby(key_ord, sort=False)
        cnt = grp.cumcount().to_numpy(dtype=np.float64)
        prev_sum = (grp.cumsum() - sig_ord).to_numpy(dtype=np.float64)

        prior_ord = (prev_sum + smooth * global_prior) / (cnt + smooth)
        cnt_ord = np.log1p(cnt)

        prior = np.empty(n, dtype=np.float32)
        hist_count = np.empty(n, dtype=np.float32)
        prior[ordered_pos] = prior_ord.astype(np.float32)
        hist_count[ordered_pos] = cnt_ord.astype(np.float32)

        out[f"{col}_ordered_pit_prior"] = prior
        out[f"{col}_ordered_hist_count"] = hist_count
    ...
    for key in [
        ...
        "Driver_Compound",
        "Race_Stint",
        "Driver_Race_StintBin",
    ]:
        add_ordered_prior(key)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000874"],
        "research_usage_note": "Verified assigned hypothesis 000874.",
    }, sort_keys=True))
