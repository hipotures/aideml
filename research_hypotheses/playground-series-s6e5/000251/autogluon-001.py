...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    keys = [out["Year"], out["Race"].astype("string"), out["Driver"].astype("string")]
    grp_cols = ["Year", "Race", "Driver"]

    out["_is_slick_tmp"] = is_slick
    out["_is_wet_tmp"] = is_wet

    used_slick_count = pd.Series(0, index=out.index, dtype="int8")
    for c in slicks:
        seen_c = comp.eq(c).groupby(keys, sort=False).cummax()
        used_slick_count = used_slick_count + seen_c.astype("int8")

    dry_race_has_wet = (
        out.groupby(grp_cols, sort=False)["_is_wet_tmp"].transform("max").astype("int8")
    )
    wet_rule_exempt = dry_race_has_wet

    current_counts = is_slick
    remaining_legal = (2 - used_slick_count).clip(lower=0).astype("int8")
    remaining_legal = remaining_legal.mask(wet_rule_exempt.eq(1), 0).astype("int8")
    can_finish = (remaining_legal.eq(0) | wet_rule_exempt.eq(1)).astype("int8")

    race_norm = out["Race"].astype("string").str.lower()
    monaco_2025 = (
        out["Year"].eq(2025) & race_norm.str.contains("monaco", regex=False, na=False)
    ).astype("int8")

    stops_so_far = out["Stint"].fillna(1).astype(float).sub(1).clip(lower=0)
    min_stops_remaining = (2 - stops_so_far).clip(lower=0)
    min_stops_remaining = min_stops_remaining.where(
        monaco_2025.eq(1), remaining_legal.astype(float)
    )
    min_stops_remaining = min_stops_remaining.astype("float32")

    laps_remaining_est = (
        (1.0 - out["RaceProgress"].astype(float).clip(0, 1))
        * (
            out["LapNumber"].astype(float)
            / out["RaceProgress"].astype(float).clip(lower=0.01)
        )
    ).clip(lower=0)
    laps_until_rule_breach_window = (
        (laps_remaining_est - min_stops_remaining)
        .clip(lower=0, upper=25)
        .astype("float32")
    )

    out["used_slick_count"] = used_slick_count.astype("int8")
    out["current_compound_counts_toward_rule"] = current_counts.astype("int8")
    out["remaining_legal_obligations"] = remaining_legal.astype("int8")
    out["can_finish_legally_without_stop"] = can_finish.astype("int8")
    out["wet_rule_exempt"] = wet_rule_exempt.astype("int8")
    out["monaco_2025_forced_two_stop"] = monaco_2025.astype("int8")
    out["min_stops_remaining"] = min_stops_remaining
    out["laps_until_rule_breach_window"] = laps_until_rule_breach_window

    return out.drop(columns=["_is_slick_tmp", "_is_wet_tmp"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000251"],
        "research_usage_note": "Verified assigned hypothesis 000251.",
    }, sort_keys=True))
