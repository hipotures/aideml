...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    gcols = ["Race", "Year", "LapNumber"]

    group_size = (
        out.groupby(gcols, observed=True)["TyreLife"].transform("size").astype(float)
    )
    damp = (group_size / 20.0).clip(0.0, 1.0)

    def damped_pct_rank(col, ascending=True):
        r = out.groupby(gcols, observed=True)[col].rank(pct=True, ascending=ascending)
        return (0.5 + (r.fillna(0.5) - 0.5) * damp).astype("float32")

    lap_median = out.groupby(gcols, observed=True)["LapTime (s)"].transform("median")
    out["race_lap_group_size"] = group_size.astype("float32")
    out["race_lap_damp"] = damp.astype("float32")
    out["tyrelife_lap_pct"] = damped_pct_rank("TyreLife", ascending=True)
    out["position_lap_pct"] = damped_pct_rank("Position", ascending=True)
    out["degradation_lap_pct"] = damped_pct_rank(
        "Cumulative_Degradation", ascending=True
    )

    out["pace_residual_lap"] = (out["LapTime (s)"] - lap_median).astype("float32")
    out["pace_residual_lap_pct"] = damped_pct_rank("pace_residual_lap", ascending=True)
    out["laptime_delta_lap_pct"] = damped_pct_rank("LapTime_Delta", ascending=True)
    ...
    out["late_stint_pressure"] = (
        out["RaceProgress"] * np.log1p(out["TyreLife"].clip(lower=0))
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000638"],
        "research_usage_note": "Verified assigned hypothesis 000638.",
    }, sort_keys=True))
