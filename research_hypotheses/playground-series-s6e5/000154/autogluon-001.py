...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year = out["Year"].astype("int16").astype("string")
    stint = out["Stint"].astype("int16").astype("string")
    compound = out["Compound"].astype("string")
    driver = out["Driver"].astype("string")
    race = out["Race"].astype("string")

    out["Race_Year"] = race + "_" + year
    out["Driver_Race_Year"] = driver + "_" + out["Race_Year"]
    out["Driver_Compound"] = driver + "_" + compound
    out["Race_Year_Stint"] = out["Race_Year"] + "_" + stint
    out["Driver_Stint_Compound"] = driver + "_" + stint + "_" + compound
    out["Race_Compound"] = race + "_" + compound
    out["Compound_Stint"] = compound + "_" + stint
    ...
    out["TyreLife_sq"] = tyre * tyre
    out["TyreLife_progress"] = tyre * progress
    out["TyreLife_per_stint"] = tyre / out["Stint"].astype("float32").clip(lower=1)
    out["Lap_frac_est"] = lap / progress.replace(0, np.nan)
    out["RemainingProgress"] = 1.0 - progress
    out["LateRace_TyreLife"] = tyre * out["RemainingProgress"]
    out["Degradation_per_tyre_lap"] = deg / tyre.clip(lower=1)
    out["Abs_LapTime_Delta"] = lap_delta.abs()
    out["LapTime_Delta_x_TyreLife"] = lap_delta * tyre
    out["LapTime_log"] = np.log1p(lap_time.clip(lower=0))
    out["Position_Change_abs"] = pos_chg.abs()
    out["Position_x_TyreLife"] = pos * tyre
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000154"],
        "research_usage_note": "Verified assigned hypothesis 000154.",
    }, sort_keys=True))
