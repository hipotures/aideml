...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["team_proxy"] = df["Year"].astype(str) + "_" + df["Driver"].astype(str)
    df["race_key"] = df["Year"].astype(str) + "_" + df["Race"].astype(str)
    df["is_wet"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    df["is_caution_like"] = (df["LapTime_Delta"] > 10).astype("int8")

    df = df.sort_values(
        ["Year", "Race", "LapNumber", "Driver", "Stint", "_row_id"],
        kind="mergesort",
    ).reset_index(drop=True)

    def prior_mean(group_cols, value_col, out_col, mask=None):
        if mask is None:
            g = df.groupby(group_cols, sort=False)
            cnt = g.cumcount()
            csum = g[value_col].cumsum() - df[value_col]
            df[out_col] = (csum / cnt.where(cnt > 0, 1)).where(cnt > 0, -1.0)
        else:
            tmp_v = df[value_col].where(mask, 0.0).astype("float32")
            tmp_c = mask.astype("int8")
            g = pd.DataFrame({"v": tmp_v, "c": tmp_c}).groupby(
                [df[c] for c in group_cols], sort=False
            )
            csum = g["v"].cumsum() - tmp_v
            cnt = g["c"].cumsum() - tmp_c
            df[out_col] = (csum / cnt.where(cnt > 0, 1)).where(cnt > 0, -1.0)

    prior_mean(["team_proxy", "Compound"], "PitStop", "team_compound_pit_rate_prior")
    prior_mean(["team_proxy", "is_wet"], "PitStop", "team_wet_pit_rate_prior")
    prior_mean(
        ["team_proxy", "is_caution_like"],
        "PitStop",
        "team_caution_pit_rate_prior",
    )

    first_stop_mask = (df["PitStop"] == 1) & (df["Stint"] == 1)
    prior_mean(
        ["team_proxy"],
        "LapNumber",
        "team_first_stop_lap_prior",
        mask=first_stop_mask,
    )
    prior_mean(
        ["team_proxy"],
        "TyreLife",
        "team_first_stop_tyre_prior",
        mask=first_stop_mask,
    )

    df["lap_vs_team_first_stop"] = df["LapNumber"] - df["team_first_stop_lap_prior"]
    df["tyre_vs_team_first_stop"] = df["TyreLife"] - df["team_first_stop_tyre_prior"]
    df["team_compound_pressure"] = (
        df["TyreLife"] * (df["team_compound_pit_rate_prior"] + 1.0)
    ).astype("float32")
    df["team_wet_pressure"] = (
        df["RaceProgress"] * (df["team_wet_pit_rate_prior"] + 1.0)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000427.",
    }, sort_keys=True))
