...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Compound_Code"] = df["Compound"].map(compound_map).astype(float)
    df["Is_WetLike"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype(np.int8)

    stint_keys = ["Year", "Race", "Driver", "Stint"]
    race_keys = ["Year", "Race"]
    lap_keys = ["Year", "Race", "LapNumber"]

    g_stint = df.groupby(stint_keys, sort=False)
    g_race = df.groupby(race_keys, sort=False)
    g_lap = df.groupby(lap_keys, sort=False)

    df["Stint_Lap_Count"] = g_stint["LapNumber"].transform("count").astype(np.float32)
    df["Stint_Lap_Index"] = g_stint.cumcount().add(1).astype(np.float32)
    df["Stint_Laps_Remaining"] = (df["Stint_Lap_Count"] - df["Stint_Lap_Index"]).astype(
        np.float32
    )
    df["Stint_Progress"] = (
        df["Stint_Lap_Index"] / df["Stint_Lap_Count"].clip(lower=1)
    ).astype(np.float32)

    df["Stint_TyreLife_Max"] = g_stint["TyreLife"].transform("max").astype(np.float32)
    df["Stint_Degradation_Max"] = (
        g_stint["Cumulative_Degradation"].transform("max").astype(np.float32)
    )
    df["Stint_LapDelta_Mean"] = (
        g_stint["LapTime_Delta"].transform("mean").astype(np.float32)
    )
    df["Stint_LapDelta_Std"] = (
        g_stint["LapTime_Delta"].transform("std").fillna(0).astype(np.float32)
    )

    df["TyreLife_In_Stint_Pct"] = (
        df["TyreLife"] / df["Stint_TyreLife_Max"].replace(0, 1)
    ).astype(np.float32)
    df["Deg_In_Stint_Pct"] = (
        (df["Cumulative_Degradation"] / df["Stint_Degradation_Max"].replace(0, 1))
    ...
    df["LapDelta_Stint_Z"] = (
        (
            (df["LapTime_Delta"] - df["Stint_LapDelta_Mean"])
            / df["Stint_LapDelta_Std"].replace(0, 1)
        )
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype(np.float32)
    ...
    df["Candidate_Urgency"] = (
        0.34 * df["TyreLife_In_Stint_Pct"]
        + 0.28 * df["Deg_In_Stint_Pct"]
        + 0.18 * df["LapDelta_Stint_Z"]
        + 0.12 * df["RaceProgress"]
        + 0.08 * (4.0 - df["Compound_Code"])
    ).astype(np.float32)

    df["Urgency_Rank_In_Stint"] = (
        g_stint["Candidate_Urgency"].rank(method="average", pct=True).astype(np.float32)
    )
    df["TyreLife_Rank_In_Stint"] = (
        g_stint["TyreLife"].rank(method="average", pct=True).astype(np.float32)
    )
    df["Deg_Rank_In_Stint"] = (
        g_stint["Cumulative_Degradation"]
        .rank(method="average", pct=True)
        .astype(np.float32)
    )
    df["LapDelta_Rank_In_Stint"] = (
        g_stint["LapTime_Delta"].rank(method="average", pct=True).astype(np.float32)
    )
    df["Position_Rank_In_Stint"] = (
        g_stint["Position"]
        .rank(method="average", pct=True, ascending=False)
        .astype(np.float32)
    )

    df["Race_Max_Lap"] = g_race["LapNumber"].transform("max").astype(np.float32)
    df["Laps_To_Race_End"] = (df["Race_Max_Lap"] - df["LapNumber"]).astype(np.float32)
    df["TyreLife_x_Deg"] = (df["TyreLife"] * df["Cumulative_Degradation"]).astype(
        np.float32
    )
    df["TyreLife_x_Softness"] = (df["TyreLife"] * df["Compound_Code"]).astype(
        np.float32
    )
    df["Deg_x_LapDelta"] = (df["Cumulative_Degradation"] * df["LapTime_Delta"]).astype(
        np.float32
    )
    df["Urgency_x_Remaining"] = (
        df["Candidate_Urgency"] * df["Laps_To_Race_End"]
    ).astype(np.float32)

    df["Lap_Field_Mean_Delta"] = (
        g_lap["LapTime_Delta"].transform("mean").astype(np.float32)
    )
    df["Lap_Field_Mean_Deg"] = (
        g_lap["Cumulative_Degradation"].transform("mean").astype(np.float32)
    )
    df["Lap_Field_Mean_TyreLife"] = (
        g_lap["TyreLife"].transform("mean").astype(np.float32)
    )

    df["LapDelta_vs_Field"] = (df["LapTime_Delta"] - df["Lap_Field_Mean_Delta"]).astype(
        np.float32
    )
    df["Deg_vs_Field"] = (
        df["Cumulative_Degradation"] - df["Lap_Field_Mean_Deg"]
    ).astype(np.float32)
    df["TyreLife_vs_Field"] = (df["TyreLife"] - df["Lap_Field_Mean_TyreLife"]).astype(
        np.float32
    )

    df["Pressure_Signal"] = (
        0.5 * df["Position_Change"].abs()
        + 0.3 * df["LapDelta_vs_Field"]
        + 0.2 * (21 - df["Position"])
    ).astype(np.float32)
    df["Pressure_Rank_Lap"] = (
        g_lap["Pressure_Signal"].rank(method="average", pct=True).astype(np.float32)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000686"],
        "research_usage_note": "Verified assigned hypothesis 000686.",
    }, sort_keys=True))
