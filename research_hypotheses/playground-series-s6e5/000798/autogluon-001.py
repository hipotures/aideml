...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Compound_Service_Window"] = df["Compound"].map(compound_window).astype(float)
    df["Race_Total_Laps_Est"] = (
        df.groupby(race_keys)["LapNumber"].transform("max").astype(float)
    )
    df["Laps_Remaining_Est"] = (df["Race_Total_Laps_Est"] - df["LapNumber"]).clip(
        lower=0
    )

    df["Service_Window_Pressure"] = df["TyreLife"] / df["Compound_Service_Window"]
    df["Service_Age_Distance"] = df["TyreLife"] - df["Compound_Service_Window"]

    df["TyreAge_At_Finish_Est"] = df["TyreLife"] + df["Laps_Remaining_Est"]
    df["Finish_Stop_Pressure"] = (
        df["TyreAge_At_Finish_Est"] - df["Compound_Service_Window"]
    )
    df["Cannot_Finish_On_Current_Tyre"] = (df["Finish_Stop_Pressure"] > 0).astype(int)

    pit_seen = df["PitStop"].fillna(0).astype(int)
    pit_count_before = (
        pit_seen.groupby([df[k] for k in driver_keys]).cumsum() - pit_seen
    )
    df["Laps_Since_Last_Pit"] = (
        df["TyreLife"].where(pit_count_before > 0, 0).astype(float)
    )
    df["Fresh_Tyre_Score"] = 1.0 / (1.0 + df["Laps_Since_Last_Pit"])

    df["Service_Window_Overage_Flag"] = (df["Service_Age_Distance"] > 0).astype(int)
    df["Critical_Last_Laps_Flag"] = (
        (df["Laps_Remaining_Est"] <= 5) & (df["Cannot_Finish_On_Current_Tyre"] == 1)
    ).astype(int)

    df["Normalized_Degradation"] = df["Cumulative_Degradation"] / (df["TyreLife"] + 1.0)
    df["Delta_Per_TyreLap"] = df["LapTime_Delta"] / (df["TyreLife"] + 1.0)

    df["Urgency_x_Position"] = df["Service_Window_Pressure"] * df["Position"]
    df["Urgency_x_Stint"] = df["Service_Window_Pressure"] * df["Stint"]
    df["Finish_Pressure_x_Progress"] = df["Finish_Stop_Pressure"] * df["RaceProgress"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000798.",
    }, sort_keys=True))
