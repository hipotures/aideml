...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["race_lap_group_size"] = (
        X.groupby(grp)["Driver"].transform("size").astype("float32")
    )
    ...
    for col, ascending in rank_specs.items():
        ...
        X[f"{col}_lap_gap_to_best"] = (
            X[col] - (g.transform("min") if ascending else g.transform("max"))
        ).astype("float32")
        X[f"{col}_lap_gap_to_median"] = (X[col] - g.transform("median")).astype(
            "float32"
        )

    # Relative pit-pressure proxy inside the same race/lap context.
    X["pit_urgency_raw"] = (
        0.35 * X["TyreLife_lap_pct_rank"]
        + 0.35 * X["Cumulative_Degradation_lap_pct_rank"]
        + 0.20 * X["LapTime_Delta_lap_pct_rank"]
        + 0.10 * (-X["Position_Change_lap_z"])
    ).astype("float32")

    g_urg = X.groupby(grp)["pit_urgency_raw"]
    X["pit_urgency_lap_rank"] = g_urg.rank(
        method="average", pct=True, ascending=True
    ).astype("float32")
    X["pit_urgency_lap_z"] = (
        (
            (X["pit_urgency_raw"] - g_urg.transform("mean"))
            / g_urg.transform("std").replace(0, np.nan)
        )
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    ...
    # Interaction features so the classifier can exploit groupwise ordering by tyre/stint context.
    X["urgency_x_stint"] = (X["pit_urgency_raw"] * X["Stint"]).astype("float32")
    X["urgency_x_progress"] = (X["pit_urgency_raw"] * X["RaceProgress"]).astype(
        "float32"
    )
    X["tyrelife_x_degradation_rank"] = (
        X["TyreLife_lap_pct_rank"] * X["Cumulative_Degradation_lap_pct_rank"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000978.",
    }, sort_keys=True))
