...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_cols = ["Year", "Race", "LapNumber"]
    base_cols = ["Year", "Race"]

    lap_time = out["LapTime (s)"].astype(float)
    race_median = out.groupby(base_cols, observed=True)["LapTime (s)"].transform(
        "median"
    )
    race_mad = (
        (lap_time - race_median)
        .abs()
        .groupby([out["Year"], out["Race"]], observed=True)
        .transform("median")
    )
    race_mad = race_mad.replace(0, np.nan).fillna(race_mad.median())
    out["sc_laptime_z"] = ((lap_time - race_median) / (1.4826 * race_mad + 1e-6)).clip(
        -5, 8
    )

    out["sc_lap_median_laptime"] = out.groupby(race_cols, observed=True)[
        "LapTime (s)"
    ].transform("median")
    out["sc_lap_time_inflation"] = (
        out["sc_lap_median_laptime"] / (race_median + 1e-6) - 1.0
    ).clip(-0.25, 2.0)

    pos_var = (
        out.groupby(race_cols, observed=True)["Position_Change"]
        .transform("std")
        .fillna(0.0)
    )
    out["sc_position_suppression"] = (1.0 / (1.0 + pos_var)).clip(0, 1)

    out["sc_pit_cluster_rate"] = (
        out.groupby(race_cols, observed=True)["PitStop"].transform("mean").clip(0, 1)
    )
    out["sc_pit_cluster_count"] = (
        out.groupby(race_cols, observed=True)["PitStop"].transform("sum").clip(0, 20)
    )

    raw_score = (
        1.4 * out["sc_lap_time_inflation"].fillna(0)
        + 0.7 * out["sc_position_suppression"].fillna(0)
        + 1.2 * out["sc_pit_cluster_rate"].fillna(0)
        + 0.25 * (out["sc_laptime_z"].fillna(0) > 1.5).astype(float)
        - 0.35
    )
    out["sc_vsc_detector_prob"] = 1.0 / (1.0 + np.exp(-3.0 * raw_score.clip(-4, 4)))
    out["is_likely_sc_vsc"] = (out["sc_vsc_detector_prob"] > 0.55).astype("int8")

    out["tyrelife_x_sc_prob"] = (
        out["TyreLife"].astype(float) * out["sc_vsc_detector_prob"]
    )
    out["raceprogress_x_sc_prob"] = (
        out["RaceProgress"].astype(float) * out["sc_vsc_detector_prob"]
    )
    out["stint_x_sc_prob"] = out["Stint"].astype(float) * out["sc_vsc_detector_prob"]
    out["green_flag_weight"] = 1.0 - out["sc_vsc_detector_prob"]
    out["tyrelife_green"] = out["TyreLife"].astype(float) * out["green_flag_weight"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000344"],
        "research_usage_note": "Verified assigned hypothesis 000344.",
    }, sort_keys=True))
