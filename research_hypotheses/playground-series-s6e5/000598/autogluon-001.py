...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected = comp.map(compound_life).fillna(26.0).astype(float)
    ...
    out["ExpectedTyreLife"] = expected
    out["TyreLife_ExpectedLife_Ratio"] = tyre_life / expected.clip(lower=1.0)
    out["ExpectedLife_Remaining"] = (expected - tyre_life).clip(lower=0.0)
    out["ExpectedLife_Overrun"] = (tyre_life - expected).clip(lower=0.0)
    out["LapsRemaining_Est"] = laps_remaining
    out["LapsRemaining_AfterExpectedLife"] = (
        laps_remaining - (expected - tyre_life)
    ).clip(lower=0.0)
    out["Compound_LapsBeyondFinishWindow"] = (
        tyre_life - np.minimum(expected, tyre_life + laps_remaining)
    ).clip(lower=0.0)

    cooldown = (4.0 - tyre_life).clip(lower=0.0) / 4.0
    out["PitCooldown_Suppression"] = cooldown
    readiness = (
        0.45 * out["TyreLife_ExpectedLife_Ratio"]
        + 0.25 * (out["LapsRemaining_AfterExpectedLife"] / expected.clip(lower=1.0))
        + 0.20 * out["ExpectedLife_Overrun"] / expected.clip(lower=1.0)
        + 0.10 * np.log1p(stint.clip(lower=1.0))
    )
    out["PitReadiness_CooldownAdjusted"] = (readiness - cooldown).astype(float)

    out["LateRace_NoStopPressure"] = ((1.0 - progress) * (expected - tyre_life)).astype(
        float
    )
    out["Wear_x_RaceProgress"] = out["TyreLife_ExpectedLife_Ratio"] * progress
    out["Wear_x_Stint"] = out["TyreLife_ExpectedLife_Ratio"] * stint
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000598.",
    }, sort_keys=True))
