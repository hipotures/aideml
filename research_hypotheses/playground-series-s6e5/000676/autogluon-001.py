...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_wet_compound"] = compound.isin(wet_compounds).astype("int8")
    df["is_slick_compound"] = compound.isin(slick_compounds).astype("int8")
    df["is_testing_event"] = (
        df["Race"]
        .astype(str)
        .str.contains("Testing", case=False, na=False)
        .astype("int8")
    )

    df["lap_progress_left"] = 1.0 - df["RaceProgress"].astype(float)
    df["stint_progress"] = df["TyreLife"].astype(float) / df["LapNumber"].clip(
        lower=1
    ).astype(float)
    df["degradation_per_tyre_lap"] = df["Cumulative_Degradation"].astype(float) / df[
        "TyreLife"
    ].clip(lower=1).astype(float)
    df["degradation_per_race_lap"] = df["Cumulative_Degradation"].astype(float) / df[
        "LapNumber"
    ].clip(lower=1).astype(float)

    df["abs_lap_delta"] = df["LapTime_Delta"].abs()
    df["position_pressure"] = (21 - df["Position"].astype(float)) / 20.0
    df["stint_x_degradation"] = (
        df["TyreLife"].astype(float) * df["degradation_per_tyre_lap"]
    )
    df["late_race_x_tyre"] = df["lap_progress_left"] * df["TyreLife"].astype(float)

    df["wet_tyre_life"] = df["is_wet_compound"] * df["TyreLife"].astype(float)
    df["wet_abs_lap_delta"] = df["is_wet_compound"] * df["abs_lap_delta"]
    df["slick_degradation"] = df["is_slick_compound"] * df["degradation_per_tyre_lap"]
    df["testing_abs_lap_delta"] = df["is_testing_event"] * df["abs_lap_delta"]
    ...
    for col in event_cols:
        grp = df.groupby("Race_Year", sort=False)[col]
        mean = grp.transform("mean")
        std = grp.transform("std").replace(0, np.nan)
        df[f"{col}_event_z"] = ((df[col] - mean) / std).fillna(0.0).clip(-8, 8)

    df["caution_like_regime"] = (
        (df["abs_lap_delta"] > 12).astype("int8")
        | (df["LapTime (s)_event_z"].abs() > 2.5).astype("int8")
        | (df["is_wet_compound"] == 1)
    ).astype("int8")

    df["expert_gate_wet"] = df["is_wet_compound"]
    df["expert_gate_caution"] = df["caution_like_regime"]
    df["expert_gate_green_slick"] = (
        (df["is_slick_compound"] == 1) & (df["caution_like_regime"] == 0)
    ).astype("int8")

    for col in ["Driver", "Race", "Compound", "Race_Year"]:
        freq = df[col].astype(str).value_counts(dropna=False)
        df[f"{col}_freq"] = df[col].astype(str).map(freq).astype(float)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000676.",
    }, sort_keys=True))
