...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Explicit categorical interaction tokens to help models capture strategy context.
    ...
    out["Year_Race"] = out["Year"] + "__" + out["Race"]
    out["Year_Compound"] = out["Year"] + "__" + out["Compound"]
    out["Driver_Race_Compound"] = (
        out["Driver"] + "__" + out["Race"] + "__" + out["Compound"]
    )
    out["Stint_Compound"] = out["Stint"].astype(str) + "__" + out["Compound"]
    out["Driver_Stint"] = out["Driver"] + "__" + out["Stint"].astype(str)

    eps = 1e-6

    # Simple numeric features linked to pit-stop timing.
    out["TyreLife_by_Lap"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["TyreLife_by_Progress"] = out["TyreLife"] / out["RaceProgress"].clip(lower=eps)
    out["Degradation_per_TyreLife"] = out["Cumulative_Degradation"] / out[
        "TyreLife"
    ].clip(lower=1)
    out["Degradation_per_Lap"] = out["Cumulative_Degradation"] / out["LapNumber"].clip(
        lower=1
    )
    ...
    out["Stint_progress"] = out["TyreLife"] / out["Stint"].clip(lower=1)
    out["Late_race_tyre"] = out["TyreLife"] * out["RaceProgress"]
    out["Late_race_deg"] = out["Cumulative_Degradation"] * out["RaceProgress"]
    out["LapTime_x_TyreLife"] = out["LapTime (s)"] * out["TyreLife"]
    out["LapDelta_x_Deg"] = out["LapTime_Delta"] * out["Cumulative_Degradation"]

    # Deterministic threshold-style flags that often align with pit windows.
    out["Is_Long_Stint"] = (out["TyreLife"] >= 15).astype("int8")
    out["Is_Very_Long_Stint"] = (out["TyreLife"] >= 25).astype("int8")
    out["Is_Late_Race"] = (out["RaceProgress"] >= 0.7).astype("int8")
    out["Is_Big_LapDelta"] = (out["LapTime_delta_abs"] >= 1.5).astype("int8")
    out["Is_Big_Degradation"] = (
        out["Cumulative_Degradation"] >= out["Cumulative_Degradation"].median()
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000234.",
    }, sort_keys=True))
