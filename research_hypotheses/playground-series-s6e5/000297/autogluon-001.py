...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = [c for c in ["Compound", "Driver", "Race"] if c in X.columns]
    for c in cat_cols:
        X[c] = X[c].astype("string").fillna("UNK")

    grp_cols = ["Year", "Race", "Driver"]
    sort_cols = grp_cols + ["LapNumber", "Stint", "TyreLife", "_row_id"]
    X = X.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)
    g = X.groupby(grp_cols, sort=False)

    lap_num = X["LapNumber"].astype("float32")
    tyre_life = X["TyreLife"].astype("float32")
    race_prog = X["RaceProgress"].astype("float32").replace(0, np.nan)
    ...
    race_laps_est = (lap_num / race_prog).round()
    race_laps_est = race_laps_est.replace([np.inf, -np.inf], np.nan)
    race_laps_est = race_laps_est.fillna(
        g["LapNumber"].transform("max").astype("float32")
    )
    race_laps_est = np.maximum(
        race_laps_est.to_numpy(dtype=np.float32), lap_num.to_numpy(dtype=np.float32)
    )
    X["Race_Laps_Est"] = race_laps_est.astype("float32")
    X["Laps_Remaining"] = np.clip(X["Race_Laps_Est"] - lap_num, 0, None).astype(
        "float32"
    )
    X["Race_Progress_Left"] = (
        (1.0 - X["RaceProgress"].astype("float32")).clip(0, 1).astype("float32")
    )

    X["TyreLife_Ratio"] = (
        (tyre_life / X["Race_Laps_Est"].replace(0, np.nan)).fillna(0).astype("float32")
    )
    X["Degradation_Per_Lap"] = (
        (degr / tyre_life.replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )
    X["LapDelta_Per_TyreLap"] = (
        (lap_delta / tyre_life.replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )
    X["LapTime_Per_Lap"] = (
        (lap_time / lap_num.replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )

    X["Prev_PitStop"] = g["PitStop"].shift(1).fillna(0).astype("float32")
    X["Prev_LapTime_Delta"] = g["LapTime_Delta"].shift(1).fillna(0).astype("float32")
    X["Prev_Position_Change"] = (
        g["Position_Change"].shift(1).fillna(0).astype("float32")
    )
    X["Prev_Position"] = g["Position"].shift(1).fillna(pos).astype("float32")
    X["Prev_TyreLife"] = g["TyreLife"].shift(1).fillna(0).astype("float32")

    X["Pit_Count_Before"] = (
        (g["PitStop"].cumsum() - pit_now).clip(lower=0).astype("float32")
    )
    X["_pit_lap_marker"] = lap_num.where(pit_now.eq(1))
    X["_last_pit_seen"] = g["_pit_lap_marker"].ffill()
    prev_last_pit = g["_last_pit_seen"].shift(1)
    X["Laps_Since_Last_Pit"] = (
        (lap_num - prev_last_pit.fillna(0)).clip(lower=0).astype("float32")
    )
    X["TyreLife_Minus_LapsSincePit"] = (tyre_life - X["Laps_Since_Last_Pit"]).astype(
        "float32"
    )

    X["Position_Trend"] = (pos - X["Prev_Position"]).astype("float32")
    X["Abs_LapTime_Delta"] = lap_delta.abs().astype("float32")
    X["Abs_Position_Change"] = pos_chg.abs().astype("float32")

    X["Stint_Pressure"] = (tyre_life * X["Degradation_Per_Lap"]).astype("float32")
    X["Late_Stint_Pressure"] = (
        X["Laps_Since_Last_Pit"] * X["Degradation_Per_Lap"]
    ).astype("float32")
    X["RaceEnd_Pressure"] = (tyre_life / (X["Laps_Remaining"] + 1.0)).astype("float32")
    X["Delta_x_Remaining"] = (lap_delta * X["Laps_Remaining"]).astype("float32")
    X["Stint_x_Progress"] = (stint * X["RaceProgress"].astype("float32")).astype(
        "float32"
    )

    compound = X["Compound"]
    X["Is_Slick"] = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["Is_WetLike"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    X["Is_Soft"] = compound.eq("SOFT").astype("int8")
    X["Is_Medium"] = compound.eq("MEDIUM").astype("int8")
    X["Is_Hard"] = compound.eq("HARD").astype("int8")

    for c in ["Compound", "Driver", "Race"]:
        if c in X.columns:
            X[c] = X[c].astype("category")

    X = X.sort_values("_row_id", kind="mergesort").drop(
        columns=["_row_id", "_pit_lap_marker", "_last_pit_seen"]
    )
    return X
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000297.",
    }, sort_keys=True))
