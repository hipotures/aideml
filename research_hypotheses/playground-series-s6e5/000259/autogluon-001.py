...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["tyre_frac"] = out["TyreLife"] / out.groupby(group_cols)["TyreLife"].transform(
        "max"
    ).clip(lower=1)
    out["stint_lap_pos"] = out.groupby(group_cols).cumcount() + 1
    out["stint_len_seen"] = out.groupby(group_cols)["LapNumber"].transform("count")
    out["stint_pos_frac"] = out["stint_lap_pos"] / out["stint_len_seen"].clip(lower=1)
    ...
    for c in [
        ...
        r = g.rank(method="average", pct=True)
        out[f"{c}_grp_pct_rank"] = r.astype("float32")
        out[f"{c}_grp_centered"] = (out[c] - g.transform("mean")).astype("float32")

    out["deg_per_tyre_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["lap_delta_abs"] = out["LapTime_Delta"].abs()
    out["position_loss"] = out["Position_Change"].clip(lower=0)
    out["urgency_proxy"] = (
        0.40 * out["TyreLife_grp_pct_rank"]
        + 0.25 * out["RaceProgress_grp_pct_rank"]
        + 0.20 * out["Cumulative_Degradation_grp_pct_rank"]
        + 0.10 * out["LapTime_Delta_grp_pct_rank"]
        + 0.05 * out["position_loss"] / 18.0
    ).astype("float32")

    out["candidate_lap_000259"] = (
        (out["tyre_frac"] >= 0.55)
        | (out["TyreLife"] >= 15)
        | (out["urgency_proxy"] >= 0.65)
    ).astype("int8")
    out["candidate_rank_000259"] = (
        out.groupby(group_cols)["urgency_proxy"]
        .rank(method="average", pct=True)
        .astype("float32")
    )
    out["candidate_local_score_000259"] = (
        out["candidate_lap_000259"] * out["candidate_rank_000259"]
    ).astype("float32")
    ...
    out["compound_tyre_life"] = (
        ...
        + pd.cut(
            out["TyreLife"],
            bins=[0, 8, 15, 25, 40, 100],
            labels=False,
            include_lowest=True,
        ).astype(str)
    )
    out["compound_tyre_life"] = out["compound_tyre_life"].astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000259"],
        "research_usage_note": "Verified assigned hypothesis 000259.",
    }, sort_keys=True))
