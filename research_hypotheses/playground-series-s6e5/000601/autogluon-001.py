...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["race_lap_qid"] = pd.factorize(race_key, sort=False)[0].astype("int32")
    ...
    out["degr_per_tyre_lap"] = degr / (tyre + 1.0)
    out["abs_laptime_delta"] = lap_delta.abs()
    out["position_pressure"] = pos + pos_chg.abs()

    base_score = (
        0.42 * tyre
        + 0.30 * out["degr_per_tyre_lap"]
        + 0.16 * progress * 77.0
        + 0.08 * stint * 10.0
        + 0.04 * out["abs_laptime_delta"].clip(0, 120)
    )
    dry_score = (
        base_score
        + out["is_soft"] * 5.0
    ...
    wet_score = base_score + out["is_wet_compound"] * (
        8.0 + out["abs_laptime_delta"].clip(0, 180) * 0.05
    )
    out["pit_rank_candidate_score"] = np.where(
        out["is_wet_compound"].eq(1), wet_score, dry_score
    ).astype("float32")

    grp = out.groupby("race_lap_qid", sort=False)
    size = grp["race_lap_qid"].transform("size").astype(float).replace(1.0, np.nan)

    for col in [
        "pit_rank_candidate_score",
        "TyreLife",
        ...
        out[f"{col}_race_lap_rank_pct"] = (
            ((r - 1.0) / size).fillna(0.0).astype("float32")
        )

    out["race_lap_field_size"] = grp["race_lap_qid"].transform("size").astype("int16")
    out["race_lap_score_gap_to_best"] = (
        grp["pit_rank_candidate_score"].transform("max")
        - out["pit_rank_candidate_score"]
    ).astype("float32")
    out["race_lap_score_z"] = (
        (
            (
                out["pit_rank_candidate_score"]
                - grp["pit_rank_candidate_score"].transform("mean")
            )
            / (
                grp["pit_rank_candidate_score"].transform("std").replace(0, np.nan)
                + 1e-6
            )
        )
        .fillna(0.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000601"],
        "research_usage_note": "Verified assigned hypothesis 000601.",
    }, sort_keys=True))
