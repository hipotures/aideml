...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_testing = (race == "Pre-Season Testing").astype(np.int8)
    is_wet = (compound.isin(["WET", "INTERMEDIATE"]) & (is_testing == 0)).astype(
        np.int8
    )
    is_dry = (compound.isin(["SOFT", "MEDIUM", "HARD"]) & (is_testing == 0)).astype(
        np.int8
    )

    df["regime_testing"] = is_testing
    df["regime_wet"] = is_wet
    df["regime_dry"] = is_dry

    df["compound_is_soft"] = (compound == "SOFT").astype(np.int8)
    df["compound_is_medium"] = (compound == "MEDIUM").astype(np.int8)
    df["compound_is_hard"] = (compound == "HARD").astype(np.int8)
    df["compound_is_intermediate"] = (compound == "INTERMEDIATE").astype(np.int8)
    df["compound_is_wet"] = (compound == "WET").astype(np.int8)

    eps = 1e-6
    tyre_life = df["TyreLife"].astype(float)
    lap_num = df["LapNumber"].astype(float)
    degr = df["Cumulative_Degradation"].astype(float)
    lap_time = df["LapTime (s)"].astype(float)
    lap_delta = df["LapTime_Delta"].astype(float)
    race_prog = df["RaceProgress"].astype(float)
    pos = df["Position"].astype(float)
    pos_chg = df["Position_Change"].astype(float)
    stint = df["Stint"].astype(float)
    pit_prev = df["PitStop"].astype(float)

    df["degradation_per_lap"] = degr / (tyre_life + eps)
    df["tyre_life_frac"] = tyre_life / (lap_num + eps)
    df["lap_time_delta_ratio"] = lap_delta / (lap_time + eps)
    df["position_pressure"] = pos * race_prog
    df["stint_progress_proxy"] = tyre_life / (
        tyre_life + (lap_num - tyre_life).clip(lower=0) + eps
    )
    df["fresh_tyre_flag"] = (tyre_life <= 2).astype(np.int8)
    df["late_race_flag"] = (race_prog >= 0.8).astype(np.int8)

    grp_driver_race = ["Year", "Race", "Driver"]
    grp_driver_race_stint = ["Year", "Race", "Driver", "Stint"]
    ...
    for col in [
        ...
        mean_dr = s.groupby([df[c] for c in grp_driver_race]).transform("mean")
        std_dr = (
            s.groupby([df[c] for c in grp_driver_race]).transform("std").fillna(0.0)
        )
        df[f"{col}_drv_race_z"] = (s - mean_dr) / (std_dr + eps)

    max_tyre_stint = tyre_life.groupby(
        [df[c] for c in grp_driver_race_stint]
    ).transform("max")
    max_lap_stint = lap_num.groupby([df[c] for c in grp_driver_race_stint]).transform(
        "max"
    )
    df["tyre_life_vs_stint_max"] = tyre_life / (max_tyre_stint + eps)
    df["lap_vs_stint_max"] = lap_num / (max_lap_stint + eps)

    dry_core = {
        "dry_tyre_life": tyre_life,
        "dry_degradation": degr,
        "dry_deg_per_lap": df["degradation_per_lap"],
        "dry_position": pos,
        "dry_position_pressure": df["position_pressure"],
        "dry_race_progress": race_prog,
        "dry_stint": stint,
        "dry_prev_pit": pit_prev,
    }
    wet_core = {
        "wet_lap_delta": lap_delta,
        "wet_lap_time": lap_time,
        "wet_delta_ratio": df["lap_time_delta_ratio"],
        "wet_position_change": pos_chg,
        "wet_position": pos,
        "wet_race_progress": race_prog,
        "wet_prev_pit": pit_prev,
    }
    testing_core = {
        "test_tyre_life": tyre_life,
        "test_degradation": degr,
        "test_lap_time": lap_time,
        "test_lap_delta": lap_delta,
        "test_position_change": pos_chg,
        "test_prev_pit": pit_prev,
    }

    for name, values in dry_core.items():
        df[name] = values * is_dry
    for name, values in wet_core.items():
        df[name] = values * is_wet
    for name, values in testing_core.items():
        df[name] = values * is_testing

    df["dry_soft_gate"] = is_dry * df["compound_is_soft"]
    df["dry_medium_gate"] = is_dry * df["compound_is_medium"]
    df["dry_hard_gate"] = is_dry * df["compound_is_hard"]
    df["wet_intermediate_gate"] = is_wet * df["compound_is_intermediate"]
    df["wet_fullwet_gate"] = is_wet * df["compound_is_wet"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000895"],
        "research_usage_note": "Verified assigned hypothesis 000895.",
    }, sort_keys=True))
