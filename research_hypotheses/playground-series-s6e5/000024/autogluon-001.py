...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    soft_rank = {"SOFT": 3, "MEDIUM": 2, "HARD": 1}
    ...
    race_max_lap = (
        out.groupby(race_cols, sort=False)["LapNumber"].transform("max").astype(float)
    )
    laps_remaining_after_next = (race_max_lap - lap).clip(lower=0.0)

    median_slick_life = (
        out.loc[compound.isin(slicks)]
        .assign(_compound=compound[compound.isin(slicks)])
        .groupby(["Year", "Race", "_compound"], sort=False)["TyreLife"]
        .median()
    )
    global_slick_life = (
        out.loc[compound.isin(slicks)]
        .assign(_compound=compound[compound.isin(slicks)])
        .groupby("_compound", sort=False)["TyreLife"]
        .median()
    )

    tmp = out[group_cols + ["LapNumber"]].copy()
    tmp["_orig_order"] = np.arange(len(out))
    tmp["_compound"] = compound
    tmp = tmp.sort_values(group_cols + ["LapNumber", "_orig_order"], kind="mergesort")

    unseen_counts = np.zeros(len(out), dtype=np.int8)
    softest_unseen = np.zeros(len(out), dtype=np.int8)
    unseen_can_finish = np.zeros(len(out), dtype=np.int8)

    for s in slicks:
        seen_col = "_seen_" + s
        tmp[seen_col] = (
            (tmp["_compound"].eq(s))
            .groupby([tmp[c] for c in group_cols], sort=False)
            .cummax()
        )
        seen_current_or_prior = tmp.sort_values("_orig_order", kind="mergesort")[
            seen_col
        ].to_numpy(dtype=bool)

        med_map = (
            median_slick_life.xs(s, level="_compound", drop_level=False)
            if s in median_slick_life.index.get_level_values("_compound")
            else pd.Series(dtype=float)
        )
        finish_cap = (
            out.set_index(race_cols)
            .index.map(med_map.droplevel("_compound"))
            .to_numpy(dtype=float)
        )
        fallback = float(global_slick_life.get(s, tyre_life.median()))
        finish_cap = np.where(np.isnan(finish_cap), fallback, finish_cap)

        viable = finish_cap >= laps_remaining_after_next.to_numpy()
        unseen_viable = viable & (~seen_current_or_prior)

        unseen_counts += unseen_viable.astype(np.int8)
        softest_unseen = np.maximum(
            softest_unseen, unseen_viable.astype(np.int8) * soft_rank[s]
        )
        unseen_can_finish |= unseen_viable.astype(np.int8)

    out["unseen_viable_slick_count"] = unseen_counts
    out["softest_unseen_viable_slick"] = softest_unseen
    out["unseen_different_slick_can_finish"] = unseen_can_finish.astype(np.int8)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000024"],
        "research_usage_note": "Verified assigned hypothesis 000024.",
    }, sort_keys=True))
