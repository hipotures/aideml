...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Race", "Driver", "Compound"]:
        out[c] = out[c].astype("string").fillna("UNK").str.upper().str.strip()

    out["orig_row_order"] = row_order
    out["race_key"] = out["Year"].astype(str) + "_" + out["Race"].astype(str)
    out["driver_race_key"] = (
        out["race_key"].astype(str) + "_" + out["Driver"].astype(str)
    )

    sort_cols = ["Year", "Race", "LapNumber", "Position", "Driver", "orig_row_order"]
    sidx = out.sort_values(sort_cols, kind="mergesort").index
    sorted_pos = pd.Series(np.arange(n), index=sidx).reindex(out.index).astype("int64")
    out["chrono_rank"] = sorted_pos
    out["chrono_frac"] = sorted_pos / max(n - 1, 1)

    n_folds = 5
    fold = np.floor(out["chrono_frac"] * n_folds).clip(0, n_folds - 1).astype("int8")
    out["purged_walk_forward_fold_000694"] = fold
    boundary_dist = np.minimum(
        np.mod(sorted_pos, max(n // n_folds, 1)),
        max(n // n_folds, 1) - np.mod(sorted_pos, max(n // n_folds, 1)),
    )
    out["embargo_boundary_flag_000694"] = (boundary_dist <= 2).astype("int8")

    out["lap_remaining"] = (
        (1.0 - out["RaceProgress"]) * out["LapNumber"] / out["RaceProgress"].clip(0.01)
    )
    out["tyre_life_frac_race"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["stint_lap_ratio"] = out["TyreLife"] / (out["Stint"] + 1)
    ...
    out["stint_progress_pressure"] = out["Stint"] * out["RaceProgress"]
    out["pit_recent_risk"] = ((out["PitStop"] == 1) | (out["TyreLife"] <= 2)).astype(
        "int8"
    )

    dry = out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    wet = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["is_dry_compound"] = dry
    out["is_wet_compound"] = wet

    compound_order = {"SOFT": 1, "MEDIUM": 2, "HARD": 3, "INTERMEDIATE": 4, "WET": 5}
    out["compound_code_ord"] = (
        out["Compound"].map(compound_order).fillna(0).astype("int8")
    )
    out["compound_tyrelife"] = out["compound_code_ord"] * out["TyreLife"]

    for c in ["Race", "Driver", "Compound", "race_key", "driver_race_key"]:
        freq = out[c].map(out[c].value_counts(dropna=False)).astype("float32")
        out[c + "_freq"] = freq
        out[c + "_logfreq"] = np.log1p(freq)

    grp = out.groupby("race_key", sort=False)
    for c in [
        "LapTime (s)",
        "LapTime_Delta",
        ...
        out[c.replace(" ", "_").replace("(", "").replace(")", "") + "_race_z"] = (
            (out[c] - mean) / std
        ).fillna(0)

    comp_grp = out.groupby(["Year", "Race", "Compound"], sort=False)
    out["compound_laptime_mean"] = comp_grp["LapTime (s)"].transform("mean")
    out["compound_deg_mean"] = comp_grp["Cumulative_Degradation"].transform("mean")
    out["laptime_vs_compound"] = out["LapTime (s)"] - out["compound_laptime_mean"]
    out["deg_vs_compound"] = out["Cumulative_Degradation"] - out["compound_deg_mean"]

    out = out.drop(columns=["race_key", "driver_race_key"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000694.",
    }, sort_keys=True))
