...
for c in cat_features:
    cats = pd.Index(
        pd.concat([train_fe[c], test_fe[c]], axis=0)
        .fillna(MISSING)
        .astype(str)
        .unique()
    )
    train_fe[c] = pd.Categorical(
        train_fe[c].fillna(MISSING).astype(str), categories=cats
    )
    test_fe[c] = pd.Categorical(test_fe[c].fillna(MISSING).astype(str), categories=cats)
...
def make_logistic(cols):
    local_cat = [c for c in cols if not is_numeric_dtype(train_fe[c])]
...
def fit_predict_model(model_name, tr_idx, va_idx=None, fit_all=False):
    ...
    try:
        if model_name == "lgbm":
            ...
            te_pred = (
                model.predict_proba(test_fe[feature_cols])[:, 1] if fit_all else None
            )
        ...
        if model_name == "cat":
            ...
            xtr = as_catboost_frame(train_fe.iloc[tr_idx][feature_cols])
            model.fit(xtr, y[tr_idx], cat_features=cat_idx)
            va_pred = (
                model.predict_proba(
                    as_catboost_frame(train_fe.iloc[va_idx][feature_cols])
                )[:, 1]
                if va_idx is not None
                else None
            )
            te_pred = (
                model.predict_proba(as_catboost_frame(test_fe[feature_cols]))[:, 1]
                if fit_all
                else None
            )
            return va_pred, te_pred
        ...
        model = make_logistic(horizon_cols)
...
def safe_auc(labels, preds):
    return roc_auc_score(labels, preds) if len(np.unique(labels)) == 2 else np.nan


sort_cols = ["Year", "Race_ord", "LapNumber", ID_COL]
order = train_fe.sort_values(sort_cols, kind="mergesort").index.to_numpy()
fold_blocks = np.array_split(order, N_SPLITS + 1)
validation_blocks = fold_blocks[1:]

model_names = ["lgbm", "cat", "horizon_logit"]
oof_base = np.full((len(train_fe), len(model_names)), np.nan, dtype=np.float32)

for fold, va_idx in enumerate(validation_blocks, 1):
    ...
    fold_prior = float(y[tr_idx].mean()) if len(tr_idx) else global_prior
    ...
    for m, name in enumerate(model_names):
        ...
        if pred is None:
            ...
            pred = np.full(len(va_idx), fold_prior, dtype=float)
        oof_base[va_idx, m] = np.clip(pred, 1e-6, 1 - 1e-6)
        print(f"  {name} fold AUC: {safe_auc(y[va_idx], oof_base[va_idx, m]):.6f}")
...
for m, name in enumerate(model_names):
    ...
    if te_pred is None:
        te_pred = np.full(len(test_fe), global_prior, dtype=float)
...
if ID_COL in test.columns and not np.array_equal(
    test[ID_COL].values, sample[ID_COL].values
):
    pred_series = pd.Series(test_pred, index=test[ID_COL].values)
    aligned_pred = (
        pred_series.reindex(sample[ID_COL].values).fillna(global_prior).values
    )
else:
    aligned_pred = test_pred

submission = pd.DataFrame({ID_COL: sample[ID_COL].values, TARGET: aligned_pred})
...
review = {
    ...
    "cv_scheme": "5 purged walk-forward validation folds after an initial chronological warm-up block with sigmoid logistic OOF stacker",
}
