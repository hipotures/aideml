...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in num_cols:
        ...
        X[c] = s.fillna(s.median()).astype("float32")

    for c in cat_cols:
        s = out[c].astype("string").fillna("__NA__")
        codes, _ = pd.factorize(s, sort=True)
        freq = s.map(s.value_counts(dropna=False)).astype("float32") / max(n, 1)
        X[c + "_code"] = codes.astype("int32")
        X[c + "_freq"] = freq.astype("float32")
        out[c] = s.astype("category")

    if n_train > 0 and n > n_train:
        y_adv = np.zeros(n, dtype=np.uint8)
        y_adv[n_train:] = 1

        rng = np.random.default_rng(564)
        m = min(90000, n_train, n - n_train)
        train_idx = rng.choice(np.arange(n_train), size=m, replace=False)
        test_idx = rng.choice(np.arange(n_train, n), size=m, replace=False)
        fit_idx = np.concatenate([train_idx, test_idx])
        ...
        clf.fit(X.iloc[fit_idx], y_adv[fit_idx])
        p = clf.predict_proba(X)[:, 1].astype("float32")
        p = np.clip(p, 0.01, 0.99)

        odds = p / (1.0 - p)
        weight = np.clip(odds, 0.25, 4.0).astype("float32")
        train_mean = float(np.mean(weight[:n_train]))
        if train_mean > 0:
            weight = weight / train_mean

        out["av_test_likeness"] = p
        out["sample_weight"] = weight.astype("float32")
    else:
        out["av_test_likeness"] = np.float32(0.5)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000564"],
        "research_usage_note": "Verified assigned hypothesis 000564.",
    }, sort_keys=True))
