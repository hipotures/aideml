...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.sort_values(
        group_cols + ["Position", "Driver"], kind="mergesort"
    ).reset_index(drop=True)

    g = df.groupby(group_cols, sort=False)

    df["field_size"] = g["Driver"].transform("size").astype("float32")
    df["position_pct"] = (df["Position"] / df["field_size"]).astype("float32")
    df["tyrelife_field_rank"] = (
        g["TyreLife"].rank(method="average", pct=True).astype("float32")
    )
    df["degradation_field_rank"] = (
        g["Cumulative_Degradation"].rank(method="average", pct=True).astype("float32")
    )

    for k in (1, 2, 3):
        df[f"ahead_tyrelife_{k}"] = g["TyreLife"].shift(k)
        df[f"behind_tyrelife_{k}"] = g["TyreLife"].shift(-k)
        df[f"ahead_deg_{k}"] = g["Cumulative_Degradation"].shift(k)
        df[f"behind_deg_{k}"] = g["Cumulative_Degradation"].shift(-k)
        df[f"ahead_compound_{k}"] = g["Compound"].shift(k)
        df[f"behind_compound_{k}"] = g["Compound"].shift(-k)

    ahead_tyre_cols = [f"ahead_tyrelife_{k}" for k in (1, 2, 3)]
    behind_tyre_cols = [f"behind_tyrelife_{k}" for k in (1, 2, 3)]
    ahead_deg_cols = [f"ahead_deg_{k}" for k in (1, 2, 3)]
    behind_deg_cols = [f"behind_deg_{k}" for k in (1, 2, 3)]

    df["ahead_tyrelife_mean3"] = df[ahead_tyre_cols].mean(axis=1).astype("float32")
    df["ahead_tyrelife_min3"] = df[ahead_tyre_cols].min(axis=1).astype("float32")
    df["behind_tyrelife_mean3"] = df[behind_tyre_cols].mean(axis=1).astype("float32")
    df["behind_tyrelife_min3"] = df[behind_tyre_cols].min(axis=1).astype("float32")

    df["ahead_deg_mean3"] = df[ahead_deg_cols].mean(axis=1).astype("float32")
    df["ahead_deg_min3"] = df[ahead_deg_cols].min(axis=1).astype("float32")
    df["behind_deg_mean3"] = df[behind_deg_cols].mean(axis=1).astype("float32")
    df["behind_deg_min3"] = df[behind_deg_cols].min(axis=1).astype("float32")

    df["compound_mismatch_ahead"] = (
        (df["Compound"] != df["ahead_compound_1"]).fillna(False).astype("int8")
    )
    df["compound_mismatch_behind"] = (
        (df["Compound"] != df["behind_compound_1"]).fillna(False).astype("int8")
    )

    df["pit_window_flag"] = (
        (df["tyrelife_field_rank"] >= 0.75) | (df["degradation_field_rank"] >= 0.75)
    ).astype("int8")

    df["pit_window_ahead_1"] = g["pit_window_flag"].shift(1).fillna(0)
    df["pit_window_ahead_2"] = g["pit_window_flag"].shift(2).fillna(0)
    df["pit_window_ahead_3"] = g["pit_window_flag"].shift(3).fillna(0)
    df["pit_window_behind_1"] = g["pit_window_flag"].shift(-1).fillna(0)
    df["pit_window_behind_2"] = g["pit_window_flag"].shift(-2).fillna(0)
    df["pit_window_behind_3"] = g["pit_window_flag"].shift(-3).fillna(0)

    df["nearby_pit_pressure"] = (
        df["pit_window_ahead_1"]
        + df["pit_window_ahead_2"]
        + df["pit_window_ahead_3"]
        + df["pit_window_behind_1"]
        + df["pit_window_behind_2"]
        + df["pit_window_behind_3"]
    ).astype("float32")

    df["ahead_undercut_pressure"] = (
        (df["TyreLife"] - df["ahead_tyrelife_mean3"]).fillna(0)
        + (df["Cumulative_Degradation"] - df["ahead_deg_mean3"]).fillna(0) / 100.0
    ).astype("float32")
    df["behind_attack_pressure"] = (
        (df["TyreLife"] - df["behind_tyrelife_mean3"]).fillna(0)
        + (df["Cumulative_Degradation"] - df["behind_deg_mean3"]).fillna(0) / 100.0
    ).astype("float32")

    df["position_x_pressure"] = (df["Position"] * df["nearby_pit_pressure"]).astype(
        "float32"
    )
    df["progress_x_pressure"] = (df["RaceProgress"] * df["nearby_pit_pressure"]).astype(
        "float32"
    )
    df["position_x_tyre_rank"] = (df["Position"] * df["tyrelife_field_rank"]).astype(
        "float32"
    )
    df["progress_x_deg_rank"] = (
        df["RaceProgress"] * df["degradation_field_rank"]
    ).astype("float32")
    ...
    df = df.sort_values("_row_order", kind="mergesort").drop(columns=drop_cols)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000930"],
        "research_usage_note": "Verified assigned hypothesis 000930.",
    }, sort_keys=True))
