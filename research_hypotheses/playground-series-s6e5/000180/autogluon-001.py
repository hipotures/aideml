...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_code"] = compound.cat.codes.astype("int8")

    wetish = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["WetCompound"] = wetish
    out["SoftCompound"] = (out["Compound"] == "SOFT").astype("int8")
    out["HardCompound"] = (out["Compound"] == "HARD").astype("int8")
    ...
    out["LapFrac_x_Stint"] = out["RaceProgress"] * out["Stint"]
    out["LateRace"] = (out["RaceProgress"] >= 0.70).astype("int8")
    out["OldTyre"] = (out["TyreLife"] >= 20).astype("int8")
    out["PitWindowFlag"] = (
        (out["TyreLife"] >= 12) & (out["RaceProgress"].between(0.15, 0.90))
    ).astype("int8")

    g = [out["Race"], out["Year"], out["LapNumber"]]
    for col in [
        "TyreLife",
        ...
        grp = out.groupby(g, sort=False)[col]
        mean = grp.transform("mean")
        std = grp.transform("std").replace(0, np.nan)
        out[f"{col}_lap_rank_pct"] = grp.rank(method="average", pct=True).astype(
            "float32"
        )
        out[f"{col}_lap_diff"] = (out[col] - mean).astype("float32")
        out[f"{col}_lap_z"] = ((out[col] - mean) / std).fillna(0).astype("float32")

    cg = [out["Race"], out["Year"], out["LapNumber"], out["Compound"]]
    out["CompoundLapCount"] = (
        out.groupby(cg, sort=False)["LapNumber"].transform("size").astype("int16")
    )
    out["TyreLife_compound_lap_rank_pct"] = (
        out.groupby(cg, sort=False)["TyreLife"]
        .rank(method="average", pct=True)
        .astype("float32")
    )
    out["Deg_compound_lap_rank_pct"] = (
        out.groupby(cg, sort=False)["Cumulative_Degradation"]
        .rank(method="average", pct=True)
        .astype("float32")
    )

    out["LapGroupSize"] = (
        out.groupby(g, sort=False)["LapNumber"].transform("size").astype("int16")
    )
    out["RaceYearLapKey"] = (
        out["Race_code"].astype("int32") * 100000
        + out["Year"].astype("int32") * 100
        + out["LapNumber"].astype("int32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000180"],
        "research_usage_note": "Verified assigned hypothesis 000180.",
    }, sort_keys=True))
