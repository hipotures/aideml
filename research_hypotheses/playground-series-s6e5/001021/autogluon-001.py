...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound_clean = out["Compound"].astype(str).str.upper().fillna("UNKNOWN")
    lap_time = (
        pd.to_numeric(out["LapTime (s)"], errors="coerce").astype(float).to_numpy()
    )
    tyre_life = pd.to_numeric(out["TyreLife"], errors="coerce").astype(float).to_numpy()
    stint = pd.to_numeric(out["Stint"], errors="coerce").astype(float).to_numpy()
    pit_now = (
        pd.to_numeric(out["PitStop"], errors="coerce").fillna(0).astype(int).to_numpy()
    )

    valid_delta = (
        pd.to_numeric(out["LapTime_Delta"], errors="coerce")
        .where((out["TyreLife"] > 1) & (out["PitStop"] == 0))
        .where(lambda s: s.abs() <= 15)
    )
    base_slope_series = valid_delta.groupby(compound_clean).median()
    default_base = (
        float(valid_delta.median()) if pd.notna(valid_delta.median()) else 0.15
    )
    base_slope_map = {k: float(v) for k, v in base_slope_series.dropna().items()}
    for k, v in {
        "SOFT": 0.18,
        "MEDIUM": 0.14,
        "HARD": 0.11,
        ...
        base_slope_map.setdefault(k, v if np.isfinite(v) else default_base)
    ...
    work = out[order_cols].copy()
    work["_row_id"] = np.arange(len(out))
    work["_compound"] = compound_clean.to_numpy()
    work["_lap_time"] = lap_time
    work["_tyre_life"] = tyre_life
    work["_stint"] = stint
    work["_pit_now"] = pit_now

    work = work.sort_values(order_cols + ["_row_id"], kind="mergesort").reset_index(
        drop=True
    )

    n = len(work)
    latent_pace = np.empty(n, dtype=np.float32)
    latent_slope = np.empty(n, dtype=np.float32)
    one_step_resid = np.empty(n, dtype=np.float32)
    freshness = np.empty(n, dtype=np.float32)
    ...
    for i in range(n):
        key = (work.at[i, "Year"], work.at[i, "Race"], work.at[i, "Driver"])
        comp = work.at[i, "_compound"]
        y = float(work.at[i, "_lap_time"])
        tl = float(work.at[i, "_tyre_life"])
        st = float(work.at[i, "_stint"])
        pnow = int(work.at[i, "_pit_now"])

        seq_reset = key != prev_key
        observed_reset = not seq_reset and (
            pending_reset
            or (np.isfinite(st) and np.isfinite(prev_stint) and st != prev_stint)
            or (np.isfinite(tl) and np.isfinite(prev_tyre) and tl < prev_tyre)
        )

        base_slope = float(base_slope_map.get(comp, default_base))
        scale = float(freshness_scale_map.get(comp, 14.0))

        if seq_reset or observed_reset or not np.isfinite(pace_state):
            pred = y
            resid = 0.0
            pace_state = y
            slope_state = base_slope
        else:
            dt = tl - prev_tyre if np.isfinite(tl) and np.isfinite(prev_tyre) else 1.0
            dt = float(np.clip(dt, 1.0, 3.0))
            pred = pace_state + slope_state * dt
            resid = y - pred

            k_level = 0.32
            k_slope = 0.07
            pace_state = pred + k_level * resid
            slope_state = 0.97 * slope_state + k_slope * (resid / dt)

            lower = min(-1.0, base_slope - 1.5)
            upper = max(3.0, base_slope + 2.5)
            slope_state = float(np.clip(slope_state, lower, upper))

        latent_pace[i] = np.float32(pace_state)
        latent_slope[i] = np.float32(slope_state)
        one_step_resid[i] = np.float32(resid)
        freshness[i] = np.float32(
            np.exp(-max(tl - 1.0, 0.0) / scale) if np.isfinite(tl) else 0.0
        )

        pending_reset = pnow == 1
        prev_key = key
        prev_stint = st
        prev_tyre = tl
    ...
    feature_frame = pd.DataFrame(
        {
            "_row_id": work["_row_id"].to_numpy(),
            "latent_pace_post": latent_pace,
            "latent_deg_slope": latent_slope,
            "latent_one_step_resid": one_step_resid,
            "latent_post_reset_freshness": freshness,
        }
    ).sort_values("_row_id", kind="mergesort")

    out["latent_pace_post"] = feature_frame["latent_pace_post"].to_numpy()
    out["latent_deg_slope"] = feature_frame["latent_deg_slope"].to_numpy()
    out["latent_one_step_resid"] = feature_frame["latent_one_step_resid"].to_numpy()
    out["latent_post_reset_freshness"] = feature_frame[
        "latent_post_reset_freshness"
    ].to_numpy()

    out["latent_wear_pressure"] = out["latent_deg_slope"] * pd.to_numeric(
        out["TyreLife"], errors="coerce"
    ).fillna(0).astype(np.float32)
    out["latent_resid_x_progress"] = out["latent_one_step_resid"] * pd.to_numeric(
        out["RaceProgress"], errors="coerce"
    ).fillna(0).astype(np.float32)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001021"],
        "research_usage_note": "Verified assigned hypothesis 001021.",
    }, sort_keys=True))
