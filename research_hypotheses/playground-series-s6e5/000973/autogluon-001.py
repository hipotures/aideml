...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").str.strip().str.upper()
    driver = out["Driver"].astype("string").str.strip().str.upper()
    compound = out["Compound"].astype("string").str.strip().str.upper()
    ...
    out["cv_race_year_group"] = (year + "|" + race).astype("category")
    out["cv_driver_year_group"] = (year + "|" + driver).astype("category")
    out["cv_driver_race_group"] = (driver + "|" + race).astype("category")
    out["cv_driver_race_year_group"] = (year + "|" + race + "|" + driver).astype(
        "category"
    )

    out["cv_lap_embargo_bin"] = ((lap - 1) // 3).astype("int16")
    out["cv_race_lap_embargo_group"] = (
        out["cv_race_year_group"].astype("string")
        + "|B"
        + out["cv_lap_embargo_bin"].astype("string")
    ).astype("category")
    ...
    count_cols = [
        ...
        "Compound",
        "cv_race_year_group",
        "cv_driver_year_group",
        "cv_driver_race_group",
        "cv_driver_race_year_group",
        "cv_race_lap_embargo_group",
    ]
    for col in count_cols:
        s = out[col].astype("string")
        counts = s.map(s.value_counts(dropna=False)).astype("float32")
        out[col + "_freq"] = counts
        out[col + "_rarity"] = (1.0 / counts).astype("float32")
        out[col + "_log_freq"] = np.log1p(counts).astype("float32")

    out["driver_first_year"] = (
        out.groupby("Driver", observed=True)["Year"].transform("min").astype("int16")
    )
    out["driver_year_age"] = (out["Year"] - out["driver_first_year"]).astype("int16")
    out["driver_is_first_year"] = (out["driver_year_age"] == 0).astype("int8")
    out["driver_race_year_nunique"] = (
        out.groupby("Driver", observed=True)["cv_race_year_group"]
        .transform("nunique")
        .astype("int16")
    )

    out["race_first_year"] = (
        out.groupby("Race", observed=True)["Year"].transform("min").astype("int16")
    )
    out["race_year_age"] = (out["Year"] - out["race_first_year"]).astype("int16")
    out["race_is_first_year"] = (out["race_year_age"] == 0).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000973"],
        "research_usage_note": "Verified assigned hypothesis 000973.",
    }, sort_keys=True))
