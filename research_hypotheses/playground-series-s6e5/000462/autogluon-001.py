...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_wet_group"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype(np.int8)
    ...
    def _loo(prefix, keys):
        gkeys = [out[k] if k in out.columns else k for k in keys]
        cnt = pit.groupby(gkeys, sort=False).transform("count").astype(float)
        sm = pit.groupby(gkeys, sort=False).transform("sum").astype(float)
        loo_cnt = (cnt - 1.0).clip(lower=0.0)
        loo_sum = sm - pit
        out[prefix + "_loo_pit_count"] = loo_sum.astype(np.float32)
        out[prefix + "_loo_pit_rate"] = (
            (loo_sum / loo_cnt.replace(0.0, np.nan)).fillna(0.0).astype(np.float32)
        )

    _loo("field_lap", ["Race_Year", "LapNumber"])
    _loo("compound_lap", ["Race_Year", "LapNumber", "Compound"])
    _loo("stint_lap", ["Race_Year", "LapNumber", "Stint"])
    _loo("posband_lap", ["Race_Year", "LapNumber", "position_band"])
    _loo("wetgrp_lap", ["Race_Year", "LapNumber", "is_wet_group"])

    lap_field = (
        out.groupby(["Race_Year", "LapNumber"], sort=False)["PitStop"]
        .agg(["sum", "count"])
        .rename(columns={"sum": "lap_pit_count", "count": "lap_field_count"})
        .reset_index()
        .sort_values(["Race_Year", "LapNumber"])
    )
    lap_field["lap_pit_rate"] = lap_field["lap_pit_count"] / lap_field[
        "lap_field_count"
    ].replace(0, np.nan)
    for lag in (1, 2, 3):
        lap_field[f"field_prev{lag}_pit_count"] = (
            lap_field.groupby("Race_Year", sort=False)["lap_pit_count"]
            .shift(lag)
            .fillna(0.0)
        )
        lap_field[f"field_prev{lag}_pit_rate"] = (
            lap_field.groupby("Race_Year", sort=False)["lap_pit_rate"]
            .shift(lag)
            .fillna(0.0)
        )
    lap_field["previous_lap_pit_wave_decay"] = (
        0.60 * lap_field["field_prev1_pit_rate"]
        + 0.30 * lap_field["field_prev2_pit_rate"]
        + 0.10 * lap_field["field_prev3_pit_rate"]
    )
    lap_field["large_wave_seen_before"] = (
        lap_field.groupby("Race_Year", sort=False)["lap_pit_count"]
        .shift(1)
        .fillna(0.0)
        .ge(3)
        .groupby(lap_field["Race_Year"], sort=False)
        .cummax()
        .astype(np.int8)
    )
    lap_field["first_large_pit_wave_of_race"] = (
        lap_field["lap_pit_count"].ge(3) & lap_field["large_wave_seen_before"].eq(0)
    ).astype(np.int8)

    keep = [
        "Race_Year",
        "LapNumber",
        "previous_lap_pit_wave_decay",
        "first_large_pit_wave_of_race",
    ]
    for lag in (1, 2, 3):
        keep += [f"field_prev{lag}_pit_count", f"field_prev{lag}_pit_rate"]
    out = out.merge(
        lap_field[keep], on=["Race_Year", "LapNumber"], how="left", sort=False
    )

    for name, keys in {
        "compound": ["Race_Year", "Compound"],
        "stint": ["Race_Year", "Stint"],
        "posband": ["Race_Year", "position_band"],
        "wetgrp": ["Race_Year", "is_wet_group"],
    }.items():
        agg = (
            out.groupby(keys + ["LapNumber"], sort=False)["PitStop"]
            .sum()
            .rename("pit_count")
            .reset_index()
        ...
        for lag in (1, 2, 3):
            agg[f"{name}_prev{lag}_pit_count"] = (
                agg.groupby(keys, sort=False)["pit_count"].shift(lag).fillna(0.0)
            )
        out = out.merge(
            agg[
                keys + ["LapNumber"] + [f"{name}_prev{i}_pit_count" for i in (1, 2, 3)]
            ],
            on=keys + ["LapNumber"],
            how="left",
            sort=False,
    ...
    out["driver_not_pitted_but_field_pitting"] = (
        out["PitStop"].eq(0) & out["field_lap_loo_pit_count"].gt(0)
    ).astype(np.int8)
    out["same_compound_pit_wave"] = (
        out["compound_lap_loo_pit_count"].ge(2) | out["compound_prev1_pit_count"].ge(2)
    ).astype(np.int8)
    out["field_wave_pressure"] = (
        out["field_lap_loo_pit_rate"] + out["previous_lap_pit_wave_decay"]
    ).astype(np.float32)

    return out.loc[:, out.columns].set_index(orig_index)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000462"],
        "research_usage_note": "Verified assigned hypothesis 000462.",
    }, sort_keys=True))
