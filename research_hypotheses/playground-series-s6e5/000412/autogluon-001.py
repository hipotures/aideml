...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = df["Compound"].astype("string")
    race = df["Race"].astype("string")

    is_testing = (race == "Pre-Season Testing").astype("int8")
    is_wet = compound.isin(wet_compounds).astype("int8")
    is_slick = compound.isin(slick_compounds).astype("int8")
    is_dry_core = ((is_slick == 1) & (is_testing == 0)).astype("int8")
    is_fallback = ((is_wet == 1) | (is_testing == 1)).astype("int8")

    df["is_testing"] = is_testing
    df["is_wet_compound"] = is_wet
    df["is_slick_compound"] = is_slick
    df["is_dry_core_000412"] = is_dry_core
    df["is_fallback_000412"] = is_fallback
    ...
    degradation = df["Cumulative_Degradation"].astype("float32")
    lap_time = df["LapTime (s)"].astype("float32")
    lap_delta = df["LapTime_Delta"].astype("float32")
    position = df["Position"].astype("float32")
    pos_change = df["Position_Change"].astype("float32")

    df["tyre_life_ratio"] = (tyre_life / lap_number.replace(0, np.nan)).astype(
        "float32"
    )
    df["stint_progress_ratio"] = (tyre_life / (stint * 20.0 + 1.0)).astype("float32")
    df["degradation_per_lap"] = (degradation / tyre_life.replace(0, np.nan)).astype(
        "float32"
    )
    df["degradation_x_tyre_life"] = (degradation * tyre_life).astype("float32")
    df["lap_delta_x_tyre_life"] = (lap_delta * tyre_life).astype("float32")
    df["lap_delta_x_progress"] = (lap_delta * race_progress).astype("float32")
    df["position_pressure"] = ((21.0 - position) * race_progress).astype("float32")
    df["position_change_x_progress"] = (pos_change * race_progress).astype("float32")
    df["late_race_stint_end"] = (tyre_life * (1.0 - race_progress)).astype("float32")
    df["lap_time_minus_degradation"] = (lap_time - degradation).astype("float32")

    dry_mask = is_dry_core == 1
    fallback_mask = is_fallback == 1

    routed_cols = {
        "dry_tyre_life": tyre_life,
        "dry_degradation": degradation,
        "dry_degradation_per_lap": df["degradation_per_lap"].astype("float32"),
        "dry_lap_delta": lap_delta,
        "dry_position_pressure": df["position_pressure"].astype("float32"),
        "dry_late_race_stint_end": df["late_race_stint_end"].astype("float32"),
        "fallback_tyre_life": tyre_life,
        "fallback_degradation": degradation,
        "fallback_lap_delta": lap_delta,
        "fallback_position": position,
        "fallback_race_progress": race_progress,
    }

    for name, values in routed_cols.items():
        if name.startswith("dry_"):
            df[name] = values.where(dry_mask, np.nan).astype("float32")
        else:
            df[name] = values.where(fallback_mask, np.nan).astype("float32")

    df["compound_group"] = np.where(
        is_slick == 1, "slick", np.where(is_wet == 1, "wet", "other")
    )
    df["race_domain_000412"] = np.where(
        is_testing == 1, "testing", np.where(is_wet == 1, "wet", "race")
    )

    for col in ["Compound", "Race", "Driver", "Year", "Stint"]:
        vc = df[col].astype("string").fillna("__nan__").value_counts(dropna=False)
        df[f"{col}_freq"] = (
            df[col].astype("string").fillna("__nan__").map(vc).astype("float32")
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000412.",
    }, sort_keys=True))
