...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap_comp = ["Year", "Race", "LapNumber", "Compound"]

    pit = out["PitStop"].astype(float)
    grp = out.groupby(race_lap, observed=True)["PitStop"]
    lap_pit_sum = grp.transform("sum").astype(float)
    lap_car_count = grp.transform("count").astype(float)

    out["race_lap_pit_count_other"] = lap_pit_sum - pit
    out["race_lap_pit_share_other"] = out["race_lap_pit_count_other"] / (
        lap_car_count - 1
    ).replace(0, np.nan)
    out["race_lap_field_size"] = lap_car_count

    pos = out["Position"].astype(float)
    for width in (1, 2, 3):
        band = (pos / width).round().astype("int16")
        tmp_key = "__pos_band_" + str(width)
        out[tmp_key] = band
        band_cols = race_lap + [tmp_key]
        bgrp = out.groupby(band_cols, observed=True)["PitStop"]
        bsum = bgrp.transform("sum").astype(float)
        bcnt = bgrp.transform("count").astype(float)
        out[f"near_pos{width}_pit_count_other"] = bsum - pit
        out[f"near_pos{width}_pit_share_other"] = (bsum - pit) / (bcnt - 1).replace(
            0, np.nan
        )
        out.drop(columns=[tmp_key], inplace=True)
    ...
    for col in [
        ...
        g = out.groupby(race_lap, observed=True)[col]
        mean = g.transform("mean")
        std = g.transform("std").replace(0, np.nan)
        out[f"{col}_race_lap_z"] = (x - mean) / std
        out[f"{col}_race_lap_pct"] = g.rank(pct=True, method="average")

        gc = out.groupby(race_lap_comp, observed=True)[col]
        cmean = gc.transform("mean")
        cstd = gc.transform("std").replace(0, np.nan)
        out[f"{col}_compound_lap_z"] = (x - cmean) / cstd
        out[f"{col}_compound_lap_pct"] = gc.rank(pct=True, method="average")

    out["slow_relative_pace"] = out["LapTime (s)_race_lap_z"].clip(lower=0)
    out["slow_relative_delta"] = out["LapTime_Delta_race_lap_z"].clip(lower=0)
    out["degradation_relative_high"] = out["Cumulative_Degradation_race_lap_pct"]

    out["slow_x_lap_pit_wave"] = out["slow_relative_pace"] * out[
        "race_lap_pit_share_other"
    ].fillna(0)
    out["slow_x_nearby_pit_wave"] = out["slow_relative_pace"] * out[
        "near_pos2_pit_share_other"
    ].fillna(0)
    out["deg_x_nearby_pit_wave"] = out["degradation_relative_high"] * out[
        "near_pos2_pit_share_other"
    ].fillna(0)
    out["position_x_nearby_pit_wave"] = out["Position"].astype(float) * out[
        "near_pos2_pit_share_other"
    ].fillna(0)

    wave_cols = [c for c in out.columns if "share_other" in c]
    out[wave_cols] = out[wave_cols].fillna(0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000130"],
        "research_usage_note": "Verified assigned hypothesis 000130.",
    }, sort_keys=True))
