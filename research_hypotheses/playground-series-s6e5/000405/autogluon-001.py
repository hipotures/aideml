...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Race", "Driver", "Compound"]:
        out[col] = out[col].astype(str).fillna("UNK")

    out["Race_Year"] = out["Race"] + "_" + out["Year"].astype(str)
    out["Driver_Race_Year"] = out["Driver"] + "_" + out["Race_Year"]
    out["Compound_Stint"] = out["Compound"] + "_S" + out["Stint"].astype(str)

    out["is_wet_like"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    ...
    lap_num = out["LapNumber"].clip(lower=1)
    tyre_life = out["TyreLife"].clip(lower=1)
    lap_time = out["LapTime (s)"].clip(lower=1)
    lap_delta = out["LapTime_Delta"]
    degr = out["Cumulative_Degradation"]
    stint = out["Stint"].clip(lower=1)

    est_total_laps = (lap_num / rp).clip(upper=100)
    laps_remaining_est = (est_total_laps - lap_num).clip(lower=0, upper=100)
    tyre_used_frac = (
        (tyre_life / est_total_laps.replace(0, np.nan)).fillna(0).clip(0, 2)
    )
    stint_progress = (tyre_life / lap_num).clip(0, 5)

    degr_per_lap = (degr / tyre_life).replace([np.inf, -np.inf], np.nan).fillna(0)
    degr_per_progress = (degr / rp).replace([np.inf, -np.inf], np.nan).fillna(0)
    delta_per_lap = (lap_delta / lap_num).replace([np.inf, -np.inf], np.nan).fillna(0)

    out["EstTotalLaps"] = est_total_laps
    out["LapsRemainingEst"] = laps_remaining_est
    out["TyreUsedFrac"] = tyre_used_frac
    out["StintProgressRatio"] = stint_progress
    out["DegradationPerLap"] = degr_per_lap
    out["DegradationPerProgress"] = degr_per_progress
    out["LapDeltaPerLap"] = delta_per_lap

    out["TyreLife_x_Degradation"] = tyre_life * degr
    out["TyreLife_x_RaceProgress"] = tyre_life * rp
    out["TyreLife_x_Stint"] = tyre_life * stint
    out["LapDelta_x_Degradation"] = lap_delta * degr
    out["LapTime_x_TyreLife"] = lap_time * tyre_life
    out["PositionPressure"] = out["Position"] * rp
    out["PositionChangePressure"] = out["Position_Change"] * rp
    out["PitStop_x_TyreLife"] = out["PitStop"] * tyre_life
    out["PitStop_x_LapDelta"] = out["PitStop"] * lap_delta

    out["LateRaceOldTyres"] = ((rp > 0.7) & (tyre_life > 15)).astype("int8")
    out["VeryOldTyres"] = (tyre_life > 25).astype("int8")
    out["LargePaceDrop"] = (lap_delta > 2.5).astype("int8")
    out["SevereDegradation"] = (degr_per_lap > 3.0).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000405.",
    }, sort_keys=True))
