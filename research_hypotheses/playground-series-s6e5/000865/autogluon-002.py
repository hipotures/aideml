...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.copy()
    key = ["Year", "Race", "LapNumber"]
    eps = 1e-6

    X["_abs_pos_change"] = X["Position_Change"].abs()

    lap = (
        X.groupby(key, sort=False, observed=True)
        .agg(
            field_size=("LapNumber", "size"),
            field_laptime_median=("LapTime (s)", "median"),
            field_laptime_mean=("LapTime (s)", "mean"),
            field_laptime_std=("LapTime (s)", "std"),
            field_delta_median=("LapTime_Delta", "median"),
            field_delta_mean=("LapTime_Delta", "mean"),
            field_delta_std=("LapTime_Delta", "std"),
            field_pit_count=("PitStop", "sum"),
            field_pit_rate=("PitStop", "mean"),
            field_abs_pos_change_mean=("_abs_pos_change", "mean"),
        )
        .reset_index()
    )

    stat_cols = [
        "field_laptime_median",
        "field_laptime_mean",
        "field_laptime_std",
        "field_delta_median",
        "field_delta_mean",
    ...
    prev = lap[key + stat_cols].copy()
    prev["LapNumber"] = prev["LapNumber"] + 1
    prev = prev.rename(columns={c: "prev_" + c for c in stat_cols})
    lap = lap.merge(prev, on=key, how="left", sort=False)

    lap = lap.sort_values(["Year", "Race", "LapNumber"], kind="mergesort")
    race_g = lap.groupby(["Year", "Race"], sort=False, observed=True)
    lap["race_delta_med_baseline"] = race_g["field_delta_median"].transform("median")
    lap["race_delta_med_sd"] = race_g["field_delta_median"].transform("std")
    lap["race_laptime_med_sd"] = race_g["field_laptime_median"].transform("std")
    lap["race_spread_baseline"] = race_g["field_laptime_std"].transform("median")
    lap["race_pit_rate_baseline"] = (
        race_g["field_pit_rate"]
        .transform(lambda s: s.shift(1).expanding(min_periods=1).mean())
        .fillna(0.0)
    )

    lap["field_delta_z"] = (
        lap["field_delta_median"] - lap["race_delta_med_baseline"]
    ) / (lap["race_delta_med_sd"].fillna(0) + eps)
    lap["field_spread_ratio"] = lap["field_laptime_std"] / (
        lap["race_spread_baseline"].fillna(0) + eps
    )
    lap["field_spread_compression"] = 1.0 - lap["field_spread_ratio"]
    lap["field_pit_rate_excess"] = lap["field_pit_rate"] - lap[
        "race_pit_rate_baseline"
    ].fillna(0)
    lap["field_laptime_slowdown"] = (
        lap["field_laptime_median"] - lap["prev_field_laptime_median"]
    )
    lap["field_delta_slowdown"] = (
        lap["field_delta_median"] - lap["prev_field_delta_median"]
    )
    lap["field_laptime_slowdown_z"] = lap["field_laptime_slowdown"] / (
        lap["race_laptime_med_sd"].fillna(0) + eps
    )
    lap["field_delta_slowdown_z"] = lap["field_delta_slowdown"] / (
        lap["race_delta_med_sd"].fillna(0) + eps
    )

    raw = (
        0.90 * lap["field_delta_z"].clip(-5, 5).fillna(0)
        + 0.70 * lap["field_laptime_slowdown_z"].clip(-5, 5).fillna(0)
        + 0.45 * lap["field_delta_slowdown_z"].clip(-5, 5).fillna(0)
        + 1.80 * (5.0 * lap["field_pit_rate_excess"]).clip(-3, 3).fillna(0)
        + 0.90 * lap["field_spread_compression"].clip(-2, 2).fillna(0)
        - 0.35
    )
    lap["latent_neutralization_score"] = 1.0 / (1.0 + np.exp(-raw.clip(-12, 12)))
    lap["latent_neutralization_flag"] = (
        (lap["latent_neutralization_score"] > 0.70)
        | ((lap["field_pit_rate_excess"] > 0.12) & (lap["field_delta_z"] > 0.5))
    ).astype("int8")
    ...
    add_cols = [
        ...
        "field_pit_count",
        "field_pit_rate",
        "field_abs_pos_change_mean",
        "field_delta_z",
        "field_spread_ratio",
        "field_spread_compression",
        "field_pit_rate_excess",
        "field_laptime_slowdown",
        "field_delta_slowdown",
        "field_laptime_slowdown_z",
        "field_delta_slowdown_z",
        "latent_neutralization_score",
        "latent_neutralization_flag",
    ]
    X = X.drop(columns=["_abs_pos_change"]).merge(
        lap[key + add_cols], on=key, how="left", sort=False
    )

    X["pit_pressure_base"] = (
        X["TyreLife"] * (1.0 + 0.25 * (X["Stint"] - 1)) * (0.5 + X["RaceProgress"])
    )
    X["neutral_x_pit_pressure"] = (
        X["latent_neutralization_score"] * X["pit_pressure_base"]
    )
    X["neutral_x_tyre_life"] = X["latent_neutralization_score"] * X["TyreLife"]
    X["neutral_x_race_progress"] = X["latent_neutralization_score"] * X["RaceProgress"]
    X["neutral_x_lap_delta"] = X["latent_neutralization_score"] * X["LapTime_Delta"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000865"],
        "research_usage_note": "Verified assigned hypothesis 000865.",
    }, sort_keys=True))
