...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_median = out.groupby(lap_cols, sort=False)["LapTime (s)"].transform("median")
    out["Pace_Deviation"] = out["LapTime (s)"] - lap_median

    driver_cols = ["Year", "Race", "Driver"]
    ds = out.sort_values(driver_cols + ["LapNumber"], kind="mergesort")
    dg = ds.groupby(driver_cols, sort=False)
    prev_stint = dg["Stint"].shift(1)
    stint_changed = (ds["Stint"].ne(prev_stint) & prev_stint.notna()).astype("int8")
    ds["_stint_changed"] = stint_changed
    prev_pit = dg["PitStop"].shift(1).fillna(0)
    prev_change = (
        ds.groupby(driver_cols, sort=False)["_stint_changed"].shift(1).fillna(0)
    )
    pit_recent = (
        (ds["PitStop"].fillna(0) > 0)
        | (prev_pit > 0)
        | (stint_changed > 0)
        | (prev_change > 0)
    ).astype("int8")
    out["Own_StintChanged"] = stint_changed.reindex(out.index).fillna(0).astype("int8")
    out["Own_PitRecent"] = pit_recent.reindex(out.index).fillna(0).astype("int8")

    fs = out.sort_values(lap_cols + ["Position"], kind="mergesort")
    fg = fs.groupby(lap_cols, sort=False)
    ...
    for col in [
        ...
        out[f"Ahead_{col}"] = fg[col].shift(1).reindex(out.index)
        out[f"Behind_{col}"] = fg[col].shift(-1).reindex(out.index)

    out["Ahead_Compound"] = fg["Compound"].shift(1).reindex(out.index).fillna("NO_CAR")
    out["Behind_Compound"] = (
        fg["Compound"].shift(-1).reindex(out.index).fillna("NO_CAR")
    )

    out["Gap_To_Ahead_Pos"] = out["Position"] - out["Ahead_Position"]
    out["Gap_To_Behind_Pos"] = out["Behind_Position"] - out["Position"]
    out["TyreLife_vs_Ahead"] = out["TyreLife"] - out["Ahead_TyreLife"]
    out["TyreLife_vs_Behind"] = out["TyreLife"] - out["Behind_TyreLife"]
    out["PaceDev_vs_Ahead"] = out["Pace_Deviation"] - out["Ahead_Pace_Deviation"]
    out["PaceDev_vs_Behind"] = out["Pace_Deviation"] - out["Behind_Pace_Deviation"]
    out["LapDelta_vs_Ahead"] = out["LapTime_Delta"] - out["Ahead_LapTime_Delta"]
    out["LapDelta_vs_Behind"] = out["LapTime_Delta"] - out["Behind_LapTime_Delta"]
    out["Ahead_SameCompound"] = (out["Compound"] == out["Ahead_Compound"]).astype(
        "int8"
    )
    out["Behind_SameCompound"] = (out["Compound"] == out["Behind_Compound"]).astype(
        "int8"
    )
    out["Neighbor_PitRecent"] = np.nanmax(
        out[["Ahead_Own_PitRecent", "Behind_Own_PitRecent"]].to_numpy(dtype=float),
        axis=1,
    )

    lap_key = pd.MultiIndex.from_frame(out[lap_cols])
    life_key = pd.MultiIndex.from_frame(out[lap_cols + ["TyreLife"]])
    compounds = ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]

    for comp in compounds:
        mask = out["Compound"].eq(comp).astype("int16")
        out[f"Field_{comp}_Count"] = (
            mask.groupby(lap_key, sort=False).transform("sum").astype("int16")
        )

        per_life = mask.groupby(life_key, sort=True).sum()
        cum_le = per_life.groupby(level=list(range(len(lap_cols)))).cumsum()
        cum_lt = cum_le - per_life

        le_vals = cum_le.reindex(life_key).to_numpy()
        lt_vals = cum_lt.reindex(life_key).to_numpy()
        total_vals = out[f"Field_{comp}_Count"].to_numpy()

        out[f"Field_{comp}_OlderTyre_Count"] = (total_vals - le_vals).astype("int16")
        out[f"Field_{comp}_NewerTyre_Count"] = lt_vals.astype("int16")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000617"],
        "research_usage_note": "Verified assigned hypothesis 000617.",
    }, sort_keys=True))
