...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp = out["Compound"].astype("string").str.upper().fillna("")
    race = out["Race"].astype("string").str.upper().fillna("")
    driver = out["Driver"].astype("string").str.upper().fillna("")
    year = _pd.to_numeric(out["Year"], errors="coerce").fillna(0).astype("int16")
    lap = _pd.to_numeric(out["LapNumber"], errors="coerce").fillna(0).astype("float32")
    progress = _pd.to_numeric(out["RaceProgress"], errors="coerce").astype("float32")

    lap_arr = lap.to_numpy(dtype=np.float32)
    prog_arr = progress.to_numpy(dtype=np.float32)
    est_total = lap_arr.copy()
    valid_prog = np.isfinite(prog_arr) & (prog_arr > 0)
    est_total[valid_prog] = lap_arr[valid_prog] / np.clip(
        prog_arr[valid_prog], 1e-4, 1.0
    )
    est_total = np.minimum(np.maximum(np.rint(est_total), lap_arr), 120.0)
    laps_remaining = np.maximum(0.0, est_total - lap_arr).astype(np.float32)

    is_monaco_2025 = (year.to_numpy() == 2025) & race.str.contains(
        "MONACO", regex=False
    ).to_numpy()
    current_slick = comp.isin(["SOFT", "MEDIUM", "HARD"]).to_numpy()
    current_wet = comp.isin(["INTERMEDIATE", "WET"]).to_numpy()

    sort_frame = _pd.DataFrame(
        {
            "y": year.to_numpy(),
            "r": race.to_numpy(),
            "d": driver.to_numpy(),
    ...
    order = sort_frame.sort_values(
        ["y", "r", "d", "lap", "pos"], kind="mergesort"
    ).index.to_numpy()
    gy = sort_frame["y"].to_numpy()[order]
    gr = sort_frame["r"].to_numpy()[order]
    gd = sort_frame["d"].to_numpy()[order]
    groups = [gy, gr, gd]

    comp_vals = comp.to_numpy()
    seen = {}
    for name in ("SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"):
        s = _pd.Series((comp_vals[order] == name).astype(np.int8))
        cs = s.groupby(groups, sort=False).cummax().to_numpy(dtype=np.int8)
        arr = np.empty(n, dtype=np.int8)
        arr[order] = cs
        seen[name] = arr

    stint = (
        _pd.to_numeric(out["Stint"], errors="coerce")
        .fillna(1)
        .clip(lower=1)
        .astype("float32")
        .to_numpy()
    )
    stint_s = _pd.Series(stint[order])
    sets_seen_s = (
        stint_s.groupby(groups, sort=False).cummax().to_numpy(dtype=np.float32)
    )
    sets_seen = np.empty(n, dtype=np.float32)
    sets_seen[order] = sets_seen_s

    slick_compounds_seen = (seen["SOFT"] + seen["MEDIUM"] + seen["HARD"]).astype(
        np.int8
    )
    wet_rule_relaxed = (seen["INTERMEDIATE"] + seen["WET"]) > 0

    required_sets = np.where(is_monaco_2025, 3.0, 1.0).astype(np.float32)
    required_stops = np.where(is_monaco_2025, 2.0, 0.0).astype(np.float32)
    stops_taken = np.maximum(0.0, sets_seen - 1.0).astype(np.float32)

    rule_debt_sets = np.maximum(0.0, required_sets - sets_seen).astype(np.float32)
    rule_debt_stop_count = np.maximum(0.0, required_stops - stops_taken).astype(
        np.float32
    )
    rule_debt_compounds = np.where(
        wet_rule_relaxed, 0, np.maximum(0, 2 - slick_compounds_seen)
    ).astype(np.int8)
    compound_stop_debt = (rule_debt_compounds > 0).astype(np.float32)
    rule_debt_stops = np.maximum.reduce(
        [rule_debt_sets, rule_debt_stop_count, compound_stop_debt]
    ).astype(np.float32)

    legal_if_finish_now = (
        (rule_debt_sets <= 0) & (rule_debt_stop_count <= 0) & (rule_debt_compounds <= 0)
    )
    laps_until_forced = np.where(
        rule_debt_stops > 0,
        np.maximum(0.0, laps_remaining - rule_debt_stops + 1.0),
        99.0,
    ).astype(np.float32)
    denom = laps_remaining + 1.0

    out["rule_current_is_slick"] = current_slick.astype(np.int8)
    out["rule_current_is_wet"] = current_wet.astype(np.int8)
    out["rule_monaco_2025_two_stop"] = is_monaco_2025.astype(np.int8)
    out["rule_sets_seen"] = sets_seen
    out["rule_stops_taken"] = stops_taken
    out["rule_slick_compounds_seen"] = slick_compounds_seen
    out["rule_wet_relaxed_compound_rule"] = wet_rule_relaxed.astype(np.int8)
    out["rule_seen_soft"] = seen["SOFT"]
    out["rule_seen_medium"] = seen["MEDIUM"]
    out["rule_seen_hard"] = seen["HARD"]
    out["rule_seen_intermediate"] = seen["INTERMEDIATE"]
    out["rule_seen_wet"] = seen["WET"]
    out["rule_debt_sets"] = rule_debt_sets
    out["rule_debt_compounds"] = rule_debt_compounds
    out["rule_debt_stops"] = rule_debt_stops
    out["rule_must_change_compound_before_finish"] = (rule_debt_compounds > 0).astype(
        np.int8
    )
    out["rule_legal_if_finish_now"] = legal_if_finish_now.astype(np.int8)
    out["rule_laps_remaining_est"] = laps_remaining
    out["rule_laps_until_forced_legal_state"] = laps_until_forced
    out["rule_must_stop_every_remaining_lap"] = (
        (rule_debt_stops > 0) & (laps_remaining <= rule_debt_stops)
    ).astype(np.int8)
    out["rule_stop_debt_pressure"] = (rule_debt_stops / denom).astype(np.float32)
    out["rule_compound_debt_pressure"] = (
        rule_debt_compounds.astype(np.float32) / denom
    ).astype(np.float32)
    out["rule_set_debt_pressure"] = (rule_debt_sets / denom).astype(np.float32)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000873"],
        "research_usage_note": "Verified assigned hypothesis 000873.",
    }, sort_keys=True))
