...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["SurfaceClass"] = np.where(
        df["Compound"].isin(["INTERMEDIATE", "WET"]), "wet", "slick"
    )
    df["Deg_Per_TyreLife"] = df["Cumulative_Degradation"] / df["TyreLife"].replace(
        0, np.nan
    )

    snapshot_groups = {
        "snap_all": ["Race_Year", "LapNumber"],
        "snap_surface": ["Race_Year", "LapNumber", "SurfaceClass"],
    }
    base_cols = [
        "LapTime (s)",
        "LapTime_Delta",
    ...
    for prefix, grp in snapshot_groups.items():
        g = df.groupby(grp, sort=False)
        for col in base_cols:
            pct_name = f"{prefix}_{col}_pct"
            med_name = f"{prefix}_{col}_medres"
            df[pct_name] = g[col].rank(method="average", pct=True) - 0.5
            df[med_name] = df[col] - g[col].transform("median")

    seq_group = ["Race_Year", "Driver"]
    order = ["Race_Year", "Driver", "LapNumber", "Stint", "TyreLife"]
    df = df.sort_values(order, kind="mergesort")

    residual_cols = [
        c for c in df.columns if c.endswith("_pct") or c.endswith("_medres")
    ]
    seq_g = df.groupby(seq_group, sort=False)

    for col in residual_cols:
        prev1 = seq_g[col].shift(1)
        prev2 = seq_g[col].shift(2)
        prev3 = seq_g[col].shift(3)
        df[f"{col}_chg1"] = prev1 - seq_g[col].shift(2)
        df[f"{col}_chg2"] = prev1 - seq_g[col].shift(3)
        df[f"{col}_chg3"] = prev1 - seq_g[col].shift(4)
        df[f"{col}_lag1"] = prev1
        df[f"{col}_mean3"] = pd.concat([prev1, prev2, prev3], axis=1).mean(axis=1)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000381"],
        "research_usage_note": "Verified assigned hypothesis 000381.",
    }, sort_keys=True))
