...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "RaceProgress", "_orig_order"]
    ...
    g = out.groupby(group_cols, sort=False, observed=True)
    base_cols = [
        "TyreLife",
        "Position",
        "Position_Change",
    ...
    trend_cols = [c for c in trend_cols if c in out.columns]

    for c in trend_cols:
        past = g[c].shift(1)
        for w in (5, 10):
            r = past.groupby(
                [out[k] for k in group_cols], sort=False, observed=True
            ).rolling(w, min_periods=1)
            mean = r.mean().reset_index(level=group_cols, drop=True)
            std = r.std().reset_index(level=group_cols, drop=True).fillna(0)
            mn = r.min().reset_index(level=group_cols, drop=True)
            mx = r.max().reset_index(level=group_cols, drop=True)

            out[f"{c}_past{w}_mean"] = mean
            out[f"{c}_past{w}_std"] = std
            out[f"{c}_past{w}_min"] = mn
            out[f"{c}_past{w}_max"] = mx
            out[f"{c}_vs_past{w}_mean"] = out[c] - mean

        out[f"{c}_lag1_to_lag5_delta"] = out[f"{c}_lag1"] - out[f"{c}_lag5"]
        out[f"{c}_lag1_to_lag10_delta"] = out[f"{c}_lag1"] - out[f"{c}_lag10"]
        out[f"{c}_past5_vs_past10_mean"] = (
            out[f"{c}_past5_mean"] - out[f"{c}_past10_mean"]
        )

    if "PitStop" in out.columns:
        past_pit = g["PitStop"].shift(1).fillna(0)
        for w in (5, 10):
            out[f"PitStop_past{w}_count"] = (
                past_pit.groupby(
                    [out[k] for k in group_cols], sort=False, observed=True
                )
                .rolling(w, min_periods=1)
    ...
    if {"LapTime (s)", "LapTime_Delta"}.issubset(out.columns):
        out["lap_pace_instability"] = out["LapTime_Delta"].abs() / (
            out["LapTime (s)"].abs() + 1e-6
        )
    ...
    for c in ["Compound", "Driver", "Race"]:
        if c in out.columns:
            vc = out[c].astype("string").fillna("NA").value_counts(dropna=False)
            out[f"{c}_freq"] = (
                out[c].astype("string").fillna("NA").map(vc).astype("float32")
            )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000754"],
        "research_usage_note": "Verified assigned hypothesis 000754.",
    }, sort_keys=True))
