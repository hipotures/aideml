...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = ["Year", "Race", "Driver", "Stint"]

    # Stable ordering for within-stint hazard style features.
    X["_row_order"] = np.arange(len(X))
    X = X.sort_values(grp + ["LapNumber", "_row_order"], kind="mergesort")

    g = X.groupby(grp, sort=False)

    # Discrete-time hazard framing: time spent in current stint and survival context.
    X["stint_lap_idx"] = g.cumcount() + 1
    X["stint_rows"] = g["LapNumber"].transform("size")
    X["stint_progress_rows"] = X["stint_lap_idx"] / X["stint_rows"].clip(lower=1)

    max_tyre = g["TyreLife"].transform("max")
    X["stint_progress_tyre"] = X["TyreLife"] / max_tyre.clip(lower=1)

    X["survival_so_far_rows"] = 1.0 - (X["stint_lap_idx"] - 1) / X["stint_rows"].clip(
        lower=1
    )
    X["survival_so_far_tyre"] = 1.0 - (X["TyreLife"] - 1) / max_tyre.clip(lower=1)

    # Hazard-shape nonlinearities.
    X["tyrelife_sq"] = X["TyreLife"] ** 2
    X["degradation_pos"] = X["Cumulative_Degradation"].clip(lower=0)
    X["degradation_sq_pos"] = X["degradation_pos"] ** 2
    X["late_stint_risk"] = X["stint_progress_tyre"] * X["degradation_pos"]
    X["late_stint_delta_risk"] = X["stint_progress_tyre"] * X["LapTime_Delta"]
    X["late_race_stint_risk"] = X["stint_progress_tyre"] * X["RaceProgress"]

    # Relative pace/state inside the current stint.
    stint_mean_laptime = g["LapTime (s)"].transform("mean")
    stint_mean_delta = g["LapTime_Delta"].transform("mean")
    stint_mean_deg = g["Cumulative_Degradation"].transform("mean")

    X["laptime_vs_stint_mean"] = X["LapTime (s)"] - stint_mean_laptime
    X["delta_vs_stint_mean"] = X["LapTime_Delta"] - stint_mean_delta
    X["deg_vs_stint_mean"] = X["Cumulative_Degradation"] - stint_mean_deg

    # Position dynamics can proxy strategic windows.
    X["position_pressure"] = X["Position"] * X["stint_progress_tyre"]
    X["position_change_pressure"] = X["Position_Change"] * X["stint_progress_tyre"]

    # Discrete bins often work well for hazard-like classifiers.
    X["tyrelife_bin4"] = pd.cut(
        X["TyreLife"], bins=[0, 10, 20, 35, 100], labels=False, include_lowest=True
    ).astype("float32")
    X["stint_progress_bin5"] = pd.cut(
        X["stint_progress_tyre"],
        bins=[0, 0.2, 0.4, 0.6, 0.8, 1.01],
        labels=False,
        include_lowest=True,
    ).astype("float32")
    X["degradation_bin5"] = pd.cut(
        X["degradation_pos"],
        bins=[-1e-9, 5, 20, 50, 100, 1e9],
        labels=False,
        include_lowest=True,
    ).astype("float32")

    # Restore original row order.
    X = X.sort_values("_row_order", kind="mergesort").drop(columns=["_row_order"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000981"],
        "research_usage_note": "Verified assigned hypothesis 000981.",
    }, sort_keys=True))
