...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_wet = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    is_soft = (compound == "SOFT").astype("int8")
    is_medium = (compound == "MEDIUM").astype("int8")
    is_hard = (compound == "HARD").astype("int8")

    # Survival-style state features
    out["stint_age"] = tyre_life
    out["stint_age_sq"] = tyre_life * tyre_life
    out["stint_age_log1p"] = np.log1p(tyre_life)
    out["stint_age_pct_race"] = tyre_life / lap.clip(lower=1.0)
    out["stint_number_pct"] = stint / 8.0

    # Proxy for cumulative survival within stint: higher TyreLife means longer survived without stopping
    out["stint_survival_exp"] = np.exp(-tyre_life / 12.0)
    out["stint_survival_linear"] = 1.0 - (tyre_life / lap.clip(lower=1.0)).clip(
        0.0, 1.0
    )

    # Hazard-aligned progress features
    out["laps_remaining_est"] = lap * (1.0 / race_prog - 1.0)
    out["tyre_to_remaining_ratio"] = tyre_life / (out["laps_remaining_est"].abs() + 1.0)
    out["degradation_per_lap"] = degr / tyre_life.clip(lower=1.0)
    out["delta_per_tyre_life"] = lap_delta / tyre_life.clip(lower=1.0)
    out["degradation_x_age"] = degr * tyre_life
    out["delta_x_age"] = lap_delta * tyre_life

    # Race-relative normalization without target use
    race_keys = ["Year", "Race"]
    race_max_lap = (
        out.groupby(race_keys, sort=False)["LapNumber"]
        .transform("max")
        .astype(float)
        .clip(lower=1.0)
    )
    out["lap_pct_in_race"] = lap / race_max_lap
    out["tyre_pct_of_race_max"] = tyre_life / race_max_lap
    out["age_minus_expected"] = tyre_life - lap * race_prog

    # Discrete-time hazard bins
    age_bins = pd.cut(
        tyre_life,
        bins=[-np.inf, 5, 10, 15, 20, 25, 30, 40, np.inf],
        labels=False,
    ).astype("float32")
    lap_bins = pd.cut(
        out["lap_pct_in_race"],
    ...
    out["hazard_age_bin"] = age_bins
    out["hazard_race_bin"] = lap_bins
    out["hazard_bin_joint"] = age_bins * 10.0 + lap_bins

    # Interactions to let AutoML emulate a hazard head
    out["hazard_age_x_wet"] = age_bins * is_wet
    out["hazard_age_x_soft"] = age_bins * is_soft
    out["hazard_age_x_medium"] = age_bins * is_medium
    out["hazard_age_x_hard"] = age_bins * is_hard
    out["hazard_joint_x_wet"] = out["hazard_bin_joint"] * (is_wet + 1.0)
    out["age_x_degradation_per_lap"] = tyre_life * out["degradation_per_lap"]
    out["age_x_delta_per_tyre"] = tyre_life * out["delta_per_tyre_life"]

    # Current-lap state combinations relevant for next-lap stop risk
    out["position_pressure"] = pos / 20.0 + pos_chg.abs() / 20.0
    out["pit_window_pressure"] = (
        0.35 * out["stint_age_pct_race"].clip(0, 2)
        + 0.25 * out["lap_pct_in_race"].clip(0, 1)
        + 0.20 * out["degradation_per_lap"].clip(-100, 100)
        + 0.20 * out["delta_per_tyre_life"].clip(-100, 100)
    )
    out["pit_window_pressure_wet"] = out["pit_window_pressure"] * (is_wet + 1.0)
    out["pit_window_pressure_soft"] = out["pit_window_pressure"] * (is_soft + 1.0)

    # Stable categorical interaction token
    out["compound_hazard_state"] = (
        compound
        + "_A"
        + age_bins.fillna(-1).astype(int).astype(str)
        + "_L"
        + lap_bins.fillna(-1).astype(int).astype(str)
        + "_S"
        + stint.fillna(-1).astype(int).astype(str)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000281.",
    }, sort_keys=True))
