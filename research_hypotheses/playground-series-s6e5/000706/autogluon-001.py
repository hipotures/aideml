...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap_compound_keys = ["Year", "Race", "LapNumber", "Compound"]
    ...
    for col in base_cols:
        g = out.groupby(race_lap_keys, sort=False)[col]
        ...
        out[f"{col}_race_lap_med_gap"] = out[col] - med
        out[f"{col}_race_lap_z"] = (out[col] - mean) / (std + eps)
        out[f"{col}_race_lap_pct"] = g.rank(pct=True, method="average")
        out[f"{col}_race_lap_rank"] = g.rank(method="average")
        out[f"{col}_race_lap_iqr_pos"] = (out[col] - med) / ((q75 - q25).abs() + eps)

        gc = out.groupby(race_lap_compound_keys, sort=False)[col]
        comp_med = gc.transform("median")
        out[f"{col}_race_lap_compound_med_gap"] = out[col] - comp_med

    lap_g = out.groupby(race_lap_keys, sort=False)["LapTime_Delta"]
    out["LapTime_Delta_gap_to_fast_q25"] = out["LapTime_Delta"] - lap_g.transform(
        lambda s: s.quantile(0.25)
    )

    tyre_g = out.groupby(race_lap_keys, sort=False)["TyreLife"]
    out["TyreLife_gap_to_young_q25"] = out["TyreLife"] - tyre_g.transform(
        lambda s: s.quantile(0.25)
    )
    out["TyreLife_gap_to_old_q75"] = out["TyreLife"] - tyre_g.transform(
        lambda s: s.quantile(0.75)
    )

    comp_codes = out["Compound"].astype("category").cat.codes.astype("int16")
    out["compound_code"] = comp_codes

    out["rel_tyre_age_x_position"] = out["TyreLife_race_lap_z"] * out["Position"]
    out["rel_lap_delta_x_compound"] = out["LapTime_Delta_race_lap_z"] * comp_codes
    out["rel_degradation_x_stint"] = (
        out["Cumulative_Degradation_race_lap_z"] * out["Stint"]
    )
    out["rel_position_change_x_tyre_age"] = (
        out["Position_Change_race_lap_z"] * out["TyreLife_race_lap_z"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000706"],
        "research_usage_note": "Verified assigned hypothesis 000706.",
    }, sort_keys=True))
