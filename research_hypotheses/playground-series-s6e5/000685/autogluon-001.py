...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_life = comp.map(compound_base_life).fillna(24.0).astype(float)
    ...
    laps_to_finish = (estimated_total_laps - lap).clip(lower=0.0)

    out["TyreLife_RatioToCompoundLife"] = tyre / base_life
    out["TyreLife_ToFinish_CompoundRatio"] = (tyre + laps_to_finish) / base_life
    out["Compound_LapsBeyondFinishWindow"] = (tyre + laps_to_finish - base_life).clip(
        lower=0.0
    )
    out["Compound_LapsBeyondCurrentWindow"] = (tyre - base_life).clip(lower=0.0)
    out["Positive_DegradationPressure"] = deg.clip(lower=0.0)
    out["Positive_LapTimeDeltaPressure"] = lap_delta.clip(lower=0.0)
    out["FinishWindow_Deficit"] = (base_life - laps_to_finish).clip(lower=0.0)
    out["LapsSinceLastStop_Proxy"] = tyre
    out["RaceRemaining_Laps"] = laps_to_finish
    out["LateRace_TyrePressure"] = out["TyreLife_RatioToCompoundLife"] * progress
    out["Degradation_Per_TyreLife"] = deg / tyre.clip(lower=1.0)
    out["PositionLoss_Pressure"] = out["Position_Change"].astype(float).clip(lower=0.0)
    out["Stint_TyreLife_Product"] = out["Stint"].astype(float) * tyre
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000685"],
        "research_usage_note": "Verified assigned hypothesis 000685.",
    }, sort_keys=True))
