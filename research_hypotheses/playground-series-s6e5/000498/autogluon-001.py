...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    key_ry = ["Race", "Year"]
    key_driver = ["Race", "Year", "Driver"]

    # Stable row id for restoring original order after chronological transforms.
    out["_row_id_tmp"] = np.arange(len(out))

    lap = out["LapTime (s)"].astype(float)
    q_low = out.groupby(key_ry, observed=True)["LapTime (s)"].transform(
        lambda s: s.quantile(0.01)
    )
    q_high = out.groupby(key_ry, observed=True)["LapTime (s)"].transform(
        lambda s: s.quantile(0.99)
    )
    out["LapTime_Outlier_Flag"] = ((lap < q_low) | (lap > q_high)).astype("int8")
    out["LapTime_Winsor"] = lap.clip(q_low, q_high)

    sort_cols = key_driver + ["LapNumber", "_row_id_tmp"]
    tmp = out.sort_values(sort_cols, kind="mergesort")
    tmp["ElapsedRaceTime"] = tmp.groupby(key_driver, observed=True)[
        "LapTime_Winsor"
    ].cumsum()
    tmp["PrevLapTime_Winsor"] = tmp.groupby(key_driver, observed=True)[
        "LapTime_Winsor"
    ].shift(1)
    tmp["PrevElapsedRaceTime"] = tmp.groupby(key_driver, observed=True)[
        "ElapsedRaceTime"
    ].shift(1)
    tmp["DriverLapSeq"] = tmp.groupby(key_driver, observed=True).cumcount() + 1

    out = tmp.sort_values("_row_id_tmp", kind="mergesort")

    lap_max = (
        out.groupby(key_ry, observed=True)["LapNumber"].transform("max").astype(float)
    )
    out["LapsRemaining_Est"] = (lap_max - out["LapNumber"].astype(float)).clip(lower=0)
    out["RaceDriver_LapFrac"] = out["DriverLapSeq"].astype(float) / lap_max.replace(
        0, np.nan
    )

    grid = out.sort_values(
        key_ry + ["LapNumber", "ElapsedRaceTime", "_row_id_tmp"], kind="mergesort"
    )
    lap_key = key_ry + ["LapNumber"]

    grid["GapAhead_Est"] = grid.groupby(lap_key, observed=True)[
        "ElapsedRaceTime"
    ].diff()
    grid["GapBehind_Est"] = -grid.groupby(lap_key, observed=True)[
        "ElapsedRaceTime"
    ].diff(-1)

    for seconds in (2, 5, 10, 20):
        ahead_count = grid.groupby(lap_key, observed=True)["ElapsedRaceTime"].transform(
            lambda s, sec=seconds: np.searchsorted(
                s.to_numpy(), s.to_numpy() + sec, side="right"
            )
            - np.arange(len(s))
            - 1
        )
        behind_count = grid.groupby(lap_key, observed=True)[
            "ElapsedRaceTime"
        ].transform(
            lambda s, sec=seconds: np.arange(len(s))
            - np.searchsorted(s.to_numpy(), s.to_numpy() - sec, side="left")
        )
        grid[f"CarsWithin{seconds}s"] = (ahead_count + behind_count).astype("int16")
        grid[f"CarsAheadWithin{seconds}s"] = ahead_count.astype("int16")
        grid[f"CarsBehindWithin{seconds}s"] = behind_count.astype("int16")

    grid["CleanAir_5s"] = (grid["GapAhead_Est"].fillna(99) > 5).astype("int8")
    grid["TrafficPressure_10s"] = grid["CarsWithin10s"].astype(float) / 19.0
    grid["GapAhead_x_TyreLife"] = grid["GapAhead_Est"].fillna(30).clip(0, 60) * grid[
        "TyreLife"
    ].astype(float)
    grid["GapBehind_x_TyreLife"] = grid["GapBehind_Est"].fillna(30).clip(0, 60) * grid[
        "TyreLife"
    ].astype(float)
    grid["PackDensity_x_LapsRemaining"] = grid["CarsWithin10s"].astype(float) * grid[
        "LapsRemaining_Est"
    ].astype(float)
    grid["Position_x_GapBehind"] = grid["Position"].astype(float) * grid[
        "GapBehind_Est"
    ].fillna(30).clip(0, 60)

    out = grid.sort_values("_row_id_tmp", kind="mergesort").drop(
        columns=["_row_id_tmp"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000498"],
        "research_usage_note": "Verified assigned hypothesis 000498.",
    }, sort_keys=True))
