...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    race_keys = ["Race", "Year"] if "Year" in out.columns else ["Race"]

    lap_cols = race_keys + ["LapNumber"]
    lap = (
        out[lap_cols + ["LapTime (s)", "PitStop", "TyreLife", "Position"]]
        .groupby(lap_cols, sort=False)
        .agg(
            field_lap_median=("LapTime (s)", "median"),
            field_lap_q25=("LapTime (s)", lambda s: s.quantile(0.25)),
            field_lap_q75=("LapTime (s)", lambda s: s.quantile(0.75)),
            field_lap_std=("LapTime (s)", "std"),
            field_lap_min=("LapTime (s)", "min"),
            field_lap_max=("LapTime (s)", "max"),
            current_pitstop_share=("PitStop", "mean"),
            field_tyre_life_median=("TyreLife", "median"),
            field_size=("LapTime (s)", "size"),
        )
        .reset_index()
        .sort_values(race_keys + ["LapNumber"])
    )
    lap["field_lap_iqr"] = (lap["field_lap_q75"] - lap["field_lap_q25"]).replace(
        0, np.nan
    )
    lap["field_compression"] = lap["field_lap_iqr"] / (
        lap["field_lap_median"].abs() + 1e-6
    )
    lap["field_lap_range"] = lap["field_lap_max"] - lap["field_lap_min"]

    g = lap.groupby(race_keys, sort=False)
    prior_med = g["field_lap_median"].transform(
        lambda s: s.shift(1).rolling(5, min_periods=2).median()
    )
    prior_iqr = g["field_lap_iqr"].transform(
        lambda s: s.shift(1).rolling(5, min_periods=2).median()
    )
    prior_std = g["field_lap_std"].transform(
        lambda s: s.shift(1).rolling(5, min_periods=2).median()
    )
    prior_comp = g["field_compression"].transform(
        lambda s: s.shift(1).rolling(5, min_periods=2).median()
    )

    lap["prior_field_median_5"] = prior_med
    lap["prior_field_iqr_5"] = prior_iqr
    lap["prior_field_std_5"] = prior_std
    lap["prior_field_compression_5"] = prior_comp
    scale = prior_iqr.fillna(prior_std).replace(0, np.nan)
    lap["lap_slowdown_z"] = (lap["field_lap_median"] - prior_med) / (scale + 1e-6)
    lap["compression_change"] = lap["field_compression"] - prior_comp

    slow_ref = prior_med + 1.5 * (scale + 1e-6)
    tmp = out[lap_cols + ["LapTime (s)"]].merge(
        lap[
            lap_cols
            + ["prior_field_median_5", "prior_field_iqr_5", "prior_field_std_5"]
        ],
        on=lap_cols,
        how="left",
        sort=False,
    ...
    tmp_scale = (
        tmp["prior_field_iqr_5"].fillna(tmp["prior_field_std_5"]).replace(0, np.nan)
    )
    tmp["is_extreme_slow_lap"] = (
        tmp["LapTime (s)"] > tmp["prior_field_median_5"] + 1.5 * (tmp_scale + 1e-6)
    ).astype("float32")
    extreme = (
        tmp.groupby(lap_cols, sort=False)["is_extreme_slow_lap"]
        .mean()
        .rename("extreme_slow_frac")
        .reset_index()
    )
    lap = lap.merge(extreme, on=lap_cols, how="left", sort=False)
    ...
    keep = lap_cols + [
        ...
        "field_lap_range",
        "field_compression",
        "current_pitstop_share",
        "field_tyre_life_median",
        "field_size",
        "prior_field_median_5",
        "prior_field_iqr_5",
        "prior_field_std_5",
        "prior_field_compression_5",
        "lap_slowdown_z",
        "compression_change",
        "extreme_slow_frac",
    ]
    out = out.merge(lap[keep], on=lap_cols, how="left", sort=False)

    out["lap_slowdown_z"] = (
        out["lap_slowdown_z"].replace([np.inf, -np.inf], np.nan).fillna(0).clip(-10, 10)
    )
    out["compression_change"] = (
        out["compression_change"].replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    out["extreme_slow_frac"] = out["extreme_slow_frac"].fillna(0)
    out["current_pitstop_share"] = out["current_pitstop_share"].fillna(0)

    out["cheap_stop_score"] = (
        out["lap_slowdown_z"]
        * (1.0 + out["current_pitstop_share"])
        * np.sqrt(out["TyreLife"].clip(lower=0))
    )
    out["slowdown_x_tyre_life"] = out["lap_slowdown_z"] * out["TyreLife"]
    out["slowdown_x_race_progress"] = out["lap_slowdown_z"] * out["RaceProgress"]
    out["pitshare_x_tyre_life"] = out["current_pitstop_share"] * out["TyreLife"]
    out["compression_x_progress"] = out["compression_change"] * out["RaceProgress"]
    out["driver_vs_field_lap"] = out["LapTime (s)"] - out["field_lap_median"]
    out["driver_vs_prior_field"] = out["LapTime (s)"] - out["prior_field_median_5"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000627"],
        "research_usage_note": "Verified assigned hypothesis 000627.",
    }, sort_keys=True))
