...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    circuit_meta = {
        "Abu Dhabi Grand Prix": (20.5, "street_hybrid", 0.55, 0, 0),
        "Australian Grand Prix": (22.0, "street_hybrid", 0.55, 0, 0),
        "Austrian Grand Prix": (19.5, "permanent", 0.35, 0, 0),
        "Azerbaijan Grand Prix": (20.0, "street", 0.45, 1, 0),
        "Bahrain Grand Prix": (23.0, "permanent", 0.45, 0, 0),
        "Belgian Grand Prix": (22.5, "permanent", 0.35, 0, 0),
        "British Grand Prix": (21.5, "permanent", 0.45, 0, 0),
        "Canadian Grand Prix": (18.5, "street_hybrid", 0.45, 1, 0),
        "Dutch Grand Prix": (21.0, "permanent", 0.70, 1, 0),
        "Emilia Romagna Grand Prix": (27.0, "permanent", 0.75, 1, 0),
        "Hungarian Grand Prix": (21.5, "permanent", 0.80, 1, 0),
        "Italian Grand Prix": (24.5, "permanent", 0.35, 0, 0),
        "Japanese Grand Prix": (22.0, "permanent", 0.60, 0, 0),
        "Las Vegas Grand Prix": (22.0, "street", 0.45, 0, 0),
        "Mexico City Grand Prix": (22.5, "permanent", 0.55, 0, 0),
        "Miami Grand Prix": (20.5, "street_hybrid", 0.55, 0, 0),
        "Monaco Grand Prix": (20.0, "street", 0.95, 1, 1),
        "Pre-Season Testing": (22.0, "permanent", 0.55, 0, 0),
        "Qatar Grand Prix": (24.0, "permanent", 0.55, 0, 0),
        "Sakhir Grand Prix": (23.0, "permanent", 0.45, 0, 0),
        "Saudi Arabian Grand Prix": (20.0, "street", 0.45, 1, 0),
        "Singapore Grand Prix": (28.0, "street", 0.85, 1, 1),
        "Spanish Grand Prix": (22.5, "permanent", 0.65, 0, 0),
        "São Paulo Grand Prix": (21.0, "permanent", 0.50, 0, 0),
        "United States Grand Prix": (20.5, "permanent", 0.45, 0, 0),
    }
    ...
    meta.index.name = "Race"

    out = out.join(meta, on="Race")
    out["CircuitPitLoss"] = out["CircuitPitLoss"].fillna(22.0)
    out["CircuitType"] = out["CircuitType"].fillna("unknown")
    out["OvertakeDifficulty"] = out["OvertakeDifficulty"].fillna(0.55)
    out["PitLaneTight"] = out["PitLaneTight"].fillna(0).astype("int8")
    out["PitSpeedLimitLow"] = out["PitSpeedLimitLow"].fillna(0).astype("int8")

    race_progress = out["RaceProgress"].clip(lower=0.01)
    est_total_laps = (out["LapNumber"] / race_progress).clip(40, 90)
    out["LapsRemaining_Est"] = (est_total_laps - out["LapNumber"]).clip(lower=0)

    tyre_pressure_proxy = out["Cumulative_Degradation"] / (
        out["TyreLife"].clip(lower=1)
    )
    out["CurrentTyreFinishPressure"] = tyre_pressure_proxy.replace(
        [np.inf, -np.inf], np.nan
    ).fillna(0)

    out["StopCost_x_TyrePressure"] = (
        out["CircuitPitLoss"] * out["CurrentTyreFinishPressure"]
    )
    out["StopCost_x_Position"] = out["CircuitPitLoss"] * out["Position"]
    out["StopCost_x_RaceProgress"] = out["CircuitPitLoss"] * out["RaceProgress"]
    out["StopCost_x_LapsRemaining"] = out["CircuitPitLoss"] * out["LapsRemaining_Est"]
    out["OvertakeDiff_x_Position"] = out["OvertakeDifficulty"] * out["Position"]
    out["OvertakeDiff_x_TyrePressure"] = (
        out["OvertakeDifficulty"] * out["CurrentTyreFinishPressure"]
    )
    out["TightPit_x_RaceProgress"] = out["PitLaneTight"] * out["RaceProgress"]
    out["LowSpeedPit_x_LapsRemaining"] = (
        out["PitSpeedLimitLow"] * out["LapsRemaining_Est"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000380"],
        "research_usage_note": "Verified assigned hypothesis 000380.",
    }, sort_keys=True))
