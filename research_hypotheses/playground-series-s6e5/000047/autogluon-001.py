...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Year"] = pd.to_numeric(out["Year"], errors="coerce").fillna(-1).astype("int16")
    out["Race_Year"] = out["Race"].astype("string") + "_" + out["Year"].astype("string")

    n = float(len(out))
    for col in ["Driver", "Race", "Compound", "Year", "Race_Year"]:
        freq = out[col].map(out[col].value_counts(dropna=False)).astype("float32") / n
        out[f"{col}_freq"] = freq.astype("float32")

    for col in ["Driver", "Race_Year"]:
        out[f"{col}_log_freq"] = np.log1p(out[f"{col}_freq"] * n).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000047.",
    }, sort_keys=True))
