...
def preprocess(df: "pd.DataFrame") -> "pd.DataFrame":
    ...
    tyre_life = (
        pd.to_numeric(out["TyreLife"], errors="coerce").astype(float).clip(lower=1.0)
    )
    stint = pd.to_numeric(out["Stint"], errors="coerce").astype(float)
    pos = pd.to_numeric(out["Position"], errors="coerce").astype(float)
    pos_chg = pd.to_numeric(out["Position_Change"], errors="coerce").astype(float)
    lap_delta = pd.to_numeric(out["LapTime_Delta"], errors="coerce").astype(float)
    ...
    est_total_laps = (lap / progress).clip(5, 90)
    laps_left = (est_total_laps - lap).clip(lower=0)

    compound_deg_mult = compound.map(
        {
            "SOFT": 1.25,
            "MEDIUM": 1.00,
            "HARD": 0.82,
    ...
    race_lower = race.str.lower()
    pit_loss = pd.Series(22.0, index=out.index)
    pit_loss = pit_loss.mask(
        race_lower.str.contains("monaco|singapore", regex=True, na=False), 25.0
    )
    pit_loss = pit_loss.mask(
        race_lower.str.contains("monza|austria", regex=True, na=False), 19.0
    )
    pit_loss = pit_loss.mask(
        race_lower.str.contains("spa|baku|jeddah|las vegas", regex=True, na=False), 21.0
    )
    pit_loss = pit_loss.mask(
        race_lower.str.contains("pre-season", regex=True, na=False), 20.0
    )

    deg_rate = (cum_deg / tyre_life).replace([np.inf, -np.inf], 0).clip(-8, 25)
    recent_pace_penalty = lap_delta.clip(-20, 60)
    worn_next_penalty = (
        deg_rate * compound_deg_mult + 0.12 * recent_pace_penalty
    ).clip(-10, 45)
    worn_two_lap_penalty = (
        worn_next_penalty * 2.15 + 0.08 * tyre_life * compound_deg_mult
    ).clip(-20, 100)

    fresh_finish_margin = (fresh_life - laps_left).clip(-60, 60)
    fresh_shortfall_penalty = (-fresh_finish_margin).clip(lower=0) * 0.35

    stop_now_value = pit_loss + fresh_shortfall_penalty
    stop_next_value = (
        worn_next_penalty
        + pit_loss
        + (fresh_shortfall_penalty - 0.20 * compound_deg_mult).clip(lower=0)
    )
    extend_two_value = (
        worn_two_lap_penalty
        + pit_loss
        + (fresh_shortfall_penalty - 0.40 * compound_deg_mult).clip(lower=0)
    )

    out["sim_pit_loss_proxy"] = pit_loss.astype(float)
    out["sim_laps_left"] = laps_left.astype(float)
    out["sim_deg_rate"] = deg_rate.astype(float)
    out["sim_fresh_finish_margin"] = fresh_finish_margin.astype(float)
    out["sim_stop_now_value"] = stop_now_value.astype(float)
    out["sim_stop_next_value"] = stop_next_value.astype(float)
    out["sim_extend_two_value"] = extend_two_value.astype(float)
    out["sim_stop_next_minus_now"] = (stop_next_value - stop_now_value).astype(float)
    out["sim_extend_two_minus_now"] = (extend_two_value - stop_now_value).astype(float)
    out["sim_best_extend_advantage"] = (
        np.minimum(stop_next_value, extend_two_value) - stop_now_value
    )
    out["sim_undercut_pressure"] = (
        ((21.0 - pos).clip(0, 20) / 20.0)
        * (-pos_chg).clip(0, 18)
        * (worn_next_penalty.clip(lower=0) + 1.0)
    )
    out["sim_overcut_pressure"] = (
        (pos.clip(1, 20) / 20.0)
        * pos_chg.clip(0, 18)
        * fresh_finish_margin.clip(lower=0)
    )
    out["sim_late_stint_value_gap"] = (tyre_life / (fresh_life + 1.0)) * (
        stop_next_value - stop_now_value
    )
    out["sim_pit_window_pressure"] = (laps_left <= fresh_life + 2).astype(float) * (
        tyre_life >= 0.55 * fresh_life
    ).astype(float)
    out["sim_position_value_gap"] = out["sim_stop_next_minus_now"] * (
        1.0 + (21.0 - pos).clip(0, 20) / 20.0
    )
    out["sim_stint_strategy_age"] = tyre_life / (stint.clip(lower=1) + 1.0)

    out.attrs["research_hypotheses_llm_claimed_used"] = ["000278"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000278"],
        "research_usage_note": "Verified assigned hypothesis 000278.",
    }, sort_keys=True))
