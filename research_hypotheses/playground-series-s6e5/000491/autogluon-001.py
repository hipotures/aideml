...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_meta = {
        "Monaco Grand Prix": (1, 0, 1, 3, 2, 0),
        "Singapore Grand Prix": (1, 1, 1, 3, 2, 1),
        "Azerbaijan Grand Prix": (1, 0, 1, 2, 2, 0),
        "Saudi Arabian Grand Prix": (1, 0, 1, 2, 2, 0),
        "Las Vegas Grand Prix": (1, 0, 1, 2, 2, 0),
        "Miami Grand Prix": (1, 1, 0, 2, 2, 1),
        "Australian Grand Prix": (1, 0, 0, 2, 2, 1),
        "Canadian Grand Prix": (1, 1, 0, 2, 2, 1),
        "Dutch Grand Prix": (0, 1, 0, 3, 2, 1),
        "Hungarian Grand Prix": (0, 1, 1, 3, 2, 1),
        "Spanish Grand Prix": (0, 1, 0, 2, 2, 0),
        "Bahrain Grand Prix": (0, 1, 0, 1, 2, 0),
        "British Grand Prix": (0, 1, 0, 1, 2, 1),
        "Belgian Grand Prix": (0, 0, 1, 1, 2, 1),
        "Italian Grand Prix": (0, 0, 0, 1, 1, 0),
        "Austrian Grand Prix": (0, 1, 0, 1, 1, 1),
        "Japanese Grand Prix": (0, 1, 0, 2, 2, 1),
        "Qatar Grand Prix": (0, 1, 0, 2, 2, 0),
        "United States Grand Prix": (0, 1, 0, 1, 2, 0),
        "Mexico City Grand Prix": (0, 0, 1, 2, 2, 1),
        "Sao Paulo Grand Prix": (0, 1, 0, 1, 1, 1),
        "Abu Dhabi Grand Prix": (0, 0, 1, 2, 2, 0),
        "Chinese Grand Prix": (0, 1, 0, 1, 2, 1),
        "Emilia Romagna Grand Prix": (0, 0, 1, 3, 2, 1),
        "French Grand Prix": (0, 1, 0, 1, 2, 0),
        "Pre-Season Testing": (0, 0, 0, 1, 1, 0),
    }
    meta_cols = [
        "Race_StreetTemp",
        "Race_HighDeg",
        "Race_OvercutFriendly",
        "Race_OvertakeDifficulty",
        "Race_PitLossBand",
        "Race_WetProne",
    ]

    meta = pd.DataFrame.from_dict(race_meta, orient="index", columns=meta_cols)
    joined = out[["Race"]].join(meta, on="Race")
    for c in meta_cols:
        out[c] = joined[c].fillna(meta[c].median()).astype("float32")

    lap = out["LapNumber"].astype("float32")
    progress = out["RaceProgress"].clip(0.01, 1.0).astype("float32")
    tyre = out["TyreLife"].astype("float32")
    pos = out["Position"].astype("float32")
    compound = out["Compound"].astype(str)

    est_total_laps = (lap / progress).clip(40, 90)
    out["LapsRemaining_Est"] = (est_total_laps - lap).clip(lower=0).astype("float32")
    out["TyreLife_RaceProgress"] = (tyre * progress).astype("float32")
    out["TyreLife_LapsRemaining_Ratio"] = (
        tyre / (out["LapsRemaining_Est"] + 1.0)
    ).astype("float32")

    out["Compound_SoftFlag"] = compound.eq("SOFT").astype("int8")
    out["Compound_HardFlag"] = compound.eq("HARD").astype("int8")
    out["Compound_WetFlag"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")

    field_count = out.groupby(["Year", "Race", "LapNumber"], sort=False)[
        "Position"
    ].transform("count")
    out["FieldDensity_RaceLap"] = (field_count / 20.0).clip(0, 2).astype("float32")
    out["FrontTrackPosition"] = ((21.0 - pos) / 20.0).clip(0, 1).astype("float32")

    out["MetaDeg_TyreLife"] = (out["Race_HighDeg"] * tyre).astype("float32")
    out["MetaDeg_Soft_TyreLife"] = (
        out["Race_HighDeg"] * out["Compound_SoftFlag"] * tyre
    ).astype("float32")
    out["MetaOvercut_Hard_TyreLife"] = (
        out["Race_OvercutFriendly"] * out["Compound_HardFlag"] * tyre
    ).astype("float32")
    out["MetaOvertake_Position"] = (out["Race_OvertakeDifficulty"] * pos).astype(
        "float32"
    )
    out["MetaPitLoss_LapsRemaining"] = (
        out["Race_PitLossBand"] * out["LapsRemaining_Est"]
    ).astype("float32")
    out["MetaStreet_FieldDensity"] = (
        out["Race_StreetTemp"] * out["FieldDensity_RaceLap"]
    ).astype("float32")
    out["MetaWet_WetCompound"] = (
        out["Race_WetProne"] * out["Compound_WetFlag"]
    ).astype("float32")
    out["PitWindow_ArchetypeScore"] = (
        0.35 * out["Race_HighDeg"] * np.log1p(tyre)
        + 0.25 * out["Race_OvertakeDifficulty"] * out["FrontTrackPosition"]
        - 0.20 * out["Race_OvercutFriendly"] * out["Compound_HardFlag"]
        + 0.10
        * out["Race_PitLossBand"]
        * (out["LapsRemaining_Est"] < 35).astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000491.",
    }, sort_keys=True))
