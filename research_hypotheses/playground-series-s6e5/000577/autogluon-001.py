...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_groups = X.groupby(group_cols, sort=False)
    ...
    X["Is_Softest"] = (X["Compound"] == "SOFT").astype("int8")

    X["TyreLife_Share"] = X["TyreLife"] / X["LapNumber"].clip(lower=1)
    X["Degradation_Per_TyreLap"] = X["Cumulative_Degradation"] / X["TyreLife"].clip(
        lower=1
    )
    X["Degradation_Per_RaceLap"] = X["Cumulative_Degradation"] / X["LapNumber"].clip(
        lower=1
    )
    X["Stint_Progress"] = X["TyreLife"] / (X["TyreLife"] + 3.0)
    X["TyreLife_x_Degradation"] = X["TyreLife"] * X["Cumulative_Degradation"]
    X["TyreLife_x_Compound"] = X["TyreLife"] * X["Compound_Code"]
    X["Delta_x_Degradation"] = X["LapTime_Delta"] * X["Cumulative_Degradation"]
    X["Delta_x_TyreLife"] = X["LapTime_Delta"] * X["TyreLife"]
    X["Position_Pressure"] = X["Position"] * X["LapTime_Delta"]
    X["RacePhase_x_Stint"] = X["RaceProgress"] * X["Stint"]

    X["Cars_On_Lap"] = lap_groups["Driver"].transform("size").astype("int16")
    ...
    for col in relative_cols:
        grp_mean = lap_groups[col].transform("mean")
        grp_std = lap_groups[col].transform("std").fillna(0.0)
        grp_min = lap_groups[col].transform("min")
        grp_max = lap_groups[col].transform("max")

        safe_std = grp_std.replace(0, 1.0)
        safe_range = (grp_max - grp_min).replace(0, 1.0)

        clean_name = (
            col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        )

        X[f"{clean_name}_LapMeanDiff"] = X[col] - grp_mean
        X[f"{clean_name}_LapZ"] = (X[col] - grp_mean) / safe_std
        X[f"{clean_name}_LapMinMax"] = (X[col] - grp_min) / safe_range
        X[f"{clean_name}_LapRankPct"] = lap_groups[col].rank(method="average", pct=True)

    X["Leader_Position_Gap"] = X["Position"] - lap_groups["Position"].transform("min")
    X["FastestLap_Gap"] = X["LapTime (s)"] - lap_groups["LapTime (s)"].transform("min")
    X["BestDelta_Gap"] = X["LapTime_Delta"] - lap_groups["LapTime_Delta"].transform(
        "min"
    )
    X["WorstTyreLife_Gap"] = X["TyreLife"] - lap_groups["TyreLife"].transform("max")
    X["WorstDeg_Gap"] = X["Cumulative_Degradation"] - lap_groups[
        "Cumulative_Degradation"
    ].transform("max")

    X["PitPressureScore"] = (
        0.35 * X["TyreLife_LapRankPct"]
        + 0.30 * X["Cumulative_Degradation_LapRankPct"]
        + 0.20 * X["LapTime_Delta_LapRankPct"]
        + 0.15 * X["Position_LapRankPct"]
    )
    X["UndercutRiskScore"] = (
        X["LapTime_Delta_LapZ"] * X["Position_LapRankPct"] - X["Position_Change_LapZ"]
    )
    X["LateStintStress"] = (
        X["TyreLife_Share"] * X["Degradation_Per_TyreLap"] * (1.0 + X["RaceProgress"])
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000577.",
    }, sort_keys=True))
