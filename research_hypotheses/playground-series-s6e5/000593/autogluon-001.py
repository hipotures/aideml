...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wear_level = np.zeros(n, dtype=np.float32)
    wear_accel = np.zeros(n, dtype=np.float32)
    warmup_debt = np.zeros(n, dtype=np.float32)
    residual_scale = np.zeros(n, dtype=np.float32)
    filtered_pace = np.zeros(n, dtype=np.float32)

    laptime_col = "LapTime (s)"
    keys = ["Year", "Race", "Driver"]

    tmp = out[
        keys + ["LapNumber", laptime_col, "PitStop", "TyreLife", "RaceProgress"]
    ].copy()
    tmp["_idx"] = np.arange(n)
    tmp = tmp.sort_values(keys + ["LapNumber", "_idx"], kind="mergesort")

    for _, g in tmp.groupby(keys, sort=False):
        idx = g["_idx"].to_numpy()
        lap_time = g[laptime_col].to_numpy(dtype=np.float64)
        pit = g["PitStop"].to_numpy(dtype=np.int8)
        tyre_life = g["TyreLife"].to_numpy(dtype=np.float64)
        progress = g["RaceProgress"].to_numpy(dtype=np.float64)
        ...
        scale = 1.0
        fuel_slope = 0.0
        prev_clean_time = lap_time[0]
        prev_pit = 0

        for j in range(len(idx)):
            contaminated = (pit[j] == 1) or (prev_pit == 1) or (lap_time[j] > 300.0)

            if prev_pit == 1 or tyre_life[j] <= 2.0:
                warm = max(warm, 1.0)
                wear = min(wear, 0.05)

            expected = (
                base
                + fuel_slope * max(progress[j] - progress[0], 0.0)
                + 1.35 * warm
                + wear
            )
            resid = lap_time[j] - expected

            wear_level[idx[j]] = wear
            wear_accel[idx[j]] = wear - prev_wear
            warmup_debt[idx[j]] = warm
            residual_scale[idx[j]] = scale
            filtered_pace[idx[j]] = expected

            if not contaminated:
                raw_delta = lap_time[j] - prev_clean_time
                if abs(raw_delta) < 8.0:
                    fuel_slope = 0.96 * fuel_slope + 0.04 * raw_delta

                base_resid = lap_time[j] - (
                    base
                    + fuel_slope * max(progress[j] - progress[0], 0.0)
                    + 1.35 * warm
                )
                positive_wear_signal = max(base_resid, 0.0)
                age_gain = min(max(tyre_life[j] / 35.0, 0.05), 1.0)
                prev_wear = wear
                wear = 0.90 * wear + 0.10 * positive_wear_signal * age_gain

                clean_resid = lap_time[j] - (
                    base
                    + fuel_slope * max(progress[j] - progress[0], 0.0)
                    + 1.35 * warm
                    + wear
                )
                scale = 0.92 * scale + 0.08 * min(abs(clean_resid), 15.0)

                prev_base = base
                base = 0.985 * base + 0.015 * min(
                    lap_time[j] - 1.35 * warm - wear, base + 5.0
                )
                prev_clean_time = lap_time[j]
            else:
                prev_wear = wear
    ...
    out["latent_wear_level"] = wear_level
    out["latent_wear_accel"] = wear_accel
    out["warmup_debt"] = warmup_debt
    out["filter_residual_scale"] = residual_scale
    out["filtered_pace_estimate"] = filtered_pace
    out["pace_minus_filter"] = (
        out[laptime_col].astype("float32") - out["filtered_pace_estimate"]
    )
    out["wear_x_tyre_life"] = out["latent_wear_level"] * out["TyreLife"].astype(
        "float32"
    )
    out["warmup_x_compound"] = out["warmup_debt"] * out["Compound"].astype(
        "category"
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000593"],
        "research_usage_note": "Verified assigned hypothesis 000593.",
    }, sort_keys=True))
