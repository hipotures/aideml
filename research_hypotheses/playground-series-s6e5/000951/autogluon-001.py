...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tmp = X[["Year", "Race", "Driver", "LapNumber"]].copy()
    tmp["_orig_idx"] = orig_idx
    tmp = tmp.sort_values(
        ["Year", "Race", "Driver", "LapNumber", "_orig_idx"], kind="mergesort"
    )

    tmp["time_order"] = np.arange(len(tmp), dtype=np.int32)
    tmp["driver_race_lap_idx"] = (
        tmp.groupby(["Year", "Race", "Driver"], sort=False).cumcount().astype(np.int16)
    )
    tmp["race_lap_idx"] = (
        tmp.groupby(["Year", "Race"], sort=False).cumcount().astype(np.int32)
    )

    tmp = tmp.sort_values("_orig_idx", kind="mergesort")
    X["time_order"] = tmp["time_order"].to_numpy()
    X["driver_race_lap_idx"] = tmp["driver_race_lap_idx"].to_numpy()
    X["race_lap_idx"] = tmp["race_lap_idx"].to_numpy()

    X["tyre_progress"] = X["TyreLife"] / X["LapNumber"].clip(lower=1)
    X["stint_progress"] = X["TyreLife"] / X["Stint"].clip(lower=1)
    X["degradation_per_lap"] = X["Cumulative_Degradation"] / X["TyreLife"].clip(lower=1)
    X["lap_delta_per_lap"] = X["LapTime_Delta"] / X["LapNumber"].clip(lower=1)
    X["position_pressure"] = X["Position"] * (1.0 - X["RaceProgress"])
    X["late_race_tyre"] = X["TyreLife"] * X["RaceProgress"]
    X["stint_x_compound"] = (
    ...
    X["driver_x_compound"] = (
        X["Driver"].astype(str) + "_" + X["Compound"].astype(str)
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000951.",
    }, sort_keys=True))
