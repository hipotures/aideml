...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["causal_sort_year"] = out["Year"].astype("int16")
    out["causal_sort_lap"] = out["LapNumber"].astype("int16")
    out["race_driver_lap_key"] = (
        ...
        + out["Driver"]
        + "__"
        + out["LapNumber"].astype("string").str.zfill(3)
    )

    eps = 1e-6
    out["tyre_life_progress_ratio"] = out["TyreLife"] / (out["LapNumber"] + eps)
    out["laps_remaining_est"] = out["LapNumber"] * (
        1.0 / np.clip(out["RaceProgress"], eps, 1.0) - 1.0
    )
    out["stint_tyre_interaction"] = out["Stint"] * out["TyreLife"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001016"],
        "research_usage_note": "Verified assigned hypothesis 001016.",
    }, sort_keys=True))
