...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cat_cols:
        ...
        out[c + "_freq"] = out[c].map(freq).astype("float32")
    ...
    out["lap_progress_left"] = 1.0 - out["RaceProgress"]
    out["stint_x_tyre_life"] = out["Stint"] * out["TyreLife"]
    out["year_from_2022"] = out["Year"] - 2022
    out["is_recent_year"] = (out["Year"] >= 2024).astype("int8")
    out["is_wet_compound"] = (
        out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["compound_stint"] = (
        out["Compound"].astype(str) + "_s" + out["Stint"].astype(str)
    )
    out["race_year"] = out["Race"].astype(str) + "_" + out["Year"].astype(str)

    if train_n > 0 and test_n > 0 and n == train_n + test_n:
        domain_y = np.r_[
            np.zeros(train_n, dtype=np.int8), np.ones(test_n, dtype=np.int8)
        ]
        disc_cols = [c for c in num_cols + cat_cols if c in out.columns]
        Xd = out[disc_cols].copy()
        for c in cat_cols:
            if c in Xd.columns:
        ...
        pre = ColumnTransformer(
            transformers=[
                (
                    ...
                    [c for c in cat_cols if c in Xd.columns],
                ),
                ("num", "passthrough", [c for c in num_cols if c in Xd.columns]),
            ],
            remainder="drop",
        ...
        if min(train_n, test_n) >= 5:
            cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=200)
            for tr, va in cv.split(Xd, domain_y):
                model = make_pipeline(pre, base)
                model.fit(Xd.iloc[tr], domain_y[tr])
                score[va] = model.predict_proba(Xd.iloc[va])[:, 1].astype("float32")
        out["domain_test_likeness"] = np.clip(score, 0.02, 0.98)
        out["domain_importance_weight_proxy"] = np.clip(
            out["domain_test_likeness"] / (1.0 - out["domain_test_likeness"]), 0.1, 10.0
        )
        out["recent_x_test_likeness"] = (
            out["is_recent_year"] * out["domain_test_likeness"]
        )
    else:
        out["domain_test_likeness"] = 0.5
        out["domain_importance_weight_proxy"] = 1.0
        out["recent_x_test_likeness"] = 0.5 * out["is_recent_year"]

    for c in ["compound_stint", "race_year"]:
        out[c] = out[c].astype("category")
        freq = out[c].value_counts(dropna=False)
        out[c + "_freq"] = out[c].map(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000200"],
        "research_usage_note": "Verified assigned hypothesis 000200.",
    }, sort_keys=True))
