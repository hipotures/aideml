...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year_span = (out["Year"].max() - out["Year"].min())
    if pd.isna(year_span) or year_span == 0:
        year_recency = pd.Series(0.5, index=out.index, dtype="float32")
    else:
        year_recency = ((out["Year"] - out["Year"].min()) / year_span).clip(0, 1)

    shift_score = (
        0.35
        + 0.45 * year_recency.astype(float)
        + 0.10 * out["RaceProgress"].astype(float).clip(0, 1)
        + 0.10 * out["is_recent_year"].astype(float)
    ).clip(0.05, 0.95)
    out["year_shift_likeness"] = shift_score.astype("float32")
    out["year_importance_weight_proxy"] = np.clip(
        shift_score / (1.0 - shift_score), 0.1, 10.0
    ).astype("float32")
    out["recent_x_year_shift"] = (
        out["is_recent_year"].astype(float) * out["year_shift_likeness"]
    ).astype("float32")
