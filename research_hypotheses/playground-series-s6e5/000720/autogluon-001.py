...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["RaceLap_WetShare"] = (
        out.groupby(race_lap, observed=True)["IsWetCompound"]
        .transform("mean")
        .astype("float32")
    )
    out["WetShare_x_TyreLife"] = out["RaceLap_WetShare"] * out["TyreLife"]

    if "PitStop" in out.columns:
        out["RaceLap_PitWave"] = (
            out.groupby(race_lap, observed=True)["PitStop"]
            .transform("mean")
            .astype("float32")
        )
    else:
        out["RaceLap_PitWave"] = 0.0

    order_cols = ["Year", "Race", "Driver", "LapNumber"]
    tmp = out[order_cols + ["Compound", "IsWetCompound", "RaceLap_PitWave"]].copy()
    tmp["_row"] = np.arange(len(out))
    tmp = tmp.sort_values(order_cols, kind="mergesort")

    grp = tmp.groupby(["Year", "Race", "Driver"], observed=True, sort=False)
    tmp["PrevIsWetCompound"] = (
        grp["IsWetCompound"].shift(1).fillna(tmp["IsWetCompound"]).astype("int8")
    )
    tmp["CompoundChangedFromPrev"] = (
        grp["Compound"].shift(1).fillna(tmp["Compound"]) != tmp["Compound"]
    ).astype("int8")
    tmp["PrevRaceLap_PitWave"] = (
        grp["RaceLap_PitWave"].shift(1).fillna(0.0).astype("float32")
    )

    tmp = tmp.sort_values("_row", kind="mergesort")
    out["PrevIsWetCompound"] = tmp["PrevIsWetCompound"].to_numpy()
    out["CompoundChangedFromPrev"] = tmp["CompoundChangedFromPrev"].to_numpy()
    out["PrevRaceLap_PitWave"] = tmp["PrevRaceLap_PitWave"].to_numpy()

    out["WetTransitionContext"] = (
        (
            out["IsWetCompound"]
            + out["PrevIsWetCompound"]
            + out["CompoundChangedFromPrev"]
            + (out["RaceLap_WetShare"] > 0).astype("int8")
        )
        .clip(0, 1)
        .astype("int8")
    )

    lap_median = out.groupby(race_lap, observed=True)["LapTime (s)"].transform("median")
    out["LapTime_vs_RaceLapMedian"] = (out["LapTime (s)"] - lap_median).astype(
        "float32"
    )
    out["LapTimeInflationFlag"] = (
        out["LapTime_vs_RaceLapMedian"]
        > out["LapTime_vs_RaceLapMedian"].median()
        + 2.0 * out["LapTime_vs_RaceLapMedian"].std()
    ).astype("int8")

    out["Gate_WetTransitionScore"] = (
        0.45 * out["IsWetCompound"]
        + 0.20 * out["RaceLap_WetShare"]
        + 0.15 * out["CompoundChangedFromPrev"]
        + 0.10 * out["PrevRaceLap_PitWave"]
        + 0.10 * out["LapTimeInflationFlag"]
    ).astype("float32")

    out["DryExpert_TyrePressure"] = (
        out["IsDryCompound"] * out["TyreLife"] * out["RaceProgress"]
    ).astype("float32")
    out["WetExpert_TransitionPressure"] = (
        out["WetTransitionContext"]
        * (
            out["RaceLap_WetShare"]
            + out["CompoundChangedFromPrev"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000720"],
        "research_usage_note": "Verified assigned hypothesis 000720.",
    }, sort_keys=True))
