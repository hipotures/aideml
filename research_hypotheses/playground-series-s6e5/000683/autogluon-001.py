...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", idx_name]
    work = out.sort_values(sort_cols, kind="mergesort")

    pit = work["PitStop"].astype(float).fillna(0).clip(0, 1)
    prior_stops = pit.groupby([work[c] for c in group_cols], sort=False).cumsum() - pit
    work["prior_stop_count"] = prior_stops.astype("int16")
    work["stop_stratum"] = np.minimum(work["prior_stop_count"], 2).astype("int8")
    work["event_stint_index"] = (work["prior_stop_count"] + 1).astype("int16")

    lap_when_stop = work["LapNumber"].where(pit.eq(1))
    last_stop_lap = lap_when_stop.groupby(
        [work[c] for c in group_cols], sort=False
    ).ffill()
    prev_stop_lap = last_stop_lap.groupby(
        [work[c] for c in group_cols], sort=False
    ).shift(1)
    work["laps_since_last_stop"] = (
        (work["LapNumber"] - prev_stop_lap)
        .fillna(work["LapNumber"])
        .clip(lower=0)
        .astype("float32")
    )

    work = work.sort_values(idx_name, kind="mergesort")
    for col in [
        "prior_stop_count",
        "stop_stratum",
        "event_stint_index",
    ...
    out["is_slick_compound"] = (~compound.isin(["INTERMEDIATE", "WET"])).astype("int8")

    out["tyrelife_x_stop_stratum"] = out["TyreLife"].astype(float) * out["stop_stratum"]
    out["progress_x_stop_stratum"] = (
        out["RaceProgress"].astype(float) * out["stop_stratum"]
    )
    out["degradation_x_stop_stratum"] = (
        out["Cumulative_Degradation"].astype(float) * out["stop_stratum"]
    )
    out["since_stop_x_wet"] = out["laps_since_last_stop"] * out["is_wet_compound"]
    out["tyrelife_x_wet"] = out["TyreLife"].astype(float) * out["is_wet_compound"]
    out["stint_gap_vs_event_history"] = out["Stint"].astype(float) - out[
        "event_stint_index"
    ].astype(float)

    out["stop_phase"] = out["stop_stratum"].astype(str)
    out["compound_stop_phase"] = compound + "_S" + out["stop_phase"]

    return out.drop(columns=[idx_name])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000683"],
        "research_usage_note": "Verified assigned hypothesis 000683.",
    }, sort_keys=True))
