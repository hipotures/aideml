...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Is_Dry"] = out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["Is_WetLike"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    max_lap_race = out.groupby("Race_Year")["LapNumber"].transform("max").replace(0, 1)
    max_stint_seq = (
        out.groupby("Driver_Race_Year")["Stint"].transform("max").replace(0, 1)
    )

    out["Lap_Progress"] = (out["LapNumber"] / max_lap_race).astype("float32")
    out["Stint_Progress"] = (out["TyreLife"] / out["LapNumber"].clip(lower=1)).astype(
        "float32"
    )
    out["Stint_Index_Norm"] = (out["Stint"] / max_stint_seq).astype("float32")

    out["TyreLife_x_Degradation"] = (
        out["TyreLife"] * out["Cumulative_Degradation"]
    ).astype("float32")
    out["TyreLife_per_Progress"] = (
        out["TyreLife"] / out["RaceProgress"].clip(lower=1e-3)
    ).astype("float32")
    out["Deg_per_TyreLife"] = (
        out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1)
    ).astype("float32")
    out["LapDelta_per_TyreLife"] = (
        out["LapTime_Delta"] / out["TyreLife"].clip(lower=1)
    ).astype("float32")
    out["LapTime_x_Progress"] = (out["LapTime (s)"] * out["RaceProgress"]).astype(
        "float32"
    )

    out["Position_Pressure"] = ((21 - out["Position"]) * out["RaceProgress"]).astype(
        "float32"
    )
    out["PositionChange_x_Progress"] = (
        out["Position_Change"] * out["RaceProgress"]
    ).astype("float32")
    out["PitStop_x_TyreLife"] = (out["PitStop"] * out["TyreLife"]).astype("float32")
    out["PitStop_x_Progress"] = (out["PitStop"] * out["RaceProgress"]).astype("float32")
    ...
    for col in [
        ...
        out[f"{col}_Freq"] = out[col].map(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000486.",
    }, sort_keys=True))
