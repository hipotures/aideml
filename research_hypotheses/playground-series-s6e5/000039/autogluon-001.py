...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_norm = tyre / (out["LapNumber"].astype(float).clip(lower=1.0))
    degr_norm = degr / (tyre.clip(lower=1.0))
    slow_flag = (ltd > ltd.quantile(0.75)).astype(float)
    old_tyre_flag = (tyre >= tyre.quantile(0.75)).astype(float)
    degr_flag = (degr_norm > degr_norm.quantile(0.75)).astype(float)
    pos_loss_flag = (pos_chg > 0).astype(float)
    window_flag = ((progress > 0.18) & (progress < 0.88)).astype(float)

    out["own_pressure_score"] = (
        0.30 * old_tyre_flag
        + 0.25 * degr_flag
        + 0.20 * slow_flag
        + 0.10 * pos_loss_flag
    ...
    out["own_high_pressure"] = (out["own_pressure_score"] >= 0.45).astype(float)
    out["tyre_life_lap_ratio"] = tyre_norm
    out["degradation_per_tyre_lap"] = degr_norm
    ...
    counts = grouped["PitStop"].transform("count").astype(float)

    for col in peer_cols:
        vals = out[col].astype(float)
        peer_sum = grouped[col].transform("sum") - vals
        peer_count = (counts - 1.0).clip(lower=0.0)
        global_mean = float(vals.mean())
        peer_mean = peer_sum / peer_count.replace(0.0, np.nan)
        peer_mean = peer_mean.fillna(global_mean)
        safe_name = col.replace(" ", "_").replace("(", "").replace(")", "")
        out[f"peer_loo_mean_{safe_name}"] = peer_mean
        out[f"own_minus_peer_{safe_name}"] = vals - peer_mean

    out["peer_pressure_count"] = (counts - 1.0).clip(lower=0.0)
    out["peer_high_pressure_rate"] = out["peer_loo_mean_own_high_pressure"]
    out["peer_mean_pressure"] = out["peer_loo_mean_own_pressure_score"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000039.",
    }, sort_keys=True))
