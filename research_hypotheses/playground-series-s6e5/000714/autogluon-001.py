...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    s = out.sort_values(keys + ["Driver", "LapNumber", "_orig_idx"]).copy()
    ...
    for col in [
        ...
        prev = s.groupby(drv_key, observed=True)[col].shift(1)
        roll3 = (
            prev.groupby([s[k] for k in drv_key], observed=True)
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=[0, 1, 2], drop=True)
        )
        s[f"{col}_drv_prev"] = prev
        s[f"{col}_drv_roll3_prev"] = roll3
        s[f"{col}_vs_drv_roll3_prev"] = s[col] - roll3

    s["deg_change_drv"] = (
        s["Cumulative_Degradation"] - s["Cumulative_Degradation_drv_prev"]
    )
    s["laptime_jump_drv"] = s["LapTime (s)"] - s["LapTime (s)_drv_roll3_prev"]
    s["delta_jump_drv"] = s["LapTime_Delta"] - s["LapTime_Delta_drv_roll3_prev"]
    s["is_low_grip_compound"] = (
        s["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    s["is_fresh_tyre"] = (s["TyreLife"] <= 3).astype("int8")
    s["is_old_tyre"] = (s["TyreLife"] >= 20).astype("int8")
    s["late_race"] = (s["RaceProgress"] >= 0.65).astype("int8")
    s["pit_window_age"] = (
        (s["TyreLife"] >= 12) & (s["RaceProgress"].between(0.18, 0.85))
    ).astype("int8")

    lap_agg = (
        s.groupby(lap_key, observed=True)
        .agg(
            lap_peer_n=("LapTime_Delta", "size"),
            lap_delta_median=("LapTime_Delta", "median"),
            lap_delta_q10=("LapTime_Delta", lambda x: x.quantile(0.10)),
            lap_delta_q90=("LapTime_Delta", lambda x: x.quantile(0.90)),
            lap_time_median=("LapTime (s)", "median"),
            lap_degradation_change_median=("deg_change_drv", "median"),
            lap_pit_fraction=("PitStop", "mean"),
            lap_low_grip_fraction=("is_low_grip_compound", "mean"),
            lap_position_change_abs_median=(
                "Position_Change",
                lambda x: x.abs().median(),
            ),
            lap_driver_jump_median=("laptime_jump_drv", "median"),
        )
        .reset_index()
    )

    lap_agg = lap_agg.sort_values(keys + ["LapNumber"])
    g = lap_agg.groupby(keys, observed=True)
    for col in [
        "lap_delta_median",
        "lap_delta_q10",
        "lap_delta_q90",
        "lap_time_median",
        "lap_degradation_change_median",
        "lap_pit_fraction",
        "lap_low_grip_fraction",
        "lap_position_change_abs_median",
        "lap_driver_jump_median",
    ]:
        lap_agg[f"{col}_prev1"] = g[col].shift(1)
        lap_agg[f"{col}_roll3_prev"] = (
            g[col]
            .shift(1)
            .groupby([lap_agg[k] for k in keys], observed=True)
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=[0, 1], drop=True)
        )

    lap_agg["lap_delta_spread"] = lap_agg["lap_delta_q90"] - lap_agg["lap_delta_q10"]
    lap_agg["lap_delta_spread_prev1"] = g["lap_delta_spread"].shift(1)
    lap_agg["lap_delta_spread_roll3_prev"] = (
        g["lap_delta_spread"]
        .shift(1)
        .groupby([lap_agg[k] for k in keys], observed=True)
        .rolling(3, min_periods=1)
    ...
    keep = lap_key + [c for c in lap_agg.columns if c not in lap_key]
    s = s.merge(lap_agg[keep], on=lap_key, how="left", sort=False)

    s["driver_delta_vs_lap_median"] = s["LapTime_Delta"] - s["lap_delta_median"]
    s["driver_time_vs_lap_median"] = s["LapTime (s)"] - s["lap_time_median"]
    s["peer_shock_score"] = (
        s["lap_delta_spread"].fillna(0)
        + s["lap_driver_jump_median"].abs().fillna(0)
        + s["lap_degradation_change_median"].abs().fillna(0)
    )
    s["prev_peer_shock_score"] = (
        s["lap_delta_spread_prev1"].fillna(0)
        + s["lap_driver_jump_median_prev1"].abs().fillna(0)
        + s["lap_degradation_change_median_prev1"].abs().fillna(0)
    )
    s["rolling_peer_shock_score"] = (
        s["lap_delta_spread_roll3_prev"].fillna(0)
        + s["lap_driver_jump_median_roll3_prev"].abs().fillna(0)
        + s["lap_degradation_change_median_roll3_prev"].abs().fillna(0)
    )
    s["neutralization_like_lap"] = (
        (s["lap_pit_fraction"] >= 0.18)
        | (
            s["lap_delta_spread"]
            >= s["lap_delta_spread_roll3_prev"].fillna(s["lap_delta_spread"]) * 1.8
        )
        | (s["lap_low_grip_fraction"].diff().abs().fillna(0) > 0.15)
    ).astype("int8")

    s["shock_x_tyre_life"] = s["peer_shock_score"] * s["TyreLife"]
    s["prev_shock_x_tyre_life"] = s["prev_peer_shock_score"] * s["TyreLife"]
    s["shock_x_position"] = s["peer_shock_score"] * (21 - s["Position"])
    s["shock_x_old_tyre"] = s["peer_shock_score"] * s["is_old_tyre"]
    s["shock_x_low_grip"] = s["peer_shock_score"] * s["is_low_grip_compound"]
    s["pit_fraction_x_stint"] = s["lap_pit_fraction"].fillna(0) * s["Stint"]
    s["pit_fraction_x_tyre_life"] = s["lap_pit_fraction"].fillna(0) * s["TyreLife"]
    s["compound_tyre_bucket"] = (
        s["Compound"].astype(str)
        + "_"
        + pd.cut(
            s["TyreLife"], [-1, 3, 10, 20, 100], labels=["new", "young", "mid", "old"]
        ).astype(str)
    )

    s = s.sort_values("_orig_idx")
    s = s.drop(columns=["_orig_idx"])
    return s
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000714"],
        "research_usage_note": "Verified assigned hypothesis 000714.",
    }, sort_keys=True))
