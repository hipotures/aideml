...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_monaco = race.str.contains("Monaco", case=False, na=False).astype("int8")
    is_monaco_2025 = ((is_monaco == 1) & (year == 2025)).astype("int8")

    max_lap_by_race_year = (
        out.groupby(["Race", "Year"], sort=False)["LapNumber"]
        .transform("max")
        .astype("float32")
    )
    est_total_laps = np.maximum(max_lap_by_race_year, lap / progress.clip(lower=0.01))
    laps_to_finish = (est_total_laps - lap).clip(lower=0).astype("float32")

    dry_pressure = ((is_slick == 1) & (laps_to_finish <= 18) & (stint <= 2)).astype(
        "int8"
    )
    late_dry_pressure = ((is_slick == 1) & (laps_to_finish <= 10)).astype("int8")
    long_stint_pressure = (
        (is_slick == 1) & (tyre_life >= 18) & (laps_to_finish <= 25)
    ).astype("int8")

    out["regime_wet_or_inter"] = is_wet
    out["regime_dry_slick"] = is_slick
    out["regime_monaco_2025"] = is_monaco_2025
    out["regime_dry_rule_pressure"] = dry_pressure
    out["regime_late_dry_pressure"] = late_dry_pressure
    out["regime_long_stint_pressure"] = long_stint_pressure
    out["est_total_laps"] = est_total_laps.astype("float32")
    out["laps_to_finish"] = laps_to_finish
    out["race_progress_x_laps_left"] = (progress * laps_to_finish).astype("float32")
    ...
    for col in [
        ...
        out[f"{col}_wet_gate"] = (x * is_wet).astype("float32")
        out[f"{col}_dry_gate"] = (x * is_slick).astype("float32")
        out[f"{col}_dry_pressure_gate"] = (x * dry_pressure).astype("float32")
        out[f"{col}_monaco2025_gate"] = (x * is_monaco_2025).astype("float32")

    out["tyre_life_per_lap_left"] = (tyre_life / (laps_to_finish + 1.0)).astype(
        "float32"
    )
    out["stint_x_laps_left"] = (stint.astype("float32") * laps_to_finish).astype(
        "float32"
    )
    out["slick_tyre_age_pressure"] = (is_slick * tyre_life * (1.0 - progress)).astype(
        "float32"
    )
    out["wet_tyre_age_late"] = (is_wet * tyre_life * progress).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000554"],
        "research_usage_note": "Verified assigned hypothesis 000554.",
    }, sort_keys=True))
