...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.copy()
    pit = X["PitStop"].astype("float32").fillna(0.0)

    X["Race_Year"] = X["Race"].astype(str) + "_" + X["Year"].astype(str)
    X["Race_Compound"] = X["Race"].astype(str) + "_" + X["Compound"].astype(str)
    X["Driver_Compound"] = X["Driver"].astype(str) + "_" + X["Compound"].astype(str)
    X["Driver_Compound_Stint"] = (
        X["Driver"].astype(str)
        + "_"
    ...
    lap_bucket = np.clip(
        (X["RaceProgress"].astype("float32") * 10).fillna(0).astype(int), 0, 10
    )
    X["Race_LapBucket"] = X["Race"].astype(str) + "_" + lap_bucket.astype(str)

    global_count = np.arange(len(X), dtype=np.float32)
    global_sum = pit.cumsum().shift(1, fill_value=0).astype("float32")
    global_prior = (global_sum + 1.0) / (global_count + 2.0)
    X["ord_global_pit_prior"] = global_prior.astype("float32")

    def add_ordered_prior(key_col, out_col, parent_col, alpha):
        grp = X.groupby(key_col, sort=False)["PitStop"]
        prev_sum = grp.cumsum().shift(1, fill_value=0).astype("float32")
        prev_count = grp.cumcount().astype("float32")
        parent = X[parent_col].astype("float32")
        post = (prev_sum + alpha * parent) / (prev_count + alpha)
        X[out_col] = post.astype("float32")
        X[out_col + "_minus_parent"] = (post - parent).astype("float32")
        X[out_col + "_count"] = prev_count.astype("float32")

    add_ordered_prior("Driver", "ord_driver_pit_prior", "ord_global_pit_prior", 20.0)
    add_ordered_prior("Race", "ord_race_pit_prior", "ord_global_pit_prior", 20.0)
    add_ordered_prior(
        "Race_Year", "ord_race_year_pit_prior", "ord_race_pit_prior", 12.0
    )
    add_ordered_prior(
        "Race_Compound", "ord_race_compound_pit_prior", "ord_race_pit_prior", 12.0
    )
    add_ordered_prior(
        "Driver_Compound", "ord_driver_compound_pit_prior", "ord_driver_pit_prior", 10.0
    )
    add_ordered_prior(
        "Driver_Compound_Stint",
        "ord_driver_compound_stint_pit_prior",
        "ord_driver_compound_pit_prior",
        8.0,
    )
    add_ordered_prior(
        "Race_LapBucket", "ord_race_lapbucket_pit_prior", "ord_race_pit_prior", 10.0
    )

    X["ord_driver_x_race_gap"] = (
        X["ord_driver_pit_prior"] - X["ord_race_pit_prior"]
    ).astype("float32")
    X["ord_driver_compound_x_race_compound_gap"] = (
        X["ord_driver_compound_pit_prior"] - X["ord_race_compound_pit_prior"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000830"],
        "research_usage_note": "Verified assigned hypothesis 000830.",
    }, sort_keys=True))
