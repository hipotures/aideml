...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reset consistency around observed current-lap pit stops
    out["pitstop_prev_lap"] = out["PitStop_lag1"].fillna(0)
    out["pitstop_last2"] = out["PitStop_lag1"].fillna(0) + out["PitStop_lag2"].fillna(0)
    out["aftermath_flag"] = (out["pitstop_prev_lap"] > 0).astype("int8")
    out["current_pit_reset_match"] = (
        (out["PitStop"] > 0) & (out["TyreLife"] <= 2)
    ).astype("int8")
