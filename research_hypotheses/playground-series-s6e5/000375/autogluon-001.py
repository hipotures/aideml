...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    snapshot_keys = ["Year", "Race", "LapNumber"]
    compound_col = "Compound"

    X = X.sort_values(snapshot_keys + ["Driver", "Stint"], kind="mergesort")

    snap = (
        X.groupby(snapshot_keys, sort=False)
        .agg(
            snapshot_pit_share=("PitStop", "mean"),
            snapshot_pit_count=("PitStop", "sum"),
            snapshot_field_size=("PitStop", "size"),
            snapshot_laptime_delta_median=("LapTime_Delta", "median"),
            snapshot_laptime_delta_std=("LapTime_Delta", "std"),
            snapshot_tyrelife_median=("TyreLife", "median"),
            snapshot_deg_median=("Cumulative_Degradation", "median"),
            snapshot_position_median=("Position", "median"),
        )
        .reset_index()
    )

    snap["snapshot_laptime_delta_std"] = snap["snapshot_laptime_delta_std"].fillna(0.0)

    compound_mix = (
        X.groupby(snapshot_keys + [compound_col], sort=False)
        .size()
        .unstack(fill_value=0)
        .reset_index()
    )
    compound_names = [c for c in compound_mix.columns if c not in snapshot_keys]
    for c in compound_names:
        compound_mix[f"compound_share_{str(c).lower()}"] = compound_mix[
            c
        ] / compound_mix[compound_names].sum(axis=1)
    compound_mix = compound_mix[
        snapshot_keys + [f"compound_share_{str(c).lower()}" for c in compound_names]
    ]

    snap = snap.merge(compound_mix, on=snapshot_keys, how="left", sort=False)

    race_keys = ["Year", "Race"]
    snap = snap.sort_values(snapshot_keys, kind="mergesort")

    snap["prev_lap_pit_count"] = (
        snap.groupby(race_keys, sort=False)["snapshot_pit_count"].shift(1).fillna(0.0)
    )
    snap["prev_lap_pit_share"] = (
        snap.groupby(race_keys, sort=False)["snapshot_pit_share"].shift(1).fillna(0.0)
    )

    lag_cols = [
        "snapshot_pit_share",
        "snapshot_pit_count",
        "snapshot_laptime_delta_median",
        "snapshot_laptime_delta_std",
    ]
    for col in lag_cols:
        snap[f"{col}_lag1_delta"] = snap[col] - snap.groupby(race_keys, sort=False)[
            col
        ].shift(1)
        snap[f"{col}_lag1_delta"] = snap[f"{col}_lag1_delta"].fillna(0.0)

    X = X.merge(snap, on=snapshot_keys, how="left", sort=False)

    X["laptime_delta_vs_snapshot_median"] = (
        X["LapTime_Delta"] - X["snapshot_laptime_delta_median"]
    )
    X["tyrelife_vs_snapshot_median"] = X["TyreLife"] - X["snapshot_tyrelife_median"]
    X["degradation_vs_snapshot_median"] = (
        X["Cumulative_Degradation"] - X["snapshot_deg_median"]
    )
    X["position_vs_snapshot_median"] = X["Position"] - X["snapshot_position_median"]

    X["laptime_delta_z_snapshot"] = X["laptime_delta_vs_snapshot_median"] / (
        X["snapshot_laptime_delta_std"] + 1e-6
    )

    compound_snapshot_keys = snapshot_keys + [compound_col]
    X["tyrelife_pct_within_compound_snapshot"] = (
        X.groupby(compound_snapshot_keys, sort=False)["TyreLife"]
        .rank(method="average", pct=True)
        .astype("float32")
    )
    X["degradation_pct_within_compound_snapshot"] = (
        X.groupby(compound_snapshot_keys, sort=False)["Cumulative_Degradation"]
        .rank(method="average", pct=True)
        .astype("float32")
    )

    X["snapshot_pit_pressure_vs_prev"] = (
        X["snapshot_pit_share"] - X["prev_lap_pit_share"]
    )
    X["snapshot_pit_count_excl_self"] = X["snapshot_pit_count"] - X["PitStop"]
    X["snapshot_pit_share_excl_self"] = X["snapshot_pit_count_excl_self"] / np.clip(
        X["snapshot_field_size"] - 1, 1, None
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000375"],
        "research_usage_note": "Verified assigned hypothesis 000375.",
    }, sort_keys=True))
