...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["regime"] = np.where(df["Compound"].isin(["INTERMEDIATE", "WET"]), "wet", "dry")
    df["lap_query"] = (
        df["Year"].astype(str)
        + "_"
        + df["Race"].astype(str)
    ...
    risk_group = ["Year", "Race", "LapNumber", "regime"]
    num_cols = [
        "TyreLife",
    ...
    g = df.groupby(risk_group, sort=False)
    df["risk_set_size"] = g["Driver"].transform("size").astype("float32")

    for col in num_cols:
        s = df[col].astype("float32")
        mean = g[col].transform("mean").astype("float32")
        std = g[col].transform("std").fillna(0).astype("float32")
        minv = g[col].transform("min").astype("float32")
        maxv = g[col].transform("max").astype("float32")
        med = g[col].transform("median").astype("float32")

        df[f"{col}_grp_mean"] = mean
        df[f"{col}_grp_gap_mean"] = (s - mean).astype("float32")
        df[f"{col}_grp_gap_min"] = (s - minv).astype("float32")
        df[f"{col}_grp_gap_max"] = (maxv - s).astype("float32")
        df[f"{col}_grp_z"] = ((s - mean) / (std + 1e-3)).astype("float32")
        df[f"{col}_grp_range_pos"] = ((s - minv) / (maxv - minv + 1e-3)).astype(
            "float32"
        )
        df[f"{col}_grp_rank_pct"] = (
            g[col].rank(method="average", pct=True).astype("float32")
        )
        df[f"{col}_grp_rank_dense"] = g[col].rank(method="dense").astype("float32")
        df[f"{col}_grp_gap_med"] = (s - med).astype("float32")
    ...
    df["stint_x_tyre_life"] = (df["Stint"] * df["TyreLife"]).astype("float32")
    df["deg_x_progress"] = (df["Cumulative_Degradation"] * df["RaceProgress"]).astype(
        "float32"
    )
    df["pace_loss_x_tyre"] = (df["LapTime_Delta"] * df["TyreLife"]).astype("float32")
    df["position_pressure"] = ((21 - df["Position"]) / 20.0).astype("float32")
    df["late_race_flag"] = (df["RaceProgress"] >= 0.7).astype("int8")
    df["old_tyre_flag"] = (df["TyreLife"] >= g["TyreLife"].transform("median")).astype(
        "int8"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 001007.",
    }, sort_keys=True))
