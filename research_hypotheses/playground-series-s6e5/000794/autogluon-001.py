...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ff1_is_wet_compound"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["ff1_is_slick_compound"] = comp.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")

    lap_groups = out.groupby(lap_key, sort=False, observed=True)
    race_groups = out.groupby(race_key, sort=False, observed=True)

    out["ff1_lap_field_size"] = lap_groups["Driver"].transform("count").astype("int16")
    out["ff1_lap_pit_count"] = lap_groups["PitStop"].transform("sum").astype("float32")
    out["ff1_lap_pit_rate"] = (
        out["ff1_lap_pit_count"] / out["ff1_lap_field_size"].clip(lower=1)
    ).astype("float32")
    out["ff1_lap_wet_count"] = (
        lap_groups["ff1_is_wet_compound"].transform("sum").astype("float32")
    )
    out["ff1_lap_wet_rate"] = (
        out["ff1_lap_wet_count"] / out["ff1_lap_field_size"].clip(lower=1)
    ).astype("float32")

    abs_pos_change = out["Position_Change"].abs()
    out["ff1_lap_position_churn_mean"] = (
        lap_groups[abs_pos_change.name].transform("mean").astype("float32")
        if abs_pos_change.name in out.columns
        else lap_groups["Position_Change"]
        .transform(lambda s: s.abs().mean())
        .astype("float32")
    )
    out["ff1_large_position_change"] = (out["Position_Change"].abs() >= 4).astype(
        "int8"
    )
    out["ff1_lap_incident_proxy_count"] = (
        lap_groups["ff1_large_position_change"].transform("sum").astype("float32")
    )
    out["ff1_lap_incident_proxy_rate"] = (
        out["ff1_lap_incident_proxy_count"] / out["ff1_lap_field_size"].clip(lower=1)
    ).astype("float32")

    lap_median_time = lap_groups["LapTime (s)"].transform("median")
    lap_median_delta = lap_groups["LapTime_Delta"].transform("median")
    out["ff1_lap_time_vs_field_median"] = (out["LapTime (s)"] - lap_median_time).astype(
        "float32"
    )
    out["ff1_lap_delta_vs_field_median"] = (
        out["LapTime_Delta"] - lap_median_delta
    ).astype("float32")

    race_lap = out[
        lap_key
        + ["ff1_lap_pit_rate", "ff1_lap_wet_rate", "ff1_lap_incident_proxy_rate"]
    ].drop_duplicates(lap_key)
    race_lap = race_lap.sort_values(lap_key)
    g = race_lap.groupby(race_key, sort=False, observed=True)

    race_lap["ff1_prev_lap_pit_rate"] = (
        g["ff1_lap_pit_rate"].shift(1).fillna(0).astype("float32")
    )
    race_lap["ff1_prev_lap_wet_rate"] = (
        g["ff1_lap_wet_rate"].shift(1).fillna(0).astype("float32")
    )
    race_lap["ff1_prev_lap_incident_proxy_rate"] = (
        g["ff1_lap_incident_proxy_rate"].shift(1).fillna(0).astype("float32")
    )
    race_lap["ff1_wet_transition"] = (
        race_lap["ff1_lap_wet_rate"] - race_lap["ff1_prev_lap_wet_rate"]
    ).astype("float32")
    race_lap["ff1_incident_transition"] = (
        race_lap["ff1_lap_incident_proxy_rate"]
        - race_lap["ff1_prev_lap_incident_proxy_rate"]
    ).astype("float32")
    ...
    out = out.merge(
        race_lap[
            lap_key
            + [
                "ff1_prev_lap_pit_rate",
                "ff1_prev_lap_wet_rate",
                "ff1_prev_lap_incident_proxy_rate",
                "ff1_wet_transition",
                "ff1_incident_transition",
            ]
        ],
        on=lap_key,
        how="left",
    ...
    out["ff1_status_disruption_score"] = (
        0.45 * out["ff1_lap_incident_proxy_rate"]
        + 0.35 * out["ff1_lap_pit_rate"]
        + 0.20 * out["ff1_lap_wet_rate"]
    ).astype("float32")

    out["ff1_laps_since_race_start"] = (out["LapNumber"] - 1).astype("int16")
    out["ff1_laps_to_race_end_proxy"] = (
        ((out["LapNumber"] / out["RaceProgress"].clip(lower=0.01)) - out["LapNumber"])
        .clip(lower=0)
        .astype("float32")
    )
    out["ff1_tyre_age_progress_ratio"] = (
        out["TyreLife"] / out["LapNumber"].clip(lower=1)
    ).astype("float32")
    out["ff1_stint_progress_pressure"] = (out["TyreLife"] * out["RaceProgress"]).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000794.",
    }, sort_keys=True))
