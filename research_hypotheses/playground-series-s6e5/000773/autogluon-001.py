...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Robust clipping to reduce the impact of safety-car / anomalous lap spikes.
    ltd_lo, ltd_hi = out["LapTime_Delta"].quantile([0.01, 0.99])
    deg_lo, deg_hi = out["Cumulative_Degradation"].quantile([0.01, 0.99])
    out["LapTime_Delta_clip"] = out["LapTime_Delta"].clip(ltd_lo, ltd_hi)
    out["Cumulative_Degradation_clip"] = out["Cumulative_Degradation"].clip(
        deg_lo, deg_hi
    )

    g = out.groupby(group_cols, sort=False)

    out["ltd_lag1"] = g["LapTime_Delta_clip"].shift(1)
    out["ltd_lag2"] = g["LapTime_Delta_clip"].shift(2)
    out["deg_lag1"] = g["Cumulative_Degradation_clip"].shift(1)
    out["deg_lag2"] = g["Cumulative_Degradation_clip"].shift(2)

    out["ltd_diff1"] = out["LapTime_Delta_clip"] - out["ltd_lag1"]
    out["ltd_diff2"] = out["ltd_lag1"] - out["ltd_lag2"]
    out["ltd_second_diff"] = (
        out["LapTime_Delta_clip"] - 2.0 * out["ltd_lag1"] + out["ltd_lag2"]
    )

    out["deg_diff1"] = out["Cumulative_Degradation_clip"] - out["deg_lag1"]
    out["deg_second_diff"] = (
        out["Cumulative_Degradation_clip"] - 2.0 * out["deg_lag1"] + out["deg_lag2"]
    )

    out["ltd_slope3"] = (out["LapTime_Delta_clip"] - out["ltd_lag2"]) / 2.0
    out["deg_slope3"] = (out["Cumulative_Degradation_clip"] - out["deg_lag2"]) / 2.0

    out["ltd_ewm"] = g["LapTime_Delta_clip"].transform(
        lambda s: s.ewm(alpha=0.35, adjust=False).mean()
    )
    out["deg_ewm"] = g["Cumulative_Degradation_clip"].transform(
        lambda s: s.ewm(alpha=0.35, adjust=False).mean()
    )

    pos_pace_loss = out["LapTime_Delta_clip"].clip(lower=0)
    pos_deg_gain = out["deg_diff1"].clip(lower=0)
    out["latent_wear_state"] = (
        (pos_pace_loss.fillna(0) + 0.5 * pos_deg_gain.fillna(0))
        .groupby([out[c] for c in group_cols], sort=False)
        .transform(lambda s: s.ewm(alpha=0.22, adjust=False).mean())
    )

    race_max_lap = (
        out.groupby(["Year", "Race"], sort=False)["LapNumber"]
        .transform("max")
        .replace(0, np.nan)
    )
    out["laps_remaining_est"] = (race_max_lap - out["LapNumber"]).clip(lower=0)
    out["stint_progress"] = out["TyreLife"] / (
        out["TyreLife"] + out["laps_remaining_est"] + 1.0
    )

    compounds = ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]
    for comp in compounds:
        mask = (out["Compound"] == comp).astype("int8")
        out[f"is_{comp.lower()}"] = mask
        out[f"wear_x_{comp.lower()}"] = out["latent_wear_state"] * mask
        out[f"cliff_x_{comp.lower()}"] = out["ltd_second_diff"] * mask

    out["wear_x_laps_remaining"] = out["latent_wear_state"] * out["laps_remaining_est"]
    out["cliff_x_laps_remaining"] = out["ltd_second_diff"] * out["laps_remaining_est"]
    out["deg_accel_x_progress"] = out["deg_second_diff"] * out["stint_progress"]

    out = (
        out.sort_values("_orig_order", kind="mergesort")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000773"],
        "research_usage_note": "Verified assigned hypothesis 000773.",
    }, sort_keys=True))
