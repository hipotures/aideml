...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    baseline = np.full(n, float(fit_med), dtype="float64")
    ...
    try:
        ...
        qs = y.quantile([0.005, 0.995]).to_numpy()
        if np.all(np.isfinite(qs)) and qs[1] > qs[0]:
            y_fit = y.clip(qs[0], qs[1]).fillna(fit_med)
        else:
            y_fit = y.fillna(fit_med)

        if fit_rows >= 200:
            n_splits = 3
            folds = np.arange(fit_rows) % n_splits
        ...
        baseline = fallback.fillna(fit_med).to_numpy(dtype="float64")
