...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    deg_col = "Cumulative_Degradation"
    progress_col = "RaceProgress"
    pos_col = "Position"
    poschg_col = "Position_Change"

    comp = out[compound_col].astype("category")
    race = out[race_col].astype("category")
    driver = out[driver_col].astype("category")

    comp_code = comp.cat.codes.astype("int16")
    race_code = race.cat.codes.astype("int16")
    driver_code = driver.cat.codes.astype("int16")
    out["Compound_code"] = comp_code
    out["Race_code"] = race_code
    out["Driver_code"] = driver_code

    comp_str = out[compound_col].astype(str)
    out["is_wet_compound"] = comp_str.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["is_soft"] = (comp_str == "SOFT").astype("int8")
    out["is_medium"] = (comp_str == "MEDIUM").astype("int8")
    out["is_hard"] = (comp_str == "HARD").astype("int8")

    tyre = out[tyre_col].astype("float32")
    lap = out[lap_col].astype("float32")
    stint = out[stint_col].astype("float32")
    progress = out[progress_col].astype("float32")
    laptime = out[laptime_col].astype("float32")
    delta = out[delta_col].astype("float32")
    deg = out[deg_col].astype("float32")
    pos = out[pos_col].astype("float32")
    poschg = out[poschg_col].astype("float32")

    out["tyre_life_sq"] = tyre * tyre
    out["tyre_life_log1p"] = np.log1p(tyre).astype("float32")
    out["stint_progress"] = (tyre / lap.clip(lower=1)).astype("float32")
    out["race_remaining"] = (1.0 - progress).astype("float32")
    out["late_race_tyre"] = (tyre * progress).astype("float32")
    out["degradation_per_life"] = (deg / tyre.clip(lower=1)).astype("float32")
    out["abs_laptime_delta"] = delta.abs().astype("float32")
    out["position_pressure"] = ((21.0 - pos) * progress).astype("float32")
    out["position_change_abs"] = poschg.abs().astype("float32")
    out["wet_tyre_life"] = (out["is_wet_compound"].astype("float32") * tyre).astype(
        "float32"
    )

    comp_life = (
        out.groupby(compound_col, observed=True)[tyre_col]
        .transform("median")
        .astype("float32")
    )
    comp_delta = (
        out.groupby(compound_col, observed=True)[delta_col]
        .transform("median")
        .astype("float32")
    )
    race_lap_med = (
        out.groupby(race_col, observed=True)[laptime_col]
        .transform("median")
        .astype("float32")
    )
    out["compound_life_ratio"] = (tyre / comp_life.clip(lower=1)).astype("float32")
    out["delta_vs_compound"] = (delta - comp_delta).astype("float32")
    out["laptime_vs_race_median"] = (laptime - race_lap_med).astype("float32")

    obs = (
        0.60 * np.tanh(np.asarray(out["laptime_vs_race_median"], dtype="float32") / 8.0)
        + 0.25 * np.tanh(np.asarray(out["delta_vs_compound"], dtype="float32") / 6.0)
        + 0.15
        * np.tanh(np.asarray(out["degradation_per_life"], dtype="float32") / 12.0)
    ).astype("float32")

    sort_key = pd.DataFrame(
        {
            "driver": np.asarray(driver_code, dtype=np.int32),
            "race": np.asarray(race_code, dtype=np.int32),
            "year": np.asarray(out[year_col], dtype=np.int32),
            "lap": np.asarray(out[lap_col], dtype=np.int16),
            "stint": np.asarray(out[stint_col], dtype=np.int16),
            "idx": np.arange(n, dtype=np.int32),
        }
    )
    order = np.lexsort(
        (
            sort_key["idx"].to_numpy(),
            sort_key["lap"].to_numpy(),
            sort_key["stint"].to_numpy(),
            sort_key["year"].to_numpy(),
            sort_key["race"].to_numpy(),
            sort_key["driver"].to_numpy(),
        )
    )

    level_feat = np.zeros(n, dtype="float32")
    trend_feat = np.zeros(n, dtype="float32")
    uncert_feat = np.zeros(n, dtype="float32")
    shock_feat = np.zeros(n, dtype="float32")
    state_age_feat = np.zeros(n, dtype="float32")

    drv_arr = sort_key["driver"].to_numpy()
    race_arr = sort_key["race"].to_numpy()
    year_arr = sort_key["year"].to_numpy()
    stint_arr = sort_key["stint"].to_numpy()
    pit_arr = np.asarray(out[pit_col], dtype=np.int8)
    ...
    state_age = 0.0
    prev_i = -1
    prev_driver = prev_race = prev_year = prev_stint = None
    ...
    for i in order:
        new_seq = (
            prev_i < 0
            or drv_arr[i] != prev_driver
            or race_arr[i] != prev_race
            or year_arr[i] != prev_year
            or stint_arr[i] != prev_stint
            or pit_arr[prev_i] == 1
        )
        if new_seq:
            level = 0.0
        ...
        pred_level = level + trend
        pred_uncert = uncertainty + q_level + q_trend

        level_feat[i] = pred_level
        trend_feat[i] = trend
        uncert_feat[i] = pred_uncert
        state_age_feat[i] = state_age

        residual = float(obs[i] - pred_level)
        residual = max(-max_shock, min(max_shock, residual))
        shock_feat[i] = residual

        gain = pred_uncert / (pred_uncert + r_obs)
        new_level = pred_level + gain * residual
        trend = 0.82 * trend + 0.18 * (new_level - level)
        level = new_level
        uncertainty = (1.0 - gain) * pred_uncert + 0.01
        state_age += 1.0

        prev_i = i
        prev_driver = drv_arr[i]
        prev_race = race_arr[i]
        prev_year = year_arr[i]
        prev_stint = stint_arr[i]

    out["tyre_filter_level"] = level_feat
    out["tyre_filter_trend"] = trend_feat
    out["tyre_filter_uncertainty"] = uncert_feat
    out["tyre_filter_shock"] = shock_feat
    out["tyre_filter_state_age"] = state_age_feat
    out["tyre_filter_risk"] = (
        level_feat + 1.5 * trend_feat + 0.25 * shock_feat + 0.05 * tyre
    ).astype("float32")
    out["tyre_filter_level_x_life"] = (
        level_feat * np.log1p(np.asarray(tyre, dtype="float32"))
    ).astype("float32")
    out["tyre_filter_wet_risk"] = (
        out["is_wet_compound"].astype("float32") * out["tyre_filter_risk"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000391"],
        "research_usage_note": "Verified assigned hypothesis 000391.",
    }, sort_keys=True))
