...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stint_start_lap = g["LapNumber"].transform("min")
    s["stint_lap"] = s["LapNumber"] - stint_start_lap + 1
    s["stint_progress"] = s["stint_lap"] / g["LapNumber"].transform("count").clip(
        lower=1
    )

    window = 3
    s["landmark_window"] = ((s["stint_lap"] - 1) // window).astype("int16")
    s["landmark_phase"] = ((s["stint_lap"] - 1) % window).astype("int8")
    s["is_landmark_start"] = (s["landmark_phase"] == 0).astype("int8")

    s["tyrelife_x_progress"] = s["TyreLife"] * s["RaceProgress"]
    s["degradation_per_tyre_lap"] = s["Cumulative_Degradation"] / s["TyreLife"].clip(
        lower=1
    )
    s["lap_delta_x_tyrelife"] = s["LapTime_Delta"] * s["TyreLife"]
    s["position_pressure"] = s["Position"] + s["Position_Change"]
    ...
    for col in [
        ...
        s[f"prev_{col}"] = lag
        if col != "PitStop":
            s[f"chg_{col}"] = s[col] - lag

    s["prior_pit_in_stint"] = (
        g["PitStop"]
        .shift(1)
        .fillna(0)
        .groupby([s[k] for k in keys], observed=True, sort=False)
        .cumsum()
    )
    s["laps_since_prev_pit"] = s["stint_lap"] - s["prior_pit_in_stint"].replace(
        0, np.nan
    )
    s["laps_since_prev_pit"] = s["laps_since_prev_pit"].fillna(s["stint_lap"])

    num_cols = s.select_dtypes(include=[np.number]).columns
    s[num_cols] = s[num_cols].replace([np.inf, -np.inf], np.nan).fillna(0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000705"],
        "research_usage_note": "Verified assigned hypothesis 000705.",
    }, sort_keys=True))
