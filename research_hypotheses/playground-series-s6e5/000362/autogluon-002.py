...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    static_cat = ["Year", "Race", "Driver", "Compound"]
    for c in static_cat:
        out[c] = out[c].astype("category")

    seq_key = ["Year", "Race", "Driver"]
    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    out = out.sort_values(sort_cols, kind="mergesort")

    g = out.groupby(seq_key, sort=False)
    gs = out.groupby(seq_key + ["Stint"], sort=False)
    ...
    seq_groups = [out[c] for c in seq_key]
    stint_groups = [out[c] for c in seq_key + ["Stint"]]

    # Causal last-N-lap tokens within each driver-race trajectory.
    for col in dyn_cols:
        s = g[col]
        shifted = s.shift(1)
        out[f"{col}_lag1"] = s.shift(1)
        out[f"{col}_lag2"] = s.shift(2)
        out[f"{col}_diff1"] = out[col] - out[f"{col}_lag1"]
        out[f"{col}_roll3_mean"] = (
            shifted.groupby(seq_groups, sort=False)
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=list(range(len(seq_key))), drop=True)
        )
        out[f"{col}_roll5_mean"] = (
            shifted.groupby(seq_groups, sort=False)
    ...
    # Within-stint causal state.
    out["stint_lap_idx"] = gs.cumcount() + 1
    out["stint_progress"] = out["stint_lap_idx"] / out["TyreLife"].clip(lower=1)
    out["stint_progress_sq"] = out["stint_progress"] ** 2
    ...
    for col in [
        ...
        s = gs[col]
        shifted = s.shift(1)
        out[f"{col}_stint_lag1"] = s.shift(1)
        out[f"{col}_stint_diff1"] = out[col] - out[f"{col}_stint_lag1"]
        out[f"{col}_stint_roll3_mean"] = (
            shifted.groupby(stint_groups, sort=False)
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=list(range(len(seq_key) + 1)), drop=True)
        )

    # Sequence-specialist style interactions between static tyre context and dynamic wear/pace.
    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    out["Compound_code"] = (
        out["Compound"].astype(str).map(compound_map).fillna(-1).astype("int8")
    )
    out["degradation_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["pace_x_tyre_life"] = out["LapTime_Delta"] * out["TyreLife"]
    out["pace_x_deg"] = out["LapTime_Delta"] * out["Cumulative_Degradation"]
    out["progress_x_deg"] = out["RaceProgress"] * out["Cumulative_Degradation"]
    out["compound_x_tyre_life"] = out["Compound_code"] * out["TyreLife"]
    out["compound_x_deg"] = out["Compound_code"] * out["Cumulative_Degradation"]
    out["position_pressure"] = out["Position"] * out["RaceProgress"]
    out["late_race_stint"] = out["stint_progress"] * out["RaceProgress"]

    # Missingness indicators help when early laps have little causal history.
    new_cols = [c for c in out.columns if c not in df.columns and c != "_row_id"]
    for c in new_cols:
        if out[c].dtype.kind in "biufc":
            out[f"{c}_na"] = out[c].isna().astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000362"],
        "research_usage_note": "Verified assigned hypothesis 000362.",
    }, sort_keys=True))
