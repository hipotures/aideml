...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Robust local pace reference and lap-time excess.
    ry_med = out.groupby(key_ry)["LapTime (s)"].transform("median")
    r_med = out.groupby(key_r)["LapTime (s)"].transform("median")
    global_med = float(out["LapTime (s)"].median())
    out["lap_time_excess_ry"] = out["LapTime (s)"] - ry_med.fillna(r_med).fillna(
        global_med
    )

    # Sort only for within-driver/race/stint lag features, then restore original row order.
    s = out[
        [
            "Race",
            "Year",
    ...
    s["_orig_idx"] = np.arange(len(s))
    s = s.sort_values(["Race", "Year", "Driver", "Stint", "LapNumber", "_orig_idx"])

    g_stint = s.groupby(["Race", "Year", "Driver", "Stint"], sort=False)
    prev_lap_time = g_stint["LapTime (s)"].shift(1)
    prev_tyre_life = g_stint["TyreLife"].shift(1)
    local_slope = (s["LapTime (s)"] - prev_lap_time) / (
        s["TyreLife"] - prev_tyre_life
    ).replace(0, np.nan)
    s["local_deg_slope"] = local_slope.replace([np.inf, -np.inf], np.nan).clip(-10, 10)

    # Post-stop warm-up proxy: first three tyre-life laps after a stop/stint start.
    s["warmup_lap"] = s["TyreLife"].where(s["TyreLife"].between(1, 3), np.nan)
    warm_src = s[s["warmup_lap"].notna()].copy()

    warm_ry = warm_src.groupby(key_ry)["lap_time_excess_ry"].median()
    warm_r = warm_src.groupby(key_r)["lap_time_excess_ry"].median()
    warm_global = (
        float(warm_src["lap_time_excess_ry"].median()) if len(warm_src) else 0.0
    )

    pit_src = out[out["PitStop"] == 1]
    pit_ry = pit_src.groupby(key_ry)["lap_time_excess_ry"].median()
    pit_r = pit_src.groupby(key_r)["lap_time_excess_ry"].median()
    pit_global = float(pit_src["lap_time_excess_ry"].median()) if len(pit_src) else 0.0

    slope_aligned = pd.Series(
        s["local_deg_slope"].values, index=s["_orig_idx"]
    ).sort_index()
    out["local_deg_slope"] = slope_aligned.reindex(np.arange(len(out))).values
    out["local_deg_slope"] = out["local_deg_slope"].fillna(
        out.groupby(key_ry)["local_deg_slope"].transform("median")
    )
    out["local_deg_slope"] = out["local_deg_slope"].fillna(
        out.groupby(key_r)["local_deg_slope"].transform("median")
    )
    out["local_deg_slope"] = out["local_deg_slope"].fillna(
        float(out["local_deg_slope"].median())
        if out["local_deg_slope"].notna().any()
        else 0.0
    )

    ry_index = pd.MultiIndex.from_frame(out[key_ry])
    out["race_year_pit_loss_excess"] = (
        pd.Series(ry_index.map(pit_ry), index=out.index)
        .fillna(out["Race"].map(pit_r))
        .fillna(pit_global)
        .astype(float)
    )
    out["race_year_warmup_penalty"] = (
        pd.Series(ry_index.map(warm_ry), index=out.index)
        .fillna(out["Race"].map(warm_r))
        .fillna(warm_global)
        .astype(float)
    )

    # Utility estimates for stopping now versus staying out.
    current_deg_cost_3 = np.maximum(out["local_deg_slope"], 0) * (
        out["TyreLife"] + 1.0 + out["TyreLife"] + 2.0 + out["TyreLife"] + 3.0
    )
    fresh_warm_cost_3 = out["race_year_warmup_penalty"] * 3.0
    pit_loss = np.maximum(out["race_year_pit_loss_excess"], 0)

    out["expected_next_3_lap_gain_minus_pit_loss"] = (
        current_deg_cost_3 - fresh_warm_cost_3 - pit_loss
    )
    per_lap_recovery = np.maximum(
        out["local_deg_slope"] * np.maximum(out["TyreLife"], 1.0)
        - out["race_year_warmup_penalty"],
        eps,
    )
    out["break_even_laps_to_recover_stop"] = (pit_loss / per_lap_recovery).clip(0, 100)
    out["pit_economics_pressure"] = out["expected_next_3_lap_gain_minus_pit_loss"] / (
        out["break_even_laps_to_recover_stop"] + 1.0
    )
    out["tyre_life_x_pit_loss"] = out["TyreLife"] * pit_loss
    out["degradation_minus_warmup"] = (
        out["local_deg_slope"] * out["TyreLife"] - out["race_year_warmup_penalty"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000502.",
    }, sort_keys=True))
