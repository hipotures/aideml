...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_len_est = (lap / progress).replace([np.inf, -np.inf], np.nan)
    laps_remaining = (race_len_est - lap).clip(lower=0)

    out["LapBin"] = pd.cut(
        laps_remaining,
        bins=[-0.1, 5, 10, 15, 20, 30, 45, 1000],
        labels=False,
    ).astype("int16")
    out["RaceYear"] = out["Race"].astype(str) + "_" + out["Year"].astype(str)
    out["AbsCompound"] = out["Compound"].astype(str)
    out["IsWetCompound"] = (
        out["AbsCompound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["LapsRemaining_Est"] = laps_remaining.astype("float32")
    out["TyreLife_Remaining_Ratio"] = (
        out["TyreLife"].astype(float) / (laps_remaining + 1.0)
    ).astype("float32")
    out["Stint_Lap_Interaction"] = (out["Stint"].astype(float) * lap).astype("float32")

    y = out["PitStop"].astype(float)
    global_mean = float(y.mean())
    global_alpha = 80.0

    def loo_prior(keys, name, alpha, parent=None):
        if isinstance(keys, str):
            keys = [keys]
        g = out.groupby(keys, observed=True)["PitStop"]
        sums = g.transform("sum").astype(float) - y
        cnts = g.transform("count").astype(float) - 1.0
        if parent is None:
            base = global_mean
        else:
            base = parent
        prior = (sums + alpha * base) / (cnts + alpha)
        out[name] = prior.astype("float32")
        out[name + "_gap_global"] = (prior - global_mean).astype("float32")
        if parent is not None:
            out[name + "_gap_parent"] = (prior - parent).astype("float32")
        out[name + "_count_log"] = np.log1p(cnts.clip(lower=0)).astype("float32")
        return prior

    p_abs_stint = loo_prior(
        ["AbsCompound", "Stint"], "Prior_AbsCompound_Stint", global_alpha
    )
    p_race_comp = loo_prior(
        ["RaceYear", "AbsCompound"], "Prior_RaceYear_AbsCompound", 60.0
    )
    p_race_comp_stint = loo_prior(
        ["RaceYear", "AbsCompound", "Stint"],
        "Prior_RaceYear_AbsCompound_Stint",
        35.0,
        parent=0.5 * p_race_comp + 0.5 * p_abs_stint,
    )
    loo_prior(
        ["RaceYear", "AbsCompound", "Stint", "LapBin"],
        "Prior_RaceYear_AbsCompound_Stint_LapBin",
        20.0,
        parent=p_race_comp_stint,
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000186.",
    }, sort_keys=True))
