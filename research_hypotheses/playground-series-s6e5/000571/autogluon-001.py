...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["field_size_lap"] = (
        out.groupby(keys)["Position"].transform("max").astype("float32")
    )
    out["cars_on_lap"] = (
        out.groupby(keys)["Position"].transform("count").astype("float32")
    )

    pit_by_lap = out.groupby(keys)["PitStop"].transform("sum").astype("float32")
    out["same_lap_pitters"] = pit_by_lap
    out["same_lap_pit_frac"] = (pit_by_lap / out["cars_on_lap"].clip(lower=1)).astype(
        "float32"
    )
    ...
    race_pos_mean = out.groupby(race_keys)["Position"].transform("mean")
    est_loss = (pit_pos_mean.fillna(race_pos_mean) * 0.55 + 5.0).clip(3, 14)
    out["est_pit_loss_pos"] = est_loss.astype("float32")

    rejoin_pos = (out["Position"].astype("float32") + out["est_pit_loss_pos"]).clip(
        lower=1, upper=out["field_size_lap"].clip(lower=1)
    )
    out["expected_rejoin_pos"] = rejoin_pos.astype("float32")
    out["rejoin_field_pct"] = (rejoin_pos / out["field_size_lap"].clip(lower=1)).astype(
        "float32"
    )
    out["positions_lost_if_stop"] = (rejoin_pos - out["Position"]).astype("float32")

    pos = out["Position"].astype("float32")
    out["gap_to_front_pos"] = (
        (pos - out.groupby(keys)["Position"].transform("min"))
        .clip(lower=0)
        .astype("float32")
    )
    out["gap_to_back_pos"] = (
        (out.groupby(keys)["Position"].transform("max") - pos)
        .clip(lower=0)
        .astype("float32")
    )
    out["traffic_density_rejoin"] = (
        out.groupby(keys)["Position"].transform(
            lambda s: s.astype(float).rolling(5, min_periods=1, center=True).count()
        )
        / 5.0
    ).astype("float32")

    lap_pit_rate = out.groupby(keys)["PitStop"].transform("mean").astype("float32")
    race_lap_p75 = lap_pit_rate.groupby([out["Year"], out["Race"]]).transform(
        lambda s: s.quantile(0.75)
    )
    out["open_window_lap"] = (
        lap_pit_rate.ge(race_lap_p75) & out["RaceProgress"].between(0.18, 0.88)
    ).astype("int8")
    out["near_common_stop_age"] = (
        (
            out["TyreLife"]
            - out["TyreLife"]
            .where(out["PitStop"].eq(1))
            .groupby([out["Year"], out["Race"], out["Compound"]])
            .transform("median")
        )
        .fillna(0)
        .astype("float32")
    ...
    out["cover_attack_rival"] = (
        (out["Position"].between(2, 12))
        & (out["Position_Change"].abs().le(2))
        & (
            out["LapTime_Delta"]
            .abs()
            .le(
        ...
        & out["RaceProgress"].between(0.20, 0.85)
    ).astype("int8")

    out["rejoin_clear_air_proxy"] = (
        (out["expected_rejoin_pos"].ge(out["field_size_lap"] - 3))
        | (out["traffic_density_rejoin"].le(0.45))
    ).astype("int8")
    out["rejoin_traffic_risk"] = (
        (out["traffic_density_rejoin"] * (1.0 - out["rejoin_field_pct"])).clip(0, 1)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000571"],
        "research_usage_note": "Verified assigned hypothesis 000571.",
    }, sort_keys=True))
