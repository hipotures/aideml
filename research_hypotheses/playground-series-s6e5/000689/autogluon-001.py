...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre = out["TyreLife"].astype(float)
    lap_delta = out["LapTime_Delta"].astype(float)
    degr = out["Cumulative_Degradation"].astype(float)
    stint = out["Stint"].astype(float)
    progress = out["RaceProgress"].astype(float)

    g = out.groupby(group_cols, sort=False)
    field_size = g["Position"].transform("count").astype(float).clip(lower=1.0)
    min_pos = g["Position"].transform("min").astype(float)
    max_pos = g["Position"].transform("max").astype(float)
    pos_span = (max_pos - min_pos).replace(0, np.nan)

    out["field_size_lap"] = field_size
    out["position_norm_lap"] = ((pos - min_pos) / pos_span).fillna(0.0)
    out["front_field_gap_pos"] = pos - min_pos
    out["rear_field_gap_pos"] = max_pos - pos

    out["lapdelta_pct_lap"] = (
        g["LapTime_Delta"].rank(pct=True, method="average").astype(float)
    )
    out["degradation_pct_lap"] = (
        g["Cumulative_Degradation"].rank(pct=True, method="average").astype(float)
    )
    out["tyrelife_pct_lap"] = (
        g["TyreLife"].rank(pct=True, method="average").astype(float)
    )
    out["stint_pct_lap"] = g["Stint"].rank(pct=True, method="average").astype(float)

    lap_mean_delta = g["LapTime_Delta"].transform("mean").astype(float)
    lap_std_delta = g["LapTime_Delta"].transform("std").replace(0, np.nan).astype(float)
    lap_mean_degr = g["Cumulative_Degradation"].transform("mean").astype(float)
    lap_std_degr = (
        g["Cumulative_Degradation"].transform("std").replace(0, np.nan).astype(float)
    )

    out["lapdelta_vs_field"] = lap_delta - lap_mean_delta
    out["lapdelta_z_lap"] = ((lap_delta - lap_mean_delta) / lap_std_delta).fillna(0.0)
    out["degradation_vs_field"] = degr - lap_mean_degr
    out["degradation_z_lap"] = ((degr - lap_mean_degr) / lap_std_degr).fillna(0.0)

    sort_cols = group_cols + ["Position"]
    s = out[
        sort_cols
        + ["Compound", "TyreLife", "LapTime_Delta", "Cumulative_Degradation", "Stint"]
    ].sort_values(sort_cols)
    sg = s.groupby(group_cols, sort=False)

    ahead_pos = sg["Position"].shift(1).astype(float).reindex(out.index)
    behind_pos = sg["Position"].shift(-1).astype(float).reindex(out.index)
    ahead_comp = sg["Compound"].shift(1).reindex(out.index)
    behind_comp = sg["Compound"].shift(-1).reindex(out.index)
    ahead_tyre = sg["TyreLife"].shift(1).astype(float).reindex(out.index)
    behind_tyre = sg["TyreLife"].shift(-1).astype(float).reindex(out.index)
    ahead_delta = sg["LapTime_Delta"].shift(1).astype(float).reindex(out.index)
    behind_delta = sg["LapTime_Delta"].shift(-1).astype(float).reindex(out.index)
    ahead_degr = sg["Cumulative_Degradation"].shift(1).astype(float).reindex(out.index)
    behind_degr = (
        sg["Cumulative_Degradation"].shift(-1).astype(float).reindex(out.index)
    )
    ahead_stint = sg["Stint"].shift(1).astype(float).reindex(out.index)
    behind_stint = sg["Stint"].shift(-1).astype(float).reindex(out.index)

    out["gap_pos_to_ahead"] = (pos - ahead_pos).fillna(99.0)
    out["gap_pos_to_behind"] = (behind_pos - pos).fillna(99.0)
    out["has_rival_ahead"] = ahead_pos.notna().astype("int8")
    out["has_rival_behind"] = behind_pos.notna().astype("int8")
    out["rival_ahead_within2"] = (out["gap_pos_to_ahead"] <= 2).astype("int8")
    out["rival_behind_within2"] = (out["gap_pos_to_behind"] <= 2).astype("int8")
    out["rivals_within2_pos"] = out["rival_ahead_within2"] + out["rival_behind_within2"]

    out["tyrelife_vs_ahead"] = (tyre - ahead_tyre).fillna(0.0)
    out["tyrelife_vs_behind"] = (tyre - behind_tyre).fillna(0.0)
    out["lapdelta_vs_ahead"] = (lap_delta - ahead_delta).fillna(0.0)
    out["lapdelta_vs_behind"] = (lap_delta - behind_delta).fillna(0.0)
    out["degradation_vs_ahead"] = (degr - ahead_degr).fillna(0.0)
    out["degradation_vs_behind"] = (degr - behind_degr).fillna(0.0)
    out["stint_vs_ahead"] = (stint - ahead_stint).fillna(0.0)
    out["stint_vs_behind"] = (stint - behind_stint).fillna(0.0)

    out["compound_mismatch_ahead"] = (
        (out["Compound"] != ahead_comp) & ahead_comp.notna()
    ).astype("int8")
    out["compound_mismatch_behind"] = (
        (out["Compound"] != behind_comp) & behind_comp.notna()
    ).astype("int8")
    out["nearby_compound_mismatch"] = (
        out["compound_mismatch_ahead"] * out["rival_ahead_within2"]
        + out["compound_mismatch_behind"] * out["rival_behind_within2"]
    ).astype("int8")

    out["undercut_pressure_proxy"] = (
        out["rival_behind_within2"] * (out["lapdelta_vs_behind"] > 0).astype("int8")
        + out["rival_behind_within2"] * (out["tyrelife_vs_behind"] > 0).astype("int8")
        + out["compound_mismatch_behind"] * out["rival_behind_within2"]
    ).astype("int8")

    out["overcut_pressure_proxy"] = (
        out["rival_ahead_within2"] * (out["lapdelta_vs_ahead"] > 0).astype("int8")
        + out["rival_ahead_within2"] * (out["tyrelife_vs_ahead"] > 0).astype("int8")
        + out["compound_mismatch_ahead"] * out["rival_ahead_within2"]
    ).astype("int8")

    out["traffic_after_stop_proxy"] = ((pos + 3.0) / field_size).clip(0, 1)
    out["clean_air_after_stop_proxy"] = (out["gap_pos_to_behind"] > 2).astype(
        "int8"
    ) * (1.0 - out["position_norm_lap"])
    out["strategy_pressure_score"] = (
        out["rivals_within2_pos"].astype(float)
        + out["nearby_compound_mismatch"].astype(float)
        + out["lapdelta_pct_lap"]
        + out["degradation_pct_lap"]
        + progress
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000689"],
        "research_usage_note": "Verified assigned hypothesis 000689.",
    }, sort_keys=True))
