...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap_med = laptime.groupby(
        [out["Race"], out["Year"], out["LapNumber"]], sort=False
    ).transform("median")
    out["laptime_vs_field"] = laptime - race_lap_med

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber"]
    order = (
        out[sort_cols]
        .assign(_idx=np.arange(n))
        .sort_values(sort_cols + ["_idx"])
        .index.to_numpy()
    )

    state_level = np.zeros(n, dtype=np.float32)
    state_slope = np.zeros(n, dtype=np.float32)
    state_var = np.zeros(n, dtype=np.float32)
    state_resid = np.zeros(n, dtype=np.float32)
    state_gain = np.zeros(n, dtype=np.float32)

    obs_df = pd.DataFrame(
        {
            "key": list(zip(out["Year"], out["Race"], out["Driver"], out["Stint"])),
            "lap": lap,
            "obs": (
                0.50 * ltd.clip(-25, 25)
                + 0.30 * out["laptime_vs_field"].clip(-25, 25)
                + 0.20
                * deg.groupby(
    ...
    q_level, q_slope, r = 0.08, 0.015, 6.0
    last_key = None
    x0 = x1 = 0.0
    p00, p01, p11 = 25.0, 0.0, 4.0

    for idx in order:
        key = obs_df.at[idx, "key"]
        z = float(obs_df.at[idx, "obs"])
        ...
        if key != last_key:
            ...
            x0 = x0 + x1
            p00, p01, p11 = p00 + 2.0 * p01 + p11 + q_level, p01 + p11, p11 + q_slope
        ...
        p01_new = (1.0 - k0) * p01
        p11_new = p11 - k1 * p01
        p00, p01, p11 = max(p00_new, 1e-6), p01_new, max(p11_new, 1e-6)

        state_level[idx] = x0
        state_slope[idx] = x1
        state_var[idx] = p00
        state_resid[idx] = resid
        state_gain[idx] = k0

    out["kf_tyre_level"] = state_level
    out["kf_tyre_slope"] = state_slope
    out["kf_tyre_var"] = state_var
    out["kf_tyre_resid"] = state_resid
    out["kf_tyre_gain"] = state_gain
    out["kf_cliff_score"] = out["kf_tyre_slope"] * out["TyreLife"].astype(float) + out[
        "kf_tyre_resid"
    ].clip(lower=0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001002"],
        "research_usage_note": "Verified assigned hypothesis 001002.",
    }, sort_keys=True))
