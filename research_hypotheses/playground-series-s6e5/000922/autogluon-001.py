...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    keys_lap = ["Year", "Race", "LapNumber"]
    lap_col = "LapTime (s)"

    field_med = out.groupby(keys_lap, sort=False)[lap_col].transform("median")
    abs_dev = (out[lap_col] - field_med).abs()
    field_mad = abs_dev.groupby(
        [out["Year"], out["Race"], out["LapNumber"]], sort=False
    ).transform("median")

    out["pace_field_median"] = field_med
    out["pace_residual"] = out[lap_col] - field_med
    out["pace_resid_robust_z"] = out["pace_residual"] / (1.4826 * field_mad + eps)

    orig_idx = np.arange(len(out))
    work = out.assign(_orig_idx=orig_idx).sort_values(
        ["Year", "Race", "Driver", "Stint", "LapNumber", "_orig_idx"], kind="mergesort"
    )

    gcols = ["Year", "Race", "Driver", "Stint"]
    g = work.groupby(gcols, sort=False)

    prev_resid = g["pace_resid_robust_z"].shift(1)
    prev2_resid = g["pace_resid_robust_z"].shift(2)

    work["pace_resid_prev"] = prev_resid
    work["pace_resid_accel"] = prev_resid - prev2_resid
    work["pace_resid_ewm"] = g["pace_resid_robust_z"].transform(
        lambda s: s.shift(1).ewm(alpha=0.35, adjust=False, min_periods=1).mean()
    )

    first_resid = g["pace_resid_robust_z"].transform("first")
    work["pace_resid_vs_stint_start"] = prev_resid - first_resid

    pos_prev = prev_resid.clip(lower=0).fillna(0)
    work["pace_resid_cum_pos"] = pos_prev.groupby(
        [work["Year"], work["Race"], work["Driver"], work["Stint"]], sort=False
    ).cumsum()

    work["pace_resid_roll3_mean"] = g["pace_resid_robust_z"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).mean()
    )
    work["pace_resid_roll3_max"] = g["pace_resid_robust_z"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).max()
    )

    out = work.sort_values("_orig_idx", kind="mergesort").drop(columns=["_orig_idx"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000922"],
        "research_usage_note": "Verified assigned hypothesis 000922.",
    }, sort_keys=True))
