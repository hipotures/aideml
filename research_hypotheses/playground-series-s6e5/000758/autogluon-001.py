...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cum_deg = out["Cumulative_Degradation"].astype("float32")
    ...
    race_lap_base = (
        lap_delta.groupby([out["Year"], out["Race"], out["LapNumber"]], sort=False)
        .transform("median")
        .astype("float32")
    )
    compound_base = (
        lap_delta.groupby(out["Compound"], sort=False)
        .transform("median")
        .astype("float32")
    )
    signal = (
        (lap_delta - race_lap_base - 0.5 * compound_base)
    ...
    order = np.lexsort(
        (
            np.arange(n),
            out["LapNumber"].to_numpy(),
            out["Driver"].astype(str).to_numpy(),
            out["Race"].astype(str).to_numpy(),
            out["Year"].to_numpy(),
        )
    )
    ...
    sig = signal.to_numpy(dtype=np.float32)[order]
    deg = cum_deg.to_numpy(dtype=np.float32)[order]
    life = tyre_life.to_numpy(dtype=np.float32)[order]
    pit_s = pit.to_numpy(dtype=np.int8)[order]

    wear = np.zeros(n, dtype=np.float32)
    slope = np.zeros(n, dtype=np.float32)
    innov = np.zeros(n, dtype=np.float32)
    pred_next = np.zeros(n, dtype=np.float32)
    reset_age = np.zeros(n, dtype=np.float32)
    ...
    for pos, idx in enumerate(order):
        ...
        pred = x_wear + x_slope
        obs = 0.75 * sig[pos] + 0.25 * deg_weight * deg[pos]
        err = obs - pred

        x_wear = pred + alpha * err
        x_slope = x_slope + beta * err
        age += 1.0

        wear[idx] = x_wear
        slope[idx] = x_slope
        innov[idx] = err
        pred_next[idx] = x_wear + x_slope
        reset_age[idx] = age

    out["latent_wear_000758"] = wear
    out["latent_wear_slope_000758"] = slope
    out["latent_wear_innovation_000758"] = innov
    out["latent_next_deterioration_000758"] = pred_next
    out["latent_reset_age_000758"] = reset_age
    out["latent_wear_per_tyre_life_000758"] = wear / (
        tyre_life.to_numpy(dtype=np.float32) + 1.0
    )
    out["latent_pressure_000758"] = (
        pred_next
        * np.log1p(tyre_life.to_numpy(dtype=np.float32))
        * (1.0 + race_progress.to_numpy(dtype=np.float32))
    )
    out["latent_stint_pressure_000758"] = pred_next * np.sqrt(
        np.maximum(stint.to_numpy(dtype=np.float32), 1.0)
    )
    out["latent_abs_innovation_000758"] = np.abs(innov).astype("float32")
    out["latent_reset_life_gap_000758"] = (
        tyre_life.to_numpy(dtype=np.float32) - reset_age
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000758"],
        "research_usage_note": "Verified assigned hypothesis 000758.",
    }, sort_keys=True))
