...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    race_lap = ["Race", "LapNumber"]

    wet_mask = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("float32")
    out["is_wet_or_inter"] = wet_mask
    out["is_slick"] = (1.0 - wet_mask).astype("float32")
    ...
    g = out.groupby(race_lap, sort=False, observed=True)
    out["race_lap_field_size"] = g["LapTime_Delta"].transform("size").astype("float32")
    out["race_lap_delta_median"] = (
        g["LapTime_Delta"].transform("median").astype("float32")
    )
    out["race_lap_delta_mean"] = g["LapTime_Delta"].transform("mean").astype("float32")
    out["race_lap_delta_std"] = (
        g["LapTime_Delta"].transform("std").fillna(0).astype("float32")
    )
    out["race_lap_delta_p95"] = (
        g["LapTime_Delta"].transform(lambda s: s.quantile(0.95)).astype("float32")
    )
    out["race_lap_time_median"] = g["LapTime (s)"].transform("median").astype("float32")
    out["race_lap_time_p95"] = (
        g["LapTime (s)"].transform(lambda s: s.quantile(0.95)).astype("float32")
    )
    out["race_lap_pit_count"] = g["PitStop"].transform("sum").astype("float32")
    out["race_lap_pit_share"] = (
        out["race_lap_pit_count"] / out["race_lap_field_size"].clip(lower=1)
    ).astype("float32")
    out["race_lap_wet_share"] = (
        wet_mask.groupby([out["Race"], out["LapNumber"]], sort=False)
        .transform("mean")
        .astype("float32")
    )
    ...
    lap_level = out[
        [
            *race_lap,
            ...
            "race_lap_delta_p95",
            "race_lap_time_median",
            "race_lap_pit_share",
            "race_lap_wet_share",
        ]
    ].drop_duplicates(race_lap)
    lap_level = lap_level.sort_values(["Race", "LapNumber"])
    for col in [
        "race_lap_delta_median",
        "race_lap_delta_p95",
        "race_lap_time_median",
        ...
        lap_level[col + "_prev"] = lap_level.groupby("Race", sort=False)[col].shift(1)
        lap_level[col + "_jump"] = (
            (lap_level[col] - lap_level[col + "_prev"]).fillna(0).astype("float32")
        )

    jump_cols = [c for c in lap_level.columns if c.endswith("_jump")]
    out = out.merge(
        lap_level[[*race_lap, *jump_cols]], on=race_lap, how="left", sort=False
    )
    for col in jump_cols:
        out[col] = out[col].fillna(0).astype("float32")

    out["driver_vs_field_delta"] = (
        out["LapTime_Delta"] - out["race_lap_delta_median"]
    ).astype("float32")
    out["driver_vs_field_laptime"] = (
        out["LapTime (s)"] - out["race_lap_time_median"]
    ).astype("float32")
    out["neutralized_slowdown_score"] = (
        out["race_lap_delta_median"].clip(lower=0)
        + 0.5 * out["race_lap_delta_p95"].clip(lower=0)
        + out["race_lap_delta_median_jump"].clip(lower=0)
    ).astype("float32")
    out["transition_pit_burst_score"] = (
        out["race_lap_pit_share"]
        + out["race_lap_wet_share"]
        + out["race_lap_wet_share_jump"].clip(lower=0)
    ).astype("float32")

    out["tyre_life_x_pit_burst"] = (out["TyreLife"] * out["race_lap_pit_share"]).astype(
        "float32"
    )
    out["tyre_life_x_wet_share"] = (out["TyreLife"] * out["race_lap_wet_share"]).astype(
        "float32"
    )
    out["progress_x_slowdown"] = (
        out["RaceProgress"] * out["neutralized_slowdown_score"]
    ).astype("float32")
    out["stint_x_transition"] = (
        out["Stint"] * out["transition_pit_burst_score"]
    ).astype("float32")
    out["late_race_transition_gate"] = (
        (out["RaceProgress"] > 0.55).astype("float32")
        * out["transition_pit_burst_score"]
    ).astype("float32")
    out["research_hypotheses_llm_claimed_used"] = '["000743"]'
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000743"],
        "research_usage_note": "Verified assigned hypothesis 000743.",
    }, sort_keys=True))
