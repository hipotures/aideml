...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre = out["TyreLife"].astype(float).clip(lower=0)
    stint = out["Stint"].astype(float).clip(lower=1)
    ...
    est_total_laps = est_total_laps.fillna(lap).clip(lower=lap, upper=90)
    laps_left = (est_total_laps - lap).clip(lower=0)

    out["est_total_laps"] = est_total_laps
    out["laps_left"] = laps_left
    out["tyre_life_sq"] = tyre * tyre
    out["tyre_life_race_frac"] = tyre / est_total_laps.clip(lower=1)
    out["stint_lap_ratio"] = tyre / stint
    out["degradation_pos"] = degr.clip(lower=0)
    out["degradation_per_lap"] = out["degradation_pos"] / tyre.clip(lower=1)
    out["lap_delta_pos"] = delta.clip(lower=0)
    out["lap_delta_neg_abs"] = (-delta).clip(lower=0)
    out["delta_pos_per_tyre_lap"] = out["lap_delta_pos"] / tyre.clip(lower=1)
    out["finish_excess"] = (tyre - laps_left).clip(lower=0)
    out["fresh_finish_margin"] = (laps_left - tyre).clip(lower=-90, upper=90)
    out["fresh_tyre_advantage"] = 1.0 / (tyre + 1.0)
    out["wear_pressure"] = (
        0.45 * out["tyre_life_race_frac"]
        + 0.35 * np.log1p(out["degradation_pos"])
        + 0.20 * np.log1p(out["lap_delta_pos"])
    )
    ...
    out["is_dry_compound"] = (
        out[["is_soft", "is_medium", "is_hard"]].max(axis=1).astype("int8")
    )

    out["early_race"] = (progress < 0.25).astype("int8")
    out["mid_race"] = ((progress >= 0.25) & (progress < 0.75)).astype("int8")
    out["late_race"] = (progress >= 0.75).astype("int8")
    out["old_tyre"] = (tyre >= 18).astype("int8")
    out["very_old_tyre"] = (tyre >= 30).astype("int8")
    out["near_finish"] = (laps_left <= 8).astype("int8")

    out["tyre_life_x_progress"] = tyre * progress
    out["degradation_per_lap_x_progress"] = out["degradation_per_lap"] * progress
    out["delta_pos_x_progress"] = out["lap_delta_pos"] * progress
    out["soft_wear_pressure"] = out["wear_pressure"] * out["is_soft"]
    out["hard_wear_pressure"] = out["wear_pressure"] * out["is_hard"]
    out["wet_wear_pressure"] = out["wear_pressure"] * out["is_wet"]
    out["late_wear_pressure"] = out["wear_pressure"] * out["late_race"]
    out["finish_excess_late"] = out["finish_excess"] * out["late_race"]

    out["position_loss"] = out["Position_Change"].astype(float).clip(lower=0)
    out["position_gain"] = (-out["Position_Change"].astype(float)).clip(lower=0)
    out["position_pressure"] = (
        out["Position"].astype(float) / 20.0 + out["position_loss"] / 20.0
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000458"],
        "research_usage_note": "Verified assigned hypothesis 000458.",
    }, sort_keys=True))
