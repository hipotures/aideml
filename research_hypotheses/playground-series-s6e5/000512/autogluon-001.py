...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").fillna("missing")
    year = out["Year"].astype("string").fillna("missing")
    race_year = (race + "_" + year).astype("string")
    out["Race_Year"] = race_year.astype("category")

    lap = pd.to_numeric(out["LapNumber"], errors="coerce")
    lap_time = pd.to_numeric(out["LapTime (s)"], errors="coerce")
    pit = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0)
    nonpit = pit.eq(0) & lap_time.notna()

    base_frame = pd.DataFrame(
        {
            "Race_Year": race_year.to_numpy(dtype=object),
            "LapNumber": lap.to_numpy(dtype="float64"),
            "LapTime": lap_time.to_numpy(dtype="float64"),
            "nonpit": nonpit.to_numpy(),
        }
    )

    lap_base = (
        base_frame.loc[base_frame["nonpit"]]
        .groupby(["Race_Year", "LapNumber"], sort=False)["LapTime"]
        .median()
    )
    base_frame = base_frame.join(
        lap_base.rename("lap_base"), on=["Race_Year", "LapNumber"]
    )

    race_base = (
        base_frame.loc[base_frame["nonpit"]]
        .groupby("Race_Year", sort=False)["LapTime"]
        .median()
    )
    global_base = float(base_frame.loc[base_frame["nonpit"], "LapTime"].median())
    if not np.isfinite(global_base):
        fallback = float(lap_time.median()) if lap_time.notna().any() else 0.0
        global_base = fallback if np.isfinite(fallback) else 0.0

    base = (
        base_frame["lap_base"]
        .fillna(base_frame["Race_Year"].map(race_base))
        .fillna(global_base)
    )
    residual_raw = lap_time.to_numpy(dtype="float64") - base.to_numpy(dtype="float64")
    residual_raw = np.nan_to_num(residual_raw, nan=0.0, posinf=0.0, neginf=0.0)
    residual = np.nan_to_num(np.clip(residual_raw, -60.0, 60.0), nan=0.0)

    out["race_lap_baseline_pace"] = base.to_numpy(dtype="float32")
    out["pace_residual_raw"] = residual_raw.astype("float32")
    out["pace_residual_clipped"] = residual.astype("float32")

    driver = out["Driver"].astype("string").fillna("missing")
    stint = out["Stint"].astype("string").fillna("missing")
    group_key = (race_year + "|" + driver + "|" + stint).astype("string")
    group_code = pd.factorize(group_key, sort=False)[0].astype("int32")
        ...
        pd.DataFrame(
            {
                ...
                "grp": group_code,
                "LapNumber": lap.fillna(-1).to_numpy(dtype="float64"),
                "resid": residual.astype("float64"),
            }
        )
        .sort_values(["grp", "LapNumber", "orig"], kind="mergesort")
        .reset_index(drop=True)
    )

    g = work.groupby("grp", sort=False)
    work["stint_lap_index"] = (g.cumcount() + 1).astype("float32")
    work["resid_lag1"] = g["resid"].shift(1)
    work["resid_diff1"] = work["resid"] - work["resid_lag1"]
    work["_resid_prior"] = g["resid"].shift(1)

    feature_cols = ["stint_lap_index", "resid_lag1", "resid_diff1"]
    windows = (2, 3, 5)

    for w in windows:
        roll = g["resid"].rolling(window=w, min_periods=1)
        work[f"resid_roll_mean_{w}"] = roll.mean().reset_index(level=0, drop=True)
        work[f"resid_roll_std_{w}"] = roll.std(ddof=0).reset_index(level=0, drop=True)

        lag_end = g["resid"].shift(w - 1)
        work[f"resid_slope_{w}"] = (work["resid"] - lag_end) / float(max(1, w - 1))

        if w == 2:
            prev = g["resid"].shift(1)
            prev2 = g["resid"].shift(2)
            work[f"resid_curvature_{w}"] = (work["resid"] - prev) - (prev - prev2)
        else:
            mid = max(1, (w - 1) // 2)
            work[f"resid_curvature_{w}"] = (
                work["resid"] - 2.0 * g["resid"].shift(mid) + lag_end
            )

        prior_roll = work.groupby("grp", sort=False)["_resid_prior"].rolling(
            window=w, min_periods=2
        )
        prior_mean = prior_roll.mean().reset_index(level=0, drop=True)
        prior_std = prior_roll.std(ddof=0).reset_index(level=0, drop=True)
        work[f"resid_last_z_{w}"] = (work["resid"] - prior_mean) / (prior_std + 0.25)

        work[f"resid_accel_{w}"] = work[f"resid_slope_{w}"] - work.groupby(
            "grp", sort=False
        )[f"resid_slope_{w}"].shift(1)

        feature_cols.extend(
            [
                f"resid_roll_mean_{w}",
                f"resid_roll_std_{w}",
                f"resid_slope_{w}",
                f"resid_curvature_{w}",
                f"resid_last_z_{w}",
                f"resid_accel_{w}",
            ]
        )

    orig = work["orig"].to_numpy(dtype=np.int64)
    for col in feature_cols:
        ...
        restored = np.empty(n, dtype="float32")
        restored[orig] = values
        out[col] = restored
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000512"],
        "research_usage_note": "Verified assigned hypothesis 000512.",
    }, sort_keys=True))
