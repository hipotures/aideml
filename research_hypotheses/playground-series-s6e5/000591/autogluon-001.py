...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["Is_WetRaceTyre"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    race_max_lap = (
        X.groupby(race_group, sort=False)["LapNumber"]
        .transform("max")
        .replace(0, np.nan)
    )
    X["LapFrac_FromLapNumber"] = (X["LapNumber"] / race_max_lap).astype("float32")
    X["TyreLife_Ratio"] = (X["TyreLife"] / X["LapNumber"].replace(0, np.nan)).astype(
        "float32"
    )
    X["TyreLife_vs_Progress"] = (
        X["TyreLife"] / (1.0 + 100.0 * X["RaceProgress"])
    ).astype("float32")
    X["Degradation_per_TyreLap"] = (
        X["Cumulative_Degradation"] / X["TyreLife"].replace(0, np.nan)
    ).astype("float32")
    X["LapDelta_per_LapTime"] = (
        X["LapTime_Delta"] / X["LapTime (s)"].replace(0, np.nan)
    ).astype("float32")
    X["Position_x_Progress"] = (X["Position"] * X["RaceProgress"]).astype("float32")
    X["Stint_x_TyreLife"] = (X["Stint"] * X["TyreLife"]).astype("float32")

    grp = X.groupby(lap_group, sort=False)
    X["LapGroup_Size"] = grp["Driver"].transform("size").astype("int16")
    ...
    for col in [
        ...
        gmean = grp[col].transform("mean")
        gstd = grp[col].transform("std").fillna(0.0)
        gmin = grp[col].transform("min")
        gmax = grp[col].transform("max")
        grank = grp[col].rank(method="average", pct=True)

        base = col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        X[f"{base}_lap_mean_diff"] = (X[col] - gmean).astype("float32")
        X[f"{base}_lap_z"] = ((X[col] - gmean) / (gstd + 1e-6)).astype("float32")
        X[f"{base}_lap_range_pos"] = ((X[col] - gmin) / ((gmax - gmin) + 1e-6)).astype(
            "float32"
        )
        X[f"{base}_lap_rank_pct"] = grank.astype("float32")

    race_grp = X.groupby(race_group, sort=False)
    for col in ["LapTime (s)", "LapTime_Delta", "TyreLife", "Cumulative_Degradation"]:
        rmean = race_grp[col].transform("mean")
        rstd = race_grp[col].transform("std").fillna(0.0)
        base = col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        X[f"{base}_race_z"] = ((X[col] - rmean) / (rstd + 1e-6)).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000591"],
        "research_usage_note": "Verified assigned hypothesis 000591.",
    }, sort_keys=True))
