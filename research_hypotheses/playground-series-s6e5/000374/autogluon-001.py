...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    total_laps_est = (lap / progress).clip(1, 100)
    out["LapsRemaining_Est"] = (total_laps_est - lap).clip(0, 100)
    ...
    default = (
        ...
        0.5,
        0.4,
    )  # street, tyre_stress, undercut, overtake_hard, safety_car
    meta = {
        "Bahrain Grand Prix": (0, 2.0, 0.8, 0.3, 0.3),
        "Saudi Arabian Grand Prix": (1, 1.2, 0.6, 0.4, 0.8),
        "Australian Grand Prix": (1, 1.0, 0.5, 0.5, 0.6),
        "Emilia Romagna Grand Prix": (0, 1.5, 0.7, 0.7, 0.5),
        "Miami Grand Prix": (1, 1.1, 0.6, 0.5, 0.5),
        "Monaco Grand Prix": (1, 0.8, 0.3, 1.0, 0.9),
        "Spanish Grand Prix": (0, 2.0, 0.8, 0.4, 0.3),
        "Canadian Grand Prix": (1, 1.1, 0.5, 0.5, 0.8),
        "Austrian Grand Prix": (0, 1.5, 0.7, 0.3, 0.4),
        "British Grand Prix": (0, 2.0, 0.6, 0.4, 0.5),
        "Hungarian Grand Prix": (0, 1.7, 0.7, 0.8, 0.4),
        "Belgian Grand Prix": (0, 1.8, 0.6, 0.3, 0.6),
        "Dutch Grand Prix": (0, 1.6, 0.7, 0.7, 0.6),
        "Italian Grand Prix": (0, 1.0, 0.5, 0.2, 0.4),
        "Singapore Grand Prix": (1, 1.4, 0.8, 0.9, 0.9),
        "Japanese Grand Prix": (0, 2.0, 0.6, 0.5, 0.5),
        "Qatar Grand Prix": (0, 2.0, 0.6, 0.4, 0.4),
        "United States Grand Prix": (0, 1.8, 0.7, 0.4, 0.4),
        "Mexico City Grand Prix": (0, 1.1, 0.5, 0.5, 0.4),
        "Sao Paulo Grand Prix": (0, 1.5, 0.7, 0.5, 0.7),
        "Las Vegas Grand Prix": (1, 0.8, 0.4, 0.3, 0.8),
        "Abu Dhabi Grand Prix": (0, 1.2, 0.6, 0.4, 0.5),
        "Azerbaijan Grand Prix": (1, 0.9, 0.5, 0.5, 0.9),
        "Chinese Grand Prix": (0, 1.6, 0.6, 0.4, 0.4),
        "French Grand Prix": (0, 1.5, 0.6, 0.3, 0.3),
        "Pre-Season Testing": (0, 1.0, 0.3, 0.3, 0.2),
    }

    race_meta = out["Race"].map(meta)
    vals = np.array(
        [x if isinstance(x, tuple) else default for x in race_meta], dtype=float
    )
    names = [
        "CircuitStreet",
        "TyreStressScore",
    ...
    remaining = out["LapsRemaining_Est"].astype(float)

    out["TyreLife_x_TyreStress"] = tyre_life * out["TyreStressScore"]
    out["TyreLife_x_Undercut"] = tyre_life * out["UndercutScore"]
    out["Delta_x_Undercut"] = lap_delta * out["UndercutScore"]
    out["Delta_x_TyreStress"] = lap_delta * out["TyreStressScore"]
    out["Position_x_OvertakeHard"] = position * out["OvertakeHardScore"]
    out["Remaining_x_Undercut"] = remaining * out["UndercutScore"]
    out["Remaining_x_SafetyCarRisk"] = remaining * out["SafetyCarRiskScore"]
    out["Street_TyreLife"] = out["CircuitStreet"] * tyre_life
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000374.",
    }, sort_keys=True))
