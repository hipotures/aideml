...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["IsWet"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["IsDry"] = 1 - out["IsWet"]
    ...
    out["RaceLapKey"] = (
        ...
        + "_"
        + out["IsWet"].astype(str)
    )
    out["RaceLapStintKey"] = out["RaceLapKey"] + "_S" + out["Stint"].astype(str)
    out["DriverRaceKey"] = (
        out["Year"].astype(str)
        + "_"
        + out["Race"].astype(str)
    ...
    out["CompoundStintKey"] = (
        out["Compound"].astype(str) + "_S" + out["Stint"].astype(str)
    )

    grp = out.groupby(["Year", "Race", "LapNumber", "IsWet"], sort=False)
    out["ContextSize"] = grp["Driver"].transform("size").astype("int16")
    ...
    for col in numeric_cols:
        mean_col = grp[col].transform("mean")
        std_col = grp[col].transform("std").fillna(0.0)
        min_col = grp[col].transform("min")
        max_col = grp[col].transform("max")
        denom = (max_col - min_col).replace(0, 1)
        ...
        out[f"{safe_name}_ctx_mean_diff"] = out[col] - mean_col
        out[f"{safe_name}_ctx_z"] = (out[col] - mean_col) / std_col.replace(0, 1)
        out[f"{safe_name}_ctx_minmax"] = (out[col] - min_col) / denom

        rank = grp[col].rank(method="average", pct=True)
        out[f"{safe_name}_ctx_rank"] = rank.astype("float32")

    out["TyreLife_by_Progress"] = out["TyreLife"] / out["RaceProgress"].replace(
        0, np.nan
    )
    out["Degradation_per_Lap"] = out["Cumulative_Degradation"] / out[
        "TyreLife"
    ].replace(0, np.nan)
    out["Degradation_x_TyreLife"] = out["Cumulative_Degradation"] * out["TyreLife"]
    out["LapDelta_x_TyreLife"] = out["LapTime_Delta"] * out["TyreLife"]
    out["PositionLossPressure"] = (out["Position_Change"] > 0).astype("int8") * out[
        "TyreLife"
    ]
    out["LateStintFlag"] = (
        out["TyreLife"]
        >= out.groupby(["Year", "Race", "Driver", "Stint"], sort=False)[
            "TyreLife"
        ].transform("median")
    ).astype("int8")

    for col in [
        "TyreLife_by_Progress",
        "Degradation_per_Lap",
        "Degradation_x_TyreLife",
        "LapDelta_x_TyreLife",
    ]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000702"],
        "research_usage_note": "Verified assigned hypothesis 000702.",
    }, sort_keys=True))
