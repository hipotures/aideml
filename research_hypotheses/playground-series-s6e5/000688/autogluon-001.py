...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Race_Year"] = out["Year"].astype("string") + "_" + out["Race"]

    sort_cols = ["Year", "Race", "LapNumber", "Position", "Driver", "Stint"]
    tmp = out[sort_cols].copy()
    tmp["Race"] = tmp["Race"].astype("string")
    tmp["Driver"] = tmp["Driver"].astype("string")
    out["chrono_rank"] = (
    ...
    out["chrono_rank"] = out["chrono_rank"] / max(float(len(out)), 1.0)

    for c in ["Race_Year", "Race", "Driver", "Compound"]:
        freq = out[c].map(out[c].value_counts(normalize=True))
        out[c + "_freq"] = freq.astype("float32")

    out["lap_x_progress"] = (out["LapNumber"] * out["RaceProgress"]).astype("float32")
    out["tyrelife_x_stint"] = (out["TyreLife"] * out["Stint"]).astype("float32")
    ...
    out["is_old_tyre"] = (out["TyreLife"] >= 18).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000688"],
        "research_usage_note": "Verified assigned hypothesis 000688.",
    }, sort_keys=True))
