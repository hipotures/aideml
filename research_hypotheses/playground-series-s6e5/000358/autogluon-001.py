...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(c in out.columns for c in key_cols):
        ...
        agg_cols = [c for c in num_cols if c != "PitStop"]
        for c in agg_cols:
            mean = g[c].transform("mean").astype("float32")
            std = g[c].transform("std").fillna(0).astype("float32")
            mn = g[c].transform("min").astype("float32")
            mx = g[c].transform("max").astype("float32")

            out[f"snapshot_{c}_mean"] = mean
            out[f"snapshot_{c}_std"] = std
            out[f"snapshot_{c}_min"] = mn
            out[f"snapshot_{c}_max"] = mx
            out[f"rel_{c}_to_snapshot_mean"] = (out[c] - mean).astype("float32")
            out[f"z_{c}_snapshot"] = (
                ((out[c] - mean) / (std + 1e-3)).clip(-20, 20).astype("float32")
            )
            out[f"rank_{c}_in_snapshot"] = (
                g[c].rank(method="average", pct=True).astype("float32")
            )

        out["snapshot_field_size"] = g["LapNumber"].transform("size").astype("int16")

        if "PitStop" in out.columns:
            out["snapshot_current_pit_count"] = (
                g["PitStop"].transform("sum").astype("float32")
            )
            out["snapshot_current_pit_rate"] = (
                out["snapshot_current_pit_count"]
                / out["snapshot_field_size"].clip(lower=1)
            ).astype("float32")

        if {"Position", "TyreLife"}.issubset(out.columns):
            out["front_age_pressure"] = (
                out["TyreLife"] * (21.0 - out["Position"]).clip(lower=1)
            ).astype("float32")
            out["snapshot_front_age_pressure_mean"] = (
                g["front_age_pressure"].transform("mean").astype("float32")
            )
            out["rel_front_age_pressure"] = (
                out["front_age_pressure"] - out["snapshot_front_age_pressure_mean"]
            ).astype("float32")
        ...
        for c in [
            ...
            mcol = f"snapshot_{c}_mean"
            scol = f"snapshot_{c}_std"
            if mcol in out.columns:
                sig_parts.append(
                    pd.qcut(
                        out[mcol].rank(method="first"),
                        8,
                        labels=False,
                        duplicates="drop",
                    )
        ...
        if sig_parts:
            ...
            for part in sig_parts[1:]:
                sig = sig + "_" + part
            out["snapshot_bin_signature"] = sig.astype("category")

    if {"Race", "Year"}.issubset(out.columns):
        out["Race_Year"] = (
            out["Race"].astype(str) + "_" + out["Year"].astype(str)
        ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000358"],
        "research_usage_note": "Verified assigned hypothesis 000358.",
    }, sort_keys=True))
