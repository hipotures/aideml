...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_life = (
        pd.to_numeric(out["TyreLife"], errors="coerce").astype("float32").clip(1, 90)
    )
    lap_number = (
        pd.to_numeric(out["LapNumber"], errors="coerce").astype("float32").clip(1, 100)
    )
    progress = (
    ...
    pit_stop = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0).astype("int8")

    race_lap_median = lap_time.groupby(
        [out["Year"], out["Race"], out["LapNumber"]], sort=False
    ).transform("median")
    residual = (lap_time - race_lap_median).astype("float32")

    clean_residual = residual.mask((pit_stop == 1) | (residual.abs() > 35)).clip(-8, 20)
    valid = clean_residual.notna() & tyre_life.notna()
    x = tyre_life.where(valid)
    y = clean_residual.where(valid)

    def slope_for(keys, min_count):
        group_keys = [out[k] for k in keys]
        n = y.groupby(group_keys, sort=False).transform("count").astype("float32")
        sx = x.groupby(group_keys, sort=False).transform("sum").astype("float32")
        sy = y.groupby(group_keys, sort=False).transform("sum").astype("float32")
        sxx = (x * x).groupby(group_keys, sort=False).transform("sum").astype("float32")
        sxy = (x * y).groupby(group_keys, sort=False).transform("sum").astype("float32")
        denom = n * sxx - sx * sx
        slope = (n * sxy - sx * sy) / denom.replace(0, np.nan)
        return (
            slope.where((n >= min_count) & (denom > 1e-6))
            .clip(0.02, 1.25)
            .astype("float32")
        )

    slope_rc = slope_for(["Year", "Race", "Compound"], 10)
    slope_rcs = slope_for(["Year", "Race", "Compound", "Stint"], 8)
    slope_compound = slope_for(["Compound"], 30)

    x_mean = x.mean()
    y_mean = y.mean()
    global_denom = ((x - x_mean) ** 2).sum()
    if pd.notna(global_denom) and global_denom > 1e-6:
        global_slope = float((((x - x_mean) * (y - y_mean)).sum() / global_denom))
    else:
        global_slope = 0.12
    global_slope = float(np.clip(global_slope, 0.02, 1.25))

    pit_residual = residual.where((pit_stop == 1) & residual.between(10, 80))
    inferred_pit_loss = pit_residual.median()
    pit_loss = (
        23.5
        if pd.isna(inferred_pit_loss)
        else float(np.clip(inferred_pit_loss, 20.0, 30.0))
    )

    slope_main = (
        slope_rc.fillna(slope_compound)
        .fillna(global_slope)
        .clip(0.02, 1.25)
        .astype("float32")
    )
    slope_stint = slope_rcs.fillna(slope_main).clip(0.02, 1.25).astype("float32")
    ...
    horizon = remaining_laps.clip(0, 35)

    optimal = np.sqrt((2.0 * pit_loss) / slope_main).clip(3, 80).astype("float32")
    diff = (tyre_life - optimal).clip(-80, 80).astype("float32")
    gain = (
        (slope_main * tyre_life * horizon - pit_loss).clip(-40, 120).astype("float32")
    )

    optimal_stint = (
        np.sqrt((2.0 * pit_loss) / slope_stint).clip(3, 80).astype("float32")
    )
    diff_stint = (tyre_life - optimal_stint).clip(-80, 80).astype("float32")
    gain_stint = (
        (slope_stint * tyre_life * horizon - pit_loss).clip(-40, 120).astype("float32")
    )

    out["degradation_slope_race_compound"] = slope_main
    out["degradation_slope_race_compound_stint"] = slope_stint
    out["estimated_optimal_stint"] = optimal
    out["tyrelife_minus_optimal"] = diff
    out["remaining_laps_gain_estimate"] = gain
    out["estimated_optimal_stint_race_compound_stint"] = optimal_stint
    out["tyrelife_minus_optimal_race_compound_stint"] = diff_stint
    out["remaining_laps_gain_estimate_race_compound_stint"] = gain_stint

    out["estimated_optimal_stint_bin"] = np.digitize(
        optimal, [8, 12, 16, 20, 25, 30, 40, 55]
    ).astype("int8")
    out["tyrelife_minus_optimal_bin"] = np.digitize(
        diff, [-20, -12, -8, -5, -2, 0, 2, 5, 8, 12, 20]
    ).astype("int8")
    out["remaining_laps_gain_estimate_bin"] = np.digitize(
        gain, [-25, -15, -8, -3, 0, 3, 8, 15, 25, 40]
    ).astype("int8")
    out["estimated_optimal_stint_race_compound_stint_bin"] = np.digitize(
        optimal_stint, [8, 12, 16, 20, 25, 30, 40, 55]
    ).astype("int8")
    out["tyrelife_minus_optimal_race_compound_stint_bin"] = np.digitize(
        diff_stint, [-20, -12, -8, -5, -2, 0, 2, 5, 8, 12, 20]
    ).astype("int8")
    out["remaining_laps_gain_estimate_race_compound_stint_bin"] = np.digitize(
        gain_stint, [-25, -15, -8, -3, 0, 3, 8, 15, 25, 40]
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000632"],
        "research_usage_note": "Verified assigned hypothesis 000632.",
    }, sort_keys=True))
