...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_life = out["TyreLife"].astype("float32").clip(lower=1.0)
    ...
    race_progress = out["RaceProgress"].astype("float32").clip(0.0, 1.0)
    race_remaining = (1.0 - race_progress).astype("float32")

    deg_per_tyre_life = (deg / tyre_life).clip(-80.0, 80.0)
    deg_per_expected_life = (deg / expected_life).clip(-80.0, 80.0)
    abs_deg = deg.abs().clip(0.0, 2500.0)
    positive_deg = deg.clip(lower=0.0, upper=2500.0)

    out["Deg_Per_TyreLife"] = deg_per_tyre_life.astype("float32")
    out["Deg_Per_ExpectedCompoundLife"] = deg_per_expected_life.astype("float32")
    out["Abs_Degradation"] = abs_deg.astype("float32")
    out["Positive_Degradation"] = positive_deg.astype("float32")
    out["Positive_Deg_Per_TyreLife"] = (
        (positive_deg / tyre_life).clip(0.0, 80.0).astype("float32")
    )
    out["Positive_Deg_Per_ExpectedLife"] = (
        (positive_deg / expected_life).clip(0.0, 80.0).astype("float32")
    )
    out["Deg_TyreLife_x_RaceProgress"] = (deg_per_tyre_life * race_progress).astype(
        "float32"
    )
    out["Deg_ExpectedLife_x_RaceProgress"] = (
        deg_per_expected_life * race_progress
    ).astype("float32")
    out["Positive_Deg_To_FinishPressure"] = (
        (positive_deg / expected_life).clip(0.0, 80.0) * race_remaining
    ).astype("float32")
    out["Abs_Deg_To_FinishPressure"] = (
        (abs_deg / expected_life).clip(0.0, 80.0) * race_remaining
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000029"],
        "research_usage_note": "Verified assigned hypothesis 000029.",
    }, sort_keys=True))
