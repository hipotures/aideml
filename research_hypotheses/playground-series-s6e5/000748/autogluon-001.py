...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry_compound = (1 - is_wet_compound).astype("int8")
    ...
    est_total_laps = est_total_laps.clip(lower=lap, upper=100)
    ...
    first_dry_stint = ((stint <= 1) & (is_dry_compound == 1)).astype("int8")
    plausible_single_dry_spec = (
        (first_dry_stint == 1)
        | ((stint <= 2) & (tyre_life >= lap * 0.45) & (is_dry_compound == 1))
    ).astype("int8")

    late_enough_for_mandatory_stop = (
        (progress >= 0.45) & (laps_remaining >= 1.0)
    ).astype("int8")
    must_stop_before_finish = (
        (is_dry_compound == 1)
        & (is_wet_compound == 0)
        & (is_monaco == 0)
        & (plausible_single_dry_spec == 1)
        & (late_enough_for_mandatory_stop == 1)
    ).astype("int8")

    out["dry_compound_flag"] = is_dry_compound
    out["wet_or_inter_flag"] = is_wet_compound
    out["monaco_flag"] = is_monaco
    out["est_total_laps"] = est_total_laps.astype("float32")
    out["laps_remaining_est"] = laps_remaining.astype("float32")
    out["first_dry_stint_flag"] = first_dry_stint
    out["plausible_single_dry_spec_flag"] = plausible_single_dry_spec
    out["must_stop_before_finish_proxy"] = must_stop_before_finish

    out["mandatory_stop_pressure"] = (
        must_stop_before_finish * (1.0 / (laps_remaining + 1.0))
    ).astype("float32")
    out["mandatory_stop_late_pressure"] = (must_stop_before_finish * progress).astype(
        "float32"
    )
    out["mandatory_stop_tyre_age_pressure"] = (
        must_stop_before_finish * tyre_life / (laps_remaining + 1.0)
    ).astype("float32")
    out["dry_laps_remaining"] = (is_dry_compound * laps_remaining).astype("float32")
    out["wet_path_laps_remaining"] = (is_wet_compound * laps_remaining).astype(
        "float32"
    )
    out["dry_stint1_late_flag"] = (
        (first_dry_stint == 1) & (progress >= 0.60) & (laps_remaining >= 1.0)
    ).astype("int8")
    out["mandatory_x_degradation"] = (must_stop_before_finish * degradation).astype(
        "float32"
    )
    out["mandatory_x_laptime_delta"] = (must_stop_before_finish * lap_delta).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000748.",
    }, sort_keys=True))
