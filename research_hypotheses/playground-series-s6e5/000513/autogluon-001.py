...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit = out["PitStop"].astype(float)
    lap_key = [race_year, out["LapNumber"]]

    lap_count = out.groupby(lap_key)["PitStop"].transform("size").astype(float)
    lap_pits = pit.groupby(lap_key).transform("sum").astype(float)
    out["field_pit_count_lap"] = lap_pits
    out["field_pit_rate_lap"] = lap_pits / lap_count.clip(lower=1)

    comp_key = [race_year, out["LapNumber"], out["Compound"].astype(str)]
    comp_count = out.groupby(comp_key)["PitStop"].transform("size").astype(float)
    comp_pits = pit.groupby(comp_key).transform("sum").astype(float)
    out["compound_pit_count_lap"] = comp_pits
    out["compound_pit_rate_lap"] = comp_pits / comp_count.clip(lower=1)

    band_key = [race_year, out["LapNumber"], out["pos_band"]]
    band_count = out.groupby(band_key)["PitStop"].transform("size").astype(float)
    band_pits = pit.groupby(band_key).transform("sum").astype(float)
    out["pos_band_pit_count_lap"] = band_pits
    out["pos_band_pit_rate_lap"] = band_pits / band_count.clip(lower=1)

    lap_tbl = (
        out[["Race_Year", "LapNumber", "PitStop"]]
        .groupby(["Race_Year", "LapNumber"], sort=False)
        .agg(lap_pits=("PitStop", "sum"), lap_cars=("PitStop", "size"))
        .reset_index()
        .sort_values(["Race_Year", "LapNumber"])
    )
    lap_tbl["lap_rate"] = lap_tbl["lap_pits"] / lap_tbl["lap_cars"].clip(lower=1)
    for lag in (1, 2):
        lap_tbl[f"field_pit_count_lag{lag}"] = lap_tbl.groupby("Race_Year")[
            "lap_pits"
        ].shift(lag)
        lap_tbl[f"field_pit_rate_lag{lag}"] = lap_tbl.groupby("Race_Year")[
            "lap_rate"
        ].shift(lag)
    lap_tbl["field_pit_count_lag1_lag2_sum"] = lap_tbl["field_pit_count_lag1"].fillna(
        0
    ) + lap_tbl["field_pit_count_lag2"].fillna(0)
    lap_tbl["field_pit_rate_lag1_lag2_mean"] = (
        lap_tbl["field_pit_rate_lag1"].fillna(0)
        + lap_tbl["field_pit_rate_lag2"].fillna(0)
    ) / 2.0
    out = out.merge(
        lap_tbl[
            [
                ...
                "field_pit_rate_lag2",
                "field_pit_count_lag1_lag2_sum",
                "field_pit_rate_lag1_lag2_mean",
            ]
        ],
        on=["Race_Year", "LapNumber"],
        how="left",
        sort=False,
    )

    pos_tbl = (
        out[["Race_Year", "LapNumber", "Position", "PitStop"]]
        .groupby(["Race_Year", "LapNumber", "Position"], sort=False)["PitStop"]
        .sum()
        .rename("pos_pits")
        .reset_index()
    )
    base = (
        out[["Race_Year", "LapNumber", "Position"]]
        .drop_duplicates()
        .reset_index(drop=True)
    )
    local = base.copy()
    local["nearby_pos_pit_count_pm2"] = 0.0
    local["ahead_pos_pitted_pm2"] = 0.0
    local["behind_pos_pitted_pm2"] = 0.0

    for d in (-2, -1, 1, 2):
        tmp = pos_tbl.copy()
        tmp["Position"] = tmp["Position"] - d
        tmp = tmp.rename(columns={"pos_pits": f"pits_d{d}"})
        local = local.merge(
            tmp, on=["Race_Year", "LapNumber", "Position"], how="left", sort=False
        )
        vals = local[f"pits_d{d}"].fillna(0.0)
        local["nearby_pos_pit_count_pm2"] += vals
        if d < 0:
            local["ahead_pos_pitted_pm2"] = np.maximum(
                local["ahead_pos_pitted_pm2"], (vals > 0).astype(float)
            )
        else:
            local["behind_pos_pitted_pm2"] = np.maximum(
                local["behind_pos_pitted_pm2"], (vals > 0).astype(float)
            )
        local = local.drop(columns=[f"pits_d{d}"])

    out = out.merge(
        local, on=["Race_Year", "LapNumber", "Position"], how="left", sort=False
    )
    out["nearby_pos_pit_rate_pm2"] = out["nearby_pos_pit_count_pm2"] / 4.0
    out["field_minus_local_pit_count"] = (
        out["field_pit_count_lap"] - out["nearby_pos_pit_count_pm2"] - pit
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000513"],
        "research_usage_note": "Verified assigned hypothesis 000513.",
    }, sort_keys=True))
