...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_orig_idx_000247"] = orig_idx

    group_cols = [c for c in ["Year", "Race", "Driver", "Stint"] if c in out.columns]
    sort_cols = (
        group_cols
        + (["LapNumber"] if "LapNumber" in out.columns else [])
        + ["_orig_idx_000247"]
    )
    ...
    if group_cols:
        ...
        g = s.groupby(np.zeros(len(s), dtype=np.int8), sort=False)

    for col in ["LapTime_Delta", "Cumulative_Degradation"]:
        if col not in s.columns:
            continue

        x = pd.to_numeric(s[col], errors="coerce").astype("float32")
        prev1 = g[col].shift(1).astype("float32")
        prev2 = g[col].shift(2).astype("float32")
        prev3 = g[col].shift(3).astype("float32")

        s[f"{col}_prev1_000247"] = prev1
        s[f"{col}_slope1_000247"] = (prev1 - prev2).astype("float32")
        s[f"{col}_slope3_000247"] = ((prev1 - prev3) / 2.0).astype("float32")
        s[f"{col}_curvature_000247"] = (prev1 - 2.0 * prev2 + prev3).astype("float32")

        shifted = (
            x.groupby([s[c] for c in group_cols], sort=False, observed=True).shift(1)
            if group_cols
            else x.shift(1)
        )
        roll_mean = (
            shifted.groupby(
                [s[c] for c in group_cols], sort=False, observed=True
            ).transform(lambda v: v.rolling(3, min_periods=2).mean())
            if group_cols
            else shifted.rolling(3, min_periods=2).mean()
        )
        roll_std = (
            shifted.groupby(
                [s[c] for c in group_cols], sort=False, observed=True
            ).transform(lambda v: v.rolling(3, min_periods=2).std())
            if group_cols
            else shifted.rolling(3, min_periods=2).std()
        )
        s[f"{col}_roll3_mean_prior_000247"] = roll_mean.astype("float32")
        s[f"{col}_roll3_std_prior_000247"] = roll_std.fillna(0).astype("float32")

        early_mask = pd.to_numeric(
            s.get("TyreLife", pd.Series(np.nan, index=s.index)), errors="coerce"
        ).le(3)
        early_vals = x.where(early_mask)
        early_base = (
            early_vals.groupby(
                [s[c] for c in group_cols], sort=False, observed=True
            ).transform("mean")
            if group_cols
            else pd.Series(early_vals.mean(), index=s.index)
        )
        s[f"{col}_early_dev_000247"] = (prev1 - early_base).astype("float32")

        stable = s[f"{col}_roll3_std_prior_000247"].le(
            s[f"{col}_roll3_std_prior_000247"].median(skipna=True)
        )
        s[f"{col}_abrupt_fade_000247"] = (
            stable
            & s[f"{col}_curvature_000247"].gt(
                s[f"{col}_curvature_000247"].quantile(0.80)
            )
        ).astype("int8")

    if "Compound" in s.columns:
        comp = s["Compound"].astype("string")
        s["is_wet_regime_000247"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
        s["is_soft_slick_000247"] = comp.eq("SOFT").astype("int8")
        s["is_hard_slick_000247"] = comp.eq("HARD").astype("int8")
    else:
        s["is_wet_regime_000247"] = 0
        s["is_soft_slick_000247"] = 0
        s["is_hard_slick_000247"] = 0

    rp = (
        pd.to_numeric(s["RaceProgress"], errors="coerce").astype("float32")
        if "RaceProgress" in s.columns
        else pd.Series(0.0, index=s.index)
    )
    for base in [
        "LapTime_Delta_curvature_000247",
        "Cumulative_Degradation_curvature_000247",
        "LapTime_Delta_early_dev_000247",
        "Cumulative_Degradation_early_dev_000247",
    ]:
        if base in s.columns:
            s[f"{base}_x_progress_000247"] = (
                s[base].fillna(0).astype("float32") * rp
            ).astype("float32")
            s[f"{base}_x_wet_000247"] = (
                s[base].fillna(0).astype("float32") * s["is_wet_regime_000247"]
            ).astype("float32")
            s[f"{base}_x_soft_000247"] = (
                s[base].fillna(0).astype("float32") * s["is_soft_slick_000247"]
            ).astype("float32")

    s = s.sort_values("_orig_idx_000247", kind="mergesort").drop(
        columns=["_orig_idx_000247"]
    )
    return s
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000247"],
        "research_usage_note": "Verified assigned hypothesis 000247.",
    }, sort_keys=True))
