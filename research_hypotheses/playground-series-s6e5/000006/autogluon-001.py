...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    remaining_laps = remaining_laps.fillna(0.0).clip(lower=0.0)

    soft_margin = 18.0 - remaining_laps
    medium_margin = 28.0 - remaining_laps
    hard_margin = 40.0 - remaining_laps

    entry_count = (
        (soft_margin >= 0).astype("int8")
        + (medium_margin >= 0).astype("int8")
        + (hard_margin >= 0).astype("int8")
    )

    out["EntryCount"] = entry_count.astype("int8")
    out["AnyFreshEntryNow"] = (entry_count > 0).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000006"],
        "research_usage_note": "Verified assigned hypothesis 000006.",
    }, sort_keys=True))
