...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_testing = df["Race"].eq("Pre-Season Testing")
    df["IsPreSeasonTesting"] = is_testing.astype("int8")
    df["RaceDomain"] = np.where(is_testing, "testing", "grand_prix")
    ...
    for col in numeric_cols:
        vals = df[col].astype("float32")
        df[f"{col}_testing_only"] = vals.where(is_testing, 0.0)
        df[f"{col}_gp_only"] = vals.where(~is_testing, 0.0)

    df["CompoundDomain"] = (
        df["Compound"].astype(str) + "_" + df["RaceDomain"].astype(str)
    )
    df["DriverDomain"] = df["Driver"].astype(str) + "_" + df["RaceDomain"].astype(str)
    df["YearDomain"] = df["Year"].astype(str) + "_" + df["RaceDomain"].astype(str)

    df["TyreLife_x_Testing"] = (
        df["TyreLife"].astype("float32") * df["IsPreSeasonTesting"]
    )
    df["Degradation_x_Testing"] = (
        df["Cumulative_Degradation"].astype("float32") * df["IsPreSeasonTesting"]
    )
    df["LapTimeDelta_x_Testing"] = (
        df["LapTime_Delta"].astype("float32") * df["IsPreSeasonTesting"]
    )
    df["RaceProgress_x_Testing"] = (
        df["RaceProgress"].astype("float32") * df["IsPreSeasonTesting"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000904.",
    }, sort_keys=True))
