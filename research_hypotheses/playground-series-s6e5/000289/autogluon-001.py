...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", "_row_id"]
    X = X.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)

    g = X.groupby(group_cols, sort=False)

    # Current stint progression; TyreLife is effectively lap-in-stint, but keep an explicit hazard term.
    X["LapInStint"] = X["TyreLife"].astype(float)
    X["LapInStint_sq"] = X["LapInStint"] ** 2
    X["LapInStint_log"] = np.log1p(X["LapInStint"].clip(lower=0))

    # Race distance proxy and remaining laps for the baseline hazard over race time.
    est_total_laps_from_progress = X["LapNumber"] / X["RaceProgress"].clip(lower=1e-3)
    race_max_lap = (
        X.groupby(["Year", "Race"], sort=False)["LapNumber"]
        .transform("max")
        .astype(float)
    )
    X["RaceTotalLaps_Est"] = np.where(
        X["RaceProgress"] > 0,
        np.maximum(race_max_lap, est_total_laps_from_progress),
        race_max_lap,
    )
    X["LapsRemaining_Est"] = (X["RaceTotalLaps_Est"] - X["LapNumber"]).clip(lower=0)
    X["LapsRemaining_log"] = np.log1p(X["LapsRemaining_Est"])

    # Ongoing-stint censoring logic: hazard is meaningful when this lap does not itself terminate the stint.
    X["CurrentStintOngoing"] = (X["PitStop"] == 0).astype("int8")
    X["CurrentLapPit"] = X["PitStop"].astype("int8")

    # Position within race-driver sequence.
    X["SeqLapIdx"] = g.cumcount() + 1
    seq_len = g["LapNumber"].transform("size").astype(float)
    X["SeqProgress"] = X["SeqLapIdx"] / seq_len.clip(lower=1.0)

    # Within-sequence pace/degradation stress, all causal from the current/past rows only.
    X["PrevLapTime"] = g["LapTime (s)"].shift(1)
    X["PrevDeg"] = g["Cumulative_Degradation"].shift(1)
    X["PrevPos"] = g["Position"].shift(1)
    X["LapTime_to_prev"] = X["LapTime (s)"] - X["PrevLapTime"]
    X["Deg_to_prev"] = X["Cumulative_Degradation"] - X["PrevDeg"]
    X["Pos_to_prev"] = X["Position"] - X["PrevPos"]

    cum_mean_laptime = g["LapTime (s)"].transform(lambda s: s.expanding().mean())
    cum_mean_delta = g["LapTime_Delta"].transform(lambda s: s.expanding().mean())
    cum_mean_deg = g["Cumulative_Degradation"].transform(lambda s: s.expanding().mean())

    X["LapTime_vs_seq_mean"] = X["LapTime (s)"] - cum_mean_laptime
    X["LapDelta_vs_seq_mean"] = X["LapTime_Delta"] - cum_mean_delta
    X["Deg_vs_seq_mean"] = X["Cumulative_Degradation"] - cum_mean_deg

    # Compound/weather regime interactions for the hazard surface.
    compound_upper = X["Compound"].astype(str).str.upper()
    X["IsWetRegime"] = compound_upper.isin(["INTERMEDIATE", "WET"]).astype("int8")
    X["IsSoft"] = (compound_upper == "SOFT").astype("int8")
    X["IsMedium"] = (compound_upper == "MEDIUM").astype("int8")
    X["IsHard"] = (compound_upper == "HARD").astype("int8")

    X["Hazard_LapInStint_x_Wet"] = X["LapInStint"] * X["IsWetRegime"]
    X["Hazard_LapInStint_x_Soft"] = X["LapInStint"] * X["IsSoft"]
    X["Hazard_LapInStint_x_Medium"] = X["LapInStint"] * X["IsMedium"]
    X["Hazard_LapInStint_x_Hard"] = X["LapInStint"] * X["IsHard"]
    X["Hazard_Remaining_x_Wet"] = X["LapsRemaining_Est"] * X["IsWetRegime"]

    # Stint-aware hazard intensity proxies.
    X["Deg_per_stint_lap"] = X["Cumulative_Degradation"] / X["LapInStint"].replace(
        0, np.nan
    )
    X["Deg_per_race_lap"] = X["Cumulative_Degradation"] / X["LapNumber"].replace(
        0, np.nan
    )
    X["LapDelta_per_stint_lap"] = X["LapTime_Delta"] / X["LapInStint"].replace(
        0, np.nan
    )
    X["Hazard_core"] = (X["LapInStint"] / (X["LapsRemaining_Est"] + 1.0)) * (
        1.0 + 0.5 * X["IsWetRegime"]
    )
    X["Hazard_core_ongoing"] = X["Hazard_core"] * X["CurrentStintOngoing"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000289"],
        "research_usage_note": "Verified assigned hypothesis 000289.",
    }, sort_keys=True))
