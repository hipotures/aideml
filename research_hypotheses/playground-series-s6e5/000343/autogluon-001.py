...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver_race = race_year + "_" + out["Driver"].astype(str)
    ...
    pace = out["LapTime_Delta"].astype(float)
    tyre = out["TyreLife"].astype(float)
    lap = out["LapNumber"].astype(float)

    g_same = [race_year, out["LapNumber"], out["Compound"]]
    cnt_same = pace.groupby(g_same, observed=True).transform("count")
    med_same = pace.groupby(g_same, observed=True).transform("median")
    fast_same = pace.groupby(g_same, observed=True).transform("min")
    rank_same = pace.groupby(g_same, observed=True).rank(method="average", pct=True)

    out["peer_compound_count"] = cnt_same.astype(float)
    out["peer_compound_delta_rank"] = rank_same.fillna(0.5)
    out["peer_compound_delta_vs_median"] = pace - med_same
    out["peer_compound_delta_vs_fastest"] = pace - fast_same
    out["peer_compound_sparse"] = (cnt_same < 4).astype("int8")
    out["peer_compound_pressure_tyre"] = out["peer_compound_delta_rank"] * np.log1p(
        tyre
    )
    out["peer_compound_gap_tyre"] = out["peer_compound_delta_vs_median"] * np.log1p(
        tyre
    )

    pos_bin = pd.cut(
        out["Position"], bins=[0, 5, 10, 15, 20], labels=False, include_lowest=True
    ).astype("float")
    out["Position_Bin"] = pos_bin

    g_pos = [race_year, out["LapNumber"], out["Compound"], pos_bin]
    cnt_pos = pace.groupby(g_pos, observed=True).transform("count")
    med_pos = pace.groupby(g_pos, observed=True).transform("median")
    rank_pos = pace.groupby(g_pos, observed=True).rank(method="average", pct=True)

    out["peer_pos_compound_count"] = cnt_pos.astype(float)
    out["peer_pos_compound_delta_rank"] = rank_pos.fillna(
        out["peer_compound_delta_rank"]
    )
    out["peer_pos_compound_delta_vs_median"] = (pace - med_pos).fillna(
        out["peer_compound_delta_vs_median"]
    )
    out["peer_pos_pressure_tyre"] = out["peer_pos_compound_delta_rank"] * np.log1p(tyre)
    ...
    tmp = pd.DataFrame(
        {
            ...
            "lap": lap,
            "rank_same": out["peer_compound_delta_rank"].astype(float),
            "rank_pos": out["peer_pos_compound_delta_rank"].astype(float),
        }
    ).sort_values(["driver_race", "lap", "idx"])

    tmp["prev_rank_same"] = tmp.groupby("driver_race", sort=False)["rank_same"].shift(1)
    tmp["prev_rank_pos"] = tmp.groupby("driver_race", sort=False)["rank_pos"].shift(1)
    tmp = tmp.sort_values("idx")

    out["peer_compound_rank_change"] = (
        tmp["rank_same"].to_numpy() - tmp["prev_rank_same"].to_numpy()
    )
    out["peer_pos_rank_change"] = (
        tmp["rank_pos"].to_numpy() - tmp["prev_rank_pos"].to_numpy()
    )
    out["peer_compound_rank_change"] = out["peer_compound_rank_change"].fillna(0.0)
    out["peer_pos_rank_change"] = out["peer_pos_rank_change"].fillna(0.0)

    out["peer_rank_change_tyre"] = out["peer_compound_rank_change"] * np.log1p(tyre)
    out["peer_late_stint_pressure"] = (
        out["peer_compound_delta_rank"] * tyre * out["RaceProgress"].astype(float)
    )
    out["peer_fast_gap_per_tyre_lap"] = out["peer_compound_delta_vs_fastest"] / (
        tyre + 1.0
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000343"],
        "research_usage_note": "Verified assigned hypothesis 000343.",
    }, sort_keys=True))
