...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ryl_laptime_median"] = g["LapTime (s)"].transform("median")
    out["ryl_delta_median"] = g["LapTime_Delta"].transform("median")
    out["ryl_laptime_std"] = g["LapTime (s)"].transform("std").fillna(0.0)
    out["ryl_delta_std"] = g["LapTime_Delta"].transform("std").fillna(0.0)
    out["ryl_driver_count"] = g["LapTime (s)"].transform("count").astype(np.float32)

    out["driver_laptime_vs_ryl_median"] = lap_time - out["ryl_laptime_median"]
    out["driver_delta_vs_ryl_median"] = lap_delta - out["ryl_delta_median"]
    out["driver_laptime_rank_pct_ryl"] = g["LapTime (s)"].rank(
        method="average", pct=True
    )
    out["driver_delta_rank_pct_ryl"] = g["LapTime_Delta"].rank(
        method="average", pct=True
    )

    race_delta_med = out.groupby(ry_keys, sort=False)["LapTime_Delta"].transform(
        "median"
    )
    race_delta_std = (
        out.groupby(ry_keys, sort=False)["LapTime_Delta"]
        .transform("std")
        .replace(0, np.nan)
    )
    slow_flag = (lap_delta > race_delta_med + 2.0 * race_delta_std).astype(np.int8)
    out["ryl_large_slowdown_count"] = (
        slow_flag.groupby([out[c] for c in keys], sort=False)
        .transform("sum")
        .astype(np.float32)
    )
    out["ryl_large_slowdown_frac"] = out["ryl_large_slowdown_count"] / out[
        "ryl_driver_count"
    ].clip(lower=1)
    ...
    lap_agg = (
        out[
            ...
            + [
                ...
                "ryl_laptime_std",
                "ryl_delta_std",
                "ryl_large_slowdown_count",
                "ryl_large_slowdown_frac",
            ]
        ]
    ...
    for col in lag_cols:
        lap_agg[col + "_lag1"] = lap_agg.groupby(ry_keys, sort=False)[col].shift(1)
        lap_agg[col + "_lag2"] = lap_agg.groupby(ry_keys, sort=False)[col].shift(2)

    lagged = lap_agg[
        keys + [c + "_lag1" for c in lag_cols] + [c + "_lag2" for c in lag_cols]
    ]
    out = out.merge(lagged, on=keys, how="left", sort=False)

    new_lag_cols = [
        c for c in out.columns if c.endswith("_lag1") or c.endswith("_lag2")
    ]
    out[new_lag_cols] = out[new_lag_cols].fillna(0.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000606"],
        "research_usage_note": "Verified assigned hypothesis 000606.",
    }, sort_keys=True))
