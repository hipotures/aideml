...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Event keys for race-heldout validation outside preprocess.
    df["Year_Race"] = df["Year"].astype(str) + "_" + df["Race"].astype(str)
    df["Driver_Year_Race"] = (
        df["Driver"].astype(str) + "_" + df["Year_Race"].astype(str)
    )

    # Robust numeric transforms centered on pit-window mechanics.
    df["TyreLife_ratio"] = df["TyreLife"] / df["LapNumber"].clip(lower=1)
    df["TyreLife_remaining_proxy"] = (1.0 - df["RaceProgress"]).clip(lower=0) * df[
        "TyreLife"
    ]
    df["Deg_per_tyre_lap"] = df["Cumulative_Degradation"] / df["TyreLife"].clip(lower=1)
    df["Deg_per_race_progress"] = df["Cumulative_Degradation"] / df[
        "RaceProgress"
    ].clip(lower=0.01)
    df["LapTime_delta_abs"] = df["LapTime_Delta"].abs()
    df["LapTime_over_tyre"] = df["LapTime (s)"] / df["TyreLife"].clip(lower=1)
    df["LapTimeDelta_over_tyre"] = df["LapTime_Delta"] / df["TyreLife"].clip(lower=1)
    df["Position_pressure"] = df["Position"] / 20.0
    df["Position_change_abs"] = df["Position_Change"].abs()
    df["Stint_lap_pressure"] = df["Stint"] * df["TyreLife"]
    df["Late_stint_flag"] = (df["TyreLife_ratio"] > 0.6).astype("int8")
    df["Late_race_flag"] = (df["RaceProgress"] > 0.7).astype("int8")
    df["Pit_window_proxy"] = (
        0.45 * df["TyreLife_ratio"]
        + 0.35 * (df["RaceProgress"].clip(0, 1))
        + 0.20
        * (df["Deg_per_tyre_lap"].replace([np.inf, -np.inf], 0).fillna(0) > 0).astype(
            float
        )
    )

    # Compound normalization and grip classes.
    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    df["Compound_code"] = df["Compound"].map(compound_map).astype("float32")
    df["Is_dry"] = df["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    df["Is_wetlike"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    # Lightweight group statistics using only visible feature columns.
    race_cols = ["Race", "Year"]
    for col in ["LapTime (s)", "LapTime_Delta", "TyreLife", "Cumulative_Degradation"]:
        grp_mean = df.groupby(race_cols, observed=True)[col].transform("mean")
        grp_std = df.groupby(race_cols, observed=True)[col].transform("std").fillna(0)
        df[f"{col}_race_z"] = (df[col] - grp_mean) / grp_std.replace(0, 1)

    # Frequency encodings allowed by the wrapper contract.
    for col in ["Driver", "Race", "Year_Race", "Compound"]:
        freq = df[col].value_counts(dropna=False)
        df[f"{col}_freq"] = df[col].map(freq).astype("float32")

    # Clean infs from ratio features without altering row order/count.
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000615.",
    }, sort_keys=True))
