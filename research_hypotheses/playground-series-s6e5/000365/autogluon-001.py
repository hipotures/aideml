...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    eps = 1e-6
    x_lap = X["LapNumber"].astype(float)
    x_prog = X["RaceProgress"].astype(float).clip(eps, 1.0)
    x_tyre = X["TyreLife"].astype(float).clip(lower=1.0)
    x_deg = X["Cumulative_Degradation"].astype(float)
    x_delta = X["LapTime_Delta"].astype(float)
    x_laptime = X["LapTime (s)"].astype(float)

    est_total_laps = (x_lap / x_prog).clip(lower=x_lap, upper=90.0)
    laps_remaining = (est_total_laps - x_lap).clip(lower=0.0)

    X["est_total_laps"] = est_total_laps
    X["laps_remaining"] = laps_remaining
    X["tyre_life_ratio"] = x_tyre / est_total_laps.clip(lower=1.0)
    X["stint_progress_ratio"] = x_tyre / (x_lap.clip(lower=1.0))

    deg_per_lap_raw = (x_deg / x_tyre).replace([np.inf, -np.inf], np.nan)
    deg_per_lap_raw = deg_per_lap_raw.fillna(deg_per_lap_raw.median())
    X["deg_per_lap_raw"] = deg_per_lap_raw
    X["deg_accel_proxy"] = x_delta - deg_per_lap_raw

    pit_mask = X["PitStop"].astype(float).eq(1.0)
    pit_loss_global = x_delta[pit_mask].median()
    if pd.isna(pit_loss_global):
        pit_loss_global = x_delta.quantile(0.95)

    pit_loss_race = (
        x_delta.where(pit_mask).groupby([X["Year"], X["Race"]]).transform("median")
    )
    pit_loss_year = x_delta.where(pit_mask).groupby(X["Year"]).transform("median")
    pit_loss_baseline = (
        pit_loss_race.fillna(pit_loss_year).fillna(pit_loss_global).astype(float)
    )
    X["pit_loss_baseline"] = pit_loss_baseline

    comp_deg_template = deg_per_lap_raw.groupby(X["Compound"]).transform("median")
    comp_deg_global = float(deg_per_lap_raw.median())
    comp_deg_template = comp_deg_template.fillna(comp_deg_global).clip(lower=0.0)
    X["compound_deg_template"] = comp_deg_template

    fresh_mask = X["TyreLife"].astype(float).le(3.0)
    fresh_deg_global = deg_per_lap_raw[fresh_mask].median()
    if pd.isna(fresh_deg_global):
        fresh_deg_global = comp_deg_global * 0.5

    fresh_compound_deg_map = (
        pd.DataFrame(
            {
                "Compound": X["Compound"].astype(str),
                "fresh_deg": deg_per_lap_raw.where(fresh_mask),
            }
        )
        .groupby("Compound")["fresh_deg"]
        .median()
        .to_dict()
    )
    compounds = [str(c) for c in X["Compound"].astype(str).unique()]
    fresh_by_compound = {
        c: float(
            fresh_compound_deg_map.get(
                c,
                (
                    fresh_deg_global
                    if pd.notna(fresh_deg_global)
                    else comp_deg_global * 0.5
                ),
            )
        )
        for c in compounds
    ...
    current_future_cost = comp_deg_template * laps_remaining
    wait1_future_cost = comp_deg_template * (laps_remaining + 1.0)
    wait2_future_cost = comp_deg_template * (laps_remaining + 2.0)

    next_compound_costs = pd.DataFrame(index=X.index)
    for c in compounds:
        next_compound_costs[c] = (
            pit_loss_baseline + fresh_by_compound[c] * laps_remaining
        )

    best_next_cost = next_compound_costs.min(axis=1)
    best_next_compound = next_compound_costs.idxmin(axis=1).astype("category")
    current_compound_cost = (
        next_compound_costs.lookup(X.index, X["Compound"].astype(str))
        if hasattr(next_compound_costs, "lookup")
        else next_compound_costs.to_numpy()[
            np.arange(len(X)),
            pd.Index(compounds).get_indexer(X["Compound"].astype(str)),
        ]
    )
    current_compound_cost = pd.Series(current_compound_cost, index=X.index, dtype=float)

    stop_now_advantage = current_future_cost - best_next_cost
    regret_if_wait_1lap = (
        pit_loss_baseline + fresh_deg_global * laps_remaining + comp_deg_template
    ) - stop_now_advantage
    regret_if_wait_1lap = (wait1_future_cost - best_next_cost) - stop_now_advantage
    regret_if_wait_2lap = (wait2_future_cost - best_next_cost) - stop_now_advantage
    best_next_compound_margin = current_compound_cost - best_next_cost

    yellow_flag_option_value_proxy = (
        pit_loss_baseline.rank(pct=True) * (1.0 - x_prog) * (1.0 + x_tyre / 20.0)
    )

    X["best_stop_now_advantage"] = stop_now_advantage
    X["regret_if_wait_1lap"] = regret_if_wait_1lap
    X["regret_if_wait_2lap"] = regret_if_wait_2lap
    X["best_next_compound_margin"] = best_next_compound_margin
    X["yellow_flag_option_value_proxy"] = yellow_flag_option_value_proxy
    X["best_next_compound"] = best_next_compound

    X["pit_loss_x_remaining"] = pit_loss_baseline * laps_remaining
    X["deg_cost_to_finish"] = current_future_cost
    X["stop_now_value_ratio"] = stop_now_advantage / pit_loss_baseline.replace(
        0.0, np.nan
    )
    X["wait1_vs_now_ratio"] = regret_if_wait_1lap / pit_loss_baseline.replace(
        0.0, np.nan
    )
    X["counterfactual_pressure"] = (
        stop_now_advantage
        + 0.5 * best_next_compound_margin
        + 0.25 * yellow_flag_option_value_proxy
    )

    for c in [
        "stop_now_value_ratio",
        "wait1_vs_now_ratio",
        "counterfactual_pressure",
        "best_stop_now_advantage",
        "regret_if_wait_1lap",
        "regret_if_wait_2lap",
        "best_next_compound_margin",
        "yellow_flag_option_value_proxy",
    ]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000365.",
    }, sort_keys=True))
