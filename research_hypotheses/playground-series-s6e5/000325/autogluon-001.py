...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["Deg_per_TyreLife"] = X["Cumulative_Degradation"] / (X["TyreLife"] + eps)
    X["Deg_per_Lap"] = X["Cumulative_Degradation"] / (X["LapNumber"] + eps)
    X["LapTime_per_TyreLife"] = X["LapTime (s)"] / (X["TyreLife"] + eps)
    X["LapTime_per_Lap"] = X["LapTime (s)"] / (X["LapNumber"] + eps)
    X["TyreLife_over_RaceProgress"] = X["TyreLife"] / (X["RaceProgress"] + eps)
    X["Stint_x_TyreLife"] = X["Stint"] * X["TyreLife"]
    X["Stint_x_CompoundCode"] = X["Stint"] * X["CompoundCode"]
    X["Progress_x_TyreLife"] = X["RaceProgress"] * X["TyreLife"]
    X["Progress_x_Degradation"] = X["RaceProgress"] * X["Cumulative_Degradation"]
    X["Position_x_Progress"] = X["Position"] * X["RaceProgress"]
    X["Delta_x_Degradation"] = X["LapTime_Delta"] * X["Cumulative_Degradation"]
    X["PositionChange_abs"] = X["Position_Change"].abs()
    X["LapTime_Delta_abs"] = X["LapTime_Delta"].abs()

    grp = ["Year", "Race", "Driver"]
    order = (
        X.reset_index()
        .rename(columns={"index": "_orig_idx"})
        .sort_values(grp + ["LapNumber", "Stint", "_orig_idx"])
    )
    g = order.groupby(grp, sort=False)

    order["Prev_LapTime"] = g["LapTime (s)"].shift(1)
    order["Prev_LapTime_Delta"] = g["LapTime_Delta"].shift(1)
    order["Prev_Position"] = g["Position"].shift(1)
    order["Prev_Position_Change"] = g["Position_Change"].shift(1)
    order["Prev_TyreLife"] = g["TyreLife"].shift(1)
    order["Prev_Degradation"] = g["Cumulative_Degradation"].shift(1)
    order["Prev_Stint"] = g["Stint"].shift(1)
    order["Prev_CompoundCode"] = g["CompoundCode"].shift(1)

    order["LapTime_change_1"] = order["LapTime (s)"] - order["Prev_LapTime"]
    order["Delta_change_1"] = order["LapTime_Delta"] - order["Prev_LapTime_Delta"]
    order["Degradation_change_1"] = (
        order["Cumulative_Degradation"] - order["Prev_Degradation"]
    )
    order["TyreLife_change_1"] = order["TyreLife"] - order["Prev_TyreLife"]
    order["Position_change_1"] = order["Position"] - order["Prev_Position"]
    order["Stint_changed"] = (order["Stint"] != order["Prev_Stint"]).astype("int8")
    order["Compound_changed"] = (
        order["CompoundCode"] != order["Prev_CompoundCode"]
    ).astype("int8")
    ...
    for col in [
        ...
        order[f"{col}_roll3_mean"] = g[col].transform(
            lambda s: s.shift(1).rolling(3, min_periods=1).mean()
        )
        order[f"{col}_roll3_std"] = g[col].transform(
            lambda s: s.shift(1).rolling(3, min_periods=2).std()
        )
        order[f"{col}_roll5_mean"] = g[col].transform(
            lambda s: s.shift(1).rolling(5, min_periods=1).mean()
        )

    order["LapTime_vs_roll3"] = order["LapTime (s)"] - order["LapTime (s)_roll3_mean"]
    order["Delta_vs_roll3"] = order["LapTime_Delta"] - order["LapTime_Delta_roll3_mean"]
    order["Deg_vs_roll3"] = (
        order["Cumulative_Degradation"] - order["Cumulative_Degradation_roll3_mean"]
    )
    order["PosChange_vs_roll3"] = (
        order["Position_Change"] - order["Position_Change_roll3_mean"]
    )

    order = order.sort_values("_orig_idx")
    order = order.drop(columns=["_orig_idx"])

    num_cols = order.select_dtypes(include=[np.number]).columns
    order[num_cols] = order[num_cols].replace([np.inf, -np.inf], np.nan)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000325"],
        "research_usage_note": "Verified assigned hypothesis 000325.",
    }, sort_keys=True))
