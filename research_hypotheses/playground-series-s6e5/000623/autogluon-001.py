...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["CompoundHardness"] = out["Compound"].map(compound_order).astype("float32")
    ...
    out["TyreLife_per_Stint"] = (out["TyreLife"] / (out["Stint"] + 1)).astype("float32")
    out["Deg_per_TyreLife"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"] + 1)
    ).astype("float32")
    out["LapDelta_per_TyreLife"] = (
        out["LapTime_Delta"] / (out["TyreLife"] + 1)
    ).astype("float32")
    out["LateStint"] = (out["TyreLife"] >= 18).astype("int8")
    out["LateRaceOldTyre"] = (
        (out["RaceProgress"] >= 0.65) & (out["TyreLife"] >= 14)
    ).astype("int8")
    out["PitStop_x_TyreLife"] = (out["PitStop"] * out["TyreLife"]).astype("float32")

    group_specs = [
        (["Year", "Race"], "race"),
        (["Year", "Race", "LapNumber"], "race_lap"),
        (["Year", "Race", "LapNumber", "Stint"], "race_lap_stint"),
    ]

    for keys, prefix in group_specs:
        g = out.groupby(keys, sort=False, observed=True)
        out[f"{prefix}_size"] = g["LapNumber"].transform("size").astype("float32")
        ...
        for col in num_cols:
            ...
            out[f"{prefix}_{safe_col}_rank"] = rank.fillna(0.5).astype("float32")
            out[f"{prefix}_{safe_col}_center"] = (out[col] - mean).astype("float32")
            out[f"{prefix}_{safe_col}_z"] = (
                ((out[col] - mean) / std)
    ...
    out["RaceDriverFreq"] = (
        out.groupby(["Year", "Race", "Driver"], sort=False, observed=True)["LapNumber"]
        .transform("size")
        .astype("float32")
    )
    out["DriverFreq"] = (
        out.groupby("Driver", sort=False, observed=True)["LapNumber"]
        .transform("size")
        .astype("float32")
    )
    out["RaceCompoundFreq"] = (
        out.groupby(["Year", "Race", "Compound"], sort=False, observed=True)[
            "LapNumber"
        ]
        .transform("size")
        .astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000623.",
    }, sort_keys=True))
