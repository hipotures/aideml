...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected_life = comp.map(compound_life).fillna(28.0).astype(float)
    ...
    est_total_laps = (lap / progress.replace(0, np.nan)).replace(
        [np.inf, -np.inf], np.nan
    )
    est_total_laps = est_total_laps.fillna(lap.max()).clip(lower=lap)
    laps_remaining = (est_total_laps - lap).clip(lower=0)

    out["expected_compound_life"] = expected_life
    out["tyre_age_vs_expected"] = tyre / expected_life
    out["tyre_life_over_expected"] = (tyre - expected_life) / expected_life
    out["laps_remaining"] = laps_remaining
    out["laps_remaining_vs_expected_life"] = laps_remaining / expected_life
    out["finish_tyre_deficit"] = (tyre + laps_remaining - expected_life) / expected_life
    out["old_tyre_late_race_pressure"] = out["tyre_age_vs_expected"] * progress
    out["pit_window_pressure"] = (
        0.45 * out["tyre_age_vs_expected"]
        + 0.35 * out["finish_tyre_deficit"].clip(lower=0)
        + 0.20 * progress
    )
    ...
    out["late_race"] = (progress >= 0.70).astype("int8")
    out["early_stint"] = (tyre <= 3).astype("int8")
    out["high_deg"] = (deg > deg.groupby(comp).transform("median")).astype("int8")

    out["stint_progress_pressure"] = stint * progress
    out["position_pressure"] = (21.0 - pos) * out["pit_window_pressure"]
    out["degradation_per_tyre_lap"] = deg / tyre.clip(lower=1)
    out["lap_delta_abs"] = lap_delta.abs()
    out["slow_lap_old_tyre"] = out["lap_delta_abs"] * out["tyre_age_vs_expected"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 001013.",
    }, sort_keys=True))
