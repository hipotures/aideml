...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber"]

    eps = 1e-6
    s = out.sort_values(sort_cols).copy()

    g_stint = s.groupby(stint_cols, sort=False)

    prev_lap_time = g_stint["LapTime (s)"].shift(1)
    prev_deg = g_stint["Cumulative_Degradation"].shift(1)
    prev_tyre_life = g_stint["TyreLife"].shift(1)
    stint_lap_count = g_stint.cumcount()

    s["deg_slope_prior"] = (prev_deg / prev_tyre_life.clip(lower=1)).replace(
        [np.inf, -np.inf], np.nan
    )
    s["deg_slope_prior"] = s["deg_slope_prior"].fillna(0.0)

    lag_delta = g_stint["LapTime_Delta"].shift(1)
    lag_delta_sum = (
        lag_delta.fillna(0).groupby([s[c] for c in stint_cols], sort=False).cumsum()
    )
    lag_delta_cnt = stint_lap_count.replace(0, np.nan)
    s["stint_delta_slope_prior"] = (
        (lag_delta_sum / lag_delta_cnt).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    )

    event_max_lap = s.groupby(event_cols, sort=False)["LapNumber"].transform("max")
    s["expected_remaining_laps"] = (event_max_lap - s["LapNumber"]).clip(lower=0)

    stop_proxy = ((s["TyreLife"].astype(float) <= 2.0) & (s["Stint"].astype(float) > 1.0))
    pit_disruption = (s["LapTime (s)"] - prev_lap_time).where(stop_proxy)
    event_pit_loss = pit_disruption.groupby(
        [s[c] for c in event_cols], sort=False
    ).transform("median")
    global_pit_loss = (
        float(pit_disruption.median()) if pit_disruption.notna().any() else 22.0
    )
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 22.0
    s["event_pit_loss_proxy"] = event_pit_loss.fillna(global_pit_loss).clip(
        lower=0, upper=80
    )

    s["degradation_remaining_cost"] = (
        s["deg_slope_prior"].clip(lower=-5, upper=50) * s["expected_remaining_laps"]
    )
    s["delta_remaining_cost"] = (
        s["stint_delta_slope_prior"].clip(lower=-10, upper=20)
        * s["expected_remaining_laps"]
    )

    fresh_tyre_gain = s["degradation_remaining_cost"].clip(lower=0) + s[
        "delta_remaining_cost"
    ].clip(lower=0)
    s["utility_of_staying_out"] = -fresh_tyre_gain
    s["utility_of_pitting_now"] = fresh_tyre_gain - s["event_pit_loss_proxy"]
    s["pit_utility_residual"] = (
        s["utility_of_pitting_now"] - s["utility_of_staying_out"]
    )

    optimal_stint = (
        s["event_pit_loss_proxy"] / s["deg_slope_prior"].clip(lower=0.25)
    ).clip(lower=1, upper=80)
    s["distance_to_optimal_stint"] = s["TyreLife"] - optimal_stint
    s["abs_distance_to_optimal_stint"] = s["distance_to_optimal_stint"].abs()

    add_cols = [
        "deg_slope_prior",
        "stint_delta_slope_prior",
        "expected_remaining_laps",
        "event_pit_loss_proxy",
        "degradation_remaining_cost",
        "delta_remaining_cost",
        "utility_of_staying_out",
        "utility_of_pitting_now",
        "pit_utility_residual",
        "distance_to_optimal_stint",
        "abs_distance_to_optimal_stint",
    ]
    out[add_cols] = s.loc[idx, add_cols]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000248"],
        "research_usage_note": "Verified assigned hypothesis 000248.",
    }, sort_keys=True))
