...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_orig_order_tmp"] = np.arange(len(out), dtype=np.int64)

    group_cols = ["Year", "Race", "Driver", "Stint"]
    sort_cols = group_cols + ["LapNumber", "_orig_order_tmp"]
    s = out.sort_values(sort_cols, kind="mergesort").copy()

    g = s.groupby(group_cols, sort=False, observed=True)
    within_pos = g.cumcount()
    s["seq_lap_pos"] = within_pos.astype("int16")
    ...
    for col in ["LapTime_Delta", "Cumulative_Degradation"]:
        ...
        s[f"{col}_roll2_mean_lag"] = (lag1 + lag2) / 2.0
        s[f"{col}_roll3_mean_lag"] = (lag1 + lag2 + lag3) / 3.0
        s[f"{col}_roll3_std_lag"] = pd.concat([lag1, lag2, lag3], axis=1).std(axis=1)

    pace_l1 = s["LapTime_Delta_lag1"]
    pace_l2 = s["LapTime_Delta_lag2"]
    pace_l3 = s["LapTime_Delta_lag3"]
    degr_l1 = s["Cumulative_Degradation_lag1"]
    degr_l2 = s["Cumulative_Degradation_lag2"]
    degr_l3 = s["Cumulative_Degradation_lag3"]

    s["pace_slope_ewm_lag"] = 0.65 * (pace_l1 - pace_l2) + 0.35 * (pace_l2 - pace_l3)
    s["degr_slope_ewm_lag"] = 0.65 * (degr_l1 - degr_l2) + 0.35 * (degr_l2 - degr_l3)
    s["pace_accel_lag"] = pace_l1 - 2.0 * pace_l2 + pace_l3
    s["degr_accel_lag"] = degr_l1 - 2.0 * degr_l2 + degr_l3

    s["pace_cliff_flag_lag"] = (
        (s["pace_accel_lag"] > 0)
        & (s["pace_slope_ewm_lag"] > 0)
        & (pace_l1 > s["LapTime_Delta_roll3_mean_lag"])
    ).astype("int8")
    s["degr_cliff_flag_lag"] = (
        (s["degr_accel_lag"] > 0)
        & (s["degr_slope_ewm_lag"] > 0)
        & (degr_l1 > s["Cumulative_Degradation_roll3_mean_lag"])
    ).astype("int8")

    x1 = within_pos.shift(1).where(g["_orig_order_tmp"].shift(1).notna())
    x2 = within_pos.shift(2).where(g["_orig_order_tmp"].shift(2).notna())
    x3 = within_pos.shift(3).where(g["_orig_order_tmp"].shift(3).notna())
    x_mean = (x1 + x2 + x3) / 3.0
    y_mean = s["LapTime_Delta_roll3_mean_lag"]
    xx = (x1 - x_mean) ** 2 + (x2 - x_mean) ** 2 + (x3 - x_mean) ** 2
    xy = (
        (x1 - x_mean) * (pace_l1 - y_mean)
        + (x2 - x_mean) * (pace_l2 - y_mean)
        + (x3 - x_mean) * (pace_l3 - y_mean)
    )
    trend_slope = xy / xx.replace(0, np.nan)
    trend_pred = y_mean + trend_slope * (within_pos - x_mean)
    s["pace_resid_to_stint_trend_lag"] = pace_l1 - trend_pred

    prev_pace_min = (
        g["LapTime_Delta"]
        .cummin()
        .groupby([s[c] for c in group_cols], sort=False, observed=True)
        .shift(1)
    )
    is_new_local_min = (s["LapTime_Delta"] <= prev_pace_min) | prev_pace_min.isna()
    local_min_pos = within_pos.where(is_new_local_min)
    last_local_min_pos = (
        local_min_pos.groupby([s[c] for c in group_cols], sort=False, observed=True)
        .ffill()
        .groupby([s[c] for c in group_cols], sort=False, observed=True)
        .shift(1)
    )
    s["laps_since_pace_local_min_lag"] = (within_pos - last_local_min_pos).astype(
        "float32"
    )
    s["pace_above_local_min_lag"] = pace_l1 - prev_pace_min

    s["tyrelife_x_degr_slope_lag"] = s["TyreLife"] * s["degr_slope_ewm_lag"]
    s["tyrelife_x_pace_slope_lag"] = s["TyreLife"] * s["pace_slope_ewm_lag"]
    s["raceprog_x_pace_cliff_lag"] = s["RaceProgress"] * s["pace_cliff_flag_lag"]

    new_cols = [c for c in s.columns if c not in df.columns and c != "_orig_order_tmp"]
    numeric_new = s[new_cols].select_dtypes(include=[np.number]).columns
    s[numeric_new] = s[numeric_new].replace([np.inf, -np.inf], np.nan).fillna(0)

    s = s.sort_values("_orig_order_tmp", kind="mergesort")
    s.index = orig_index
    s = s.drop(columns=["_orig_order_tmp"])
    return s
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000704"],
        "research_usage_note": "Verified assigned hypothesis 000704.",
    }, sort_keys=True))
