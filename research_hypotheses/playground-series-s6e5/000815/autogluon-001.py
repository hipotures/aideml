...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    car_keys = ["Year", "Race", "Driver"]

    compound_order = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    out["compound_code"] = (
        out["Compound"].map(compound_order).fillna(-1).astype("int16")
    )
    ...
    sort_cols = race_keys + ["Driver", "LapNumber", "Stint"]
    s = out.sort_values(sort_cols, kind="mergesort").copy()
    g = s.groupby(car_keys, sort=False)

    s["lag_laptime"] = g["LapTime (s)"].shift(1)
    s["lag_delta"] = g["LapTime_Delta"].shift(1)
    s["lag_tyre_life"] = g["TyreLife"].shift(1)
    s["lag_degradation"] = g["Cumulative_Degradation"].shift(1)
    s["prev_pitstop"] = g["PitStop"].shift(1).fillna(0)
    s["laps_since_pit"] = s["TyreLife"].where(s["TyreLife"].notna(), s["LapNumber"])

    roll3 = g["LapTime (s)"].shift(1).rolling(3, min_periods=1)
    s["old_pace_recent_mean"] = roll3.mean().reset_index(level=[0, 1, 2], drop=True)
    s["old_pace_recent_std"] = (
        roll3.std().reset_index(level=[0, 1, 2], drop=True).fillna(0)
    )

    s["old_tyre_loss_trend"] = (s["lag_laptime"] - s["old_pace_recent_mean"]).fillna(0)
    s["degradation_rate_lag"] = (
        (s["lag_degradation"] / s["lag_tyre_life"].replace(0, np.nan))
    ...
    lag_cols = [
        ...
        "lag_degradation",
        "prev_pitstop",
        "old_pace_recent_mean",
        "old_pace_recent_std",
        "old_tyre_loss_trend",
        "degradation_rate_lag",
    ]
    out.loc[s.index, lag_cols] = s[lag_cols]

    lap_mean = out.groupby(lap_keys, sort=False)["LapTime (s)"].transform("mean")
    lap_median = out.groupby(lap_keys, sort=False)["LapTime (s)"].transform("median")
    lap_std = (
        out.groupby(lap_keys, sort=False)["LapTime (s)"].transform("std").fillna(0)
    )
    out["field_lap_mean"] = lap_mean
    out["field_lap_median"] = lap_median
    out["field_lap_std"] = lap_std
    out["field_relative_pace"] = out["LapTime (s)"] - lap_median
    out["field_relative_delta"] = out["LapTime_Delta"] - out.groupby(
        lap_keys, sort=False
    )["LapTime_Delta"].transform("median")

    rc_comp = out.groupby(race_keys + ["Compound"], sort=False)[
        "LapTime (s)"
    ].transform("median")
    rc_comp_tl = (
        out[out["TyreLife"] <= 5]
        .groupby(race_keys + ["Compound"], sort=False)["LapTime (s)"]
        .transform("median")
    )
    out["fresh_compound_pace_est"] = (
        rc_comp_tl.reindex(idx).fillna(rc_comp).fillna(lap_median)
    )

    race_lap_range = out.groupby(race_keys, sort=False)["LapNumber"].transform(
        "max"
    ) - out.groupby(race_keys, sort=False)["LapNumber"].transform("min")
    out["pit_loss_prior"] = (18.0 + 0.09 * race_lap_range + 0.015 * lap_median).clip(
        18, 30
    )

    out["old_tyre_loss_est"] = (
        0.45 * out["old_tyre_loss_trend"].fillna(0)
        + 0.25 * out["field_relative_pace"].fillna(0)
        + 0.012 * out["Cumulative_Degradation"].fillna(0)
        + 0.08 * out["TyreLife"].fillna(0)
    )
    out["fresh_tyre_gain_est"] = (
        out["LapTime (s)"] - out["fresh_compound_pace_est"]
    ).clip(-10, 25)
    out["counterfactual_stop_value"] = (
        out["old_tyre_loss_est"] + out["fresh_tyre_gain_est"] - out["pit_loss_prior"]
    )

    pos_lap = out.groupby(lap_keys + ["Position"], sort=False)["LapTime (s)"].median()
    ahead_pos = (out["Position"] - 1).clip(lower=1)
    behind_pos = out["Position"] + 1
    ahead_key = pd.MultiIndex.from_arrays(
        [out["Year"], out["Race"], out["LapNumber"], ahead_pos]
    )
    behind_key = pd.MultiIndex.from_arrays(
        [out["Year"], out["Race"], out["LapNumber"], behind_pos]
    )
    ahead_time = pd.Series(pos_lap.reindex(ahead_key).to_numpy(), index=idx)
    behind_time = pd.Series(pos_lap.reindex(behind_key).to_numpy(), index=idx)

    out["gap_to_ahead_proxy"] = (out["LapTime (s)"] - ahead_time).fillna(0)
    out["gap_to_behind_proxy"] = (behind_time - out["LapTime (s)"]).fillna(0)
    out["car_ahead_vulnerable_undercut"] = (
        (out["gap_to_ahead_proxy"] > 0).astype(float)
        * (out["Position"] > 1).astype(float)
        * (out["counterfactual_stop_value"] > -8).astype(float)
    )
    out["traffic_pocket_risk"] = (
        (out["gap_to_behind_proxy"].abs() < 1.5)
        | (out["gap_to_ahead_proxy"].abs() < 1.5)
    ).astype("int8")
    out["undercut_exposure_score"] = (
        out["counterfactual_stop_value"]
        + 0.6 * out["gap_to_ahead_proxy"]
        - 0.4 * out["traffic_pocket_risk"]
        + 0.15 * out["Position_Change"].fillna(0)
    )

    out["late_race_stop_pressure"] = (
        out["RaceProgress"] * out["counterfactual_stop_value"]
    )
    out["stint_tyre_pressure"] = out["TyreLife"] / (
        out["Stint"].replace(0, np.nan).fillna(1)
    )
    out["fresh_vs_field_delta"] = (
        out["fresh_compound_pace_est"] - out["field_lap_median"]
    )

    for col in out.select_dtypes(include=["float64"]).columns:
        out[col] = (
            out[col].replace([np.inf, -np.inf], np.nan).fillna(0).astype("float32")
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000815"],
        "research_usage_note": "Verified assigned hypothesis 000815.",
    }, sort_keys=True))
