...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    n = len(out)
    out["_orig_order"] = np.arange(n)

    group_cols = ["Race", "Driver", "Stint"]
    sort_cols = ["Race", "Driver", "Stint", "LapNumber", "_orig_order"]
    out = out.sort_values(sort_cols, kind="mergesort")

    lap_col = "LapTime (s)"
    race_lap_med = out.groupby(["Race", "Year", "LapNumber"], sort=False)[
        lap_col
    ].transform("median")
    out["pace_resid_race_lap"] = out[lap_col] - race_lap_med

    g = out.groupby(group_cols, sort=False)
    prev_resid = g["pace_resid_race_lap"].shift(1)
    prev_laptime_delta = g["LapTime_Delta"].shift(1)
    prev_deg = g["Cumulative_Degradation"].shift(1)

    out["prev_pace_resid"] = prev_resid.fillna(0.0)
    out["prev_laptime_delta"] = prev_laptime_delta.fillna(0.0)
    out["prev_cum_degradation"] = prev_deg.fillna(0.0)

    delta_resid = prev_resid - g["pace_resid_race_lap"].shift(2)
    out["deg_accel_lag"] = (
        delta_resid
        - delta_resid.groupby([out[c] for c in group_cols], sort=False).shift(1)
    ).fillna(0.0)

    out["roll3_resid_mean_lag"] = (
        prev_resid.groupby([out[c] for c in group_cols], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
        .fillna(0.0)
    )
    out["roll5_resid_mean_lag"] = (
        prev_resid.groupby([out[c] for c in group_cols], sort=False)
    ...
    out["roll3_resid_slope_lag"] = (
        delta_resid.groupby([out[c] for c in group_cols], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
        .fillna(0.0)
    )
    out["roll5_resid_slope_lag"] = (
        delta_resid.groupby([out[c] for c in group_cols], sort=False)
        .rolling(5, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
    ...
    positive_resid = prev_resid.clip(lower=0)
    out["cum_positive_resid_lag"] = (
        positive_resid.groupby([out[c] for c in group_cols], sort=False)
        .cumsum()
        .fillna(0.0)
    )
    ...
    out["tyrelife_minus_compound_optimum"] = out["TyreLife"] - expected_optimum
    out["compound_optimum_abs_distance"] = out["tyrelife_minus_compound_optimum"].abs()
    out["late_stint_degradation_pressure"] = out[
        "tyrelife_minus_compound_optimum"
    ].clip(lower=0) * out["roll5_resid_slope_lag"].clip(lower=0)
    out["resid_volatility_pressure"] = out["roll5_resid_std_lag"] * np.log1p(
        out["TyreLife"]
    )
    out["stint_progress_pressure"] = out["RaceProgress"] * out["TyreLife"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000613"],
        "research_usage_note": "Verified assigned hypothesis 000613.",
    }, sort_keys=True))
