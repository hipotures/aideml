...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if sort_cols:
        order = out.sort_values(sort_cols, kind="mergesort").index
    else:
        order = out.index

    eps = 1e-6

    if "Race" in out.columns and "LapNumber" in out.columns:
        lap_key = (
            ["Year", "Race", "LapNumber"]
            if "Year" in out.columns
            else ["Race", "LapNumber"]
        )
        lap_mean = out.groupby(lap_key)["LapTime (s)"].transform("mean")
        lap_std = (
            out.groupby(lap_key)["LapTime (s)"].transform("std").replace(0, np.nan)
        )
        out["lap_pace_resid"] = out["LapTime (s)"] - lap_mean
        out["lap_pace_z"] = out["lap_pace_resid"] / (lap_std + eps)

        out["same_lap_pace_rank_pct"] = out.groupby(lap_key)["LapTime (s)"].rank(
            pct=True, method="average"
        )
        out["same_lap_pos_rank_pct"] = out.groupby(lap_key)["Position"].rank(
            pct=True, method="average"
        )
        pos_counts = out.groupby(lap_key)["Position"].transform("count")
        out["lap_field_size"] = pos_counts.astype(float)
        out["midfield_crowding"] = (
            1.0 - (out["same_lap_pos_rank_pct"] - 0.5).abs() * 2.0
        ).clip(0, 1)
    else:
        out["lap_pace_resid"] = 0.0
        out["lap_pace_z"] = 0.0
        out["same_lap_pace_rank_pct"] = 0.5
        out["same_lap_pos_rank_pct"] = 0.5
        out["lap_field_size"] = 0.0
        out["midfield_crowding"] = 0.0

    if all(c in out.columns for c in ["Year", "Race", "Driver"]):
        base_key = ["Year", "Race", "Driver"]
    elif all(c in out.columns for c in ["Race", "Driver"]):
        base_key = ["Race", "Driver"]
    else:
        base_key = ["Driver"] if "Driver" in out.columns else None

    if base_key is not None:
        baseline = out.groupby(base_key)["LapTime (s)"].transform("median")
        out["personal_pace_resid"] = out["LapTime (s)"] - baseline
        ordered = out.loc[order]
        shifted_change = ordered.groupby(base_key)["Position_Change"].shift(1).fillna(0)
        stalled = (shifted_change <= 0).astype(float)
        out.loc[order, "stalled_prev_lap"] = stalled.values
        out.loc[order, "stalled_3lap_mean"] = (
            stalled.groupby([ordered[c] for c in base_key])
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=list(range(len(base_key))), drop=True)
            .values
        )
    else:
        out["personal_pace_resid"] = 0.0
        out["stalled_prev_lap"] = 0.0
        out["stalled_3lap_mean"] = 0.0

    tyre_norm = (
        out["TyreLife"]
        / (out.groupby(["Year", "Race"])["TyreLife"].transform("median") + eps)
        if all(c in out.columns for c in ["Year", "Race", "TyreLife"])
        else out["TyreLife"] / (out["TyreLife"].median() + eps)
    )
    deg_norm = (
        out["Cumulative_Degradation"]
        / (
            out.groupby(["Year", "Race"])["Cumulative_Degradation"]
            .transform("median")
            .abs()
            + 1.0
        )
        if all(c in out.columns for c in ["Year", "Race", "Cumulative_Degradation"])
        else out["Cumulative_Degradation"]
        / (abs(out["Cumulative_Degradation"].median()) + 1.0)
    )

    out["normal_tyre_but_slow"] = (
        (out["lap_pace_z"].clip(-3, 3) > 0).astype(float)
        * (tyre_norm < 1.2).astype(float)
        * (deg_norm.abs() < 2.0).astype(float)
        * out["lap_pace_z"].clip(0, 3)
    )

    out["traffic_pressure"] = (
        0.35 * out["midfield_crowding"]
        + 0.25 * out["same_lap_pace_rank_pct"]
        + 0.25 * out["stalled_3lap_mean"]
        + 0.15 * out["normal_tyre_but_slow"]
    )

    phase = out["RaceProgress"].clip(0, 1) if "RaceProgress" in out.columns else 0.5
    out["clean_air_gain_if_pit_now"] = (
        out["traffic_pressure"]
        * (0.5 + phase)
        * (1.0 + out["lap_pace_z"].clip(0, 3) / 3.0)
    )
    out["clean_air_gain_x_phase"] = out["clean_air_gain_if_pit_now"] * phase
    out["traffic_x_tyrelife"] = out["traffic_pressure"] * out["TyreLife"]
    out["traffic_x_stint"] = out["traffic_pressure"] * out["Stint"]

    if "Compound" in out.columns:
        comp = out["Compound"].astype(str).str.upper()
        for name in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
            out[f"clean_air_gain_comp_{name.lower()}"] = out[
                "clean_air_gain_if_pit_now"
            ] * (comp == name).astype(float)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000993"],
        "research_usage_note": "Verified assigned hypothesis 000993.",
    }, sort_keys=True))
