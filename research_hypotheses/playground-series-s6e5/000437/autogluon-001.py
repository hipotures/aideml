...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_norm = out["Race"].astype(str).str.lower()
    comp = out["Compound"].astype(str).str.upper()

    total_laps = (
        out.groupby(["Year", "Race"])["LapNumber"].transform("max").astype(float)
    )
    out["LapsRemaining_Est"] = (total_laps - out["LapNumber"].astype(float)).clip(
        lower=0
    )
    out["LapFrac_Remaining"] = 1.0 - out["RaceProgress"].astype(float).clip(0, 1)
    ...
    out["CompoundHardness"] = comp.map(hard_map).fillna(2.0)

    meta = {
        "bahrain grand prix": (22.5, 3.5, 0.35, 0.60, 3.0),
        "saudi arabian grand prix": (20.5, 3.0, 0.55, 0.35, 2.0),
        "australian grand prix": (21.0, 4.0, 0.45, 0.45, 2.0),
        "azerbaijan grand prix": (21.5, 4.0, 0.55, 0.35, 2.0),
        "miami grand prix": (22.0, 4.0, 0.45, 0.45, 2.0),
        "monaco grand prix": (19.0, 2.5, 0.75, 0.20, 1.0),
        "spanish grand prix": (23.5, 4.5, 0.20, 0.70, 3.0),
        "canadian grand prix": (20.5, 4.0, 0.50, 0.40, 2.0),
        "austrian grand prix": (20.0, 3.5, 0.30, 0.60, 2.0),
        "british grand prix": (22.5, 4.5, 0.45, 0.45, 3.0),
        "hungarian grand prix": (22.0, 4.0, 0.30, 0.60, 2.0),
        "belgian grand prix": (22.5, 5.0, 0.35, 0.55, 3.0),
        "dutch grand prix": (21.0, 4.0, 0.40, 0.50, 2.0),
        "italian grand prix": (24.0, 4.0, 0.70, 0.25, 2.0),
        "singapore grand prix": (28.0, 5.0, 0.45, 0.45, 1.0),
        "japanese grand prix": (23.0, 4.5, 0.25, 0.65, 3.0),
        "qatar grand prix": (24.0, 5.5, 0.15, 0.75, 3.0),
        "united states grand prix": (22.5, 4.5, 0.25, 0.65, 3.0),
        "mexico city grand prix": (21.5, 4.0, 0.45, 0.45, 2.0),
        "sao paulo grand prix": (22.0, 4.5, 0.35, 0.55, 2.0),
        "las vegas grand prix": (21.5, 4.5, 0.50, 0.40, 2.0),
        "abu dhabi grand prix": (21.0, 3.5, 0.45, 0.45, 2.0),
        "pre-season testing": (22.0, 4.0, 0.40, 0.50, 2.0),
    }

    default = pd.Series(
        (22.0, 4.0, 0.42, 0.48, 2.0),
        index=[
            "RacePitLossMed",
            "RacePitLossIQR",
            "RaceOneStopRate",
            "RaceTwoStopRate",
            "RaceCompoundNominationHardness",
        ],
    )
    meta_df = pd.DataFrame.from_dict(meta, orient="index", columns=default.index)

    joined = meta_df.reindex(race_norm).reset_index(drop=True)
    joined = joined.fillna(default)
    for c in joined.columns:
        out[c] = joined[c].to_numpy(dtype=float)

    out["PitLossPerLapRemain"] = out["RacePitLossMed"] / (
        out["LapsRemaining_Est"] + 1.0
    )
    out["TyreLife_x_RaceTwoStopRate"] = (
        out["TyreLife"].astype(float) * out["RaceTwoStopRate"]
    )
    out["TyreLife_x_PitLoss"] = out["TyreLife"].astype(float) * out["RacePitLossMed"]
    out["LapsRemain_x_OneStopRate"] = out["LapsRemaining_Est"] * out["RaceOneStopRate"]
    out["CompoundHardnessGap"] = (
        out["CompoundHardness"] - out["RaceCompoundNominationHardness"]
    )
    out["CompoundHardness_x_TyreLife"] = out["CompoundHardness"] * out[
        "TyreLife"
    ].astype(float)
    out["DegradationPerTyreLap"] = out["Cumulative_Degradation"].astype(float) / (
        out["TyreLife"].astype(float) + 1.0
    )
    out["RaceStopPressure"] = (
        out["RaceTwoStopRate"] * out["TyreLife"].astype(float)
        - out["RaceOneStopRate"] * out["LapsRemaining_Est"]
        + out["PitLossPerLapRemain"]
    )

    for c in ["Race", "Driver", "Compound"]:
        freq = out[c].astype(str).map(out[c].astype(str).value_counts(normalize=True))
        out[c + "_freq"] = freq.astype(float)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000437.",
    }, sort_keys=True))
