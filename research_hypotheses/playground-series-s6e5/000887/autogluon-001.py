...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Code"] = out["Compound"].map(compound_map).astype("float32")
    out["Is_Dry_Tyre"] = out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["Is_Wet_Tyre"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    race_total_laps_est = (
        out["LapNumber"] / out["RaceProgress"].clip(lower=1e-3)
    ).clip(1, 100)
    out["Race_Total_Laps_Est"] = race_total_laps_est.astype("float32")
    out["Laps_Remaining_Est"] = (
        (race_total_laps_est - out["LapNumber"]).clip(0, 100).astype("float32")
    )
    out["Tyre_Stint_Progress"] = (
        (out["TyreLife"] / race_total_laps_est).clip(0, 2).astype("float32")
    )
    out["TyreLife_per_Stint"] = (out["TyreLife"] / out["Stint"].clip(lower=1)).astype(
        "float32"
    )
    ...
    out["Deg_Per_RaceLap"] = (
        (out["Cumulative_Degradation"] / out["LapNumber"].clip(lower=1))
        .replace([np.inf, -np.inf], 0)
        .astype("float32")
    )
    out["LapDelta_Per_TyreLap"] = (
        (out["LapTime_Delta"] / out["TyreLife"].clip(lower=1))
        .replace([np.inf, -np.inf], 0)
        .astype("float32")
    )
    out["LapDelta_x_TyreLife"] = (out["LapTime_Delta"] * out["TyreLife"]).astype(
        "float32"
    )
    out["Deg_x_TyreLife"] = (out["Cumulative_Degradation"] * out["TyreLife"]).astype(
        "float32"
    )
    out["Deg_x_LapsRemaining"] = (
        out["Cumulative_Degradation"] * out["Laps_Remaining_Est"]
    ).astype("float32")
    out["Position_Pressure"] = (out["Position"] * out["Tyre_Stint_Progress"]).astype(
        "float32"
    )

    out = out.sort_values(["Year", "Race", "Driver", "LapNumber", "_row_id"]).copy()
    grp = out.groupby(["Year", "Race", "Driver"], sort=False)

    out["Prev_LapTime"] = grp["LapTime (s)"].shift(1)
    out["Prev_LapDelta"] = grp["LapTime_Delta"].shift(1)
    out["Prev_Position"] = grp["Position"].shift(1)
    out["Prev_TyreLife"] = grp["TyreLife"].shift(1)
    out["Prev_Stint"] = grp["Stint"].shift(1)
    out["Prev_Compound"] = grp["Compound"].shift(1)
    out["Prev_Deg"] = grp["Cumulative_Degradation"].shift(1)

    out["LapTime_Change"] = (
        (out["LapTime (s)"] - out["Prev_LapTime"]).fillna(0).astype("float32")
    )
    out["LapDelta_Change"] = (
        (out["LapTime_Delta"] - out["Prev_LapDelta"]).fillna(0).astype("float32")
    )
    out["Position_Change_FromPrev"] = (
        (out["Position"] - out["Prev_Position"]).fillna(0).astype("float32")
    )
    out["TyreLife_Change"] = (
        (out["TyreLife"] - out["Prev_TyreLife"]).fillna(0).astype("float32")
    )
    out["Deg_Change"] = (
        (out["Cumulative_Degradation"] - out["Prev_Deg"]).fillna(0).astype("float32")
    )
    out["Compound_Changed"] = (
        (out["Compound"] != out["Prev_Compound"]).fillna(False).astype("int8")
    )
    out["Stint_Changed"] = (
        (out["Stint"] != out["Prev_Stint"]).fillna(False).astype("int8")
    )

    out["Tyre_Reset_Flag"] = (
        (out["TyreLife_Change"] < 0)
        | out["Compound_Changed"].eq(1)
        | out["Stint_Changed"].eq(1)
    ).astype("int8")

    out["LapTimeDelta_Roll3"] = (
        grp["LapTime_Delta"]
    ...
    out["Pit_Window_Score"] = (
        out["TyreLife"].astype("float32")
        + 0.03 * out["Cumulative_Degradation"].astype("float32")
        + 0.5 * out["LapTime_Delta"].astype("float32")
        + 2.0 * out["Is_Wet_Tyre"].astype("float32")
        - 0.05 * out["Laps_Remaining_Est"].astype("float32")
    ).astype("float32")

    out = out.sort_values("_row_id").drop(columns=["_row_id", "Prev_Compound"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000887.",
    }, sort_keys=True))
