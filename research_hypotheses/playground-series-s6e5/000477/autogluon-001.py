...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "PitStop" in out.columns:
        ...
        lead = np.full(n, -1, dtype=np.int8)
        lead[same_next_lap] = pit[next_idx_by_orig[same_next_lap]].astype(np.int8)
        out["LeadPitStop"] = lead
        out["LeadPitStop_valid"] = (lead >= 0).astype(np.int8)
