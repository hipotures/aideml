...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    segment = (orig_pos >= train_n).astype(np.int8)

    sort_cols = []
    for c in ["Year", "Race", "Driver", "LapNumber"]:
        if c in out.columns:
            sort_cols.append(c)

    tmp = out[sort_cols].copy()
    tmp["_segment"] = segment
    tmp["_orig_pos"] = orig_pos

    order = tmp.sort_values(
        ["_segment"] + sort_cols + ["_orig_pos"], kind="mergesort"
    ).index.to_numpy()

    sorted_pos = pd.Series(np.arange(n), index=order).sort_index().to_numpy()
    next_order = np.empty(n, dtype=np.int64)
    next_order[:-1] = order[1:]
    next_order[-1] = -1

    next_idx_by_orig = np.empty(n, dtype=np.int64)
    next_idx_by_orig[order] = next_order

    valid_next = next_idx_by_orig >= 0
    same_segment = np.zeros(n, dtype=bool)
    same_segment[valid_next] = (
        segment[valid_next] == segment[next_idx_by_orig[valid_next]]
    )

    same_driver_race = same_segment.copy()
    for c in ["Year", "Race", "Driver"]:
        if c in out.columns:
            vals = out[c].to_numpy()
            eq = np.zeros(n, dtype=bool)
            eq[valid_next] = vals[valid_next] == vals[next_idx_by_orig[valid_next]]
            same_driver_race &= eq

    if "LapNumber" in out.columns:
        lap = out["LapNumber"].to_numpy()
        next_gap = np.full(n, -1.0, dtype=np.float32)
        next_gap[valid_next] = lap[next_idx_by_orig[valid_next]] - lap[valid_next]
        out["NextLapGap"] = next_gap
        same_next_lap = same_driver_race & (next_gap == 1)
    else:
        out["NextLapGap"] = -1.0
        same_next_lap = same_driver_race

    if "PitStop" in out.columns:
        pit = out["PitStop"].to_numpy()
        prev_order = np.empty(n, dtype=np.int64)
        prev_order[0] = -1
        prev_order[1:] = order[:-1]

        prev_idx_by_orig = np.empty(n, dtype=np.int64)
        prev_idx_by_orig[order] = prev_order
        valid_prev = prev_idx_by_orig >= 0

        same_prev_driver_race = valid_prev.copy()
        for c in ["Year", "Race", "Driver"]:
            if c in out.columns:
                vals = out[c].to_numpy()
                eq = np.zeros(n, dtype=bool)
                eq[valid_prev] = vals[valid_prev] == vals[prev_idx_by_orig[valid_prev]]
                same_prev_driver_race &= eq

        prev_pit = np.zeros(n, dtype=np.int8)
        prev_pit[same_prev_driver_race] = pit[
            prev_idx_by_orig[same_prev_driver_race]
        ].astype(np.int8)
        out["PrevPitStop"] = prev_pit

        hist = pd.DataFrame({"_orig": order, "_segment": segment[order], "pit": pit[order]})
        hist_group_cols = ["_segment"]
        for c in ["Year", "Race", "Driver"]:
            if c in out.columns:
                hist[c] = out[c].to_numpy()[order]
                hist_group_cols.append(c)
        prior_count = hist.groupby(hist_group_cols, sort=False)["pit"].cumsum() - hist["pit"]
        prior_seen = np.zeros(n, dtype=np.int8)
        prior_seen[hist["_orig"].to_numpy(dtype=np.int64)] = prior_count.gt(0).astype(np.int8)
        out["PriorPitStopSeen"] = prior_seen

    out["SameDriverRaceNextRow"] = same_driver_race.astype(np.int8)
    out["SameDriverRaceNextLap"] = same_next_lap.astype(np.int8)
    out["EndOfSequence"] = (~same_driver_race).astype(np.int8)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000477"],
        "research_usage_note": "Verified assigned hypothesis 000477.",
    }, sort_keys=True))
