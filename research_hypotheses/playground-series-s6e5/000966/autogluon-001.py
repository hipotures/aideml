...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    peer_cols = ["Year", "Race", "LapNumber"]

    work = df.copy()
    work["_ord"] = np.arange(len(work), dtype=np.int64)
    work = work.sort_values(seq_cols + ["LapNumber", "_ord"], kind="mergesort")

    seq = work.groupby(seq_cols, sort=False, observed=True).ngroup()
    seq_start = seq.ne(seq.shift())

    delta_raw = work["LapTime_Delta"].astype("float32")
    delta = delta_raw.clip(-60, 60).astype("float32")
    tyre_life = work["TyreLife"].astype("float32").clip(lower=1)
    deg_per_lap = (
        (work["Cumulative_Degradation"].astype("float32") / tyre_life)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
    )
    deg_per_lap = deg_per_lap.clip(-100, 200).astype("float32")

    feat = pd.DataFrame(index=work.index)
    feat["Deg_Per_Lap"] = deg_per_lap

    delta_g = delta.groupby(seq, sort=False)
    deg_g = deg_per_lap.groupby(seq, sort=False)

    delta_lag1 = delta_g.shift(1)
    delta_lag2 = delta_g.shift(2)
    deg_lag1 = deg_g.shift(1)
    deg_change = (deg_per_lap - deg_lag1).fillna(0).astype("float32")

    feat["Deg_Per_Lap_Chg"] = deg_change
    feat["LapDelta_Second_Diff"] = (
        (delta - 2 * delta_lag1 + delta_lag2).fillna(0).astype("float32")
    )
    feat["Deg_Per_Lap_Second_Diff"] = (
        (deg_change - deg_change.groupby(seq, sort=False).shift(1))
        .fillna(0)
        .astype("float32")
    )
    ...
    for w in (2, 3, 5):
        denom = float(w - 1)
        feat[f"LapDelta_Slope_{w}"] = (
            ((delta - delta_g.shift(w - 1)) / denom).fillna(0).astype("float32")
        )
        feat[f"Deg_Per_Lap_Slope_{w}"] = (
            ((deg_per_lap - deg_g.shift(w - 1)) / denom).fillna(0).astype("float32")
        )
        feat[f"LapDelta_RollMean_{w}"] = (
            delta_g.rolling(w, min_periods=1)
            .mean()
            .reset_index(level=0, drop=True)
            .astype("float32")
        )
        feat[f"DegChg_RollMean_{w}"] = (
            deg_change.groupby(seq, sort=False)
    ...
    work["_Deg_Per_Lap"] = deg_per_lap
    peer_g = work.groupby(peer_cols, sort=False, observed=True)

    delta_peer_mean = peer_g["LapTime_Delta"].transform("mean").astype("float32")
    delta_peer_std = (
        peer_g["LapTime_Delta"].transform("std").replace(0, np.nan).astype("float32")
    )
    deg_peer_mean = peer_g["_Deg_Per_Lap"].transform("mean").astype("float32")
    deg_peer_std = (
        peer_g["_Deg_Per_Lap"].transform("std").replace(0, np.nan).astype("float32")
    )

    delta_peer_z = (
        ((delta_raw - delta_peer_mean) / delta_peer_std)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .clip(-10, 10)
        .astype("float32")
    ...
    deg_peer_z = (
        ((deg_per_lap - deg_peer_mean) / deg_peer_std)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .clip(-10, 10)
        .astype("float32")
    ...
    feat["LapDelta_Peer_Z"] = delta_peer_z
    feat["Deg_Per_Lap_Peer_Z"] = deg_peer_z

    for w in (2, 3, 5):
        feat[f"LapDelta_Peer_Z_RollMean_{w}"] = (
            delta_peer_z.groupby(seq, sort=False)
            .rolling(w, min_periods=1)
            .mean()
            .reset_index(level=0, drop=True)
            .astype("float32")
        )
        feat[f"LapDelta_Peer_Z_RollMax_{w}"] = (
            delta_peer_z.groupby(seq, sort=False)
            .rolling(w, min_periods=1)
            .max()
            .reset_index(level=0, drop=True)
        ...
        feat[f"DegPeer_Z_RollMean_{w}"] = (
            deg_peer_z.groupby(seq, sort=False)
            .rolling(w, min_periods=1)
            .mean()
            .reset_index(level=0, drop=True)
            .astype("float32")
    ...
    positive_delta = delta.gt(0)
    run_block = ((~positive_delta) | seq_start).cumsum()
    feat["Positive_Delta_Run"] = (
        positive_delta.astype("int16")
        .groupby(run_block, sort=False)
        .cumsum()
        .where(positive_delta, 0)
        .astype("int16")
    )

    pit_now = work["PitStop"].eq(1)
    stint_block = (seq_start | pit_now).cumsum()
    slow = delta.clip(lower=0).where(~pit_now, 0).astype("float32")
    feat["Slowdown_Cusum_Since_Pit"] = (
        slow.groupby(stint_block, sort=False).cumsum().astype("float32")
    )

    feat = feat.replace([np.inf, -np.inf], 0).fillna(0)
    return out.join(feat.reindex(out.index))
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000966"],
        "research_usage_note": "Verified assigned hypothesis 000966.",
    }, sort_keys=True))
