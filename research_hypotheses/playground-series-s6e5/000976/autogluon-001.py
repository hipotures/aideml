...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", "_orig_idx"]
    X = X.sort_values(sort_cols, kind="mergesort")

    g = X.groupby(group_cols, sort=False)

    # Past-only trajectory state
    X["lap_in_driver_race"] = g.cumcount() + 1
    X["laps_remaining"] = (g["LapNumber"].transform("max") - X["LapNumber"]).clip(
        lower=0
    )
    X["prev_pitstop"] = g["PitStop"].shift(1).fillna(0)
    X["past_pit_count"] = (
        g["PitStop"]
        .shift(1)
        .fillna(0)
        .groupby([X["Year"], X["Race"], X["Driver"]], sort=False)
        .cumsum()
    )

    # Stint/hazard progression
    X["stint_progress"] = X["TyreLife"] / (X["LapNumber"].replace(0, np.nan))
    X["stint_progress"] = (
        X["stint_progress"].replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    X["tyre_life_sq"] = X["TyreLife"] ** 2
    X["laps_remaining_sq"] = X["laps_remaining"] ** 2
    X["race_progress_left"] = 1.0 - X["RaceProgress"]

    # Degradation and pace pressure
    tyre_life_safe = X["TyreLife"].replace(0, np.nan)
    lapnum_safe = X["LapNumber"].replace(0, np.nan)
    X["deg_per_tyre_lap"] = (
        (X["Cumulative_Degradation"] / tyre_life_safe)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
    )
    X["deg_per_race_lap"] = (
        (X["Cumulative_Degradation"] / lapnum_safe)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
    )
    X["lap_delta_per_tyre_lap"] = (
        (X["LapTime_Delta"] / tyre_life_safe)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
    )
    X["position_loss_pressure"] = (-X["Position_Change"]).clip(lower=0)
    X["traffic_pressure"] = X["Position"] / 20.0

    # Person-period style lag features (past-only)
    for col in [
        "LapTime (s)",
        "LapTime_Delta",
        "Position",
        ...
        X[f"{col}_lag1"] = g[col].shift(1)
        X[f"{col}_chg1"] = X[col] - X[f"{col}_lag1"]

    # Short rolling summaries using only past/current information
    for col in ["LapTime_Delta", "Position_Change", "Cumulative_Degradation"]:
        X[f"{col}_roll3_mean"] = g[col].transform(
            lambda s: s.rolling(3, min_periods=1).mean()
        )
        X[f"{col}_roll5_mean"] = g[col].transform(
            lambda s: s.rolling(5, min_periods=1).mean()
        )

    # Compound normalization and interactions
    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    X["compound_code"] = X["Compound"].map(compound_map).fillna(-1).astype("int16")
    X["is_dry"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["is_wet"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    X["soft_wear_pressure"] = (X["Compound"] == "SOFT").astype("int8") * X["TyreLife"]
    X["medium_wear_pressure"] = (X["Compound"] == "MEDIUM").astype("int8") * X[
        "TyreLife"
    ]
    X["hard_wear_pressure"] = (X["Compound"] == "HARD").astype("int8") * X["TyreLife"]
    X["wet_wear_pressure"] = X["is_wet"] * X["TyreLife"]

    # Deterministic proxy for hazard horizon / time-to-next-pit urgency
    X["pit_urgency_proxy"] = (
        0.30 * X["TyreLife"].astype(float)
        + 0.20 * X["deg_per_tyre_lap"]
        + 0.15 * X["LapTime_Delta"].astype(float)
        + 0.10 * X["position_loss_pressure"]
        + 0.10 * X["Stint"].astype(float)
        - 0.20 * X["laps_remaining"].astype(float)
        - 0.10 * X["prev_pitstop"].astype(float)
    )
    X["estimated_laps_to_pit_proxy"] = (
        X["laps_remaining"] / (1.0 + X["pit_urgency_proxy"].clip(lower=0))
    ).astype(float)
    X["hazard_x_remaining"] = X["pit_urgency_proxy"] * X["laps_remaining"]
    X["hazard_x_progressleft"] = X["pit_urgency_proxy"] * X["race_progress_left"]
    X["hazard_x_position"] = X["pit_urgency_proxy"] * X["Position"]

    # Global frequency encodings are allowed by contract
    for c in ["Driver", "Race", "Compound"]:
        freq = X[c].value_counts(dropna=False)
        X[f"{c}_freq"] = X[c].map(freq).astype("float32")

    X = X.sort_values("_orig_idx", kind="mergesort").drop(columns=["_orig_idx"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000976.",
    }, sort_keys=True))
