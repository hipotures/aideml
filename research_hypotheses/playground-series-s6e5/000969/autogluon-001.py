...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = ["Year", "Race", "Driver"]
    sort_cols = [
        c
        for c in ["Year", "Race", "Driver", "LapNumber", "Stint", idx_name]
        if c in out.columns
    ]
    work = out.sort_values(sort_cols, kind="mergesort").copy()
    g = (
        work.groupby(cat_cols, sort=False, observed=True)
        if all(c in work.columns for c in cat_cols)
        else None
    )
    ...
    seq_cols = [c for c in seq_cols if c in work.columns]

    if g is not None:
        for c in seq_cols:
            base = pd.to_numeric(work[c], errors="coerce")
            for lag in (1, 2, 3, 5):
                work[f"{c}_lag{lag}"] = g[c].shift(lag)
            work[f"{c}_diff1"] = base - pd.to_numeric(g[c].shift(1), errors="coerce")
            work[f"{c}_roll3_mean"] = (
                g[c]
                .shift(1)
                .rolling(3, min_periods=1)
                .mean()
                .reset_index(level=cat_cols, drop=True)
            )
            work[f"{c}_roll5_mean"] = (
                g[c]
                .shift(1)
        ...
        if "LapNumber" in work.columns:
            work["seq_lap_in_driver_race"] = g.cumcount() + 1
        if "PitStop" in work.columns and "LapNumber" in work.columns:
            last_pit_lap = work["LapNumber"].where(work["PitStop"].eq(1))
            prev_pit_lap = (
                last_pit_lap.groupby(
                    [work[c] for c in cat_cols], sort=False, observed=True
                )
                .ffill()
                .shift(1)
            )
            work["laps_since_prev_pit"] = (
                pd.to_numeric(work["LapNumber"], errors="coerce") - prev_pit_lap
            )
            work["prior_pit_count_driver_race"] = (
                g["PitStop"].cumsum() - work["PitStop"]
            )

    if "TyreLife" in work.columns and "RaceProgress" in work.columns:
        work["tyrelife_x_progress"] = work["TyreLife"] * work["RaceProgress"]
    if "TyreLife" in work.columns and "Stint" in work.columns:
        work["tyrelife_per_stint"] = work["TyreLife"] / (
            work["Stint"].replace(0, np.nan)
        )
    if "LapTime_Delta" in work.columns and "Cumulative_Degradation" in work.columns:
        work["delta_plus_degradation"] = (
            work["LapTime_Delta"] + work["Cumulative_Degradation"]
        )

    for c in ["Compound", "Race", "Driver"]:
        if c in work.columns:
            s = work[c].astype("string").fillna("missing")
            work[c] = s.astype("category")
            freq = s.value_counts(normalize=True)
            work[f"{c}_freq"] = s.map(freq).astype(float)

    work = work.sort_values(idx_name, kind="mergesort").drop(columns=[idx_name])
    return work
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000969"],
        "research_usage_note": "Verified assigned hypothesis 000969.",
    }, sort_keys=True))
