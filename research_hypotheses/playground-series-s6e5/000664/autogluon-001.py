...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["compound_wet_flag"] = X["Compound"].map(wet_map).fillna(0).astype("int8")
    X["compound_grip_code"] = X["Compound"].map(compound_grip).fillna(-1).astype("int8")
    X["lap_time_per_progress"] = X["LapTime (s)"] / X["RaceProgress"].clip(lower=0.01)
    X["degradation_per_tyre_life"] = X["Cumulative_Degradation"] / X["TyreLife"].clip(
        lower=1.0
    )
    X["stint_progress"] = X["TyreLife"] / X["LapNumber"].clip(lower=1.0)
    X["position_pressure"] = X["Position"] / 20.0
    X["late_race_flag"] = (X["RaceProgress"] >= 0.7).astype("int8")

    sort_cols = ["Year", "Race", "LapNumber", "Driver", "_row_order"]
    X = X.sort_values(sort_cols).reset_index(drop=True)

    lap_key = ["Year", "Race", "LapNumber"]
    lap_stats = (
        X.groupby(lap_key, sort=False)
        .agg(
            lap_car_count=("Driver", "size"),
            lap_pit_count=("PitStop", "sum"),
            lap_wet_share=("compound_wet_flag", "mean"),
            lap_laptime_mean=("LapTime (s)", "mean"),
            lap_laptime_std=("LapTime (s)", "std"),
            lap_delta_mean=("LapTime_Delta", "mean"),
            lap_delta_std=("LapTime_Delta", "std"),
            lap_poschg_abs_mean=("Position_Change", lambda s: np.mean(np.abs(s))),
            lap_deg_mean=("Cumulative_Degradation", "mean"),
            lap_tyrelife_mean=("TyreLife", "mean"),
        )
        .reset_index()
    )

    lap_stats["lap_laptime_std"] = lap_stats["lap_laptime_std"].fillna(0.0)
    lap_stats["lap_delta_std"] = lap_stats["lap_delta_std"].fillna(0.0)

    race_groups = lap_stats.groupby(["Year", "Race"], sort=False)
    for col in [
        "lap_pit_count",
        "lap_wet_share",
        "lap_laptime_mean",
        ...
        lap_stats[f"{col}_prev1"] = race_groups[col].shift(1)
        lap_stats[f"{col}_chg1"] = lap_stats[col] - lap_stats[f"{col}_prev1"]

    lap_stats["lap_recent_pit_sum_3"] = race_groups["lap_pit_count"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).sum()
    )
    lap_stats["lap_recent_wet_mean_3"] = race_groups["lap_wet_share"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).mean()
    )
    lap_stats["lap_recent_delta_std_mean_3"] = race_groups["lap_delta_std"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).mean()
    )

    lap_stats["proxy_status_change_flag"] = (
        (lap_stats["lap_pit_count_chg1"].fillna(0) >= 3)
        | (lap_stats["lap_wet_share_chg1"].fillna(0).abs() >= 0.2)
        | (
            lap_stats["lap_delta_std_chg1"].fillna(0)
            > lap_stats["lap_delta_std"].median()
        )
    ).astype("int8")
    lap_stats["proxy_caution_flag"] = (
        (lap_stats["lap_pit_count"] >= 2)
        | (lap_stats["lap_delta_std"] >= lap_stats["lap_delta_std"].quantile(0.75))
        | (
            lap_stats["lap_poschg_abs_mean"]
            >= lap_stats["lap_poschg_abs_mean"].quantile(0.75)
        )
    ).astype("int8")
    lap_stats["proxy_wet_track_flag"] = (
        (lap_stats["lap_wet_share"] > 0)
        | (lap_stats["lap_recent_wet_mean_3"].fillna(0) > 0)
    ).astype("int8")

    X = X.merge(lap_stats, on=lap_key, how="left", sort=False)

    drv_key = ["Year", "Race", "Driver"]
    drv_groups = X.groupby(drv_key, sort=False)
    X["prev_pitstop"] = drv_groups["PitStop"].shift(1).fillna(0).astype("int8")
    X["prev_compound_wet_flag"] = (
        drv_groups["compound_wet_flag"].shift(1).fillna(0).astype("int8")
    )
    X["prev_laptime_delta"] = drv_groups["LapTime_Delta"].shift(1).fillna(0.0)
    X["prev_position_change"] = drv_groups["Position_Change"].shift(1).fillna(0.0)
    X["laps_since_last_pit_proxy"] = drv_groups["PitStop"].cumsum() - X["PitStop"]
    X["laps_since_last_pit_proxy"] = (
        X.groupby(drv_key + ["laps_since_last_pit_proxy"], sort=False).cumcount() + 1
    ).astype("int16")

    X["pit_pressure_vs_field"] = X["lap_pit_count"] / X["lap_car_count"].clip(lower=1)
    X["driver_vs_field_tyre_life"] = X["TyreLife"] - X["lap_tyrelife_mean"]
    X["driver_vs_field_deg"] = X["Cumulative_Degradation"] - X["lap_deg_mean"]
    X["delta_vs_field"] = X["LapTime_Delta"] - X["lap_delta_mean"]
    X["laptime_vs_field"] = X["LapTime (s)"] - X["lap_laptime_mean"]
    X["strategy_window_proxy"] = (
        X["pit_pressure_vs_field"] * X["late_race_flag"]
        + X["proxy_caution_flag"]
        + X["proxy_wet_track_flag"]
    )

    X = X.sort_values("_row_order").drop(columns=["_row_order"])
    return X
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000664.",
    }, sort_keys=True))
