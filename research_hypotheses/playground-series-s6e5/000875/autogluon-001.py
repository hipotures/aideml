...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_step = g["TyreLife"].diff().fillna(1.0).replace(0, 1.0)

    lap_delta_diff = g["LapTime_Delta"].diff()
    lap_time_diff = g["LapTime (s)"].diff()
    degr_diff = g["Cumulative_Degradation"].diff()

    out["ltd_slope"] = (lap_delta_diff / tyre_step).fillna(0.0).clip(-300, 300)
    out["lt_slope"] = (lap_time_diff / tyre_step).fillna(0.0).clip(-300, 300)
    out["deg_slope"] = (degr_diff / tyre_step).fillna(0.0).clip(-300, 300)

    out["ltd_curvature"] = g["ltd_slope"].diff().fillna(0.0).clip(-300, 300)
    out["lt_curvature"] = g["lt_slope"].diff().fillna(0.0).clip(-300, 300)
    out["deg_curvature"] = g["deg_slope"].diff().fillna(0.0).clip(-300, 300)

    out["ltd_slope_recent"] = (
        g["ltd_slope"]
    ...
    out["ltd_slope_prev"] = g["ltd_slope_recent"].shift(2).fillna(0.0)
    out["lt_slope_prev"] = g["lt_slope_recent"].shift(2).fillna(0.0)
    out["deg_slope_prev"] = g["deg_slope_recent"].shift(2).fillna(0.0)

    out["recent_acceleration_in_loss_ltd"] = (
        out["ltd_slope_recent"] - out["ltd_slope_prev"]
    ).clip(-300, 300)
    out["recent_acceleration_in_loss_lt"] = (
        out["lt_slope_recent"] - out["lt_slope_prev"]
    ).clip(-300, 300)
    out["recent_acceleration_in_loss_deg"] = (
        out["deg_slope_recent"] - out["deg_slope_prev"]
    ).clip(-300, 300)

    stint_len = g["LapNumber"].transform("count").clip(lower=1)
    stint_pos = g.cumcount() + 1
    out["stint_phase_fraction"] = stint_pos / stint_len
    out["tyre_life_vs_stint_max"] = out["TyreLife"] / g["TyreLife"].transform(
        "max"
    ).replace(0, 1)

    positive_ltd = out["recent_acceleration_in_loss_ltd"].clip(lower=0)
    positive_lt = out["recent_acceleration_in_loss_lt"].clip(lower=0)
    positive_deg = out["recent_acceleration_in_loss_deg"].clip(lower=0)
    curvature_combo = (
        out["ltd_curvature"].clip(lower=0)
        + out["lt_curvature"].clip(lower=0)
        + out["deg_curvature"].clip(lower=0)
    )

    out["phase_change_score"] = (
        0.45 * positive_ltd
        + 0.35 * positive_lt
        + 0.20 * positive_deg
        + 0.25 * curvature_combo
    ).clip(-1e6, 1e6)

    out["phase_change_score_x_tyre"] = out["phase_change_score"] * out["TyreLife"]
    out["phase_change_score_x_progress"] = (
        out["phase_change_score"] * out["RaceProgress"]
    )
    out["phase_change_score_x_stint_phase"] = (
        out["phase_change_score"] * out["stint_phase_fraction"]
    )

    out["pre_cliff_vs_post_cliff"] = pd.cut(
        out["phase_change_score"],
        bins=[-float("inf"), 0.03, 0.15, 0.50, float("inf")],
        labels=["stable", "warning", "transition", "cliff"],
    ).astype(str)

    out["combined_cliff_pressure"] = (
        out["phase_change_score"]
        * (1.0 + out["tyre_life_vs_stint_max"])
        * (1.0 + out["stint_phase_fraction"])
    ).clip(-1e6, 1e6)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000875"],
        "research_usage_note": "Verified assigned hypothesis 000875.",
    }, sort_keys=True))
