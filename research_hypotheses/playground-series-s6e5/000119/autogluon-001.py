...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_keys = ["Year", "Race", "Driver"]
    stint_keys = ["Year", "Race", "Driver", "Stint"]
    sort_cols = race_keys + ["LapNumber", "_orig_order"]
    ...
    out["CompoundOrdinal"] = out["Compound"].map(compound_map).astype("float32")
    out["IsDryCompound"] = (
        out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    )
    out["IsWetCompound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    grp_race = out.groupby(race_keys, sort=False)
    grp_stint = out.groupby(stint_keys, sort=False)

    base_cols = ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]

    for col in base_cols:
        out[f"{col}_lag1"] = grp_race[col].shift(1).astype("float32")

    for w in [2, 3, 4, 5]:
        for col in base_cols:
            lag1 = grp_stint[col].shift(1)
            lagw = grp_stint[col].shift(w)
            slope = (lag1 - lagw) / (w - 1)
            out[f"{col}_slope_{w}"] = slope.astype("float32")

    for col in base_cols:
        s2 = out[f"{col}_slope_2"]
        s3 = out[f"{col}_slope_3"]
        s5 = out[f"{col}_slope_5"]
        out[f"{col}_accel_2_3"] = (s2 - s3).astype("float32")
        out[f"{col}_accel_2_5"] = (s2 - s5).astype("float32")

    # Causal within-stint baseline and residuals after each reset.
    for col in base_cols:
        prior_csum = grp_stint[col].cumsum() - out[col]
        prior_count = grp_stint.cumcount()
        baseline = prior_csum / prior_count.replace(0, np.nan)
        out[f"{col}_stint_baseline"] = baseline.astype("float32")
        out[f"{col}_stint_resid"] = (out[col] - baseline).astype("float32")

    # Relative progress inside stint and normalized degradation state.
    stint_lap_idx = grp_stint.cumcount() + 1
    out["StintLapIndex"] = stint_lap_idx.astype("float32")
    out["TyreLife_minus_StintLap"] = (out["TyreLife"] - stint_lap_idx).astype("float32")
    out["Degradation_per_TyreLife"] = (
        out["Cumulative_Degradation"] / out["TyreLife"].replace(0, np.nan)
    ).astype("float32")
    out["LapTimeDelta_per_TyreLife"] = (
        out["LapTime_Delta"] / out["TyreLife"].replace(0, np.nan)
    ).astype("float32")
    out["Degradation_x_RaceProgress"] = (
        out["Cumulative_Degradation"] * out["RaceProgress"]
    ).astype("float32")

    # Reset-aware interactions around stop window.
    out["TyreLife_x_CompoundOrdinal"] = (
        out["TyreLife"] * out["CompoundOrdinal"]
    ).astype("float32")
    out["StintLap_x_CompoundOrdinal"] = (
        out["StintLapIndex"] * out["CompoundOrdinal"]
    ).astype("float32")
    out["PositionChange_x_LapTimeDelta"] = (
        out["Position_Change"] * out["LapTime_Delta"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000119"],
        "research_usage_note": "Verified assigned hypothesis 000119.",
    }, sort_keys=True))
