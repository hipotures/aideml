...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Compound" in X.columns:
        comp = X["Compound"].astype(str)
        X["Compound_is_soft"] = (comp == "SOFT").astype("int8")
        X["Compound_is_medium"] = (comp == "MEDIUM").astype("int8")
        X["Compound_is_hard"] = (comp == "HARD").astype("int8")
        X["Compound_is_wet_family"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
    ...
    for c in num_cols:
        ...
        X[f"{c}_clip"] = s.clip(s.quantile(0.01), s.quantile(0.99))
        X[f"{c}_rank"] = s.rank(method="average", pct=True).astype("float32")
        X[f"{c}_signed_log1p"] = np.sign(s) * np.log1p(np.abs(s))

    grp_cols = [c for c in ["Year", "Race"] if c in X.columns]
    race_group = X.groupby(grp_cols, observed=True) if grp_cols else None

    if race_group is not None:
        for c in [
            k
            for k in [
                "TyreLife",
            ...
            X[f"{c}_race_rank"] = (
                race_group[c].rank(method="average", pct=True).astype("float32")
            )
            X[f"{c}_race_z"] = (
                (
                    (X[c] - race_group[c].transform("mean"))
                    / (race_group[c].transform("std").replace(0, np.nan))
                )
                .fillna(0.0)
                .astype("float32")
            )

    if {"TyreLife", "Stint"}.issubset(X.columns):
        X["TyreLife_per_Stint"] = X["TyreLife"] / X["Stint"].replace(0, 1)
        X["TyreLife_x_Stint"] = X["TyreLife"] * X["Stint"]

    if {"TyreLife", "RaceProgress"}.issubset(X.columns):
        X["PitWindowPressure"] = X["TyreLife"] * X["RaceProgress"]
        X["TyreLife_over_Progress"] = X["TyreLife"] / X["RaceProgress"].clip(lower=1e-3)

    if {"Cumulative_Degradation", "TyreLife"}.issubset(X.columns):
        X["Deg_per_TyreLife"] = X["Cumulative_Degradation"] / X["TyreLife"].replace(
            0, np.nan
        )
        X["Deg_x_TyreLife"] = X["Cumulative_Degradation"] * X["TyreLife"]

    if {"LapTime_Delta", "Position_Change"}.issubset(X.columns):
        X["PaceTrackPosStress"] = X["LapTime_Delta"] * (-X["Position_Change"])
        X["GainWithSlowLap"] = (X["Position_Change"] > 0).astype("int8") * X[
            "LapTime_Delta"
        ]

    if {"Position", "RaceProgress"}.issubset(X.columns):
        X["Position_x_Progress"] = X["Position"] * X["RaceProgress"]

    if {"LapNumber", "TyreLife"}.issubset(X.columns):
        X["FreshnessGap"] = X["LapNumber"] - X["TyreLife"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000634.",
    }, sort_keys=True))
