...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g = out.groupby(keys, sort=False, observed=True)
    lap_id = g.ngroup().astype("int32")
    field_size = g["Position"].transform("count").astype("float32")
    out["field_size"] = field_size

    for col in ["TyreLife", "LapTime_Delta", "Cumulative_Degradation"]:
        base = col.replace(" ", "_").replace("(", "").replace(")", "")
        mean = g[col].transform("mean")
        std = g[col].transform("std").replace(0, np.nan)
        out[f"{base}_lap_pct"] = (
            g[col].rank(method="average", pct=True).astype("float32").fillna(0.5)
        )
        out[f"{base}_lap_z"] = (
            ((out[col] - mean) / std).clip(-5, 5).astype("float32").fillna(0)
        )

    ordered = out.sort_values(keys + ["Position"], kind="mergesort")
    og = ordered.groupby(keys, sort=False, observed=True)
    pos_ord = ordered["Position"].astype("float32")
    ahead_pos = og["Position"].shift(1).astype("float32")
    behind_pos = og["Position"].shift(-1).astype("float32")

    out["pos_gap_ahead"] = 99.0
    out["pos_gap_behind"] = 99.0
    out.loc[ordered.index, "pos_gap_ahead"] = (
        (pos_ord - ahead_pos).fillna(99).clip(0, 99).astype("float32")
    )
    out.loc[ordered.index, "pos_gap_behind"] = (
        (behind_pos - pos_ord).fillna(99).clip(0, 99).astype("float32")
    )
    out["pos_gap_min"] = np.minimum(out["pos_gap_ahead"], out["pos_gap_behind"]).astype(
        "float32"
    )
    out["pos_clean_air"] = (
        (out["pos_gap_ahead"] + out["pos_gap_behind"]).clip(0, 99).astype("float32")
    )

    pos_int = out["Position"].astype("int16")
    pos_counts = (
        pd.Series(
            np.ones(n, dtype="int16"),
            index=pd.MultiIndex.from_arrays([lap_id, pos_int]),
        )
        .groupby(level=[0, 1])
        .sum()
    )

    def count_at(position_values):
        idx = pd.MultiIndex.from_arrays([lap_id, position_values.astype("int16")])
        return pos_counts.reindex(idx, fill_value=0).to_numpy(dtype="float32")

    for band in (1, 2, 3):
        density = np.zeros(n, dtype="float32")
        for offset in range(-band, band + 1):
            density += count_at(pos_int + offset)
        density = np.maximum(density - 1.0, 0.0)
        out[f"rival_density_pm{band}"] = density

    lap_tyre_median = g["TyreLife"].transform("median")
    old_tyre = (out["TyreLife"] >= lap_tyre_median).astype("int8")
    old_ord = old_tyre.loc[ordered.index]
    cumsum_old = old_ord.groupby(
        [ordered["Year"], ordered["Race"], ordered["LapNumber"]],
        sort=False,
        observed=True,
    ).cumsum()
    total_old = old_ord.groupby(
        [ordered["Year"], ordered["Race"], ordered["LapNumber"]],
        sort=False,
        observed=True,
    ).transform("sum")

    out["old_tyre_ahead"] = 0.0
    out["old_tyre_behind"] = 0.0
    out.loc[ordered.index, "old_tyre_ahead"] = (cumsum_old - old_ord).astype("float32")
    out.loc[ordered.index, "old_tyre_behind"] = (total_old - cumsum_old).astype(
        "float32"
    )
    out["old_tyre_traffic_balance"] = (
        out["old_tyre_ahead"] - out["old_tyre_behind"]
    ).astype("float32")

    race_g = out.groupby("Race", sort=False, observed=True)
    race_lap_med = race_g["LapTime (s)"].transform("median").astype("float32")
    lap_min = float(race_lap_med.min())
    lap_rng = (
        float(race_lap_med.max() - lap_min)
        if float(race_lap_med.max() - lap_min) != 0
        else 1.0
    )
    race_lap_scaled = ((race_lap_med - lap_min) / lap_rng).clip(0, 1).astype("float32")
    race_pit_rate = race_g["PitStop"].transform("mean").astype("float32")
    race_deg_abs = (
        race_g["Cumulative_Degradation"]
        .transform(lambda s: s.abs().median())
        .astype("float32")
    )

    out["race_pit_rate_prior"] = race_pit_rate
    out["race_deg_prior"] = race_deg_abs
    pit_loss_pos_prior = (
        (3.0 + 6.0 * race_lap_scaled + 3.0 * race_pit_rate)
        .clip(3, 12)
        .astype("float32")
    )
    out["pit_loss_pos_prior"] = pit_loss_pos_prior

    rejoin_pos = (
        (out["Position"].astype("float32") + pit_loss_pos_prior)
        .round()
        .clip(1, 30)
        .astype("int16")
    )
    rejoin_density = np.zeros(n, dtype="float32")
    for offset in range(-2, 3):
        rejoin_density += count_at(rejoin_pos + offset)
    out["rejoin_density_pm2"] = rejoin_density
    out["sparse_rejoin_pocket"] = (1.0 / (1.0 + rejoin_density)).astype("float32")

    out["undercut_pocket_score"] = (
        out["TyreLife_lap_pct"]
        * out["Cumulative_Degradation_lap_pct"]
        * (1.0 + out["old_tyre_ahead"])
        * out["sparse_rejoin_pocket"]
        * (1.0 + race_pit_rate)
    ).astype("float32")
    out["traffic_risk_score"] = (
        (
            out["rival_density_pm2"]
            * (1.0 - out["sparse_rejoin_pocket"])
            * (1.0 + out["Position"].astype("float32") / field_size.replace(0, np.nan))
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000805"],
        "research_usage_note": "Verified assigned hypothesis 000805.",
    }, sort_keys=True))
