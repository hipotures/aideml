...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in seq_cols:
        ...
        work[f"{col}_lag1"] = prior
        work[f"{col}_lag2"] = grouped[col].shift(2)

        for window in (3, 5):
            roll = prior.groupby([work[c] for c in group_cols], sort=False).rolling(
                window, min_periods=1
            )
            mean_vals = roll.mean().reset_index(
            ...
            work[f"{col}_roll{window}_mean"] = mean_vals
            work[f"{col}_roll{window}_std"] = std_vals
            work[f"{col}_minus_roll{window}_mean"] = work[col] - mean_vals

        work[f"{col}_delta_lag1"] = work[col] - work[f"{col}_lag1"]

    work["pace_worse_lag1"] = (
        work["LapTime_Delta"] > work["LapTime_Delta_lag1"]
    ).astype("int8")
    work["position_loss_lag1"] = (work["Position"] > work["Position_lag1"]).astype(
        "int8"
    )
    work["position_change_worse_lag1"] = (
        work["Position_Change"] > work["Position_Change_lag1"]
    ).astype("int8")
    work["degradation_worse_lag1"] = (
        work["Cumulative_Degradation"] > work["Cumulative_Degradation_lag1"]
    ).astype("int8")
    work["tyrelife_increased_lag1"] = (work["TyreLife"] > work["TyreLife_lag1"]).astype(
        "int8"
    )
    work["pitted_last_lap"] = work["PitStop_lag1"].fillna(0).astype("int8")

    work["stint_lap_ratio"] = work["TyreLife"] / (work["LapNumber"].replace(0, np.nan))
    work["degradation_per_tyre_lap"] = work["Cumulative_Degradation"] / (
        work["TyreLife"].replace(0, np.nan)
    )
    work["race_progress_x_tyre_life"] = work["RaceProgress"] * work["TyreLife"]

    return work.loc[orig_index]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000916"],
        "research_usage_note": "Verified assigned hypothesis 000916.",
    }, sort_keys=True))
