...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["tyre_bin"] = X["TyreLife"].round().clip(1, 80).astype(np.int16)
    X["progress_bin"] = np.floor(X["RaceProgress"].clip(0, 0.999999) * 10).astype(
        np.int8
    )
    X["surface"] = np.where(
        X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]), "DRY", "WET"
    )

    key_cols = ["surface", "Compound", "Stint", "progress_bin"]
    m = 25.0

    counts = (
        X.groupby(key_cols + ["tyre_bin"], observed=True)
        .size()
        .rename("risk_n")
        .reset_index()
        .sort_values(key_cols + ["tyre_bin"])
    )
    counts["next_risk_n"] = (
        counts.groupby(key_cols, observed=True)["risk_n"].shift(-1).fillna(0.0)
    )
    counts["hazard_raw"] = 1.0 - counts["next_risk_n"] / counts["risk_n"].clip(lower=1)

    base = (
        X.groupby(["surface", "tyre_bin"], observed=True)
        .size()
        .rename("base_risk_n")
        .reset_index()
        .sort_values(["surface", "tyre_bin"])
    )
    base["base_next_risk_n"] = (
        base.groupby(["surface"], observed=True)["base_risk_n"].shift(-1).fillna(0.0)
    )
    base["base_hazard"] = 1.0 - base["base_next_risk_n"] / base["base_risk_n"].clip(
        lower=1
    )
    base = base[["surface", "tyre_bin", "base_hazard"]]

    counts = counts.merge(base, on=["surface", "tyre_bin"], how="left")
    counts["hazard_smoothed"] = (
        counts["risk_n"] * counts["hazard_raw"] + m * counts["base_hazard"].fillna(0.0)
    ) / (counts["risk_n"] + m)
    counts["hazard_smoothed"] = counts.groupby(key_cols, observed=True)[
        "hazard_smoothed"
    ].cummax()

    first_risk = (
        counts.groupby(key_cols, observed=True)["risk_n"].transform("max").clip(lower=1)
    )
    counts["survival_proxy"] = counts["risk_n"] / first_risk
    counts["hazard_prev"] = counts.groupby(key_cols, observed=True)[
        "hazard_smoothed"
    ].shift(1)
    counts["hazard_next"] = counts.groupby(key_cols, observed=True)[
        "hazard_smoothed"
    ].shift(-1)
    counts["hazard_delta_prev"] = counts["hazard_smoothed"] - counts[
        "hazard_prev"
    ].fillna(counts["hazard_smoothed"])
    counts["hazard_delta_next"] = (
        counts["hazard_next"].fillna(counts["hazard_smoothed"])
        - counts["hazard_smoothed"]
    )
    ...
    X = X.merge(counts[feat_cols], on=key_cols + ["tyre_bin"], how="left")

    X["hazard_smoothed"] = (
        X["hazard_smoothed"]
        .fillna(X["surface"].map({"DRY": 0.02, "WET": 0.03}))
        .astype(np.float32)
    )
    X["survival_proxy"] = X["survival_proxy"].fillna(1.0).astype(np.float32)
    X["hazard_delta_prev"] = X["hazard_delta_prev"].fillna(0.0).astype(np.float32)
    X["hazard_delta_next"] = X["hazard_delta_next"].fillna(0.0).astype(np.float32)

    X["hazard_x_degradation"] = (
        X["hazard_smoothed"] * X["Cumulative_Degradation"]
    ).astype(np.float32)
    X["hazard_x_laptime_delta"] = (X["hazard_smoothed"] * X["LapTime_Delta"]).astype(
        np.float32
    )
    X["survival_x_progress"] = (X["survival_proxy"] * X["RaceProgress"]).astype(
        np.float32
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000933"],
        "research_usage_note": "Verified assigned hypothesis 000933.",
    }, sort_keys=True))
