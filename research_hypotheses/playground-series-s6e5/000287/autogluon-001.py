...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["LapTime_Delta", "TyreLife", "Cumulative_Degradation"]:
        med = g[col].transform("median").astype("float32")
        mean = g[col].transform("mean").astype("float32")
        std = g[col].transform("std").replace(0, np.nan).astype("float32")
        out[f"{col}_lap_rel"] = (out[col] - med).astype("float32")
        out[f"{col}_lap_z"] = ((out[col] - mean) / std).fillna(0).astype("float32")
        out[f"{col}_lap_pct"] = g[col].rank(pct=True).astype("float32")

    lap_size = g["Position"].transform("count").astype("float32")
    out["position_lap_pct"] = (
        ((out["Position"] - 1) / (lap_size - 1).replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )
    ...
    for shift, name in [(-1, "ahead"), (1, "behind")]:
        for col in [
            "LapTime_Delta",
            "TyreLife",
            "Cumulative_Degradation",
            ...
            ordered[f"{col}_{name}"] = og[col].shift(shift)

    ordered["pace_vs_ahead"] = (
        ordered["LapTime_Delta"] - ordered["LapTime_Delta_ahead"]
    ).astype("float32")
    ordered["pace_vs_behind"] = (
        ordered["LapTime_Delta"] - ordered["LapTime_Delta_behind"]
    ).astype("float32")
    ordered["tyrelife_vs_ahead"] = (
        ordered["TyreLife"] - ordered["TyreLife_ahead"]
    ).astype("float32")
    ordered["tyrelife_vs_behind"] = (
        ordered["TyreLife"] - ordered["TyreLife_behind"]
    ).astype("float32")
    ordered["degradation_vs_ahead"] = (
        ordered["Cumulative_Degradation"] - ordered["Cumulative_Degradation_ahead"]
    ).astype("float32")
    ordered["degradation_vs_behind"] = (
        ordered["Cumulative_Degradation"] - ordered["Cumulative_Degradation_behind"]
    ).astype("float32")

    ordered["stuck_behind_slower"] = (
        (ordered["Position"] > 1)
        & (ordered["LapTime_Delta"] < ordered["LapTime_Delta_ahead"])
        & (ordered["TyreLife"] >= ordered["TyreLife_ahead"])
    ).astype("int8")

    ordered["under_pressure_from_behind"] = (
        ordered["LapTime_Delta_behind"].notna()
        & (ordered["LapTime_Delta_behind"] < ordered["LapTime_Delta"])
        & (ordered["TyreLife_behind"] <= ordered["TyreLife"])
    ).astype("int8")

    ordered["clean_air_ahead_proxy"] = (
        ordered["LapTime_Delta_ahead"].isna()
        | (ordered["LapTime_Delta_ahead"] < ordered["LapTime_Delta"])
    ).astype("int8")

    ordered["undercut_opportunity"] = (
        (ordered["stuck_behind_slower"] == 1)
        & (ordered["LapTime_Delta_lap_pct"] <= 0.45)
        & (ordered["TyreLife_lap_pct"] >= 0.55)
    ).astype("int8")

    ordered["overcut_risk_or_need"] = (
        (ordered["under_pressure_from_behind"] == 1)
        & (ordered["LapTime_Delta_lap_pct"] >= 0.55)
        & (ordered["Cumulative_Degradation_lap_pct"] >= 0.55)
    ).astype("int8")

    new_cols = [
        c
        for c in ordered.columns
        if c not in out.columns or c.endswith(("_ahead", "_behind"))
    ]
    out = out.join(ordered[new_cols].sort_index())

    fill_cols = [
        c for c in out.columns if c.endswith(("_ahead", "_behind")) or "_vs_" in c
    ]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000287"],
        "research_usage_note": "Verified assigned hypothesis 000287.",
    }, sort_keys=True))
