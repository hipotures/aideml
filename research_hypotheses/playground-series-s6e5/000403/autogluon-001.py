...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cat_cols:
        if c in out.columns:
            out[c] = out[c].astype("string").str.strip().str.upper().fillna("MISSING")
            freq = out[c].value_counts(dropna=False)
            out[f"{c}_freq"] = out[c].map(freq).astype("float32")
            out[f"{c}_freq_log"] = np.log1p(out[f"{c}_freq"]).astype("float32")

    if {"Race", "Year"}.issubset(out.columns):
        out["RaceYear"] = (
            out["Race"].astype("string") + "_" + out["Year"].astype("string")
        )
    if {"Race", "Year", "Driver"}.issubset(out.columns):
        out["RaceYearDriver"] = (
            out["Race"].astype("string")
            + "_"
            + out["Year"].astype("string")
            + "_"
            + out["Driver"].astype("string")
        )
    if {"Race", "Year", "Driver", "Stint"}.issubset(out.columns):
        out["Query_RaceYearDriverStint"] = (
            out["Race"].astype("string")
            + "_"
            + out["Year"].astype("string")
            + "_"
            + out["Driver"].astype("string")
            + "_"
            + out["Stint"].astype("string")
        )

    for c in ["RaceYear", "RaceYearDriver", "Query_RaceYearDriverStint"]:
        if c in out.columns:
            freq = out[c].value_counts(dropna=False)
            out[f"{c}_freq"] = out[c].map(freq).astype("float32")
            out[f"{c}_freq_log"] = np.log1p(out[f"{c}_freq"]).astype("float32")

    group_cols = [c for c in ["Race", "Year", "Driver", "Stint"] if c in out.columns]
    num_cols = [
        c
        for c in [
            "LapNumber",
    ...
    if group_cols:
        ...
        out["query_group_size"] = g[group_cols[0]].transform("size").astype("float32")
        ...
        for c in num_cols:
            ...
            out[f"{c}_q_rel_mean"] = (s - mean).astype("float32")
            out[f"{c}_q_z"] = (
                ((s - mean) / std)
    ...
    if {"TyreLife", "LapNumber"}.issubset(out.columns):
        out["tyrelife_lap_ratio"] = (
            pd.to_numeric(out["TyreLife"], errors="coerce")
            / (pd.to_numeric(out["LapNumber"], errors="coerce") + 1.0)
        ).astype("float32")
    if {"Cumulative_Degradation", "TyreLife"}.issubset(out.columns):
        out["degradation_per_tyre_lap"] = (
            ...
            / (pd.to_numeric(out["TyreLife"], errors="coerce") + 1.0)
        ).astype("float32")
    if {"RaceProgress", "TyreLife"}.issubset(out.columns):
        out["progress_x_tyre_life"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000403"],
        "research_usage_note": "Verified assigned hypothesis 000403.",
    }, sort_keys=True))
