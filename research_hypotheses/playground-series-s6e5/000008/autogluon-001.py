...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_progress = out["RaceProgress"].astype(float).clip(0.0, 1.0)
    ...
    pos_change = out["Position_Change"].astype(float)

    race_key = out["Year"].astype(str) + "|" + out["Race"].astype(str)
    race_max_lap = (
        out.groupby(race_key, sort=False)["LapNumber"].transform("max").astype(float)
    )
    laps_remaining = (race_max_lap - lap).clip(lower=0.0)

    is_slick = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype(np.int8)
    is_soft = (compound == "SOFT").astype(np.int8)
    is_medium = (compound == "MEDIUM").astype(np.int8)
    is_hard = (compound == "HARD").astype(np.int8)
    is_inter = (compound == "INTERMEDIATE").astype(np.int8)
    is_wet = (compound == "WET").astype(np.int8)

    tyre_age_share = tyre_life / (lap + 1.0)
    stint_share = tyre_life / (stint + 1.0)
    degr_per_lap_on_tyre = degr / tyre_life.clip(lower=1.0)
    delta_per_lap_on_tyre = lap_delta / tyre_life.clip(lower=1.0)

    out["race_max_lap"] = race_max_lap
    out["laps_remaining"] = laps_remaining
    out["tyre_age_share"] = tyre_age_share
    out["stint_share"] = stint_share

    # Archetype 1: current-tyre finish feasibility
    out["finish_gap_laps"] = laps_remaining - tyre_life
    out["finish_load_ratio"] = laps_remaining / tyre_life.clip(lower=1.0)
    out["projected_finish_degradation"] = degr_per_lap_on_tyre * laps_remaining
    out["projected_finish_delta"] = delta_per_lap_on_tyre * laps_remaining
    out["late_old_tyre_pressure"] = race_progress * tyre_life
    out["late_finish_risk"] = (
        race_progress
        * out["finish_load_ratio"]
        * (1.0 + degr_per_lap_on_tyre.clip(lower=0.0))
    )

    # Archetype 2: compact fresh-slick option richness
    out["is_slick"] = is_slick
    out["is_soft"] = is_soft
    out["is_medium"] = is_medium
    out["is_hard"] = is_hard
    out["is_inter"] = is_inter
    out["is_wet"] = is_wet
    out["slick_phase"] = is_slick * race_progress
    out["wet_phase"] = (is_inter + is_wet) * race_progress
    out["pace_deg_interaction"] = lap_delta * degr_per_lap_on_tyre
    out["compound_age_pressure"] = tyre_life * (
        1.5 * is_soft + 1.0 * is_medium + 0.7 * is_hard + 1.2 * is_inter + 1.4 * is_wet
    )
    out["fresh_option_pull"] = (
        laps_remaining / (tyre_life + 3.0) + (1 - is_slick) * 0.5 + race_progress * 0.5
    )

    # Archetype 3: strict stop-debt / rule-state style
    expected_stops_done = np.floor((lap - 1.0) / race_max_lap.clip(lower=1.0) * 2.0)
    out["stops_done_proxy"] = (stint - 1.0).clip(lower=0.0)
    out["stop_debt"] = expected_stops_done - out["stops_done_proxy"]
    out["stop_debt_late"] = out["stop_debt"] * race_progress
    out["stint_finish_gap"] = laps_remaining - (race_max_lap / (stint + 1.0))
    out["overdue_pit_flag"] = (
        (out["finish_gap_laps"] < 0) | (out["stop_debt"] > 0.5)
    ).astype(np.int8)

    # Context interactions that often help tree models separate pit windows
    out["position_risk"] = pos * out["finish_load_ratio"]
    out["track_position_pressure"] = (21.0 - pos) * pos_change
    out["degr_position_mix"] = degr_per_lap_on_tyre * pos
    out["lap_time_vs_progress"] = lap_time * race_progress
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000008.",
    }, sort_keys=True))
