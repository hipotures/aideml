...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    keys = ["Race", "Driver", "Stint"]
    sort_cols = [
        c for c in ["Year", "Race", "Driver", "Stint", "LapNumber"] if c in out.columns
    ]
    work = (
        out.assign(_orig_order=orig_idx)
        .sort_values(sort_cols, kind="mergesort")
        .reset_index(drop=True)
    )

    numeric_cols = ["LapTime_Delta", "LapTime (s)", "Cumulative_Degradation"]
    g = work.groupby(keys, sort=False, observed=True)

    for col in numeric_cols:
        base = work[col].astype("float32")
        prev1 = g[col].shift(1).astype("float32")
        prev2 = g[col].shift(2).astype("float32")
        prev3 = g[col].shift(3).astype("float32")

        slope = (prev1 - prev2).fillna(0).clip(-80, 80)
        accel = (prev1 - 2 * prev2 + prev3).fillna(0).clip(-80, 80)
        work[f"{col}_past_slope"] = slope.astype("float32")
        work[f"{col}_past_accel"] = accel.astype("float32")
        work[f"{col}_jump_abs"] = slope.abs().astype("float32")

        recent_max = (
            g[col]
            .shift(1)
            .groupby([work[k] for k in keys], sort=False, observed=True)
        ...
        work[f"{col}_recent_range4"] = (
            (recent_max - recent_min).fillna(0).clip(0, 120).astype("float32")
        )

    lap_slope_med = work.groupby(["Race", "LapNumber"], sort=False, observed=True)[
        "LapTime_Delta_past_slope"
    ].transform("median")
    work["delta_slope_vs_race_lap"] = (
        (work["LapTime_Delta_past_slope"] - lap_slope_med)
        .fillna(0)
        .clip(-80, 80)
        .astype("float32")
    )

    pos_delta = work["LapTime_Delta_past_slope"].clip(lower=0)
    is_pos = (pos_delta > 0).astype("int16")
    reset_id = (
        (is_pos.eq(0))
        .groupby([work[k] for k in keys], sort=False, observed=True)
        .cumsum()
    )
    work["positive_delta_streak"] = (
        is_pos.groupby([work[k] for k in keys] + [reset_id], sort=False, observed=True)
        .cumsum()
        .astype("int16")
    )
    work["recent_max_deterioration"] = (
        work[
            [
                "LapTime_Delta_past_slope",
                "LapTime (s)_past_slope",
                "Cumulative_Degradation_past_slope",
            ]
        ]
        .max(axis=1)
        .clip(0, 100)
    ...
    band_keys = ["Race", "Compound"]
    band_med = work.groupby(band_keys, sort=False, observed=True)[
        "LapTime_Delta_past_slope"
    ].transform("median")
    band_q75 = work.groupby(band_keys, sort=False, observed=True)[
        "LapTime_Delta_past_slope"
    ].transform(lambda s: s.quantile(0.75))
    band_q25 = work.groupby(band_keys, sort=False, observed=True)[
        "LapTime_Delta_past_slope"
    ].transform(lambda s: s.quantile(0.25))
    band_thr = band_med + 1.5 * (band_q75 - band_q25).clip(lower=0.05)
    work["pace_change_point_flag"] = (
        work["LapTime_Delta_past_slope"] > band_thr
    ).astype("int8")
    work["degradation_change_point_flag"] = (
        work["Cumulative_Degradation_past_slope"]
        > work.groupby(band_keys, sort=False, observed=True)[
            "Cumulative_Degradation_past_slope"
        ].transform("median")
        + 1.5
        * work.groupby(band_keys, sort=False, observed=True)[
    ...
    tyre = work["TyreLife"].astype("float32")
    pos = work["Position"].astype("float32")
    work["tyre_cliff_pressure"] = (
        (tyre * work["recent_max_deterioration"]).clip(0, 4000).astype("float32")
    )
    work["tyre_accel_pressure"] = (
        (tyre * work["LapTime_Delta_past_accel"].clip(lower=0))
        .clip(0, 4000)
        .astype("float32")
    )
    work["position_adjusted_cliff"] = (
        work["recent_max_deterioration"] / np.sqrt(pos.clip(lower=1))
    ).astype("float32")
    work["late_stint_change_flag"] = (
        (work["pace_change_point_flag"] == 1) & (tyre >= 8)
    ).astype("int8")

    for comp in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        mask = (work["Compound"] == comp).astype("int8")
        work[f"compound_{comp.lower()}_cliff"] = (
            mask * work["recent_max_deterioration"]
        ).astype("float32")
        work[f"compound_{comp.lower()}_tyrelife"] = (mask * tyre).astype("float32")

    work = (
        work.sort_values("_orig_order", kind="mergesort")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000187"],
        "research_usage_note": "Verified assigned hypothesis 000187.",
    }, sort_keys=True))
