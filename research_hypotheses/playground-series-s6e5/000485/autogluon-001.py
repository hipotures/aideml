...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_fresh = (out["TyreLife"] <= 3).astype("int8")
    is_late = (out["RaceProgress"] >= 0.70).astype("int8")
    is_testing = race.str.contains("Pre-Season", case=False, regex=False).astype("int8")

    stop_debt = (
        out["RaceProgress"].astype(float) * 3.0
        - out["Stint"].astype(float)
        + (out["TyreLife"].astype(float) / 25.0)
    )

    out["seg_dry_slick"] = is_slick
    out["seg_wet_phase"] = is_wet
    out["seg_fresh_tyre"] = is_fresh
    out["seg_late_stop_debt"] = ((is_late == 1) & (stop_debt > 0.75)).astype("int8")
    out["seg_testing"] = is_testing
    out["stop_debt_score"] = stop_debt.astype("float32")

    out["race_phase"] = np.select(
        [out["RaceProgress"] < 0.33, out["RaceProgress"] < 0.67],
        [0, 1],
        default=2,
    ).astype("int8")
    ...
    gates = {
        ...
        "wet": is_wet,
        "fresh": is_fresh,
        "late_debt": out["seg_late_stop_debt"],
        "testing": is_testing,
    }

    for col in num_cols:
        s = out[col].astype(float)
        out[f"{col}_global_rank"] = s.rank(pct=True).astype("float32")
        for name, gate in gates.items():
            masked = s.where(gate.astype(bool))
            out[f"{col}_{name}_rank"] = (
                masked.rank(pct=True).fillna(0).astype("float32")
            )
            out[f"{col}_{name}_value"] = s.mul(gate).astype("float32")

    out["compound_stint"] = compound + "_S" + out["Stint"].astype("string")
    out["race_compound"] = race + "_" + compound
    out["driver_race"] = out["Driver"].astype("string").fillna("UNKNOWN") + "_" + race
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000485"],
        "research_usage_note": "Verified assigned hypothesis 000485.",
    }, sort_keys=True))
