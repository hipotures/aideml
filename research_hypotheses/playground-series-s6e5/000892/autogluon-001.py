...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_total_laps = out.groupby(race_keys)["LapNumber"].transform("max")
    out["RaceTotalLaps"] = race_total_laps
    out["LapsRemaining"] = race_total_laps - out["LapNumber"]
    out["RaceProgressInverse"] = 1.0 - out["RaceProgress"]

    tyre_q = (
        out.groupby("Compound")["TyreLife"]
        .quantile([0.50, 0.75, 0.90])
        .unstack()
        .rename(
            columns={
                0.50: "CompoundTyreLifeQ50",
                0.75: "CompoundTyreLifeQ75",
                0.90: "CompoundTyreLifeQ90",
            }
        )
    )
    out = out.join(tyre_q, on="Compound")

    out["RemainingStintProxy"] = out["CompoundTyreLifeQ90"] - out["TyreLife"]
    out["TyreLifeOverQ50"] = out["TyreLife"] / (out["CompoundTyreLifeQ50"] + 1e-6)
    out["TyreLifeOverQ75"] = out["TyreLife"] / (out["CompoundTyreLifeQ75"] + 1e-6)
    out["TyreLifeOverQ90"] = out["TyreLife"] / (out["CompoundTyreLifeQ90"] + 1e-6)

    out["DegradationPerLap"] = out["Cumulative_Degradation"] / (out["TyreLife"] + 1e-6)
    out["DeltaPerLap"] = out["LapTime_Delta"] / (out["TyreLife"] + 1e-6)
    out["DegTimesTyreLife"] = out["Cumulative_Degradation"] * out["TyreLife"]
    out["DegTimesLapsRemaining"] = out["Cumulative_Degradation"] * (
        out["LapsRemaining"] + 1.0
    )
    out["TyreLifeToRaceRemaining"] = out["TyreLife"] / (out["LapsRemaining"] + 1.0)
    out["StintToRaceRemaining"] = out["Stint"] / (out["LapsRemaining"] + 1.0)

    positive_delta = np.clip(out["LapTime_Delta"], 0, None)
    out["PositiveDelta"] = positive_delta
    out["PressureScore"] = (
        0.35 * out["TyreLifeOverQ75"]
        + 0.25 * out["TyreLifeOverQ90"]
        + 0.20 * out["DegradationPerLap"]
        + 0.20 * (positive_delta / (np.abs(out["LapTime (s)"]) + 1e-6))
    )

    for h in (1, 2, 3, 5):
        out[f"HazardWindow_{h}"] = (out["RemainingStintProxy"] <= h).astype("int8")
        out[f"RemainingStintOverH_{h}"] = out["RemainingStintProxy"] / h
        out[f"RaceAndStintWindow_{h}"] = (
            (out["RemainingStintProxy"] <= h) & (out["LapsRemaining"] >= h)
        ).astype("int8")

    work = out.sort_values(driver_keys + ["LapNumber"]).copy()
    g = work.groupby(driver_keys, sort=False)

    work["PrevLapTimeDelta"] = g["LapTime_Delta"].shift(1)
    work["PrevPositionChange"] = g["Position_Change"].shift(1)
    work["PrevPitStop"] = g["PitStop"].shift(1)

    for w in (2, 3, 5):
        work[f"RollingDeltaMean_{w}"] = g["LapTime_Delta"].transform(
            lambda s: s.shift(1).rolling(w, min_periods=1).mean()
        )
        work[f"RollingDeltaMax_{w}"] = g["LapTime_Delta"].transform(
            lambda s: s.shift(1).rolling(w, min_periods=1).max()
        )
        work[f"RollingPosChangeMean_{w}"] = g["Position_Change"].transform(
            lambda s: s.shift(1).rolling(w, min_periods=1).mean()
        )

    work["LapsSincePitProxy"] = g["PitStop"].cumsum() - work["PitStop"]
    work["LapsSincePitProxy"] = (
        work.groupby(driver_keys + ["LapsSincePitProxy"], sort=False).cumcount() + 1
    )
    ...
    out = out.join(
        work[
            [
                ...
                "RollingDeltaMean_2",
                "RollingDeltaMax_2",
                "RollingPosChangeMean_2",
                "RollingDeltaMean_3",
                "RollingDeltaMax_3",
                "RollingPosChangeMean_3",
                "RollingDeltaMean_5",
                "RollingDeltaMax_5",
                "RollingPosChangeMean_5",
                "LapsSincePitProxy",
            ]
        ]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000892"],
        "research_usage_note": "Verified assigned hypothesis 000892.",
    }, sort_keys=True))
