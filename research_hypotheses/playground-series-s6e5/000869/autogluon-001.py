...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_keys = ["Year", "Race"]
    race_max_lap = X.groupby(race_keys)["LapNumber"].transform("max").astype(float)
    X["RaceTotalLaps"] = race_max_lap
    X["LapsRemaining"] = (race_max_lap - X["LapNumber"]).clip(lower=0)

    tyre_life = X["TyreLife"].astype(float)
    lap_num = X["LapNumber"].astype(float)
    stint = X["Stint"].astype(float)
    race_progress = X["RaceProgress"].astype(float).clip(1e-6, 1.0)

    X["TyreLifeFrac"] = tyre_life / (race_max_lap + 1e-6)
    X["LapsRemainingFrac"] = X["LapsRemaining"] / (race_max_lap + 1e-6)
    X["TyreLife_to_LapsRemaining"] = tyre_life / (X["LapsRemaining"] + 1.0)
    X["TyreLife_x_RaceProgress"] = tyre_life * race_progress
    X["TyreLife_x_Stint"] = tyre_life * stint
    X["StintProgressProxy"] = tyre_life / (lap_num + 1.0)

    lap_time = X["LapTime (s)"].astype(float)
    lap_delta = X["LapTime_Delta"].astype(float)
    degr = X["Cumulative_Degradation"].astype(float)
    pos = X["Position"].astype(float)
    pos_chg = X["Position_Change"].astype(float)

    X["DegPerTyreLap"] = degr / (tyre_life + 1.0)
    X["DegPerRaceLap"] = degr / (lap_num + 1.0)
    X["LapDeltaPerTyreLap"] = lap_delta / (tyre_life + 1.0)
    X["LapTime_x_TyreLife"] = lap_time * tyre_life
    X["LapDelta_x_LapsRemaining"] = lap_delta * X["LapsRemaining"]
    X["PositionPressure"] = pos * tyre_life
    X["TrackPositionVolatility"] = pos_chg * tyre_life

    X["LikelyCensoredFinalStint"] = (
        (X["LapsRemaining"] <= 3) | (race_progress >= 0.97)
    ).astype("int8")
    X["EndgamePitWindow"] = (
        (X["LapsRemaining"] >= 1) & (X["LapsRemaining"] <= 5)
    ).astype("int8")
    X["MidRacePitWindow"] = ((race_progress >= 0.25) & (race_progress <= 0.85)).astype(
        "int8"
    )
    ...
    X["DryTyre"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["WetTyre"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    X["SoftRisk"] = X["Compound"].eq("SOFT").astype(float) * tyre_life
    X["MediumRisk"] = X["Compound"].eq("MEDIUM").astype(float) * tyre_life
    X["HardRisk"] = X["Compound"].eq("HARD").astype(float) * tyre_life
    X["WetRisk"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype(float) * tyre_life

    grp_driver_race = ["Year", "Race", "Driver"]
    grp_driver_race_stint = ["Year", "Race", "Driver", "Stint"]

    X["DriverRace_LapTimeMean"] = X.groupby(grp_driver_race)["LapTime (s)"].transform(
        "mean"
    )
    X["DriverRace_LapDeltaMean"] = X.groupby(grp_driver_race)[
        "LapTime_Delta"
    ].transform("mean")
    X["DriverRaceStint_TyreLifeMax"] = X.groupby(grp_driver_race_stint)[
        "TyreLife"
    ].transform("max")
    X["DriverRaceStint_LapDeltaMean"] = X.groupby(grp_driver_race_stint)[
        "LapTime_Delta"
    ].transform("mean")

    X["LapTime_vs_DriverRaceMean"] = lap_time - X["DriverRace_LapTimeMean"]
    X["LapDelta_vs_DriverRaceMean"] = lap_delta - X["DriverRace_LapDeltaMean"]
    X["TyreLife_vs_StintMax"] = tyre_life / (X["DriverRaceStint_TyreLifeMax"] + 1.0)
    X["LapDelta_vs_StintMean"] = lap_delta - X["DriverRaceStint_LapDeltaMean"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000869.",
    }, sort_keys=True))
