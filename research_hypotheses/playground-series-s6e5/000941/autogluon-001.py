...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize core categoricals for downstream native categorical handling.
    cat_cols = ["Driver", "Race", "Compound"]
    for c in cat_cols:
        X[c] = X[c].astype("string").fillna("UNK")

    # Estimated race length and laps remaining from current race state only.
    rp = X["RaceProgress"].clip(lower=eps, upper=1.0)
    est_total_laps = (X["LapNumber"] / rp).clip(lower=X["LapNumber"], upper=120)
    laps_remaining = (est_total_laps - X["LapNumber"]).clip(lower=0)

    X["EstimatedTotalLaps"] = est_total_laps.astype("float32")
    X["LapsRemaining"] = laps_remaining.astype("float32")
    X["LapFracOfRace"] = (X["LapNumber"] / (est_total_laps + eps)).astype("float32")
    X["TyreLifeFrac"] = (X["TyreLife"] / (est_total_laps + eps)).astype("float32")
    X["StintFrac"] = (X["TyreLife"] / (X["LapNumber"] + eps)).astype("float32")

    # Monotone-friendly wear / degradation / pace features.
    X["DegradationPerLap"] = (
        X["Cumulative_Degradation"] / (X["TyreLife"] + eps)
    ).astype("float32")
    X["DegradationPerRaceLap"] = (
        X["Cumulative_Degradation"] / (X["LapNumber"] + eps)
    ).astype("float32")
    X["LapDeltaPerTyreLap"] = (X["LapTime_Delta"] / (X["TyreLife"] + eps)).astype(
        "float32"
    )
    X["LapTimePerRaceLap"] = (X["LapTime (s)"] / (X["LapNumber"] + eps)).astype(
        "float32"
    )
    X["LapTimeVsTyreLife"] = (X["LapTime (s)"] / (X["TyreLife"] + 1.0)).astype(
        "float32"
    )

    # Nonlinear but deterministic numeric transforms.
    X["TyreLifeSq"] = (X["TyreLife"] ** 2).astype("float32")
    X["LapsRemainingSq"] = (laps_remaining**2).astype("float32")
    X["AbsLapTimeDelta"] = X["LapTime_Delta"].abs().astype("float32")
    X["AbsPositionChange"] = X["Position_Change"].abs().astype("float32")
    X["FieldPressure"] = ((21 - X["Position"]) / 20.0).astype("float32")
    X["TrackPositionRisk"] = (
        (X["Position"] <= 5).astype("int8") * X["TyreLife"]
    ).astype("float32")

    # Race-state interaction categories for high-cardinality categorical learners.
    X["Driver_Race"] = (X["Driver"] + "__" + X["Race"]).astype("category")
    X["Driver_Compound"] = (X["Driver"] + "__" + X["Compound"]).astype("category")
    X["Race_Compound"] = (X["Race"] + "__" + X["Compound"]).astype("category")
    X["Year_Race"] = (X["Year"].astype("string") + "__" + X["Race"]).astype("category")
    X["Stint_Compound"] = (X["Stint"].astype("string") + "__" + X["Compound"]).astype(
        "category"
    )

    # Stable frequency encodings computed from the full provided table only.
    for c in [
        "Driver",
        "Race",
        "Compound",
        ...
        X[f"{c}_Freq"] = X[c].astype("string").map(vc).astype("float32")

    # Compact bucketed state features for tree diversity.
    X["TyreLifeBucket"] = pd.cut(
        X["TyreLife"],
        bins=[0, 7, 15, 25, 40, 80],
        labels=["vlow", "low", "mid", "high", "vhigh"],
        include_lowest=True,
    ).astype("category")
    X["LapsRemainingBucket"] = pd.cut(
        laps_remaining,
        bins=[-0.1, 3, 8, 15, 30, 200],
        labels=["imminent", "short", "mid", "long", "vlong"],
        include_lowest=True,
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000941.",
    }, sort_keys=True))
