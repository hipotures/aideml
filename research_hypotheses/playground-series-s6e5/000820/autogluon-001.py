...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    total_laps = X.groupby(race_keys)["LapNumber"].transform("max").clip(lower=1)
    laps_remaining = (total_laps - X["LapNumber"]).clip(lower=0)
    ...
    expected_life = X["Compound"].map(compound_life).fillna(22.0)
    compound_speed_score = X["Compound"].map(compound_speed).fillna(0.85)

    tyre_life = X["TyreLife"].astype(float).clip(lower=0)
    degradation = X["Cumulative_Degradation"].astype(float)
    lap_delta = X["LapTime_Delta"].astype(float)
    lap_time = X["LapTime (s)"].astype(float)

    wear_ratio = tyre_life / expected_life
    deg_per_lap = degradation / tyre_life.clip(lower=1.0)
    deg_pressure = degradation / (expected_life * 8.0)
    finish_pressure = 1.0 / (laps_remaining + 1.0)
    track_value = (21.0 - X["Position"].astype(float).clip(1, 20)) / 20.0
    recent_stop = X["PitStop"].astype(float)

    pit_rows = X["PitStop"].eq(1)
    pit_loss_source = lap_delta.where(pit_rows).clip(lower=0)
    global_pit_loss = (
        float(pit_loss_source.median())
        if pit_rows.any()
        else float(lap_delta.clip(lower=0).median())
    )
    if not np.isfinite(global_pit_loss) or global_pit_loss <= 0:
        global_pit_loss = float(np.nanmedian(np.clip(lap_delta.to_numpy(), 0, None)))
    if not np.isfinite(global_pit_loss) or global_pit_loss <= 0:
        global_pit_loss = 20.0

    race_pit_loss = pit_loss_source.groupby([X[k] for k in race_keys]).transform(
        "median"
    )
    race_pit_loss = race_pit_loss.fillna(global_pit_loss).clip(lower=1.0)

    race_base_lap = (
        lap_time.groupby([X[k] for k in race_keys]).transform("median").clip(lower=1.0)
    )
    pit_loss_norm = race_pit_loss / race_base_lap

    sort_cols = ["Year", "Race", "LapNumber", "Position", "Driver"]
    sorted_idx = X.sort_values(sort_cols, kind="mergesort").index
    S = X.loc[
        sorted_idx, ["Year", "Race", "LapNumber", "Position", "LapTime (s)"]
    ].copy()
    grp_change = (
        S["Year"].ne(S["Year"].shift())
        | S["Race"].ne(S["Race"].shift())
        | S["LapNumber"].ne(S["LapNumber"].shift())
    )

    prev_gap = (S["LapTime (s)"] - S["LapTime (s)"].shift()).where(~grp_change)
    next_gap = (S["LapTime (s)"].shift(-1) - S["LapTime (s)"]).where(
        ~grp_change.shift(-1).fillna(True)
    )
    nearest_gap = pd.concat([prev_gap.abs(), next_gap.abs()], axis=1).min(axis=1)
    nearest_gap = nearest_gap.reindex(X.index).fillna(nearest_gap.median())
    if nearest_gap.isna().any():
        nearest_gap = nearest_gap.fillna(5.0)
    rival_close = np.exp(-nearest_gap.clip(lower=0) / 2.5)

    lap_progress_left = laps_remaining / total_laps
    overdue = (wear_ratio - 1.0).clip(lower=0)
    degradation_risk = deg_per_lap / (race_base_lap / 100.0)
    degradation_risk = degradation_risk.replace([np.inf, -np.inf], 0).fillna(0)

    pit_now_value = (
        1.55 * wear_ratio
        + 1.00 * deg_pressure
        + 0.60 * degradation_risk
        + 0.45 * lap_progress_left
        + 0.35 * rival_close
        + 0.25 * track_value
        + 0.80 * overdue
        - 1.10 * pit_loss_norm
    ...
    pit_next_value = (
        1.35 * wear_ratio
        + 0.90 * deg_pressure
        + 0.50 * degradation_risk
        + 0.40 * lap_progress_left
        + 0.25 * rival_close
        + 0.20 * track_value
        + 0.55 * overdue
        - 0.95 * pit_loss_norm
        - 0.30 * compound_speed_score
        - 1.60 * recent_stop
    )

    go_long_value = (
        1.35 * finish_pressure
        + 0.70 * track_value
        + 0.75 * pit_loss_norm
        + 0.20 * compound_speed_score
        - 1.10 * wear_ratio
        - 0.95 * deg_pressure
        - 0.45 * degradation_risk
        - 0.20 * rival_close
        + 0.90 * recent_stop
    )

    values = np.vstack([pit_now_value, pit_next_value, go_long_value]).T
    best_action_idx = values.argmax(axis=1)
    sorted_values = np.sort(values, axis=1)
    best_margin = sorted_values[:, -1] - sorted_values[:, -2]

    X["Policy_Laps_Remaining"] = laps_remaining
    X["Policy_PitLossPrior"] = race_pit_loss
    X["Policy_RivalGapSec"] = nearest_gap
    X["Policy_RivalClose"] = rival_close
    X["Policy_WearRatio"] = wear_ratio
    X["Policy_DegRisk"] = degradation_risk
    X["Policy_PitNowValue"] = pit_now_value
    X["Policy_PitNextValue"] = pit_next_value
    X["Policy_GoLongValue"] = go_long_value
    X["Policy_NowMinusNext"] = pit_now_value - pit_next_value
    X["Policy_NowMinusLong"] = pit_now_value - go_long_value
    X["Policy_NextMinusLong"] = pit_next_value - go_long_value
    X["Policy_BestMargin"] = best_margin
    X["Policy_BestAction"] = pd.Categorical.from_codes(
        best_action_idx,
        categories=["pit_now", "pit_next_lap", "go_long"],
    )
    X["Policy_IsPitNowBest"] = (best_action_idx == 0).astype("int8")
    X["Policy_IsPitNextBest"] = (best_action_idx == 1).astype("int8")
    X["Policy_IsGoLongBest"] = (best_action_idx == 2).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000820"],
        "research_usage_note": "Verified assigned hypothesis 000820.",
    }, sort_keys=True))
