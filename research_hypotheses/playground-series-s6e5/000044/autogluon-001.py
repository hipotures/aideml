...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work["_finish_pressure"] = 1.0 - work["RaceProgress"]

    peer_cols = {
        "TyreLife": "tyre_life",
        "LapTime_Delta": "lap_delta",
        "Cumulative_Degradation": "degradation",
        "_finish_pressure": "finish_pressure",
    }

    grouped = work.groupby(grp_cols, sort=False)
    for col, name in peer_cols.items():
        ahead = grouped[col].shift(1)
        behind = grouped[col].shift(-1)

        work[f"ahead_{name}"] = ahead
        work[f"behind_{name}"] = behind
        work[f"delta_ahead_{name}"] = work[col] - ahead
        work[f"delta_behind_{name}"] = work[col] - behind

    work["has_car_ahead"] = grouped["Position"].shift(1).notna().astype("int8")
    work["has_car_behind"] = grouped["Position"].shift(-1).notna().astype("int8")

    work = work.sort_index()
    new_cols = [
        c for c in work.columns if c not in out.columns and c != "_finish_pressure"
    ]
    out[new_cols] = work[new_cols]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000044"],
        "research_usage_note": "Verified assigned hypothesis 000044.",
    }, sort_keys=True))
