...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre = out["TyreLife"].astype(float).clip(lower=0.0)
    ...
    out["aux_laps_remaining_est"] = laps_left.astype("float32")
    out["aux_tyre_age_ratio"] = (tyre / est_total.clip(1.0)).astype("float32")
    out["aux_deg_per_tyre_lap"] = (deg / tyre.clip(1.0)).replace([np.inf, -np.inf], 0).fillna(0).astype("float32")
    out["aux_delta_pressure"] = np.tanh((delta / 5.0).clip(-10, 10)).astype("float32")

    for horizon in (1, 2, 3, 5):
        h_age = tyre + horizon
        h_progress = ((lap + horizon) / est_total).clip(0, 1.1)
        score = (
            0.09 * h_age
            + 1.10 * h_progress
            + 0.018 * out["aux_deg_per_tyre_lap"].astype(float)
            + 0.45 * out["aux_delta_pressure"].astype(float)
            - 2.4
        )
        out[f"aux_stop_within_{horizon}_proxy"] = (1.0 / (1.0 + np.exp(-score.clip(-20, 20)))).astype("float32")
        out[f"aux_laps_left_after_{horizon}"] = (laps_left - horizon).clip(lower=0).astype("float32")

    out["aux_horizon_rank_proxy"] = out[["aux_stop_within_1_proxy", "aux_stop_within_2_proxy", "aux_stop_within_3_proxy", "aux_stop_within_5_proxy"]].mean(axis=1).astype("float32")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000222"],
        "research_usage_note": "Verified assigned hypothesis 000222.",
    }, sort_keys=True))
