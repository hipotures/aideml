...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["session"] = out["Year"].astype("string") + "_" + out["Race"]
    out["driver_session"] = out["session"] + "_" + out["Driver"]
    out["driver_stint"] = out["driver_session"] + "_" + out["Stint"].astype("string")

    out["lap_frac"] = out["LapNumber"] / out.groupby("session")["LapNumber"].transform(
        "max"
    ).clip(lower=1)
    out["laps_remaining_frac"] = 1.0 - out["RaceProgress"]
    out["stint_progress"] = out["TyreLife"] / out.groupby("driver_stint")[
        "TyreLife"
    ...
    out["pit_window_mid"] = (
        (out["RaceProgress"] > 0.22) & (out["RaceProgress"] < 0.78)
    ).astype("int8")
    out["pit_window_late"] = (out["RaceProgress"] > 0.72).astype("int8")

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber", "_row_order"]
    s = out.sort_values(sort_cols)
    g = s.groupby("driver_stint", sort=False)
    ...
    for col in [
        ...
        s[f"prev_{col}"] = g[col].shift(1)
        s[f"prev2_{col}"] = g[col].shift(2)
    ...
    for col in [
        ...
        prev = g[col].shift(1)
        s[f"hist_mean_{col}"] = (
            prev.groupby(s["driver_stint"], sort=False)
            .expanding()
            .mean()
        ...
        s[f"delta_vs_hist_{col}"] = s[col] - s[f"hist_mean_{col}"]

    s["laps_since_prev_pit"] = g["PitStop"].shift(1).fillna(0)
    s["laps_since_prev_pit"] = s.groupby("driver_stint", sort=False)[
        "laps_since_prev_pit"
    ].transform(lambda x: x.eq(1).cumsum())
    s["prev_lap_time_jump"] = s["prev_LapTime (s)"] - s["prev2_LapTime (s)"]
    s["prev_degradation_jump"] = (
        s["prev_Cumulative_Degradation"] - s["prev2_Cumulative_Degradation"]
    )
    s["prev_position_loss"] = (s["prev_Position_Change"] > 0).astype("int8")

    out = s.sort_values("_row_order").drop(columns=["_row_order"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000738"],
        "research_usage_note": "Verified assigned hypothesis 000738.",
    }, sort_keys=True))
