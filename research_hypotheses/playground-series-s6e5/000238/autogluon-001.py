...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["track_energy_bin"] = race.map(energy_map).fillna(1).astype("int8")
    severity = out["track_energy_bin"].map({0: 0.82, 1: 1.0, 2: 1.18}).astype(float)

    base_life = compound.map(compound_life).fillna(25.0).astype(float)
    expected_life = base_life / severity
    ...
    est_total_laps = (out["LapNumber"].astype(float) / progress).clip(1, 120)
    laps_remaining = (est_total_laps - out["LapNumber"].astype(float)).clip(0, 120)

    out["severity_expected_stint_life"] = expected_life
    out["severity_life_ratio"] = tyre_life / (expected_life + 1e-6)
    out["severity_excess_wear"] = tyre_life - expected_life
    out["severity_excess_wear_pct"] = (tyre_life - expected_life) / (
        expected_life + 1e-6
    )
    out["severity_degradation_pressure"] = degr * severity
    out["severity_degradation_per_lap"] = degr / tyre_life.clip(lower=1)
    out["severity_adjusted_degradation_per_lap"] = (
        out["severity_degradation_per_lap"] * severity
    )
    out["severity_laps_remaining"] = laps_remaining * severity
    out["severity_late_stop_margin"] = laps_remaining - (expected_life - tyre_life)
    out["severity_can_reach_finish_margin"] = expected_life - tyre_life - laps_remaining
    out["compound_energy_key"] = compound + "_E" + out["track_energy_bin"].astype(str)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000238.",
    }, sort_keys=True))
