...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    max_lap = out.groupby(group)["LapNumber"].transform("max").astype(float)
    min_lap = out.groupby(group)["LapNumber"].transform("min").astype(float)

    out["RaceLap_Frac"] = (lap - min_lap) / (max_lap - min_lap + 1.0)
    out["Laps_To_Race_End"] = max_lap - lap
    out["TyreLife_RaceFrac"] = tyre / (max_lap + 1.0)
    out["TyreLife_Progress_Ratio"] = tyre / (progress * max_lap + 1.0)
    out["Stint_Lap_Ratio"] = tyre / (stint + 1.0)

    gap = 2.0
    out["Race_Start_Gap"] = (lap <= min_lap + gap).astype("int8")
    out["Race_End_Gap"] = (lap >= max_lap - gap).astype("int8")
    out["Race_Boundary_Gap"] = (
        (out["Race_Start_Gap"] == 1) | (out["Race_End_Gap"] == 1)
    ).astype("int8")

    out["Degradation_per_TyreLife"] = out["Cumulative_Degradation"].astype(float) / (
        tyre + 1.0
    )
    out["LapTime_per_Progress"] = out["LapTime (s)"].astype(float) / progress
    out["Abs_LapTime_Delta"] = out["LapTime_Delta"].astype(float).abs()
    out["Abs_Position_Change"] = out["Position_Change"].astype(float).abs()

    for c in ["Compound", "Driver", "Race", "Year_Race"]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000897"],
        "research_usage_note": "Verified assigned hypothesis 000897.",
    }, sort_keys=True))
