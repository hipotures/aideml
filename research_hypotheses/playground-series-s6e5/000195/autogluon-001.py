...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    max_lap_by_race_year = (
        out.groupby(["Race", "Year"])["LapNumber"].transform("max").astype(float)
    )
    laps_remaining = (max_lap_by_race_year - out["LapNumber"].astype(float)).clip(
        lower=0
    )
    out["laps_remaining_est"] = laps_remaining
    out["race_lap_frac_est"] = out["LapNumber"].astype(
        float
    ) / max_lap_by_race_year.replace(0, np.nan)

    dry_compound = ~compound.isin(["INTERMEDIATE", "WET"])
    out["reg_wet_exemption"] = (~dry_compound).astype("int8")
    out["reg_mandatory_dry_change"] = dry_compound.astype("int8")

    monaco = race.str.contains("Monaco", case=False, na=False)
    out["reg_monaco_2025_special"] = (monaco & (year == 2025)).astype("int8")
    out["reg_compound_rule_pressure"] = (
        out["reg_mandatory_dry_change"] + out["reg_monaco_2025_special"]
    )
    ...
    out["circuit_energy_class"] = race.map(energy_map).fillna(2).astype("int8")
    out["expected_stop_tendency"] = race.map(stop_map).fillna(2).astype("int8")
    out["expected_two_stop_prior"] = (out["expected_stop_tendency"] >= 2).astype("int8")

    tyre_life = out["TyreLife"].astype(float)
    stint = out["Stint"].astype(float)
    progress = out["RaceProgress"].astype(float)

    out["energy_x_tyre_life"] = out["circuit_energy_class"].astype(float) * tyre_life
    out["energy_x_laps_remaining"] = (
        out["circuit_energy_class"].astype(float) * laps_remaining
    )
    out["stop_prior_x_stint"] = out["expected_stop_tendency"].astype(float) * stint
    out["rule_pressure_x_tyre_life"] = (
        out["reg_compound_rule_pressure"].astype(float) * tyre_life
    )
    out["rule_pressure_x_laps_remaining"] = (
        out["reg_compound_rule_pressure"].astype(float) * laps_remaining
    )
    out["monaco2025_x_stint"] = out["reg_monaco_2025_special"].astype(float) * stint
    out["wet_exempt_x_tyre_life"] = out["reg_wet_exemption"].astype(float) * tyre_life
    out["late_race_rule_pressure"] = (
        out["reg_compound_rule_pressure"].astype(float) * progress
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000195"],
        "research_usage_note": "Verified assigned hypothesis 000195.",
    }, sort_keys=True))
