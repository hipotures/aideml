...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound_norm = X["Compound"].astype(str).str.upper().str.strip()

    is_monaco = race_norm.str.contains("monaco", na=False)
    is_wet = compound_norm.isin(["WET", "INTERMEDIATE"])
    is_monaco_2025 = is_monaco & (X["Year"] == 2025)

    lap_num = pd.to_numeric(X["LapNumber"], errors="coerce").astype(float)
    race_prog = (
        pd.to_numeric(X["RaceProgress"], errors="coerce").astype(float).clip(0.01, 1.0)
    )
    tyre_life = pd.to_numeric(X["TyreLife"], errors="coerce").astype(float)
    stint = pd.to_numeric(X["Stint"], errors="coerce").astype(float)
    pit_now = (
        pd.to_numeric(X["PitStop"], errors="coerce").fillna(0).clip(0, 1).astype(float)
    )

    est_total_laps = (lap_num / race_prog).replace([np.inf, -np.inf], np.nan)
    est_total_laps = est_total_laps.fillna(lap_num).clip(lower=lap_num, upper=100)
    laps_remaining = (est_total_laps - lap_num).clip(lower=0)

    sort_cols = ["Year", "Race", "Driver", "LapNumber", "_row_id"]
    XS = (
        X.assign(_race_norm=race_norm, _compound_norm=compound_norm)
    ...
    XS["pit_prev_1"] = (
        XS.groupby(grp)["PitStop"].shift(1).fillna(0).clip(0, 1).astype(float)
    )
    XS["pit_prev_2"] = (
        XS.groupby(grp)["PitStop"].shift(2).fillna(0).clip(0, 1).astype(float)
    )
    XS["observed_stops_incl_now"] = (
        XS.groupby(grp)["PitStop"].cumsum().clip(lower=0).astype(float)
    )
    XS["observed_stops_before_now"] = (
        XS["observed_stops_incl_now"] - XS["PitStop"].clip(0, 1)
    ).clip(lower=0)
    XS["stint_prev"] = XS.groupby(grp)["Stint"].shift(1)
    XS["stint_jump"] = (XS["Stint"] - XS["stint_prev"]).fillna(0)
    XS["fresh_stint_flag"] = (XS["stint_jump"] > 0).astype(int)

    XS = XS.sort_values("_row_id")
    X["pit_prev_1"] = XS["pit_prev_1"].to_numpy()
    X["pit_prev_2"] = XS["pit_prev_2"].to_numpy()
    X["observed_stops_incl_now"] = XS["observed_stops_incl_now"].to_numpy()
    X["observed_stops_before_now"] = XS["observed_stops_before_now"].to_numpy()
    X["fresh_stint_flag"] = XS["fresh_stint_flag"].to_numpy()

    dry_monaco_two_stop_required = (is_monaco_2025 & (~is_wet)).astype(int)
    required_stops = np.where(dry_monaco_two_stop_required == 1, 2.0, 1.0)
    stop_debt_now = np.maximum(required_stops - X["observed_stops_incl_now"], 0.0)
    stop_debt_before = np.maximum(required_stops - X["observed_stops_before_now"], 0.0)

    feasible_extra_stop = (laps_remaining >= 1.0).astype(float)
    feasible_debt = (laps_remaining >= stop_debt_now).astype(float)
    mid_late_phase = ((race_prog >= 0.20) & (race_prog <= 0.98)).astype(float)

    X["monaco_two_stop_rule_active"] = dry_monaco_two_stop_required
    X["monaco_two_stop_debt_now"] = np.where(
        dry_monaco_two_stop_required == 1,
        stop_debt_now * feasible_debt * mid_late_phase,
        0.0,
    )
    X["monaco_two_stop_debt_before"] = np.where(
        dry_monaco_two_stop_required == 1,
        stop_debt_before * feasible_extra_stop * mid_late_phase,
        0.0,
    )
    X["monaco_two_stop_urgent"] = (
        (X["monaco_two_stop_debt_now"] > 0)
        & (laps_remaining <= np.maximum(3.0, 2.0 * X["monaco_two_stop_debt_now"]))
    ).astype(int)

    X["dry_tyre_life_pressure"] = np.where(~is_wet, tyre_life, 0.0)
    X["degradation_per_lap"] = X["Cumulative_Degradation"] / lap_num.clip(lower=1)
    X["tyre_life_x_phase"] = tyre_life * race_prog
    X["stint_x_phase"] = stint * race_prog
    X["position_loss_pressure"] = (
        -pd.to_numeric(X["Position_Change"], errors="coerce").fillna(0)
    ).clip(lower=0)
    X["pit_recent_any"] = (
        (X["pit_prev_1"] > 0) | (X["pit_prev_2"] > 0) | (pit_now > 0)
    ).astype(int)
    X["pit_cycle_signal"] = X["fresh_stint_flag"] + X["pit_prev_1"] + pit_now
    X["remaining_laps_est"] = laps_remaining
    X["late_race_flag"] = (race_prog >= 0.75).astype(int)
    X["early_race_flag"] = (race_prog <= 0.25).astype(int)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000022.",
    }, sort_keys=True))
