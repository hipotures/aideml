...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "_orig_order_"]
    out = out.sort_values(sort_cols, kind="mergesort")

    group_keys = ["Year", "Race", "Driver"]
    slick_compounds = ["HARD", "MEDIUM", "SOFT"]

    is_slick = out["Compound"].isin(slick_compounds).astype("int8")
    out["is_slick_compound"] = is_slick

    slick_dummies = pd.get_dummies(out["Compound"])
    slick_dummies = slick_dummies.reindex(columns=slick_compounds, fill_value=0).astype(
        "int8"
    )

    seen_slick = (
        slick_dummies.groupby([out[k] for k in group_keys], sort=False)
        .cummax()
        .astype("int8")
    )
    seen_counts = slick_dummies.groupby(
        [out[k] for k in group_keys], sort=False
    ).cumsum()

    out["current_counted_slick_diversity"] = seen_slick.sum(axis=1).astype("int8")
    out["remaining_dry_compound_debt"] = (
        (2 - out["current_counted_slick_diversity"]).clip(lower=0).astype("int8")
    )

    current_is_new = np.zeros(len(out), dtype=np.int8)
    for c in slick_compounds:
        current_is_new = np.where(
            (out["Compound"].values == c) & (seen_counts[c].values == 1),
            1,
            current_is_new,
        )
        out[f"seen_{c.lower()}_by_now"] = seen_slick[c].astype("int8")
    out["current_slick_compound_is_new"] = current_is_new.astype("int8")

    out["late_race_limited_diversity_flag"] = (
        (out["RaceProgress"] >= 0.7) & (out["current_counted_slick_diversity"] < 2)
    ).astype("int8")
    out["very_late_dry_compound_debt_flag"] = (
        (out["RaceProgress"] >= 0.85) & (out["remaining_dry_compound_debt"] > 0)
    ).astype("int8")
    out["late_race_single_slick_seen_flag"] = (
        (out["RaceProgress"] >= 0.85) & (out["current_counted_slick_diversity"] <= 1)
    ).astype("int8")

    out = out.sort_values("_orig_order_", kind="mergesort").drop(columns="_orig_order_")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000018.",
    }, sort_keys=True))
