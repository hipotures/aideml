...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    race_cols = ["Year", "Race"]
    lap_cols = ["Year", "Race", "LapNumber"]

    compound = out["Compound"].astype(str).str.upper()
    wet_row = compound.isin(["WET", "INTERMEDIATE"]).astype(np.int8)
    dry_softness = compound.map(
        {"HARD": 0.0, "MEDIUM": 1.0, "SOFT": 2.0, "INTERMEDIATE": -1.0, "WET": -2.0}
    ).fillna(0.0)

    tmp = pd.DataFrame(
        {
            "Year": out["Year"].values,
            "Race": out["Race"].values,
            "LapNumber": out["LapNumber"].values,
            "lap_time": out["LapTime (s)"].values,
            "lap_delta": out["LapTime_Delta"].values,
            "pos_change_abs": out["Position_Change"].abs().values,
            "pit": out["PitStop"].values,
            "wet": wet_row.values,
            "softness": dry_softness.values,
        }
    )

    lap = (
        tmp.groupby(lap_cols, sort=False, observed=True)
        .agg(
            lap_time_med=("lap_time", "median"),
            lap_delta_med=("lap_delta", "median"),
            pos_change_abs_med=("pos_change_abs", "median"),
            lap_pit_share=("pit", "mean"),
            rainfall=("wet", "mean"),
            compound_softness_lap=("softness", "mean"),
        )
        .reset_index()
    )

    lap = lap.sort_values(lap_cols, kind="mergesort").reset_index(drop=True)
    g = lap.groupby(race_cols, sort=False, observed=True)

    prior_n = g.cumcount().astype(float)
    lt = lap["lap_time_med"].astype(float)
    ld = lap["lap_delta_med"].astype(float)
    lt_sum = g["lap_time_med"].cumsum() - lt
    ld_sum = g["lap_delta_med"].cumsum() - ld
    lt_sq = lt * lt
    ld_sq = ld * ld
    lt_sq_sum = (
        lt_sq.groupby([lap["Year"], lap["Race"]], sort=False, observed=True).cumsum()
        - lt_sq
    )
    ld_sq_sum = (
        ld_sq.groupby([lap["Year"], lap["Race"]], sort=False, observed=True).cumsum()
        - ld_sq
    )

    prior_mean_lt = (lt_sum / prior_n.replace(0, np.nan)).fillna(lt)
    prior_mean_ld = (ld_sum / prior_n.replace(0, np.nan)).fillna(0.0)
    prior_var_lt = (
        (lt_sq_sum - (lt_sum * lt_sum / prior_n.replace(0, np.nan)))
        / (prior_n - 1).replace(0, np.nan)
    ).clip(lower=1.0)
    prior_var_ld = (
        (ld_sq_sum - (ld_sum * ld_sum / prior_n.replace(0, np.nan)))
        / (prior_n - 1).replace(0, np.nan)
    ).clip(lower=1.0)

    lap["track_slow_z"] = (
        (lt - prior_mean_lt) / np.sqrt(prior_var_lt.fillna(1.0))
    ).clip(-8, 8)
    lap["delta_slow_z"] = (
        (ld - prior_mean_ld) / np.sqrt(prior_var_ld.fillna(1.0))
    ).clip(-8, 8)
    lap["rainfall_prev"] = g["rainfall"].shift(1).fillna(0.0)
    lap["rainfall_change"] = (lap["rainfall"] - lap["rainfall_prev"]).clip(-1, 1)

    lap["is_SC"] = (
        (lap["track_slow_z"] > 2.5)
        | (lap["delta_slow_z"] > 2.25)
        | (lap["lap_pit_share"] > 0.45)
    ).astype(np.int8)
    lap["is_VSC"] = (
        (
            (lap["track_slow_z"] > 1.25)
            | (lap["delta_slow_z"] > 1.15)
            | (lap["pos_change_abs_med"] > 2.5)
        )
        & (lap["is_SC"] == 0)
    ).astype(np.int8)
    lap["track_status_code"] = lap["is_SC"] * 2 + lap["is_VSC"]

    prev_status = g["track_status_code"].shift(1).fillna(0)
    lap["status_changed"] = (lap["track_status_code"] != prev_status).astype(np.int8)
    status_block = (
        lap["status_changed"]
        .groupby([lap["Year"], lap["Race"]], sort=False, observed=True)
        .cumsum()
    )
    lap["laps_since_status_change"] = (
        lap.groupby([lap["Year"], lap["Race"], status_block], sort=False, observed=True)
        .cumcount()
        .astype(np.int16)
    )

    neutral_start = ((lap["track_status_code"] > 0) & (prev_status == 0)).astype(
        np.int8
    )
    lap["prior_neutralizations"] = (
        neutral_start.groupby(
            [lap["Year"], lap["Race"]], sort=False, observed=True
        ).cumsum()
        - neutral_start
    )

    prev_slow = g["track_slow_z"].shift(1).fillna(0.0)
    lap["status_change_next_1_2_laps"] = (
        (lap["track_status_code"] == 0)
        & ((lap["track_slow_z"] - prev_slow > 0.75) | (lap["rainfall_change"] > 0.15))
        & ((lap["track_slow_z"] > 0.8) | (lap["rainfall"] > 0.15))
    ).astype(np.int8)

    race_temp = {
        "Bahrain Grand Prix": 27,
        "Saudi Arabian Grand Prix": 29,
        "Australian Grand Prix": 21,
        "Japanese Grand Prix": 20,
        "Chinese Grand Prix": 23,
        "Miami Grand Prix": 28,
        "Emilia Romagna Grand Prix": 23,
        "Monaco Grand Prix": 24,
        "Canadian Grand Prix": 20,
        "Spanish Grand Prix": 27,
        "Austrian Grand Prix": 22,
        "British Grand Prix": 18,
        "Hungarian Grand Prix": 27,
        "Belgian Grand Prix": 18,
        "Dutch Grand Prix": 19,
        "Italian Grand Prix": 25,
        "Azerbaijan Grand Prix": 24,
        "Singapore Grand Prix": 30,
        "United States Grand Prix": 27,
        "Mexico City Grand Prix": 22,
        "Sao Paulo Grand Prix": 24,
        "Las Vegas Grand Prix": 16,
        "Qatar Grand Prix": 29,
        "Abu Dhabi Grand Prix": 28,
        "Pre-Season Testing": 24,
    }
    lap["air_temp"] = (
        lap["Race"].map(race_temp).fillna(23).astype(float) - 5.0 * lap["rainfall"]
    )
    lap["track_temp"] = (
        lap["air_temp"]
        + 9.0
        + 2.0 * lap["compound_softness_lap"].clip(-1, 2)
        - 8.0 * lap["rainfall"]
    )
    lap["cheap_stop_window"] = (
        (lap["track_status_code"] > 0)
        | (lap["laps_since_status_change"] <= 2)
        | (lap["rainfall_change"] > 0.10)
    ).astype(np.int8)
    ...
    add_cols = [
        ...
        "prior_neutralizations",
        "status_change_next_1_2_laps",
        "rainfall",
        "rainfall_change",
        "air_temp",
        "track_temp",
    ...
    out = out.merge(lap[lap_cols + add_cols], on=lap_cols, how="left", sort=False)

    out["wet_or_intermediate"] = wet_row
    for c in [
        "is_SC",
        "is_VSC",
        "cheap_stop_window",
        ...
        out[c + "_wet_interaction"] = out[c].astype(float) * out[
            "wet_or_intermediate"
        ].astype(float)

    out["tyrelife_in_status_window"] = out["TyreLife"].astype(float) * out[
        "cheap_stop_window"
    ].astype(float)
    out["stint_in_status_window"] = out["Stint"].astype(float) * out[
        "cheap_stop_window"
    ].astype(float)
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000184"],
        "research_usage_note": "Verified assigned hypothesis 000184.",
    }, sort_keys=True))
