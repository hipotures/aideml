...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Compound", "Driver", "Race"]:
        ...
        out[f"{col}_freq"] = freq.astype("float32")

    compound = out["Compound"].astype("string")
    softness = compound.map(
        {"HARD": 1.0, "MEDIUM": 2.0, "SOFT": 3.0, "INTERMEDIATE": 0.5, "WET": 0.0}
    ).fillna(1.5)
    out["compound_softness"] = softness.astype("float32")
    out["compound_is_wet"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["compound_is_dry"] = compound.isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")

    lap = out["LapNumber"].astype("float32")
    progress = out["RaceProgress"].astype("float32").clip(eps, 1.0)
    ...
    poschg = out["Position_Change"].astype("float32")

    est_total = (lap / progress).clip(1, 120)
    remaining = (est_total - lap).clip(0, 120)
    out["surv_est_total_laps"] = est_total.astype("float32")
    out["surv_laps_remaining"] = remaining.astype("float32")
    out["surv_progress_remaining"] = (1.0 - progress).astype("float32")
    out["surv_log_tyre_life"] = np.log1p(tyre).astype("float32")
    out["surv_log_laps_remaining"] = np.log1p(remaining).astype("float32")
    out["surv_tyre_frac_race"] = (tyre / (est_total + eps)).astype("float32")
    out["surv_tyre_frac_remaining"] = (tyre / (tyre + remaining + eps)).astype(
        "float32"
    )
    out["surv_censor_pressure"] = (remaining / (tyre + 1.0)).astype("float32")

    out["hazard_age_bin"] = np.searchsorted(
        [3, 6, 10, 14, 18, 24, 32, 45, 60], tyre
    ).astype("int8")
    out["hazard_progress_bin"] = np.searchsorted(
        [0.15, 0.30, 0.45, 0.60, 0.75, 0.88, 0.96], progress
    ).astype("int8")
    out["hazard_remaining_bin"] = np.searchsorted(
        [1, 2, 3, 5, 8, 12, 18, 25, 40], remaining
    ).astype("int8")
    out["hazard_stint_age"] = (stint * np.log1p(tyre)).astype("float32")
    out["hazard_age_x_softness"] = (tyre * softness.astype("float32")).astype("float32")
    out["hazard_age_x_progress"] = (tyre * progress).astype("float32")
    out["hazard_remaining_x_softness"] = (
        remaining * softness.astype("float32")
    ).astype("float32")

    out["deg_per_tyre_lap"] = (deg / (tyre + eps)).astype("float32")
    out["deg_per_race_lap"] = (deg / (lap + eps)).astype("float32")
    out["abs_laptime_delta"] = np.abs(ltd).astype("float32")
    out["pos_loss"] = np.maximum(poschg, 0).astype("float32")
    out["pos_gain"] = np.maximum(-poschg, 0).astype("float32")
    out["position_frac"] = (pos / 20.0).astype("float32")
    out["late_race"] = (progress >= 0.75).astype("int8")
    out["very_late_race"] = (progress >= 0.90).astype("int8")

    keys = ["Year", "Race", "Driver"]
    order = (
        out.assign(_row_id=np.arange(len(out)))
        .sort_values(keys + ["LapNumber", "_row_id"])
        .index
    )
    seq = out.loc[
        order,
    ...
    grp = seq.groupby(keys, sort=False)

    past_pits = grp["PitStop"].cumsum() - seq["PitStop"]
    out.loc[order, "past_pit_count"] = past_pits.astype("float32").to_numpy()

    for lag in [1, 2, 3]:
        out.loc[order, f"pit_lag_{lag}"] = (
            grp["PitStop"].shift(lag).fillna(0).astype("float32").to_numpy()
        )
    out["recent_pit_last3"] = (
        out["pit_lag_1"] + out["pit_lag_2"] + out["pit_lag_3"]
    ).astype("float32")
    ...
    for col in [
        ...
        base = col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        p1 = grp[col].shift(1)
        p2 = grp[col].shift(2)
        p3 = grp[col].shift(3)
        vals = pd.concat([p1, p2, p3], axis=1)
        out.loc[order, f"prev_{base}"] = p1.astype("float32").to_numpy()
        out.loc[order, f"{base}_diff_prev"] = (
            (seq[col] - p1).astype("float32").to_numpy()
        )
        out.loc[order, f"{base}_past3_mean"] = (
            vals.mean(axis=1).astype("float32").to_numpy()
        )
        out.loc[order, f"{base}_past3_std"] = (
            vals.std(axis=1).fillna(0).astype("float32").to_numpy()
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000402"],
        "research_usage_note": "Verified assigned hypothesis 000402.",
    }, sort_keys=True))
