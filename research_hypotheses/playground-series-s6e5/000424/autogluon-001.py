...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year = pd.to_numeric(out["Year"], errors="coerce").fillna(0).astype("int16")
    ...
    scheduled_laps = {
        ...
        "QATAR GRAND PRIX": 57,
        "SAO PAULO GRAND PRIX": 71,
        "SAUDI ARABIAN GRAND PRIX": 50,
        "SINGAPORE GRAND PRIX": 62,
        "SPANISH GRAND PRIX": 66,
        "UNITED STATES GRAND PRIX": 56,
    }
    total_laps = race_key.map(scheduled_laps).astype("float32")

    year_start_overrides = {
        (2022, "SINGAPORE GRAND PRIX"): 61,
        (2023, "ITALIAN GRAND PRIX"): 51,
        (2024, "SAO PAULO GRAND PRIX"): 69,
    }
    for (yy, rr), val in year_start_overrides.items():
        total_laps = total_laps.mask((year == yy) & (race_key == rr), float(val))

    causal_updates = {
        (2022, "MONACO GRAND PRIX"): ((61, 64),),
        (2022, "SINGAPORE GRAND PRIX"): ((58, 59),),
        (2022, "JAPANESE GRAND PRIX"): ((27, 28),),
    }
    for (yy, rr), updates in causal_updates.items():
        base_mask = (year == yy) & (race_key == rr)
        for first_known_lap, val in updates:
            total_laps = total_laps.mask(
                base_mask & (lap >= first_known_lap), float(val)
            )

    known_state = total_laps.notna()
    laps_remaining = (total_laps - lap).clip(lower=0)
    race_frac = (lap / total_laps).where(total_laps > 0).clip(lower=0, upper=1.25)
    next_frac = (
        ((lap + 1.0) / total_laps).where(total_laps > 0).clip(lower=0, upper=1.25)
    )

    out["FastF1_LapCountState_TotalLaps"] = total_laps.astype("float32")
    out["FastF1_LapCountState_Known"] = known_state.astype("int8")
    out["FastF1_LapsRemaining"] = laps_remaining.astype("float32")
    out["FastF1_RaceFrac"] = race_frac.astype("float32")
    out["FastF1_NextLapFrac"] = next_frac.astype("float32")
    out["FastF1_FinalLapWindow"] = (known_state & (laps_remaining <= 1)).astype("int8")
    out["FastF1_Last3LapWindow"] = (known_state & (laps_remaining <= 3)).astype("int8")
    out["FastF1_Last5LapWindow"] = (known_state & (laps_remaining <= 5)).astype("int8")
    out["FastF1_Last10PctWindow"] = (known_state & (race_frac >= 0.90)).astype("int8")

    tyre_life_limit = compound.map(
        {
            "SOFT": 24.0,
            "MEDIUM": 34.0,
            "HARD": 48.0,
    ...
    current_life_left = (tyre_life_limit - tyre_life).clip(lower=0)

    current_finish_margin = tyre_life_limit - tyre_life - laps_remaining
    new_tyre_finish_margin = tyre_life_limit - laps_remaining
    two_stint_finish_margin = current_life_left + tyre_life_limit - laps_remaining

    out["FastF1_CurrentTyreFinishMargin"] = current_finish_margin.astype("float32")
    out["FastF1_NewTyreFinishMargin"] = new_tyre_finish_margin.astype("float32")
    out["FastF1_TwoStintFinishMargin"] = two_stint_finish_margin.astype("float32")
    out["FastF1_CurrentTyreCanFinish"] = (
        known_state & (current_finish_margin >= 0)
    ).astype("int8")
    out["FastF1_NewTyreCanFinish"] = (
        known_state & (new_tyre_finish_margin >= 0)
    ).astype("int8")
    out["FastF1_TwoStintCanFinish"] = (
        known_state & (two_stint_finish_margin >= 0)
    ).astype("int8")

    tyre_need_after_current = (laps_remaining - current_life_left).clip(lower=0)
    tyre_stop_debt = np.ceil(
        tyre_need_after_current / tyre_life_limit.replace(0, np.nan)
    ).clip(0, 4)

    dry_compound = compound.isin(["SOFT", "MEDIUM", "HARD"])
    completed_stops = (
        pd.to_numeric(out["Stint"], errors="coerce").astype("float32") - 1.0
    ).clip(lower=0)
    monaco_two_stop = (year == 2025) & (race_key == "MONACO GRAND PRIX")
    required_stops = pd.Series(
        np.where(monaco_two_stop, 2.0, np.where(dry_compound, 1.0, 0.0)),
        index=out.index,
        dtype="float32",
    )
    mandatory_stop_debt = (required_stops - completed_stops).clip(lower=0, upper=2)
    total_stop_debt = pd.Series(
        np.fmax(
            tyre_stop_debt.to_numpy(dtype="float32"),
            mandatory_stop_debt.to_numpy(dtype="float32"),
        ),
        index=out.index,
    )

    out["FastF1_TyreStopDebt"] = tyre_stop_debt.astype("float32")
    out["FastF1_MandatoryStopDebt"] = mandatory_stop_debt.astype("float32")
    out["FastF1_TotalStopDebt"] = total_stop_debt.astype("float32")
    out["FastF1_StopDebtPressure"] = (total_stop_debt / (laps_remaining + 1.0)).astype(
        "float32"
    )
    out["FastF1_LastCallForStopDebt"] = (
        known_state & (total_stop_debt > 0) & (laps_remaining <= total_stop_debt)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000424"],
        "research_usage_note": "Verified assigned hypothesis 000424.",
    }, sort_keys=True))
