...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_age_scaled = (out["TyreLife"].astype("float32") * compound_scale).astype(
        "float32"
    )
    finish_pressure = (
        out["RaceProgress"].astype("float32")
        * np.log1p(out["TyreLife"].astype("float32"))
        * compound_scale
    ).astype("float32")

    lap_keys = ["Year", "Race", "LapNumber"]
    lap_count = out.groupby(lap_keys)["Position"].transform("count").astype("float32")
    lap_rank = (
        out.groupby(lap_keys)["Position"]
        .rank(method="average", ascending=True)
        .astype("float32")
    )
    pos_pct = (
        ((lap_rank - 1.0) / (lap_count - 1.0).replace(0, np.nan))
        .fillna(0.0)
        .astype("float32")
    )

    position = out["Position"].astype("float32")
    lost_pos = out["Position_Change"].astype("float32").clip(lower=0)
    top_flag = (position <= 5).astype("int8")
    mid_flag = ((position > 5) & (position <= 14)).astype("int8")
    back_flag = (position > 14).astype("int8")
    fresh_flag = (out["TyreLife"].astype("float32") <= 2).astype("int8")
    pit_now = out["PitStop"].astype("int8")

    out["h59_compound_tyre_age"] = tyre_age_scaled
    out["h59_finish_pressure"] = finish_pressure
    out["h59_position_pct"] = pos_pct
    out["h59_lost_position"] = lost_pos
    out["h59_top_flag"] = top_flag
    out["h59_mid_flag"] = mid_flag
    out["h59_back_flag"] = back_flag
    out["h59_fresh_tyre"] = fresh_flag

    out["h59_age_x_position"] = tyre_age_scaled * position
    out["h59_age_x_pos_pct"] = tyre_age_scaled * pos_pct
    out["h59_age_x_lost_pos"] = tyre_age_scaled * lost_pos
    out["h59_age_x_top"] = tyre_age_scaled * top_flag
    out["h59_age_x_mid"] = tyre_age_scaled * mid_flag
    out["h59_age_x_back"] = tyre_age_scaled * back_flag

    out["h59_finish_x_position"] = finish_pressure * position
    out["h59_finish_x_pos_pct"] = finish_pressure * pos_pct
    out["h59_finish_x_lost_pos"] = finish_pressure * lost_pos
    out["h59_finish_x_top"] = finish_pressure * top_flag
    out["h59_finish_x_mid"] = finish_pressure * mid_flag
    out["h59_finish_x_back"] = finish_pressure * back_flag

    out["h59_pit_age_pressure"] = tyre_age_scaled * pit_now
    out["h59_pit_finish_pressure"] = finish_pressure * pit_now
    out["h59_fresh_position_pressure"] = fresh_flag * position
    out["h59_fresh_pos_pct_pressure"] = fresh_flag * pos_pct
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000059"],
        "research_usage_note": "Verified assigned hypothesis 000059.",
    }, sort_keys=True))
