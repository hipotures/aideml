...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out["_orig_order"] = np.arange(len(out), dtype=np.int64)
    out["_is_train"] = (out["_orig_order"] < 439140).astype("int8")
    ...
    out["race_year"] = out["Year"].astype(str) + "_" + out["Race"].astype(str)

    sort_cols = ["race_year", "Driver", "LapNumber", "_orig_order"]
    s = out.sort_values(sort_cols, kind="mergesort").copy()
    g = s.groupby(["race_year", "Driver"], sort=False)

    s["prior_pit_count"] = g["PitStop"].cumsum() - s["PitStop"]
    last_pit_lap = (
        s["LapNumber"]
        .where(s["PitStop"].eq(1))
        .groupby([s["race_year"], s["Driver"]], sort=False)
        .ffill()
    )
    s["last_pit_lap"] = last_pit_lap
    s["laps_since_pit"] = (
        (s["LapNumber"] - last_pit_lap).fillna(s["LapNumber"]).clip(lower=0)
    )

    s["prior_slick_count"] = g["is_slick"].cumsum() - s["is_slick"]
    s["prior_wet_count"] = g["is_wet_compound"].cumsum() - s["is_wet_compound"]
    s["prior_max_tyre_life"] = g["TyreLife"].cummax().shift(1).fillna(0)
    ...
    for col in [
        ...
        s[f"{col}_prev"] = shifted
        s[f"{col}_roll3_mean"] = (
            shifted.groupby([s["race_year"], s["Driver"]], sort=False)
    ...
    s["degradation_per_tyre_lap"] = s["Cumulative_Degradation"] / (s["TyreLife"] + 1.0)
    s["lap_delta_per_tyre_lap"] = s["LapTime_Delta"] / (s["TyreLife"] + 1.0)
    s["stint_progress"] = s["TyreLife"] / (s["LapNumber"] + 1.0)

    out = s.sort_values("_orig_order", kind="mergesort").drop(
        columns=["_orig_order", "_is_train"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000497.",
    }, sort_keys=True))
