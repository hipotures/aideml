...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(group_cols + ["Position"], kind="mergesort")
    ...
    for k in (1, 2, 3):
        for col in base_num_cols:
            out[f"{col}_ahead_{k}"] = g[col].shift(k)
            out[f"{col}_behind_{k}"] = g[col].shift(-k)

        out[f"Compound_ahead_{k}"] = g["Compound"].shift(k)
        out[f"Compound_behind_{k}"] = g["Compound"].shift(-k)

        out[f"delta_tyrelife_ahead_{k}"] = out["TyreLife"] - out[f"TyreLife_ahead_{k}"]
        out[f"delta_tyrelife_behind_{k}"] = (
            out["TyreLife"] - out[f"TyreLife_behind_{k}"]
        )

        out[f"delta_deg_ahead_{k}"] = (
            out["Cumulative_Degradation"] - out[f"Cumulative_Degradation_ahead_{k}"]
        )
        out[f"delta_deg_behind_{k}"] = (
            out["Cumulative_Degradation"] - out[f"Cumulative_Degradation_behind_{k}"]
        )

        out[f"delta_lapdelta_ahead_{k}"] = (
            out["LapTime_Delta"] - out[f"LapTime_Delta_ahead_{k}"]
        )
        out[f"delta_lapdelta_behind_{k}"] = (
            out["LapTime_Delta"] - out[f"LapTime_Delta_behind_{k}"]
        )

        out[f"delta_stint_ahead_{k}"] = out["Stint"] - out[f"Stint_ahead_{k}"]
        out[f"delta_stint_behind_{k}"] = out["Stint"] - out[f"Stint_behind_{k}"]

        out[f"compound_mismatch_ahead_{k}"] = (
            (out["Compound"] != out[f"Compound_ahead_{k}"])
            & out[f"Compound_ahead_{k}"].notna()
        ).astype("int8")
        out[f"compound_mismatch_behind_{k}"] = (
            (out["Compound"] != out[f"Compound_behind_{k}"])
            & out[f"Compound_behind_{k}"].notna()
        ).astype("int8")

        out[f"neighbor_pitted_ahead_{k}"] = (
            out[f"PitStop_ahead_{k}"].fillna(0).astype("int8")
        )
        out[f"neighbor_pitted_behind_{k}"] = (
            out[f"PitStop_behind_{k}"].fillna(0).astype("int8")
        )

        out[f"neighbor_newstint_ahead_{k}"] = (
            out[f"TyreLife_ahead_{k}"].fillna(99) <= 2
        ).astype("int8")
        out[f"neighbor_newstint_behind_{k}"] = (
            out[f"TyreLife_behind_{k}"].fillna(99) <= 2
        ).astype("int8")
    ...
    for k in (1, 2, 3):
        ahead_t = f"abs_delta_tyrelife_ahead_{k}"
        behind_t = f"abs_delta_tyrelife_behind_{k}"
        out[ahead_t] = (out["TyreLife"] - out[f"TyreLife_ahead_{k}"]).abs()
        out[behind_t] = (out["TyreLife"] - out[f"TyreLife_behind_{k}"]).abs()
        local_delta_cols += [ahead_t, behind_t]

        ahead_d = f"abs_delta_deg_ahead_{k}"
        behind_d = f"abs_delta_deg_behind_{k}"
        out[ahead_d] = (
            out["Cumulative_Degradation"] - out[f"Cumulative_Degradation_ahead_{k}"]
        ).abs()
        out[behind_d] = (
        ...
        local_deg_cols += [ahead_d, behind_d]

        ahead_l = f"abs_delta_lapdelta_ahead_{k}"
        behind_l = f"abs_delta_lapdelta_behind_{k}"
        out[ahead_l] = (out["LapTime_Delta"] - out[f"LapTime_Delta_ahead_{k}"]).abs()
        out[behind_l] = (out["LapTime_Delta"] - out[f"LapTime_Delta_behind_{k}"]).abs()
        local_lapdelta_cols += [ahead_l, behind_l]

        local_mismatch_cols += [
            f"compound_mismatch_ahead_{k}",
            f"compound_mismatch_behind_{k}",
        ]
        local_pit_cols += [f"neighbor_pitted_ahead_{k}", f"neighbor_pitted_behind_{k}"]
        local_new_cols += [
            f"neighbor_newstint_ahead_{k}",
            f"neighbor_newstint_behind_{k}",
        ]

    out["local3_tyrelife_gap_min"] = out[local_delta_cols].min(axis=1)
    out["local3_tyrelife_gap_med"] = out[local_delta_cols].median(axis=1)

    out["local3_deg_gap_min"] = out[local_deg_cols].min(axis=1)
    out["local3_deg_gap_med"] = out[local_deg_cols].median(axis=1)

    out["local3_lapdelta_gap_min"] = out[local_lapdelta_cols].min(axis=1)
    out["local3_lapdelta_gap_med"] = out[local_lapdelta_cols].median(axis=1)

    out["local3_compound_mismatch_rate"] = out[local_mismatch_cols].mean(axis=1)
    out["local3_neighbor_pit_rate"] = out[local_pit_cols].mean(axis=1)
    out["local3_neighbor_newstint_rate"] = out[local_new_cols].mean(axis=1)

    out["pressure_ahead_vs_behind_tyrelife"] = (
        out["delta_tyrelife_ahead_1"].abs() - out["delta_tyrelife_behind_1"].abs()
    )
    out["pressure_ahead_vs_behind_deg"] = (
        out["delta_deg_ahead_1"].abs() - out["delta_deg_behind_1"].abs()
    )
    out["pressure_ahead_vs_behind_lapdelta"] = (
        out["delta_lapdelta_ahead_1"].abs() - out["delta_lapdelta_behind_1"].abs()
    )

    out = out.sort_values("__row_order", kind="mergesort").drop(columns="__row_order")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000927"],
        "research_usage_note": "Verified assigned hypothesis 000927.",
    }, sort_keys=True))
