...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dry = ~out["Compound"].isin(["WET", "INTERMEDIATE"])
    wet_regime = ~dry
    first_stop = dry & (out["Stint"] <= 1)
    later_stop = dry & (out["Stint"] > 1)

    out["regime_dry_first_stop"] = first_stop.astype("int8")
    out["regime_dry_later_stop"] = later_stop.astype("int8")
    out["regime_wet_intermediate"] = wet_regime.astype("int8")

    lap_pit_count = out.groupby(lap_keys, sort=False)["PitStop"].transform("sum")
    out["same_lap_pit_wave_count"] = (lap_pit_count - out["PitStop"]).astype("int16")
    out["same_lap_pit_wave_frac"] = out["same_lap_pit_wave_count"] / (
        out.groupby(lap_keys, sort=False)["PitStop"].transform("size").clip(lower=1)
    )

    neighbor = out[lap_keys + ["Position", "PitStop"]].copy()
    neighbor["_row_id"] = np.arange(len(out), dtype=np.int64)
    neighbor = neighbor.sort_values(lap_keys + ["Position", "_row_id"], kind="mergesort")
    neighbor_grp = neighbor.groupby(lap_keys, sort=False)
    row_id = neighbor["_row_id"].to_numpy(dtype=np.int64)

    ahead_vals = np.zeros(len(out), dtype=np.int8)
    behind_vals = np.zeros(len(out), dtype=np.int8)
    ahead_pit = neighbor_grp["PitStop"].shift(1).fillna(0).astype(np.int8).to_numpy()
    ahead_vals[row_id] = ahead_pit

    neighbor_desc = neighbor.sort_values(
        lap_keys + ["Position", "_row_id"],
        ascending=[True] * len(lap_keys) + [False, True],
        kind="mergesort",
    )
    desc_grp = neighbor_desc.groupby(lap_keys, sort=False)
    desc_row_id = neighbor_desc["_row_id"].to_numpy(dtype=np.int64)
    behind_pit = desc_grp["PitStop"].shift(1).fillna(0).astype(np.int8).to_numpy()
    behind_vals[desc_row_id] = behind_pit

    out["driver_ahead_pitted_now"] = ahead_vals
    out["driver_behind_pitted_now"] = behind_vals

    out["driver_ahead_pitted_now"] = (
        out["driver_ahead_pitted_now"].fillna(0).astype("int8")
    )
    out["driver_behind_pitted_now"] = (
        out["driver_behind_pitted_now"].fillna(0).astype("int8")
    )
    out["nearby_driver_pitted_now"] = (
        out["driver_ahead_pitted_now"] | out["driver_behind_pitted_now"]
    ).astype("int8")

    lap_grp = out.groupby(lap_keys, sort=False)
    out["lap_time_rank_pct"] = lap_grp["LapTime (s)"].rank(pct=True).astype("float32")
    out["delta_rank_pct"] = lap_grp["LapTime_Delta"].rank(pct=True).astype("float32")
    out["degradation_rank_pct"] = (
        lap_grp["Cumulative_Degradation"].rank(pct=True).astype("float32")
    )
    out["position_rank_pct"] = (
        lap_grp["Position"].rank(pct=True, ascending=True).astype("float32")
    )
    ...
    out["pack_density_position_band"] = (
        out.groupby(lap_keys + ["position_band"], sort=False)["Position"]
        .transform("size")
        .astype("int8")
    )

    out["mandatory_dry_stop_debt"] = (
        dry & (out["Stint"] <= 1) & (out["RaceProgress"] >= 0.25)
    ).astype("int8")
    out["late_mandatory_dry_stop_debt"] = (
        dry & (out["Stint"] <= 1) & (out["RaceProgress"] >= 0.65)
    ).astype("int8")

    out["tyre_life_x_dry_first"] = out["TyreLife"] * out["regime_dry_first_stop"]
    out["tyre_life_x_dry_later"] = out["TyreLife"] * out["regime_dry_later_stop"]
    out["tyre_life_x_wet"] = out["TyreLife"] * out["regime_wet_intermediate"]
    out["pit_wave_x_dry_first"] = (
        out["same_lap_pit_wave_count"] * out["regime_dry_first_stop"]
    )
    out["nearby_pit_x_dry_first"] = (
        out["nearby_driver_pitted_now"] * out["regime_dry_first_stop"]
    )
    out["debt_x_tyre_life"] = out["mandatory_dry_stop_debt"] * out["TyreLife"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000451"],
        "research_usage_note": "Verified assigned hypothesis 000451.",
    }, sort_keys=True))
