...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ahead = out[lap_keys + ["Position", "PitStop"]].copy()
    ahead["Position"] = ahead["Position"] + 1
    ahead = ahead.rename(columns={"PitStop": "driver_ahead_pitted_now"})
    out = out.merge(ahead, on=lap_keys + ["Position"], how="left", sort=False)

    behind = df[lap_keys + ["Position", "PitStop"]].copy()
    behind["Position"] = behind["Position"] - 1
    behind = behind.rename(columns={"PitStop": "driver_behind_pitted_now"})
    out = out.merge(behind, on=lap_keys + ["Position"], how="left", sort=False)
