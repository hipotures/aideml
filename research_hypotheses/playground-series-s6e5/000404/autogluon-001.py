...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["snapshot_group_size"] = group_size
    ...
    for col, ascending in rank_specs.items():
        ...
        df[f"{col}_snapshot_pct_rank"] = pct_rank
        med = g[col].transform("median")
        mean = g[col].transform("mean")
        std = g[col].transform("std").replace(0, np.nan)
        df[f"{col}_snapshot_median_gap"] = (df[col] - med).astype("float32")
        df[f"{col}_snapshot_mean_gap"] = (df[col] - mean).astype("float32")
        df[f"{col}_snapshot_z"] = (
            ((df[col] - mean) / std)
    ...
    df["TyreLife_x_Degradation"] = (
        df["TyreLife"] * df["Cumulative_Degradation"]
    ).astype("float32")
    df["TyreLife_x_LapTimeDelta"] = (df["TyreLife"] * df["LapTime_Delta"]).astype(
        "float32"
    )
    df["Degradation_x_LapTimeDelta"] = (
        df["Cumulative_Degradation"] * df["LapTime_Delta"]
    ).astype("float32")
    df["LateRace_TyreLife"] = (df["RaceProgress"] * df["TyreLife"]).astype("float32")
    df["LateRace_Degradation"] = (
        df["RaceProgress"] * df["Cumulative_Degradation"]
    ).astype("float32")

    compound_count = (
        df.groupby(group_cols + ["Compound"], sort=False)["Driver"]
        .transform("size")
        .astype("float32")
    )
    df["compound_snapshot_count"] = compound_count
    df["compound_snapshot_share"] = (
        (compound_count / group_size)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )
    df["compound_snapshot_rarity"] = (1.0 - df["compound_snapshot_share"]).astype(
        "float32"
    )

    compound_group_cols = group_cols + ["Compound"]
    cg = df.groupby(compound_group_cols, sort=False)
    for col in ["LapTime_Delta", "LapTime (s)", "TyreLife", "Cumulative_Degradation"]:
        c_med = cg[col].transform("median")
        c_mean = cg[col].transform("mean")
        df[f"{col}_compound_snapshot_median_gap"] = (df[col] - c_med).astype("float32")
        df[f"{col}_compound_snapshot_mean_gap"] = (df[col] - c_mean).astype("float32")
        df[f"{col}_compound_snapshot_pct_rank"] = (
            cg[col].rank(method="average", pct=True, ascending=True).astype("float32")
        )

    df["pit_window_pressure"] = (
        df["TyreLife_snapshot_pct_rank"]
        + df["Cumulative_Degradation_snapshot_pct_rank"]
        + df["LapTime_Delta_snapshot_pct_rank"]
        + df["RaceProgress_snapshot_pct_rank"]
    ).astype("float32")

    df["relative_risk_minus_position"] = (
        df["pit_window_pressure"] - df["Position_snapshot_pct_rank"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000404.",
    }, sort_keys=True))
