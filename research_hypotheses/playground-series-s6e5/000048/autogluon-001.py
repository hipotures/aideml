...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for new_col, cols in interactions.items():
        if all(c in out.columns for c in cols):
            vals = out[cols[0]].astype("string").fillna("__NA__")
            for c in cols[1:]:
                vals = vals + "|" + out[c].astype("string").fillna("__NA__")
            out[new_col] = vals.astype("category")
            freq = out[new_col].value_counts(dropna=False)
            out[new_col + "_freq"] = out[new_col].map(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000048.",
    }, sort_keys=True))
