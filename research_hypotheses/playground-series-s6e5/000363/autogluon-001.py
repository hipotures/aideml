...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["h000363_is_wet"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["h000363_is_intermediate"] = (compound == "INTERMEDIATE").astype("int8")
    out["h000363_is_slick"] = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["h000363_compound_ord"] = (
        compound.map({"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4})
        .fillna(-1)
        .astype("int8")
    )

    out["h000363_monaco_like"] = race.str.contains(
        "Monaco|Singapore|Hungarian", case=False, na=False
    ).astype("int8")

    rp = out["RaceProgress"].astype("float32")
    out["h000363_phase"] = np.select([rp < 0.33, rp < 0.67], [0, 1], default=2).astype(
        "int8"
    )
    out["h000363_early"] = (rp < 0.33).astype("int8")
    out["h000363_late"] = (rp >= 0.67).astype("int8")

    tyre = out["TyreLife"].astype("float32")
    out["h000363_fresh_tyre"] = (tyre <= 3).astype("int8")
    out["h000363_old_tyre"] = (tyre >= 18).astype("int8")
    out["h000363_very_old_tyre"] = (tyre >= 30).astype("int8")

    deg = out["Cumulative_Degradation"].astype("float32")
    delta = out["LapTime_Delta"].astype("float32")
    out["h000363_debt"] = ((deg > 0) & (delta > 0)).astype("int8")
    out["h000363_high_debt"] = ((deg > 50) | (delta > 2.5)).astype("int8")
    out["h000363_negative_debt"] = ((deg < 0) & (delta < 0)).astype("int8")

    out["h000363_wet_late"] = (out["h000363_is_wet"] * out["h000363_late"]).astype(
        "int8"
    )
    out["h000363_old_late"] = (out["h000363_old_tyre"] * out["h000363_late"]).astype(
        "int8"
    )
    out["h000363_debt_late"] = (out["h000363_debt"] * out["h000363_late"]).astype(
        "int8"
    )
    out["h000363_wet_debt"] = (out["h000363_is_wet"] * out["h000363_debt"]).astype(
        "int8"
    )

    out["h000363_regime"] = (
        out["h000363_is_wet"].astype("int16") * 24
        + out["h000363_monaco_like"].astype("int16") * 12
        + out["h000363_phase"].astype("int16") * 4
        + out["h000363_old_tyre"].astype("int16") * 2
        + out["h000363_debt"].astype("int16")
    ).astype("int16")

    out["h000363_tyre_progress"] = (
        (tyre / (out["LapNumber"].astype("float32") + 1.0))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )

    out["h000363_deg_per_tyre_lap"] = (
        (deg / (tyre + 1.0)).replace([np.inf, -np.inf], 0).fillna(0).astype("float32")
    )

    out["h000363_delta_x_old"] = (
        delta * out["h000363_old_tyre"].astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000363.",
    }, sort_keys=True))
