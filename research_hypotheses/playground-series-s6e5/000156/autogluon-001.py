...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    raw_cols = ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation", "TyreLife"]
    for c in raw_cols:
        out[c] = pd.to_numeric(out[c], errors="coerce").astype("float32")

    if race_keys:
        g = out.groupby(race_keys, sort=False)
        for c in raw_cols:
            med = g[c].transform("median")
            q25 = g[c].transform(lambda s: s.quantile(0.25))
            q75 = g[c].transform(lambda s: s.quantile(0.75))
            out[f"{c}_race_z"] = (
                ((out[c] - med) / (q75 - q25 + eps)).clip(-8, 8).astype("float32")
            )
    else:
        for c in raw_cols:
            med = out[c].median()
            ...
            out[f"{c}_race_z"] = (
                ((out[c] - med) / (iqr + eps)).clip(-8, 8).astype("float32")
            )

    group_cols = [c for c in ["Year", "Race", "Driver"] if c in out.columns]
    sort_cols = group_cols + [c for c in ["LapNumber", "Stint"] if c in out.columns]
    order = (
        out.sort_values(sort_cols, kind="mergesort").index if sort_cols else out.index
    )

    state = pd.Series(0.0, index=out.index, dtype="float32")
    trend = pd.Series(0.0, index=out.index, dtype="float32")
    resid = pd.Series(0.0, index=out.index, dtype="float32")
    ahead = pd.Series(0.0, index=out.index, dtype="float32")
    shock = pd.Series(0.0, index=out.index, dtype="float32")
    since_reset = pd.Series(0, index=out.index, dtype="int16")

    obs = (
        0.38 * out["LapTime (s)_race_z"].fillna(0)
        + 0.30 * out["LapTime_Delta_race_z"].fillna(0)
        + 0.22 * out["Cumulative_Degradation_race_z"].fillna(0)
        + 0.10 * out["TyreLife_race_z"].fillna(0)
    ).astype("float32")
    ...
    for idx in order:
        key = tuple(out.at[idx, c] for c in group_cols) if group_cols else ("all",)
        stint = out.at[idx, "Stint"] if "Stint" in out.columns else None
        pit = (
            int(out.at[idx, "PitStop"])
            if "PitStop" in out.columns and pd.notna(out.at[idx, "PitStop"])
            else 0
        )

        if key != prev_key or stint != prev_stint or pit == 1:
            s = 0.0
            t = 0.0
            age = 0
        ...
        resid.at[idx] = r
        ahead.at[idx] = s + t
        shock.at[idx] = max(0.0, r)
        since_reset.at[idx] = age

        age += 1
        prev_key = key
        prev_stint = stint

    out["latent_deg_state"] = state
    out["latent_deg_trend"] = trend
    out["latent_deg_residual"] = resid.clip(-8, 8).astype("float32")
    out["latent_deg_next_est"] = ahead.clip(-8, 8).astype("float32")
    out["latent_deg_positive_shock"] = shock.clip(0, 8).astype("float32")
    out["laps_since_latent_reset"] = since_reset
    out["latent_age_x_state"] = (
        out["laps_since_latent_reset"].astype("float32") * out["latent_deg_state"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000156"],
        "research_usage_note": "Verified assigned hypothesis 000156.",
    }, sort_keys=True))
