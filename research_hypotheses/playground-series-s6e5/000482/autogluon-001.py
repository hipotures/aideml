...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_lap = out.groupby(lap_keys)["LapNumber"].transform("size").astype("float32")
    ...
    for col, name in [
        ...
        out[f"{name}_lap_pct"] = (
            out.groupby(lap_keys)[col].rank(pct=True).astype("float32")
        )

    lap_delta_mean = out.groupby(lap_keys)["LapTime_Delta"].transform("mean")
    lap_delta_std = (
        out.groupby(lap_keys)["LapTime_Delta"].transform("std").replace(0, np.nan)
    )
    out["laptime_delta_lap_z"] = (
        ((out["LapTime_Delta"] - lap_delta_mean) / lap_delta_std)
        .fillna(0)
        .astype("float32")
    )

    for comp in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        mask = out["Compound"].eq(comp).astype("int8")
        cnt = (
            mask.groupby([out[k] for k in lap_keys]).transform("sum").astype("float32")
        )
        cname = comp.lower()
        out[f"lap_compound_{cname}_count"] = cnt
        out[f"lap_compound_{cname}_share"] = (cnt / n_lap).astype("float32")

    pit_cnt = (
        out["PitStop"]
        .astype("int8")
        .groupby([out[k] for k in lap_keys])
        .transform("sum")
        .astype("float32")
    )
    out["lap_pitstop_count"] = pit_cnt
    out["lap_pitstop_share"] = (pit_cnt / n_lap).astype("float32")

    peer_keys = ["Year", "Race", "LapNumber", "Compound", "Stint"]
    peer_pct = (
        out.groupby(peer_keys)["TyreLife"].rank(pct=True).fillna(0.5).astype("float32")
    )
    out["tyrelife_compound_stint_lap_pct"] = peer_pct
    out["early_tyre_compound_stint_lap"] = (peer_pct <= 0.15).astype("int8")
    out["late_tyre_compound_stint_lap"] = (peer_pct >= 0.85).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000482.",
    }, sort_keys=True))
