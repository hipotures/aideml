...
base_features = [c for c in train.columns if c not in [TARGET]]

cat_cols = ["Driver", "Race", "Compound"]
num_cols = [c for c in base_features if c not in cat_cols and c != ID_COL]
...
def add_basic_features(df):
    ...
    out["IsSlick"] = (~out["Compound"].isin(["INTERMEDIATE", "WET"])).astype(int)
    return out
...
def make_prior_maps(fit_df, alpha=40.0):
    ...
    d["Deg_per_TyreLife"] = d["Cumulative_Degradation"] / (d["TyreLife"] + 1.0)
    d["IsWetCompound"] = d["Compound"].isin(["INTERMEDIATE", "WET"]).astype(int)
    d["late_stint_stop"] = ((d[TARGET] == 1) & (d["RaceProgress"] >= 0.45)).astype(int)

    global_hazard = d[TARGET].mean()
    global_stint = d["TyreLife"].median()
    global_deg = d["Deg_per_TyreLife"].median()
    global_late = d["late_stint_stop"].mean()

    maps = {}

    def shrunk_mean(group_cols, value_col, global_value):
        g = d.groupby(group_cols)[value_col].agg(["mean", "count"]).reset_index()
        g["value"] = (g["mean"] * g["count"] + global_value * alpha) / (
            g["count"] + alpha
        )
        return g[group_cols + ["value"]]

    def median_map(group_cols, value_col, global_value):
        g = d.groupby(group_cols)[value_col].agg(["median", "count"]).reset_index()
        g["value"] = (g["median"] * g["count"] + global_value * alpha) / (
            g["count"] + alpha
        )
        return g[group_cols + ["value"]]

    maps["race_hazard"] = shrunk_mean(["Race"], TARGET, global_hazard)
    maps["race_year_hazard"] = shrunk_mean(["Race_Year"], TARGET, global_hazard)
    maps["race_compound_hazard"] = shrunk_mean(
        ["Race", "Compound"], TARGET, global_hazard
    )
    maps["race_stint_median"] = median_map(["Race"], "TyreLife", global_stint)
    maps["race_year_stint_median"] = median_map(["Race_Year"], "TyreLife", global_stint)
    maps["race_deg_regime"] = median_map(["Race"], "Deg_per_TyreLife", global_deg)
    maps["race_late_stop_proxy"] = shrunk_mean(["Race"], "late_stint_stop", global_late)
    maps["compound_hazard"] = shrunk_mean(["Compound"], TARGET, global_hazard)
    maps["wet_hazard"] = shrunk_mean(["IsWetCompound"], TARGET, global_hazard)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train, y), 1):
    ...
    test_fold = apply_prior_maps(test.copy(), maps, globals_)
