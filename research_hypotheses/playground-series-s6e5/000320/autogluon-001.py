...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Race_Year_Compound"] = (
        out["Race_Year"].astype(str) + "_" + out["Compound"].astype(str)
    )
    ...
    glob = out[num_cols].mean(numeric_only=True)

    def add_prior(keys, cols, prefix, shrink=30.0):
        g = out.groupby(keys, observed=True)
        cnt = g["LapNumber"].transform("size").astype(float)
        w = cnt / (cnt + shrink)
        for col in cols:
            mean_v = g[col].transform("mean")
            med_v = g[col].transform("median")
            out[f"{prefix}_{col}_mean_shr"] = w * mean_v + (1.0 - w) * glob[col]
            out[f"{prefix}_{col}_median"] = med_v

    add_prior("Race", num_cols, "race", 80.0)
    add_prior("Race_Year", num_cols, "race_year", 55.0)
    add_prior(
        "Race_Compound",
        ["TyreLife", "LapNumber", "Cumulative_Degradation", "LapTime_Delta", "PitStop"],
        "race_compound",
        45.0,
    )
    add_prior(
    ...
    out["tyrelife_vs_race_compound"] = (
        out["TyreLife"] - out["race_compound_TyreLife_median"]
    )
    out["tyrelife_ratio_race_compound"] = out["TyreLife"] / (
        out["race_compound_TyreLife_median"].abs() + 1.0
    )
    out["lap_vs_race_stop_prior"] = (
        out["LapNumber"] - out["race_PitStop_mean_shr"] * out["race_LapNumber_median"]
    )
    out["degradation_vs_race"] = (
        out["Cumulative_Degradation"] - out["race_Cumulative_Degradation_mean_shr"]
    )
    out["degradation_vs_ry_compound"] = (
        out["Cumulative_Degradation"]
        - out["ry_compound_Cumulative_Degradation_mean_shr"]
    )
    out["laptime_delta_vs_race_compound"] = (
        out["LapTime_Delta"] - out["race_compound_LapTime_Delta_mean_shr"]
    )
    out["stint_vs_race_year"] = out["Stint"] - out["race_year_Stint_median"]
    out["progress_x_race_pit_prior"] = (
        out["RaceProgress"] * out["race_PitStop_mean_shr"]
    )
    out["tyrelife_x_compound_pit_prior"] = (
        out["TyreLife"] * out["race_compound_PitStop_mean_shr"]
    )
    out["high_deg_regime"] = (
        out["race_Cumulative_Degradation_mean_shr"] > glob["Cumulative_Degradation"]
    ).astype("int8")
    out["slow_warmup_proxy"] = (
        out["race_compound_LapTime_Delta_mean_shr"] > glob["LapTime_Delta"]
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000320"],
        "research_usage_note": "Verified assigned hypothesis 000320.",
    }, sort_keys=True))
