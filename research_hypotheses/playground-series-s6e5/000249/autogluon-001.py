...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").fillna("")
    compound = out["Compound"].astype("string").fillna("").str.upper()

    is_testing = race.eq("Pre-Season Testing").astype("int8")
    is_wet_inter = compound.isin(["WET", "INTERMEDIATE"]).astype("int8")
    is_slick = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    is_dry_race = ((is_testing == 0) & (is_wet_inter == 0)).astype("int8")

    out["regime_preseason"] = is_testing
    out["regime_wet_intermediate"] = is_wet_inter
    out["regime_slick_race"] = is_dry_race
    out["regime_id_000249"] = is_testing * 2 + is_wet_inter

    out["main_dry_weight_proxy_000249"] = np.where(
        is_testing == 1, 0.25, np.where(is_wet_inter == 1, 0.55, 1.0)
    ).astype("float32")
    out["specialist_weight_proxy_000249"] = np.where(
        is_testing == 1, 1.0, np.where(is_wet_inter == 1, 0.85, 0.35)
    ).astype("float32")
    ...
    lap_time = out["LapTime (s)"].astype("float32")

    out["tyre_life_x_preseason"] = tyre * is_testing
    out["tyre_life_x_wet_intermediate"] = tyre * is_wet_inter
    out["tyre_life_x_slick_race"] = tyre * is_dry_race

    out["stint_x_preseason"] = stint * is_testing
    out["stint_x_wet_intermediate"] = stint * is_wet_inter
    out["stint_x_slick_race"] = stint * is_dry_race

    out["progress_x_preseason"] = progress * is_testing
    out["progress_x_wet_intermediate"] = progress * is_wet_inter
    out["progress_x_slick_race"] = progress * is_dry_race

    out["degradation_x_preseason"] = deg * is_testing
    out["degradation_x_wet_intermediate"] = deg * is_wet_inter
    out["degradation_x_slick_race"] = deg * is_dry_race

    out["lap_delta_x_preseason"] = lap_delta * is_testing
    out["lap_delta_x_wet_intermediate"] = lap_delta * is_wet_inter
    out["lap_delta_x_slick_race"] = lap_delta * is_dry_race

    out["position_x_wet_intermediate"] = pos * is_wet_inter
    out["position_change_x_wet_intermediate"] = pos_chg * is_wet_inter
    out["lap_time_x_preseason"] = lap_time * is_testing
    out["lap_time_x_wet_intermediate"] = lap_time * is_wet_inter

    out["tyre_progress"] = tyre * progress
    out["tyre_stint"] = tyre * stint
    out["late_old_tyre"] = tyre * progress * is_dry_race
    out["wet_late_old_tyre"] = tyre * progress * is_wet_inter
    out["testing_lap_stint"] = lap * stint * is_testing

    out["compound_race_regime"] = (
        compound.astype("string") + "_" + race.astype("string")
    )
    out["race_regime_000249"] = np.select(
        [is_testing == 1, is_wet_inter == 1],
        ["preseason", "wet_intermediate"],
        default="slick_race",
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000249"],
        "research_usage_note": "Verified assigned hypothesis 000249.",
    }, sort_keys=True))
