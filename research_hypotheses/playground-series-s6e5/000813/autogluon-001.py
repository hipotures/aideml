...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = X.sort_values(
        seq_keys + ["LapNumber", "TyreLife", "_orig_idx"], kind="mergesort"
    )

    g_seq = X.groupby(seq_keys, sort=False)
    g_stint = X.groupby(stint_keys, sort=False)

    X["lap_in_stint"] = g_stint.cumcount() + 1
    X["laps_since_race_start"] = g_seq.cumcount() + 1
    ...
    for col in [
        ...
        X[f"{col}_lag1"] = g_seq[col].shift(1)
        X[f"{col}_lag2"] = g_seq[col].shift(2)

    X["laptime_delta_change_1"] = X["LapTime_Delta"] - X["LapTime_Delta_lag1"]
    X["degradation_change_1"] = (
        X["Cumulative_Degradation"] - X["Cumulative_Degradation_lag1"]
    )
    X["position_change_momentum"] = X["Position_Change"] - X["Position_Change_lag1"]
    X["tyrelife_jump"] = X["TyreLife"] - X["TyreLife_lag1"]

    shifted_lap_delta = g_seq["LapTime_Delta"].shift(1)
    shifted_deg = g_seq["Cumulative_Degradation"].shift(1)
    shifted_laptime = g_seq["LapTime (s)"].shift(1)

    X["lap_delta_roll3_mean"] = (
        shifted_lap_delta.groupby([X[k] for k in seq_keys], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=seq_keys, drop=True)
    )
    X["lap_delta_roll5_mean"] = (
        shifted_lap_delta.groupby([X[k] for k in seq_keys], sort=False)
    ...
    X["deg_roll3_mean"] = (
        shifted_deg.groupby([X[k] for k in seq_keys], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=seq_keys, drop=True)
    )
    X["laptime_roll3_mean"] = (
        shifted_laptime.groupby([X[k] for k in seq_keys], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=seq_keys, drop=True)
    )

    X["lap_delta_ewm"] = shifted_lap_delta.groupby(
        [X[k] for k in seq_keys], sort=False
    ).transform(lambda s: s.ewm(alpha=0.4, adjust=False).mean())
    X["deg_ewm"] = shifted_deg.groupby([X[k] for k in seq_keys], sort=False).transform(
        lambda s: s.ewm(alpha=0.4, adjust=False).mean()
    )

    X["stint_progress"] = X["lap_in_stint"] / X["TyreLife"].clip(lower=1)
    X["race_progress_remaining"] = 1.0 - X["RaceProgress"]
    X["degradation_per_tyre_lap"] = X["Cumulative_Degradation"] / X["TyreLife"].clip(
        lower=1
    )
    X["degradation_per_race_progress"] = X["Cumulative_Degradation"] / X[
        "RaceProgress"
    ].clip(lower=1e-3)
    X["pace_x_deg"] = X["LapTime_Delta"] * X["Cumulative_Degradation"]
    X["pace_x_tyrelife"] = X["LapTime_Delta"] * X["TyreLife"]
    X["deg_x_stint_progress"] = X["Cumulative_Degradation"] * X["stint_progress"]
    X["position_pressure"] = X["Position"] * X["RaceProgress"]
    X["late_race_old_tyre"] = X["race_progress_remaining"] * X["TyreLife"]

    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    X["CompoundOrdinal"] = X["Compound"].map(compound_map).astype("float32")
    X["is_dry_compound"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["is_wet_compound"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    X["soft_or_medium"] = X["Compound"].isin(["SOFT", "MEDIUM"]).astype("int8")
    ...
    fill_zero_cols = [
        ...
        "LapTime_Delta_lag1",
        "LapTime_Delta_lag2",
        "Cumulative_Degradation_lag1",
        "Cumulative_Degradation_lag2",
        "Position_lag1",
        "Position_lag2",
        "Position_Change_lag1",
        "Position_Change_lag2",
        "TyreLife_lag1",
        "TyreLife_lag2",
        "laptime_delta_change_1",
        "degradation_change_1",
        "position_change_momentum",
        "tyrelife_jump",
        "lap_delta_roll3_mean",
        "lap_delta_roll5_mean",
        "deg_roll3_mean",
        "laptime_roll3_mean",
        "lap_delta_ewm",
        "deg_ewm",
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000813"],
        "research_usage_note": "Verified assigned hypothesis 000813.",
    }, sort_keys=True))
