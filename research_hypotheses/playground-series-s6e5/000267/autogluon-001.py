...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["CompoundGrip"] = out["Compound"].map(compound_map).astype(float)
    out["IsDry"] = out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["IsWetLike"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    prev_tyre = grp["TyreLife"].shift(1)
    prev_laptime = grp["LapTime (s)"].shift(1)
    prev_deg = grp["Cumulative_Degradation"].shift(1)
    prev_pos = grp["Position"].shift(1)
    prev_stint = grp["Stint"].shift(1)

    out["TyreLife_Delta1"] = (out["TyreLife"] - prev_tyre).fillna(0.0)
    out["LapTime_Trend1"] = (out["LapTime (s)"] - prev_laptime).fillna(0.0)
    out["Deg_Trend1"] = (out["Cumulative_Degradation"] - prev_deg).fillna(0.0)
    out["Position_Trend1"] = (out["Position"] - prev_pos).fillna(0.0)
    out["Stint_Changed"] = (out["Stint"] != prev_stint).fillna(0).astype("int8")
    ...
    est_total_laps = (out["LapNumber"] / rp).clip(lower=1.0, upper=90.0)
    out["EstTotalLaps"] = est_total_laps
    out["EstLapsRemaining"] = (est_total_laps - out["LapNumber"]).clip(lower=0.0)
    out["RaceEndSoon_3"] = (out["EstLapsRemaining"] <= 3).astype("int8")

    out["StintFracToRaceEnd"] = out["TyreLife"] / (out["EstLapsRemaining"] + 1.0)
    out["DegPerLap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1.0)
    out["LapTimeDeltaAbs"] = out["LapTime_Delta"].abs()
    out["PositionPressure"] = (21 - out["Position"]).astype(float)

    out["HazardBase_1"] = (
        0.45 * out["TyreLife"]
        + 0.35 * out["Cumulative_Degradation"].clip(lower=0.0)
        + 0.25 * out["LapTime_Delta"].clip(lower=0.0)
        - 0.20 * out["EstLapsRemaining"]
    )
    out["HazardBase_2"] = (
        out["HazardBase_1"]
        + 0.30 * out["Deg_Trend3"].clip(lower=0.0)
        + 0.20 * out["LapTime_Trend3"].clip(lower=0.0)
    )
    out["HazardBase_3"] = (
        out["HazardBase_2"]
        + 0.20 * out["StintFracToRaceEnd"]
        + 0.15 * (3.0 - out["CompoundGrip"]).clip(lower=0.0)
    )

    out["HazardUrgency_1"] = out["HazardBase_1"] / (out["EstLapsRemaining"] + 1.0)
    out["HazardUrgency_2"] = out["HazardBase_2"] / (out["EstLapsRemaining"] + 2.0)
    out["HazardUrgency_3"] = out["HazardBase_3"] / (out["EstLapsRemaining"] + 3.0)

    out["HazardTyreDeg"] = out["TyreLife"] * out["DegPerLap"]
    out["HazardTyrePace"] = out["TyreLife"] * out["LapTime_Delta"].clip(lower=0.0)
    out["HazardDegTrend"] = out["Cumulative_Degradation"].clip(lower=0.0) * out[
        "Deg_Trend3"
    ].clip(lower=0.0)
    out["HazardDrySoftPush"] = out["IsDry"] * out["CompoundGrip"] * out["TyreLife"]
    out["HazardWetReset"] = out["IsWetLike"] * out["Stint_Changed"]

    out["Next3LapHazardProxy"] = (
        0.5 * out["HazardUrgency_1"]
        + 0.3 * out["HazardUrgency_2"]
        + 0.2 * out["HazardUrgency_3"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000267.",
    }, sort_keys=True))
