...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_delta = out["LapTime_Delta"].astype("float32")
    tyre = out["TyreLife"].astype("float32")
    race_progress = out["RaceProgress"].astype("float32")

    lap_keys = ["Year", "Race", "LapNumber"]

    out["pos_bucket_3"] = ((pos - 1) // 3).astype("int16")
    out["pos_bucket_5"] = ((pos - 1) // 5).astype("int16")

    field_size = (
        out.groupby(lap_keys, sort=False)["Position"]
        .transform("count")
        .astype("float32")
    )
    out["field_size_lap"] = field_size
    out["position_pct_field"] = ((pos - 1) / np.maximum(field_size - 1, 1)).astype(
        "float32"
    )

    out["pos_bucket3_density"] = (
        out.groupby(lap_keys + ["pos_bucket_3"], sort=False)["Position"]
        .transform("count")
        .astype("float32")
        / field_size
    )
    out["pos_bucket5_density"] = (
        out.groupby(lap_keys + ["pos_bucket_5"], sort=False)["Position"]
        .transform("count")
        .astype("float32")
        / field_size
    )

    out["lap_delta_rank_in_lap"] = (
        out.groupby(lap_keys, sort=False)["LapTime_Delta"]
        .rank(pct=True)
        .astype("float32")
    )
    out["lap_time_rank_in_lap"] = (
        out.groupby(lap_keys, sort=False)["LapTime (s)"]
        .rank(pct=True)
        .astype("float32")
    )
    out["tyre_age_rank_in_lap"] = (
        out.groupby(lap_keys, sort=False)["TyreLife"].rank(pct=True).astype("float32")
    )

    race_comp_median_tyre = out.groupby(["Year", "Race", "Compound"], sort=False)[
        "TyreLife"
    ].transform("median")
    out["tyre_age_vs_race_compound"] = (
        tyre / race_comp_median_tyre.replace(0, np.nan)
    ).astype("float32")
    ...
    seq["_order"] = np.arange(len(seq))
    seq = seq.sort_values(
        ["Year", "Race", "Driver", "LapNumber", "_order"], kind="mergesort"
    )
    g = seq.groupby(["Year", "Race", "Driver"], sort=False)

    seq["position_volatility_4"] = (
        g["Position"]
    ...
    seq["_abs_position_change"] = seq["Position_Change"].abs()
    seq["position_abs_change_mean4"] = (
        g["_abs_position_change"]
        .rolling(4, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
        .fillna(0)
    )
    seq["lap_delta_mean3_driver"] = (
        g["LapTime_Delta"]
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
    )
    seq["lap_delta_worse_vs_recent"] = (
        seq["LapTime_Delta"] - seq["lap_delta_mean3_driver"]
    )

    seq = seq.sort_values("_order", kind="mergesort")
    out["position_volatility_4"] = seq["position_volatility_4"].to_numpy(
        dtype="float32"
    )
    out["position_abs_change_mean4"] = seq["position_abs_change_mean4"].to_numpy(
        dtype="float32"
    )
    out["lap_delta_worse_vs_recent"] = seq["lap_delta_worse_vs_recent"].to_numpy(
        dtype="float32"
    )
    ...
    out["compound_warmup_penalty"] = warmup_penalty
    out["compound_grip_speed"] = grip_speed

    bad_delta_rank = out["lap_delta_rank_in_lap"].astype("float32").fillna(0.5)
    tyre_rank = out["tyre_age_rank_in_lap"].astype("float32").fillna(0.5)
    congestion = out["pos_bucket3_density"].astype("float32").fillna(0)

    front_score = np.clip((6.0 - pos) / 5.0, 0, 1).astype("float32")
    undercut_band = np.clip(1.0 - (np.abs(pos - 8.0) / 8.0), 0, 1).astype("float32")
    low_position_vol = (
        1.0 / (1.0 + out["position_volatility_4"].astype("float32"))
    ).astype("float32")
    low_recent_moves = (
        1.0 / (1.0 + out["position_abs_change_mean4"].astype("float32"))
    ).astype("float32")

    out["traffic_train_score"] = (
        low_position_vol * low_recent_moves * bad_delta_rank * (0.5 + congestion)
    ).astype("float32")
    out["clean_air_front_proxy"] = (
        front_score * (1.0 - congestion.clip(0, 1)) * (1.0 - bad_delta_rank)
    ).astype("float32")

    out["undercut_feasibility_score"] = (
        bad_delta_rank * tyre_rank * undercut_band * (1.0 + congestion) * grip_speed
    ).astype("float32")
    out["undercut_vs_warmup_score"] = (
        out["undercut_feasibility_score"] / (1.0 + warmup_penalty)
    ).astype("float32")
    out["overcut_hold_proxy"] = (
        out["clean_air_front_proxy"]
        * warmup_penalty
        * tyre_rank
        * (1.0 - race_progress)
    ).astype("float32")
    out["traffic_warmup_interaction"] = (
        out["traffic_train_score"] * warmup_penalty
    ).astype("float32")

    out["race_compound"] = (out["Race"].astype("string") + "_" + compound).astype(
        "object"
    )
    ...
    created = [
        ...
        "pos_bucket3_density",
        "pos_bucket5_density",
        "lap_delta_rank_in_lap",
        "lap_time_rank_in_lap",
        "tyre_age_rank_in_lap",
        "tyre_age_vs_race_compound",
        "position_volatility_4",
        "position_abs_change_mean4",
        "lap_delta_worse_vs_recent",
        "compound_warmup_penalty",
        "compound_grip_speed",
        "traffic_train_score",
        "clean_air_front_proxy",
        "undercut_feasibility_score",
        "undercut_vs_warmup_score",
        "overcut_hold_proxy",
        "traffic_warmup_interaction",
    ]
    out[created] = (
        out[created].replace([np.inf, -np.inf], np.nan).fillna(0).astype("float32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000707"],
        "research_usage_note": "Verified assigned hypothesis 000707.",
    }, sort_keys=True))
