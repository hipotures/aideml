...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["qid_driver_race_stint"] = (
        out["qid_driver_race"].astype(str) + "_" + out["Stint"].astype(str)
    ).astype("category")

    g_stint = ["Year", "Race", "Driver", "Stint"]
    g_driver_race = ["Year", "Race", "Driver"]
    g_race_lap = ["Year", "Race", "LapNumber"]

    out["seq_pos_stint"] = out.groupby(g_stint, observed=True).cumcount() + 1
    out["seq_len_stint"] = out.groupby(g_stint, observed=True)["LapNumber"].transform(
        "size"
    )
    out["seq_frac_stint"] = out["seq_pos_stint"] / (out["seq_len_stint"] + eps)
    out["laps_left_stint_seen"] = out["seq_len_stint"] - out["seq_pos_stint"]

    out["seq_pos_driver_race"] = (
        out.groupby(g_driver_race, observed=True).cumcount() + 1
    )
    out["seq_len_driver_race"] = out.groupby(g_driver_race, observed=True)[
        "LapNumber"
    ].transform("size")
    out["seq_frac_driver_race"] = out["seq_pos_driver_race"] / (
        out["seq_len_driver_race"] + eps
    )

    out["tyre_life_frac_race"] = out["TyreLife"] / (out["LapNumber"] + eps)
    out["tyre_life_x_progress"] = out["TyreLife"] * out["RaceProgress"]
    out["stint_x_tyre_life"] = out["Stint"] * out["TyreLife"]
    ...
    out["tyre_life_rank_stint"] = out.groupby(g_stint, observed=True)["TyreLife"].rank(
        method="average", pct=True
    )
    out["degradation_rank_stint"] = out.groupby(g_stint, observed=True)[
        "Cumulative_Degradation"
    ].rank(method="average", pct=True)
    out["laptime_delta_rank_stint"] = out.groupby(g_stint, observed=True)[
        "LapTime_Delta"
    ].rank(method="average", pct=True)

    out["race_lap_pos_mean"] = out.groupby(g_race_lap, observed=True)[
        "Position"
    ].transform("mean")
    out["race_lap_pos_diff"] = out["Position"] - out["race_lap_pos_mean"]
    out["race_lap_tyre_mean"] = out.groupby(g_race_lap, observed=True)[
        "TyreLife"
    ].transform("mean")
    out["race_lap_tyre_diff"] = out["TyreLife"] - out["race_lap_tyre_mean"]

    out["compound_tyre_mean"] = out.groupby("Compound", observed=True)[
        "TyreLife"
    ].transform("mean")
    out["compound_tyre_diff"] = out["TyreLife"] - out["compound_tyre_mean"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000136"],
        "research_usage_note": "Verified assigned hypothesis 000136.",
    }, sort_keys=True))
