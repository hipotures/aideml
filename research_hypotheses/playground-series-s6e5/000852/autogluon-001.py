...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["est_total_laps"] = (
        out["LapNumber"] / out["RaceProgress"].clip(lower=0.01)
    ).clip(1, 120)
    race_year = ["Race", "Year"]
    ...
    for col in [
        ...
        g = out.groupby(race_year, observed=True)[col]
        out[f"ry_{col}_mean"] = g.transform("mean").astype("float32")
        out[f"ry_{col}_std"] = g.transform("std").fillna(0).astype("float32")

    race_g = out.groupby("Race", observed=True)
    out["race_total_laps_mean"] = (
        race_g["est_total_laps"].transform("mean").astype("float32")
    )
    out["race_deg_median"] = (
        race_g["Cumulative_Degradation"].transform("median").astype("float32")
    )
    out["race_laptime_median"] = (
        race_g["LapTime (s)"].transform("median").astype("float32")
    )
    out["race_tlife_q75"] = (
        race_g["TyreLife"].transform(lambda s: s.quantile(0.75)).astype("float32")
    )

    out["lap_frac_est"] = (
        (out["LapNumber"] / out["ry_est_total_laps_mean"].clip(lower=1))
        .clip(0, 1.5)
        .astype("float32")
    )
    out["tyre_frac_est"] = (
        (out["TyreLife"] / out["ry_est_total_laps_mean"].clip(lower=1))
        .clip(0, 1.5)
        .astype("float32")
    )
    out["deg_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1)
    ).astype("float32")
    out["lap_time_vs_race"] = (out["LapTime (s)"] - out["race_laptime_median"]).astype(
        "float32"
    )
    out["deg_vs_race"] = (
        out["Cumulative_Degradation"] - out["race_deg_median"]
    ).astype("float32")

    out["is_recent"] = (out["Year"] >= 2024).astype("int8")
    out["years_from_latest"] = (out["Year"].max() - out["Year"]).astype("float32")
    out["recency_weight_proxy"] = np.exp(-0.45 * out["years_from_latest"]).astype(
        "float32"
    )

    early = out["RaceProgress"] <= 0.35
    out["early_phase"] = early.astype("int8")
    out["late_phase"] = (out["RaceProgress"] >= 0.70).astype("int8")
    out["old_tyre_late_race"] = (
        (out["TyreLife"] >= out["race_tlife_q75"]) & (out["RaceProgress"] >= 0.55)
    ).astype("int8")

    comp_freq = (
        out["Compound"]
        .map(out["Compound"].value_counts(normalize=True))
        .astype("float32")
    )
    race_freq = (
    ...
    out["compound_freq"] = comp_freq
    out["race_freq"] = race_freq
    out["driver_freq"] = driver_freq

    out["race_compound"] = (
        out["Race"].astype(str) + "_" + out["Compound"].astype(str)
    ).astype("category")
    out["race_year_key"] = (
        out["Race"].astype(str) + "_" + out["Year"].astype("int16").astype(str)
    ).astype("category")
    out["driver_race"] = (
        out["Driver"].astype(str) + "_" + out["Race"].astype(str)
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000852"],
        "research_usage_note": "Verified assigned hypothesis 000852.",
    }, sort_keys=True))
