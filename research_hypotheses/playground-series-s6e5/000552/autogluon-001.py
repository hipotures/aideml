...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["lap_group_size"] = out.groupby(grp)["Driver"].transform("size").astype("int16")
    out["race_lap_frac"] = out["LapNumber"] / out.groupby(["Year", "Race"])[
        "LapNumber"
    ...
    for c in num_cols:
        ...
        out[f"{c}_lap_mean"] = mean
        out[f"{c}_lap_std"] = std
        out[f"{c}_lap_diff"] = out[c] - mean
        out[f"{c}_lap_z"] = out[f"{c}_lap_diff"] / (std + 1e-6)
        out[f"{c}_lap_rank"] = g.rank(method="average", pct=True).astype("float32")

    out["tyre_stint_pressure"] = out["TyreLife"] / (out["Stint"] + 1)
    out["late_old_tyre"] = out["RaceProgress"] * out["TyreLife"]
    out["degradation_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["position_loss_pressure"] = (
        out["Position_Change"].clip(lower=0) * out["TyreLife"]
    )
    out["recent_pit_same_lap_share"] = out.groupby(grp)["PitStop"].transform("mean")

    out["compound_lap_count"] = (
        out.groupby(grp + ["Compound"])["Driver"].transform("size").astype("int16")
    )
    out["driver_freq"] = (
        out.groupby("Driver")["Driver"].transform("size").astype("int32")
    )
    out["race_freq"] = out.groupby("Race")["Race"].transform("size").astype("int32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000552"],
        "research_usage_note": "Verified assigned hypothesis 000552.",
    }, sort_keys=True))
