...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["IsTesting"] = is_testing

    out["IsDryCompound"] = (~compound.isin(["INTERMEDIATE", "WET"])).astype("int8")
    out["IsWetCompound"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["IsSoft"] = compound.eq("SOFT").astype("int8")
    out["IsMedium"] = compound.eq("MEDIUM").astype("int8")
    out["IsHard"] = compound.eq("HARD").astype("int8")

    race_progress = out["RaceProgress"].clip(0.01, 1.0)
    est_total_laps = (out["LapNumber"] / race_progress).clip(1, 120)
    laps_remaining = (est_total_laps - out["LapNumber"]).clip(lower=0)
    out["EstTotalLaps"] = est_total_laps.astype("float32")
    out["LapsRemaining"] = laps_remaining.astype("float32")
    out["RaceEndPressure"] = race_progress.pow(2).astype("float32")

    tyre_life = out["TyreLife"].astype("float32")
    stint = out["Stint"].astype("float32")
    pit_now = out["PitStop"].astype("int8")

    out["TyreLife_x_Progress"] = (tyre_life * race_progress).astype("float32")
    out["TyreLife_x_Stint"] = (tyre_life * stint).astype("float32")
    out["PitNow_x_TyreLife"] = (pit_now * tyre_life).astype("float32")
    out["PitNow_x_Stint"] = (pit_now * stint).astype("float32")

    out["Testing_TyreLife"] = (is_testing * tyre_life).astype("float32")
    out["Testing_Stint"] = (is_testing * stint).astype("float32")
    out["Testing_PitNow"] = (is_testing * pit_now).astype("int8")
    out["Testing_LapNumber"] = (is_testing * out["LapNumber"]).astype("float32")
    out["Testing_Degradation"] = (is_testing * out["Cumulative_Degradation"]).astype(
        "float32"
    )
    out["Testing_LapDelta"] = (is_testing * out["LapTime_Delta"]).astype("float32")

    dry_race = ((1 - is_testing) * out["IsDryCompound"]).astype("int8")
    out["RaceOnly_DryCompoundDebt"] = (
        dry_race * np.maximum(0, race_progress - 0.35)
    ).astype("float32")
    out["RaceOnly_LateDryTyreAge"] = (
        dry_race * tyre_life * np.maximum(0, race_progress - 0.55)
    ).astype("float32")
    out["RaceOnly_LapsRemaining"] = ((1 - is_testing) * laps_remaining).astype(
        "float32"
    )
    ...
    out["RaceFreq"] = race.map(race.value_counts(normalize=True)).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000469"],
        "research_usage_note": "Verified assigned hypothesis 000469.",
    }, sort_keys=True))
