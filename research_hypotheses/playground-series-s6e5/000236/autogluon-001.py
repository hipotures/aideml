...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g = out.groupby(keys, sort=False)["LapTime_Delta"]
    med = g.transform("median").astype("float32")
    q25 = g.transform(lambda s: s.quantile(0.25)).astype("float32")
    q75 = g.transform(lambda s: s.quantile(0.75)).astype("float32")
    std = g.transform("std").fillna(0).astype("float32")
    cnt = g.transform("size").astype("float32")
    iqr = (q75 - q25).astype("float32")

    out["rlap_delta_median"] = med
    out["rlap_delta_iqr"] = iqr
    out["rlap_delta_std"] = std
    out["rlap_driver_delta_minus_median"] = (delta - med).astype("float32")
    out["rlap_driver_delta_z"] = (
        ((delta - med) / (std + 1e-3)).clip(-20, 20).astype("float32")
    )
    out["rlap_field_count"] = cnt

    large_pos = (delta > (med + np.maximum(3.0, 1.5 * iqr))).astype("float32")
    out["rlap_large_positive_delta_frac"] = (
        large_pos.groupby([out["Year"], out["Race"], out["LapNumber"]], sort=False)
        .transform("mean")
        .astype("float32")
    )

    med_pos = (med.clip(lower=0) / 25.0).clip(0, 1)
    spread_compact = 1.0 - (iqr / 15.0).clip(0, 1)
    field_sync = out["rlap_large_positive_delta_frac"]
    proxy = (0.50 * med_pos + 0.35 * field_sync + 0.15 * spread_compact).clip(0, 1)

    out["neutralization_proxy_score"] = proxy.astype("float32")
    out["neutralization_proxy_flag"] = (proxy >= 0.45).astype("int8")
    out["near_neutralization_proxy_flag"] = (proxy >= 0.30).astype("int8")
    out["neutralized_x_tyre_life"] = (proxy * out["TyreLife"].astype("float32")).astype(
        "float32"
    )
    out["neutralized_x_race_progress"] = (
        proxy * out["RaceProgress"].astype("float32")
    ).astype("float32")
    out["neutralized_x_stint"] = (proxy * out["Stint"].astype("float32")).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000236"],
        "research_usage_note": "Verified assigned hypothesis 000236.",
    }, sort_keys=True))
