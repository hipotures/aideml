...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").str.lower()
    year = out["Year"].astype("int16")
    compound = out["Compound"].astype("string").str.upper()

    is_monaco_2025 = race.str.contains("monaco", regex=False, na=False) & (year == 2025)
    is_qatar_2023 = race.str.contains("qatar", regex=False, na=False) & (year == 2023)
    is_qatar_2025 = race.str.contains("qatar", regex=False, na=False) & (year == 2025)
    is_qatar_cap = is_qatar_2023 | is_qatar_2025
    wet_like = compound.isin(["WET", "INTERMEDIATE"])

    prior_pit_count = (out["Stint"].astype("float32") - 1).clip(lower=0)
    ...
    qatar_cap_laps = (
        is_qatar_2023.astype("int8") * 18 + is_qatar_2025.astype("int8") * 25
    )
    laps_to_qatar_cap = qatar_cap_laps.astype("float32") - tyre_life
    laps_to_qatar_cap = laps_to_qatar_cap.where(is_qatar_cap, 99).clip(
        lower=-5, upper=99
    )

    dry_rule_debt = (
        1 - wet_like.astype("int8") - (prior_pit_count >= 1).astype("int8")
    ).clip(lower=0)
    monaco_three_set_debt = (
        (3 - out["Stint"].astype("float32")).clip(lower=0).where(is_monaco_2025, 0)
    )

    out["Race_Year_Monaco_2025"] = is_monaco_2025.astype("int8")
    out["Race_Year_Qatar_2023"] = is_qatar_2023.astype("int8")
    out["Race_Year_Qatar_2025"] = is_qatar_2025.astype("int8")
    out["PriorPitCount_rule"] = prior_pit_count.astype("float32")
    out["wet_rule_exemption_seen"] = wet_like.astype("int8")
    out["required_remaining_stops_by_rule"] = dry_rule_debt.astype("float32")
    out["laps_to_Qatar_stint_cap"] = laps_to_qatar_cap.astype("float32")
    out["Qatar_stint_cap_pressure"] = (
        (is_qatar_cap) & (laps_to_qatar_cap <= 3)
    ).astype("int8")
    out["Monaco_three_set_debt"] = monaco_three_set_debt.astype("float32")
    out["rule_debt_x_tyre_life"] = (
        out["required_remaining_stops_by_rule"] * tyre_life
    ).astype("float32")
    out["qatar_cap_pressure_x_tyre_life"] = (
        (99 - laps_to_qatar_cap).clip(lower=0) * is_qatar_cap.astype("int8")
    ).astype("float32")
    out["monaco_debt_x_stint"] = (
        monaco_three_set_debt * out["Stint"].astype("float32")
    ).astype("float32")
    out["rule_debt_late_race"] = (
        out["required_remaining_stops_by_rule"] * race_progress
    ).astype("float32")
    out["qatar_cap_lap_ratio"] = (
        (tyre_life / qatar_cap_laps.replace(0, 99).astype("float32"))
        .clip(0, 2)
        .astype("float32")
    )
    out["monaco_late_debt_pressure"] = (
        monaco_three_set_debt * race_progress * lap_number
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000507.",
    }, sort_keys=True))
