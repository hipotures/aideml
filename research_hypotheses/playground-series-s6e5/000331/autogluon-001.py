...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    level_feat = np.zeros(n, dtype=np.float32)
    slope_feat = np.zeros(n, dtype=np.float32)
    var_feat = np.zeros(n, dtype=np.float32)
    reset_feat = np.zeros(n, dtype=np.float32)

    resid = out["LapTime_Delta"].astype("float64").to_numpy()
    resid = np.nan_to_num(resid, nan=0.0, posinf=45.0, neginf=-45.0)
    resid = np.clip(resid, -45.0, 45.0)

    pit = out["PitStop"].astype("int8").to_numpy()
    tyre_life = out["TyreLife"].astype("float64").to_numpy()
    stint = out["Stint"].astype("int16").to_numpy()

    order_cols = ["Driver", "Race", "Year", "LapNumber"]
    if "RaceProgress" in out.columns:
        order_cols.append("RaceProgress")

    work = out[["Driver", "Race", "Year"]].copy()
    work["_i"] = np.arange(n)
    for c in order_cols:
        if c not in work.columns:
            work[c] = out[c].to_numpy()

    sorted_i = work.sort_values(order_cols + ["_i"], kind="mergesort")["_i"].to_numpy()
    ...
    for idx in sorted_i:
        key = (out.at[idx, "Driver"], out.at[idx, "Race"], out.at[idx, "Year"])
        ...
        is_reset = (
            (pit[idx] == 1)
            or (tyre_life[idx] <= 1.0 and out.at[idx, "LapNumber"] > 1)
            or (stint[idx] != prev_stint)
        )
        if is_reset:
            reset_trace = abs(pred_level) + 2.0 * abs(pred_slope)
            pred_level = 0.0
            pred_slope = 0.0
            pp00 = init_p_level
            pp01 = 0.0
        ...
        if pit[idx] == 1:
            ...
            p01 = pp01
            p11 = pp11
            ew_var = max(ew_var, r_obs)
        else:
            innovation = resid[idx] - pred_level
            s = pp00 + r_obs
            k0 = pp00 / s
            k1 = pp01 / s
            ...
            p01 = (1.0 - k0) * pp01
            p11 = pp11 - k1 * pp01
            ew_var = 0.92 * ew_var + 0.08 * min(innovation * innovation, 2025.0)

        reset_trace *= 0.88

        level_feat[idx] = level
        slope_feat[idx] = slope
        var_feat[idx] = ew_var
        reset_feat[idx] = reset_trace
        prev_stint = stint[idx]

    out["ss_tyre_level"] = level_feat
    out["ss_tyre_slope"] = slope_feat
    out["ss_resid_var"] = var_feat
    out["ss_post_stop_reset"] = reset_feat
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000331"],
        "research_usage_note": "Verified assigned hypothesis 000331.",
    }, sort_keys=True))
