...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")

    out["Monaco_2025_Special_Rule"] = (
        (out["Year"] == 2025)
        & out["Race"].astype("string").str.contains("Monaco", case=False, na=False)
    ).astype("int8")

    out["Is_Wet_Compound"] = is_wet
    out["Wet_Seen_So_Far"] = (
        out.groupby(grp, sort=False)["Is_Wet_Compound"].cummax().astype("int8")
    )
    out["Wet_Exemption_Flag"] = out["Wet_Seen_So_Far"]

    out["Sets_Used_So_Far"] = out["Stint"].clip(lower=1).astype("int16")
    out["Stops_Made_So_Far"] = (
        (out["Sets_Used_So_Far"] - 1).clip(lower=0).astype("int16")
    )

    for c in ["HARD", "MEDIUM", "SOFT"]:
        col = f"Used_{c}_So_Far"
        out[col] = (
            (compound == c)
            .astype("int8")
            .groupby([out[k] for k in grp], sort=False)
            .cummax()
        ).astype("int8")
    out["Dry_Compounds_Used_So_Far"] = (
        out[["Used_HARD_So_Far", "Used_MEDIUM_So_Far", "Used_SOFT_So_Far"]]
        .sum(axis=1)
        .astype("int8")
    )

    race_max_lap = (
        out.groupby(race_grp, sort=False)["LapNumber"].transform("max").astype("int16")
    )
    out["Race_Max_Lap"] = race_max_lap
    out["Laps_Left_In_Race"] = (
        (race_max_lap - out["LapNumber"]).clip(lower=0).astype("int16")
    )

    normal_required_stops = (
        (out["Wet_Exemption_Flag"] == 0) & (out["Dry_Compounds_Used_So_Far"] < 2)
    ).astype("int8")
    monaco_required_stops = np.where(
        out["Wet_Exemption_Flag"] == 1, 0, np.maximum(0, 2 - out["Stops_Made_So_Far"])
    ).astype("int16")

    out["Required_Stops_Remaining"] = np.where(
        out["Monaco_2025_Special_Rule"] == 1,
        monaco_required_stops,
        normal_required_stops,
    ).astype("int16")

    out["Required_Dry_Compounds_Remaining"] = np.where(
        out["Wet_Exemption_Flag"] == 1,
        0,
        np.maximum(0, 2 - out["Dry_Compounds_Used_So_Far"]),
    ).astype("int8")

    laps_left_safe = out["Laps_Left_In_Race"].replace(0, 1)
    out["Rule_Pressure_Per_Lap_Left"] = (
        out["Required_Stops_Remaining"] / laps_left_safe
    ).astype("float32")
    out["Compound_Compliance_Pressure"] = (
        out["Required_Dry_Compounds_Remaining"] / laps_left_safe
    ).astype("float32")

    out["Current_Set_Age_Share"] = (
        out["TyreLife"] / out["Race_Max_Lap"].replace(0, 1)
    ).astype("float32")
    out["Stint_Progress_Ratio"] = (
        out["TyreLife"] / out["LapNumber"].replace(0, 1)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000954.",
    }, sort_keys=True))
