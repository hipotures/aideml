...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Race", "Driver", "LapNumber", "_row_order"]
    work = work.sort_values(sort_cols, kind="mergesort")

    grp = work.groupby(["Race", "Driver"], sort=False)
    pit = work["PitStop"].astype("int8")

    current_stop_count = grp["PitStop"].cumsum().astype("int16")
    prior_stop_count = (current_stop_count - pit).astype("int16")

    work["obs_prior_pit_count"] = prior_stop_count
    work["obs_current_pit_count"] = current_stop_count
    work["obs_no_prior_stop"] = (prior_stop_count == 0).astype("int8")
    work["obs_no_current_or_prior_stop"] = (current_stop_count == 0).astype("int8")
    work["obs_current_lap_is_pit"] = pit.astype("int8")
    work["obs_remaining_one_stop_debt_prior"] = (prior_stop_count == 0).astype("int8")
    work["obs_remaining_one_stop_debt_current"] = (current_stop_count == 0).astype(
        "int8"
    )

    pit_lap = work["LapNumber"].where(pit.eq(1))
    last_pit_lap = pit_lap.groupby([work["Race"], work["Driver"]], sort=False).ffill()
    work["obs_laps_since_last_stop"] = (
        (work["LapNumber"] - last_pit_lap).fillna(-1).astype("float32")
    )
    work["obs_has_seen_stop"] = last_pit_lap.notna().astype("int8")

    work = work.sort_values("_row_order", kind="mergesort")
    new_cols = [
        "obs_prior_pit_count",
        "obs_current_pit_count",
        "obs_no_prior_stop",
        "obs_no_current_or_prior_stop",
        "obs_current_lap_is_pit",
        "obs_remaining_one_stop_debt_prior",
        "obs_remaining_one_stop_debt_current",
        "obs_laps_since_last_stop",
        "obs_has_seen_stop",
    ]

    out = work.drop(columns=["_orig_index", "_row_order"])
    out.index = orig_idx
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000021"],
        "research_usage_note": "Verified assigned hypothesis 000021.",
    }, sort_keys=True))
