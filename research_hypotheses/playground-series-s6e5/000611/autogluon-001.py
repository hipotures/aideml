...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Stable binnings for hazard-state approximation
    ...
    # Hazard-state keys inspired by hypothesis 000611
    ...
    def add_freq_features(frame, key_col, parent_col, prefix):
        key_count = (
        ...
        parent_count = (
            frame.groupby(parent_col, observed=True)[parent_col]
            .transform("size")
            .astype("float32")
        )
    ...
    # Compound-normalized ageing/degradation state
    ...
    # Race-specific lap progress to expose track strategy windows
    race_max_lap = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000611"],
        "research_usage_note": "Verified assigned hypothesis 000611.",
    }, sort_keys=True))
