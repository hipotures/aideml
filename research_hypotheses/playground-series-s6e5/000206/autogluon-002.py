...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    row_id = "__h000206_row_id"
    out[row_id] = np.arange(len(out))

    race_keys = ["Year", "Race"]
    lap_keys = ["Year", "Race", "LapNumber"]

    pit_mask = ((out["TyreLife"].astype(float) <= 2) & (out["Stint"].astype(float) > 1))
    lap_group = out.groupby(lap_keys, sort=False)

    field_size = lap_group["Position"].transform("count").astype(float)
    out["h000206_field_size"] = field_size
    out["h000206_position_pct"] = out["Position"].astype(float) / field_size.clip(
        lower=1.0
    )

    lap_median_time = lap_group["LapTime (s)"].transform("median")
    pit_excess = (out["LapTime (s)"].astype(float) - lap_median_time).where(pit_mask)
    race_pit_loss = pit_excess.groupby(
        [out["Year"], out["Race"]], sort=False
    ).transform("median")
    global_pit_loss = np.nanmedian(pit_excess.to_numpy(dtype=float))
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 22.0
    race_pit_loss = race_pit_loss.fillna(global_pit_loss).clip(10.0, 45.0)

    abs_delta = out["LapTime_Delta"].astype(float).abs()
    nonpit_abs_delta = abs_delta.where(~pit_mask)
    race_delta_scale = nonpit_abs_delta.groupby(
        [out["Year"], out["Race"]], sort=False
    ).transform("median")
    global_delta_scale = np.nanmedian(nonpit_abs_delta.to_numpy(dtype=float))
    if not np.isfinite(global_delta_scale):
        global_delta_scale = 2.0
    race_delta_scale = race_delta_scale.fillna(global_delta_scale).clip(0.25, 8.0)

    out["h000206_pit_loss_time_proxy"] = race_pit_loss
    out["h000206_race_delta_scale"] = race_delta_scale
    pit_loss_pos = np.rint(race_pit_loss / (race_delta_scale + 2.0))
    pit_loss_pos = np.clip(pit_loss_pos, 2, np.maximum(field_size - 1, 2)).astype(int)
    out["h000206_pit_loss_pos_proxy"] = pit_loss_pos

    rejoin_pos = out["Position"].astype(float) + pit_loss_pos
    out["h000206_rejoin_position_proxy"] = np.minimum(rejoin_pos, field_size).astype(
        int
    )
    out["h000206_rejoin_band"] = out["h000206_rejoin_position_proxy"].astype(
        float
    ) / field_size.clip(lower=1.0)
    out["h000206_rejoin_backfield_flag"] = (out["h000206_rejoin_band"] >= 0.70).astype(
        int
    )

    ordered = out[lap_keys + ["Position", "LapTime_Delta", row_id]].sort_values(
        lap_keys + ["Position", row_id], kind="mergesort"
    )
    ordered["__abs_delta"] = ordered["LapTime_Delta"].astype(float).abs()
    ordered["__pos_delta"] = ordered["LapTime_Delta"].astype(float).clip(lower=0.0)

    og = ordered.groupby(lap_keys, sort=False)
    idx_drop = list(range(len(lap_keys)))
    ordered["__zone_delta_mean5"] = (
        og["LapTime_Delta"]
        .rolling(5, center=True, min_periods=1)
        .mean()
        .reset_index(level=idx_drop, drop=True)
    )
    ordered["__zone_abs_mean5"] = (
        og["__abs_delta"]
        .rolling(5, center=True, min_periods=1)
        .mean()
        .reset_index(level=idx_drop, drop=True)
    )
    ordered["__zone_pos_mean5"] = (
        og["__pos_delta"]
        .rolling(5, center=True, min_periods=1)
        .mean()
        .reset_index(level=idx_drop, drop=True)
    ...
    current = ordered[
        ...
        columns={
            "__zone_delta_mean5": "h000206_current_near_delta_mean5",
            "__zone_abs_mean5": "h000206_current_near_abs_mean5",
            "__zone_pos_mean5": "h000206_current_near_slow_mean5",
            "__zone_vol5": "h000206_current_near_vol5",
        }
    )
    out = out.merge(current, on=row_id, how="left", sort=False)
    ...
    zone = (
        ...
        .drop_duplicates(lap_keys + ["Position"], keep="first")
        .rename(
            columns={
                "Position": "h000206_rejoin_position_proxy",
                "__zone_delta_mean5": "h000206_rejoin_delta_mean5",
                "__zone_abs_mean5": "h000206_rejoin_abs_mean5",
                "__zone_pos_mean5": "h000206_rejoin_slow_mean5",
                "__zone_vol5": "h000206_rejoin_vol5",
            }
        )
    )
    out = out.merge(
        zone, on=lap_keys + ["h000206_rejoin_position_proxy"], how="left", sort=False
    )

    scale = out["h000206_race_delta_scale"].astype(float) + 1e-6
    out["h000206_rejoin_abs_norm"] = out["h000206_rejoin_abs_mean5"].fillna(0.0) / scale
    out["h000206_rejoin_slow_norm"] = (
        out["h000206_rejoin_slow_mean5"].fillna(0.0) / scale
    )
    out["h000206_rejoin_vol_norm"] = out["h000206_rejoin_vol5"].fillna(0.0) / scale
    out["h000206_rejoin_vs_current_abs"] = out["h000206_rejoin_abs_mean5"].fillna(
        0.0
    ) - out["h000206_current_near_abs_mean5"].fillna(0.0)
    out["h000206_clean_air_flag"] = (
        (out["h000206_rejoin_abs_norm"] < 0.85)
        & (out["h000206_rejoin_vol_norm"] < 0.85)
        & (out["h000206_rejoin_slow_norm"] < 0.70)
    ).astype(int)
    out["h000206_bad_traffic_flag"] = (
        (out["h000206_rejoin_abs_norm"] > 1.25)
        | (out["h000206_rejoin_vol_norm"] > 1.25)
        | (out["h000206_rejoin_slow_norm"] > 1.10)
    ).astype(int)

    out = out.sort_values(row_id, kind="mergesort")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000206"],
        "research_usage_note": "Verified assigned hypothesis 000206.",
    }, sort_keys=True))
