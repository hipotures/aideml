...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap_med = out.groupby(["Race", "LapNumber"])[lap_col].transform("median")
    comp_med = out.groupby("Compound")[lap_col].transform("median")
    pos_med = out.groupby("Position")[lap_col].transform("median")
    prog_bin = pd.cut(
        out["RaceProgress"], bins=np.linspace(0, 1, 11), include_lowest=True
    )
    prog_med = out.groupby(prog_bin, observed=False)[lap_col].transform("median")
    global_med = out[lap_col].median()

    expected = (
        0.55 * race_lap_med.fillna(global_med)
        + 0.20 * comp_med.fillna(global_med)
        + 0.15 * pos_med.fillna(global_med)
        + 0.10 * prog_med.fillna(global_med)
    )
    out["tyre_health_resid"] = out[lap_col] - expected

    ordered = out.sort_values(sort_cols, kind="mergesort")
    g = ordered.groupby(group_cols, sort=False)

    prev_resid = g["tyre_health_resid"].shift(1)
    ordered["resid_prev"] = prev_resid
    ordered["resid_roll3_mean"] = (
        prev_resid.groupby([ordered[c] for c in group_cols], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
    )
    ordered["resid_roll5_mean"] = (
        prev_resid.groupby([ordered[c] for c in group_cols], sort=False)
    ...
    ordered["resid_slope_3"] = g["tyre_health_resid"].shift(1) - g[
        "tyre_health_resid"
    ].shift(4)
    prev_slope = ordered["resid_slope_3"]
    ordered["resid_accel_3"] = prev_slope - g["resid_slope_3"].shift(1)

    ordered["degradation_slope_3"] = g["Cumulative_Degradation"].shift(1) - g[
        "Cumulative_Degradation"
    ].shift(4)
    ordered["laptime_delta_roll3"] = (
        g["LapTime_Delta"]
        .shift(1)
        .groupby([ordered[c] for c in group_cols], sort=False)
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=[0, 1, 2], drop=True)
    )
    ...
    tyre_bin = out["TyreLife"].round().clip(1, 80)
    z_group = [out["Race"], out["Compound"], tyre_bin]
    shifted_resid = out.groupby(group_cols, sort=False)["tyre_health_resid"].shift(1)
    bin_mean = shifted_resid.groupby(z_group, sort=False).transform("mean")
    bin_std = (
        shifted_resid.groupby(z_group, sort=False).transform("std").replace(0, np.nan)
    )
    out["resid_race_comp_tyre_z"] = (
        ((shifted_resid - bin_mean) / bin_std)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000625"],
        "research_usage_note": "Verified assigned hypothesis 000625.",
    }, sort_keys=True))
