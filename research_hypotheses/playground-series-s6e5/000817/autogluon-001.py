...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_norm = (
        ...
        .str.strip()
        .str.replace("Saudi Arabia Grand Prix", "Saudi Arabian Grand Prix", regex=False)
    )
    comp = out["Compound"].astype("string").str.upper().str.strip()
    ...
    race_sets = {
        2022: (
            ...
            "123 234 235 234 234 123 345 345 345 123 345 234 234 234 123 234 345 123 234 234 234 345",
        ),
        2023: (
            (
                "Bahrain Grand Prix",
            ...
            "123 234 234 345 234 345 123 345 345 123 345 234 123 345 345 123 123 234 345 234 345 345",
        ),
        2024: (
            (
                "Bahrain Grand Prix",
            ...
            "123 234 345 123 234 234 345 345 345 123 345 123 345 234 123 345 345 345 234 345 345 345 123 345",
        ),
        2025: (
            (
                "Australian Grand Prix",
            ...
            "345 234 123 123 345 345 456 456 123 456 345 234 134 345 234 345 456 345 134 245 234 345 123 345",
        ),
    }

    rows = []
    for y, (races, specs) in race_sets.items():
        rows += [
            (y, r, int(c[0]), int(c[1]), int(c[2]))
            for r, c in zip(races, specs.split())
        ]
    nom_index = pd.MultiIndex.from_tuples([(r[0], r[1]) for r in rows])
    key = pd.MultiIndex.from_arrays([year.to_numpy(), race_norm.to_numpy()])
    nom_h = (
        pd.Series([r[2] for r in rows], index=nom_index)
        .reindex(key)
        .to_numpy(dtype="float32")
    )
    nom_m = (
        pd.Series([r[3] for r in rows], index=nom_index)
        .reindex(key)
        .to_numpy(dtype="float32")
    )
    nom_s = (
        pd.Series([r[4] for r in rows], index=nom_index)
        .reindex(key)
        .to_numpy(dtype="float32")
    )
    known = ~np.isnan(nom_h)
    nom_h = np.where(known, nom_h, 2.0).astype("float32")
    nom_m = np.where(known, nom_m, 3.0).astype("float32")
    nom_s = np.where(known, nom_s, 4.0).astype("float32")

    is_hard = comp.eq("HARD").fillna(False).to_numpy(dtype=bool)
    is_medium = comp.eq("MEDIUM").fillna(False).to_numpy(dtype=bool)
    is_soft = comp.eq("SOFT").fillna(False).to_numpy(dtype=bool)
    is_wet = (
        (comp.eq("INTERMEDIATE") | comp.eq("WET")).fillna(False).to_numpy(dtype=bool)
    )
    is_dry = is_hard | is_medium | is_soft
    actual = np.full(n, -1.0, dtype="float32")
    actual[is_hard], actual[is_medium], actual[is_soft] = (
        nom_h[is_hard],
        nom_m[is_medium],
        nom_s[is_soft],
    )

    out["tyre_nomination_known"] = known.astype("int8")
    out["is_preseason_testing"] = (
        race_norm.eq("Pre Season Testing")
        .fillna(False)
        .to_numpy(dtype=bool)
        .astype("int8")
    )
    out["weekend_softness_index"] = ((nom_h + nom_m + nom_s) / 3.0).astype("float32")
    out["weekend_nomination_span"] = (nom_s - nom_h).astype("float32")
    out["actual_pirelli_compound"] = actual
    out["actual_compound_rel_softness"] = np.where(
        is_dry, (actual - nom_h) / np.maximum(nom_s - nom_h, 1.0), -1.0
    ).astype("float32")
    out["is_dry_tyre"] = is_dry.astype("int8")
    out["is_wet_tyre"] = is_wet.astype("int8")
    ...
    group_key = pd.MultiIndex.from_arrays(
        [
            year.to_numpy(),
            race_norm.to_numpy(),
            out["Driver"].astype("string").fillna("").to_numpy(),
        ]
    )
    group_codes = pd.factorize(group_key, sort=False)[0]
    order = np.lexsort((np.arange(n), lap, group_codes))
    g_sorted, actual_sorted, dry_sorted = (
        group_codes[order],
        actual[order].astype("int8"),
        is_dry[order],
    )

    dry_count_sorted = np.zeros(n, dtype="int8")
    for spec in range(1, 7):
        dry_count_sorted += (
            pd.Series(dry_sorted & (actual_sorted == spec))
            .groupby(g_sorted, sort=False)
            .cummax()
            .to_numpy(dtype="int8")
        )
    wet_seen_sorted = (
        pd.Series(is_wet[order])
        .groupby(g_sorted, sort=False)
        .cummax()
        .to_numpy(dtype=bool)
    )

    dry_count = np.empty(n, dtype="int8")
    wet_exception = np.empty(n, dtype=bool)
    dry_count[order], wet_exception[order] = dry_count_sorted, wet_seen_sorted
    mandatory_used = wet_exception | (dry_count >= 2)
    needs_second = is_dry & (~wet_exception) & (dry_count < 2)
    ...
    laps_remaining = np.maximum(lap / np.maximum(progress, 0.01) - lap, 0.0)
    pressure = (
        needs_second.astype("float32")
        * (progress**2)
        * (1.0 + np.minimum(tyre_life / 25.0, 3.0))
        / (1.0 + laps_remaining / 8.0)
    )

    out["dry_specs_used_so_far"] = dry_count
    out["wet_exception_active"] = wet_exception.astype("int8")
    out["mandatory_spec_used"] = mandatory_used.astype("int8")
    out["second_dry_spec_needed"] = needs_second.astype("int8")
    out["second_dry_stop_pressure"] = pressure.astype("float32")
    out["dry_rule_laps_remaining"] = np.where(
        needs_second, laps_remaining, -1.0
    ).astype("float32")
    out["dry_rule_next_5_laps"] = (needs_second & (laps_remaining <= 5.0)).astype(
        "int8"
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000817"],
        "research_usage_note": "Verified assigned hypothesis 000817.",
    }, sort_keys=True))
