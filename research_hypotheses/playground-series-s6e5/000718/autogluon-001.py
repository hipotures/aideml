...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["tyrelife_progress_ratio"] = out["TyreLife"] / (out["LapNumber"] + 1.0)
    ...
    out["late_stint_pressure"] = out["TyreLife"] * out["RaceProgress"]
    out["pit_window_proxy"] = (
        (out["RaceProgress"] > 0.18) & (out["RaceProgress"] < 0.82)
    ).astype("int8")
    ...
    for c in [
        ...
        out[f"{c}_race_lap_rank_pct"] = out.groupby(race_lap, observed=True)[c].rank(
            pct=True, method="average"
        )
        out[f"{c}_race_lap_centered"] = out[c] - out.groupby(race_lap, observed=True)[
            c
        ].transform("median")
    ...
    ordered = out.sort_values(race_lap + ["Position"], kind="mergesort")
    for c in [
        "LapTime (s)",
        "LapTime_Delta",
        "TyreLife",
        ...
        ahead = ordered.groupby(race_lap, observed=True)[c].shift(1)
        behind = ordered.groupby(race_lap, observed=True)[c].shift(-1)
        ordered[f"{c}_vs_car_ahead"] = ordered[c] - ahead
        ordered[f"{c}_vs_car_behind"] = ordered[c] - behind
        ordered[f"{c}_nearest_abs_gap"] = np.minimum(
            ordered[f"{c}_vs_car_ahead"].abs().fillna(np.inf),
            ordered[f"{c}_vs_car_behind"].abs().fillna(np.inf),
        ).replace(np.inf, np.nan)

    local = ordered.groupby(race_lap, observed=True)["LapTime (s)"]
    ordered["local_laptime_mean3"] = (
        local.rolling(3, center=True, min_periods=1)
        .mean()
        .reset_index(level=race_lap, drop=True)
    )
    ordered["local_laptime_gap3"] = (
        ordered["LapTime (s)"] - ordered["local_laptime_mean3"]
    )

    ordered["nearby_position_contest"] = (
        (
            ordered["LapTime (s)_nearest_abs_gap"]
            <= ordered["LapTime (s)"]
            .groupby([ordered["Year"], ordered["Race"], ordered["LapNumber"]])
            .transform("std")
            .fillna(0)
            + 0.75
        )
    ...
    contest_cols = [c for c in ordered.columns if c not in out.columns]
    out = out.join(ordered[contest_cols].sort_index())

    out["compound_is_wet"] = (
        out["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    )
    out["compound_dry_softness"] = (
        out["Compound"]
    ...
    for c in ["Driver", "Race", "Compound"]:
        ...
        out[f"{c}_freq"] = freq.astype("float32") / float(len(out))
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000718.",
    }, sort_keys=True))
