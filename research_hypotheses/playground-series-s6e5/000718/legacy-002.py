...
def add_features(df):
    ...
    sdf = df.sort_values(["Year", "Race", "LapNumber", "Position"]).copy()
    ...
    for col in local_cols:
        ...
        sdf[f"{col}_ahead_delta"] = sdf[col] - ahead
        sdf[f"{col}_behind_delta"] = sdf[col] - behind
        nearest = np.minimum(
            np.abs(sdf[f"{col}_ahead_delta"].fillna(np.inf)),
            np.abs(sdf[f"{col}_behind_delta"].fillna(np.inf)),
        )
        sdf[f"{col}_nearest_abs_delta"] = nearest.replace(np.inf, np.nan)
    ...
    lap_std = sdf["LapTime_s"].groupby(sdf["race_lap"]).transform("std").fillna(0)
...
hazard_features = [c for c in features if c in cat_cols or c != "Driver"]
...
def build_pairs(df, label_series, max_pairs=350000):
    tmp = df[
        ["race_lap", "Position", "LapTime_s", "TyreLife", "local_contest", "PitStop"]
    ].copy()
    tmp["_y"] = label_series.loc[tmp.index].astype(int).to_numpy()
...
def make_rank_pool(df, labels):
    label_series = pd.Series(np.asarray(labels, dtype=int), index=df.index)
    pairs_orig = build_pairs(df, label_series)
    ...
    pool = Pool(
        rank_df[features],
        label=label_series.loc[rank_df.index].to_numpy(),
        cat_features=cat_idx,
        group_id=rank_df["race_lap"].astype(str).to_numpy(),
        pairs=pairs,
    )
...
def score_weights(w):
    wm, wr, wh = w
    return roc_auc_score(y, wm * main_r + wr * rank_r + wh * hazard_r), w


if Parallel is not None and len(candidates) > 1:
    workers = min(16, os.cpu_count() or 1)
    print(f"Evaluating {len(candidates)} blend candidates with {workers} workers")
    scored = Parallel(n_jobs=workers, prefer="threads")(
        delayed(score_weights)(w) for w in candidates
    )
else:
    scored = [score_weights(w) for w in candidates]

best_auc, best_w = max(scored, key=lambda x: x[0])
...
pd.DataFrame({"row": np.arange(n_train), "target": y, "prediction": oof_pred}).to_csv(
