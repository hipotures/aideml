...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_life = pd.to_numeric(out["TyreLife"], errors="coerce")
    out["Stint_Ordinal"] = stint_num.fillna(0).astype("int16")
    out["Is_FirstLapOfStint"] = tyre_life.eq(1).astype("int8")
    out["Is_SecondLapOfStint"] = tyre_life.eq(2).astype("int8")

    prev_compound = g["Compound"].shift(1)
    prior_compound_changed = s["Compound"].ne(prev_compound) & prev_compound.notna()

    prev_stint = g["Stint"].shift(1)
    stint_boundary = s["Stint"].ne(prev_stint) & prev_stint.notna()
    first_lap = pd.to_numeric(s["TyreLife"], errors="coerce").eq(1) | stint_boundary

    out.loc[s.index, "Is_FirstLapOfStint"] = first_lap.astype("int8").to_numpy()
    out.loc[s.index, "PriorCompoundChanged"] = prior_compound_changed.astype(
        "int8"
    ).to_numpy()

    stint_group_cols = [s[c] for c in group_cols] + [s["Stint"]]
    changed_this_stint = (
        prior_compound_changed.astype("int8")
        .groupby(stint_group_cols, sort=False)
        .cummax()
    )
    out.loc[s.index, "CompoundChangedThisStint"] = changed_this_stint.astype(
        "int8"
    ).to_numpy()

    if "PitStop" in out.columns:
        prior_pit = pd.to_numeric(g["PitStop"].shift(1), errors="coerce").fillna(0)
        out.loc[s.index, "PriorPitStop"] = prior_pit.astype("int8").to_numpy()
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000009"],
        "research_usage_note": "Verified assigned hypothesis 000009.",
    }, sort_keys=True))
