...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g0 = out.groupby(group_cols, sort=False)
    out["_pit_reset"] = g0["PitStop"].cumsum()
    seq_cols = group_cols + ["_pit_reset"]
    g = out.groupby(seq_cols, sort=False)

    lap_time = out["LapTime (s)"].astype(float)
    prev_count = g.cumcount().astype(float)
    prev_sum = g["LapTime (s)"].cumsum() - lap_time
    prev_sumsq = (lap_time * lap_time).groupby(
        [out[c] for c in seq_cols], sort=False
    ).cumsum() - (lap_time * lap_time)

    prev_mean = prev_sum / prev_count.replace(0, np.nan)
    prev_var = (prev_sumsq / prev_count.replace(0, np.nan)) - (prev_mean * prev_mean)
    prev_var = prev_var.clip(lower=1e-9)
    out["laptime_z_past"] = ((lap_time - prev_mean) / np.sqrt(prev_var)).fillna(0.0)

    out["deg_delta"] = g["Cumulative_Degradation"].diff().fillna(0.0)

    base_features = {
        "LapTime_Delta": out["LapTime_Delta"].astype(float),
        "laptime_z_past": out["laptime_z_past"].astype(float),
        "deg_delta": out["deg_delta"].astype(float),
    ...
    for name, series in base_features.items():
        l1 = (
            ...
            else series.groupby([out[c] for c in seq_cols], sort=False).shift(1)
        )
        l2 = (
            g[name].shift(2)
            if name in out.columns
            else series.groupby([out[c] for c in seq_cols], sort=False).shift(2)
        )
        l3 = (
            g[name].shift(3)
            if name in out.columns
            else series.groupby([out[c] for c in seq_cols], sort=False).shift(3)
        )

        out[f"{name}_lag1"] = l1.fillna(0.0)
        out[f"{name}_lag2"] = l2.fillna(0.0)
        out[f"{name}_lag3"] = l3.fillna(0.0)
        out[f"{name}_slope_3"] = ((l1 - l3) / 2.0).fillna(0.0)
        out[f"{name}_vol_3"] = pd.concat([l1, l2, l3], axis=1).std(axis=1).fillna(0.0)

    out["lap_progress_left"] = 1.0 - out["RaceProgress"].astype(float)
    out["tyrelife_minus_lap"] = out["TyreLife"].astype(float) - out["LapNumber"].astype(
        float
    )

    out = out.sort_values("_row_id", kind="mergesort").drop(
        columns=["_row_id", "_pit_reset"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000693.",
    }, sort_keys=True))
