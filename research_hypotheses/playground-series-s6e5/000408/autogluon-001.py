...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = out["Compound"].astype("string").fillna("UNKNOWN").str.upper()
    ...
    is_testing = race.str.contains(
        "Pre-Season Testing", case=False, regex=False
    ).astype("int8")
    is_dry_race = ((is_slick == 1) & (is_testing == 0)).astype("int8")
    ...
    owed_proxy = (
        (is_dry_race == 1) & (stint <= 1) & (progress < 0.86) & (pit_now < 0.5)
    ).astype("int8")
    finish_window = ((progress >= 0.70) & (pit_now < 0.5)).astype("int8")

    out["regime_slick_dry"] = is_dry_race
    out["regime_wet_intermediate"] = is_wet
    out["regime_preseason"] = is_testing
    out["dry_stop_still_owed_proxy"] = owed_proxy
    out["dry_free_to_finish_proxy"] = ((is_dry_race == 1) & (owed_proxy == 0)).astype(
        "int8"
    )
    out["finish_window"] = finish_window

    out["tyre_progress"] = tyre * progress
    out["tyre_lap_ratio"] = tyre / (lap + 1.0)
    ...
    out["large_time_loss_flag"] = (delta > 3.0).astype("int8")
    ...
    for prefix, gate in {
        ...
        g = gate.astype("float32")
        out[f"{prefix}_tyre_life"] = tyre * g
        out[f"{prefix}_race_progress"] = progress * g
        out[f"{prefix}_stint"] = stint * g
        out[f"{prefix}_degradation"] = degr * g
        out[f"{prefix}_laptime_delta"] = delta * g
        out[f"{prefix}_pit_now"] = pit_now * g
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000408.",
    }, sort_keys=True))
