...
def add_features(df):
    ...
    dry = out["is_slick"].eq(1)
    keys = ["Year", "Race", "Driver"]
    slick_compounds_seen = (
        out.where(dry)
        .groupby(keys, sort=False)["Compound"]
        .transform(
            lambda s: s.ffill()
            .expanding()
    ...
    out["slick_compounds_seen"] = slick_compounds_seen.fillna(0).astype(float)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X_train, y), 1):
    ...
    for name in ["testing", "wet", "dry_owed", "dry_free"]:
        ...
        fallback_va = va_pred.copy()
        fallback_te = te_pred.copy()
        ...
        if tr_mask.sum() >= 200 and len(np.unique(y_tr[tr_mask])) == 2:
            ...
            if va_mask.any():
                fallback_va[va_mask] = sub_va_pred
            if te_mask.any():
                fallback_te[te_mask] = sub_te_pred

        oof_expert[name][va_idx] = fallback_va
        test_expert_folds[name].append(fallback_te)
