...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp_score = out["Compound"].map(soft_rank).astype("float32")
    out["compound_score"] = comp_score

    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    s = out.sort_values(sort_cols, kind="mergesort")
    gd = s.groupby(["Year", "Race", "Driver"], sort=False)
    recent_pit_s = (
        gd["PitStop"].shift(1).fillna(0).astype("float32")
        + gd["PitStop"].shift(2).fillna(0).astype("float32")
    ).clip(0, 1)
    out["recent_pit_1_2"] = recent_pit_s.reindex(out.index).astype("float32")

    pos = out["Position"].astype("float32").to_numpy()
    tyre = out["TyreLife"].astype("float32").to_numpy()
    ltd = out["LapTime_Delta"].astype("float32").to_numpy()
    recent = out["recent_pit_1_2"].astype("float32").to_numpy()
    comp = comp_score.fillna(comp_score.median()).astype("float32").to_numpy()

    cnt_recent = np.zeros(n, dtype="float32")
    tyre_def = np.zeros(n, dtype="float32")
    fresher_frac = np.zeros(n, dtype="float32")
    comp_fresher_frac = np.zeros(n, dtype="float32")
    soft_knn = np.zeros(n, dtype="float32")

    grouped = out.groupby(["Year", "Race", "LapNumber"], sort=False).indices
    for idx in grouped.values():
        ...
        self_mask = np.eye(m, dtype=bool)

        nearby = (dp <= 3.0) & (~self_mask)
        denom = nearby.sum(axis=1).astype("float32")
        denom_safe = np.maximum(denom, 1.0)

        cnt_recent[idx] = (nearby * r[None, :]).sum(axis=1).astype("float32")
        tyre_def[idx] = (nearby * np.maximum(dt, 0.0)).sum(axis=1).astype(
            "float32"
        ) / denom_safe
        fresher_frac[idx] = (nearby * (dt >= 3.0)).sum(axis=1).astype(
            "float32"
        ) / denom_safe
        comp_fresher_frac[idx] = (nearby * (c[None, :] > c[:, None])).sum(
            axis=1
        ).astype("float32") / denom_safe

        w = np.exp(-dp / 2.0 - dd / 4.0 - np.abs(dt) / 8.0).astype("float32")
        w[self_mask] = 0.0
        pressure = (
            r[None, :]
            + 0.35 * (dt >= 3.0).astype("float32")
            + 0.20 * (c[None, :] > c[:, None]).astype("float32")
        )
        soft_knn[idx] = (w * pressure).sum(axis=1) / np.maximum(w.sum(axis=1), 1e-6)

    out["local_recent_pit_count"] = cnt_recent
    out["local_tyre_life_deficit"] = tyre_def
    out["local_fresher_tyre_frac"] = fresher_frac
    out["local_fresher_compound_frac"] = comp_fresher_frac
    out["local_undercut_knn_score"] = soft_knn

    pos_pressure = (21.0 - out["Position"].astype("float32")).clip(lower=1.0) / 20.0
    lap_loss = out["LapTime_Delta"].astype("float32").clip(lower=0.0)
    out["undercut_score_x_front_position"] = (
        out["local_undercut_knn_score"] * pos_pressure
    )
    out["undercut_score_x_lap_loss"] = out["local_undercut_knn_score"] * np.log1p(
        lap_loss
    )
    out["recent_pit_count_x_lap_loss"] = out["local_recent_pit_count"] * np.log1p(
        lap_loss
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000548"],
        "research_usage_note": "Verified assigned hypothesis 000548.",
    }, sort_keys=True))
