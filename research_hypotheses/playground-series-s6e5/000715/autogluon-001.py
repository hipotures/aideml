...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry_compound = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    is_testing = race.str.contains("testing", regex=False).astype("int8")
    is_atypical_session = is_testing
    is_gp_like_session = (1 - is_atypical_session).astype("int8")

    stint = pd.to_numeric(out["Stint"], errors="coerce").fillna(0)
    tyre_life = pd.to_numeric(out["TyreLife"], errors="coerce").fillna(0)
    lap = pd.to_numeric(out["LapNumber"], errors="coerce").fillna(0)
    progress = pd.to_numeric(out["RaceProgress"], errors="coerce").fillna(0)

    out["reg_is_wet_compound"] = is_wet_compound
    out["reg_is_dry_compound"] = is_dry_compound
    out["reg_is_gp_like_session"] = is_gp_like_session
    out["reg_is_atypical_session"] = is_atypical_session

    out["reg_wet_rule_relief_proxy"] = (is_wet_compound & is_gp_like_session).astype(
        "int8"
    )
    out["reg_dry_mandatory_active_proxy"] = (
        is_dry_compound & is_gp_like_session & (1 - is_wet_compound)
    ).astype("int8")
    out["reg_dry_switch_needed_proxy"] = (
        (stint <= 1)
        & (progress >= 0.45)
        & (is_dry_compound == 1)
        & (is_gp_like_session == 1)
    ).astype("int8")
    out["reg_mandatory_satisfied_proxy"] = (
        (stint >= 2) | (is_wet_compound == 1) | (is_atypical_session == 1)
    ).astype("int8")

    out["reg_stint_stage_pre_switch_pressure"] = (
        out["reg_dry_switch_needed_proxy"] * progress
    )
    out["reg_late_dry_unsatisfied"] = out["reg_dry_switch_needed_proxy"] * (
        progress >= 0.65
    ).astype("int8")
    out["reg_tyre_life_x_dry_rule"] = tyre_life * out["reg_dry_mandatory_active_proxy"]
    out["reg_progress_x_wet"] = progress * is_wet_compound
    out["reg_progress_x_atypical"] = progress * is_atypical_session
    out["reg_stint_x_rule_satisfied"] = stint * out["reg_mandatory_satisfied_proxy"]
    out["reg_lap_x_gp_like"] = lap * is_gp_like_session
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000715"],
        "research_usage_note": "Verified assigned hypothesis 000715.",
    }, sort_keys=True))
