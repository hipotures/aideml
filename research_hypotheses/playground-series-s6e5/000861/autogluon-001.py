...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    est_total_laps = (lap / progress).clip(1, 120)
    laps_left = (est_total_laps - lap).clip(lower=0)
    laps_since_last_pit = tyre.clip(lower=0)

    out["laps_left_est"] = laps_left.astype("float32")
    out["laps_since_last_pit"] = laps_since_last_pit.astype("float32")
    out["service_pressure"] = (tyre / np.sqrt(laps_left + 1.0)).astype("float32")
    out["finish_pressure"] = ((progress**2) * (tyre + 1.0)).astype("float32")
    out["tyre_life_finish_margin"] = (tyre - laps_left).astype("float32")
    out["cannot_finish_flag"] = (tyre + laps_left > 45).astype("int8")

    out["remaining_stop_debt"] = ((stint <= 1) & (progress > 0.55)).astype("int8")
    out["remaining_compound_debt"] = (
        (stint <= 1)
        & out["Compound"].isin(["INTERMEDIATE", "WET"]).astype(bool)
        & (progress > 0.45)
    ).astype("int8")
    out["late_single_stint_pressure"] = (
        out["remaining_stop_debt"].astype("float32") * progress
    ).astype("float32")

    out["degradation_per_tyre_lap"] = (degr / (tyre + 1.0)).astype("float32")
    out["laptime_delta_abs"] = delta.abs().astype("float32")
    out["pace_stress"] = (
        (delta.clip(lower=0) + out["degradation_per_tyre_lap"].clip(lower=0))
        / (laptime.clip(lower=1))
    ).astype("float32")
    out["track_position_risk"] = (
        (21.0 - pos) / 20.0 + pos_chg.clip(lower=0) / 18.0
    ).astype("float32")

    out["physics_residual_expert_signal"] = (
        0.30 * out["service_pressure"].rank(pct=True)
        + 0.20 * out["finish_pressure"].rank(pct=True)
        + 0.18 * out["laps_since_last_pit"].rank(pct=True)
        + 0.14 * out["remaining_stop_debt"].astype("float32")
        + 0.08 * out["remaining_compound_debt"].astype("float32")
        + 0.10 * out["cannot_finish_flag"].astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000861"],
        "research_usage_note": "Verified assigned hypothesis 000861.",
    }, sort_keys=True))
