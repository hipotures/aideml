...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["tyrelife_rank_in_racelap"] = df.groupby(race_lap)["TyreLife"].rank(
        method="average", ascending=True
    )
    df["tyrelife_pct_in_racelap"] = df.groupby(race_lap)["TyreLife"].rank(
        method="average", ascending=True, pct=True
    )
    df["position_pct_in_racelap"] = df.groupby(race_lap)["Position"].rank(
        method="average", ascending=True, pct=True
    )
    df["position_vs_tyrelife_pct"] = (
        df["position_pct_in_racelap"] - df["tyrelife_pct_in_racelap"]
    )

    df["current_lap_pit_count"] = df.groupby(race_lap)["PitStop"].transform("sum")
    df["current_lap_pit_share"] = df.groupby(race_lap)["PitStop"].transform("mean")
    df["cars_in_racelap"] = df.groupby(race_lap)["Driver"].transform("size")

    lap_pits = (
        df.groupby(race_lap, as_index=False)["PitStop"]
        .sum()
        .rename(columns={"PitStop": "lap_pit_count_tmp"})
        .sort_values(["Race", "LapNumber"])
    )
    lap_pits["prev_lap_pit_count_by_race"] = (
        lap_pits.groupby("Race")["lap_pit_count_tmp"].shift(1).fillna(0)
    )
    df = df.merge(
        lap_pits[["Race", "LapNumber", "prev_lap_pit_count_by_race"]],
        on=["Race", "LapNumber"],
        how="left",
    )

    df["position_band"] = ((df["Position"] - 1) // 5).clip(0, 3).astype("int8")
    df["median_tyrelife_posband_racelap"] = df.groupby(
        ["Race", "LapNumber", "position_band"]
    )["TyreLife"].transform("median")
    df["tyrelife_vs_posband_median"] = (
        df["TyreLife"] - df["median_tyrelife_posband_racelap"]
    )

    order = np.arange(len(df))
    tmp = df[["Race", "LapNumber", "Position", "TyreLife"]].copy()
    tmp["_row"] = order
    tmp = tmp.sort_values(["Race", "LapNumber", "Position", "_row"])

    grp = tmp.groupby(["Race", "LapNumber"], sort=False)
    tmp["pos_ahead"] = grp["Position"].shift(1)
    tmp["pos_behind"] = grp["Position"].shift(-1)
    tmp["tyre_ahead"] = grp["TyreLife"].shift(1)
    tmp["tyre_behind"] = grp["TyreLife"].shift(-1)

    tmp["gap_pos_ahead"] = tmp["Position"] - tmp["pos_ahead"]
    tmp["gap_pos_behind"] = tmp["pos_behind"] - tmp["Position"]
    tmp["gap_tyrelife_ahead"] = tmp["TyreLife"] - tmp["tyre_ahead"]
    tmp["gap_tyrelife_behind"] = tmp["tyre_behind"] - tmp["TyreLife"]

    tmp = tmp.sort_values("_row")
    for c in [
        "gap_pos_ahead",
        "gap_pos_behind",
        "gap_tyrelife_ahead",
    ...
    df["ahead_exists"] = df["gap_pos_ahead"].notna().astype("int8")
    df["behind_exists"] = df["gap_pos_behind"].notna().astype("int8")

    df = df.drop(columns=["position_band"])
    return df
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000612"],
        "research_usage_note": "Verified assigned hypothesis 000612.",
    }, sort_keys=True))
