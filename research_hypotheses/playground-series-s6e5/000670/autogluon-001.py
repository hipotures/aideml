...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    est_total_laps = est_total_laps.fillna(lap.max()).clip(
        lower=lap.max() * 0.5, upper=90
    )
    laps_remaining = (est_total_laps - lap).clip(lower=0)

    out["haz_laps_in_stint"] = tyre
    out["haz_laps_remaining"] = laps_remaining
    out["haz_stint_progress"] = (tyre / (tyre + laps_remaining + 1.0)).clip(0, 1)
    out["haz_race_remaining_frac"] = (1.0 - progress).clip(0, 1)
    out["haz_lap_frac"] = (lap / est_total_laps).clip(0, 1)

    out["haz_tyre_sq"] = tyre * tyre
    out["haz_tyre_log1p"] = np.log1p(tyre)
    out["haz_remaining_log1p"] = np.log1p(laps_remaining)
    out["haz_tyre_x_stint"] = tyre * stint
    out["haz_tyre_x_progress"] = tyre * progress
    out["haz_deg_per_tyre_lap"] = deg / tyre.clip(lower=1)
    out["haz_delta_abs"] = delta.abs()
    out["haz_laptime_delta_ratio"] = delta / laptime.clip(lower=1)

    for cut in (3, 5, 8, 12, 16, 20, 25, 30, 35, 45):
        out[f"haz_tyre_gt_{cut}"] = (tyre > cut).astype("int8")
        out[f"haz_tyre_hinge_{cut}"] = (tyre - cut).clip(lower=0)

    for cut in (1, 2, 3, 5, 8, 12, 16, 20):
        out[f"haz_remaining_le_{cut}"] = (laps_remaining <= cut).astype("int8")
        out[f"haz_remaining_hinge_{cut}"] = (cut - laps_remaining).clip(lower=0)

    out["haz_tyre_bin"] = pd.cut(
        tyre,
        bins=[0, 3, 6, 10, 14, 18, 24, 32, 45, 90],
        labels=False,
        include_lowest=True,
    ).astype("int16")
    out["haz_remaining_bin"] = pd.cut(
        laps_remaining,
        bins=[-1, 1, 3, 5, 8, 12, 18, 26, 90],
        labels=False,
        include_lowest=True,
    ).astype("int16")

    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    comp_code = out["Compound"].map(compound_map).fillna(-1).astype("int16")
    out["haz_compound_code"] = comp_code
    out["haz_tyre_x_compound"] = tyre * (comp_code + 1)
    out["haz_wet_or_inter"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )

    out["haz_stint1"] = (out["Stint"] == 1).astype("int8")
    out["haz_late_race"] = (progress >= 0.75).astype("int8")
    out["haz_early_stint_late_race"] = ((tyre <= 5) & (progress >= 0.75)).astype("int8")
    out["haz_old_tyre_not_late"] = ((tyre >= 20) & (progress < 0.75)).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000670"],
        "research_usage_note": "Verified assigned hypothesis 000670.",
    }, sort_keys=True))
