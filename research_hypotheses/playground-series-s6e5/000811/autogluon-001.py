...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound_s = out["Compound"].astype("string")
    compound_wear = (
        compound_s.map(
            {
                "HARD": 1.0,
    ...
    out["race_progress_sq"] = progress**2
    out["race_remaining"] = 1.0 - progress
    out["estimated_total_laps"] = est_total_laps.clip(1, 120)
    out["estimated_laps_remaining"] = laps_remaining.clip(0, 120)
    out["lap_frac_of_est_total"] = lap / (est_total_laps + eps)

    out["tyre_life_log1p"] = np.log1p(tyre.clip(lower=0))
    out["tyre_life_sq"] = tyre**2
    out["tyre_life_per_lap"] = tyre / (lap + eps)
    out["tyre_life_per_stint"] = tyre / (stint + eps)
    out["wear_weighted_tyre_life"] = tyre * compound_wear
    out["late_old_tyre"] = tyre * progress
    out["remaining_x_tyre_life"] = (1.0 - progress) * tyre

    out["degradation_per_tyre_life"] = degr / (tyre + eps)
    out["degradation_per_lap"] = degr / (lap + eps)
    out["abs_degradation"] = degr.abs()
    out["positive_degradation"] = degr.clip(lower=0)
    out["negative_degradation"] = (-degr).clip(lower=0)
    out["wear_degradation_interaction"] = compound_wear * degr

    out["lap_time_log1p"] = np.log1p(lap_time.clip(lower=0))
    out["lap_time_clipped"] = lap_time.clip(upper=400)
    out["lap_time_extreme_flag"] = (lap_time > 300).astype("int8")
    out["abs_lap_delta"] = delta.abs()
    out["lap_delta_clipped"] = delta.clip(-120, 120)
    out["slow_lap_flag"] = (delta > 10).astype("int8")
    out["fast_lap_flag"] = (delta < -10).astype("int8")
    out["delta_per_tyre_life"] = delta / (tyre + eps)

    out["position_norm"] = (pos - 1.0) / 19.0
    out["front_pack"] = (pos <= 5).astype("int8")
    out["back_pack"] = (pos >= 16).astype("int8")
    out["abs_position_change"] = pos_chg.abs()
    out["lost_positions"] = pos_chg.clip(lower=0)
    out["gained_positions"] = (-pos_chg).clip(lower=0)
    out["position_pressure"] = out["position_norm"] * progress

    out["is_soft"] = (compound_s == "SOFT").astype("int8")
    out["is_medium"] = (compound_s == "MEDIUM").astype("int8")
    out["is_hard"] = (compound_s == "HARD").astype("int8")
    out["is_wet_or_inter"] = compound_s.isin(["WET", "INTERMEDIATE"]).astype("int8")
    out["compound_wear_factor"] = compound_wear

    out["current_pitstop"] = pit
    out["fresh_tyre_or_pitted"] = ((tyre <= 2) | (pit > 0)).astype("int8")
    out["pit_x_old_tyre"] = pit * tyre
    out["pit_x_progress"] = pit * progress
    out["stint_progress_pressure"] = stint * progress

    out["Year"] = out["Year"].astype("category")
    out["Stint"] = out["Stint"].astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000811"],
        "research_usage_note": "Verified assigned hypothesis 000811.",
    }, sort_keys=True))
