...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_year = X["Race"].astype(str) + "__" + X["Year"].astype(str)
    driver = X["Driver"].astype(str)
    race = X["Race"].astype(str)
    compound = X["Compound"].astype(str)
    stint = X["Stint"].astype(str)

    lap_bin = ((X["LapNumber"] - 1) // 5).astype("int16").astype(str)
    progress_bin = np.clip((X["RaceProgress"] * 10).astype("int16"), 0, 10).astype(str)

    X["_ry"] = race_year
    X["_driver"] = driver
    X["_race"] = race
    X["_driver_compound"] = driver + "__" + compound
    X["_driver_stint"] = driver + "__" + stint
    X["_race_compound"] = race + "__" + compound
    X["_race_lapbin"] = race + "__" + lap_bin
    X["_driver_progbin"] = driver + "__" + progress_bin

    y = X["PitStop"].astype(float)
    global_mean = float(y.mean())

    def heldout_smooth_mean(key_col: str, prior, alpha: float) -> pd.Series:
        tmp = pd.DataFrame(
            {
                "key": X[key_col].astype("category"),
                "ry": X["_ry"].astype("category"),
                "y": y,
            }
        )

        key_stats = tmp.groupby("key", observed=True)["y"].agg(["sum", "count"])
        pair_stats = tmp.groupby(["ry", "key"], observed=True)["y"].agg(
            ["sum", "count"]
        )

        key_sum = tmp["key"].map(key_stats["sum"]).astype("float64")
        key_cnt = tmp["key"].map(key_stats["count"]).astype("float64")

        pair_index = pd.MultiIndex.from_arrays([tmp["ry"], tmp["key"]])
        pair_sum = (
            pd.Series(pair_index.map(pair_stats["sum"]), index=tmp.index)
            .fillna(0.0)
            .astype("float64")
        )
        pair_cnt = pd.Series(
            pair_index.map(pair_stats["count"]), index=tmp.index
        ).fillna(0.0).astype("float64")

        excl_sum = key_sum - pair_sum
        excl_cnt = key_cnt - pair_cnt

        if np.isscalar(prior):
            prior_series = pd.Series(float(prior), index=tmp.index)
        else:
            prior_series = pd.Series(prior, index=tmp.index).astype(float)

        enc = (excl_sum + alpha * prior_series) / (excl_cnt + alpha)
        return enc.astype("float32")

    X["te_driver"] = heldout_smooth_mean("_driver", global_mean, alpha=50.0)
    X["te_race"] = heldout_smooth_mean("_race", global_mean, alpha=50.0)

    X["te_driver_compound"] = heldout_smooth_mean(
        "_driver_compound", X["te_driver"], alpha=25.0
    )
    X["te_driver_stint"] = heldout_smooth_mean(
        "_driver_stint", X["te_driver"], alpha=25.0
    )
    X["te_race_compound"] = heldout_smooth_mean(
        "_race_compound", X["te_race"], alpha=25.0
    )
    X["te_race_lapbin"] = heldout_smooth_mean("_race_lapbin", X["te_race"], alpha=25.0)
    X["te_driver_progbin"] = heldout_smooth_mean(
        "_driver_progbin", X["te_driver"], alpha=25.0
    )

    X["te_driver_compound_gap"] = (X["te_driver_compound"] - X["te_driver"]).astype(
        "float32"
    )
    X["te_driver_stint_gap"] = (X["te_driver_stint"] - X["te_driver"]).astype("float32")
    X["te_race_compound_gap"] = (X["te_race_compound"] - X["te_race"]).astype("float32")
    X["te_race_lapbin_gap"] = (X["te_race_lapbin"] - X["te_race"]).astype("float32")
    X["te_driver_progbin_gap"] = (X["te_driver_progbin"] - X["te_driver"]).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000793"],
        "research_usage_note": "Verified assigned hypothesis 000793.",
    }, sort_keys=True))
