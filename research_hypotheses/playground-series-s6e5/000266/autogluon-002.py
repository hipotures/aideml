...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = X.sort_values(grp_cols + ["LapNumber"]).reset_index(drop=False)
    X.rename(columns={"index": "_orig_index"}, inplace=True)

    X["compound_group"] = np.where(
        X["Compound"].isin(["INTERMEDIATE", "WET"]), "wet", "slick"
    )
    X["is_wet"] = (X["compound_group"] == "wet").astype("int8")
    X["is_slick"] = 1 - X["is_wet"]
    ...
    stop_proxy = (
        (X["Stint"] > prev_stint.fillna(X["Stint"]))
        | (X["TyreLife"] < (prev_tyre - 1).fillna(X["TyreLife"]))
    ).astype("int8")
    X["_stop_segment"] = stop_proxy.groupby([X[c] for c in grp_cols], sort=False).cumsum()
    X["laps_since_prev_pit"] = X.groupby(grp_cols + ["_stop_segment"], sort=False).cumcount()
    X["recent_pit_flag"] = (X["laps_since_prev_pit"] <= 2).astype("int8")
    X["state_reset_confidence"] = np.exp(-X["laps_since_prev_pit"] / 2.0) * (
        1.0 + 0.5 * stop_proxy
    )

    X["prev_laptime"] = g["LapTime (s)"].shift(1)
    X["prev_delta"] = g["LapTime_Delta"].shift(1)
    X["prev_deg"] = g["Cumulative_Degradation"].shift(1)
    X["prev_tyrelife"] = g["TyreLife"].shift(1)
    X["prev_position"] = g["Position"].shift(1)

    X["pace_residual"] = X["LapTime (s)"] - X["LapTime_Delta"]
    X["prev_pace_residual"] = g["pace_residual"].shift(1)

    X["deg_slope_1"] = (X["prev_deg"] - g["Cumulative_Degradation"].shift(2)) / (
        (X["prev_tyrelife"] - g["TyreLife"].shift(2)).replace(0, np.nan)
    )
    X["deg_slope_1"] = X["deg_slope_1"].fillna(0.0)

    X["ewm_pace_smooth"] = (
        g["prev_pace_residual"].transform(
            lambda s: s.ewm(alpha=0.18, adjust=False).mean()
        )
    ).fillna(0.0)
    X["ewm_pace_volatile"] = (
        g["prev_pace_residual"].transform(
            lambda s: s.ewm(alpha=0.45, adjust=False).mean()
        )
    ).fillna(0.0)
    X["ewm_deg_smooth"] = (
        g["prev_deg"].transform(lambda s: s.ewm(alpha=0.20, adjust=False).mean())
    ).fillna(0.0)
    X["ewm_deg_volatile"] = (
        g["prev_deg"].transform(lambda s: s.ewm(alpha=0.50, adjust=False).mean())
    ).fillna(0.0)

    X["pace_surprise_1"] = X["prev_pace_residual"] - X["ewm_pace_smooth"]
    X["deg_surprise_1"] = X["prev_deg"] - X["ewm_deg_smooth"]

    X["latent_wear_slick"] = (
        0.65 * X["ewm_deg_smooth"]
        + 0.35 * X["ewm_pace_smooth"]
        + 0.25 * X["deg_slope_1"]
    ) * X["is_slick"]
    X["latent_wear_wet"] = (
        0.55 * X["ewm_deg_volatile"]
        + 0.45 * X["ewm_pace_volatile"]
        + 0.20 * X["deg_slope_1"]
    ) * X["is_wet"]
    X["latent_wear_gap"] = X["latent_wear_volatile_minus_smooth"] = (
        X["ewm_pace_volatile"] - X["ewm_pace_smooth"]
    )

    X["post_reset_wear"] = (X["latent_wear_slick"] + X["latent_wear_wet"]) * X[
        "state_reset_confidence"
    ]
    X["stint_pressure"] = (X["TyreLife"] / X["LapNumber"].clip(lower=1)) * (
        1.0 - X["state_reset_confidence"]
    )
    X["position_risk"] = (X["Position"] - X["prev_position"]).fillna(0.0) + X[
        "Position_Change"
    ].fillna(0.0)

    X = X.sort_values("_orig_index").drop(columns=["_orig_index", "_stop_segment"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000266"],
        "research_usage_note": "Verified assigned hypothesis 000266.",
    }, sort_keys=True))
