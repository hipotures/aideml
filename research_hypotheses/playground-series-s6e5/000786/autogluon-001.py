...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(c in out.columns for c in key_cols):
        ...
        prior_stops = g["PitStop"].cumsum() - out["PitStop"]
        out["prior_stop_count"] = prior_stops.astype("int16")
        out["race_driver_lap_idx"] = g.cumcount().astype("int16") + 1
    else:
        out["prior_stop_count"] = 0
        out["race_driver_lap_idx"] = np.arange(len(out), dtype=np.int32) + 1

    out["stint_reset_id"] = out["Stint"].astype("int16")
    out["tyre_life_sq"] = (out["TyreLife"] ** 2).astype("float32")
    out["tyre_life_log1p"] = np.log1p(out["TyreLife"]).astype("float32")
    out["stint_x_tyre_life"] = (out["Stint"] * out["TyreLife"]).astype("float32")
    out["progress_x_tyre_life"] = (out["RaceProgress"] * out["TyreLife"]).astype(
        "float32"
    )
    out["progress_x_stint"] = (out["RaceProgress"] * out["Stint"]).astype("float32")
    out["prior_stops_x_tyre_life"] = (out["prior_stop_count"] * out["TyreLife"]).astype(
        "float32"
    )
    out["prior_stops_x_progress"] = (
        out["prior_stop_count"] * out["RaceProgress"]
    ).astype("float32")

    out["tyre_life_bin"] = pd.cut(
        out["TyreLife"],
        bins=[0, 5, 10, 15, 20, 30, 45, 100],
        labels=False,
        include_lowest=True,
    ).astype("int8")
    out["race_progress_bin"] = pd.cut(
        out["RaceProgress"],
        bins=[0, 0.15, 0.3, 0.45, 0.6, 0.75, 0.9, 1.01],
        labels=False,
        include_lowest=True,
    ).astype("int8")
    ...
    out["is_soft_compound"] = (comp == "SOFT").astype("int8")
    out["is_hard_compound"] = (comp == "HARD").astype("int8")
    out["wet_x_tyre_life"] = (out["is_wet_compound"] * out["TyreLife"]).astype(
        "float32"
    )
    out["soft_x_tyre_life"] = (out["is_soft_compound"] * out["TyreLife"]).astype(
        "float32"
    )
    out["hard_x_tyre_life"] = (out["is_hard_compound"] * out["TyreLife"]).astype(
        "float32"
    )

    out["late_stint_pressure"] = (
        out["TyreLife"] * out["RaceProgress"] * (1 + out["prior_stop_count"])
    ).astype("float32")
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"] + 1.0)
    ).astype("float32")
    out["lap_delta_x_tyre_life"] = (out["LapTime_Delta"] * out["TyreLife"]).astype(
        "float32"
    )
    out["position_loss_pressure"] = (
        np.maximum(out["Position_Change"], 0) * out["TyreLife"]
    ).astype("float32")
    ...
    out["hazard_cell"] = (
        ...
        + "_"
        + out["tyre_life_bin"].astype(str)
        + "_"
        + out["race_progress_bin"].astype(str)
        + "_"
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000786"],
        "research_usage_note": "Verified assigned hypothesis 000786.",
    }, sort_keys=True))
