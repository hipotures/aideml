...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Driver", "Race", "Compound", "Year", "Stint"]:
        freq = out.groupby(col)[col].transform("size").astype(np.float32)
        out[f"{col}_freq_log1p"] = np.log1p(freq).astype(np.float32)
        out[f"{col}_rarity"] = (1.0 / (1.0 + freq)).astype(np.float32)

    driver_count = out.groupby("Driver").size()
    race_count = out.groupby("Race").size()

    out["Driver_global_count"] = (
        out["Driver"].map(driver_count).fillna(0).astype(np.float32)
    )
    out["Race_global_count"] = (
        out["Race"].map(race_count).fillna(0).astype(np.float32)
    )
    out["Driver_rare_global"] = (out["Driver_global_count"] <= 2).astype(np.float32)
    ...
    shift_parts = np.column_stack(
        [
            out["Driver_rarity"].to_numpy(np.float32),
            out["Race_rarity"].to_numpy(np.float32),
            out["Compound_rarity"].to_numpy(np.float32),
            out["Year_rarity"].to_numpy(np.float32),
            wet_flag.to_numpy(np.float32),
            preseason_flag.to_numpy(np.float32),
            year_2025_flag.to_numpy(np.float32),
            out["Driver_rare_global"].to_numpy(np.float32),
            (1.0 / (1.0 + out["Driver_global_count"].to_numpy(np.float32))),
            (1.0 / (1.0 + out["Race_global_count"].to_numpy(np.float32))),
        ]
    )
