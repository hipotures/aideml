...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    n = len(out)
    n_test = 188165 if n > 188165 else 0

    for col in ["Compound", "Driver", "Race"]:
        out[col] = out[col].astype("string").fillna("NA")

    is_test_segment = np.zeros(n, dtype=np.float32)
    if n_test:
        is_test_segment[-n_test:] = 1.0
    out["_is_test_segment"] = is_test_segment

    wet_flag = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype(np.float32)
    preseason_flag = (out["Race"] == "Pre-Season Testing").astype(np.float32)
    year_2025_flag = (out["Year"] == 2025).astype(np.float32)

    for col in ["Driver", "Race", "Compound", "Year", "Stint"]:
        out[f"{col}_test_share"] = (
            out.groupby(col)["_is_test_segment"].transform("mean").astype(np.float32)
        )

    if n_test:
        train_part = out.iloc[:-n_test]
        driver_train_count = train_part.groupby("Driver").size()
        race_train_count = train_part.groupby("Race").size()
        train_drivers = set(train_part["Driver"].unique())
    else:
        driver_train_count = out.groupby("Driver").size()
        race_train_count = out.groupby("Race").size()
        train_drivers = set(out["Driver"].unique())

    out["Driver_train_count"] = (
        out["Driver"].map(driver_train_count).fillna(0).astype(np.float32)
    )
    out["Race_train_count"] = (
        out["Race"].map(race_train_count).fillna(0).astype(np.float32)
    )
    out["Driver_unseen_train"] = (~out["Driver"].isin(train_drivers)).astype(np.float32)
    out["Driver_rare_train"] = (out["Driver_train_count"] <= 2).astype(np.float32)

    out["TyreLife_by_Lap"] = (out["TyreLife"] / out["LapNumber"].clip(lower=1)).astype(
        np.float32
    )
    out["Degradation_by_TyreLife"] = (
        out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1)
    ).astype(np.float32)
    out["LapDelta_by_LapTime"] = (
        out["LapTime_Delta"] / out["LapTime (s)"].clip(lower=1)
    ).astype(np.float32)
    out["StintProgress"] = (
        out["TyreLife"]
        / (out["LapNumber"] - out["PitStop"].clip(lower=0) + 1).clip(lower=1)
    ).astype(np.float32)

    shift_parts = np.column_stack(
        [
            out["Driver_test_share"].to_numpy(np.float32),
            out["Race_test_share"].to_numpy(np.float32),
            out["Compound_test_share"].to_numpy(np.float32),
            out["Year_test_share"].to_numpy(np.float32),
            wet_flag.to_numpy(np.float32),
            preseason_flag.to_numpy(np.float32),
            year_2025_flag.to_numpy(np.float32),
            out["Driver_unseen_train"].to_numpy(np.float32),
            out["Driver_rare_train"].to_numpy(np.float32),
            (1.0 / (1.0 + out["Driver_train_count"].to_numpy(np.float32))),
        ]
    )
    out["adv_shift_score"] = shift_parts.mean(axis=1).astype(np.float32)

    q80 = float(np.quantile(out["adv_shift_score"], 0.80))
    q95 = float(np.quantile(out["adv_shift_score"], 0.95))
    out["gate_high_shift"] = (out["adv_shift_score"] >= q80).astype(np.float32)
    out["gate_extreme_shift"] = (out["adv_shift_score"] >= q95).astype(np.float32)
    out["gate_wet"] = wet_flag
    out["gate_2025"] = year_2025_flag
    out["gate_preseason"] = preseason_flag
    ...
    for base in gated_cols:
        base_vals = out[base].astype(np.float32)
        for gate in gates:
            out[f"{base}_x_{gate}"] = (base_vals * out[gate].astype(np.float32)).astype(
                np.float32
            )

    out["wet_high_shift"] = (out["gate_wet"] * out["gate_high_shift"]).astype(
        np.float32
    )
    out["year2025_high_shift"] = (out["gate_2025"] * out["gate_high_shift"]).astype(
        np.float32
    )
    out["preseason_high_shift"] = (
        out["gate_preseason"] * out["gate_high_shift"]
    ).astype(np.float32)

    out = out.drop(columns=["_is_test_segment"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000968.",
    }, sort_keys=True))
