...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Compound", "Driver", "Race"]:
        out[c] = out[c].astype("string").fillna("missing").str.upper().str.strip()
    ...
    progress = out["RaceProgress"].astype(float).clip(0, 1)
    ...
    max_lap_by_race_year = (
        out.groupby(["Year", "Race"])["LapNumber"].transform("max").astype(float)
    )
    max_lap_by_race = out.groupby("Race")["LapNumber"].transform("max").astype(float)
    est_total_laps = max_lap_by_race_year.where(
        max_lap_by_race_year > 1, max_lap_by_race
    ).clip(lower=1)

    out["clock_total_lap"] = lap
    out["clock_stint_lap"] = tyre
    out["clock_total_remaining"] = (est_total_laps - lap).clip(lower=0)
    out["clock_total_frac"] = (lap / est_total_laps).clip(0, 1.5)
    out["clock_stint_frac_of_race"] = (tyre / est_total_laps).clip(0, 1.5)
    out["clock_stint_x_total"] = tyre * out["clock_total_frac"]
    out["clock_stint_x_progress"] = tyre * progress
    out["clock_stint_per_stint_no"] = tyre / (stint + 1.0)
    out["clock_total_per_stint_no"] = lap / (stint + 1.0)
    ...
    expected_life = out["Compound"].map(compound_life).astype(float).fillna(24.0)
    out["compound_expected_life"] = expected_life
    out["tyre_life_minus_expected"] = tyre - expected_life
    out["tyre_life_ratio_expected"] = tyre / (expected_life + 1.0)
    out["surrogate_hazard_linear"] = out["tyre_life_ratio_expected"] * (0.5 + progress)
    out["surrogate_hazard_quadratic"] = np.square(
        out["tyre_life_ratio_expected"].clip(0, 4)
    )
    out["surrogate_hazard_late_race"] = out["surrogate_hazard_quadratic"] * (
        1.0 + progress
    )
    out["pit_window_soft"] = ((tyre >= 10) & (tyre <= 24)).astype("int8")
    out["pit_window_medium"] = ((tyre >= 15) & (tyre <= 34)).astype("int8")
    out["pit_window_hard"] = ((tyre >= 22) & (tyre <= 48)).astype("int8")
    out["beyond_expected_life"] = (tyre > expected_life).astype("int8")

    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["degradation_x_stint_clock"] = degr * np.log1p(tyre)
    out["lap_delta_x_stint_clock"] = delta * np.log1p(tyre)
    out["lap_time_per_total_clock"] = laptime / (lap + 1.0)
    out["position_pressure"] = (21.0 - pos).clip(1, 20)
    out["position_loss_flag"] = (poschg > 0).astype("int8")
    out["position_gain_flag"] = (poschg < 0).astype("int8")
    out["position_change_abs"] = poschg.abs()
    out["pitstop_prev_lap_context"] = out["PitStop"].astype("int8")

    for c in ["Compound", "Driver", "Race"]:
        freq = out[c].map(out[c].value_counts(dropna=False)).astype(float)
        out[c + "_freq"] = freq
        out[c + "_freq_log"] = np.log1p(freq)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000997.",
    }, sort_keys=True))
