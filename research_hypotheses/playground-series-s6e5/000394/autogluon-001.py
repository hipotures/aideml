...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["Compound"] = X["Compound"].astype("string").str.upper()
    X["Driver"] = X["Driver"].astype("string")
    X["Race"] = X["Race"].astype("string")

    X["WetRegime"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    race_progress = X["RaceProgress"].astype("float32").clip(lower=0.01)
    lap_number = X["LapNumber"].astype("float32")
    total_laps_est = (lap_number / race_progress).clip(lower=1.0, upper=90.0)
    laps_remaining_est = (total_laps_est - lap_number).clip(lower=0.0, upper=90.0)
    X["LapsRemaining_Est"] = laps_remaining_est.astype("float32")

    X["TyreLife_Bin"] = (
        np.floor(X["TyreLife"].astype("float32") / 2.0).clip(0, 40).astype("int16")
    )
    X["LapsRemaining_Bin"] = (
        np.floor(laps_remaining_est / 2.0).clip(0, 45).astype("int16")
    )
    X["RaceProgress_Bin"] = np.floor(race_progress * 20.0).clip(0, 19).astype("int8")
    X["Stint_Bin"] = X["Stint"].clip(1, 8).astype("int8")

    pit_obs = X["PitStop"].astype("float32").clip(0.0, 1.0)
    global_rate = float(pit_obs.mean())

    fold_key = pd.util.hash_pandas_object(
        X[["Driver", "Race", "Year", "LapNumber", "Stint"]], index=False
    ).astype("uint64")
    folds = (fold_key % 5).astype("int8")
    X["HazardFold"] = folds

    def _oof_smoothed_rate(keys, prior, strength):
        base_groupers = [X[k] for k in keys]
        fold_groupers = base_groupers + [folds]

        total_sum = pit_obs.groupby(base_groupers, dropna=False).transform("sum")
        total_cnt = pit_obs.groupby(base_groupers, dropna=False).transform("count")
        fold_sum = pit_obs.groupby(fold_groupers, dropna=False).transform("sum")
        fold_cnt = pit_obs.groupby(fold_groupers, dropna=False).transform("count")

        num = total_sum - fold_sum
        den = total_cnt - fold_cnt

        if np.isscalar(prior):
            prior_val = np.float32(prior)
        else:
            prior_val = prior.astype("float32")

        return ((num + strength * prior_val) / (den + strength)).astype("float32")

    hazard_regime_progress = _oof_smoothed_rate(
        ["WetRegime", "RaceProgress_Bin"], global_rate, 30.0
    )
    hazard_compound_phase = _oof_smoothed_rate(
        ["Compound", "WetRegime", "RaceProgress_Bin"], hazard_regime_progress, 20.0
    )
    hazard_stint_life = _oof_smoothed_rate(
        ["WetRegime", "Stint_Bin", "TyreLife_Bin"], hazard_regime_progress, 20.0
    )
    hazard_full = _oof_smoothed_rate(
        [
            "Compound",
            "WetRegime",
            "Stint_Bin",
        ...
        0.5 * (hazard_compound_phase + hazard_stint_life),
        12.0,
    )

    X["PitHazard_RegimeProgress"] = hazard_regime_progress
    X["PitHazard_CompoundPhase"] = hazard_compound_phase
    X["PitHazard_StintLife"] = hazard_stint_life
    X["PitHazard_Full"] = hazard_full

    X["PitHazard_RegimeProgress_DeltaGlobal"] = (
        hazard_regime_progress - global_rate
    ).astype("float32")
    X["PitHazard_CompoundPhase_DeltaRegime"] = (
        hazard_compound_phase - hazard_regime_progress
    ).astype("float32")
    X["PitHazard_StintLife_DeltaRegime"] = (
        hazard_stint_life - hazard_regime_progress
    ).astype("float32")
    X["PitHazard_Full_DeltaGlobal"] = (hazard_full - global_rate).astype("float32")
    X["PitHazard_Full_DeltaPhase"] = (hazard_full - hazard_compound_phase).astype(
        "float32"
    )
    X["PitHazard_Full_DeltaLife"] = (hazard_full - hazard_stint_life).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000394"],
        "research_usage_note": "Verified assigned hypothesis 000394.",
    }, sort_keys=True))
