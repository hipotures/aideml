...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fresh_mask = out["TyreLife"] <= 2
    worn_mask = out["TyreLife"] >= 8

    out["rc_fresh_lapdelta"] = (
        out["LapTime_Delta"]
        .where(fresh_mask)
        .groupby([out[k] for k in rc_keys])
        .transform("median")
    )
    out["rc_worn_lapdelta"] = (
        out["LapTime_Delta"]
        .where(worn_mask)
        .groupby([out[k] for k in rc_keys])
        .transform("median")
    ...
    out["rc_fresh_lapdelta"] = out["rc_fresh_lapdelta"].fillna(
        out.groupby("Compound")["LapTime_Delta"].transform("median")
    )
    out["rc_worn_lapdelta"] = out["rc_worn_lapdelta"].fillna(
        out.groupby("Compound")["LapTime_Delta"].transform("median")
    )
    out["rc_fresh_deg"] = out["rc_fresh_deg"].fillna(
        out.groupby("Compound")["Cumulative_Degradation"].transform("median")
    )
    out["rc_worn_deg"] = out["rc_worn_deg"].fillna(
        out.groupby("Compound")["Cumulative_Degradation"].transform("median")
    )

    deg_span = (out["rc_worn_deg"] - out["rc_fresh_deg"]).replace(0, np.nan)
    out["expected_warmup_loss"] = out["rc_fresh_lapdelta"]
    out["expected_undercut_payoff"] = out["rc_worn_lapdelta"] - out["rc_fresh_lapdelta"]
    out["deg_to_time_slope"] = (
        (out["expected_undercut_payoff"] / deg_span)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
    )
    out["current_wear_excess"] = out["Cumulative_Degradation"] - out["rc_fresh_deg"]
    out["pit_benefit_minus_warmup"] = (
        out["current_wear_excess"] * out["deg_to_time_slope"]
        - out["expected_warmup_loss"]
    )

    out["lap_median_time"] = out.groupby(lap_keys)["LapTime (s)"].transform("median")
    out["lap_median_delta"] = out.groupby(lap_keys)["LapTime_Delta"].transform("median")
    out["rel_lap_time"] = out["LapTime (s)"] - out["lap_median_time"]
    out["rel_lap_delta"] = out["LapTime_Delta"] - out["lap_median_delta"]

    out = out.sort_values(lap_keys + ["Position", "_row_id"], kind="mergesort")
    grp = out.groupby(lap_keys, sort=False)
    ...
    for col in [
        ...
        out[f"ahead_{col}"] = grp[col].shift(1)
        out[f"behind_{col}"] = grp[col].shift(-1)

    out["ahead_same_compound"] = (
        (out["Compound"] == out["ahead_Compound"]).astype("float32").fillna(0.0)
    )
    out["behind_same_compound"] = (
        (out["Compound"] == out["behind_Compound"]).astype("float32").fillna(0.0)
    )

    out["ahead_tyre_age_gap"] = (out["TyreLife"] - out["ahead_TyreLife"]).fillna(0.0)
    out["behind_tyre_age_gap"] = (out["TyreLife"] - out["behind_TyreLife"]).fillna(0.0)
    out["ahead_deg_gap"] = (
        out["Cumulative_Degradation"] - out["ahead_Cumulative_Degradation"]
    ).fillna(0.0)
    out["behind_deg_gap"] = (
        out["Cumulative_Degradation"] - out["behind_Cumulative_Degradation"]
    ).fillna(0.0)
    out["ahead_pace_gap"] = (out["rel_lap_time"] - out["ahead_rel_lap_time"]).fillna(
        0.0
    )
    out["behind_pace_gap"] = (out["rel_lap_time"] - out["behind_rel_lap_time"]).fillna(
        0.0
    )

    out["undercut_threat"] = (
        out["pit_benefit_minus_warmup"].clip(lower=0.0)
        * out["behind_tyre_age_gap"].clip(lower=0.0)
        * out["behind_pace_gap"].clip(lower=0.0)
        * (1.0 + out["behind_same_compound"])
    )

    out["overcut_viability"] = (
        out["ahead_tyre_age_gap"].clip(lower=0.0)
        * out["ahead_pace_gap"].clip(lower=0.0)
        * (1.0 + out["ahead_same_compound"])
        - out["expected_warmup_loss"]
    )

    out["pit_pressure_balance"] = out["undercut_threat"] - out["overcut_viability"]
    out["neighbor_strategy_pressure"] = (
        out["ahead_deg_gap"].clip(lower=0.0)
        + out["behind_pace_gap"].clip(lower=0.0)
        + out["behind_tyre_age_gap"].clip(lower=0.0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000900"],
        "research_usage_note": "Verified assigned hypothesis 000900.",
    }, sort_keys=True))
