...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_intermediate"] = (compound == "INTERMEDIATE").astype("int8")
    out["is_wet"] = (compound == "WET").astype("int8")

    keys = ["Race", "Year", "LapNumber"]
    shares = (
        out.groupby(keys, observed=True)[["is_slick", "is_intermediate", "is_wet"]]
        .mean()
        .rename(
            columns={
                "is_slick": "field_slick_share",
                "is_intermediate": "field_intermediate_share",
                "is_wet": "field_wet_share",
            }
        )
        .reset_index()
    ...
    for c in ["field_slick_share", "field_intermediate_share", "field_wet_share"]:
        shares[c + "_prev"] = shares.groupby(["Race", "Year"], observed=True)[c].shift(
            1
        )
        shares[c + "_delta"] = shares[c] - shares[c + "_prev"]

    shares = shares.fillna(0.0)
    out = out.merge(shares, on=keys, how="left", sort=False)

    share_cols = ["field_slick_share", "field_intermediate_share", "field_wet_share"]
    share_values = out[share_cols].to_numpy()
    majority_idx = np.argmax(share_values, axis=1)

    current_idx = np.select(
        [out["is_slick"].eq(1), out["is_intermediate"].eq(1), out["is_wet"].eq(1)],
        [0, 1, 2],
        default=-1,
    )
    current_share = np.choose(
        np.clip(current_idx, 0, 2),
        [
            out["field_slick_share"],
            out["field_intermediate_share"],
            out["field_wet_share"],
        ],
    )

    out["driver_on_minority_compound"] = (current_share < 0.34).astype("int8")
    out["compound_mismatch_to_field_majority"] = (current_idx != majority_idx).astype(
        "int8"
    )

    wet_gain = out["field_wet_share_delta"] + out["field_intermediate_share_delta"]
    slick_gain = out["field_slick_share_delta"]
    out["slick_to_wet_pressure"] = np.maximum(wet_gain, 0.0) * out["is_slick"]
    out["wet_to_slick_pressure"] = np.maximum(slick_gain, 0.0) * (
        out["is_wet"] + out["is_intermediate"]
    )

    out["slick_to_wet_pressure_progress"] = (
        out["slick_to_wet_pressure"] * out["RaceProgress"]
    )
    out["wet_to_slick_pressure_progress"] = (
        out["wet_to_slick_pressure"] * out["RaceProgress"]
    )
    out["slick_to_wet_pressure_position"] = (
        out["slick_to_wet_pressure"] * out["Position"]
    )
    out["wet_to_slick_pressure_position"] = (
        out["wet_to_slick_pressure"] * out["Position"]
    )
    out["minority_compound_progress"] = (
        out["driver_on_minority_compound"] * out["RaceProgress"]
    )
    out["mismatch_position"] = (
        out["compound_mismatch_to_field_majority"] * out["Position"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000514"],
        "research_usage_note": "Verified assigned hypothesis 000514.",
    }, sort_keys=True))
