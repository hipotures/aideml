...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp = out["Compound"].astype("string").str.upper()
    slick_bool = comp.isin(["SOFT", "MEDIUM", "HARD"])
    slick_num = slick_bool.astype("float32")

    lap_keys = ["Year", "Race", "LapNumber"]
    driver_keys = ["Year", "Race", "Driver"]
    lap_group = [out[k] for k in lap_keys]

    lap_slick_frac = (
        slick_num.groupby(lap_group, sort=False).transform("mean").astype("float32")
    )
    dry_rule_lap = slick_bool & (lap_slick_frac >= 0.65)

    out["is_slick_compound"] = slick_bool.astype("int8")
    out["field_lap_slick_frac"] = lap_slick_frac
    out["field_dry_rule_lap"] = dry_rule_lap.astype("int8")

    sdf = out.assign(
        __ord=np.arange(n),
        __compound_upper=comp.to_numpy(),
    ).sort_values(driver_keys + ["LapNumber", "__ord"], kind="mergesort")

    sorted_group = [sdf[k] for k in driver_keys]
    ord_idx = sdf["__ord"].to_numpy()
    pit_sorted = sdf["PitStop"].astype("float32")
    pit_cum_sorted = (
        pit_sorted.groupby(sorted_group, sort=False).cumsum().astype("float32")
    )
    prior_pit_sorted = pit_cum_sorted - pit_sorted

    prior_pits = np.empty(n, dtype="float32")
    pits_through = np.empty(n, dtype="float32")
    prior_pits[ord_idx] = prior_pit_sorted.to_numpy(dtype="float32")
    pits_through[ord_idx] = pit_cum_sorted.to_numpy(dtype="float32")

    seen_through_parts = []
    seen_prior_parts = []
    for c in ["SOFT", "MEDIUM", "HARD"]:
        flag = (sdf["__compound_upper"] == c).astype("int8")
        seen = flag.groupby(sorted_group, sort=False).cummax().astype("int8")
        seen_prior = (
            seen.groupby(sorted_group, sort=False).shift(1, fill_value=0).astype("int8")
        )
        seen_through_parts.append(seen.to_numpy(dtype="int8"))
        seen_prior_parts.append(seen_prior.to_numpy(dtype="int8"))

    slicks_through_sorted = np.sum(seen_through_parts, axis=0).astype("float32")
    prior_slicks_sorted = np.sum(seen_prior_parts, axis=0).astype("float32")

    slicks_through = np.empty(n, dtype="float32")
    prior_slicks = np.empty(n, dtype="float32")
    slicks_through[ord_idx] = slicks_through_sorted
    prior_slicks[ord_idx] = prior_slicks_sorted

    out["own_prior_pit_count"] = prior_pits
    out["own_pit_count_through_lap"] = pits_through
    out["own_prior_slick_compound_count"] = prior_slicks
    out["own_slick_compound_count_through_lap"] = slicks_through
    out["own_has_first_stop"] = (pits_through > 0).astype("int8")

    race_lower = out["Race"].astype("string").str.lower()
    monaco_rule = (
        race_lower.str.contains("monaco", na=False)
        & (out["Year"].astype("int16") >= 2025)
        & dry_rule_lap
    )
    normal_debt = dry_rule_lap & (slicks_through < 2)
    rule_debt = normal_debt | (monaco_rule & (pits_through < 2))

    out["own_mandatory_rule_debt"] = rule_debt.astype("int8")
    out["own_monaco_extra_stop_debt"] = (monaco_rule & (pits_through < 2)).astype(
        "int8"
    )

    lap_count = (
        out["LapNumber"]
        .groupby(lap_group, sort=False)
        .transform("count")
        .astype("float32")
    )
    denom_other = (lap_count - 1.0).clip(lower=1.0)
    dry_float = dry_rule_lap.astype("float32")

    debt_s = pd.Series(rule_debt.astype("float32"), index=out.index)
    first_s = pd.Series((pits_through > 0).astype("float32"), index=out.index)
    pit_s = out["PitStop"].astype("float32")

    debt_sum = debt_s.groupby(lap_group, sort=False).transform("sum")
    first_sum = first_s.groupby(lap_group, sort=False).transform("sum")
    pit_sum = pit_s.groupby(lap_group, sort=False).transform("sum")

    out["field_rule_debt_frac"] = (
        ((debt_sum - debt_s) / denom_other) * dry_float
    ).astype("float32")
    out["field_first_stop_frac"] = (
        ((first_sum - first_s) / denom_other) * dry_float
    ).astype("float32")
    out["field_pit_now_frac"] = (((pit_sum - pit_s) / denom_other) * dry_float).astype(
        "float32"
    )

    same_comp_group = [out["Year"], out["Race"], out["LapNumber"], comp]
    same_count = (
        out["TyreLife"]
        .groupby(same_comp_group, sort=False)
        .transform("count")
        .astype("float32")
    )
    same_rank = (
    ...
    out["field_same_comp_older_tyre_frac"] = (
        ((same_count - same_rank) / (same_count - 1.0).clip(lower=1.0)) * dry_float
    ).astype("float32")

    race_progress = out["RaceProgress"].astype("float32").clip(lower=0.01, upper=1.0)
    lap_number = out["LapNumber"].astype("float32")
    laps_left = ((lap_number / race_progress) - lap_number).clip(lower=0.0, upper=120.0)
    tyre_life = out["TyreLife"].astype("float32").clip(lower=0.0)

    out["laps_left_est"] = laps_left.astype("float32")
    out["tyre_age_if_finish_est"] = (tyre_life + laps_left).astype("float32")
    out["tyre_finish_pressure"] = ((tyre_life / (laps_left + 1.0)) * dry_float).astype(
        "float32"
    )

    field_cols = [
        "field_rule_debt_frac",
        "field_first_stop_frac",
        "field_same_comp_older_tyre_frac",
        "field_pit_now_frac",
    ]
    position = out["Position"].astype("float32")
    finish_pressure = out["tyre_finish_pressure"].astype("float32")

    for col in field_cols:
        base = out[col].astype("float32")
        out[f"{col}_x_prior_pits"] = (base * out["own_prior_pit_count"]).astype(
            "float32"
        )
        out[f"{col}_x_prior_slicks"] = (
            base * out["own_prior_slick_compound_count"]
        ).astype("float32")
        out[f"{col}_x_position"] = (base * position).astype("float32")
        out[f"{col}_x_finish_pressure"] = (base * finish_pressure).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000460"],
        "research_usage_note": "Verified assigned hypothesis 000460.",
    }, sort_keys=True))
