...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Compound", "Driver", "Race"]:
        ...
        freq = out[c].value_counts(dropna=False, normalize=True)
    ...
    out["late_race_old_tyre"] = out["RaceProgress"] * np.log1p(out["TyreLife"])
    ...
    out["is_hard_compound"] = (out["Compound"] == "HARD").astype("int8")
    ...
    for c in [
        ...
        if c in out.columns:
            shifted = out.groupby(grp, observed=True)[c].shift(1)
            out[c + "_prev_driver_race"] = shifted.astype("float32")
            out[c + "_diff_prev_driver_race"] = (out[c] - shifted).astype("float32")

    stint_grp = ["Year", "Race", "Driver", "Stint"]
    out["stint_lap_count_seen"] = (
        out.groupby(stint_grp, observed=True).cumcount().astype("float32")
    )
    out["driver_race_lap_count_seen"] = (
        out.groupby(grp, observed=True).cumcount().astype("float32")
    )

    for c in ["LapTime_Delta", "Position_Change", "Cumulative_Degradation"]:
        shifted = out.groupby(grp, observed=True)[c].shift(1)
        roll = shifted.groupby([out[k] for k in grp], observed=True).rolling(
            3, min_periods=1
        )
        out[c + "_prev3_mean"] = (
            roll.mean().reset_index(level=[0, 1, 2], drop=True).astype("float32")
        )
        out[c + "_prev3_std"] = (
            roll.std()
    ...
    out["compound_driver_freq"] = (
        out.groupby(["Compound", "Driver"], observed=True)["Driver"].transform("size")
        / len(out)
    ).astype("float32")
    out["race_compound_freq"] = (
        out.groupby(["Race", "Compound"], observed=True)["Compound"].transform("size")
        / len(out)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000698"],
        "research_usage_note": "Verified assigned hypothesis 000698.",
    }, sort_keys=True))
