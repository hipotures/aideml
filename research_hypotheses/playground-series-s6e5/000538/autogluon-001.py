...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = out["Compound"].astype(str).fillna("UNKNOWN")
    race = out["Race"].astype(str).fillna("UNKNOWN")

    is_wet = compound.isin(["WET", "INTERMEDIATE"]).astype("int8")
    is_dry = (~compound.isin(["WET", "INTERMEDIATE"])).astype("int8")
    is_testing = race.str.contains("Testing", case=False, regex=False).astype("int8")
    is_final_zone = ((out["RaceProgress"] >= 0.9) | (out["LapNumber"] <= 3)).astype(
        "int8"
    )
    high_rule_pressure = (
        is_dry
        ...
        & (
            ...
            >= out["TyreLife"]
            .clip(lower=1)
            .groupby([out["Race"], out["Year"]])
            .transform("median")
        )
    ).astype("int8")

    out["regime_dry_race"] = (is_dry & (1 - is_testing)).astype("int8")
    out["regime_wet"] = is_wet
    out["regime_testing"] = is_testing
    out["regime_final_zone"] = is_final_zone
    out["regime_rule_pressure"] = high_rule_pressure

    out["compound_family"] = np.select(
        [
            compound.eq("SOFT"),
            compound.eq("MEDIUM"),
            compound.eq("HARD"),
        ...
        ["slick_soft", "slick_medium", "slick_hard", "wet_intermediate", "wet_full"],
        default="other",
    )

    out["tyre_progress"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["degradation_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["lap_delta_per_tyre_lap"] = out["LapTime_Delta"] / out["TyreLife"].clip(lower=1)
    out["position_pressure"] = out["Position"] / 20.0
    out["stint_progress_proxy"] = out["TyreLife"] / (out["LapNumber"] + 1.0)

    gate_cols = {
        "dry": out["regime_dry_race"],
        "wet": out["regime_wet"],
        "testing": out["regime_testing"],
        "final": out["regime_final_zone"],
        "pressure": out["regime_rule_pressure"],
    }
    base_cols = [
        "TyreLife",
        ...
        "tyre_progress",
        "degradation_per_lap",
        "lap_delta_per_tyre_lap",
        "position_pressure",
        "stint_progress_proxy",
    ]

    for gate_name, gate in gate_cols.items():
        g = gate.astype("float32")
        for col in base_cols:
            out[f"{col}_x_{gate_name}"] = out[col].astype("float32") * g

    out["late_dry_long_stint"] = (
        out["regime_dry_race"].astype(bool)
        & (out["RaceProgress"] >= 0.7)
        & (out["TyreLife"] >= 15)
    ).astype("int8")

    out["wet_track_position_risk"] = (
        out["regime_wet"].astype("float32") * out["Position"].astype("float32") / 20.0
    )

    out["testing_longrun_flag"] = (
        out["regime_testing"].astype(bool)
        & (out["TyreLife"] >= out["TyreLife"].median())
    ).astype("int8")

    out["no_more_stop_proxy"] = (
        (out["RaceProgress"] >= 0.95) | ((out["LapNumber"] - out["TyreLife"]) <= 2)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000538.",
    }, sort_keys=True))
