...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver = out["Driver"].astype("category")
    ...
    race_progress = out["RaceProgress"].clip(0, 1)
    ...
    out["is_soft_compound"] = soft
    out["is_hard_compound"] = hard
    out["is_medium_compound"] = medium
    ...
    out["tyre_age_band"] = pd.cut(
        tyre_life,
        bins=[-0.001, 5, 12, 20, 35, 1000],
        labels=["fresh", "young", "used", "old", "very_old"],
    ).astype("category")

    out["regime_drywet_phase"] = (
        np.where(wet == 1, "wet_", "dry_") + out["race_phase"].astype(str)
    ).astype("category")
    out["regime_compound_phase"] = (
        out["Compound"].astype(str) + "_" + out["race_phase"].astype(str)
    ).astype("category")

    out["tyre_life_log1p"] = np.log1p(tyre_life)
    out["lap_number_log1p"] = np.log1p(lap_number)
    out["stint_log1p"] = np.log1p(stint)
    out["tyre_life_progress"] = tyre_life * race_progress
    out["stint_progress"] = stint * race_progress
    out["late_tyre_hazard"] = tyre_life * np.square(race_progress)
    out["wet_tyre_hazard"] = wet * tyre_life
    out["soft_tyre_hazard"] = soft * tyre_life
    out["pitstop_stint_interaction"] = out["PitStop"] * stint
    ...
    for col in [
        ...
        q_low = out[col].quantile(0.001)
        q_high = out[col].quantile(0.999)
        clipped = out[col].clip(q_low, q_high)
        out[col + "_clip"] = clipped
        out[col + "_abs"] = clipped.abs()
        out[col + "_sign"] = np.sign(clipped).astype("int8")

    out["position_inverse"] = 1.0 / out["Position"].clip(lower=1)
    out["position_x_progress"] = out["Position"] * race_progress
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / (tyre_life + 1.0)
    out["lap_delta_per_tyre_lap"] = out["LapTime_Delta"] / (tyre_life + 1.0)
    ...
    for col in [
        ...
        "Compound",
        "regime_drywet_phase",
        "regime_compound_phase",
    ]:
        freq = out[col].map(out[col].value_counts(normalize=True))
        out[col + "_freq"] = freq.astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000574"],
        "research_usage_note": "Verified assigned hypothesis 000574.",
    }, sort_keys=True))
