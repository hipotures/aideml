...
def train_and_predict(
    ...
    for fold, ((tr_idx, val_idx), (or_tr_idx, _or_val_idx)) in enumerate(
        ...
        print(
            f"{Fore.GREEN}{Style.BRIGHT}Fold {fold} | AUC Score: "
...
def main() -> None:
    ...
    oof_preds, test_preds = train_and_predict(
        X,
        y,
        X_test,
        orig,
    ...
    print("\n" + "=" * 24)
    print(
        f"Overall OOF AUC: {Fore.BLACK}{Style.BRIGHT}{oof_score:.5f}"
        f"{Style.RESET_ALL}"
    )
    print("=" * 24)
    ...
    print(
        ...
        + json.dumps(
            {
                "is_bug": False,
                "summary": "RealMLP/PyTabKit 5-fold OOF validation completed successfully.",
