...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    session_keys = ["Year", "Race"]

    compound = out["Compound"].astype(str).str.upper()
    out["compound_wet"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["compound_soft"] = (compound == "SOFT").astype("int8")
    out["compound_hard"] = (compound == "HARD").astype("int8")

    g_lap = out.groupby(race_keys, sort=False)
    g_session = out.groupby(session_keys, sort=False)

    lap_time = out["LapTime (s)"].astype(float)
    lap_delta = out["LapTime_Delta"].astype(float)
    deg = out["Cumulative_Degradation"].astype(float)

    out["race_lap_mean_laptime"] = g_lap["LapTime (s)"].transform("mean")
    out["race_lap_std_laptime"] = g_lap["LapTime (s)"].transform("std").fillna(0)
    out["race_lap_mean_delta"] = g_lap["LapTime_Delta"].transform("mean")
    out["race_lap_pit_rate"] = g_lap["PitStop"].transform("mean")
    out["race_lap_wet_rate"] = g_lap["compound_wet"].transform("mean")
    out["race_lap_mean_deg"] = g_lap["Cumulative_Degradation"].transform("mean")

    session_laptime_med = g_session["LapTime (s)"].transform("median")
    session_delta_med = g_session["LapTime_Delta"].transform("median")
    out["lap_time_vs_session_med"] = lap_time - session_laptime_med
    out["lap_delta_vs_session_med"] = lap_delta - session_delta_med

    lap_index = out.groupby(race_keys, sort=False).ngroup()
    lap_state = out[
        [
            *race_keys,
            "race_lap_mean_delta",
    ...
    ].drop_duplicates(race_keys)
    lap_state = lap_state.sort_values(session_keys + ["LapNumber"])

    lap_state["lap_mean_delta_prev"] = lap_state.groupby(session_keys, sort=False)[
        "race_lap_mean_delta"
    ].shift(1)
    lap_state["lap_wet_rate_prev"] = (
        lap_state.groupby(session_keys, sort=False)["race_lap_wet_rate"]
        .shift(1)
        .fillna(0)
    )
    lap_state["lap_deg_prev"] = lap_state.groupby(session_keys, sort=False)[
        "race_lap_mean_deg"
    ].shift(1)

    shock = (
        lap_state["race_lap_mean_delta"]
        > lap_state.groupby(session_keys, sort=False)["race_lap_mean_delta"].transform(
            "median"
        )
        + 8.0
    ) | (lap_state["race_lap_pit_rate"] >= 0.25)
    wet_transition = (
        lap_state["race_lap_wet_rate"] - lap_state["lap_wet_rate_prev"]
    ).abs() >= 0.15

    lap_state["neutralization_proxy"] = shock.astype("int8")
    lap_state["weather_transition_proxy"] = wet_transition.astype("int8")
    lap_state["lap_delta_jump"] = (
        lap_state["race_lap_mean_delta"] - lap_state["lap_mean_delta_prev"]
    ).fillna(0)
    lap_state["lap_deg_jump"] = (
        lap_state["race_lap_mean_deg"] - lap_state["lap_deg_prev"]
    ).fillna(0)

    flags = lap_state["neutralization_proxy"].to_numpy()
    laps = lap_state["LapNumber"].to_numpy()
    since = np.zeros(len(lap_state), dtype=np.float32)
    for _, idx in lap_state.groupby(session_keys, sort=False).indices.items():
        last = np.nan
        for pos in idx:
            if flags[pos]:
                last = laps[pos]
                ...
                since[pos] = 99.0 if np.isnan(last) else float(laps[pos] - last)

    lap_state["laps_since_neutralization_proxy"] = since
    join_cols = [
        *race_keys,
        "neutralization_proxy",
        "weather_transition_proxy",
        "lap_delta_jump",
        "lap_deg_jump",
        "laps_since_neutralization_proxy",
    ]
    out = out.merge(lap_state[join_cols], on=race_keys, how="left", sort=False)

    out["neutralized_pit_value_proxy"] = (
        out["neutralization_proxy"].astype(float)
        * (
            out["TyreLife"].astype(float)
            / (out["LapNumber"].astype(float).clip(lower=1))
        )
        * (1.0 + out["race_lap_pit_rate"].astype(float))
    )
    out["wet_tyre_mismatch_proxy"] = (
        (out["race_lap_wet_rate"] >= 0.2) & (out["compound_wet"] == 0)
    ).astype("int8")
    out["late_race_old_tyre"] = out["RaceProgress"].astype(float) * out[
        "TyreLife"
    ].astype(float)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000680"],
        "research_usage_note": "Verified assigned hypothesis 000680.",
    }, sort_keys=True))
