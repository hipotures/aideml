...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.copy()
    X["_order"] = np.arange(len(X))

    for col in ["Race", "Driver"]:
        X[col] = X[col].astype("string").str.strip()
    X["Compound"] = X["Compound"].astype("string").str.strip().str.upper()

    grp = ["Year", "Race", "Driver"]
    race_lap_grp = ["Year", "Race", "LapNumber"]

    X = X.sort_values(grp + ["LapNumber", "_order"], kind="mergesort")
    g = X.groupby(grp, sort=False)

    X["prev_compound"] = g["Compound"].shift(1)

    field_lap_median = X.groupby(race_lap_grp, sort=False)["LapTime (s)"].transform(
        "median"
    )
    X["lap_residual_vs_field"] = X["LapTime (s)"] - field_lap_median

    X["pit_position_loss"] = np.where(
        X["PitStop"].eq(1),
        (-X["Position_Change"]).clip(lower=0),
        np.nan,
    )
    X["pit_compound_changed"] = np.where(
        X["PitStop"].eq(1),
        X["Compound"].ne(X["prev_compound"]).astype("float32"),
        np.nan,
    )
    X["pit_lap_residual"] = np.where(
        X["PitStop"].eq(1),
        X["lap_residual_vs_field"],
        np.nan,
    )

    X["abnormal_stop_flag"] = (
        X["PitStop"].eq(1)
        & ((X["lap_residual_vs_field"] > 8.0) | ((-X["Position_Change"]) >= 3.0))
    ).astype("int8")
    X["pit_abnormal_flag"] = np.where(
        X["PitStop"].eq(1), X["abnormal_stop_flag"], np.nan
    )

    X["last_pit_lap"] = X["LapNumber"].where(X["PitStop"].eq(1))
    X["last_pit_lap"] = g["last_pit_lap"].ffill()
    X["laps_since_last_pit"] = (
        (X["LapNumber"] - X["last_pit_lap"]).fillna(-1).astype("float32")
    )

    for src, dst in [
        ("pit_lap_residual", "last_pit_lap_residual"),
        ("pit_position_loss", "last_pit_position_loss"),
        ("pit_compound_changed", "last_pit_compound_changed"),
        ("pit_abnormal_flag", "last_pit_abnormal_flag"),
    ]:
        X[dst] = g[src].ffill()

    X["last_abnormal_pit_lap"] = X["LapNumber"].where(X["abnormal_stop_flag"].eq(1))
    X["last_abnormal_pit_lap"] = g["last_abnormal_pit_lap"].ffill()
    X["laps_since_abnormal_stop"] = (
        (X["LapNumber"] - X["last_abnormal_pit_lap"]).fillna(-1).astype("float32")
    )

    X["stop_segment"] = g["PitStop"].cumsum()
    X["post_stop_slow_current"] = (
        X["laps_since_last_pit"].isin([1.0, 2.0]) & (X["lap_residual_vs_field"] > 5.0)
    ).astype("int8")

    seg_g = X.groupby(grp + ["stop_segment"], sort=False)
    X["post_stop_first2_slow_seen"] = (
        seg_g["post_stop_slow_current"].cummax().astype("int8")
    )
    X["post_stop_first2_slow_count_seen"] = (
        seg_g["post_stop_slow_current"].cumsum().clip(0, 2).astype("int8")
    )

    X["recent_abnormal_stop_window"] = X["last_pit_abnormal_flag"].fillna(0).astype(
        "float32"
    ) * X["laps_since_last_pit"].between(0, 5).astype("float32")

    X = X.sort_values("_order", kind="mergesort").drop(
        columns=[
            "_order",
            "prev_compound",
            "pit_position_loss",
            "pit_compound_changed",
            "pit_lap_residual",
            "pit_abnormal_flag",
            "last_pit_lap",
            "last_abnormal_pit_lap",
            "stop_segment",
            "post_stop_slow_current",
        ]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000488"],
        "research_usage_note": "Verified assigned hypothesis 000488.",
    }, sort_keys=True))
