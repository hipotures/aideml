...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        "Race" not in out.columns
        or "LapNumber" not in out.columns
        or "Position" not in out.columns
    ):
        return out

    sort_cols = ["Race", "LapNumber", "Position"]
    if "Year" in out.columns:
        sort_cols = ["Year"] + sort_cols

    s = out.sort_values(sort_cols, kind="mergesort").copy()

    group_cols = [c for c in ["Year", "Race", "LapNumber"] if c in s.columns]
    seq_cols = [c for c in ["Year", "Race", "Driver"] if c in s.columns]

    if "PitStop" in s.columns and seq_cols:
        s["_prev_pitstop_tmp"] = (
            s.sort_values(seq_cols + ["LapNumber"], kind="mergesort")
            .groupby(seq_cols, observed=True)["PitStop"]
            .shift(1)
            .fillna(0)
            .astype("int8")
        )
    else:
        s["_prev_pitstop_tmp"] = 0

    rival_base = [
        c
        for c in ["TyreLife", "Stint", "LapTime_Delta", "Position_Change", "PitStop"]
        if c in s.columns
    ]
    if "Compound" in s.columns:
        s["_compound_code_tmp"] = pd.Categorical(s["Compound"]).codes.astype("int16")
        rival_base.append("_compound_code_tmp")
    rival_base.append("_prev_pitstop_tmp")

    for k in (1, 2):
        for c in rival_base:
            public_name = (
                "Compound_code"
                if c == "_compound_code_tmp"
                else ("PrevPitStop" if c == "_prev_pitstop_tmp" else c)
            )
            s[f"ahead{k}_{public_name}"] = s.groupby(group_cols, observed=True)[
                c
            ].shift(k)
            s[f"behind{k}_{public_name}"] = s.groupby(group_cols, observed=True)[
                c
            ].shift(-k)

    if "TyreLife" in s.columns:
        for k in (1, 2):
            for side in ("ahead", "behind"):
                col = f"{side}{k}_TyreLife"
                s[f"{col}_diff"] = s["TyreLife"] - s[col]
                s[f"{col}_older_flag"] = (s[col] > s["TyreLife"]).astype("int8")
                s[f"{col}_newer_flag"] = (s[col] < s["TyreLife"]).astype("int8")

        s["_old_tyre_tmp"] = (s["TyreLife"] >= 18).astype("int8")
        if "Stint" in s.columns:
            s["_same_stint_old_tmp"] = s["_old_tyre_tmp"] * (
                s["Stint"]
                == s.groupby(group_cols, observed=True)["Stint"].transform("median")
            ).astype("int8")
        else:
            s["_same_stint_old_tmp"] = s["_old_tyre_tmp"]

        pos_rank = s.groupby(group_cols, observed=True).cumcount() + 1
        s["_top3_old_tmp"] = ((pos_rank <= 3) & (s["_same_stint_old_tmp"] == 1)).astype(
            "int8"
        )
        s["_top5_old_tmp"] = ((pos_rank <= 5) & (s["_same_stint_old_tmp"] == 1)).astype(
            "int8"
        )
        s["race_lap_top3_same_stint_old_tyre_count"] = (
            s.groupby(group_cols, observed=True)["_top3_old_tmp"]
            .transform("sum")
            .astype("int16")
        )
        s["race_lap_top5_same_stint_old_tyre_count"] = (
            s.groupby(group_cols, observed=True)["_top5_old_tmp"]
            .transform("sum")
            .astype("int16")
        )
    ...
    if "Stint" in s.columns:
        for k in (1, 2):
            for side in ("ahead", "behind"):
                col = f"{side}{k}_Stint"
                s[f"{col}_same_flag"] = (s[col] == s["Stint"]).astype("int8")
                s[f"{col}_diff"] = s["Stint"] - s[col]

    if "PitStop" in s.columns:
        pit_neighbor_cols = [
            c
            for c in s.columns
            if c.startswith(("ahead", "behind"))
            and (c.endswith("_PitStop") or c.endswith("_PrevPitStop"))
        ]
        if pit_neighbor_cols:
            s["nearby_rival_pit_pressure"] = (
                s[pit_neighbor_cols].fillna(0).sum(axis=1).astype("int16")
            )

    drop_tmp = [c for c in s.columns if c.endswith("_tmp")]
    s = s.drop(columns=drop_tmp, errors="ignore")
    out = s.reindex(orig_idx)
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000653"],
        "research_usage_note": "Verified assigned hypothesis 000653.",
    }, sort_keys=True))
