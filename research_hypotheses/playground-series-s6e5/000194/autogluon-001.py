...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["wet_flag"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    df["dry_flag"] = 1 - df["wet_flag"]

    rp = df["RaceProgress"].clip(0, 1)
    df["race_progress_bin"] = np.minimum((rp * 10).astype("int8"), 9)

    lap_key = ["Race", "Year", "LapNumber"]
    lap_size = df.groupby(lap_key)["Driver"].transform("size").astype("float32")
    df["lap_car_count"] = lap_size

    df["lap_stint_count"] = (
        df.groupby(lap_key + ["Stint"])["Driver"].transform("size").astype("float32")
    )
    df["lap_compound_count"] = (
        df.groupby(lap_key + ["Compound"])["Driver"].transform("size").astype("float32")
    )
    df["lap_wet_count"] = (
        df.groupby(lap_key + ["wet_flag"])["Driver"].transform("size").astype("float32")
    )
    df["lap_progress_bin_count"] = (
        df.groupby(lap_key + ["race_progress_bin"])["Driver"]
        .transform("size")
        .astype("float32")
    )
    df["lap_ctx_count"] = (
        df.groupby(lap_key + ["race_progress_bin", "Compound", "Stint", "wet_flag"])[
            "Driver"
        ]
        .transform("size")
        .astype("float32")
    ...
    denom = lap_size.replace(0, 1)
    df["lap_stint_share"] = (df["lap_stint_count"] / denom).astype("float32")
    df["lap_compound_share"] = (df["lap_compound_count"] / denom).astype("float32")
    df["lap_wet_share"] = (df["lap_wet_count"] / denom).astype("float32")
    df["lap_progress_bin_share"] = (df["lap_progress_bin_count"] / denom).astype(
        "float32"
    )
    df["lap_ctx_share"] = (df["lap_ctx_count"] / denom).astype("float32")

    df["tyre_life_frac"] = (df["TyreLife"] / df["LapNumber"].clip(lower=1)).astype(
        "float32"
    )
    df["stint_progress"] = (df["TyreLife"] / (df["LapNumber"] + 1)).astype("float32")
    df["deg_per_tyre_lap"] = (
        df["Cumulative_Degradation"] / df["TyreLife"].clip(lower=1)
    ).astype("float32")
    df["deg_x_tyre_life"] = (df["Cumulative_Degradation"] * df["TyreLife"]).astype(
        "float32"
    )
    df["deg_x_progress"] = (df["Cumulative_Degradation"] * df["RaceProgress"]).astype(
        "float32"
    )
    df["laptime_delta_x_progress"] = (df["LapTime_Delta"] * df["RaceProgress"]).astype(
        "float32"
    )
    df["position_pressure"] = ((21 - df["Position"]) / 20.0).astype("float32")
    df["pos_change_pressure"] = (df["Position_Change"].abs() / 20.0).astype("float32")

    df["ctx_pressure_score"] = (
        0.35 * df["lap_ctx_share"]
        + 0.20 * df["lap_compound_share"]
        + 0.20 * df["lap_stint_share"]
        + 0.15 * df["lap_progress_bin_share"]
        + 0.10 * df["lap_wet_share"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000194.",
    }, sort_keys=True))
