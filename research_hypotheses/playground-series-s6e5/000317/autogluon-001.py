...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_group = out.groupby(lap_key, sort=False)
    race_group = out.groupby(race_key, sort=False)

    # Race-level finish window state
    out["RaceMaxLap"] = race_group["LapNumber"].transform("max").astype(float)
    out["LapsRemaining"] = (out["RaceMaxLap"] - out["LapNumber"]).clip(lower=0)
    out["FinishWindow_5"] = (out["LapsRemaining"] <= 5).astype("int8")
    out["FinishWindow_10"] = (out["LapsRemaining"] <= 10).astype("int8")
    out["LateRace"] = (out["RaceProgress"] >= 0.8).astype("int8")
    out["VeryLateRace"] = (out["RaceProgress"] >= 0.9).astype("int8")

    # Field-relative lap features
    for col in num_cols:
        grp = lap_group[col]
        ...
        out[f"{col}_lap_diff_median"] = out[col] - med
        out[f"{col}_lap_diff_mean"] = out[col] - mean
        out[f"{col}_lap_zscore"] = (out[col] - mean) / std.replace(0, 1.0)
        out[f"{col}_lap_minmax"] = (out[col] - minv) / rng
        out[f"{col}_lap_pct"] = grp.rank(method="average", pct=True)

    # Stint-age ranking emphasis
    out["TyreLife_stint_rank"] = lap_group["TyreLife"].rank(method="average", pct=True)
    out["Degradation_rank"] = lap_group["Cumulative_Degradation"].rank(
        method="average", pct=True
    )
    out["Stint_rank"] = lap_group["Stint"].rank(method="average", pct=True)

    # Compound rarity on the current lap
    comp_cnt = (
        out.groupby(lap_key + ["Compound"], sort=False)["Compound"]
        .transform("size")
        .astype(float)
    )
    lap_cnt = lap_group["Compound"].transform("size").astype(float)
    out["Compound_lap_freq"] = comp_cnt / lap_cnt
    out["Compound_lap_rarity"] = 1.0 - out["Compound_lap_freq"]

    # Simple legality/strategy state proxies from available columns
    out["IsDryCompound"] = (
        out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    )
    out["IsWetCompound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["UsedMultipleCompounds"] = (out["Stint"] >= 2).astype("int8")
    out["ShortRunPressure"] = (out["TyreLife"] <= 3).astype("int8")
    out["LongRunPressure"] = (
        out["TyreLife"] >= lap_group["TyreLife"].transform("median")
    ).astype("int8")

    # Interaction terms consistent with within-lap ranking
    out["TyreLife_x_Degradation"] = out["TyreLife"] * out["Cumulative_Degradation"]
    out["LateRace_x_TyreLifePct"] = out["LateRace"] * out["TyreLife_lap_pct"]
    out["LateRace_x_DegradationRank"] = out["LateRace"] * out["Degradation_rank"]
    out["FinishWindow_x_Position"] = out["FinishWindow_10"] * out["Position_lap_pct"]

    # Use identifiers only for grouped feature construction, not as direct model inputs
    out = out.drop(columns=["Race", "Driver"], errors="ignore")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000317"],
        "research_usage_note": "Verified assigned hypothesis 000317.",
    }, sort_keys=True))
