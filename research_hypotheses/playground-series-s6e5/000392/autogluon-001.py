...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Compound", "Driver", "Race"]:
        ...
        out[col + "_freq"] = (
            out[col].map(out[col].value_counts(normalize=True)).astype("float32")
        )
    ...
    out["tyre_life_sq"] = (tyre * tyre).astype("float32")
    out["race_progress_sq"] = (progress * progress).astype("float32")
    out["lap_frac_from_end"] = (1.0 - progress).astype("float32")
    out["tyre_per_progress"] = (tyre / (progress + eps)).astype("float32")
    out["degradation_per_tyre"] = (degr / (tyre + eps)).astype("float32")
    out["delta_per_tyre"] = (delta / (tyre + eps)).astype("float32")
    out["laptime_ratio_to_lap"] = (laptime / (lap + eps)).astype("float32")
    out["position_pressure"] = ((21.0 - pos) / 20.0 + poschg.abs() / 18.0).astype(
        "float32"
    )

    out["late_stint_window"] = ((tyre >= 12) & (progress >= 0.25)).astype("int8")
    out["very_late_stint_window"] = ((tyre >= 20) & (progress >= 0.45)).astype("int8")
    out["hard_auc_window_000392"] = (
        ((tyre >= 10) & (tyre <= 45) & (progress >= 0.25) & (progress <= 0.95))
        | ((delta > 0) & (tyre >= 8) & (progress >= 0.2))
    ).astype("int8")

    out["hard_window_tyre"] = (tyre * out["hard_auc_window_000392"]).astype("float32")
    out["hard_window_progress"] = (progress * out["hard_auc_window_000392"]).astype(
        "float32"
    )
    out["hard_window_degradation"] = (degr * out["hard_auc_window_000392"]).astype(
        "float32"
    )
    out["hard_window_delta"] = (delta * out["hard_auc_window_000392"]).astype("float32")

    out["compound_tyre"] = (out["Compound_code"].astype("float32") * tyre).astype(
        "float32"
    )
    out["compound_progress"] = (
        out["Compound_code"].astype("float32") * progress
    ).astype("float32")
    out["stint_tyre"] = (stint * tyre).astype("float32")
    out["stint_progress"] = (stint * progress).astype("float32")
    out["tyre_progress_interaction"] = (tyre * progress).astype("float32")
    out["degradation_progress_interaction"] = (degr * progress).astype("float32")
    out["delta_progress_interaction"] = (delta * progress).astype("float32")

    group_cols = ["Race", "Year", "Compound", "Stint"]
    out["tyre_rank_in_window"] = (
        out.groupby(group_cols, sort=False)["TyreLife"].rank(pct=True).astype("float32")
    )
    out["progress_rank_in_window"] = (
        out.groupby(group_cols, sort=False)["RaceProgress"]
        .rank(pct=True)
        .astype("float32")
    )
    out["degradation_rank_in_window"] = (
        out.groupby(group_cols, sort=False)["Cumulative_Degradation"]
        .rank(pct=True)
        .astype("float32")
    )
    out["delta_rank_in_window"] = (
    ...
    race_year = ["Race", "Year"]
    out["tyre_vs_race_median"] = (
        tyre
        - out.groupby(race_year, sort=False)["TyreLife"]
        .transform("median")
        .astype("float32")
    ).astype("float32")
    out["degradation_vs_race_median"] = (
        degr
        - out.groupby(race_year, sort=False)["Cumulative_Degradation"]
        .transform("median")
        .astype("float32")
    ).astype("float32")
    out["delta_vs_race_median"] = (
        delta
        - out.groupby(race_year, sort=False)["LapTime_Delta"]
        .transform("median")
        .astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000392.",
    }, sort_keys=True))
