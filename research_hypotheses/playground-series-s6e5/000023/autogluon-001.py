...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    laps_remaining = (est_total_laps - lap).clip(lower=0, upper=90)
    denom = laps_remaining.clip(lower=1.0) + 0.5

    is_dry = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype(int)
    is_wet = compound.isin(["INTERMEDIATE", "WET"]).astype(int)
    ...
    current_tyre_can_finish = (expected_life - tyre_life >= laps_remaining).astype(int)
    next_stop_can_finish = (expected_life >= laps_remaining).astype(int)

    stop_debt = ((stint <= 1) & (progress < 0.92)).astype(int)
    compound_debt = ((is_dry == 1) & (stint <= 1) & (progress > 0.35)).astype(int)
    tyre_life_debt = np.maximum(
        0.0, tyre_life + laps_remaining - expected_life
    ) / expected_life.clip(lower=1.0)

    out["est_laps_remaining"] = laps_remaining
    out["laps_remaining_invalid_progress"] = (
        (out["RaceProgress"].astype(float) <= 0)
        | ~np.isfinite(out["RaceProgress"].astype(float))
    ).astype(int)

    out["stop_debt_urgency"] = stop_debt / denom
    out["compound_debt_urgency"] = compound_debt / denom
    out["tyre_life_debt_urgency"] = tyre_life_debt / denom

    out["few_laps_left_stop_debt"] = ((laps_remaining <= 8) & (stop_debt == 1)).astype(
        int
    )
    out["few_laps_left_compound_debt"] = (
        (laps_remaining <= 8) & (compound_debt == 1)
    ).astype(int)
    out["few_laps_left_tyre_debt"] = (
        (laps_remaining <= 8) & (tyre_life_debt > 0)
    ).astype(int)

    out["current_tyre_can_finish"] = current_tyre_can_finish
    out["next_stop_can_finish"] = next_stop_can_finish
    out["stop_urgency_x_current_cannot_finish"] = out["stop_debt_urgency"] * (
        1 - current_tyre_can_finish
    )
    out["compound_urgency_x_next_can_finish"] = (
        out["compound_debt_urgency"] * next_stop_can_finish
    )
    out["tyre_urgency_x_current_cannot_finish"] = out["tyre_life_debt_urgency"] * (
        1 - current_tyre_can_finish
    )
    out["dry_stop_urgency"] = out["stop_debt_urgency"] * is_dry
    out["wet_stop_urgency"] = out["stop_debt_urgency"] * is_wet
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000023.",
    }, sort_keys=True))
