...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g = out.groupby(query_cols, sort=False, observed=True)
    out["race_lap_size"] = g["LapNumber"].transform("size").astype("float32")

    for c in rank_cols:
        if c in out.columns:
            vals = out[c].astype("float32")
            mean = g[c].transform("mean").astype("float32")
            std = g[c].transform("std").replace(0, np.nan).astype("float32")
            out[f"{c}_race_lap_diff"] = (vals - mean).astype("float32")
            out[f"{c}_race_lap_z"] = ((vals - mean) / std).fillna(0).astype("float32")
            out[f"{c}_race_lap_rank_pct"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001015"],
        "research_usage_note": "Verified assigned hypothesis 001015.",
    }, sort_keys=True))
