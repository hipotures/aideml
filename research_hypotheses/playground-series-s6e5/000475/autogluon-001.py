...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out[keys + ["LapNumber", "PitStop", "Stint", "TyreLife"]].copy()
    work["_orig_idx"] = np.arange(len(work))
    work = work.sort_values(sort_cols + ["_orig_idx"], kind="mergesort")

    g = work.groupby(keys, sort=False)
    pit = work["PitStop"].fillna(0).astype("int8")
    prev_pit = g["PitStop"].shift(1).fillna(0).astype("int8")
    prev2_pit = g["PitStop"].shift(2).fillna(0).astype("int8")

    last_pit_lap = work["LapNumber"].where(prev_pit.eq(1))
    last_pit_lap = last_pit_lap.groupby([work[k] for k in keys], sort=False).ffill()
    laps_since_last_pit = (work["LapNumber"] - last_pit_lap).fillna(99).clip(0, 99)

    stint_start_lap = work["LapNumber"] - work["TyreLife"] + 1
    laps_until_current_stint_start = (work["LapNumber"] - stint_start_lap).clip(lower=0)

    long_stint_without_stop = (
        (laps_since_last_pit >= 18) & (work["TyreLife"] >= 18) & pit.eq(0)
    ).astype("int8")

    feat = pd.DataFrame(
        {
            "laps_since_last_pit": laps_since_last_pit.astype("float32"),
            "laps_until_current_stint_start": laps_until_current_stint_start.astype(
                "float32"
            ),
            "previous_lap_pitstop": prev_pit,
            "current_is_outlap_proxy": prev_pit,
            "first_2_laps_after_stop": ((prev_pit.eq(1)) | (prev2_pit.eq(1))).astype(
                "int8"
            ),
            "long_stint_without_stop": long_stint_without_stop,
        },
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000475.",
    }, sort_keys=True))
