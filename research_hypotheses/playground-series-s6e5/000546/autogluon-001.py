...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    interactions = {
        ...
        "Driver_Race_Compound": driver + "|" + race + "|" + compound,
    }
    for c, v in interactions.items():
        out[c] = v.astype("category")

    lap = out["LapNumber"].astype("float32")
    tyre = out["TyreLife"].astype("float32")
    stint_f = out["Stint"].astype("float32")
    ...
    out["TyreLife_per_Stint"] = tyre / (stint_f + 0.5)
    out["Lap_per_Stint"] = lap / (stint_f + 0.5)
    out["Degradation_per_TyreLife"] = degr / (tyre + 1.0)
    out["Abs_LapTime_Delta"] = np.abs(delta)
    out["LapTime_x_Progress"] = lap_time * progress
    out["Late_Stint_Tyre"] = tyre * (stint_f >= 2).astype("float32")
    out["PitWindow_Score"] = progress * np.log1p(tyre) * np.sqrt(stint_f)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000546.",
    }, sort_keys=True))
