...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_orig_order_rule_state"] = np.arange(n)
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "_orig_order_rule_state"]
    s = out.sort_values(sort_cols, kind="mergesort").copy()
    g = s.groupby(["Year", "Race", "Driver"], sort=False, observed=True)

    compound = s["Compound"].astype("string").str.upper()
    is_slick = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    is_wet_compound = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    ...
    for comp in ["SOFT", "MEDIUM", "HARD"]:
        ...
        s[f"rule_seen_{comp.lower()}"] = seen

    s["rule_slick_compounds_used"] = (
        s["rule_seen_soft"] + s["rule_seen_medium"] + s["rule_seen_hard"]
    ).astype("int8")
    s["rule_has_wet_exemption"] = (
        is_wet_compound.groupby([s["Year"], s["Race"], s["Driver"]], sort=False)
        .cummax()
        .astype("int8")
    )

    stint_changed = g["Stint"].diff().fillna(0).ne(0).astype("int8")
    s["rule_stop_count_so_far"] = (
        stint_changed.groupby([s["Year"], s["Race"], s["Driver"]], sort=False)
        .cumsum()
        .astype("int16")
    )
    s["rule_tyre_sets_used_so_far"] = (s["rule_stop_count_so_far"] + 1).astype("int16")

    is_monaco_2025 = s["Year"].eq(2025) & s["Race"].astype("string").str.contains(
        "Monaco", case=False, na=False
    )
    dry_rule_active = s["rule_has_wet_exemption"].eq(0)

    normal_debt = np.maximum(0, 2 - s["rule_slick_compounds_used"].to_numpy())
    monaco_stop_debt = np.maximum(0, 2 - s["rule_stop_count_so_far"].to_numpy())
    monaco_set_debt = np.maximum(0, 3 - s["rule_tyre_sets_used_so_far"].to_numpy())

    s["rule_normal_slick_spec_debt"] = np.where(dry_rule_active, normal_debt, 0).astype(
        "int8"
    )
    s["rule_monaco_2025"] = is_monaco_2025.astype("int8")
    s["rule_mandatory_stop_debt"] = np.where(
        is_monaco_2025 & dry_rule_active,
        monaco_stop_debt,
        np.maximum(0, s["rule_normal_slick_spec_debt"].to_numpy() - 1),
    ).astype("int8")
    s["needs_two_more_stops"] = (
        is_monaco_2025 & dry_rule_active & (s["rule_stop_count_so_far"] <= 0)
    ).astype("int8")
    s["needs_one_more_stop"] = (
        is_monaco_2025 & dry_rule_active & (s["rule_stop_count_so_far"] == 1)
    ).astype("int8")
    s["rule_monaco_set_debt"] = np.where(
        is_monaco_2025 & dry_rule_active, monaco_set_debt, 0
    ).astype("int8")

    new_slick_now = is_slick.eq(1) & (
        ((compound.eq("SOFT")) & s["rule_seen_soft"].eq(0))
        | ((compound.eq("MEDIUM")) & s["rule_seen_medium"].eq(0))
        | ((compound.eq("HARD")) & s["rule_seen_hard"].eq(0))
    )
    s["current_stop_would_complete_rule"] = (
        dry_rule_active
        & (
            (
                is_monaco_2025
                & (s["rule_stop_count_so_far"] >= 1)
                & (s["rule_tyre_sets_used_so_far"] >= 2)
            )
            | (~is_monaco_2025 & (s["rule_slick_compounds_used"] >= 1) & new_slick_now)
        )
    ).astype("int8")

    s["rule_laps_remaining_est"] = (
        (s["LapNumber"] / s["RaceProgress"].clip(lower=0.01) - s["LapNumber"])
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .clip(lower=0, upper=80)
        .astype("float32")
    )
    s["rule_debt_pressure"] = (
        s["rule_mandatory_stop_debt"] / (s["rule_laps_remaining_est"] + 1.0)
    ).astype("float32")
    ...
    added = [
        ...
        "rule_seen_medium",
        "rule_seen_hard",
        "rule_slick_compounds_used",
        "rule_has_wet_exemption",
        "rule_stop_count_so_far",
        "rule_tyre_sets_used_so_far",
        "rule_normal_slick_spec_debt",
        "rule_monaco_2025",
        "rule_mandatory_stop_debt",
        "needs_two_more_stops",
        "needs_one_more_stop",
        "rule_monaco_set_debt",
        "current_stop_would_complete_rule",
        "rule_laps_remaining_est",
        "rule_debt_pressure",
    ]
    out = s.sort_values("_orig_order_rule_state", kind="mergesort").set_index(idx)
    out = out.drop(columns=["_orig_order_rule_state"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000430"],
        "research_usage_note": "Verified assigned hypothesis 000430.",
    }, sort_keys=True))
