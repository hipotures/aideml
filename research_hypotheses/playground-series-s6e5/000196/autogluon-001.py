...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ahead_pos = g["Position"].shift(1)
    behind_pos = g["Position"].shift(-1)

    out["rival_has_ahead"] = (
        ahead_pos.notna().reindex(out.index).fillna(False).astype("int8")
    )
    out["rival_has_behind"] = (
        behind_pos.notna().reindex(out.index).fillna(False).astype("int8")
    )
    out["rival_pos_gap_ahead"] = (
        (s["Position"] - ahead_pos).reindex(out.index).fillna(0).astype("float32")
    )
    out["rival_pos_gap_behind"] = (
        (behind_pos - s["Position"]).reindex(out.index).fillna(0).astype("float32")
    )
    ...
    for col, name in [
        ...
        out[f"rival_{name}_gap_ahead"] = (
            (s[col] - ahead).reindex(out.index).fillna(0).astype("float32")
        )
        out[f"rival_{name}_gap_behind"] = (
            (s[col] - behind).reindex(out.index).fillna(0).astype("float32")
        )

    tmp = out[keys + ["PitStop", "Position_Change"]].copy()
    tmp["_abs_position_change"] = tmp["Position_Change"].abs()
    lap_stats = (
        tmp.groupby(keys, sort=False)
        .agg(
            field_pit_rate=("PitStop", "mean"),
            field_pit_count=("PitStop", "sum"),
            field_position_churn=("_abs_position_change", "mean"),
        )
        .reset_index()
    )

    lap_stats = lap_stats.sort_values(keys, kind="mergesort")
    rg = lap_stats.groupby(["Race", "Year"], sort=False)

    for n in (1, 2, 3):
        lap_stats[f"field_pit_rate_lag{n}"] = rg["field_pit_rate"].shift(n)
    ...
    valid_lags = sum(lap_stats[c].notna().astype("int8") for c in lag_rate_cols)
    lag_sum = sum(lap_stats[c].fillna(0.0) for c in lag_rate_cols)
    lap_stats["field_pit_rate_last3"] = np.where(
        valid_lags > 0, lag_sum / valid_lags, 0.0
    )

    lap_stats["field_pit_count_lag1"] = rg["field_pit_count"].shift(1)
    lap_stats["field_position_churn_lag1"] = rg["field_position_churn"].shift(1)
    lap_stats["overtake_difficulty_lag1"] = 1.0 / (
        1.0 + lap_stats["field_position_churn_lag1"].fillna(0.0)
    )

    stat_cols = lag_rate_cols + [
        "field_pit_rate_last3",
        "field_pit_count_lag1",
        "field_position_churn_lag1",
        "overtake_difficulty_lag1",
    ]
    out = out.merge(lap_stats[keys + stat_cols], on=keys, how="left", sort=False)
    ...
    close_threshold = 2.0 + 0.03 * tyre.clip(0, 50) + 3.0 * out["field_pit_rate_last3"]

    race_phase = out["RaceProgress"].between(0.10, 0.94)
    tyre_ready = tyre.ge(6)
    not_pitting_now = out["PitStop"].eq(0)

    close_ahead = (
        out["rival_has_ahead"].eq(1)
        & out["rival_pos_gap_ahead"].between(0.5, 2.5)
        & out["rival_lap_delta_gap_ahead"].abs().le(close_threshold)
    )
    close_behind = (
        out["rival_has_behind"].eq(1)
        & out["rival_pos_gap_behind"].between(0.5, 2.5)
        & out["rival_lap_delta_gap_behind"].abs().le(close_threshold)
    )

    out["undercut_range_ahead"] = close_ahead.astype("int8")
    out["undercut_range_behind"] = close_behind.astype("int8")
    out["undercut_window_ahead"] = (
        close_ahead & tyre_ready & race_phase & not_pitting_now
    ).astype("int8")
    out["undercut_window_behind"] = (
        close_behind & tyre_ready & race_phase & not_pitting_now
    ).astype("int8")

    front_position = ((21.0 - pos).clip(0, 20) / 20.0).astype("float32")
    midfield_density = (1.0 - ((pos - 10.5).abs() / 9.5)).clip(0, 1).astype("float32")
    tyre_age_norm = (tyre.clip(0, 60) / 60.0).astype("float32")

    out["clear_air_opportunity_proxy"] = (
        out["field_pit_rate_last3"]
        * front_position
        * (1.0 + out["overtake_difficulty_lag1"])
    ).astype("float32")
    out["recent_traffic_window_proxy"] = (
        out["field_pit_rate_last3"] * midfield_density * tyre_age_norm
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000196"],
        "research_usage_note": "Verified assigned hypothesis 000196.",
    }, sort_keys=True))
