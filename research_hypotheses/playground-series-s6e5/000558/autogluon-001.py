...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    train = out.iloc[:n_train] if n_train > 0 else out
    ...
    base_lap = train.groupby(race_keys)["LapTime (s)"].median()
    pit_lap = (
        train.loc[train["PitStop"].eq(1)].groupby(race_keys)["LapTime (s)"].median()
    )
    ...
    fresh = (
        train.loc[train["TyreLife"].le(2)].groupby(race_keys)["LapTime_Delta"].median()
    )
    settled = (
        train.loc[train["TyreLife"].between(4, 8)]
