...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = work.sort_values(order_cols + ["_idx"], kind="mergesort")

    g = work.groupby(keys, sort=False, observed=True)

    d_lap_delta = g["LapTime_Delta"].diff() / g["LapNumber"].diff().replace(0, np.nan)
    d_cum_age = work["Cumulative_Degradation"] / work["TyreLife"].clip(lower=1)
    d_cum_delta = g["Cumulative_Degradation"].diff() / g["LapNumber"].diff().replace(
        0, np.nan
    )

    d_raw = (
        0.50 * d_cum_age.replace([np.inf, -np.inf], np.nan)
        + 0.30 * d_cum_delta.replace([np.inf, -np.inf], np.nan)
        + 0.20 * d_lap_delta.replace([np.inf, -np.inf], np.nan)
    )
    d_raw = d_raw.clip(-5.0, 20.0)

    d_recent = d_raw.groupby(
        [work[c] for c in keys], sort=False, observed=True
    ).transform(lambda s: s.shift(1).rolling(4, min_periods=1).mean())

    compound_prior = d_raw.groupby(work["Compound"], observed=True).transform("median")
    race_compound_prior = d_raw.groupby(
        [work["Race"], work["Compound"]], observed=True
    ).transform("median")
    global_prior = float(np.nanmedian(d_raw.to_numpy()))
    if not np.isfinite(global_prior) or global_prior <= 0:
        global_prior = 0.35

    d_hat = (
        d_recent.fillna(race_compound_prior).fillna(compound_prior).fillna(global_prior)
    )
    d_hat = d_hat.clip(lower=0.03, upper=10.0)

    pit_loss_prior = 22.0
    opt_len = np.sqrt((2.0 * pit_loss_prior) / d_hat).clip(3.0, 45.0)
    age = work["TyreLife"].astype(float)
    laps_remaining_est = (
    ...
    features["DegSlope_Est"] = d_hat.astype("float32")
    features["OptimalStintLen_Est"] = opt_len.astype("float32")
    features["TyreLifeMinusOptimal"] = (age - opt_len).astype("float32")
    features["TyreLifeOverOptimal"] = (age / opt_len.clip(lower=1.0)).astype("float32")
    features["OptimalStopPressure"] = (
        1.0 / (1.0 + np.exp(-(age - opt_len) / 3.0))
    ).astype("float32")
    features["LapsRemainingMinusOptimalFreshWindow"] = (
        laps_remaining_est - opt_len
    ).astype("float32")
    features["OptimalPressure_x_RaceProgress"] = (
        features["OptimalStopPressure"] * work["RaceProgress"]
    ).astype("float32")

    features["_idx"] = work["_idx"].to_numpy()
    features = features.sort_values("_idx", kind="mergesort").drop(columns="_idx")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000240"],
        "research_usage_note": "Verified assigned hypothesis 000240.",
    }, sort_keys=True))
