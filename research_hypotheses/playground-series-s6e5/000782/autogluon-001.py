...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").str.strip().str.upper().fillna("MISSING")
    compound = (
        out["Compound"].astype("string").str.strip().str.upper().fillna("MISSING")
    )
    driver = out["Driver"].astype("string").str.strip().str.upper().fillna("MISSING")

    out["Race"] = race.astype("category")
    out["Compound"] = compound.astype("category")
    out["Driver"] = driver.astype("category")

    year = pd.to_numeric(out["Year"], errors="coerce").fillna(0)
    progress = pd.to_numeric(out["RaceProgress"], errors="coerce").fillna(0).clip(0, 1)
    tyre_life = pd.to_numeric(out["TyreLife"], errors="coerce").fillna(0)
    degradation = pd.to_numeric(out["Cumulative_Degradation"], errors="coerce").fillna(
        0
    )
    lap_delta = pd.to_numeric(out["LapTime_Delta"], errors="coerce").fillna(0)
    lap_time = pd.to_numeric(out["LapTime (s)"], errors="coerce").fillna(0)
    pit_now = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0)

    out["g_wet"] = compound.isin(["INTERMEDIATE", "WET"]).astype(float)
    out["g_dry"] = 1.0 - out["g_wet"]
    out["g_monaco_2025"] = (
        race.str.contains("MONACO", regex=False) & (year == 2025)
    ).astype(float)
    out["g_preseason"] = race.str.contains("PRE-SEASON", regex=False).astype(float)

    out["g_early_race"] = ((0.45 - progress) / 0.45).clip(0, 1)
    out["g_mid_race"] = (1.0 - (progress - 0.50).abs() / 0.35).clip(0, 1)
    out["g_late_race"] = ((progress - 0.55) / 0.45).clip(0, 1)

    tyre_rank = tyre_life.groupby(compound).rank(pct=True).fillna(0.5)
    deg_rank = degradation.clip(lower=0).groupby(compound).rank(pct=True).fillna(0.5)
    delta_rank = lap_delta.clip(lower=0).rank(pct=True).fillna(0.5)
    stress = (0.40 * tyre_rank + 0.40 * deg_rank + 0.20 * delta_rank).clip(0, 1)
    out["tyre_stress_score"] = stress
    out["g_high_tyre_stress"] = stress
    out["g_low_tyre_stress"] = 1.0 - stress

    pit_mask = pit_now > 0.5
    global_pit_loss = lap_time[pit_mask].median() - lap_time[~pit_mask].median()
    if pd.isna(global_pit_loss):
        global_pit_loss = 0.0

    pit_med = lap_time[pit_mask].groupby(race[pit_mask]).median()
    nonpit_med = lap_time[~pit_mask].groupby(race[~pit_mask]).median()
    pit_loss_by_race = (pit_med - nonpit_med).dropna()

    if len(pit_loss_by_race):
        race_pit_loss = race.map(pit_loss_by_race).astype(float).fillna(global_pit_loss)
        pit_loss_rank = pit_loss_by_race.rank(pct=True)
        high_pit_loss = race.map(pit_loss_rank).astype(float).fillna(0.5).clip(0, 1)
    else:
        race_pit_loss = pd.Series(global_pit_loss, index=out.index, dtype=float)
        high_pit_loss = pd.Series(0.5, index=out.index, dtype=float)

    out["race_pit_loss_est"] = race_pit_loss
    out["g_high_pit_loss"] = high_pit_loss
    out["g_low_pit_loss"] = 1.0 - high_pit_loss

    out["dro_wet_late"] = out["g_wet"] * out["g_late_race"]
    out["dro_dry_high_stress"] = out["g_dry"] * out["g_high_tyre_stress"]
    out["dro_late_high_stress"] = out["g_late_race"] * out["g_high_tyre_stress"]
    out["dro_high_pit_loss_late"] = out["g_high_pit_loss"] * out["g_late_race"]
    out["dro_high_pit_loss_stress"] = out["g_high_pit_loss"] * out["g_high_tyre_stress"]
    out["dro_monaco2025_late"] = out["g_monaco_2025"] * out["g_late_race"]
    out["dro_preseason_lowstress"] = out["g_preseason"] * out["g_low_tyre_stress"]
    ...
    for col in group_cols:
        mean_membership = float(out[col].mean()) + eps
        rarity = rarity + out[col] / mean_membership
    out["soft_dro_rarity_proxy"] = rarity / len(group_cols)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000782"],
        "research_usage_note": "Verified assigned hypothesis 000782.",
    }, sort_keys=True))
