...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["TyreLife_per_Lap"] = tyre / lap.clip(lower=1)
    out["TyreLife_per_Stint"] = tyre / stint
    out["Lap_in_Stint_Ratio"] = tyre / (lap + 1.0)
    out["RemainingRace"] = 1.0 - progress
    out["LateRace_TyreLife"] = tyre * out["RemainingRace"]

    out["Abs_LapTime_Delta"] = out["LapTime_Delta"].astype(float).abs()
    out["Abs_Position_Change"] = out["Position_Change"].astype(float).abs()
    out["Degradation_per_TyreLife"] = out["Cumulative_Degradation"].astype(
        float
    ) / tyre.clip(lower=1)
    out["LapTime_per_TyreLife"] = out["LapTime (s)"].astype(float) / tyre.clip(lower=1)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000918"],
        "research_usage_note": "Verified assigned hypothesis 000918.",
    }, sort_keys=True))
