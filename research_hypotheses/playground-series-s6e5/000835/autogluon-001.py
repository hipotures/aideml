...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Race", "Driver", "Compound"]:
        if col in out:
            ...
            out[col + "_freq"] = out[col].map(freq).astype("float32")
    ...
    if {"TyreLife", "RaceProgress"}.issubset(out.columns):
        ...
        out["TyreLife_per_Progress"] = (
            out["TyreLife"] / (out["RaceProgress"] + eps)
        ).astype("float32")
    ...
    if {"TyreLife", "Stint"}.issubset(out.columns):
        ...
        out["LateStintAge"] = (out["TyreLife"] >= 18).astype("int8")
        out["VeryLateStintAge"] = (out["TyreLife"] >= 28).astype("int8")

    if {"LapNumber", "TyreLife"}.issubset(out.columns):
        out["LapMinusTyreLife"] = (out["LapNumber"] - out["TyreLife"]).astype("float32")
        out["TyreLifeShareOfLap"] = (out["TyreLife"] / (out["LapNumber"] + eps)).astype(
            "float32"
        )
    ...
    if {"LapTime (s)", "LapTime_Delta"}.issubset(out.columns):
        ...
        out["RelLapTimeDelta"] = (
            out["LapTime_Delta"] / (out["LapTime (s)"] + eps)
        ).astype("float32")

    if {"Cumulative_Degradation", "TyreLife"}.issubset(out.columns):
        out["DegradationPerTyreLap"] = (
            out["Cumulative_Degradation"] / (out["TyreLife"] + eps)
        ).astype("float32")
        out["AbsDegradation"] = out["Cumulative_Degradation"].abs().astype("float32")

    if {"Position", "Position_Change"}.issubset(out.columns):
        out["AbsPositionChange"] = out["Position_Change"].abs().astype("float32")
        out["PositionAfterChange"] = (out["Position"] + out["Position_Change"]).astype(
            "float32"
        )
        out["FrontRunner"] = (out["Position"] <= 5).astype("int8")
        out["BackMarker"] = (out["Position"] >= 16).astype("int8")

    if {"RaceProgress", "Stint"}.issubset(out.columns):
        out["LateRace"] = (out["RaceProgress"] >= 0.70).astype("int8")
        out["EarlyRace"] = (out["RaceProgress"] <= 0.20).astype("int8")
        out["Stint_x_Progress"] = (out["Stint"] * out["RaceProgress"]).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000835.",
    }, sort_keys=True))
