...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    total_laps_est = (lap / progress).clip(lower=lap, upper=110)
    laps_remaining = (total_laps_est - lap).clip(lower=0, upper=80)
    laps_after_next_stop = (laps_remaining - 1).clip(lower=0, upper=80)
    ...
    current_finish_life = comp.map(life_map).fillna(34.0).astype(float)

    tyre_life = out["TyreLife"].astype(float)
    current_finish_margin = current_finish_life - (tyre_life + laps_remaining)
    current_finish_pressure = -current_finish_margin

    best_fresh_slick_life = 48.0
    fresh_slick_finish_margin = best_fresh_slick_life - laps_after_next_stop

    current_fails = current_finish_margin < 0
    fresh_slick_can_finish = fresh_slick_finish_margin >= 0
    late_race = laps_remaining <= 28

    out["est_total_laps"] = total_laps_est
    out["est_laps_remaining"] = laps_remaining
    out["current_finish_margin"] = current_finish_margin
    out["current_finish_pressure"] = current_finish_pressure
    out["fresh_slick_finish_margin_after_next"] = fresh_slick_finish_margin
    out["current_fails_fresh_slick_can_finish"] = (
        current_fails & fresh_slick_can_finish
    ).astype("int8")
    out["pressure_x_fresh_margin"] = current_finish_pressure * fresh_slick_finish_margin
    out["positive_pressure_x_fresh_margin"] = current_finish_pressure.clip(
        lower=0
    ) * fresh_slick_finish_margin.clip(lower=0)
    out["late_current_fails_fresh_slick_can_finish"] = (
        late_race & current_fails & fresh_slick_can_finish
    ).astype("int8")
    out["late_pressure_x_fresh_margin"] = out[
        "positive_pressure_x_fresh_margin"
    ] * late_race.astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000017"],
        "research_usage_note": "Verified assigned hypothesis 000017.",
    }, sort_keys=True))
