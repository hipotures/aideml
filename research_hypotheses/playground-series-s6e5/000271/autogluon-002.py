...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out.sort_values(key_cols).copy()
    grp = work.groupby(["Year", "Race", "Driver"], sort=False)

    # Survival-consistent age within the current driver-race history.
    work["laps_since_start"] = grp.cumcount() + 1
    prev_stint = grp["Stint"].shift(1)
    prev_tyre = grp["TyreLife"].shift(1)
    stop_proxy = (
        (work["Stint"] > prev_stint.fillna(work["Stint"]))
        | (work["TyreLife"] < (prev_tyre - 1).fillna(work["TyreLife"]))
    ).astype("int8")
    stop_segment = stop_proxy.groupby(
        [work["Year"], work["Race"], work["Driver"]], sort=False
    ).cumsum()
    work["laps_since_last_stop"] = work.groupby(
        [work["Year"], work["Race"], work["Driver"], stop_segment], sort=False
    ).cumcount()
    work["lap_in_stint"] = work["TyreLife"].astype(float)
    work["stint_age_sq"] = work["lap_in_stint"] ** 2
    work["stint_age_cu"] = work["lap_in_stint"] ** 3

    # Piecewise hazard bins for early / mid / late stint risk.
    bins = [-np.inf, 5, 10, 15, 20, 25, 30, 40, np.inf]
    work["stint_age_bin"] = pd.cut(
        work["lap_in_stint"], bins=bins, labels=False
    ).astype("float32")
    work["race_progress_bin"] = pd.cut(
        work["RaceProgress"], bins=[-np.inf, 0.2, 0.4, 0.6, 0.8, np.inf], labels=False
    ).astype("float32")

    # Hazard-shape interactions.
    work["deg_per_lap"] = work["Cumulative_Degradation"] / work["lap_in_stint"].replace(
        0, np.nan
    )
    work["lap_delta_per_age"] = work["LapTime_Delta"] / work["lap_in_stint"].replace(
        0, np.nan
    )
    work["lap_time_per_progress"] = work["LapTime (s)"] / work["RaceProgress"].replace(
        0, np.nan
    )
    work["age_x_deg"] = work["lap_in_stint"] * work["Cumulative_Degradation"]
    work["age_x_progress"] = work["lap_in_stint"] * work["RaceProgress"]
    work["age_x_position"] = work["lap_in_stint"] * work["Position"]
    work["age_x_pos_change"] = work["lap_in_stint"] * work["Position_Change"]
    work["deg_x_progress"] = work["Cumulative_Degradation"] * work["RaceProgress"]

    # Compound-specific hazard offsets without using target leakage.
    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    work["compound_code"] = work["Compound"].map(compound_map).astype("float32")
    work["compound_age"] = work["compound_code"] * work["lap_in_stint"]
    work["compound_deg"] = work["compound_code"] * work["Cumulative_Degradation"]

    # Whether the row is immediately after a stop, which usually suppresses next-lap hazard.
    work["just_pitted"] = stop_proxy.astype("int8")
    work["fresh_tyre_flag"] = (work["lap_in_stint"] <= 3).astype("int8")
    work["late_stint_flag"] = (work["lap_in_stint"] >= 15).astype("int8")

    # Simple within-trajectory changes using only past/current ordering.
    work["deg_change"] = grp["Cumulative_Degradation"].diff().fillna(0.0)
    work["laptime_change"] = grp["LapTime (s)"].diff().fillna(0.0)
    work["position_change_prev"] = grp["Position"].diff().fillna(0.0)

    # Keep output row order identical to input.
    work = work.loc[orig_index]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000271"],
        "research_usage_note": "Verified assigned hypothesis 000271.",
    }, sort_keys=True))
