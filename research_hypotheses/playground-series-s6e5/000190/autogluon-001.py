...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key = race + "_" + year + "_" + driver
    out["_orig_order_kf"] = np.arange(n)

    sort_cols = [key.rename("_seq_key"), out["LapNumber"], out["_orig_order_kf"]]
    order = (
        pd.concat(sort_cols, axis=1)
        .sort_values(["_seq_key", "LapNumber", "_orig_order_kf"], kind="mergesort")
        .index.to_numpy()
    )

    lap_delta = out["LapTime_Delta"].astype(float).to_numpy()
    lap_time = out["LapTime (s)"].astype(float).to_numpy()
    tyre_life = out["TyreLife"].astype(float).to_numpy()
    pit = out["PitStop"].astype(float).to_numpy()
    keys = key.to_numpy()

    kf_mean = np.zeros(n, dtype=np.float32)
    kf_slope = np.zeros(n, dtype=np.float32)
    kf_innov = np.zeros(n, dtype=np.float32)
    kf_unc = np.zeros(n, dtype=np.float32)
    kf_gain = np.zeros(n, dtype=np.float32)
    ...
    out["kf_wear_mean"] = kf_mean
    out["kf_wear_slope"] = kf_slope
    out["kf_wear_innovation"] = kf_innov
    out["kf_wear_uncertainty"] = kf_unc
    out["kf_wear_gain"] = kf_gain
    out["kf_cliff_score"] = out["kf_wear_mean"] + 3.0 * out["kf_wear_slope"]
    out["kf_abs_innovation"] = np.abs(out["kf_wear_innovation"])
    out["kf_signal_to_uncertainty"] = out["kf_wear_mean"] / (
        out["kf_wear_uncertainty"] + 1e-3
    )

    grp = out.groupby(key, sort=False)
    out["prev_pitstop"] = grp["PitStop"].shift(1).fillna(0).astype("float32")
    out["laps_since_pit"] = grp["PitStop"].cumsum()
    out["laps_since_pit"] = (
        out.groupby([key, out["laps_since_pit"]], sort=False)
        .cumcount()
        .astype("float32")
    )
    out["tyre_life_sq"] = (tyre_life**2).astype("float32")
    out["tyre_life_x_stint"] = (
        out["TyreLife"].astype(float) * out["Stint"].astype(float)
    ).astype("float32")
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"].astype(float)
        / (out["TyreLife"].astype(float) + 1.0)
    ).astype("float32")
    out["lap_time_per_tyre_lap"] = (lap_time / (tyre_life + 1.0)).astype("float32")

    for c in ["Compound", "Driver", "Race"]:
        freq = out[c].map(out[c].value_counts(normalize=True)).astype("float32")
        out[c + "_freq"] = freq

    out = out.drop(columns=["_orig_order_kf"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000190"],
        "research_usage_note": "Verified assigned hypothesis 000190.",
    }, sort_keys=True))
