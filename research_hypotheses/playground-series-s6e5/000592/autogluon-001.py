...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    race_keys = ["Year", "Race"]
    weights = {1: 1.0, 2: 0.55, 3: 0.30}

    lap_num = out["LapNumber"].astype(float)
    progress = out["RaceProgress"].astype(float).clip(lower=1e-4)
    out["laps_remaining_est"] = (
        ((lap_num / progress) - lap_num)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .clip(0, 120)
    )
    out["tyre_life_log1p"] = np.log1p(out["TyreLife"].astype(float).clip(lower=0))

    lap_tbl = (
        out.groupby(race_keys + ["LapNumber"], observed=True, sort=False)
        .agg(_race_pits=("PitStop", "sum"), _race_rows=("PitStop", "size"))
        .reset_index()
    )
    for lag in weights:
        lagged = lap_tbl.copy()
        lagged["LapNumber"] = lagged["LapNumber"] + lag
        lagged = lagged.rename(
            columns={
                "_race_pits": f"race_pits_prev{lag}",
                "_race_rows": f"race_rows_prev{lag}",
            }
        )
        out = out.merge(lagged, on=race_keys + ["LapNumber"], how="left", sort=False)
        out[f"race_pits_prev{lag}"] = (
            out[f"race_pits_prev{lag}"].fillna(0).astype(float)
        )
        out[f"race_rows_prev{lag}"] = (
            out[f"race_rows_prev{lag}"].fillna(0).astype(float)
        )
        out[f"race_pit_rate_prev{lag}"] = (
            out[f"race_pits_prev{lag}"] / out[f"race_rows_prev{lag}"].replace(0, np.nan)
        ).fillna(0)
    out["race_pit_intensity_3lap"] = sum(
        weights[lag] * out[f"race_pits_prev{lag}"] for lag in weights
    )
    out["race_pit_rate_3lap"] = sum(
        weights[lag] * out[f"race_pit_rate_prev{lag}"] for lag in weights
    ) / sum(weights.values())

    pos_tbl = (
        out.groupby(race_keys + ["LapNumber", "Position"], observed=True, sort=False)
        .agg(_pos_pits=("PitStop", "sum"), _pos_rows=("PitStop", "size"))
        .reset_index()
    )
    for lag in weights:
        parts = []
        ...
        near = (
            ...
            .groupby(race_keys + ["LapNumber", "Position"], observed=True, sort=False)
            .agg(_near_pits=("_pos_pits", "sum"), _near_rows=("_pos_rows", "sum"))
            .reset_index()
        )
        near = near.rename(
            columns={
                "_near_pits": f"near_pos_pits_prev{lag}",
                "_near_rows": f"near_pos_rows_prev{lag}",
            }
        )
        out = out.merge(
            near, on=race_keys + ["LapNumber", "Position"], how="left", sort=False
        )
        out[f"near_pos_pits_prev{lag}"] = (
            out[f"near_pos_pits_prev{lag}"].fillna(0).astype(float)
        )
        out[f"near_pos_rows_prev{lag}"] = (
            out[f"near_pos_rows_prev{lag}"].fillna(0).astype(float)
        )
        out[f"near_pos_pit_rate_prev{lag}"] = (
            out[f"near_pos_pits_prev{lag}"]
            / out[f"near_pos_rows_prev{lag}"].replace(0, np.nan)
        ).fillna(0)
    out["near_pos_pit_intensity_3lap"] = sum(
        weights[lag] * out[f"near_pos_pits_prev{lag}"] for lag in weights
    )
    out["near_pos_pit_rate_3lap"] = sum(
        weights[lag] * out[f"near_pos_pit_rate_prev{lag}"] for lag in weights
    ) / sum(weights.values())

    for peer_cols, prefix in [
        (["Compound"], "same_compound"),
        (["Stint"], "same_stint"),
        (["Compound", "Stint"], "same_compound_stint"),
    ]:
        peer_tbl = (
            out.groupby(
                race_keys + ["LapNumber"] + peer_cols, observed=True, sort=False
            )
            .agg(_peer_pits=("PitStop", "sum"), _peer_rows=("PitStop", "size"))
            .reset_index()
        )
        for lag in weights:
            lagged = peer_tbl.copy()
            lagged["LapNumber"] = lagged["LapNumber"] + lag
            lagged = lagged.rename(
                columns={
                    "_peer_pits": f"{prefix}_pits_prev{lag}",
                    "_peer_rows": f"{prefix}_rows_prev{lag}",
                }
            )
            out = out.merge(
                lagged, on=race_keys + ["LapNumber"] + peer_cols, how="left", sort=False
            )
            out[f"{prefix}_pits_prev{lag}"] = (
                out[f"{prefix}_pits_prev{lag}"].fillna(0).astype(float)
            )
            out[f"{prefix}_rows_prev{lag}"] = (
                out[f"{prefix}_rows_prev{lag}"].fillna(0).astype(float)
            )
            out[f"{prefix}_pit_rate_prev{lag}"] = (
                out[f"{prefix}_pits_prev{lag}"]
                / out[f"{prefix}_rows_prev{lag}"].replace(0, np.nan)
            ).fillna(0)
        out[f"{prefix}_pit_intensity_3lap"] = sum(
            weights[lag] * out[f"{prefix}_pits_prev{lag}"] for lag in weights
        )
        out[f"{prefix}_pit_rate_3lap"] = sum(
            weights[lag] * out[f"{prefix}_pit_rate_prev{lag}"] for lag in weights
        ) / sum(weights.values())

    driver_tbl = (
        out.groupby(race_keys + ["Driver", "LapNumber"], observed=True, sort=False)
        .agg(_driver_pit=("PitStop", "max"))
        .reset_index()
        .sort_values(race_keys + ["Driver", "LapNumber"], kind="mergesort")
    )
    grp_keys = [driver_tbl[k] for k in race_keys + ["Driver"]]
    driver_tbl["_pit_lap"] = driver_tbl["LapNumber"].where(
        driver_tbl["_driver_pit"] > 0
    )
    driver_tbl["last_own_pit_lap"] = (
        driver_tbl["_pit_lap"].groupby(grp_keys, sort=False).shift(1)
    )
    driver_tbl["last_own_pit_lap"] = (
        driver_tbl["last_own_pit_lap"].groupby(grp_keys, sort=False).ffill()
    )
    driver_tbl["laps_since_own_pit"] = (
        (driver_tbl["LapNumber"] - driver_tbl["last_own_pit_lap"])
        .fillna(99)
        .clip(0, 99)
    )

    out = out.merge(
        driver_tbl[race_keys + ["Driver", "LapNumber", "laps_since_own_pit"]],
        on=race_keys + ["Driver", "LapNumber"],
        how="left",
        sort=False,
    )
    out["laps_since_own_pit"] = out["laps_since_own_pit"].fillna(99).astype(float)
    out["own_pit_cooldown"] = 1.0 / (1.0 + out["laps_since_own_pit"])

    for lag in weights:
        own = driver_tbl[race_keys + ["Driver", "LapNumber", "_driver_pit"]].copy()
        own["LapNumber"] = own["LapNumber"] + lag
        own = own.rename(columns={"_driver_pit": f"own_pit_prev{lag}"})
        out = out.merge(
            own, on=race_keys + ["Driver", "LapNumber"], how="left", sort=False
        )
        out[f"own_pit_prev{lag}"] = out[f"own_pit_prev{lag}"].fillna(0).astype(float)
    out["own_recent_pit_intensity_3lap"] = sum(
        weights[lag] * out[f"own_pit_prev{lag}"] for lag in weights
    )

    out["race_intensity_x_tyre_life"] = out["race_pit_intensity_3lap"] * out[
        "TyreLife"
    ].astype(float)
    out["near_pos_intensity_x_tyre_life"] = out["near_pos_pit_intensity_3lap"] * out[
        "TyreLife"
    ].astype(float)
    out["same_compound_intensity_x_tyre_life"] = out[
        "same_compound_pit_intensity_3lap"
    ] * out["TyreLife"].astype(float)
    out["race_intensity_x_laps_remaining"] = (
        out["race_pit_intensity_3lap"] * out["laps_remaining_est"]
    )
    out["near_pos_intensity_x_laps_remaining"] = (
        out["near_pos_pit_intensity_3lap"] * out["laps_remaining_est"]
    )
    out["race_intensity_x_own_cooldown"] = (
        out["race_pit_intensity_3lap"] * out["own_pit_cooldown"]
    )
    out["near_pos_intensity_x_own_cooldown"] = (
        out["near_pos_pit_intensity_3lap"] * out["own_pit_cooldown"]
    )
    out["peer_intensity_balance"] = (
        out["same_compound_pit_intensity_3lap"] - out["race_pit_intensity_3lap"]
    )
    out["local_wave_pressure"] = (
        (
            0.45 * out["race_pit_rate_3lap"]
            + 0.35 * out["near_pos_pit_rate_3lap"]
            + 0.20 * out["same_compound_stint_pit_rate_3lap"]
        )
        * (1.0 + out["tyre_life_log1p"])
        * (1.0 - out["own_pit_cooldown"])
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000592.",
    }, sort_keys=True))
