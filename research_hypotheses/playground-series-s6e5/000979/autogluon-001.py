...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["regime_is_wet_inter"] = is_wet
    out["regime_is_dry"] = is_dry
    out["regime_early_race"] = (progress <= 0.25).astype("int8")
    out["regime_mid_race"] = ((progress > 0.25) & (progress < 0.70)).astype("int8")
    out["regime_late_race"] = (progress >= 0.70).astype("int8")
    out["regime_late_stint"] = (tyre >= 18).astype("int8")
    out["regime_pit_window"] = (
        (tyre >= 12) & (progress >= 0.20) & (progress <= 0.90)
    ).astype("int8")

    out["tyre_life_x_dry"] = tyre * is_dry
    out["tyre_life_x_wet"] = tyre * is_wet
    out["progress_x_dry"] = progress * is_dry
    out["progress_x_wet"] = progress * is_wet
    out["late_stint_x_dry"] = out["regime_late_stint"] * is_dry
    out["pit_window_x_dry"] = out["regime_pit_window"] * is_dry
    out["pit_window_x_wet"] = out["regime_pit_window"] * is_wet

    out["tyre_life_ratio"] = tyre / (lap + 1.0)
    out["stint_lap_pressure"] = tyre / (stint + 1.0)
    out["race_remaining"] = 1.0 - progress
    out["position_pressure"] = (21.0 - pos) / 20.0
    out["position_change_abs"] = pos_chg.abs()
    out["lap_delta_abs"] = lap_delta.abs()
    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["lap_time_clipped"] = lap_time.clip(60, 180)
    out["lap_delta_clipped"] = lap_delta.clip(-30, 30)
    out["degradation_clipped"] = degr.clip(-100, 500)

    out["compound_race"] = compound.fillna("NA") + "_" + race.fillna("NA")
    out["driver_compound"] = driver.fillna("NA") + "_" + compound.fillna("NA")
    out["race_year"] = (
        race.fillna("NA") + "_" + out["Year"].astype("string").fillna("NA")
    )
    ...
    for col in [
        ...
        freq = (
            out[col]
            .astype("string")
            .map(out[col].astype("string").value_counts())
            .astype("float32")
        )
        out[col + "_freq"] = np.log1p(freq)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000979"],
        "research_usage_note": "Verified assigned hypothesis 000979.",
    }, sort_keys=True))
