...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["softness_rank"] = (
        out["Compound"]
        .map({"SOFT": 3, "MEDIUM": 2, "HARD": 1, "INTERMEDIATE": 0, "WET": 0})
        .fillna(0)
        .astype("int8")
    )

    pit_rows = out["PitStop"].eq(1)
    pit_loss_by_race = (
        out.loc[pit_rows]
        .groupby(race_key)["LapTime_Delta"]
        .median()
        .abs()
        .replace([np.inf, -np.inf], np.nan)
    )
    global_pit_loss = float(out.loc[pit_rows, "LapTime_Delta"].abs().median())
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 22.0

    out["pit_loss_proxy"] = (
        out.set_index(race_key).index.map(pit_loss_by_race).astype(float)
    )
    out["pit_loss_proxy"] = (
        out["pit_loss_proxy"].fillna(global_pit_loss).clip(12.0, 35.0)
    )

    lap_pace = out.groupby(race_lap)["LapTime (s)"].transform("median")
    out["lap_pace_gap"] = (out["LapTime (s)"] - lap_pace).clip(-20.0, 20.0)

    field_size = out.groupby(race_lap)["Position"].transform("max").clip(lower=1)
    approx_gap_per_place = (lap_pace / field_size).clip(1.0, 6.0)
    out["projected_positions_lost"] = (
        out["pit_loss_proxy"] / approx_gap_per_place
    ).clip(1, 20)
    out["projected_rejoin_position"] = (
        out["Position"] + out["projected_positions_lost"]
    ).clip(1, 20)

    ordered = out[race_lap + ["Position", "LapTime (s)"]].copy()
    ordered["_idx"] = np.arange(len(out))
    ordered = ordered.sort_values(race_lap + ["Position", "_idx"], kind="mergesort")
    grp = ordered.groupby(race_lap, sort=False)
    pos = ordered["Position"].astype("float32")
    ahead1_pos = grp["Position"].shift(1)
    behind1_pos = grp["Position"].shift(-1)
    ahead2_pos = grp["Position"].shift(2)
    behind2_pos = grp["Position"].shift(-2)

    ordered["gap_to_car_ahead_pos"] = (pos - ahead1_pos).abs().fillna(20).clip(0, 20)
    ordered["gap_to_car_behind_pos"] = (
        (behind1_pos - pos).abs().fillna(20).clip(0, 20)
    )
    ordered["local_field_density"] = (
        ordered["gap_to_car_ahead_pos"].le(1).astype("int8")
        + ordered["gap_to_car_behind_pos"].le(1).astype("int8")
        + (pos - ahead2_pos).abs().le(2).fillna(False).astype("int8")
        + (behind2_pos - pos).abs().le(2).fillna(False).astype("int8")
    )

    ahead_pace = grp["LapTime (s)"].shift(1)
    behind_pace = grp["LapTime (s)"].shift(-1)
    ordered["nearby_rival_pace_delta"] = (
        (
            ordered["LapTime (s)"]
            - pd.concat([ahead_pace, behind_pace], axis=1).mean(axis=1)
        )
        .fillna(0)
        .clip(-20.0, 20.0)
    )
    aligned = ordered.sort_values("_idx", kind="mergesort")
    out["gap_to_car_ahead_pos"] = aligned["gap_to_car_ahead_pos"].to_numpy(
        dtype="float32"
    )
    out["gap_to_car_behind_pos"] = aligned["gap_to_car_behind_pos"].to_numpy(
        dtype="float32"
    )
    out["local_field_density"] = aligned["local_field_density"].to_numpy(dtype="int8")
    out["nearby_rival_pace_delta"] = aligned["nearby_rival_pace_delta"].to_numpy(
        dtype="float32"
    )

    out["clean_air_after_stop"] = (
        (out["projected_rejoin_position"] > field_size - 2)
        | (out["local_field_density"] <= 1)
    ).astype("int8")
    out["undercut_cover_margin"] = (
        out["pit_loss_proxy"]
        - out["LapTime_Delta"].clip(-10, 10)
        - out["local_field_density"] * approx_gap_per_place
    ).clip(-30.0, 30.0)
    out["traffic_blocked_stop_window"] = (
        (out["local_field_density"] >= 3)
        & (out["projected_rejoin_position"] < field_size - 1)
        & (out["RaceProgress"].between(0.18, 0.90))
    ).astype("int8")

    out["pit_window_pressure"] = (
        out["TyreLife"] * (1.0 + 0.15 * out["softness_rank"])
        + 12.0 * out["RaceProgress"]
        - 2.0 * out["Stint"]
        - 3.0 * out["traffic_blocked_stop_window"]
        + 2.0 * out["clean_air_after_stop"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000169"],
        "research_usage_note": "Verified assigned hypothesis 000169.",
    }, sort_keys=True))
