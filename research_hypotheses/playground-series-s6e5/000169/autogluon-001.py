...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = out.groupby(race_lap, sort=False)
    pos = out["Position"]
    ...
    out["gap_to_car_ahead_pos"] = (pos - ahead1_pos).abs().fillna(20).clip(0, 20)
    out["gap_to_car_behind_pos"] = (behind1_pos - pos).abs().fillna(20).clip(0, 20)
    out["local_field_density"] = (
        out["gap_to_car_ahead_pos"].le(1).astype("int8")
        + out["gap_to_car_behind_pos"].le(1).astype("int8")
    ...
    out["nearby_rival_pace_delta"] = (
        (out["LapTime (s)"] - pd.concat([ahead_pace, behind_pace], axis=1).mean(axis=1))
        .fillna(0)
        .clip(-20.0, 20.0)
    )
