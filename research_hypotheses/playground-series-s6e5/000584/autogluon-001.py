...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre = out["TyreLife"].astype(float).clip(0, 80)
    lap = out["LapNumber"].astype(float).clip(1, 100)
    progress = out["RaceProgress"].astype(float).clip(1e-4, 1.0)
    total_laps_est = (lap / progress).clip(lap, 100)
    laps_remaining = (total_laps_est - lap).clip(0, 80)

    out["TyreLife_clip"] = tyre
    out["LapsRemaining_Est"] = laps_remaining
    out["TotalLaps_Est"] = total_laps_est

    out["TyreLife_log1p"] = np.log1p(tyre)
    out["LapsRemaining_log1p"] = np.log1p(laps_remaining)
    out["LateRaceUrgency"] = 1.0 / (1.0 + laps_remaining)
    out["TyreAge_x_Urgency"] = tyre * out["LateRaceUrgency"]

    tyre_bin = pd.cut(
        tyre,
        bins=[-0.1, 1, 3, 6, 10, 15, 20, 28, 40, 80],
        labels=False,
        include_lowest=True,
    ).astype("int16")
    rem_bin = pd.cut(
        laps_remaining,
        bins=[-0.1, 2, 5, 10, 15, 20, 30, 45, 80],
        labels=False,
        include_lowest=True,
    ).astype("int16")

    out["TyreLife_bin"] = tyre_bin
    out["LapsRemaining_bin"] = rem_bin
    out["HazardSurface_bin"] = (tyre_bin * 10 + (7 - rem_bin)).astype("int16")

    compound = out["Compound"].astype(str).str.upper().fillna("UNKNOWN")
    out["Compound_norm"] = compound
    out["Compound_HazardSurface"] = (
        compound + "_" + out["HazardSurface_bin"].astype(str)
    )

    out["Cooldown_Tyre"] = (tyre <= 2).astype("int8")
    out["Cooldown_StintStart"] = (
        (out["Stint"].astype(float) <= 1) | (tyre <= 2)
    ).astype("int8")
    out["PostPit_Cooldown"] = ((out["PitStop"].astype(float) > 0) | (tyre <= 1)).astype(
        "int8"
    )

    out["AgeDegradation"] = tyre * out["Cumulative_Degradation"].astype(float)
    out["AgeDelta"] = tyre * out["LapTime_Delta"].astype(float)
    out["Remaining_x_Progress"] = laps_remaining * progress
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000584.",
    }, sort_keys=True))
