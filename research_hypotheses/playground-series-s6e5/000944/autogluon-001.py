...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    dry = out["Compound"].isin(["SOFT", "MEDIUM", "HARD"])
    out["is_dry_compound"] = dry.astype("int8")
    out["is_wet_or_inter"] = (~dry).astype("int8")

    order_cols = ["Year", "Race", "Driver", "LapNumber"]
    s = out[order_cols + ["Compound", "PitStop", "RaceProgress", "TyreLife"]].copy()
    s["_idx"] = np.arange(len(s))
    s = s.sort_values(order_cols + ["_idx"], kind="mergesort")

    g = s.groupby(["Year", "Race", "Driver"], sort=False)
    s["stops_so_far"] = g["PitStop"].cumsum().astype("int16")
    s["prior_stops"] = (s["stops_so_far"] - s["PitStop"]).clip(lower=0).astype("int16")
    s["race_max_lap"] = g["LapNumber"].transform("max")
    s["laps_remaining"] = (
        (s["race_max_lap"] - s["LapNumber"]).clip(lower=0).astype("int16")
    )
    ...
    for comp in ["SOFT", "MEDIUM", "HARD"]:
        ...
        s[f"seen_{comp.lower()}"] = seen.astype("int8")

    s["dry_compounds_used_so_far"] = (
        s["seen_soft"] + s["seen_medium"] + s["seen_hard"]
    ).astype("int8")
    s["has_changed_dry_compound"] = (s["dry_compounds_used_so_far"] >= 2).astype("int8")
    s["needs_dry_compound_change"] = (
        dry.reindex(s.index).to_numpy()
        & (s["dry_compounds_used_so_far"].to_numpy() < 2)
    ).astype("int8")

    race_lower = s["Race"].astype(str).str.lower()
    s["is_monaco"] = race_lower.str.contains("monaco", regex=False).astype("int8")
    s["is_2025_monaco"] = ((s["Year"] == 2025) & (s["is_monaco"] == 1)).astype("int8")

    final_third = s["RaceProgress"] >= 0.66
    late = s["RaceProgress"] >= 0.75
    very_late = s["RaceProgress"] >= 0.85

    s["dry_no_compound_change_late"] = (
        (s["needs_dry_compound_change"] == 1) & late
    ).astype("int8")
    s["dry_no_compound_change_very_late"] = (
        (s["needs_dry_compound_change"] == 1) & very_late
    ).astype("int8")
    s["one_stop_only_final_third"] = (
        dry.reindex(s.index).to_numpy()
        & final_third.to_numpy()
        & (s["stops_so_far"].to_numpy() <= 1)
    ).astype("int8")
    s["zero_stop_after_half"] = (
        dry.reindex(s.index).to_numpy()
        & (s["RaceProgress"].to_numpy() >= 0.50)
        & (s["stops_so_far"].to_numpy() == 0)
    ).astype("int8")
    s["monaco_2025_less_than_two_stops_late"] = (
        (s["is_2025_monaco"] == 1) & final_third & (s["stops_so_far"] < 2)
    ).astype("int8")

    denom = s["laps_remaining"].astype(float) + 1.0
    s["stop_pressure_per_lap_remaining"] = (
        s["needs_dry_compound_change"].astype(float) / denom
    ).astype("float32")
    s["monaco_2025_stop_pressure"] = (
        np.maximum(0, 2 - s["stops_so_far"]).astype(float) / denom
    ).astype("float32")
    s["tyrelife_x_needs_compound_change"] = (
        s["TyreLife"].astype(float) * s["needs_dry_compound_change"].astype(float)
    ).astype("float32")
    ...
    add_cols = [
        ...
        "prior_stops",
        "laps_remaining",
        "dry_compounds_used_so_far",
        "has_changed_dry_compound",
        "needs_dry_compound_change",
        "is_monaco",
        "is_2025_monaco",
        "dry_no_compound_change_late",
        "dry_no_compound_change_very_late",
        "one_stop_only_final_third",
        "zero_stop_after_half",
        "monaco_2025_less_than_two_stops_late",
        "stop_pressure_per_lap_remaining",
        "monaco_2025_stop_pressure",
        "tyrelife_x_needs_compound_change",
    ]
    out.loc[s["_idx"].to_numpy(), add_cols] = s[add_cols].to_numpy()
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000944"],
        "research_usage_note": "Verified assigned hypothesis 000944.",
    }, sort_keys=True))
