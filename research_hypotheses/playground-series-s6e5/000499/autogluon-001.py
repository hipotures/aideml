...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Race distance proxy and stop-need context.
    lap_est = (out["LapNumber"] / out["RaceProgress"].clip(lower=eps)).replace(
        [np.inf, -np.inf], np.nan
    )
    out["_LapEst"] = lap_est
    race_laps = (
        out.groupby("Race_Year")["_LapEst"]
        .transform("median")
        .fillna(out["LapNumber"].max())
    )
    out["LapsRemaining_Est"] = (race_laps - out["LapNumber"]).clip(lower=0)
    out["NeedsAnotherStop"] = (
        (out["RaceProgress"] < 0.82)
        & (out["TyreLife"] >= 12)
        & (out["Stint"] <= 4)
        & (~out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]))
    ).astype("int8")

    keys = ["Race_Year", "LapNumber"]
    g = out.groupby(keys, sort=False)
    n = g["LapTime_Delta"].transform("count").astype(float)

    # Leave-one-driver-out mean-style field proxies; robust enough and deterministic.
    ltd_sum = g["LapTime_Delta"].transform("sum")
    ltd_sq_sum = (
        (out["LapTime_Delta"] ** 2)
        .groupby([out["Race_Year"], out["LapNumber"]])
        .transform("sum")
    )
    pc_sum = g["Position_Change"].transform("sum")
    pc_sq_sum = (
        (out["Position_Change"] ** 2)
        .groupby([out["Race_Year"], out["LapNumber"]])
        .transform("sum")
    )
    pit_sum = g["PitStop"].transform("sum")

    denom = (n - 1).clip(lower=1)
    loo_ltd_mean = np.where(n > 1, (ltd_sum - out["LapTime_Delta"]) / denom, 0.0)
    loo_pc_mean = np.where(n > 1, (pc_sum - out["Position_Change"]) / denom, 0.0)
    loo_pit_share = np.where(n > 1, (pit_sum - out["PitStop"]) / denom, 0.0)

    loo_ltd_var = np.where(
        n > 2,
        ((ltd_sq_sum - out["LapTime_Delta"] ** 2) / denom) - loo_ltd_mean**2,
        0.0,
    )
    loo_pc_var = np.where(
        n > 2,
        ((pc_sq_sum - out["Position_Change"] ** 2) / denom) - loo_pc_mean**2,
        0.0,
    )

    out["Neutral_LOO_LapTimeDelta"] = loo_ltd_mean
    out["Neutral_LOO_LapTimeDisp"] = np.sqrt(np.maximum(loo_ltd_var, 0.0))
    out["Neutral_LOO_PitShare"] = loo_pit_share
    out["Neutral_LOO_PosChangeVol"] = np.sqrt(np.maximum(loo_pc_var, 0.0))

    # Build lap-level prior context, shifted by race lap so row order is unchanged.
    lap_level = (
        out.groupby(keys, sort=False)
        .agg(
            Lap_Field_LapTimeDelta=("Neutral_LOO_LapTimeDelta", "mean"),
            Lap_Field_LapTimeDisp=("Neutral_LOO_LapTimeDisp", "mean"),
            Lap_Field_PitShare=("Neutral_LOO_PitShare", "mean"),
            Lap_Field_PosChangeVol=("Neutral_LOO_PosChangeVol", "mean"),
        )
        .sort_index()
    )

    for col in lap_level.columns:
        lap_level[col + "_lag1"] = lap_level.groupby(level=0)[col].shift(1)
        mapper0 = lap_level[col].to_dict()
        mapper1 = lap_level[col + "_lag1"].to_dict()
        idx = list(zip(out["Race_Year"], out["LapNumber"]))
        out[col + "_t"] = (
            pd.Series(idx, index=out.index).map(mapper0).fillna(0.0).astype("float32")
        )
        out[col + "_tminus1"] = (
            pd.Series(idx, index=out.index).map(mapper1).fillna(0.0).astype("float32")
        )

    neutral_cols = [
        "Lap_Field_LapTimeDelta_t",
        "Lap_Field_LapTimeDelta_tminus1",
        "Lap_Field_LapTimeDisp_t",
        "Lap_Field_LapTimeDisp_tminus1",
        "Lap_Field_PitShare_t",
        "Lap_Field_PitShare_tminus1",
        "Lap_Field_PosChangeVol_t",
        "Lap_Field_PosChangeVol_tminus1",
    ]
    for col in neutral_cols:
        out[col + "_x_NeedsStop"] = (out[col] * out["NeedsAnotherStop"]).astype(
            "float32"
        )
        out[col + "_x_LapsRemaining"] = (out[col] * out["LapsRemaining_Est"]).astype(
            "float32"
        )

    out.drop(columns=["Race_Year", "_LapEst"], inplace=True)
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000499"],
        "research_usage_note": "Verified assigned hypothesis 000499.",
    }, sort_keys=True))
