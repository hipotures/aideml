...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    order = out.sort_values(car_keys + ["LapNumber"], kind="mergesort").index
    prev_comp = pd.Series(pd.NA, index=out.index, dtype="string")
    prev_stint = pd.Series(np.nan, index=out.index, dtype="float64")

    sorted_comp = compound.loc[order]
    sorted_stint = out.loc[order, "Stint"]
    prev_comp.loc[order] = (
        sorted_comp.groupby([out.loc[order, k] for k in car_keys], sort=False)
        .shift(1)
        .values
    )
    prev_stint.loc[order] = (
        sorted_stint.groupby([out.loc[order, k] for k in car_keys], sort=False)
        .shift(1)
        .values
    )

    low_tyre = (out["TyreLife"] <= 2).astype("int8")
    fresh_tyre = (out["TyreLife"] <= 3).astype("int8")
    new_stint = (out["Stint"].astype("float64") > prev_stint).fillna(False)
    new_compound = compound.ne(prev_comp).fillna(False)
    new_slick = (
        (is_slick == 1) & ((out["TyreLife"] <= 3) | new_stint | new_compound)
    ).astype("int8")
    pit_active = out["PitStop"].astype("int8")

    out["own_low_tyre"] = low_tyre
    out["own_fresh_tyre"] = fresh_tyre
    out["own_new_slick"] = new_slick

    lap_count = (
        out.groupby(lap_keys, sort=False)["LapNumber"]
        .transform("count")
        .astype("float64")
    )
    denom = (lap_count - 1.0).replace(0.0, np.nan)

    for name, flag in {
        "peer_low_tyre_rate": low_tyre,
        "peer_fresh_tyre_rate": fresh_tyre,
        "peer_new_slick_rate": new_slick,
        "peer_pit_active_rate": pit_active,
    }.items():
        lap_sum = (
            flag.groupby([out[k] for k in lap_keys], sort=False)
            .transform("sum")
            .astype("float64")
        )
        out[name] = (
            ((lap_sum - flag.astype("float64")) / denom).fillna(0.0).astype("float32")
        )

    out["peer_fresh_wave"] = (
        0.40 * out["peer_low_tyre_rate"]
        + 0.35 * out["peer_new_slick_rate"]
        + 0.25 * out["peer_pit_active_rate"]
    ).astype("float32")

    tyre_age = out["TyreLife"].astype("float32")
    degr = out["Cumulative_Degradation"].astype("float32")
    progress = out["RaceProgress"].astype("float32")
    pos_pressure = ((21.0 - out["Position"].astype("float32")) / 20.0).clip(0, 1)
    stint_pressure = out["Stint"].astype("float32")

    out["peer_wave_x_tyre_life"] = (out["peer_fresh_wave"] * tyre_age).astype("float32")
    out["peer_wave_x_degradation"] = (out["peer_fresh_wave"] * degr).astype("float32")
    out["peer_wave_x_progress"] = (out["peer_fresh_wave"] * progress).astype("float32")
    out["peer_wave_x_position_pressure"] = (
        out["peer_fresh_wave"] * pos_pressure
    ).astype("float32")
    out["peer_new_slick_x_stint"] = (
        out["peer_new_slick_rate"] * stint_pressure
    ).astype("float32")
    out["peer_pit_x_low_tyre"] = (
        out["peer_pit_active_rate"] * low_tyre.astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000041"],
        "research_usage_note": "Verified assigned hypothesis 000041.",
    }, sort_keys=True))
