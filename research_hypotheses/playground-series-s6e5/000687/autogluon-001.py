...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_wet"] = out["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    out["is_dry"] = 1 - out["is_wet"]

    tyre_bins = [-np.inf, 3, 7, 12, 18, 25, 35, np.inf]
    tyre_labels = ["t0_3", "t4_7", "t8_12", "t13_18", "t19_25", "t26_35", "t36p"]
    out["TyreLifeBucket"] = pd.cut(
        out["TyreLife"], bins=tyre_bins, labels=tyre_labels
    ).astype("string")

    prog_bins = np.linspace(0, 1, 11)
    prog_labels = [f"p{i}" for i in range(10)]
    out["RaceProgressBucket"] = pd.cut(
        out["RaceProgress"].clip(0, 1),
        bins=prog_bins,
        labels=prog_labels,
        include_lowest=True,
    ).astype("string")

    out["StintStr"] = out["Stint"].astype("int16").astype("string")
    out["YearStr"] = out["Year"].astype("int16").astype("string")
    ...
    base_key = (
        ...
        + out["StintStr"]
        + "|"
        + out["RaceProgressBucket"]
        + "|"
        + out["is_wet"].astype("string")
    )
    out["hazard_key_global"] = base_key
    out["hazard_key_year"] = out["YearStr"] + "|" + base_key
    out["hazard_key_race"] = out["Race"] + "|" + base_key

    out["hazard_backoff_ct"] = (
        out["Compound"] + "|" + out["TyreLifeBucket"] + "|" + out["StintStr"]
    )
    out["hazard_backoff_cp"] = (
        out["Compound"]
        + "|"
        + out["RaceProgressBucket"]
        + "|"
    ...
    out["hazard_backoff_tp"] = (
        out["TyreLifeBucket"] + "|" + out["RaceProgressBucket"] + "|" + out["StintStr"]
    )
    out["hazard_backoff_driver"] = (
        out["Driver"] + "|" + out["Compound"] + "|" + out["StintStr"]
    )

    out["tyre_life_log"] = np.log1p(out["TyreLife"])
    out["stint_tyre_ratio"] = out["TyreLife"] / out["Stint"].clip(lower=1)
    out["deg_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1)
    out["deg_x_progress"] = out["Cumulative_Degradation"] * out["RaceProgress"]
    out["lap_delta_per_tyre"] = out["LapTime_Delta"] / out["TyreLife"].clip(lower=1)
    out["position_pressure"] = out["Position"] / 20.0
    out["laps_remaining_proxy"] = out["LapNumber"] * (
        1.0 / out["RaceProgress"].clip(lower=1e-3) - 1.0
    )
    out["late_race_old_tyre"] = (out["RaceProgress"] >= 0.7).astype("int8") * (
        out["TyreLife"] >= 15
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000687"],
        "research_usage_note": "Verified assigned hypothesis 000687.",
    }, sort_keys=True))
