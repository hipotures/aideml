...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp_lap_keys = ["Race", "Year", "LapNumber", "Compound"]
    ...
    # Same-lap, same-compound peer percentiles: current observable competitive context.
    for c in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta", "LapTime (s)"]:
        out[f"{c}_comp_lap_pct"] = (
            out.groupby(comp_lap_keys, observed=True)[c]
            .rank(pct=True, method="average")
            .astype("float32")
        )
        out[f"{c}_race_lap_pct"] = (
            out.groupby(lap_keys, observed=True)[c]
            .rank(pct=True, method="average")
            .astype("float32")
        )

    comp_peer = (
        out.groupby(comp_lap_keys, observed=True)
        .agg(
            comp_lap_tyre_mean=("TyreLife", "mean"),
            comp_lap_deg_mean=("Cumulative_Degradation", "mean"),
            comp_lap_delta_mean=("LapTime_Delta", "mean"),
            comp_lap_count=("TyreLife", "size"),
        )
        .reset_index()
    )
    out = out.merge(comp_peer, on=comp_lap_keys, how="left", sort=False)

    out["tyre_vs_comp_lap_mean"] = (out["TyreLife"] - out["comp_lap_tyre_mean"]).astype(
        "float32"
    )
    out["deg_vs_comp_lap_mean"] = (
        out["Cumulative_Degradation"] - out["comp_lap_deg_mean"]
    ).astype("float32")
    out["delta_vs_comp_lap_mean"] = (
        out["LapTime_Delta"] - out["comp_lap_delta_mean"]
    ).astype("float32")

    # Nearby rivals with older tyres, using current same-lap positions only.
    base_cols = lap_keys + ["Position", "TyreLife", "Cumulative_Degradation"]
    base = out[base_cols].copy()
    base["_row_id"] = np.arange(len(out))
    out["_row_id"] = base["_row_id"]

    out["older_tyre_ahead_1_2"] = 0
    out["older_tyre_behind_1_2"] = 0
    out["higher_deg_ahead_1_2"] = 0
    out["higher_deg_behind_1_2"] = 0

    for offset in (1, 2):
        rival = base.rename(
            columns={
                "Position": "rival_position",
                "TyreLife": f"rival_tyre_{offset}",
                "Cumulative_Degradation": f"rival_deg_{offset}",
            }
        )

        ahead = base.copy()
        ahead["rival_position"] = ahead["Position"] - offset
        ahead = ahead.merge(
            rival[
                lap_keys
                + ["rival_position", f"rival_tyre_{offset}", f"rival_deg_{offset}"]
            ],
            on=lap_keys + ["rival_position"],
            how="left",
            sort=False,
        )
        out.loc[ahead["_row_id"], "older_tyre_ahead_1_2"] += (
            (ahead[f"rival_tyre_{offset}"] > ahead["TyreLife"])
            .fillna(False)
            .astype("int8")
            .to_numpy()
        )
        out.loc[ahead["_row_id"], "higher_deg_ahead_1_2"] += (
            (ahead[f"rival_deg_{offset}"] > ahead["Cumulative_Degradation"])
            .fillna(False)
            .astype("int8")
            .to_numpy()
        )
        ...
        behind = base.copy()
        behind["rival_position"] = behind["Position"] + offset
        behind = behind.merge(
            rival[
                lap_keys
                + ["rival_position", f"rival_tyre_{offset}", f"rival_deg_{offset}"]
        ...
        out.loc[behind["_row_id"], "older_tyre_behind_1_2"] += (
            (behind[f"rival_tyre_{offset}"] > behind["TyreLife"])
            .fillna(False)
            .astype("int8")
            .to_numpy()
        )
        out.loc[behind["_row_id"], "higher_deg_behind_1_2"] += (
            (behind[f"rival_deg_{offset}"] > behind["Cumulative_Degradation"])
            .fillna(False)
            .astype("int8")
            .to_numpy()
        )

    # Prior-lap pit-wave intensity only; no same-lap or future aggregate pit usage.
    lap_pits = (
        out.groupby(lap_keys, observed=True)
        .agg(
            lap_pit_count=("PitStop", "sum"),
            lap_car_count=("PitStop", "size"),
        )
        .reset_index()
    )
    lap_pits = lap_pits.sort_values(race_keys + ["LapNumber"])
    lap_pits["prev_lap_pit_count"] = (
        lap_pits.groupby(race_keys, observed=True)["lap_pit_count"].shift(1).fillna(0)
    )
    lap_pits["prev_lap_car_count"] = (
        lap_pits.groupby(race_keys, observed=True)["lap_car_count"].shift(1).fillna(0)
    )
    lap_pits["prev_lap_pit_rate"] = (
        lap_pits["prev_lap_pit_count"]
        / lap_pits["prev_lap_car_count"].replace(0, np.nan)
    ).fillna(0)

    out = out.merge(
        lap_pits[lap_keys + ["prev_lap_pit_count", "prev_lap_pit_rate"]],
        on=lap_keys,
        how="left",
        sort=False,
    )
    ...
    out["undercut_pressure"] = (
        out["older_tyre_ahead_1_2"].astype("float32")
        + 0.5 * out["higher_deg_ahead_1_2"].astype("float32")
        + out["TyreLife_comp_lap_pct"].astype("float32")
        + out["Cumulative_Degradation_comp_lap_pct"].astype("float32")
    ).astype("float32")
    out["defend_from_behind_pressure"] = (
        out["older_tyre_behind_1_2"].astype("float32")
        + 0.5 * out["higher_deg_behind_1_2"].astype("float32")
        + out["prev_lap_pit_rate"].astype("float32")
    ).astype("float32")
    out["pit_wave_old_tyre_interaction"] = (
        out["prev_lap_pit_rate"] * out["TyreLife_comp_lap_pct"]
    ).astype("float32")
    out["clean_air_proxy"] = (
        (out["Position"] <= 10).astype("int8")
        - out["older_tyre_behind_1_2"].astype("int8")
        + out["older_tyre_ahead_1_2"].astype("int8")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000303"],
        "research_usage_note": "Verified assigned hypothesis 000303.",
    }, sort_keys=True))
