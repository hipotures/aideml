...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["compound_deg_order"] = (
        out["Compound"].map(compound_deg).fillna(2.0).astype("float32")
    )
    out["is_wet_compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["is_dry_compound"] = out["Compound"].map(dry_comp).fillna(1.0).astype("int8")

    eps = np.float32(1e-3)
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"].clip(lower=1) + eps)
    ).astype("float32")
    out["lap_delta_per_tyre_lap"] = (
        out["LapTime_Delta"] / (out["TyreLife"].clip(lower=1) + eps)
    ).astype("float32")
    out["tyre_life_progress_ratio"] = (
        out["TyreLife"] / (out["LapNumber"].clip(lower=1) + eps)
    ).astype("float32")
    out["late_stint_pressure"] = (
        out["TyreLife"] * out["RaceProgress"] * out["compound_deg_order"]
    ).astype("float32")
    out["position_loss_pressure"] = (
        out["Position_Change"].clip(lower=0)
        * out["TyreLife"]
        * out["compound_deg_order"]
    ).astype("float32")

    race_stats = train.groupby("Race", observed=True).agg(
        race_deg_mean=("degradation_per_tyre_lap", "mean"),
        race_delta_mean=("LapTime_Delta", "mean"),
        race_tyre_mean=("TyreLife", "mean"),
        race_wet_share=("is_wet_compound", "mean"),
        race_pit_rate=("PitStop", "mean"),
    )
    global_stats = {
        c: np.float32(race_stats[c].mean()) if len(race_stats) else np.float32(0.0)
        for c in race_stats.columns
    }
    for c in race_stats.columns:
        out[c] = (
            out["Race"].map(race_stats[c]).fillna(global_stats[c]).astype("float32")
        )

    rc_stats = train.groupby(["Race", "Compound"], observed=True).agg(
        rc_deg_mean=("degradation_per_tyre_lap", "mean"),
        rc_delta_mean=("LapTime_Delta", "mean"),
        rc_tyre_mean=("TyreLife", "mean"),
    )
    rc_index = pd.MultiIndex.from_frame(out[["Race", "Compound"]])
    for c in rc_stats.columns:
        vals = pd.Series(rc_stats[c].reindex(rc_index).to_numpy(), index=out.index)
        out[c] = vals.fillna(out[c.replace("rc_", "race_")]).astype("float32")

    deg_q1 = (
        np.float32(race_stats["race_deg_mean"].quantile(0.33))
        if len(race_stats)
        else np.float32(0.0)
    )
    deg_q2 = (
        np.float32(race_stats["race_deg_mean"].quantile(0.67))
        if len(race_stats)
        else np.float32(0.0)
    )
    out["race_deg_archetype"] = np.select(
        [
            out["race_wet_share"] > 0.25,
            out["race_deg_mean"] >= deg_q2,
            out["race_deg_mean"] <= deg_q1,
        ],
        [3, 2, 0],
        default=1,
    ).astype("int8")

    out["race_deg_centered"] = (
        out["degradation_per_tyre_lap"] - out["race_deg_mean"]
    ).astype("float32")
    out["compound_deg_centered"] = (
        out["degradation_per_tyre_lap"] - out["rc_deg_mean"]
    ).astype("float32")
    out["race_delta_centered"] = (out["LapTime_Delta"] - out["race_delta_mean"]).astype(
        "float32"
    )
    out["compound_delta_centered"] = (
        out["LapTime_Delta"] - out["rc_delta_mean"]
    ).astype("float32")

    high_deg = (out["race_deg_archetype"] == 2).astype("float32")
    low_deg = (out["race_deg_archetype"] == 0).astype("float32")
    wet_regime = (out["race_deg_archetype"] == 3).astype("float32")
    out["undercut_pressure"] = (
        high_deg * out["TyreLife"] * (out["compound_deg_centered"].clip(lower=0) + 1)
        + out["position_loss_pressure"] * 0.1
    ).astype("float32")
    out["overcut_pressure"] = (
        low_deg * out["TyreLife"] * (-out["compound_delta_centered"]).clip(lower=0)
    ).astype("float32")
    out["wet_strategy_pressure"] = (
        wet_regime
        * (out["RaceProgress"] + out["LapTime_Delta"].abs() / 100.0)
        * (out["TyreLife"] + 1)
    ).astype("float32")

    out["race_archetype_compound"] = (
        out["race_deg_archetype"].astype(str) + "_" + out["Compound"].astype(str)
    ).astype("category")
    out["race_archetype_stint"] = (
        out["race_deg_archetype"].astype(str)
        + "_"
        + out["Stint"].round().astype("Int64").astype(str)
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001018"],
        "research_usage_note": "Verified assigned hypothesis 001018.",
    }, sort_keys=True))
