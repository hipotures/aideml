...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = df["Compound"].fillna("UNKNOWN")
    is_soft = (compound == "SOFT").astype("int8")
    is_medium = (compound == "MEDIUM").astype("int8")
    is_hard = (compound == "HARD").astype("int8")
    is_intermediate = (compound == "INTERMEDIATE").astype("int8")
    is_wet = (compound == "WET").astype("int8")

    is_dry = (is_soft | is_medium | is_hard).astype("int8")
    is_crossover = (is_intermediate | is_wet).astype("int8")

    df["is_dry_regime"] = is_dry
    df["is_crossover_regime"] = is_crossover
    df["is_soft"] = is_soft
    df["is_medium"] = is_medium
    df["is_hard"] = is_hard
    df["is_intermediate"] = is_intermediate
    df["is_wet"] = is_wet

    grp_keys = ["Year", "Race", "Driver"]
    grp = df.groupby(grp_keys, sort=False)

    soft_cum = grp["is_soft"].cumsum()
    medium_cum = grp["is_medium"].cumsum()
    hard_cum = grp["is_hard"].cumsum()

    soft_seen_before = (soft_cum - df["is_soft"] > 0).astype("int8")
    medium_seen_before = (medium_cum - df["is_medium"] > 0).astype("int8")
    hard_seen_before = (hard_cum - df["is_hard"] > 0).astype("int8")

    soft_seen_now = (soft_cum > 0).astype("int8")
    medium_seen_now = (medium_cum > 0).astype("int8")
    hard_seen_now = (hard_cum > 0).astype("int8")

    dry_types_used_before = (
        soft_seen_before + medium_seen_before + hard_seen_before
    ).astype("int8")
    dry_types_used_now = (soft_seen_now + medium_seen_now + hard_seen_now).astype(
        "int8"
    )

    df["dry_types_used_before"] = dry_types_used_before
    df["dry_types_used_now"] = dry_types_used_now
    df["dry_obligation_likely_met"] = (
        (is_dry == 1) & (dry_types_used_now >= 2)
    ).astype("int8")
    df["dry_obligation_unmet"] = ((is_dry == 1) & (dry_types_used_now < 2)).astype(
        "int8"
    )

    race_progress = df["RaceProgress"].clip(lower=1e-3, upper=1.0)
    est_total_laps = (df["LapNumber"] / race_progress).clip(
        lower=df["LapNumber"], upper=90
    )
    remaining_laps = (est_total_laps - df["LapNumber"]).clip(lower=0)
    df["estimated_total_laps"] = est_total_laps.astype("float32")
    df["remaining_laps"] = remaining_laps.astype("float32")

    life_limits = compound.map(
        {
            "SOFT": 20.0,
            "MEDIUM": 26.0,
            "HARD": 32.0,
    ...
    tyre_life_after_finish = df["TyreLife"] + remaining_laps
    finish_margin = life_limits - tyre_life_after_finish
    df["current_tyre_finish_margin"] = finish_margin.astype("float32")
    df["current_compound_can_finish"] = (finish_margin >= 0).astype("int8")

    hard_fresh_can_finish = (remaining_laps <= 32.0).astype("int8")
    medium_fresh_can_finish = (remaining_laps <= 26.0).astype("int8")
    soft_fresh_can_finish = (remaining_laps <= 20.0).astype("int8")

    fresh_alt_dry_can_finish = (
        ((compound != "HARD") & (hard_fresh_can_finish == 1))
        | ((compound != "MEDIUM") & (medium_fresh_can_finish == 1))
        | ((compound != "SOFT") & (soft_fresh_can_finish == 1))
    ).astype("int8")
    df["fresh_alt_dry_can_finish"] = (
        (is_dry == 1) & (fresh_alt_dry_can_finish == 1)
    ).astype("int8")

    unseen_alt_dry_can_finish = (
        ((hard_seen_now == 0) & (compound != "HARD") & (hard_fresh_can_finish == 1))
        | (
            (medium_seen_now == 0)
            & (compound != "MEDIUM")
            & (medium_fresh_can_finish == 1)
        )
        | ((soft_seen_now == 0) & (compound != "SOFT") & (soft_fresh_can_finish == 1))
    ).astype("int8")
    df["fresh_unseen_dry_can_finish"] = (
        (is_dry == 1) & (unseen_alt_dry_can_finish == 1)
    ).astype("int8")

    df["rule_pressure_stop"] = (
        (is_dry == 1)
        & (df["dry_obligation_unmet"] == 1)
        & (df["fresh_unseen_dry_can_finish"] == 1)
        & (df["current_compound_can_finish"] == 0)
    ).astype("int8")

    df["degradation_per_lap"] = (
        df["Cumulative_Degradation"] / df["TyreLife"].clip(lower=1)
    ).astype("float32")
    df["tyre_life_ratio_to_remaining"] = (
        df["TyreLife"] / (remaining_laps + 1.0)
    ).astype("float32")
    df["lap_delta_per_progress"] = (df["LapTime_Delta"] / race_progress).astype(
        "float32"
    )

    df["dry_finish_margin"] = (finish_margin * is_dry).astype("float32")
    df["crossover_finish_margin"] = (finish_margin * is_crossover).astype("float32")
    df["dry_degradation_signal"] = (df["degradation_per_lap"] * is_dry).astype(
        "float32"
    )
    df["crossover_degradation_signal"] = (
        df["degradation_per_lap"] * is_crossover
    ).astype("float32")
    df["crossover_time_delta_signal"] = (df["LapTime_Delta"] * is_crossover).astype(
        "float32"
    )
    df["dry_rule_pressure_x_progress"] = (
        df["rule_pressure_stop"] * (1.0 - race_progress)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000806"],
        "research_usage_note": "Verified assigned hypothesis 000806.",
    }, sort_keys=True))
