...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize categorical columns while preserving them for AutoGluon models.
    ...
    est_total_laps = est_total_laps.fillna(lap).clip(lower=lap, upper=100)
    laps_remaining = (est_total_laps - lap).clip(lower=0)

    out["surv_time"] = lap
    out["surv_tyre_age"] = tyre
    out["surv_laps_remaining"] = laps_remaining
    out["surv_progress_left"] = (1.0 - progress).clip(0, 1)
    out["surv_stint_age_ratio"] = tyre / (lap + 1.0)
    out["surv_est_total_laps"] = est_total_laps

    out["hazard_tyre_bucket"] = pd.cut(
        tyre, bins=[0, 3, 8, 14, 20, 28, 38, 55, 100], labels=False, include_lowest=True
    ).astype("int16")
    out["hazard_lap_bucket"] = pd.cut(
        lap, bins=[0, 5, 12, 20, 30, 40, 52, 65, 100], labels=False, include_lowest=True
    ).astype("int16")
    out["hazard_remaining_bucket"] = pd.cut(
        laps_remaining,
        bins=[-1, 1, 3, 6, 10, 16, 25, 40, 100],
        labels=False,
        include_lowest=True,
    ).astype("int16")

    out["hazard_tyre_x_stint"] = out["hazard_tyre_bucket"].astype(float) * stint
    out["hazard_tyre_x_compound"] = (
        out["Compound"].astype(str) + "_age_" + out["hazard_tyre_bucket"].astype(str)
    ).astype("category")
    out["hazard_race_x_remaining"] = (
        out["Race"].astype(str) + "_rem_" + out["hazard_remaining_bucket"].astype(str)
    ).astype("category")

    out["deg_per_tyre_lap"] = deg / (tyre + 1.0)
    out["abs_laptime_delta"] = delta.abs()
    out["laptime_delta_per_tyre"] = delta / (tyre + 1.0)
    out["laptime_vs_est_race"] = laptime / (est_total_laps + 1.0)
    out["position_pressure"] = pos + pos_chg.abs()
    out["late_stint_pressure"] = tyre * progress
    out["hazard_load_score"] = (
        0.04 * tyre
        + 0.20 * stint
        + 0.01 * deg.clip(-200, 800)
        + 0.03 * delta.clip(-30, 60)
        - 0.03 * laps_remaining
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000164"],
        "research_usage_note": "Verified assigned hypothesis 000164.",
    }, sort_keys=True))
