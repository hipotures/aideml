...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_progress = out["RaceProgress"].clip(lower=0.005)
    total_laps_est = (out["LapNumber"] / race_progress).clip(lower=1.0, upper=100.0)
    laps_remaining = (total_laps_est - out["LapNumber"]).clip(lower=0.0)

    tyre_life = out["TyreLife"].clip(lower=1.0)
    tyre_age_at_finish = tyre_life + laps_remaining

    out["LapsRemaining_Est"] = laps_remaining
    out["Compound_FinishWindow"] = finish_window
    out["TyreLife_ToFinish_CompoundRatio"] = tyre_age_at_finish / finish_window.clip(
        lower=1.0
    )
    out["Compound_LapsBeyondFinishWindow"] = tyre_age_at_finish - finish_window
    out["Positive_LapTime_Delta"] = out["LapTime_Delta"].clip(lower=0.0)
    out["Deg_Per_TyreLife"] = out["Cumulative_Degradation"] / tyre_life
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000225"],
        "research_usage_note": "Verified assigned hypothesis 000225.",
    }, sort_keys=True))
