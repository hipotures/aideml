...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in numeric_cols:
        for lag in range(1, 6):
            out[f"{col}__lag_{lag}"] = g[col].shift(lag)

        for lag in range(1, 5):
            out[f"{col}__lagdiff_{lag}_{lag+1}"] = (
                out[f"{col}__lag_{lag}"] - out[f"{col}__lag_{lag+1}"]
            )

    out["seq_laptime_trend_3"] = out["LapTime (s)__lag_1"] - out["LapTime (s)__lag_3"]
    out["seq_delta_trend_3"] = out["LapTime_Delta__lag_1"] - out["LapTime_Delta__lag_3"]
    out["seq_deg_trend_3"] = (
        out["Cumulative_Degradation__lag_1"] - out["Cumulative_Degradation__lag_3"]
    )
    out["seq_tyrelife_trend_3"] = out["TyreLife__lag_1"] - out["TyreLife__lag_3"]
    out["seq_pos_trend_3"] = out["Position__lag_1"] - out["Position__lag_3"]

    out["seq_laptime_trend_5"] = out["LapTime (s)__lag_1"] - out["LapTime (s)__lag_5"]
    out["seq_delta_trend_5"] = out["LapTime_Delta__lag_1"] - out["LapTime_Delta__lag_5"]
    out["seq_deg_trend_5"] = (
        out["Cumulative_Degradation__lag_1"] - out["Cumulative_Degradation__lag_5"]
    )
    out["seq_tyrelife_trend_5"] = out["TyreLife__lag_1"] - out["TyreLife__lag_5"]
    out["seq_pos_trend_5"] = out["Position__lag_1"] - out["Position__lag_5"]

    out["seq_recent_pitstop_count_5"] = (
        out["PitStop__lag_1"].fillna(0)
        + out["PitStop__lag_2"].fillna(0)
        + out["PitStop__lag_3"].fillna(0)
        + out["PitStop__lag_4"].fillna(0)
        + out["PitStop__lag_5"].fillna(0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000549"],
        "research_usage_note": "Verified assigned hypothesis 000549.",
    }, sort_keys=True))
