...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    deg_factor = comp.map(compound_deg).fillna(1.0)
    fresh_factor = comp.map(compound_fresh).fillna(1.0)

    tyre_life = out["TyreLife"].astype(float).clip(lower=1)
    lap_number = out["LapNumber"].astype(float).clip(lower=1)
    race_progress = out["RaceProgress"].astype(float).clip(0.0, 1.0)
    position = out["Position"].astype(float).clip(1, 20)
    ...
    est_total_laps = (lap_number / race_progress.replace(0, np.nan)).replace(
        [np.inf, -np.inf], np.nan
    )
    est_total_laps = est_total_laps.fillna(lap_number.max()).clip(lower=lap_number)
    remaining_laps = (est_total_laps - lap_number).clip(lower=0)

    deg_per_lap = (cum_deg / tyre_life).replace([np.inf, -np.inf], 0).fillna(0)
    old_tyre_loss_next_lap = (
        deg_per_lap * deg_factor
        + 0.10 * tyre_life * deg_factor
        + 0.18 * lap_delta.clip(lower=0)
    )

    fresh_tyre_recovery = fresh_factor * (
        0.65 * old_tyre_loss_next_lap.clip(lower=0) + 0.035 * tyre_life
    ) + 0.08 * cum_deg.clip(lower=0) / (remaining_laps + 1.0)

    lost_positions = (20.0 - position).clip(lower=0) / 19.0
    high_position = (21.0 - position).clip(lower=1) / 20.0
    traffic_penalty = (
        lost_positions * (1.0 + pos_change.clip(lower=0) / 6.0)
        + 0.35 * (-pos_change).clip(lower=0) / 6.0
    )

    remaining_lap_value = np.log1p(remaining_laps) * fresh_tyre_recovery
    pit_now_value = (
        fresh_tyre_recovery + 0.08 * remaining_lap_value - 0.55 * traffic_penalty
    )
    stay_out_value = (
        old_tyre_loss_next_lap
        + 0.18 * deg_factor * np.sqrt(tyre_life)
        + 0.15 * high_position
    )

    low_deg_slope = 1.0 / (1.0 + old_tyre_loss_next_lap.clip(lower=0))
    undercut_pressure = fresh_tyre_recovery * (
        lost_positions + pos_change.clip(lower=0) / 10.0
    )
    overcut_viability = (
        low_deg_slope * high_position * (1.0 + remaining_laps / (est_total_laps + 1.0))
    )

    out["old_tyre_loss_next_lap"] = old_tyre_loss_next_lap
    out["fresh_tyre_recovery"] = fresh_tyre_recovery
    out["remaining_lap_value"] = remaining_lap_value
    out["track_position_penalty"] = traffic_penalty
    out["pit_now_value"] = pit_now_value
    out["stay_out_value"] = stay_out_value
    out["pit_economic_edge"] = pit_now_value - stay_out_value
    out["undercut_pressure"] = undercut_pressure
    out["overcut_viability"] = overcut_viability
    out["pit_edge_x_remaining"] = out["pit_economic_edge"] * np.log1p(remaining_laps)
    out["fresh_gain_x_stint"] = fresh_tyre_recovery * np.log1p(stint)
    out["old_loss_x_position"] = old_tyre_loss_next_lap * high_position
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000701"],
        "research_usage_note": "Verified assigned hypothesis 000701.",
    }, sort_keys=True))
