...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["lap_progress_gap"] = lap - progress * lap.max()
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"].astype(
        float
    ) / tyre.clip(lower=1.0)
    out["delta_per_tyre_lap"] = out["LapTime_Delta"].astype(float) / tyre.clip(
        lower=1.0
    )

    base = pd.Series(laptime, index=out.index)
    for col in ["Compound", "Race", "Year"]:
        means = out.groupby(col, observed=True)["LapTime (s)"].transform("mean")
        base = base - means + laptime.mean()

    expected = (
        base.groupby(pd.qcut(lap.rank(method="first"), q=20, duplicates="drop"))
        .transform("mean")
        .to_numpy()
        + out.groupby("Compound", observed=True)["LapTime_Delta"]
        .transform("mean")
        .to_numpy()
    ) / 2.0
    residual = laptime.to_numpy() - expected
    ...
    work = pd.DataFrame(
        {
            ...
            "_lap": lap.to_numpy(),
            "_tyre": tyre.to_numpy(),
            "_driver": out["Driver"].astype(str).to_numpy(),
            "_race": out["Race"].astype(str).to_numpy(),
            "_stint": out["Stint"].to_numpy(),
            "_compound": out["Compound"].astype(str).to_numpy(),
            "_pit": (
                out["PitStop"].to_numpy() if "PitStop" in out.columns else np.zeros(n)
            ),
        }
    )
    work = work.sort_values(
        ["_driver", "_race", "_stint", "_lap", "_orig"], kind="mergesort"
    )

    level = np.zeros(n, dtype=np.float32)
    slope = np.zeros(n, dtype=np.float32)
    accel = np.zeros(n, dtype=np.float32)
    surprise = np.zeros(n, dtype=np.float32)
    uncert = np.zeros(n, dtype=np.float32)
    since_reset = np.zeros(n, dtype=np.float32)

    q_level, q_slope, r_obs = 0.08, 0.015, 4.0

    for _, g in work.groupby(["_driver", "_race", "_stint"], sort=False, observed=True):
        x0, x1 = 0.0, 0.0
        ...
        for row in g.itertuples(index=False):
            i = int(row._orig)
            dt = 1.0 if prev_lap is None else max(1.0, float(row._lap - prev_lap))

            pred0 = x0 + dt * x1
            pred1 = x1
            pp00 = p00 + 2.0 * dt * p01 + dt * dt * p11 + q_level
            pp01 = p01 + dt * p11
            pp11 = p11 + q_slope

            innov = float(row._res) - pred0
            s = pp00 + r_obs
            k0 = pp00 / s
            k1 = pp01 / s
            ...
            level[i] = x0
            slope[i] = x1
            accel[i] = x1 - prev_slope
            surprise[i] = innov / np.sqrt(s)
            uncert[i] = np.sqrt(max(p00, 0.0))
            since_reset = reset_age + 1.0
            reset_age = 0.0 if row._pit == 1 else since_reset
            prev_slope = x1
            prev_lap = float(row._lap)

    out["latent_tyre_level"] = level
    out["latent_tyre_slope"] = slope
    out["latent_tyre_accel"] = accel
    out["latent_tyre_surprise"] = surprise
    out["latent_tyre_uncertainty"] = uncert
    out["latent_tyre_slope_x_life"] = slope * tyre.to_numpy()
    out["latent_surprise_x_life"] = surprise * tyre.to_numpy()
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000171"],
        "research_usage_note": "Verified assigned hypothesis 000171.",
    }, sort_keys=True))
