...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Driver", "Race", "Compound"]:
        X[col] = X[col].astype(str).str.upper().fillna("UNKNOWN")

    compound_map = {"HARD": 0, "MEDIUM": 1, "SOFT": 2, "INTERMEDIATE": 3, "WET": 4}
    X["Compound_Code"] = X["Compound"].map(compound_map).fillna(-1).astype("int8")
    ...
    for col in seq_cols:
        for lag in lags:
            X[f"{col}_lag{lag}"] = g[col].shift(lag)
    ...
    for col in [
        ...
        lag_cols_3 = [f"{col}_lag{k}" for k in [1, 2, 3]]
        lag_cols_5 = [f"{col}_lag{k}" for k in [1, 2, 3, 5]]
        X[f"{col}_recent3_mean"] = X[lag_cols_3].mean(axis=1)
        X[f"{col}_recent3_std"] = X[lag_cols_3].std(axis=1)
        X[f"{col}_recent5_mean"] = X[lag_cols_5].mean(axis=1)
        X[f"{col}_recent5_std"] = X[lag_cols_5].std(axis=1)

    X["LapTime_Delta_trend_1_3"] = X["LapTime_Delta_lag1"] - X["LapTime_Delta_lag3"]
    X["LapTime_Delta_trend_1_5"] = X["LapTime_Delta_lag1"] - X["LapTime_Delta_lag5"]
    X["Degradation_trend_1_3"] = (
        X["Cumulative_Degradation_lag1"] - X["Cumulative_Degradation_lag3"]
    )
    X["Degradation_trend_1_5"] = (
        X["Cumulative_Degradation_lag1"] - X["Cumulative_Degradation_lag5"]
    )
    X["Position_trend_1_3"] = X["Position_lag1"] - X["Position_lag3"]
    X["Position_trend_1_5"] = X["Position_lag1"] - X["Position_lag5"]
    X["TyreLife_trend_1_3"] = X["TyreLife_lag1"] - X["TyreLife_lag3"]

    X["recent_pit_count_3"] = X[[f"PitStop_lag{k}" for k in [1, 2, 3]]].sum(axis=1)
    X["recent_pit_count_5"] = X[[f"PitStop_lag{k}" for k in [1, 2, 3, 5]]].sum(axis=1)
    X["recent_compound_mean_3"] = X[[f"Compound_Code_lag{k}" for k in [1, 2, 3]]].mean(
        axis=1
    )
    X["recent_compound_mean_5"] = X[
        [f"Compound_Code_lag{k}" for k in [1, 2, 3, 5]]
    ].mean(axis=1)
    X["compound_changed_from_prev"] = (
        X["Compound_Code"] != X["Compound_Code_lag1"]
    ).astype("int8")

    X["pace_minus_recent3"] = X["LapTime_Delta"] - X["LapTime_Delta_recent3_mean"]
    X["pace_minus_recent5"] = X["LapTime_Delta"] - X["LapTime_Delta_recent5_mean"]
    X["degradation_minus_recent3"] = (
        X["Cumulative_Degradation"] - X["Cumulative_Degradation_recent3_mean"]
    )
    X["position_change_vs_recent3"] = (
        X["Position_Change"] - X["Position_Change_recent3_mean"]
    )

    X["stint_lap_progress"] = X["TyreLife"] / X["LapNumber"].clip(lower=1)
    X["remaining_race_laps_proxy"] = (1.0 - X["RaceProgress"]) * X["LapNumber"].clip(
        lower=1
    )
    X["degradation_per_tyre_lap"] = X["Cumulative_Degradation"] / X["TyreLife"].replace(
        0, np.nan
    )
    X["pace_x_degradation"] = X["LapTime_Delta"] * X["Cumulative_Degradation"]
    X["position_loss_pressure"] = (
        X["Position_Change_recent3_mean"] * X["LapTime_Delta_recent3_mean"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000471"],
        "research_usage_note": "Verified assigned hypothesis 000471.",
    }, sort_keys=True))
