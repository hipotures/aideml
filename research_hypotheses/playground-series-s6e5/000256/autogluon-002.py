...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g = out.groupby(grp, sort=False)

    lap_count = g["Driver"].transform("size").astype("float32")
    stop_proxy = (
        (out["TyreLife"].astype(float) <= 2) & (out["Stint"].astype(float) > 1)
    ).astype("float32")
    pit_sum = stop_proxy.groupby([out[c] for c in grp], sort=False).transform("sum").astype("float32")
    ltd_median = g["LapTime_Delta"].transform("median").astype("float32")
    ltd_std = g["LapTime_Delta"].transform("std").fillna(0).astype("float32")
    ltd_mean = g["LapTime_Delta"].transform("mean").astype("float32")

    out["rl_field_size"] = lap_count
    out["rl_pit_share"] = (
        (pit_sum / lap_count.replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["rl_pit_share_loo"] = (
        (
            (pit_sum - stop_proxy.astype("float32"))
            / (lap_count - 1).replace(0, np.nan)
        )
        .fillna(0)
        .astype("float32")
    )

    out["rl_ltd_median"] = ltd_median
    out["rl_ltd_std"] = ltd_std
    out["rl_ltd_mean"] = ltd_mean
    out["rl_ltd_vs_median"] = (out["LapTime_Delta"] - ltd_median).astype("float32")
    out["rl_ltd_z"] = (
        ((out["LapTime_Delta"] - ltd_mean) / ltd_std.replace(0, np.nan))
    ...
    extreme_pos = (
        out["LapTime_Delta"] >= (ltd_median + np.maximum(2.5, 1.5 * ltd_std))
    ).astype("int8")
    extreme_neg = (
        out["LapTime_Delta"] <= (ltd_median - np.maximum(1.5, 1.0 * ltd_std))
    ).astype("int8")
    fresh_push = ((out["TyreLife"] <= 2) & (out["LapTime_Delta"] < ltd_median)).astype(
        "int8"
    )
    very_fresh_push = (
        (out["TyreLife"] <= 3)
        & (out["LapTime_Delta"] <= (ltd_median - np.maximum(1.0, 0.5 * ltd_std)))
    ).astype("int8")

    extreme_pos_sum = (
        extreme_pos.groupby([out[c] for c in grp], sort=False)
        .transform("sum")
        .astype("float32")
    )
    extreme_neg_sum = (
        extreme_neg.groupby([out[c] for c in grp], sort=False)
        .transform("sum")
        .astype("float32")
    )
    fresh_push_sum = (
        fresh_push.groupby([out[c] for c in grp], sort=False)
        .transform("sum")
        .astype("float32")
    )
    very_fresh_push_sum = (
        very_fresh_push.groupby([out[c] for c in grp], sort=False)
        .transform("sum")
        .astype("float32")
    )

    out["rl_extreme_pos_share"] = (
        (extreme_pos_sum / lap_count.replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["rl_extreme_neg_share"] = (
        (extreme_neg_sum / lap_count.replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["rl_fresh_push_share"] = (
        (fresh_push_sum / lap_count.replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["rl_very_fresh_push_share"] = (
        (very_fresh_push_sum / lap_count.replace(0, np.nan)).fillna(0).astype("float32")
    )

    out["rl_neutralization_proxy"] = (
        0.45 * out["rl_pit_share"]
        + 0.20 * out["rl_extreme_pos_share"]
        + 0.15 * (out["rl_ltd_std"] / (out["rl_ltd_std"] + 5.0))
        + 0.10 * out["rl_fresh_push_share"]
        + 0.10 * out["rl_very_fresh_push_share"]
    ).astype("float32")

    out["rl_cheap_stop_proxy"] = (
        out["rl_neutralization_proxy"] * (1.0 - out["RaceProgress"].clip(0, 1))
    ).astype("float32")
    out["rl_late_cheap_stop_proxy"] = (
        out["rl_neutralization_proxy"] * out["RaceProgress"].clip(0, 1)
    ).astype("float32")
    out["rl_pit_wave_strength"] = (
        out["rl_pit_share"]
        * out["rl_extreme_pos_share"]
        * np.log1p(out["rl_field_size"])
    ).astype("float32")
    out["rl_fresh_wave_strength"] = (
        out["rl_fresh_push_share"]
        * out["rl_extreme_neg_share"]
        * np.log1p(out["rl_field_size"])
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000256"],
        "research_usage_note": "Verified assigned hypothesis 000256.",
    }, sort_keys=True))
