...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pos_change = out["Position_Change"].astype("float32")
    position = out["Position"].astype("float32")
    tyre_life = out["TyreLife"].astype("float32")
    lap_delta = out["LapTime_Delta"].astype("float32")

    out["pos_change_abs"] = pos_change.abs()
    out["pos_lost_amt"] = (-pos_change).clip(lower=0)
    out["pos_gained_amt"] = pos_change.clip(lower=0)

    grp = ["Year", "Race", "LapNumber"]
    field_size = out.groupby(grp)["Position"].transform("count").astype("float32")
    out["position_field_pct"] = (
        ((position - 1.0) / (field_size - 1.0).replace(0, np.nan))
        .fillna(0.0)
        .astype("float32")
    )
    ...
    out["old_for_compound"] = (tyre_life >= compound_old_cut).astype("int8")
    out["positive_laptime_delta"] = lap_delta.clip(lower=0)

    out["lost_pos_x_old_compound"] = out["pos_lost_amt"] * out["old_for_compound"]
    out["lost_pos_x_pos_lapdelta"] = out["pos_lost_amt"] * out["positive_laptime_delta"]
    out["lost_old_x_pos_lapdelta"] = (
        out["pos_lost_amt"] * out["old_for_compound"] * out["positive_laptime_delta"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000045"],
        "research_usage_note": "Verified assigned hypothesis 000045.",
    }, sort_keys=True))
