...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["_race_key"] = race_key
    X["_driver_race_key"] = driver_race_key

    X = X.sort_values(["_driver_race_key", "LapNumber"], kind="mergesort")

    pit_flag = X["PitStop"].fillna(0).astype(int).clip(0, 1)
    pit_segment = pit_flag.groupby(X["_driver_race_key"]).cumsum()
    X["LapsSincePit"] = X.groupby(["_driver_race_key", pit_segment]).cumcount()
    X["IsOutLap"] = (X["LapsSincePit"] == 0).astype("int8")
    X["FreshTyrePhase"] = (X["LapsSincePit"] <= 3).astype("int8")

    ltd = X["LapTime_Delta"].astype(float)
    deg = X["Cumulative_Degradation"].astype(float)
    tyre_life = X["TyreLife"].astype(float)

    early_12 = ltd.where(tyre_life.between(1, 2))
    early_58 = ltd.where(tyre_life.between(5, 8))
    race_early_12 = early_12.groupby(X["_race_key"]).transform("mean")
    race_early_58 = early_58.groupby(X["_race_key"]).transform("mean")
    X["Race_EarlyStint_DeltaSlope"] = (race_early_58 - race_early_12).fillna(0.0)

    X["Race_DegradationSpread"] = (
        deg.groupby(X["_race_key"]).transform("std").fillna(0.0)
    )

    fresh_13 = ltd.where(X["LapsSincePit"].between(1, 3))
    settled_48 = ltd.where(X["LapsSincePit"].between(4, 8))

    comp_fresh = fresh_13.groupby([X["_race_key"], X["Compound"]]).transform("mean")
    comp_settled = settled_48.groupby([X["_race_key"], X["Compound"]]).transform("mean")
    X["Compound_WarmupPenalty"] = (comp_fresh - comp_settled).fillna(0.0)

    X["Race_WarmupPenalty"] = (
        X["Compound_WarmupPenalty"]
        .groupby(X["_race_key"])
        .transform("mean")
        .fillna(0.0)
    )
    X["Race_FreshOutlapVar"] = (
        ltd.where(X["LapsSincePit"] <= 1)
        .groupby(X["_race_key"])
        .transform("var")
        .fillna(0.0)
    )
    ...
    def _zscore(s: pd.Series) -> pd.Series:
        mu = s.groupby(X["_race_key"]).transform("mean")
        sd = s.groupby(X["_race_key"]).transform("std").replace(0, np.nan)
        return ((s - mu) / sd).fillna(0.0)

    slope_z = _zscore(X["Race_EarlyStint_DeltaSlope"])
    spread_z = _zscore(X["Race_DegradationSpread"])
    warmup_z = _zscore(X["Race_WarmupPenalty"])
    outlap_var_z = _zscore(X["Race_FreshOutlapVar"])

    regime_logit = (
        1.25 * slope_z + 0.90 * spread_z - 1.10 * warmup_z - 0.70 * outlap_var_z
    )
    X["UndercutRegimeProb"] = 1.0 / (1.0 + np.exp(-regime_logit.clip(-8, 8)))
    X["OvercutRegimeProb"] = 1.0 - X["UndercutRegimeProb"]
    X["RegimeClass"] = (X["UndercutRegimeProb"] >= 0.5).astype("int8")

    X["TyreLife_x_Undercut"] = tyre_life * X["UndercutRegimeProb"]
    X["TyreLife_x_Overcut"] = tyre_life * X["OvercutRegimeProb"]
    X["Deg_x_Undercut"] = deg * X["UndercutRegimeProb"]
    X["Deg_x_Overcut"] = deg * X["OvercutRegimeProb"]
    X["Stint_x_Undercut"] = X["Stint"].astype(float) * X["UndercutRegimeProb"]
    X["Progress_x_Overcut"] = X["RaceProgress"].astype(float) * X["OvercutRegimeProb"]

    X["PitUrgency_Undercut"] = (
        tyre_life
        * (1.0 + X["RaceProgress"].astype(float))
        * (1.0 + X["UndercutRegimeProb"])
        * (
            1.0
            + np.maximum(deg, 0.0)
            / (np.abs(deg).groupby(X["_race_key"]).transform("mean").fillna(1.0) + 1.0)
        )
    )

    X["TrackPositionDefense_Overcut"] = (
        (21.0 - X["Position"].astype(float))
        * X["OvercutRegimeProb"]
        * (1.0 - X["RaceProgress"].astype(float))
    )

    X["WarmupRisk_x_Compound"] = (
        X["Compound"].astype(str) + "_" + X["RegimeClass"].astype(str)
    )
    X["RaceRegime_x_Compound"] = (
        X["Compound"].astype(str)
        + "_"
        + pd.cut(
            X["UndercutRegimeProb"],
    ...
    X = X.sort_index()
    return X.drop(columns=["_race_key", "_driver_race_key"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000257.",
    }, sort_keys=True))
