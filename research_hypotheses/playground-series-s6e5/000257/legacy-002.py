...
train["_orig_row"] = np.arange(len(train), dtype=np.int64)
test["_orig_row"] = np.arange(len(test), dtype=np.int64)
train["is_train"] = 1
test["is_train"] = 0
test[TARGET] = np.nan
...
early = obs[obs["TyreLife"].between(2, 12)].copy()
...
race_stats = race_stats.join(fresh_var, how="left")
...
logit = np.clip(regime["undercut_score"] - regime["overcut_score"], -30, 30)
regime["regime_prob_undercut"] = 1.0 / (1.0 + np.exp(-logit))
...
regime["regime_bucket"] = (
    pd.Series(bucket, index=regime.index).astype(float).fillna(1).astype(int)
)
...
all_df = all_df.merge(
    regime[regime_cols], left_on="race_year", right_index=True, how="left"
)

for c in regime_cols:
    if c == "regime_bucket":
        all_df[c] = all_df[c].fillna(1).astype(int)
    else:
        all_df[c] = all_df[c].fillna(regime[c].median())
...
drop_cols = [TARGET, ID_COL, "is_train", "race_year", "_orig_row"]
...
test_pred_sorted = np.zeros(len(tst), dtype=float)
fold_scores = []
...
for fold, (tr_idx, va_idx) in enumerate(cv.split(X, y, groups), 1):
    ...
    test_pred_sorted += model.predict_proba(X_test)[:, 1] / n_splits
...
# Critical bugfix: model predictions are in sorted tst order; submission must be in sample id order.
pred_by_id = pd.Series(test_pred_sorted, index=tst[ID_COL].values)
submission = sample[[ID_COL]].copy()
submission[TARGET] = pred_by_id.reindex(submission[ID_COL].values).values
if submission[TARGET].isna().any():
    missing = submission.loc[submission[TARGET].isna(), ID_COL].head().tolist()
...
oof_df = (
    pd.DataFrame(
        {
            "row": trn["_orig_row"].astype(int).values,
            "target": y,
            "prediction": oof,
        }
    )
...
result = {
    ...
    "bugfix": "reindexed sorted test predictions back to sample_submission id order",
}
