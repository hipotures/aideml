...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit = out["PitStop"].astype(int)

    # Exclude obvious pit-entry/exit and extreme-lap outliers from online wear fitting.
    valid_wear = (
        (pit == 0)
        & lap_time.between(50.0, 200.0)
        & lap_delta.between(-30.0, 30.0)
        & cum_deg.between(-100.0, 1500.0)
    )

    stint_grp = ["Year", "Race", "Driver", "Stint"]

    x = tyre_life.where(valid_wear, 0.0)
    y = cum_deg.where(valid_wear, 0.0)
    one = valid_wear.astype(float)

    g = out.groupby(stint_grp, sort=False)

    n = one.groupby([out[c] for c in stint_grp]).cumsum()
    sx = x.groupby([out[c] for c in stint_grp]).cumsum()
    sy = y.groupby([out[c] for c in stint_grp]).cumsum()
    sxx = (x * x).groupby([out[c] for c in stint_grp]).cumsum()
    sxy = (x * y).groupby([out[c] for c in stint_grp]).cumsum()

    denom = n * sxx - sx * sx
    online_slope = np.where(
        (n >= 2) & (denom > 1e-9), (n * sxy - sx * sy) / denom, np.nan
    )
    out["wear_online_slope"] = online_slope
    out["wear_clean_laps"] = n

    # Prior-year race/compound slope prior.
    prior_base = out.loc[valid_wear, ["Year", "Race", "Compound"]].copy()
    prior_base["x"] = tyre_life[valid_wear].to_numpy()
    prior_base["y"] = cum_deg[valid_wear].to_numpy()
    prior_base["xx"] = (tyre_life[valid_wear] ** 2).to_numpy()
    prior_base["xy"] = (tyre_life[valid_wear] * cum_deg[valid_wear]).to_numpy()
    prior_base["n"] = 1.0

    rc = prior_base.groupby(["Year", "Race", "Compound"], as_index=False).agg(
        n=("n", "sum"),
        sx=("x", "sum"),
        sy=("y", "sum"),
        sxx=("xx", "sum"),
    ...
    rc_denom = rc["n"] * rc["sxx"] - rc["sx"] * rc["sx"]
    rc["wear_race_compound_slope"] = np.where(
        (rc["n"] >= 5) & (rc_denom > 1e-9),
        (rc["n"] * rc["sxy"] - rc["sx"] * rc["sy"]) / rc_denom,
        np.nan,
    )
    rc = rc[["Year", "Race", "Compound", "wear_race_compound_slope"]].copy()
    rc["Year"] = rc["Year"] + 1

    cc = prior_base.groupby(["Year", "Compound"], as_index=False).agg(
        n=("n", "sum"),
        sx=("x", "sum"),
        sy=("y", "sum"),
        sxx=("xx", "sum"),
    ...
    cc_denom = cc["n"] * cc["sxx"] - cc["sx"] * cc["sx"]
    cc["wear_compound_slope_prior"] = np.where(
        (cc["n"] >= 10) & (cc_denom > 1e-9),
        (cc["n"] * cc["sxy"] - cc["sx"] * cc["sy"]) / cc_denom,
        np.nan,
    )
    cc = cc[["Year", "Compound", "wear_compound_slope_prior"]].copy()
    cc["Year"] = cc["Year"] + 1

    out = out.merge(rc, on=["Year", "Race", "Compound"], how="left")
    out = out.merge(cc, on=["Year", "Compound"], how="left")

    slope_prior = out["wear_race_compound_slope"].fillna(
        out["wear_compound_slope_prior"]
    )
    shrink_w = out["wear_clean_laps"] / (out["wear_clean_laps"] + 3.0)
    out["wear_posterior_slope"] = np.where(
        out["wear_online_slope"].notna(),
        shrink_w * out["wear_online_slope"] + (1.0 - shrink_w) * slope_prior,
        slope_prior,
    )

    # Prior-year circuit/compound expected stint-end tyre life for finish-margin.
    stint_end_mask = (pit == 1) | (out["RaceProgress"].astype(float) >= 0.995)
    end_base = out.loc[stint_end_mask, ["Year", "Race", "Compound", "TyreLife"]].copy()

    re = end_base.groupby(["Year", "Race", "Compound"], as_index=False)[
        "TyreLife"
    ].median()
    re = re.rename(columns={"TyreLife": "prior_race_compound_end_life"})
    re["Year"] = re["Year"] + 1

    ce = end_base.groupby(["Year", "Compound"], as_index=False)["TyreLife"].median()
    ce = ce.rename(columns={"TyreLife": "prior_compound_end_life"})
    ce["Year"] = ce["Year"] + 1

    out = out.merge(re, on=["Year", "Race", "Compound"], how="left")
    out = out.merge(ce, on=["Year", "Compound"], how="left")

    expected_end_life = out["prior_race_compound_end_life"].fillna(
        out["prior_compound_end_life"]
    )
    out["wear_finish_margin"] = expected_end_life - tyre_life

    prior_gap = (out["wear_online_slope"] - slope_prior).abs()
    out["wear_uncertainty"] = (1.0 / np.sqrt(out["wear_clean_laps"] + 1.0)) * (
        1.0 + prior_gap.fillna(1.0)
    )

    out["wear_progress_pressure"] = out["wear_posterior_slope"] * out[
        "RaceProgress"
    ].astype(float)
    out["wear_margin_x_progress"] = out["wear_finish_margin"] * out[
        "RaceProgress"
    ].astype(float)

    out = (
        out.sort_values("_orig_order")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000399.",
    }, sort_keys=True))
