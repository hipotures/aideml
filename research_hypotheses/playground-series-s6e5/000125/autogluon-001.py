...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Ordinal"] = (
        out["Compound"].map(compound_map).fillna(-1).astype("int8")
    )
    ...
    g = out.groupby(query, sort=False)
    group_size = g["LapNumber"].transform("size").astype("float32")
    out["Query_FieldSize"] = group_size
    ...
    for col in numeric_cols:
        ...
        out[f"{safe}_QueryRankPct"] = rank.astype("float32")
        out[f"{safe}_VsQueryMean"] = (out[col] - mean).astype("float32")
        out[f"{safe}_QueryZ"] = ((out[col] - mean) / std).fillna(0).astype("float32")
        out[f"{safe}_QueryRangePos"] = (
            ((out[col] - mn) / (mx - mn).replace(0, np.nan)).fillna(0).astype("float32")
        )

    out["OldTyre_HighDegradation"] = (
    ...
    out["PositionLoss_x_LapDelta"] = (
        out["Position_Change"] * out["LapTime_Delta"]
    ).astype("float32")
    out["FieldRelativePitUrgency"] = (
        out["TyreLife_QueryRankPct"]
        + out["Cumulative_Degradation_QueryRankPct"]
        + out["LapTime_Delta_QueryRankPct"]
        + out["RaceProgress_QueryRankPct"]
        - out["Position_QueryRankPct"]
    ).astype("float32")

    out["Driver_Freq"] = (
        out["Driver"].map(out["Driver"].value_counts()).astype("float32")
    )
    out["Race_Freq"] = out["Race"].map(out["Race"].value_counts()).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000125"],
        "research_usage_note": "Verified assigned hypothesis 000125.",
    }, sort_keys=True))
