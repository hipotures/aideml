...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_wet = out["Compound"].astype(str).isin(["WET", "INTERMEDIATE"]).astype("int8")
    is_soft = (out["Compound"].astype(str) == "SOFT").astype("int8")
    is_medium = (out["Compound"].astype(str) == "MEDIUM").astype("int8")
    is_hard = (out["Compound"].astype(str) == "HARD").astype("int8")

    rp = out["RaceProgress"].clip(0.001, 0.999)
    tyre = out["TyreLife"].clip(lower=0)
    lap = out["LapNumber"].clip(lower=1)
    stint = out["Stint"].clip(lower=1)

    out["expert_wet"] = is_wet
    out["expert_dry"] = (1 - is_wet).astype("int8")
    out["expert_soft"] = is_soft
    out["expert_medium"] = is_medium
    out["expert_hard"] = is_hard
    out["expert_first_stint"] = (stint == 1).astype("int8")
    out["expert_later_stint"] = (stint > 1).astype("int8")
    out["expert_early_race"] = (rp < 0.33).astype("int8")
    out["expert_mid_race"] = ((rp >= 0.33) & (rp < 0.67)).astype("int8")
    out["expert_late_race"] = (rp >= 0.67).astype("int8")

    out["race_progress_log"] = np.log(rp)
    out["race_progress_log1p_inv"] = np.log1p(-rp)
    out["race_progress_logit"] = np.log(rp / (1.0 - rp))
    out["tyre_life_log1p"] = np.log1p(tyre)
    out["lap_log1p"] = np.log1p(lap)

    out["tyre_life_x_progress"] = tyre * rp
    out["tyre_life_per_lap"] = tyre / lap
    out["stint_x_progress"] = stint * rp
    out["position_x_progress"] = out["Position"] * rp
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / (tyre + 1.0)
    out["lap_delta_abs"] = out["LapTime_Delta"].abs()
    out["position_change_abs"] = out["Position_Change"].abs()

    out["wet_tyre_life"] = is_wet * tyre
    out["dry_tyre_life"] = (1 - is_wet) * tyre
    out["wet_progress"] = is_wet * rp
    out["dry_progress"] = (1 - is_wet) * rp
    out["late_tyre_life"] = (rp >= 0.67).astype("int8") * tyre
    out["later_stint_tyre_life"] = (stint > 1).astype("int8") * tyre

    freq_cols = ["Driver", "Race", "Compound"]
    n = float(len(out))
    for col in freq_cols:
        out[col + "_freq"] = (
            out[col].map(out[col].value_counts(dropna=False)).astype("float32") / n
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000974.",
    }, sort_keys=True))
