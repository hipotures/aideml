...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", "Stint", "TyreLife", "_row_id"]
    out = out.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)

    out["lap_delta_clip"] = out["LapTime_Delta"].clip(-120, 120)

    race_lap_med = out.groupby(["Year", "Race", "LapNumber"])["LapTime (s)"].transform(
        "median"
    )
    out["pace_residual"] = (out["LapTime (s)"] - race_lap_med).clip(-120, 120)

    g = out.groupby(group_cols, sort=False)

    out["prior_best_lap_delta"] = g["lap_delta_clip"].transform(
        lambda s: s.shift(1).cummin()
    )
    out["prior_med_lap_delta"] = g["lap_delta_clip"].transform(
        lambda s: s.shift(1).rolling(5, min_periods=2).median()
    )

    out["prior_best_pace_resid"] = g["pace_residual"].transform(
        lambda s: s.shift(1).cummin()
    )
    out["prior_med_pace_resid"] = g["pace_residual"].transform(
        lambda s: s.shift(1).rolling(5, min_periods=2).median()
    )

    out["decay_from_best_lap_delta"] = (
        out["lap_delta_clip"] - out["prior_best_lap_delta"]
    ).fillna(0.0)
    out["decay_from_med_lap_delta"] = (
        out["lap_delta_clip"] - out["prior_med_lap_delta"]
    ).fillna(0.0)

    out["decay_from_best_pace_resid"] = (
        out["pace_residual"] - out["prior_best_pace_resid"]
    ).fillna(0.0)
    out["decay_from_med_pace_resid"] = (
        out["pace_residual"] - out["prior_med_pace_resid"]
    ).fillna(0.0)

    out["late_race_decay_flag"] = (
        (out["RaceProgress"] >= 0.7)
        & (
            (out["decay_from_best_lap_delta"] > 5.0)
            | (out["decay_from_best_pace_resid"] > 5.0)
        )
    ).astype("int8")

    out["best_so_far_missing"] = out["prior_best_lap_delta"].isna().astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000037"],
        "research_usage_note": "Verified assigned hypothesis 000037.",
    }, sort_keys=True))
