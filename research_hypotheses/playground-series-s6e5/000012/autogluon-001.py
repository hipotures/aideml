...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    life_ref = comp.map(compound_life).fillna(26.0).astype("float32")
    ...
    next_tyre_life = tyre_life + 1.0
    est_total_laps = (lap / progress).replace([np.inf, -np.inf], np.nan).fillna(lap)
    laps_remaining_now = (est_total_laps - lap).clip(lower=0)
    laps_remaining_next = (laps_remaining_now - 1.0).clip(lower=0)

    ratio_now = tyre_life / life_ref
    ratio_next = next_tyre_life / life_ref
    finish_margin_now = laps_remaining_now - (life_ref - tyre_life)
    finish_margin_next = laps_remaining_next - (life_ref - next_tyre_life)

    out["next_tyre_life"] = next_tyre_life
    out["next_laps_remaining_est"] = laps_remaining_next.astype("float32")
    out["next_compound_life_ratio"] = ratio_next.astype("float32")
    out["next_compound_life_margin"] = (next_tyre_life - life_ref).astype("float32")
    out["next_finish_margin"] = finish_margin_next.astype("float32")

    out["cross_life_80_next"] = ((ratio_now < 0.80) & (ratio_next >= 0.80)).astype(
        "int8"
    )
    out["cross_life_100_next"] = ((ratio_now < 1.00) & (ratio_next >= 1.00)).astype(
        "int8"
    )
    out["cross_finish_window_next"] = (
        (finish_margin_now < 0.0) & (finish_margin_next >= 0.0)
    ).astype("int8")
    out["late_race_life_cross_next"] = (
        (progress >= 0.70)
        & (
            (ratio_now < 0.80) & (ratio_next >= 0.80)
            | (ratio_now < 1.00) & (ratio_next >= 1.00)
        )
    ).astype("int8")
    out["late_race_finish_cross_next"] = (
        (progress >= 0.70) & (finish_margin_now < 0.0) & (finish_margin_next >= 0.0)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000012"],
        "research_usage_note": "Verified assigned hypothesis 000012.",
    }, sort_keys=True))
