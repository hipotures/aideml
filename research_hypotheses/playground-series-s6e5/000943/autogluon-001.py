...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_bins = np.array(
        [0, 3, 6, 9, 12, 15, 18, 22, 26, 30, 35, 40, 46, 53, 61, 100], dtype=float
    )
    prog_bins = np.linspace(0.0, 1.0, 21)

    df["TyreLifeBin"] = pd.cut(
        df["TyreLife"].clip(lower=0, upper=99),
        bins=tyre_bins,
        labels=False,
        include_lowest=True,
        right=True,
    ...
    df["RaceProgressBin"] = pd.cut(
        df["RaceProgress"].clip(lower=0, upper=1),
        bins=prog_bins,
        labels=False,
        include_lowest=True,
        right=True,
    ...
    df["LapInStintBin"] = pd.cut(
        (df["LapNumber"] - df["TyreLife"]).clip(lower=-5, upper=80),
        bins=[-10, 0, 5, 10, 15, 20, 30, 50, 100],
        labels=False,
        include_lowest=True,
        right=True,
    ).astype("int16")

    df["DegPerLap"] = df["Cumulative_Degradation"] / df["TyreLife"].clip(lower=1)
    ...
    df["PosLossPressure"] = (-df["Position_Change"]).clip(lower=0)
    df["StintProgress"] = df["TyreLife"] / df["LapNumber"].clip(lower=1)

    df["Race_Compound_Stint_TyreLifeBin"] = (
        df["Race"]
        + "|"
        + df["Compound"]
        + "|S"
    ...
    df["Race_Stint_Progress_TyreBin"] = (
        df["Race"]
        + "|S"
        + df["Stint"].astype(str)
        + "|P"
    ...
    df["Driver_Compound_Stint"] = (
        df["Driver"] + "|" + df["Compound"] + "|S" + df["Stint"].astype(str)
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000943.",
    }, sort_keys=True))
