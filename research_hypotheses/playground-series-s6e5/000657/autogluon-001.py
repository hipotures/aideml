...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_keys = [c for c in ["Year", "Race"] if c in out.columns]
    driver_keys = race_keys + [c for c in ["Driver"] if c in out.columns]
    lap_keys = race_keys + [c for c in ["LapNumber"] if c in out.columns]

    if "PitStop" in out.columns and "LapNumber" in out.columns and driver_keys:
        tmp = out[driver_keys + ["LapNumber", "PitStop"]].copy()
        tmp["_row"] = np.arange(n)
        tmp = tmp.sort_values(driver_keys + ["LapNumber", "_row"], kind="mergesort")
        prev = tmp.groupby(driver_keys, sort=False)["PitStop"].shift(1).fillna(0)
        pit_inc = (tmp["PitStop"].astype(float) - prev.astype(float)).clip(lower=0)
        stopped_before = tmp["PitStop"].astype(float).gt(0).astype("int8")
        out["_pit_increment"] = (
            pd.Series(pit_inc.values, index=tmp["_row"].values).sort_index().values
        )
        out["_stopped_before"] = (
            pd.Series(stopped_before.values, index=tmp["_row"].values)
            .sort_index()
            .values
        )
    elif "PitStop" in out.columns:
        out["_pit_increment"] = out["PitStop"].astype(float).clip(lower=0)
        out["_stopped_before"] = out["PitStop"].astype(float).gt(0).astype("int8")
    else:
        out["_pit_increment"] = 0.0
        out["_stopped_before"] = 0

    if lap_keys:
        lap_count = (
            out.groupby(lap_keys, sort=False)["_pit_increment"]
            .transform("size")
            .replace(0, np.nan)
        )
        out["field_pit_wave_count"] = out.groupby(lap_keys, sort=False)[
            "_pit_increment"
        ].transform("sum")
        out["field_pit_wave_share"] = out["field_pit_wave_count"] / lap_count
        out["field_stopped_share"] = out.groupby(lap_keys, sort=False)[
            "_stopped_before"
        ].transform("mean")

        lap_table = (
            out[lap_keys + ["_pit_increment"]]
            .groupby(lap_keys, sort=False, as_index=False)["_pit_increment"]
            .sum()
            .rename(columns={"_pit_increment": "_lap_pit_count"})
            .sort_values(
                race_keys + ["LapNumber"] if race_keys else ["LapNumber"],
                kind="mergesort",
            )
        )
        if race_keys:
            lap_table["field_pit_count_lag1"] = (
                lap_table.groupby(race_keys, sort=False)["_lap_pit_count"]
                .shift(1)
                .fillna(0)
            )
            lap_table["field_pit_count_lag2"] = (
                lap_table.groupby(race_keys, sort=False)["_lap_pit_count"]
                .shift(2)
                .fillna(0)
            )
            lap_table["field_pit_count_prev3"] = (
                lap_table.groupby(race_keys, sort=False)["_lap_pit_count"]
            ...
            lap_table["field_pit_count_lag1"] = (
                lap_table["_lap_pit_count"].shift(1).fillna(0)
            )
            lap_table["field_pit_count_lag2"] = (
                lap_table["_lap_pit_count"].shift(2).fillna(0)
            )
            lap_table["field_pit_count_prev3"] = (
                lap_table["_lap_pit_count"]
                .shift(1)
        ...
        out = out.merge(
            lap_table[
                ...
                + [
                    ...
                    "field_pit_count_lag2",
                    "field_pit_count_prev3",
                ]
            ],
            on=lap_keys,
            how="left",
    ...
    if lap_keys and "Stint" in out.columns:
        stint_keys = lap_keys + ["Stint"]
        out["stint_stopped_share"] = out.groupby(stint_keys, sort=False)[
            "_stopped_before"
        ].transform("mean")
        out["stint_pit_wave_share"] = out.groupby(stint_keys, sort=False)[
            "_pit_increment"
        ].transform("mean")

    if lap_keys and "Compound" in out.columns:
        compound_keys = lap_keys + ["Compound"]
        out["compound_stopped_share"] = out.groupby(compound_keys, sort=False)[
            "_stopped_before"
        ].transform("mean")
        out["compound_pit_wave_share"] = out.groupby(compound_keys, sort=False)[
            "_pit_increment"
        ].transform("mean")

    for c in [
        "field_pit_wave_count",
        "field_pit_wave_share",
        "field_stopped_share",
        "field_pit_count_lag1",
        "field_pit_count_lag2",
        "field_pit_count_prev3",
        "stint_stopped_share",
        "stint_pit_wave_share",
        "compound_stopped_share",
        "compound_pit_wave_share",
    ]:
        if c in out.columns:
            out[c] = out[c].fillna(0).astype("float32")

    if "Position" in out.columns and "field_pit_wave_share" in out.columns:
        out["pit_wave_x_position"] = out["field_pit_wave_share"] * out[
            "Position"
        ].astype(float)
    if "TyreLife" in out.columns and "field_pit_wave_share" in out.columns:
        out["pit_wave_x_tyre_life"] = out["field_pit_wave_share"] * out[
            "TyreLife"
        ].astype(float)
    if "RaceProgress" in out.columns and "field_pit_count_prev3" in out.columns:
        out["recent_pit_pressure_x_progress"] = out["field_pit_count_prev3"] * out[
            "RaceProgress"
        ].astype(float)
    if "TyreLife" in out.columns and "field_pit_count_lag1" in out.columns:
        out["lag1_pressure_x_tyre_life"] = out["field_pit_count_lag1"] * out[
            "TyreLife"
        ].astype(float)

    out = out.drop(columns=["_pit_increment", "_stopped_before"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000657"],
        "research_usage_note": "Verified assigned hypothesis 000657.",
    }, sort_keys=True))
