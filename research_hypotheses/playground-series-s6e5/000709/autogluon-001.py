...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    seq_cols = ["Driver", "Year", "Race"]
    sort_cols = seq_cols + ["LapNumber"]
    ...
    out["Compound_ord"] = (
        out["Compound"].map(compound_map).fillna(0.0).astype("float32")
    )
    ...
    for c in base_cols:
        ...
        prev1 = g[c].shift(1)
        out[f"{c}_prev1"] = prev1.astype("float32")
        out[f"{c}_d1"] = (s - prev1).astype("float32")

        for k in (3, 5, 8):
            r = prev1.groupby([out[x] for x in seq_cols], sort=False).rolling(
                k, min_periods=1
            )
            mean_v = r.mean().reset_index(level=seq_cols, drop=True)
            std_v = r.std().reset_index(level=seq_cols, drop=True).fillna(0.0)
            out[f"{c}_hist{k}_mean"] = mean_v.astype("float32")
            out[f"{c}_hist{k}_std"] = std_v.astype("float32")
            out[f"{c}_hist{k}_dev"] = (s - mean_v).astype("float32")

    for k in (3, 5, 8):
        out[f"pit_hist{k}_sum"] = (
            g["PitStop"]
            .shift(1)
            .groupby([out[x] for x in seq_cols], sort=False)
            .rolling(k, min_periods=1)
            .sum()
            .reset_index(level=seq_cols, drop=True)
            .fillna(0.0)
            .astype("float32")
        )
        out[f"compound_change_hist{k}"] = (
            (g["Compound_ord"].shift(1) != g["Compound_ord"].shift(2))
            .astype("float32")
            .groupby([out[x] for x in seq_cols], sort=False)
            .rolling(k, min_periods=1)
            .sum()
    ...
    out["laps_since_pit"] = g["PitStop"].cumsum().astype("int16")
    last_pit_lap = (
        out["LapNumber"]
    ...
    out["laps_from_last_pit"] = (out["LapNumber"] - last_pit_lap.fillna(0)).astype(
        "float32"
    )
    ...
    out["race_laps_est"] = (
        out["LapNumber"] / out["RaceProgress"].clip(lower=1e-4)
    ).astype("float32")
    out["laps_remaining_est"] = (out["race_laps_est"] - out["LapNumber"]).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000709"],
        "research_usage_note": "Verified assigned hypothesis 000709.",
    }, sort_keys=True))
