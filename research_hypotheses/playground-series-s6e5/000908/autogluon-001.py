...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Compound" in out.columns:
        ...
        out["Compound_ord"] = out["Compound"].map(comp_map).fillna(0).astype("float32")
        out["Compound_freq"] = (
            out["Compound"]
    ...
    if "Cumulative_Degradation" in out.columns:
        cd = out["Cumulative_Degradation"].astype(float)
        base = base + (
            (cd - cd.median()) / (cd.quantile(0.75) - cd.quantile(0.25) + 1e-6)
        ).clip(-8, 8)
    if "LapTime_Delta" in out.columns:
        ld = out["LapTime_Delta"].astype(float)
        base = base + 0.5 * (
            (ld - ld.median()) / (ld.quantile(0.75) - ld.quantile(0.25) + 1e-6)
        ).clip(-8, 8)
    if "LapTime (s)" in out.columns:
        lt = out["LapTime (s)"].astype(float)
        base = base + 0.25 * (
            (lt - lt.median()) / (lt.quantile(0.75) - lt.quantile(0.25) + 1e-6)
        ).clip(-8, 8)
    out["_wear_obs"] = base.astype("float32")

    group_cols = [c for c in ["Year", "Race", "Driver", "Stint"] if c in out.columns]
    sort_cols = (
        group_cols
        + [c for c in ["LapNumber", "TyreLife"] if c in out.columns]
        + ["_orig_idx"]
    )
    work = out.assign(_orig_idx=idx).sort_values(sort_cols, kind="mergesort")
    ...
    q_level, q_drift, r_obs = 0.08, 0.015, 1.0
    for _, g in work.groupby(group_cols, sort=False) if group_cols else [(None, work)]:
        m = 0.0
        d = 0.0
        p = 2.0
        last_m = 0.0
        for pos, row in zip(g["_orig_idx"].to_numpy(), g["_wear_obs"].to_numpy()):
            pred = m + d
            p_pred = p + q_level + q_drift
            e = float(row) - pred
            k = p_pred / (p_pred + r_obs)
            m = pred + k * e
            d = 0.85 * d + 0.15 * (m - last_m)
            p = (1.0 - k) * p_pred
            level[pos] = m
            drift[pos] = d
            var[pos] = p
    ...
    out["latent_wear_level"] = level
    out["latent_wear_drift"] = drift
    out["latent_wear_residual"] = resid
    out["latent_wear_var"] = var
    out["latent_wear_signal"] = (level + 2.0 * drift - 0.25 * var).astype("float32")

    if "TyreLife" in out.columns:
        out["tyrelife_sq"] = (out["TyreLife"].astype(float) ** 2).astype("float32")
        out["wear_per_tyre_lap"] = (
            out["latent_wear_level"] / (out["TyreLife"].astype(float) + 1.0)
        ).astype("float32")
    if "RaceProgress" in out.columns:
        out["late_race_wear"] = (
            out["latent_wear_level"] * out["RaceProgress"].astype(float)
        ).astype("float32")
    if "PitStop" in out.columns:
        out["prev_pitstop"] = (
            work.groupby(group_cols, sort=False)["PitStop"]
            .shift(1)
            .reindex(out.index)
            .fillna(0)
            .astype("float32")
    ...
    return out.drop(columns=["_wear_obs"], errors="ignore")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000908"],
        "research_usage_note": "Verified assigned hypothesis 000908.",
    }, sort_keys=True))
