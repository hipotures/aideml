...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    keys = ["Race", "Year", "Driver", "Stint"]
    sort_cols = keys + ["LapNumber", "__row_order"]
    work = out.sort_values(sort_cols, kind="mergesort")

    gb = work.groupby(keys, sort=False, dropna=False)
    gid = gb.ngroup()
    stint_pos = gb.cumcount() + 1

    metrics = {
        "LapTime_Delta": "ltd",
        "LapTime (s)": "lts",
        "Cumulative_Degradation": "cdeg",
    }
    cliff_cols = []
    new_cols = []

    for col, prefix in metrics.items():
        v = work[col].astype("float32")
        prev = gb[col].shift(1).astype("float32")
        diff = v - prev

        dg = diff.groupby(gid, sort=False)
        slope3 = dg.transform(lambda s: s.rolling(3, min_periods=1).median())
        slope5 = dg.transform(lambda s: s.rolling(5, min_periods=1).median())
        prior3 = dg.transform(lambda s: s.shift(3).rolling(3, min_periods=1).median())
        prior5 = dg.transform(lambda s: s.shift(5).rolling(5, min_periods=1).median())

        early = v.where(stint_pos <= 3)
        early_sum = early.fillna(0).groupby(gid, sort=False).cumsum()
        early_cnt = (stint_pos <= 3).astype("int16").groupby(gid, sort=False).cumsum()
        baseline = early_sum / early_cnt.replace(0, np.nan)

        scale = (
            diff.abs()
            .groupby(gid, sort=False)
            .transform(lambda s: s.shift(1).rolling(8, min_periods=2).median())
        )
        cliff = (
            (slope3 > prior3.fillna(0) + 1.5 * scale.fillna(0))
            & (slope3 > 0)
            & (stint_pos >= 5)
        ).astype("int8")

        names = [
            f"{prefix}_slope3_med",
            f"{prefix}_slope5_med",
            f"{prefix}_accel3_med",
            f"{prefix}_accel5_med",
            f"{prefix}_dev_stint_base3",
            f"{prefix}_cliff_flag",
        ]
        work[names[0]] = slope3
        work[names[1]] = slope5
        work[names[2]] = slope3 - prior3
        work[names[3]] = slope5 - prior5
        work[names[4]] = v - baseline
        work[names[5]] = cliff

        new_cols.extend(names)
        cliff_cols.append(names[5])

    work["stint_cliff_count"] = work[cliff_cols].sum(axis=1).astype("int8")
    work["stint_any_cliff"] = (work["stint_cliff_count"] > 0).astype("int8")
    new_cols.extend(["stint_cliff_count", "stint_any_cliff"])

    work[new_cols] = work[new_cols].replace([np.inf, -np.inf], np.nan).fillna(0)
    work = work.sort_values("__row_order", kind="mergesort").drop(
        columns=["__row_order"]
    )
    return work
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000410"],
        "research_usage_note": "Verified assigned hypothesis 000410.",
    }, sort_keys=True))
