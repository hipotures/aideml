...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["RaceProgress_bin"] = (
        pd.cut(
            out["RaceProgress"].astype(float),
            bins=[-0.01, 0.10, 0.20, 0.35, 0.50, 0.65, 0.80, 0.92, 1.01],
            labels=False,
            include_lowest=True,
        )
        .astype("int16")
    ...
    out["Race_Stint"] = (
        out["Race"].astype("string") + "_S" + out["Stint"].astype("string")
    ).astype("category")
    out["Race_ProgressBin"] = (
        out["Race"].astype("string") + "_P" + out["RaceProgress_bin"].astype("string")
    ).astype("category")

    eps = 1e-6
    out["TyreLife_x_Progress"] = out["TyreLife"] * out["RaceProgress"]
    out["TyreLife_per_Stint"] = out["TyreLife"] / (out["Stint"] + eps)
    out["Lap_to_TyreLife_Ratio"] = out["LapNumber"] / (out["TyreLife"] + eps)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000533.",
    }, sort_keys=True))
