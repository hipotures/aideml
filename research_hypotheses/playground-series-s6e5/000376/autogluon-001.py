...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap = pd.to_numeric(out["LapTime (s)"], errors="coerce").to_numpy(dtype="float64")
    delta = pd.to_numeric(out["LapTime_Delta"], errors="coerce").to_numpy(
        dtype="float64"
    )
    cumdeg = pd.to_numeric(out["Cumulative_Degradation"], errors="coerce").to_numpy(
        dtype="float64"
    )
    pit = (
    ...
    stint = (
        pd.to_numeric(out["Stint"], errors="coerce").fillna(0).to_numpy(dtype="int32")
    )
    year = pd.to_numeric(out["Year"], errors="coerce").fillna(0).to_numpy(dtype="int32")

    lap_med = np.nanmedian(lap) if np.isfinite(lap).any() else 90.0
    lap = np.where(np.isfinite(lap), lap, lap_med)
    delta = np.where(np.isfinite(delta), delta, 0.0)
    cumdeg = np.where(np.isfinite(cumdeg), cumdeg, 0.0)

    driver_code = pd.factorize(out["Driver"].astype("string").fillna(""), sort=False)[0]
    race_code = pd.factorize(out["Race"].astype("string").fillna(""), sort=False)[0]
    order = np.lexsort((np.arange(n), lapnum, stint, year, race_code, driver_code))

    level_feat = np.zeros(n, dtype="float32")
    slope_feat = np.zeros(n, dtype="float32")
    resid_feat = np.zeros(n, dtype="float32")
    uncert_feat = np.zeros(n, dtype="float32")
    alarm_feat = np.zeros(n, dtype="int8")

    if n:
        grouped = np.empty(n, dtype=bool)
        grouped[0] = True
        grouped[1:] = (
            (driver_code[order[1:]] != driver_code[order[:-1]])
            | (race_code[order[1:]] != race_code[order[:-1]])
            | (year[order[1:]] != year[order[:-1]])
            | (stint[order[1:]] != stint[order[:-1]])
        )
        starts = np.flatnonzero(grouped)
        ends = np.r_[starts[1:], n]

        for start, end in zip(starts, ends):
            level, slope = 0.0, 0.0
            p00, p01, p11 = 0.80, 0.0, 0.08
            baseline = np.nan

            for pos in order[start:end]:
                if pit[pos] >= 0.5:
                    level, slope = 0.0, 0.0
                    p00, p01, p11 = 0.60, 0.0, 0.06
                    baseline = np.nan
                    level_feat[pos] = 0.0
                    slope_feat[pos] = 0.0
                    resid_feat[pos] = 0.0
                    uncert_feat[pos] = np.sqrt(p00)
                    alarm_feat[pos] = 0
                    continue
                ...
                slow_lap = (li - baseline > max(14.0, 0.12 * baseline)) or (
                    delta[pos] > 22.0
                )
                pace_signal = np.clip((li - baseline) / 4.0, -2.5, 4.0)
                delta_signal = np.clip(delta[pos] / 6.0, -3.0, 4.0)
                deg_signal = np.sign(cumdeg[pos]) * np.log1p(abs(cumdeg[pos])) / 4.0
                deg_signal = np.clip(deg_signal, -2.0, 3.0)

                if slow_lap:
                    z = 0.20 * pace_signal + 0.10 * delta_signal + 0.20 * deg_signal
                    obs_var = 2.25
                else:
                    z = 0.55 * pace_signal + 0.30 * delta_signal + 0.15 * deg_signal
                    obs_var = 0.55

                pred_level = level + slope
                pred_slope = slope
                pp00 = p00 + 2.0 * p01 + p11 + 0.025
                pp01 = p01 + p11
                pp11 = p11 + 0.003
                ...
                level = pred_level + k0 * resid
                slope = pred_slope + k1 * resid
                p00 = max((1.0 - k0) * pp00, 1e-4)
                p01 = (1.0 - k0) * pp01
                p11 = max(pp11 - k1 * pp01, 1e-4)

                level = float(np.clip(level, -2.5, 5.5))
                slope = float(np.clip(slope, -0.75, 0.75))

                level_feat[pos] = level
                slope_feat[pos] = slope
                resid_feat[pos] = np.clip(resid, -5.0, 5.0)
                uncert_feat[pos] = np.sqrt(p00)
                alarm_feat[pos] = int(
                    (not slow_lap)
                    and (
                        (level > 1.25 and slope > 0.025)
                        or (resid > 1.15 and pred_level > 0.70)
                    )
                )

                if li < baseline:
                    baseline = 0.80 * baseline + 0.20 * li
                elif not slow_lap:
                    baseline = 0.995 * baseline + 0.005 * li

    out["latent_wear_level"] = level_feat
    out["latent_wear_slope"] = slope_feat
    out["latent_wear_residual"] = resid_feat
    out["latent_wear_uncertainty"] = uncert_feat
    out["latent_wear_alarm"] = alarm_feat
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000376"],
        "research_usage_note": "Verified assigned hypothesis 000376.",
    }, sort_keys=True))
