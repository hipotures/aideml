...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_life_bucket = out["TyreLife"].round().astype("Int16").astype(str)
    out["TyreLife_Compound"] = (
        out["Compound"].astype(str) + "_TL" + tyre_life_bucket
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000614"],
        "research_usage_note": "Verified assigned hypothesis 000614.",
    }, sort_keys=True))
