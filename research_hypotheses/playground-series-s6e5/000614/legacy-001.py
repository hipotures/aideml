...
def fit_predict(spec_name, model, mode, x_tr, y_tr, x_va, x_te):
    if mode in ("native_cat", "pandas_cat"):
        x_tr_m, x_va_m, x_te_m = (
            prepare_native_cat(x_tr),
            prepare_native_cat(x_va),
            prepare_native_cat(x_te),
        )
        fit_kwargs = {}
        ...
        x_tr_m, x_va_m, x_te_m = (
            prepare_catboost(x_tr),
            prepare_catboost(x_va),
            prepare_catboost(x_te),
        )
...
best = {"auc": -1, "weights": None, "rank": False}
...
print(f"OOF ROC AUC: {roc_auc_score(y, final_oof):.6f}")
print(
    "Best ensemble:",
...
submission[TARGET] = np.clip(final_test, 0, 1)
...
test_predictions[TARGET] = np.clip(final_test, 0, 1)
...
result = {
    "metric": "roc_auc",
    "cv_score": float(roc_auc_score(y, final_oof)),
