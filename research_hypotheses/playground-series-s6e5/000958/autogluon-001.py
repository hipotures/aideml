...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype(np.int8)
    out["IsDry"] = is_dry
    ...
    expected_life = compound.map(expected_life_map).astype(float)
    out["Expected_Compound_Life"] = expected_life

    tyre_life = out["TyreLife"].astype(float)
    lap_num = out["LapNumber"].astype(float)
    race_progress = out["RaceProgress"].astype(float).clip(0.01, 1.0)
    ...
    est_total_laps = (lap_num / race_progress).clip(lower=1.0, upper=90.0)
    laps_remaining = (est_total_laps - lap_num).clip(lower=0.0)
    out["Estimated_Total_Laps"] = est_total_laps
    out["Laps_Remaining"] = laps_remaining

    tyre_life_ratio = tyre_life / expected_life.replace(0, np.nan)
    tyre_life_ratio = tyre_life_ratio.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    out["TyreLife_ExpectedLife_Ratio"] = tyre_life_ratio * is_dry

    degradation_per_lap = degradation / tyre_life.clip(lower=1.0)
    degradation_per_lap = degradation_per_lap.replace([np.inf, -np.inf], 0.0).fillna(
        0.0
    )
    out["Degradation_Per_Lap"] = degradation_per_lap

    lap_time_loss = lap_delta.clip(lower=0.0)
    out["LapTime_Loss"] = lap_time_loss * is_dry

    wear_pressure = (
        0.55 * out["TyreLife_ExpectedLife_Ratio"]
        + 0.30 * degradation_per_lap.clip(lower=0.0)
        + 0.15 * (lap_time_loss / lap_time.clip(lower=1.0))
    )
    out["Wear_Pressure"] = wear_pressure * is_dry

    laps_to_expected_limit = (expected_life - tyre_life).clip(lower=0.0)
    out["Laps_To_Expected_Limit"] = laps_to_expected_limit

    laps_remaining_after_expected_life = laps_remaining - laps_to_expected_limit
    out["LapsRemaining_AfterExpectedLife"] = laps_remaining_after_expected_life * is_dry

    out["ExpectedLife_Exceeded"] = (
        (tyre_life > expected_life).astype(np.int8) * is_dry
    ).astype(np.int8)
    out["High_Wear_Endgame"] = (
        (out["TyreLife_ExpectedLife_Ratio"] > 0.9)
        & (laps_remaining > 3.0)
        & (is_dry == 1)
    ).astype(np.int8)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000958.",
    }, sort_keys=True))
