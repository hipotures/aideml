...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Compound" in out.columns:
        ...
        out["is_medium"] = (comp == "MEDIUM").astype("int8")
        out["is_hard"] = (comp == "HARD").astype("int8")
        out["is_wet_or_inter"] = comp.isin(["WET", "INTERMEDIATE"]).astype("int8")

    if {"Race", "LapNumber"}.issubset(out.columns):
        glap = [out["Race"], out["LapNumber"]]
        n = (
            out.groupby(glap, sort=False)["Position"]
            .transform("count")
            .astype("float32")
        )
        pos_rank = (
        ...
        out["race_lap_field_size"] = n
        out["race_lap_pos_pct"] = ((pos_rank - 1.0) / (n - 1.0 + eps)).astype("float32")

        for col, asc, name in [
            ("LapTime_Delta", True, "delta"),
            ("TyreLife", False, "tyrelife_old"),
            ("Cumulative_Degradation", False, "degradation"),
            ("LapTime (s)", True, "laptime"),
        ]:
            if col in out.columns:
                r = (
                    out.groupby(glap, sort=False)[col]
                    .rank(method="average", ascending=asc)
                    .astype("float32")
                )
                out[f"race_lap_{name}_rank"] = r
                out[f"race_lap_{name}_pct"] = ((r - 1.0) / (n - 1.0 + eps)).astype(
                    "float32"
                )
                med = (
                    out.groupby(glap, sort=False)[col]
                ...
                out[f"race_lap_{name}_vs_median"] = (out[col] - med).astype("float32")

        if {"Position", "TyreLife", "LapTime_Delta"}.issubset(out.columns):
            key = pd.MultiIndex.from_frame(out[["Race", "LapNumber", "Position"]])
            tyre_map = pd.Series(out["TyreLife"].to_numpy(), index=key)
            delta_map = pd.Series(out["LapTime_Delta"].to_numpy(), index=key)
            prev_key = pd.MultiIndex.from_arrays(
                [out["Race"], out["LapNumber"], out["Position"] - 1]
            )
            next_key = pd.MultiIndex.from_arrays(
                [out["Race"], out["LapNumber"], out["Position"] + 1]
            )

            leader_tyre = tyre_map.reindex(prev_key).to_numpy(dtype="float32")
            follower_tyre = tyre_map.reindex(next_key).to_numpy(dtype="float32")
            leader_delta = delta_map.reindex(prev_key).to_numpy(dtype="float32")
            follower_delta = delta_map.reindex(next_key).to_numpy(dtype="float32")

            out["leader_tyre_gap"] = (
                out["TyreLife"].to_numpy(dtype="float32") - leader_tyre
            ).astype("float32")
            out["follower_tyre_gap"] = (
                out["TyreLife"].to_numpy(dtype="float32") - follower_tyre
            ).astype("float32")
            out["leader_delta_gap"] = (
                out["LapTime_Delta"].to_numpy(dtype="float32") - leader_delta
            ).astype("float32")
            out["follower_delta_gap"] = (
                out["LapTime_Delta"].to_numpy(dtype="float32") - follower_delta
            ).astype("float32")

            has_leader = np.isfinite(leader_tyre)
            has_follower = np.isfinite(follower_tyre)
            out["has_close_rival_context"] = (has_leader | has_follower).astype("int8")
            out["vulnerable_to_undercut"] = (
                has_follower
                & (out["follower_tyre_gap"].to_numpy() > 3.0)
                & (out["follower_delta_gap"].to_numpy() > 0.0)
                & (out["RaceProgress"].to_numpy(dtype="float32") > 0.15)
                & (out["RaceProgress"].to_numpy(dtype="float32") < 0.90)
            ).astype("int8")
            out["cover_stop_pressure"] = (
                has_leader
                & (out["leader_tyre_gap"].to_numpy() > 2.0)
                & (out["leader_delta_gap"].to_numpy() > 0.0)
                & (out["Position"].to_numpy(dtype="float32") <= 12.0)
            ).astype("int8")
            out["rival_pressure_score"] = (
                np.nan_to_num(out["follower_tyre_gap"].to_numpy(), nan=0.0)
                + np.nan_to_num(out["follower_delta_gap"].to_numpy(), nan=0.0)
                + 0.5 * np.nan_to_num(out["leader_tyre_gap"].to_numpy(), nan=0.0)
            ).astype("float32")
    ...
    if {"TyreLife", "Stint"}.issubset(out.columns):
        out["tyrelife_per_stint"] = (out["TyreLife"] / (out["Stint"] + eps)).astype(
            "float32"
        )
    if {"Position", "Position_Change"}.issubset(out.columns):
        out["position_pressure_band"] = (
            out["Position"] + 0.5 * out["Position_Change"]
        ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000753"],
        "research_usage_note": "Verified assigned hypothesis 000753.",
    }, sort_keys=True))
