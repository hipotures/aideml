...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in feats:
        ...
        for w in (3, 5):
            out[f"{col}_ewm{w}"] = g[col].transform(
                lambda x, span=w: x.ewm(span=span, adjust=False).mean()
            )
            out[f"{col}_std{w}"] = g[col].transform(
                lambda x, win=w: x.rolling(win, min_periods=2).std()
            )
            out[f"{col}_slope{w}"] = (s - g[col].shift(w - 1)) / max(w - 1, 1)

            prior_mean = g[col].transform(
                lambda x, win=w: x.shift(1).rolling(win, min_periods=2).mean()
            )
            prior_std = g[col].transform(
                lambda x, win=w: x.shift(1).rolling(win, min_periods=2).std()
            )
            out[f"{col}_shock{w}"] = (
                (s - prior_mean).abs() > (2.0 * prior_std)
            ).astype("int8")

            out[f"{col}_max_det{w}"] = g[col].transform(
                lambda x, win=w: x.diff().rolling(win, min_periods=1).max()
            )
