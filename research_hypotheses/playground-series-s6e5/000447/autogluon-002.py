...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", "__row_id__"]
    ...
    group_key_series = [out[c] for c in group_cols]
    group_levels = list(range(len(group_cols)))
    ...
    for col in feats:
        ...
        out[f"{col}_accel2"] = s - 2 * lag1 + lag2
        ...
        for w in (3, 5):
            out[f"{col}_ewm{w}"] = (
                ...
                .mean()
                .reset_index(level=group_levels, drop=True)
            )
            roll = g[col].rolling(w, min_periods=2)
            out[f"{col}_std{w}"] = (
                roll.std().reset_index(level=group_levels, drop=True)
            )
            out[f"{col}_slope{w}"] = (s - g[col].shift(w - 1)) / max(w - 1, 1)

            prior_roll = lag1.groupby(group_key_series, sort=False).rolling(
                w, min_periods=2
            )
            prior_mean = prior_roll.mean().reset_index(level=group_levels, drop=True)
            prior_std = prior_roll.std().reset_index(level=group_levels, drop=True)
            out[f"{col}_shock{w}"] = (
                (s - prior_mean).abs() > (2.0 * prior_std)
            ).astype("int8")

            out[f"{col}_max_det{w}"] = (
                delta1.groupby(group_key_series, sort=False)
                .rolling(w, min_periods=1)
                .max()
                .reset_index(level=group_levels, drop=True)
            )

    out = out.sort_values("__row_id__", kind="mergesort").drop(columns="__row_id__")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000447"],
        "research_usage_note": "Verified assigned hypothesis 000447.",
    }, sort_keys=True))
