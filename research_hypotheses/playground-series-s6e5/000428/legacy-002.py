...
def candidate_window(df, stage1_pred=None, train_mode=False):
    ...
    if stage1_pred is not None:
        q = 0.78 if train_mode else 0.75
        base |= stage1_pred >= np.quantile(stage1_pred, q)
...
cat_model_cols = [c for c in cat_cols if c in train_fe.columns]
...
for fold, (tr_idx, va_idx) in enumerate(cv.split(train_fe, y, groups), 1):
    ...
    if y2_tr.sum() > 0 and len(np.unique(y2_tr)) == 2:
        ...
        if cand_va.sum() > 20 and len(np.unique(y_va[cand_va])) == 2:
            eval_set = [(X_va.loc[cand_va], y_va[cand_va])]
    ...
    fold_pred = p1_va.copy()
    fold_pred[cand_va] = 0.55 * p1_va[cand_va] + 0.45 * p2_va[cand_va]
    oof_final[va_idx] = fold_pred
