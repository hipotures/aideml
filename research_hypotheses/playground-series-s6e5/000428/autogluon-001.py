...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_wet_compound"] = comp.isin(["INTERMEDIATE", "WET"]).astype(np.int8)
    out["is_soft_compound"] = (comp == "SOFT").astype(np.int8)
    out["compound_stop_risk"] = compound_risk.astype(np.float32)

    out["tyre_x_compound_risk"] = (tyre * compound_risk).astype(np.float32)
    out["stint_progress"] = (stint * progress).astype(np.float32)
    out["tyre_progress"] = (tyre * progress).astype(np.float32)
    out["degradation_per_tyre_lap"] = (degr / (tyre + 1.0)).astype(np.float32)
    out["lap_delta_abs"] = ltd.abs().astype(np.float32)
    out["position_change_abs"] = poschg.abs().astype(np.float32)

    plausible_age = tyre >= np.select(
        [
            comp.eq("SOFT"),
            comp.eq("MEDIUM"),
            comp.eq("HARD"),
            comp.isin(["INTERMEDIATE", "WET"]),
        ],
        [8.0, 12.0, 16.0, 3.0],
        default=10.0,
    )
    late_enough = (progress >= 0.18) | (stint >= 2)
    degradation_signal = degr >= np.nanpercentile(degr.to_numpy(), 62)
    pace_signal = ltd >= np.nanpercentile(ltd.to_numpy(), 70)
    change_signal = poschg <= -1

    candidate = (
        plausible_age
        & late_enough
        & (degradation_signal | pace_signal | change_signal | (tyre >= 20))
    )
    hard_negative_proxy = (
        (~candidate)
        & late_enough
        & ((tyre >= 0.75 * np.nanmedian(tyre)) | pace_signal | degradation_signal)
    )

    out["pit_candidate_window"] = candidate.astype(np.int8)
    out["hard_negative_proxy"] = hard_negative_proxy.astype(np.int8)
    out["candidate_tyre_life"] = (tyre * candidate.astype(float)).astype(np.float32)
    out["candidate_degradation"] = (degr * candidate.astype(float)).astype(np.float32)
    out["candidate_lap_delta"] = (ltd * candidate.astype(float)).astype(np.float32)
    out["candidate_position_loss"] = (
        (-poschg).clip(lower=0) * candidate.astype(float)
    ).astype(np.float32)

    out["late_stint_pressure"] = (
        compound_risk * np.log1p(tyre) * (0.5 + progress) * (1.0 + 0.15 * stint)
    ).astype(np.float32)
    out["stop_pressure_score"] = (
        0.45 * np.log1p(tyre) * compound_risk
        + 0.25 * np.log1p(np.maximum(degr, 0))
        + 0.15 * np.maximum(ltd, 0)
        + 0.10 * np.maximum(-poschg, 0)
        + 0.05 * progress * 10.0
    ).astype(np.float32)

    out["race_freq"] = race.map(race.value_counts(normalize=True)).astype(np.float32)
    out["driver_freq"] = driver.map(driver.value_counts(normalize=True)).astype(
        np.float32
    )
    out["race_driver_freq"] = (
        (race + "|" + driver)
        .map((race + "|" + driver).value_counts(normalize=True))
        .astype(np.float32)
    )

    out["position_inv"] = (1.0 / (pos + 1.0)).astype(np.float32)
    out["front_runner_candidate"] = ((pos <= 5) & candidate).astype(np.int8)
    out["traffic_candidate"] = ((pos >= 12) & candidate).astype(np.int8)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000428.",
    }, sort_keys=True))
