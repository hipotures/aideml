...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Compound", "Driver", "Race"]:
        ...
        out[f"{c}_freq"] = out[c].map(freq).astype("float32")

    race_key = out["Year"].astype("string") + "_" + out["Race"].astype("string")
    out["Year_Race"] = race_key.astype("category")
    out["Year_Race_freq"] = race_key.map(race_key.value_counts()).astype("float32")
    ...
    out["tyrelife_x_progress"] = tyre * progress
    out["stint_x_tyrelife"] = stint * tyre
    out["stint_x_progress"] = stint * progress
    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["laptime_per_progress"] = lap_time / (progress + 0.01)
    out["abs_laptime_delta"] = delta.abs()
    out["abs_position_change"] = out["Position_Change"].astype("float32").abs()
    out["position_pressure"] = (21.0 - pos) / 20.0

    max_lap = (
        out.groupby(["Year", "Race"], observed=True)["LapNumber"]
        .transform("max")
        .astype("float32")
    )
    out["race_lap_remaining"] = max_lap - lap
    out["race_lap_fraction"] = lap / (max_lap + 1.0)

    out["embargo_near_prev_lap"] = (lap <= 2).astype("int8")
    out["embargo_near_next_lap"] = (out["race_lap_remaining"] <= 2).astype("int8")
    out["ordered_race_lap_block"] = (
        out["Year"].astype("int16") * 1000 + lap.astype("int16")
    ).astype("int32")

    out["is_dry_compound"] = (
        out["Compound"].astype("string").isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    )
    out["is_wet_compound"] = (
        out["Compound"].astype("string").isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000980.",
    }, sort_keys=True))
