...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_slick = comp.isin(slicks).astype("int8")
    is_wet = comp.isin(wets).astype("int8")
    out["is_slick"] = is_slick
    out["is_wet_or_inter"] = is_wet

    keys = ["Year", "Race", "Driver"]
    if "LapNumber" in out.columns:
        max_lap_by_seq = out.groupby(keys, sort=False)["LapNumber"].transform("max")
        est_total_laps = np.maximum(
            max_lap_by_seq,
            np.ceil(out["LapNumber"] / out["RaceProgress"].clip(0.01, 1.0)),
        )
        out["laps_remaining_est"] = (est_total_laps - out["LapNumber"]).clip(lower=0)
    else:
        out["laps_remaining_est"] = (
            (1.0 - out["RaceProgress"]) / out["RaceProgress"].clip(0.01, 1.0)
        ).clip(lower=0)

    out["race_had_wet_so_far"] = (
        out.groupby(keys, sort=False)["is_wet_or_inter"]
        .cumsum()
        .clip(upper=1)
        .astype("int8")
    )
    dry_regime = (out["race_had_wet_so_far"] == 0).astype("int8")
    out["dry_rule_active"] = (dry_regime & (is_slick == 1)).astype("int8")

    for c in ("SOFT", "MEDIUM", "HARD"):
        cur = ((comp == c) & (is_slick == 1)).astype("int8")
        prev_seen = (
            cur.groupby([out[k] for k in keys], sort=False).cumsum().shift(1).fillna(0)
        )
        seq_start = out.groupby(keys, sort=False).cumcount().eq(0)
        prev_seen = prev_seen.mask(seq_start, 0).clip(upper=1).astype("int8")
        out[f"prior_used_{c.lower()}"] = prev_seen
        out[f"used_{c.lower()}_so_far"] = (prev_seen | cur.astype(bool)).astype("int8")

    prior_cols = [f"prior_used_{c.lower()}" for c in ("SOFT", "MEDIUM", "HARD")]
    used_cols = [f"used_{c.lower()}_so_far" for c in ("SOFT", "MEDIUM", "HARD")]

    out["prior_distinct_slicks"] = out[prior_cols].sum(axis=1).astype("int8")
    out["distinct_slicks_so_far"] = out[used_cols].sum(axis=1).astype("int8")
    out["mandatory_second_slick_satisfied"] = (
        (out["distinct_slicks_so_far"] >= 2) | (out["dry_rule_active"] == 0)
    ).astype("int8")

    current_seen_before = np.zeros(len(out), dtype=np.int8)
    for c in ("SOFT", "MEDIUM", "HARD"):
        current_seen_before |= (
            ((comp == c).to_numpy()) & (out[f"prior_used_{c.lower()}"].to_numpy() == 1)
        ).astype(np.int8)
    out["current_slick_repeats_prior"] = (
        current_seen_before & is_slick.to_numpy()
    ).astype("int8")

    out["legal_slick_options_remaining"] = np.where(
        out["dry_rule_active"].eq(1), 3 - out["distinct_slicks_so_far"], 0
    ).astype("int8")

    tyre_life = out["TyreLife"].astype(float)
    laps_rem = out["laps_remaining_est"].astype(float)
    needs_second = (
        (out["dry_rule_active"] == 1) & (out["mandatory_second_slick_satisfied"] == 0)
    ).astype("int8")

    out["must_change_compound_pressure"] = (
        needs_second * (laps_rem <= np.maximum(3.0, tyre_life * 0.35))
    ).astype("int8")
    out["one_stop_legal_window_pressure"] = (
        needs_second * (laps_rem / (tyre_life + 1.0))
    ).clip(0, 5)
    out["two_stop_legal_path_pressure"] = needs_second * (
        (laps_rem <= 2.0 * np.maximum(8.0, tyre_life * 0.55)) & (laps_rem > 3)
    ).astype("int8")
    out["late_unsatisfied_dry_rule"] = (
        needs_second * (out["RaceProgress"].astype(float) >= 0.72)
    ).astype("int8")
    out["slick_diversity_pressure"] = (
        np.where(out["dry_rule_active"].eq(1), 2 - out["distinct_slicks_so_far"], 0)
        .clip(0, 2)
        .astype("int8")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000264"],
        "research_usage_note": "Verified assigned hypothesis 000264.",
    }, sort_keys=True))
