...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound"] = out["Compound"].str.upper()
    out["Driver"] = out["Driver"].str.upper()
    ...
    out["race_lap_query_group"] = (
        out["Year"].astype("Int64").astype("string")
        + "|"
        + out["Race"].astype("string")
        + "|"
        + out["LapNumber"].astype("Int64").astype("string")
    )

    q_keys = ["Year", "Race", "LapNumber"]
    qg = out.groupby(q_keys, sort=False)
    out["race_lap_query_size"] = qg["LapNumber"].transform("size").astype("int32")
    ...
    rank_cols = {
        ...
        "Stint": "stint",
        "PitStop": "current_pitstop",
    }

    for col, name in rank_cols.items():
        s = out[col].astype("float32")
        mean = qg[col].transform("mean").astype("float32")
        std = qg[col].transform("std").replace(0, np.nan).astype("float32")
        out[f"{name}_query_delta"] = (s - mean).fillna(0).astype("float32")
        out[f"{name}_query_z"] = (
            ((s - mean) / std)
        ...
        out[f"{name}_query_pct_rank"] = (
            qg[col].rank(method="average", pct=True).fillna(0.5).astype("float32")
        )

    out["abs_position_change"] = out["Position_Change"].abs().astype("float32")
    out["abs_position_change_query_pct_rank"] = (
        qg["abs_position_change"]
        .rank(method="average", pct=True)
        .fillna(0.5)
        .astype("float32")
    )

    out["race_lap_compound_stint_group"] = (
        out["race_lap_query_group"].astype("string")
        + "|"
        + out["Compound"].astype("string")
        + "|"
        + out["Stint"].astype("Int64").astype("string")
    )

    b_keys = ["Year", "Race", "LapNumber", "Compound", "Stint"]
    bg = out.groupby(b_keys, sort=False)
    out["compound_stint_query_size"] = bg["LapNumber"].transform("size").astype("int32")
    ...
    for col, name in {
        ...
        s = out[col].astype("float32")
        bmean = bg[col].transform("mean").astype("float32")
        out[f"{name}_compound_stint_delta"] = (s - bmean).fillna(0).astype("float32")
        out[f"{name}_compound_stint_pct_rank"] = (
            bg[col].rank(method="average", pct=True).fillna(0.5).astype("float32")
        )

    proxy_cols = [
        "tyrelife_query_pct_rank",
        "degradation_query_pct_rank",
        "laptime_delta_query_pct_rank",
        "laptime_query_pct_rank",
        "abs_position_change_query_pct_rank",
        "tyrelife_compound_stint_pct_rank",
        "degradation_compound_stint_pct_rank",
        "laptime_delta_compound_stint_pct_rank",
    ]
    out["within_lap_ranker_proxy"] = out[proxy_cols].mean(axis=1).astype("float32")

    for col in [
        "Compound",
        "Driver",
        "Race",
        "race_lap_query_group",
        "race_lap_compound_stint_group",
    ]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000864"],
        "research_usage_note": "Verified assigned hypothesis 000864.",
    }, sort_keys=True))
