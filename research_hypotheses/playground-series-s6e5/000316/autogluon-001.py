...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry = compound.isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")

    out["Is_WetWeather"] = is_wet
    out["Is_DryCompound"] = is_dry
    out["Wet_Exemption_Flag"] = is_wet

    prev_is_wet = g["Is_WetWeather"].shift(1).fillna(0).astype("int8")
    out["Wet_To_Dry_Transition"] = ((is_dry == 1) & (prev_is_wet == 1)).astype("int8")

    total_laps_est = np.where(
        out["RaceProgress"] > 0,
        out["LapNumber"] / out["RaceProgress"].clip(lower=1e-6),
        out["LapNumber"],
    )
    total_laps_est = np.clip(total_laps_est, out["LapNumber"], 90)
    out["LapsRemaining_Est"] = np.clip(total_laps_est - out["LapNumber"], 0, 90)

    for c in ["HARD", "MEDIUM", "SOFT"]:
        cur = (compound == c).astype("int8")
        prior_used = g[
            (
                cur.name
                if hasattr(cur, "name") and cur.name in out.columns
                else "LapNumber"
            )
        ].transform(lambda s: 0)
        prior_used = (
            g[cur.rename(f"_is_{c.lower()}")].shift(1).fillna(0) if False else None
        )
    ...
    out["_is_hard"] = is_hard
    out["_is_medium"] = is_medium
    out["_is_soft"] = is_soft

    out["_hard_used_prior"] = (
        g["_is_hard"].cumsum().sub(out["_is_hard"]).gt(0).astype("int8")
    )
    out["_medium_used_prior"] = (
        g["_is_medium"].cumsum().sub(out["_is_medium"]).gt(0).astype("int8")
    )
    out["_soft_used_prior"] = (
        g["_is_soft"].cumsum().sub(out["_is_soft"]).gt(0).astype("int8")
    )

    out["Current_Compound_Already_Used"] = (
        (is_hard & out["_hard_used_prior"])
        | (is_medium & out["_medium_used_prior"])
        | (is_soft & out["_soft_used_prior"])
    ).astype("int8")

    out["_hard_used_incl"] = g["_is_hard"].cumsum().gt(0).astype("int8")
    out["_medium_used_incl"] = g["_is_medium"].cumsum().gt(0).astype("int8")
    out["_soft_used_incl"] = g["_is_soft"].cumsum().gt(0).astype("int8")

    out["Used_Dry_Set_Size"] = (
        out["_hard_used_incl"] + out["_medium_used_incl"] + out["_soft_used_incl"]
    ).astype("int8")

    out["Remaining_Required_Dry_Count"] = np.where(
        is_dry == 1,
        np.clip(2 - out["Used_Dry_Set_Size"], 0, 2),
        0,
    ).astype("int8")

    out["Stop_Debt_If_Dry"] = (
        (is_dry == 1) & (out["Remaining_Required_Dry_Count"] > 0)
    ).astype("int8")

    out["Laps_Remaining_When_Still_Illegal"] = (
        out["LapsRemaining_Est"] * out["Stop_Debt_If_Dry"]
    ).astype("float32")

    out["Legality_Laps_Interaction"] = (
        out["Remaining_Required_Dry_Count"] * out["LapsRemaining_Est"]
    ).astype("float32")
    out["Legality_TyreLife_Interaction"] = (
        out["Stop_Debt_If_Dry"] * out["TyreLife"]
    ).astype("float32")
    out["Legality_Degradation_Interaction"] = (
        out["Stop_Debt_If_Dry"] * out["Cumulative_Degradation"]
    ).astype("float32")
    out["Repeated_Dry_Long_Stint"] = (
        out["Current_Compound_Already_Used"] * out["TyreLife"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000316.",
    }, sort_keys=True))
