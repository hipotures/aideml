...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in cat_cols:
        out[col] = (
            out[col].astype("string").fillna("missing").str.strip().astype("category")
        )

    out["Year_str"] = out["Year"].astype("int64").astype("string")
    out["Stint_str"] = out["Stint"].astype("int64").astype("string")

    out["Race_Compound"] = (
        out["Race"].astype("string") + "_" + out["Compound"].astype("string")
    ).astype("category")
    out["Driver_Race"] = (
        out["Driver"].astype("string") + "_" + out["Race"].astype("string")
    ).astype("category")
    out["Year_Race"] = (out["Year_str"] + "_" + out["Race"].astype("string")).astype(
        "category"
    )
    out["Compound_Stint"] = (
        out["Compound"].astype("string") + "_" + out["Stint_str"]
    ).astype("category")
    out["Driver_Compound"] = (
    ...
    if "PitStop" in out.columns:
        grp = [out["Year"], out["Race"], out["Driver"]]
        pit = out["PitStop"].fillna(0).astype(np.int16)
        out["PriorPitStops"] = pit.groupby(grp, sort=False).cumsum() - pit
        out["HasPriorPitStop"] = (out["PriorPitStops"] > 0).astype(np.int8)

    out["TyreLife_x_Stint"] = out["TyreLife"] * out["Stint"]
    out["Progress_x_TyreLife"] = out["RaceProgress"] * out["TyreLife"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000923.",
    }, sort_keys=True))
