...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected_life = comp.map(compound_expected_life).fillna(26.0).astype(float)
    wear_factor = comp.map(compound_wear).fillna(1.0).astype(float)
    ...
    pos_change = out["Position_Change"].astype(float).fillna(0)
    cum_deg = out["Cumulative_Degradation"].astype(float).fillna(0)
    lap_delta = out["LapTime_Delta"].astype(float).fillna(0)
    stint = out["Stint"].astype(float).clip(lower=1)
    current_stop = out["PitStop"].astype(float).fillna(0).clip(0, 1)
    ...
    deg_per_tyre_lap = (cum_deg / tyre_life).replace([np.inf, -np.inf], 0).fillna(0)
    pace_pressure = (
        0.28 * lap_delta.clip(lower=0, upper=30)
        + 0.05 * cum_deg.clip(lower=0, upper=500) / np.sqrt(tyre_life + 1.0)
        + 0.15 * pos_change.clip(lower=0, upper=8)
    )
    life_margin = expected_life - tyre_life
    finish_margin = remaining_laps - life_margin.clip(lower=0)
    horizon_proxy = (
        life_margin
        - 0.85 * pace_pressure * wear_factor
        - 0.35 * deg_per_tyre_lap.clip(lower=0, upper=30)
        - 0.20 * stint
    ).clip(lower=0, upper=12)

    horizon_logits = {}
    for h in (1, 3, 5, 8):
        logit = (h - horizon_proxy) / 1.75
        horizon_logits[h] = 1.0 / (1.0 + np.exp(-logit.clip(-12, 12)))
        out[f"aux_horizon_le_{h}_proxy"] = horizon_logits[h].astype("float32")

    out["aux_stop_time_proxy"] = horizon_proxy.astype("float32")
    out["aux_stop_urgency_proxy"] = (12.0 - horizon_proxy).astype("float32")
    out["aux_finish_margin_proxy"] = finish_margin.astype("float32")
    out["aux_service_life_margin"] = life_margin.astype("float32")
    out["aux_pace_pressure_proxy"] = pace_pressure.astype("float32")
    out["aux_horizon_stack_mean"] = (
        0.45 * horizon_logits[1]
        + 0.25 * horizon_logits[3]
        + 0.18 * horizon_logits[5]
        + 0.12 * horizon_logits[8]
    ).astype("float32")
    out["aux_borderline_window"] = (
        (horizon_proxy <= 5.0) & (horizon_proxy >= 1.0)
    ).astype("int8")
    out["aux_fresh_reset_current"] = ((current_stop > 0) & (tyre_life <= 3)).astype(
        "int8"
    )
    out["aux_position_horizon_pressure"] = (
        out["aux_horizon_stack_mean"] * ((21.0 - position) / 20.0)
    ).astype("float32")
