...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = out["RaceProgress"].clip(lower=1e-6)
    est_total_laps = out["LapNumber"] / progress
    laps_remaining_now = est_total_laps - out["LapNumber"]
    laps_remaining_next = np.maximum(laps_remaining_now - 1.0, 0.0)
    ...
    for compound, life in slick_life.items():
        feasible_now = laps_remaining_now <= life
        feasible_next = laps_remaining_next <= life
        newly_viable = (~feasible_now) & feasible_next

        col = f"final_window_entry_{compound.lower()}"
        out[col] = newly_viable.astype("int8")
        new_flags.append(out[col].to_numpy())

        margin_col = f"fresh_{compound.lower()}_finish_margin"
        out[margin_col] = (life - laps_remaining_now).astype("float32")
        margins_now.append(out[margin_col].to_numpy())

    flag_matrix = np.column_stack(new_flags)
    margin_matrix = np.column_stack(margins_now)

    out["final_window_entry_any"] = flag_matrix.any(axis=1).astype("int8")
    out["final_window_entry_count"] = flag_matrix.sum(axis=1).astype("int8")

    out["softest_newly_viable_slick"] = np.select(
        [
            out["final_window_entry_soft"].eq(1),
            out["final_window_entry_medium"].eq(1),
            out["final_window_entry_hard"].eq(1),
        ],
        [3, 2, 1],
        default=0,
    ).astype("int8")

    nearest_idx = np.abs(margin_matrix).argmin(axis=1)
    out["final_window_boundary_margin"] = margin_matrix[
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000015.",
    }, sort_keys=True))
