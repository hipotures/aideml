...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Year", "Stint"]:
        out[c] = out[c].astype("int64").astype("string")
    ...
    out["Race_Year"] = out["Race"] + "_" + out["Year"]
    out["Driver_Race_Compound"] = (
        out["Driver"] + "_" + out["Race"] + "_" + out["Compound"]
    )
    out["Race_Compound_Stint"] = (
        out["Race"] + "_" + out["Compound"] + "_" + out["Stint"]
    )
    out["Driver_Race_Year"] = out["Driver"] + "_" + out["Race"] + "_" + out["Year"]
    ...
    out["tyre_life_frac"] = out["TyreLife"] / (out["LapNumber"] + 1.0)
    ...
    out["front_pack"] = (out["Position"] <= 5).astype("int8")
    out["back_pack"] = (out["Position"] >= 15).astype("int8")
    out["late_race"] = (out["RaceProgress"] >= 0.70).astype("int8")
    out["early_stint"] = (out["TyreLife"] <= 3).astype("int8")
    out["old_tyre"] = (out["TyreLife"] >= 20).astype("int8")
    out["wet_or_intermediate"] = (
        out["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    )
    out["stint_lap_pressure"] = out["TyreLife"] * out["RaceProgress"]
    out["delta_x_tyre_life"] = out["LapTime_Delta"] * out["TyreLife"]
    out["degradation_x_progress"] = out["Cumulative_Degradation"] * out["RaceProgress"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000622.",
    }, sort_keys=True))
