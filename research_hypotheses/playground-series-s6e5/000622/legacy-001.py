...
def safe_div(a, b):
    ...
    return np.divide(a, b, out=np.zeros_like(a, dtype=float), where=np.abs(b) > 1e-9)
...
for fold, (tr_idx, va_idx) in enumerate(splits, 1):
    ...
    if HAVE_AG:
        try:
            ...
            print(
                f"Fold {fold}: AutoGluon failed, continuing with CatBoost for this fold: {e}"
            )

    if ag_val is not None:
        fold_pred = clean_proba(CAT_WEIGHT * fold_cat_val + AG_WEIGHT * ag_val)
    else:
        fold_pred = fold_cat_val
