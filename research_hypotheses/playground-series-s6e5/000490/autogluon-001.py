...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver_key = race_year + "_" + out["Driver"].astype(str)

    order = pd.DataFrame(
        {
            "_idx": np.arange(len(out)),
            "_key": driver_key.values,
            "_lap": out["LapNumber"].values,
        }
    ...
    is_dry = comp.isin(["SOFT", "MEDIUM", "HARD"]).astype(np.int8)

    sorted_idx = order["_idx"].to_numpy()
    sorted_key = order["_key"]

    wet_sorted = pd.Series(is_wet.to_numpy()[sorted_idx], index=order.index)
    wet_seen_sorted = wet_sorted.groupby(sorted_key, sort=False).cummax().to_numpy()
    wet_seen = np.empty(len(out), dtype=np.int8)
    wet_seen[sorted_idx] = wet_seen_sorted.astype(np.int8)

    dry_distinct = np.zeros(len(out), dtype=np.int8)
    for c in ["SOFT", "MEDIUM", "HARD"]:
        flag_sorted = pd.Series(
            (comp.to_numpy()[sorted_idx] == c).astype(np.int8), index=order.index
        )
        seen_sorted = flag_sorted.groupby(sorted_key, sort=False).cummax().to_numpy()
        dry_distinct[sorted_idx] += seen_sorted.astype(np.int8)

    total_laps = out.groupby(race_year, sort=False)["LapNumber"].transform("max")
    laps_remaining = (total_laps - out["LapNumber"]).clip(lower=0).astype(float)

    dry_compound_debt = ((wet_seen == 0) & (dry_distinct < 2)).astype(np.int8)
    current_tyre_can_reach_finish = (
        out["TyreLife"].astype(float) >= laps_remaining
    ).astype(np.int8)

    out["RaceYear_Driver_WetSeen"] = wet_seen
    out["RaceYear_Driver_DryDistinctUsed"] = dry_distinct
    out["DryCompoundDebt_WetExempt"] = dry_compound_debt
    out["LapsRemaining_Est"] = laps_remaining
    out["CurrentTyreCanReachFinish"] = current_tyre_can_reach_finish
    out["Debt_x_LapsRemaining"] = dry_compound_debt * laps_remaining
    out["Debt_x_CannotReachFinish"] = dry_compound_debt * (
        1 - current_tyre_can_reach_finish
    )
    out["Debt_x_LateRace"] = dry_compound_debt * (
        out["RaceProgress"].astype(float) >= 0.75
    ).astype(np.int8)
    out["WetSeen_x_LapsRemaining"] = wet_seen * laps_remaining
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000490"],
        "research_usage_note": "Verified assigned hypothesis 000490.",
    }, sort_keys=True))
