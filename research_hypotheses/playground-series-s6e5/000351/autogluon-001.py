...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Is_WetCompound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    pace_cols = ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]
    group_lap = ["Race_Year", "LapNumber"]
    group_comp = ["Race_Year", "LapNumber", "Compound"]
    group_slick = ["Race_Year", "LapNumber", "Is_Slick"]

    for col in pace_cols:
        lap_med = out.groupby(group_lap, sort=False)[col].transform("median")
        comp_med = out.groupby(group_comp, sort=False)[col].transform("median")
        comp_cnt = out.groupby(group_comp, sort=False)[col].transform("count")
        slick_med = out.groupby(group_slick, sort=False)[col].transform("median")
        slick_cnt = out.groupby(group_slick, sort=False)[col].transform("count")

        base = comp_med.where(comp_cnt >= 4, slick_med.where(slick_cnt >= 6, lap_med))
        safe = base.replace(0, np.nan)

        name = col.replace(" ", "_").replace("(", "").replace(")", "")
        out[f"{name}_same_lap_base"] = base
        out[f"{name}_same_lap_resid"] = out[col] - base
        out[f"{name}_same_lap_ratio"] = out[col] / safe
        out[f"{name}_lap_rank_pct"] = out.groupby(group_lap, sort=False)[col].rank(
            pct=True
        )
        out[f"{name}_compound_rank_pct"] = out.groupby(group_comp, sort=False)[
            col
        ].rank(pct=True)

    sort_cols = ["Race_Year", "Driver", "Stint", "LapNumber"]
    ordered = out.sort_values(sort_cols, kind="mergesort")
    driver_stint = [ordered["Race_Year"], ordered["Driver"], ordered["Stint"]]

    resid = ordered["LapTime_s_same_lap_resid"]
    ewm_mean = resid.groupby(driver_stint, sort=False).transform(
        lambda s: s.shift(1).ewm(span=3, adjust=False, min_periods=1).mean()
    )
    prev_resid = resid.groupby(driver_stint, sort=False).shift(1)
    prev2_resid = resid.groupby(driver_stint, sort=False).shift(2)

    ordered["LapTime_resid_vs_own_ewm3"] = resid - ewm_mean
    ordered["LapTime_resid_diff1"] = resid - prev_resid
    ordered["LapTime_resid_curvature2"] = resid - 2 * prev_resid + prev2_resid
    ordered["TyreLife_x_resid"] = ordered["TyreLife"] * resid
    ordered["RaceProgress_x_resid"] = ordered["RaceProgress"] * resid

    out = ordered.sort_index(kind="mergesort")

    out["TyreLife_RaceProgress"] = out["TyreLife"] * out["RaceProgress"]
    out["Stint_RaceProgress"] = out["Stint"] * out["RaceProgress"]
    out["LapNumber_remaining_est"] = (
        out["LapNumber"]
        * (1 - out["RaceProgress"])
        / out["RaceProgress"].replace(0, np.nan)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000351"],
        "research_usage_note": "Verified assigned hypothesis 000351.",
    }, sort_keys=True))
