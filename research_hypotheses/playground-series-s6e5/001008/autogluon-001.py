...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_time = pd.to_numeric(out["LapTime (s)"], errors="coerce").to_numpy(
        dtype="float64"
    )
    progress = pd.to_numeric(out["RaceProgress"], errors="coerce").to_numpy(
        dtype="float64"
    )
    tyre_life = pd.to_numeric(out["TyreLife"], errors="coerce").to_numpy(
        dtype="float64"
    )
    lap_number = pd.to_numeric(out["LapNumber"], errors="coerce").to_numpy(
        dtype="float64"
    )
    pit = (
        pd.to_numeric(out["PitStop"], errors="coerce").fillna(0).to_numpy(dtype="int8")
    )

    finite_laps = lap_time[np.isfinite(lap_time)]
    median_lap = float(np.nanmedian(finite_laps)) if finite_laps.size else 90.0
    fuel_span = float(np.clip(median_lap * 0.065, 4.0, 9.0))
    progress_clip = np.clip(np.nan_to_num(progress, nan=0.5), 0.0, 1.0)
    fuel_adjusted = lap_time - fuel_span * (1.0 - progress_clip)

    level = np.zeros(n, dtype="float32")
    slope = np.zeros(n, dtype="float32")
    accel = np.zeros(n, dtype="float32")
    innovation = np.zeros(n, dtype="float32")
    cliff_prob = np.zeros(n, dtype="float32")
    cliff_flag = np.zeros(n, dtype="int8")

    if n:
        year_code = pd.factorize(out["Year"], sort=True)[0].astype("int32")
        race_code = pd.factorize(out["Race"], sort=True)[0].astype("int32")
        driver_code = pd.factorize(out["Driver"], sort=True)[0].astype("int32")
        stint_code = pd.factorize(out["Stint"], sort=True)[0].astype("int32")
        pos = np.arange(n, dtype="int64")

        order = np.lexsort(
            (pos, lap_number, stint_code, driver_code, race_code, year_code)
        )
        group_start = np.ones(n, dtype=bool)
        group_start[1:] = (
            (year_code[order[1:]] != year_code[order[:-1]])
            | (race_code[order[1:]] != race_code[order[:-1]])
            | (driver_code[order[1:]] != driver_code[order[:-1]])
            | (stint_code[order[1:]] != stint_code[order[:-1]])
        )
        ...
        for start, end in zip(starts, ends):
            ...
            for j, p in enumerate(inds):
                y = (
                    float(fuel_adjusted[p])
                    if np.isfinite(fuel_adjusted[p])
                    else median_lap
                )
                reset = (
                    j == 0
                    or (prev_pos >= 0 and pit[prev_pos] == 1)
                    or (j > 0 and tyre_life[p] <= prev_tyre)
                    or (j > 0 and lap_number[p] <= prev_lap)
                )
                ...
                if reset:
                    ...
                    dt = tyre_life[p] - prev_tyre
                    if not np.isfinite(dt) or dt <= 0:
                        lap_dt = lap_number[p] - prev_lap
                        dt = lap_dt if np.isfinite(lap_dt) and lap_dt > 0 else 1.0

                    pred = prev_level + prev_slope * dt
                    resid = y - pred
                    noise_safe = max(float(noise), min_noise)
                    update_clip = 3.5 * noise_safe + 1.5
                    clipped_resid = float(np.clip(resid, -update_clip, update_clip))

                    new_level = pred + alpha * clipped_resid
                    new_slope = float(
                        np.clip(
                            prev_slope + beta * clipped_resid / max(dt, 1.0), -1.0, 2.5
                        )
                    )
                    new_accel = (new_slope - prev_slope) / max(dt, 1.0)

                    deterioration = resid - prev_resid - max(prev_slope, 0.0) * dt
                    z = (0.7 * deterioration + 0.3 * max(resid, 0.0)) / (
                        noise_safe + 1e-6
                    ) - 1.25
                    prob = 1.0 / (1.0 + np.exp(-float(np.clip(z, -20.0, 20.0))))
                    ...
                    innovation[p] = resid
                    cliff_prob[p] = prob
                    cliff_flag[p] = int(prob > 0.75 and resid > 0.0)

                    prev_level = new_level
                    prev_slope = new_slope
                    prev_resid = resid
                    noise = max(
                        min_noise, 0.90 * noise_safe + 0.10 * min(abs(resid), 25.0)
                    )

                prev_pos = p
                prev_tyre = tyre_life[p]
                prev_lap = lap_number[p]

    out["tyre_fuel_adjusted_laptime"] = fuel_adjusted.astype("float32")
    out["tyre_latent_level"] = level
    out["tyre_latent_slope"] = slope
    out["tyre_latent_accel"] = accel
    out["tyre_latent_innovation"] = innovation
    out["tyre_cliff_prob"] = cliff_prob
    out["tyre_cliff_flag"] = cliff_flag
    out.attrs["research_hypotheses_llm_claimed_used"] = ["001008"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001008"],
        "research_usage_note": "Verified assigned hypothesis 001008.",
    }, sort_keys=True))
