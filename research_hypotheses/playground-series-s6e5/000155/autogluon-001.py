...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["Compound_Regime"] = np.where(X["Compound"].isin(dry_compounds), "DRY", "WET")
    X["Race_Year"] = X["Year"].astype(str) + "_" + X["Race"].astype(str)
    X["Lap_Query"] = X["Race_Year"] + "_L" + X["LapNumber"].astype(str)
    X["Lap_Query_Regime"] = X["Lap_Query"] + "_" + X["Compound_Regime"].astype(str)

    X["Laps_Remaining_Proxy"] = 1.0 - X["RaceProgress"]
    X["Tyre_Usage_Ratio"] = X["TyreLife"] / X["LapNumber"].clip(lower=1)
    X["Degradation_per_Lap"] = X["Cumulative_Degradation"] / X["TyreLife"].clip(lower=1)
    X["Stint_Pressure"] = X["TyreLife"] * (1.0 + X["RaceProgress"])
    X["Position_Pressure"] = X["Position"] * X["RaceProgress"]
    X["Pace_Wear_Interaction"] = X["LapTime_Delta"] * X["TyreLife"]
    X["Deg_Wear_Interaction"] = X["Cumulative_Degradation"] * X["TyreLife"]

    lap_groups = [("Lap_Query", "lap"), ("Lap_Query_Regime", "lap_regime")]
    rank_cols = [
        "LapTime (s)",
        "LapTime_Delta",
        "TyreLife",
    ...
    for gcol, suffix in lap_groups:
        grp = X.groupby(gcol, sort=False)
        X[f"{suffix}_group_size"] = grp[gcol].transform("size")

        for col in rank_cols:
            X[f"{col}_{suffix}_rank"] = grp[col].rank(method="average", pct=False)
            X[f"{col}_{suffix}_pct"] = grp[col].rank(method="average", pct=True)

            mean = grp[col].transform("mean")
            std = grp[col].transform("std").fillna(0)
            X[f"{col}_{suffix}_diff_mean"] = X[col] - mean
            X[f"{col}_{suffix}_z"] = (X[col] - mean) / std.replace(0, 1)

    driver_race = X.groupby(["Race_Year", "Driver"], sort=False)
    X["Driver_Race_MeanLapTimeDelta"] = driver_race["LapTime_Delta"].transform("mean")
    X["Driver_Race_MeanDeg"] = driver_race["Cumulative_Degradation"].transform("mean")
    X["Driver_Race_MeanTyreLife"] = driver_race["TyreLife"].transform("mean")
    X["LapTimeDelta_vs_DriverRaceMean"] = (
        X["LapTime_Delta"] - X["Driver_Race_MeanLapTimeDelta"]
    )
    X["Deg_vs_DriverRaceMean"] = X["Cumulative_Degradation"] - X["Driver_Race_MeanDeg"]
    X["TyreLife_vs_DriverRaceMean"] = X["TyreLife"] - X["Driver_Race_MeanTyreLife"]

    X["Dry_Race_Context"] = (X["Compound_Regime"] == "DRY").astype(np.int8)
    X["Dry_LapTimeDelta_lap_rank"] = X["Dry_Race_Context"] * X["LapTime_Delta_lap_rank"]
    X["Dry_TyreLife_lap_rank"] = X["Dry_Race_Context"] * X["TyreLife_lap_rank"]
    X["Dry_Deg_lap_rank"] = X["Dry_Race_Context"] * X["Cumulative_Degradation_lap_rank"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000155.",
    }, sort_keys=True))
