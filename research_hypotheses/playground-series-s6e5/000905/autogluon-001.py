...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wet = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype(float)
    pit = out["PitStop"].astype(float)

    out["_late_stop_signal"] = pit * out["RaceProgress"].astype(float)
    out["_wet_signal"] = wet
    out["_abs_pos_change"] = out["Position_Change"].abs()
    out["_degradation_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["_lap_delta_abs"] = out["LapTime_Delta"].abs()

    prof = out.groupby("Driver", sort=False).agg(
        drv_n=("Driver", "size"),
        drv_tyre_mean=("TyreLife", "mean"),
        drv_tyre_median=("TyreLife", "median"),
        drv_tyre_max=("TyreLife", "max"),
        drv_stint_mean=("Stint", "mean"),
        drv_stint_max=("Stint", "max"),
        drv_pit_rate=("PitStop", "mean"),
        drv_late_stop=(" _late_stop_signal".strip(), "mean"),
        drv_progress_mean=("RaceProgress", "mean"),
        drv_deg_mean=("Cumulative_Degradation", "mean"),
        drv_deg_per_lap=(" _degradation_per_lap".strip(), "mean"),
        drv_lap_delta_mean=("LapTime_Delta", "mean"),
        drv_lap_delta_std=("LapTime_Delta", "std"),
        drv_lap_delta_abs=(" _lap_delta_abs".strip(), "mean"),
        drv_pos_mean=("Position", "mean"),
        drv_pos_chg_mean=("Position_Change", "mean"),
        drv_pos_chg_abs=(" _abs_pos_change".strip(), "mean"),
        drv_wet_share=(" _wet_signal".strip(), "mean"),
    )

    comp_med = out.pivot_table(
        index="Driver",
        columns="Compound",
        values="TyreLife",
        aggfunc="median",
    )
    comp_med.columns = [f"drv_tyre_med_{str(c).lower()}" for c in comp_med.columns]
    prof = prof.join(comp_med)

    race_prior = out.groupby("Race", sort=False).agg(
        race_pit_rate=("PitStop", "mean"),
        race_tyre_mean=("TyreLife", "mean"),
        race_late_stop=("_late_stop_signal", "mean"),
        race_wet_share=("_wet_signal", "mean"),
    )
    out = out.join(race_prior, on="Race")

    num_cols = prof.columns
    global_prof = prof[num_cols].median(numeric_only=True)
    prof = prof.fillna(global_prof)

    counts = prof["drv_n"].clip(lower=0)
    weight = (counts / (counts + 20.0)).astype(float)
    smooth_cols = [c for c in prof.columns if c != "drv_n"]
    prof.loc[:, smooth_cols] = prof[smooth_cols].mul(weight, axis=0) + global_prof[
        smooth_cols
    ].mul(1.0 - weight, axis=0)

    cluster_cols = [c for c in prof.columns if c != "drv_n"]
    n_clusters = int(min(24, max(2, len(prof) // 12)))
    scaled = StandardScaler().fit_transform(prof[cluster_cols])
    prof["Driver_Archetype"] = (
        KMeans(
    ...
    out = out.join(prof[keep_profile_cols], on="Driver")

    out["tyre_life_x_stint"] = out["TyreLife"] * out["Stint"]
    out["tyre_life_progress"] = out["TyreLife"] * out["RaceProgress"]
    out["pit_rate_vs_race"] = out["drv_pit_rate"] - out["race_pit_rate"]
    out["late_stop_vs_race"] = out["drv_late_stop"] - out["race_late_stop"]
    out["tyre_mean_vs_race"] = out["drv_tyre_mean"] - out["race_tyre_mean"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000905"],
        "research_usage_note": "Verified assigned hypothesis 000905.",
    }, sort_keys=True))
