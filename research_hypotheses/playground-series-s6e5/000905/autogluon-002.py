...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prof = prof.replace([np.inf, -np.inf], np.nan).fillna(global_prof).fillna(0.0)
    ...
    if len(prof) < 2 or not cluster_cols:
        prof["Driver_Archetype"] = 0
    else:
        n_clusters = int(min(24, max(2, len(prof) // 12), len(prof)))
