...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "_row_order"]
    df = df.sort_values(sort_cols, kind="mergesort")

    grp = df.groupby(["Year", "Race", "Driver"], sort=False)

    monaco_rule_active = df["Race"].eq("Monaco Grand Prix") & df["Year"].ge(2025)
    is_wet_or_inter = df["Compound"].isin(["WET", "INTERMEDIATE"])

    used_wet_or_inter = grp["Compound"].transform(
        lambda s: s.isin(["WET", "INTERMEDIATE"]).cummax()
    )

    stops_completed = grp["PitStop"].cumsum().astype("int16")
    sets_used = (stops_completed + 1).astype("int16")

    slick_compounds_used = pd.Series(0, index=df.index, dtype="int8")
    for comp in ["HARD", "MEDIUM", "SOFT"]:
        seen_comp = (
            grp["Compound"].transform(lambda s, c=comp: s.eq(c).cummax()).astype("int8")
        )
        slick_compounds_used = slick_compounds_used.add(seen_comp, fill_value=0).astype(
            "int8"
        )

    dry_rule_active = (monaco_rule_active & ~used_wet_or_inter).astype("int8")

    est_total_laps = (df["LapNumber"] / df["RaceProgress"].clip(lower=1e-6)).round()
    est_total_laps = est_total_laps.clip(lower=df["LapNumber"], upper=90).fillna(
        df["LapNumber"]
    )
    laps_remaining_est = (est_total_laps - df["LapNumber"]).clip(lower=0)

    required_stops_remaining = (
        (2 - stops_completed).clip(lower=0) * monaco_rule_active.astype("int8")
    ).astype("int8")
    required_slick_compounds_remaining = (
        (2 - slick_compounds_used).clip(lower=0) * dry_rule_active
    ).astype("int8")

    df["monaco_2025_rule_active"] = monaco_rule_active.astype("int8")
    df["dry_rule_active"] = dry_rule_active
    df["used_wet_or_inter"] = used_wet_or_inter.astype("int8")
    df["slick_compounds_used"] = slick_compounds_used
    df["sets_used"] = sets_used
    df["stops_completed"] = stops_completed
    df["required_stops_remaining"] = required_stops_remaining
    df["required_slick_compounds_remaining"] = required_slick_compounds_remaining
    df["LapsRemaining_Est"] = laps_remaining_est.astype("float32")

    pressure_den = (laps_remaining_est + 1.0).astype("float32")
    df["monaco_stop_pressure"] = (required_stops_remaining / pressure_den).astype(
        "float32"
    )
    df["monaco_slick_pressure"] = (
        required_slick_compounds_remaining / pressure_den
    ).astype("float32")
    df["monaco_stop_pressure_x_progress"] = (
        df["monaco_stop_pressure"] * df["RaceProgress"]
    ).astype("float32")
    df["monaco_stop_pressure_x_tyrelife"] = (
        df["monaco_stop_pressure"] * df["TyreLife"]
    ).astype("float32")
    df["monaco_stop_pressure_x_deg"] = (
        df["monaco_stop_pressure"] * df["Cumulative_Degradation"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000336.",
    }, sort_keys=True))
