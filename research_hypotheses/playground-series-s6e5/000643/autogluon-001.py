...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_med = out.groupby(["Race", "LapNumber"], observed=True)[
        "LapTime (s)"
    ].transform("median")
    out["pace_dev_racelap"] = (out["LapTime (s)"] - lap_med).astype("float32")
    ...
    group_specs = {
        "racelap": ["Race", "LapNumber"],
        "racestint": ["Race", "Stint"],
    }

    for suffix, keys in group_specs.items():
        g = out.groupby(keys, observed=True)
        for c in rank_cols:
            out[f"{c}_{suffix}_pct"] = (
                g[c].rank(method="average", pct=True).astype("float32")
            )
            out[f"{c}_{suffix}_rank"] = g[c].rank(method="average").astype("float32")

    pressure_keys = ["Race", "Compound", "Stint"]
    g_pressure = out.groupby(pressure_keys, observed=True)
    tyre_pct = g_pressure["TyreLife"].rank(method="average", pct=True)
    degr_pct = g_pressure["Cumulative_Degradation"].rank(method="average", pct=True)
    pace_pct = g_pressure["pace_dev_racelap"].rank(method="average", pct=True)
    out["race_compound_stint_pressure_pct"] = (
        0.45 * tyre_pct + 0.35 * degr_pct + 0.20 * pace_pct
    ).astype("float32")

    out["tyre_life_x_progress"] = (out["TyreLife"] * out["RaceProgress"]).astype(
        "float32"
    )
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"].abs() + 1.0)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000643"],
        "research_usage_note": "Verified assigned hypothesis 000643.",
    }, sort_keys=True))
