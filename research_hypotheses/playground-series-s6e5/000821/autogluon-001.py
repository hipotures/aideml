...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work["_orig_idx"] = orig_idx
    work["_wet"] = work["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    work = work.sort_values(
        ["Year", "Race", "LapNumber", "_orig_idx"], kind="mergesort"
    ).reset_index(drop=True)

    drv_race = ["Year", "Race", "Driver"]
    g = work.groupby(drv_race, sort=False)

    lap = work["LapTime (s)"].astype("float32")
    pit = work["PitStop"].astype("int8")
    pos = work["Position"].astype("float32")

    clean_lap = lap.where((pit == 0) & lap.between(55, 220))
    clean_sum = (
        clean_lap.fillna(0).groupby([work[k] for k in drv_race], sort=False).cumsum()
    )
    clean_cnt = (
        clean_lap.notna()
        .astype("int16")
        .groupby([work[k] for k in drv_race], sort=False)
        .cumsum()
    )
    prior_sum = (
        clean_sum.groupby([work[k] for k in drv_race], sort=False).shift(1).fillna(0)
    )
    prior_cnt = (
        clean_cnt.groupby([work[k] for k in drv_race], sort=False).shift(1).fillna(0)
    )
    prior_pace = (prior_sum / prior_cnt.replace(0, np.nan)).astype("float32")

    prev_lap = g["LapTime (s)"].shift(1).astype("float32")
    prev_pit = g["PitStop"].shift(1).fillna(0).astype("int8")
    prev_pos = g["Position"].shift(1).astype("float32")

    completed_stop = prev_pit.eq(1) & prev_lap.notna() & prior_pace.notna()
    stop_loss = (prev_lap + lap - 2.0 * prior_pace).where(completed_stop).clip(-10, 90)
    pos_loss = (pos - prev_pos).where(completed_stop).clip(-12, 18)
    fast_stop = (stop_loss <= 22.0).where(completed_stop)

    def add_stats(prefix, keys):
        key_series = [work[k] for k in keys]
        ev = stop_loss.astype("float32")
        ps = pos_loss.astype("float32")
        cnt_ev = ev.notna().astype("float32")
        val = ev.fillna(0)
        val2 = (val * val).where(cnt_ev.eq(1), 0)
        pval = ps.fillna(0)
        fval = fast_stop.fillna(False).astype("float32")

        cnt = cnt_ev.groupby(key_series, sort=False).cumsum() - cnt_ev
        sm = val.groupby(key_series, sort=False).cumsum() - val
        sm2 = val2.groupby(key_series, sort=False).cumsum() - val2
        psm = pval.groupby(key_series, sort=False).cumsum() - pval
        fsm = fval.groupby(key_series, sort=False).cumsum() - fval

        mean = sm / cnt.replace(0, np.nan)
        var = (sm2 / cnt.replace(0, np.nan) - mean * mean).clip(lower=0)
        pmean = psm / cnt.replace(0, np.nan)
        frate = fsm / cnt.replace(0, np.nan)

        work[f"{prefix}_loss_mean"] = mean.astype("float32")
        work[f"{prefix}_loss_var"] = var.fillna(0).astype("float32")
        work[f"{prefix}_pos_loss_mean"] = pmean.astype("float32")
        work[f"{prefix}_fast_rate"] = frate.astype("float32")
        work[f"{prefix}_count"] = np.log1p(cnt).astype("float32")

    add_stats("pit_prior_drv_race_wet", ["Year", "Race", "Driver", "_wet"])
    add_stats("pit_prior_race_wet", ["Year", "Race", "_wet"])
    add_stats("pit_prior_year_wet", ["Year", "_wet"])

    base_loss = np.float32(22.0)
    race_m = work["pit_prior_race_wet_loss_mean"].fillna(base_loss)
    drv_m = work["pit_prior_drv_race_wet_loss_mean"]
    year_m = work["pit_prior_year_wet_loss_mean"].fillna(base_loss)
    drv_c = np.expm1(work["pit_prior_drv_race_wet_count"])
    race_c = np.expm1(work["pit_prior_race_wet_count"])
    year_c = np.expm1(work["pit_prior_year_wet_count"])

    work["pit_prior_smoothed_loss"] = (
        drv_m.fillna(0) * drv_c + race_m * 3.0 + year_m * 1.5 + base_loss * 4.0
    ) / (drv_c + 3.0 + 1.5 + 4.0)
    work["pit_prior_smoothed_count"] = np.log1p(drv_c + race_c + year_c).astype(
        "float32"
    )

    feature_cols = [c for c in work.columns if c.startswith("pit_prior_")]
    restored = work[["_orig_idx"] + feature_cols].sort_values(
        "_orig_idx", kind="mergesort"
    )

    for c in feature_cols:
        out[c] = restored[c].to_numpy()
        if out[c].isna().any():
            if c.endswith("_loss_mean") or c.endswith("_smoothed_loss"):
                out[c] = out[c].fillna(base_loss)
            else:
                out[c] = out[c].fillna(0)

    out["wet_regime"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000821"],
        "research_usage_note": "Verified assigned hypothesis 000821.",
    }, sort_keys=True))
