...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    query_keys = ["Year", "Race", "LapNumber"]
    race_year = out["Race"].astype("string") + "_" + out["Year"].astype("string")
    out["h172_race_year"] = race_year.astype("category")
    out["h172_query_id"] = (
        race_year + "_L" + out["LapNumber"].astype("string")
    ).astype("category")

    compound = out["Compound"].astype("string").str.upper()
    out["h172_compound_regime"] = np.select(
        [compound.isin(["SOFT", "MEDIUM", "HARD"]), compound.eq("INTERMEDIATE"), compound.eq("WET")],
        ["DRY", "INTER", "WET"],
        default="OTHER",
    )
    out["h172_query_regime"] = (
        out["h172_query_id"].astype("string")
        + "_"
        + pd.Series(out["h172_compound_regime"], index=out.index).astype("string")
    ).astype("category")

    query = out.groupby(query_keys, sort=False)
    out["h172_query_size"] = query["Driver"].transform("size").astype("int16")
    ...
    for col, high_is_ready in rank_specs.items():
        values = out[col].astype("float32")
        centered = values - query[col].transform("mean").astype("float32")
        spread = query[col].transform("std").fillna(0).astype("float32")
        rank_pct = query[col].rank(method="average", pct=True, ascending=True)
        if not high_is_ready:
            rank_pct = 1.0 - rank_pct
        safe_name = col.lower().replace(" ", "_").replace("(", "").replace(")", "")
        out[f"h172_{safe_name}_query_center"] = centered.astype("float32")
        out[f"h172_{safe_name}_query_z"] = (
            centered / (spread + 1e-3)
        ).clip(-10, 10).astype("float32")
        out[f"h172_{safe_name}_ready_rank"] = rank_pct.fillna(0.5).astype("float32")

    tyre = out["TyreLife"].astype("float32")
    progress = out["RaceProgress"].astype("float32").clip(0.0, 1.0)
    degradation = out["Cumulative_Degradation"].astype("float32")
    lap_delta = out["LapTime_Delta"].astype("float32")

    out["h172_rank_priority_score"] = (
        0.30 * out["h172_tyrelife_ready_rank"]
        + 0.25 * out["h172_cumulative_degradation_ready_rank"]
        + 0.20 * out["h172_laptime_delta_ready_rank"]
        + 0.15 * out["h172_raceprogress_ready_rank"]
        + 0.10 * out["h172_position_ready_rank"]
    ).astype("float32")
    out["h172_tyre_progress_pressure"] = (tyre * progress).astype("float32")
    out["h172_degradation_per_tyre_lap"] = (
        degradation / tyre.clip(lower=1.0)
    ).astype("float32")
    out["h172_slow_old_tyre"] = (lap_delta.clip(lower=0) * np.log1p(tyre)).astype(
        "float32"
    )
    out["h172_rank_x_pressure"] = (
        out["h172_rank_priority_score"] * out["h172_tyre_progress_pressure"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000172"],
        "research_usage_note": "Verified assigned hypothesis 000172.",
    }, sort_keys=True))
