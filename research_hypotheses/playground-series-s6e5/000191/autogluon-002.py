...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def robust_group_z(series, group_keys):
        med = series.groupby(group_keys).transform("median")
        mad = (series - med).abs().groupby(group_keys).transform("median")
        z = (series - med) / (1.4826 * mad.replace(0, np.nan))
        return z.fillna(0.0), med, mad.fillna(0.0)

    lap_z, lap_med, lap_mad = robust_group_z(out["LapTime (s)"], grp)
    delta_z, delta_med, delta_mad = robust_group_z(out["LapTime_Delta"], grp)
    poschg_z, poschg_med, poschg_mad = robust_group_z(out["Position_Change"], grp)

    out["race_lap_field_std"] = (
        out.groupby(grp)["LapTime (s)"].transform("std").fillna(0.0)
    )
    out["race_lap_field_range"] = (
        out.groupby(grp)["LapTime (s)"].transform("max")
        - out.groupby(grp)["LapTime (s)"].transform("min")
    ).fillna(0.0)
    disp_med = out.groupby(grp)["race_lap_field_std"].transform("median")
    disp_mad = (
        (out["race_lap_field_std"] - disp_med)
        .abs()
        .groupby(out[grp].apply(tuple, axis=1))
        .transform("median")
    )
    out["race_lap_dispersion_z"] = (
        (out["race_lap_field_std"] - disp_med) / (1.4826 * disp_mad.replace(0, np.nan))
    ).fillna(0.0)

    out["laptime_robust_z"] = lap_z.clip(-20, 20)
    out["lapdelta_robust_z"] = delta_z.clip(-20, 20)
    out["poschange_robust_z"] = poschg_z.clip(-20, 20)

    out["anomaly_laptime"] = (out["laptime_robust_z"].abs() > 3.5).astype("int8")
    out["anomaly_lapdelta"] = (out["lapdelta_robust_z"].abs() > 3.5).astype("int8")
    out["anomaly_poschange"] = (
        (out["poschange_robust_z"].abs() > 3.5) | (out["Position_Change"].abs() >= 5)
    ).astype("int8")
    out["anomaly_dispersion"] = (out["race_lap_dispersion_z"] > 3.0).astype("int8")

    out["anomaly_score"] = (
        out["laptime_robust_z"].abs()
        + out["lapdelta_robust_z"].abs()
        + 0.5 * out["poschange_robust_z"].abs()
        + out["race_lap_dispersion_z"].clip(lower=0)
    )
    out["anomaly_flag"] = (
        out["anomaly_laptime"]
        | out["anomaly_lapdelta"]
        | out["anomaly_poschange"]
        | out["anomaly_dispersion"]
    ).astype("int8")

    # Censor distorted timing/degradation signals toward race-lap medians instead of dropping rows.
    lap_cap = 3.5 * 1.4826 * lap_mad.replace(0, np.nan)
    delta_cap = 3.5 * 1.4826 * delta_mad.replace(0, np.nan)

    out["LapTime_clean"] = np.where(
        out["anomaly_flag"] == 1,
        np.clip(
            out["LapTime (s)"], lap_med - lap_cap.fillna(0), lap_med + lap_cap.fillna(0)
        ),
        out["LapTime (s)"],
    )
    out["LapTime_Delta_clean"] = np.where(
        out["anomaly_flag"] == 1,
        np.clip(
            out["LapTime_Delta"],
            delta_med - delta_cap.fillna(0),
            delta_med + delta_cap.fillna(0),
        ),
        out["LapTime_Delta"],
    )

    degr_med = out.groupby(["Year", "Race", "Stint", "Compound"])[
        "Cumulative_Degradation"
    ].transform("median")
    out["Cumulative_Degradation_clean"] = np.where(
        out["anomaly_flag"] == 1,
        0.7 * degr_med.fillna(out["Cumulative_Degradation"])
        + 0.3 * out["Cumulative_Degradation"],
        out["Cumulative_Degradation"],
    )

    out["laptime_vs_clean"] = out["LapTime (s)"] - out["LapTime_clean"]
    out["lapdelta_vs_clean"] = out["LapTime_Delta"] - out["LapTime_Delta_clean"]
    out["degradation_vs_clean"] = (
        out["Cumulative_Degradation"] - out["Cumulative_Degradation_clean"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000191"],
        "research_usage_note": "Verified assigned hypothesis 000191.",
    }, sort_keys=True))
