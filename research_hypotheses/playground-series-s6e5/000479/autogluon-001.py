...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = X.sort_values(group_cols + ["LapNumber", "_orig_order"]).reset_index(drop=True)

    # Basic tyre / race-state encodings
    compound_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    X["CompoundCode"] = X["Compound"].map(compound_map).astype("float32")
    X["IsDry"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["IsWetRaceTyre"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    rp = X["RaceProgress"].clip(0.01, 1.0)
    est_total_laps = (X["LapNumber"] / rp).clip(lower=X["LapNumber"])
    X["EstimatedTotalLaps"] = est_total_laps.astype("float32")
    X["EstimatedLapsRemaining"] = (
        (est_total_laps - X["LapNumber"]).clip(lower=0).astype("float32")
    )
    X["StintProgressInRace"] = (X["TyreLife"] / est_total_laps).astype("float32")

    tyre_life_safe = X["TyreLife"].clip(lower=1)
    lap_no_safe = X["LapNumber"].clip(lower=1)

    X["DegPerTyreLap"] = (X["Cumulative_Degradation"] / tyre_life_safe).astype(
        "float32"
    )
    X["DeltaPerTyreLap"] = (X["LapTime_Delta"] / tyre_life_safe).astype("float32")
    X["DegPerRaceLap"] = (X["Cumulative_Degradation"] / lap_no_safe).astype("float32")
    X["DeltaPerRaceLap"] = (X["LapTime_Delta"] / lap_no_safe).astype("float32")
    X["TyreLifeByStint"] = (X["TyreLife"] / X["Stint"].clip(lower=1)).astype("float32")

    g = X.groupby(group_cols, sort=False)

    # Past-only lap dynamics
    X["PrevLapTime"] = g["LapTime (s)"].shift(1)
    X["PrevLapDelta"] = g["LapTime_Delta"].shift(1)
    X["PrevPosition"] = g["Position"].shift(1)
    X["LapTimeChange"] = (
        (X["LapTime (s)"] - X["PrevLapTime"]).fillna(0).astype("float32")
    )
    X["LapDeltaChange"] = (
        (X["LapTime_Delta"] - X["PrevLapDelta"]).fillna(0).astype("float32")
    )
    X["PositionDrift"] = (X["Position"] - X["PrevPosition"]).fillna(0).astype("float32")

    prior_count = g.cumcount()
    prior_count_safe = prior_count.replace(0, np.nan)

    laptime_csum = g["LapTime (s)"].cumsum()
    delta_csum = g["LapTime_Delta"].cumsum()
    deg_csum = g["Cumulative_Degradation"].cumsum()

    X["PastMeanLapTime"] = (
        (laptime_csum - X["LapTime (s)"]) / prior_count_safe
    ).astype("float32")
    X["PastMeanDelta"] = ((delta_csum - X["LapTime_Delta"]) / prior_count_safe).astype(
        "float32"
    )
    X["PastMeanDeg"] = (
        (deg_csum - X["Cumulative_Degradation"]) / prior_count_safe
    ).astype("float32")

    X["LapTimeVsPastMean"] = (
        (X["LapTime (s)"] - X["PastMeanLapTime"]).fillna(0).astype("float32")
    )
    X["DeltaVsPastMean"] = (
        (X["LapTime_Delta"] - X["PastMeanDelta"]).fillna(0).astype("float32")
    )
    X["DegVsPastMean"] = (
        (X["Cumulative_Degradation"] - X["PastMeanDeg"]).fillna(0).astype("float32")
    )

    # Hypothesis 000479 proxy: expected stint length and hazard-style time-to-next-stop features
    pit_now = X["PitStop"].astype("int8")
    completed_stint_len = X["TyreLife"].where(pit_now.eq(1), 0.0)

    prior_completed_sum = g[completed_stint_len.name].cumsum() - completed_stint_len
    prior_completed_cnt = g["PitStop"].cumsum() - pit_now

    X["PriorCompletedStints"] = prior_completed_cnt.astype("float32")
    X["PriorMeanCompletedStintLen"] = (
        prior_completed_sum / prior_completed_cnt.replace(0, np.nan)
    ).astype("float32")

    last_completed = X["TyreLife"].where(pit_now.eq(1))
    X["PrevCompletedStintLen"] = g[last_completed.name].shift(1)
    X["PrevCompletedStintLen"] = X.groupby(group_cols, sort=False)[
        "PrevCompletedStintLen"
    ].ffill()

    pit_rows = pit_now.eq(1)
    global_pit_median = (
        float(X.loc[pit_rows, "TyreLife"].median())
        if pit_rows.any()
        else float(X["TyreLife"].median())
    )
    compound_pit_median = (
        X.loc[pit_rows].groupby("Compound")["TyreLife"].median().to_dict()
        if pit_rows.any()
        else {}
    )
    compound_stint_pit_median = (
        X.loc[pit_rows].groupby(["Compound", "Stint"])["TyreLife"].median().to_dict()
        if pit_rows.any()
        else {}
    )

    fallback_compound_stint = X.set_index(["Compound", "Stint"]).index.map(
        compound_stint_pit_median
    )
    fallback_compound = X["Compound"].map(compound_pit_median)

    expected_stint = X["PriorMeanCompletedStintLen"].copy()
    expected_stint = expected_stint.fillna(X["PrevCompletedStintLen"])
    expected_stint = expected_stint.fillna(
        pd.Series(fallback_compound_stint, index=X.index, dtype="float64")
    )
    expected_stint = expected_stint.fillna(fallback_compound)
    expected_stint = expected_stint.fillna(global_pit_median).clip(lower=1)

    X["ExpectedStintLen"] = expected_stint.astype("float32")
    X["StintCompletionRatio"] = (X["TyreLife"] / expected_stint).astype("float32")
    X["StintOverrun"] = (X["TyreLife"] - expected_stint).astype("float32")
    X["LapsToNextStopProxy"] = (
        (expected_stint - X["TyreLife"]).clip(lower=0).astype("float32")
    )
    X["StopWithin3Proxy"] = (X["LapsToNextStopProxy"] <= 3).astype("int8")
    X["StopWithin5Proxy"] = (X["LapsToNextStopProxy"] <= 5).astype("int8")
    X["CensoredToFinishProxy"] = (
        X["EstimatedLapsRemaining"] <= X["LapsToNextStopProxy"]
    ).astype("int8")

    # Interaction features around stint-end risk
    X["DegXCompletion"] = (X["DegPerTyreLap"] * X["StintCompletionRatio"]).astype(
        "float32"
    )
    X["DeltaXCompletion"] = (X["DeltaPerTyreLap"] * X["StintCompletionRatio"]).astype(
        "float32"
    )
    X["PositionXCompletion"] = (X["Position"] * X["StintCompletionRatio"]).astype(
        "float32"
    )
    X["RaceLeftVsStopProxy"] = (
        X["EstimatedLapsRemaining"] - X["LapsToNextStopProxy"]
    ).astype("float32")

    X = (
        X.sort_values("_orig_order")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000479.",
    }, sort_keys=True))
