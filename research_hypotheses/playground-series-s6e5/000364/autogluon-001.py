...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_slick = comp.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    is_wet = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")

    out["compound_is_slick"] = is_slick
    out["compound_is_wet"] = is_wet
    out["compound_code_rule"] = (
        comp.map({"SOFT": 1, "MEDIUM": 2, "HARD": 3, "INTERMEDIATE": 4, "WET": 5})
        .fillna(0)
        .astype("int8")
    )

    grp = out.groupby(gcols, sort=False)
    prior_pit = grp["PitStop"].cumsum() - out["PitStop"]
    out["pit_sets_so_far_rule"] = (prior_pit + 1).astype("int16")

    for c in ["SOFT", "MEDIUM", "HARD"]:
        used_now = (comp.eq(c) & (out["PitStop"].eq(1) | grp.cumcount().eq(0))).astype(
            "int8"
        )
        out[f"prior_used_{c.lower()}"] = grp[
            used_now.name if False else "_orig_order"
        ].transform(lambda x: 0)

    start_new_set = (out["PitStop"].eq(1) | grp.cumcount().eq(0)).astype("int8")
    for c in ["SOFT", "MEDIUM", "HARD"]:
        flag = ((comp == c) & start_new_set.eq(1)).astype("int8")
        out[f"prior_used_{c.lower()}"] = (
            flag.groupby([out[k] for k in gcols], sort=False)
            .cumsum()
            .shift(fill_value=0)
        )
        first_in_group = grp.cumcount().eq(0)
        out.loc[first_in_group, f"prior_used_{c.lower()}"] = 0
        out[f"prior_used_{c.lower()}"] = out[f"prior_used_{c.lower()}"].astype("int8")

    prior_slick_specs = (
        out[["prior_used_soft", "prior_used_medium", "prior_used_hard"]]
        .sum(axis=1)
        .astype("int8")
    )
    prior_wet_seen = (
        is_wet.groupby([out[k] for k in gcols], sort=False).cumsum().shift(fill_value=0)
    )
    prior_wet_seen.loc[grp.cumcount().eq(0)] = 0

    current_slick_new = (
        is_slick.eq(1)
        & (
            ((comp == "SOFT") & out["prior_used_soft"].eq(0))
            | ((comp == "MEDIUM") & out["prior_used_medium"].eq(0))
            | ((comp == "HARD") & out["prior_used_hard"].eq(0))
        )
    ).astype("int8")

    slick_specs_including_current = (
        (prior_slick_specs + current_slick_new).clip(0, 3).astype("int8")
    )
    dry_proxy = (prior_wet_seen.eq(0) & is_wet.eq(0)).astype("int8")
    monaco_2025 = (
        out["Year"].eq(2025)
        & out["Race"].astype(str).str.contains("Monaco", case=False, na=False)
    ).astype("int8")

    required_sets = np.where(monaco_2025.eq(1), 3, 2)
    required_slick_specs = np.where(dry_proxy.eq(1), 2, 0)

    out["dry_rule_proxy"] = dry_proxy.astype("int8")
    out["slick_compounds_used_rule"] = slick_specs_including_current.astype("int8")
    out["must_use_new_compound"] = (
        (dry_proxy.eq(1)) & (slick_specs_including_current < required_slick_specs)
    ).astype("int8")

    sets_so_far_including_current = out["pit_sets_so_far_rule"] + out["PitStop"].astype(
        "int16"
    )
    owed_by_sets = np.maximum(0, required_sets - sets_so_far_including_current)
    owed_by_specs = np.where(
        dry_proxy.eq(1),
        np.maximum(0, required_slick_specs - slick_specs_including_current),
        0,
    )
    out["min_future_stops_owed"] = np.maximum(owed_by_sets, owed_by_specs).astype(
        "int8"
    )
    out["can_finish_but_owes_stop"] = (
        (out["RaceProgress"] >= 0.80) & (out["min_future_stops_owed"] > 0)
    ).astype("int8")
    out["monaco_2025_three_set_rule"] = monaco_2025.astype("int8")

    out = out.sort_values("_orig_order", kind="mergesort").drop(columns=["_orig_order"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000364"],
        "research_usage_note": "Verified assigned hypothesis 000364.",
    }, sort_keys=True))
