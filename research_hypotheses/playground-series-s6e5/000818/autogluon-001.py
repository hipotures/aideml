...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    est_total_now = out["LapNumber"] / out["RaceProgress"].clip(lower=eps)
    out["Estimated_Total_Laps"] = (
        g_traj["LapNumber"]
        .transform("max")
        .where(
            False,
            g_traj.apply(
                lambda x: (x["LapNumber"] / x["RaceProgress"].clip(lower=eps)).max()
            )
            .reindex(pd.MultiIndex.from_frame(out[traj_keys]))
            .to_numpy(),
        )
    )
