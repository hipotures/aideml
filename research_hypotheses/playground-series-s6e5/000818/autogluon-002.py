...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    traj_keys = ["Year", "Race", "Driver"]
    stint_keys = ["Year", "Race", "Driver", "Stint"]

    out = out.sort_values(traj_keys + ["LapNumber", "_row_id"]).reset_index(drop=True)

    g_traj = out.groupby(traj_keys, sort=False)
    g_stint = out.groupby(stint_keys, sort=False)

    eps = 1e-6

    # Estimate total race laps from observed progress and current lap, then derive remaining distance.
    est_total_now = (out["LapNumber"] / out["RaceProgress"].clip(lower=eps)).replace(
        [np.inf, -np.inf], np.nan
    )
    out["Estimated_Total_Laps"] = est_total_now.groupby(
        [out[c] for c in traj_keys], sort=False
    ).transform("max")
    out["Estimated_Total_Laps"] = out["Estimated_Total_Laps"].fillna(
        g_traj["LapNumber"].transform("max")
    )
    out["Estimated_Total_Laps"] = np.maximum(
        out["Estimated_Total_Laps"], g_traj["LapNumber"].transform("max")
    )
    out["Laps_Remaining"] = (out["Estimated_Total_Laps"] - out["LapNumber"]).clip(
        lower=0
    )
    out["Laps_Remaining_Frac"] = out["Laps_Remaining"] / out[
        "Estimated_Total_Laps"
    ].clip(lower=1)

    # Stint progression and tyre/degradation intensity.
    out["Stint_Max_TyreLife"] = g_stint["TyreLife"].transform("max")
    out["Stint_Progress"] = out["TyreLife"] / out["Stint_Max_TyreLife"].clip(lower=1)
    out["Degradation_Per_Lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["Degradation_x_StintProgress"] = (
        out["Degradation_Per_Lap"] * out["Stint_Progress"]
    )

    # Past-only trajectory deltas for discrete hazard dynamics.
    out["Prev_LapTime_Delta"] = g_traj["LapTime_Delta"].shift(1)
    out["Prev_Position_Change"] = g_traj["Position_Change"].shift(1)
    out["Prev_Degradation_Per_Lap"] = g_traj["Degradation_Per_Lap"].shift(1)

    out["LapTime_Delta_Change"] = out["LapTime_Delta"] - out["Prev_LapTime_Delta"]
    out["Position_Change_Trend"] = out["Position_Change"] - out["Prev_Position_Change"]
    out["Degradation_Accel"] = (
        out["Degradation_Per_Lap"] - out["Prev_Degradation_Per_Lap"]
    )

    # Cause-specific proxy features:
    # pit hazard rises with old tyres/degradation; finish hazard rises when few laps remain.
    out["Finish_Hazard_Proxy"] = 1.0 / (out["Laps_Remaining"] + 1.0)
    out["Pit_Hazard_Proxy"] = (
        0.35 * out["TyreLife"]
        + 0.25 * out["Degradation_Per_Lap"]
        + 0.20 * out["LapTime_Delta"].clip(lower=0)
        + 0.10 * out["LapTime_Delta_Change"].clip(lower=0).fillna(0)
        - 0.10 * out["Position_Change"].fillna(0)
    )
    out["Pit_vs_Finish_Risk"] = out["Pit_Hazard_Proxy"] * (
        1.0 - out["Finish_Hazard_Proxy"]
    )
    out["LateRace_OldTyre_Suppressed"] = out["TyreLife"] * out["Laps_Remaining_Frac"]
    out["NeedStop_NotFinish"] = (
        out["Degradation_x_StintProgress"] * out["Laps_Remaining_Frac"]
    )

    # Compound-specific hazard modulation.
    compound_upper = out["Compound"].astype(str).str.upper()
    out["Is_Soft"] = (compound_upper == "SOFT").astype(np.int8)
    out["Is_Medium"] = (compound_upper == "MEDIUM").astype(np.int8)
    out["Is_Hard"] = (compound_upper == "HARD").astype(np.int8)
    out["Is_WetFamily"] = compound_upper.isin(["INTERMEDIATE", "WET"]).astype(np.int8)

    out["Soft_PitPressure"] = out["Pit_vs_Finish_Risk"] * out["Is_Soft"]
    out["Medium_PitPressure"] = out["Pit_vs_Finish_Risk"] * out["Is_Medium"]
    out["Hard_PitPressure"] = out["Pit_vs_Finish_Risk"] * out["Is_Hard"]
    out["Wet_PitPressure"] = out["Pit_vs_Finish_Risk"] * out["Is_WetFamily"]

    # Small set of bounded categorical frequency features, safe on concatenated feature-only data.
    for col in ["Driver", "Race", "Compound"]:
        freq = out[col].value_counts(dropna=False)
        out[f"{col}_Freq"] = out[col].map(freq).astype(np.float32)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000818"],
        "research_usage_note": "Verified assigned hypothesis 000818.",
    }, sort_keys=True))
