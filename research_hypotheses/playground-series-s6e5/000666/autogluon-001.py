...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year = out["Year"].astype("int64").astype("string")
    lap = out["LapNumber"].astype("int64").astype("string")

    out["race_year_query"] = (race + "_" + year).astype("category")
    out["race_lap_query"] = (race + "_" + year + "_L" + lap).astype("category")

    q = out.groupby("race_lap_query", observed=True, sort=False)
    out["query_field_size"] = q["LapNumber"].transform("size").astype("int16")
    ...
    denom = out["query_field_size"].clip(lower=1).astype("float32")
    for col in rank_cols:
        if col in out.columns:
            x = pd.to_numeric(out[col], errors="coerce").astype("float32")
            mean = q[col].transform("mean").astype("float32")
            out[f"{col}_query_mean"] = mean
            out[f"{col}_query_diff"] = (x - mean).astype("float32")
            out[f"{col}_query_rank_pct"] = (
                q[col].rank(method="average", pct=True).astype("float32")
            )
            out[f"{col}_query_rank_desc_pct"] = (
                q[col]
                .rank(method="average", ascending=False, pct=True)
                .astype("float32")
            )

    out["tyre_life_x_progress"] = (
    ...
    out["pace_stress"] = (
        out["LapTime_Delta"].astype("float32")
        + out["Cumulative_Degradation"].astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000666.",
    }, sort_keys=True))
