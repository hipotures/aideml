...
def sorted_indices_by_group(df, indices):
    return (
        ...
        .sort_values(["query_group", id_col], kind="mergesort")
        .index.to_numpy()
    )
...
test_order = sorted_indices_by_group(test, np.arange(len(test)))
test_pool = Pool(
    data=test.loc[test_order, feature_cols],
    group_id=test.loc[test_order, "query_group"].astype(str).values,
    cat_features=cat_cols,
)
...
for fold, (tr_idx, va_idx) in enumerate(splits, start=1):
    train_pool, _ = make_rank_pool(train, tr_idx, y)
    ...
    test_raw_folds[test_order, fold - 1] = model.predict(test_pool).astype(np.float32)

    fold_auc = roc_auc_score(y[va_order], oof_raw[va_order])
    print(f"Fold {fold} ROC AUC raw rank score: {fold_auc:.6f}")
...
pred_by_id = pd.DataFrame({id_col: test[id_col].values, target_col: test_pred})
...
print(f"OOF ROC AUC raw rank score: {raw_auc:.6f}")
