...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["laps_remaining_est"] = (lap / progress - lap).clip(lower=0)
    out["tyre_life_frac_of_race"] = tyre / (lap / progress).clip(lower=1)
    out["tyre_x_progress"] = tyre * progress
    out["stint_x_tyre"] = stint * tyre
    out["degradation_per_tyre_lap"] = degr / tyre.clip(lower=1)
    out["abs_laptime_delta"] = lap_delta.abs()
    out["lap_delta_per_tyre_lap"] = lap_delta / tyre.clip(lower=1)
    out["lap_time_vs_race_progress"] = lap_time / progress
    ...
    out["old_tyre"] = (tyre >= 20).astype("int8")
    out["very_old_tyre"] = (tyre >= 35).astype("int8")

    current_stop_proxy = ((tyre <= 2) & (stint > 1)).astype("float32")
    out["current_pit_x_tyre"] = current_stop_proxy * tyre
    out["current_pit_x_progress"] = current_stop_proxy * progress

    for col in ["Race", "Driver", "Compound", "Race_Year", "Race_Year_Driver"]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000217.",
    }, sort_keys=True))
