...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sc_like_all = (delta.abs() > delta.abs().quantile(0.97)) | (
        laptime > laptime.quantile(0.985)
    )

    race_lower = out["Race"].astype("string").str.lower().fillna("")
    learned_loss = pd.Series(22.0, index=out.index, dtype="float32")
    learned_loss += np.where(
        race_lower.str.contains("monaco|singapore|hungar", regex=True), 4.0, 0.0
    ).astype("float32")
    learned_loss += np.where(
        race_lower.str.contains("italian|monza|las vegas", regex=True), -3.0, 0.0
    ).astype("float32")
    learned_loss += np.where(
        race_lower.str.contains("saudi|jeddah|austria", regex=True), -1.5, 0.0
    ).astype("float32")
    learned_loss += np.where(wet_all.to_numpy(), -2.0, 0.0).astype("float32")
    learned_loss += np.where(sc_like_all.to_numpy(), -5.0, 0.0).astype("float32")
    learned_loss = learned_loss.clip(8, 45).astype("float32")
    out["learned_race_pit_loss"] = learned_loss

    compound_warmup = (
        out["Compound"]
        .astype(str)
        .map({"SOFT": 0.7, "MEDIUM": 1.1, "HARD": 1.6, "INTERMEDIATE": 2.0, "WET": 2.4})
        .fillna(1.3)
        .astype("float32")
    )
    warmup = (compound_warmup + 0.015 * tyre + 0.05 * pos.clip(1, 20)).clip(-5, 12)
    undercut = (
        0.18 * delta.clip(-30, 30)
        + 0.035 * tyre
        + 0.002 * degr.clip(-100, 400)
        - 0.04 * pos_chg.clip(-10, 10)
    ).clip(-12, 18)
    sc_loss = (learned_loss * np.where(sc_like_all.to_numpy(), 0.55, 0.85)).clip(5, 35)
