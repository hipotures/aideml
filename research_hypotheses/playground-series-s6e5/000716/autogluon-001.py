...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap = out["LapNumber"].astype("float32")
    tyre = out["TyreLife"].astype("float32")
    progress = out["RaceProgress"].astype("float32")
    laptime = out["LapTime (s)"].astype("float32")
    delta = out["LapTime_Delta"].astype("float32")
    degr = out["Cumulative_Degradation"].astype("float32")
    pit = out["PitStop"].astype("float32")
    pos = out["Position"].astype("float32")
    pos_chg = out["Position_Change"].astype("float32")
    stint = out["Stint"].astype("float32")

    out["tyre_life_sq"] = tyre * tyre
    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["lap_progress_x_tyre"] = progress * tyre
    out["late_stint_pressure"] = np.log1p(tyre) * progress
    out["pit_last_lap"] = pit
    out["fresh_tyre_after_stop"] = ((pit > 0.5) & (tyre <= 3)).astype("int8")
    out["traffic_pressure"] = (21.0 - pos) + pos_chg.abs()
    out["is_wet_compound"] = (
        out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]).astype("int8")
    )

    peer_key = [out["Race"], out["Year"], out["LapNumber"]]
    lap_peer_median = (
        laptime.groupby(peer_key, observed=True).transform("median").astype("float32")
    )
    lap_peer_delta = (laptime - lap_peer_median).clip(-120, 120).astype("float32")
    out["lap_peer_time_delta"] = lap_peer_delta

    train = out.iloc[:n_train].copy()
    train_shock = lap_peer_delta.iloc[:n_train]
    train_pit = train["PitStop"].astype("float32")
    stop_rows = train_pit > 0.5

    wet_train = train["Compound"].astype(str).isin(["INTERMEDIATE", "WET"])
    sc_like_train = (
        train["LapTime_Delta"].abs() > train["LapTime_Delta"].abs().quantile(0.97)
    ) | (train["LapTime (s)"] > train["LapTime (s)"].quantile(0.985))
    regime_train = np.select(
        [wet_train.to_numpy(), sc_like_train.to_numpy()], ["wet", "slow"], default="dry"
    )
    train["_econ_regime"] = pd.Categorical(regime_train)

    global_pit_loss = (
        float(train_shock[stop_rows].median()) if bool(stop_rows.any()) else 20.0
    )
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 20.0

    race_loss = (
        train.loc[stop_rows]
        .assign(_shock=train_shock[stop_rows])
        .groupby("Race", observed=True)["_shock"]
        .median()
    )
    race_year_loss = (
        train.loc[stop_rows]
        .assign(_shock=train_shock[stop_rows])
        .groupby(["Race", "Year"], observed=True)["_shock"]
        .median()
    )
    race_regime_loss = (
        train.loc[stop_rows]
        .assign(_shock=train_shock[stop_rows])
        .groupby(["Race", "_econ_regime"], observed=True)["_shock"]
        .median()
    )

    race_loss_map = out["Race"].map(race_loss).astype("float32")
    race_year_loss_map = pd.Series(
        pd.MultiIndex.from_frame(out[["Race", "Year"]]).map(race_year_loss),
        index=out.index,
        dtype="float32",
    )

    wet_all = out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"])
    sc_like_all = (delta.abs() > train["LapTime_Delta"].abs().quantile(0.97)) | (
        laptime > train["LapTime (s)"].quantile(0.985)
    )
    regime_all = np.select(
        [wet_all.to_numpy(), sc_like_all.to_numpy()], ["wet", "slow"], default="dry"
    )
    race_regime_map = pd.Series(
        pd.MultiIndex.from_arrays([out["Race"], regime_all]).map(race_regime_loss),
        index=out.index,
        dtype="float32",
    )

    learned_loss = (
        race_year_loss_map.fillna(race_regime_map)
        .fillna(race_loss_map)
        .fillna(global_pit_loss)
        .astype("float32")
    )
    learned_loss = learned_loss.clip(-10, 80)
    out["learned_race_pit_loss"] = learned_loss

    stop_train = train.loc[stop_rows].copy()
    stop_train["_shock"] = train_shock[stop_rows]
    stop_train["_warmup_proxy"] = (
        stop_train["LapTime_Delta"] - stop_train["_shock"]
    ).clip(-60, 60)
    stop_train["_undercut_proxy"] = (
        -stop_train["LapTime_Delta"] - 0.08 * stop_train["TyreLife"]
    ).clip(-40, 40)
    stop_train["_sc_loss_proxy"] = np.where(
        stop_train["_econ_regime"].astype(str).eq("slow"), stop_train["_shock"], np.nan
    )

    global_warmup = (
        float(stop_train["_warmup_proxy"].median()) if len(stop_train) else 0.0
    )
    global_undercut = (
        float(stop_train["_undercut_proxy"].median()) if len(stop_train) else 0.0
    )
    global_sc_loss = (
        float(pd.Series(stop_train["_sc_loss_proxy"]).median())
        if len(stop_train)
        else global_pit_loss * 0.55
    )
    for v_name, fallback in [
        ("global_warmup", 0.0),
        ("global_undercut", 0.0),
        ("global_sc_loss", global_pit_loss * 0.55),
    ]:
        if not np.isfinite(locals()[v_name]):
            locals()[v_name] = fallback

    warmup_by_race = stop_train.groupby("Race", observed=True)["_warmup_proxy"].median()
    undercut_by_race = stop_train.groupby("Race", observed=True)[
        "_undercut_proxy"
    ].median()
    sc_loss_by_race = stop_train.groupby("Race", observed=True)[
        "_sc_loss_proxy"
    ].median()
    ...
    warmup = (
        ...
        .map(warmup_by_race)
        .astype("float32")
        .fillna(global_warmup)
        .clip(-30, 45)
    )
    undercut = (
        out["Race"]
        .map(undercut_by_race)
        .astype("float32")
        .fillna(global_undercut)
        .clip(-25, 25)
    )
    sc_loss = (
        out["Race"]
        .map(sc_loss_by_race)
        .astype("float32")
        .fillna(global_sc_loss)
        .clip(-10, 60)
    )

    out["learned_warmup_penalty"] = warmup.astype("float32")
    out["learned_undercut_benefit"] = undercut.astype("float32")
    out["learned_sc_adjusted_loss"] = sc_loss.astype("float32")
    out["sc_like_proxy"] = sc_like_all.astype("int8")

    tyre_pressure = np.log1p(tyre) + 0.015 * degr.clip(-100, 400) + 0.35 * stint
    wait_cost = (
        0.35 * tyre_pressure + 0.12 * delta.clip(-30, 30) + 0.18 * pos_chg.clip(-10, 10)
    )
    stop_now_value = wait_cost + undercut - learned_loss
    wait_one_lap_value = (
        wait_cost + 0.45 * np.log1p(tyre + 1.0) - warmup - 0.15 * learned_loss
    )

    out["stop_now_value_learned"] = stop_now_value.astype("float32")
    out["wait_one_lap_value_learned"] = wait_one_lap_value.astype("float32")
    out["stop_vs_wait_value_learned"] = (stop_now_value - wait_one_lap_value).astype(
        "float32"
    )
    out["pit_loss_per_progress_left"] = (
        learned_loss / (1.02 - progress).clip(0.05, 1.02)
    ).astype("float32")
    out["undercut_x_tyre_pressure"] = (undercut * tyre_pressure).astype("float32")
    out["sc_loss_discount"] = (learned_loss - sc_loss).astype("float32")
    out["race_lap_loss_pressure"] = (learned_loss * progress * np.log1p(tyre)).astype(
        "float32"
    )

    race_freq = (
        out["Race"].map(out["Race"].value_counts(normalize=True)).astype("float32")
    )
    driver_freq = (
        out["Driver"].map(out["Driver"].value_counts(normalize=True)).astype("float32")
    )
    out["race_freq"] = race_freq
    out["driver_freq"] = driver_freq
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000716.",
    }, sort_keys=True))
