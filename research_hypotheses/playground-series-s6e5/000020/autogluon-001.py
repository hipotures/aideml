...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(
        seq_cols + ["LapNumber", "TyreLife", "_row_id"], kind="mergesort"
    )

    progress = out["RaceProgress"].clip(lower=0.01)
    est_total_laps = (out["LapNumber"] / progress).clip(
        lower=out["LapNumber"], upper=90
    )
    ...
    out["compound_life_cap"] = out["Compound"].map(compound_life).fillna(28.0)
    out["est_total_laps"] = est_total_laps
    out["laps_remaining_est"] = laps_remaining
    out["tyre_laps_needed_to_finish"] = out["TyreLife"] + laps_remaining
    out["tyre_finish_margin"] = (
        out["compound_life_cap"] - out["tyre_laps_needed_to_finish"]
    )
    out["can_finish_current_tyre"] = (out["tyre_finish_margin"] >= 0).astype("int8")
    ...
    for c in ["SOFT", "MEDIUM", "HARD"]:
        col = f"seen_{c.lower()}"
        out[col] = (out["Compound"] == c).astype("int8")
        out[col] = out.groupby(seq_cols, sort=False)[col].cummax()

    out["seen_dry_compounds_so_far"] = out[
        ["seen_soft", "seen_medium", "seen_hard"]
    ].sum(axis=1)
    out["stops_made_so_far"] = (out["Stint"] - 1).clip(lower=0)

    dry_debt = (
        (out["wet_rule_exempt"] == 0)
        & (out["can_finish_current_tyre"] == 1)
        & (out["seen_dry_compounds_so_far"] < 2)
    )
    stop_debt = (
        (out["wet_rule_exempt"] == 0)
        & (out["can_finish_current_tyre"] == 1)
        & (out["stops_made_so_far"] < 1)
    )

    out["dry_compound_debt"] = dry_debt.astype("int8")
    out["stop_debt"] = stop_debt.astype("int8")
    out["late_race_phase"] = (out["RaceProgress"] >= 0.7).astype("int8")
    out["late_race_legal_pressure"] = (
        out["late_race_phase"]
        * ((out["dry_compound_debt"] == 1) | (out["stop_debt"] == 1)).astype("int8")
    ).astype("int8")

    out["can_finish_but_owes_dry_compound"] = (
        (out["can_finish_current_tyre"] == 1) & (out["dry_compound_debt"] == 1)
    ).astype("int8")
    out["can_finish_but_owes_stop"] = (
        (out["can_finish_current_tyre"] == 1) & (out["stop_debt"] == 1)
    ).astype("int8")
    out["finish_margin_x_legal_pressure"] = out["tyre_finish_margin"] * (
        (out["dry_compound_debt"] == 1) | (out["stop_debt"] == 1)
    ).astype("int8")
    out["finish_margin_x_late_pressure"] = (
        out["tyre_finish_margin"] * out["late_race_legal_pressure"]
    )
    out["remaining_laps_x_debt"] = out["laps_remaining_est"] * (
        out["dry_compound_debt"] + out["stop_debt"]
    )
    out["degradation_finish_buffer"] = (
        out["tyre_finish_margin"] - out["Cumulative_Degradation"] / 100.0
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000020"],
        "research_usage_note": "Verified assigned hypothesis 000020.",
    }, sort_keys=True))
