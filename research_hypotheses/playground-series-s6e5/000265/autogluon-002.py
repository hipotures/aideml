...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Compound", "Driver", "Race"]:
        s = out[col].astype("string").str.strip().str.upper().fillna("MISSING")
        out[col] = s.astype("category")
        out[f"{col}_freq"] = s.map(s.value_counts(dropna=False)).astype("int32")
    ...
    comp = out["Compound"].astype("string")
    base_life = (
        comp.map(
            {
                "SOFT": 16.0,
    ...
    out["compound_base_life"] = base_life
    out["compound_soft"] = comp.eq("SOFT").astype("int8")
    out["compound_medium"] = comp.eq("MEDIUM").astype("int8")
    out["compound_hard"] = comp.eq("HARD").astype("int8")
    out["compound_wet_or_inter"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")

    year_s = out["Year"].round().astype("Int64").astype("string")
    stint_s = out["Stint"].round().astype("Int64").astype("string")
    out["Compound_Stint"] = (comp + "_S" + stint_s).astype("category")
    out["Race_Compound"] = (out["Race"].astype("string") + "_" + comp).astype(
        "category"
    )
    out["Year_Race"] = (year_s + "_" + out["Race"].astype("string")).astype("category")
    out["Driver_Race"] = (
        out["Driver"].astype("string") + "_" + out["Race"].astype("string")
    ).astype("category")

    for col in ["Compound_Stint", "Race_Compound", "Year_Race", "Driver_Race"]:
        s = out[col].astype("string")
        out[f"{col}_freq"] = s.map(s.value_counts(dropna=False)).astype("int32")

    eps = np.float32(1e-3)
    tyre_life = out["TyreLife"].clip(lower=1).astype("float32")
    lap_num = out["LapNumber"].clip(lower=1).astype("float32")
    race_progress = out["RaceProgress"].clip(lower=eps, upper=1.0).astype("float32")

    out["tyre_life_log1p"] = np.log1p(tyre_life).astype("float32")
    out["tyre_life_sq"] = (tyre_life * tyre_life).astype("float32")
    out["tyre_life_ratio_base"] = (tyre_life / base_life.clip(lower=1)).astype(
        "float32"
    )
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / tyre_life
    ).astype("float32")
    out["laptime_delta_abs"] = out["LapTime_Delta"].abs().astype("float32")
    out["position_change_abs"] = out["Position_Change"].abs().astype("float32")
    out["position_inv"] = (21.0 - out["Position"].clip(lower=1, upper=20)).astype(
        "float32"
    )
    out["stint_x_tyre_life"] = (out["Stint"] * tyre_life).astype("float32")
    out["progress_x_tyre_life"] = (race_progress * tyre_life).astype("float32")

    keys = ["Year", "Race", "Driver"]
    sort_cols = keys + ["LapNumber"]
    seq_cols = list(
        dict.fromkeys(
            sort_cols
            + [
    ...
    sdf = out[seq_cols].sort_values(sort_cols, kind="mergesort")
    base_g = sdf.groupby(keys, sort=False, observed=True)
    prev_stint = base_g["Stint"].shift(1)
    prev_tyre = base_g["TyreLife"].shift(1)
    sdf["_stop_signal"] = (
        (sdf["Stint"] > prev_stint.fillna(sdf["Stint"]))
        | (sdf["TyreLife"] < (prev_tyre - 1).fillna(sdf["TyreLife"]))
    ).astype("int8")
    g = sdf.groupby(keys, sort=False, observed=True)

    seq_features = []
    sdf["seq_lap_ord"] = (g.cumcount() + 1).astype("int16")
    sdf["stops_so_far"] = g["_stop_signal"].cumsum().astype("int16")
    sdf["current_stop_proxy"] = sdf["_stop_signal"].astype("int8")
    sdf["_last_pit_lap"] = sdf["LapNumber"].where(sdf["_stop_signal"].eq(1))
    sdf["_last_pit_lap"] = sdf.groupby(keys, sort=False, observed=True)[
        "_last_pit_lap"
    ].ffill()
    sdf["laps_since_pit"] = (
        (sdf["LapNumber"] - sdf["_last_pit_lap"].fillna(0))
        .clip(lower=0)
        .astype("float32")
    )
    seq_features.extend(["seq_lap_ord", "stops_so_far", "laps_since_pit", "current_stop_proxy"])

    for lag in (1, 2, 3):
        name = f"pit_prev{lag}"
        sdf[name] = g["_stop_signal"].shift(lag).fillna(0).astype("int8")
        seq_features.append(name)

    for window in (3, 5, 10):
        name = f"pit_roll{window}_sum"
        sdf[name] = (
            g["_stop_signal"]
    ...
    for col, prefix in [
        ...
        prev = g[col].shift(1)
        prev_name = f"{prefix}_prev1"
        chg_name = f"{prefix}_chg1"
        sdf[prev_name] = prev.astype("float32")
        sdf[chg_name] = (sdf[col] - prev).fillna(0).astype("float32")
        seq_features.extend([prev_name, chg_name])

        for window in (3, 5):
            mean_name = f"{prefix}_roll{window}_mean"
            std_name = f"{prefix}_roll{window}_std"
            mean = (
                g[col]
                .rolling(window, min_periods=1)
                .mean()
            ...
            sdf[mean_name] = mean.astype("float32")
            sdf[std_name] = std.fillna(0).astype("float32")
            seq_features.extend([mean_name, std_name])

    for col in seq_features:
        out[col] = sdf[col].reindex(out.index)

    total_laps_est = (lap_num / race_progress).replace([np.inf, -np.inf], np.nan)
    total_laps_est = total_laps_est.fillna(lap_num.max()).clip(lower=lap_num)
    out["estimated_total_laps"] = total_laps_est.astype("float32")
    out["estimated_laps_remaining"] = (
        (total_laps_est - lap_num).clip(lower=0).astype("float32")
    )

    pit_now = out["current_stop_proxy"].fillna(0).clip(0, 1).astype("float32")
    stops = out["stops_so_far"].fillna(0).astype("float32")
    deg_signal = np.tanh(
        (out["degradation_per_tyre_lap"].fillna(0) / 35.0).clip(-20, 20)
    ).astype("float32")
    delta_signal = np.tanh((out["LapTime_Delta"].fillna(0) / 4.0).clip(-20, 20)).astype(
        "float32"
    )

    rank_parts = []
    for h in (1, 2, 3):
        tyre_h = (tyre_life + h).astype("float32")
        progress_h = (
            ((lap_num + h) / total_laps_est).clip(lower=0, upper=1.05).astype("float32")
        )
        remaining_h = (total_laps_est - lap_num - h).clip(lower=0).astype("float32")
        life_ratio_h = (tyre_h / base_life.clip(lower=1)).astype("float32")

        out[f"hazard_h{h}_tyre_life"] = tyre_h
        out[f"hazard_h{h}_life_ratio"] = life_ratio_h
        out[f"hazard_h{h}_race_progress"] = progress_h
        out[f"hazard_h{h}_laps_remaining"] = remaining_h

        score = (
            1.45 * (life_ratio_h - 0.75)
            + 0.90 * (progress_h - 0.35)
            + 0.50 * deg_signal
            + 0.25 * delta_signal
            + 0.10 * out["Stint"].fillna(1).astype("float32")
            - 0.28 * stops
            - 2.25 * pit_now
            - 0.015 * remaining_h
        )
        score = 1.0 / (1.0 + np.exp(-np.asarray(score).clip(-20, 20)))
        score = pd.Series(score, index=out.index).astype("float32")
        out[f"hazard_h{h}_proxy"] = score
        rank_parts.append(score.rank(method="average", pct=True).astype("float32"))

    out["hazard_rank_blend_123"] = (
        pd.concat(rank_parts, axis=1).mean(axis=1).astype("float32")
    )
    out["post_pit_cooldown"] = np.exp(
        -(out["laps_since_pit"].fillna(lap_num) / 3.0).clip(lower=0, upper=20)
    ).astype("float32")

    for col in out.select_dtypes(include=["float64"]).columns:
        out[col] = out[col].astype("float32")

    out.attrs["research_hypotheses_llm_claimed_used"] = ["000265"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000265"],
        "research_usage_note": "Verified assigned hypothesis 000265.",
    }, sort_keys=True))
