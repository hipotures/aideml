...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sdf["PitStop"] = sdf["PitStop"].fillna(0).clip(0, 1).astype("int8")
    g = sdf.groupby(keys, sort=False, observed=True)

    seq_features = []
    sdf["seq_lap_ord"] = (g.cumcount() + 1).astype("int16")
    sdf["stops_so_far"] = g["PitStop"].cumsum().astype("int16")
    sdf["_last_pit_lap"] = sdf["LapNumber"].where(sdf["PitStop"].eq(1))
    ...
    seq_features.extend(["seq_lap_ord", "stops_so_far", "laps_since_pit"])

    for lag in (1, 2, 3):
        name = f"pit_prev{lag}"
        sdf[name] = g["PitStop"].shift(lag).fillna(0).astype("int8")
    ...
    pit_now = out["PitStop"].fillna(0).clip(0, 1).astype("float32")
