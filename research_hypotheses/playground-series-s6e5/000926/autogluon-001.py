...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_orig_order_000926"] = np.arange(len(out))

    sort_cols = ["Race", "Year", "Driver", "LapNumber"]
    out = out.sort_values(sort_cols + ["_orig_order_000926"], kind="mergesort")

    base_group = out.groupby(["Race", "Year", "Driver"], sort=False)
    stint_change = base_group["Stint"].diff().fillna(0).ne(0)
    prev_pit = base_group["PitStop"].shift(1).fillna(0).eq(1)
    out["_seg_000926"] = (
        (stint_change | prev_pit)
        .groupby([out["Race"], out["Year"], out["Driver"]], sort=False)
        .cumsum()
    )

    seg_keys = ["Race", "Year", "Driver", "_seg_000926"]
    seg_group = out.groupby(seg_keys, sort=False)

    cols = ["LapTime_Delta", "Cumulative_Degradation", "Position_Change", "TyreLife"]
    for col in cols:
        safe = col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        out[f"{safe}_lag1_000926"] = seg_group[col].shift(1)
        out[f"{safe}_lag2_000926"] = seg_group[col].shift(2)
        out[f"{safe}_roll3_mean_000926"] = seg_group[col].transform(
            lambda s: s.rolling(3, min_periods=1).mean()
        )
        out[f"{safe}_roll5_mean_000926"] = seg_group[col].transform(
            lambda s: s.rolling(5, min_periods=1).mean()
        )
        out[f"{safe}_slope3_000926"] = (out[col] - seg_group[col].shift(2)) / 2.0
        out[f"{safe}_slope5_000926"] = (out[col] - seg_group[col].shift(4)) / 4.0

    out["laps_in_reset_segment_000926"] = seg_group.cumcount() + 1
    out["pit_lag1_000926"] = base_group["PitStop"].shift(1).fillna(0).astype("int8")
    out["pit_lag2_000926"] = base_group["PitStop"].shift(2).fillna(0).astype("int8")
    out["stint_changed_000926"] = stint_change.astype("int8")
    out["out_lap_like_000926"] = ((out["pit_lag1_000926"] == 1) | stint_change).astype(
        "int8"
    )

    ltd_roll = out["LapTime_Delta_roll3_mean_000926"]
    deg_slope = out["Cumulative_Degradation_slope3_000926"]
    out["sudden_pace_drop_000926"] = (out["LapTime_Delta"] > (ltd_roll + 5.0)).astype(
        "int8"
    )
    out["degradation_accel_000926"] = (
        deg_slope > out["Cumulative_Degradation_slope5_000926"]
    ).astype("int8")
    out["last_lap_pit_anomaly_000926"] = (
        (out["pit_lag1_000926"] == 1) | (out["LapTime_Delta_lag1_000926"].abs() > 20.0)
    ).astype("int8")

    out = out.sort_values("_orig_order_000926", kind="mergesort").drop(
        columns=["_orig_order_000926", "_seg_000926"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000926"],
        "research_usage_note": "Verified assigned hypothesis 000926.",
    }, sort_keys=True))
