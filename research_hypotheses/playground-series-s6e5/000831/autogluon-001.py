...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    peer_cols = ["TyreLife", "LapTime_Delta", "Cumulative_Degradation", "Position"]

    g = out.groupby(grp_cols, sort=False)
    field_size = g["Position"].transform("count").astype("float32")
    out["field_size"] = field_size
    out["position_pct_field"] = (
        ((out["Position"] - 1) / (field_size - 1).replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )

    for c in peer_cols:
        rank = g[c].rank(method="average", ascending=True)
        pct = g[c].rank(method="average", pct=True, ascending=True)
        ...
        out[f"{c}_field_rank"] = rank.astype("float32")
        out[f"{c}_field_pct"] = pct.astype("float32")
        out[f"{c}_field_z"] = ((out[c] - mean) / std).fillna(0).astype("float32")
        out[f"{c}_field_quantile_gap_q25"] = (out[c] - q25).astype("float32")
        out[f"{c}_field_quantile_gap_q75"] = (out[c] - q75).astype("float32")

    ordered = out.sort_values(grp_cols + ["Position"], kind="mergesort")
    og = ordered.groupby(grp_cols, sort=False)

    for c in peer_cols:
        ordered[f"ahead_{c}"] = og[c].shift(1)
        ordered[f"behind_{c}"] = og[c].shift(-1)
        ordered[f"gap_to_ahead_{c}"] = (ordered[c] - ordered[f"ahead_{c}"]).astype(
            "float32"
        )
        ordered[f"gap_to_behind_{c}"] = (ordered[f"behind_{c}"] - ordered[c]).astype(
            "float32"
        )

    for c in ["TyreLife", "LapTime_Delta", "Cumulative_Degradation"]:
        local_sum = ordered[c].copy()
        local_cnt = np.ones(len(ordered), dtype="float32")
        for k in (1, 2):
            prev_v = og[c].shift(k)
            next_v = og[c].shift(-k)
            local_sum = local_sum.add(prev_v.fillna(0), fill_value=0).add(
                next_v.fillna(0), fill_value=0
            )
            local_cnt += prev_v.notna().astype("float32").to_numpy()
            local_cnt += next_v.notna().astype("float32").to_numpy()

        ordered[f"local_pm2_{c}_mean"] = (local_sum / local_cnt).astype("float32")
        ordered[f"local_pm2_{c}_diff"] = (
            ordered[c] - ordered[f"local_pm2_{c}_mean"]
        ).astype("float32")

    ordered["ahead_older"] = (
        (ordered["ahead_TyreLife"] > ordered["TyreLife"]).fillna(False).astype("int8")
    )
    ordered["ahead_slower"] = (
        (ordered["ahead_LapTime_Delta"] > ordered["LapTime_Delta"])
        .fillna(False)
        .astype("int8")
    )
    ordered["behind_undercut_threat"] = (
        (
            (ordered["behind_TyreLife"] < ordered["TyreLife"])
            & (ordered["behind_LapTime_Delta"] < ordered["LapTime_Delta"])
        )
        .fillna(False)
        .astype("int8")
    )
    ordered["dirty_air_pressure"] = (
        ordered["ahead_slower"]
        * (ordered["TyreLife"] - ordered["ahead_TyreLife"]).fillna(0)
    ).astype("float32")
    ordered["clean_air_after_pit_proxy"] = (
        (ordered["Position"] <= 3).astype("int8")
        + (ordered["gap_to_behind_LapTime_Delta"].fillna(0) > 0).astype("int8")
        - ordered["ahead_slower"]
    ).astype("int8")
    ...
    gap_cols = [
        ...
        if c.startswith(
            ("ahead_", "behind_", "gap_to_", "local_pm2_", "dirty_air_", "clean_air_")
        )
    ]
    out = out.join(ordered[new_cols + [c for c in gap_cols if c not in new_cols]])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000831"],
        "research_usage_note": "Verified assigned hypothesis 000831.",
    }, sort_keys=True))
