...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    laps_remaining = (est_total_laps - lap).clip(0, 120)

    out["LapsRemaining_Est"] = laps_remaining
    out["RaceLap_EstTotal"] = est_total_laps
    out["Deg_Per_TyreLife"] = deg / tyre.clip(lower=1.0)
    out["LapTimeDelta_Per_TyreLife"] = delta / tyre.clip(lower=1.0)
    out["TyreLife_RaceProgress"] = tyre * progress
    out["TyreLife_LapShare"] = tyre / lap.clip(lower=1.0)
    out["Stint_TyreLife_Ratio"] = tyre / out["Stint"].astype(float).clip(lower=1.0)

    expected_stint = est_total_laps / (out["Stint"].astype(float) + 1.0)
    finish_margin = expected_stint - tyre
    out["CurrentTyreFinishMargin"] = finish_margin
    out["CurrentTyreFinishPressure"] = (-finish_margin).clip(lower=0)
    out["LapsBeyondFinishWindow"] = (tyre - expected_stint).clip(lower=0)
    out["FinishWindow_AbsMargin"] = finish_margin.abs()

    out["PitWindowPressure"] = (
        0.45 * out["CurrentTyreFinishPressure"]
        + 0.30 * out["Deg_Per_TyreLife"].clip(-20, 80)
        + 0.15 * tyre
        - 0.10 * laps_remaining
    )
    ...
    rank_cols = [
        "TyreLife",
        "Deg_Per_TyreLife",
        "CurrentTyreFinishPressure",
        "LapsBeyondFinishWindow",
        "CurrentTyreFinishMargin",
        "LapsRemaining_Est",
        "PitWindowPressure",
    ]
    for c in rank_cols:
        s = out[c].astype(float).replace([np.inf, -np.inf], np.nan)
        lo, hi = s.quantile(0.001), s.quantile(0.999)
        clipped = s.clip(lo, hi).fillna(s.median())
        out[c + "_Robust"] = clipped
        out[c + "_Rank"] = clipped.rank(method="average", pct=True).astype(float)

    for c in ["Compound", "Driver", "Race"]:
        freq = (
            out[c]
            .astype("category")
            .map(out[c].value_counts(normalize=True))
            .astype(float)
        )
        out[c + "_Freq"] = freq.fillna(0.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000496"],
        "research_usage_note": "Verified assigned hypothesis 000496.",
    }, sort_keys=True))
