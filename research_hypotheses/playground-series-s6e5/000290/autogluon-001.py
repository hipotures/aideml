...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_wet"] = compound.isin(["WET", "INTERMEDIATE"]).astype("int8")
    out["is_slick"] = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["is_soft"] = (compound == "SOFT").astype("int8")
    out["is_medium"] = (compound == "MEDIUM").astype("int8")
    out["is_hard"] = (compound == "HARD").astype("int8")

    rp = out["RaceProgress"].astype(float)
    out["race_phase"] = pd.cut(
        rp, bins=[-np.inf, 0.2, 0.6, np.inf], labels=["early", "mid", "late"]
    ).astype(str)
    out["is_early_race"] = (rp <= 0.2).astype("int8")
    out["is_late_race"] = (rp >= 0.6).astype("int8")
    out["is_first_stint"] = (out["Stint"] == 1).astype("int8")
    out["is_later_stint"] = (out["Stint"] >= 2).astype("int8")

    out["tyre_life_ratio"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["degradation_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["degradation_per_progress"] = out["Cumulative_Degradation"] / out[
        "RaceProgress"
    ].clip(lower=0.01)
    out["pace_vs_tyre_age"] = out["LapTime_Delta"] / out["TyreLife"].clip(lower=1)
    out["position_pressure"] = (21 - out["Position"]).astype(float) * out[
        "RaceProgress"
    ]
    out["recent_stop_pressure"] = out["PitStop"].astype(float) * out["is_late_race"]
    out["wet_tyre_life"] = out["is_wet"].astype(float) * out["TyreLife"]
    out["wet_degradation"] = out["is_wet"].astype(float) * out["Cumulative_Degradation"]
    out["late_deg"] = out["is_late_race"].astype(float) * out["Cumulative_Degradation"]
    out["first_stint_soft"] = out["is_first_stint"].astype(float) * out[
        "is_soft"
    ].astype(float)
    out["later_stint_hard"] = out["is_later_stint"].astype(float) * out[
        "is_hard"
    ].astype(float)

    key_cols = ["Year", "Race", "Driver", "LapNumber"]
    work = out.sort_values(key_cols, kind="mergesort").copy()
    grp_keys = [work["Year"], work["Race"], work["Driver"]]
    drv_grp = work.groupby(["Year", "Race", "Driver"], sort=False)

    work["driver_lap_idx"] = drv_grp.cumcount().astype("int16")
    work["laps_left_est"] = (
        work["LapNumber"] / work["RaceProgress"].clip(lower=0.01) - work["LapNumber"]
    ).clip(lower=0)

    for col, new_col in [
        ("TyreLife", "lag1_tyre_life"),
        ("LapTime_Delta", "lag1_lap_delta"),
        ("Position_Change", "lag1_pos_change"),
        ("Cumulative_Degradation", "lag1_degradation"),
        ("PitStop", "lag1_pitstop"),
        ("Position", "lag1_position"),
    ]:
        work[new_col] = drv_grp[col].shift(1)

    work["delta_tyre_life"] = work["TyreLife"] - work["lag1_tyre_life"]
    work["delta_degradation"] = (
        work["Cumulative_Degradation"] - work["lag1_degradation"]
    )
    work["delta_position"] = work["Position"] - work["lag1_position"]
    work["delta_lap_delta"] = work["LapTime_Delta"] - work["lag1_lap_delta"]

    for base_col, feat_name in [
        ("LapTime_Delta", "roll3_lap_delta_mean"),
        ("Position_Change", "roll3_pos_change_mean"),
        ("Cumulative_Degradation", "roll3_deg_mean"),
        ("PitStop", "roll3_pitstop_mean"),
    ]:
        shifted = drv_grp[base_col].shift(1)
        work[feat_name] = (
            shifted.groupby(grp_keys, sort=False)
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=[0, 1, 2], drop=True)
        )

    lap_grp = work.groupby(["Year", "Race", "LapNumber"], sort=False)
    work["lap_pos_pct"] = lap_grp["Position"].rank(method="average", pct=True)
    work["lap_tyre_life_pct"] = lap_grp["TyreLife"].rank(method="average", pct=True)
    work["lap_deg_pct"] = lap_grp["Cumulative_Degradation"].rank(
        method="average", pct=True
    )
    work["lap_delta_pct"] = lap_grp["LapTime_Delta"].rank(method="average", pct=True)

    work["rank_gate_wet_late"] = (
        work["is_wet"].astype(float)
        * work["is_late_race"].astype(float)
        * work["lap_tyre_life_pct"]
    )
    work["rank_gate_first_stint"] = (
        work["is_first_stint"].astype(float) * work["lap_deg_pct"]
    )
    work["rank_gate_slick_early"] = (
        work["is_slick"].astype(float)
        * work["is_early_race"].astype(float)
        * work["lap_delta_pct"]
    )

    new_cols = [c for c in work.columns if c not in out.columns]
    out.loc[work.index, new_cols] = work[new_cols]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000290"],
        "research_usage_note": "Verified assigned hypothesis 000290.",
    }, sort_keys=True))
