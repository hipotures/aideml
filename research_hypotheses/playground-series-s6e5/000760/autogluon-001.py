...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_n = g["TyreLife"].transform("size").astype(float)

    for col in rel_cols:
        vals = out[col].astype(float)
        sums = g[col].transform("sum").astype(float)
        sums2 = (
            (vals * vals)
        ...
        loo_n = (group_n - 1.0).clip(lower=1.0)
        loo_mean = (sums - vals) / loo_n
        loo_var = ((sums2 - vals * vals) / loo_n) - loo_mean * loo_mean
        loo_std = np.sqrt(np.maximum(loo_var, 0.0))

        out[f"{col}_race_lap_pct"] = (
            out.groupby(group_cols, sort=False)[col]
            .rank(method="average", pct=True)
            .astype(float)
        )
        out[f"{col}_race_lap_loo_z"] = np.where(
            (group_n > 1.0) & (loo_std > 1e-9),
            (vals - loo_mean) / loo_std,
            0.0,
        )

    sort_cols = ["Year", "Race", "LapNumber", "Position"]
    tmp = out[sort_cols + ["TyreLife", "Cumulative_Degradation"]].sort_values(
        sort_cols, kind="mergesort"
    )
    sg = tmp.groupby(group_cols, sort=False)

    tmp["TyreLife_ahead_gap"] = tmp["TyreLife"].astype(float) - sg["TyreLife"].shift(
        1
    ).astype(float)
    tmp["TyreLife_behind_gap"] = sg["TyreLife"].shift(-1).astype(float) - tmp[
        "TyreLife"
    ].astype(float)
    tmp["Deg_ahead_gap"] = tmp["Cumulative_Degradation"].astype(float) - sg[
        "Cumulative_Degradation"
    ].shift(1).astype(float)
    tmp["Deg_behind_gap"] = sg["Cumulative_Degradation"].shift(-1).astype(float) - tmp[
        "Cumulative_Degradation"
    ].astype(float)
    ...
    for col in [
        ...
        out[col] = tmp[col].reindex(out.index).fillna(0.0)

    same_comp = out.groupby(group_cols + ["Compound"], sort=False)
    same_stint = out.groupby(group_cols + ["Stint"], sort=False)
    same_comp_stint = out.groupby(group_cols + ["Compound", "Stint"], sort=False)

    out["same_compound_count"] = same_comp["TyreLife"].transform("size").astype(float)
    out["same_stint_count"] = same_stint["TyreLife"].transform("size").astype(float)
    out["same_compound_stint_count"] = (
        same_comp_stint["TyreLife"].transform("size").astype(float)
    )

    out["same_compound_older_tyre_count"] = (
        same_comp["TyreLife"].rank(method="min", ascending=False) - 1.0
    ).astype(float)
    out["same_stint_older_tyre_count"] = (
        same_stint["TyreLife"].rank(method="min", ascending=False) - 1.0
    ).astype(float)
    out["same_compound_stint_older_tyre_count"] = (
        same_comp_stint["TyreLife"].rank(method="min", ascending=False) - 1.0
    ).astype(float)

    out["is_oldest_same_compound"] = (
        out["TyreLife"].astype(float)
        == same_comp["TyreLife"].transform("max").astype(float)
    ).astype("int8")
    out["is_oldest_same_compound_stint"] = (
        out["TyreLife"].astype(float)
        == same_comp_stint["TyreLife"].transform("max").astype(float)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000760"],
        "research_usage_note": "Verified assigned hypothesis 000760.",
    }, sort_keys=True))
