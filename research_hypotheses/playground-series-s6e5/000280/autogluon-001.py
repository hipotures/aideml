...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_preseason"] = race.str.contains(
        "Pre-Season", case=False, regex=False
    ).astype("int8")
    out["is_gp"] = (1 - out["is_preseason"]).astype("int8")
    ...
    out["is_early_race"] = (rp <= 0.33).astype("int8")
    out["is_mid_race"] = ((rp > 0.33) & (rp <= 0.66)).astype("int8")
    ...
    out["driver_race_freq"] = (
        out["driver_race"]
        .map(out["driver_race"].value_counts(normalize=True))
        .astype("float32")
    )
    ...
    for c in num_cols:
        s = out[c].astype("float32")
        out[f"{c}_global_rank"] = s.rank(method="average", pct=True).astype("float32")
        out[f"{c}_year_rank"] = (
            s.groupby(out["Year"], observed=True)
            .rank(method="average", pct=True)
            .astype("float32")
        )
        out[f"{c}_compound_rank"] = (
            s.groupby(compound, observed=True)
            .rank(method="average", pct=True)
            .astype("float32")
        )
        out[f"{c}_regime_rank"] = (
            s.groupby(
                [out["Year"], compound, out["is_preseason"], out["race_phase"]],
                observed=True,
            )
            .rank(method="average", pct=True)
            .astype("float32")
    ...
    out["tyre_life_x_late"] = out["TyreLife"].astype("float32") * out["is_late_race"]
    out["degradation_x_late"] = (
        out["Cumulative_Degradation"].astype("float32") * out["is_late_race"]
    )
    out["tyre_life_x_wet"] = out["TyreLife"].astype("float32") * out["is_wet_compound"]
    out["degradation_x_wet"] = (
        out["Cumulative_Degradation"].astype("float32") * out["is_wet_compound"]
    )
    out["lap_delta_abs"] = out["LapTime_Delta"].astype("float32").abs()
    out["position_loss_flag"] = (out["Position_Change"].astype("float32") > 0).astype(
        "int8"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000280.",
    }, sort_keys=True))
