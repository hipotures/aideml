...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    field_group = out.groupby(group_cols, sort=False)

    out["field_size"] = field_group["Position"].transform("count").astype("float32")
    out["position_pct"] = (
        ((out["Position"] - 1) / (out["field_size"] - 1).replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )
    out["inverse_position_pct"] = (1.0 - out["position_pct"]).astype("float32")

    out["lap_delta_rank_pct"] = (
        field_group["LapTime_Delta"].rank(method="average", pct=True).astype("float32")
    )
    out["laptime_rank_pct"] = (
        field_group["LapTime (s)"].rank(method="average", pct=True).astype("float32")
    )
    out["tyrelife_rank_pct"] = (
        field_group["TyreLife"].rank(method="average", pct=True).astype("float32")
    )

    comp_group = out.groupby(group_cols + ["Compound"], sort=False)
    comp_delta_mean = comp_group["LapTime_Delta"].transform("mean")
    comp_delta_std = comp_group["LapTime_Delta"].transform("std").replace(0, np.nan)
    comp_laptime_mean = comp_group["LapTime (s)"].transform("mean")
    out["compound_delta_resid"] = (out["LapTime_Delta"] - comp_delta_mean).astype(
        "float32"
    )
    out["compound_delta_z"] = (
        ((out["LapTime_Delta"] - comp_delta_mean) / comp_delta_std)
        .fillna(0)
        .astype("float32")
    )
    out["compound_laptime_resid"] = (out["LapTime (s)"] - comp_laptime_mean).astype(
        "float32"
    )

    field_delta_mean = field_group["LapTime_Delta"].transform("mean")
    field_delta_std = field_group["LapTime_Delta"].transform("std").replace(0, np.nan)
    out["field_delta_resid"] = (out["LapTime_Delta"] - field_delta_mean).astype(
        "float32"
    )
    out["field_delta_z"] = (
        ((out["LapTime_Delta"] - field_delta_mean) / field_delta_std)
        .fillna(0)
        .astype("float32")
    )

    ordered = out.sort_values(group_cols + ["Position"])
    keys = [ordered[c] for c in group_cols]
    ...
    for col in [
        ...
        prev1 = ordered.groupby(keys, sort=False)[col].shift(1)
        next1 = ordered.groupby(keys, sort=False)[col].shift(-1)
        prev2 = ordered.groupby(keys, sort=False)[col].shift(2)
        next2 = ordered.groupby(keys, sort=False)[col].shift(-2)
        neigh_mean = pd.concat([prev2, prev1, next1, next2], axis=1).mean(axis=1)
        neigh_min = pd.concat([prev2, prev1, next1, next2], axis=1).min(axis=1)
        neigh_max = pd.concat([prev2, prev1, next1, next2], axis=1).max(axis=1)
        ordered[f"nearby_{col}_mean"] = neigh_mean.astype("float32")
        ordered[f"nearby_{col}_min"] = neigh_min.astype("float32")
        ordered[f"nearby_{col}_max"] = neigh_max.astype("float32")
        ordered[f"nearby_{col}_diff"] = (
            (ordered[col] - neigh_mean).fillna(0).astype("float32")
        )

    similar_age = []
    for offset in [-2, -1, 1, 2]:
        shifted = ordered.groupby(keys, sort=False)["TyreLife"].shift(offset)
        similar_age.append(
            (ordered["TyreLife"].sub(shifted).abs() <= 3).astype("float32")
        )
    ordered["nearby_similar_tyre_age_count"] = (
        pd.concat(similar_age, axis=1).sum(axis=1).astype("float32")
    )

    faster_nearby = []
    for offset in [-2, -1, 1, 2]:
        shifted = ordered.groupby(keys, sort=False)["LapTime_Delta"].shift(offset)
        faster_nearby.append((shifted < ordered["LapTime_Delta"]).astype("float32"))
    ordered["nearby_faster_rivals_count"] = (
        pd.concat(faster_nearby, axis=1).sum(axis=1).astype("float32")
    )

    neighbor_cols = [c for c in ordered.columns if c.startswith("nearby_")]
    out.loc[ordered.index, neighbor_cols] = ordered[neighbor_cols]

    out["undercut_available"] = (
        (out["Position"] > 1).astype("float32")
        * (out["TyreLife"] >= field_group["TyreLife"].transform("median")).astype(
            "float32"
        )
        * (out["compound_delta_z"] > 0).astype("float32")
        * (out["nearby_faster_rivals_count"] <= 2).astype("float32")
    ).astype("float32")
    ...
    out["overcut_viable"] = (
        ...
        * (out["position_pct"] < 0.65).astype("float32")
    ).astype("float32")

    out["traffic_penalty_if_pit_now"] = (
        out["position_pct"]
        + 0.20 * out["nearby_similar_tyre_age_count"].fillna(0)
        + 0.15 * out["nearby_faster_rivals_count"].fillna(0)
        + 0.10 * out["lap_delta_rank_pct"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000866"],
        "research_usage_note": "Verified assigned hypothesis 000866.",
    }, sort_keys=True))
