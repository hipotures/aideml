...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_preseason_testing"] = (
        out["Race"].astype(str) == "Pre-Season Testing"
    ).astype("int8")

    out["_row_order"] = range(len(out))
    out = out.sort_values(keys + ["Stint", "LapNumber", "_row_order"], kind="mergesort")

    g = out.groupby(keys, sort=False)
    out["is_stint_start"] = (
        g["Stint"].transform(lambda s: s.ne(s.shift())).astype("int8")
    )
    out.loc[g.cumcount().eq(0), "is_stint_start"] = 1

    stint_start_life = out["TyreLife"].where(out["is_stint_start"].eq(1))
    out["StintStartTyreLife"] = (
        stint_start_life.groupby([out[k] for k in keys]).ffill().fillna(out["TyreLife"])
    )
    out["used_set_start"] = (
        (out["is_stint_start"].eq(1)) & (out["StintStartTyreLife"] > 1)
    ).astype("int8")
    out["new_set_start"] = (
        (out["is_stint_start"].eq(1)) & (out["StintStartTyreLife"] <= 1)
    ).astype("int8")
    ...
    for c in compounds:
        start_c = (
            (out["Compound"].astype(str) == c) & out["is_stint_start"].eq(1)
        ).astype("int16")
        prior_c = start_c.groupby([out[k] for k in keys]).cumsum() - start_c
        out[f"prior_{c.lower()}_stints"] = prior_c.astype("int16")

        fresh_c = (
            (out["Compound"].astype(str) == c) & out["new_set_start"].eq(1)
        ).astype("int16")
        used_c = (
            (out["Compound"].astype(str) == c) & out["used_set_start"].eq(1)
        ).astype("int16")
        out[f"fresh_{c.lower()}_starts_so_far"] = (
            fresh_c.groupby([out[k] for k in keys]).cumsum().astype("int16")
        )
        out[f"used_{c.lower()}_starts_so_far"] = (
            used_c.groupby([out[k] for k in keys]).cumsum().astype("int16")
        )

    comp_start = out["Compound"].where(out["is_stint_start"].eq(1))
    prev_comp_start = (
        comp_start.groupby([out[k] for k in keys])
        .ffill()
        .groupby([out[k] for k in keys])
        .shift()
    )
    out["same_compound_as_prev_stint"] = (
        (comp_start.fillna(out["Compound"]).eq(prev_comp_start))
        .fillna(False)
        .astype("int8")
    )

    stint_comp_key = out["Compound"].astype(str) + "_" + out["Stint"].astype(str)
    first_lap_in_comp_stint = (
        out.groupby(keys + ["Compound", "Stint"], sort=False)
        .cumcount()
        .eq(0)
        .astype("int16")
    )
    repeated = (
        first_lap_in_comp_stint.groupby(
            [out[k] for k in keys] + [out["Compound"]]
        ).cumsum()
        - first_lap_in_comp_stint
    )
    out["repeated_compound_stint_count"] = repeated.astype("int16")

    fresh_slick_start = out["new_set_start"].eq(1) & out["Compound"].isin(slicks)
    used_slick_start = out["used_set_start"].eq(1) & out["Compound"].isin(slicks)
    out["likely_fresh_slicks_consumed"] = (
        fresh_slick_start.astype("int16")
        .groupby([out[k] for k in keys])
        .cumsum()
        .astype("int16")
    )
    out["likely_used_slick_reuses"] = (
        used_slick_start.astype("int16")
        .groupby([out[k] for k in keys])
        .cumsum()
        .astype("int16")
    )

    out["fresh_slicks_x_stint"] = out["likely_fresh_slicks_consumed"] * out["Stint"]
    out["fresh_slicks_x_race_progress"] = (
        out["likely_fresh_slicks_consumed"] * out["RaceProgress"]
    )
    out["used_starts_x_race_progress"] = out["used_set_start"] * out["RaceProgress"]
    out["repeat_compound_x_progress"] = (
        out["repeated_compound_stint_count"] * out["RaceProgress"]
    )
    out["stint_start_life_x_stint"] = out["StintStartTyreLife"] * out["Stint"]

    out = out.sort_values("_row_order", kind="mergesort").drop(columns=["_row_order"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000494"],
        "research_usage_note": "Verified assigned hypothesis 000494.",
    }, sort_keys=True))
