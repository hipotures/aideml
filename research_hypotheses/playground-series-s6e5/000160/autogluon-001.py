...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out[cols].copy()
    work["__row"] = np.arange(n)
    work = work.sort_values(
        ["Driver", "Race", "Year", "LapNumber", "__row"], kind="mergesort"
    )
    ...
    def rz(a):
        ...
        if not np.isfinite(scale) or scale < 1e-6:
            scale = 1.0
        return np.clip((np.nan_to_num(a, nan=med) - med) / scale, -8.0, 8.0)

    lap = work["LapNumber"].to_numpy(dtype=np.float64)
    tyre = work["TyreLife"].to_numpy(dtype=np.float64)
    pit = np.clip(
        np.nan_to_num(work["PitStop"].to_numpy(dtype=np.float64), nan=0.0), 0.0, 1.0
    )
    lap_time = work["LapTime (s)"].to_numpy(dtype=np.float64)
    lap_delta = work["LapTime_Delta"].to_numpy(dtype=np.float64)
    cum_deg = work["Cumulative_Degradation"].to_numpy(dtype=np.float64)
    tyre_safe = np.where(np.isfinite(tyre) & (tyre > 0), tyre, 1.0)

    obs = (
        0.40 * rz(lap_delta)
        + 0.25 * rz(cum_deg / tyre_safe)
        + 0.20 * rz(cum_deg)
        + 0.15 * rz(lap_time)
    )

    seq = (
        work.groupby(["Driver", "Race", "Year"], sort=False)
        .ngroup()
        .to_numpy(dtype=np.int64)
    )
    row = work["__row"].to_numpy(dtype=np.int64)

    state = np.zeros(n, dtype=np.float32)
    slope = np.zeros(n, dtype=np.float32)
    shock = np.zeros(n, dtype=np.float32)
    norm_shock = np.zeros(n, dtype=np.float32)
    post_var = np.zeros(n, dtype=np.float32)
    stint_age = np.zeros(n, dtype=np.float32)

    x, v, p00, p01, p11, age = 0.0, 0.0, 1.0, 0.0, 0.5, 0.0
    q_level, q_slope, r = 0.08, 0.015, 0.65

    for i in range(n):
        reset = i == 0 or seq[i] != seq[i - 1] or pit[i] >= 0.5
        if reset:
            age = 0.0
            x = 0.0 if pit[i] >= 0.5 else float(obs[i])
            v, p00, p01, p11 = 0.0, 0.8, 0.0, 0.4
            innovation = 0.0
            innov_var = p00 + r
        else:
            dtyre = tyre[i] - tyre[i - 1]
            dlap = lap[i] - lap[i - 1]
            if np.isfinite(dtyre) and 0.0 < dtyre <= 5.0:
                dt = float(dtyre)
            elif np.isfinite(dlap) and 0.0 < dlap <= 5.0:
                dt = float(dlap)
            else:
                dt = 1.0
            ...
            x_pred = x + v * dt
            v_pred = v
            p00_pred = p00 + 2.0 * dt * p01 + dt * dt * p11 + q_level
            p01_pred = p01 + dt * p11
            p11_pred = p11 + q_slope

            innovation = float(obs[i]) - x_pred
            innov_var = max(p00_pred + r, 1e-6)
            k0 = p00_pred / innov_var
            k1 = p01_pred / innov_var

            x = x_pred + k0 * innovation
            v = v_pred + k1 * innovation
            p00 = max((1.0 - k0) * p00_pred, 1e-6)
            p01 = (1.0 - k0) * p01_pred
            p11 = max(p11_pred - k1 * p01_pred, 1e-6)
            age += dt

        state[i] = x
        slope[i] = v
        shock[i] = innovation
        norm_shock[i] = innovation / np.sqrt(max(innov_var, 1e-6))
        post_var[i] = p00
        stint_age[i] = age

    out["latent_deg_state_000160"] = 0.0
    out["latent_deg_slope_000160"] = 0.0
    out["latent_innovation_000160"] = 0.0
    out["latent_norm_innovation_000160"] = 0.0
    out["latent_posterior_var_000160"] = 0.0
    out["latent_stint_age_000160"] = 0.0

    out.iloc[row, out.columns.get_loc("latent_deg_state_000160")] = state
    out.iloc[row, out.columns.get_loc("latent_deg_slope_000160")] = slope
    out.iloc[row, out.columns.get_loc("latent_innovation_000160")] = shock
    out.iloc[row, out.columns.get_loc("latent_norm_innovation_000160")] = norm_shock
    out.iloc[row, out.columns.get_loc("latent_posterior_var_000160")] = post_var
    out.iloc[row, out.columns.get_loc("latent_stint_age_000160")] = stint_age
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000160"],
        "research_usage_note": "Verified assigned hypothesis 000160.",
    }, sort_keys=True))
