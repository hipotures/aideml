...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = out.groupby(gcols, observed=True, sort=False)
    out["lap_field_size"] = grp["Driver"].transform("count").astype("int16")

    for c in num_cols:
        x = out[c].astype("float32")
        mean = grp[c].transform("mean").astype("float32")
        std = grp[c].transform("std").replace(0, np.nan).fillna(1).astype("float32")
        mn = grp[c].transform("min").astype("float32")
        mx = grp[c].transform("max").astype("float32")

        out[c + "_lap_center"] = (x - mean).astype("float32")
        out[c + "_lap_z"] = ((x - mean) / std).astype("float32")
        out[c + "_lap_rank_pct"] = (
            grp[c].rank(method="average", pct=True).astype("float32")
        )
        out[c + "_lap_to_max"] = (mx - x).astype("float32")
        out[c + "_lap_to_min"] = (x - mn).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000580"],
        "research_usage_note": "Verified assigned hypothesis 000580.",
    }, sort_keys=True))
