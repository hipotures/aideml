...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["pos_band_near"] = ((out["Position"].astype(float) - 1) // 5).astype("int16")
    out["pos_tercile"] = np.select(
        [out["Position"] <= 7, out["Position"] <= 14],
        [0, 1],
        default=2,
    ).astype("int8")

    lap_g = out.groupby(keys, sort=False)
    band_g = out.groupby(keys + ["pos_band_near"], sort=False)
    comp_g = out.groupby(keys + ["Compound"], sort=False)
    terc_g = out.groupby(keys + ["pos_tercile"], sort=False)
    race_g = out.groupby(["Year", "Race"], sort=False)
    ...
    for col in [
        ...
        out[f"{col}_lap_rank_pct"] = lap_g[col].rank(pct=True).astype("float32")
        out[f"{col}_near_rank_pct"] = band_g[col].rank(pct=True).astype("float32")

    out["same_compound_lap_count"] = (
        comp_g["TyreLife"].transform("size").astype("float32")
    )
    out["lap_field_count"] = lap_g["TyreLife"].transform("size").astype("float32")
    out["same_compound_density"] = (
        out["same_compound_lap_count"] / out["lap_field_count"].clip(lower=1)
    ).astype("float32")

    out["lap_time_near_median"] = band_g["LapTime (s)"].transform("median")
    out["tyre_life_near_median"] = band_g["TyreLife"].transform("median")
    out["degradation_near_median"] = band_g["Cumulative_Degradation"].transform(
        "median"
    )
    out["lap_time_comp_tercile_median"] = terc_g["LapTime (s)"].transform("median")
    out["tyre_life_compound_median"] = comp_g["TyreLife"].transform("median")

    out["lap_time_vs_near"] = (out["LapTime (s)"] - out["lap_time_near_median"]).astype(
        "float32"
    )
    out["tyre_age_vs_near"] = (out["TyreLife"] - out["tyre_life_near_median"]).astype(
        "float32"
    )
    out["degradation_vs_near"] = (
        out["Cumulative_Degradation"] - out["degradation_near_median"]
    ).astype("float32")
    out["lap_time_vs_pos_tercile"] = (
        out["LapTime (s)"] - out["lap_time_comp_tercile_median"]
    ).astype("float32")
    out["tyre_age_vs_compound"] = (
        out["TyreLife"] - out["tyre_life_compound_median"]
    ).astype("float32")

    slow_near = out["lap_time_vs_near"] > 0
    old_near = out["tyre_age_vs_near"] > 0
    losing_pos = out["Position_Change"] > 0
    out["undercut_pressure_flag"] = (slow_near & old_near & losing_pos).astype("int8")
    out["overcut_window_flag"] = (
        (out["lap_time_vs_near"] < 0) & (out["tyre_age_vs_near"] >= 0)
    ).astype("int8")
    out["compound_pressure_score"] = (
        out["LapTime_Delta_near_rank_pct"] * 0.35
        + out["TyreLife_near_rank_pct"] * 0.35
        + out["Cumulative_Degradation_near_rank_pct"] * 0.20
        + out["same_compound_density"] * 0.10
    ).astype("float32")

    race_deg = race_g["Cumulative_Degradation"].transform("median")
    race_tyre = race_g["TyreLife"].transform("median")
    race_vol = race_g["LapTime_Delta"].transform("std").fillna(0)
    race_stint = race_g["Stint"].transform("max")
    race_soft_share = (
        (out["Compound"].astype(str).eq("SOFT"))
        .groupby([out["Year"], out["Race"]], sort=False)
        .transform("mean")
    )

    out["race_strategy_signature"] = (
        race_deg.rank(pct=True) * 0.30
        + race_tyre.rank(pct=True) * 0.25
        + race_vol.rank(pct=True) * 0.25
        + race_stint.rank(pct=True) * 0.10
        + race_soft_share.rank(pct=True) * 0.10
    ).astype("float32")
    out["race_undercut_regime"] = (
        out["race_strategy_signature"] >= out["race_strategy_signature"].median()
    ).astype("int8")

    out["pressure_x_undercut_regime"] = (
        out["compound_pressure_score"] * out["race_undercut_regime"]
    ).astype("float32")
    out["slow_old_x_undercut_regime"] = (
        out["undercut_pressure_flag"] * out["race_undercut_regime"]
    ).astype("int8")
    out["overcut_x_nonundercut_regime"] = (
        out["overcut_window_flag"] * (1 - out["race_undercut_regime"])
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000527"],
        "research_usage_note": "Verified assigned hypothesis 000527.",
    }, sort_keys=True))
