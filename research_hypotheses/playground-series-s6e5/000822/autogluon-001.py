...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["compound_softness"] = X["Compound"].map(compound_softness).astype("float32")
    X["compound_warmup"] = X["Compound"].map(compound_warmup).astype("float32")
    X["is_wet_compound"] = X["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    X["race_key"] = X["Year"].astype("string") + "|" + X["Race"]
    X["driver_race_key"] = X["race_key"] + "|" + X["Driver"]

    sorted_idx = X.sort_values(
        ["Year", "Race", "Driver", "LapNumber"], kind="mergesort"
    ).index
    S = X.loc[sorted_idx].copy()
    g = S.groupby("driver_race_key", sort=False)

    S["prev_lap_time"] = g["LapTime (s)"].shift(1)
    S["prev_lap_delta"] = g["LapTime_Delta"].shift(1)
    S["prev_deg"] = g["Cumulative_Degradation"].shift(1)
    S["prev_pos"] = g["Position"].shift(1)
    S["prev_tyre_life"] = g["TyreLife"].shift(1)

    S["deg_per_lap"] = S["Cumulative_Degradation"] / S["TyreLife"].clip(lower=1.0)
    S["deg_slope_recent"] = (
        (S["Cumulative_Degradation"] - S["prev_deg"])
        / (S["TyreLife"] - S["prev_tyre_life"]).replace(0, np.nan)
    ).replace([np.inf, -np.inf], np.nan)
    S["laptime_gain"] = (S["prev_lap_time"] - S["LapTime (s)"]).astype("float32")
    S["lapdelta_change"] = (S["LapTime_Delta"] - S["prev_lap_delta"]).astype("float32")
    S["position_gain"] = (S["prev_pos"] - S["Position"]).astype("float32")

    lap_grp = S.groupby(["race_key", "LapNumber"], sort=False)
    race_grp = S.groupby("race_key", sort=False)
    race_name_grp = S.groupby("Race", sort=False)

    S["field_laptime_med"] = lap_grp["LapTime (s)"].transform("median")
    S["field_lapdelta_med"] = lap_grp["LapTime_Delta"].transform("median")
    S["field_deg_med"] = lap_grp["Cumulative_Degradation"].transform("median")
    S["field_tyre_med"] = lap_grp["TyreLife"].transform("median")
    S["field_poschg_med"] = lap_grp["Position_Change"].transform("median")

    S["pace_vs_field"] = S["LapTime (s)"] - S["field_laptime_med"]
    S["lapdelta_vs_field"] = S["LapTime_Delta"] - S["field_lapdelta_med"]
    S["deg_vs_field"] = S["Cumulative_Degradation"] - S["field_deg_med"]
    S["tyrelife_vs_field"] = S["TyreLife"] - S["field_tyre_med"]
    S["poschg_vs_field"] = S["Position_Change"] - S["field_poschg_med"]

    S["race_deg_proxy"] = race_name_grp["deg_per_lap"].transform("median")
    S["race_delta_proxy"] = race_name_grp["LapTime_Delta"].transform("median")
    S["race_softness_proxy"] = race_name_grp["compound_softness"].transform("mean")
    S["race_pace_spread"] = race_grp["pace_vs_field"].transform("std").fillna(0.0)

    S["stint_progress"] = S["TyreLife"] / S.groupby(
        ["driver_race_key", "Stint"], sort=False
    )["TyreLife"].transform("max").clip(lower=1.0)
    S["lap_progress_centered"] = S["RaceProgress"] - 0.5
    S["position_norm"] = (S["Position"] - 10.5) / 9.5

    gate_score = (
        1.8 * S["race_deg_proxy"].clip(-5, 25)
        + 0.9 * S["deg_per_lap"].clip(-5, 25)
        + 0.5 * S["deg_slope_recent"].fillna(0).clip(-10, 10)
        - 0.35 * S["race_delta_proxy"].clip(-30, 30)
        - 0.25 * S["lapdelta_vs_field"].clip(-30, 30)
        - 0.15 * S["pace_vs_field"].clip(-20, 20)
        + 0.45 * S["compound_softness"]
        + 0.35 * S["compound_warmup"]
        + 0.25 * S["stint_progress"].clip(0, 2)
        + 0.10 * S["position_norm"].clip(-2, 2)
    ) / 8.0

    S["undercut_favored"] = (1.0 / (1.0 + np.exp(-gate_score))).astype("float32")
    S["overcut_favored"] = (1.0 - S["undercut_favored"]).astype("float32")
    S["strategy_regime"] = pd.cut(
        S["undercut_favored"],
        bins=[-np.inf, 0.33, 0.67, np.inf],
        labels=["overcut", "balanced", "undercut"],
    ).astype("string")

    S["undercut_tyre_life"] = (S["undercut_favored"] * S["TyreLife"]).astype("float32")
    S["undercut_deg"] = (S["undercut_favored"] * S["Cumulative_Degradation"]).astype(
        "float32"
    )
    S["undercut_deg_per_lap"] = (S["undercut_favored"] * S["deg_per_lap"]).astype(
        "float32"
    )
    S["undercut_lapdelta"] = (S["undercut_favored"] * S["LapTime_Delta"]).astype(
        "float32"
    )
    S["undercut_pace_vs_field"] = (S["undercut_favored"] * S["pace_vs_field"]).astype(
        "float32"
    )
    S["overcut_tyre_life"] = (S["overcut_favored"] * S["TyreLife"]).astype("float32")
    S["overcut_lapdelta"] = (S["overcut_favored"] * S["LapTime_Delta"]).astype(
        "float32"
    )
    S["overcut_pace_vs_field"] = (S["overcut_favored"] * S["pace_vs_field"]).astype(
        "float32"
    )
    S["overcut_position"] = (S["overcut_favored"] * S["Position"]).astype("float32")
    S["regime_compound"] = (S["strategy_regime"] + "_" + S["Compound"]).astype("string")
    S["regime_race"] = (S["strategy_regime"] + "_" + S["Race"]).astype("string")

    S = S.drop(columns=["race_key", "driver_race_key"])
    return S.loc[orig_index]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000822"],
        "research_usage_note": "Verified assigned hypothesis 000822.",
    }, sort_keys=True))
