...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["LapBucket"] = pd.cut(
        lap,
        bins=[0, 5, 10, 15, 20, 30, 40, 50, 60, 80, 200],
        labels=False,
        include_lowest=True,
    ).astype("int16")
    ...
    out["Race_LapBucket"] = out["Race"] + "_LB" + out["LapBucket"].astype("string")
    out["Race_Compound_LapBucket"] = (
        out["Race_Compound"] + "_LB" + out["LapBucket"].astype("string")
    )

    out["TyreLife_x_Progress"] = tyre * progress
    out["TyreLife_per_Stint"] = tyre / (stint + 1.0)
    out["Lap_in_Stint_Ratio"] = tyre / (lap + 1.0)
    out["RemainingRace"] = 1.0 - progress
    out["Progress_x_Stint"] = progress * stint
    out["IsWetCompound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["IsSoftCompound"] = (out["Compound"] == "SOFT").astype("int8")
    out["LateRaceOldTyre"] = ((progress > 0.65) & (tyre >= 15)).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000279.",
    }, sort_keys=True))
