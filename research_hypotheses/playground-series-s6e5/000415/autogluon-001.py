...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_col = "LapTime (s)"
    group_base = ["Year", "Race", "Compound"]

    if lap_col in out.columns:
        med = out.groupby(group_base, observed=True)[lap_col].transform("median")
        fallback = out.groupby(["Year", "Race"], observed=True)[lap_col].transform(
            "median"
        )
        global_med = float(out[lap_col].median())
        baseline = med.fillna(fallback).fillna(global_med)
        out["latent_laptime_resid"] = out[lap_col] - baseline
    else:
        out["latent_laptime_resid"] = 0.0

    sort_cols = [c for c in ["Year", "Race", "Driver", "LapNumber"] if c in out.columns]
    work = out[
        sort_cols
        + ["latent_laptime_resid"]
        + (["PitStop"] if "PitStop" in out.columns else [])
    ].copy()
    work["_idx"] = np.arange(len(out))
    work = (
        work.sort_values(sort_cols + ["_idx"], kind="mergesort") if sort_cols else work
    )

    n = len(work)
    wear = np.zeros(n, dtype=np.float32)
    accel = np.zeros(n, dtype=np.float32)
    pred_var = np.zeros(n, dtype=np.float32)
    reset_resid = np.zeros(n, dtype=np.float32)

    q_level, q_accel, r_obs = 0.035, 0.012, 1.0
    prev_key = None
    level, slope, p_level, p_slope, last_level = 0.0, 0.0, 1.0, 0.25, 0.0

    key_cols = [c for c in ["Year", "Race", "Driver"] if c in work.columns]
    pit_values = (
        work["PitStop"].to_numpy() if "PitStop" in work.columns else np.zeros(n)
    )
    resid_values = work["latent_laptime_resid"].to_numpy(dtype=float)
    key_values = (
        work[key_cols].astype(str).agg("|".join, axis=1).to_numpy()
        if key_cols
        else np.zeros(n)
    )
    ...
    for i in range(n):
        key = key_values[i]
        if prev_key is None or key != prev_key or pit_values[i] == 1:
            level, slope, p_level, p_slope, last_level = 0.0, 0.0, 1.0, 0.25, 0.0

        pred_level = level + slope
        pred_slope = slope
        p_level = p_level + p_slope + q_level
        p_slope = p_slope + q_accel
        innovation = resid_values[i] - pred_level
        s_var = p_level + r_obs
        k = p_level / (s_var + eps)

        level = pred_level + k * innovation
        slope = pred_slope + 0.15 * k * innovation
        p_level = (1.0 - k) * p_level

        wear[i] = level
        accel[i] = level - last_level
        pred_var[i] = s_var
        reset_resid[i] = innovation
        last_level = level
        prev_key = key

    inv = np.empty(n, dtype=np.int64)
    inv[work["_idx"].to_numpy()] = np.arange(n)

    out["latent_tyre_wear_posterior"] = wear[inv]
    out["latent_tyre_wear_accel"] = accel[inv]
    out["latent_tyre_pred_var"] = pred_var[inv]
    out["latent_reset_adjusted_resid"] = reset_resid[inv]

    if "TyreLife" in out.columns:
        out["latent_wear_per_tyrelife"] = out["latent_tyre_wear_posterior"] / (
            out["TyreLife"].astype(float) + 1.0
        )
    if "RaceProgress" in out.columns:
        out["latent_wear_progress_interaction"] = out[
            "latent_tyre_wear_posterior"
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000415"],
        "research_usage_note": "Verified assigned hypothesis 000415.",
    }, sort_keys=True))
