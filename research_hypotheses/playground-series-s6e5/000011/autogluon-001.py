...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = out["RaceProgress"].astype(float).clip(lower=0.01, upper=1.0)
    lap_number = out["LapNumber"].astype(float).clip(lower=1.0)
    tyre_life = out["TyreLife"].astype(float).clip(lower=0.0)

    est_total_laps = (lap_number / progress).replace([np.inf, -np.inf], np.nan)
    est_total_laps = est_total_laps.clip(lower=45.0, upper=90.0).fillna(lap_number)
    laps_remaining = (est_total_laps - lap_number).clip(lower=0.0)
    ...
    expected_life = compound.map(compound_life_map).fillna(24.0).astype(float)

    tyre_age_to_finish = tyre_life + laps_remaining
    finish_margin = expected_life - tyre_age_to_finish
    finish_pressure = tyre_age_to_finish / expected_life.clip(lower=1.0)

    out["Estimated_Total_Laps"] = est_total_laps
    out["Laps_Remaining"] = laps_remaining
    out["CurrentTyre_Age_To_Finish"] = tyre_age_to_finish
    out["CurrentTyre_Finish_Margin"] = finish_margin
    out["CurrentTyre_Finish_Pressure"] = finish_pressure.clip(lower=0.0, upper=5.0)
    out["CurrentTyre_Cannot_Finish"] = (tyre_age_to_finish > expected_life).astype(int)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000011.",
    }, sort_keys=True))
