...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in numeric_cols:
        X[f"{col}_grp_mean"] = g[col].transform("mean")
        X[f"{col}_grp_std"] = g[col].transform("std").fillna(0.0)
        X[f"{col}_vs_grp_mean"] = X[col] - X[f"{col}_grp_mean"]
    ...
    for col, ascending in rank_specs.items():
        ...
        rank_features.append(rcol)

    X["group_size"] = g["Driver"].transform("size").astype("float32")
    X["dry_flag"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["same_compound_count"] = (
        X.groupby(grp + ["Compound"], sort=False)["Driver"]
        .transform("size")
        .astype("float32")
    )

    X["query_rank_proxy"] = (
        0.30 * X["TyreLife_grp_pct"]
        + 0.30 * X["Cumulative_Degradation_grp_pct"]
        + 0.20 * X["LapTime_Delta_grp_pct"]
        + 0.10 * X["LapTime (s)_grp_pct"]
        + 0.05 * (1.0 - X["Position_Change_grp_pct"])
        + 0.05 * (1.0 - X["Position_grp_pct"])
    ).astype("float32")

    X["query_rank_proxy_dry"] = (X["query_rank_proxy"] * X["dry_flag"]).astype(
        "float32"
    )
    X["query_rank_proxy_same_compound"] = (
        X["query_rank_proxy"]
        * (X["same_compound_count"] / X["group_size"].replace(0, 1))
    ).astype("float32")

    X["wear_pressure"] = (
        X["TyreLife_grp_pct"] * X["Cumulative_Degradation_grp_pct"] * X["dry_flag"]
    ).astype("float32")

    X["loss_pressure"] = (
        X["LapTime_Delta_grp_pct"] * (1.0 - X["Position_Change_grp_pct"])
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000314.",
    }, sort_keys=True))
