...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_cols = ["Year", "Race"]
    compound_cols = ["Year", "Race", "Compound"]

    for c in ["Compound", "Driver", "Race"]:
        X[c] = X[c].astype("category")

    g = X.groupby(group_cols, sort=False)
    grp_size = g["LapNumber"].transform("size").astype("float32")
    grp_min_lap = g["LapNumber"].transform("min").astype("float32")
    grp_max_lap = g["LapNumber"].transform("max").astype("float32")

    X["stint_lap_idx"] = (X["LapNumber"].astype("float32") - grp_min_lap + 1).astype(
        "float32"
    )
    X["stint_laps_remaining"] = (grp_max_lap - X["LapNumber"].astype("float32")).astype(
        "float32"
    )
    X["stint_progress_pct"] = (X["stint_lap_idx"] / grp_size.replace(0, 1)).astype(
        "float32"
    )
    X["stint_progress_rev_pct"] = (
        X["stint_laps_remaining"] / grp_size.replace(0, 1)
    ).astype("float32")
    X["lap_in_stint_rank_pct"] = (
        (X["stint_lap_idx"] - 1) / grp_size.clip(lower=1)
    ).astype("float32")

    tyre_life = X["TyreLife"].astype("float32")
    cum_deg = X["Cumulative_Degradation"].astype("float32")
    lap_time = X["LapTime (s)"].astype("float32")
    lap_delta = X["LapTime_Delta"].astype("float32")
    race_progress = X["RaceProgress"].astype("float32")
    pos = X["Position"].astype("float32")
    pos_change = X["Position_Change"].astype("float32")

    X["tyrelife_vs_stint_progress"] = (tyre_life - X["stint_lap_idx"]).astype("float32")
    X["deg_per_tyre_lap"] = (cum_deg / tyre_life.clip(lower=1)).astype("float32")
    X["deg_per_stint_lap"] = (cum_deg / X["stint_lap_idx"].clip(lower=1)).astype(
        "float32"
    )
    X["lap_delta_per_tyre_lap"] = (lap_delta / tyre_life.clip(lower=1)).astype(
        "float32"
    )
    X["lap_delta_x_progress"] = (lap_delta * X["stint_progress_pct"]).astype("float32")
    X["deg_x_progress"] = (cum_deg * X["stint_progress_pct"]).astype("float32")
    X["tyrelife_x_raceprogress"] = (tyre_life * race_progress).astype("float32")
    X["position_pressure"] = (pos * X["stint_progress_rev_pct"]).astype("float32")
    X["position_change_pressure"] = (pos_change * X["stint_progress_pct"]).astype(
        "float32"
    )

    race_mean_laptime = (
        X.groupby(race_cols, sort=False)["LapTime (s)"]
        .transform("mean")
        .astype("float32")
    )
    race_mean_delta = (
        X.groupby(race_cols, sort=False)["LapTime_Delta"]
        .transform("mean")
        .astype("float32")
    )
    race_mean_deg = (
        X.groupby(race_cols, sort=False)["Cumulative_Degradation"]
        .transform("mean")
        .astype("float32")
    )
    race_mean_tyre = (
        X.groupby(race_cols, sort=False)["TyreLife"].transform("mean").astype("float32")
    )

    comp_mean_laptime = (
        X.groupby(compound_cols, sort=False)["LapTime (s)"]
        .transform("mean")
        .astype("float32")
    )
    comp_mean_delta = (
        X.groupby(compound_cols, sort=False)["LapTime_Delta"]
        .transform("mean")
        .astype("float32")
    )
    comp_mean_deg = (
        X.groupby(compound_cols, sort=False)["Cumulative_Degradation"]
        .transform("mean")
        .astype("float32")
    )
    comp_mean_tyre = (
        X.groupby(compound_cols, sort=False)["TyreLife"]
        .transform("mean")
        .astype("float32")
    )

    X["laptime_vs_race_mean"] = (lap_time - race_mean_laptime).astype("float32")
    X["lapdelta_vs_race_mean"] = (lap_delta - race_mean_delta).astype("float32")
    X["deg_vs_race_mean"] = (cum_deg - race_mean_deg).astype("float32")
    X["tyrelife_vs_race_mean"] = (tyre_life - race_mean_tyre).astype("float32")

    X["laptime_vs_compound_mean"] = (lap_time - comp_mean_laptime).astype("float32")
    X["lapdelta_vs_compound_mean"] = (lap_delta - comp_mean_delta).astype("float32")
    X["deg_vs_compound_mean"] = (cum_deg - comp_mean_deg).astype("float32")
    X["tyrelife_vs_compound_mean"] = (tyre_life - comp_mean_tyre).astype("float32")

    X["laptime_compound_ratio"] = (
        lap_time / comp_mean_laptime.replace(0, np.nan)
    ).astype("float32")
    X["deg_compound_ratio"] = (cum_deg / comp_mean_deg.replace(0, np.nan)).astype(
        "float32"
    )

    prev_lap_delta = g["LapTime_Delta"].shift(1).astype("float32")
    prev2_lap_delta = g["LapTime_Delta"].shift(2).astype("float32")
    prev_deg = g["Cumulative_Degradation"].shift(1).astype("float32")
    prev2_deg = g["Cumulative_Degradation"].shift(2).astype("float32")
    prev_pos = g["Position"].shift(1).astype("float32")

    X["lapdelta_change_1"] = (lap_delta - prev_lap_delta).fillna(0).astype("float32")
    X["lapdelta_change_2"] = (lap_delta - prev2_lap_delta).fillna(0).astype("float32")
    X["deg_change_1"] = (cum_deg - prev_deg).fillna(0).astype("float32")
    X["deg_change_2"] = (cum_deg - prev2_deg).fillna(0).astype("float32")
    X["position_change_1"] = (pos - prev_pos).fillna(0).astype("float32")

    X["rank_hazard_score"] = (
        0.35 * X["stint_progress_pct"]
        + 0.25 * X["deg_per_tyre_lap"].replace([np.inf, -np.inf], np.nan).fillna(0)
        + 0.20 * X["lapdelta_vs_compound_mean"].fillna(0)
        + 0.20 * X["lapdelta_change_1"].fillna(0)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000952"],
        "research_usage_note": "Verified assigned hypothesis 000952.",
    }, sort_keys=True))
