...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry = comp.isin(dry_compounds).astype("int8")
    is_wet = comp.isin(wet_compounds).astype("int8")

    keys = ["Year", "Race", "Driver"]
    order_cols = keys + ["LapNumber", "Stint"]
    ordered = out[order_cols].copy()
    ordered["_orig_idx"] = np.arange(len(out))
    ordered["_compound"] = comp.values
    ordered["_is_dry"] = is_dry.values
    ordered["_is_wet"] = is_wet.values
    ordered = ordered.sort_values(order_cols + ["_orig_idx"], kind="mergesort")

    grp = ordered.groupby(keys, sort=False)
    ordered["_wet_seen"] = grp["_is_wet"].cummax()
    ordered["_dry_key"] = np.where(ordered["_is_dry"].eq(1), ordered["_compound"], "")
    dry_seen = (
        ordered.assign(_dry_seen_flag=ordered["_dry_key"].ne("").astype("int8"))
        .groupby(keys + ["_dry_key"], sort=False)["_dry_seen_flag"]
        .cummax()
    )
    ordered["_distinct_dry_seen"] = dry_seen.where(ordered["_dry_key"].ne(""), 0)
    ordered["_dry_count_seen"] = ordered.groupby(keys, sort=False)[
        "_distinct_dry_seen"
    ].cumsum()
    ordered["_dry_count_seen"] = ordered["_dry_count_seen"].clip(upper=3)

    back = ordered.sort_values("_orig_idx", kind="mergesort")
    dry_count_seen = back["_dry_count_seen"].to_numpy(dtype=np.float32)
    wet_seen = back["_wet_seen"].to_numpy(dtype=np.int8)
    ...
    missing_dry = np.maximum(0, 2 - dry_count_seen)
    dry_rule_context = ((is_dry.to_numpy() == 1) & (wet_seen == 0)).astype("int8")
    current_can_finish_legal = ((dry_count_seen >= 2) & (dry_rule_context == 1)).astype(
        "int8"
    )
    pressure = (missing_dry * dry_rule_context) / (
        laps_remaining.to_numpy(dtype=np.float32) + 1.0
    )

    out["dry_rule_context_000275"] = dry_rule_context
    out["dry_compounds_seen_000275"] = dry_count_seen
    out["missing_mandatory_dry_000275"] = missing_dry.astype("float32")
    out["current_compound_can_finish_legal_000275"] = current_can_finish_legal
    out["dry_legality_pressure_000275"] = pressure.astype("float32")
    out["dry_pressure_x_laps_remaining_000275"] = (
        pressure * laps_remaining.to_numpy(dtype=np.float32)
    ).astype("float32")
    out["dry_pressure_x_stint_000275"] = (
        pressure * out["Stint"].astype(float).to_numpy(dtype=np.float32)
    ).astype("float32")
    out["missing_dry_x_laps_remaining_000275"] = (
        missing_dry * laps_remaining.to_numpy(dtype=np.float32)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000275"],
        "research_usage_note": "Verified assigned hypothesis 000275.",
    }, sort_keys=True))
