...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lower = out["Race"].str.lower().fillna("")

    green_pit_loss = pd.Series(21.5, index=out.index, dtype="float64")
    vsc_mult = pd.Series(0.65, index=out.index, dtype="float64")
    sc_mult = pd.Series(0.40, index=out.index, dtype="float64")
    caution_risk = pd.Series(0.18, index=out.index, dtype="float64")

    race_priors = [
        (r"monaco", 19.0, 0.70, 0.45, 0.36),
        (r"singapore", 28.0, 0.62, 0.38, 0.34),
        (r"baku|azerbaijan", 20.0, 0.62, 0.38, 0.32),
        (r"saudi|jeddah", 22.0, 0.62, 0.38, 0.30),
        (r"las vegas", 20.0, 0.64, 0.40, 0.28),
        (r"canadian|montreal", 21.0, 0.62, 0.38, 0.29),
        (r"australian|melbourne", 22.0, 0.64, 0.40, 0.24),
        (r"miami", 21.0, 0.64, 0.40, 0.23),
        (r"sao paulo|brazil", 22.0, 0.63, 0.39, 0.24),
        (r"mexico", 22.0, 0.64, 0.40, 0.22),
        (r"silverstone|british", 24.0, 0.65, 0.40, 0.19),
        (r"spa|belgian", 22.0, 0.60, 0.35, 0.20),
        (r"suzuka|japanese", 23.0, 0.65, 0.40, 0.17),
        (r"monza|italian", 19.0, 0.65, 0.40, 0.15),
        (r"hungarian", 21.0, 0.65, 0.40, 0.17),
        (r"zandvoort|dutch", 21.0, 0.64, 0.40, 0.17),
        (r"barcelona|spanish", 22.0, 0.65, 0.40, 0.15),
        (r"red bull ring|austrian", 20.0, 0.64, 0.40, 0.17),
        (r"imola|emilia", 21.0, 0.64, 0.40, 0.18),
        (r"bahrain", 23.0, 0.66, 0.42, 0.14),
        (r"qatar", 22.0, 0.65, 0.40, 0.16),
        (r"abu dhabi", 21.0, 0.65, 0.40, 0.14),
        (r"pre-season testing", 25.0, 0.75, 0.55, 0.05),
    ]

    for pattern, gp, vm, sm, cr in race_priors:
        mask = race_lower.str.contains(pattern, regex=True)
        green_pit_loss.loc[mask] = gp
        vsc_mult.loc[mask] = vm
        sc_mult.loc[mask] = sm
        caution_risk.loc[mask] = cr
    ...
    race_progress = out["RaceProgress"].clip(0.01, 1.0)
    est_total_laps = (out["LapNumber"] / race_progress).clip(45.0, 90.0)
    laps_remaining = (est_total_laps - out["LapNumber"]).clip(0.0, 80.0)

    avg_discount_mult = 0.55 * vsc_mult + 0.45 * sc_mult
    expected_pit_loss = green_pit_loss * (
        (1.0 - caution_risk) + caution_risk * avg_discount_mult
    )
    cheap_stop_value = green_pit_loss - expected_pit_loss
    ...
    deg_per_lap = deg_per_lap.clip(-8.0, 25.0)
    lap_delta_abs = out["LapTime_Delta"].abs().clip(0.0, 30.0)

    service_window_pressure = (
        (out["TyreLife"] * compound_wear) / (laps_remaining + 1.0)
    ).clip(0.0, 5.0)
    finishability_gap = ((out["TyreLife"] * compound_wear) - laps_remaining).clip(
        -80.0, 80.0
    )

    track_position_weight = ((21.0 - out["Position"]) / 20.0).clip(0.05, 1.0)
    traffic_weight = (1.0 - ((out["Position"] - 10.5).abs() / 10.5)).clip(0.0, 1.0)
    late_race_weight = race_progress
    borderline_window = 1.0 / (1.0 + (service_window_pressure - 1.0).abs())

    out["race_laps_estimate"] = est_total_laps
    out["laps_remaining_estimate"] = laps_remaining
    out["green_pit_loss_prior"] = green_pit_loss
    out["caution_risk_prior"] = caution_risk
    out["vsc_discount_prior"] = vsc_mult
    out["sc_discount_prior"] = sc_mult
    out["expected_pit_loss"] = expected_pit_loss
    out["cheap_stop_value"] = cheap_stop_value
    out["service_window_pressure"] = service_window_pressure
    out["finishability_gap"] = finishability_gap
    out["deg_per_lap"] = deg_per_lap
    out["compound_wear_factor"] = compound_wear

    out["caution_adjusted_undercut_value"] = (deg_per_lap + 0.35 * lap_delta_abs) * (
        1.0 + traffic_weight
    ) * track_position_weight - expected_pit_loss / 12.0

    out["wait_for_cheap_stop_pressure"] = (
        cheap_stop_value
        * caution_risk
        * late_race_weight
        * borderline_window
        * (1.0 + 0.5 * traffic_weight)
    )

    out["pit_loss_vs_degradation_ratio"] = expected_pit_loss / (
        1.0 + deg_per_lap.abs() + 0.25 * lap_delta_abs
    )

    out["stretch_value_index"] = (
        cheap_stop_value
        * caution_risk
        * (1.0 + np.maximum(finishability_gap, 0.0) / 10.0)
        / (1.0 + service_window_pressure)
    )

    out["must_box_vs_wait_index"] = (
        service_window_pressure * (1.0 + np.maximum(finishability_gap, 0.0) / 5.0)
        - cheap_stop_value * caution_risk * borderline_window
    )

    out["late_stint_caution_leverage"] = (
        late_race_weight * caution_risk * cheap_stop_value * (1.0 + out["Stint"] / 4.0)
    )

    out["position_caution_pressure"] = (
        caution_risk
        * cheap_stop_value
        * traffic_weight
        * (1.0 + out["Position_Change"].abs() / 5.0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000772"],
        "research_usage_note": "Verified assigned hypothesis 000772.",
    }, sort_keys=True))
