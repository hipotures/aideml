...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["rl_laptime_median"] = grp["LapTime (s)"].transform("median")
    out["rl_delta_median"] = grp["LapTime_Delta"].transform("median")
    out["rl_delta_q25"] = grp["LapTime_Delta"].transform(lambda s: s.quantile(0.25))
    out["rl_delta_q75"] = grp["LapTime_Delta"].transform(lambda s: s.quantile(0.75))
    out["rl_delta_iqr"] = out["rl_delta_q75"] - out["rl_delta_q25"]

    delta_thr = out["LapTime_Delta"].quantile(0.95)
    out["rl_big_delta_share"] = (
        (out["LapTime_Delta"] > delta_thr)
        .groupby([out["Year"], out["Race"], out["LapNumber"]], sort=False)
        .transform("mean")
    )
    out["rl_pit_share"] = grp["PitStop"].transform("mean")
    out["rl_field_size"] = grp["Driver"].transform("count")
    ...
    rgrp = lap_aggs.groupby(["Year", "Race"], sort=False)
    for c in [
        "rl_laptime_median",
        "rl_delta_median",
        "rl_delta_iqr",
        ...
        lap_aggs[c + "_chg"] = rgrp[c].diff().fillna(0.0)
        lap_aggs[c + "_lag1"] = rgrp[c].shift(1).fillna(0.0)

    shock_score = (
        lap_aggs["rl_delta_median"].rank(pct=True)
        + lap_aggs["rl_delta_iqr"].rank(pct=True)
        + lap_aggs["rl_big_delta_share"].rank(pct=True)
        + lap_aggs["rl_pit_share"].rank(pct=True)
    )
    lap_aggs["rl_caution_shock"] = (shock_score >= shock_score.quantile(0.90)).astype(
        "int8"
    )
    lap_aggs["rl_restart_flag"] = (
        (rgrp["rl_caution_shock"].shift(1).fillna(0) == 1)
        & (lap_aggs["rl_delta_median_chg"] < 0)
    ).astype("int8")
    lap_aggs["rl_caution_lag1"] = (
        rgrp["rl_caution_shock"].shift(1).fillna(0).astype("int8")
    )
    lap_aggs["rl_restart_lag1"] = (
        rgrp["rl_restart_flag"].shift(1).fillna(0).astype("int8")
    )
    ...
    out = out.merge(
        lap_aggs[
            keys
            + [
                "rl_laptime_median_chg",
                "rl_delta_median_chg",
                "rl_delta_iqr_chg",
                "rl_big_delta_share_chg",
                "rl_pit_share_chg",
                "rl_laptime_median_lag1",
                "rl_delta_median_lag1",
                "rl_delta_iqr_lag1",
                "rl_big_delta_share_lag1",
                "rl_pit_share_lag1",
                "rl_caution_shock",
                "rl_restart_flag",
                "rl_caution_lag1",
    ...
    out["tyre_pressure"] = out["TyreLife"] / (out["laps_remaining_est"] + 1.0)
    out["degradation_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1.0
    )
    out["field_pace_gap"] = out["LapTime (s)"] - out["rl_laptime_median"]
    out["field_delta_gap"] = out["LapTime_Delta"] - out["rl_delta_median"]

    out["caution_tyre_pressure"] = out["rl_caution_lag1"] * out["tyre_pressure"]
    out["restart_tyre_pressure"] = out["rl_restart_lag1"] * out["tyre_pressure"]
    out["caution_laps_remaining"] = out["rl_caution_lag1"] * out["laps_remaining_est"]
    out["restart_laps_remaining"] = out["rl_restart_lag1"] * out["laps_remaining_est"]
    ...
    for name in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        flag = (out["Compound"] == name).astype("int8")
        out[f"caution_compound_{name.lower()}"] = out["rl_caution_lag1"] * flag
        out[f"restart_compound_{name.lower()}"] = out["rl_restart_lag1"] * flag
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000457"],
        "research_usage_note": "Verified assigned hypothesis 000457.",
    }, sort_keys=True))
