...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out.reset_index(drop=False).rename(columns={"index": "_orig_index"})
    work["_row_order"] = np.arange(len(work))

    if sort_cols:
        work = work.sort_values(sort_cols + ["_row_order"], kind="mergesort")

    group_cols = [c for c in ["Year", "Race", "Driver", "Stint"] if c in work.columns]
    if not group_cols:
        group_cols = ["Driver"] if "Driver" in work.columns else ["Race"]

    g = work.groupby(group_cols, sort=False, observed=True)

    base_cols = ["LapTime_Delta", "LapTime (s)", "Cumulative_Degradation"]
    for col in base_cols:
        ...
        slope1 = lag1 - lag2
        slope2 = (lag1 - lag3) / 2.0
        slope3 = (lag1 - lag4) / 3.0
        recent_slope = pd.concat([slope1, slope2, slope3], axis=1).mean(axis=1)

        work[f"{safe_name}_slope_1"] = slope1.fillna(0)
        work[f"{safe_name}_slope_recent"] = recent_slope.fillna(0)
        work[f"{safe_name}_accel_recent"] = (slope1 - slope2).fillna(0)
        work[f"{safe_name}_curvature"] = (lag1 - 2.0 * lag2 + lag3).fillna(0)
        work[f"{safe_name}_slope_pos_turn"] = ((slope1 > 0) & (slope2 <= 0)).astype(
            "int8"
        )
        work[f"{safe_name}_slope_flat_to_pos"] = (
            (slope1 > 0) & (slope2.abs() < 0.05)
        ).astype("int8")

        clipped = recent_slope.clip(
            recent_slope.quantile(0.01),
            recent_slope.quantile(0.99),
        )
        work[f"{safe_name}_slope_recent_clip"] = clipped.fillna(0)

    compound = (
        work["Compound"].astype(str).str.upper()
        if "Compound" in work.columns
        else pd.Series("", index=work.index)
    )
    work["is_wet_compound"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    work["is_soft_compound"] = (compound == "SOFT").astype("int8")
    work["is_hard_compound"] = (compound == "HARD").astype("int8")
    ...
    progress = (
        work["RaceProgress"]
        if "RaceProgress" in work.columns
        else pd.Series(0, index=work.index)
    )

    work["wear_phase_warmup"] = (tyre <= 3).astype("int8")
    work["wear_phase_mid"] = ((tyre > 3) & (tyre <= 18)).astype("int8")
    work["wear_phase_late"] = (tyre > 18).astype("int8")
    work["tyrelife_sq"] = tyre.astype(float) ** 2
    work["tyrelife_x_progress"] = tyre.astype(float) * progress.astype(float)
    work["wet_x_progress"] = work["is_wet_compound"] * progress.astype(float)

    for prefix in ["LapTime_Delta", "LapTime_s", "Cumulative_Degradation"]:
        slope_col = f"{prefix}_slope_recent"
        accel_col = f"{prefix}_accel_recent"
        if slope_col in work.columns:
            work[f"{prefix}_slope_x_progress"] = work[slope_col] * progress.astype(
                float
            )
            work[f"{prefix}_slope_x_late"] = work[slope_col] * work["wear_phase_late"]
            work[f"{prefix}_slope_x_wet"] = work[slope_col] * work["is_wet_compound"]
            work[f"{prefix}_slope_x_soft"] = work[slope_col] * work["is_soft_compound"]
        if accel_col in work.columns:
            work[f"{prefix}_accel_x_late"] = work[accel_col] * work["wear_phase_late"]

    work = work.sort_values("_row_order", kind="mergesort")
    work = work.drop(columns=["_orig_index", "_row_order"], errors="ignore")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000243"],
        "research_usage_note": "Verified assigned hypothesis 000243.",
    }, sort_keys=True))
