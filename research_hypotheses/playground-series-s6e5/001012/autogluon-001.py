...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_size = g["Driver"].transform("size").astype("float32")

    # Basic normalized race-state features
    X["LapFrac"] = (
        X["LapNumber"]
        / (
            X["LapNumber"]
            .groupby([X["Year"], X["Race"]])
            .transform("max")
            .replace(0, 1)
        )
    ).astype("float32")
    X["TyreLifeFrac"] = (
        (X["TyreLife"] / X["LapNumber"].replace(0, 1))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    X["StintPressure"] = (X["TyreLife"] * (1 + X["Stint"])).astype("float32")
    X["DegPerTyreLap"] = (
        (X["Cumulative_Degradation"] / X["TyreLife"].replace(0, 1))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    X["DegPerRaceLap"] = (
        (X["Cumulative_Degradation"] / X["LapNumber"].replace(0, 1))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    X["DeltaPerTyreLap"] = (
        (X["LapTime_Delta"] / X["TyreLife"].replace(0, 1))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )

    # Same-lap relative ranks: ranker-style specialist signals for ordering drivers within a lap
    rank_specs = {
        "Position": True,
        "TyreLife": False,
        "Cumulative_Degradation": False,
        "LapTime_Delta": False,
        "LapTime (s)": False,
    ...
    for col, ascending in rank_specs.items():
        r = g[col].rank(method="average", pct=True, ascending=ascending)
        X[f"{col}_lap_rank"] = r.astype("float32")
        X[f"{col}_lap_centered"] = (r - 0.5).astype("float32")

    # Same-lap standardized pressure relative to opponents
    z_cols = [
        "TyreLife",
    ...
    for col in z_cols:
        ...
        std = g[col].transform("std").fillna(0)
        X[f"{col}_lap_z"] = (
            ((X[col] - mean) / std.replace(0, 1))
            .replace([np.inf, -np.inf], 0)
            .fillna(0)
            .astype("float32")
        )

    # Relative gaps to contemporaneous field
    for col in [
        "TyreLife",
        "Cumulative_Degradation",
        "LapTime_Delta",
        ...
        X[f"{col}_lap_mean_gap"] = (X[col] - g[col].transform("mean")).astype("float32")
        X[f"{col}_lap_max_gap"] = (g[col].transform("max") - X[col]).astype("float32")
        X[f"{col}_lap_min_gap"] = (X[col] - g[col].transform("min")).astype("float32")

    # Compound prevalence within lap: opponent-aware tyre context
    comp_counts = g["Compound"].transform("count").astype("float32")
    comp_same = (
        X.groupby(group_cols + ["Compound"], sort=False)["Compound"]
        .transform("size")
        .astype("float32")
    )
    X["Compound_lap_share"] = (comp_same / comp_counts.replace(0, 1)).astype("float32")
    X["Compound_lap_rarity"] = (1.0 - X["Compound_lap_share"]).astype("float32")

    # Interactions aligned with stop-pressure intuition
    X["Pressure_x_DeltaRank"] = (
        X["TyreLife_lap_rank"] * X["LapTime_Delta_lap_rank"]
    ).astype("float32")
    X["Pressure_x_DegRank"] = (
        X["TyreLife_lap_rank"] * X["Cumulative_Degradation_lap_rank"]
    ).astype("float32")
    X["PitNow_x_TyreLifeRank"] = (X["PitStop"] * X["TyreLife_lap_rank"]).astype(
        "float32"
    )
    X["PitNow_x_DeltaRank"] = (X["PitStop"] * X["LapTime_Delta_lap_rank"]).astype(
        "float32"
    )
    X["RearPos_x_WornTyres"] = (
        (1.0 - X["Position_lap_rank"]) * X["TyreLife_lap_rank"]
    ).astype("float32")
    X["LateRace_x_WornTyres"] = (X["RaceProgress"] * X["TyreLife_lap_rank"]).astype(
        "float32"
    )
    X["LateRace_x_Deg"] = (
        X["RaceProgress"] * X["Cumulative_Degradation_lap_rank"]
    ).astype("float32")
    X["SmallGroupFlag"] = (group_size <= 10).astype("int8")

    # Stable categorical normalization
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 001012.",
    }, sort_keys=True))
