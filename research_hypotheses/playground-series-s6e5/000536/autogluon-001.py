...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(c in out.columns for c in race_cols):
        out["ff1_total_laps"] = (
            out.groupby(race_cols)["LapNumber"].transform("max").astype("float32")
        )
        out["ff1_lap_frac"] = (
            out["LapNumber"] / out["ff1_total_laps"].replace(0, np.nan)
        ).astype("float32")
    else:
        out["ff1_total_laps"] = out["LapNumber"].max()
        out["ff1_lap_frac"] = (
            out["LapNumber"] / max(float(out["LapNumber"].max()), 1.0)
        ).astype("float32")

    comp = out["Compound"].astype(str).str.upper()
    out["ff1_rainfall_flag"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")

    if all(c in out.columns for c in key_cols):
        side = (
            out.groupby(key_cols, observed=False)
            .agg(
                ff1_lap_median_time=("LapTime (s)", "median"),
                ff1_lap_mean_time=("LapTime (s)", "mean"),
                ff1_lap_rain_share=("ff1_rainfall_flag", "mean"),
                ff1_lap_pit_share=("PitStop", "mean"),
            )
            .reset_index()
        )

        race_base = side.groupby(race_cols, observed=False)[
            "ff1_lap_median_time"
        ].transform("median")
        race_mad = (
            (side["ff1_lap_median_time"] - race_base)
            .abs()
            .groupby([side["Year"], side["Race"]], observed=False)
            .transform("median")
        )
        side["ff1_safety_car_flag"] = (
            side["ff1_lap_median_time"] > race_base + 3.0 * race_mad.clip(lower=1.0)
        ).astype("int8")
        side["ff1_rainfall_flag_lap"] = (side["ff1_lap_rain_share"] > 0.05).astype(
            "int8"
        )
        side["ff1_pit_window_lap"] = (side["ff1_lap_pit_share"] > 0.08).astype("int8")

        side = side.sort_values(key_cols)
        side["ff1_prev_rainfall_flag"] = (
            side.groupby(race_cols, observed=False)["ff1_rainfall_flag_lap"]
            .shift(1)
            .fillna(0)
            .astype("int8")
        )
        side["ff1_wet_dry_transition"] = (
            side["ff1_rainfall_flag_lap"] != side["ff1_prev_rainfall_flag"]
        ).astype("int8")
        side["ff1_recent_safety_car"] = (
            side.groupby(race_cols, observed=False)["ff1_safety_car_flag"]
        ...
        out = out.merge(
            side[
                ...
                + [
                    "ff1_safety_car_flag",
                    "ff1_recent_safety_car",
                    "ff1_rainfall_flag_lap",
                    "ff1_wet_dry_transition",
                    "ff1_pit_window_lap",
                    "ff1_lap_median_time",
                ]
            ],
        ...
        out["ff1_safety_car_flag"] = 0
        out["ff1_recent_safety_car"] = 0
        out["ff1_rainfall_flag_lap"] = out["ff1_rainfall_flag"]
        out["ff1_wet_dry_transition"] = 0
        out["ff1_pit_window_lap"] = 0
        out["ff1_lap_median_time"] = out["LapTime (s)"]

    out["ff1_lap_time_ratio"] = (
        (out["LapTime (s)"] / out["ff1_lap_median_time"].replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(1.0)
        .astype("float32")
    )

    out["ff1_wet_compound_mismatch"] = (
        out["ff1_rainfall_flag_lap"].astype(int) != out["ff1_rainfall_flag"].astype(int)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000536"],
        "research_usage_note": "Verified assigned hypothesis 000536.",
    }, sort_keys=True))
