...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Driver"] = out["Driver"].astype(str).str.strip().str.upper()
    out["Race"] = out["Race"].astype(str).str.strip()
    out["Compound"] = out["Compound"].astype(str).str.strip().str.upper()

    out["Driver_Compound"] = out["Driver"] + "_" + out["Compound"]
    out["Race_Compound"] = out["Race"] + "_" + out["Compound"]
    out["Driver_Stint"] = out["Driver"] + "_S" + out["Stint"].astype(str)
    out["Driver_Race"] = out["Driver"] + "_" + out["Race"]
    out["Driver_Race_Compound"] = out["Driver_Race"] + "_" + out["Compound"]

    work_cols = ["Year", "Race", "LapNumber", "Driver", "Compound", "Stint", "PitStop"]
    work = out[work_cols].copy()
    work["__pos"] = np.arange(n, dtype=np.int64)
    work["__y"] = work["PitStop"].astype("float32")
    work = work.sort_values(["Year", "Race", "LapNumber", "__pos"], kind="mergesort")

    y_sorted = work["__y"].to_numpy(dtype=np.float32)
    pos_sorted = work["__pos"].to_numpy()
    base = float(np.nanmean(y_sorted)) if n else 0.0
    if not np.isfinite(base):
        base = 0.0

    prev_sum = np.cumsum(y_sorted, dtype=np.float64) - y_sorted
    prev_cnt = np.arange(n, dtype=np.float32)
    global_sorted = ((prev_sum + 100.0 * base) / (prev_cnt + 100.0)).astype("float32")
    global_ord = np.empty(n, dtype=np.float32)
    global_ord[pos_sorted] = global_sorted
    out["te_global_ordered"] = global_ord

    def ordered_mean(name, keys, prior, alpha):
        g = work.groupby(keys, sort=False, observed=True)["__y"]
        cnt = g.cumcount().to_numpy(dtype=np.float32)
        csum = (g.cumsum().to_numpy(dtype=np.float32) - y_sorted).astype(np.float32)

        if np.isscalar(prior):
            prior_sorted = np.full(n, float(prior), dtype=np.float32)
        else:
            prior_sorted = np.asarray(prior, dtype=np.float32)[pos_sorted]

        enc_sorted = (
            (csum + float(alpha) * prior_sorted) / (cnt + float(alpha))
        ).astype(np.float32)
        enc = np.empty(n, dtype=np.float32)
        enc[pos_sorted] = enc_sorted
        out[name] = enc
        return enc

    te_driver = ordered_mean("te_driver_ord", ["Driver"], global_ord, 30.0)
    te_race = ordered_mean("te_race_ord", ["Race"], global_ord, 30.0)
    te_compound = ordered_mean("te_compound_ord", ["Compound"], global_ord, 40.0)

    te_driver_race = ordered_mean(
        "te_driver_race_ord", ["Driver", "Race"], 0.5 * (te_driver + te_race), 60.0
    )
    te_driver_compound = ordered_mean(
        "te_driver_compound_ord",
        ["Driver", "Compound"],
        0.5 * (te_driver + te_compound),
        60.0,
    )
    te_race_compound = ordered_mean(
        "te_race_compound_ord",
        ["Race", "Compound"],
        0.5 * (te_race + te_compound),
        60.0,
    )
    ordered_mean("te_driver_stint_ord", ["Driver", "Stint"], te_driver, 45.0)
    ordered_mean(
        "te_driver_race_compound_ord",
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000838"],
        "research_usage_note": "Verified assigned hypothesis 000838.",
    }, sort_keys=True))
