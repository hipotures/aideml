...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_prog = df["RaceProgress"].astype(float).clip(lower=1e-3)
    pit_now = df["PitStop"].astype(float)
    stint = df["Stint"].astype(float)

    est_total_laps = (lap_num / race_prog).clip(lower=lap_num, upper=100.0)
    laps_remaining_est = (est_total_laps - lap_num).clip(lower=0.0)

    fresh_tyre = (tyre_life <= 2).astype(np.int8)
    finish_pressure = laps_remaining_est / (tyre_life + 1.0)
    tyre_surplus = tyre_life - laps_remaining_est

    df["FreshTyre"] = fresh_tyre
    df["LapsRemaining_Est"] = laps_remaining_est
    df["CurrentTyreFinishPressure"] = finish_pressure
    df["CurrentTyreSurplus"] = tyre_surplus

    df["PitStop_x_TyreLife"] = pit_now * tyre_life
    df["PitStop_x_FreshTyre"] = pit_now * fresh_tyre
    df["PitStop_x_Stint"] = pit_now * stint
    df["PitStop_x_RaceProgress"] = pit_now * race_prog
    df["PitStop_x_LapsRemaining_Est"] = pit_now * laps_remaining_est
    df["PitStop_x_FinishPressure"] = pit_now * finish_pressure
    df["PitStop_x_TyreSurplus"] = pit_now * tyre_surplus

    compound = df["Compound"].astype(str)
    for c in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        flag = (compound == c).astype(np.int8)
        df[f"Compound_{c}"] = flag
        df[f"PitStop_x_Compound_{c}"] = pit_now * flag

    df["PitStop_Compound"] = compound + "_pit_" + df["PitStop"].astype(str)
    df["ROC_AUC_metric"] = "ROC AUC"
    print("ROC AUC")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000053.",
    }, sort_keys=True))
