...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Stable compact categoricals/frequencies for AutoGluon.
    for c in ["Compound", "Race", "Driver"]:
        out[c] = out[c].astype("category")
        out[c + "_freq"] = out[c].map(out[c].value_counts()).astype("float32")

    g = out.groupby(keys, sort=False, observed=True)

    ltd_prev = g["LapTime_Delta"].shift(1)
    deg_prev = g["Cumulative_Degradation"].shift(1)
    pit_prev = g["PitStop"].shift(1).fillna(0)

    out["ltd_prev"] = ltd_prev.fillna(0).astype("float32")
    out["deg_prev"] = deg_prev.fillna(0).astype("float32")
    out["pit_prev"] = pit_prev.astype("float32")

    for w in (3, 5, 8):
        r_ltd = ltd_prev.groupby(
            [out[k] for k in keys], sort=False, observed=True
        ).rolling(w, min_periods=2)
        r_deg = deg_prev.groupby(
            [out[k] for k in keys], sort=False, observed=True
        ).rolling(w, min_periods=2)

        out[f"ltd_roll_mean_{w}"] = (
            r_ltd.mean().reset_index(level=keys, drop=True).fillna(0).astype("float32")
        )
        out[f"ltd_roll_std_{w}"] = (
            r_ltd.std().reset_index(level=keys, drop=True).fillna(0).astype("float32")
        )
        out[f"deg_roll_mean_{w}"] = (
            r_deg.mean().reset_index(level=keys, drop=True).fillna(0).astype("float32")
        )
    ...
    deg_diff1 = (
        g["Cumulative_Degradation"]
        .diff()
        .groupby([out[k] for k in keys], sort=False, observed=True)
        .shift(1)
    )
    out["ltd_slope_1"] = ltd_diff1.fillna(0).astype("float32")
    out["deg_accel_1"] = deg_diff1.diff().fillna(0).astype("float32")
    out["ltd_curvature_1"] = ltd_diff1.diff().fillna(0).astype("float32")

    out["pace_cliff_resid_5"] = (out["ltd_prev"] - out["ltd_roll_mean_5"]).astype(
        "float32"
    )
    out["deg_cliff_resid_5"] = (out["deg_prev"] - out["deg_roll_mean_5"]).astype(
        "float32"
    )
    out["pace_cliff_z_5"] = (
        (out["pace_cliff_resid_5"] / (out["ltd_roll_std_5"].abs() + 1e-3))
        .clip(-20, 20)
        .astype("float32")
    )

    reset_id = pit_prev.groupby(
        [out[k] for k in keys], sort=False, observed=True
    ).cumsum()
    lap_in_regime = out.groupby(
        keys + [reset_id.rename("_reset")], sort=False, observed=True
    ).cumcount()
    out["stable_regime_len"] = lap_in_regime.astype("float32")
    out["short_regime_flag"] = (lap_in_regime < 3).astype("int8")

    out["tyre_age_x_compound_freq"] = (
        out["TyreLife"].astype("float32") * out["Compound_freq"].astype("float32")
    ).astype("float32")
    out["progress_x_stint"] = (
        out["RaceProgress"].astype("float32") * out["Stint"].astype("float32")
    ).astype("float32")

    out.attrs["research_hypotheses_llm_claimed_used"] = ["000395"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000395"],
        "research_usage_note": "Verified assigned hypothesis 000395.",
    }, sort_keys=True))
