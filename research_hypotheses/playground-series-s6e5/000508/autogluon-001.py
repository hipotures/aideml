...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    poschg = out["Position_Change"].astype(float)

    out["haz_race_progress_bin"] = pd.cut(
        rp, bins=[-0.001, 0.15, 0.30, 0.50, 0.70, 0.85, 1.001], labels=False
    ).astype("int16")
    out["haz_tyre_life_bin"] = pd.cut(
        tyre, bins=[0, 3, 6, 10, 15, 22, 32, 45, 90], labels=False, include_lowest=True
    ).astype("int16")
    out["haz_lap_bin"] = pd.cut(
        lap, bins=[0, 3, 8, 15, 25, 38, 55, 90], labels=False, include_lowest=True
    ).astype("int16")

    out["haz_tyre_x_progress"] = tyre * rp
    out["haz_tyre_per_stint"] = tyre / (stint + 1.0)
    out["haz_lap_remaining_proxy"] = (
        (lap / rp.replace(0, np.nan) - lap).replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    out["haz_final_window"] = (rp >= 0.82).astype("int8")
    out["haz_early_stint_cooldown_zone"] = (tyre <= 3).astype("int8")
    ...
    out["haz_compound_prior"] = (
        out["Compound"].astype(str).map(compound_risk).fillna(0.75).astype("float32")
    )

    out["haz_degradation_debt"] = (
        (degr.clip(lower=0) / (tyre + 1.0))
        .replace([np.inf, -np.inf], 0)
        .astype("float32")
    )
    out["haz_laptime_debt"] = ltd.clip(lower=0).astype("float32")
    out["haz_position_loss_debt"] = poschg.clip(lower=0).astype("float32")

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber", "_orig_idx"]
    s = out.sort_values(sort_cols, kind="mergesort")
    gcols = ["Year", "Race", "Driver"]
    pit = s["PitStop"].astype(float)

    s["_prior_pit_count"] = pit.groupby([s[c] for c in gcols]).cumsum() - pit
    s["_last_pit_lap"] = (
        s["LapNumber"].where(pit.eq(1)).groupby([s[c] for c in gcols]).ffill()
    )
    s["_laps_since_pit"] = (s["LapNumber"] - s["_last_pit_lap"]).fillna(99).clip(0, 99)
    s["_stint_prior_pit"] = (
        pit.groupby([s[c] for c in ["Year", "Race", "Driver", "Stint"]]).cumsum() - pit
    )

    out = s.sort_values("_orig_idx", kind="mergesort")
    out["haz_prior_pit_count"] = out["_prior_pit_count"].astype("float32")
    out["haz_laps_since_pit"] = out["_laps_since_pit"].astype("float32")
    out["haz_repeat_stop_cooldown"] = (out["haz_laps_since_pit"] <= 2).astype("int8")
    out["haz_stint_already_stopped"] = (out["_stint_prior_pit"] > 0).astype("int8")

    base = (
        -3.0
        + 0.055 * out["TyreLife"].astype(float)
        + 1.25 * out["RaceProgress"].astype(float)
        + 0.28 * out["haz_compound_prior"].astype(float)
        + 0.012 * out["haz_degradation_debt"].astype(float).clip(0, 80)
        + 0.004 * out["haz_laptime_debt"].astype(float).clip(0, 120)
        - 1.15 * out["haz_repeat_stop_cooldown"].astype(float)
        + 0.20 * out["haz_prior_pit_count"].astype(float).clip(0, 5)
    )
    out["hazard_sidecar_proxy"] = (1.0 / (1.0 + np.exp(-base.clip(-20, 20)))).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000508.",
    }, sort_keys=True))
