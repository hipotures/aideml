...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["is_soft_compound"] = (X["Compound"] == "SOFT").astype("int8")
    X["is_hard_compound"] = (X["Compound"] == "HARD").astype("int8")

    X["degradation_per_lap"] = X["Cumulative_Degradation"] / (X["TyreLife"] + eps)
    X["degradation_per_progress"] = X["Cumulative_Degradation"] / (
        X["RaceProgress"] + eps
    )
    X["tyre_life_ratio"] = X["TyreLife"] / (X["LapNumber"] + eps)
    X["stint_progress"] = X["TyreLife"] / (X["TyreLife"] + X["LapNumber"] - 1 + eps)
    X["laps_remaining_proxy"] = (
        X["LapNumber"] / (X["RaceProgress"] + eps) - X["LapNumber"]
    )

    X["laptime_over_tyrelife"] = X["LapTime (s)"] / (X["TyreLife"] + eps)
    X["lapdelta_over_tyrelife"] = X["LapTime_Delta"] / (X["TyreLife"] + eps)
    X["lapdelta_abs"] = X["LapTime_Delta"].abs()
    X["position_change_abs"] = X["Position_Change"].abs()

    X["degradation_x_tyrelife"] = X["Cumulative_Degradation"] * X["TyreLife"]
    X["degradation_x_stint"] = X["Cumulative_Degradation"] * X["Stint"]
    X["lapdelta_x_progress"] = X["LapTime_Delta"] * X["RaceProgress"]
    X["position_x_progress"] = X["Position"] * X["RaceProgress"]

    X["log_tyrelife"] = np.log1p(X["TyreLife"].clip(lower=0))
    X["log_lapnumber"] = np.log1p(X["LapNumber"].clip(lower=0))
    X["log_degradation"] = np.sign(X["Cumulative_Degradation"]) * np.log1p(
        X["Cumulative_Degradation"].abs()
    )
    X["log_lapdelta"] = np.sign(X["LapTime_Delta"]) * np.log1p(X["LapTime_Delta"].abs())

    grp_driver_race = ["Year", "Race", "Driver"]
    grp_race = ["Year", "Race"]

    driver_race_lap_med = X.groupby(grp_driver_race, observed=True)[
        "LapTime (s)"
    ].transform("median")
    race_lap_med = X.groupby(grp_race, observed=True)["LapTime (s)"].transform("median")
    driver_race_delta_med = X.groupby(grp_driver_race, observed=True)[
        "LapTime_Delta"
    ].transform("median")
    race_delta_med = X.groupby(grp_race, observed=True)["LapTime_Delta"].transform(
        "median"
    )
    driver_race_pos_med = X.groupby(grp_driver_race, observed=True)[
        "Position"
    ].transform("median")

    X["laptime_vs_driver_race_med"] = X["LapTime (s)"] - driver_race_lap_med
    X["laptime_vs_race_med"] = X["LapTime (s)"] - race_lap_med
    X["lapdelta_vs_driver_race_med"] = X["LapTime_Delta"] - driver_race_delta_med
    X["lapdelta_vs_race_med"] = X["LapTime_Delta"] - race_delta_med
    X["position_vs_driver_race_med"] = X["Position"] - driver_race_pos_med
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000810.",
    }, sort_keys=True))
