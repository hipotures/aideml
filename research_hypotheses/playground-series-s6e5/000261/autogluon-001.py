...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in rel_cols:
        if col in out.columns:
            ...
            out[f"{col}_grp_pct"] = g[col].rank(pct=True, method="average")
            out[f"{col}_grp_rank"] = g[col].rank(method="average")
            out[f"{col}_grp_med_gap"] = out[col] - med
            out[f"{col}_grp_mean_gap"] = out[col] - mean
            out[f"{col}_grp_z"] = ((out[col] - mean) / std).fillna(0)

    out["race_year_lap_size"] = g["LapNumber"].transform("size")
    out["tyrelife_x_progress"] = out["TyreLife"] * out["RaceProgress"]
    out["stint_tyrelife_ratio"] = out["TyreLife"] / out["Stint"].replace(0, np.nan)
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / out[
        "TyreLife"
    ].replace(0, np.nan)
    out["lap_delta_per_tyre_lap"] = out["LapTime_Delta"] / out["TyreLife"].replace(
        0, np.nan
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000261.",
    }, sort_keys=True))
