...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["regime_is_wet"] = is_wet
    out["regime_is_dry_slick"] = is_slick

    keys = ["Year", "Race", "Driver"]
    order_cols = keys + ["LapNumber", "Stint"]
    s = out.sort_values(order_cols, kind="mergesort")

    s_comp = s["Compound"].astype(str).str.upper()
    g = s.groupby(keys, sort=False)

    wet_seen = (
        s_comp.isin(["INTERMEDIATE", "WET"])
        .astype("int8")
    ...
    unique_slicks = (hard_seen + medium_seen + soft_seen).astype("int8")

    tmp = pd.DataFrame(index=s.index)
    tmp["wet_seen_in_race"] = wet_seen.astype("int8")
    tmp["unique_slicks_seen"] = unique_slicks
    tmp = tmp.reindex(out.index)

    out["wet_seen_in_race"] = tmp["wet_seen_in_race"].fillna(0).astype("int8")
    out["unique_slicks_seen"] = tmp["unique_slicks_seen"].fillna(0).astype("int8")

    dry_legal_regime = (
        (out["regime_is_wet"] == 0) & (out["wet_seen_in_race"] == 0)
    ).astype("int8")
    out["dry_legal_regime"] = dry_legal_regime
    out["mandatory_second_dry_needed"] = (
        (dry_legal_regime == 1) & (out["unique_slicks_seen"] < 2)
    ).astype("int8")
    out["currently_first_unique_slick"] = (
        (dry_legal_regime == 1)
        & (out["unique_slicks_seen"] <= 1)
        & (out["regime_is_dry_slick"] == 1)
    ).astype("int8")
    out["currently_second_or_later_slick"] = (
        (dry_legal_regime == 1)
        & (out["unique_slicks_seen"] >= 2)
        & (out["regime_is_dry_slick"] == 1)
    ).astype("int8")

    total_laps = (out["LapNumber"] / out["RaceProgress"].clip(lower=0.01)).replace(
        [np.inf, -np.inf], np.nan
    )
    laps_remaining = (total_laps - out["LapNumber"]).clip(lower=0)
    out["estimated_total_laps"] = total_laps
    out["laps_remaining_est"] = laps_remaining
    out["dry_laps_remaining_if_change_slick"] = (
        laps_remaining * out["mandatory_second_dry_needed"]
    )

    out["wet_warmup_life"] = out["TyreLife"].clip(upper=5) * out["regime_is_wet"]
    out["wet_tyrelife_x_degradation"] = (
        out["TyreLife"] * out["Cumulative_Degradation"] * out["regime_is_wet"]
    )
    out["dry_tyrelife_x_second_needed"] = (
        out["TyreLife"] * out["mandatory_second_dry_needed"]
    )
    out["dry_progress_x_second_needed"] = (
        out["RaceProgress"] * out["mandatory_second_dry_needed"]
    )
    out["wet_progress_x_life"] = (
        out["RaceProgress"] * out["TyreLife"] * out["regime_is_wet"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000284.",
    }, sort_keys=True))
