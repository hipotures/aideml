...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ltd = out["LapTime_Delta"].astype(float)
    laptime = out["LapTime (s)"].astype(float)

    late_gate = 1.0 / (1.0 + np.exp(-(lap - 20.0) / 2.0))
    early_gate = 1.0 - late_gate
    out["h000411_late_gate"] = late_gate.astype("float32")
    out["h000411_early_gate"] = early_gate.astype("float32")
    out["h000411_gate_band_18_22"] = ((lap >= 18) & (lap <= 22)).astype("int8")

    out["h000411_laps_to_20"] = np.maximum(0.0, 20.0 - lap).astype("float32")
    out["h000411_laps_after_20"] = np.maximum(0.0, lap - 20.0).astype("float32")
    out["h000411_progress_gate"] = (
        1.0 / (1.0 + np.exp(-(progress - (20.0 / 70.0)) / 0.035))
    ).astype("float32")

    comp = out["Compound"].astype(str).str.upper()
    out["h000411_is_soft"] = (comp == "SOFT").astype("int8")
    out["h000411_is_medium"] = (comp == "MEDIUM").astype("int8")
    out["h000411_is_hard"] = (comp == "HARD").astype("int8")
    out["h000411_is_wet_or_inter"] = comp.isin(["WET", "INTERMEDIATE"]).astype("int8")
    out["h000411_rule_debt"] = (
        (stint <= 1) & (~comp.isin(["WET", "INTERMEDIATE"]))
    ).astype("int8")

    out["h000411_open_pos_pressure"] = (early_gate * (21.0 - pos) / 20.0).astype(
        "float32"
    )
    out["h000411_open_soft_age"] = (
        early_gate * tyre * (comp == "SOFT").astype(float)
    ).astype("float32")
    out["h000411_open_medium_age"] = (
        early_gate * tyre * (comp == "MEDIUM").astype(float)
    ).astype("float32")
    out["h000411_open_hard_age"] = (
        early_gate * tyre * (comp == "HARD").astype(float)
    ).astype("float32")
    out["h000411_open_rule_debt"] = (
        early_gate * out["h000411_rule_debt"].astype(float)
    ).astype("float32")
    out["h000411_open_position_change"] = (early_gate * pos_chg).astype("float32")

    race_lap = [out["Race"], out["Year"], out["LapNumber"]]
    out["h000411_lap_pos_mean"] = (
        out.groupby(race_lap, observed=True)["Position"]
        .transform("mean")
        .astype("float32")
    )
    out["h000411_lap_tyre_mean"] = (
        out.groupby(race_lap, observed=True)["TyreLife"]
        .transform("mean")
        .astype("float32")
    )
    out["h000411_lap_deg_mean"] = (
        out.groupby(race_lap, observed=True)["Cumulative_Degradation"]
        .transform("mean")
        .astype("float32")
    )
    out["h000411_lap_delta_mean"] = (
        out.groupby(race_lap, observed=True)["LapTime_Delta"]
        .transform("mean")
        .astype("float32")
    )

    out["h000411_tyre_vs_lap_mean"] = (tyre - out["h000411_lap_tyre_mean"]).astype(
        "float32"
    )
    out["h000411_deg_vs_lap_mean"] = (deg - out["h000411_lap_deg_mean"]).astype(
        "float32"
    )
    out["h000411_delta_vs_lap_mean"] = (ltd - out["h000411_lap_delta_mean"]).astype(
        "float32"
    )
    out["h000411_position_vs_lap_mean"] = (pos - out["h000411_lap_pos_mean"]).astype(
        "float32"
    )

    out["h000411_late_tyre_pressure"] = (late_gate * tyre).astype("float32")
    out["h000411_late_degradation"] = (late_gate * deg).astype("float32")
    out["h000411_late_laptime_delta"] = (late_gate * ltd).astype("float32")
    out["h000411_late_laptime"] = (late_gate * laptime).astype("float32")
    out["h000411_late_tyre_vs_field"] = (
        late_gate * out["h000411_tyre_vs_lap_mean"]
    ).astype("float32")
    out["h000411_late_deg_vs_field"] = (
        late_gate * out["h000411_deg_vs_lap_mean"]
    ).astype("float32")
    out["h000411_late_delta_vs_field"] = (
        late_gate * out["h000411_delta_vs_lap_mean"]
    ).astype("float32")

    finish_pressure = np.maximum(0.0, progress - 0.65) * tyre
    out["h000411_feasible_finish_pressure"] = finish_pressure.astype("float32")
    out["h000411_late_finish_pressure"] = (late_gate * finish_pressure).astype(
        "float32"
    )
    out["h000411_late_stint_tyre"] = (late_gate * stint * tyre).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000411"],
        "research_usage_note": "Verified assigned hypothesis 000411.",
    }, sort_keys=True))
