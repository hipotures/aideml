...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    signals = {
        "ltd": "LapTime_Delta",
        "ltime": "LapTime (s)",
        "deg": "Cumulative_Degradation",
    }

    tmp = out[keys + ["LapNumber"] + list(signals.values())].copy()
    tmp["__row_pos"] = np.arange(n, dtype=np.int64)
    tmp = tmp.sort_values(
        keys + ["LapNumber", "__row_pos"], kind="mergesort"
    ).reset_index(drop=True)

    g = tmp.groupby(keys, sort=False, observed=True)
    weights5 = np.arange(5, dtype="float64") - 2.0
    denom5 = float((weights5 * weights5).sum())
    score_parts = []
    new_cols = []

    def slope5(col, offset):
        acc = None
        for i, w in enumerate(weights5):
            part = g[col].shift(offset + 4 - i).astype("float64") * w
            acc = part if acc is None else acc + part
        return acc / denom5

    def mean_window(col, width, offset):
        acc = None
        for i in range(width):
            part = g[col].shift(offset + i).astype("float64")
            acc = part if acc is None else acc + part
        return acc / float(width)

    def var_window(col, width):
        mean = mean_window(col, width, 0)
        acc = None
        for i in range(width):
            part = g[col].shift(i).astype("float64")
            part = part * part
            acc = part if acc is None else acc + part
        var = acc / float(width) - mean * mean
        return var.clip(lower=0.0)

    for short_name, col in signals.items():
        post_slope = slope5(col, 0)
        pre_slope = slope5(col, 5)
        slope_jump = post_slope - pre_slope
        resid_var = var_window(col, 8)

        scale = np.sqrt(resid_var) + 1e-3
        valid = pre_slope.notna() & post_slope.notna() & resid_var.notna()
        z = ((slope_jump * 5.0) / scale).where(valid, 0.0).clip(-6.0, 6.0).fillna(0.0)

        for name, values in [
            (f"cp_{short_name}_pre_slope5", pre_slope),
            (f"cp_{short_name}_post_slope5", post_slope),
            (f"cp_{short_name}_slope_jump5", slope_jump),
            (f"cp_{short_name}_resid_var8", resid_var),
            (f"cp_{short_name}_z", z),
        ]:
            tmp[name] = values.astype("float32").fillna(0.0)
            new_cols.append(name)

        score_parts.append(z.clip(lower=0.0))

    score = (score_parts[0] + score_parts[1] + score_parts[2]) / 3.0
    score = score.clip(0.0, 6.0).fillna(0.0)
    prob = 1.0 / (1.0 + np.exp(-(score - 1.5)))

    tmp["tyre_cliff_cp_score"] = score.astype("float32")
    tmp["tyre_cliff_cp_prob"] = prob.astype("float32")
    detected = tmp["tyre_cliff_cp_prob"] >= 0.65

    seq_pos = g.cumcount().astype("float64")
    det_pos = seq_pos.where(detected, np.nan)
    last_det = det_pos.groupby(
        [tmp[k] for k in keys], sort=False, observed=True
    ).cummax()
    laps_since = (seq_pos - last_det).where(last_det.notna(), 99.0).clip(0.0, 99.0)

    tmp["tyre_cliff_detected"] = detected.astype("int8")
    tmp["tyre_cliff_laps_since"] = laps_since.astype("float32")
    tmp["tyre_cliff_recent3"] = (laps_since <= 3.0).astype("int8")
    new_cols += [
        "tyre_cliff_cp_score",
        "tyre_cliff_cp_prob",
        "tyre_cliff_detected",
        "tyre_cliff_laps_since",
        "tyre_cliff_recent3",
    ]

    row_pos = tmp["__row_pos"].to_numpy()
    for col in new_cols:
        arr = np.empty(n, dtype=tmp[col].dtype)
        arr[row_pos] = tmp[col].to_numpy()
        out[col] = arr
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000788"],
        "research_usage_note": "Verified assigned hypothesis 000788.",
    }, sort_keys=True))
