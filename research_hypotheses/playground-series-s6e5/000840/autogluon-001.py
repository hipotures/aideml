...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["rl_laptime_delta_median"] = (
        g["LapTime_Delta"].transform("median").astype("float32")
    )
    q75 = g["LapTime_Delta"].transform(lambda s: s.quantile(0.75))
    q25 = g["LapTime_Delta"].transform(lambda s: s.quantile(0.25))
    out["rl_laptime_delta_iqr"] = (q75 - q25).astype("float32")
    out["rl_laptime_median"] = g["LapTime (s)"].transform("median").astype("float32")
    out["rl_field_size"] = g["Driver"].transform("count").astype("int16")

    delta_global_q90 = out["LapTime_Delta"].quantile(0.90)
    out["_extreme_pos_delta"] = (out["LapTime_Delta"] > delta_global_q90).astype(
        "float32"
    )
    out["rl_frac_extreme_pos_delta"] = (
        g["_extreme_pos_delta"].transform("mean").astype("float32")
    )

    out["rl_abs_pos_change_mean"] = (
        g["Position_Change"].transform(lambda s: s.abs().mean()).astype("float32")
    )
    out["rl_pos_change_std"] = (
        g["Position_Change"].transform("std").fillna(0).astype("float32")
    )
    out["rl_position_compression"] = (
        1.0 / (1.0 + out["rl_abs_pos_change_mean"])
    ).astype("float32")

    med = out["rl_laptime_delta_median"]
    iqr = out["rl_laptime_delta_iqr"].replace(0, np.nan)
    out["rl_neutralization_score"] = (
        med.clip(lower=0) / (iqr.fillna(iqr.median()) + 1.0)
        + out["rl_frac_extreme_pos_delta"] * 2.0
        + out["rl_position_compression"]
    ).astype("float32")

    out["neutral_x_pitstop"] = (out["rl_neutralization_score"] * out["PitStop"]).astype(
        "float32"
    )
    out["neutral_x_tyrelife"] = (
        out["rl_neutralization_score"] * out["TyreLife"]
    ).astype("float32")
    out["neutral_x_laps_remaining"] = (
        out["rl_neutralization_score"] * out["Laps_Remaining_Est"]
    ).astype("float32")
    out["neutral_x_wet"] = (
        out["rl_neutralization_score"] * out["Is_Wet_Compound"]
    ).astype("float32")
    out["neutral_x_dry"] = (
        out["rl_neutralization_score"] * out["Is_Dry_Compound"]
    ).astype("float32")

    out = out.drop(columns=["_extreme_pos_delta"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000840"],
        "research_usage_note": "Verified assigned hypothesis 000840.",
    }, sort_keys=True))
