...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = (
        out["Compound"].astype("string").str.upper().str.strip().fillna("UNKNOWN")
    )
    out["Compound_norm"] = compound.astype("category")

    group_cols = ["Year", "Race", "LapNumber", "Compound_norm"]
    base_cols = ["TyreLife", "LapTime_Delta", "Cumulative_Degradation", "Position"]

    g = out.groupby(group_cols, observed=True, sort=False)
    group_size = g["LapNumber"].transform("size").astype("float32")
    out["same_compound_group_size"] = group_size
    valid_group = group_size >= 3.0

    for col in base_cols:
        x = out[col].astype("float32")
        med = g[col].transform("median").astype("float32")
        q25 = g[col].transform(lambda s: s.quantile(0.25)).astype("float32")
        q75 = g[col].transform(lambda s: s.quantile(0.75)).astype("float32")
        spread = (q75 - q25).replace(0, np.nan).astype("float32")

        delta = (x - med).astype("float32")
        rel = (delta / spread).replace([np.inf, -np.inf], 0).fillna(0).astype("float32")
        rank_pct = g[col].rank(method="average", pct=True).astype("float32")

        out[f"same_compound_{col}_med_delta"] = delta.where(valid_group, 0).astype(
            "float32"
        )
        out[f"same_compound_{col}_iqr_scaled_delta"] = rel.where(valid_group, 0).astype(
            "float32"
        )
        out[f"same_compound_{col}_rank_pct"] = rank_pct.where(valid_group, 0.5).astype(
            "float32"
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000043"],
        "research_usage_note": "Verified assigned hypothesis 000043.",
    }, sort_keys=True))
