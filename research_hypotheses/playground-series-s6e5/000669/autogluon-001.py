...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Stable ordering within each driver-race sequence.
    X["_row_id"] = np.arange(len(X))
    X = X.sort_values(grp + ["LapNumber", "_row_id"], kind="mergesort")

    g = X.groupby(grp, sort=False)
    sg = X.groupby(stint_grp, sort=False)

    # Group-size and position features to express "urgency within sequence".
    X["drv_race_laps_total"] = g["LapNumber"].transform("max")
    X["drv_race_laps_done_rank"] = g.cumcount() + 1
    X["drv_race_laps_left"] = X["drv_race_laps_total"] - X["LapNumber"]
    X["drv_race_progress_rank"] = X["drv_race_laps_done_rank"] / g[
        "LapNumber"
    ].transform("size").clip(lower=1)

    X["stint_laps_total"] = sg["LapNumber"].transform("size")
    X["stint_lap_index"] = sg.cumcount() + 1
    X["stint_progress_rank"] = X["stint_lap_index"] / X["stint_laps_total"].clip(
        lower=1
    )

    # Relative ranks within driver-race to mimic a ranking formulation.
    rank_cols = [
        "TyreLife",
    ...
    for c in rank_cols:
        X[f"{c}_drv_race_pct"] = g[c].rank(method="average", pct=True)

    # Relative ranks within stint for local pit-window detection.
    for c in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta", "LapTime (s)"]:
        X[f"{c}_stint_pct"] = sg[c].rank(method="average", pct=True)

    # Normalize key signals inside each driver-race sequence.
    for c in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta", "LapTime (s)"]:
        cmin = g[c].transform("min")
        cmax = g[c].transform("max")
        X[f"{c}_drv_race_minmax"] = (X[c] - cmin) / (cmax - cmin).replace(0, np.nan)

    # Interactions that highlight rising pit urgency.
    X["wear_x_deg"] = X["TyreLife"] * X["Cumulative_Degradation"]
    X["wear_x_delta"] = X["TyreLife"] * X["LapTime_Delta"]
    X["deg_x_delta"] = X["Cumulative_Degradation"] * X["LapTime_Delta"]
    X["progress_x_wear"] = X["RaceProgress"] * X["TyreLife"]
    X["progress_x_deg"] = X["RaceProgress"] * X["Cumulative_Degradation"]
    X["laps_left_x_deg"] = X["drv_race_laps_left"] * X["Cumulative_Degradation"]
    X["stint_progress_x_deg"] = X["stint_progress_rank"] * X["Cumulative_Degradation"]
    X["stint_progress_x_delta"] = X["stint_progress_rank"] * X["LapTime_Delta"]

    # Compound-aware urgency flags without target leakage.
    ...
    X["soft_high_wear"] = X["is_soft"] * X["TyreLife_drv_race_pct"]
    X["soft_high_deg"] = X["is_soft"] * X["Cumulative_Degradation_drv_race_pct"]
    X["wet_progress"] = X["is_wet_like"] * X["RaceProgress"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000669"],
        "research_usage_note": "Verified assigned hypothesis 000669.",
    }, sort_keys=True))
