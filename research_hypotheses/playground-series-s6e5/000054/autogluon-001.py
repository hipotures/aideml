...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Preserve high-signal identifiers/state.
    out["Year"] = df["Year"].astype("int16")
    out["Race"] = df["Race"].astype("category")
    out["Driver"] = df["Driver"].astype("category")
    out["Compound"] = df["Compound"].astype("category")
    out["Stint"] = df["Stint"].astype("int8")
    out["PitStop"] = df["PitStop"].astype("int8")

    # Canonical continuous / normalized features.
    tyre_life = df["TyreLife"].astype("float32")
    lap_number = df["LapNumber"].astype("float32")
    race_progress = df["RaceProgress"].astype("float32").clip(0.0, 1.0)
    cum_deg = df["Cumulative_Degradation"].astype("float32")
    lap_delta = df["LapTime_Delta"].astype("float32")
    position = df["Position"].astype("float32")
    pos_change = df["Position_Change"].astype("float32")

    out["race_progress_norm"] = race_progress
    out["pace_margin"] = lap_delta
    out["deg_per_tyre_lap"] = cum_deg / np.maximum(tyre_life, 1.0)
    out["tyre_age_vs_race_ratio"] = tyre_life / np.maximum(lap_number, 1.0)
    out["position_norm"] = position / 20.0
    out["position_change_norm"] = pos_change / 20.0

    # Compact binary threshold flags instead of many overlapping interactions.
    comp = df["Compound"].astype(str)
    dry_threshold = np.where(
        comp == "SOFT",
        16.0,
        np.where(comp == "MEDIUM", 22.0, np.where(comp == "HARD", 28.0, 12.0)),
    )
    out["is_wet_compound"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["in_pit_window"] = (tyre_life >= dry_threshold).astype("int8")
    out["late_stint_pressure"] = (
        (tyre_life >= dry_threshold * 1.25) | (cum_deg >= 120.0)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000054"],
        "research_usage_note": "Verified assigned hypothesis 000054.",
    }, sort_keys=True))
