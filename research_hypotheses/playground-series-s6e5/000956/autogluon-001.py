...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit_loss_band = {
        ...
        "Brazilian Grand Prix": 0,
        "Sao Paulo Grand Prix": 0,
    ...
    degradation_band = {
        ...
        "Sao Paulo Grand Prix": 1,
        "Russian Grand Prix": 0,
        "Pre-Season Testing": 1,
    }
    ...
    df["ExpectedTyreLife"] = df["Compound"].map(compound_life).fillna(26.0)
    df["TyreLife_ExpectedLife_Ratio"] = df["TyreLife"] / df["ExpectedTyreLife"]

    progress = df["RaceProgress"].clip(0.01, 1.0)
    est_total_laps = (df["LapNumber"] / progress).clip(lower=df["LapNumber"], upper=90)
    df["Estimated_Laps_Remaining"] = (est_total_laps - df["LapNumber"]).clip(lower=0)

    df["pit_loss_band"] = df["Race"].map(pit_loss_band).fillna(1).astype("int8")
    df["degradation_band"] = df["Race"].map(degradation_band).fillna(1).astype("int8")
    df["overtake_difficulty"] = (
        df["Race"].map(overtake_difficulty).fillna(1).astype("int8")
    )
    df["wet_sensitivity"] = df["Race"].map(wet_sensitivity).fillna(1).astype("int8")
    ...
    df["compound_code"] = compound_code

    df["pitloss_x_stint"] = df["pit_loss_band"] * df["Stint"]
    df["degradation_x_tyre_ratio"] = (
        df["degradation_band"] * df["TyreLife_ExpectedLife_Ratio"]
    )
    df["degradation_x_laps_remaining"] = (
        df["degradation_band"] * df["Estimated_Laps_Remaining"]
    )
    df["overtake_x_position"] = df["overtake_difficulty"] * df["Position"]
    df["wet_x_compound"] = df["wet_sensitivity"] * compound_code
    df["pitloss_x_compound"] = df["pit_loss_band"] * compound_code
    df["stint_x_laps_remaining"] = df["Stint"] * df["Estimated_Laps_Remaining"]
    df["tyre_ratio_x_laps_remaining"] = (
        df["TyreLife_ExpectedLife_Ratio"] * df["Estimated_Laps_Remaining"]
    )

    is_wet = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    is_slick = df["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    df["wet_strategy_pressure"] = (
        is_wet * df["wet_sensitivity"] * (1.0 + df["Estimated_Laps_Remaining"] / 10.0)
    )
    df["slick_deg_pressure"] = (
        is_slick * df["degradation_band"] * df["TyreLife_ExpectedLife_Ratio"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000956.",
    }, sort_keys=True))
