...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    laps_remaining = (
        out.groupby(["Year", "Race"])["LapNumber"].transform("max") - out["LapNumber"]
    ).astype("float32")
    out["Laps_Remaining_Est"] = laps_remaining
    out["TyreLife_x_LapsRemaining"] = (out["TyreLife"] * laps_remaining).astype(
        "float32"
    )
    out["TyreLife_div_Stint"] = (out["TyreLife"] / (out["Stint"].clip(lower=1))).astype(
        "float32"
    )
    out["RaceProgress_x_TyreLife"] = (out["RaceProgress"] * out["TyreLife"]).astype(
        "float32"
    )
    out["Position_x_TyreLife"] = (out["Position"] * out["TyreLife"]).astype("float32")

    comp = out["Compound"].astype("string").fillna("UNKNOWN")
    out["is_wet_or_inter"] = comp.isin(["WET", "INTERMEDIATE"]).astype("int8")
    out["is_soft"] = (comp == "SOFT").astype("int8")
    out["is_hard"] = (comp == "HARD").astype("int8")

    lap_agg = (
        out.groupby(key, sort=False)
        .agg(
            lap_median_time=("LapTime (s)", "median"),
            lap_q25_time=("LapTime (s)", lambda s: s.quantile(0.25)),
            lap_q75_time=("LapTime (s)", lambda s: s.quantile(0.75)),
            lap_median_delta=("LapTime_Delta", "median"),
            lap_pit_count=("PitStop", "sum"),
            lap_car_count=("PitStop", "size"),
            lap_wet_inter_share=("is_wet_or_inter", "mean"),
            lap_abs_poschg_mean=("Position_Change", lambda s: s.abs().mean()),
        )
        .reset_index()
        .sort_values(key)
    )
    lap_agg["lap_iqr_time"] = (
        lap_agg["lap_q75_time"] - lap_agg["lap_q25_time"]
    ).astype("float32")
    lap_agg["lap_pit_share"] = (
        lap_agg["lap_pit_count"] / lap_agg["lap_car_count"].clip(lower=1)
    ).astype("float32")

    g = lap_agg.groupby(["Year", "Race"], sort=False)
    for col in [
        "lap_median_time",
        "lap_median_delta",
        "lap_pit_share",
        ...
        past = g[col].shift(1)
        base = (
            past.groupby([lap_agg["Year"], lap_agg["Race"]], sort=False)
        ...
        lap_agg[col + "_past5_mean"] = base
        lap_agg[col + "_past5_std"] = spread.replace(0, np.nan)
        lap_agg[col + "_z_past5"] = (
            ((lap_agg[col] - base) / lap_agg[col + "_past5_std"])
            .fillna(0)
            .astype("float32")
        )

    lap_agg["mass_pit_wave"] = (
        (lap_agg["lap_pit_count"] >= 3) | (lap_agg["lap_pit_share"] >= 0.18)
    ).astype("int8")
    lap_agg["abnormal_slow_lap"] = (
        (lap_agg["lap_median_time_z_past5"] > 2.0)
        | (lap_agg["lap_median_delta_z_past5"] > 2.0)
    ).astype("int8")
    lap_agg["compressed_field_proxy"] = (
        lap_agg["lap_abs_poschg_mean_z_past5"] < -1.0
    ).astype("int8")
    lap_agg["prior_lap_mass_pit_wave"] = (
        g["mass_pit_wave"].shift(1).fillna(0).astype("int8")
    )
    lap_agg["prior_lap_abnormal_slow"] = (
        g["abnormal_slow_lap"].shift(1).fillna(0).astype("int8")
    )
    lap_agg["wave_or_slow_context"] = (
        (lap_agg["mass_pit_wave"] == 1)
        | (lap_agg["abnormal_slow_lap"] == 1)
        | (lap_agg["prior_lap_mass_pit_wave"] == 1)
        | (lap_agg["prior_lap_abnormal_slow"] == 1)
    ).astype("int8")
    ...
    keep = [
        ...
        "lap_pit_share",
        "lap_wet_inter_share",
        "lap_median_time_z_past5",
        "lap_median_delta_z_past5",
        "lap_pit_share_z_past5",
        "lap_abs_poschg_mean_z_past5",
        "mass_pit_wave",
        "abnormal_slow_lap",
        "compressed_field_proxy",
        "prior_lap_mass_pit_wave",
        "prior_lap_abnormal_slow",
        "wave_or_slow_context",
    ]
    out = out.merge(lap_agg[keep], on=key, how="left", sort=False)

    out["context_x_tyre_life"] = (out["wave_or_slow_context"] * out["TyreLife"]).astype(
        "float32"
    )
    out["context_x_position"] = (out["wave_or_slow_context"] * out["Position"]).astype(
        "float32"
    )
    out["context_x_laps_remaining"] = (
        out["wave_or_slow_context"] * out["Laps_Remaining_Est"]
    ).astype("float32")
    out["mass_wave_x_tyre_life"] = (out["mass_pit_wave"] * out["TyreLife"]).astype(
        "float32"
    )
    out["slow_lap_x_wet_inter"] = (
        out["abnormal_slow_lap"] * out["is_wet_or_inter"]
    ).astype("int8")
    out["pit_share_x_tyre_life"] = (out["lap_pit_share"] * out["TyreLife"]).astype(
        "float32"
    )
    out["pit_share_x_position"] = (out["lap_pit_share"] * out["Position"]).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000719"],
        "research_usage_note": "Verified assigned hypothesis 000719.",
    }, sort_keys=True))
