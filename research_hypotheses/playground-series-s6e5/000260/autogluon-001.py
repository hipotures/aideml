...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_priors = {
        "Bahrain Grand Prix": (1, 2, 1),
        "Saudi Arabian Grand Prix": (1, 0, 0),
        "Australian Grand Prix": (1, 1, 1),
        "Emilia Romagna Grand Prix": (2, 0, 2),
        "Miami Grand Prix": (1, 1, 1),
        "Spanish Grand Prix": (1, 2, 1),
        "Monaco Grand Prix": (2, 0, 2),
        "Azerbaijan Grand Prix": (0, 0, 0),
        "Canadian Grand Prix": (1, 1, 2),
        "British Grand Prix": (1, 1, 1),
        "Austrian Grand Prix": (0, 2, 1),
        "French Grand Prix": (1, 2, 1),
        "Hungarian Grand Prix": (2, 1, 2),
        "Belgian Grand Prix": (2, 1, 2),
        "Dutch Grand Prix": (1, 2, 1),
        "Italian Grand Prix": (0, 0, 0),
        "Singapore Grand Prix": (2, 0, 2),
        "Japanese Grand Prix": (1, 2, 2),
        "Qatar Grand Prix": (1, 2, 1),
        "United States Grand Prix": (1, 2, 1),
        "Mexico City Grand Prix": (1, 0, 2),
        "Sao Paulo Grand Prix": (0, 2, 1),
        "São Paulo Grand Prix": (0, 2, 1),
        "Las Vegas Grand Prix": (1, 0, 2),
        "Abu Dhabi Grand Prix": (1, 1, 0),
        "Chinese Grand Prix": (1, 1, 1),
        "Russian Grand Prix": (1, 0, 2),
        "Pre-Season Testing": (1, 1, 1),
    }

    pit_map = {k: v[0] for k, v in race_priors.items()}
    stop_map = {k: v[1] for k, v in race_priors.items()}
    warm_map = {k: v[2] for k, v in race_priors.items()}

    race = out["Race"].astype(str)
    pit_loss = race.map(pit_map).fillna(1).astype("int8")
    two_stop = race.map(stop_map).fillna(1).astype("int8")
    warmup = race.map(warm_map).fillna(1).astype("int8")

    progress = out["RaceProgress"].clip(lower=0.01, upper=1.0)
    est_total_laps = (out["LapNumber"] / progress).clip(lower=1.0, upper=100.0)
    laps_remaining = (est_total_laps - out["LapNumber"]).clip(lower=0.0, upper=80.0)
    tyre_to_remaining = (out["TyreLife"] / (laps_remaining + 1.0)).clip(
    ...
    out["LapsRemaining_Est"] = laps_remaining.astype("float32")
    out["TyreLife_To_Remaining"] = tyre_to_remaining.astype("float32")
    out["Race_PitLoss_Bucket"] = pit_loss
    out["Race_TwoStopBias_Bucket"] = two_stop
    out["Race_Warmup_Bucket"] = warmup
    out["Compound_Warmup_Bucket"] = compound_warm
    out["Compound_Wear_Bucket"] = compound_wear
    out["PitLoss_x_LapsRemaining"] = (
        pit_loss.astype("float32") * laps_remaining
    ).astype("float32")
    out["TwoStopBias_x_TyreLifeToRemaining"] = (
        two_stop.astype("float32") * tyre_to_remaining
    ).astype("float32")
    out["Warmup_x_CompoundWarmup"] = (
        warmup.astype("float32") * compound_warm.astype("float32")
    ).astype("float32")
    out["PitLoss_x_CompoundWear"] = (
        pit_loss.astype("float32") * compound_wear.astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000260"],
        "research_usage_note": "Verified assigned hypothesis 000260.",
    }, sort_keys=True))
