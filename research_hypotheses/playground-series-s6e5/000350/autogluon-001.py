...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key_cols = ["Year", "Race", "Driver"]
    sort_cols = key_cols + ["LapNumber", "_row_id"]
    work = out.sort_values(sort_cols).copy()

    race_grp = work.groupby(["Year", "Race"], sort=False)
    seq_grp = work.groupby(key_cols, sort=False)
    stint_grp = work.groupby(key_cols + ["Stint"], sort=False)

    seq_idx = seq_grp.cumcount()
    stint_idx = stint_grp.cumcount()

    work["race_max_lap"] = race_grp["LapNumber"].transform("max")
    work["laps_remaining_in_race"] = work["race_max_lap"] - work["LapNumber"]
    work["seq_lap_idx"] = seq_idx + 1
    work["stint_lap_idx"] = stint_idx + 1

    work["compound"] = work["Compound"].astype(str)
    ...
    work["is_intermediate"] = (work["compound"] == "INTERMEDIATE").astype("int8")
    work["is_wet"] = (work["compound"] == "WET").astype("int8")
    work["is_slick"] = (
        work[["is_soft", "is_medium", "is_hard"]].max(axis=1).astype("int8")
    )

    work["stint_progress_est"] = work["TyreLife"] / (
        work["TyreLife"] + work["laps_remaining_in_race"] + 1.0
    )
    work["race_progress_remaining"] = 1.0 - work["RaceProgress"]
    ...
    for col in lag_cols:
        work[f"{col}_lag1"] = seq_grp[col].shift(1)

    work["lap_time_change_1"] = work["LapTime (s)"] - work["LapTime (s)_lag1"]
    work["lap_delta_change_1"] = work["LapTime_Delta"] - work["LapTime_Delta_lag1"]
    work["degradation_change_1"] = (
        work["Cumulative_Degradation"] - work["Cumulative_Degradation_lag1"]
    )
    work["position_change_vs_prev"] = work["Position"] - work["Position_lag1"]

    stint_counts = stint_grp.cumcount()
    for col in ["LapTime_Delta", "Cumulative_Degradation", "Position_Change"]:
        csum = stint_grp[col].cumsum()
        prev_sum = csum - work[col]
        denom = stint_counts.replace(0, np.nan)
        work[f"{col}_stint_mean_prev"] = prev_sum / denom

    work["lap_delta_vs_stint_mean"] = (
        work["LapTime_Delta"] - work["LapTime_Delta_stint_mean_prev"]
    )
    work["degradation_vs_stint_mean"] = (
        work["Cumulative_Degradation"] - work["Cumulative_Degradation_stint_mean_prev"]
    )
    work["pos_change_vs_stint_mean"] = (
        work["Position_Change"] - work["Position_Change_stint_mean_prev"]
    )

    work["deg_per_tyre_lap"] = work["Cumulative_Degradation"] / work[
        "TyreLife"
    ].replace(0, np.nan)
    work["deg_x_tyre_life"] = work["Cumulative_Degradation"] * work["TyreLife"]
    work["delta_x_tyre_life"] = work["LapTime_Delta"] * work["TyreLife"]
    work["delta_x_remaining"] = work["LapTime_Delta"] * work["laps_remaining_in_race"]
    work["deg_x_remaining"] = (
        work["Cumulative_Degradation"] * work["laps_remaining_in_race"]
    )
    work["stint_x_remaining"] = work["stint_lap_idx"] * work["laps_remaining_in_race"]
    work["wet_x_progress_remaining"] = work["is_wet"] * work["race_progress_remaining"]
    work["slick_x_stint_progress"] = work["is_slick"] * work["stint_progress_est"]

    work["likely_end_of_stint"] = (
        (work["TyreLife"] >= 15).astype("int8")
        + (work["LapTime_Delta"] > 0).astype("int8")
        + (work["Cumulative_Degradation"] > 0).astype("int8")
        + (work["laps_remaining_in_race"] <= 5).astype("int8")
    ).astype("int8")

    work = work.sort_values("_row_id").drop(columns=["_row_id", "compound"])
    return work
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000350"],
        "research_usage_note": "Verified assigned hypothesis 000350.",
    }, sort_keys=True))
