...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["RaceLap_FieldSize"] = out.groupby(keys)["LapNumber"].transform("size")

    out["RaceLap_Median_LapTime_Delta"] = out.groupby(keys)["LapTime_Delta"].transform(
        "median"
    )
    out["RaceLap_Mean_LapTime"] = out.groupby(keys)["LapTime (s)"].transform("mean")
    out["RaceLap_Median_LapTime"] = out.groupby(keys)["LapTime (s)"].transform("median")
    out["RaceLap_PitFrac"] = out.groupby(keys)["PitStop"].transform("mean")
    out["RaceLap_PitCount"] = out.groupby(keys)["PitStop"].transform("sum")

    race_med = out.groupby(["Year", "Race"])["RaceLap_Median_LapTime_Delta"].transform(
        "median"
    )
    race_mad = (
        (out["RaceLap_Median_LapTime_Delta"] - race_med)
        .abs()
        .groupby([out["Year"], out["Race"]])
        .transform("median")
    )
    out["FieldInflation_RobustZ"] = (
        (out["RaceLap_Median_LapTime_Delta"] - race_med) / (1.4826 * race_mad + 1e-6)
    ).clip(-12, 12)
    out["Neutralization_Heuristic"] = (
        (out["FieldInflation_RobustZ"] > 2.5) | (out["RaceLap_PitFrac"] > 0.18)
    ).astype("int8")

    lap_tbl = (
        out[keys + ["RaceLap_PitFrac", "RaceLap_PitCount", "RaceLap_FieldSize"]]
        .drop_duplicates(keys)
        .sort_values(keys)
    )
    lap_tbl["PitFrac_Lag1"] = (
        lap_tbl.groupby(["Year", "Race"])["RaceLap_PitFrac"].shift(1).fillna(0)
    )
    lap_tbl["PitFrac_Lag2"] = (
        lap_tbl.groupby(["Year", "Race"])["RaceLap_PitFrac"].shift(2).fillna(0)
    )
    lap_tbl["PitCount_Lag1"] = (
        lap_tbl.groupby(["Year", "Race"])["RaceLap_PitCount"].shift(1).fillna(0)
    )
    lap_tbl["PitCount_Lag2"] = (
        lap_tbl.groupby(["Year", "Race"])["RaceLap_PitCount"].shift(2).fillna(0)
    )
    lap_tbl["PitWave_Last2Frac"] = lap_tbl["PitFrac_Lag1"] + lap_tbl["PitFrac_Lag2"]
    out = out.merge(
        lap_tbl[
            keys
            + [
    ...
    front_pits = out["PitStop"].where(out["Position"] <= 6, 0)
    front_tbl = out[keys].copy()
    front_tbl["FrontPit"] = front_pits
    front_tbl = (
        front_tbl.groupby(keys, as_index=False)["FrontPit"].sum().sort_values(keys)
    )
    front_tbl["TopPitCount_Lag1"] = (
        front_tbl.groupby(["Year", "Race"])["FrontPit"].shift(1).fillna(0)
    )
    front_tbl["TopPitCount_Lag2"] = (
        front_tbl.groupby(["Year", "Race"])["FrontPit"].shift(2).fillna(0)
    )
    front_tbl["TopPitCount_Last2"] = (
        front_tbl["TopPitCount_Lag1"] + front_tbl["TopPitCount_Lag2"]
    )
    out = out.merge(
        front_tbl[keys + ["TopPitCount_Lag1", "TopPitCount_Lag2", "TopPitCount_Last2"]],
        on=keys,
        how="left",
        sort=False,
    )

    out["Position_Pct"] = out.groupby(keys)["Position"].rank(pct=True, method="average")
    out["GapToWave_Pct"] = (out["Position_Pct"] - out["RaceLap_PitFrac"]).clip(-1, 1)
    out["Wave_x_LapsRemaining"] = out["PitWave_Last2Frac"] * out["LapsRemaining_Est"]
    out["Neutral_x_LapsRemaining"] = (
        out["FieldInflation_RobustZ"].clip(lower=0) * out["LapsRemaining_Est"]
    )
    out["Wave_x_Position"] = out["PitWave_Last2Frac"] * out["Position"]
    out["Neutral_x_Position"] = out["Neutralization_Heuristic"] * out["Position"]

    comp = out["Compound"].astype("category")
    out["Compound_Code"] = comp.cat.codes.astype("int16")
    out["Wave_x_Compound"] = out["PitWave_Last2Frac"] * out["Compound_Code"]
    out["Neutral_x_Compound"] = out["Neutralization_Heuristic"] * out["Compound_Code"]
    out["TyreLife_x_Wave"] = out["TyreLife"] * out["PitWave_Last2Frac"]
    out["TyreLife_x_Neutral"] = out["TyreLife"] * out["Neutralization_Heuristic"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000166"],
        "research_usage_note": "Verified assigned hypothesis 000166.",
    }, sort_keys=True))
