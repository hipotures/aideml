...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Code"] = out["Compound"].map(compound_map).fillna(-2).astype("int8")
    out["Is_Wet_Compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )

    out["TyreLife"] = out["TyreLife"].astype(float)
    out["LapNumber"] = out["LapNumber"].astype(float)
    out["RaceProgress"] = out["RaceProgress"].astype(float)
    out["LapTime_Delta"] = out["LapTime_Delta"].astype(float)
    out["Cumulative_Degradation"] = out["Cumulative_Degradation"].astype(float)
    out["Position_Change"] = out["Position_Change"].astype(float)
    out["PitStop"] = out["PitStop"].astype(float)

    out["TyreLife_Ratio"] = out["TyreLife"] / out["LapNumber"].clip(lower=1.0)
    out["Lap_Remaining_Frac"] = 1.0 - out["RaceProgress"]
    out["Stint_Pressure"] = out["TyreLife"] * out["RaceProgress"]
    out["Deg_per_TyreLife"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1.0
    )
    out["Drift_per_TyreLife"] = out["LapTime_Delta"] / out["TyreLife"].clip(lower=1.0)

    for base_col, prefix in [
        ("TyreLife", "TL"),
        ("Cumulative_Degradation", "DEG"),
        ("LapTime_Delta", "LTD"),
    ]:
        grp = out.groupby(["Year", "Compound"])[base_col]
        med = grp.transform("median")
        q75 = grp.transform(lambda s: s.quantile(0.75))
        q90 = grp.transform(lambda s: s.quantile(0.90))
        iqr = grp.transform(lambda s: s.quantile(0.75) - s.quantile(0.25))
        out[f"{prefix}_Above_Med"] = (out[base_col] - med).astype("float32")
        out[f"{prefix}_RobustZ"] = (
            ((out[base_col] - med) / iqr.replace(0, np.nan))
            .replace([np.inf, -np.inf], 0)
            .fillna(0)
            .astype("float32")
        )
        out[f"{prefix}_High75"] = (out[base_col] >= q75).astype("int8")
        out[f"{prefix}_High90"] = (out[base_col] >= q90).astype("int8")

    out = out.sort_values(
        ["Year", "Race", "Driver", "LapNumber", "_row_id"]
    ).reset_index(drop=True)
    grp_keys = ["Year", "Race", "Driver"]
    g = out.groupby(grp_keys, sort=False)

    out["Prev_LapTime_Delta"] = g["LapTime_Delta"].shift(1).fillna(0)
    out["Prev_Cumulative_Degradation"] = g["Cumulative_Degradation"].shift(1).fillna(0)
    out["Prev_Position_Change"] = g["Position_Change"].shift(1).fillna(0)
    out["Prev_PitStop"] = g["PitStop"].shift(1).fillna(0)

    out["LapTime_Delta_Change"] = (
        out["LapTime_Delta"] - out["Prev_LapTime_Delta"]
    ).astype("float32")
    out["Deg_Change"] = (
        out["Cumulative_Degradation"] - out["Prev_Cumulative_Degradation"]
    ).astype("float32")
    out["PosChange_Abs2"] = (
        out["Position_Change"].abs() + out["Prev_Position_Change"].abs()
    ).astype("float32")

    prev_pit_lap = out["LapNumber"].where(out["Prev_PitStop"] > 0, np.nan)
    out["Last_Pit_Lap"] = prev_pit_lap.groupby(
        [out[k] for k in grp_keys], sort=False
    ).ffill()
    out["Laps_Since_Last_Pit"] = (
        (out["LapNumber"] - out["Last_Pit_Lap"]).fillna(999).astype("float32")
    )
    out["Fresh_Outlap_Window"] = out["Laps_Since_Last_Pit"].between(1, 3).astype("int8")

    out["HardNeg_Window_Sum"] = (
        out["TL_High75"]
        + out["DEG_High75"]
        + out["LTD_High75"]
        + (out["LapTime_Delta_Change"] > 0).astype("int8")
        + (out["Deg_Change"] > 0).astype("int8")
    ).astype("int8")

    out["HardNeg_Window_Strict"] = (
        (out["TL_High75"] == 1) & (out["DEG_High75"] == 1) & (out["LTD_High75"] == 1)
    ).astype("int8")

    out["HardNeg_Specialist_Score"] = (
        0.35 * out["TL_RobustZ"]
        + 0.35 * out["DEG_RobustZ"]
        + 0.20 * out["LTD_RobustZ"]
        + 0.10 * (out["LapTime_Delta_Change"] > 0).astype(float)
    ).astype("float32")

    out["LateStint_x_Deg"] = (out["TyreLife"] * out["DEG_RobustZ"]).astype("float32")
    out["LateStint_x_Drift"] = (out["TyreLife"] * out["LTD_RobustZ"]).astype("float32")
    out["Deg_x_Drift"] = (out["DEG_RobustZ"] * out["LTD_RobustZ"]).astype("float32")
    out["Risk_x_PositionLoss"] = (
        out["HardNeg_Specialist_Score"] * out["Position_Change"].clip(lower=0)
    ).astype("float32")

    out = out.sort_values("_row_id").drop(columns=["_row_id", "Last_Pit_Lap"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000397.",
    }, sort_keys=True))
