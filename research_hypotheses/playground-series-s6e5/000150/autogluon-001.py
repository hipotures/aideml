...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Race_Year"] = (out["Race"] + "_" + out["Year"].astype("string")).astype(
        "category"
    )
    out["Driver_Race"] = (out["Driver"] + "_" + out["Race"]).astype("category")
    out["Driver_Compound"] = (out["Driver"] + "_" + out["Compound"]).astype("category")
    out["Race_Compound"] = (out["Race"] + "_" + out["Compound"]).astype("category")
    out["Driver_Stint"] = (out["Driver"] + "_" + out["Stint"].astype("string")).astype(
        "category"
    )
    ...
    for col in [
        ...
        out[f"{col}_freq"] = out[col].map(vc).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000150.",
    }, sort_keys=True))
