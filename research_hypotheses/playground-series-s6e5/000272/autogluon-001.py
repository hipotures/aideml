...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_base = X.groupby(["Year", "Race", "LapNumber"])["LapTime (s)"].transform(
        "median"
    )
    lap_resid = X["LapTime (s)"] - lap_base
    ...
    compound_bias = X["Compound"].map(compound_map).fillna(0.0)

    obs = (
        0.40 * np.log1p(X["TyreLife"].clip(lower=0))
        + 0.25 * np.tanh(X["Cumulative_Degradation"] / 180.0)
        + 0.20 * np.tanh(X["LapTime_Delta"] / 6.0)
        + 0.15 * np.tanh(lap_resid / 6.0)
        + compound_bias
    ).to_numpy(dtype=float)

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber", "_orig_order"]
    XS = X.sort_values(sort_cols, kind="mergesort")
    idx = XS.index.to_numpy()

    stint_vals = XS["Stint"].to_numpy()
    lap_vals = XS["LapNumber"].to_numpy()
    pit_vals = XS["PitStop"].to_numpy()

    wear_level = np.zeros(len(XS), dtype=np.float32)
    wear_slope = np.zeros(len(XS), dtype=np.float32)
    wear_innov = np.zeros(len(XS), dtype=np.float32)
    wear_abs_innov = np.zeros(len(XS), dtype=np.float32)
    wear_pred = np.zeros(len(XS), dtype=np.float32)
    wear_age = np.zeros(len(XS), dtype=np.float32)
    wear_reset = np.zeros(len(XS), dtype=np.int8)

    alpha = 0.55
    beta = 0.12
    decay = 0.92

    group_indices = XS.groupby(["Year", "Race", "Driver", "Stint"], sort=False).indices
    for _, positions in group_indices.items():
        pos = np.asarray(positions, dtype=int)
        level = float(obs[idx[pos[0]]])
        slope = 0.0
        prev_lap = None
        age = 0.0
        ...
        for k, p in enumerate(pos):
            row_idx = idx[p]
            current_obs = float(obs[row_idx])

            reset_now = (
                1
                if (k == 0 or (prev_lap is not None and lap_vals[p] <= prev_lap))
                else 0
            )
            if reset_now:
                level = current_obs
                ...
                pred = level + slope
                innov = current_obs - pred
                level = pred + alpha * innov
                slope = decay * slope + beta * innov
                age += 1.0

            wear_level[p] = level
            wear_slope[p] = slope
            wear_innov[p] = innov
            wear_abs_innov[p] = abs(innov)
            wear_pred[p] = pred
            wear_age[p] = age
            wear_reset[p] = reset_now

            prev_lap = lap_vals[p]

    XS["latent_wear_level"] = wear_level
    XS["latent_wear_slope"] = wear_slope
    XS["latent_wear_innovation"] = wear_innov
    XS["latent_wear_abs_innovation"] = wear_abs_innov
    XS["latent_wear_forecast"] = wear_pred
    XS["latent_wear_age"] = wear_age
    XS["latent_state_reset"] = wear_reset.astype(np.int8)

    XS["wear_x_tyre_life"] = XS["latent_wear_level"] * XS["TyreLife"]
    XS["wear_x_deg"] = XS["latent_wear_level"] * XS["Cumulative_Degradation"]
    XS["slope_x_progress"] = XS["latent_wear_slope"] * XS["RaceProgress"]
    XS["innov_x_pit"] = XS["latent_wear_innovation"] * XS["PitStop"]

    X = XS.sort_values("_orig_order", kind="mergesort").drop(columns=["_orig_order"])
    return X
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000272"],
        "research_usage_note": "Verified assigned hypothesis 000272.",
    }, sort_keys=True))
