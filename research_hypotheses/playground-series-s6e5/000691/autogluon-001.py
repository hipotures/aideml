...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    gcols = ["Year", "Race", "LapNumber"]

    compound_map = {"SOFT": 3, "MEDIUM": 2, "HARD": 1, "INTERMEDIATE": 0, "WET": 0}
    out["Compound_Code"] = out["Compound"].map(compound_map).fillna(-1).astype("int16")
    out["Is_Wet_Compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )

    est_total_laps = out["LapNumber"] / out["RaceProgress"].clip(lower=0.01)
    ...
    out["TyreLife_Stint"] = out["TyreLife"] * out["Stint"]
    out["Degradation_per_TyreLife"] = out["Cumulative_Degradation"] / out[
        "TyreLife"
    ].clip(lower=1)
    out["LapTime_per_TyreLife"] = out["LapTime (s)"] / out["TyreLife"].clip(lower=1)
    out["PitPressure_Base"] = (
        out["TyreLife"].clip(0, 100) * 0.45
        + out["RaceProgress"].clip(0, 1) * 30
        + out["Stint"].clip(1, 10) * 4
        + out["Degradation_per_TyreLife"].clip(-50, 80) * 0.08
        - out["Position"].clip(1, 20) * 0.25
    )

    grp = out.groupby(gcols, sort=False)
    out["Lap_Field_Size"] = grp["LapNumber"].transform("size").astype("int16")
    ...
    for col in rank_features:
        ...
        out[f"{safe}_lap_z"] = ((out[col] - mean) / std).fillna(0).clip(-8, 8)
        out[f"{safe}_lap_rank_pct"] = (
            grp[col].rank(method="average", pct=True).astype("float32")
        )
        out[f"{safe}_lap_rank_desc_pct"] = (
            grp[col].rank(method="average", ascending=False, pct=True).astype("float32")
        )
        out[f"{safe}_lap_diff_mean"] = (out[col] - mean).astype("float32")

    out["Oldest_Tyre_In_Lap"] = (out["TyreLife_lap_rank_pct"] >= 0.95).astype("int8")
    out["Highest_Degradation_In_Lap"] = (
        out["Cumulative_Degradation_lap_rank_pct"] >= 0.95
    ).astype("int8")
    out["High_Pressure_In_Lap"] = (out["PitPressure_Base_lap_rank_pct"] >= 0.90).astype(
        "int8"
    )
    out["Pressure_x_OldTyreRank"] = (
        out["PitPressure_Base"] * out["TyreLife_lap_rank_pct"]
    )
    out["Pressure_x_DegradationRank"] = (
        out["PitPressure_Base"] * out["Cumulative_Degradation_lap_rank_pct"]
    )
    ...
    out["Driver_Compound"] = (
        out["Driver"].astype(str) + "_" + out["Compound"].astype(str)
    )
    out["Stint_Compound"] = out["Stint"].astype(str) + "_" + out["Compound"].astype(str)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000691.",
    }, sort_keys=True))
