...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0).astype(float)
    keys = ["Year", "Race", "LapNumber"]

    lap_count = (
        pit.groupby([out[k] for k in keys], sort=False).transform("count").astype(float)
    )
    lap_sum = (
        pit.groupby([out[k] for k in keys], sort=False).transform("sum").astype(float)
    )

    peer_n = (lap_count - 1.0).clip(lower=0.0)
    peer_pits = (lap_sum - pit).clip(lower=0.0)
    peer_rate = peer_pits / peer_n.replace(0.0, np.nan)

    out["same_lap_peer_pit_count"] = peer_pits
    out["same_lap_peer_pit_rate"] = peer_rate.fillna(0.0)
    out["same_lap_peer_pit_any"] = (peer_pits > 0).astype("int8")
    out["same_lap_peer_field_size"] = peer_n
    ...
    tyre_pressure = (
        np.log1p(tyre_life.clip(lower=0.0))
        + np.log1p(degradation.clip(lower=0.0)) / 10.0
    )
    late_race = (progress >= 0.70).astype("int8")
    slick = out["Compound"].astype(str).isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    different_fresh_slick_feasible = (
        (slick == 1) & (tyre_life >= 8) & (progress <= 0.92)
    ).astype("int8")

    out["tyre_pressure_simple"] = tyre_pressure
    out["late_race_flag"] = late_race
    out["different_fresh_slick_feasible"] = different_fresh_slick_feasible
    out["peer_pit_rate_x_tyre_pressure"] = out["same_lap_peer_pit_rate"] * tyre_pressure
    out["peer_pit_count_x_late_race"] = peer_pits * late_race
    out["peer_pit_rate_x_fresh_slick_feasible"] = (
        out["same_lap_peer_pit_rate"] * different_fresh_slick_feasible
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000040.",
    }, sort_keys=True))
