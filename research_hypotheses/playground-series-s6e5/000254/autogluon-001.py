...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["tyre_life_log1p"] = np.log1p(np.maximum(tyre, 0))
    out["stint_tyre_ratio"] = tyre / (stint + eps)
    out["tyre_progress_ratio"] = tyre / (progress * 100.0 + eps)
    out["lap_progress_ratio"] = lap / (progress * 100.0 + eps)
    out["degradation_per_tyre_lap"] = degr / (tyre + eps)
    out["abs_lap_delta"] = np.abs(delta)
    out["lap_delta_ratio"] = delta / (lap_time + eps)
    out["position_pressure"] = (21.0 - pos) * progress
    out["position_change_abs"] = np.abs(pos_chg)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000254"],
        "research_usage_note": "Verified assigned hypothesis 000254.",
    }, sort_keys=True))
