...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = [c for c in keys + ["LapNumber"] if c in out.columns]
    work = out.assign(_orig_order=orig_idx).sort_values(
        sort_cols + ["_orig_order"], kind="mergesort"
    )

    compound = work["Compound"].astype("string").str.upper()
    ...
    work["_prior_pit_count"] = grp["PitStop"].cumsum().shift(fill_value=0)
    first_in_group = grp.cumcount().eq(0)
    work.loc[first_in_group, "_prior_pit_count"] = 0
    work["_prior_pit_count"] = work["_prior_pit_count"].astype("int16")

    work["_wet_exempt"] = (
        is_wet.groupby([work[k] for k in keys], sort=False).cummax().astype("int8")
    )
    ...
    for comp in ["SOFT", "MEDIUM", "HARD"]:
        seen = (
            compound.eq(comp)
            .astype("int8")
            .groupby([work[k] for k in keys], sort=False)
            .cummax()
        )
        col = f"_seen_{comp.lower()}"
        work[col] = seen.astype("int8")
        slick_seen.append(col)

    work["_dry_slick_count_used"] = work[slick_seen].sum(axis=1).astype("int8")
    work["_dry_noncompliant_one_slick_used"] = (
        (work["_wet_exempt"] == 0) & (work["_dry_slick_count_used"] == 1)
    ).astype("int8")
    work["_dry_compliant"] = (
        (work["_wet_exempt"] == 1) | (work["_dry_slick_count_used"] >= 2)
    ).astype("int8")

    race_progress = work["RaceProgress"].replace(0, np.nan).astype(float)
    est_total_laps = (work["LapNumber"].astype(float) / race_progress).replace(
        [np.inf, -np.inf], np.nan
    )
    work["_laps_remaining_rule"] = (
        (est_total_laps - work["LapNumber"].astype(float)).clip(lower=0).fillna(0)
    )

    tyre_life = work["TyreLife"].astype(float).clip(lower=0)
    stint = work["Stint"].astype(float)
    laps_remaining = work["_laps_remaining_rule"]

    work["_current_tyre_can_finish_but_owes_compound_change"] = (
        (work["_dry_noncompliant_one_slick_used"] == 1)
        & (is_slick == 1)
        & (laps_remaining <= tyre_life)
    ).astype("int8")

    work["_legal_laps_remaining_x_one_slick"] = (
        laps_remaining * work["_dry_noncompliant_one_slick_used"]
    )
    work["_legal_tyre_life_x_one_slick"] = (
        tyre_life * work["_dry_noncompliant_one_slick_used"]
    )
    work["_legal_stint_x_one_slick"] = stint * work["_dry_noncompliant_one_slick_used"]
    work["_legal_prior_pits_x_one_slick"] = (
        work["_prior_pit_count"] * work["_dry_noncompliant_one_slick_used"]
    )
    work["_wet_exempt_x_laps_remaining"] = laps_remaining * work["_wet_exempt"]
    work["_dry_compliant_x_tyre_life"] = tyre_life * work["_dry_compliant"]
    work["_owed_change_pressure"] = work["_dry_noncompliant_one_slick_used"] * (
        1.0 / (laps_remaining + 1.0)
    )

    added = [
        "_prior_pit_count",
        "_wet_exempt",
        "_dry_slick_count_used",
        "_dry_noncompliant_one_slick_used",
        "_dry_compliant",
        "_current_tyre_can_finish_but_owes_compound_change",
        "_laps_remaining_rule",
        "_legal_laps_remaining_x_one_slick",
        "_legal_tyre_life_x_one_slick",
        "_legal_stint_x_one_slick",
        "_legal_prior_pits_x_one_slick",
        "_wet_exempt_x_laps_remaining",
        "_dry_compliant_x_tyre_life",
        "_owed_change_pressure",
    ]

    work = work.sort_values("_orig_order", kind="mergesort")
    for col in added:
        out[col[1:]] = work[col].to_numpy()
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000420.",
    }, sort_keys=True))
