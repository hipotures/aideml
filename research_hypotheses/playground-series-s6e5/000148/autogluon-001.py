...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound_stint_keys = ["Compound", "Stint"]

    race_max_lap = X.groupby(race_keys)["LapNumber"].transform("max").astype(float)
    X["RaceMaxLap"] = race_max_lap
    X["LapsRemaining"] = (race_max_lap - X["LapNumber"]).clip(lower=0)
    X["LapFrac"] = X["LapNumber"] / race_max_lap.replace(0, np.nan)
    X["TyreLifeFracRace"] = X["TyreLife"] / race_max_lap.replace(0, np.nan)
    X["TyreLifeFracLap"] = X["TyreLife"] / X["LapNumber"].replace(0, np.nan)
    X["TyreLifeFracRemaining"] = X["TyreLife"] / (X["LapsRemaining"] + 1.0)

    X["TyreLifePctByCompound"] = X.groupby(compound_keys)["TyreLife"].rank(pct=True)
    X["TyreLifePctByCompoundStint"] = X.groupby(compound_stint_keys)["TyreLife"].rank(
        pct=True
    )
    X["DegPctByCompound"] = X.groupby(compound_keys)["Cumulative_Degradation"].rank(
        pct=True
    )
    X["LapDeltaPctByCompound"] = X.groupby(compound_keys)["LapTime_Delta"].rank(
        pct=True
    )

    X["RaceProgressBinQ"] = pd.qcut(
        X["RaceProgress"], q=4, labels=False, duplicates="drop"
    ).astype("float")
    X["TyreLifeBinQ"] = pd.qcut(
        X["TyreLife"], q=4, labels=False, duplicates="drop"
    ).astype("float")
    X["LapsRemainingBinQ"] = pd.qcut(
        X["LapsRemaining"], q=4, labels=False, duplicates="drop"
    ).astype("float")
    X["DegBinQ"] = pd.qcut(
        X["Cumulative_Degradation"], q=4, labels=False, duplicates="drop"
    ).astype("float")

    X["HazardClock"] = (
        0.35 * X["TyreLifePctByCompound"].fillna(0.5)
        + 0.25 * X["TyreLifePctByCompoundStint"].fillna(0.5)
        + 0.20 * X["DegPctByCompound"].fillna(0.5)
        + 0.20 * X["RaceProgress"].fillna(0.5)
    )

    X["HazardClockBinQ"] = pd.qcut(
        X["HazardClock"], q=4, labels=False, duplicates="drop"
    ).astype("float")
    X["NearFinishLateStint"] = (X["LapsRemaining"] <= 5).astype(int) * (
        X["TyreLifePctByCompound"] >= 0.75
    ).astype(int)
    X["LateStintHighDeg"] = (X["TyreLifePctByCompoundStint"] >= 0.75).astype(int) * (
        X["DegPctByCompound"] >= 0.75
    ).astype(int)

    X["Deg_x_TyreLife"] = X["Cumulative_Degradation"] * X["TyreLife"]
    X["DegPerLapTyre"] = X["Cumulative_Degradation"] / X["TyreLife"].replace(0, np.nan)
    X["LapDelta_x_TyreLife"] = X["LapTime_Delta"] * X["TyreLife"]
    X["LapDelta_x_Deg"] = X["LapTime_Delta"] * X["Cumulative_Degradation"]
    X["PosChange_x_LapDelta"] = X["Position_Change"] * X["LapTime_Delta"]
    X["PosPressure"] = X["Position"] / 20.0
    X["GainedPlacesFlag"] = (X["Position_Change"] > 0).astype(int)
    X["LostPlacesFlag"] = (X["Position_Change"] < 0).astype(int)

    X["Compound_Stint"] = (
        X["Compound"].astype(str) + "_S" + X["Stint"].astype(str)
    ).astype("category")
    X["Race_Compound"] = (
        X["Race"].astype(str) + "_" + X["Compound"].astype(str)
    ).astype("category")
    X["HazardState"] = (
        X["Compound"].astype(str)
        + "_H"
        + X["HazardClockBinQ"].fillna(-1).astype(int).astype(str)
        + "_R"
        + X["RaceProgressBinQ"].fillna(-1).astype(int).astype(str)
    ).astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000148.",
    }, sort_keys=True))
