...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = ["Driver", "Race", "Year", "Compound", "Stint"]
    for col in cat_cols:
        out[col] = out[col].astype("string").fillna("__MISSING__")

    out["Driver__Compound"] = out["Driver"] + "__" + out["Compound"]
    out["Race__Compound"] = out["Race"] + "__" + out["Compound"]
    out["Race__Stint"] = out["Race"] + "__" + out["Stint"]
    out["Race__Year"] = out["Race"] + "__" + out["Year"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000432.",
    }, sort_keys=True))
