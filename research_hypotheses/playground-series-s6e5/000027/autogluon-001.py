...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected = out["Compound"].map(expected_life_map).fillna(28.0).astype(float)
    tyre_life = out["TyreLife"].astype(float)
    race_progress = out["RaceProgress"].astype(float)

    out["CompoundExpectedLife"] = expected
    out["TyreLifeExpectedRatio"] = tyre_life / expected.clip(lower=1.0)
    out["TyreLifeRemainingExpected"] = expected - tyre_life
    out["TyreLifeOverExpected"] = np.maximum(tyre_life - expected, 0.0)
    out["TyreOldForCompound"] = (tyre_life >= 0.80 * expected).astype("int8")
    out["TyreVeryOldForCompound"] = (tyre_life >= expected).astype("int8")
    out["TyreRatio_x_RaceProgress"] = out["TyreLifeExpectedRatio"] * race_progress
    out["RemainingExpected_x_RaceProgress"] = (
        out["TyreLifeRemainingExpected"] * race_progress
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000027.",
    }, sort_keys=True))
