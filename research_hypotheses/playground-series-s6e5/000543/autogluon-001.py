...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound"] = out["Compound"].astype("string").str.upper().fillna("UNKNOWN")
    out["Race"] = out["Race"].astype("string").fillna("UNKNOWN")
    out["Driver"] = out["Driver"].astype("string").fillna("UNKNOWN")
    ...
    out["cr_compound_plan_life"] = (
        out["Compound"].map(compound_life).fillna(28.0).astype(float)
    )

    lap = out["LapNumber"].astype(float)
    progress = out["RaceProgress"].astype(float).clip(0.01, 1.0)
    tyre = out["TyreLife"].astype(float).clip(lower=1.0)
    remaining = (lap / progress - lap).clip(lower=0.0, upper=120.0)

    out["cr_est_total_laps"] = (lap + remaining).clip(1.0, 120.0)
    out["cr_laps_to_finish"] = remaining
    out["cr_finish_1lap"] = (remaining <= 1.05).astype("int8")
    out["cr_finish_3laps"] = (remaining <= 3.05).astype("int8")
    out["cr_finish_5laps"] = (remaining <= 5.05).astype("int8")

    out["cr_tyre_life_ratio"] = tyre / (out["cr_compound_plan_life"] + eps)
    out["cr_expected_life_left"] = out["cr_compound_plan_life"] - tyre
    out["cr_finish_margin"] = out["cr_expected_life_left"] - remaining
    out["cr_can_finish_expected"] = (out["cr_finish_margin"] >= 0).astype("int8")
    out["cr_must_extend_to_finish"] = (-out["cr_finish_margin"]).clip(lower=0.0)
    out["cr_old_tyre_near_finish"] = out["cr_tyre_life_ratio"] * (
        1.0 / (1.0 + remaining)
    )

    deg = out["Cumulative_Degradation"].astype(float)
    delta = out["LapTime_Delta"].astype(float)
    out["cr_deg_per_tyre_lap"] = deg / tyre
    out["cr_signed_log_deg"] = np.sign(deg) * np.log1p(np.abs(deg))
    out["cr_signed_log_delta"] = np.sign(delta) * np.log1p(np.abs(delta))
    out["cr_degradation_finish_load"] = out["cr_deg_per_tyre_lap"] * remaining

    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    tmp = out[sort_cols + ["PitStop"]].copy()
    tmp["_row"] = np.arange(len(out))
    tmp = tmp.sort_values(sort_cols + ["_row"], kind="mergesort")
    grp = tmp.groupby(["Year", "Race", "Driver"], sort=False)

    tmp["cr_prev_pit"] = grp["PitStop"].shift(1).fillna(0).astype("int8")
    tmp["cr_prev2_pit"] = grp["PitStop"].shift(2).fillna(0).astype("int8")
    tmp["cr_prior_pit_count"] = (grp["PitStop"].cumsum() - tmp["PitStop"]).clip(lower=0)
    last_pit_lap = (
        tmp["LapNumber"]
        .where(tmp["PitStop"].eq(1))
        .groupby([tmp["Year"], tmp["Race"], tmp["Driver"]], sort=False)
        .ffill()
    )
    tmp["cr_laps_since_seq_pit"] = (
        (tmp["LapNumber"] - last_pit_lap).fillna(tmp["LapNumber"]).clip(lower=0)
    )
    ...
    for col in [
        ...
        "cr_prev2_pit",
        "cr_prior_pit_count",
        "cr_laps_since_seq_pit",
    ]:
        out[col] = tmp[col].to_numpy()

    pit_now = out["PitStop"].astype(float)
    tyre_pressure = out["cr_tyre_life_ratio"].clip(0.0, 4.0)
    bad_delta = out["cr_signed_log_delta"].clip(-4.0, 4.0)
    cannot_finish = (out["cr_finish_margin"] < 0).astype(float)

    pit_score = (
        1.25 * tyre_pressure
        + 0.45 * cannot_finish
        + 0.20 * out["Stint"].astype(float)
        + 0.04 * out["Position"].astype(float)
        + 0.10 * bad_delta
        - 1.80 * out["cr_finish_1lap"].astype(float)
        - 1.20 * pit_now
        - 0.70 * out["cr_prev_pit"].astype(float)
    )
    finish_score = (
        1.60 * out["cr_can_finish_expected"].astype(float)
        + 1.20 * out["cr_finish_3laps"].astype(float)
        + 0.70 * out["cr_finish_5laps"].astype(float)
        - 0.55 * cannot_finish
        - 0.25 * tyre_pressure
    )

    out["cr_pit_hazard_proxy"] = 1.0 / (1.0 + np.exp(-pit_score.clip(-20.0, 20.0)))
    out["cr_finish_without_pit_proxy"] = 1.0 / (
        1.0 + np.exp(-finish_score.clip(-20.0, 20.0))
    )
    out["cr_pit_vs_finish_margin"] = (
        out["cr_pit_hazard_proxy"] - out["cr_finish_without_pit_proxy"]
    )
    out["cr_one_lap_cif_proxy"] = out["cr_pit_hazard_proxy"] * (
        1.0 - out["cr_finish_without_pit_proxy"]
    )
    out["cr_finish_without_further_pit_label_proxy"] = (
        (out["cr_can_finish_expected"].eq(1)) & (remaining <= 5.05)
    ).astype("int8")

    out["cr_compound_phase"] = (
        out["Compound"].astype(str)
        + "_"
        + np.select(
            [remaining <= 3.05, remaining <= 10.05, progress >= 0.66],
            ["finish", "late", "final_third"],
            default="early_mid",
        )
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000543"],
        "research_usage_note": "Verified assigned hypothesis 000543.",
    }, sort_keys=True))
