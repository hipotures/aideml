...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    slick_map = {"SOFT": 1, "MEDIUM": 2, "HARD": 4}
    slick_bit = compound.map(slick_map).fillna(0).astype(np.int16)
    is_slick = slick_bit.ne(0).astype(np.int8)
    is_wet_compound = compound.isin(["INTERMEDIATE", "WET"]).astype(np.int8)

    order_cols = ["Year", "Race", "Driver", "LapNumber"]
    sidx = out.sort_values(order_cols, kind="mergesort").index
    gkeys = [out.loc[sidx, "Year"], out.loc[sidx, "Race"], out.loc[sidx, "Driver"]]

    pit_s = out.loc[sidx, "PitStop"].fillna(0).astype(np.int8)
    stint_s = out.loc[sidx, "Stint"].astype(float)
    tyre_life_s = out.loc[sidx, "TyreLife"].astype(float)
    progress_s = out.loc[sidx, "RaceProgress"].astype(float)
    lap_s = out.loc[sidx, "LapNumber"].astype(float)
    slick_s = is_slick.loc[sidx]
    wet_s = is_wet_compound.loc[sidx]
    bit_s = slick_bit.loc[sidx]

    stops_so_far = pit_s.groupby(gkeys, sort=False).cumsum().astype(np.int16)
    wet_used_so_far = wet_s.groupby(gkeys, sort=False).cummax().astype(np.int8)

    hard_used = bit_s.eq(4).groupby(gkeys, sort=False).cummax().astype(np.int8)
    medium_used = bit_s.eq(2).groupby(gkeys, sort=False).cummax().astype(np.int8)
    soft_used = bit_s.eq(1).groupby(gkeys, sort=False).cummax().astype(np.int8)
    slick_compounds_used = (hard_used + medium_used + soft_used).astype(np.int8)

    new_stint = stint_s.ne(stint_s.groupby(gkeys, sort=False).shift()).fillna(True)
    slick_set_start = (new_stint & slick_s.eq(1)).astype(np.int8)
    slick_sets_used = (
        slick_set_start.groupby(gkeys, sort=False).cumsum().clip(0, 8).astype(np.int8)
    )

    standard_rule = ((year >= 2022) & race.ne("Pre-Season Testing")).astype(np.int8)
    monaco_2025 = ((year == 2025) & race.eq("Monaco Grand Prix")).astype(np.int8)
    qatar_2023 = ((year == 2023) & race.eq("Qatar Grand Prix")).astype(np.int8)

    std_active = (
        standard_rule.eq(1)
        & monaco_2025.eq(0)
        & wet_used_so_far.reindex(out.index).eq(0)
    )
    mono_active = monaco_2025.eq(1) & wet_used_so_far.reindex(out.index).eq(0)

    out["rule_slick_compounds_used_so_far"] = slick_compounds_used.reindex(
        out.index
    ).astype(np.int8)
    out["rule_slick_sets_used_so_far"] = slick_sets_used.reindex(out.index).astype(
        np.int8
    )
    out["rule_pitstops_so_far"] = stops_so_far.reindex(out.index).astype(np.int16)
    out["rule_wet_exemption_used_so_far"] = wet_used_so_far.reindex(out.index).astype(
        np.int8
    )

    std_debt = (2 - out["rule_slick_compounds_used_so_far"]).clip(lower=0)
    mono_stop_debt = (2 - out["rule_pitstops_so_far"]).clip(lower=0)
    mono_set_debt = (3 - out["rule_slick_sets_used_so_far"]).clip(lower=0)

    out["rule_standard_compound_debt"] = np.where(std_active, std_debt, 0).astype(
        np.int8
    )
    out["rule_monaco_2025_stop_debt"] = np.where(mono_active, mono_stop_debt, 0).astype(
        np.int8
    )
    out["rule_monaco_2025_set_debt"] = np.where(mono_active, mono_set_debt, 0).astype(
        np.int8
    )
    out["rule_mandatory_stop_debt"] = np.maximum(
        out["rule_standard_compound_debt"],
        np.maximum(out["rule_monaco_2025_stop_debt"], out["rule_monaco_2025_set_debt"]),
    ).astype(np.int8)

    out["rule_minimum_stops_remaining"] = out["rule_mandatory_stop_debt"].astype(
        np.int8
    )
    out["rule_satisfied"] = (out["rule_mandatory_stop_debt"].eq(0)).astype(np.int8)

    remaining_laps_est = (
        out["LapNumber"].astype(float)
        * (1.0 - out["RaceProgress"].astype(float))
        / out["RaceProgress"].clip(0.01, 1.0)
    ).clip(0, 80)
    out["rule_remaining_laps_est"] = remaining_laps_est.astype(np.float32)
    out["rule_laps_per_required_stop"] = (
        (remaining_laps_est / out["rule_minimum_stops_remaining"].replace(0, np.nan))
        .fillna(99)
        .clip(0, 99)
        .astype(np.float32)
    )

    qatar_active = qatar_2023.eq(1)
    max_stint_cap = 18.0
    out["rule_qatar_2023_max_stint_margin"] = np.where(
        qatar_active, max_stint_cap - out["TyreLife"].astype(float), 99.0
    ).astype(np.float32)
    out["rule_qatar_2023_breach_risk"] = np.where(
        qatar_active,
        (out["TyreLife"].astype(float) >= max_stint_cap - 1).astype(np.int8),
        0,
    ).astype(np.int8)

    legal_paths = np.where(
        mono_active,
        (
            (out["rule_pitstops_so_far"] <= 2)
            & (out["rule_slick_sets_used_so_far"] <= 3)
        ).astype(np.int8),
        np.where(
            std_active,
            (out["rule_slick_compounds_used_so_far"] <= 3).astype(np.int8),
            1,
        ),
    )
    out["rule_remaining_legal_path_exists"] = legal_paths.astype(np.int8)

    out["rule_debt_pressure"] = (
        (out["rule_mandatory_stop_debt"].astype(float) / (remaining_laps_est + 1.0))
        .clip(0, 3)
        .astype(np.float32)
    )
    out["rule_late_unsatisfied"] = (
        (out["RaceProgress"].astype(float) >= 0.75) & (out["rule_satisfied"].eq(0))
    ).astype(np.int8)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000795"],
        "research_usage_note": "Verified assigned hypothesis 000795.",
    }, sort_keys=True))
