...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound_map = {"WET": 0, "INTERMEDIATE": 1, "HARD": 2, "MEDIUM": 3, "SOFT": 4}
    X["Compound_Code"] = X["Compound"].map(compound_map).astype("float32")
    X["Is_WetLike"] = X["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber", "_row_order"]
    X = X.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)

    gid = X.groupby(["Year", "Race", "Driver", "Stint"], sort=False).ngroup()
    grp = X.groupby(gid, sort=False)

    tyre_life = X["TyreLife"].astype("float32").clip(lower=1.0)
    ...
    pace_obs = (
        ...
        + 0.03 * cum_deg
        + 0.15 * tyre_life
        - 0.25 * X["Position_Change"].astype("float32")
    ).astype("float32")

    latent_pace = (
        pace_obs.groupby(gid)
    ...
    latent_delta = (
        lap_delta.groupby(gid)
        .ewm(alpha=0.40, adjust=False)
        .mean()
        .reset_index(level=0, drop=True)
        .astype("float32")
    )
    ...
    pace_slope = grp[
        latent_pace.name if hasattr(latent_pace, "name") else None
    ].transform(lambda s: s)
    pace_slope = latent_pace.groupby(gid).diff().fillna(latent_delta).astype("float32")
    pace_accel = pace_slope.groupby(gid).diff().fillna(0.0).astype("float32")
    deg_slope = latent_deg.groupby(gid).diff().fillna(0.0).astype("float32")
    deg_accel = deg_slope.groupby(gid).diff().fillna(0.0).astype("float32")

    prev_level = latent_pace.groupby(gid).shift(1)
    prev_slope = pace_slope.groupby(gid).shift(1)
    pred_next = (prev_level + prev_slope).astype("float32")
    innovation = (
        (pace_obs - pred_next)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
    ...
    X["Latent_Pace_Level"] = latent_pace
    X["Latent_Pace_Slope"] = pace_slope
    X["Latent_Pace_Accel"] = pace_accel
    X["Latent_Degradation_Level"] = latent_deg
    X["Latent_Degradation_Slope"] = deg_slope
    X["Latent_Degradation_Accel"] = deg_accel
    X["Latent_Innovation"] = innovation
    X["Latent_Pace_x_Degradation"] = (latent_pace * latent_deg).astype("float32")
    X["Latent_Slope_x_TyreLife"] = (pace_slope * tyre_life).astype("float32")
    X["Latent_Innovation_Abs"] = innovation.abs().astype("float32")
    X["Latent_Innovation_Sq"] = (innovation**2).astype("float32")

    X = (
        X.sort_values("_row_order", kind="mergesort")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000894"],
        "research_usage_note": "Verified assigned hypothesis 000894.",
    }, sort_keys=True))
