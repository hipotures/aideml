...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["same_lap_field_size"] = field_size

    for col, name in rel_cols.items():
        vals = out[col].astype("float32")
        pct = g[col].rank(method="average", pct=True).astype("float32")
        med = g[col].transform("median").astype("float32")
        std = g[col].transform("std").astype("float32").replace(0, np.nan)

        out[f"{name}_same_lap_pct"] = pct
        out[f"{name}_same_lap_pct_centered"] = (pct - 0.5).astype("float32")
        out[f"{name}_same_lap_med_diff"] = (vals - med).astype("float32")
        out[f"{name}_same_lap_z"] = (
            ((vals - med) / std)
    ...
    traffic = out[keys + ["Position"] + traffic_cols].copy()
    traffic["_row_id"] = np.arange(n, dtype=np.int64)
    traffic = traffic.sort_values(keys + ["Position"], kind="mergesort")
    tg = traffic.groupby(keys, sort=False, observed=True)
    row_ids = traffic["_row_id"].to_numpy()

    nearby_count = np.zeros(n, dtype=np.float32)
    fresher_count = np.zeros(n, dtype=np.float32)
    lower_deg_count = np.zeros(n, dtype=np.float32)
    faster_delta_count = np.zeros(n, dtype=np.float32)

    for offset in (-2, -1, 1, 2):
        shifted = tg[["Position"] + traffic_cols].shift(-offset)
        valid = (
            (shifted["Position"] - traffic["Position"])
            .eq(offset)
            .fillna(False)
            .to_numpy()
        )
        label = "ahead" + str(abs(offset)) if offset < 0 else "behind" + str(offset)

        nearby_count[row_ids] += valid.astype(np.float32)
        fresher_count[row_ids] += (
            valid & (shifted["TyreLife"].to_numpy() < traffic["TyreLife"].to_numpy())
        ).astype(np.float32)
        lower_deg_count[row_ids] += (
            valid
            & (
                shifted["Cumulative_Degradation"].to_numpy()
                < traffic["Cumulative_Degradation"].to_numpy()
            )
        ).astype(np.float32)
        faster_delta_count[row_ids] += (
            valid
            & (
                shifted["LapTime_Delta"].to_numpy()
                < traffic["LapTime_Delta"].to_numpy()
            )
        ).astype(np.float32)

        for col in traffic_cols:
            arr = np.full(n, np.nan, dtype=np.float32)
            diff = traffic[col].to_numpy(dtype=np.float32) - shifted[col].to_numpy(
                dtype=np.float32
            )
            arr[row_ids] = np.where(valid, diff, np.nan).astype(np.float32)
            out[f"{traffic_names[col]}_gap_to_{label}"] = arr

    out["nearby_rival_count_pm2"] = nearby_count
    out["nearby_fresher_tyre_count_pm2"] = fresher_count
    out["nearby_lower_deg_count_pm2"] = lower_deg_count
    out["nearby_faster_delta_count_pm2"] = faster_delta_count
    out["nearby_fresher_tyre_frac_pm2"] = np.divide(
        fresher_count,
        nearby_count,
        out=np.zeros(n, dtype=np.float32),
        where=nearby_count > 0,
    )
    out["nearby_lower_deg_frac_pm2"] = np.divide(
        lower_deg_count,
        nearby_count,
        out=np.zeros(n, dtype=np.float32),
        where=nearby_count > 0,
    ...
    out.attrs["research_hypotheses_llm_claimed_used"] = ["000836"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000836"],
        "research_usage_note": "Verified assigned hypothesis 000836.",
    }, sort_keys=True))
