...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out[base_cols].copy()
    work["_row_id"] = np.arange(n)

    work = work.sort_values(
        race_keys + ["Driver", "LapNumber", "Position"], kind="mergesort"
    )
    dg = work.groupby(race_keys + ["Driver"], sort=False)
    prev_lap = dg["LapNumber"].shift(1)
    is_last_lap = work["LapNumber"].sub(prev_lap).eq(1)
    work["_own_pit_prev"] = (
        dg["PitStop"].shift(1).where(is_last_lap, 0).fillna(0).astype("int8")
    )

    snap = work.sort_values(snap_keys + ["Position", "Driver"], kind="mergesort")
    sg = snap.groupby(snap_keys, sort=False)

    feat = pd.DataFrame({"_row_id": snap["_row_id"].to_numpy()})
    pos = snap["Position"]
    own_tyre = snap["TyreLife"].astype("float32")
    own_stint = snap["Stint"].astype("float32")
    own_prev_pit = snap["_own_pit_prev"].astype("int8")
    progress = snap["RaceProgress"].astype("float32")

    feat["own_pitted_prev_lap"] = own_prev_pit.to_numpy(dtype="int8")

    tyre_series = [own_tyre]
    stint_series = [own_stint]
    pit_prev = {}
    tyre_vals = {}

    offsets = [(-2, "ahead2"), (-1, "ahead1"), (1, "behind1"), (2, "behind2")]
    for off, name in offsets:
        shift_n = -off
        npos = sg["Position"].shift(shift_n)
        valid = npos.eq(pos + off)

        n_tyre = sg["TyreLife"].shift(shift_n).where(valid)
        n_stint = sg["Stint"].shift(shift_n).where(valid)
        n_comp = sg["Compound"].shift(shift_n).where(valid, "NONE").fillna("NONE")
        n_prev_pit = (
            sg["_own_pit_prev"].shift(shift_n).where(valid, 0).fillna(0).astype("int8")
        )

        feat[f"nbr_{name}_exists"] = valid.astype("int8").to_numpy()
        feat[f"nbr_{name}_tyre_life"] = n_tyre.fillna(0).astype("float32").to_numpy()
        feat[f"nbr_{name}_stint"] = n_stint.fillna(0).astype("float32").to_numpy()
        feat[f"nbr_{name}_compound"] = n_comp.astype("object").to_numpy()
        feat[f"nbr_{name}_same_compound"] = (
            n_comp.eq(snap["Compound"]).where(valid, False).astype("int8").to_numpy()
        )
        feat[f"nbr_{name}_tyre_delta"] = (
            (own_tyre - n_tyre).where(valid, 0).astype("float32").to_numpy()
        )
        feat[f"nbr_{name}_stint_delta"] = (
            (own_stint - n_stint).where(valid, 0).astype("float32").to_numpy()
        )
        feat[f"nbr_{name}_pitted_prev_lap"] = n_prev_pit.to_numpy(dtype="int8")

        tyre_series.append(n_tyre.astype("float32"))
        stint_series.append(n_stint.astype("float32"))
        pit_prev[name] = n_prev_pit.astype("float32")
        tyre_vals[name] = n_tyre.astype("float32")

    tyre_mat = np.vstack([s.to_numpy(dtype="float32") for s in tyre_series]).T
    stint_mat = np.vstack([s.to_numpy(dtype="float32") for s in stint_series]).T

    feat["local_tyre_life_std"] = np.nanstd(tyre_mat, axis=1).astype("float32")
    feat["local_tyre_life_range"] = (
        np.nanmax(tyre_mat, axis=1) - np.nanmin(tyre_mat, axis=1)
    ).astype("float32")
    feat["local_stint_std"] = np.nanstd(stint_mat, axis=1).astype("float32")
    feat["local_stint_range"] = (
        np.nanmax(stint_mat, axis=1) - np.nanmin(stint_mat, axis=1)
    ).astype("float32")

    ahead_prev = pit_prev["ahead1"] + pit_prev["ahead2"]
    behind_prev = pit_prev["behind1"] + pit_prev["behind2"]
    adjacent_prev = pit_prev["ahead1"] + pit_prev["behind1"]

    pit_window = progress.between(0.12, 0.95)
    not_just_pitted = own_prev_pit.eq(0)

    feat["ahead_pitted_prev_count"] = ahead_prev.clip(0, 2).astype("int8").to_numpy()
    feat["behind_pitted_prev_count"] = behind_prev.clip(0, 2).astype("int8").to_numpy()
    feat["adjacent_any_pitted_prev_lap"] = (adjacent_prev > 0).astype("int8").to_numpy()

    ahead_pressure = pit_prev["ahead1"] * (own_tyre - tyre_vals["ahead1"]).clip(
        lower=0
    ).fillna(0) + 0.5 * pit_prev["ahead2"] * (own_tyre - tyre_vals["ahead2"]).clip(
        lower=0
    ).fillna(
        0
    )
    behind_pressure = pit_prev["behind1"] * (own_tyre - tyre_vals["behind1"]).clip(
        lower=0
    ).fillna(0) + 0.5 * pit_prev["behind2"] * (own_tyre - tyre_vals["behind2"]).clip(
        lower=0
    ).fillna(
        0
    )

    feat["ahead_cover_window_open"] = (
        ((ahead_prev > 0) & not_just_pitted & pit_window).astype("int8").to_numpy()
    )
    feat["behind_undercut_window_open"] = (
        ((behind_prev > 0) & not_just_pitted & pit_window).astype("int8").to_numpy()
    )
    feat["ahead_cover_pressure"] = (
        ahead_pressure.where(not_just_pitted & pit_window, 0)
        .astype("float32")
        .to_numpy()
    )
    feat["behind_undercut_pressure"] = (
        behind_pressure.where(not_just_pitted & pit_window, 0)
        .astype("float32")
        .to_numpy()
    )
    feat["same_team_double_stack_proxy"] = (
        ((adjacent_prev > 0) & not_just_pitted & pit_window).astype("int8").to_numpy()
    )

    feat = feat.sort_values("_row_id", kind="mergesort").drop(columns="_row_id")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000367"],
        "research_usage_note": "Verified assigned hypothesis 000367.",
    }, sort_keys=True))
