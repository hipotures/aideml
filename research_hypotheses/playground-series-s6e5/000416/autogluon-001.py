...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col, name in [
        ("TyreLife", "lap_tyre_life_pct"),
        ("Cumulative_Degradation", "lap_degradation_pct"),
        ("LapTime_Delta", "lap_delta_pct"),
    ]:
        out[name] = (
    ...
    out["lap_tyre_life_mean"] = g["TyreLife"].transform("mean").astype("float32")
    out["lap_degradation_mean"] = (
        g["Cumulative_Degradation"].transform("mean").astype("float32")
    )
    out["lap_delta_median"] = g["LapTime_Delta"].transform("median").astype("float32")
    out["lap_field_size"] = g["Position"].transform("count").astype("int16")

    out["rel_tyre_life_vs_lap"] = (tyre - out["lap_tyre_life_mean"]).astype("float32")
    out["rel_degradation_vs_lap"] = (degr - out["lap_degradation_mean"]).astype(
        "float32"
    )
    out["rel_delta_vs_lap_median"] = (delta - out["lap_delta_median"]).astype("float32")
    ...
    tmp = out[["TyreLife", "Cumulative_Degradation", "LapTime_Delta", "PitStop"]].copy()
    tmp["_key"] = key.values
    tmp["_pos"] = out["Position"].astype(int).values
    tmp = tmp.set_index(["_key", "_pos"])

    idx_key = key.values
    idx_pos = out["Position"].astype(int).values

    older_ahead = np.zeros(len(out), dtype=np.float32)
    fresher_behind = np.zeros(len(out), dtype=np.float32)
    local_delta_sum = np.zeros(len(out), dtype=np.float32)
    local_delta_n = np.zeros(len(out), dtype=np.float32)
    local_pitstop_sum = np.zeros(len(out), dtype=np.float32)
    clear_air_slots = np.zeros(len(out), dtype=np.float32)
    neighbor_degr_sum = np.zeros(len(out), dtype=np.float32)
    neighbor_degr_n = np.zeros(len(out), dtype=np.float32)

    for offset in (-2, -1, 1, 2):
        neigh_pos = idx_pos + offset
        neigh = tmp.reindex(pd.MultiIndex.from_arrays([idx_key, neigh_pos]))
        n_tyre = neigh["TyreLife"].to_numpy(dtype=np.float32)
        n_degr = neigh["Cumulative_Degradation"].to_numpy(dtype=np.float32)
        n_delta = neigh["LapTime_Delta"].to_numpy(dtype=np.float32)
        n_pit = neigh["PitStop"].to_numpy(dtype=np.float32)
        present = ~np.isnan(n_tyre)

        if offset < 0:
            older_ahead += (
                (n_tyre > tyre.to_numpy(dtype=np.float32)) & present
            ).astype(np.float32)
        else:
            fresher_behind += (
                (n_tyre < tyre.to_numpy(dtype=np.float32)) & present
        ...
        local_delta_sum += np.where(present, n_delta, 0.0).astype(np.float32)
        local_delta_n += present.astype(np.float32)
        local_pitstop_sum += np.where(present, n_pit, 0.0).astype(np.float32)
        neighbor_degr_sum += np.where(present, n_degr, 0.0).astype(np.float32)
        neighbor_degr_n += present.astype(np.float32)
        clear_air_slots += (~present).astype(np.float32)

    out["older_tyre_cars_ahead_pos2"] = older_ahead
    out["fresher_tyre_cars_behind_pos2"] = fresher_behind
    out["local_position_delta_mean_pos2"] = np.divide(
        local_delta_sum,
        local_delta_n,
        out=np.zeros(len(out), dtype=np.float32),
        where=local_delta_n > 0,
    )
    out["local_position_degradation_mean_pos2"] = np.divide(
        neighbor_degr_sum,
        neighbor_degr_n,
        out=np.zeros(len(out), dtype=np.float32),
        where=neighbor_degr_n > 0,
    )
    out["nearby_current_pitstops_pos2"] = local_pitstop_sum
    out["position_clear_air_slots_pos2"] = clear_air_slots
    out["undercut_pressure_pos2"] = (
        out["older_tyre_cars_ahead_pos2"]
        + out["fresher_tyre_cars_behind_pos2"]
        + out["lap_tyre_life_pct"]
    ).astype("float32")
    out["clear_air_if_pit_proxy"] = (
        clear_air_slots
        + (pos >= 16).to_numpy(dtype=np.float32)
        + (out["RaceProgress"].astype(float).to_numpy(dtype=np.float32) > 0.35).astype(
            np.float32
        )
        - local_pitstop_sum
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000416"],
        "research_usage_note": "Verified assigned hypothesis 000416.",
    }, sort_keys=True))
