...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wet = out["Compound"].isin(["INTERMEDIATE", "WET"])

    group_cols = ["Year", "Race", "Driver"]
    order_cols = group_cols + ["LapNumber"]
    s = out[order_cols + ["Compound", "Stint", "TyreLife", "RaceProgress"]].copy()
    s["_idx"] = np.arange(len(s))
    s["_dry"] = dry.astype("int8").to_numpy()
    s["_wet"] = wet.astype("int8").to_numpy()
    s = s.sort_values(order_cols + ["_idx"], kind="mergesort")

    g = s.groupby(group_cols, sort=False)
    s["_sets_used"] = g["Stint"].cummax().astype("int16")
    s["_wet_seen"] = g["_wet"].cummax().astype("int8")

    dry_compounds = ["SOFT", "MEDIUM", "HARD"]
    distinct_dry = np.zeros(len(s), dtype=np.int8)
    for comp in dry_compounds:
        seen = s["Compound"].eq(comp).astype("int8")
        distinct_dry += (
            seen.groupby([s[c] for c in group_cols], sort=False)
            .cummax()
            .to_numpy(dtype=np.int8)
        )
    s["_distinct_dry_compounds_used"] = distinct_dry

    total_laps = (
        (s["LapNumber"] / s["RaceProgress"].clip(lower=0.01))
        .round()
        .clip(lower=s["LapNumber"])
    )
    s["_laps_remaining_est"] = (
        (total_laps - s["LapNumber"]).clip(lower=0).astype("float32")
    )

    dry_debt = np.maximum(0, 2 - s["_distinct_dry_compounds_used"].to_numpy())
    dry_debt = np.where(s["_wet_seen"].to_numpy() == 1, 0, dry_debt)
    monaco_2025 = (
        s["Year"].eq(2025)
        & s["Race"].astype(str).str.contains("Monaco", case=False, na=False)
    ).to_numpy()
    set_debt = np.where(monaco_2025, np.maximum(0, 3 - s["_sets_used"].to_numpy()), 0)

    s["_mandatory_dry_compound_debt"] = dry_debt.astype("int8")
    s["_monaco_2025_set_debt"] = set_debt.astype("int8")
    s["_mandatory_stops_remaining_min"] = np.maximum(dry_debt, set_debt).astype("int8")
    s["_compliant_if_finish_now"] = (
        s["_mandatory_stops_remaining_min"].to_numpy() == 0
    ).astype("int8")
    s["_rule_pressure_now"] = (
        s["_mandatory_stops_remaining_min"].to_numpy()
        / (s["_laps_remaining_est"].to_numpy() + 1.0)
    ).astype("float32")
    s["_must_stop_soon_rule"] = (
        (s["_mandatory_stops_remaining_min"].to_numpy() > 0)
        & (
            s["_laps_remaining_est"].to_numpy()
            <= (s["_mandatory_stops_remaining_min"].to_numpy() * 3 + 1)
        )
    ).astype("int8")
    s["_dry_rule_active"] = ((s["_wet_seen"].to_numpy() == 0) & (dry_debt > 0)).astype(
        "int8"
    )
    s["_monaco_three_set_rule_active"] = ((monaco_2025) & (set_debt > 0)).astype("int8")

    add_cols = [
        "_sets_used",
        "_wet_seen",
        "_distinct_dry_compounds_used",
        "_laps_remaining_est",
        "_mandatory_dry_compound_debt",
        "_monaco_2025_set_debt",
        "_mandatory_stops_remaining_min",
        "_compliant_if_finish_now",
        "_rule_pressure_now",
        "_must_stop_soon_rule",
        "_dry_rule_active",
        "_monaco_three_set_rule_active",
    ]
    s = s.sort_values("_idx", kind="mergesort")
    for c in add_cols:
        out[c[1:]] = s[c].to_numpy()

    out["rule_pressure_x_tyre_life"] = (
        out["rule_pressure_now"] * out["TyreLife"]
    ).astype("float32")
    out["rule_pressure_x_race_progress"] = (
        out["rule_pressure_now"] * out["RaceProgress"]
    ).astype("float32")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000783"],
        "research_usage_note": "Verified assigned hypothesis 000783.",
    }, sort_keys=True))
