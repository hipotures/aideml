...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["hazard_tyre_bin"] = pd.cut(
        tyre,
        bins=[0, 3, 7, 12, 18, 25, 35, 50, 100],
        labels=False,
        include_lowest=True,
    ).astype("int16")
    out["hazard_progress_bin"] = pd.cut(
        progress,
        bins=[0, 0.15, 0.30, 0.45, 0.60, 0.75, 0.88, 1.01],
        labels=False,
        include_lowest=True,
    ).astype("int16")
    out["stint_progress"] = stint * progress
    out["tyre_x_stint"] = tyre * stint
    out["tyre_x_progress"] = tyre * progress
    out["late_stint_hazard"] = (tyre >= 0.22 * lap.clip(lower=1)).astype("int8")
    out["very_old_tyre"] = (tyre >= 28).astype("int8")
    out["pit_window_soft"] = ((out["Compound"] == "SOFT") & (tyre >= 10)).astype("int8")
    out["pit_window_medium"] = ((out["Compound"] == "MEDIUM") & (tyre >= 16)).astype(
        "int8"
    )
    out["pit_window_hard"] = ((out["Compound"] == "HARD") & (tyre >= 22)).astype("int8")
    out["wet_weather_compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["degradation_per_tyre_lap"] = deg / tyre.clip(lower=1)
    out["lap_delta_per_tyre_lap"] = delta / tyre.clip(lower=1)
    out["laptime_over_tyre_lap"] = laptime / tyre.clip(lower=1)

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber", idx_name]
    s = out.sort_values(sort_cols, kind="mergesort")
    stint_keys = ["Year", "Race", "Driver", "Stint"]

    stint_tbl = (
        s.groupby(stint_keys, sort=False)
        .agg(
            stint_len=("TyreLife", "max"),
            stint_end_lap=("LapNumber", "max"),
            stint_compound=("Compound", "last"),
            stint_deg_rate=("degradation_per_tyre_lap", "mean"),
        )
        .reset_index()
        .sort_values(["Year", "Race", "Driver", "Stint"], kind="mergesort")
    )
    stint_tbl["expected_len_by_compound"] = stint_tbl.groupby("stint_compound")[
        "stint_len"
    ].transform("mean")
    stint_tbl["stint_len_resid"] = (
        stint_tbl["stint_len"] - stint_tbl["expected_len_by_compound"]
    )
    ...
    for key, name in [
        ...
        (["Driver", "stint_compound"], "driver_compound"),
        (["Race", "stint_compound"], "race_compound"),
    ]:
        grp = stint_tbl.groupby(key, sort=False)["stint_len_resid"]
        shifted = grp.shift(1)
        stint_tbl[f"frailty_{name}_prior_resid_mean"] = (
            shifted.groupby([stint_tbl[k] for k in key], sort=False)
            .expanding()
            .mean()
            .reset_index(level=list(range(len(key))), drop=True)
        )
        stint_tbl[f"frailty_{name}_prior_count"] = grp.cumcount().astype("int16")

    keep = stint_keys + [
        "stint_len",
        "stint_len_resid",
        "frailty_driver_prior_resid_mean",
        "frailty_driver_prior_count",
        "frailty_race_prior_resid_mean",
        "frailty_race_prior_count",
        "frailty_year_prior_resid_mean",
        "frailty_year_prior_count",
        "frailty_compound_prior_resid_mean",
        "frailty_compound_prior_count",
        "frailty_driver_compound_prior_resid_mean",
        "frailty_driver_compound_prior_count",
        "frailty_race_compound_prior_resid_mean",
        "frailty_race_compound_prior_count",
    ]
    out = out.merge(stint_tbl[keep], on=stint_keys, how="left", sort=False)

    for c in [
        c
        for c in out.columns
        if c.startswith("frailty_") and c.endswith("_prior_resid_mean")
    ]:
        out[c] = out[c].fillna(0.0)
    for c in [
        c
        for c in out.columns
        if c.startswith("frailty_") and c.endswith("_prior_count")
    ]:
        out[c] = out[c].fillna(0).astype("int16")

    out["current_stint_resid_to_compound"] = out["stint_len_resid"].fillna(0.0)
    out["tyre_vs_stint_len"] = tyre / out["stint_len"].clip(lower=1).fillna(
        tyre.clip(lower=1)
    )
    out["frailty_blend_prior_resid"] = (
        out["frailty_driver_prior_resid_mean"]
        + out["frailty_race_prior_resid_mean"]
        + out["frailty_compound_prior_resid_mean"]
    ) / 3.0

    out = out.sort_values(idx_name, kind="mergesort").drop(columns=[idx_name])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000695"],
        "research_usage_note": "Verified assigned hypothesis 000695.",
    }, sort_keys=True))
