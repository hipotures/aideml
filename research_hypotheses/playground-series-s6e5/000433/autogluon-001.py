...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in base_cols:
        ...
        out[f"{col}_clip"] = out[col].clip(q1, q99)

    g = out.groupby(group_cols, sort=False)

    for col in [f"{c}_clip" for c in base_cols]:
        s = g[col]

        prev1 = s.shift(1)
        prev2 = s.shift(2)
        out[f"{col}_diff1"] = out[col] - prev1
        out[f"{col}_diff2"] = out[col] - prev2
        out[f"{col}_curv2"] = out[col] - 2 * prev1 + prev2

        for w in (2, 3, 5):
            lagw = s.shift(w)
            out[f"{col}_slope_{w}"] = (out[col] - lagw) / w

            roll = s.shift(1).rolling(w, min_periods=1)
            rmean = roll.mean().reset_index(level=group_cols, drop=True)
            rstd = roll.std().reset_index(level=group_cols, drop=True)
            rmin = roll.min().reset_index(level=group_cols, drop=True)
            rmax = roll.max().reset_index(level=group_cols, drop=True)

            out[f"{col}_trailmean_{w}"] = rmean
            out[f"{col}_trailstd_{w}"] = rstd.fillna(0.0)
            out[f"{col}_range_{w}"] = (rmax - rmin).fillna(0.0)
            out[f"{col}_surprise_{w}"] = (out[col] - rmean).fillna(0.0)
            out[f"{col}_zsurprise_{w}"] = (
                ((out[col] - rmean) / (rstd + 1e-6))
                .replace([np.inf, -np.inf], 0.0)
                .fillna(0.0)
            )

        ewm_mean_3 = s.shift(1).transform(lambda x: x.ewm(span=3, adjust=False).mean())
        ewm_mean_5 = s.shift(1).transform(lambda x: x.ewm(span=5, adjust=False).mean())
        out[f"{col}_ewm_3"] = ewm_mean_3.fillna(method="bfill")
        out[f"{col}_ewm_5"] = ewm_mean_5.fillna(method="bfill")
        out[f"{col}_ewm_gap_3"] = (out[col] - ewm_mean_3).fillna(0.0)
        out[f"{col}_ewm_gap_5"] = (out[col] - ewm_mean_5).fillna(0.0)

    out["stint_progress"] = out["TyreLife"] / out.groupby(group_cols, sort=False)[
        "TyreLife"
    ].transform("max").replace(0, np.nan)
    out["laps_left_in_stint_proxy"] = (
        out.groupby(group_cols, sort=False)["LapNumber"].transform("max")
        - out["LapNumber"]
    )
    out["deg_per_tyre_life"] = out["Cumulative_Degradation"] / out["TyreLife"].replace(
        0, np.nan
    )
    out["lap_delta_per_tyre_life"] = out["LapTime_Delta"] / out["TyreLife"].replace(
        0, np.nan
    )

    delta_diff = out.groupby(group_cols, sort=False)["LapTime_Delta_clip_diff1"].shift(
        1
    )
    out["latent_pace_loss_proxy"] = (
        delta_diff.clip(lower=0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000433"],
        "research_usage_note": "Verified assigned hypothesis 000433.",
    }, sort_keys=True))
