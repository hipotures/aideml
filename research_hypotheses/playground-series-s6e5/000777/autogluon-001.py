...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Year_Race"] = out["Year"].astype("string") + "_" + out["Race"]
    out["Driver_Race"] = out["Driver"] + "_" + out["Race"]
    out["Compound_Stint"] = out["Compound"] + "_" + out["Stint"].astype("string")

    for c in ["Year_Race", "Driver_Race", "Compound_Stint"]:
        freq = out[c].value_counts(normalize=True)
        out[c + "_freq"] = out[c].map(freq).astype("float32")
    ...
    out["tyre_life_per_stint"] = tyre / (stint + 1.0)
    out["lap_per_stint"] = lap / (stint + 1.0)
    out["laps_remaining_est"] = lap * (1.0 - progress) / progress.clip(lower=0.01)
    out["late_stint_pressure"] = tyre * np.sqrt(progress.clip(lower=0.0))
    out["pit_now_x_tyre_life"] = pit_now * tyre
    out["pit_now_x_lap"] = pit_now * lap
    out["position_x_progress"] = pos * progress
    ...
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / tyre.clip(
        lower=1.0
    )
    out["laptime_per_lap"] = out["LapTime (s)"] / lap.clip(lower=1.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000777.",
    }, sort_keys=True))
