...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_seq_code"] = (
        out["Compound"].map(compound_map).fillna(-1).astype("int16")
    )
    ...
    ordered_idx = out.sort_values(sort_cols, kind="mergesort").index
    hist = out.loc[ordered_idx, group_cols + seq_cols].copy()
    g = hist.groupby(group_cols, sort=False)

    for lag in range(1, 9):
        shifted = g[seq_cols].shift(lag)
        shifted.index = ordered_idx
        out[f"hist_mask_lag{lag}"] = shifted["TyreLife"].notna().astype("int8")
        for col in seq_cols:
            safe = (
                col.replace(" ", "_")
                .replace("(", "")
            ...
            out[f"hist_lag{lag}_{safe}"] = shifted[col].reindex(out.index).fillna(-1)
    ...
    for col in [
        ...
        s = hist[col]
        prev = g[col].shift(1)
        hist[f"{safe}_prev"] = prev
        hist[f"{safe}_roll3_mean"] = (
            prev.groupby([hist[c] for c in group_cols], sort=False)
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=[0, 1, 2], drop=True)
        )
        hist[f"{safe}_roll5_mean"] = (
            prev.groupby([hist[c] for c in group_cols], sort=False)
            .rolling(5, min_periods=1)
            .mean()
            .reset_index(level=[0, 1, 2], drop=True)
        )
        hist[f"{safe}_roll5_std"] = (
            prev.groupby([hist[c] for c in group_cols], sort=False)
            .rolling(5, min_periods=2)
            .std()
            .reset_index(level=[0, 1, 2], drop=True)
        )
        hist.index = ordered_idx
        out[f"hist_{safe}_delta_prev"] = (
            out[col] - hist[f"{safe}_prev"].reindex(out.index)
        ).fillna(0)
        out[f"hist_{safe}_roll3_mean"] = (
            hist[f"{safe}_roll3_mean"].reindex(out.index).fillna(-1)
        )
        out[f"hist_{safe}_roll5_mean"] = (
            hist[f"{safe}_roll5_mean"].reindex(out.index).fillna(-1)
        )
        out[f"hist_{safe}_roll5_std"] = (
            hist[f"{safe}_roll5_std"].reindex(out.index).fillna(0)
        )

    out["hist_laps_seen"] = (
        out[[f"hist_mask_lag{i}" for i in range(1, 9)]].sum(axis=1).astype("int8")
    )
    out["hist_tyrelife_x_stint"] = out["TyreLife"] * out["Stint"]
    out["hist_degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / (
        out["TyreLife"].abs() + 1.0
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000523"],
        "research_usage_note": "Verified assigned hypothesis 000523.",
    }, sort_keys=True))
