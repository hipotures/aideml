...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_pace_spread = (
        (s["LapTime_s_clip"].astype("float32") - lap_time_median)
        .abs()
        .groupby([s["Year"], s["Race"]], sort=False)
        .transform("median")
        .fillna(0.0)
        .astype("float32")
    )
    race_pit_loss = (
        18.0 + 1.5 * field_delta_std + 0.20 * race_pace_spread
    ).clip(12.0, 45.0).astype("float32")
