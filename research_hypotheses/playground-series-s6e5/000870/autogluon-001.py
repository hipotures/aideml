...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    n = len(out)
    out["_row_order_tmp"] = np.arange(n, dtype=np.int64)

    out["LapTime_Delta_clip"] = (
        out["LapTime_Delta"].astype("float32").clip(-120.0, 180.0)
    )
    out["LapTime_s_clip"] = out["LapTime (s)"].astype("float32").clip(50.0, 300.0)
    out["Cumulative_Degradation_clip"] = (
        out["Cumulative_Degradation"].astype("float32").clip(-100.0, 1000.0)
    )

    keys = ["Year", "Race", "LapNumber"]
    s = out.sort_values(keys + ["Position", "Driver"], kind="mergesort")
    order = s["_row_order_tmp"].to_numpy()
    g = s.groupby(keys, sort=False)

    field_size = g["Position"].transform("count").astype("float32")
    field_max_pos = g["Position"].transform("max").astype("float32").clip(lower=1.0)
    tmp = np.empty(n, dtype=np.float32)
    tmp[order] = field_size.to_numpy(dtype=np.float32, copy=False)
    out["field_size"] = tmp
    tmp = np.empty(n, dtype=np.float32)
    tmp[order] = (s["Position"].astype("float32") / field_max_pos).to_numpy(
        dtype=np.float32, copy=False
    )
    out["position_field_pct"] = tmp

    pct_arrays = {}
    for col, short in [
        ("TyreLife", "tyrelife"),
        ("LapTime_Delta_clip", "lapdelta"),
        ("Cumulative_Degradation_clip", "degradation"),
        ("LapTime_s_clip", "laptime"),
        ...
        pct_arrays[short] = vals
        tmp = np.empty(n, dtype=np.float32)
        tmp[order] = vals
        out[f"{short}_field_pct"] = tmp

    field_delta_mean = g["LapTime_Delta_clip"].transform("mean").astype("float32")
    field_delta_std = (
        g["LapTime_Delta_clip"].transform("std").fillna(0.0).astype("float32")
    )
    field_tyre_mean = g["TyreLife"].transform("mean").astype("float32")
    field_tyre_std = g["TyreLife"].transform("std").fillna(0.0).astype("float32")
    field_deg_mean = (
        g["Cumulative_Degradation_clip"].transform("mean").astype("float32")
    )
    field_deg_std = (
        g["Cumulative_Degradation_clip"].transform("std").fillna(0.0).astype("float32")
    )

    cur_delta = s["LapTime_Delta_clip"].to_numpy(dtype=np.float32, copy=False)
    cur_tyre = s["TyreLife"].astype("float32").to_numpy(dtype=np.float32, copy=False)
    cur_deg = s["Cumulative_Degradation_clip"].to_numpy(dtype=np.float32, copy=False)
    delta_std = np.maximum(field_delta_std.to_numpy(dtype=np.float32, copy=False), 1.0)
    tyre_std = np.maximum(field_tyre_std.to_numpy(dtype=np.float32, copy=False), 1.0)
    deg_std = np.maximum(field_deg_std.to_numpy(dtype=np.float32, copy=False), 1.0)

    for name, vals in [
        (
            "lapdelta_field_z",
            (cur_delta - field_delta_mean.to_numpy(dtype=np.float32, copy=False))
            / delta_std,
        ),
        (
            "tyrelife_field_z",
            (cur_tyre - field_tyre_mean.to_numpy(dtype=np.float32, copy=False))
            / tyre_std,
        ),
        (
            "degradation_field_z",
            (cur_deg - field_deg_mean.to_numpy(dtype=np.float32, copy=False)) / deg_std,
        ),
    ]:
        tmp = np.empty(n, dtype=np.float32)
        tmp[order] = vals.astype(np.float32)
        out[name] = tmp
    ...
    base_cols = [c for c, _ in neighbor_cols]
    for offset, label in [(-2, "pos_m2"), (-1, "pos_m1"), (1, "pos_p1"), (2, "pos_p2")]:
        shifted = g[base_cols].shift(-offset)
        tmp = np.empty(n, dtype=np.float32)
        tmp[order] = (
            shifted["TyreLife"]
            .notna()
        ...
        out[f"nbr_{label}_exists"] = tmp
        for col, short in neighbor_cols:
            vals = (
                (s[col].astype("float32") - shifted[col].astype("float32"))
                .fillna(0.0)
                .astype("float32")
            )
            tmp = np.empty(n, dtype=np.float32)
            tmp[order] = vals.to_numpy(dtype=np.float32, copy=False)
            out[f"nbr_{label}_{short}_diff"] = tmp

    lap_time_median = g["LapTime_s_clip"].transform("median").astype("float32")
    pit_loss_signal = (s["LapTime_s_clip"].astype("float32") - lap_time_median).where(
        s["PitStop"].astype("float32") > 0.5
    )
    pit_vals = pit_loss_signal.to_numpy(dtype=np.float32, copy=False)
    pit_vals = pit_vals[np.isfinite(pit_vals) & (pit_vals > 5.0) & (pit_vals < 80.0)]
    global_pit_loss = float(np.median(pit_vals)) if pit_vals.size else 22.0
    race_pit_loss = pit_loss_signal.groupby(
        [s["Year"], s["Race"]], sort=False
    ).transform("median")
    race_pit_loss = (
        race_pit_loss.fillna(global_pit_loss).clip(12.0, 45.0).astype("float32")
    )
    sec_per_pos = (3.0 + 0.35 * field_delta_std).clip(3.0, 7.0).astype("float32")
    pit_pos_loss = (race_pit_loss / sec_per_pos).clip(3.0, 8.0).astype("float32")

    for name, vals in [
        (
            "estimated_pit_loss_seconds",
            race_pit_loss.to_numpy(dtype=np.float32, copy=False),
        ),
        ("estimated_pit_pos_loss", pit_pos_loss.to_numpy(dtype=np.float32, copy=False)),
    ]:
        tmp = np.empty(n, dtype=np.float32)
        tmp[order] = vals
        out[name] = tmp

    pit_pos_arr = pit_pos_loss.to_numpy(dtype=np.float32, copy=False)
    base_weight = np.zeros(len(s), dtype=np.float32)
    total_weight = np.zeros(len(s), dtype=np.float32)
    weighted_pace = np.zeros(len(s), dtype=np.float32)
    weighted_tyre = np.zeros(len(s), dtype=np.float32)
    weighted_deg = np.zeros(len(s), dtype=np.float32)

    for off in range(3, 9):
        w = np.exp(-0.5 * ((pit_pos_arr - float(off)) / 1.25) ** 2).astype(np.float32)
        shifted = g[
            ["LapTime_Delta_clip", "TyreLife", "Cumulative_Degradation_clip"]
        ].shift(-off)
        pace = shifted["LapTime_Delta_clip"].to_numpy(dtype=np.float32, copy=False)
        tyre = shifted["TyreLife"].to_numpy(dtype=np.float32, copy=False)
        deg = shifted["Cumulative_Degradation_clip"].to_numpy(
            dtype=np.float32, copy=False
        )
        valid = np.isfinite(pace)
        wv = w * valid.astype(np.float32)
        base_weight += w
        total_weight += wv
        weighted_pace += wv * np.nan_to_num(pace, nan=0.0)
        weighted_tyre += wv * np.nan_to_num(tyre, nan=0.0)
        weighted_deg += wv * np.nan_to_num(deg, nan=0.0)

    field_delta_mean_arr = field_delta_mean.to_numpy(dtype=np.float32, copy=False)
    field_tyre_mean_arr = field_tyre_mean.to_numpy(dtype=np.float32, copy=False)
    field_deg_mean_arr = field_deg_mean.to_numpy(dtype=np.float32, copy=False)
    rejoin_pace = np.where(
        total_weight > 0,
        weighted_pace / np.maximum(total_weight, 1e-3),
        field_delta_mean_arr,
    )
    rejoin_tyre = np.where(
        total_weight > 0,
        weighted_tyre / np.maximum(total_weight, 1e-3),
        field_tyre_mean_arr,
    )
    rejoin_deg = np.where(
        total_weight > 0,
        weighted_deg / np.maximum(total_weight, 1e-3),
        field_deg_mean_arr,
    )
    rejoin_missing = np.clip(
        1.0 - total_weight / np.maximum(base_weight, 1e-3), 0.0, 1.0
    ).astype(np.float32)

    rejoin_pace_z = (rejoin_pace - field_delta_mean_arr) / delta_std
    rejoin_tyre_z = (rejoin_tyre - field_tyre_mean_arr) / tyre_std
    rejoin_deg_z = (rejoin_deg - field_deg_mean_arr) / deg_std
    traffic_penalty = (
        np.maximum(rejoin_pace_z, 0.0)
        + 0.25 * np.maximum(rejoin_tyre_z, 0.0)
        + 0.15 * np.maximum(rejoin_deg_z, 0.0)
        - 0.5 * rejoin_missing
    )
    clear_air = -traffic_penalty

    for name, vals in [
        ("rejoin_traffic_pace_z", rejoin_pace_z),
        ("rejoin_traffic_tyre_z", rejoin_tyre_z),
        ("rejoin_traffic_degradation_z", rejoin_deg_z),
        ("rejoin_traffic_missing_frac", rejoin_missing),
        ("clear_air_after_pit", clear_air),
    ]:
        tmp = np.empty(n, dtype=np.float32)
        tmp[order] = vals.astype(np.float32)
        out[name] = tmp

    ahead_block = np.zeros(len(s), dtype=np.float32)
    ahead_w = np.zeros(len(s), dtype=np.float32)
    for off, w in [(-1, 1.0), (-2, 0.5)]:
        pace = (
            g["LapTime_Delta_clip"].shift(-off).to_numpy(dtype=np.float32, copy=False)
        )
        valid = np.isfinite(pace)
        ahead_block += w * np.where(
            valid, np.maximum(pace - cur_delta, 0.0), 0.0
        ).astype(np.float32)
        ahead_w += w * valid.astype(np.float32)
    ahead_block = ahead_block / np.maximum(ahead_w, 1e-3)

    behind_threat = np.zeros(len(s), dtype=np.float32)
    behind_newer = np.zeros(len(s), dtype=np.float32)
    behind_w = np.zeros(len(s), dtype=np.float32)
    for off, w in [(1, 1.0), (2, 0.5)]:
        pace = (
            g["LapTime_Delta_clip"].shift(-off).to_numpy(dtype=np.float32, copy=False)
        )
        tyre = g["TyreLife"].shift(-off).to_numpy(dtype=np.float32, copy=False)
        valid = np.isfinite(pace)
        behind_threat += w * np.where(
            valid, np.maximum(cur_delta - pace, 0.0), 0.0
        ).astype(np.float32)
        behind_newer += w * np.where(
            valid, np.maximum(cur_tyre - tyre, 0.0), 0.0
        ).astype(np.float32)
        behind_w += w * valid.astype(np.float32)
    behind_threat = behind_threat / np.maximum(behind_w, 1e-3)
    behind_newer = behind_newer / np.maximum(behind_w, 1e-3)

    ahead_block_z = ahead_block / delta_std
    behind_threat_z = behind_threat / delta_std
    behind_newer_z = behind_newer / tyre_std
    stay_cost = (
        0.70 * pct_arrays["tyrelife"]
        + 0.70 * pct_arrays["degradation"]
        + 0.60 * pct_arrays["lapdelta"]
        + 0.45 * ahead_block_z
        + 0.45 * behind_threat_z
        + 0.25 * behind_newer_z
    )
    pit_now_cost = (
        (pit_pos_arr / 8.0)
        + traffic_penalty
        - 0.35 * (pct_arrays["tyrelife"] + pct_arrays["degradation"])
    )
    counterfactual_delta = stay_cost - pit_now_cost

    for name, vals in [
        ("nearby_ahead_block_seconds", ahead_block),
        ("nearby_behind_threat_seconds", behind_threat),
        ("nearby_behind_newer_tyre_laps", behind_newer),
        ("stay_out_one_lap_pressure", stay_cost),
        ("pit_now_traffic_cost", pit_now_cost),
        ("pit_vs_stay_counterfactual_delta", counterfactual_delta),
    ]:
        tmp = np.empty(n, dtype=np.float32)
        tmp[order] = vals.astype(np.float32)
        out[name] = tmp

    return out.drop(columns=["_row_order_tmp"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000870"],
        "research_usage_note": "Verified assigned hypothesis 000870.",
    }, sort_keys=True))
