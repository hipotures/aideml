...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    eps = 1e-6
    out["lap_frac"] = out["LapNumber"] / (out["LapNumber"].max() + eps)
    out["tyre_life_frac"] = out["TyreLife"] / (out["LapNumber"].max() + eps)
    out["stint_lap_ratio"] = out["TyreLife"] / (out["LapNumber"] + eps)
    out["race_remaining"] = 1.0 - out["RaceProgress"]
    out["progress_x_tyre"] = out["RaceProgress"] * out["TyreLife"]
    out["progress_x_stint"] = out["RaceProgress"] * out["Stint"]
    out["pit_x_stint"] = out["PitStop"] * out["Stint"]
    out["pit_x_tyre"] = out["PitStop"] * out["TyreLife"]
    ...
    out["lap_delta_per_life"] = out["LapTime_Delta"] / (out["TyreLife"] + eps)
    ...
    for c in ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]:
        ...
        out[c + "_clip"] = out[c].clip(lo, hi)
        out[c + "_logabs"] = np.log1p(out[c].abs())
    ...
    for c in [
        ...
        out[c + "_race_lap_rank"] = out.groupby(grp, observed=True)[c].rank(
            pct=True, method="average"
        )
        med = out.groupby(grp, observed=True)[c].transform("median")
        out[c + "_race_lap_diff"] = out[c] - med
    ...
    for name, cols in combo_defs.items():
        ...
        out[name + "_freq"] = s.map(s.value_counts()).astype("float32") / max(n, 1)

    for c in cat_cols:
        counts = out[c].astype(str).value_counts()
        out[c + "_freq"] = out[c].astype(str).map(counts).astype("float32") / max(n, 1)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000960.",
    }, sort_keys=True))
