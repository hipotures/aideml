...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["Race", "Driver"]
    stint_cols = ["Race", "Driver", "Stint"]

    sort_cols = [
        c for c in ["Race", "Driver", "Stint", "LapNumber"] if c in out.columns
    ]
    s = out.sort_values(sort_cols, kind="mergesort") if sort_cols else out

    if all(c in out.columns for c in group_cols):
        g = s.groupby(group_cols, observed=True, sort=False)
        out.loc[s.index, "drv_race_lap_rank_pct"] = (
            g["LapNumber"].rank(pct=True).astype("float32")
        )
        out.loc[s.index, "drv_race_lap_count"] = (
            g["LapNumber"].transform("count").astype("float32")
        )
        out.loc[s.index, "drv_race_tl_pct"] = (
            g["TyreLife"].rank(pct=True).astype("float32")
        )
        out.loc[s.index, "drv_race_degr_pct"] = (
            g["Cumulative_Degradation"].rank(pct=True).astype("float32")
        )
        out.loc[s.index, "drv_race_laptime_pct"] = (
            g["LapTime (s)"].rank(pct=True).astype("float32")
        )

    if all(c in out.columns for c in stint_cols):
        gs = s.groupby(stint_cols, observed=True, sort=False)
        out.loc[s.index, "stint_lap_rank_pct"] = (
            gs["LapNumber"].rank(pct=True).astype("float32")
        )
        out.loc[s.index, "stint_len"] = (
            gs["LapNumber"].transform("count").astype("float32")
        )
        out.loc[s.index, "stint_tyre_rank_pct"] = (
            gs["TyreLife"].rank(pct=True).astype("float32")
        )
        out.loc[s.index, "stint_degr_rank_pct"] = (
            gs["Cumulative_Degradation"].rank(pct=True).astype("float32")
        )
        ...
        for col in [
            ...
            if col in out.columns:
                prev1 = gs[col].shift(1)
                next1 = gs[col].shift(-1)
                out.loc[s.index, f"{col}_prev1"] = prev1.astype("float32")
                out.loc[s.index, f"{col}_next1"] = next1.astype("float32")
                out.loc[s.index, f"{col}_local3_mean"] = (
                    (prev1 + s[col] + next1) / 3.0
                ).astype("float32")
                out.loc[s.index, f"{col}_local3_max"] = (
                    pd.concat([prev1, s[col], next1], axis=1)
                    .max(axis=1)
                    .astype("float32")
                )

    out["tyre_x_progress"] = (out["TyreLife"] * out["RaceProgress"]).astype("float32")
    out["degr_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"] + 1.0)
    ).astype("float32")
    out["late_old_tyre_window"] = (
        (out["TyreLife"] >= 12) & (out["RaceProgress"].between(0.18, 0.92))
    ).astype("int8")
    out["terminal_race_flag"] = (out["RaceProgress"] > 0.94).astype("int8")
    out["early_stint_flag"] = (out["TyreLife"] <= 3).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000651"],
        "research_usage_note": "Verified assigned hypothesis 000651.",
    }, sort_keys=True))
