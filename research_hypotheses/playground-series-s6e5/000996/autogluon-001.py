...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Year_str"] = out["Year"].astype("string")
    out["Stint_str"] = out["Stint"].astype("string")

    out["Driver_Compound"] = out["Driver"] + "_" + out["Compound"]
    out["Year_Race"] = out["Year_str"] + "_" + out["Race"]
    out["Race_Stint_Compound"] = (
        out["Race"] + "_" + out["Stint_str"] + "_" + out["Compound"]
    )
    out["Driver_Race"] = out["Driver"] + "_" + out["Race"]
    out["Year_Race_Driver"] = out["Year_str"] + "_" + out["Race"] + "_" + out["Driver"]

    eps = 1e-6
    out["TyreLife_x_Progress"] = out["TyreLife"] * out["RaceProgress"]
    out["TyreLife_per_Stint"] = out["TyreLife"] / (out["Stint"] + eps)
    out["LapNumber_per_RaceProgress"] = out["LapNumber"] / (out["RaceProgress"] + eps)
    out["Degradation_per_TyreLife"] = out["Cumulative_Degradation"] / (
        out["TyreLife"] + eps
    )
    out["LapDelta_abs"] = out["LapTime_Delta"].abs()
    out["PositionChange_abs"] = out["Position_Change"].abs()
    out["IsLateRace"] = (out["RaceProgress"] >= 0.70).astype("int8")
    out["IsOldTyre"] = (out["TyreLife"] >= 18).astype("int8")
    out["IsFirstStint"] = (out["Stint"] == 1).astype("int8")

    sort_cols = [c for c in ["Year", "Race", "Driver", "LapNumber"] if c in out.columns]
    ordered = out.sort_values(sort_cols, kind="mergesort") if sort_cols else out
    g = ordered.groupby(["Year", "Race", "Driver"], sort=False, observed=True)
    ...
    for c in [
        ...
        s = g[c].shift(1)
        ordered[f"{c}_prev"] = s
        ordered[f"{c}_roll3_mean"] = (
            s.groupby(
                [ordered["Year"], ordered["Race"], ordered["Driver"]],
                sort=False,
                observed=True,
            )
    ...
    for c in freq_cols:
        out[f"{c}_freq"] = (
            out[c].map(out[c].value_counts(dropna=False)).astype("float32") / n
        )

    out = out.drop(columns=["Year_str", "Stint_str"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000996"],
        "research_usage_note": "Verified assigned hypothesis 000996.",
    }, sort_keys=True))
