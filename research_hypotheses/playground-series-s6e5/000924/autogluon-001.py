...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Estimated_Laps_Remaining"] = (
        est_total_laps - out["LapNumber"].astype(float)
    ).clip(lower=0)

    g = out.groupby(keys, sort=False)
    field_size = g["TyreLife"].transform("size").astype(float)
    tyre_rank_max = g["TyreLife"].rank(method="max", ascending=True)
    out["field_tyrelife_pctile"] = g["TyreLife"].rank(method="average", pct=True)
    out["field_older_tyre_count"] = (field_size - tyre_rank_max).clip(lower=0)
    out["field_older_tyre_share"] = out["field_older_tyre_count"] / (
        field_size - 1
    ).replace(0, np.nan)
    out["field_older_tyre_share"] = out["field_older_tyre_share"].fillna(0)

    out["_pos_band"] = pd.cut(
        pos,
        bins=[0, 5, 10, 15, 20],
        labels=False,
        include_lowest=True,
    ).astype("int8")
    out["pos_band_lap_delta_median"] = out.groupby(keys + ["_pos_band"], sort=False)[
        "LapTime_Delta"
    ].transform("median")
    out["lap_delta_vs_pos_band"] = lap_delta - out["pos_band_lap_delta_median"]

    sums = g[["TyreLife", "LapTime_Delta"]].transform("sum")
    out["field_tyrelife_mean_ex_self"] = (
        (sums["TyreLife"] - tyre) / (field_size - 1).replace(0, np.nan)
    ).fillna(tyre)
    out["field_lapdelta_mean_ex_self"] = (
        (sums["LapTime_Delta"] - lap_delta) / (field_size - 1).replace(0, np.nan)
    ).fillna(lap_delta)

    ordered = out.sort_values(keys + ["Position"])
    og = ordered.groupby(keys, sort=False)
    neigh_tyre = (og["TyreLife"].shift(1) + og["TyreLife"].shift(-1)) / (
        og["TyreLife"].shift(1).notna().astype(float)
        + og["TyreLife"].shift(-1).notna().astype(float)
    )
    neigh_delta = (og["LapTime_Delta"].shift(1) + og["LapTime_Delta"].shift(-1)) / (
        og["LapTime_Delta"].shift(1).notna().astype(float)
        + og["LapTime_Delta"].shift(-1).notna().astype(float)
    )

    out["neighbor_tyrelife_mean"] = neigh_tyre.reindex(out.index).fillna(tyre)
    out["neighbor_lapdelta_mean"] = neigh_delta.reindex(out.index).fillna(lap_delta)
    out["neighbor_older_tyre"] = (out["neighbor_tyrelife_mean"] > tyre).astype("int8")
    out["neighbor_worse_pace"] = (out["neighbor_lapdelta_mean"] > lap_delta).astype(
        "int8"
    )
    out["neighbor_pressure"] = out["neighbor_older_tyre"] + out["neighbor_worse_pace"]

    out["undercut_pressure_score"] = (
        out["field_older_tyre_share"] * (21.0 - pos) / 20.0
        + out["neighbor_pressure"] * 0.5
        + out["lap_delta_vs_pos_band"].clip(lower=0) / 10.0
    )
    out["older_share_x_position"] = out["field_older_tyre_share"] * pos
    out["older_share_x_pos_change"] = out["field_older_tyre_share"] * out[
        "Position_Change"
    ].astype(float)
    out["pressure_x_race_progress"] = out["undercut_pressure_score"] * progress
    out["pressure_x_laps_remaining"] = (
        out["undercut_pressure_score"] * out["Estimated_Laps_Remaining"]
    )

    out = out.drop(columns=["_pos_band"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000924"],
        "research_usage_note": "Verified assigned hypothesis 000924.",
    }, sort_keys=True))
