...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    valid_progress = progress.notna() & np.isfinite(progress) & (progress > 0)
    raw_total = lap / progress.where(valid_progress, np.nan)

    out["est_total_laps_raw"] = raw_total
    out["invalid_progress_flag"] = (~valid_progress).astype("int8")
    out["est_total_laps_clipped_flag"] = (
        ((raw_total < 40) | (raw_total > 90)).fillna(False).astype("int8")
    )

    total_clipped = raw_total.clip(lower=40, upper=90)
    out["est_total_laps"] = total_clipped.fillna(lap.clip(lower=40, upper=90))
    out["est_laps_remaining"] = (out["est_total_laps"] - lap).clip(lower=0)
    out["est_laps_remaining_frac"] = out["est_laps_remaining"] / out[
        "est_total_laps"
    ].replace(0, np.nan)

    out["race_progress_sq"] = progress**2
    out["race_progress_sqrt"] = np.sqrt(progress.clip(lower=0))
    out["early_phase"] = (progress <= 0.25).astype("int8")
    out["mid_phase"] = ((progress > 0.25) & (progress <= 0.65)).astype("int8")
    out["late_phase"] = ((progress > 0.65) & (progress <= 0.90)).astype("int8")
    out["final_phase"] = (progress > 0.90).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000025"],
        "research_usage_note": "Verified assigned hypothesis 000025.",
    }, sort_keys=True))
