...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = work.sort_values(group_cols + ["LapNumber", "_row_order"], kind="mergesort")

    race_max_lap = work.groupby(race_cols, sort=False)["LapNumber"].transform("max")
    work["LapsRemaining"] = (race_max_lap - work["LapNumber"]).clip(lower=0)

    work["_is_hard"] = work["Compound"].eq("HARD").astype("int8")
    work["_is_medium"] = work["Compound"].eq("MEDIUM").astype("int8")
    work["_is_soft"] = work["Compound"].eq("SOFT").astype("int8")
    work["_is_wet_like"] = work["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    work["UsedHardToDate"] = (
        work.groupby(group_cols, sort=False)["_is_hard"].cumsum().gt(0).astype("int8")
    )
    work["UsedMediumToDate"] = (
        work.groupby(group_cols, sort=False)["_is_medium"].cumsum().gt(0).astype("int8")
    )
    work["UsedSoftToDate"] = (
        work.groupby(group_cols, sort=False)["_is_soft"].cumsum().gt(0).astype("int8")
    )
    work["WetUsedToDate"] = (
        work.groupby(group_cols, sort=False)["_is_wet_like"]
        .cumsum()
        .gt(0)
        .astype("int8")
    )

    work["DrySpecsUsedToDate"] = (
        work["UsedHardToDate"] + work["UsedMediumToDate"] + work["UsedSoftToDate"]
    ).clip(upper=3)

    pit_now = pd.to_numeric(work["PitStop"], errors="coerce").fillna(0)
    work["PitCountToDate"] = pit_now.groupby(
        [work[c] for c in group_cols], sort=False
    ).cumsum()

    work["RemainingMandatoryCompoundDebt"] = (
        (2 - work["DrySpecsUsedToDate"]).clip(lower=0) * (1 - work["WetUsedToDate"])
    ).astype("int8")

    work["RemainingMandatoryStopDebt"] = (
        (1 - work["PitCountToDate"]).clip(lower=0) * (1 - work["WetUsedToDate"])
    ).astype("int8")

    work["Monaco2025Flag"] = (
        work["Year"].eq(2025)
        & work["Race"].str.contains("Monaco", case=False, na=False)
    ).astype("int8")

    work["Monaco2025RemainingStopDebt"] = (
        (2 - work["PitCountToDate"]).clip(lower=0) * work["Monaco2025Flag"]
    ).astype("int8")

    laps_left_scale = work["LapsRemaining"] + 1.0
    stint_num = pd.to_numeric(work["Stint"], errors="coerce").fillna(0)
    tyre_life = pd.to_numeric(work["TyreLife"], errors="coerce").fillna(0)

    work["RuleStopUrgency"] = work["RemainingMandatoryStopDebt"] / laps_left_scale
    work["RuleCompoundUrgency"] = (
        work["RemainingMandatoryCompoundDebt"] / laps_left_scale
    )
    work["MonacoTwoStopUrgency"] = work["Monaco2025RemainingStopDebt"] / laps_left_scale

    work["RuleStopUrgency_x_Stint"] = work["RuleStopUrgency"] * stint_num
    work["RuleCompoundUrgency_x_Stint"] = work["RuleCompoundUrgency"] * stint_num
    work["MonacoTwoStopUrgency_x_Stint"] = work["MonacoTwoStopUrgency"] * stint_num

    work["RuleStopUrgency_x_TyreLife"] = work["RuleStopUrgency"] * tyre_life
    work["RuleCompoundUrgency_x_TyreLife"] = work["RuleCompoundUrgency"] * tyre_life
    work["MonacoTwoStopUrgency_x_TyreLife"] = work["MonacoTwoStopUrgency"] * tyre_life

    work["RuleCompoundUrgency_x_HARD"] = work["RuleCompoundUrgency"] * work["_is_hard"]
    work["RuleCompoundUrgency_x_MEDIUM"] = (
        work["RuleCompoundUrgency"] * work["_is_medium"]
    )
    work["RuleCompoundUrgency_x_SOFT"] = work["RuleCompoundUrgency"] * work["_is_soft"]
    work["MonacoTwoStopUrgency_x_SOFT"] = (
        work["MonacoTwoStopUrgency"] * work["_is_soft"]
    )

    work = work.sort_values("_row_order", kind="mergesort").drop(
        columns=[
            "_row_order",
            "_is_hard",
            "_is_medium",
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000859.",
    }, sort_keys=True))
