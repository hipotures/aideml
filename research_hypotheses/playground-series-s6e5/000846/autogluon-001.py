...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = X["RaceProgress"].clip(lower=0.01, upper=1.0)
    est_total_laps = (X["LapNumber"] / progress).clip(lower=X["LapNumber"], upper=90)
    laps_remaining = (est_total_laps - X["LapNumber"]).clip(lower=0)

    pit_mask = X["PitStop"].fillna(0).astype(float) > 0
    compound_pit_life = (
        X.loc[pit_mask].groupby("Compound", observed=False)["TyreLife"].median()
    )
    overall_pit_life = X.loc[pit_mask, "TyreLife"].median()
    if pd.isna(overall_pit_life):
        overall_pit_life = X["TyreLife"].median()

    cap = X["Compound"].map(compound_pit_life).astype(float).fillna(overall_pit_life)
    cap = cap.clip(lower=5)

    remaining_capacity = (cap - X["TyreLife"]).clip(lower=0)
    X["Estimated_Total_Laps"] = est_total_laps
    X["Laps_Remaining"] = laps_remaining
    X["Compound_Stint_Capacity"] = cap
    X["Service_Pressure"] = (X["TyreLife"] / cap).clip(lower=0, upper=5)
    X["Finish_Pressure"] = (laps_remaining / (remaining_capacity + 1.0)).clip(
        lower=0, upper=10
    )
    X["Cannot_Finish_Current_Stint"] = (laps_remaining > remaining_capacity).astype(
        np.int8
    )
    X["Fresh_After_Pit"] = ((X["PitStop"].fillna(0) > 0) | (X["TyreLife"] <= 2)).astype(
        np.int8
    )
    X["Pit_Cooldown"] = (1.0 / (X["TyreLife"] + 1.0)).clip(lower=0, upper=1)
    X["Degradation_Pressure"] = (X["Cumulative_Degradation"].clip(lower=0) / cap).clip(
        lower=0, upper=50
    )
    X["LapTimeDelta_Pressure"] = (
        X["LapTime_Delta"].clip(lower=0) / (X["TyreLife"] + 1.0)
    ).clip(lower=0, upper=200)
    X["Track_Position_Pressure"] = ((X["Position"].astype(float) - 1.0) / 19.0).clip(
        lower=0, upper=1
    )
    X["Progress_x_Service"] = (progress * X["Service_Pressure"]).clip(lower=0, upper=5)
    X["Remaining_x_CannotFinish"] = (
        laps_remaining * X["Cannot_Finish_Current_Stint"]
    ...
    keep = [
        ...
        "Laps_Remaining",
        "Compound_Stint_Capacity",
        "Service_Pressure",
        "Finish_Pressure",
        "Cannot_Finish_Current_Stint",
        "Fresh_After_Pit",
        "Pit_Cooldown",
        "Degradation_Pressure",
        "LapTimeDelta_Pressure",
        "Track_Position_Pressure",
        "Progress_x_Service",
        "Remaining_x_CannotFinish",
    ]
    return X[keep]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000846.",
    }, sort_keys=True))
