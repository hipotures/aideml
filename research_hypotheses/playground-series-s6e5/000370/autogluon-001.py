...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cls = pd.Series(
        np.where(comp.eq("WET"), 0, np.where(comp.eq("INTERMEDIATE"), 1, 2)),
        index=out.index,
        name="_compound_class",
    )
    out["compound_class_wetdry"] = cls.astype("int8")
    out["is_wet_tyre"] = (cls == 0).astype("int8")
    out["is_inter_tyre"] = (cls == 1).astype("int8")
    out["is_slick_tyre"] = (cls == 2).astype("int8")
    out["is_weather_tyre"] = (cls < 2).astype("int8")

    lap = out[keys].copy()
    lap["_wet"] = (cls == 0).astype("float32")
    lap["_inter"] = (cls == 1).astype("float32")
    lap["_slick"] = (cls == 2).astype("float32")
    lap["_lap_time"] = out["LapTime (s)"].astype("float32")
    lap["_lap_delta"] = out["LapTime_Delta"].astype("float32")

    agg = lap.groupby(keys, sort=False).agg(
        field_size=("_wet", "size"),
        wet_share=("_wet", "mean"),
        inter_share=("_inter", "mean"),
        slick_share=("_slick", "mean"),
        wet_median_laptime=("_lap_time", lambda x: np.nan),
    )

    med = lap.groupby(keys + ["_wet", "_inter", "_slick"], sort=False)[
        "_lap_time"
    ].median()
    med = med.reset_index()
    med["_class"] = np.select(
        [med["_wet"].eq(1), med["_inter"].eq(1)], [0, 1], default=2
    )
    med = med.pivot_table(
        index=keys, columns="_class", values="_lap_time", aggfunc="median"
    )
    med = med.rename(columns={0: "wet_med_lap", 1: "inter_med_lap", 2: "slick_med_lap"})
    agg = agg.drop(columns=["wet_median_laptime"]).join(med, how="left").reset_index()
    ...
    for c in [
        ...
        agg[c + "_lag1"] = agg.groupby(race_group, sort=False)[c].shift(1)
        agg[c + "_lag2"] = agg.groupby(race_group, sort=False)[c].shift(2)

    agg["to_inter_recent"] = (
        (agg["inter_share_lag1"] - agg["inter_share_lag2"]).clip(lower=0).fillna(0)
    ).astype("float32")
    agg["to_slick_recent"] = (
        (agg["slick_share_lag1"] - agg["slick_share_lag2"]).clip(lower=0).fillna(0)
    ).astype("float32")
    agg["away_from_wet_recent"] = (
        (agg["wet_share_lag2"] - agg["wet_share_lag1"]).clip(lower=0).fillna(0)
    ).astype("float32")
    agg["changeable_mix_lag1"] = (
        (
            1.0
            - agg[["wet_share_lag1", "inter_share_lag1", "slick_share_lag1"]].max(
                axis=1
            )
        )
        .fillna(0)
    ...
    agg["slick_gain_vs_inter_lag1"] = (
        (agg["inter_med_lap_lag1"] - agg["slick_med_lap_lag1"])
        .fillna(0)
        .astype("float32")
    )
    agg["inter_gain_vs_wet_lag1"] = (
        (agg["wet_med_lap_lag1"] - agg["inter_med_lap_lag1"])
        .fillna(0)
        .astype("float32")
    )
    agg["slick_gain_vs_wet_lag1"] = (
        (agg["wet_med_lap_lag1"] - agg["slick_med_lap_lag1"])
        .fillna(0)
        .astype("float32")
    )
    ...
    keep = keys + [
        ...
        "to_inter_recent",
        "to_slick_recent",
        "away_from_wet_recent",
        "changeable_mix_lag1",
        "wet_to_dry_pressure",
        "slick_gain_vs_inter_lag1",
        "inter_gain_vs_wet_lag1",
        "slick_gain_vs_wet_lag1",
    ]
    out = out.merge(agg[keep], on=keys, how="left", sort=False)

    for c in keep[3:]:
        out[c] = out[c].fillna(0).astype("float32")
    out["field_size"] = out["field_size"].fillna(0).astype("int16")

    cls_arr = out["compound_class_wetdry"].to_numpy()
    faster_gain = np.select(
        [cls_arr == 0, cls_arr == 1],
        [
            out["inter_gain_vs_wet_lag1"].to_numpy(),
            out["slick_gain_vs_inter_lag1"].to_numpy(),
        ],
        default=np.zeros(len(out), dtype="float32"),
    )
    out["faster_compound_proven"] = (faster_gain > 0.25).astype("int8")
    out["faster_compound_gain"] = np.maximum(faster_gain, 0).astype("float32")

    tyre = out["TyreLife"].astype("float32")
    pos = out["Position"].astype("float32")
    prog = out["RaceProgress"].astype("float32")
    pressure = out["wet_to_dry_pressure"].astype("float32")
    out["crossover_pressure_x_tyre_life"] = (pressure * tyre).astype("float32")
    out["crossover_pressure_x_position"] = (pressure * (21.0 - pos)).astype("float32")
    out["crossover_pressure_x_progress"] = (pressure * prog).astype("float32")
    out["proven_gain_x_tyre_life"] = (out["faster_compound_gain"] * tyre).astype(
        "float32"
    )
    out["weather_tyre_late_stint"] = (
        out["is_weather_tyre"].astype("float32") * tyre * prog
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000370"],
        "research_usage_note": "Verified assigned hypothesis 000370.",
    }, sort_keys=True))
