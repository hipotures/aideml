...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["race_lap_median_000373"] = (
        out.groupby(["Year", "Race", "LapNumber"])["LapTime (s)"]
        .transform("median")
        .astype("float32")
    )
    out["pace_resid_000373"] = (
        out["LapTime (s)"] - out["race_lap_median_000373"]
    ).astype("float32")

    keys = ["Year", "Race", "Driver", "Stint"]
    work = out.sort_values(keys + ["LapNumber", "_ord_000373"], kind="mergesort")
    g = work.groupby(keys, sort=False)

    work["stint_lap_idx_000373"] = g.cumcount().astype("float32") + 1.0
    early_mask = work["stint_lap_idx_000373"] <= 3

    base_cols = ["LapTime_Delta", "Cumulative_Degradation", "pace_resid_000373"]
    for c in base_cols:
        prev = g[c].shift(1)
        slope = prev - g[c].shift(4)
        roll_mean = prev.groupby([work[k] for k in keys], sort=False).transform(
            lambda s: s.rolling(5, min_periods=3).mean()
        )
        roll_std = prev.groupby([work[k] for k in keys], sort=False).transform(
            lambda s: s.rolling(5, min_periods=3).std()
        )
        resid = (prev - roll_mean).astype("float32")
        z = (
            (resid / (roll_std + 1e-3))
        ...
        accel = (
            slope - slope.groupby([work[k] for k in keys], sort=False).shift(1)
        ).astype("float32")
        cusum = (
            z.clip(lower=0)
        ...
        safe = (~early_mask).astype("float32")
        name = c.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        work[f"{name}_prior_slope_000373"] = slope.fillna(0).astype("float32") * safe
        work[f"{name}_prior_accel_000373"] = accel.fillna(0).astype("float32") * safe
        work[f"{name}_ewma_resid_000373"] = (
            resid.groupby([work[k] for k in keys], sort=False)
            .transform(lambda s: s.ewm(alpha=0.45, adjust=False).mean())
            .fillna(0)
            .astype("float32")
            * safe
        )
        work[f"{name}_cusum_cliff_000373"] = cusum.fillna(0).astype("float32") * safe
        work[f"{name}_cliff_flag_000373"] = ((z > 1.5) & (~early_mask)).astype("int8")

    warm = out.copy()
    warm["warm_age_000373"] = pd.to_numeric(warm["TyreLife"], errors="coerce")
    warm["warm_pace_000373"] = out["pace_resid_000373"]
    warm = warm[warm["warm_age_000373"].between(1, 3)]
    prior = warm.groupby(["Race", "Compound"])["warm_pace_000373"].mean()
    global_prior = float(warm["warm_pace_000373"].mean()) if len(warm) else 0.0

    mapped = (
        pd.MultiIndex.from_frame(out[["Race", "Compound"]]).map(prior).astype("float32")
    )
    out["warmup_prior_race_compound_000373"] = (
        pd.Series(mapped, index=out.index).fillna(global_prior).astype("float32")
    )
    out["warmup_age_decay_000373"] = (
        out["warmup_prior_race_compound_000373"] / (out["TyreLife"].clip(lower=1) + 1.0)
    ).astype("float32")
    out["warmup_position_interaction_000373"] = (
        out["warmup_prior_race_compound_000373"] * (21.0 - out["Position"]) / 20.0
    ).astype("float32")

    work = work.sort_values("_ord_000373", kind="mergesort")
    add_cols = [
        c
        for c in work.columns
        if c.endswith("_000373") and c not in out.columns and c != "_ord_000373"
    ]
    for c in add_cols:
        out[c] = work[c].to_numpy()

    out["tyrelife_progress_000373"] = (out["TyreLife"] * out["RaceProgress"]).astype(
        "float32"
    )
    out["old_tyre_position_pressure_000373"] = (
        out["TyreLife"] * (21.0 - out["Position"]) / 20.0
    ).astype("float32")
    out = out.drop(columns=["_ord_000373"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000373"],
        "research_usage_note": "Verified assigned hypothesis 000373.",
    }, sort_keys=True))
