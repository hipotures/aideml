...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    key_cols = ["Race", "Year", "Driver", "Stint"]
    sort_cols = key_cols + ["LapNumber"]

    out["_orig_order"] = np.arange(len(out))
    out = out.sort_values(sort_cols, kind="mergesort")

    g = out.groupby(key_cols, sort=False, observed=True)
    ...
    race_lap_med = out.groupby(["Race", "Year", "LapNumber"], observed=True)[
        "LapTime (s)"
    ].transform("median")
    residual = lap_time - race_lap_med
    out["latent_lap_residual_prev"] = (
        g[residual.name if residual.name in out.columns else "LapTime (s)"].shift(1)
        if False
        else residual.groupby([out[c] for c in key_cols], sort=False).shift(1)
    )

    out["latent_delta_prev"] = g["LapTime_Delta"].shift(1)
    out["latent_degr_prev"] = g["Cumulative_Degradation"].shift(1)
    out["latent_tyre_life_prev"] = g["TyreLife"].shift(1)

    prev_resid = out["latent_lap_residual_prev"].fillna(0.0)
    prev_delta = out["latent_delta_prev"].fillna(0.0)
    prev_degr = out["latent_degr_prev"].fillna(0.0)

    out["latent_resid_ewm"] = prev_resid.groupby(
        [out[c] for c in key_cols], sort=False
    ).transform(lambda s: s.ewm(alpha=0.35, adjust=False).mean())
    out["latent_delta_ewm"] = prev_delta.groupby(
        [out[c] for c in key_cols], sort=False
    ).transform(lambda s: s.ewm(alpha=0.35, adjust=False).mean())
    out["latent_degr_ewm"] = prev_degr.groupby(
        [out[c] for c in key_cols], sort=False
    ).transform(lambda s: s.ewm(alpha=0.25, adjust=False).mean())

    degr_step = degr - g["Cumulative_Degradation"].shift(1)
    out["latent_degr_slope_prev"] = degr_step.groupby(
        [out[c] for c in key_cols], sort=False
    ).shift(1)
    out["latent_degr_slope_ewm"] = (
        out["latent_degr_slope_prev"]
        .fillna(0.0)
        .groupby([out[c] for c in key_cols], sort=False)
        .transform(lambda s: s.ewm(alpha=0.40, adjust=False).mean())
    )

    slope_accel = out["latent_degr_slope_prev"] - out.groupby(
        key_cols, sort=False, observed=True
    )["latent_degr_slope_prev"].shift(1)
    out["latent_degr_accel_prev"] = slope_accel.fillna(0.0)

    stint_start_degr = g["Cumulative_Degradation"].transform("first")
    stint_start_lap_time = g["LapTime (s)"].transform("first")
    out["latent_post_reset_degr_depth"] = (degr - stint_start_degr).clip(lower=0)
    out["latent_post_reset_pace_loss"] = (
        (lap_time - stint_start_lap_time)
        .groupby([out[c] for c in key_cols], sort=False)
        .shift(1)
        .fillna(0.0)
    )

    out["latent_degr_per_tyre_lap"] = out["latent_degr_prev"] / out[
        "latent_tyre_life_prev"
    ].replace(0, np.nan)
    out["latent_resid_per_tyre_lap"] = out["latent_lap_residual_prev"] / out[
        "latent_tyre_life_prev"
    ].replace(0, np.nan)
    out["latent_cliff_pressure"] = (
        out["latent_resid_ewm"].clip(lower=0)
        + out["latent_delta_ewm"].clip(lower=0)
        + out["latent_degr_slope_ewm"].clip(lower=0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000218"],
        "research_usage_note": "Verified assigned hypothesis 000218.",
    }, sort_keys=True))
