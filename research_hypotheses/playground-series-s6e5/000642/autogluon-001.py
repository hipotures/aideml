...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = df["Compound"].astype("string").str.upper().fillna("UNKNOWN")
    is_soft = compound.eq("SOFT").astype("int8")
    is_medium = compound.eq("MEDIUM").astype("int8")
    is_hard = compound.eq("HARD").astype("int8")
    is_intermediate = compound.eq("INTERMEDIATE").astype("int8")
    is_wet = compound.eq("WET").astype("int8")

    is_dry = (is_soft | is_medium | is_hard).astype("int8")
    is_wet_regime = (is_intermediate | is_wet).astype("int8")
    is_unknown_compound = (
        ~compound.isin(["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"])
    ).astype("int8")
    is_soft_family = (compound.isin(["SOFT", "MEDIUM"])).astype("int8")

    df["is_dry_compound"] = is_dry
    df["is_wet_intermediate"] = is_wet_regime
    df["is_unknown_compound"] = is_unknown_compound
    df["is_soft_family"] = is_soft_family
    df["compound_is_soft"] = is_soft
    df["compound_is_medium"] = is_medium
    df["compound_is_hard"] = is_hard
    df["compound_is_intermediate"] = is_intermediate
    df["compound_is_wet"] = is_wet
    ...
    race_prog = df["RaceProgress"].astype("float32")
    ...
    df["tyre_life_sq"] = tyre_life * tyre_life
    df["degradation_per_lap"] = degr / tyre_life.clip(lower=1.0)
    df["stint_progress_proxy"] = tyre_life / lap_num.clip(lower=1.0)
    df["remaining_race_laps_proxy"] = lap_num * (1.0 / race_prog.clip(lower=1e-3) - 1.0)
    df["position_pressure"] = pos * race_prog
    df["track_position_gain_pressure"] = pos_chg * race_prog
    df["lap_delta_abs"] = lap_delta.abs()
    df["lap_time_log"] = np.log1p(lap_time.clip(lower=0))
    df["degradation_x_stint"] = degr * stint
    df["tyre_life_x_stint"] = tyre_life * stint
    ...
    for base_col in [
        ...
        x = df[base_col].astype("float32")
        df[f"{base_col}_x_dry"] = x * is_dry
        df[f"{base_col}_x_wet"] = x * is_wet_regime
        df[f"{base_col}_x_soft_family"] = x * is_soft_family

    df["dry_degr_pressure"] = degr * tyre_life * is_dry
    df["dry_position_degr"] = degr * pos * is_dry
    df["dry_end_stint_signal"] = tyre_life * race_prog * is_dry
    df["wet_track_pos_signal"] = pos * is_wet_regime
    df["wet_delta_signal"] = lap_delta.abs() * is_wet_regime
    df["wet_late_race_signal"] = race_prog * is_wet_regime
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000642.",
    }, sort_keys=True))
