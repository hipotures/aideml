...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = keys + ["LapNumber", "TyreLife"]
    s = out.sort_values(sort_cols, kind="mergesort").copy()

    x = s["TyreLife"].astype("float64").to_numpy()
    x2 = x * x
    x3 = x2 * x
    x4 = x2 * x2

    g = s.groupby(keys, sort=False)
    n = g.cumcount().astype("float64").to_numpy()

    sx = (
        pd.Series(x, index=s.index)
        .groupby([s[k] for k in keys], sort=False)
        .cumsum()
        .to_numpy()
        - x
    )
    sx2 = (
        pd.Series(x2, index=s.index)
        .groupby([s[k] for k in keys], sort=False)
        .cumsum()
        .to_numpy()
        - x2
    ...
    for col, prefix in [("LapTime (s)", "lt"), ("LapTime_Delta", "ltd")]:
        y = s[col].astype("float64").to_numpy()
        xy = x * y
        x2y = x2 * y
        y2 = y * y

        sy = (
            pd.Series(y, index=s.index)
            .groupby([s[k] for k in keys], sort=False)
            .cumsum()
            .to_numpy()
            - y
        ...
        a00, a01, a02 = n, sx, sx2
        a11, a12, a22 = sx2, sx3, sx4
        b0, b1, b2 = sy, sxy, sx2y
        ...
        ok_quad = (n >= 3) & (np.abs(det) > 1e-9)
        intercept = np.where(ok_quad, c0_num / det, np.nan)
        slope = np.where(ok_quad, c1_num / det, np.nan)
        curvature = np.where(ok_quad, c2_num / det, np.nan)

        lin_den = n * sx2 - sx * sx
        ok_lin = (n >= 2) & (np.abs(lin_den) > 1e-9)
        lin_slope = np.where(ok_lin, (n * sxy - sx * sy) / lin_den, np.nan)
        lin_intercept = np.where(
            ok_lin, (sy - lin_slope * sx) / np.maximum(n, 1), np.nan
        )

        slope = np.where(np.isfinite(slope), slope, lin_slope)
        curvature = np.where(np.isfinite(curvature), curvature, 0.0)
        intercept = np.where(np.isfinite(intercept), intercept, lin_intercept)

        pred = intercept + slope * x + curvature * x2
        resid = y - pred

        past_mean = sy / np.maximum(n, 1)
        past_var = sy2 / np.maximum(n, 1) - past_mean * past_mean
        past_std = np.sqrt(np.maximum(past_var, 1e-6))
        resid_z = resid / past_std

        fit_slope_now = slope + 2.0 * curvature * x
        s[prefix + "_online_n"] = n
        s[prefix + "_online_slope"] = fit_slope_now
        s[prefix + "_online_curvature"] = curvature
        s[prefix + "_online_resid"] = np.where(n >= 2, resid, 0.0)
        s[prefix + "_online_resid_z"] = np.where(n >= 2, resid_z, 0.0)
        s[prefix + "_online_slope_chg"] = (
            pd.Series(fit_slope_now, index=s.index)
            .groupby([s[k] for k in keys], sort=False)
            .diff()
            .fillna(0.0)
            .to_numpy()
    ...
    new_cols = [c for c in s.columns if c not in out.columns]
    out.loc[s.index, new_cols] = (
        s[new_cols].replace([np.inf, -np.inf], np.nan).fillna(0.0)
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000143"],
        "research_usage_note": "Verified assigned hypothesis 000143.",
    }, sort_keys=True))
