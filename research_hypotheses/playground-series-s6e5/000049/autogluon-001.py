...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    same_lap_keys = [c for c in ["Year", "Race", "LapNumber"] if c in out.columns]
    if same_lap_keys:
        for col in num_cols:
            safe = (
                col.replace(" ", "_")
            ...
            med = out.groupby(same_lap_keys, observed=True)[col].transform("median")
            out[f"{safe}_minus_race_lap_median"] = out[col] - med

    for key in ["Compound", "Year"]:
        if key in out.columns:
            for col in num_cols:
                ...
                med = out.groupby(key, observed=True)[col].transform("median")
                out[f"{safe}_minus_{key}_median"] = out[col] - med
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000049"],
        "research_usage_note": "Verified assigned hypothesis 000049.",
    }, sort_keys=True))
