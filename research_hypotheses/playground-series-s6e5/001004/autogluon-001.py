...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(c in out.columns for c in group_cols):
        ...
        out["Lap_Group_Size"] = g["LapNumber"].transform("size").astype("int16")
        ...
        for c in [x for x in rank_cols if x in out.columns]:
            mean = g[c].transform("mean")
            std = g[c].transform("std").replace(0, np.nan)
            out[f"{c}_lap_z"] = ((out[c] - mean) / std).fillna(0).astype("float32")
            out[f"{c}_lap_pct_rank"] = (
    ...
    if {"TyreLife", "Stint"}.issubset(out.columns):
        out["TyreLife_per_Stint"] = (out["TyreLife"] / (out["Stint"] + 1)).astype(
            "float32"
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001004"],
        "research_usage_note": "Verified assigned hypothesis 001004.",
    }, sort_keys=True))
