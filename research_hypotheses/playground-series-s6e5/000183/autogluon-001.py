...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    default_nom = {
        "bahrain_grand_prix": (1, 2, 3),
        "saudi_arabian_grand_prix": (2, 3, 4),
        "australian_grand_prix": (2, 3, 4),
        "emilia_romagna_grand_prix": (3, 4, 5),
        "miami_grand_prix": (2, 3, 4),
        "spanish_grand_prix": (1, 2, 3),
        "monaco_grand_prix": (3, 4, 5),
        "azerbaijan_grand_prix": (3, 4, 5),
        "canadian_grand_prix": (3, 4, 5),
        "british_grand_prix": (1, 2, 3),
        "austrian_grand_prix": (3, 4, 5),
        "french_grand_prix": (2, 3, 4),
        "hungarian_grand_prix": (3, 4, 5),
        "belgian_grand_prix": (2, 3, 4),
        "dutch_grand_prix": (1, 2, 3),
        "italian_grand_prix": (3, 4, 5),
        "singapore_grand_prix": (3, 4, 5),
        "japanese_grand_prix": (1, 2, 3),
        "qatar_grand_prix": (1, 2, 3),
        "united_states_grand_prix": (2, 3, 4),
        "mexico_city_grand_prix": (3, 4, 5),
        "sao_paulo_grand_prix": (2, 3, 4),
        "las_vegas_grand_prix": (3, 4, 5),
        "abu_dhabi_grand_prix": (3, 4, 5),
        "pre_season_testing": (1, 2, 3),
        "chinese_grand_prix": (2, 3, 4),
    }
    overrides = {
        (2024, "qatar_grand_prix"): (1, 2, 3),
        (2024, "las_vegas_grand_prix"): (3, 4, 5),
        (2024, "australian_grand_prix"): (3, 4, 5),
        (2024, "spanish_grand_prix"): (1, 2, 3),
        (2024, "dutch_grand_prix"): (1, 2, 3),
        (2024, "singapore_grand_prix"): (3, 4, 5),
        (2025, "australian_grand_prix"): (3, 4, 5),
    }

    hard_c = np.full(len(out), 2.0)
    med_c = np.full(len(out), 3.0)
    soft_c = np.full(len(out), 4.0)
    for r, vals in default_nom.items():
        m = race_key.eq(r).to_numpy()
        hard_c[m], med_c[m], soft_c[m] = vals
    for (y, r), vals in overrides.items():
        m = (year.eq(y) & race_key.eq(r)).to_numpy()
        hard_c[m], med_c[m], soft_c[m] = vals
    ...
    abs_c = np.select(
        ...
        [hard_c, med_c, soft_c, 0.5, 0.0],
        default=med_c,
    )

    out["AbsCompoundC"] = abs_c
    out["AbsCompoundSoftness"] = abs_c / 5.0
    out["WeekendHardC"] = hard_c
    out["WeekendMediumC"] = med_c
    out["WeekendSoftC"] = soft_c
    out["WeekendSoftnessSpan"] = soft_c - hard_c

    expected_life = np.select(
        [abs_c <= 0.5, abs_c <= 1.5, abs_c <= 2.5, abs_c <= 3.5, abs_c <= 4.5],
        [18.0, 36.0, 30.0, 24.0, 18.0],
        default=14.0,
    )
    tyre_life = out["TyreLife"].astype(float)
    progress = out["RaceProgress"].astype(float).clip(0.01, 1.0)
    est_total_laps = out["LapNumber"].astype(float) / progress
    laps_remaining = (est_total_laps - out["LapNumber"].astype(float)).clip(lower=0)

    out["ExpectedLifeAbsCompound"] = expected_life
    out["TyreLifeExpectedRatio"] = tyre_life / (expected_life + 1e-6)
    out["TyreLifeOverExpected"] = tyre_life - expected_life
    out["LapsRemaining_Est"] = laps_remaining
    out["ExpectedLifeRemaining"] = expected_life - tyre_life
    out["LifePressure_x_Progress"] = out["TyreLifeExpectedRatio"] * progress
    out["Softness_x_TyreLife"] = out["AbsCompoundSoftness"] * tyre_life
    out["Softness_x_LapsRemaining"] = out["AbsCompoundSoftness"] * laps_remaining

    two_stop_prior = (
        race_key.map(
            {
                "bahrain_grand_prix": 0.75,
                "saudi_arabian_grand_prix": 0.35,
                "australian_grand_prix": 0.55,
                "emilia_romagna_grand_prix": 0.35,
                "miami_grand_prix": 0.45,
                "spanish_grand_prix": 0.70,
                "monaco_grand_prix": 0.15,
                "azerbaijan_grand_prix": 0.35,
                "canadian_grand_prix": 0.45,
                "british_grand_prix": 0.65,
                "austrian_grand_prix": 0.55,
                "french_grand_prix": 0.50,
                "hungarian_grand_prix": 0.60,
                "belgian_grand_prix": 0.45,
                "dutch_grand_prix": 0.55,
                "italian_grand_prix": 0.30,
                "singapore_grand_prix": 0.45,
                "japanese_grand_prix": 0.60,
                "qatar_grand_prix": 0.85,
                "united_states_grand_prix": 0.60,
                "mexico_city_grand_prix": 0.40,
                "sao_paulo_grand_prix": 0.55,
                "las_vegas_grand_prix": 0.35,
                "abu_dhabi_grand_prix": 0.35,
                "chinese_grand_prix": 0.45,
                "pre_season_testing": 0.50,
            }
        )
        .fillna(0.50)
        .astype(float)
    ...
    out["TwoStopPrior"] = two_stop_prior
    out["OneStopPrior"] = 1.0 - two_stop_prior
    out["TwoStopPrior_x_Progress"] = two_stop_prior * progress
    out["TwoStopPrior_x_LifeRatio"] = two_stop_prior * out["TyreLifeExpectedRatio"]
    out["PriorLifePressure"] = two_stop_prior * out["TyreLifeExpectedRatio"] * progress
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000183"],
        "research_usage_note": "Verified assigned hypothesis 000183.",
    }, sort_keys=True))
