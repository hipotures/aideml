...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Stable numeric cleanup for the observed variables used by the recursive filter.
    lap_time = pd.to_numeric(out["LapTime (s)"], errors="coerce").astype(float)
    ...
    pit = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0).astype(int)
    ...
    # Residualize lap time with only broad same-row context, avoiding future target use.
    compound_med = lap_time.groupby(out["Compound"], sort=False).transform("median")
    race_med = lap_time.groupby(out["Race"], sort=False).transform("median")
    global_med = float(lap_time.median())
    pace_resid = (
        lap_time.fillna(global_med)
        - 0.55 * compound_med.fillna(global_med)
        - 0.45 * race_med.fillna(global_med)
    )

    obs = (
        (
            0.50 * pace_resid
            + 0.30 * lap_delta.fillna(0.0)
            + 0.20 * (cum_deg.fillna(cum_deg.median()) / 25.0)
        )
    ...
    order = (
        out.assign(_orig=np.arange(n))
        .sort_values(sort_cols + ["_orig"], kind="mergesort")["_orig"]
        .to_numpy()
    )
    ...
    uncert = np.zeros(n, dtype=float)
    pred_next = np.zeros(n, dtype=float)
    ...
    races = out["Race"].to_numpy()
    drivers = out["Driver"].to_numpy()
    stints = (
        pd.to_numeric(out["Stint"], errors="coerce").fillna(0).astype(int).to_numpy()
    )
    tyre_arr = tyre_life.fillna(0.0).to_numpy(float)
    pit_arr = pit.to_numpy()

    for idx in order:
        key = (years[idx], races[idx], drivers[idx], stints[idx])
        if key != prev_key or tyre_arr[idx] <= 1.0:
            x_level = 0.0
            x_slope = 0.0
            prev_slope = 0.0
            x_uncert = 1.0
        ...
        predicted = x_level + x_slope
        innovation = obs[idx] - predicted
        gain = x_uncert / (x_uncert + r)

        new_level = predicted + alpha * gain * innovation
        new_slope = x_slope + beta * gain * innovation
        new_uncert = (
            (1.0 - gain) * x_uncert + q + min(max(tyre_arr[idx], 0.0), 80.0) * 0.001
        )

        level[idx] = new_level
        slope[idx] = new_slope
        accel[idx] = new_slope - prev_slope
        uncert[idx] = new_uncert
        pred_next[idx] = new_level + new_slope
        resid[idx] = innovation
    ...
    out["ss_tyre_level"] = level
    out["ss_tyre_slope"] = slope
    out["ss_tyre_accel"] = accel
    out["ss_tyre_uncert"] = uncert
    out["ss_pred_next_pace_loss"] = pred_next
    out["ss_pace_residual"] = resid
    out["ss_level_x_tyre_life"] = level * tyre_life.fillna(0.0).to_numpy(float)
    out["ss_risk_score"] = pred_next + 0.5 * slope + 0.25 * uncert
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000982"],
        "research_usage_note": "Verified assigned hypothesis 000982.",
    }, sort_keys=True))
