...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").fillna("NA")
    driver = out["Driver"].astype("string").fillna("NA")
    compound = out["Compound"].astype("string").fillna("NA")
    year = out["Year"].astype("int16")
    lap = out["LapNumber"].astype("float32")
    progress = out["RaceProgress"].clip(0.001, 1.0).astype("float32")
    stint = out["Stint"].astype("int16")
    pit = out["PitStop"].astype("int8")

    est_total_raw = (lap / progress).replace([np.inf, -np.inf], np.nan)
    race_year_key = race + "_" + year.astype("string")
    est_total = (
        est_total_raw.groupby(race_year_key)
        .transform("median")
        .fillna(est_total_raw.median())
        .clip(30, 90)
    )
    est_bucket = (np.round(est_total / 5) * 5).astype("int16")
    lap_frac = (lap / est_total).clip(0, 1.25).astype("float32")

    order = pd.DataFrame(
        {
            "_idx": np.arange(n),
            "_key": race_year_key + "_" + driver,
            "_lap": lap,
            "_compound": compound,
            "_pit": pit,
            "_frac": lap_frac,
        }
    ).sort_values(["_key", "_lap", "_idx"], kind="mergesort")

    start_compound = order.groupby("_key")["_compound"].transform("first")
    out.loc[order["_idx"].to_numpy(), "StartCompound"] = start_compound.astype(
        str
    ).to_numpy()
    start_compound = out["StartCompound"].astype("string").fillna(compound)

    prior_pit_count_sorted = order.groupby("_key")["_pit"].cumsum() - order["_pit"]
    prior_pit_count = pd.Series(index=idx, data=np.nan, dtype="float32")
    prior_pit_count.iloc[order["_idx"].to_numpy()] = prior_pit_count_sorted.astype(
        "float32"
    ).to_numpy()
    prior_pit_count = prior_pit_count.fillna(0)

    pit_rows = pd.DataFrame(
        {
            "Race": race,
            "EstBucket": est_bucket,
    ...
    pit_rows = pit_rows[pit_rows["PitStop"] == 1].copy()

    def add_template(keys, prefix):
        if len(pit_rows) == 0:
            out[prefix + "_next_stop_frac"] = np.nan
            out[prefix + "_stop_count"] = 0.0
            out[prefix + "_confidence"] = 0.0
            return

        stats = (
            pit_rows.groupby(keys + ["Stint"], observed=True)["PitFrac"]
            .agg(["median", "count"])
            .reset_index()
        )
        counts = (
            pit_rows.groupby(keys, observed=True)["PitFrac"]
            .size()
            .rename(prefix + "_stop_count")
            .reset_index()
        )
        max_count = float(counts[prefix + "_stop_count"].max()) if len(counts) else 1.0
        ...
        merged = base.merge(stats, how="left", on=keys + ["Stint"])
        count_merged = pd.DataFrame(
            {
                "Race": race,
                "EstBucket": est_bucket,
                "StartCompound": start_compound,
        ...
        ).merge(counts, how="left", on=keys)

        out[prefix + "_next_stop_frac"] = merged["median"].astype("float32").to_numpy()
        out[prefix + "_confidence"] = (
            (merged["count"].fillna(0).clip(0, 25) / 25.0).astype("float32").to_numpy()
        )
        out[prefix + "_stop_count"] = (
            (count_merged[prefix + "_stop_count"].fillna(0) / max_count)
            .astype("float32")
            .to_numpy()
        )

    add_template(["Race", "EstBucket", "StartCompound", "Compound"], "tpl_exact")
    add_template(["Race", "EstBucket", "Compound"], "tpl_compound")
    add_template(["Race", "EstBucket"], "tpl_race")

    exact_conf = out["tpl_exact_confidence"].to_numpy()
    comp_conf = out["tpl_compound_confidence"].to_numpy()
    exact_next = out["tpl_exact_next_stop_frac"].to_numpy()
    comp_next = out["tpl_compound_next_stop_frac"].to_numpy()
    race_next = out["tpl_race_next_stop_frac"].to_numpy()

    next_frac = np.where(
        exact_conf > 0, exact_next, np.where(comp_conf > 0, comp_next, race_next)
    )
    confidence = np.maximum.reduce(
        [
            out["tpl_exact_confidence"].to_numpy(),
            out["tpl_compound_confidence"].to_numpy() * 0.75,
            out["tpl_race_confidence"].to_numpy() * 0.50,
        ]
    )
    stop_count = np.where(
        exact_conf > 0,
        out["tpl_exact_stop_count"].to_numpy(),
        np.where(
            comp_conf > 0,
            out["tpl_compound_stop_count"].to_numpy(),
            out["tpl_race_stop_count"].to_numpy(),
        ),
    )

    signed_dist_laps = (next_frac - lap_frac.to_numpy()) * est_total.to_numpy()
    signed_dist_laps = np.nan_to_num(
        signed_dist_laps, nan=99.0, posinf=99.0, neginf=-99.0
    )

    out["EstimatedTotalLaps"] = est_total.astype("float32")
    out["LapFraction"] = lap_frac.astype("float32")
    out["PriorPitCount"] = prior_pit_count.astype("float32")
    out["TemplateNextStopFrac"] = np.nan_to_num(next_frac, nan=-1.0).astype("float32")
    out["TemplateSignedDistLaps"] = np.clip(signed_dist_laps, -30, 30).astype("float32")
    out["TemplateAbsDistLaps"] = np.abs(out["TemplateSignedDistLaps"]).astype("float32")
    out["TemplateStopCount"] = np.nan_to_num(stop_count, nan=0.0).astype("float32")
    out["TemplateConfidence"] = np.nan_to_num(confidence, nan=0.0).astype("float32")
    out["TemplateDueSoon"] = (
        (out["TemplateSignedDistLaps"] >= -1) & (out["TemplateSignedDistLaps"] <= 2)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000515"],
        "research_usage_note": "Verified assigned hypothesis 000515.",
    }, sort_keys=True))
