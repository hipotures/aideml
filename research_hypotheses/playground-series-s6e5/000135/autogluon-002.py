...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_count = out.groupby(lap_keys, sort=False)["Driver"].transform("size").astype(
        "float32"
    )
    lap_pit_sum = out.groupby(lap_keys, sort=False)["PitStop"].transform("sum").astype(
        "float32"
    )
    other_count = (lap_count - 1.0).clip(lower=1.0)

    out["h135_field_pit_count_other"] = (lap_pit_sum - pit_now).clip(lower=0).astype(
        "float32"
    )
    out["h135_field_pit_rate_other"] = (
        out["h135_field_pit_count_other"] / other_count
    ).astype("float32")

    pos = out["Position"].astype("float32")
    position_pits = (
        out[lap_keys + ["Position", "PitStop"]]
        .groupby(lap_keys + ["Position"], sort=False)["PitStop"]
        .sum()
    )
    nearby_pits = np.zeros(len(out), dtype="float32")
    nearby_slots = np.zeros(len(out), dtype="float32")
    for offset in (-2, -1, 1, 2):
        lookup = pd.MultiIndex.from_frame(
            out[lap_keys].assign(Position=out["Position"] + offset)
        )
        values = pd.Series(lookup.map(position_pits), index=out.index)
        nearby_pits += values.fillna(0).astype("float32").to_numpy()
        nearby_slots += values.notna().astype("float32").to_numpy()

    out["h135_nearby_position_pit_count"] = nearby_pits
    out["h135_nearby_position_pit_rate"] = (
        nearby_pits / pd.Series(nearby_slots, index=out.index).clip(lower=1.0)
    ).astype("float32")

    lap_delta = out["LapTime_Delta"].astype("float32")
    lap_delta_med = out.groupby(lap_keys, sort=False)["LapTime_Delta"].transform(
        "median"
    )
    race_delta_med = out.groupby(race_keys, sort=False)["LapTime_Delta"].transform(
        "median"
    )
    compound_delta_med = out.groupby(
        race_keys + ["Compound"], sort=False
    )["LapTime_Delta"].transform("median")
    out["h135_lap_delta_vs_field"] = (lap_delta - lap_delta_med).astype("float32")
    out["h135_lap_delta_vs_race"] = (lap_delta - race_delta_med).astype("float32")
    out["h135_lap_delta_vs_compound"] = (lap_delta - compound_delta_med).astype(
        "float32"
    )

    rank_pct = out.groupby(lap_keys, sort=False)["Position"].rank(
        method="average", pct=True
    )
    out["h135_position_pct"] = rank_pct.astype("float32")
    out["h135_front_clean_air_proxy"] = (
        (1.0 - rank_pct.fillna(1.0)) * (1.0 - out["h135_field_pit_rate_other"])
    ).astype("float32")
    out["h135_traffic_density_proxy"] = (
        out["h135_nearby_position_pit_rate"]
        + (1.0 - (pos - 10.5).abs().clip(0, 10) / 10.0)
    ).astype("float32")

    max_lap = out.groupby(race_keys, sort=False)["LapNumber"].transform("max").astype(
        "float32"
    )
    laps_remaining = (max_lap - out["LapNumber"].astype("float32")).clip(lower=0)
    compound_pressure = (
        out["Compound"]
        .astype("string")
        .str.upper()
        .map({"SOFT": 1.20, "MEDIUM": 1.00, "HARD": 0.85, "INTERMEDIATE": 1.10, "WET": 1.05})
        .fillna(1.0)
        .astype("float32")
    )
    tyre_life = out["TyreLife"].astype("float32")

    out["h135_cover_stop_pressure"] = (
        out["h135_field_pit_rate_other"]
        * (1.0 + out["h135_nearby_position_pit_count"])
        * (1.0 + tyre_life.clip(0, 60) / 30.0)
        * compound_pressure
    ).astype("float32")
    out["h135_undercut_vulnerability"] = (
        out["h135_traffic_density_proxy"]
        * out["h135_lap_delta_vs_field"].clip(lower=0)
        * out["RaceProgress"].astype("float32").clip(0.0, 1.0)
    ).astype("float32")
    out["h135_laps_remaining_x_cover"] = (
        laps_remaining * out["h135_cover_stop_pressure"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000135"],
        "research_usage_note": "Verified assigned hypothesis 000135.",
    }, sort_keys=True))
