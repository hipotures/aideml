...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_slick = compound.isin(slicks)
    is_wet = compound.isin(["INTERMEDIATE", "WET"])

    keys = ["Year", "Race", "Driver"]
    ordered = out.assign(_orig_order=np.arange(len(out)), _compound=compound).sort_values(
        keys + ["LapNumber", "_orig_order"], kind="mergesort"
    )
    g = ordered.groupby(keys, sort=False)
    prev_compound = g["_compound"].shift(1)
    compound_changed = ordered["_compound"].ne(prev_compound) & prev_compound.notna()
    prior_change = compound_changed.groupby([ordered[c] for c in keys], sort=False).shift(1).fillna(False)

    for c in slicks:
        seen = ordered["_compound"].eq(c).groupby([ordered[k] for k in keys], sort=False).cummax()
        ordered[f"used_{c.lower()}_before"] = seen.groupby([ordered[k] for k in keys], sort=False).shift(1).fillna(False).astype("int8")

    used_slick_count = sum(ordered[f"used_{c.lower()}_before"] for c in slicks)
    out_cols = [f"used_{c.lower()}_before" for c in slicks]
    restored = ordered.sort_values("_orig_order", kind="mergesort")
    out[out_cols] = restored[out_cols].to_numpy()

    progress = out["RaceProgress"].astype(float).clip(0.001, 1.0)
    lap = out["LapNumber"].astype(float).clip(lower=1.0)
    est_total = (lap / progress).clip(1.0, 120.0)
    laps_left = (est_total - lap).clip(lower=0.0)
    ...
    out["next_compound_prior_strength"] = compound.map(base_pref).fillna(0.5).astype("float32")
    out["next_compound_legal_options"] = (3 - used_slick_count.reindex(restored.index).to_numpy()).clip(0, 3).astype("float32")
    out["next_compound_need_new_slick"] = ((is_slick.to_numpy()) & (out["next_compound_legal_options"] > 0)).astype("int8")
    out["next_compound_wet_context"] = is_wet.astype("int8")
    out["next_compound_laps_left"] = laps_left.astype("float32")
    out["next_compound_finish_feasible"] = (
        (laps_left <= (out["TyreLife"].astype(float).clip(lower=1.0) + 18.0))
        | is_wet.astype(bool)
    ).astype("int8")
    out["compound_transition_seen_before"] = prior_change.reindex(restored.index).fillna(False).to_numpy().astype("int8")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000205"],
        "research_usage_note": "Verified assigned hypothesis 000205.",
    }, sort_keys=True))
