...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["CompoundOrdinal"] = compound.map(compound_order).fillna(-1).astype("int8")
    df["WarmupRisk"] = compound.map(warmup_risk_map).fillna(0.5).astype("float32")
    df["FreshTyreGripGain"] = compound.map(grip_gain_map).fillna(0.5).astype("float32")
    df["IsHard"] = (compound == "HARD").astype("int8")
    df["IsCoolWet"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")

    race_keys = ["Year", "Race"]
    lap_keys = ["Year", "Race", "LapNumber"]
    race_compound_keys = ["Year", "Race", "Compound"]
    driver_race_keys = ["Year", "Race", "Driver"]

    field_size = df.groupby(race_keys)["Position"].transform("max").clip(lower=1)
    df["FieldSize"] = field_size.astype("float32")
    df["PositionPct"] = (df["Position"] / field_size).astype("float32")
    df["CarsAhead"] = (df["Position"] - 1).astype("float32")
    df["CarsBehind"] = (field_size - df["Position"]).astype("float32")
    df["CleanAirProxy"] = (1.0 - df["PositionPct"]).astype("float32")
    df["TrafficPenaltyProxy"] = (df["CarsAhead"] / field_size).astype("float32")

    lap_median_time = df.groupby(lap_keys)["LapTime (s)"].transform("median")
    lap_median_delta = df.groupby(lap_keys)["LapTime_Delta"].transform("median")
    compound_lap_median_delta = df.groupby(lap_keys + ["Compound"])[
        "LapTime_Delta"
    ].transform("median")
    compound_lap_median_deg = df.groupby(lap_keys + ["Compound"])[
        "Cumulative_Degradation"
    ].transform("median")

    df["PeerLapTimeVsMedian"] = (df["LapTime (s)"] - lap_median_time).astype("float32")
    df["PeerDeltaVsMedian"] = (df["LapTime_Delta"] - lap_median_delta).astype("float32")
    df["PeerCompoundDeltaVsMedian"] = (
        (df["LapTime_Delta"] - compound_lap_median_delta)
        .fillna(df["LapTime_Delta"] - lap_median_delta)
        .astype("float32")
    )
    df["PeerCompoundDegVsMedian"] = (
        (df["Cumulative_Degradation"] - compound_lap_median_deg)
        .fillna(0)
        .astype("float32")
    )

    race_median_lap = df.groupby(race_keys)["LapTime (s)"].transform("median")
    race_iqr_lap = (
        (
            df.groupby(race_keys)["LapTime (s)"].transform(
                lambda s: s.quantile(0.75) - s.quantile(0.25)
            )
        )
        .replace(0, np.nan)
        .fillna(race_median_lap * 0.05)
    )
    df["RacePitLossProxy"] = (race_median_lap * 0.28 + race_iqr_lap * 0.6).astype(
        "float32"
    )

    fresh_mask = df["TyreLife"] <= 3
    fresh_delta_baseline = (
        df["LapTime_Delta"]
        .where(fresh_mask)
        .groupby([df["Year"], df["Race"], df["Compound"]])
        .transform("median")
    )
    race_fresh_delta = (
        df["LapTime_Delta"]
        .where(fresh_mask)
        .groupby([df["Year"], df["Race"]])
        .transform("median")
    )
    fresh_deg_baseline = (
    ...
    df["FreshDeltaBaseline"] = (
        fresh_delta_baseline.fillna(race_fresh_delta).fillna(0).astype("float32")
    )
    df["FreshDegBaseline"] = (
        fresh_deg_baseline.fillna(race_fresh_deg).fillna(0).astype("float32")
    )

    tyre_life_safe = df["TyreLife"].clip(lower=1)
    df["DegPerLap"] = (
        (df["Cumulative_Degradation"] / tyre_life_safe)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    df["NextLapDegCost"] = (
        df["DegPerLap"] * (1.0 + 0.35 * df["WarmupRisk"] + 0.25 * df["RaceProgress"])
    ).astype("float32")

    df["CurrentTyrePenalty"] = (df["LapTime_Delta"] - df["FreshDeltaBaseline"]).astype(
        "float32"
    )
    df["DegradationPenalty"] = (
        df["Cumulative_Degradation"] - df["FreshDegBaseline"]
    ).astype("float32")
    df["FreshTyreBenefitProxy"] = (
        0.65 * df["CurrentTyrePenalty"]
        + 0.20 * df["DegradationPenalty"]
        + 0.15 * df["FreshTyreGripGain"] * df["TyreLife"]
    ).astype("float32")

    df["UndercutValueProxy"] = (
        df["FreshTyreBenefitProxy"]
        - 0.045 * df["RacePitLossProxy"]
        - 0.9 * df["TrafficPenaltyProxy"]
        + 0.6 * df["CleanAirProxy"]
        - 0.5 * df["WarmupRisk"]
        - 0.35 * df["NextLapDegCost"]
    ).astype("float32")

    df["StayOutValueProxy"] = (
        0.7 * df["CleanAirProxy"]
        - 0.5 * df["CurrentTyrePenalty"]
        - 0.3 * df["NextLapDegCost"]
        + 0.4 * df["TrafficPenaltyProxy"]
    ).astype("float32")

    df["PitNowMargin"] = (df["UndercutValueProxy"] - df["StayOutValueProxy"]).astype(
        "float32"
    )
    df["OvercutRisk"] = (
        (
            0.8 * df["WarmupRisk"]
            + 0.4 * df["TrafficPenaltyProxy"]
            - 0.35 * df["CleanAirProxy"]
        )
        * (1.0 + 0.6 * df["IsHard"] + 0.8 * df["IsCoolWet"])
    ).astype("float32")

    df["HardTyreOvercutRisk"] = (df["OvercutRisk"] * df["IsHard"]).astype("float32")
    df["CoolWetOvercutRisk"] = (df["OvercutRisk"] * df["IsCoolWet"]).astype("float32")
    df["ProtectedTrackPosition"] = (
        (df["CarsBehind"] >= 5).astype("int8") * (df["Position"] <= 10).astype("int8")
    ).astype("int8")
    df["UndercutWindowFlag"] = (
        (df["PitNowMargin"] > df["PitNowMargin"].median()).astype("int8")
        * (df["Position"].between(2, 15)).astype("int8")
    ).astype("int8")

    stint_median_life = df.groupby(race_keys + ["Stint"])["TyreLife"].transform(
        "median"
    )
    df["TyreLifeVsStintMedian"] = (df["TyreLife"] - stint_median_life).astype("float32")
    race_driver_pos_med = df.groupby(driver_race_keys)["Position"].transform("median")
    df["PositionVsDriverRaceMedian"] = (df["Position"] - race_driver_pos_med).astype(
        "float32"
    )

    for col in ["Driver", "Race", "Compound"]:
        freq = df[col].astype(str).value_counts(dropna=False)
        df[f"{col}_Freq"] = df[col].astype(str).map(freq).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000407"],
        "research_usage_note": "Verified assigned hypothesis 000407.",
    }, sort_keys=True))
