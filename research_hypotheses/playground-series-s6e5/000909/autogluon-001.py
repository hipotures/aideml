...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = out["Compound"].astype("category")
    comp_code = compound.cat.codes.astype("int16")
    out["CompoundCode"] = comp_code

    soft = (out["Compound"] == "SOFT").astype("int8")
    medium = (out["Compound"] == "MEDIUM").astype("int8")
    hard = (out["Compound"] == "HARD").astype("int8")
    inter = (out["Compound"] == "INTERMEDIATE").astype("int8")
    wet = (out["Compound"] == "WET").astype("int8")
    ...
    pit_now = out["PitStop"].astype("int8")

    out["TyreLife2"] = tyre * tyre
    out["TyreLife_Log"] = np.log1p(tyre)
    out["RemainingRace"] = (1.0 - progress).astype("float32")
    out["LapPerStint"] = (lap / (stint + 1.0)).astype("float32")
    out["TyreLifeProgress"] = (tyre * progress).astype("float32")
    out["DegradationPerTyreLap"] = (degr / (tyre + 1.0)).astype("float32")
    out["AbsLapTimeDelta"] = delta.abs().astype("float32")
    out["PositionLoss"] = np.maximum(pos_chg, 0).astype("float32")
    out["PositionGain"] = np.maximum(-pos_chg, 0).astype("float32")

    early = (progress < 0.12).astype("int8")
    late = (progress > 0.82).astype("int8")
    first_lap = (lap <= 2).astype("int8")
    just_pitted = ((pit_now == 1) | (tyre <= 2)).astype("int8")

    soft_win = ((soft == 1) & (tyre >= 8) & (tyre <= 28)).astype("int8")
    med_win = ((medium == 1) & (tyre >= 12) & (tyre <= 38)).astype("int8")
    hard_win = ((hard == 1) & (tyre >= 16) & (tyre <= 50)).astype("int8")
    wet_win = (((inter == 1) | (wet == 1)) & (tyre >= 3) & (tyre <= 24)).astype("int8")

    structural_zero = (
        (first_lap == 1) | (just_pitted == 1) | ((early == 1) & (tyre < 6))
    ).astype("int8")
    pit_window = (
        ((soft_win + med_win + hard_win + wet_win) > 0) & (structural_zero == 0)
    ).astype("int8")

    risk = (
        pit_window.astype("float32") * 1.5
        + (tyre >= 10).astype("float32") * 0.4
        + (tyre >= 18).astype("float32") * 0.6
        + (tyre >= 30).astype("float32") * 0.5
        + (progress > 0.18).astype("float32") * 0.3
        + (progress > 0.55).astype("float32") * 0.3
        + (degr > degr.median()).astype("float32") * 0.3
        + (delta > 0).astype("float32") * 0.2
        + late.astype("float32") * 0.2
        - structural_zero.astype("float32") * 2.0
    )

    out["Gate_StructuralZero"] = structural_zero
    out["Gate_PitWindow"] = pit_window.astype("int8")
    out["Gate_SoftWindow"] = soft_win
    out["Gate_MediumWindow"] = med_win
    out["Gate_HardWindow"] = hard_win
    out["Gate_WetWindow"] = wet_win
    out["Gate_RiskScore"] = risk.astype("float32")
    out["Gate_RiskXTyreLife"] = (risk * tyre).astype("float32")
    out["Gate_RiskXDegradation"] = (risk * out["DegradationPerTyreLap"]).astype(
        "float32"
    )
    out["Window_TyreLife"] = (pit_window * tyre).astype("float32")
    out["Window_Degradation"] = (pit_window * degr).astype("float32")
    out["Window_LapTimeDelta"] = (pit_window * delta).astype("float32")
    out["Window_PositionLoss"] = (pit_window * out["PositionLoss"]).astype("float32")
    out["PostPit_Cooldown"] = just_pitted
    out["LateRaceWindow"] = ((pit_window == 1) & (late == 1)).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000909.",
    }, sort_keys=True))
