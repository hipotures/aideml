...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_median = X.groupby(race_lap_keys, sort=False)["LapTime (s)"].transform("median")
    X["Latent_LapTime_Residual"] = X["LapTime (s)"] - lap_median

    g = X.groupby(stint_keys, sort=False)
    resid_l1 = g["Latent_LapTime_Residual"].shift(1)
    resid_l2 = g["Latent_LapTime_Residual"].shift(2)
    resid_l3 = g["Latent_LapTime_Residual"].shift(3)

    slope = resid_l1 - resid_l2
    prev_slope = resid_l2 - resid_l3

    group_arrays = [X[c] for c in stint_keys]

    X["Latent_PostPit_Age"] = g.cumcount()
    X["Latent_Residual_EWM"] = resid_l1.groupby(group_arrays, sort=False).transform(
        lambda s: s.ewm(alpha=0.35, adjust=False).mean()
    )
    X["Latent_Residual_Slope_EWM"] = slope.groupby(group_arrays, sort=False).transform(
        lambda s: s.ewm(alpha=0.4, adjust=False).mean()
    )
    X["Latent_Residual_Accel"] = slope - prev_slope
    X["Latent_Residual_RollVar4"] = resid_l1.groupby(
        group_arrays, sort=False
    ).transform(lambda s: s.rolling(window=4, min_periods=2).var())

    X["Latent_Residual_to_TyreLife"] = resid_l1 / X["TyreLife"].clip(lower=1.0)
    X["Latent_Slope_x_Degradation"] = (
        X["Latent_Residual_Slope_EWM"] * X["Cumulative_Degradation"]
    )
    X["Latent_Var_x_Age"] = X["Latent_Residual_RollVar4"] * X["Latent_PostPit_Age"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000636"],
        "research_usage_note": "Verified assigned hypothesis 000636.",
    }, sort_keys=True))
