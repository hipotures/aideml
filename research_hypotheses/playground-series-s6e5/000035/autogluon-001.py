...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", "Stint", "_row_order"]
    ...
    for col in ["LapTime_Delta", "Cumulative_Degradation"]:
        prev = g[col].shift(1)
        prev_g = prev.groupby(gid, sort=False)

        out[f"{col}_prior"] = prev
        out[f"{col}_prior_roll3_mean"] = (
            prev_g.rolling(3, min_periods=1).mean().reset_index(level=0, drop=True)
        )
        out[f"{col}_prior_roll5_mean"] = (
            prev_g.rolling(5, min_periods=1).mean().reset_index(level=0, drop=True)
        )
        out[f"{col}_prior_ewm3"] = (
            prev_g.ewm(span=3, adjust=False).mean().reset_index(level=0, drop=True)
        )
        out[f"{col}_prior_roll3_std"] = (
            prev_g.rolling(3, min_periods=2).std().reset_index(level=0, drop=True)
        )
        out[f"{col}_prior_roll5_std"] = (
            prev_g.rolling(5, min_periods=2).std().reset_index(level=0, drop=True)
        )

        prev_lag2 = prev_g.shift(2)
        prev_lag4 = prev_g.shift(4)
        out[f"{col}_prior_slope3"] = (prev - prev_lag2) / 2.0
        out[f"{col}_prior_slope5"] = (prev - prev_lag4) / 4.0
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000035"],
        "research_usage_note": "Verified assigned hypothesis 000035.",
    }, sort_keys=True))
