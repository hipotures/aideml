...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["compound_fresh_life"] = out["Compound"].map(compound_fresh_life).fillna(12.0)
    out["compound_decay_mult"] = out["Compound"].map(compound_decay_mult).fillna(1.0)
    out["compound_fresh_delta_bonus"] = (
        out["Compound"].map(compound_fresh_delta_bonus).fillna(0.0)
    )

    race_lap_keys = ["Year", "Race", "LapNumber"]
    out["race_lap_median_laptime"] = out.groupby(race_lap_keys)[
        "LapTime (s)"
    ].transform("median")
    out["race_lap_median_delta"] = out.groupby(race_lap_keys)[
        "LapTime_Delta"
    ].transform("median")
    out["race_lap_median_deg"] = out.groupby(race_lap_keys)[
        "Cumulative_Degradation"
    ].transform("median")
    out["race_lap_median_poschg"] = out.groupby(race_lap_keys)[
        "Position_Change"
    ].transform("median")

    comp_keys = ["Year", "Compound"]
    out["compound_median_laptime"] = out.groupby(comp_keys)["LapTime (s)"].transform(
        "median"
    )
    out["compound_median_delta"] = out.groupby(comp_keys)["LapTime_Delta"].transform(
        "median"
    )
    out["compound_median_deg"] = out.groupby(comp_keys)[
        "Cumulative_Degradation"
    ].transform("median")

    out["deg_per_life"] = out["Cumulative_Degradation"] / (
        out["TyreLife"].clip(lower=1.0)
    )
    out["deg_gap_vs_field"] = out["Cumulative_Degradation"] - out["race_lap_median_deg"]
    out["pace_gap_vs_field"] = out["LapTime (s)"] - out["race_lap_median_laptime"]
    out["delta_gap_vs_field"] = out["LapTime_Delta"] - out["race_lap_median_delta"]

    # Estimated cost of staying out one more lap: degradation pressure + observed pace drift.
    out["one_more_lap_penalty"] = (
        0.55 * out["deg_per_life"] * out["compound_decay_mult"]
        + 0.30 * out["LapTime_Delta"].clip(lower=0)
        + 0.15 * out["pace_gap_vs_field"].clip(lower=0)
    )

    # Cheap fresh-tyre benefit proxy from compound baseline and current tyre age.
    out["fresh_tyre_gain"] = (
        0.35 * (out["TyreLife"] - out["compound_fresh_life"]).clip(lower=0)
        + 0.30
        * (out["Cumulative_Degradation"] - out["compound_median_deg"]).clip(lower=0)
        + 0.20 * out["delta_gap_vs_field"].clip(lower=0)
        + out["compound_fresh_delta_bonus"]
    )

    # Position/traffic risk proxies for undercut pressure.
    out["traffic_risk"] = (
        0.6 * (21 - out["Position"]).clip(lower=0)
        + 0.8 * (-out["Position_Change"]).clip(lower=0)
        + 0.4 * out["pace_gap_vs_field"].clip(lower=0)
    )
    out["front_runner_risk"] = (1.0 / (out["Position"].clip(lower=1.0))) * (
        1.0 + out["RaceProgress"]
    )
    out["midfield_risk"] = ((out["Position"] >= 5) & (out["Position"] <= 15)).astype(
        float
    ) * (1.0 + (-out["Position_Change"]).clip(lower=0))

    out["pit_now_advantage"] = out["fresh_tyre_gain"] - out["one_more_lap_penalty"]
    out["risk_adjusted_undercut_score"] = (
        out["pit_now_advantage"]
        + 0.12 * out["traffic_risk"]
        + 0.08 * out["midfield_risk"]
    )
    out["stay_out_confidence"] = out["one_more_lap_penalty"] - out["fresh_tyre_gain"]

    out["pit_now_adv_x_risk"] = out["pit_now_advantage"] * (
        1.0 + out["traffic_risk"] / 10.0
    )
    out["penalty_x_progress"] = out["one_more_lap_penalty"] * out["RaceProgress"]
    out["advantage_x_progress"] = out["pit_now_advantage"] * out["RaceProgress"]
    out["deg_x_tyrelife"] = out["Cumulative_Degradation"] * out["TyreLife"]
    out["late_stint_pressure"] = out["Stint"] * out["RaceProgress"] * out["TyreLife"]
    out["position_pressure"] = (21 - out["Position"]) * out["RaceProgress"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000298"],
        "research_usage_note": "Verified assigned hypothesis 000298.",
    }, sort_keys=True))
