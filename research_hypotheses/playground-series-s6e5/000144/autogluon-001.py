...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Driver", "Race", "Compound"]:
        out[col] = out[col].astype("string").fillna("__NA__").astype("category")

    year_s = out["Year"].astype("string")
    driver_s = out["Driver"].astype("string")
    race_s = out["Race"].astype("string")
    compound_s = out["Compound"].astype("string")

    out["Driver__Race"] = (driver_s + "__" + race_s).astype("category")
    out["Driver__Compound"] = (driver_s + "__" + compound_s).astype("category")
    out["Race__Compound"] = (race_s + "__" + compound_s).astype("category")
    out["Race__Year__Compound"] = (race_s + "__" + year_s + "__" + compound_s).astype(
        "category"
    )

    race_code = out["Race"].cat.codes.astype("int32")
    driver_code = out["Driver"].cat.codes.astype("int32")
    out["catboost_time_order_key"] = (
        out["Year"].astype("int32") * 10_000_000
        + race_code * 100_000
        + driver_code * 100
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000144.",
    }, sort_keys=True))
