...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    grp = [out["Race"], out["Year"], out["LapNumber"]]

    out["laps_remaining_est"] = (
        out["LapNumber"] / out["RaceProgress"].clip(0.01, None) - out["LapNumber"]
    ).clip(0, 100)
    out["tyre_life_x_progress"] = out["TyreLife"] * out["RaceProgress"]
    out["tyre_life_x_laps_remaining"] = out["TyreLife"] * out["laps_remaining_est"]

    delta = out["LapTime_Delta"].astype(float)
    pos_churn = out["Position_Change"].abs().astype(float)

    out["race_lap_delta_median"] = delta.groupby(grp).transform("median")
    out["race_lap_delta_q75"] = delta.groupby(grp).transform(lambda s: s.quantile(0.75))
    out["race_lap_delta_q90"] = delta.groupby(grp).transform(lambda s: s.quantile(0.90))
    out["race_lap_delta_std"] = delta.groupby(grp).transform("std").fillna(0)
    out["race_lap_pos_churn_mean"] = pos_churn.groupby(grp).transform("mean")
    out["race_lap_pit_frac"] = (
        out["PitStop"].astype(float).groupby(grp).transform("mean")
    )
    out["race_lap_pit_count"] = (
        out["PitStop"].astype(float).groupby(grp).transform("sum")
    )

    slow_thr = delta.groupby([out["Race"], out["Year"]]).transform(
        lambda s: s.quantile(0.85)
    )
    out["race_lap_large_slow_frac"] = (
        (delta > slow_thr).astype(float).groupby(grp).transform("mean")
    )

    out["latent_caution_score"] = (
        out["race_lap_delta_q75"].rank(pct=True)
        + out["race_lap_large_slow_frac"].rank(pct=True)
        + out["race_lap_pit_frac"].rank(pct=True)
        + out["race_lap_pos_churn_mean"].rank(pct=True)
    ) / 4.0

    out["caution_x_tyre_life"] = out["latent_caution_score"] * out["TyreLife"]
    out["caution_x_laps_remaining"] = (
        out["latent_caution_score"] * out["laps_remaining_est"]
    )
    out["caution_x_stint"] = out["latent_caution_score"] * out["Stint"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000400"],
        "research_usage_note": "Verified assigned hypothesis 000400.",
    }, sort_keys=True))
