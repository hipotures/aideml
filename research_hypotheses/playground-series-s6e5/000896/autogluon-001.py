...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["TyreLife_x_CompoundAge"] = tyre * out["Compound_AgeFactor"]
    out["TyreLife_per_Stint"] = tyre / (stint + eps)
    out["LapLife_Ratio"] = tyre / (lap + eps)
    out["Laps_Remaining_Est"] = lap * (1.0 - progress) / progress
    out["LateRace_TyreLoad"] = tyre * progress
    out["Degradation_per_TyreLife"] = degr / (tyre + eps)
    out["Abs_LapTime_Delta"] = delta.abs()
    out["Position_Loss_Flag"] = (pos_chg > 0).astype("int8")
    out["Position_Gain_Flag"] = (pos_chg < 0).astype("int8")

    group_keys = ["Year", "Race"]
    rank_sources = {
        "expert_tyre_hazard": out["TyreLife_x_CompoundAge"],
        "expert_degradation_hazard": out["Degradation_per_TyreLife"],
        "expert_pace_anomaly": out["Abs_LapTime_Delta"],
        "expert_slow_lap": laptime,
        "expert_late_stint": out["LateRace_TyreLoad"],
        "expert_track_position": 21.0 - pos + pos_chg.clip(lower=0),
    }

    for name, s in rank_sources.items():
        s = s.replace([np.inf, -np.inf], np.nan).fillna(s.median())
        out[name] = s.astype("float32")
        out[name + "_rank"] = s.rank(method="average", pct=True).astype("float32")
        out[name + "_race_rank"] = (
            s.groupby([out[k] for k in group_keys])
            .rank(method="average", pct=True)
            .astype("float32")
        )

    out["rank_stack_mean"] = (
        out[[c + "_rank" for c in rank_sources]].mean(axis=1).astype("float32")
    )
    out["rank_stack_max"] = (
        out[[c + "_rank" for c in rank_sources]].max(axis=1).astype("float32")
    )
    out["race_rank_stack_mean"] = (
        out[[c + "_race_rank" for c in rank_sources]].mean(axis=1).astype("float32")
    )
    out["race_rank_stack_spread"] = (
        out[[c + "_race_rank" for c in rank_sources]].max(axis=1)
        - out[[c + "_race_rank" for c in rank_sources]].min(axis=1)
    ).astype("float32")

    for c in ["Year", "LapNumber", "Position", "Stint"]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000896.",
    }, sort_keys=True))
