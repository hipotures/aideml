...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def aligned(series):
        return series.reindex(out.index)

    g = out.loc[sidx].groupby(keys, sort=False, observed=True)
    ...
    for col in num_cols:
        if col in out.columns:
            x = out.loc[sidx, col]
            for lag in (1, 2, 3):
                out[f"{col}_lag{lag}"] = aligned(g[col].shift(lag))
            for win in (3, 5, 10):
                r = g[col].rolling(win, min_periods=1)
                out[f"{col}_prev{win}_mean"] = aligned(
                    r.mean()
                    .reset_index(level=keys, drop=True)
                    .groupby([out.loc[sidx, k] for k in keys], sort=False)
                    .shift(1)
                )
                out[f"{col}_prev{win}_max"] = aligned(
                    r.max()
                    .reset_index(level=keys, drop=True)
                    .groupby([out.loc[sidx, k] for k in keys], sort=False)
                    .shift(1)
    ...
    if "Cumulative_Degradation" in out.columns:
        deg = out.loc[sidx, "Cumulative_Degradation"]
        out["degradation_growth_1"] = aligned(
            deg - g["Cumulative_Degradation"].shift(1)
        )
        out["degradation_growth_3"] = aligned(
            deg - g["Cumulative_Degradation"].shift(3)
        )

    if "Position_Change" in out.columns:
        pc = out.loc[sidx, "Position_Change"]
        pos_gain = (pc < 0).astype("int8")
        pos_loss = (pc > 0).astype("int8")
        out["position_gain_prev3"] = aligned(
            pos_gain.groupby([out.loc[sidx, k] for k in keys], sort=False)
            .rolling(3, min_periods=1)
            .sum()
            .reset_index(level=keys, drop=True)
            .shift(1)
        )
        out["position_loss_prev3"] = aligned(
            pos_loss.groupby([out.loc[sidx, k] for k in keys], sort=False)
            .rolling(3, min_periods=1)
            .sum()
            .reset_index(level=keys, drop=True)
            .shift(1)
    ...
    if "PitStop" in out.columns:
        pit = out.loc[sidx, "PitStop"]
        for win in (3, 5, 10):
            out[f"pit_count_prev{win}"] = aligned(
                g["PitStop"]
        ...
        last_pit_lap = (
            out.loc[sidx, "LapNumber"]
            .where(pit.eq(1))
            .groupby([out.loc[sidx, k] for k in keys], sort=False)
            .ffill()
        )
        out["laps_since_prev_pit"] = aligned(
            out.loc[sidx, "LapNumber"]
            - last_pit_lap.groupby([out.loc[sidx, k] for k in keys], sort=False).shift(
                1
            )
        )

    if "Compound" in out.columns:
        comp = out.loc[sidx, "Compound"].astype("category").cat.codes
        out["compound_changed"] = aligned(
            (
                comp
                != comp.groupby([out.loc[sidx, k] for k in keys], sort=False).shift(1)
            ).astype("int8")
        )

    if "Stint" in out.columns:
        st = out.loc[sidx, "Stint"]
        out["stint_changed"] = aligned((st != g["Stint"].shift(1)).astype("int8"))
        out["tyre_life_x_stint"] = out["TyreLife"] * out["Stint"]

    if {"TyreLife", "RaceProgress"}.issubset(out.columns):
        out["tyre_life_progress_ratio"] = out["TyreLife"] / (
            out["RaceProgress"].clip(lower=0.01)
        )
        out["late_old_tyre"] = out["TyreLife"] * out["RaceProgress"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000151"],
        "research_usage_note": "Verified assigned hypothesis 000151.",
    }, sort_keys=True))
