...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Compound", "Driver", "Race"]:
        out[col] = out[col].astype("string").str.strip().str.upper().fillna("UNKNOWN")

    year_s = out["Year"].astype(str)
    stint_s = out["Stint"].astype(str)
    out["h000454_query_driver_race"] = out["Driver"] + "|" + year_s + "|" + out["Race"]
    out["h000454_query_driver_race_stint"] = (
        out["h000454_query_driver_race"] + "|S" + stint_s
    )

    query = ["Driver", "Year", "Race"]

    def rank_pct(col, ascending=True):
        r = out.groupby(query, sort=False)[col].rank(
            method="average", pct=True, ascending=ascending
        )
        return r.astype("float32").fillna(0.5)

    tyre = out["TyreLife"].astype("float32")
    lap = out["LapNumber"].astype("float32")
    progress = out["RaceProgress"].astype("float32")

    out["h000454_tyre_life_rank_q"] = rank_pct("TyreLife")
    out["h000454_lap_rank_q"] = rank_pct("LapNumber")
    out["h000454_progress_rank_q"] = rank_pct("RaceProgress")
    out["h000454_degradation_rank_q"] = rank_pct("Cumulative_Degradation")
    out["h000454_laptime_delta_rank_q"] = rank_pct("LapTime_Delta")
    out["h000454_laptime_rank_q"] = rank_pct("LapTime (s)")
    out["h000454_position_front_rank_q"] = rank_pct("Position", ascending=False)

    out["h000454_pos_laptime_delta"] = (
        out["LapTime_Delta"].clip(lower=0).astype("float32")
    )
    out["h000454_abs_position_change"] = out["Position_Change"].abs().astype("float32")
    out["h000454_pos_delta_rank_q"] = rank_pct("h000454_pos_laptime_delta")
    out["h000454_pos_change_abs_rank_q"] = rank_pct("h000454_abs_position_change")
    ...
    out["h000454_compound_rank_prior"] = compound_prior

    out["h000454_tyre_to_lap_ratio"] = (
        (tyre / lap.replace(0, np.nan)).fillna(0).clip(0, 2).astype("float32")
    )
    out["h000454_progress_x_log_tyre"] = (progress * np.log1p(tyre)).astype("float32")

    score = (
        1.15 * out["h000454_tyre_life_rank_q"]
        + 0.95 * out["h000454_progress_rank_q"]
        + 0.55 * out["h000454_degradation_rank_q"]
        + 0.30 * out["h000454_pos_delta_rank_q"]
        + 0.20 * out["h000454_laptime_rank_q"]
        + 0.20 * out["h000454_compound_rank_prior"]
        + 0.12 * out["h000454_progress_x_log_tyre"]
        + 0.10 * out["h000454_tyre_to_lap_ratio"]
        + 0.06 * out["Stint"].astype("float32")
    ).replace([np.inf, -np.inf], np.nan)

    fill_value = float(score.median()) if score.notna().any() else 0.0
    score = score.fillna(fill_value)
    center = float(score.median())
    ...
    z = ((score - center) / scale).clip(-20, 20)
    out["h000454_ranker_score_raw"] = score.astype("float32")
    out["h000454_ranker_sigmoid_cal"] = (1.0 / (1.0 + np.exp(-z))).astype("float32")
    out["h000454_ranker_query_rank"] = (
        out.groupby(query, sort=False)["h000454_ranker_score_raw"]
        .rank(method="average", pct=True)
        .astype("float32")
        .fillna(0.5)
    )
    out["h000454_rankavg_calibrated"] = (
        0.5 * out["h000454_ranker_sigmoid_cal"] + 0.5 * out["h000454_ranker_query_rank"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000454"],
        "research_usage_note": "Verified assigned hypothesis 000454.",
    }, sort_keys=True))
