...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dry = {"SOFT": 1, "MEDIUM": 2, "HARD": 4}

    work = out[keys + ["Compound", "LapNumber", "RaceProgress"]].copy()
    work["_row"] = np.arange(len(work))
    work["_dry_bit"] = work["Compound"].map(dry).fillna(0).astype(np.int8)

    ordered = work.sort_values(keys + ["LapNumber", "_row"], kind="mergesort")
    ordered["_seen_mask"] = ordered.groupby(keys, sort=False)["_dry_bit"].cummax()

    for bit in (1, 2, 4):
        seen_col = f"_seen_{bit}"
        ordered[seen_col] = (
            (ordered["_dry_bit"] == bit)
            .astype(np.int8)
            .groupby([ordered[k] for k in keys], sort=False)
            .cummax()
        )

    seen_count = ordered[["_seen_1", "_seen_2", "_seen_4"]].sum(axis=1).astype(np.int8)
    ordered["_has_seen_two_dry_compounds"] = (seen_count >= 2).astype(np.int8)
    ordered["_current_compound_only_so_far"] = (
        (ordered["_dry_bit"] > 0) & (seen_count == 1)
    ).astype(np.int8)

    max_lap = out.groupby(["Race", "Year"], sort=False)["LapNumber"].transform("max")
    out["laps_remaining_proxy"] = (
        (max_lap - out["LapNumber"]).clip(lower=0).astype(np.float32)
    )

    restored = ordered.sort_values("_row", kind="mergesort")
    out["has_seen_two_dry_compounds"] = restored[
        "_has_seen_two_dry_compounds"
    ].to_numpy(dtype=np.int8)
    out["current_compound_only_so_far"] = restored[
        "_current_compound_only_so_far"
    ].to_numpy(dtype=np.int8)

    denom = np.maximum(out["laps_remaining_proxy"].to_numpy(dtype=np.float32), 1.0)
    progress = out["RaceProgress"].to_numpy(dtype=np.float32)
    pressure_base = out["current_compound_only_so_far"].to_numpy(dtype=np.float32)
    out["rule_pressure_progress"] = pressure_base * progress
    out["rule_pressure_remaining"] = pressure_base * progress / denom
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000637"],
        "research_usage_note": "Verified assigned hypothesis 000637.",
    }, sort_keys=True))
