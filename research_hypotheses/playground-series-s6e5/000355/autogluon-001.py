...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp = out["Compound"].str.upper()
    out["CompoundCode"] = comp.map(compound_map).fillna(-1).astype("int8")
    out["IsWetCompound"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["IsSoftCompound"] = (comp == "SOFT").astype("int8")
    out["IsHardCompound"] = (comp == "HARD").astype("int8")

    lap = out["LapNumber"].astype("float32")
    tyre = out["TyreLife"].astype("float32")
    progress = out["RaceProgress"].astype("float32").clip(lower=1e-4)
    ...
    deg = out["Cumulative_Degradation"].astype("float32")

    est_race_laps = lap / progress
    laps_left = (est_race_laps - lap).clip(lower=0)

    out["EstRaceLaps"] = est_race_laps.astype("float32")
    out["EstLapsLeft"] = laps_left.astype("float32")
    out["TyreLifePctRace"] = (tyre / (est_race_laps + 1e-3)).astype("float32")
    out["TyreLifeXProgress"] = (tyre * progress).astype("float32")
    out["TyreLifePerStint"] = (tyre / stint.clip(lower=1)).astype("float32")
    out["DegPerTyreLife"] = (deg / tyre.clip(lower=1)).astype("float32")
    out["AbsLapTimeDelta"] = delta.abs().astype("float32")
    out["LapTimeDeltaPerTyreLife"] = (delta / tyre.clip(lower=1)).astype("float32")
    out["PositionPressure"] = ((21.0 - pos) * progress).astype("float32")
    out["LostPosition"] = (pos_chg > 0).astype("int8")
    out["GainedPosition"] = (pos_chg < 0).astype("int8")
    out["PitStopPrevSignal"] = out["PitStop"].astype("int8")

    out["RaceYear"] = (out["Race"] + "_" + out["Year"].astype("string")).astype(
        "category"
    )
    out["RaceDriver"] = (out["RaceYear"].astype("string") + "_" + out["Driver"]).astype(
        "category"
    )
    out["QueryStint"] = (
        out["RaceDriver"].astype("string") + "_S" + out["Stint"].astype("string")
    ).astype("category")

    gcols = ["Race", "Year", "Driver", "Stint"]
    order = out.sort_values(gcols + ["LapNumber"], kind="mergesort").index
    grouped = out.loc[order].groupby(gcols, sort=False, observed=True)

    rank_start = grouped.cumcount().astype("int16") + 1
    stint_len = grouped["LapNumber"].transform("count").astype("int16")
    rank_end = (stint_len - rank_start + 1).astype("int16")
    max_lap = grouped["LapNumber"].transform("max").astype("float32")
    max_tyre = grouped["TyreLife"].transform("max").astype("float32")
    max_progress = grouped["RaceProgress"].transform("max").astype("float32")

    out.loc[order, "StintRankFromStart"] = rank_start.to_numpy()
    out.loc[order, "StintRankFromEnd"] = rank_end.to_numpy()
    out.loc[order, "StintLapCount"] = stint_len.to_numpy()
    out.loc[order, "StintPctComplete"] = (
        (rank_start / stint_len.clip(lower=1)).astype("float32").to_numpy()
    )
    out.loc[order, "LapsToStintEnd"] = (
        max_lap - out.loc[order, "LapNumber"].astype("float32")
    ).to_numpy()
    out.loc[order, "TyreLifeToStintEnd"] = (
        max_tyre - out.loc[order, "TyreLife"].astype("float32")
    ).to_numpy()
    out.loc[order, "ProgressToStintEnd"] = (
        max_progress - out.loc[order, "RaceProgress"].astype("float32")
    ).to_numpy()

    rfe = out["StintRankFromEnd"].astype("float32")
    out["StopImminentRankProxy"] = (rfe == 1).astype("int8")
    out["HardNegativeNearStopProxy"] = ((rfe >= 2) & (rfe <= 4)).astype("int8")
    out["TailFiveStintProxy"] = (rfe <= 5).astype("int8")
    out["RankRelevanceProxy"] = np.select(
        [rfe == 1, rfe <= 4, rfe <= 8], [3, 1, 0.5], default=0
    ).astype("float32")
    out["TailPressureXTyreLife"] = (
        out["TailFiveStintProxy"].astype("float32") * tyre
    ).astype("float32")
    out["HardNegativeXDelta"] = (
        out["HardNegativeNearStopProxy"].astype("float32") * delta
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000355"],
        "research_usage_note": "Verified assigned hypothesis 000355.",
    }, sort_keys=True))
