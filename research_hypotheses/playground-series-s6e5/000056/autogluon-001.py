...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_len_est = (lap / progress.clip(lower=0.01)).clip(1, 120)
    laps_left_est = (race_len_est - lap).clip(lower=0)

    out["finish_tyre_pressure"] = (tyre / laps_left_est.add(1.0)).clip(0, 20)
    out["positive_laptime_loss"] = np.log1p(lap_delta.clip(lower=0))
    out["degradation_per_tyre_life"] = degr / tyre
    out["late_race_pressure"] = progress.pow(2)
    out["stop_debt_urgency"] = (
        (tyre - 18).clip(lower=0) / 35.0 + (stint <= 1).astype(float) * progress
    ).clip(0, 3)
    out["next_stop_feasibility"] = (
        (laps_left_est > 1).astype(float)
        * (laps_left_est < 35).astype(float)
        * (1 - pit_now)
    ).astype(float)

    deg_component = np.log1p(out["degradation_per_tyre_life"].clip(lower=0))
    pos_pressure = (
        (pos - 10).clip(lower=0) / 10.0 + (-pos_chg).clip(lower=0) / 10.0
    ).clip(0, 3)

    out["pit_pressure_score"] = (
        0.28 * np.tanh(out["finish_tyre_pressure"] / 4.0)
        + 0.22 * np.tanh(out["positive_laptime_loss"] / 3.0)
        + 0.18 * np.tanh(deg_component / 2.0)
        + 0.16 * out["late_race_pressure"]
        + 0.16 * np.tanh(out["stop_debt_urgency"])
    ) * out["next_stop_feasibility"]

    out["pit_pressure_interaction"] = (
        (1.0 + out["late_race_pressure"])
        * (1.0 + np.tanh(out["positive_laptime_loss"] / 3.0))
        * (1.0 + np.tanh(deg_component / 2.0))
        * np.tanh((out["finish_tyre_pressure"] + pos_pressure) / 5.0)
        * out["next_stop_feasibility"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000056"],
        "research_usage_note": "Verified assigned hypothesis 000056.",
    }, sort_keys=True))
