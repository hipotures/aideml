...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    seq = work.sort_values(seq_keys + ["LapNumber", "_row_id"], kind="mergesort")
    seq_grp = seq.groupby(seq_keys, sort=False)
    seq_rows = seq["_row_id"].to_numpy(dtype=np.int64)

    prev_pit = np.zeros(n, dtype=np.int8)
    lost_pos = np.zeros(n, dtype=np.int8)
    prev_pit[seq_rows] = (
        seq_grp["PitStop"].shift(1).fillna(0).astype(np.int8).to_numpy()
    )
    lost_pos[seq_rows] = (
        seq_grp["Position"].diff().fillna(0).gt(0).astype(np.int8).to_numpy()
    )
    out["prev_driver_pitstop"] = prev_pit
    out["lost_position_recently"] = lost_pos

    lap_median = work.groupby(keys, sort=False)[
        ["LapTime (s)", "LapTime_Delta"]
    ].transform("median")
    out["rel_laptime_lap_median"] = (
        pd.to_numeric(out["LapTime (s)"], errors="coerce") - lap_median["LapTime (s)"]
    )
    out["rel_delta_lap_median"] = (
        pd.to_numeric(out["LapTime_Delta"], errors="coerce")
        - lap_median["LapTime_Delta"]
    )

    neigh_cols = {
        "PitStop": ("ahead_pit_current", "behind_pit_current"),
        "prev_driver_pitstop": ("ahead_pit_prev", "behind_pit_prev"),
        "LapTime (s)": ("ahead_laptime", "behind_laptime"),
        "LapTime_Delta": ("ahead_laptime_delta", "behind_laptime_delta"),
        "TyreLife": ("ahead_tyrelife", "behind_tyrelife"),
    }
    neighbor = out[
        keys
        + [
    ...
    for src, (ahead_name, behind_name) in neigh_cols.items():
    ...
    out["ahead_car_pitted_recently"] = (
        (out["ahead_pit_current"] + out["ahead_pit_prev"]) > 0
    ).astype("int8")
    out["behind_car_pitted_recently"] = (
        (out["behind_pit_current"] + out["behind_pit_prev"]) > 0
    ).astype("int8")
    out["nearby_pit_pressure"] = (
        out["ahead_car_pitted_recently"] + out["behind_car_pitted_recently"]
    )

    out["gap_laptime_to_ahead"] = out["LapTime (s)"] - out["ahead_laptime"]
    out["gap_laptime_to_behind"] = out["LapTime (s)"] - out["behind_laptime"]
    out["gap_delta_to_ahead"] = out["LapTime_Delta"] - out["ahead_laptime_delta"]
    out["gap_delta_to_behind"] = out["LapTime_Delta"] - out["behind_laptime_delta"]
    out["tyrelife_vs_ahead"] = out["TyreLife"] - out["ahead_tyrelife"]
    out["tyrelife_vs_behind"] = out["TyreLife"] - out["behind_tyrelife"]

    out["behind_pit_x_tyrelife"] = out["behind_car_pitted_recently"] * out["TyreLife"]
    out["ahead_pit_x_tyrelife"] = out["ahead_car_pitted_recently"] * out["TyreLife"]
    out["nearby_pit_x_raceprogress"] = out["nearby_pit_pressure"] * out["RaceProgress"]
    out["nearby_pit_x_stint"] = out["nearby_pit_pressure"] * out["Stint"]

    race_codes = pd.factorize(out["Race"].astype("string"))[0].astype("int16")
    out["race_code_x_nearby_pit"] = race_codes * out["nearby_pit_pressure"]

    created = [
        "rel_laptime_lap_median",
        "rel_delta_lap_median",
        "gap_laptime_to_ahead",
        "gap_laptime_to_behind",
        "gap_delta_to_ahead",
        "gap_delta_to_behind",
        "tyrelife_vs_ahead",
        "tyrelife_vs_behind",
        "behind_pit_x_tyrelife",
        "ahead_pit_x_tyrelife",
        "nearby_pit_x_raceprogress",
        "nearby_pit_x_stint",
        "race_code_x_nearby_pit",
    ]
    for c in created:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000463"],
        "research_usage_note": "Verified assigned hypothesis 000463.",
    }, sort_keys=True))
