...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stint_progress = (tyre / (tyre + laps_left + eps)).clip(0, 1)
    ...
    service_life = out["Compound"].map(compound_service).astype(float).fillna(24.0)

    out["tyre_service_pressure"] = (tyre / service_life).clip(0, 4)
    out["tyre_service_margin"] = ((service_life - tyre) / service_life).clip(-3, 1)
    out["finishability_pressure"] = (laps_left / (service_life - tyre + 1.0)).clip(
        0, 20
    )
    out["finishability_margin"] = (
        (service_life - tyre - laps_left) / service_life
    ).clip(-5, 2)
    out["laps_since_pit_proxy"] = tyre
    out["tyre_age_overage"] = ((tyre - service_life) / service_life).clip(0, 4)
    out["stint_progress_pressure"] = stint_progress
    out["race_phase_late_pressure"] = progress
    out["pit_window_pressure"] = (
        out["tyre_service_pressure"]
        * (0.5 + progress)
        * (1.0 + 0.08 * (stint - 1).clip(0, 7))
    ).clip(0, 10)

    out["degradation_per_tyre_lap"] = (degr / (tyre + eps)).clip(-50, 120)
    out["degradation_pressure"] = np.log1p(np.maximum(degr, 0))
    out["lap_delta_abs"] = lap_delta.abs().clip(0, 300)
    out["slow_lap_pressure"] = np.log1p(np.maximum(lap_delta, 0))
    out["position_loss_pressure"] = np.maximum(
        out["Position_Change"].astype(float), 0
    ).clip(0, 18)
    out["traffic_pressure"] = ((21.0 - pos) / 20.0).clip(0, 1)

    out["service_x_degradation"] = (
        out["tyre_service_pressure"] * out["degradation_pressure"]
    )
    out["finish_x_slowlap"] = out["finishability_pressure"] * out["slow_lap_pressure"]
    out["late_race_service_pressure"] = progress * out["tyre_service_pressure"]

    for col in ["Compound", "Driver", "Race"]:
        vc = out[col].value_counts(dropna=False)
        out[col + "_freq"] = out[col].map(vc).astype(float) / len(out)

    out["compound_service_life"] = service_life
    out["is_wet_compound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype(int)
    out["is_dry_soft"] = (out["Compound"] == "SOFT").astype(int)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000769.",
    }, sort_keys=True))
