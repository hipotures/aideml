...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prog = pd.to_numeric(out["RaceProgress"], errors="coerce").astype("float32")
    stint = pd.to_numeric(out["Stint"], errors="coerce").astype("float32")

    eps = np.float32(1e-3)
    lap_den = lap.clip(lower=1.0)
    total_laps = (lap / prog.clip(lower=eps)).replace([np.inf, -np.inf], np.nan)
    total_laps = total_laps.clip(lower=lap_den)
    remaining = (total_laps - lap).clip(lower=0.0)

    out["tyrelife_to_lap"] = (tyre / lap_den).astype("float32")
    out["tyrelife_to_est_total_laps"] = (tyre / total_laps.clip(lower=1.0)).astype(
        "float32"
    )
    out["tyrelife_to_est_laps_remaining"] = (tyre / remaining.clip(lower=1.0)).astype(
        "float32"
    )
    out["est_laps_remaining"] = remaining.astype("float32")
    out["late_race_small_remaining_flag"] = (remaining < 3).astype("int8")
    out["early_race_small_lap_flag"] = (lap < 3).astype("int8")
    ...
    expected = out["Compound"].map(compound_life).fillna(24.0).astype("float32")
    tyre_vs_expected = tyre / expected.clip(lower=1.0)

    out["tyrelife_vs_compound_expected"] = tyre_vs_expected.astype("float32")
    out["stint_x_tyrelife_to_lap"] = (stint * out["tyrelife_to_lap"]).astype("float32")
    out["stint_x_tyrelife_to_total"] = (
        stint * out["tyrelife_to_est_total_laps"]
    ).astype("float32")
    out["stint_x_tyrelife_to_remaining"] = (
        stint * out["tyrelife_to_est_laps_remaining"]
    ).astype("float32")
    out["stint_x_tyrelife_vs_compound_expected"] = (stint * tyre_vs_expected).astype(
        "float32"
    )
    out["compound_expected_x_race_progress"] = (expected * prog).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000060"],
        "research_usage_note": "Verified assigned hypothesis 000060.",
    }, sort_keys=True))
