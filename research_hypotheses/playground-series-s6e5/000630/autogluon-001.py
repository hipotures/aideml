...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dry = comp.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    wet = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")

    out["is_dry_compound"] = dry
    out["is_wet_or_inter"] = wet
    ...
    for col in numeric_cols:
        if col in out.columns:
            ...
            out[f"dry_{safe}"] = vals * dry
            out[f"reactive_{safe}"] = vals * wet

    keys = ["Year", "Race", "LapNumber"]
    lap_time = pd.to_numeric(out["LapTime (s)"], errors="coerce")
    lap_delta = pd.to_numeric(out["LapTime_Delta"], errors="coerce")
    pit = pd.to_numeric(out["PitStop"], errors="coerce").fillna(0)

    race_mean_lap = lap_time.groupby([out["Year"], out["Race"]]).transform("mean")
    same_lap_mean = lap_time.groupby([out[k] for k in keys]).transform("mean")
    same_lap_delta_mean = lap_delta.groupby([out[k] for k in keys]).transform("mean")
    same_lap_pit_count = pit.groupby([out[k] for k in keys]).transform("sum")

    slowdown = (same_lap_mean - race_mean_lap).replace([np.inf, -np.inf], 0).fillna(0)
    out["reactive_same_lap_slowdown"] = slowdown * wet
    out["reactive_same_lap_delta_mean"] = (
        same_lap_delta_mean.replace([np.inf, -np.inf], 0).fillna(0) * wet
    )
    out["reactive_same_lap_pit_count"] = same_lap_pit_count.fillna(0) * wet
    out["reactive_field_pit_pressure"] = (same_lap_pit_count.fillna(0) - pit).clip(
        lower=0
    ) * wet

    out["dry_tyrelife_x_progress"] = (
        pd.to_numeric(out["TyreLife"], errors="coerce").fillna(0)
        * pd.to_numeric(out["RaceProgress"], errors="coerce").fillna(0)
        * dry
    )
    out["reactive_tyrelife_x_slowdown"] = (
        pd.to_numeric(out["TyreLife"], errors="coerce").fillna(0) * slowdown * wet
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000630"],
        "research_usage_note": "Verified assigned hypothesis 000630.",
    }, sort_keys=True))
