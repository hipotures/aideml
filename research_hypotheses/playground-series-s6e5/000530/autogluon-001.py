...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col, ascending in rank_specs.items():
        r = g[col].rank(method="average", ascending=ascending)
        n = g[col].transform("count").clip(lower=1)
        out[
            col.replace(" ", "_").replace("(", "").replace(")", "") + "_grp_rank_pct"
        ] = ((r - 1) / (n - 1).replace(0, 1)).astype("float32")

    out["lap_group_size"] = g["TyreLife"].transform("count").astype("int16")
    out["tyrelife_vs_lap_mean"] = (
        out["TyreLife"] - g["TyreLife"].transform("mean")
    ).astype("float32")
    out["degradation_vs_lap_mean"] = (
        out["Cumulative_Degradation"] - g["Cumulative_Degradation"].transform("mean")
    ).astype("float32")
    out["laptime_delta_vs_lap_mean"] = (
        out["LapTime_Delta"] - g["LapTime_Delta"].transform("mean")
    ).astype("float32")
    out["position_vs_lap_mean"] = (
        out["Position"] - g["Position"].transform("mean")
    ).astype("float32")

    tyre_rank = out["TyreLife_grp_rank_pct"]
    deg_rank = out["Cumulative_Degradation_grp_rank_pct"]
    stint_rank = out["Stint_grp_rank_pct"]
    pos_rank = out["Position_grp_rank_pct"]
    delta_rank = out["LapTime_Delta_grp_rank_pct"]
    progress_rank = out["RaceProgress_grp_rank_pct"]

    out["rank_sidecar_pit_imminence"] = (
        0.30 * tyre_rank
        + 0.20 * deg_rank
        + 0.15 * stint_rank
        + 0.15 * pos_rank
    ...
    out["rank_sidecar_query_rank_pct"] = (
        g["rank_sidecar_pit_imminence"].rank(method="average", ascending=False) - 1
    ) / (out["lap_group_size"] - 1).replace(0, 1)
    out["rank_sidecar_query_rank_pct"] = out["rank_sidecar_query_rank_pct"].astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000530.",
    }, sort_keys=True))
