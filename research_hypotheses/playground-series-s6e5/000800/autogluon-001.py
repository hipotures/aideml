...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_keys = lap_keys + ["Position", "_orig_idx"]

    work = df.sort_values(sort_keys).copy()
    g = work.groupby(lap_keys, sort=False)
    lap_size = g["Position"].transform("size").astype("float32")
    ...
    for col in [
        ...
        work[f"{col}_Ahead_1"] = g[col].shift(1)
        work[f"{col}_Behind_1"] = g[col].shift(-1)
        work[f"{col}_Ahead_2"] = g[col].shift(2)
        work[f"{col}_Behind_2"] = g[col].shift(-2)

    work["Gap_Ahead_LapTime"] = work["LapTime (s)"] - work["LapTime (s)_Ahead_1"]
    work["Gap_Behind_LapTime"] = work["LapTime (s)_Behind_1"] - work["LapTime (s)"]
    work["Gap_Ahead_Delta"] = work["LapTime_Delta"] - work["LapTime_Delta_Ahead_1"]
    work["Gap_Behind_Delta"] = work["LapTime_Delta_Behind_1"] - work["LapTime_Delta"]

    work["Gap_Ahead2_LapTime"] = work["LapTime (s)"] - work["LapTime (s)_Ahead_2"]
    work["Gap_Behind2_LapTime"] = work["LapTime (s)_Behind_2"] - work["LapTime (s)"]
    work["Gap_Ahead2_Delta"] = work["LapTime_Delta"] - work["LapTime_Delta_Ahead_2"]
    work["Gap_Behind2_Delta"] = work["LapTime_Delta_Behind_2"] - work["LapTime_Delta"]

    work["Deg_Rank_Pct_InLap"] = (
        g["Cumulative_Degradation"].rank(pct=True).astype("float32")
    )
    work["Delta_Rank_Pct_InLap"] = g["LapTime_Delta"].rank(pct=True).astype("float32")
    work["TyreLife_Rank_Pct_InLap"] = g["TyreLife"].rank(pct=True).astype("float32")
    work["LapTime_Rank_Pct_InLap"] = g["LapTime (s)"].rank(pct=True).astype("float32")

    work["Midfield_Proximity"] = 1.0 - (work["Position"] - 1) / (lap_size - 1).replace(
        0, 1
    )
    work["Local_Position_Centering"] = (
        work["Position"] - (lap_size + 1.0) / 2.0
    ) / lap_size

    work["Abs_Gap_Ahead_Delta"] = work["Gap_Ahead_Delta"].abs()
    work["Abs_Gap_Behind_Delta"] = work["Gap_Behind_Delta"].abs()
    work["Abs_Gap_Ahead_LapTime"] = work["Gap_Ahead_LapTime"].abs()
    work["Abs_Gap_Behind_LapTime"] = work["Gap_Behind_LapTime"].abs()

    work["Tight_Ahead_Delta"] = (work["Abs_Gap_Ahead_Delta"] <= 1.0).astype("int8")
    work["Tight_Behind_Delta"] = (work["Abs_Gap_Behind_Delta"] <= 1.0).astype("int8")
    work["Tight_Ahead_LapTime"] = (work["Abs_Gap_Ahead_LapTime"] <= 1.5).astype("int8")
    work["Tight_Behind_LapTime"] = (work["Abs_Gap_Behind_LapTime"] <= 1.5).astype(
        "int8"
    )
    work["Local_Traffic_Density"] = (
        work["Tight_Ahead_Delta"]
        + work["Tight_Behind_Delta"]
        + work["Tight_Ahead_LapTime"]
        + work["Tight_Behind_LapTime"]
    ).astype("int8")

    work["Clean_Air_Behind"] = (
        (work["Abs_Gap_Behind_Delta"] > 1.5) & (work["Abs_Gap_Behind_LapTime"] > 2.0)
    ).astype("int8")
    work["Traffic_Ahead"] = (
        (work["Abs_Gap_Ahead_Delta"] <= 1.0) | (work["Abs_Gap_Ahead_LapTime"] <= 1.5)
    ).astype("int8")

    work["Within_Undercut_Window"] = (
        work["Traffic_Ahead"].eq(1)
        & work["Clean_Air_Behind"].eq(1)
        & (work["Deg_Rank_Pct_InLap"] >= 0.6)
        & (work["TyreLife_Rank_Pct_InLap"] >= 0.5)
        & (work["RaceProgress"] >= 0.15)
        & (work["RaceProgress"] <= 0.9)
        & (work["PitStop"] == 0)
    ).astype("int8")

    work["Within_Overcut_Window"] = (
        (work["Clean_Air_Behind"] == 0)
        & (work["Traffic_Ahead"] == 0)
        & (work["Abs_Gap_Ahead_Delta"] > 0.5)
        & (work["Deg_Rank_Pct_InLap"] <= 0.5)
        & (work["RaceProgress"] >= 0.15)
        & (work["RaceProgress"] <= 0.9)
        & (work["PitStop"] == 0)
    ).astype("int8")

    work["Undercut_Pressure"] = (
        work["Deg_Rank_Pct_InLap"].fillna(0.5)
        * (1.0 - work["Abs_Gap_Ahead_Delta"].clip(0, 3) / 3.0)
        * (0.5 + 0.5 * work["Clean_Air_Behind"])
    ).astype("float32")

    work["Rivalry_Intensity"] = (
        (1.0 - work["Abs_Gap_Ahead_Delta"].clip(0, 3) / 3.0).fillna(0)
        + (1.0 - work["Abs_Gap_Behind_Delta"].clip(0, 3) / 3.0).fillna(0)
        + (1.0 - work["Abs_Gap_Ahead_LapTime"].clip(0, 4) / 4.0).fillna(0)
        + (1.0 - work["Abs_Gap_Behind_LapTime"].clip(0, 4) / 4.0).fillna(0)
    ).astype("float32")

    work["Stint_X_TyreLife_Rank"] = (
        work["Stint"] * work["TyreLife_Rank_Pct_InLap"]
    ).astype("float32")
    work["Deg_X_TrafficAhead"] = (
        work["Cumulative_Degradation"] * work["Traffic_Ahead"]
    ).astype("float32")
    work["Delta_X_CleanAir"] = (
        work["LapTime_Delta"] * work["Clean_Air_Behind"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000800"],
        "research_usage_note": "Verified assigned hypothesis 000800.",
    }, sort_keys=True))
