...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(["Year", "Race", "Driver", "LapNumber"], kind="mergesort")

    grp = out.groupby(["Year", "Race", "Driver"], sort=False)
    prev_stint = grp["Stint"].shift(1)
    prev_tyre_life = grp["TyreLife"].shift(1)
    prev_compound = grp["Compound"].shift(1)

    recent_delta = grp["LapTime_Delta"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).median()
    )
    recent_laptime = grp["LapTime (s)"].transform(
        lambda s: s.shift(1).rolling(3, min_periods=1).median()
    )

    new_stint_lap = (out["Stint"] > prev_stint.fillna(out["Stint"])) | (
        out["TyreLife"] < prev_tyre_life.fillna(out["TyreLife"])
    )
    pit_in_lap = out["PitStop"].eq(1)

    outlap_penalty_obs = (out["LapTime_Delta"] - recent_delta).where(new_stint_lap)
    inlap_decay_obs = (out["LapTime_Delta"] - recent_delta).where(pit_in_lap)

    global_out = outlap_penalty_obs.groupby(out["Compound"]).mean()
    global_in = inlap_decay_obs.groupby(prev_compound.fillna(out["Compound"])).mean()
    ...
    lap_agg = pd.DataFrame(
        {
            ...
            "out_sum": outlap_penalty_obs.fillna(0.0).to_numpy(),
            "out_cnt": outlap_penalty_obs.notna().astype(np.float32).to_numpy(),
            "in_sum": inlap_decay_obs.fillna(0.0).to_numpy(),
            "in_cnt": inlap_decay_obs.notna().astype(np.float32).to_numpy(),
        }
    )
    lap_agg = lap_agg.groupby(["Year", "Race", "LapNumber"], as_index=False).sum()
    lap_agg = lap_agg.sort_values(["Year", "Race", "LapNumber"], kind="mergesort")

    for c in ["out_sum", "out_cnt", "in_sum", "in_cnt"]:
        lap_agg[c] = lap_agg.groupby(["Year", "Race"], sort=False)[c].cumsum()
        lap_agg[c] = (
            lap_agg.groupby(["Year", "Race"], sort=False)[c].shift(1).fillna(0.0)
        )

    lap_agg = lap_agg.rename(
        columns={
            "out_sum": "race_prev_out_sum",
            "out_cnt": "race_prev_out_cnt",
            "in_sum": "race_prev_in_sum",
            "in_cnt": "race_prev_in_cnt",
        }
    )
    out = out.merge(lap_agg, on=["Year", "Race", "LapNumber"], how="left", sort=False)

    out["race_prev_out_sum"] = out["race_prev_out_sum"].fillna(0.0)
    out["race_prev_out_cnt"] = out["race_prev_out_cnt"].fillna(0.0)
    out["race_prev_in_sum"] = out["race_prev_in_sum"].fillna(0.0)
    out["race_prev_in_cnt"] = out["race_prev_in_cnt"].fillna(0.0)

    out_backoff = out["Compound"].map(global_out).fillna(outlap_penalty_obs.mean())
    in_backoff_key = prev_compound.fillna(out["Compound"])
    in_backoff = in_backoff_key.map(global_in).fillna(inlap_decay_obs.mean())

    shrink = 5.0
    fresh_tyre_warmup_penalty = (out["race_prev_out_sum"] + shrink * out_backoff) / (
        out["race_prev_out_cnt"] + shrink
    )
    old_tyre_inlap_decay = (out["race_prev_in_sum"] + shrink * in_backoff) / (
        out["race_prev_in_cnt"] + shrink
    )

    out["fresh_tyre_warmup_penalty"] = fresh_tyre_warmup_penalty.astype(np.float32)
    out["old_tyre_inlap_decay"] = old_tyre_inlap_decay.astype(np.float32)
    out["expected_undercut_gain"] = (
        old_tyre_inlap_decay - fresh_tyre_warmup_penalty
    ).astype(np.float32)
    out["expected_overcut_gain"] = (-out["expected_undercut_gain"]).astype(np.float32)

    pace_scale = recent_laptime.abs().fillna(out["LapTime (s)"].median()) + 1.0
    track_pos = (21.0 - out["Position"].clip(1, 20)) / 20.0
    out["clean_air_rejoin_proxy"] = (
        track_pos - fresh_tyre_warmup_penalty / pace_scale
    ).astype(np.float32)

    out["same_race_pit_evidence"] = (
        out["race_prev_out_cnt"] + out["race_prev_in_cnt"]
    ).astype(np.float32)
    out["undercut_gain_x_progress"] = (
        out["expected_undercut_gain"] * out["RaceProgress"]
    ).astype(np.float32)
    out["warmup_penalty_x_tyrelife"] = (
        out["fresh_tyre_warmup_penalty"] * out["TyreLife"]
    ).astype(np.float32)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000562"],
        "research_usage_note": "Verified assigned hypothesis 000562.",
    }, sort_keys=True))
