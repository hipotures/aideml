...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out.attrs["research_hypotheses_llm_claimed_used"] = ["000567"]

    group_cols = ["Year", "Race", "LapNumber"]
    sort_cols = group_cols + ["Position"]
    s = out.sort_values(sort_cols, kind="mergesort")
    g = s.groupby(group_cols, sort=False)

    tyre = pd.to_numeric(s["TyreLife"], errors="coerce").astype("float32")
    lap_delta = pd.to_numeric(s["LapTime_Delta"], errors="coerce").astype("float32")
    degr = pd.to_numeric(s["Cumulative_Degradation"], errors="coerce").astype("float32")
    progress = pd.to_numeric(s["RaceProgress"], errors="coerce").astype("float32")
    pos_change = pd.to_numeric(s["Position_Change"], errors="coerce").astype("float32")
    pit = pd.to_numeric(s["PitStop"], errors="coerce").fillna(0).astype("int8")
    comp = s["Compound"].astype("category")

    degr_slope = degr / tyre.clip(lower=1)
    pit_loss = (
        19.0
        + 3.0 * progress.clip(0, 1)
        + 0.10 * pd.to_numeric(s["Position"], errors="coerce").astype("float32")
    ).astype("float32")
    fresh_swing_self = (0.055 * tyre + 0.018 * degr_slope.clip(-50, 80)).astype(
        "float32"
    )
    cooldown = (pit == 1) | (g["PitStop"].shift(1).fillna(0).astype("int8") == 1)

    for k in (1, 2, 3):
        ahead_tyre = g["TyreLife"].shift(k).astype("float32")
        behind_tyre = g["TyreLife"].shift(-k).astype("float32")
        ahead_delta = g["LapTime_Delta"].shift(k).astype("float32")
        behind_delta = g["LapTime_Delta"].shift(-k).astype("float32")
        ahead_degr = g["Cumulative_Degradation"].shift(k).astype("float32")
        behind_degr = g["Cumulative_Degradation"].shift(-k).astype("float32")
        ahead_comp = g["Compound"].shift(k).astype("category")
        behind_comp = g["Compound"].shift(-k).astype("category")

        ahead_slope = ahead_degr / ahead_tyre.clip(lower=1)
        behind_slope = behind_degr / behind_tyre.clip(lower=1)

        s[f"ahead{k}_tyrelife_gap"] = (tyre - ahead_tyre).fillna(0).astype("float32")
        s[f"behind{k}_tyrelife_gap"] = (tyre - behind_tyre).fillna(0).astype("float32")
        s[f"ahead{k}_lapdelta_gap"] = (
            (lap_delta - ahead_delta).fillna(0).astype("float32")
        )
        s[f"behind{k}_lapdelta_gap"] = (
            (lap_delta - behind_delta).fillna(0).astype("float32")
        )
        s[f"ahead{k}_compound_mismatch"] = (
            (comp != ahead_comp).fillna(False).astype("int8")
        )
        s[f"behind{k}_compound_mismatch"] = (
            (comp != behind_comp).fillna(False).astype("int8")
        )
        s[f"ahead{k}_degr_slope_gap"] = (
            (degr_slope - ahead_slope).fillna(0).astype("float32")
        )
        s[f"behind{k}_degr_slope_gap"] = (
            (degr_slope - behind_slope).fillna(0).astype("float32")
        )

        ahead_pressure = (
            s[f"ahead{k}_lapdelta_gap"]
            + 0.045 * s[f"ahead{k}_tyrelife_gap"]
            + 0.015 * s[f"ahead{k}_degr_slope_gap"]
            - pit_loss
            + fresh_swing_self
        )
        behind_pressure = (
            -s[f"behind{k}_lapdelta_gap"]
            - 0.045 * s[f"behind{k}_tyrelife_gap"]
            - 0.015 * s[f"behind{k}_degr_slope_gap"]
            - 0.5 * pit_loss
        )

        s[f"ahead{k}_fresh_tyre_swing"] = (
            ahead_pressure.mask(cooldown, 0).fillna(0).astype("float32")
        )
        s[f"behind{k}_undercut_threat"] = (
            behind_pressure.mask(cooldown, 0).fillna(0).astype("float32")
        )
        s[f"ahead{k}_pit_now_jump_rival"] = (
            s[f"ahead{k}_fresh_tyre_swing"] > (-2.0 * k)
        ).astype("int8")
        s[f"behind{k}_rival_jump_threat"] = (
            s[f"behind{k}_undercut_threat"] > (-1.0 * k)
        ).astype("int8")

    s["nearest_ahead_pressure_max"] = (
        s[[f"ahead{k}_fresh_tyre_swing" for k in (1, 2, 3)]]
        .max(axis=1)
        .astype("float32")
    )
    s["nearest_behind_threat_max"] = (
        s[[f"behind{k}_undercut_threat" for k in (1, 2, 3)]]
        .max(axis=1)
        .astype("float32")
    )
    s["local_undercut_pressure"] = (
        s["nearest_ahead_pressure_max"]
        + 0.5 * s["nearest_behind_threat_max"]
        - 0.2 * pos_change
    ).astype("float32")
    s["local_pit_tactical_flag"] = (
        (s["nearest_ahead_pressure_max"] > -2.5)
        | (s["nearest_behind_threat_max"] > -1.5)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000567"],
        "research_usage_note": "Verified assigned hypothesis 000567.",
    }, sort_keys=True))
