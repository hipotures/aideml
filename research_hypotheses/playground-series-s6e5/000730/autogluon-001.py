...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    order_cols = ["Year", "Race", "Driver", "LapNumber", "Stint"]
    work_cols = order_cols + ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]

    work = out[work_cols].copy()
    work["_orig_row"] = np.arange(len(work), dtype=np.int64)
    work = work.sort_values(order_cols + ["_orig_row"], kind="mergesort")

    grp = work.groupby(group_cols, sort=False, observed=True).ngroup()
    pos = work.groupby(grp, sort=False).cumcount().astype("float64") + 1.0
    work["cp_stint_lap_index"] = pos.astype("float32")

    def _prior_mean(x):
        x = pd.Series(x, index=work.index, dtype="float64")
        prev_n = pos - 1.0
        prev_sum = x.groupby(grp, sort=False).cumsum() - x
        return (
            (prev_sum / prev_n.where(prev_n > 0.0))
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
        )

    def _online_features(name, x):
        x = pd.Series(x, index=work.index, dtype="float64")
        xx = x * x
        prev_n = pos - 1.0
        prev_sum = x.groupby(grp, sort=False).cumsum() - x
        prev_sum2 = xx.groupby(grp, sort=False).cumsum() - xx

        den_mean = prev_n.where(prev_n > 0.0)
        prev_mean = prev_sum / den_mean
        prev_var = (prev_sum2 - (prev_sum * prev_sum / den_mean)) / (
            prev_n - 1.0
        ).where(prev_n > 1.0)
        prev_std = np.sqrt(prev_var.clip(lower=1e-6))
        z = (
            ((x - prev_mean) / prev_std)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .clip(-20.0, 20.0)
        )
        ...
        hazard = 0.035
        normal_like = np.exp(-0.5 * z * z)
        broad_like = np.exp(-0.5 * (z / 4.0) * (z / 4.0)) / 4.0
        p_change = (hazard * broad_like) / (
            hazard * broad_like + (1.0 - hazard) * normal_like + 1e-12
        )
        p_change = p_change.clip(0.0, 1.0)

        def _cusum(shock):
            path = shock.astype("float64").groupby(grp, sort=False).cumsum()
            floor = path.groupby(grp, sort=False).cummin().clip(upper=0.0)
            return (path - floor).clip(0.0, 50.0)

        cusum_pos = _cusum((z - 0.25).clip(-20.0, 20.0))
        cusum_neg = _cusum((-z - 0.25).clip(-20.0, 20.0))

        flag = (p_change > 0.5).astype("int16")
        segment = flag.groupby(grp, sort=False).cumsum()
        run_length = flag.groupby([grp, segment], sort=False).cumcount().clip(upper=100)

        prefix = f"cp_{name}"
        work[f"{prefix}_z"] = z.astype("float32")
        work[f"{prefix}_bocpd_prob"] = p_change.astype("float32")
        work[f"{prefix}_cusum_pos"] = cusum_pos.astype("float32")
        work[f"{prefix}_cusum_neg"] = cusum_neg.astype("float32")
        work[f"{prefix}_run_length"] = run_length.astype("float32")
        return (
            f"{prefix}_bocpd_prob",
            f"{prefix}_cusum_pos",
            f"{prefix}_cusum_neg",
            f"{prefix}_run_length",
        )

    lap_time = work["LapTime (s)"].astype("float64")
    lap_resid = (
        (lap_time - _prior_mean(lap_time)).where(pos > 1.0, 0.0).clip(-500.0, 500.0)
    )
    work["cp_laptime_prior_resid"] = lap_resid.astype("float32")

    prob_cols, pos_cols, neg_cols, run_cols = [], [], [], []
    for name, values in [
        ("lap_delta", work["LapTime_Delta"]),
        ("cumdeg", work["Cumulative_Degradation"]),
        ("lap_resid", lap_resid),
    ]:
        p_col, cpos_col, cneg_col, r_col = _online_features(name, values)
        prob_cols.append(p_col)
        pos_cols.append(cpos_col)
        neg_cols.append(cneg_col)
        run_cols.append(r_col)

    work["cp_bocpd_prob_max"] = work[prob_cols].max(axis=1).astype("float32")
    work["cp_bocpd_prob_mean"] = work[prob_cols].mean(axis=1).astype("float32")
    work["cp_cusum_pressure_sum"] = (
        work[pos_cols].sum(axis=1).clip(0.0, 150.0).astype("float32")
    )
    work["cp_cusum_relief_sum"] = (
        work[neg_cols].sum(axis=1).clip(0.0, 150.0).astype("float32")
    )
    work["cp_min_change_run_length"] = work[run_cols].min(axis=1).astype("float32")

    feature_cols = [c for c in work.columns if c.startswith("cp_")]
    work = work.sort_values("_orig_row", kind="mergesort")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000730"],
        "research_usage_note": "Verified assigned hypothesis 000730.",
    }, sort_keys=True))
