...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Race", "Year", "Driver", "Stint", "LapNumber"]
    out["_orig_order_000644"] = np.arange(len(out))
    out = out.sort_values(sort_cols + ["_orig_order_000644"], kind="mergesort")

    group_cols = ["Race", "Year", "Driver", "Stint"]
    g = out.groupby(group_cols, sort=False)

    lap_med = out.groupby(["Race", "Year", "LapNumber"], sort=False)[
        "LapTime (s)"
    ].transform("median")
    out["Pace_Deviation_From_Median"] = out["LapTime (s)"] - lap_med

    out["Stint_Lap_Index"] = g.cumcount() + 1
    out["TyreLife_Sq"] = out["TyreLife"] * out["TyreLife"]
    out["Deg_Per_TyreLife"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1
    )
    out["RaceProgress_TyreLife"] = out["RaceProgress"] * out["TyreLife"]

    for col in [
        "LapTime_Delta",
        "Pace_Deviation_From_Median",
        "Cumulative_Degradation",
    ]:
        lag1 = g[col].shift(1)
        out[f"{col}_lag1"] = lag1
        out[f"{col}_lag2"] = g[col].shift(2)
        out[f"{col}_since_stint_start"] = out[col] - g[col].transform("first")

        if col == "Cumulative_Degradation":
            inc = g[col].diff()
            out["Cumulative_Degradation_inc_lag1"] = inc.groupby(
                [out[c] for c in group_cols], sort=False
            ).shift(1)
            base = out["Cumulative_Degradation_inc_lag1"]
        else:
            base = lag1

        for w in (3, 5, 8):
            roll = base.groupby([out[c] for c in group_cols], sort=False).rolling(
                w, min_periods=1
            )
            out[f"{col}_roll{w}_mean"] = roll.mean().reset_index(
                level=list(range(len(group_cols))), drop=True
            )
            out[f"{col}_roll{w}_std"] = (
                roll.std()
                .reset_index(level=list(range(len(group_cols))), drop=True)
                .fillna(0)
            )

        out[f"{col}_trend_3v8"] = out[f"{col}_roll3_mean"] - out[f"{col}_roll8_mean"]

    for col in [
        "LapTime_Delta",
        "Pace_Deviation_From_Median",
        "Cumulative_Degradation_inc_lag1",
    ]:
        s = out[col] if col == "Cumulative_Degradation_inc_lag1" else out[f"{col}_lag1"]
        s1 = s
        s2 = s.groupby([out[c] for c in group_cols], sort=False).shift(1)
        s3 = s.groupby([out[c] for c in group_cols], sort=False).shift(2)
        out[f"{col}_slope3"] = ((s1 - s3) / 2.0).fillna(s1 - s2).fillna(0)

    out["Recent_Pace_Cliff"] = (
        out["Pace_Deviation_From_Median_roll3_mean"]
        - out["Pace_Deviation_From_Median_lag1"]
    )
    out["Recent_Deg_Accel"] = out["Cumulative_Degradation_inc_lag1_slope3"]
    out["Old_Tyre_Bad_Pace"] = (
        out["TyreLife"] * out["Pace_Deviation_From_Median_roll5_mean"]
    )

    out = out.sort_values("_orig_order_000644", kind="mergesort").drop(
        columns=["_orig_order_000644"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000644"],
        "research_usage_note": "Verified assigned hypothesis 000644.",
    }, sort_keys=True))
