...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key_cols = ["Year", "Race", "LapNumber"]
    sort_cols = key_cols + ["Position"]
    s = out.reset_index(drop=False).rename(columns={"index": "_orig_index"})
    s["_row"] = np.arange(n)
    s = s.sort_values(sort_cols, kind="mergesort")

    group = s.groupby(key_cols, sort=False)
    best_dist = pd.Series(np.inf, index=s.index)
    best_side = pd.Series(0, index=s.index, dtype="int8")
    rival_cols = [
        "Position",
        "Stint",
        "TyreLife",
    ...
    chosen = {
        c: pd.Series(
            np.nan, index=s.index, dtype="object" if c == "Compound" else "float64"
        )
        for c in rival_cols
    }

    for off in (-4, -3, -2, -1, 1, 2, 3, 4):
        cand = group[rival_cols].shift(-off)
        pos_dist = (cand["Position"] - s["Position"]).abs()
        stint_ok = (cand["Stint"] - s["Stint"]).abs() <= 1
        valid = cand["Position"].notna() & stint_ok
        better = valid & (pos_dist < best_dist)
        best_dist.loc[better] = pos_dist.loc[better]
        best_side.loc[better] = np.where(
            cand.loc[better, "Position"] < s.loc[better, "Position"], 1, -1
        )
        for c in rival_cols:
            chosen[c].loc[better] = cand.loc[better, c]

    def put(name, values):
        arr = pd.Series(values, index=s.index).sort_index()
        out[name] = arr.reindex(pd.RangeIndex(n)).to_numpy()

    s_back_order = s.sort_values("_row")
    for c in list(chosen):
        chosen[c] = chosen[c].loc[s_back_order.index].reset_index(drop=True)
    sb = s_back_order.reset_index(drop=True)
    best_dist = best_dist.loc[s_back_order.index].reset_index(drop=True)
    best_side = best_side.loc[s_back_order.index].reset_index(drop=True)

    has_rival = np.isfinite(best_dist).astype("int8")
    out["shadow_has_rival"] = has_rival
    out["shadow_rival_side"] = best_side.astype("int8")
    out["shadow_position_gap"] = best_dist.replace(np.inf, 99).astype("float32")
    out["shadow_stint_gap"] = (
        (sb["Stint"].astype(float) - chosen["Stint"].astype(float))
        .fillna(0)
        .astype("float32")
    )
    out["shadow_tyre_life_gap"] = (
        (sb["TyreLife"].astype(float) - chosen["TyreLife"].astype(float))
        .fillna(0)
        .astype("float32")
    )
    out["shadow_degradation_gap"] = (
        (
            sb["Cumulative_Degradation"].astype(float)
            - chosen["Cumulative_Degradation"].astype(float)
        )
        .fillna(0)
        .astype("float32")
    )
    out["shadow_lap_delta_gap"] = (
        (sb["LapTime_Delta"].astype(float) - chosen["LapTime_Delta"].astype(float))
        .fillna(0)
        .astype("float32")
    )
    out["shadow_laptime_gap"] = (
        (sb["LapTime (s)"].astype(float) - chosen["LapTime (s)"].astype(float))
        .fillna(0)
        .astype("float32")
    )
    out["shadow_compound_mismatch"] = (
        sb["Compound"].astype(str).to_numpy()
        != chosen["Compound"].astype(str).to_numpy()
    ).astype("int8")
    out["shadow_rival_pitted"] = chosen["PitStop"].fillna(0).astype("int8")

    race_lower = out["Race"].astype("string").str.lower().fillna("")
    position = pd.to_numeric(out["Position"], errors="coerce").fillna(10.0)
    progress = pd.to_numeric(out["RaceProgress"], errors="coerce").fillna(0.5)
    pit_loss = pd.Series(22.0, index=out.index, dtype="float64")
    pit_loss += np.where(race_lower.str.contains("monaco|singapore|hungar", regex=True), 4.0, 0.0)
    pit_loss += np.where(race_lower.str.contains("italian|monza|las vegas", regex=True), -3.0, 0.0)
    pit_loss += np.where(race_lower.str.contains("saudi|jeddah|austria", regex=True), -1.5, 0.0)
    pit_loss += (position.clip(1, 20) - 10.5) * 0.10
    pit_loss += (progress.clip(0, 1) - 0.5) * 1.5
    pit_loss = pit_loss.clip(8, 45)

    compound_rank = (
        out["Compound"]
        .map({"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4})
        .fillna(2)
    )
    rival_compound_rank = (
        chosen["Compound"]
    ...
    out["shadow_compound_rank_gap"] = (
        compound_rank.to_numpy() - rival_compound_rank.to_numpy()
    ).astype("float32")

    pressure_score = (
        0.35 * out["shadow_tyre_life_gap"].astype(float)
        + 0.02 * out["shadow_degradation_gap"].astype(float)
        + 0.25 * out["shadow_lap_delta_gap"].astype(float)
        - 0.15 * out["shadow_position_gap"].astype(float)
        + 0.50 * out["shadow_compound_rank_gap"].astype(float)
    )
    out["shadow_pressure_score"] = pressure_score.fillna(0).astype("float32")
    out["shadow_near_rival"] = (
        (out["shadow_has_rival"] == 1) & (out["shadow_position_gap"] <= 2)
    ).astype("int8")
    out["shadow_ahead_pressure"] = (
        (out["shadow_rival_side"] == 1) & (out["shadow_near_rival"] == 1)
    ).astype("int8")
    out["shadow_behind_pressure"] = (
        (out["shadow_rival_side"] == -1) & (out["shadow_near_rival"] == 1)
    ).astype("int8")

    estimated_net_after_stop = (
        out["shadow_laptime_gap"].astype(float)
        + pit_loss
        - 0.7 * out["shadow_pressure_score"].astype(float)
    )
    out["shadow_jump_rival_after_stop"] = (
        (out["shadow_ahead_pressure"] == 1) & (estimated_net_after_stop < 3.0)
    ).astype("int8")
    out["shadow_cover_rival_risk"] = (
        (out["shadow_behind_pressure"] == 1)
        & (estimated_net_after_stop > pit_loss - 3.0)
    ).astype("int8")
    out["shadow_pit_loss_prior"] = pit_loss.astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000763"],
        "research_usage_note": "Verified assigned hypothesis 000763.",
    }, sort_keys=True))
