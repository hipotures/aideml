...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit_mask = out["PitStop"].astype(float) > 0
    global_pit_loss = (
        float(out.loc[pit_mask, "LapTime_Delta"].median()) if pit_mask.any() else 22.0
    )
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 22.0
    race_loss = out.loc[pit_mask].groupby(["Year", "Race"])["LapTime_Delta"].median()
    loss_key = pd.MultiIndex.from_frame(out[["Year", "Race"]])
    pit_loss = (
        pd.Series(loss_key.map(race_loss), index=out.index)
