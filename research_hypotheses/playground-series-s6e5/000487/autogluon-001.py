...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_year = df["Year"].astype(str) + "_" + df["Race"].astype(str)
    race_year_driver = race_year + "_" + df["Driver"].astype(str)
    race_year_driver_stint = race_year_driver + "_S" + df["Stint"].astype(str)

    df["Race_Year"] = race_year
    df["Race_Year_Driver"] = race_year_driver
    df["Race_Year_Driver_Stint"] = race_year_driver_stint
    ...
    df["Compound_Ordinal"] = df["Compound"].map(compound_map).astype("float32")
    df["Is_Dry"] = df["Compound"].isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")
    df["Is_WetLike"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    eps = 1e-6
    df["TyreLife_per_Lap"] = (df["TyreLife"] / (df["LapNumber"] + eps)).astype(
        "float32"
    )
    df["Deg_per_TyreLife"] = (
        df["Cumulative_Degradation"] / (df["TyreLife"] + eps)
    ).astype("float32")
    df["Deg_per_Lap"] = (df["Cumulative_Degradation"] / (df["LapNumber"] + eps)).astype(
        "float32"
    )
    df["TyreLife_x_Stint"] = (df["TyreLife"] * df["Stint"]).astype("float32")
    df["TyreLife_x_RaceProgress"] = (df["TyreLife"] * df["RaceProgress"]).astype(
        "float32"
    )
    df["Endgame_TyrePressure"] = (df["TyreLife"] * (1.0 - df["RaceProgress"])).astype(
        "float32"
    )
    df["Deg_x_RaceProgress"] = (
        df["Cumulative_Degradation"] * df["RaceProgress"]
    ).astype("float32")
    df["LapDelta_x_TyreLife"] = (df["LapTime_Delta"] * df["TyreLife"]).astype("float32")
    df["LapDelta_abs"] = df["LapTime_Delta"].abs().astype("float32")
    df["PositionPressure"] = (
        (21 - df["Position"]) * (1.0 - df["RaceProgress"])
    ).astype("float32")
    df["TrackPosLossRisk"] = (df["Position"] * df["TyreLife"]).astype("float32")
    df["PositionChange_abs"] = df["Position_Change"].abs().astype("float32")
    ...
    race_group = df.groupby("Race_Year", sort=False)
    for col in [
        "TyreLife",
        ...
        mean = race_group[col].transform("mean")
        std = race_group[col].transform("std").fillna(0.0)
        df[f"{col}_RaceZ"] = ((df[col] - mean) / (std + eps)).astype("float32")

    driver_race_group = df.groupby("Race_Year_Driver", sort=False)
    for col in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta"]:
        mean = driver_race_group[col].transform("mean")
        df[f"{col}_DriverRaceDiff"] = (df[col] - mean).astype("float32")

    df["LapNumber_pct"] = (
        df["LapNumber"]
        / (df.groupby("Race_Year", sort=False)["LapNumber"].transform("max") + eps)
    ).astype("float32")
    df["TyreLife_pct_in_driver_race"] = (
        df["TyreLife"] / (driver_race_group["TyreLife"].transform("max") + eps)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000487.",
    }, sort_keys=True))
