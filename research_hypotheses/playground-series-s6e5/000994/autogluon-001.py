...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["CompoundOrdinal"] = X["Compound"].map(compound_map).astype("float32")
    X["IsWetTyre"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    X["IsDryTyre"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    ...
    degradation = X["Cumulative_Degradation"].astype("float32")
    position = X["Position"].astype("float32")
    pos_change = X["Position_Change"].astype("float32")
    stint = X["Stint"].astype("float32")
    lap_number = X["LapNumber"].astype("float32")
    pitstop_flag = X["PitStop"].astype("float32")

    X["TyreLifeSq"] = (tyre_life**2).astype("float32")
    X["RaceProgressSq"] = (race_progress**2).astype("float32")
    X["DegradationPerLap"] = (degradation / tyre_life.clip(lower=1)).astype("float32")
    X["DeltaPerLap"] = (lap_delta / tyre_life.clip(lower=1)).astype("float32")
    X["TyreLifeOverRaceProgress"] = (tyre_life / race_progress.clip(lower=0.01)).astype(
        "float32"
    )
    X["StintPressure"] = (tyre_life * (1 - race_progress)).astype("float32")
    X["LateRaceTyreAge"] = (tyre_life * race_progress).astype("float32")
    X["DegTyreInteraction"] = (degradation * tyre_life).astype("float32")
    X["DegProgressInteraction"] = (degradation * race_progress).astype("float32")
    X["LapDeltaAbs"] = lap_delta.abs().astype("float32")
    X["PositionLossFlag"] = (pos_change < 0).astype("int8")
    X["PositionGainFlag"] = (pos_change > 0).astype("int8")
    X["TrafficPressure"] = ((21 - position) * (1 - race_progress)).astype("float32")
    X["FreshStopFlag"] = ((pitstop_flag > 0) | (tyre_life <= 2)).astype("int8")
    X["NearWindowFlag"] = ((tyre_life >= 12) & (race_progress >= 0.2)).astype("int8")

    drv_grp = X.groupby(["Year", "Race", "Driver"], sort=False)
    stint_grp = X.groupby(["Year", "Race", "Driver", "Stint"], sort=False)

    drv_lap_mean = drv_grp["LapTime (s)"].transform("mean").astype("float32")
    drv_lap_std = drv_grp["LapTime (s)"].transform("std").fillna(0).astype("float32")
    drv_delta_mean = drv_grp["LapTime_Delta"].transform("mean").astype("float32")
    drv_tyre_max = drv_grp["TyreLife"].transform("max").clip(lower=1).astype("float32")
    stint_tyre_max = (
        stint_grp["TyreLife"].transform("max").clip(lower=1).astype("float32")
    )
    stint_deg_min = (
        stint_grp["Cumulative_Degradation"].transform("min").astype("float32")
    )
    stint_deg_max = (
        stint_grp["Cumulative_Degradation"].transform("max").astype("float32")
    )

    X["LapTimeVsDriverMean"] = (lap_time - drv_lap_mean).astype("float32")
    X["LapTimeDriverZ"] = (
        (lap_time - drv_lap_mean) / drv_lap_std.replace(0, 1)
    ).astype("float32")
    X["DeltaVsDriverMean"] = (lap_delta - drv_delta_mean).astype("float32")
    X["TyreLifeFracRace"] = (tyre_life / drv_tyre_max).astype("float32")
    X["TyreLifeFracStint"] = (tyre_life / stint_tyre_max).astype("float32")
    X["StintDegRange"] = (stint_deg_max - stint_deg_min).astype("float32")
    X["DegFromStintMin"] = (degradation - stint_deg_min).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000994"],
        "research_usage_note": "Verified assigned hypothesis 000994.",
    }, sort_keys=True))
