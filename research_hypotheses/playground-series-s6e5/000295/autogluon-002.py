...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_slick = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")

    rp = X["RaceProgress"].astype("float32")
    tyre = X["TyreLife"].astype("float32")
    lap = X["LapNumber"].astype("float32")
    stint = X["Stint"].astype("float32")
    pos = X["Position"].astype("float32")
    pos_chg = X["Position_Change"].astype("float32")
    lap_time = X["LapTime (s)"].astype("float32")
    lap_delta = X["LapTime_Delta"].astype("float32")
    degr = X["Cumulative_Degradation"].astype("float32")
    pit_now = X["PitStop"].astype("float32")

    # Regime flags for specialist-style modeling
    X["regime_wet"] = is_wet
    X["regime_dry"] = is_dry
    X["regime_endgame"] = (rp >= 0.80).astype("int8")
    X["regime_late"] = (rp >= 0.67).astype("int8")
    X["regime_mid"] = ((rp >= 0.33) & (rp < 0.67)).astype("int8")
    X["regime_opening"] = (rp < 0.20).astype("int8")

    # Mandatory-stop-debt proxy: strongest on slicks, especially early stints with long tyre life and advancing race progress
    debt_score = (
        is_slick
        * (0.55 * tyre + 22.0 * rp + 3.0 * np.maximum(stint - 1.0, 0.0) + 6.0 * pit_now)
    ).astype("float32")
    X["mandatory_stop_debt_score"] = debt_score
    X["regime_stop_debt"] = (
        is_slick.astype(bool)
        & (
            ((stint <= 2) & (tyre >= 18))
            | ((stint == 1) & (rp >= 0.35))
            | ((stint == 2) & (tyre >= 14) & (rp >= 0.25))
        )
    ).astype("int8")

    # Regime-specialist feature copies so downstream models can learn separate rules cleanly
    X["tyre_life_dry"] = (tyre * is_dry).astype("float32")
    X["tyre_life_wet"] = (tyre * is_wet).astype("float32")
    X["degradation_dry"] = (degr * is_dry).astype("float32")
    X["degradation_wet"] = (degr * is_wet).astype("float32")
    X["lap_delta_dry"] = (lap_delta * is_dry).astype("float32")
    X["lap_delta_wet"] = (lap_delta * is_wet).astype("float32")
    X["pit_now_dry"] = (pit_now * is_dry).astype("float32")
    X["pit_now_wet"] = (pit_now * is_wet).astype("float32")
    X["tyre_life_endgame"] = (tyre * X["regime_endgame"]).astype("float32")
    X["debt_endgame"] = (debt_score * X["regime_endgame"]).astype("float32")
    X["debt_dry"] = (debt_score * is_dry).astype("float32")
    X["debt_wet"] = (debt_score * is_wet).astype("float32")

    # Interactions aligned with regime decisions
    X["tyre_progress"] = (tyre * rp).astype("float32")
    X["degradation_per_lap"] = (degr / np.maximum(tyre, 1.0)).astype("float32")
    X["lap_delta_x_tyre"] = (lap_delta * tyre).astype("float32")
    X["degr_x_progress"] = (degr * rp).astype("float32")
    X["position_risk"] = ((21.0 - pos) * np.abs(pos_chg)).astype("float32")
    X["late_position_risk"] = (X["position_risk"] * X["regime_late"]).astype("float32")
    X["stint_progress"] = (stint * rp).astype("float32")
    X["fresh_stop_signal"] = (
        (pit_now == 1).astype("float32") * np.maximum(8.0 - tyre, 0.0)
    ).astype("float32")

    # Lightweight sequential context within driver-race, preserving row order and avoiding leakage
    grp = [X["Year"], X["Race"], X["Driver"]]
    X["prev_lap_delta"] = (
        X.groupby(grp, sort=False)["LapTime_Delta"].shift(1).fillna(0).astype("float32")
    )
    X["prev_position_change"] = (
        X.groupby(grp, sort=False)["Position_Change"]
        .shift(1)
        .fillna(0)
        .astype("float32")
    )
    X["prev_pitstop"] = (
        X.groupby(grp, sort=False)["PitStop"].shift(1).fillna(0).astype("float32")
    )
    X["delta_change"] = (lap_delta - X["prev_lap_delta"]).astype("float32")
    X["position_change_reversal"] = (pos_chg - X["prev_position_change"]).astype(
        "float32"
    )
    X["back_to_back_pit_pressure"] = ((pit_now + X["prev_pitstop"]) * is_dry).astype(
        "float32"
    )

    # Bucketed regime labels as low-cardinality categorical helpers
    X["race_progress_band"] = pd.cut(
        rp,
        bins=[-0.01, 0.2, 0.4, 0.6, 0.8, 1.01],
        labels=["p0", "p1", "p2", "p3", "p4"],
    ).astype(str)
    X["tyre_life_band"] = pd.cut(
        tyre,
        bins=[0, 5, 10, 15, 22, 35, 100],
        labels=["t0", "t1", "t2", "t3", "t4", "t5"],
    ).astype(str)
    wet_label = pd.Series(np.where(is_wet == 1, "wet", "dry"), index=X.index)
    debt_label = pd.Series(
        np.where(X["regime_stop_debt"] == 1, "debt", "nodebt"),
        index=X.index,
    )
    end_label = pd.Series(
        np.where(X["regime_endgame"] == 1, "end", "nonend"),
        index=X.index,
    )
    X["regime_combo"] = wet_label.str.cat(debt_label, sep="_").str.cat(
        end_label,
        sep="_",
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000295"],
        "research_usage_note": "Verified assigned hypothesis 000295.",
    }, sort_keys=True))
