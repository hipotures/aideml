...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = group_cols + ["LapNumber", "Stint", "TyreLife", "_orig_idx"]
    work = out.sort_values(sort_cols, kind="mergesort").copy()

    monaco_2025 = (
        (work["Race"] == "Monaco Grand Prix") & (work["Year"] == 2025)
    ).astype(np.int8)
    work["Monaco2025"] = monaco_2025

    required_stop_count = np.where(monaco_2025 == 1, 2, 1).astype(np.int8)
    required_tyre_sets = np.where(monaco_2025 == 1, 3, 2).astype(np.int8)
    work["required_stop_count"] = required_stop_count
    work["required_tyre_sets"] = required_tyre_sets

    pit_count_after_current = (
        work.groupby(group_cols, sort=False)["PitStop"].cumsum().astype(np.int16)
    )
    work["PitCountAfterCurrent"] = pit_count_after_current

    race_progress = work["RaceProgress"].astype(float).clip(lower=1e-6, upper=1.0)
    total_laps_est = (work["LapNumber"].astype(float) / race_progress).clip(
        lower=work["LapNumber"].astype(float), upper=100.0
    )
    laps_remaining_est = (total_laps_est - work["LapNumber"].astype(float)).clip(
        lower=0.0, upper=100.0
    )
    work["LapsRemaining_Est"] = laps_remaining_est

    stop_debt = (required_stop_count - pit_count_after_current).astype(np.int16)
    work["StopDebtToRule"] = stop_debt
    work["RuleDebtAfterCurrent"] = np.maximum(stop_debt, 0).astype(np.int8)

    work["LateRace_MonacoDebt"] = (
        monaco_2025
        * np.maximum(stop_debt, 0)
        * race_progress
        * (laps_remaining_est <= 25).astype(np.int8)
    ).astype(float)

    work["RuleDebt_x_PitCountAfterCurrent"] = (
        np.maximum(stop_debt, 0) * pit_count_after_current
    ).astype(np.int16)
    work["RuleDebt_x_LapsRemaining"] = (
        np.maximum(stop_debt, 0) * laps_remaining_est
    ).astype(float)
    work["Monaco2025_x_LapsRemaining"] = (monaco_2025 * laps_remaining_est).astype(
        float
    )
    work["Monaco2025_x_TyreLife"] = (
        monaco_2025 * work["TyreLife"].astype(float)
    ).astype(float)

    work = work.sort_values("_orig_idx", kind="mergesort").drop(columns=["_orig_idx"])
    return work
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000468.",
    }, sort_keys=True))
