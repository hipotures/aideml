...
for fold, (tr_idx, val_idx) in enumerate(splits, start=1):
    model = LGBMClassifier(**base_params, random_state=468 + fold)
    model.fit(
        X.iloc[tr_idx],
        y[tr_idx],
    ...
    best_iter = model.best_iteration_ or base_params["n_estimators"]
...
final_model = LGBMClassifier(**final_params, random_state=10468)
final_model.fit(
    X,
    y,
    categorical_feature=cat_features,
