...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_mean = df.groupby(grp)["LapTime (s)"].transform("mean")
    lap_std = df.groupby(grp)["LapTime (s)"].transform("std").fillna(0.0)
    pos_mean = df.groupby(grp)["Position"].transform("mean")
    deg_mean = df.groupby(grp)["Cumulative_Degradation"].transform("mean")
    tyre_mean = df.groupby(grp)["TyreLife"].transform("mean")

    df["laptime_rel_field"] = df["LapTime (s)"] - lap_mean
    df["laptime_z_field"] = (df["LapTime (s)"] - lap_mean) / (lap_std + 1e-6)
    df["position_rel_field"] = df["Position"] - pos_mean
    df["deg_rel_field"] = df["Cumulative_Degradation"] - deg_mean
    df["tyrelife_rel_field"] = df["TyreLife"] - tyre_mean

    grip = df["Compound"].map(compound_grip).fillna(0.65)
    deg_rate = df["Compound"].map(compound_deg).fillna(0.75)
    max_life = df["Compound"].map(compound_max_life).fillna(24.0)

    est_total_laps = (df["LapNumber"] / df["RaceProgress"].clip(lower=0.01)).clip(
        upper=90
    )
    laps_remaining = (est_total_laps - df["LapNumber"]).clip(lower=0.0, upper=80.0)
    stint_remaining_capacity = (max_life - df["TyreLife"]).clip(lower=0.0)
    finish_feasibility_gap = (laps_remaining - stint_remaining_capacity).clip(lower=0.0)

    tyre_pressure_now = (df["TyreLife"] / max_life).clip(lower=0.0, upper=3.0)
    tyre_pressure_next = ((df["TyreLife"] + 1.0) / max_life).clip(lower=0.0, upper=3.0)

    deg_norm = df["Cumulative_Degradation"].clip(lower=0.0) / 100.0
    pace_penalty_now = (
        df["laptime_rel_field"].clip(lower=0.0) / (lap_mean.abs() + 1e-6) * 10.0
    )
    pace_penalty_next = (
        pace_penalty_now
        + 0.18 * deg_rate
        + 0.10 * tyre_pressure_next
        + 0.05 * df["laptime_z_field"].clip(lower=0.0)
    )

    traffic_penalty = ((20.0 - df["Position"]).clip(lower=0.0) / 20.0) * (
        1.0 - df["RaceProgress"]
    ).clip(lower=0.0)
    track_position_cost = (df["Position"] / 20.0) * (1.0 - 0.35 * df["RaceProgress"])

    wait_1_value = (
        1.35 * tyre_pressure_next
        + 0.90 * deg_norm * deg_rate
        + 1.10 * pace_penalty_next
        + 0.85 * df["deg_rel_field"].clip(lower=0.0) / 100.0
        + 1.50 * finish_feasibility_gap / (laps_remaining + 1.0)
        + 0.45 * traffic_penalty
        + 0.20 * track_position_cost
    )

    fresh_tyre_gain = (
        1.10 * grip
        + 0.95 * tyre_pressure_now
        + 0.70 * deg_norm * deg_rate
        + 0.75 * pace_penalty_now
        + 0.80 * df["deg_rel_field"].clip(lower=0.0) / 100.0
        + 1.25 * finish_feasibility_gap / (laps_remaining + 1.0)
    )

    pit_loss_proxy = (
        1.55 * (1.0 - df["RaceProgress"]).clip(lower=0.0)
        + 0.55 * (df["Position"] / 20.0)
        + 0.25 * (df["Stint"] <= 1).astype(float)
        + 0.20 * traffic_penalty
    )

    pit_now_value = fresh_tyre_gain - pit_loss_proxy
    value_gap = pit_now_value - wait_1_value

    df["wait_1_value"] = wait_1_value
    df["pit_now_value"] = pit_now_value
    df["pit_now_value_minus_wait_1_value"] = value_gap
    df["pit_decision_pressure_x_gap"] = value_gap * (
        0.6 * tyre_pressure_now + 0.4 * deg_norm + 0.3 * pace_penalty_now
    )
    df["finish_pressure"] = finish_feasibility_gap
    df["fresh_tyre_gain"] = fresh_tyre_gain
    df["pit_loss_proxy"] = pit_loss_proxy
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000843.",
    }, sort_keys=True))
