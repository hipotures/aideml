...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_slick"] = (~out["Compound"].isin(["INTERMEDIATE", "WET"])).astype("int8")
    out["tyre_life_progress"] = out["TyreLife"] / (out["LapNumber"].replace(0, np.nan))
    out["stint_lap_ratio"] = out["TyreLife"] / (
        out["RaceProgress"] * out["LapNumber"].max() + 1.0
    )
    ...
    query_regime_cols = ["Year", "Race", "LapNumber", "is_wet_regime"]
    ...
    for keys, prefix in ((query_cols, "qlap"), (query_regime_cols, "qreg")):
        g = out.groupby(keys, sort=False, observed=True)
        size = g["LapNumber"].transform("size").astype("int16")
        out[f"{prefix}_size"] = size

        for c in num_cols:
            mean = g[c].transform("mean")
            std = g[c].transform("std").fillna(0.0)
            out[f"{prefix}_{c}_mean_diff"] = out[c] - mean
            out[f"{prefix}_{c}_z"] = (out[c] - mean) / (std + 1e-6)
            out[f"{prefix}_{c}_pct"] = (
                g[c].rank(method="average", pct=True).astype("float32")
            )
            out[f"{prefix}_{c}_rank_desc"] = (
                g[c].rank(method="average", ascending=False).astype("float32")
            )
    ...
    out["race_lap_regime_query"] = (
        out["race_lap_query"] + "_" + out["is_wet_regime"].astype(str)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000725"],
        "research_usage_note": "Verified assigned hypothesis 000725.",
    }, sort_keys=True))
