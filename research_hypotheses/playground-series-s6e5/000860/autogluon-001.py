...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    remaining_laps = (race_laps_est - lap).clip(lower=0)

    out["surv_interval_start"] = (tyre - 1).clip(lower=0).astype("float32")
    out["surv_interval_stop"] = tyre.astype("float32")
    out["surv_log_stop"] = np.log1p(tyre).astype("float32")
    out["surv_stop_sq"] = (tyre * tyre).astype("float32")
    out["surv_race_laps_est"] = race_laps_est.astype("float32")
    out["surv_remaining_laps"] = remaining_laps.astype("float32")
    out["surv_remaining_frac"] = (1.0 - progress).clip(lower=0).astype("float32")
    out["surv_stint_age_frac"] = (
        (tyre / (race_laps_est + eps)).clip(0, 1).astype("float32")
    )
    out["surv_censor_end_pressure"] = (
        ((progress - 0.82) / 0.18).clip(0, 1).astype("float32")
    )
    out["surv_is_wet_regime"] = wet.astype("int8")
    ...
    out["surv_past_pit_count"] = s["_pit_before"].to_numpy(dtype="float32")
    out["surv_prev_lap_pit"] = s["_prev_pit"].to_numpy(dtype="float32")
    out["surv_laps_since_pit"] = s["_laps_since_pit"].to_numpy(dtype="float32")
    ...
    out["surv_degradation_clipped"] = degr_clip.astype("float32")
    out["surv_laptime_delta_clipped"] = ltd_clip.astype("float32")
    out["surv_age_x_slick"] = (tyre * slick).astype("float32")
    out["surv_age_x_wet"] = (tyre * wet).astype("float32")
    out["surv_degradation_x_slick"] = (degr_clip * slick).astype("float32")
    out["surv_degradation_x_wet"] = (degr_clip * wet).astype("float32")
    out["surv_progress_x_slick"] = (progress * slick).astype("float32")
    out["surv_progress_x_wet"] = (progress * wet).astype("float32")
    ...
    z = (
        ...
        + 0.075 * stint
        + 0.025 * position
        + 0.045 * pos_chg.clip(lower=0)
        - 1.20 * pit
        + 0.35 * wet
        + 0.80 * out["surv_censor_end_pressure"].astype("float32")
    )
    out["surv_discrete_hazard_proxy"] = (1.0 / (1.0 + np.exp(-z.clip(-20, 20)))).astype(
        "float32"
    )
    out["surv_hazard_proxy_slick"] = (out["surv_discrete_hazard_proxy"] * slick).astype(
        "float32"
    )
    out["surv_hazard_proxy_wet"] = (out["surv_discrete_hazard_proxy"] * wet).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000860"],
        "research_usage_note": "Verified assigned hypothesis 000860.",
    }, sort_keys=True))
