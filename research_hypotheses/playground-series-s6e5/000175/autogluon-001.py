...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(c in out.columns for c in base_cols):
        return out
    ...
    race_lap_med = (
        out.groupby(["Year", "Race", "LapNumber"], sort=False)[lap_col]
        .transform("median")
        .astype("float32")
    )
    residual0 = lap - race_lap_med

    comp_base = (
        residual0.groupby(out["Compound"], sort=False)
        .transform("median")
        .astype("float32")
    )
    residual1 = residual0 - comp_base

    x = progress.to_numpy(dtype=np.float64)
    y = residual1.to_numpy(dtype=np.float64)
    xm = float(np.nanmean(x))
    ym = float(np.nanmean(y))
    beta = float(np.nansum((x - xm) * (y - ym)) / (np.nansum((x - xm) ** 2) + eps))
    trend = beta * (progress - xm)
    resid = (residual1 - trend).astype("float32").to_numpy()

    sort_cols = ["Year", "Race", "Driver", "Stint", "LapNumber"]
    order = np.lexsort(tuple(out[c].to_numpy() for c in reversed(sort_cols)))
    ...
    for idx in order:
        ...
        z = resid[idx]
        k = pred_v / (pred_v + r_obs)
        new_m = pred_m + k * (z - pred_m)
        new_v = (1.0 - k) * pred_v + q_slope * abs(cur_s)

        prev_s = cur_s
        prev_m = pred_m
        m = float(new_m)
        v = float(np.clip(new_v, 0.05, 25.0))
    ...
    out["latent_tyre_level_000175"] = level
    out["latent_tyre_slope_000175"] = slope
    out["latent_tyre_accel_000175"] = accel
    out["latent_tyre_uncertainty_000175"] = uncert
    out["pace_residual_000175"] = resid.astype("float32")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000175"],
        "research_usage_note": "Verified assigned hypothesis 000175.",
    }, sort_keys=True))
