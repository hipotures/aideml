...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = df["Compound"].astype("string").str.upper()
    wet_flag = compound.isin(["INTERMEDIATE", "WET"])
    slick_flag = compound.isin(["SOFT", "MEDIUM", "HARD"])

    df["Compound"] = compound.astype("category")
    df["is_wet_compound"] = wet_flag.astype("int8")
    df["is_slick_compound"] = slick_flag.astype("int8")
    df["is_soft"] = compound.eq("SOFT").astype("int8")
    df["is_medium"] = compound.eq("MEDIUM").astype("int8")
    df["is_hard"] = compound.eq("HARD").astype("int8")
    df["is_intermediate"] = compound.eq("INTERMEDIATE").astype("int8")
    df["is_wet"] = compound.eq("WET").astype("int8")

    race_progress = df["RaceProgress"].clip(0, 1)
    phase = np.where(
        race_progress < 0.33, "early", np.where(race_progress < 0.67, "mid", "late")
    )
    df["race_phase"] = pd.Series(phase, index=df.index, dtype="category")

    abs_pos_change = df["Position_Change"].abs()
    abs_lap_delta = df["LapTime_Delta"].abs()
    shock_score = abs_pos_change + abs_lap_delta.rank(method="average", pct=True)
    shock_pct = shock_score.rank(method="average", pct=True)
    shock_band = np.where(
        shock_pct >= 0.75, "high", np.where(shock_pct >= 0.40, "medium", "low")
    )
    df["shock_score"] = shock_score.astype("float32")
    df["shock_band"] = pd.Series(shock_band, index=df.index, dtype="category")
    df["is_high_shock"] = (shock_pct >= 0.75).astype("int8")

    df["is_first_stint"] = (df["Stint"] == 1).astype("int8")
    df["is_second_stint"] = (df["Stint"] == 2).astype("int8")
    df["dry_first_stint"] = (slick_flag & (df["Stint"] == 1)).astype("int8")
    df["dry_second_stint"] = (slick_flag & (df["Stint"] == 2)).astype("int8")

    tyre_life = df["TyreLife"].astype("float32")
    lap_number = df["LapNumber"].astype("float32")
    cumulative_deg = df["Cumulative_Degradation"].astype("float32")
    lap_time = df["LapTime (s)"].astype("float32")

    df["tyre_life_ratio"] = (
        (tyre_life / lap_number.replace(0, np.nan)).fillna(0).astype("float32")
    )
    df["deg_per_lap"] = (
        (cumulative_deg / tyre_life.replace(0, np.nan))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    df["deg_x_tyre_life"] = (cumulative_deg * tyre_life).astype("float32")
    df["deg_x_progress"] = (cumulative_deg * race_progress).astype("float32")
    df["late_deg_pressure"] = (
        cumulative_deg * (race_progress >= 0.67).astype("int8")
    ).astype("float32")
    df["lap_delta_x_progress"] = (
        df["LapTime_Delta"].astype("float32") * race_progress
    ).astype("float32")
    df["pos_change_x_progress"] = (
        df["Position_Change"].astype("float32") * race_progress
    ).astype("float32")
    df["lap_time_over_tyre_life"] = (
        (lap_time / tyre_life.replace(0, np.nan))
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )

    regime_base = pd.Series(np.where(wet_flag, "wet", "slick"), index=df.index, dtype="string")
    phase_s = pd.Series(phase, index=df.index, dtype="string")
    shock_s = pd.Series(shock_band, index=df.index, dtype="string")
    regime = regime_base.str.cat(phase_s, sep="_").str.cat(shock_s, sep="_")
    df["specialist_regime"] = regime.astype("category")
    df["compound_phase"] = compound.fillna("UNK").str.cat(phase_s, sep="_").astype("category")
    df["regime_compound"] = regime.str.cat(compound.fillna("UNK"), sep="_").astype("category")
    df["driver_regime"] = df["Driver"].astype("string").str.cat(regime, sep="_").astype("category")
    df["race_regime"] = df["Race"].astype("string").str.cat(regime, sep="_").astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000216"],
        "research_usage_note": "Verified assigned hypothesis 000216.",
    }, sort_keys=True))
