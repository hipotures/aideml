...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    regime = np.where(wet_flag, "wet", "slick") + "_" + phase + "_" + shock_band
    df["specialist_regime"] = pd.Series(regime, index=df.index, dtype="category")
    df["compound_phase"] = (
        compound.fillna("UNK") + "_" + pd.Series(phase, index=df.index)
    ).astype("category")
    df["regime_compound"] = (
        pd.Series(regime, index=df.index) + "_" + compound.fillna("UNK")
    ).astype("category")
    df["driver_regime"] = (
        df["Driver"].astype("string") + "_" + pd.Series(regime, index=df.index)
    ).astype("category")
    df["race_regime"] = (
        df["Race"].astype("string") + "_" + pd.Series(regime, index=df.index)
    ).astype("category")
