...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    obj_cols = [c for c in ["Compound", "Driver", "Race"] if c in X.columns]
    for c in obj_cols:
    ...
    X["CompoundOrdinal"] = X["Compound"].map(compound_order).fillna(-1).astype("int8")
    X["IsWetCompound"] = X["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    X["IsDryCompound"] = X["Compound"].isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")

    X["Race_Year"] = X["Race"] + "_" + X["Year"].astype(str)
    X["Driver_Year"] = X["Driver"] + "_" + X["Year"].astype(str)
    X["Driver_Race"] = X["Driver"] + "_" + X["Race"]
    X["Driver_Compound"] = X["Driver"] + "_" + X["Compound"]
    X["Race_Compound"] = X["Race"] + "_" + X["Compound"]
    X["Driver_Race_Compound"] = X["Driver_Race"] + "_" + X["Compound"]
    ...
    for c in [
        ...
        X[c + "_freq"] = X[c].map(X[c].value_counts(dropna=False)).astype("float32")

    race_keys = ["Year", "Race"]
    driver_race_keys = ["Year", "Race", "Driver"]
    stint_keys = ["Year", "Race", "Driver", "Stint"]
    lap_keys = ["Year", "Race", "LapNumber"]

    eps = 1e-6

    race_max_lap = X.groupby(race_keys)["LapNumber"].transform("max")
    X["RaceLapsTotal"] = race_max_lap.astype("float32")
    X["LapsRemaining"] = (race_max_lap - X["LapNumber"]).astype("float32")
    X["RaceProgressCentered"] = (X["RaceProgress"] - 0.5).astype("float32")

    stint_max_tyre = X.groupby(stint_keys)["TyreLife"].transform("max")
    X["StintTyreLifeMax"] = stint_max_tyre.astype("float32")
    X["TyreLifeToStintMax"] = (X["TyreLife"] / (stint_max_tyre + eps)).astype("float32")
    X["TyreLifeRemainingInStint"] = (stint_max_tyre - X["TyreLife"]).astype("float32")
    X["EndOfStintPressure"] = (X["TyreLifeToStintMax"] * X["RaceProgress"]).astype(
        "float32"
    )

    X["DegPerTyreLap"] = (X["Cumulative_Degradation"] / (X["TyreLife"] + eps)).astype(
        "float32"
    )
    X["DegPerRaceLap"] = (X["Cumulative_Degradation"] / (X["LapNumber"] + eps)).astype(
        "float32"
    )
    X["LapDeltaPerTyreLap"] = (X["LapTime_Delta"] / (X["TyreLife"] + eps)).astype(
        "float32"
    )
    X["LapTime_x_RaceProgress"] = (X["LapTime (s)"] * X["RaceProgress"]).astype(
        "float32"
    )
    X["PositionPressure"] = ((21 - X["Position"]) * X["TyreLifeToStintMax"]).astype(
        "float32"
    )
    X["PositionChangePressure"] = (
        X["Position_Change"].abs() * X["TyreLifeToStintMax"]
    ).astype("float32")

    lap_median_time = X.groupby(lap_keys)["LapTime (s)"].transform("median")
    lap_mean_delta = X.groupby(lap_keys)["LapTime_Delta"].transform("mean")
    lap_std_delta = X.groupby(lap_keys)["LapTime_Delta"].transform("std").fillna(0.0)
    lap_mean_deg = X.groupby(lap_keys)["Cumulative_Degradation"].transform("mean")
    lap_std_deg = (
        X.groupby(lap_keys)["Cumulative_Degradation"].transform("std").fillna(0.0)
    )

    X["LapTimeVsRaceLapMedian"] = (X["LapTime (s)"] - lap_median_time).astype("float32")
    X["LapTimeRaceLapRatio"] = (X["LapTime (s)"] / (lap_median_time + eps)).astype(
        "float32"
    )
    X["LapDeltaRaceLapZ"] = (
        (X["LapTime_Delta"] - lap_mean_delta) / (lap_std_delta + eps)
    ).astype("float32")
    X["DegRaceLapZ"] = (
        (X["Cumulative_Degradation"] - lap_mean_deg) / (lap_std_deg + eps)
    ).astype("float32")

    sorted_idx = X.sort_values(driver_race_keys + ["LapNumber"], kind="mergesort").index
    XS = X.loc[sorted_idx]

    g = XS.groupby(driver_race_keys, sort=False)
    prev_lap_time = g["LapTime (s)"].shift(1)
    prev_lap_delta = g["LapTime_Delta"].shift(1)
    prev_position = g["Position"].shift(1)
    prev_pos_change = g["Position_Change"].shift(1)
    prev_tyre = g["TyreLife"].shift(1)
    prev_deg = g["Cumulative_Degradation"].shift(1)
    prev_pit = g["PitStop"].shift(1)

    XS["PrevLapTime"] = prev_lap_time
    XS["PrevLapDelta"] = prev_lap_delta
    XS["PrevPosition"] = prev_position
    XS["PrevTyreLife"] = prev_tyre
    XS["PrevPitStop"] = prev_pit.fillna(0).astype("float32")

    XS["LapTimeChange1"] = (
        (XS["LapTime (s)"] - prev_lap_time).fillna(0).astype("float32")
    )
    XS["LapDeltaChange1"] = (
        (XS["LapTime_Delta"] - prev_lap_delta).fillna(0).astype("float32")
    )
    XS["PositionChange1"] = (XS["Position"] - prev_position).fillna(0).astype("float32")
    XS["PosChangeMomentum1"] = (
        (XS["Position_Change"] - prev_pos_change).fillna(0).astype("float32")
    )
    XS["TyreLifeStep"] = (XS["TyreLife"] - prev_tyre).fillna(0).astype("float32")
    XS["DegChange1"] = (
        (XS["Cumulative_Degradation"] - prev_deg).fillna(0).astype("float32")
    )
    XS["PostPitLapFlag"] = ((XS["PrevPitStop"] > 0) | (XS["PitStop"] > 0)).astype(
        "int8"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000931"],
        "research_usage_note": "Verified assigned hypothesis 000931.",
    }, sort_keys=True))
