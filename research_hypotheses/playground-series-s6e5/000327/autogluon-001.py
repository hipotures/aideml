...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_norm"] = comp.astype(str).str.upper().astype("category")
    compound_sens = {
        "SOFT": 1.25,
        "MEDIUM": 1.00,
        "HARD": 0.78,
    ...
    sens = out["Compound_norm"].map(compound_sens).astype(float).fillna(1.0)
    warmup = out["Compound_norm"].map(warmup_penalty).astype(float).fillna(1.0)

    tyre = out["TyreLife"].astype(float)
    prog = out["RaceProgress"].astype(float).clip(0, 1)
    ...
    ldelta = out["LapTime_Delta"].astype(float)
    ltime = out["LapTime (s)"].astype(float)

    keys = [c for c in ["Year", "Race", "Driver"] if c in out.columns]
    sort_cols = keys + ["LapNumber", "Stint"]
    sidx = out.sort_values(sort_cols, kind="mergesort").index if keys else out.index

    old_next = pd.Series(index=out.index, dtype=float)
    pace_trend = pd.Series(index=out.index, dtype=float)
    if keys:
        sd = out.loc[sidx]
        g = sd.groupby(keys, observed=True, sort=False)
        prev_tyre = g["TyreLife"].shift(1)
        prev_delta = g["LapTime_Delta"].shift(1)
        old_next.loc[sidx] = (
            sd["TyreLife"].astype(float) - prev_tyre.astype(float)
        ).fillna(1.0)
        pace_trend.loc[sidx] = (
            sd["LapTime_Delta"].astype(float) - prev_delta.astype(float)
        ).fillna(0.0)
    else:
        old_next[:] = 1.0
        pace_trend[:] = 0.0

    old_next = old_next.clip(0, 3).fillna(1.0)
    pace_trend = pace_trend.clip(-15, 15).fillna(0.0)

    late_race = np.sqrt(prog.clip(0.02, 1.0))
    tyre_stress = np.log1p(tyre.clip(lower=0)) * sens
    degradation_load = np.log1p(np.maximum(deg, 0)) / 4.0
    bad_delta = np.maximum(ldelta, 0).clip(0, 20) / 5.0
    outlier_lap = (
        (ltime > ltime.groupby(out["Race"]).transform("median") * 1.18).astype(float)
        if "Race" in out.columns
        else (ltime > ltime.median() * 1.18).astype(float)
    )

    old_tyre_penalty_next = (
        0.42 * tyre_stress
        + 0.32 * old_next * sens
        + 0.24 * degradation_load
        + 0.18 * bad_delta
        + 0.10 * np.maximum(pace_trend, 0)
    )

    fresh_outlap_penalty = (
        warmup
        + 0.42 * (stint <= 1).astype(float)
        + 0.28 * (prog < 0.18).astype(float)
        + 0.22 * outlier_lap
    )

    cheap_stop_discount = (
        1.15 * (prog > 0.72).astype(float)
        + 0.80 * (tyre >= 18).astype(float)
        + 0.45 * (stint >= 2).astype(float)
        + 0.35 * np.maximum(-pos_chg, 0).clip(0, 6) / 6.0
    )

    traffic_penalty = (
        0.70 * ((pos >= 8) & (pos <= 16)).astype(float)
        + 0.35 * (pos_chg > 0).astype(float)
        + 0.25 * (pos <= 5).astype(float)
        + 0.25 * (prog < 0.25).astype(float)
    )

    stay_out_one_more_lap = old_tyre_penalty_next + traffic_penalty
    pit_now_cost = fresh_outlap_penalty - cheap_stop_discount
    pit_now_edge = stay_out_one_more_lap - pit_now_cost

    out["old_tyre_penalty_next_lap"] = old_tyre_penalty_next.astype("float32")
    out["fresh_tyre_outlap_penalty"] = fresh_outlap_penalty.astype("float32")
    out["cheap_stop_discount"] = cheap_stop_discount.astype("float32")
    out["traffic_penalty"] = traffic_penalty.astype("float32")
    out["stay_out_one_more_lap_score"] = stay_out_one_more_lap.astype("float32")
    out["pit_now_cost_score"] = pit_now_cost.astype("float32")
    out["pit_now_edge"] = pit_now_edge.astype("float32")
    out["pit_now_edge_x_progress"] = (pit_now_edge * late_race).astype("float32")
    out["pit_now_edge_x_tyrelife"] = (
        pit_now_edge * np.log1p(tyre.clip(lower=0))
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000327"],
        "research_usage_note": "Verified assigned hypothesis 000327.",
    }, sort_keys=True))
