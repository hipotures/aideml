...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    y_adv = np.zeros(n, dtype=np.int8)
    if n > n_train:
        y_adv[n_train:] = 1
    ...
    if n > n_train and y_adv.min() != y_adv.max() and transformers:
        adv_model = make_pipeline(
        ...
        adv_model.fit(out[feature_cols], y_adv)
        p_test = adv_model.predict_proba(out[feature_cols])[:, 1]
        prior_test = max((n - n_train) / n, 1e-6)
        prior_train = max(n_train / n, 1e-6)
        odds_weight = (p_test / np.clip(1.0 - p_test, 1e-5, None)) * (
            prior_train / prior_test
        )
        out["adv_test_likeness_000631"] = p_test.astype("float32")
        out["adv_importance_weight_000631"] = np.clip(odds_weight, 0.05, 20.0).astype(
            "float32"
        )
    else:
        out["adv_test_likeness_000631"] = np.float32(0.0)
        out["adv_importance_weight_000631"] = np.float32(1.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000631.",
    }, sort_keys=True))
