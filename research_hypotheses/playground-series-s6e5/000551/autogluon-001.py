...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep raw categories and add low-cardinality interactions for ordered categorical models.
    out["Year_str"] = out["Year"].astype(str)
    out["Race_Year"] = out["Race"] + "_" + out["Year_str"]
    ...
    out["Stint_Compound"] = out["Stint"].astype(str) + "_" + out["Compound"]
    out["RaceProgressBin"] = (
        np.floor(out["RaceProgress"] * 10).clip(0, 10).astype(int).astype(str)
    )
    out["Progress_Compound"] = out["RaceProgressBin"] + "_" + out["Compound"]

    # Group structure for lap-local ranking models.
    lap_group = ["Year", "Race", "LapNumber"]
    stint_group = ["Year", "Race", "Driver", "Stint"]

    out["LapGroup"] = (
        out["Year"].astype(str) + "_" + out["Race"] + "_" + out["LapNumber"].astype(str)
    )
    out["GroupSize_Lap"] = (
        out.groupby(lap_group)["Driver"].transform("size").astype(np.int32)
    )

    # Within-lap percentile ranks: useful for "who is most likely to pit this lap?" ordering.
    rank_specs = {
        "Position": True,
        "TyreLife": True,
        "Cumulative_Degradation": True,
        "LapTime (s)": True,
        "LapTime_Delta": True,
    ...
    for col, pct in rank_specs.items():
        out[f"{col}_lap_rank_pct"] = out.groupby(lap_group)[col].rank(
            method="average", pct=pct
        )

    # Within-lap gaps from group aggregates capture relative status without using future target info.
    for col in [
        "TyreLife",
        ...
        g = out.groupby(lap_group)[col]
        out[f"{col}_lap_mean_gap"] = out[col] - g.transform("mean")
        out[f"{col}_lap_min_gap"] = out[col] - g.transform("min")
        out[f"{col}_lap_max_gap"] = out[col] - g.transform("max")

    # Stint-context features: tyre age and degradation relative to a driver's current stint.
    ...
    out["Degradation_per_TyreLife"] = out["Cumulative_Degradation"] / out[
        "TyreLife"
    ].clip(lower=1)
    out["Degradation_x_TyreLife"] = out["Cumulative_Degradation"] * out["TyreLife"]
    out["LapTimeDelta_x_TyreLife"] = out["LapTime_Delta"] * out["TyreLife"]
    out["PositionChange_x_TyreLife"] = out["Position_Change"] * out["TyreLife"]

    stint_max_tyre = out.groupby(stint_group)["TyreLife"].transform("max")
    stint_mean_deg = out.groupby(stint_group)["Cumulative_Degradation"].transform(
        "mean"
    )
    out["TyreLife_to_stint_max"] = out["TyreLife"] / stint_max_tyre.clip(lower=1)
    out["Deg_from_stint_mean"] = out["Cumulative_Degradation"] - stint_mean_deg

    # Simple nonlinearities that tree models often exploit around pit windows.
    out["TyreLife_sq"] = out["TyreLife"] ** 2
    out["RaceProgress_sq"] = out["RaceProgress"] ** 2
    out["LateRace_x_OldTyres"] = out["RaceProgress"] * out["TyreLife"]
    out["LateRace_x_Degradation"] = out["RaceProgress"] * out["Cumulative_Degradation"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000551"],
        "research_usage_note": "Verified assigned hypothesis 000551.",
    }, sort_keys=True))
