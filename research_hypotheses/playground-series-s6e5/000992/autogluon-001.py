...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    np = __import__("numpy")
    out = df.copy()
    keys = ["Year", "Race", "Driver", "Stint"]

    order = (
        out.assign(_row_order=np.arange(len(out)))
        .sort_values(keys + ["LapNumber", "_row_order"], kind="mergesort")
        .index
    )
    s = out.loc[order].copy()

    for c in ["LapNumber", "TyreLife", "LapTime (s)", "Cumulative_Degradation"]:
        s[c] = pd.to_numeric(s[c], errors="coerce").astype("float64")

    g = s.groupby(keys, sort=False, observed=True)
    x = s["TyreLife"]
    lt = s["LapTime (s)"]
    deg = s["Cumulative_Degradation"]

    s["_lt_diff"] = g["LapTime (s)"].diff()
    s["_deg_diff"] = g["Cumulative_Degradation"].diff()
    s["_lt_diff_past"] = g["_lt_diff"].shift(1)
    s["_deg_diff_past"] = g["_deg_diff"].shift(1)
    s["_life_prev"] = g["TyreLife"].shift(1)

    levels = list(range(len(keys)))
    for base, src in [("lt", "_lt_diff_past"), ("deg", "_deg_diff_past")]:
        s[f"_slope_{base}_3"] = (
            s.groupby(keys, sort=False, observed=True)[src]
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(level=levels, drop=True)
        )
        s[f"_slope_{base}_6"] = (
            s.groupby(keys, sort=False, observed=True)[src]
    ...
    s["_early_lt"] = s["_lt_diff_past"].where(s["_life_prev"].le(5))
    s["_early_deg"] = s["_deg_diff_past"].where(s["_life_prev"].le(5))
    s["_early_lt_fill"] = s["_early_lt"].fillna(0)
    s["_early_deg_fill"] = s["_early_deg"].fillna(0)
    s["_early_lt_cnt"] = s["_early_lt"].notna().astype("int16")
    s["_early_deg_cnt"] = s["_early_deg"].notna().astype("int16")
    g = s.groupby(keys, sort=False, observed=True)

    s["stint_warmup_laptime_slope"] = g["_early_lt_fill"].cumsum() / g[
        "_early_lt_cnt"
    ].cumsum().replace(0, np.nan)
    s["stint_warmup_degradation_slope"] = g["_early_deg_fill"].cumsum() / g[
        "_early_deg_cnt"
    ].cumsum().replace(0, np.nan)
    s["stint_recent_laptime_accel"] = s["_slope_lt_3"] - s["_slope_lt_6"]
    s["stint_recent_degradation_accel"] = s["_slope_deg_3"] - s["_slope_deg_6"]
    s["stint_laptime_jump_vs_recent"] = s["_lt_diff"] - s["_slope_lt_3"]
    s["stint_degradation_jump_vs_recent"] = s["_deg_diff"] - s["_slope_deg_3"]

    cnt = g.cumcount().astype("float64")
    s["_x2"] = x * x
    s["_x_lt"] = x * lt
    s["_x_deg"] = x * deg

    sum_x = g["TyreLife"].cumsum() - x
    sum_x2 = g["_x2"].cumsum() - s["_x2"]
    den = cnt * sum_x2 - sum_x * sum_x

    for name, ycol, xycol in [
        ("laptime", "LapTime (s)", "_x_lt"),
        ("degradation", "Cumulative_Degradation", "_x_deg"),
    ]:
        y = s[ycol]
        sum_y = g[ycol].cumsum() - y
        sum_xy = g[xycol].cumsum() - s[xycol]
        slope = ((cnt * sum_xy - sum_x * sum_y) / den).where(
            (cnt >= 2) & (den.abs() > 1e-9)
        )
        intercept = ((sum_y - slope * sum_x) / cnt).where(cnt > 0)
        expected = intercept + slope * x
        s[f"stint_{name}_linear_slope_past"] = slope
        s[f"stint_{name}_linear_resid"] = y - expected

    s["_lt_resid_past"] = g["stint_laptime_linear_resid"].shift(1)
    s["_deg_resid_past"] = g["stint_degradation_linear_resid"].shift(1)
    s["_lt_resid_past3"] = (
        s.groupby(keys, sort=False, observed=True)["_lt_resid_past"]
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=levels, drop=True)
    )
    s["_deg_resid_past3"] = (
        s.groupby(keys, sort=False, observed=True)["_deg_resid_past"]
        .rolling(3, min_periods=1)
        .mean()
        .reset_index(level=levels, drop=True)
    )
    s["stint_laptime_resid_accel"] = (
        s["stint_laptime_linear_resid"] - s["_lt_resid_past3"]
    )
    s["stint_degradation_resid_accel"] = (
        s["stint_degradation_linear_resid"] - s["_deg_resid_past3"]
    )

    comp = s["Compound"].astype("string").str.upper()
    threshold = comp.map(
        {"SOFT": 16.0, "MEDIUM": 22.0, "HARD": 28.0, "INTERMEDIATE": 12.0, "WET": 10.0}
    ).fillna(22.0)
    s["stint_warmup_phase"] = x.le(5).astype("int8")
    s["stint_post_warmup_age"] = (x - 5).clip(lower=0)
    s["stint_compound_cliff_ratio"] = x / threshold
    s["stint_compound_cliff_age"] = (x - threshold).clip(lower=0)

    lt_scale = s["_std_lt_6"].abs().fillna(0) + s["_slope_lt_6"].abs().fillna(0) + 1e-3
    deg_scale = (
        s["_std_deg_6"].abs().fillna(0) + s["_slope_deg_6"].abs().fillna(0) + 1e-3
    )
    lt_cliff = (
        s["stint_laptime_linear_resid"].clip(lower=0) / lt_scale
        + s["stint_laptime_resid_accel"].clip(lower=0) / lt_scale
        + s["stint_laptime_jump_vs_recent"].clip(lower=0) / lt_scale
    )
    deg_cliff = (
        s["stint_degradation_linear_resid"].clip(lower=0) / deg_scale
        + s["stint_degradation_resid_accel"].clip(lower=0) / deg_scale
        + s["stint_degradation_jump_vs_recent"].clip(lower=0) / deg_scale
    )
    s["stint_cliff_proximity_score"] = np.sqrt(
        lt_cliff.clip(lower=0) * deg_cliff.clip(lower=0)
    )
    s["stint_cliff_inflection_flag"] = (
        (s["stint_laptime_resid_accel"] > 0)
        & (s["stint_degradation_resid_accel"] > 0)
        & (x > 5)
    ).astype("int8")

    new_cols = [
        "stint_warmup_laptime_slope",
        "stint_warmup_degradation_slope",
        "stint_recent_laptime_accel",
        "stint_recent_degradation_accel",
        "stint_laptime_jump_vs_recent",
        "stint_degradation_jump_vs_recent",
        "stint_laptime_linear_slope_past",
        "stint_laptime_linear_resid",
        "stint_degradation_linear_slope_past",
        "stint_degradation_linear_resid",
        "stint_laptime_resid_accel",
        "stint_degradation_resid_accel",
        "stint_warmup_phase",
        "stint_post_warmup_age",
        "stint_compound_cliff_ratio",
        "stint_compound_cliff_age",
        "stint_cliff_proximity_score",
        "stint_cliff_inflection_flag",
    ]

    vals = s[new_cols].replace([np.inf, -np.inf], np.nan).fillna(0)
    out.loc[s.index, new_cols] = vals.astype("float32")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000992"],
        "research_usage_note": "Verified assigned hypothesis 000992.",
    }, sort_keys=True))
