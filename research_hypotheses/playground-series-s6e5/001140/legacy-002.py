...
def add_features(train_df, test_df):
    ...
    return (
        all_df.iloc[:n_train].reset_index(drop=True),
        all_df.iloc[n_train:].reset_index(drop=True),
    )
...
def encode_for_tree(tr_df, va_df, te_df):
    ...
    if cat_cols:
        xtr_cat = enc.fit_transform(tr_df[cat_cols].astype(str)).astype("float32")
        xva_cat = enc.transform(va_df[cat_cols].astype(str)).astype("float32")
        xte_cat = enc.transform(te_df[cat_cols].astype(str)).astype("float32")
    else:
        xtr_cat = np.empty((len(tr_df), 0), dtype="float32")
        xva_cat = np.empty((len(va_df), 0), dtype="float32")
        xte_cat = np.empty((len(te_df), 0), dtype="float32")
...
def group_rank_score(raw, qkey):
    ...
    iqr = np.nanpercentile(raw, 75) - np.nanpercentile(raw, 25) + 1e-6
    z = (raw - np.nanmedian(raw)) / iqr
...
def adversarial_shift_scores(train_df, test_df):
    ...
    for tr_idx, va_idx in skf.split(all_df, labels):
        ...
        if cat_cols:
            tr_cat = enc.fit_transform(tr_part[cat_cols].astype(str)).astype("float32")
            va_cat = enc.transform(va_part[cat_cols].astype(str)).astype("float32")
        else:
            tr_cat = np.empty((len(tr_part), 0), dtype="float32")
            va_cat = np.empty((len(va_part), 0), dtype="float32")
...
def fit_auc_superlearner(
    X, y, seed=42, steps=900, batch_size=4096, lr=0.06, auc_scale=8.0, weight_decay=1e-4
):
    ...
    theta = np.zeros(X.shape[1], dtype="float64")
    ...
    for t in range(1, steps + 1):
        ...
        w = exp_theta / exp_theta.sum()

        delta = X[p].astype("float64") - X[n].astype("float64")
        margin = delta.dot(w)
        coeff = -auc_scale / (1.0 + np.exp(np.clip(auc_scale * margin, -50, 50)))
        grad_w = (delta * coeff[:, None]).mean(axis=0)
        grad_theta = w * (grad_w - np.dot(grad_w, w)) + weight_decay * theta

        m = beta1 * m + (1.0 - beta1) * grad_theta
        v = beta2 * v + (1.0 - beta2) * (grad_theta**2)
        m_hat = m / (1.0 - beta1**t)
        v_hat = v / (1.0 - beta2**t)
...
print("Training AUC-optimized non-negative Super Learner with NumPy optimizer...")
