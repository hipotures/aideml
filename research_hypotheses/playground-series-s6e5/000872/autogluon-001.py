...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound"] = out["Compound"].astype("category")
    out["Driver"] = out["Driver"].astype("category")
    out["Race"] = out["Race"].astype("category")

    out["is_wet_compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["is_soft"] = (out["Compound"] == "SOFT").astype("int8")
    out["is_medium"] = (out["Compound"] == "MEDIUM").astype("int8")
    out["is_hard"] = (out["Compound"] == "HARD").astype("int8")

    lap_time_col = "LapTime (s)"
    lap_group = out.groupby(lap_key, observed=True)

    out["lap_field_median_time"] = lap_group[lap_time_col].transform("median")
    out["lap_field_mean_time"] = lap_group[lap_time_col].transform("mean")
    out["lap_field_std_time"] = lap_group[lap_time_col].transform("std").fillna(0.0)
    out["lap_field_mean_delta"] = lap_group["LapTime_Delta"].transform("mean")
    out["lap_field_std_delta"] = lap_group["LapTime_Delta"].transform("std").fillna(0.0)
    out["lap_field_mean_poschg"] = lap_group["Position_Change"].transform("mean")
    out["lap_field_wet_share"] = lap_group["is_wet_compound"].transform("mean")
    out["lap_field_pit_share"] = (
        lap_group["PitStop"].transform("mean") if "PitStop" in out.columns else 0.0
    )

    eps = 1e-6
    out["lap_slow_ratio"] = out[lap_time_col] / (out["lap_field_median_time"] + eps)
    out["abs_delta_vs_field"] = (
        out["LapTime_Delta"] - out["lap_field_mean_delta"]
    ).abs()
    out["field_compression_proxy"] = 1.0 / (1.0 + out["lap_field_std_time"])
    out["delta_compression_proxy"] = 1.0 / (1.0 + out["lap_field_std_delta"])

    race_lap = (
        out[
            lap_key
            + [
                "lap_field_median_time",
                "lap_field_std_time",
                "lap_field_std_delta",
                "lap_field_wet_share",
                "lap_field_pit_share",
        ...
        .drop_duplicates(lap_key)
        .sort_values(lap_key)
        .reset_index(drop=True)
    )

    race_status_group = race_lap.groupby(race_key, observed=True)

    race_lap["prev_median_time"] = race_status_group["lap_field_median_time"].shift(1)
    race_lap["prev_wet_share"] = (
        race_status_group["lap_field_wet_share"].shift(1).fillna(0.0)
    )
    race_lap["median_time_jump"] = (
        (race_lap["lap_field_median_time"] / (race_lap["prev_median_time"] + eps))
        .replace([np.inf, -np.inf], 1.0)
        .fillna(1.0)
    )

    race_lap["likely_sc_or_vsc"] = (
        (race_lap["median_time_jump"] > 1.12)
        & (race_lap["lap_field_std_delta"] < race_lap["lap_field_std_delta"].median())
    ).astype("int8")

    race_lap["likely_wet_transition"] = (
        ((race_lap["lap_field_wet_share"] - race_lap["prev_wet_share"]).abs() >= 0.10)
        | ((race_lap["lap_field_wet_share"] > 0) & (race_lap["prev_wet_share"] == 0))
    ).astype("int8")

    race_lap["status_proxy"] = (
        race_lap["likely_sc_or_vsc"].astype(str)
        + "_"
        + race_lap["likely_wet_transition"].astype(str)
    )
    race_lap["status_change"] = (
        race_lap["status_proxy"]
        != race_status_group["status_proxy"].shift(1).fillna(race_lap["status_proxy"])
    ).astype("int8")

    race_lap["status_change_id"] = race_status_group["status_change"].cumsum()
    race_lap["laps_since_status_change"] = race_lap.groupby(
        race_key + ["status_change_id"], observed=True
    ).cumcount()

    prev_sc = race_status_group["likely_sc_or_vsc"].shift(1).fillna(0).astype("int8")
    race_lap["first_green_lap_after_sc"] = (
        (prev_sc == 1) & (race_lap["likely_sc_or_vsc"] == 0)
    ).astype("int8")

    red_flag_like = (
        (
            race_lap["lap_field_median_time"]
            > race_lap["lap_field_median_time"].quantile(0.995)
        )
        | (race_lap["lap_field_pit_share"] > 0.35)
    ).astype("int8")
    race_lap["red_flag_restart_window"] = (
        red_flag_like.groupby([race_lap["Year"], race_lap["Race"]], observed=True)
        .shift(1)
        .fillna(0)
        .astype("int8")
    )
    ...
    wet_shift = (
        race_status_group["likely_wet_transition"].shift(1).fillna(0).astype("int8")
    )
    wet_shift2 = (
        race_status_group["likely_wet_transition"].shift(2).fillna(0).astype("int8")
    )
    race_lap["recent_wet_message"] = (
        (race_lap["likely_wet_transition"] + wet_shift + wet_shift2) > 0
    ).astype("int8")
    ...
    out["sc_tyrelife"] = out["likely_sc_or_vsc"] * out["TyreLife"]
    out["sc_progress"] = out["likely_sc_or_vsc"] * out["RaceProgress"]
    out["wet_tyrelife"] = out["recent_wet_message"] * out["TyreLife"]
    out["wet_progress"] = out["recent_wet_message"] * out["RaceProgress"]
    out["restart_tyrelife"] = out["first_green_lap_after_sc"] * out["TyreLife"]

    out["sc_soft"] = out["likely_sc_or_vsc"] * out["is_soft"]
    out["sc_medium"] = out["likely_sc_or_vsc"] * out["is_medium"]
    out["sc_hard"] = out["likely_sc_or_vsc"] * out["is_hard"]
    out["wet_on_slick"] = out["recent_wet_message"] * (1 - out["is_wet_compound"])
    out["wet_compound_progress"] = out["is_wet_compound"] * out["RaceProgress"]

    out["degradation_per_lap"] = out["Cumulative_Degradation"] / (out["TyreLife"] + eps)
    out["stint_progress"] = out["TyreLife"] / (out["LapNumber"] + eps)
    out["position_risk_window"] = out["Position"] * (1.0 - out["RaceProgress"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000872.",
    }, sort_keys=True))
