...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Compound", "Driver", "Race"]:
        if col in out.columns:
            out[col] = out[col].astype("string").fillna("UNK")
    ...
    compound = (
        ...
        else pd.Series("UNK", index=out.index, dtype="string")
    )
    out["WetSlick"] = np.where(compound.isin(["WET", "INTERMEDIATE"]), "wet", "slick")

    if "Race" in out.columns:
        race_lower = out["Race"].str.lower()
        out["RaceFamily"] = np.where(
            race_lower.str.contains("testing", na=False), "testing", "grand_prix"
        )
    else:
        out["RaceFamily"] = "UNK"

    tyre_life = (
        out["TyreLife"].fillna(0).clip(lower=0)
        if "TyreLife" in out.columns
        else pd.Series(0.0, index=out.index)
    )
    lap_number = (
        out["LapNumber"].fillna(0).clip(lower=0)
        if "LapNumber" in out.columns
        else pd.Series(0.0, index=out.index)
    )
    progress = (
        out["RaceProgress"].fillna(0.01).clip(lower=0.01, upper=1.0)
        if "RaceProgress" in out.columns
        else pd.Series(0.01, index=out.index)
    )

    out["TyreLifeBucket"] = np.searchsorted(
        np.array([2, 5, 8, 12, 16, 22, 30, 40], dtype=np.float32),
        tyre_life.to_numpy(dtype=np.float32),
        side="right",
    ).astype("int16")

    est_total_laps = (lap_number / progress).clip(lower=lap_number, upper=110)
    laps_remaining = (est_total_laps - lap_number).clip(lower=0, upper=110)
    out["LapsRemainingEst"] = laps_remaining.astype("float32")
    out["LapsRemainingBucket"] = np.searchsorted(
        np.array([0, 2, 5, 8, 12, 18, 25, 35], dtype=np.float32),
        laps_remaining.to_numpy(dtype=np.float32),
        side="right",
    ).astype("int16")
    out["StintLapRatio"] = (tyre_life / np.maximum(lap_number, 1)).astype("float32")

    event = (
        out["PitStop"].fillna(0).clip(0, 1).astype("float32")
        if "PitStop" in out.columns
        else pd.Series(0.0, index=out.index, dtype="float32")
    )
    out["_event_proxy"] = event
    fixed_prior = 0.025

    def past_rate(keys, alpha):
        g = out.groupby(keys, sort=False, observed=True)["_event_proxy"]
        past_sum = g.cumsum() - event
        past_n = g.cumcount().astype("float32")
        return ((past_sum + fixed_prior * alpha) / (past_n + alpha)).astype("float32")

    out["HazardCoarsePast"] = past_rate(
        ["Compound", "Stint", "TyreLifeBucket", "LapsRemainingBucket", "WetSlick"], 60.0
    )
    out["HazardFamilyPast"] = past_rate(
        [
            "RaceFamily",
            "Compound",
            "Stint",
    ...
    global_n = np.arange(len(out), dtype=np.float32)
    global_past_sum = event.cumsum() - event
    out["HazardGlobalPast"] = (
        (global_past_sum + fixed_prior * 200.0) / (global_n + 200.0)
    ).astype("float32")

    hazard = (
        0.45 * out["HazardRacePast"]
        + 0.25 * out["HazardFamilyPast"]
        + 0.20 * out["HazardCoarsePast"]
        + 0.10 * out["HazardGlobalPast"]
    ).clip(0.001, 0.75)
    out["HazardBaselineEmpirical"] = hazard.astype("float32")
    out["HazardCumToNow"] = (hazard * tyre_life.clip(0, 100)).astype("float32")
    out["HazardSurvivalToNow"] = np.exp(-out["HazardCumToNow"].clip(0, 50)).astype(
        "float32"
    )

    def past_mean(keys, value_col, alpha):
        values = out[value_col].astype("float32")
        prior = float(values.mean()) if len(values) else 0.0
        values = values.fillna(prior)
        out["_mean_tmp"] = values
        g = out.groupby(keys, sort=False, observed=True)["_mean_tmp"]
        past_sum = g.cumsum() - values
        past_n = g.cumcount().astype("float32")
        return ((past_sum + prior * alpha) / (past_n + alpha)).astype("float32")

    if "LapTime (s)" in out.columns:
        out["PaceExpectedPast"] = past_mean(
            ["Race", "Compound", "TyreLifeBucket"], "LapTime (s)", 40.0
        )
        out["PaceResidualBaselineMinusObserved"] = (
            (out["PaceExpectedPast"] - out["LapTime (s)"])
            .clip(-120, 120)
            .astype("float32")
        )

    if "LapTime_Delta" in out.columns:
        out["DeltaExpectedPast"] = past_mean(
            ["RaceFamily", "Compound", "TyreLifeBucket"], "LapTime_Delta", 40.0
        )
        out["DeltaResidualBaselineMinusObserved"] = (
            (out["DeltaExpectedPast"] - out["LapTime_Delta"])
            .clip(-120, 120)
            .astype("float32")
        )

    if "Cumulative_Degradation" in out.columns:
        out["DegradationExpectedPast"] = past_mean(
            ["Compound", "TyreLifeBucket", "WetSlick"], "Cumulative_Degradation", 50.0
        )
        out["DegradationResidualObservedMinusBaseline"] = (
            (out["Cumulative_Degradation"] - out["DegradationExpectedPast"])
            .clip(-500, 500)
            .astype("float32")
        )

    out = out.drop(
        columns=[c for c in ["_event_proxy", "_mean_tmp"] if c in out.columns]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000170"],
        "research_usage_note": "Verified assigned hypothesis 000170.",
    }, sort_keys=True))
