...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Race_Year"] = df["Year"].astype(str) + "__" + df["Race"].astype(str)
    df["is_wet"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    df["is_dry"] = (1 - df["is_wet"]).astype("int8")

    tyre_bins = [-0.1, 3, 6, 9, 12, 15, 18, 22, 26, 31, 37, 45, 60, 90]
    prog_bins = np.linspace(0, 1, 11)
    degr_bins = [-1e9, -50, 0, 50, 100, 150, 250, 400, 700, 1200, 1e9]
    delta_bins = [-1e9, -3, -1, -0.3, 0.3, 1, 3, 10, 1e9]

    df["tyre_life_bin"] = pd.cut(
        df["TyreLife"], bins=tyre_bins, labels=False, include_lowest=True
    ).astype("int16")
    df["race_progress_bin"] = pd.cut(
        df["RaceProgress"], bins=prog_bins, labels=False, include_lowest=True
    ).astype("int16")
    df["degr_bin"] = pd.cut(
        df["Cumulative_Degradation"], bins=degr_bins, labels=False, include_lowest=True
    ).astype("int16")
    df["delta_bin"] = pd.cut(
        df["LapTime_Delta"], bins=delta_bins, labels=False, include_lowest=True
    ).astype("int16")

    stint_max_life = df.groupby(["Race_Year", "Driver", "Stint"], observed=True)[
        "TyreLife"
    ].transform("max")
    df["stint_progress"] = (
        (df["TyreLife"] / stint_max_life.replace(0, np.nan)).fillna(0.0).clip(0, 1.5)
    )
    ...
    df["position_pressure"] = df["Position"] / (df["LapNumber"] + 1.0)

    global_rate = float(df["PitStop"].mean())
    alpha = 20.0

    def add_smoothed_rate(cols, name):
        stats = (
            df.groupby(cols, observed=True)["PitStop"]
            .agg(["sum", "count"])
            .reset_index()
        )
        stats[name] = (stats["sum"] + alpha * global_rate) / (stats["count"] + alpha)
        stats[name + "_n"] = stats["count"].astype("float32")
        stats = stats[cols + [name, name + "_n"]]
        out = df[cols].merge(stats, on=cols, how="left", sort=False)
        p = out[name].fillna(global_rate).clip(1e-4, 1 - 1e-4).astype("float32")
        df[name] = p
        df[name + "_logit"] = np.log(p / (1.0 - p)).astype("float32")
        df[name + "_n"] = out[name + "_n"].fillna(0).astype("float32")
    ...
    add_smoothed_rate(
        ["Compound", "tyre_life_bin", "Stint", "race_progress_bin", "is_wet"],
        "hazard_backoff_1",
    )
    add_smoothed_rate(
        ["Compound", "tyre_life_bin", "Stint", "is_wet"],
        "hazard_backoff_2",
    )
    add_smoothed_rate(
        ["Compound", "tyre_life_bin", "race_progress_bin", "is_wet"],
        "hazard_backoff_3",
    )
    add_smoothed_rate(
        ["is_wet", "tyre_life_bin", "Stint"],
        "hazard_backoff_4",
    )

    df["hazard_blend"] = (
        0.45 * df["hazard_full"]
        + 0.25 * df["hazard_backoff_1"]
        + 0.15 * df["hazard_backoff_2"]
        + 0.10 * df["hazard_backoff_3"]
        + 0.05 * df["hazard_backoff_4"]
    ).astype("float32")
    p = df["hazard_blend"].clip(1e-4, 1 - 1e-4)
    df["hazard_blend_logit"] = np.log(p / (1.0 - p)).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000118.",
    }, sort_keys=True))
