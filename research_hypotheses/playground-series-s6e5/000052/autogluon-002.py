...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_wetweather = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    df["Is_Slick"] = is_slick
    df["Is_WetWeather"] = is_wetweather

    race_progress = df["RaceProgress"].clip(lower=1e-6)
    est_total_laps = (df["LapNumber"] / race_progress).clip(
        lower=df["LapNumber"], upper=90
    )
    laps_remaining_after_next = (est_total_laps - df["LapNumber"] - 1.0).clip(lower=0)
    df["Estimated_Total_Laps"] = est_total_laps
    df["Laps_Remaining_After_NextLapStop"] = laps_remaining_after_next
    ...
    for tyre, life in fresh_life.items():
        margin = life - laps_remaining_after_next
        can_finish = (margin >= 0).astype("int8")
        df[f"NextStop_{tyre}_Margin"] = margin
        df[f"NextStop_{tyre}_CanFinish"] = can_finish

    slick_margins = pd.DataFrame(
        {
            "SOFT": df["NextStop_SOFT_Margin"],
            "MEDIUM": df["NextStop_MEDIUM_Margin"],
            "HARD": df["NextStop_HARD_Margin"],
        },
        index=df.index,
    )
    wet_margins = pd.DataFrame(
        {
            "INTERMEDIATE": df["NextStop_INTERMEDIATE_Margin"],
            "WET": df["NextStop_WET_Margin"],
        },
        index=df.index,
    )

    best_slick_margin = slick_margins.max(axis=1)
    best_wet_margin = wet_margins.max(axis=1)
    df["Best_Slick_NextStop_Margin"] = best_slick_margin
    df["Best_WetWeather_NextStop_Margin"] = best_wet_margin
    df["Best_Slick_NextStop_CanFinish"] = (best_slick_margin >= 0).astype("int8")
    df["Best_WetWeather_NextStop_CanFinish"] = (best_wet_margin >= 0).astype("int8")

    df["ClassAware_NextStop_Margin"] = np.where(
        is_slick == 1, best_slick_margin, best_wet_margin
    )
    df["ClassAware_NextStop_CanFinish"] = (
        df["ClassAware_NextStop_Margin"] >= 0
    ).astype("int8")

    df["Switch_To_Slick_NextStop"] = is_wetweather
    df["Switch_To_WetWeather_NextStop"] = is_slick
    df["Stay_In_Slick_Class_NextStop"] = is_slick
    df["Stay_In_WetWeather_Class_NextStop"] = is_wetweather

    df["Switch_To_Slick_Feasible"] = (is_wetweather & (best_slick_margin >= 0)).astype(
        "int8"
    )
    df["Switch_To_WetWeather_Feasible"] = (is_slick & (best_wet_margin >= 0)).astype(
        "int8"
    )
    df["Switch_To_Slick_Margin"] = np.where(is_wetweather == 1, best_slick_margin, 0.0)
    df["Switch_To_WetWeather_Margin"] = np.where(is_slick == 1, best_wet_margin, 0.0)

    df["CurrentTyreLifeFrac"] = df["TyreLife"] / est_total_laps.replace(0, np.nan)
    df["StintFracOfRace"] = df["Stint"] / est_total_laps.replace(0, np.nan)
    df["Degradation_Per_TyreLap"] = df["Cumulative_Degradation"] / df[
        "TyreLife"
    ].replace(0, np.nan)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000052.",
    }, sort_keys=True))
