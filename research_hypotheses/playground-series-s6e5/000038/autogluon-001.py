...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    field_size = g["LapNumber"].transform("size").astype("float32")
    out["same_lap_field_size"] = field_size

    for col in value_cols:
        s = out[col].astype("float32")
        median = g[col].transform("median").astype("float32")
        ...
        out[f"{safe}_same_lap_median"] = median
        out[f"{safe}_minus_same_lap_median"] = (s - median).astype("float32")
        out[f"{safe}_same_lap_z"] = ((s - mean) / std).fillna(0).astype("float32")
        out[f"{safe}_same_lap_rank"] = (
            g[col].rank(method="average", ascending=True).astype("float32")
        )
        out[f"{safe}_same_lap_pct_rank"] = (
            out[f"{safe}_same_lap_rank"] / field_size
        ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000038"],
        "research_usage_note": "Verified assigned hypothesis 000038.",
    }, sort_keys=True))
