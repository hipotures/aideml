...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_life = compound.map({
        "SOFT": 16.0,
        "MEDIUM": 24.0,
        "HARD": 32.0,
        "INTERMEDIATE": 18.0,
        "WET": 15.0,
    }).fillna(24.0).astype("float32")
    ...
    age_ratio = (tyre / base_life.clip(lower=1.0)).astype("float32")
    out["stint_survival_age_ratio"] = age_ratio
    out["stint_survival_laps_left"] = laps_left.astype("float32")
    out["stint_survival_expected_remaining"] = (base_life - tyre).clip(lower=0.0).astype("float32")
    hazard = 1.0 / (1.0 + np.exp(-(age_ratio.astype(float) - 0.85) * 5.0))
    out["stint_survival_hazard_proxy"] = hazard.astype("float32")
    out["stint_survival_survival_proxy"] = np.exp(-np.maximum(age_ratio.astype(float), 0.0) ** 2).astype("float32")
    out["stint_survival_finish_margin"] = (out["stint_survival_expected_remaining"] - laps_left).astype("float32")
    out["stint_survival_compound_phase"] = (
        compound.astype("string") + "_" + pd.cut(age_ratio, bins=[-0.01, 0.5, 0.9, 1.2, 10], labels=["young", "mid", "old", "over"] ).astype("string")
    ).astype("category")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000242"],
        "research_usage_note": "Verified assigned hypothesis 000242.",
    }, sort_keys=True))
