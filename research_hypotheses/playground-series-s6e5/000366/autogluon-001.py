...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out[idx_name] = np.arange(len(out))

    sort_cols = [
        c for c in ["Year", "Race", "Driver", "Stint", "LapNumber"] if c in out.columns
    ]
    s = out.sort_values(sort_cols + [idx_name], kind="mergesort").copy()

    gcols = [c for c in ["Year", "Race", "Driver", "Stint"] if c in s.columns]
    g = s.groupby(gcols, sort=False, observed=True)

    ltd = s["LapTime_Delta"].astype(float)
    degr = s["Cumulative_Degradation"].astype(float)
    lap_time = s["LapTime (s)"].astype(float)

    prev_pit = g["PitStop"].shift(1).fillna(0).astype(float)
    pit_mask = (s["PitStop"].astype(float) > 0) | (prev_pit > 0)
    clean_lap_time = lap_time.mask(pit_mask)

    for w in (2, 3, 4, 5):
        lag = g["LapTime_Delta"].shift(w - 1).astype(float)
        s[f"ltd_slope_{w}"] = (ltd - lag) / (w - 1)
        s[f"ltd_curv_{w}"] = s[f"ltd_slope_{w}"] - g[f"ltd_slope_{w}"].shift(1).astype(
            float
        )

    acc = ltd - g["LapTime_Delta"].shift(1).astype(float)
    s["ltd_accel"] = acc
    s["ltd_ewm_accel"] = acc.groupby(
        [s[c] for c in gcols], sort=False, observed=True
    ).transform(lambda x: x.ewm(alpha=0.5, adjust=False, min_periods=1).mean())

    pos_jump = acc.clip(lower=0)
    for w in (3, 5):
        s[f"recent_max_pos_jump_{w}"] = pos_jump.groupby(
            [s[c] for c in gcols], sort=False, observed=True
        ).transform(lambda x: x.rolling(w, min_periods=1).max())

    for w in (3, 5):
        lagd = g["Cumulative_Degradation"].shift(w - 1).astype(float)
        s[f"degradation_slope_{w}"] = (degr - lagd) / (w - 1)

    tyre_life = s["TyreLife"].astype(float)
    s["early_tyre_life"] = (tyre_life <= 3).astype(np.int8)
    s["settling_ltd_x_early"] = ltd * s["early_tyre_life"]
    s["settling_accel_x_early"] = acc * s["early_tyre_life"]

    race_lap_cols = [c for c in ["Year", "Race", "LapNumber"] if c in s.columns]
    race_lap_median = clean_lap_time.groupby(
        [s[c] for c in race_lap_cols], sort=False, observed=True
    ).transform("median")
    s["race_lap_pace_dev_clean"] = lap_time - race_lap_median
    s["race_lap_pace_dev_clean_abs"] = s["race_lap_pace_dev_clean"].abs()

    s = s.sort_values(idx_name, kind="mergesort").drop(columns=[idx_name])
    return s
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000366"],
        "research_usage_note": "Verified assigned hypothesis 000366.",
    }, sort_keys=True))
