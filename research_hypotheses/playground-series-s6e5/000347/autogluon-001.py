...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Driver", "Race", "Compound"]:
        out[c] = out[c].astype("string").str.upper().str.strip().fillna("UNK")

    group_cols = ["Year", "Race", "Driver"]
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "Stint"]
    s = out.assign(__idx=idx).sort_values(sort_cols, kind="mergesort")
    g = s.groupby(group_cols, sort=False)

    s["drv_race_seq"] = g.cumcount()
    s["drv_race_n"] = g["LapNumber"].transform("size")
    s["drv_race_seq_frac"] = s["drv_race_seq"] / (s["drv_race_n"] - 1).replace(
        0, np.nan
    )
    ...
    for c in [
        ...
        s[f"{c}_grp_rank"] = g[c].rank(method="average", pct=True)
        s[f"{c}_prev"] = g[c].shift(1)
        s[f"{c}_diff_prev"] = s[c] - s[f"{c}_prev"]

    s["pit_prev"] = g["PitStop"].shift(1).fillna(0)
    s["pit_2ago"] = g["PitStop"].shift(2).fillna(0)
    s["pit_recent2"] = s["pit_prev"] + s["pit_2ago"]
    s["laps_since_group_start"] = s["drv_race_seq"].astype(float)

    s = s.sort_values("__idx", kind="mergesort")
    add_cols = [c for c in s.columns if c not in out.columns and c != "__idx"]
    out[add_cols] = s[add_cols].to_numpy()

    out["compound_code"] = (
        out["Compound"]
        .map({"SOFT": 3, "MEDIUM": 2, "HARD": 1, "INTERMEDIATE": 4, "WET": 5})
        .fillna(0)
        .astype("int8")
    )
    out["is_wet_compound"] = (
    ...
    out["laps_remaining_est"] = (
        out["LapNumber"] / out["RaceProgress"].clip(0.01, 1.0) - out["LapNumber"]
    ).clip(0, 120)
    ...
    out["position_pressure"] = out["Position"] + out["Position_Change"].abs()
    out["late_stint_flag"] = (out["TyreLife"] >= 18).astype("int8")
    out["very_late_stint_flag"] = (out["TyreLife"] >= 28).astype("int8")
    out["pit_window_score"] = (
        out["TyreLife"].clip(0, 40) / 40
        + out["RaceProgress"].clip(0, 1)
        + out["Cumulative_Degradation"].clip(-50, 500) / 500
        + out["LapTime_Delta"].clip(-20, 80) / 80
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000347"],
        "research_usage_note": "Verified assigned hypothesis 000347.",
    }, sort_keys=True))
