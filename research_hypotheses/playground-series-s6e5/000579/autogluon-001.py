...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dry_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2}

    out["CompoundCode"] = out["Compound"].map(compound_map).fillna(-1).astype("int16")
    out["DryCompoundCode"] = out["Compound"].map(dry_map).fillna(-1).astype("int16")
    ...
    out["Progress_x_Stint"] = out["RaceProgress"] * out["Stint"]
    out["Lap_x_Progress"] = out["LapNumber"] * out["RaceProgress"]

    out["LateRace"] = (out["RaceProgress"] >= 0.70).astype("int8")
    out["VeryLateRace"] = (out["RaceProgress"] >= 0.85).astype("int8")
    out["EarlyStint"] = (out["TyreLife"] <= 3).astype("int8")
    out["OldTyre"] = (out["TyreLife"] >= 18).astype("int8")
    out["VeryOldTyre"] = (out["TyreLife"] >= 28).astype("int8")
    out["LikelyFinalStopWindow"] = (
        (out["RaceProgress"] >= 0.55)
        & (out["RaceProgress"] <= 0.90)
        & (out["TyreLife"] >= 12)
    ).astype("int8")
    out["PitWithin2Proxy"] = (
        (out["TyreLife"] >= 18)
        | ((out["LapTime_Delta"] > 2.5) & (out["TyreLife"] >= 8))
    ).astype("int8")

    out["DegPerTyreLap"] = out["Cumulative_Degradation"] / (out["TyreLife"] + 1.0)
    out["LapTimePerTyreLap"] = out["LapTime (s)"] / (out["TyreLife"] + 1.0)
    out["AbsLapDelta"] = out["LapTime_Delta"].abs()
    out["AbsPositionChange"] = out["Position_Change"].abs()
    out["PositionRisk"] = (
        (21 - out["Position"]) * (out["TyreLife"] + 1.0) * out["RaceProgress"]
    )

    grp = ["Year", "Race", "LapNumber"]
    out["LapMeanTyreLife"] = out.groupby(grp)["TyreLife"].transform("mean")
    out["LapMaxTyreLife"] = out.groupby(grp)["TyreLife"].transform("max")
    out["LapMeanPosition"] = out.groupby(grp)["Position"].transform("mean")
    out["LapPitRate"] = out.groupby(grp)["PitStop"].transform("mean")
    out["TyreLifeVsLapMean"] = out["TyreLife"] - out["LapMeanTyreLife"]
    out["TyreLifeVsLapMax"] = out["TyreLife"] - out["LapMaxTyreLife"]
    out["PositionVsLapMean"] = out["Position"] - out["LapMeanPosition"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000579"],
        "research_usage_note": "Verified assigned hypothesis 000579.",
    }, sort_keys=True))
