...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_key = ["Year", "Race"]
    race_driver_key = ["Year", "Race", "Driver"]
    ...
    X["compound_warmup_penalty"] = X["Compound"].map(warmup_penalty).astype("float32")
    X["is_slick"] = X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    X["is_wet_regime"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    race_deg = X.groupby(race_key)["Cumulative_Degradation"].transform("median")
    race_delta_vol = X.groupby(race_key)["LapTime_Delta"].transform(
        lambda s: s.abs().median()
    )
    race_pos_churn = X.groupby(race_key)["Position_Change"].transform(
        lambda s: s.abs().median()
    )
    race_pace = X.groupby(race_key)["LapTime (s)"].transform("median")

    X["race_low_stress_prior"] = (
        1.0
        / (1.0 + race_deg.abs() / (X["LapNumber"].clip(lower=1) + 1.0))
        / (1.0 + race_delta_vol.abs())
    ).astype("float32")

    X["track_position_value"] = (
        (21.0 - X["Position"]).clip(lower=1) / 20.0 / (1.0 + race_pos_churn.abs())
    ).astype("float32")

    X["clean_air_proxy"] = (
        ((21.0 - X["Position"]).clip(lower=1) / 20.0)
        * (1.0 / (1.0 + X["Position_Change"].abs()))
    ).astype("float32")

    X["tyre_age_pressure"] = (
        X["TyreLife"] / X.groupby(race_key)["TyreLife"].transform("max").clip(lower=1)
    ).astype("float32")

    X["stable_old_tyre"] = (
        X["tyre_age_pressure"]
        / (1.0 + X["LapTime_Delta"].abs())
        / (1.0 + (X["Cumulative_Degradation"].clip(lower=0) / 100.0))
    ).astype("float32")

    X["stint_progress"] = (
        X["TyreLife"]
        / X.groupby(race_driver_key)["TyreLife"].transform("max").clip(lower=1)
    ).astype("float32")

    X["warmup_risk_fresh_tyre"] = (
        X["compound_warmup_penalty"] * (1.0 - X["stint_progress"])
    ).astype("float32")

    X["overcut_regime_score"] = (
        X["race_low_stress_prior"]
        * X["track_position_value"]
        * X["compound_warmup_penalty"]
        * X["clean_air_proxy"]
        * X["stable_old_tyre"]
    ).astype("float32")

    X["overcut_vs_pit_pressure"] = (
        X["overcut_regime_score"]
        - X["tyre_age_pressure"] * (1.0 + X["LapTime_Delta"].clip(lower=0))
    ).astype("float32")

    X["overcut_compound_interaction"] = (
        X["overcut_regime_score"] * X["compound_warmup_penalty"]
    ).astype("float32")

    X["overcut_position_interaction"] = (
        X["overcut_regime_score"] * ((21.0 - X["Position"]).clip(lower=1) / 20.0)
    ).astype("float32")

    X["pace_stability_ratio"] = (
        X["LapTime (s)"] / race_pace.clip(lower=1) / (1.0 + X["LapTime_Delta"].abs())
    ).astype("float32")

    X["old_but_holding_on"] = (
        X["tyre_age_pressure"] * X["pace_stability_ratio"] * X["race_low_stress_prior"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000825"],
        "research_usage_note": "Verified assigned hypothesis 000825.",
    }, sort_keys=True))
