...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["RankGroup"] = (
        ...
        + "_"
        + out["Race_Year"].astype(str)
        + "_"
        + out["Stint"].astype(str)
    )

    group = out.groupby("RankGroup", sort=False)
    out["group_size"] = group["LapNumber"].transform("size").astype("int16")
    denom = out["group_size"].clip(lower=2) - 1
    ...
    for col in [
        ...
        r = group[col].rank(method="average", pct=False)
        out[f"{col}_grp_rank_pct"] = ((r - 1) / denom).astype("float32")
        out[f"{col}_grp_centered"] = (out[col] - group[col].transform("mean")).astype(
            "float32"
        )

    out["laps_left_in_stint_rank"] = (1.0 - out["LapNumber_grp_rank_pct"]).astype(
        "float32"
    )
    out["tyre_vs_stint_progress"] = (
        out["TyreLife_grp_rank_pct"] - out["LapNumber_grp_rank_pct"]
    ).astype("float32")
    out["degradation_per_tyre_life"] = (
        (out["Cumulative_Degradation"] / out["TyreLife"].replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .astype("float32")
    )
    out["late_old_tyre"] = (out["RaceProgress"] * out["TyreLife"]).astype("float32")
    out["stint_progress_x_degradation_rank"] = (
        out["LapNumber_grp_rank_pct"] * out["Cumulative_Degradation_grp_rank_pct"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000167"],
        "research_usage_note": "Verified assigned hypothesis 000167.",
    }, sort_keys=True))
