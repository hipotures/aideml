...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry = comp.isin(dry_compounds)
    is_testing = race.eq("Pre-Season Testing")

    out["_orig_order_000938"] = np.arange(len(out))
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "_orig_order_000938"]
    ...
    for c in ["SOFT", "MEDIUM", "HARD"]:
        seen_current = (
            ...
            .str.upper()
            .eq(c)
            .groupby([s[x] for x in gcols], sort=False)
            .cummax()
        )
        seen_before = seen_current.groupby([s[x] for x in gcols], sort=False).shift(
            fill_value=False
        )
        s[f"seen_{c.lower()}_before"] = seen_before.astype("int8")
        s[f"seen_{c.lower()}_sofar"] = seen_current.astype("int8")

    dry_seen_before = s[
        ["seen_soft_before", "seen_medium_before", "seen_hard_before"]
    ].sum(axis=1)
    dry_seen_sofar = s[["seen_soft_sofar", "seen_medium_sofar", "seen_hard_sofar"]].sum(
        axis=1
    )

    current_seen_before = np.select(
        [
            s["Compound"].astype(str).str.upper().eq("SOFT"),
            s["Compound"].astype(str).str.upper().eq("MEDIUM"),
            s["Compound"].astype(str).str.upper().eq("HARD"),
        ],
        [s["seen_soft_before"], s["seen_medium_before"], s["seen_hard_before"]],
        default=0,
    )

    lap_max = s.groupby([s[x] for x in ["Year", "Race"]], sort=False)[
        "LapNumber"
    ].transform("max")
    laps_remaining = (lap_max - s["LapNumber"]).clip(lower=0)

    s["dry_compounds_used_before"] = dry_seen_before.astype("int8")
    s["dry_compounds_used_sofar"] = dry_seen_sofar.astype("int8")
    s["current_dry_compound_seen_before"] = current_seen_before.astype("int8")
    s["has_used_two_dry_compounds"] = (dry_seen_sofar >= 2).astype("int8")
    s["is_dry_compound"] = (
        s["Compound"].astype(str).str.upper().isin(dry_compounds).astype("int8")
    )
    s["laps_remaining_est"] = laps_remaining.astype("float32")
    s["late_race_must_switch_proxy"] = (
        s["is_dry_compound"].eq(1)
        & ~s["Race"].astype(str).eq("Pre-Season Testing")
        & dry_seen_sofar.eq(1)
        & laps_remaining.le(15)
    ).astype("int8")
    s["must_switch_x_laps_remaining"] = (
        s["late_race_must_switch_proxy"] * s["laps_remaining_est"]
    ).astype("float32")
    s["must_switch_x_tyre_life"] = (
        s["late_race_must_switch_proxy"] * s["TyreLife"].astype("float32")
    ).astype("float32")
    s["dry_rule_gap_remaining"] = (
        (2 - dry_seen_sofar).clip(lower=0)
        * s["is_dry_compound"]
        * (~is_testing.loc[s.index]).astype("int8")
    ).astype("int8")

    out = s.sort_values("_orig_order_000938", kind="mergesort").drop(
        columns=["_orig_order_000938"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000938"],
        "research_usage_note": "Verified assigned hypothesis 000938.",
    }, sort_keys=True))
