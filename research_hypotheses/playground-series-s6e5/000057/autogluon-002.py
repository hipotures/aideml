...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    global_expected = comp.map(compound_life).fillna(24.0).astype(float)
    out["compound_expected_life"] = global_expected

    keys = ["Year", "Race", "Compound", "LapNumber"]
    lap_peer = (
        out.groupby(keys, sort=False)["TyreLife"]
        .agg(["mean", "count"])
        .reset_index()
        .sort_values(["Year", "Race", "Compound", "LapNumber"], kind="mergesort")
    )

    grp = lap_peer.groupby(["Year", "Race", "Compound"], sort=False)
    lap_peer["race_comp_prior_age_sum"] = grp["mean"].transform(
        lambda s: (s * lap_peer.loc[s.index, "count"]).cumsum().shift(1)
    )
    lap_peer["race_comp_prior_age_n"] = grp["count"].transform(
        lambda s: s.cumsum().shift(1)
    )
    lap_peer["race_comp_current_age_mean"] = lap_peer["mean"]
    lap_peer["race_comp_current_age_n"] = lap_peer["count"]

    out = out.merge(
        lap_peer[
            keys
            + [
                "race_comp_prior_age_sum",
                "race_comp_prior_age_n",
                "race_comp_current_age_mean",
                "race_comp_current_age_n",
            ]
        ],
        on=keys,
        how="left",
    ...
    prior_n = out["race_comp_prior_age_n"].fillna(0.0).astype(float)
    prior_mean = out["race_comp_prior_age_sum"] / prior_n.replace(0.0, np.nan)
    out["race_comp_prior_age_missing"] = (prior_n <= 0).astype("int8")

    smooth_k = 12.0
    global_expected = out["compound_expected_life"].astype(float)
    out["race_comp_expected_life"] = (
        prior_mean.fillna(global_expected) * prior_n + global_expected * smooth_k
    ) / (prior_n + smooth_k)

    current_n = out["race_comp_current_age_n"].fillna(0.0).astype(float)
    current_mean = (
        out["race_comp_current_age_mean"].fillna(out["TyreLife"]).astype(float)
    )
    out["race_comp_current_age_vs_expected"] = (
        current_mean - out["race_comp_expected_life"]
    )

    out["tyrelife_margin_race_adj"] = out["race_comp_expected_life"] - out["TyreLife"]
    out["tyrelife_ratio_race_adj"] = out["TyreLife"] / out[
        "race_comp_expected_life"
    ].replace(0.0, np.nan)
    out["tyrelife_over_race_expected"] = (out["tyrelife_margin_race_adj"] < 0).astype(
        "int8"
    )
    out["tyrelife_near_race_expected"] = (
        out["tyrelife_margin_race_adj"].between(0, 3)
    ).astype("int8")
    out["old_for_race_compound"] = (
        out["TyreLife"] >= out["race_comp_expected_life"] * 0.9
    ).astype("int8")

    out["race_comp_context_weight"] = prior_n / (prior_n + smooth_k)
    out["race_comp_peer_count_log"] = np.log1p(current_n)
    out["progress_x_margin_race_adj"] = (
        out["RaceProgress"] * out["tyrelife_margin_race_adj"]
    )
    out["stint_x_ratio_race_adj"] = out["Stint"] * out["tyrelife_ratio_race_adj"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000057"],
        "research_usage_note": "Verified assigned hypothesis 000057.",
    }, sort_keys=True))
