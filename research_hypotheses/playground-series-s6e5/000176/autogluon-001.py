...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["CompoundCode"] = df["Compound"].map(compound_map).astype("float32")

    lap_group = df.groupby(lap_keys, sort=False)
    race_group = df.groupby(race_keys, sort=False)

    df["LapTime_med_lap"] = (
        lap_group["LapTime (s)"].transform("median").astype("float32")
    )
    df["LapTime_std_lap"] = (
        lap_group["LapTime (s)"].transform("std").fillna(0).astype("float32")
    )
    df["LapDelta_med_lap"] = (
        lap_group["LapTime_Delta"].transform("median").astype("float32")
    )
    df["LapDelta_std_lap"] = (
        lap_group["LapTime_Delta"].transform("std").fillna(0).astype("float32")
    )
    df["TyreLife_med_lap"] = lap_group["TyreLife"].transform("median").astype("float32")
    df["TyreLife_rank_pct"] = (
        lap_group["TyreLife"].rank(method="average", pct=True).astype("float32")
    )

    df["pace_vs_lap_median"] = (df["LapTime (s)"] - df["LapTime_med_lap"]).astype(
        "float32"
    )
    df["delta_vs_lap_median"] = (df["LapTime_Delta"] - df["LapDelta_med_lap"]).astype(
        "float32"
    )
    df["pace_z_lap"] = (
        df["pace_vs_lap_median"] / (df["LapTime_std_lap"] + 1e-3)
    ).astype("float32")
    df["delta_z_lap"] = (
        df["delta_vs_lap_median"] / (df["LapDelta_std_lap"] + 1e-3)
    ).astype("float32")
    df["degradation_vs_lap_median"] = (
        df["Cumulative_Degradation"]
        - lap_group["Cumulative_Degradation"].transform("median")
    ).astype("float32")

    older_frac = lap_group["TyreLife"].transform(
        lambda s: s.lt(df.loc[s.index, "TyreLife"]).mean()
    )
    newer_frac = lap_group["TyreLife"].transform(
        lambda s: s.gt(df.loc[s.index, "TyreLife"]).mean()
    )
    df["peer_older_tyre_frac"] = older_frac.astype("float32")
    df["peer_newer_tyre_frac"] = newer_frac.astype("float32")

    lap_pit_rate = lap_group["PitStop"].transform("mean").astype("float32")
    lap_pit_rate_tbl = (
        df[lap_keys + ["PitStop"]]
        .groupby(lap_keys, sort=False)["PitStop"]
        .mean()
        .rename("lap_pit_rate")
        .reset_index()
        .sort_values(lap_keys)
    )
    lap_pit_rate_tbl["pit_wave_prev1"] = lap_pit_rate_tbl.groupby(
        race_keys, sort=False
    )["lap_pit_rate"].shift(1)
    lap_pit_rate_tbl["pit_wave_prev2"] = lap_pit_rate_tbl.groupby(
        race_keys, sort=False
    )["lap_pit_rate"].shift(2)
    lap_pit_rate_tbl["pit_wave_prev12"] = (
        0.7 * lap_pit_rate_tbl["pit_wave_prev1"].fillna(0)
        + 0.3 * lap_pit_rate_tbl["pit_wave_prev2"].fillna(0)
    ).astype("float32")
    df = df.merge(
        lap_pit_rate_tbl[
            lap_keys + ["pit_wave_prev1", "pit_wave_prev2", "pit_wave_prev12"]
        ],
        on=lap_keys,
        how="left",
        sort=False,
    )
    df["pit_wave_prev1"] = df["pit_wave_prev1"].fillna(0).astype("float32")
    df["pit_wave_prev2"] = df["pit_wave_prev2"].fillna(0).astype("float32")
    df["pit_wave_prev12"] = df["pit_wave_prev12"].fillna(0).astype("float32")
    df["lap_pit_rate_now"] = lap_pit_rate

    field_compression = (1.0 / (df["LapTime_std_lap"] + 1.0)).astype("float32")
    incident_proxy = (
        df["LapDelta_std_lap"]
        > race_group["LapTime_Delta"].transform("std").fillna(0).median()
    ).astype("float32")
    df["cheap_stop_score"] = (
        field_compression
        * (1.0 + df["pit_wave_prev12"])
        * (1.0 + df["peer_older_tyre_frac"])
        * (1.0 + df["RaceProgress"])
    ).astype("float32")
    df["undercut_opportunity"] = (
        df["pace_z_lap"] * df["peer_older_tyre_frac"] * (21 - df["Position"]) / 20.0
    ).astype("float32")
    df["traffic_risk"] = (
        np.abs(df["Position_Change"]).astype("float32")
        * df["peer_newer_tyre_frac"]
        * (df["Position"] / 20.0)
    ).astype("float32")
    df["incident_proxy"] = incident_proxy
    df["cheap_stop_x_position"] = (df["cheap_stop_score"] * df["Position"]).astype(
        "float32"
    )
    df["cheap_stop_x_compound"] = (
        df["cheap_stop_score"] * (df["CompoundCode"] + 1)
    ).astype("float32")
    df["undercut_x_compound"] = (
        df["undercut_opportunity"] * (df["CompoundCode"] + 1)
    ).astype("float32")
    df["pit_wave_x_position"] = (df["pit_wave_prev12"] * df["Position"]).astype(
        "float32"
    )
    df["tyrelife_x_peerolder"] = (df["TyreLife"] * df["peer_older_tyre_frac"]).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000176"],
        "research_usage_note": "Verified assigned hypothesis 000176.",
    }, sort_keys=True))
