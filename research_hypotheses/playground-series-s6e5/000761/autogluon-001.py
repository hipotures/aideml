...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dry = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    wet = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    soft = (compound == "SOFT").astype("int8")
    medium = (compound == "MEDIUM").astype("int8")
    hard = (compound == "HARD").astype("int8")

    lap = out["LapNumber"].astype(float)
    progress = out["RaceProgress"].astype(float).clip(0.001, 1.0)
    tyre = out["TyreLife"].astype(float)
    stint = out["Stint"].astype(float)
    pos = out["Position"].astype(float)
    pos_chg = out["Position_Change"].astype(float)
    lap_time = out["LapTime (s)"].astype(float)
    lap_delta = out["LapTime_Delta"].astype(float)
    deg = out["Cumulative_Degradation"].astype(float)
    pit_prev = out["PitStop"].astype(float)
    race_laps_est = (lap / progress).replace([np.inf, -np.inf], np.nan)

    race_mean_laps = race_laps_est.groupby(race).transform("median")
    laps_left = (race_mean_laps - lap).fillna((1.0 - progress) * 60).clip(lower=0)
    service_overage = tyre / (laps_left + 1.0)
    stint_progress = tyre / (tyre + laps_left + 1.0)

    lap_med = lap_time.groupby(race).transform("median")
    delta_med = lap_delta.groupby(race).transform("median")
    deg_med = deg.groupby(race).transform("median")
    pos_chg_mean = pos_chg.groupby(race).transform("mean")

    rel_laptime = lap_time - lap_med
    rel_delta = lap_delta - delta_med
    rel_deg = deg - deg_med
    field_pressure = (-pos_chg + pos_chg_mean).clip(lower=0) + (21.0 - pos) / 20.0

    comp_freq = compound.map(compound.value_counts(normalize=True)).astype(float)
    race_freq = race.map(race.value_counts(normalize=True)).astype(float)
    driver_freq = driver.map(driver.value_counts(normalize=True)).astype(float)

    cannot_finish = ((laps_left > 0) & (tyre > laps_left * 1.15) & (stint <= 3)).astype(
        "int8"
    )
    degradation_expert = (
        0.35 * np.log1p(tyre.clip(lower=0))
        + 0.25 * np.log1p(np.maximum(deg, 0))
        + 0.20 * service_overage.clip(0, 8)
        + 0.20 * np.maximum(rel_laptime, 0) / 10.0
    )

    legality_weather = (
        ((stint == 1) & (dry == 1) & (progress > 0.45)).astype(float)
        + wet.astype(float) * (progress > 0.10).astype(float)
        + ((dry == 1) & (laps_left < 18) & (stint == 1)).astype(float)
    )

    feasible_tyre = ((dry == 1) & (tyre >= 6) & (laps_left >= 8)).astype(float)
    competitive_expert = feasible_tyre * (
        0.45 * field_pressure
        + 0.25 * np.maximum(rel_delta, 0) / 5.0
        + 0.20 * (pos <= 10).astype(float)
        + 0.10 * (pit_prev == 0).astype(float)
    )

    gate_deg = (
        cannot_finish.astype(float)
        + service_overage.clip(0, 5) / 5.0
        + (tyre >= 18).astype(float)
    ) / 3.0
    gate_legality = (
        legality_weather.clip(0, 3) / 3.0
        + wet.astype(float)
        + ((stint == 1) & (progress > 0.65)).astype(float)
    ) / 3.0
    gate_comp = (
        field_pressure.clip(0, 5) / 5.0 + feasible_tyre + (pos <= 12).astype(float)
    ) / 3.0
    gate_sum = gate_deg + gate_legality + gate_comp + 1e-6

    out["compound_dry"] = dry
    out["compound_wet"] = wet
    out["compound_soft"] = soft
    out["compound_medium"] = medium
    out["compound_hard"] = hard
    out["laps_left_est"] = laps_left.astype("float32")
    out["race_laps_est"] = race_mean_laps.astype("float32")
    out["service_overage"] = service_overage.astype("float32")
    out["stint_progress"] = stint_progress.astype("float32")
    out["rel_laptime_race"] = rel_laptime.astype("float32")
    out["rel_delta_race"] = rel_delta.astype("float32")
    out["rel_degradation_race"] = rel_deg.astype("float32")
    out["field_pressure"] = field_pressure.astype("float32")
    out["cannot_finish_gate"] = cannot_finish
    out["legality_weather_gate"] = legality_weather.astype("float32")
    out["competitive_gate_raw"] = competitive_expert.astype("float32")
    out["expert_degradation"] = degradation_expert.astype("float32")
    out["expert_legality_weather"] = legality_weather.astype("float32")
    out["expert_competitive"] = competitive_expert.astype("float32")
    out["gate_degradation"] = (gate_deg / gate_sum).astype("float32")
    out["gate_legality_weather"] = (gate_legality / gate_sum).astype("float32")
    out["gate_competitive"] = (gate_comp / gate_sum).astype("float32")
    out["blend_degradation"] = (degradation_expert * gate_deg / gate_sum).astype(
        "float32"
    )
    out["blend_legality_weather"] = (
        legality_weather * gate_legality / gate_sum
    ).astype("float32")
    out["blend_competitive"] = (competitive_expert * gate_comp / gate_sum).astype(
        "float32"
    )
    out["tyrelife_x_progress"] = (tyre * progress).astype("float32")
    out["tyrelife_x_dry"] = (tyre * dry).astype("float32")
    out["tyrelife_x_wet"] = (tyre * wet).astype("float32")
    out["position_x_pressure"] = (pos * field_pressure).astype("float32")
    out["compound_freq"] = comp_freq.astype("float32")
    out["race_freq"] = race_freq.astype("float32")
    out["driver_freq"] = driver_freq.astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000761"],
        "research_usage_note": "Verified assigned hypothesis 000761.",
    }, sort_keys=True))
