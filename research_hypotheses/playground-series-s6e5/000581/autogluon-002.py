...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.copy()
    X["_row_id_000581"] = np.arange(len(X), dtype=np.int64)
    ...
    lap_sorted = X.sort_values(
        lap_keys + ["Position", "Driver", "_row_id_000581"], kind="mergesort"
    ).copy()
    lap_group = lap_sorted.groupby(lap_keys, sort=False)
    lap_sorted["ahead_driver"] = lap_group["Driver"].shift(1)
    lap_sorted["ahead_now_lap_delta"] = lap_group["LapTime_Delta"].shift(1)
    lap_sorted["ahead_now_tyre_life"] = lap_group["TyreLife"].shift(1)
    lap_sorted["ahead_now_deg"] = lap_group["Cumulative_Degradation"].shift(1)
    lap_sorted["behind_driver"] = lap_group["Driver"].shift(periods=-1)
    lap_sorted["behind_now_lap_delta"] = lap_group["LapTime_Delta"].shift(periods=-1)
    lap_sorted["behind_now_tyre_life"] = lap_group["TyreLife"].shift(periods=-1)
    lap_sorted["behind_now_deg"] = lap_group["Cumulative_Degradation"].shift(periods=-1)
    ...
    neighbor_cols = [
        ...
        "ahead_now_deg",
        "behind_driver",
        "behind_now_lap_delta",
        "behind_now_tyre_life",
        "behind_now_deg",
    ]
    X = X.join(
        lap_sorted.set_index("_row_id_000581")[neighbor_cols],
        on="_row_id_000581",
    )

    X["pace_vs_ahead_now"] = (
        X["LapTime_Delta"] - X["ahead_now_lap_delta"].fillna(X["LapTime_Delta"])
    )
    X["pace_vs_behind_now"] = (
        X["LapTime_Delta"] - X["behind_now_lap_delta"].fillna(X["LapTime_Delta"])
    )
    X["tyre_life_vs_ahead_now"] = (
        X["TyreLife"] - X["ahead_now_tyre_life"].fillna(X["TyreLife"])
    )
    X["tyre_life_vs_behind_now"] = (
        X["TyreLife"] - X["behind_now_tyre_life"].fillna(X["TyreLife"])
    )
    ...
    for side in ["ahead", "behind"]:
        for lag in [1, 2]:
            hist = rival_hist.copy()
            hist["LapNumber"] = hist["LapNumber"] + lag
            hist_indexed = (
                hist.groupby(race_keys + ["Driver", "LapNumber"], sort=False)
                .last()
                .sort_index()
            )
                ...
                pd.DataFrame(
                    {
                        "Year": X["Year"],
                        "Race": X["Race"],
                        "Driver": X[f"{side}_driver"],
                        "LapNumber": X["LapNumber"],
                    }
                )
            )
            X[f"{side}_pit_lag{lag}"] = (
                hist_indexed["PitStop"].reindex(lookup).fillna(0).to_numpy()
            ).astype("int8")
            X[f"{side}_pit_tyre_life_lag{lag}"] = (
                hist_indexed["TyreLife"].reindex(lookup).fillna(0.0).to_numpy()
            )
            X[f"{side}_pit_deg_lag{lag}"] = (
                hist_indexed["Cumulative_Degradation"]
                .reindex(lookup)
                .fillna(0.0)
                .to_numpy()
            )
            X[f"{side}_pit_lap_delta_lag{lag}"] = (
                hist_indexed["LapTime_Delta"].reindex(lookup).fillna(0.0).to_numpy()
            )
            X[f"{side}_pit_pos_lag{lag}"] = (
                hist_indexed["Position"].reindex(lookup).fillna(0.0).to_numpy()
            )
