...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_wet_compound"] = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["is_slick_compound"] = (1 - out["is_wet_compound"]).astype("int8")

    lap_group = out.groupby(keys, sort=False, observed=True)

    lap_median_time = lap_group["LapTime (s)"].transform("median")
    race_compound_group = out.groupby(
        ["Year", "Race", "Compound"], sort=False, observed=True
    )
    race_compound_med_time = race_compound_group["LapTime (s)"].transform("median")
    race_compound_std_time = (
        race_compound_group["LapTime (s)"].transform("std").replace(0, np.nan)
    )
    out["lap_time_inflation_z"] = (
        ((lap_median_time - race_compound_med_time) / race_compound_std_time)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .clip(-5, 8)
    )

    lap_abs_pos_change = lap_group["Position_Change"].transform(
        lambda s: s.abs().mean()
    )
    race_pos_baseline = out.groupby(["Year", "Race"], sort=False, observed=True)[
        "Position_Change"
    ].transform(lambda s: s.abs().mean())
    out["same_lap_position_turbulence"] = (
        (lap_abs_pos_change / (race_pos_baseline + 0.1))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(1)
        .clip(0, 8)
    )

    lap_pit_rate = lap_group["PitStop"].transform("mean")
    race_pit_rate = out.groupby(["Year", "Race"], sort=False, observed=True)[
        "PitStop"
    ].transform("mean")
    out["same_lap_pit_burst"] = (
        (lap_pit_rate / (race_pit_rate + 0.01))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0)
        .clip(0, 20)
    )

    lap_deg_jump = lap_group["Cumulative_Degradation"].transform(
        lambda s: s.diff().abs().median()
    )
    race_deg_jump = out.groupby(["Year", "Race"], sort=False, observed=True)[
        "Cumulative_Degradation"
    ].transform(lambda s: s.diff().abs().median())
    out["same_lap_degradation_discontinuity"] = (
        (lap_deg_jump / (race_deg_jump + 0.1))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(1)
        .clip(0, 20)
    )
    ...
    score = (
        0.40 * out["lap_time_inflation_z"].clip(lower=0)
        + 0.20 * out["same_lap_position_turbulence"].clip(lower=0)
        + 0.25 * out["same_lap_pit_burst"].clip(lower=0)
        + 0.15 * out["same_lap_degradation_discontinuity"].clip(lower=0)
    )
    out["latent_caution_score_000586"] = (
        score * np.where(out["is_wet_compound"].eq(1), 0.75, 1.0)
    ).clip(0, 12)

    out["latent_caution_gate_000586"] = (
        out["latent_caution_score_000586"] >= 2.5
    ).astype("int8")
    out["late_race_gate_000586"] = (out["RaceProgress"] >= 0.70).astype("int8")
    out["green_flag_gate_000586"] = (
        (out["latent_caution_gate_000586"] == 0) & (out["is_wet_compound"] == 0)
    ).astype("int8")

    for base in ["TyreLife", "RaceProgress", "Stint", "Position"]:
        out[f"{base}_x_caution_000586"] = out[base] * out["latent_caution_gate_000586"]
        out[f"{base}_x_late_000586"] = out[base] * out["late_race_gate_000586"]
        out[f"{base}_x_green_000586"] = out[base] * out["green_flag_gate_000586"]

    out.attrs["research_hypotheses_llm_claimed_used"] = ["000586"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000586"],
        "research_usage_note": "Verified assigned hypothesis 000586.",
    }, sort_keys=True))
