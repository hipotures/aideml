...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_wet = compound.eq("WET")
    is_inter = compound.eq("INTERMEDIATE")
    is_wet_family = (is_wet | is_inter).astype("int8")
    is_dry_family = (~(is_wet | is_inter)).astype("int8")

    g = X.groupby(group_cols, sort=False)

    X["compound_is_soft"] = compound.eq("SOFT").astype("int8")
    X["compound_is_medium"] = compound.eq("MEDIUM").astype("int8")
    X["compound_is_hard"] = compound.eq("HARD").astype("int8")
    X["compound_is_intermediate"] = is_inter.astype("int8")
    X["compound_is_wet"] = is_wet.astype("int8")
    X["compound_is_dry"] = is_dry_family
    X["compound_is_wet_family"] = is_wet_family

    X["wet_used_so_far"] = g["compound_is_wet_family"].cummax().astype("int8")
    X["wet_rule_exempt"] = X["wet_used_so_far"]

    for c in ["soft", "medium", "hard"]:
        X[f"used_{c}_so_far"] = g[f"compound_is_{c}"].cummax().astype("int8")

    X["dry_specs_used_so_far"] = (
        X["used_soft_so_far"] + X["used_medium_so_far"] + X["used_hard_so_far"]
    ).astype("int8")

    X["dry_rule_still_unsatisfied"] = (
        (X["wet_rule_exempt"] == 0)
        & (X["compound_is_dry"] == 1)
        & (X["dry_specs_used_so_far"] < 2)
    ).astype("int8")

    race_upper = X["Race"].astype(str).str.upper()
    X["is_monaco"] = race_upper.str.contains("MONACO", regex=False).astype("int8")
    X["monaco_2025_forced_extra_stop"] = (
        (X["Year"] == 2025) & (X["is_monaco"] == 1)
    ).astype("int8")

    X["pitstops_so_far"] = g["PitStop"].cumsum().clip(lower=0).astype("int16")
    monaco_remaining = np.where(
        X["monaco_2025_forced_extra_stop"] == 1,
        np.maximum(0, 2 - X["pitstops_so_far"]),
        0,
    )
    dry_remaining = X["dry_rule_still_unsatisfied"].astype("int8")
    X["remaining_required_stops_min"] = np.maximum(
        monaco_remaining, dry_remaining
    ).astype("int8")

    X["rule_pressure_progress"] = X["remaining_required_stops_min"] * X["RaceProgress"]
    X["rule_pressure_stint"] = X["remaining_required_stops_min"] * X["Stint"]
    X["rule_pressure_tyre_life"] = X["remaining_required_stops_min"] * X["TyreLife"]
    X["monaco_progress_pressure"] = (
        X["monaco_2025_forced_extra_stop"] * X["RaceProgress"]
    )
    X["dry_unsat_progress"] = X["dry_rule_still_unsatisfied"] * X["RaceProgress"]
    X["dry_unsat_stint"] = X["dry_rule_still_unsatisfied"] * X["Stint"]
    X["dry_unsat_current_soft"] = (
        X["dry_rule_still_unsatisfied"] * X["compound_is_soft"]
    )
    X["dry_unsat_current_medium"] = (
        X["dry_rule_still_unsatisfied"] * X["compound_is_medium"]
    )
    X["dry_unsat_current_hard"] = (
        X["dry_rule_still_unsatisfied"] * X["compound_is_hard"]
    )
    X["wet_exempt_progress"] = X["wet_rule_exempt"] * X["RaceProgress"]
    X["rule_pressure_near_finish"] = X["remaining_required_stops_min"] * (
        1.0 - X["RaceProgress"]
    )

    X["laps_remaining_est"] = (
        (1.0 - X["RaceProgress"]) * X["LapNumber"] / X["RaceProgress"].clip(lower=1e-3)
    ).clip(lower=0)
    X["required_stops_per_lap_left"] = X["remaining_required_stops_min"] / (
        X["laps_remaining_est"] + 1.0
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000850.",
    }, sort_keys=True))
