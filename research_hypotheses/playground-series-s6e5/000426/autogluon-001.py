...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["is_wet_compound"] = compound_str.isin(wet_compounds).astype("int8")
    X["is_intermediate"] = (compound_str == "INTERMEDIATE").astype("int8")
    X["is_full_wet"] = (compound_str == "WET").astype("int8")
    X["is_slick"] = (~compound_str.isin(wet_compounds)).astype("int8")

    race_keys = ["Year", "Race"]
    lap_keys = ["Year", "Race", "LapNumber"]

    lap_weather = (
        X.groupby(lap_keys, observed=True)
        .agg(
            lap_wet_share=("is_wet_compound", "mean"),
            lap_intermediate_share=("is_intermediate", "mean"),
            lap_full_wet_share=("is_full_wet", "mean"),
            lap_laptime_median=("LapTime (s)", "median"),
            lap_delta_median=("LapTime_Delta", "median"),
            lap_degradation_median=("Cumulative_Degradation", "median"),
    ...
    lap_weather["rain_now"] = (lap_weather["lap_wet_share"] > 0).astype("int8")
    lap_weather["wet_majority"] = (lap_weather["lap_wet_share"] >= 0.5).astype("int8")
    lap_weather["recent_rain_onset"] = (
        (lap_weather["rain_now"] == 1)
        & (
            lap_weather.groupby(race_keys, observed=True)["rain_now"]
    ...
    for col in [
        "lap_laptime_median",
        "lap_delta_median",
        "lap_degradation_median",
        "lap_poschg_abs_mean",
        "lap_wet_share",
    ]:
        lap_weather[f"{col}_prev"] = lap_weather.groupby(race_keys, observed=True)[
            col
        ].shift(1)
        lap_weather[f"{col}_diff"] = lap_weather[col] - lap_weather[f"{col}_prev"]

    lap_weather["pace_drop_signal"] = (
        (lap_weather["lap_laptime_median_diff"].fillna(0) > 3.0)
        | (lap_weather["lap_delta_median_diff"].fillna(0) > 2.0)
    ).astype("int8")

    lap_weather["instability_signal"] = (
        lap_weather["lap_poschg_abs_mean_diff"].fillna(0) > 0.5
    ).astype("int8")
    ...
    keep_cols = [
        ...
        "LapNumber",
        "lap_wet_share",
        "lap_intermediate_share",
        "lap_full_wet_share",
        "rain_now",
        "wet_majority",
        "recent_rain_onset",
        "lap_laptime_median_diff",
        "lap_delta_median_diff",
        "lap_degradation_median_diff",
        "lap_poschg_abs_mean_diff",
        "lap_wet_share_diff",
        "pace_drop_signal",
        "instability_signal",
    ]
    X = X.merge(lap_weather[keep_cols], on=lap_keys, how="left")

    X["compound_weather_mismatch"] = (
        ((X["is_slick"] == 1) & (X["lap_wet_share"].fillna(0) > 0))
        | ((X["is_wet_compound"] == 1) & (X["lap_wet_share"].fillna(0) == 0))
    ).astype("int8")

    X["wet_transition_pressure"] = X["is_slick"] * X["lap_wet_share"].fillna(0)
    X["full_wet_transition_pressure"] = X["is_slick"] * X["lap_full_wet_share"].fillna(
        0
    )
    X["recent_onset_on_slick"] = X["is_slick"] * X["recent_rain_onset"].fillna(0)
    X["pace_drop_on_slick"] = X["is_slick"] * X["pace_drop_signal"].fillna(0)
    X["wet_stint_pressure"] = X["Stint"] * X["lap_wet_share"].fillna(0)
    X["wet_tyre_life_pressure"] = X["TyreLife"] * X["lap_wet_share"].fillna(0)
    X["wet_progress_pressure"] = X["RaceProgress"] * X["lap_wet_share"].fillna(0)

    driver_keys = ["Year", "Race", "Driver"]
    X = X.sort_values(driver_keys + ["LapNumber"], kind="mergesort")
    for col in ["rain_now", "lap_wet_share", "pace_drop_signal", "recent_rain_onset"]:
        X[f"driver_prev_{col}"] = X.groupby(driver_keys, observed=True)[col].shift(1)
    X["driver_seen_rain"] = (
        X.groupby(driver_keys, observed=True)["rain_now"].cummax().shift(1).fillna(0)
    ).astype("int8")
    X["driver_rain_arrival"] = (
        (X["rain_now"].fillna(0) == 1) & (X["driver_prev_rain_now"].fillna(0) == 0)
    ).astype("int8")
    ...
    flag_fill = [
        ...
        "pace_drop_signal",
        "instability_signal",
        "compound_weather_mismatch",
        "recent_onset_on_slick",
        "pace_drop_on_slick",
        "driver_prev_rain_now",
        "driver_prev_pace_drop_signal",
        "driver_prev_recent_rain_onset",
        "driver_seen_rain",
        "driver_rain_arrival",
    ]
    for c in flag_fill:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000426"],
        "research_usage_note": "Verified assigned hypothesis 000426.",
    }, sort_keys=True))
