...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year_s = df["Year"].astype("string")
    stint_s = df["Stint"].astype("string")

    # CatBoost-friendly categorical crosses for ordered CTR learning
    df["Driver_Year"] = df["Driver"] + "__" + year_s
    df["Race_Compound"] = df["Race"] + "__" + df["Compound"]
    df["Driver_Compound"] = df["Driver"] + "__" + df["Compound"]

    # Extra hierarchy-style interactions closely aligned with the hypothesis
    df["Driver_Race"] = df["Driver"] + "__" + df["Race"]
    df["Year_Race"] = year_s + "__" + df["Race"]
    df["Year_Compound"] = year_s + "__" + df["Compound"]
    df["Driver_Year_Compound"] = df["Driver"] + "__" + year_s + "__" + df["Compound"]
    df["Race_Stint"] = df["Race"] + "__" + stint_s
    df["Compound_Stint"] = df["Compound"] + "__" + stint_s
    ...
    for c in cat_cols + new_cat_cols:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000137.",
    }, sort_keys=True))
