...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_prior = {
        "Bahrain Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Saudi Arabian Grand Prix": (1.0, 2.0, 1.0, 2.0),
        "Australian Grand Prix": (2.0, 2.0, 1.0, 2.0),
        "Azerbaijan Grand Prix": (1.0, 2.0, 1.0, 2.0),
        "Miami Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Emilia Romagna Grand Prix": (2.0, 2.0, 1.0, 2.0),
        "Monaco Grand Prix": (3.0, 1.0, 1.0, 1.0),
        "Spanish Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Canadian Grand Prix": (1.0, 2.0, 1.0, 2.0),
        "Austrian Grand Prix": (1.0, 3.0, 2.0, 3.0),
        "British Grand Prix": (2.0, 4.0, 2.0, 3.0),
        "Hungarian Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Belgian Grand Prix": (2.0, 4.0, 2.0, 3.0),
        "Dutch Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Italian Grand Prix": (1.0, 2.0, 1.0, 2.0),
        "Singapore Grand Prix": (3.0, 3.0, 2.0, 3.0),
        "Japanese Grand Prix": (2.0, 4.0, 2.0, 3.0),
        "Qatar Grand Prix": (2.0, 4.0, 2.0, 4.0),
        "United States Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Mexico City Grand Prix": (2.0, 2.0, 1.0, 2.0),
        "Sao Paulo Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Las Vegas Grand Prix": (1.0, 1.0, 1.0, 1.0),
        "Abu Dhabi Grand Prix": (2.0, 2.0, 1.0, 2.0),
        "Chinese Grand Prix": (2.0, 3.0, 2.0, 3.0),
        "Pre-Season Testing": (2.0, 2.0, 1.0, 2.0),
    }

    prior = out["Race"].map(race_prior)
    prior_df = prior.apply(
        lambda x: x if isinstance(x, tuple) else (2.0, 2.6, 1.5, 2.5)
    ).apply(pd.Series)
    prior_df.columns = [
        "track_pit_loss_bucket",
        "track_tyre_stress_bucket",
        "track_stop_bias",
        "track_degradation_tendency",
    ]
    ...
    max_lap_est = out["LapNumber"] / out["RaceProgress"].clip(lower=0.01)
    laps_remaining = (max_lap_est - out["LapNumber"]).clip(lower=0, upper=80)

    out["track_adjusted_tyre_age"] = (
        out["TyreLife"] * compound_deg * out["track_degradation_tendency"]
    )
    out["track_stress_age"] = out["TyreLife"] * out["track_tyre_stress_bucket"]
    out["track_progress_stop_pressure"] = (
        out["RaceProgress"]
        * out["track_stop_bias"]
        / out["track_pit_loss_bucket"].clip(lower=1)
    )
    out["track_laps_remaining"] = laps_remaining
    out["track_remaining_stress"] = laps_remaining * out["track_tyre_stress_bucket"]
    out["track_compound_degradation"] = (
        compound_deg
        * out["track_tyre_stress_bucket"]
        * out["track_degradation_tendency"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000745"],
        "research_usage_note": "Verified assigned hypothesis 000745.",
    }, sort_keys=True))
