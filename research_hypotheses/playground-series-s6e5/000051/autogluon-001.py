...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in key_cols:
        if col in out.columns:
            s = pd.to_numeric(out[col], errors="coerce").replace(
                [np.inf, -np.inf], np.nan
            )
            out[f"{col}_was_missing_or_inf"] = s.isna().astype("int8")
            out[col] = s

    eps = 1e-3

    rp = out["RaceProgress"].clip(lower=0.005, upper=1.0)
    lap = out["LapNumber"].clip(lower=1, upper=120)
    tyre = out["TyreLife"].clip(lower=0, upper=120)
    lap_time = out["LapTime (s)"].clip(lower=40, upper=600)
    lap_delta = out["LapTime_Delta"].clip(lower=-300, upper=300)
    degr = out["Cumulative_Degradation"].clip(lower=-500, upper=3000)

    est_total_laps = (lap / rp.clip(lower=eps)).clip(lower=1, upper=120)
    laps_remaining = (est_total_laps - lap).clip(lower=0, upper=120)

    out["safe_RaceProgress"] = rp
    out["safe_TyreLife"] = tyre
    out["safe_LapTime_s"] = lap_time
    out["safe_LapTime_Delta"] = lap_delta
    out["safe_Cumulative_Degradation"] = degr
    out["safe_est_total_laps"] = est_total_laps
    out["safe_laps_remaining"] = laps_remaining

    out["safe_tyre_life_frac"] = (tyre / est_total_laps.clip(lower=1)).clip(0, 2)
    out["safe_laps_remaining_frac"] = (
        laps_remaining / est_total_laps.clip(lower=1)
    ).clip(0, 1)
    out["safe_degradation_per_tyre_lap"] = (degr / (tyre + eps)).clip(-100, 200)
    out["safe_laptime_delta_frac"] = (lap_delta / lap_time.clip(lower=1)).clip(-3, 3)
    out["safe_tyre_age_x_progress"] = (tyre * rp).clip(0, 120)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000051"],
        "research_usage_note": "Verified assigned hypothesis 000051.",
    }, sort_keys=True))
