...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_wet = compound.isin(["WET", "INTERMEDIATE"]).astype(float)
    is_soft = compound.eq("SOFT").astype(float)
    is_medium = compound.eq("MEDIUM").astype(float)
    is_hard = compound.eq("HARD").astype(float)

    out["Compound_Class"] = np.select(
        [
            compound.isin(["WET", "INTERMEDIATE"]),
            compound.isin(["SOFT", "MEDIUM", "HARD"]),
        ],
        ["wet", "slick"],
        default="other",
    )
    out["Compound_Ordinal"] = np.select(
        [
            compound.eq("WET"),
    ...
    race_keys = ["Year", "Race"]
    grp = out.groupby(race_keys, sort=False)

    race_deg_median = grp["Cumulative_Degradation"].transform("median")
    race_deg_mean = grp["Cumulative_Degradation"].transform("mean")
    race_delta_std = grp["LapTime_Delta"].transform("std").fillna(0.0)
    race_tyre_mean = grp["TyreLife"].transform("mean")
    race_progress_mean = grp["RaceProgress"].transform("mean")

    out["Race_Degradation_Median"] = race_deg_median
    out["Race_LapDelta_Std"] = race_delta_std
    out["Race_TyreLife_Mean"] = race_tyre_mean
    out["Race_Progress_Mean"] = race_progress_mean

    est_total_laps = (out["LapNumber"] / out["RaceProgress"].clip(lower=1e-3)).clip(
        upper=90
    )
    laps_remaining = (est_total_laps - out["LapNumber"]).clip(lower=0)
    out["Estimated_Total_Laps"] = est_total_laps
    out["Laps_Remaining"] = laps_remaining
    out["TyreLife_Ratio"] = out["TyreLife"] / est_total_laps.clip(lower=1)
    out["Stint_Progress"] = out["TyreLife"] / out.groupby(
        race_keys + ["Driver", "Stint"], sort=False
    )["TyreLife"].transform("max").clip(lower=1)

    deg_rel = (out["Cumulative_Degradation"] - race_deg_mean) / (race_delta_std + 5.0)
    tyre_pressure = out["TyreLife"] / (race_tyre_mean.abs() + 3.0)
    progress_centered = out["RaceProgress"] - 0.75

    dry_gate = sigmoid(
        2.4 * (1.0 - is_wet) + 0.8 * is_soft + 0.4 * is_medium - 0.2 * is_hard
    )
    wet_gate = sigmoid(
        3.0 * is_wet + 0.8 * (out["LapTime_Delta"].abs() / (race_delta_std + 3.0) - 1.0)
    )
    energy_gate = sigmoid(1.6 * deg_rel + 0.9 * (tyre_pressure - 1.0) + 0.4 * is_soft)
    endgame_gate = sigmoid(10.0 * progress_centered - 0.12 * (laps_remaining - 8.0))

    gate_sum = dry_gate + wet_gate + energy_gate + endgame_gate + 1e-6
    out["Gate_Dry"] = dry_gate / gate_sum
    out["Gate_Wet"] = wet_gate / gate_sum
    out["Gate_HighEnergy"] = energy_gate / gate_sum
    out["Gate_Endgame"] = endgame_gate / gate_sum

    base_signals = {
        "TyreLife": out["TyreLife"],
        "Cumulative_Degradation": out["Cumulative_Degradation"],
        "LapTime_Delta": out["LapTime_Delta"],
        "RaceProgress": out["RaceProgress"],
        "Laps_Remaining": laps_remaining,
        "Position": out["Position"],
        "Position_Change": out["Position_Change"],
        "Stint": out["Stint"],
    }
    ...
    gates = {
        "Dry": out["Gate_Dry"],
        "Wet": out["Gate_Wet"],
        "HighEnergy": out["Gate_HighEnergy"],
        "Endgame": out["Gate_Endgame"],
    }

    for gate_name, gate_vals in gates.items():
        for feat_name, feat_vals in base_signals.items():
            out[f"{feat_name}_x_{gate_name}"] = feat_vals * gate_vals

    out["Dry_x_Soft"] = out["Gate_Dry"] * is_soft
    out["Dry_x_Hard"] = out["Gate_Dry"] * is_hard
    out["Wet_x_Inter"] = out["Gate_Wet"] * compound.eq("INTERMEDIATE").astype(float)
    out["Wet_x_Wet"] = out["Gate_Wet"] * compound.eq("WET").astype(float)
    out["Energy_x_StintProgress"] = out["Gate_HighEnergy"] * out["Stint_Progress"]
    out["Endgame_x_TyreLifeRatio"] = out["Gate_Endgame"] * out["TyreLife_Ratio"]
    out["Energy_x_Endgame"] = out["Gate_HighEnergy"] * out["Gate_Endgame"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000736.",
    }, sort_keys=True))
