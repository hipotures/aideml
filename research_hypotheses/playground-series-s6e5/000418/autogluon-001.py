...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["race_lap_est"] = out["LapNumber"] + out["laps_remaining"]
    out["tyre_life_frac"] = out["TyreLife"] / out["race_lap_est"].clip(lower=1)
    out["stint_progress"] = out["TyreLife"] / (
        out["TyreLife"] + out["laps_remaining"] + eps
    )
    out["wear_per_lap"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(lower=1)
    out["degradation_pressure"] = out["Cumulative_Degradation"] * np.log1p(
        out["TyreLife"]
    )
    out["late_worn_tyre"] = out["RaceProgress"] * np.log1p(out["TyreLife"])
    out["stop_debt"] = (
        out["TyreLife"]
        - (
            18
            + 4 * (out["Compound"].astype(str) == "HARD").astype(int)
            - 4 * (out["Compound"].astype(str) == "SOFT").astype(int)
        )
    ).clip(lower=0)
    out["pit_window_pressure"] = out["stop_debt"] * (0.4 + out["RaceProgress"])
    out["lap_delta_abs"] = out["LapTime_Delta"].abs()
    out["position_loss_pressure"] = out["Position_Change"].clip(lower=0) * np.log1p(
        out["TyreLife"]
    )
    out["track_position_value"] = (21 - out["Position"]).clip(lower=1) * out[
        "RaceProgress"
    ]
    out["is_wet_compound"] = (
        out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["is_soft_compound"] = (out["Compound"].astype(str) == "SOFT").astype("int8")
    out["is_hard_compound"] = (out["Compound"].astype(str) == "HARD").astype("int8")

    s = out.sort_values(["Year", "Race", "Driver", "LapNumber", "Stint", "_row_id"])
    ...
    for c in [
        ...
        s[f"prev_{c}"] = g[c].shift(1)

    s["laps_since_known_pit"] = (
        g["PitStop"]
        .transform(lambda x: x.groupby(x.eq(1).cumsum()).cumcount())
        .astype(float)
    )
    s["driver_lap_time_ma3"] = g["LapTime (s)"].transform(
        lambda x: x.shift(1).rolling(3, min_periods=1).mean()
    )
    s["driver_delta_ma3"] = g["LapTime_Delta"].transform(
        lambda x: x.shift(1).rolling(3, min_periods=1).mean()
    )
    s["driver_deg_ma3"] = g["Cumulative_Degradation"].transform(
        lambda x: x.shift(1).rolling(3, min_periods=1).mean()
    )

    aligned = s.sort_values("_row_id")
    for c in [
        "prev_LapTime (s)",
        "prev_LapTime_Delta",
        "prev_Position",
        ...
        "prev_TyreLife",
        "prev_Cumulative_Degradation",
        "prev_PitStop",
        "laps_since_known_pit",
        "driver_lap_time_ma3",
        "driver_delta_ma3",
        "driver_deg_ma3",
    ]:
    ...
    out["lap_time_vs_driver_ma3"] = out["LapTime (s)"] - out["driver_lap_time_ma3"]
    out["delta_vs_driver_ma3"] = out["LapTime_Delta"] - out["driver_delta_ma3"]
    out["deg_vs_driver_ma3"] = out["Cumulative_Degradation"] - out["driver_deg_ma3"]
    out["tyre_life_jump"] = out["TyreLife"] - out["prev_TyreLife"]
    out["position_change_from_prev"] = out["Position"] - out["prev_Position"]
    out["prev_pit_flag"] = out["prev_PitStop"].fillna(0).astype("int8")
    ...
    for c in rank_cols:
        out[f"{c}_race_pct_rank"] = out.groupby(["Year", "Race"], observed=True)[
            c
        ].rank(pct=True)
        out[f"{c}_compound_pct_rank"] = out.groupby(
            ["Year", "Race", "Compound"], observed=True
        )[c].rank(pct=True)

    for c in ["Driver", "Race", "Compound"]:
        freq = out[c].map(out[c].value_counts(normalize=True))
        out[f"{c}_freq"] = freq.astype(float)

    out["physics_stop_expert"] = (
        0.32 * out["tyre_life_frac"].fillna(0)
        + 0.22 * out["RaceProgress"].fillna(0)
        + 0.18 * np.log1p(out["stop_debt"].fillna(0))
        + 0.12 * out["Cumulative_Degradation_race_pct_rank"].fillna(0.5)
        + 0.08 * out["LapTime_Delta_race_pct_rank"].fillna(0.5)
        + 0.05 * out["position_loss_pressure"].fillna(0)
        - 0.10 * out["prev_pit_flag"].fillna(0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000418.",
    }, sort_keys=True))
