...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_grp = ["Year", "Race"]
    driver_stint_grp = ["Year", "Race", "Driver", "Stint"]

    cols = ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation", "Position_Change"]
    temp_cols = []

    for i, col in enumerate(cols):
        q = (
            out.groupby(race_lap_grp, observed=True)[col]
        ...
        out = out.join(q, on=race_lap_grp)
        ...
        scale = ((q75 - q25) / 1.349).replace(0, np.nan)
        out[f"{col}_win"] = out[col].clip(q05, q95)
        out[f"{col}_race_lap_rz"] = (
            ((out[col] - q50) / scale)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .clip(-8, 8)
        )
        out[f"{col}_race_lap_rank"] = (
            out.groupby(race_lap_grp, observed=True)[col]
        ...
        temp_cols.extend(
            [f"__q{i}_05", f"__q{i}_25", f"__q{i}_50", f"__q{i}_75", f"__q{i}_95"]
        )

    lap_median = out.groupby(race_lap_grp, observed=True)["LapTime (s)_win"].transform(
        "median"
    )
    race_median = out.groupby(race_grp, observed=True)["LapTime (s)_win"].transform(
        "median"
    )
    out["field_slowdown"] = lap_median - race_median
    out["field_slowdown_rank"] = (
        out.groupby(race_grp, observed=True)["field_slowdown"]
        .rank(pct=True, method="average")
        .fillna(0.5)
    )

    driver_stint_median = out.groupby(driver_stint_grp, observed=True)[
        "LapTime (s)_win"
    ].transform("median")
    out["isolated_driver_slowdown"] = (
        (out["LapTime (s)_win"] - lap_median) - (driver_stint_median - race_median)
    ).fillna(0.0)

    out["extreme_lap"] = (
        (out["LapTime (s)"] > 2000).astype("int8")
        | (out["LapTime_Delta"].abs() > 1200).astype("int8")
        | (out["LapTime (s)_race_lap_rz"].abs() > 3).astype("int8")
        | (out["LapTime_Delta_race_lap_rz"].abs() > 3).astype("int8")
    ).astype("int8")

    out["abnormal_regime_score"] = (
        out["LapTime (s)_race_lap_rz"].abs()
        + out["LapTime_Delta_race_lap_rz"].abs()
        + out["Cumulative_Degradation_race_lap_rz"].abs() * 0.5
        + out["Position_Change_race_lap_rz"].abs() * 0.5
    )

    out["LapTime_x_field_slowdown"] = out["LapTime (s)_race_lap_rz"] * (
        out["field_slowdown_rank"] - 0.5
    )
    out["Delta_x_extreme"] = out["LapTime_Delta_win"] * out["extreme_lap"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000509"],
        "research_usage_note": "Verified assigned hypothesis 000509.",
    }, sort_keys=True))
