...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Race"] = out["Compound"].astype(str) + "_" + out["Race"].astype(str)
    ...
    out["Compound_Ordinal"] = out["Compound"].map(compound_order).astype(float)
    out["Is_Dry_Tyre"] = (
        out["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype(np.int8)
    )
    out["Is_Wet_Tyre"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype(np.int8)
    ...
    out["Estimated_Total_Laps"] = lap / progress.replace(0, np.nan)
    out["Estimated_Laps_Remaining"] = (out["Estimated_Total_Laps"] - lap).clip(lower=0)
    out["TyreLife_x_Progress"] = tyre * progress
    out["TyreLife_per_Stint"] = tyre / stint.replace(0, np.nan)
    out["Lap_in_Race_Fraction"] = lap / out["Estimated_Total_Laps"].replace(0, np.nan)
    out["Stint_x_Compound"] = stint * out["Compound_Ordinal"]
    out["Tyre_Age_Risk"] = tyre * out["Compound_Ordinal"]
    out["Late_Stint"] = (tyre >= 15).astype(np.int8)
    out["Very_Late_Stint"] = (tyre >= 25).astype(np.int8)
    ...
    out["PitStop_x_TyreLife"] = out["PitStop"].astype(float) * tyre
    out["PitStop_x_Stint"] = out["PitStop"].astype(float) * stint
    out["PitStop_x_Progress"] = out["PitStop"].astype(float) * progress
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000524.",
    }, sort_keys=True))
