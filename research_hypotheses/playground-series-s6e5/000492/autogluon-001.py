...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp_cols = ["Race", "Year", "Driver", "Stint"]
    order_cols = grp_cols + ["LapNumber"]

    order = out.sort_values(order_cols, kind="mergesort").index
    start_tyre = (
        out.loc[order].groupby(grp_cols, sort=False)["TyreLife"].transform("first")
    )
    out["StintStartTyreLife"] = start_tyre.reindex(out.index).astype("float32")

    out["CurrentStintLap"] = (
        out["TyreLife"].astype("float32") - out["StintStartTyreLife"] + 1.0
    ).clip(lower=1)
    out["UsedSetAtStintStart"] = (out["StintStartTyreLife"] > 1).astype("int8")

    if "RaceProgress" in out.columns:
        rp = out["RaceProgress"].astype("float32").clip(0.01, 1.0)
        est_total_laps = out["LapNumber"].astype("float32") / rp
        out["LapsRemaining_Est"] = (
            est_total_laps - out["LapNumber"].astype("float32")
        ).clip(lower=0)
        out["CurrentStintLap_x_RaceProgress"] = out["CurrentStintLap"] * rp
        out["UsedSet_x_RaceProgress"] = out["UsedSetAtStintStart"] * rp
        out["UsedSet_x_LapsRemaining_Est"] = out["UsedSetAtStintStart"] * out[
            "LapsRemaining_Est"
        ].astype("float32")
        out["CurrentStintLap_x_LapsRemaining_Est"] = out["CurrentStintLap"] * out[
            "LapsRemaining_Est"
        ].astype("float32")

    if "Compound" in out.columns:
        compound_code = out["Compound"].astype("category").cat.codes.astype("int16")
        out["Compound_Code"] = compound_code
        out["CurrentStintLap_x_Compound"] = out["CurrentStintLap"] * compound_code
        out["UsedSet_x_Compound"] = out["UsedSetAtStintStart"] * compound_code
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000492"],
        "research_usage_note": "Verified assigned hypothesis 000492.",
    }, sort_keys=True))
