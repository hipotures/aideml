...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Explicit within-stint group key for the Race x Year x Driver x Stint ordering hypothesis.
    X["stint_group"] = (
        X["Year"].astype(str)
        + "|"
        + X["Race"].astype(str)
    ...
    # Categorical interaction to help specialist models separate stint regimes by tyre choice.
    X["driver_compound"] = X["Driver"].astype(str) + "|" + X["Compound"].astype(str)
    X["race_compound"] = X["Race"].astype(str) + "|" + X["Compound"].astype(str)

    # Leakage-safe within-stint ordering surrogates from current-lap state only.
    X["tyrelife_sq"] = X["TyreLife"] ** 2
    X["raceprogress_sq"] = X["RaceProgress"] ** 2
    X["stint_progress_proxy"] = X["TyreLife"] / (X["LapNumber"] + eps)
    X["stint_progress_x_raceprogress"] = X["TyreLife"] * X["RaceProgress"]
    X["stint_progress_x_stint"] = X["TyreLife"] * X["Stint"]

    # Pace/degradation intensity features.
    X["deg_per_lap"] = X["Cumulative_Degradation"] / (X["TyreLife"] + eps)
    X["deg_per_raceprogress"] = X["Cumulative_Degradation"] / (X["RaceProgress"] + eps)
    X["laptime_delta_per_lap"] = X["LapTime_Delta"] / (X["TyreLife"] + eps)
    X["laptime_delta_per_raceprogress"] = X["LapTime_Delta"] / (X["RaceProgress"] + eps)
    X["laptime_per_lap"] = X["LapTime (s)"] / (X["LapNumber"] + eps)

    # Interactions that sharpen late-stint stop imminence ordering.
    X["deg_x_delta"] = X["Cumulative_Degradation"] * X["LapTime_Delta"]
    X["deg_x_position"] = X["Cumulative_Degradation"] * X["Position"]
    X["delta_x_position_change"] = X["LapTime_Delta"] * X["Position_Change"]
    X["tyrelife_x_position_loss"] = X["TyreLife"] * (-X["Position_Change"]).clip(
        lower=0
    )

    # Track-condition regime flags.
    is_wet = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    is_soft = (X["Compound"] == "SOFT").astype("int8")
    is_hard = (X["Compound"] == "HARD").astype("int8")
    X["is_wet_compound"] = is_wet
    X["is_soft_compound"] = is_soft
    X["is_hard_compound"] = is_hard
    X["wet_x_tyrelife"] = is_wet * X["TyreLife"]
    X["soft_x_deg"] = is_soft * X["Cumulative_Degradation"]
    X["hard_x_deg"] = is_hard * X["Cumulative_Degradation"]

    # Simple clipped versions for extreme lap-time outliers without dropping rows.
    X["LapTime_Delta_clipped"] = X["LapTime_Delta"].clip(-300, 300)
    X["Cumulative_Degradation_clipped"] = X["Cumulative_Degradation"].clip(-100, 1500)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000304.",
    }, sort_keys=True))
