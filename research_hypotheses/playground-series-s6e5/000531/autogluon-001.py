...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Basic typed copies used for deterministic hazard-style features
    X["Compound"] = X["Compound"].astype("string")
    X["Driver"] = X["Driver"].astype("string")
    X["Race"] = X["Race"].astype("string")
    age = X["TyreLife"].round().clip(lower=1).astype("int16")
    X["TyreLife_Int"] = age
    event = X["PitStop"].fillna(0).astype("float32")

    # Global hazard by tyre age: P(stop at current lap age=a)
    global_tbl = pd.DataFrame({"age": age, "event": event})
    global_agg = (
        global_tbl.groupby("age", observed=True)["event"]
        .agg(["sum", "count"])
        .reset_index()
    )
    base_rate = float(event.mean())
    smooth = 5.0
    global_agg["hazard_global"] = (global_agg["sum"] + smooth * base_rate) / (
        global_agg["count"] + smooth
    )

    # Compound-level hazard
    comp_tbl = pd.DataFrame({"Compound": X["Compound"], "age": age, "event": event})
    comp_agg = (
        comp_tbl.groupby(["Compound", "age"], observed=True)["event"]
        .agg(["sum", "count"])
        .reset_index()
    )
    comp_agg["hazard_compound"] = (comp_agg["sum"] + smooth * base_rate) / (
        comp_agg["count"] + smooth
    )

    # Compound + stint hazard for a sharper local survival proxy
    cs_tbl = pd.DataFrame(
        {"Compound": X["Compound"], "Stint": X["Stint"], "age": age, "event": event}
    )
    cs_agg = (
        cs_tbl.groupby(["Compound", "Stint", "age"], observed=True)["event"]
        .agg(["sum", "count"])
        .reset_index()
    )
    cs_agg["hazard_compound_stint"] = (cs_agg["sum"] + smooth * base_rate) / (
        cs_agg["count"] + smooth
    )

    # Expected stint end age from observed stop laps; fallback from broader groups
    stop_mask = event > 0.5
    stop_age = age[stop_mask]
    stop_compound = X.loc[stop_mask, "Compound"]
    stop_stint = X.loc[stop_mask, "Stint"]

    if len(stop_age) > 0:
        mean_end_global = float(stop_age.mean())
        end_comp = (
            pd.DataFrame({"Compound": stop_compound, "stop_age": stop_age})
            .groupby("Compound", observed=True)["stop_age"]
            .mean()
            .reset_index()
            .rename(columns={"stop_age": "mean_end_age_compound"})
        )
        end_cs = (
            pd.DataFrame(
                {"Compound": stop_compound, "Stint": stop_stint, "stop_age": stop_age}
            )
            .groupby(["Compound", "Stint"], observed=True)["stop_age"]
            .mean()
            .reset_index()
            .rename(columns={"stop_age": "mean_end_age_compound_stint"})
        )
    else:
        mean_end_global = float(age.mean())
        end_comp = pd.DataFrame(columns=["Compound", "mean_end_age_compound"])
        end_cs = pd.DataFrame(
            columns=["Compound", "Stint", "mean_end_age_compound_stint"]
        )

    # Merge expected end-age tables
    X = X.merge(end_comp, on="Compound", how="left")
    X = X.merge(end_cs, on=["Compound", "Stint"], how="left")
    X["Expected_Stint_End_Age"] = (
        X["mean_end_age_compound_stint"]
        .fillna(X["mean_end_age_compound"])
        .fillna(mean_end_global)
        .astype("float32")
    )
    X.drop(
        columns=["mean_end_age_compound_stint", "mean_end_age_compound"], inplace=True
    )

    # Build next-lap and short-horizon hazard features from empirical survival tables
    for h in (1, 2, 3):
        key = age + h

        tmp = pd.DataFrame({"Compound": X["Compound"], "Stint": X["Stint"], "age": key})
        tmp = tmp.merge(
            cs_agg[["Compound", "Stint", "age", "hazard_compound_stint"]],
            on=["Compound", "Stint", "age"],
            how="left",
        )
        tmp = tmp.merge(
            comp_agg[["Compound", "age", "hazard_compound"]],
            on=["Compound", "age"],
            how="left",
        )
        tmp = tmp.merge(
            global_agg[["age", "hazard_global"]],
            on="age",
            how="left",
        )

        hazard_h = (
            tmp["hazard_compound_stint"]
            .fillna(tmp["hazard_compound"])
            .fillna(tmp["hazard_global"])
            .fillna(base_rate)
            .clip(0.0, 1.0)
            .astype("float32")
        )
        X[f"Pit_Hazard_Plus_{h}"] = hazard_h

    # Survival-style derived features
    X["PitRisk_NextLap_SurvivalProxy"] = X["Pit_Hazard_Plus_1"]
    X["PitRisk_Within3Laps_SurvivalProxy"] = (
        1.0
        - (1.0 - X["Pit_Hazard_Plus_1"])
        * (1.0 - X["Pit_Hazard_Plus_2"])
        * (1.0 - X["Pit_Hazard_Plus_3"])
    ).astype("float32")
    X["Expected_Remaining_Laps"] = (
        (X["Expected_Stint_End_Age"] - X["TyreLife"]).clip(lower=0.0).astype("float32")
    )
    X["Stint_Progress_Ratio"] = (
        (X["TyreLife"] / X["Expected_Stint_End_Age"].replace(0, np.nan))
        .fillna(0.0)
        .clip(lower=0.0)
        .astype("float32")
    )
    X["Degradation_Per_TyreLife"] = (
        (X["Cumulative_Degradation"] / X["TyreLife"].replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
        .astype("float32")
    )
    X["LapTimeDelta_x_StintProgress"] = (
        X["LapTime_Delta"] * X["Stint_Progress_Ratio"]
    ).astype("float32")
    X["RaceProgress_x_RemainingLaps"] = (
        X["RaceProgress"] * X["Expected_Remaining_Laps"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000531"],
        "research_usage_note": "Verified assigned hypothesis 000531.",
    }, sort_keys=True))
