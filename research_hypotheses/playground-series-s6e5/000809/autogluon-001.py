...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Previous-lap pit indicator for each car, using only visible feature columns.
    out["_orig_order"] = np.arange(len(out))
    out["_prev_pit"] = (
        out.sort_values(driver_cols + ["LapNumber", "_orig_order"])
        .groupby(driver_cols, sort=False)[pit_col]
        .shift(1)
        .fillna(0)
        .astype("int8")
    )
    out = out.sort_values("_orig_order")

    # Empirical current-stop tyre-life windows by compound/stint from visible current-lap PitStop.
    out["_pit_window"] = 0
    pit_rows = out[out[pit_col] == 1]
    if len(pit_rows) > 0:
        q = (
            pit_rows.groupby(["Compound", "Stint"], observed=True)["TyreLife"]
            .quantile([0.25, 0.75])
            .unstack()
            .rename(columns={0.25: "_pit_q25", 0.75: "_pit_q75"})
            .reset_index()
        )
        out = out.merge(q, on=["Compound", "Stint"], how="left", sort=False)
        fallback_q25 = float(pit_rows["TyreLife"].quantile(0.25))
        fallback_q75 = float(pit_rows["TyreLife"].quantile(0.75))
        out["_pit_q25"] = out["_pit_q25"].fillna(fallback_q25)
        out["_pit_q75"] = out["_pit_q75"].fillna(fallback_q75)
        out["_pit_window"] = (
            (out["TyreLife"] >= out["_pit_q25"]) & (out["TyreLife"] <= out["_pit_q75"])
        ).astype("int8")
    else:
        out["_pit_q25"] = np.nan
        out["_pit_q75"] = np.nan

    out = out.sort_values(key_cols + [pos_col, "_orig_order"])
    ...
    for offset in [-2, -1, 1, 2]:
        label = f"pos_{offset:+d}".replace("+", "plus").replace("-", "minus")
        shifted = out.groupby(key_cols, sort=False)[neighbor_base].shift(-offset)
        for col in neighbor_base:
            out[f"neighbor_{label}_{col}"] = shifted[col]

    ahead_pit = ["neighbor_pos_plus1_PitStop", "neighbor_pos_plus2_PitStop"]
    behind_pit = ["neighbor_pos_minus1_PitStop", "neighbor_pos_minus2_PitStop"]
    ahead_prev = ["neighbor_pos_plus1__prev_pit", "neighbor_pos_plus2__prev_pit"]
    behind_prev = ["neighbor_pos_minus1__prev_pit", "neighbor_pos_minus2__prev_pit"]
    ahead_life = ["neighbor_pos_plus1_TyreLife", "neighbor_pos_plus2_TyreLife"]
    behind_life = ["neighbor_pos_minus1_TyreLife", "neighbor_pos_minus2_TyreLife"]
    ahead_window = ["neighbor_pos_plus1__pit_window", "neighbor_pos_plus2__pit_window"]
    behind_window = [
        "neighbor_pos_minus1__pit_window",
        "neighbor_pos_minus2__pit_window",
    ]

    out["adjacent_rivals_pitted_this_lap"] = (
        out[ahead_pit + behind_pit].fillna(0).sum(axis=1).astype("int8")
    )
    out["adjacent_rivals_pitted_prev_lap"] = (
        out[ahead_prev + behind_prev].fillna(0).sum(axis=1).astype("int8")
    )
    out["ahead_on_older_tyres_count"] = (
        out[ahead_life].sub(out["TyreLife"], axis=0).gt(0).sum(axis=1).astype("int8")
    )
    out["behind_on_newer_tyres_count"] = (
        out[behind_life].rsub(out["TyreLife"], axis=0).gt(0).sum(axis=1).astype("int8")
    )
    out["ahead_in_pit_window_count"] = (
        out[ahead_window].fillna(0).sum(axis=1).astype("int8")
    )
    out["behind_undercut_risk_count"] = (
        out[behind_window + behind_prev].fillna(0).sum(axis=1).astype("int8")
    )
    out["cover_off_pressure"] = (
        out["ahead_on_older_tyres_count"]
        + out["adjacent_rivals_pitted_prev_lap"]
        + out["behind_undercut_risk_count"]
    ).astype("int16")

    out = out.sort_values("_orig_order").drop(
        columns=["_orig_order", "_prev_pit", "_pit_window", "_pit_q25", "_pit_q75"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000809"],
        "research_usage_note": "Verified assigned hypothesis 000809.",
    }, sort_keys=True))
