...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string")
    driver = out["Driver"].astype("string")
    compound = out["Compound"].astype("string")
    year = out["Year"].astype("string")
    lap = out["LapNumber"].astype("int16")

    out["Race_Year"] = race + "_" + year
    out["Race_Year_Lap"] = out["Race_Year"] + "_L" + lap.astype("string")
    out["Race_Year_Lap_Compound"] = out["Race_Year_Lap"] + "_" + compound
    out["Driver_Year"] = driver + "_" + year
    out["Driver_Race"] = driver + "_" + race

    dry = compound.isin(["SOFT", "MEDIUM", "HARD"])
    out["compound_regime"] = np.select(
        [dry, compound.eq("INTERMEDIATE"), compound.eq("WET")],
        ["DRY", "INTER", "WET"],
        default="UNK",
    )
    out["query_regime"] = (
        out["Race_Year_Lap"]
        + "_"
        + pd.Series(out["compound_regime"], index=out.index).astype("string")
    )

    q = out.groupby("Race_Year_Lap", sort=False)
    qc = out.groupby("Race_Year_Lap_Compound", sort=False)

    out["query_size"] = q["LapNumber"].transform("size").astype("int16")
    out["compound_query_size"] = qc["LapNumber"].transform("size").astype("int16")
    ...
    for col in numeric_cols:
        ...
        mean_q = q[col].transform("mean").astype("float32")
        max_q = q[col].transform("max").astype("float32")
        min_q = q[col].transform("min").astype("float32")
        out[f"{col}_q_center"] = (s - mean_q).astype("float32")
        out[f"{col}_q_maxdiff"] = (max_q - s).astype("float32")
        out[f"{col}_q_mindiff"] = (s - min_q).astype("float32")
        out[f"{col}_q_rank_pct"] = (
            q[col].rank(method="average", pct=True).astype("float32")
        )

    for col in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta", "RaceProgress"]:
        s = out[col].astype("float32")
        cmean = qc[col].transform("mean").astype("float32")
        out[f"{col}_compound_q_center"] = (s - cmean).astype("float32")
        out[f"{col}_compound_q_rank_pct"] = (
            qc[col].rank(method="average", pct=True).astype("float32")
        )
    ...
    lap_time_delta = out["LapTime_Delta"].astype("float32")

    out["tyre_life_x_progress"] = (tyre * progress).astype("float32")
    out["tyre_life_per_stint"] = (tyre / stint.clip(lower=1)).astype("float32")
    out["degradation_per_tyre_lap"] = (deg / tyre.clip(lower=1)).astype("float32")
    out["late_stint_pressure"] = (tyre * progress * stint).astype("float32")
    out["slow_and_old_tyre"] = (lap_time_delta * tyre).astype("float32")
    out["pit_priority_proxy"] = (
        out["TyreLife_q_rank_pct"].astype("float32")
        + out["Cumulative_Degradation_q_rank_pct"].astype("float32")
        + out["LapTime_Delta_q_rank_pct"].astype("float32")
        + out["RaceProgress_q_rank_pct"].astype("float32")
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000173"],
        "research_usage_note": "Verified assigned hypothesis 000173.",
    }, sort_keys=True))
