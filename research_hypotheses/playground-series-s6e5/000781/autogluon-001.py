...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"Race", "Year"}.issubset(out.columns):
        out["Race_Year"] = (
            out["Race"].astype("string") + "_" + out["Year"].astype("string")
        )
    if {"Driver", "Compound"}.issubset(out.columns):
        out["Driver_Compound"] = (
            out["Driver"].astype("string") + "_" + out["Compound"].astype("string")
        )
    if {"Driver", "Race_Year"}.issubset(out.columns):
        out["Driver_RaceYear"] = (
            out["Driver"].astype("string") + "_" + out["Race_Year"].astype("string")
        )

    if "PitStop" not in out.columns or n_train <= 20:
        return out

    y = out["PitStop"].iloc[:n_train].astype(float)
    ...
    specs = [
        ...
        ("Compound", None, 50.0),
        ("Race_Year", "Race", 25.0),
        ("Driver_Compound", "Driver", 20.0),
        ("Driver_RaceYear", "Race_Year", 15.0),
    ]

    train_part = out.iloc[:n_train].copy()
    full_part = out.copy()

    for col, parent, smooth in specs:
        if col not in out.columns:
            continue

        enc = np.full(n, global_mean, dtype="float32")

        for f in range(5):
            fit_mask = folds != f
            val_mask = folds == f
            fit_df = train_part.loc[fit_mask, [col]].copy()
            fit_df["_y"] = y.loc[fit_mask].to_numpy()

            stats = fit_df.groupby(col, observed=True)["_y"].agg(["sum", "count"])
            if parent is not None and parent in out.columns:
                pfit = train_part.loc[fit_mask, [parent]].copy()
                pfit["_y"] = y.loc[fit_mask].to_numpy()
                pstats = pfit.groupby(parent, observed=True)["_y"].agg(["sum", "count"])
                pmean = (pstats["sum"] + smooth * global_mean) / (
                    pstats["count"] + smooth
                )
                key_parent = (
                    train_part[[col, parent]]
                    .drop_duplicates(col)
                    .set_index(col)[parent]
                )
                prior = key_parent.map(pmean).reindex(stats.index).fillna(global_mean)
            else:
                prior = global_mean

            means = (stats["sum"] + smooth * prior) / (stats["count"] + smooth)
            enc[:n_train][val_mask] = (
                train_part.loc[val_mask, col]
                .map(means)
                .fillna(global_mean)
                .to_numpy("float32")
            )

        fit_df = train_part[[col]].copy()
        fit_df["_y"] = y.to_numpy()
        stats = fit_df.groupby(col, observed=True)["_y"].agg(["sum", "count"])

        if parent is not None and parent in out.columns:
            pfit = train_part[[parent]].copy()
            pfit["_y"] = y.to_numpy()
            pstats = pfit.groupby(parent, observed=True)["_y"].agg(["sum", "count"])
            pmean = (pstats["sum"] + smooth * global_mean) / (pstats["count"] + smooth)
            key_parent = (
                train_part[[col, parent]].drop_duplicates(col).set_index(col)[parent]
            )
            prior = key_parent.map(pmean).reindex(stats.index).fillna(global_mean)
        else:
            prior = global_mean
        ...
        if n_train < n:
            enc[n_train:] = (
                full_part.loc[n_train:, col]
                .map(means)
                .fillna(global_mean)
                .to_numpy("float32")
            )

        out[f"eb_oof_{col}"] = enc
        out[f"eb_count_{col}"] = (
            full_part[col].map(stats["count"]).fillna(0).astype("float32")
        )
    ...
    if {"LapNumber", "Stint"}.issubset(out.columns):
        out["lap_per_stint"] = (
            out["LapNumber"].astype(float) / (out["Stint"].astype(float) + 1.0)
        ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000781.",
    }, sort_keys=True))
