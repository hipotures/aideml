...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    n = len(out)
    idx_name = "__orig_idx__"
    out[idx_name] = np.arange(n)

    dry_compounds = ["SOFT", "MEDIUM", "HARD"]
    wet_compounds = ["INTERMEDIATE", "WET"]

    race_s = out["Race"].astype("string").fillna("")
    comp_s = out["Compound"].astype("string").fillna("UNKNOWN").str.upper()

    out["is_dry_compound"] = comp_s.isin(dry_compounds).astype("int8")
    out["is_wet_compound"] = comp_s.isin(wet_compounds).astype("int8")
    out["is_soft"] = (comp_s == "SOFT").astype("int8")
    out["is_medium"] = (comp_s == "MEDIUM").astype("int8")
    out["is_hard"] = (comp_s == "HARD").astype("int8")
    out["is_intermediate"] = (comp_s == "INTERMEDIATE").astype("int8")
    out["is_wet"] = (comp_s == "WET").astype("int8")
    out["is_monaco"] = race_s.str.contains("Monaco", case=False, regex=False).astype(
        "int8"
    )

    sort_cols = ["Year", "Race", "Driver", "LapNumber", idx_name]
    s = out.sort_values(sort_cols, kind="mergesort").copy()
    g = s.groupby(["Year", "Race", "Driver"], sort=False, observed=True)

    s["stop_count_so_far"] = g["PitStop"].cumsum().astype("float32")
    s["prev_pitstop"] = g["PitStop"].shift(1).fillna(0).astype("int8")
    s["laps_since_stop_proxy"] = s["TyreLife"].astype("float32")
    s["race_laps_est"] = (s["LapNumber"] / s["RaceProgress"].clip(0.01, 1.0)).replace(
        [np.inf, -np.inf], np.nan
    )
    s["laps_remaining_est"] = (
        (s["race_laps_est"] - s["LapNumber"]).clip(lower=0).astype("float32")
    )

    for c in dry_compounds:
        flag = (s["Compound"].astype("string").str.upper() == c).astype("int8")
        s["used_" + c.lower() + "_so_far"] = (
            g[flag.name].transform(lambda x: flag.loc[x.index].cummax()).astype("int8")
            if False
            else flag.groupby([s["Year"], s["Race"], s["Driver"]], sort=False)
            .cummax()
            .astype("int8")
        )
    ...
    wet_seen = (
        s["is_wet_compound"]
        .groupby([s["Year"], s["Race"], s["Driver"]], sort=False)
        .cummax()
    )
    dry_types_used = (
        s["used_soft_so_far"] + s["used_medium_so_far"] + s["used_hard_so_far"]
    ).astype("int8")

    s["wet_exemption_active"] = wet_seen.astype("int8")
    s["dry_compound_types_used"] = dry_types_used
    s["dry_obligation_remaining"] = ((wet_seen == 0) & (dry_types_used < 2)).astype(
        "int8"
    )
    s["dry_obligation_pressure"] = (
        s["dry_obligation_remaining"]
        * (s["RaceProgress"].clip(0, 1) ** 2)
        * (1 + (s["laps_remaining_est"] <= 8).astype("int8"))
    ).astype("float32")
    s["monaco_rule_pressure"] = (
        s["is_monaco"]
        * ((s["stop_count_so_far"] < 2) | (dry_types_used < 3)).astype("int8")
        * s["RaceProgress"].clip(0, 1)
    ).astype("float32")

    race_lap_g = s.groupby(["Year", "Race", "LapNumber"], sort=False, observed=True)
    s["lap_delta_abs"] = s["LapTime_Delta"].abs().astype("float32")
    s["race_lap_delta_abs_median"] = (
        race_lap_g["lap_delta_abs"].transform("median").astype("float32")
    )
    s["race_lap_time_median"] = (
        race_lap_g["LapTime (s)"].transform("median").astype("float32")
    )
    s["field_position_spread"] = (
        race_lap_g["Position"].transform("std").fillna(0).astype("float32")
    )

    neutral_proxy = (
        (s["race_lap_delta_abs_median"] > 8)
        | (
            s["race_lap_time_median"]
            > s.groupby(["Year", "Race"], sort=False, observed=True)[
                "LapTime (s)"
            ].transform("median")
            * 1.12
        )
        | (s["field_position_spread"] < 3.0)
    ).astype("int8")
    s["neutralization_proxy"] = neutral_proxy

    rg = s.groupby(["Year", "Race"], sort=False, observed=True)
    prev_neutral = rg["neutralization_proxy"].shift(1).fillna(0).astype("int8")
    onset = ((s["neutralization_proxy"] == 1) & (prev_neutral == 0)).astype("int8")
    block = onset.groupby([s["Year"], s["Race"]], sort=False).cumsum()
    lap_in_block = s.groupby(
        ["Year", "Race", block], sort=False, observed=True
    ).cumcount()
    s["laps_since_neutral_onset"] = np.where(
        s["neutralization_proxy"] == 1, lap_in_block, 99
    ).astype("int16")
    s["cheap_stop_window"] = (
        (s["neutralization_proxy"] == 1)
        & (s["laps_since_neutral_onset"] <= 3)
        & (s["PitStop"] == 0)
    ).astype("int8")

    s["fsm_stop_pressure"] = (
        0.35 * s["dry_obligation_pressure"]
        + 0.30 * s["monaco_rule_pressure"]
        + 0.20 * s["cheap_stop_window"]
        + 0.10 * (s["TyreLife"].clip(0, 60) / 60.0)
        + 0.05 * (s["Stint"].clip(1, 8) / 8.0)
    ).astype("float32")

    out = s.sort_values(idx_name, kind="mergesort").drop(columns=[idx_name])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000814.",
    }, sort_keys=True))
