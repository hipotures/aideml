...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["field_ltd_median"] = g["LapTime_Delta"].transform("median").astype("float32")
    out["field_ltd_q75"] = (
        g["LapTime_Delta"].transform(lambda s: s.quantile(0.75)).astype("float32")
    )
    out["field_ltd_q90"] = (
        g["LapTime_Delta"].transform(lambda s: s.quantile(0.90)).astype("float32")
    )
    out["field_ltd_std"] = (
        g["LapTime_Delta"].transform("std").fillna(0).astype("float32")
    )
    out["field_pos_std"] = g["Position"].transform("std").fillna(0).astype("float32")
    out["field_size"] = g["Position"].transform("size").astype("float32")

    huge_delta = (ltd > 20).astype("float32")
    out["field_huge_delta_share"] = (
        huge_delta.groupby(
            [out["Year"], out["Race"], out["LapNumber"]], sort=False, observed=True
        )
        .transform("mean")
        .astype("float32")
    )
    ...
    lap_field = out[
        ...
        + [
            ...
            "field_ltd_q75",
            "field_ltd_q90",
            "field_huge_delta_share",
            "field_pos_std",
        ]
    ].drop_duplicates(keys)
    lap_field = lap_field.sort_values(["Year", "Race", "LapNumber"])
    rg = lap_field.groupby(["Year", "Race"], sort=False, observed=True)

    lap_field["field_ltd_median_prev"] = rg["field_ltd_median"].shift(1)
    lap_field["field_ltd_median_next_signal"] = rg["field_ltd_median"].shift(-1)
    lap_field["field_huge_share_prev"] = rg["field_huge_delta_share"].shift(1)
    lap_field["field_pos_std_prev"] = rg["field_pos_std"].shift(1)

    lap_field["field_ltd_median_jump"] = (
        lap_field["field_ltd_median"] - lap_field["field_ltd_median_prev"]
    ).fillna(0)
    lap_field["field_pos_std_collapse"] = (
        lap_field["field_pos_std_prev"] - lap_field["field_pos_std"]
    ).fillna(0)

    shock_score = (
        (lap_field["field_ltd_median"] > 8).astype("float32")
        + (lap_field["field_ltd_q75"] > 15).astype("float32")
        + (lap_field["field_ltd_q90"] > 25).astype("float32")
        + (lap_field["field_huge_delta_share"] > 0.25).astype("float32")
        + (lap_field["field_ltd_median_jump"] > 6).astype("float32")
        + (lap_field["field_pos_std_collapse"] > 0.75).astype("float32")
    )
    lap_field["likely_neutralized_lap"] = (shock_score >= 2).astype("float32")
    lap_field["likely_neutralized_prev"] = (
        rg["likely_neutralized_lap"].shift(1).fillna(0).astype("float32")
    )
    lap_field["likely_neutralized_persist"] = np.maximum(
        lap_field["likely_neutralized_lap"].to_numpy(),
        lap_field["likely_neutralized_prev"].to_numpy(),
    ).astype("float32")
    ...
    out = out.merge(
        lap_field[
            keys
            + [
                "field_ltd_median_prev",
                "field_huge_share_prev",
                "field_pos_std_prev",
                "field_ltd_median_jump",
                "field_pos_std_collapse",
                "likely_neutralized_lap",
                "likely_neutralized_prev",
                "likely_neutralized_persist",
            ]
        ],
        on=keys,
        how="left",
    ...
    finish_margin = (1.0 - progress).clip(0, 1)
    pit_window_core = ((tyre >= 12) & (progress >= 0.18) & (progress <= 0.88)).astype(
        "float32"
    ) + ((stint >= 2) & (tyre >= 8)).astype("float32")

    out["finish_margin"] = finish_margin.astype("float32")
    out["pit_window_core"] = pit_window_core.astype("float32")
    out["neutral_x_tyre_life"] = (out["likely_neutralized_persist"] * tyre).astype(
        "float32"
    )
    out["neutral_x_finish_margin"] = (
        out["likely_neutralized_persist"] * finish_margin
    ).astype("float32")
    out["neutral_x_pit_window"] = (
        out["likely_neutralized_persist"] * pit_window_core
    ).astype("float32")
    out["neutral_x_position"] = (out["likely_neutralized_persist"] * pos).astype(
        "float32"
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000877"],
        "research_usage_note": "Verified assigned hypothesis 000877.",
    }, sort_keys=True))
