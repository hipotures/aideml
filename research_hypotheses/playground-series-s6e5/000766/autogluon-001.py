...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if sort_cols and group_cols:
        s = out.sort_values(sort_cols, kind="mergesort").copy()
        g = s.groupby(group_cols, sort=False, observed=True)

        hist_cols = [
            c
            for c in ["LapTime_Delta", "Cumulative_Degradation", "Position_Change"]
            if c in s.columns
        ]

        for col in hist_cols:
            for lag in (1, 2, 3):
                s[f"{col}_lag{lag}"] = g[col].shift(lag)
            ...
            s[f"{col}_roll3_mean"] = r3.mean().reset_index(
            ...
            s[f"{col}_roll5_mean"] = r5.mean().reset_index(
                level=list(range(len(group_cols))), drop=True
            )
            s[f"{col}_roll5_std"] = (
                r5.std()
            ...
            lag1 = s[f"{col}_lag1"]
            lag3 = s[f"{col}_lag3"]
            s[f"{col}_slope3"] = (lag1 - lag3) / 2.0

            ew = (
                prev.groupby([s[c] for c in group_cols], sort=False, observed=True)
                .ewm(span=4, adjust=False, min_periods=1)
                .mean()
            )
            s[f"{col}_ewm4"] = ew.reset_index(
                level=list(range(len(group_cols))), drop=True
            )

        if "LapTime_Delta" in s.columns:
            s["pace_loss_ewm_slope"] = s["LapTime_Delta_lag1"] - s["LapTime_Delta_ewm4"]

        if "PitStop" in s.columns and all(
            c in s.columns for c in ["Year", "Race", "Driver", "LapNumber"]
        ):
            gr_no_stint = s.groupby(
                ["Year", "Race", "Driver"], sort=False, observed=True
            )
            pit_lap = s["LapNumber"].where(s["PitStop"].astype(float).fillna(0).gt(0))
            last_pit_lap = pit_lap.groupby(
                [s["Year"], s["Race"], s["Driver"]], sort=False, observed=True
            ).ffill()
            s["laps_since_last_pit"] = (s["LapNumber"] - last_pit_lap).fillna(
                s["LapNumber"]
            )
            s["pit_last_lap"] = gr_no_stint["PitStop"].shift(1).fillna(0)
            s["pit_two_laps_ago"] = gr_no_stint["PitStop"].shift(2).fillna(0)

        s["stint_lap_index"] = g.cumcount() + 1

        for col in hist_cols:
            s[f"{col}_lag1_missing"] = s[f"{col}_lag1"].isna().astype("int8")

        out = s.loc[orig_index]
        new_cols = out.columns.difference(df.columns)
    ...
    if {"TyreLife", "Stint"}.issubset(out.columns):
        out["tyre_life_per_stint"] = out["TyreLife"] / (out["Stint"].replace(0, np.nan))
    if {"LapNumber", "RaceProgress"}.issubset(out.columns):
        out["estimated_total_laps"] = out["LapNumber"] / out["RaceProgress"].replace(
            0, np.nan
        )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000766"],
        "research_usage_note": "Verified assigned hypothesis 000766.",
    }, sort_keys=True))
