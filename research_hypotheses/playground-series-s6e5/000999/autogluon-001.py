...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_late_race"] = (rp >= 0.66).astype("int8")
    out["is_first_stint"] = (stint <= 1).astype("int8")
    out["is_later_stint"] = (stint >= 3).astype("int8")

    out["tyre_life_x_progress"] = tyre * rp
    out["tyre_life_x_late"] = tyre * out["is_late_race"]
    out["tyre_life_x_wet"] = tyre * out["is_wet_compound"]
    out["degradation_per_tyre_lap"] = deg / (tyre + 1.0)
    out["lap_delta_per_tyre_lap"] = delta / (tyre + 1.0)
    out["progress_remaining"] = 1.0 - rp
    out["laps_per_progress"] = lap / (rp + 0.02)
    out["position_pressure"] = (21.0 - pos) * rp
    out["lost_positions_flag"] = (pos_chg > 0).astype("int8")
    out["gained_positions_flag"] = (pos_chg < 0).astype("int8")
    ...
    for col in [
        ...
        out[col + "_rank"] = s.rank(method="average", pct=True).astype("float32")

    regime_key = (
        compound.fillna("UNKNOWN")
        + "_"
        + out["race_phase"].astype("string")
        + "_"
        + out["is_first_stint"].astype("string")
    )
    out["strategy_regime"] = regime_key.astype("category")
    ...
    out["strategy_regime_freq"] = regime_key.map(
        regime_key.value_counts(normalize=True)
    ).astype("float32")

    for cat_col in ["Compound", "Race", "Driver"]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000999.",
    }, sort_keys=True))
