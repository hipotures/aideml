...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    age_since_reset = np.zeros(n, dtype=np.float32)
    reset_level = np.zeros(n, dtype=np.float32)

    lap_col = "LapTime (s)"
    key_cols = [c for c in ["Year", "Race", "Driver"] if c in out.columns]
    sort_cols = key_cols + ["LapNumber"] if "LapNumber" in out.columns else key_cols
    work = out[sort_cols + [lap_col, "PitStop", "TyreLife", "Stint", "Compound"]].copy()
    work["_idx"] = np.arange(n)
    work = work.sort_values(sort_cols + ["_idx"], kind="mergesort")
    ...
    groups = work.groupby(key_cols, sort=False) if key_cols else [(None, work)]
    for _, g in groups:
        idx = g["_idx"].to_numpy()
        y = g[lap_col].to_numpy(dtype=float)
        pits = (
            g["PitStop"].to_numpy(dtype=float) if "PitStop" in g else np.zeros(len(g))
        )
        laps = (
            g["LapNumber"].to_numpy(dtype=float)
            if "LapNumber" in g
            else np.arange(1, len(g) + 1, dtype=float)
        )
        tyres = (
        ...
        stints = (
            g["Stint"].to_numpy(dtype=float)
            if "Stint" in g
            else np.ones(len(g), dtype=float)
        )
        comps = (
            g["Compound"].astype(str).to_numpy()
            if "Compound" in g
            else np.array(["MEDIUM"] * len(g))
        )

        good = np.isfinite(y)
        init = float(np.nanmedian(y[good])) if good.any() else 90.0
        x0 = init
        x1 = 0.0
        p00, p01, p11 = 25.0, 0.0, 1.0
        last_lap = laps[0] - 1.0 if len(laps) else 0.0
        stint_age = 0.0
        last_reset = init

        for j, row_idx in enumerate(idx):
            dt = float(np.clip(laps[j] - last_lap, 1.0, 3.0))
            last_lap = laps[j]

            pred0 = x0 + dt * x1
            pred1 = x1
            q = compound_q.get(comps[j], 0.4)
            p00 = p00 + 2.0 * dt * p01 + dt * dt * p11 + q
            p01 = p01 + dt * p11
            p11 = p11 + 0.025 * q

            obs = float(np.clip(y[j], 55.0, 180.0)) if np.isfinite(y[j]) else pred0
            r = 9.0 + 0.018 * max(0.0, obs - 120.0) ** 2
            innovation = obs - pred0
            s = p00 + r
            k0 = p00 / s
            k1 = p01 / s

            x0 = pred0 + k0 * innovation
            x1 = pred1 + k1 * innovation
            p00_new = (1.0 - k0) * p00
            p01_new = (1.0 - k0) * p01
            p11_new = p11 - k1 * p01
            p00, p01, p11 = max(p00_new, 1e-4), p01_new, max(p11_new, 1e-5)

            level[row_idx] = x0
            slope[row_idx] = x1
            uncert[row_idx] = np.sqrt(max(p00, 0.0))
            resid[row_idx] = innovation
            age_since_reset[row_idx] = stint_age
            reset_level[row_idx] = last_reset

            stint_age += 1.0
            if pits[j] == 1 or (j + 1 < len(stints) and stints[j + 1] > stints[j]):
                last_reset = x0
                x0 = obs
                x1 = 0.0
                p00, p01, p11 = 16.0, 0.0, 0.8
                stint_age = 0.0

    out["kf_tyre_level"] = level
    out["kf_tyre_slope"] = slope
    out["kf_tyre_uncertainty"] = uncert
    out["kf_lap_residual"] = resid
    out["kf_age_since_reset"] = age_since_reset
    out["kf_post_pit_reset_level"] = reset_level
    out["kf_wear_pressure"] = slope * np.asarray(out["TyreLife"], dtype=np.float32)
    out["kf_noisy_wear_score"] = resid / (uncert + 1.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000540"],
        "research_usage_note": "Verified assigned hypothesis 000540.",
    }, sort_keys=True))
