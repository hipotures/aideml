...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp_risk = comp.map(compound_base).fillna(0.70).astype(float)
    ...
    out["tyre_life_x_compound_risk"] = tyre * comp_risk
    out["stint_lap_ratio"] = tyre / (lap + 1.0)
    out["race_remaining"] = 1.0 - progress
    out["late_race_old_tyre"] = progress * tyre
    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["lap_delta_abs"] = lap_delta.abs()
    out["position_change_abs"] = pos_chg.abs()
    out["pit_recent_reset_proxy"] = ((tyre <= 2) | (pit_now > 0)).astype(int)

    base_age = np.log1p(tyre) * comp_risk
    wear = np.log1p(np.maximum(degr, 0)) / 8.0
    instability = np.log1p(np.abs(lap_delta)) / 6.0 + np.abs(pos_chg) / 20.0
    phase = progress + 0.08 * np.maximum(stint - 1.0, 0)

    prev = None
    for h in (1, 2, 3, 5):
        horizon_age = np.log1p(tyre + h) * comp_risk
        score = (
            horizon_age
            + wear
            + instability
            + phase
            - 0.35 * out["pit_recent_reset_proxy"]
        )
        prob = 1.0 / (1.0 + np.exp(-score + 2.4))
        if prev is not None:
            prob = np.maximum(prob, prev)
        out[f"hazard_proxy_le_{h}lap"] = prob
        out[f"tyre_life_plus_{h}"] = tyre + h
        prev = prob

    out["hazard_proxy_2_minus_1"] = (
        out["hazard_proxy_le_2lap"] - out["hazard_proxy_le_1lap"]
    )
    out["hazard_proxy_3_minus_2"] = (
        out["hazard_proxy_le_3lap"] - out["hazard_proxy_le_2lap"]
    )
    out["hazard_proxy_5_minus_3"] = (
        out["hazard_proxy_le_5lap"] - out["hazard_proxy_le_3lap"]
    )

    for col in ["Driver", "Race", "Compound"]:
        freq = out[col].map(out[col].value_counts(dropna=False)).astype(float)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000139"],
        "research_usage_note": "Verified assigned hypothesis 000139.",
    }, sort_keys=True))
