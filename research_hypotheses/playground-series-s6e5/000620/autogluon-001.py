...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(stint_keys + ["LapNumber", "_row_order"]).reset_index(
        drop=True
    )

    grp = out.groupby(stint_keys, sort=False)

    race_lap_median = out.groupby(lap_keys, sort=False)["LapTime (s)"].transform(
        "median"
    )
    out["pace_residual_vs_race_lap_median"] = out["LapTime (s)"] - race_lap_median

    out["prev_lap_time"] = grp["LapTime (s)"].shift(1)
    out["prev_lap_delta"] = grp["LapTime_Delta"].shift(1)
    out["prev_position_change"] = grp["Position_Change"].shift(1)
    out["prev_degradation"] = grp["Cumulative_Degradation"].shift(1)

    lag_resid = grp["pace_residual_vs_race_lap_median"].shift(1)
    out["residual_lag1"] = lag_resid

    out["residual_ewm3"] = lag_resid.groupby(
        [out[k] for k in stint_keys], sort=False
    ).transform(lambda s: s.ewm(span=3, adjust=False, min_periods=1).mean())
    out["residual_ewm5"] = lag_resid.groupby(
        [out[k] for k in stint_keys], sort=False
    ).transform(lambda s: s.ewm(span=5, adjust=False, min_periods=1).mean())
    out["residual_slope"] = out.groupby(stint_keys, sort=False)["residual_ewm3"].diff()

    out["residual_vol_3"] = (
        lag_resid.groupby([out[k] for k in stint_keys], sort=False)
    ...
    pos_lag_resid = lag_resid.clip(lower=0).fillna(0)
    out["cum_positive_residual"] = pos_lag_resid.groupby(
        [out[k] for k in stint_keys], sort=False
    ).cumsum()

    lag_resid_diff = lag_resid.groupby([out[k] for k in stint_keys], sort=False).diff()
    worsening = lag_resid_diff.gt(0).fillna(False)
    segment = (~worsening).groupby([out[k] for k in stint_keys], sort=False).cumsum()
    out["laps_since_worsening_start"] = (
        worsening.astype(np.int16)
        .groupby([out[k] for k in stint_keys] + [segment], sort=False)
        .cumsum()
        .astype("float32")
    )

    out["residual_ewm_gap"] = out["residual_ewm3"] - out["residual_ewm5"]
    out["residual_vs_prev"] = lag_resid - grp["pace_residual_vs_race_lap_median"].shift(
        2
    )
    out["tyrelife_x_residual_ewm3"] = out["TyreLife"] * out["residual_ewm3"]
    out["degradation_x_residual_ewm3"] = (
        out["Cumulative_Degradation"] * out["residual_ewm3"]
    )

    out = (
        out.sort_values("_row_order").drop(columns="_row_order").reset_index(drop=True)
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000620"],
        "research_usage_note": "Verified assigned hypothesis 000620.",
    }, sort_keys=True))
