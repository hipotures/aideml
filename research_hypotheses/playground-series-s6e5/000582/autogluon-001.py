...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp_lap = out.groupby(lap_keys, sort=False, observed=True)
    lap_stats = grp_lap.agg(
        lap_field_size=("LapTime (s)", "size"),
        lap_time_mean=("LapTime (s)", "mean"),
        lap_time_std=("LapTime (s)", "std"),
        lap_delta_mean=("LapTime_Delta", "mean"),
        lap_delta_std=("LapTime_Delta", "std"),
        pos_disrupt_mean=("position_change_abs", "mean"),
        pit_burst_rate=("PitStop", "mean"),
        pit_burst_count=("PitStop", "sum"),
        wet_rate=("Compound_is_wet", "mean"),
    ).reset_index()
    lap_stats["lap_time_std"] = lap_stats["lap_time_std"].fillna(0.0)
    lap_stats["lap_delta_std"] = lap_stats["lap_delta_std"].fillna(0.0)

    race_grp = lap_stats.groupby(race_keys, sort=False, observed=True)
    for col in [
        "lap_time_mean",
        "lap_time_std",
        "lap_delta_std",
        ...
        mu = race_grp[col].transform("mean")
        sd = race_grp[col].transform("std").replace(0, np.nan).fillna(1.0)
        lap_stats[col + "_rz"] = ((lap_stats[col] - mu) / sd).clip(-5, 5)

    lap_stats["delta_compression_rz"] = (-lap_stats["lap_delta_std_rz"]).clip(-5, 5)
    lap_stats["latent_caution_score"] = (
        0.34 * lap_stats["lap_time_mean_rz"].clip(lower=0)
        + 0.18 * lap_stats["lap_time_std_rz"].clip(lower=0)
        + 0.16 * lap_stats["delta_compression_rz"].clip(lower=0)
        + 0.16 * lap_stats["pos_disrupt_mean_rz"].clip(lower=0)
        + 0.16 * lap_stats["pit_burst_rate_rz"].clip(lower=0)
    ).clip(0, 8)

    lap_stats = lap_stats.sort_values(race_keys + ["LapNumber"])
    for lag in [1, 2]:
        lap_stats[f"latent_caution_lag{lag}"] = (
            lap_stats.groupby(race_keys, sort=False, observed=True)[
                "latent_caution_score"
            ]
            .shift(lag)
            .fillna(0.0)
        )
    lap_stats["latent_caution_lead1"] = (
        lap_stats.groupby(race_keys, sort=False, observed=True)["latent_caution_score"]
        .shift(-1)
        .fillna(0.0)
    )
    lap_stats["latent_caution_roll3"] = lap_stats.groupby(
        race_keys, sort=False, observed=True
    )["latent_caution_score"].transform(lambda s: s.rolling(3, min_periods=1).mean())

    keep = lap_keys + [
        "lap_field_size",
        "lap_time_mean_rz",
        "lap_time_std_rz",
        "delta_compression_rz",
        "pos_disrupt_mean_rz",
        "pit_burst_rate_rz",
        "pit_burst_count",
        "wet_rate",
        ...
        "latent_caution_lag1",
        "latent_caution_lag2",
        "latent_caution_lead1",
        "latent_caution_roll3",
    ]
    out = out.merge(lap_stats[keep], on=lap_keys, how="left", sort=False)

    out["caution_x_tyre_life"] = out["latent_caution_score"] * out["TyreLife"]
    out["caution_x_pit_burst"] = out["latent_caution_score"] * out["pit_burst_rate_rz"]
    out["caution_x_race_progress"] = out["latent_caution_score"] * out["RaceProgress"]
    out["caution_x_wet"] = out["latent_caution_score"] * out["Compound_is_wet"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000582"],
        "research_usage_note": "Verified assigned hypothesis 000582.",
    }, sort_keys=True))
