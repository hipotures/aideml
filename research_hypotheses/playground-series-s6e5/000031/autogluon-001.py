...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Scale"] = (
        out["Compound"].map(compound_scale).fillna(1.0).astype(float)
    )
    out["TyreLife_Scaled"] = out["TyreLife"].astype(float) * out["Compound_Scale"]

    pos_delta = out["LapTime_Delta"].clip(lower=0).astype(float)
    degr = out["Cumulative_Degradation"].astype(float)
    tyre_life = out["TyreLife"].astype(float).clip(lower=1.0)
    race_progress = out["RaceProgress"].astype(float).clip(lower=0.0, upper=1.0)

    out["PosLapTimeDelta"] = pos_delta
    out["Deg_Per_TyreLife"] = degr / tyre_life
    out["Finish_Pressure"] = out["TyreLife_Scaled"] * race_progress

    out["WearPace_x_PosDelta"] = out["TyreLife_Scaled"] * pos_delta
    out["WearPace_x_Degradation"] = out["TyreLife_Scaled"] * degr
    out["WearPace_x_DegPerLife"] = out["TyreLife_Scaled"] * out["Deg_Per_TyreLife"]
    out["WearPace_x_FinishPressure"] = out["TyreLife_Scaled"] * out["Finish_Pressure"]

    out["PosDelta_to_TyreLifeScaled"] = pos_delta / (out["TyreLife_Scaled"] + 1.0)
    out["Degradation_to_TyreLifeScaled"] = degr / (out["TyreLife_Scaled"] + 1.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000031.",
    }, sort_keys=True))
