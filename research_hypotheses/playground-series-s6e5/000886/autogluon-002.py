...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    valid = out[lapt_col].notna() & out[delta_col].notna()
    pace_base = (
        out.loc[valid]
    ...
    race_lap_spread = out.groupby(race_col, observed=True)[lapt_col].transform(
        lambda s: (s - s.median()).abs().median()
    )
    race_delta_spread = out.groupby(race_col, observed=True)[delta_col].transform(
        "std"
    )
    pit_loss = (
        18.0
        + 0.08 * race_lap_spread.fillna(0.0).astype(float)
        + 1.2 * race_delta_spread.fillna(0.0).abs().astype(float)
    ).clip(12, 45)
