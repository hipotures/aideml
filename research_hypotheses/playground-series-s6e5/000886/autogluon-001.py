...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tyre_col = "TyreLife"
    lap_col = "LapNumber"
    prog_col = "RaceProgress"
    stint_col = "Stint"
    lapt_col = "LapTime (s)"
    delta_col = "LapTime_Delta"
    degr_col = "Cumulative_Degradation"

    compounds = ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]
    comp_rank = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    comp_dur = {
        "SOFT": 16.0,
        "MEDIUM": 24.0,
        "HARD": 34.0,
    ...
    comp = out[comp_col].astype(str).str.upper().str.strip()
    out[comp_col] = comp
    out["compound_rank"] = comp.map(comp_rank).fillna(2).astype("int16")
    out["compound_expected_dur"] = comp.map(comp_dur).fillna(24.0).astype("float32")

    tyre = out[tyre_col].astype(float)
    lap = out[lap_col].astype(float)
    progress = out[prog_col].astype(float).clip(0.001, 1.0)
    stint = out[stint_col].astype(float)
    pit = out[pit_col].astype(float)
    ...
    out["laps_remaining_est"] = (
        (out["race_laps_est"] - lap).clip(0, 90).astype("float32")
    )
    out["tyre_life_frac"] = (
        (tyre / out["compound_expected_dur"]).clip(0, 4).astype("float32")
    )
    out["stint_progress"] = (
        (tyre / (tyre + out["laps_remaining_est"] + 1.0)).clip(0, 1).astype("float32")
    )
    out["late_race_pressure"] = (progress * out["tyre_life_frac"]).astype("float32")

    age_bin = pd.cut(
        tyre.clip(0, 80),
        bins=[-0.1, 2, 5, 8, 12, 16, 22, 30, 45, 90],
        labels=False,
    ).astype("int16")
    out["tyre_age_bin"] = age_bin

    valid = (pit == 0) & out[lapt_col].notna() & out[delta_col].notna()
    pace_base = (
        out.loc[valid]
        .groupby([race_col, comp_col, "tyre_age_bin"], observed=True)[lapt_col]
        .median()
    )
    pace_comp = (
        out.loc[valid]
        .groupby([comp_col, "tyre_age_bin"], observed=True)[lapt_col]
        .median()
    )
    pace_global = (
        float(out.loc[valid, lapt_col].median())
        if valid.any()
        else float(out[lapt_col].median())
    )

    deg_base = (
        out.loc[valid]
        .groupby([race_col, comp_col, "tyre_age_bin"], observed=True)[delta_col]
        .median()
    )
    deg_comp = (
        out.loc[valid]
        .groupby([comp_col, "tyre_age_bin"], observed=True)[delta_col]
        .median()
    )
    deg_global = (
        float(out.loc[valid, delta_col].median())
        if valid.any()
        else float(out[delta_col].median())
    )

    keys = pd.MultiIndex.from_frame(out[[race_col, comp_col, "tyre_age_bin"]])
    comp_keys = pd.MultiIndex.from_frame(out[[comp_col, "tyre_age_bin"]])

    emp_pace = pd.Series(pace_base.reindex(keys).to_numpy(), index=out.index)
    emp_pace = emp_pace.fillna(
        pd.Series(pace_comp.reindex(comp_keys).to_numpy(), index=out.index)
    ).fillna(pace_global)
    emp_deg = pd.Series(deg_base.reindex(keys).to_numpy(), index=out.index)
    emp_deg = emp_deg.fillna(
        pd.Series(deg_comp.reindex(comp_keys).to_numpy(), index=out.index)
    ).fillna(deg_global)

    out["dp_emp_pace"] = emp_pace.astype("float32")
    out["dp_emp_degradation"] = emp_deg.astype("float32")

    race_pit_loss = (
        out.loc[pit == 1].groupby(race_col, observed=True)[lapt_col].median()
        - out.loc[valid].groupby(race_col, observed=True)[lapt_col].median()
    )
    global_pit_loss = (
        float(race_pit_loss.replace([np.inf, -np.inf], np.nan).dropna().median())
        if len(race_pit_loss)
        else 22.0
    )
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 22.0
    pit_loss = out[race_col].map(race_pit_loss).fillna(global_pit_loss).clip(12, 45)

    warmup = 1.2 + 0.35 * out["compound_rank"].astype(float)
    age_pressure = (tyre / out["compound_expected_dur"].astype(float)).clip(0, 4)
    degradation_penalty = (
        np.maximum(emp_deg.astype(float), 0) * 0.35
        + np.maximum(out[degr_col].astype(float), 0).clip(0, 500) / 900.0
    )
    current_lap_cost = (
        emp_pace.astype(float) + degradation_penalty + age_pressure * 0.55
    )

    remaining = out["laps_remaining_est"].astype(float)
    wetish = comp.isin(["INTERMEDIATE", "WET"]).astype(float)
    dry_choice = np.where(age_pressure < 0.75, 0, np.where(age_pressure < 1.15, 1, 2))
    best_future_rank = np.where(wetish > 0, out["compound_rank"], dry_choice).astype(
        float
    )
    best_future_base = (
        pd.Series(best_future_rank)
        .map(
            {
                0.0: comp_base["SOFT"],
                1.0: comp_base["MEDIUM"],
                2.0: comp_base["HARD"],
                3.0: comp_base["INTERMEDIATE"],
                4.0: comp_base["WET"],
            }
        )
        .fillna(0.0)
    ...
    post_pit_cost = (
        pace_global + best_future_base + warmup + np.maximum(0, 3 - stint) * 0.05
    )
    pit_now_q = (
        pit_loss.astype(float)
        + post_pit_cost
        + np.maximum(remaining - 1, 0) * (pace_global + best_future_base + 0.08)
    )
    ...
    wait1_q = (
        ...
        + pit_loss.astype(float)
        + post_pit_cost * 0.92
        + np.maximum(remaining - 2, 0) * (pace_global + best_future_base + 0.08)
    )
    wait2_extra = (
        current_lap_cost
        + np.maximum(emp_deg.astype(float), 0) * 0.55
        + age_pressure * 0.7
    )
    wait2_q = (
        current_lap_cost
        ...
        + pit_loss.astype(float)
        + post_pit_cost * 0.86
        + np.maximum(remaining - 3, 0) * (pace_global + best_future_base + 0.08)
    )

    q_mat = np.vstack([pit_now_q, wait1_q, wait2_q]).T
    best_action = np.argmin(q_mat, axis=1)
    best_q = np.min(q_mat, axis=1)

    out["dp_q_pit_now"] = np.asarray(pit_now_q, dtype="float32")
    out["dp_q_wait1"] = np.asarray(wait1_q, dtype="float32")
    out["dp_q_wait2"] = np.asarray(wait2_q, dtype="float32")
    out["dp_gap_wait1_minus_pit"] = np.asarray(wait1_q - pit_now_q, dtype="float32")
    out["dp_gap_wait2_minus_pit"] = np.asarray(wait2_q - pit_now_q, dtype="float32")
    out["dp_gap_wait2_minus_wait1"] = np.asarray(wait2_q - wait1_q, dtype="float32")
    out["dp_pit_now_regret"] = np.asarray(pit_now_q - best_q, dtype="float32")
    out["dp_wait1_regret"] = np.asarray(wait1_q - best_q, dtype="float32")
    out["dp_best_action"] = best_action.astype("int8")
    out["dp_best_future_compound_rank"] = np.asarray(best_future_rank, dtype="float32")
    out["dp_pit_preferred"] = (best_action == 0).astype("int8")
    out["dp_wait_preferred"] = (best_action > 0).astype("int8")
    out["dp_pit_loss_est"] = pit_loss.astype("float32")
    out["dp_action_margin"] = np.asarray(
        np.partition(q_mat, 1, axis=1)[:, 1] - best_q, dtype="float32"
    )
    out["dp_age_over_expected"] = (
        tyre - out["compound_expected_dur"].astype(float)
    ).astype("float32")
    out["dp_laps_remaining_x_age"] = (remaining * age_pressure).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000886"],
        "research_usage_note": "Verified assigned hypothesis 000886.",
    }, sort_keys=True))
