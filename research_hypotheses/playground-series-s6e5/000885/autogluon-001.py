...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = [c for c in ["Driver", "Race", "Compound"] if c in X.columns]
    for c in cat_cols:
    ...
    if "Compound" in X.columns:
        ...
        X["CompoundCode"] = X["Compound"].map(compound_map).fillna(-1).astype("int8")
        X["IsDryCompound"] = (
            X["Compound"].isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
        )
        X["IsWetCompound"] = X["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")

    for c in cat_cols:
        freq = X[c].value_counts(dropna=False)
        X[f"{c}_Freq"] = X[c].map(freq).astype("float32") / len(X)
        X[f"{c}_Rare"] = (X[f"{c}_Freq"] <= X[f"{c}_Freq"].quantile(0.1)).astype("int8")

    if "Driver" in X.columns and "Race" in X.columns:
        dr_key = X["Driver"] + "__" + X["Race"]
        dr_freq = dr_key.value_counts(dropna=False)
        X["DriverRace_Freq"] = dr_key.map(dr_freq).astype("float32") / len(X)
    ...
    for c in num_clip_cols:
        ...
        X[f"{c}_clipped"] = X[c].clip(lo, hi).astype("float32")

    if "LapTime (s)" in X.columns:
        X["LapTime_log"] = np.log1p(np.clip(X["LapTime (s)"], 0, None)).astype(
            "float32"
        )
    if "LapNumber" in X.columns:
        X["LapNumber_log"] = np.log1p(X["LapNumber"]).astype("float32")
    if "TyreLife" in X.columns:
        X["TyreLife_log"] = np.log1p(X["TyreLife"]).astype("float32")
    if "RaceProgress" in X.columns:
        X["RaceProgress_sq"] = (X["RaceProgress"] ** 2).astype("float32")
        X["RaceProgress_inv"] = (1.0 / X["RaceProgress"].clip(lower=1e-3)).astype(
            "float32"
        )

    if {"TyreLife", "LapNumber"}.issubset(X.columns):
        X["TyreLife_per_Lap"] = (X["TyreLife"] / X["LapNumber"].clip(lower=1)).astype(
            "float32"
        )
    ...
    if {"Cumulative_Degradation", "LapNumber"}.issubset(X.columns):
        X["Deg_per_Lap"] = (
            X["Cumulative_Degradation"] / X["LapNumber"].clip(lower=1)
        ).astype("float32")
    if {"LapTime_Delta", "LapTime (s)"}.issubset(X.columns):
        X["LapDelta_over_LapTime"] = (
            X["LapTime_Delta"] / X["LapTime (s)"].clip(lower=1)
        ).astype("float32")
    if {"Position", "LapNumber"}.issubset(X.columns):
        X["Position_x_LapProgress"] = (X["Position"] * X["LapNumber"]).astype("float32")
    if {"Position_Change", "RaceProgress"}.issubset(X.columns):
        X["PositionChange_x_Progress"] = (
            X["Position_Change"] * X["RaceProgress"]
        ).astype("float32")
    if {"Stint", "TyreLife"}.issubset(X.columns):
        X["TyreLife_minus_Stint"] = (X["TyreLife"] - X["Stint"]).astype("float32")

    sort_cols = [c for c in ["Year", "Race", "Driver", "LapNumber"] if c in X.columns]
    group_cols = [c for c in ["Year", "Race", "Driver"] if c in X.columns]

    if len(group_cols) >= 2 and "LapNumber" in X.columns:
        W = X.sort_values(sort_cols).copy()
        g = W.groupby(group_cols, sort=False)
        ...
        for c in lag_cols:
            lag1 = g[c].shift(1)
            W[f"{c}_lag1"] = lag1
            W[f"{c}_diff1"] = (W[c] - lag1).astype("float32")

        W["GroupLapIdx"] = g.cumcount().astype("int16")
        prev_ct = W["GroupLapIdx"].replace(0, np.nan)
        ...
        for c in [
            ...
            csum = g[c].cumsum() - W[c]
            W[f"{c}_prior_mean"] = (csum / prev_ct).astype("float32")
            W[f"{c}_minus_prior_mean"] = (W[c] - W[f"{c}_prior_mean"]).astype("float32")

        if {"PitStop", "TyreLife"}.issubset(W.columns):
            W["AfterPitFlag"] = (g["PitStop"].shift(1).fillna(0) > 0).astype("int8")
            W["TyreResetLike"] = (
                (g["TyreLife"].shift(1) > W["TyreLife"]) & (W["TyreLife"] <= 3)
            ).astype("int8")

        if {"LapTime (s)", "LapTime_Delta"}.issubset(W.columns):
            W["PaceShock"] = (
                np.abs(W["LapTime_Delta"]) / W["LapTime (s)"].clip(lower=1)
            ).astype("float32")

        X = W.sort_index()

    for c in X.columns:
        if c not in cat_cols and X[c].dtype.kind in "fc":
            X[c] = X[c].replace([np.inf, -np.inf], np.nan).astype("float32")

    return X.loc[orig_index]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000885.",
    }, sort_keys=True))
