...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Stable categorical cleanup
    for col in ["Compound", "Driver", "Race"]:
        out[col] = out[col].astype("category")

    # Race-length proxy from current lap and progress; clipped for numerical stability
    progress = out["RaceProgress"].clip(0.01, 1.0)
    est_total_laps = (out["LapNumber"] / progress).clip(1, 90)
    laps_remaining = (est_total_laps - out["LapNumber"]).clip(0, 90)

    # Compound risk ordering for degradation-sensitive hazard features
    compound_map = {
        "SOFT": 3.0,
        "MEDIUM": 2.0,
        "HARD": 1.0,
    ...
    compound_risk = out["Compound"].astype(str).map(compound_map).fillna(2.0)

    # Core recurrent survival state: stint age, remaining race, and degradation pressure
    tyre_life = out["TyreLife"].clip(0, 100)
    deg = out["Cumulative_Degradation"].clip(-500, 3000)
    lap_time_delta = out["LapTime_Delta"].clip(-3000, 3000)
    pos_change = out["Position_Change"].clip(-20, 20)
    stint = out["Stint"].clip(1, 10)

    out["est_total_laps"] = est_total_laps
    out["laps_remaining"] = laps_remaining
    out["stint_progress_ratio"] = tyre_life / (tyre_life + laps_remaining + 1.0)
    out["degradation_per_lap"] = deg / (tyre_life + 1.0)
    out["degradation_vs_remaining"] = deg / (laps_remaining + 1.0)
    out["tyre_life_vs_remaining"] = tyre_life / (laps_remaining + 1.0)
    out["stint_x_compound_risk"] = tyre_life * compound_risk
    out["deg_x_compound_risk"] = deg * compound_risk
    out["late_stint_pressure"] = (
        out["stint_progress_ratio"] * out["degradation_per_lap"] * compound_risk
    )

    # Hazard-shaped transforms to mimic next-lap event probability from AFT-style state
    z = (
        0.045 * tyre_life
        + 0.0035 * deg
        + 0.35 * compound_risk
        + 0.025 * lap_time_delta
        + 0.08 * np.maximum(-pos_change, 0)
        + 0.22 * stint
        - 0.03 * laps_remaining
    )
    z = np.clip(z, -20, 20)
    out["next_lap_hazard_proxy"] = 1.0 / (1.0 + np.exp(-z))
    ...
    aft_scale = np.exp(
        np.clip(
            ...
            - 0.0018 * deg
            - 0.10 * compound_risk
            + 0.012 * laps_remaining,
            -10,
            10,
        )
    )
    aft_scale = np.clip(aft_scale, 0.25, 50.0)
    out["aft_scale_proxy"] = aft_scale
    out["aft_next_lap_prob_proxy"] = 1.0 - np.exp(-1.0 / aft_scale)

    # Recurrent / censoring structure proxies
    out["race_end_censor_pressure"] = 1.0 / (laps_remaining + 1.0)
    out["pit_then_next_hazard"] = out["PitStop"] * out["next_lap_hazard_proxy"]
    out["pit_recovery_flag"] = ((out["PitStop"] == 1) & (tyre_life <= 2)).astype(
        np.int8
    )

    # Interaction terms that help the downstream classifier blend survival-like signals
    out["hazard_x_race_progress"] = out["next_lap_hazard_proxy"] * progress
    out["hazard_x_position"] = out["next_lap_hazard_proxy"] * out["Position"]
    out["aft_prob_x_compound"] = out["aft_next_lap_prob_proxy"] * compound_risk
    out["survival_blend_proxy"] = (
        0.6 * out["next_lap_hazard_proxy"] + 0.4 * out["aft_next_lap_prob_proxy"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000348.",
    }, sort_keys=True))
