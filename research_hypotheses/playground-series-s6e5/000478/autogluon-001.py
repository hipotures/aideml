...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def norm_name(x):
        ...
        x = re.sub(r"[^a-z0-9]+", " ", x)
        x = re.sub(r"\b(grand prix|gp|formula 1|fia|round|race)\b", " ", x)
        return re.sub(r"\s+", " ", x).strip()

    def get_json(endpoint, params, timeout=6):
        url = base + endpoint + "?" + urllib.parse.urlencode(params)
        with urllib.request.urlopen(url, timeout=timeout) as r:
            return json.loads(r.read().decode("utf-8"))

    ext_cols = [
        "of1_track_status_code",
        "of1_yellow_flag",
        "of1_safety_car",
        "of1_vsc",
        "of1_red_flag",
        ...
        "of1_air_temp",
        "of1_wind_speed",
        "of1_weather_lag1_rainfall",
        "of1_weather_lag1_track_temp",
        "of1_weather_delta_track_temp",
        "of1_weather_delta_air_temp",
        "of1_weather_delta_wind_speed",
        "of1_external_missing",
    ]
    for c in ext_cols:
        out[c] = np.nan
    out["of1_external_missing"] = 1.0

    if not {"Year", "Race", "LapNumber"}.issubset(out.columns):
        return out

    key_df = out[["Year", "Race", "LapNumber"]].drop_duplicates().copy()
    key_df["Year"] = pd.to_numeric(key_df["Year"], errors="coerce").astype("Int64")
    key_df["LapNumber"] = pd.to_numeric(key_df["LapNumber"], errors="coerce").astype(
        "Int64"
    )
    key_df["race_norm"] = key_df["Race"].map(norm_name)

    try:
        session_rows = []
        for year in sorted(key_df["Year"].dropna().unique().tolist()):
            try:
                sess = get_json("sessions", {"year": int(year), "session_type": "Race"})
                for s in sess:
                    names = [
                        s.get("meeting_name"),
                        s.get("meeting_official_name"),
                        s.get("location"),
                        s.get("country_name"),
                        s.get("session_name"),
                    ]
                    session_rows.append(
                        {
                            ...
                            "name_norms": {norm_name(v) for v in names if v},
                        }
                    )
            except Exception:
                continue
        ...
        if session_rows:
            wanted = key_df[["Year", "Race", "race_norm"]].drop_duplicates()
            matches = []
            for _, w in wanted.iterrows():
                ...
                for s in session_rows:
                    if s["Year"] != int(w["Year"]):
                        continue
                    score = max(
                        [
                            int(wr == n) * 100 + int(wr in n or n in wr) * 50
                            for n in s["name_norms"]
                            if n
                        ]
                        or [0]
                    )
                    if score > best_score:
                        best_key, best_score = s["session_key"], score
                if best_key is not None and best_score >= 50:
                    matches.append(
                        {
                            "Year": int(w["Year"]),
                            "Race": w["Race"],
            ...
            if not match_df.empty:
                for session_key in match_df["session_key"].dropna().unique().tolist():
                    try:
                        laps = pd.DataFrame(
                            get_json("laps", {"session_key": int(session_key)})
                        )
                        if (
                            laps.empty
                            or "lap_number" not in laps.columns
                            or "date_start" not in laps.columns
                        ):
                            continue
                        lap_time = laps[["lap_number", "date_start"]].dropna().copy()
                        lap_time["LapNumber"] = pd.to_numeric(
                            lap_time["lap_number"], errors="coerce"
                        ).astype("Int64")
                        lap_time["date_start"] = pd.to_datetime(
                            lap_time["date_start"], errors="coerce", utc=True
                        )
                        lap_time = lap_time.dropna(subset=["date_start", "LapNumber"])
                        lap_time = (
                            lap_time.groupby("LapNumber", as_index=False)["date_start"]
                            .median()
                            .sort_values("date_start")
                        )

                        weather = pd.DataFrame(
                            get_json("weather", {"session_key": int(session_key)})
                        )
                        if not weather.empty and "date" in weather.columns:
                            weather["date"] = pd.to_datetime(
                                weather["date"], errors="coerce", utc=True
                            )
                            weather = weather.dropna(subset=["date"]).sort_values(
                                "date"
                            )
                            ren = {
                                "rainfall": "of1_rainfall",
                                "track_temperature": "of1_track_temp",
                                "air_temperature": "of1_air_temp",
                                "wind_speed": "of1_wind_speed",
                            }
                            weather = weather.rename(columns=ren)
                            keep = ["date"] + [
                                v for v in ren.values() if v in weather.columns
                            ]
                            lap_ext = pd.merge_asof(
                                lap_time.sort_values("date_start"),
                                weather[keep],
                                left_on="date_start",
                                right_on="date",
                                direction="backward",
                            ...
                            lap_ext = lap_time.copy()

                        rc = pd.DataFrame(
                            get_json("race_control", {"session_key": int(session_key)})
                        )
                        status = pd.DataFrame({"LapNumber": lap_time["LapNumber"]})
                        status["of1_track_status_code"] = 0.0
                        status["of1_yellow_flag"] = 0.0
                        status["of1_safety_car"] = 0.0
                        status["of1_vsc"] = 0.0
                        status["of1_red_flag"] = 0.0
                        if not rc.empty and "lap_number" in rc.columns:
                            rc["LapNumber"] = pd.to_numeric(
                                rc["lap_number"], errors="coerce"
                            ).astype("Int64")
                            txt = (
                                rc.get("message", pd.Series("", index=rc.index))
                                .fillna("")
                                .astype(str)
                                + " "
                                + rc.get("flag", pd.Series("", index=rc.index))
                                .fillna("")
                                .astype(str)
                            ).str.upper()
                            rc["of1_safety_car"] = txt.str.contains(
                                "SAFETY CAR", regex=False
                            ) & ~txt.str.contains("VIRTUAL", regex=False)
                            rc["of1_vsc"] = txt.str.contains(
                                "VIRTUAL SAFETY CAR|\\bVSC\\b", regex=True
                            )
                            rc["of1_red_flag"] = txt.str.contains("RED", regex=False)
                            rc["of1_yellow_flag"] = txt.str.contains(
                                "YELLOW", regex=False
                            )
                            rc["of1_track_status_code"] = np.select(
                                [
                                    rc["of1_red_flag"],
                                    rc["of1_safety_car"],
                                    rc["of1_vsc"],
                        ...
                        lap_ext = lap_ext.merge(status, on="LapNumber", how="left")
                        for c in [
                            "of1_track_status_code",
                            "of1_yellow_flag",
                            "of1_safety_car",
                            ...
                            lap_ext[c] = lap_ext[c].fillna(0.0)
                        ...
                        for c in [
                            ...
                            if c not in lap_ext.columns:
                                lap_ext[c] = np.nan
                        lap_ext = lap_ext.sort_values("LapNumber")
                        lap_ext["of1_weather_lag1_rainfall"] = lap_ext[
                            "of1_rainfall"
                        ].shift(1)
                        lap_ext["of1_weather_lag1_track_temp"] = lap_ext[
                            "of1_track_temp"
                        ].shift(1)
                        lap_ext["of1_weather_delta_track_temp"] = lap_ext[
                            "of1_track_temp"
                        ].diff()
                        lap_ext["of1_weather_delta_air_temp"] = lap_ext[
                            "of1_air_temp"
                        ].diff()
                        lap_ext["of1_weather_delta_wind_speed"] = lap_ext[
                            "of1_wind_speed"
                        ].diff()
                        lap_ext["of1_external_missing"] = 0.0
                        lap_ext["session_key"] = session_key
                        all_ext.append(lap_ext[["session_key", "LapNumber"] + ext_cols])
                    except Exception:
                        continue

            if all_ext:
                ext = pd.concat(all_ext, ignore_index=True)
                bridge = match_df.merge(ext, on="session_key", how="inner")
                out = out.merge(
                    bridge[["Year", "Race", "LapNumber"] + ext_cols],
                    on=["Year", "Race", "LapNumber"],
                    how="left",
                    suffixes=("", "_new"),
                )
                for c in ext_cols:
                    out[c] = out[c + "_new"].combine_first(out[c])
                    out = out.drop(columns=[c + "_new"])
    except Exception:
        pass

    out["of1_external_missing"] = out["of1_external_missing"].fillna(1.0)
    for c in [
        "of1_track_status_code",
        "of1_yellow_flag",
        "of1_safety_car",
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000478"],
        "research_usage_note": "Verified assigned hypothesis 000478.",
    }, sort_keys=True))
