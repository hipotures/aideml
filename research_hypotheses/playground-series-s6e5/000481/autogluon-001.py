...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Deg_Per_TyreLife"] = out["Cumulative_Degradation"] / (
        out["TyreLife"].clip(lower=1) + eps
    )
    ...
    for col in base_cols:
        ...
        x = work[col].clip(lo, hi)
        g = x.groupby([work[c] for c in group_cols], sort=False)
        lag1 = g.shift(1)
        lag2 = g.shift(2)
        lag3 = g.shift(3)

        work[f"{col}_lag1"] = lag1
        work[f"{col}_ewm3_lag"] = g.transform(
            lambda s: s.shift(1).ewm(span=3, adjust=False, min_periods=1).mean()
        )
        work[f"{col}_slope3"] = (lag1 - lag3) / 2.0
        work[f"{col}_accel"] = lag1 - 2.0 * lag2 + lag3
        work[f"{col}_vol3"] = g.transform(
            lambda s: s.shift(1).rolling(3, min_periods=2).std()
        )

        worsening = (lag1 > lag2).astype("int8")
        reset_key = (
            (worsening.eq(0))
            .groupby([work[c] for c in group_cols], sort=False)
            .cumsum()
        )
        work[f"{col}_worse_streak"] = (
            worsening.groupby(
                [work[c] for c in group_cols] + [reset_key], sort=False
            ).cumsum()
            * worsening
        )

    work["TyreLife_x_RaceProgress"] = work["TyreLife"] * work["RaceProgress"]
    work["DegSlope_x_TyreLife"] = work["Deg_Per_TyreLife_slope3"] * work["TyreLife"]
    work["PaceVol_x_LateRace"] = work["LapTime_Delta_vol3"] * work["RaceProgress"]
    work["LateStintStress"] = (
        work["RaceProgress"] * np.log1p(work["TyreLife"].clip(lower=0)) * work["Stint"]
    )

    work = work.loc[orig_index]
    num_cols = work.select_dtypes(include=[np.number]).columns
    work[num_cols] = work[num_cols].replace([np.inf, -np.inf], np.nan).fillna(0)
    return work
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000481"],
        "research_usage_note": "Verified assigned hypothesis 000481.",
    }, sort_keys=True))
