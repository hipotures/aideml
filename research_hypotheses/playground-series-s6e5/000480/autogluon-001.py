...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["_row_order"] = np.arange(n)
    sort_cols = [race_col, year_col, driver_col, lap_col]
    s = out.sort_values(sort_cols, kind="mergesort").copy()

    s["StartCompound"] = s.groupby([race_col, year_col, driver_col], observed=True)[
        comp_col
    ].transform("first")
    s["PositionBand"] = pd.cut(
        s[pos_col].astype(float),
        bins=[0, 5, 10, 15, 20, np.inf],
        labels=["P01_05", "P06_10", "P11_15", "P16_20", "P21p"],
        include_lowest=True,
    ).astype("object")
    s["ProgressBin"] = np.clip(
        (s[prog_col].astype(float) * 20).astype(int), 0, 20
    ).astype("int16")
    s["LapBin"] = np.clip((s[lap_col].astype(float) // 3).astype(int), 0, 30).astype(
        "int16"
    )

    global_rate = float(s[pit_col].mean()) if pit_col in s else 0.0

    def add_smoothed_prior(keys, name, alpha):
        g = s.groupby(keys, observed=True)[pit_col]
        cnt = g.transform("count").astype(float)
        sm = g.transform("sum").astype(float)
        loo_cnt = np.maximum(cnt - 1.0, 0.0)
        loo_sum = sm - s[pit_col].astype(float)
        s[name] = (loo_sum + alpha * global_rate) / (loo_cnt + alpha)

    add_smoothed_prior(
        [race_col, year_col, "StartCompound", stint_col, comp_col, "ProgressBin"],
        "cohort_hazard_progress",
        20.0,
    )
    add_smoothed_prior(
        [
        ...
        "cohort_hazard_traffic",
        35.0,
    )
    add_smoothed_prior(
        [race_col, year_col, "StartCompound", stint_col, comp_col, "LapBin"],
        "cohort_hazard_lapbin",
        20.0,
    )
    ...
    prog_keys = cohort_keys + ["ProgressBin"]
    prog_sum = (
        s.groupby(prog_keys, observed=True)[pit_col]
        .sum()
        .rename("_prog_pits")
        .reset_index()
    )
    ...
    tmpl = prog_sum.merge(prog_cnt, on=prog_keys, how="left").sort_values(
        cohort_keys + ["ProgressBin"], kind="mergesort"
    )
    tmpl["_cum_pits"] = tmpl.groupby(cohort_keys, observed=True)["_prog_pits"].cumsum()
    tmpl["_cum_cnt"] = tmpl.groupby(cohort_keys, observed=True)["_prog_cnt"].cumsum()
    tmpl["cohort_cum_stop_share"] = (tmpl["_cum_pits"] + 10.0 * global_rate) / (
        tmpl["_cum_cnt"] + 10.0
    )
    s = s.merge(tmpl[prog_keys + ["cohort_cum_stop_share"]], on=prog_keys, how="left")

    pit_rows = s.loc[
        s[pit_col].astype(float) > 0, cohort_keys + [driver_col, lap_col]
    ].copy()
    pit_rows["_pit_num"] = (
        pit_rows.groupby(cohort_keys + [driver_col], observed=True).cumcount() + 1
    )
    first_second = pit_rows.loc[pit_rows["_pit_num"].isin([1, 2])]
    med = (
        first_second.groupby(cohort_keys + ["_pit_num"], observed=True)[lap_col]
        .median()
        .unstack("_pit_num")
    )
    med = med.rename(
        columns={1: "cohort_median_first_stop_lap", 2: "cohort_median_second_stop_lap"}
    ).reset_index()
    s = s.merge(med, on=cohort_keys, how="left")

    race_lap_median = s.groupby([race_col, year_col], observed=True)[lap_col].transform(
        "median"
    )
    s["cohort_median_first_stop_lap"] = s["cohort_median_first_stop_lap"].fillna(
        race_lap_median * 0.35
    )
    s["cohort_median_second_stop_lap"] = s["cohort_median_second_stop_lap"].fillna(
        race_lap_median * 0.68
    )

    s["laps_from_first_plan"] = (
        s[lap_col].astype(float) - s["cohort_median_first_stop_lap"]
    )
    s["laps_from_second_plan"] = (
        s[lap_col].astype(float) - s["cohort_median_second_stop_lap"]
    )
    s["abs_laps_from_first_plan"] = s["laps_from_first_plan"].abs()
    s["abs_laps_from_second_plan"] = s["laps_from_second_plan"].abs()
    s["tyrelife_vs_first_plan"] = (
        s[tyre_col].astype(float) - s["cohort_median_first_stop_lap"]
    )
    s["strategy_pressure"] = s["cohort_hazard_traffic"] * (
        1.0 + s[tyre_col].astype(float) / 30.0
    )
    s["late_stint_plan_pressure"] = s["cohort_cum_stop_share"] * s[stint_col].astype(
        float
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000480"],
        "research_usage_note": "Verified assigned hypothesis 000480.",
    }, sort_keys=True))
