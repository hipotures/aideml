...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["driver_year"] = out["Year"].astype(str) + "_" + out["Driver"].astype(str)
    ...
    total_laps_est = out["LapNumber"] / (out["RaceProgress"] + eps)
    out["estimated_total_laps"] = total_laps_est.clip(1, 120)
    out["laps_remaining"] = (out["estimated_total_laps"] - out["LapNumber"]).clip(
        0, 120
    )
    out["tyre_life_frac"] = out["TyreLife"] / (out["estimated_total_laps"] + eps)
    out["stint_progress"] = out["TyreLife"] / (out["LapNumber"] + eps)
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / (
        out["TyreLife"] + eps
    )
    out["degradation_per_race_lap"] = out["Cumulative_Degradation"] / (
        out["LapNumber"] + eps
    )
    out["lap_delta_per_tyre_lap"] = out["LapTime_Delta"] / (out["TyreLife"] + eps)
    out["abs_lap_delta"] = out["LapTime_Delta"].abs()
    out["abs_position_change"] = out["Position_Change"].abs()
    out["position_pressure"] = (21 - out["Position"]) / 20.0
    out["late_stint_pressure"] = out["TyreLife"] * out["RaceProgress"]
    out["pit_window_score"] = out["TyreLife"] / (out["laps_remaining"] + 1.0)
    ...
    out["is_late_race"] = (out["RaceProgress"] >= 0.65).astype("int8")
    out["is_old_tyre"] = (out["TyreLife"] >= 20).astype("int8")
    ...
    for c in [
        ...
        out[c + "_freq"] = freq.astype("float32")

    for c in ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]:
        q01, q99 = out[c].quantile([0.01, 0.99])
        out[c + "_clip"] = out[c].clip(q01, q99)
        out[c + "_extreme"] = ((out[c] < q01) | (out[c] > q99)).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000568"],
        "research_usage_note": "Verified assigned hypothesis 000568.",
    }, sort_keys=True))
