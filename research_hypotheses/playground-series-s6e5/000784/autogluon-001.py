...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out.reset_index(names="_orig_index").sort_values(
        keys + ["Position"], kind="mergesort"
    )
    g = work.groupby(keys, sort=False)

    lap = work["LapTime (s)"].astype(float)
    tyre = work["TyreLife"].astype(float)
    deg = work["Cumulative_Degradation"].astype(float)
    pos = work["Position"].astype(float)
    progress = work["RaceProgress"].astype(float)
    ...
    for offset, name in [
        ...
        nlap = g["LapTime (s)"].shift(offset).astype(float)
        ntyre = g["TyreLife"].shift(offset).astype(float)
        ndeg = g["Cumulative_Degradation"].shift(offset).astype(float)
        work[f"ctx_laptime_delta_{name}"] = lap - nlap
        work[f"ctx_tyre_age_gap_{name}"] = tyre - ntyre
        work[f"ctx_deg_gap_{name}"] = deg - ndeg

    neighbor_laps = [lap] + [
        g["LapTime (s)"].shift(s).astype(float) for s in (1, 2, -1, -2)
    ]
    neighbor_tyres = [tyre] + [
        g["TyreLife"].shift(s).astype(float) for s in (1, 2, -1, -2)
    ]
    lap_stack = np.vstack([x.to_numpy() for x in neighbor_laps])
    tyre_stack = np.vstack([x.to_numpy() for x in neighbor_tyres])

    work["ctx_local_laptime_spread"] = np.nanmax(lap_stack, axis=0) - np.nanmin(
        lap_stack, axis=0
    )
    work["ctx_local_tyre_spread"] = np.nanmax(tyre_stack, axis=0) - np.nanmin(
        tyre_stack, axis=0
    )
    work["ctx_field_laptime_spread"] = g["LapTime (s)"].transform("max") - g[
        "LapTime (s)"
    ].transform("min")
    work["ctx_field_tyre_spread"] = g["TyreLife"].transform("max") - g[
        "TyreLife"
    ].transform("min")
    work["ctx_degradation_rank_pct"] = g["Cumulative_Degradation"].rank(pct=True)
    work["ctx_tyre_age_rank_pct"] = g["TyreLife"].rank(pct=True)

    field_n = g["Position"].transform("size").astype(int)
    median_lap = g["LapTime (s)"].transform("median").astype(float).clip(lower=1.0)
    rank_idx = g.cumcount().astype(int)
    group_start = np.arange(len(work)) - rank_idx.to_numpy()

    pit_loss = 22.5
    pit_drop = np.ceil((pit_loss / median_lap.to_numpy()) * field_n.to_numpy()).astype(
        int
    )
    pit_drop = np.clip(pit_drop, 1, np.maximum(field_n.to_numpy() - 1, 1))
    target_rank = np.minimum(rank_idx.to_numpy() + pit_drop, field_n.to_numpy() - 1)
    target_abs = group_start + target_rank

    local_spread = work["ctx_local_laptime_spread"].to_numpy()
    local_tyre_spread = work["ctx_local_tyre_spread"].to_numpy()
    target_spread = local_spread[target_abs]
    target_tyre_spread = local_tyre_spread[target_abs]

    work["ctx_est_pit_drop_positions"] = pit_drop
    work["ctx_est_post_stop_position"] = np.minimum(
        pos.to_numpy() + pit_drop, field_n.to_numpy()
    )
    work["ctx_post_stop_local_laptime_spread"] = target_spread
    work["ctx_post_stop_local_tyre_spread"] = target_tyre_spread
    work["ctx_post_stop_traffic_density"] = 1.0 / (
        1.0 + np.nan_to_num(target_spread, nan=0.0)
    )
    work["ctx_clean_air_proxy"] = target_spread / (
        work["ctx_field_laptime_spread"].to_numpy() + 1e-6
    )

    work["ctx_clean_air_x_progress"] = work["ctx_clean_air_proxy"] * progress
    work["ctx_traffic_x_progress"] = work["ctx_post_stop_traffic_density"] * progress
    work["ctx_clean_air_x_position"] = work["ctx_clean_air_proxy"] * pos
    work["ctx_deg_rank_x_progress"] = work["ctx_degradation_rank_pct"] * progress

    engineered = (
        work.drop(columns=["_orig_index"]).set_index(work["_orig_index"]).sort_index()
    )
    return engineered
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000784"],
        "research_usage_note": "Verified assigned hypothesis 000784.",
    }, sort_keys=True))
