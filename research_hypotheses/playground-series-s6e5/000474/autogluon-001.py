...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pos_col = "Position"
    time_col = "LapTime (s)"
    driver_keys = race_keys + ["Driver"]
    lap_keys = race_keys + [lap_col]
    pit_loss_s = 22.0

    work = out[race_keys + ["Driver", lap_col, pos_col, time_col]].copy()
    work["_row_id"] = np.arange(n)

    lap_time = pd.to_numeric(work[time_col], errors="coerce").replace(
        [np.inf, -np.inf], np.nan
    )
    work["_lap_time_clean"] = lap_time.fillna(95.0).clip(lower=60.0, upper=240.0)

    work = work.sort_values(driver_keys + [lap_col, "_row_id"], kind="mergesort")
    work["_elapsed_time_est"] = work.groupby(driver_keys, sort=False)[
        "_lap_time_clean"
    ].cumsum()

    row_ids = work["_row_id"].to_numpy(dtype=np.int64)
    lap_clean_arr = np.empty(n, dtype="float64")
    elapsed_arr = np.empty(n, dtype="float64")
    lap_clean_arr[row_ids] = work["_lap_time_clean"].to_numpy(dtype="float64")
    elapsed_arr[row_ids] = work["_elapsed_time_est"].to_numpy(dtype="float64")
    out["lap_time_clean_for_elapsed"] = lap_clean_arr
    out["elapsed_time_est_s"] = elapsed_arr

    grid = work.sort_values(
        lap_keys + [pos_col, "_elapsed_time_est", "_row_id"], kind="mergesort"
    ).copy()
    by_lap = grid.groupby(lap_keys, sort=False)
    ahead_elapsed = by_lap["_elapsed_time_est"].shift(1)
    behind_elapsed = by_lap["_elapsed_time_est"].shift(-1)

    gap_ahead = (grid["_elapsed_time_est"] - ahead_elapsed).clip(lower=0.0, upper=300.0)
    gap_behind = (behind_elapsed - grid["_elapsed_time_est"]).clip(
        lower=0.0, upper=300.0
    )

    gap_ahead_arr = np.full(n, np.nan, dtype="float64")
    gap_behind_arr = np.full(n, np.nan, dtype="float64")
    grid_rows = grid["_row_id"].to_numpy(dtype=np.int64)
    gap_ahead_arr[grid_rows] = gap_ahead.to_numpy(dtype="float64")
    gap_behind_arr[grid_rows] = gap_behind.to_numpy(dtype="float64")

    out["gap_to_ahead_s"] = gap_ahead_arr
    out["gap_to_behind_s"] = gap_behind_arr
    out["undercut_margin_to_ahead_s"] = pit_loss_s - gap_ahead_arr
    out["pit_window_margin_to_behind_s"] = gap_behind_arr - pit_loss_s

    elapsed_grid = work.sort_values(
        lap_keys + ["_elapsed_time_est", pos_col, "_row_id"],
        kind="mergesort",
    ).reset_index(drop=True)

    eg_elapsed = elapsed_grid["_elapsed_time_est"].to_numpy(dtype="float64")
    eg_rows = elapsed_grid["_row_id"].to_numpy(dtype=np.int64)
    eg_pos = pd.to_numeric(elapsed_grid[pos_col], errors="coerce").to_numpy(
        dtype="float64"
    )

    estimated_pos = np.full(n, np.nan, dtype="float64")
    positions_lost = np.full(n, np.nan, dtype="float64")
    rejoin_gap_ahead = np.full(n, np.nan, dtype="float64")
    rejoin_gap_behind = np.full(n, np.nan, dtype="float64")
    clean_air = np.full(n, np.nan, dtype="float64")

    for idx in elapsed_grid.groupby(lap_keys, sort=False).indices.values():
        idx = np.asarray(idx, dtype=np.int64)
        e = eg_elapsed[idx]
        rows = eg_rows[idx]
        pos = eg_pos[idx]

        rejoin_elapsed = e + pit_loss_s
        insert = np.searchsorted(e, rejoin_elapsed, side="right")
        local = np.arange(len(e))

        ahead_idx = insert - 1
        ahead_idx = np.where(ahead_idx == local, local - 1, ahead_idx)
        behind_idx = insert

        ga = np.full(len(e), np.nan, dtype="float64")
        gb = np.full(len(e), np.nan, dtype="float64")

        valid_ahead = ahead_idx >= 0
        valid_behind = behind_idx < len(e)
        ga[valid_ahead] = rejoin_elapsed[valid_ahead] - e[ahead_idx[valid_ahead]]
        gb[valid_behind] = e[behind_idx[valid_behind]] - rejoin_elapsed[valid_behind]

        ca = np.minimum(
            np.where(np.isfinite(ga), ga, np.inf),
            np.where(np.isfinite(gb), gb, np.inf),
        )
        ca[~np.isfinite(ca)] = np.nan

        est = insert.astype("float64")
        estimated_pos[rows] = est
        positions_lost[rows] = np.maximum(est - pos, 0.0)
        rejoin_gap_ahead[rows] = np.clip(ga, 0.0, 300.0)
        rejoin_gap_behind[rows] = np.clip(gb, 0.0, 300.0)
        clean_air[rows] = np.clip(ca, 0.0, 300.0)

    out["estimated_rejoin_position_after_pit_loss"] = estimated_pos
    out["estimated_positions_lost_after_pit_loss"] = positions_lost
    out["rejoin_gap_to_ahead_after_pit_loss_s"] = rejoin_gap_ahead
    out["rejoin_gap_to_behind_after_pit_loss_s"] = rejoin_gap_behind
    out["clean_air_margin_after_pit_loss_s"] = clean_air
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000474"],
        "research_usage_note": "Verified assigned hypothesis 000474.",
    }, sort_keys=True))
