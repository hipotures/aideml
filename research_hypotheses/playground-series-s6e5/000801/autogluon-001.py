...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = [c for c in ["Compound", "Driver", "Race"] if c in out.columns]
    num_cols = [
        c
        for c in [
    ...
    for c in cat_cols:
        s = out[c].astype("string").fillna("MISSING").str.strip().str.upper()
        out[c] = s.astype(str)
        codes, _ = pd.factorize(s, sort=True)
        freq = s.map(s.value_counts(dropna=False)).astype("float32")
        out[f"{c}_code"] = codes.astype("int32")
        out[f"{c}_freq_log"] = np.log1p(freq).astype("float32")
        adv_parts.append(out[f"{c}_code"].astype("float32"))
        adv_parts.append(out[f"{c}_freq_log"].astype("float32"))
    ...
    for c in num_cols:
        ...
        if not np.isfinite(med):
            med = 0.0
        out[c] = s.fillna(med)
        adv_parts.append(out[c].astype("float32"))

    for cols in [("Year", "Race"), ("Race", "Driver"), ("Year", "Race", "Compound")]:
        if all(c in out.columns for c in cols):
            key = out[list(cols)].astype(str).agg("|".join, axis=1)
            freq = key.map(key.value_counts(dropna=False)).astype("float32")
            name = "freq_" + "_".join(cols)
            out[name] = np.log1p(freq).astype("float32")
            adv_parts.append(out[name].astype("float32"))

    adv_auc = None
    score = np.full(n, 0.5, dtype="float32")
    ...
    try:
        ...
        if n > 188165 and len(adv_parts) > 0:
            n_test = 188165
            n_train = n - n_test
            y_adv = np.zeros(n, dtype=np.uint8)
            y_adv[n_train:] = 1

            rng = np.random.default_rng(801)
            train_idx = np.arange(n_train)
            test_idx = np.arange(n_train, n)
            m = min(len(train_idx), len(test_idx), 180000)
            fit_idx = np.concatenate(
                [
                    rng.choice(train_idx, size=m, replace=False),
                    rng.choice(test_idx, size=m, replace=False),
                ]
            )
            rng.shuffle(fit_idx)

            X_adv = pd.concat(adv_parts, axis=1)
            X_fit = X_adv.iloc[fit_idx].to_numpy(dtype=np.float32, copy=True)
            y_fit = y_adv[fit_idx]
            ...
            X_tr = scaler.fit_transform(X_fit[tr])
            X_va = scaler.transform(X_fit[va])
            ...
            clf.fit(X_tr, y_fit[tr])

            va_pred = clf.predict_proba(X_va)[:, 1]
            adv_auc = float(roc_auc_score(y_fit[va], va_pred))

            for start in range(0, n, 200000):
                end = min(start + 200000, n)
                X_chunk = X_adv.iloc[start:end].to_numpy(dtype=np.float32, copy=True)
                score[start:end] = clf.predict_proba(scaler.transform(X_chunk))[
                    :, 1
                ].astype("float32")
    except Exception:
        adv_auc = None

    clipped = np.clip(score, 1e-4, 1.0 - 1e-4)
    out["adv_test_likeness"] = score
    out["adv_test_likeness_logit"] = np.log(clipped / (1.0 - clipped)).astype("float32")
    out["adv_test_likeness_rank"] = (
        pd.Series(score).rank(pct=True).to_numpy(dtype=np.float32)
    )

    q75, q90 = np.quantile(score, [0.75, 0.90])
    out["adv_top25_testlike"] = (score >= q75).astype("int8")
    out["adv_top10_testlike"] = (score >= q90).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000801"],
        "research_usage_note": "Verified assigned hypothesis 000801.",
    }, sort_keys=True))
