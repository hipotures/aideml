...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Race_Year_Stint"] = out["Race_Year"] + "_S" + out["Stint"].astype(str)
    ...
    for group_col in ["Race_Year", "Race_Year_Stint"]:
        g = out.groupby(group_col, sort=False)
        group_size = g["LapNumber"].transform("size").astype("float32")
        out[f"{group_col}_size"] = group_size

        for col in rank_cols:
            r = g[col].rank(method="average", pct=True).astype("float32")
            mean = g[col].transform("mean").astype("float32")
            std = g[col].transform("std").replace(0, np.nan).astype("float32")

            safe = group_col.lower()
            out[f"{col}_{safe}_rank"] = r
            out[f"{col}_{safe}_center"] = out[col].astype("float32") - mean
            out[f"{col}_{safe}_z"] = (
                ((out[col].astype("float32") - mean) / std).fillna(0).astype("float32")
            )
    ...
    out["LapTimeDelta_abs"] = out["LapTime_Delta"].abs().astype("float32")
    out["PositionChange_abs"] = out["Position_Change"].abs().astype("float32")
    out["Is_Wet_Compound"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000517"],
        "research_usage_note": "Verified assigned hypothesis 000517.",
    }, sort_keys=True))
