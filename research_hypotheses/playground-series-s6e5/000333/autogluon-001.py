...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = out["Compound"].astype("string").fillna("")
    driver = out["Driver"].astype("string").fillna("")

    is_wet_compound = compound.isin(["INTERMEDIATE", "WET"])
    is_testing = race.str.contains("Pre-Season Testing", case=False, regex=False)
    is_monaco_2025 = race.str.contains("Monaco Grand Prix", case=False, regex=False) & (
        out["Year"].astype(int) == 2025
    )

    out["wet_exception_active"] = is_wet_compound.astype("int8")
    out["testing_event"] = is_testing.astype("int8")
    out["monaco_2025_special_rule"] = is_monaco_2025.astype("int8")
    out["slick_obligation_active"] = (
        ~is_wet_compound & ~is_testing & ~is_monaco_2025
    ).astype("int8")
    out["exceptional_regime"] = (is_wet_compound | is_testing | is_monaco_2025).astype(
        "int8"
    )

    dry_gate = out["slick_obligation_active"].astype(float)
    exc_gate = out["exceptional_regime"].astype(float)

    out["race_driver"] = race + "_" + driver
    out["race_compound"] = race + "_" + compound
    out["year_race"] = out["Year"].astype("string") + "_" + race
    ...
    for col in [
        ...
        freq = (
            out[col]
            .astype("string")
            .map(out[col].astype("string").value_counts(normalize=True))
        )
    ...
    for col in num_cols:
        if col in out.columns:
            ...
            out[col + "_dry_spec"] = (x * dry_gate).astype("float32")
            out[col + "_exception_spec"] = (x * exc_gate).astype("float32")

    out["tyre_life_progress"] = (out["TyreLife"] * out["RaceProgress"]).astype(
        "float32"
    )
    out["stint_progress"] = (out["Stint"] * out["RaceProgress"]).astype("float32")
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"] + 1.0)
    ).astype("float32")
    out["lap_delta_abs"] = out["LapTime_Delta"].abs().astype("float32")
    out["dry_tyre_life_progress"] = (out["tyre_life_progress"] * dry_gate).astype(
        "float32"
    )
    out["exception_tyre_life_progress"] = (out["tyre_life_progress"] * exc_gate).astype(
        "float32"
    )
    out["dry_degradation_per_tyre_lap"] = (
        out["degradation_per_tyre_lap"] * dry_gate
    ).astype("float32")
    out["exception_degradation_per_tyre_lap"] = (
        out["degradation_per_tyre_lap"] * exc_gate
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000333"],
        "research_usage_note": "Verified assigned hypothesis 000333.",
    }, sort_keys=True))
