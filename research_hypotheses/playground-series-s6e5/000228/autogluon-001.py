...
def preprocess(df: "pd.DataFrame") -> "pd.DataFrame":
    ...
    work = work.sort_values(group_cols + ["Position", "Driver"], kind="mergesort")
    ...
    for name, shift_n in neighbor_specs.items():
        n_pos = g["Position"].shift(shift_n)
        n_tyre = g["TyreLife"].shift(shift_n)
        n_delta = g["LapTime_Delta"].shift(shift_n)
        n_degr = g["Cumulative_Degradation"].shift(shift_n)
        n_comp = g["Compound"].shift(shift_n)

        valid = n_pos.notna()
        pos_gap = (work["Position"] - n_pos).abs()
        valid = valid & (pos_gap <= 2)

        tyre_diff = (work["TyreLife"] - n_tyre).where(valid)
        delta_diff = (work["LapTime_Delta"] - n_delta).where(valid)
        degr_diff = (work["Cumulative_Degradation"] - n_degr).where(valid)
        comp_diff = (work["Compound"].ne(n_comp) & valid).astype("int8")
        older_slower = ((tyre_diff > 0) & (delta_diff > 0) & valid).astype("int8")

        work[f"rival_{name}_tyrelife_diff"] = tyre_diff.fillna(0).astype("float32")
        work[f"rival_{name}_laptime_delta_diff"] = delta_diff.fillna(0).astype(
            "float32"
        )
        work[f"rival_{name}_degradation_diff"] = degr_diff.fillna(0).astype("float32")
        work[f"rival_{name}_different_compound"] = comp_diff

        diff_flags.append(comp_diff.to_numpy())
        if name in ("ahead1", "behind1"):
            work[f"rival_{name}_older_slower"] = older_slower
            older_slower_flags.append(older_slower.to_numpy())

    tyre_diff_cols = [f"rival_{n}_tyrelife_diff" for n in neighbor_specs]
    delta_diff_cols = [f"rival_{n}_laptime_delta_diff" for n in neighbor_specs]
    degr_diff_cols = [f"rival_{n}_degradation_diff" for n in neighbor_specs]

    work["local_rival_tyrelife_diff_mean"] = (
        work[tyre_diff_cols].mean(axis=1).astype("float32")
    )
    work["local_rival_laptime_delta_diff_mean"] = (
        work[delta_diff_cols].mean(axis=1).astype("float32")
    )
    work["local_rival_degradation_diff_mean"] = (
        work[degr_diff_cols].mean(axis=1).astype("float32")
    )
    work["local_rival_different_compound_any"] = np.maximum.reduce(diff_flags).astype(
        "int8"
    )
    work["local_rival_older_slower_any"] = np.maximum.reduce(older_slower_flags).astype(
        "int8"
    )
    ...
    worn_ref = work["Compound"].map(wear_ref).fillna(18.0)
    worn_tyre = (work["TyreLife"] >= worn_ref).astype("int8")
    late_race = (work["RaceProgress"] >= 0.55).astype("int8")

    work["local_worn_tyre_flag"] = worn_tyre
    work["late_local_diffcompound_worn"] = (
        late_race * worn_tyre * work["local_rival_different_compound_any"]
    ).astype("int8")
    work["late_local_older_slower_worn"] = (
        late_race * worn_tyre * work["local_rival_older_slower_any"]
    ).astype("int8")
    work["late_ahead_diffcompound_worn"] = (
        late_race * worn_tyre * work["rival_ahead1_different_compound"]
    ).astype("int8")
    work["late_behind_diffcompound_worn"] = (
        late_race * worn_tyre * work["rival_behind1_different_compound"]
    ).astype("int8")

    new_cols = [c for c in work.columns if c not in work_cols + ["_row_id"]]
    work = work.sort_values("_row_id", kind="mergesort")
    for col in new_cols:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000228"],
        "research_usage_note": "Verified assigned hypothesis 000228.",
    }, sort_keys=True))
