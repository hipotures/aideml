...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = out["RaceProgress"].astype(float).clip(eps, 1.0)
    tyre_life = out["TyreLife"].astype(float).clip(lower=1.0)
    ...
    est_total_laps = est_total_laps.fillna(est_total_laps.median()).clip(1, 90)
    laps_remaining = (est_total_laps - lap).clip(lower=0)

    out["est_total_laps"] = est_total_laps.astype("float32")
    out["laps_remaining"] = laps_remaining.astype("float32")
    out["tyre_life_frac"] = (tyre_life / est_total_laps.clip(lower=1)).astype("float32")
    out["race_phase_x_tyre_life"] = (progress * tyre_life).astype("float32")

    deg_slope = cum_deg / tyre_life
    delta_slope = lap_delta / tyre_life
    out["deg_slope_per_lap"] = deg_slope.astype("float32")
    out["delta_slope_per_lap"] = delta_slope.astype("float32")

    keys = ["Race", "Compound"]
    race_comp_deg = out.groupby(keys)["Cumulative_Degradation"].transform("median")
    race_comp_delta = out.groupby(keys)["LapTime_Delta"].transform("median")
    race_comp_tyre = out.groupby(keys)["TyreLife"].transform("median").clip(lower=1)
    base_deg_slope = race_comp_deg / race_comp_tyre
    base_delta_slope = race_comp_delta / race_comp_tyre

    out["race_comp_deg_slope"] = base_deg_slope.astype("float32")
    out["race_comp_delta_slope"] = base_delta_slope.astype("float32")
    out["rel_deg_slope_vs_compound"] = (deg_slope - base_deg_slope).astype("float32")
    out["rel_delta_slope_vs_compound"] = (delta_slope - base_delta_slope).astype(
        "float32"
    )

    for h in (1, 2, 3):
        horizon = np.minimum(laps_remaining, h)
        out[f"stay_out_cost_{h}lap"] = (
            horizon * deg_slope + 0.5 * horizon * (horizon + 1) * base_deg_slope
        ).astype("float32")
        out[f"stay_out_delta_cost_{h}lap"] = (
            horizon * delta_slope + 0.5 * horizon * (horizon + 1) * base_delta_slope
        ).astype("float32")

    pit_loss_proxy = (
        (
            out.groupby("Race")["LapTime (s)"].transform("median")
            - out.groupby("Race")["LapTime (s)"].transform("quantile", 0.25)
        )
        .fillna(20.0)
        .clip(lower=5.0, upper=35.0)
    )

    out["pit_loss_proxy"] = pit_loss_proxy.astype("float32")
    out["implied_opt_stint_len"] = (
        np.sqrt((2.0 * pit_loss_proxy) / deg_slope.abs().clip(lower=eps))
        .clip(1, 80)
        .astype("float32")
    )
    out["stint_over_opt_ratio"] = (
        tyre_life / out["implied_opt_stint_len"].astype(float).clip(eps)
    ).astype("float32")
    out["finish_on_current_feasible"] = (
        tyre_life + laps_remaining <= out["implied_opt_stint_len"].astype(float) * 1.15
    ).astype("int8")
    out["laps_past_opt_stint"] = (
        (tyre_life - out["implied_opt_stint_len"].astype(float))
        .clip(lower=0)
        .astype("float32")
    )

    fresh_life = 1.0
    fresh_deg_expected = base_deg_slope * fresh_life
    warmup_penalty_proxy = (
        out.groupby(keys)["LapTime_Delta"].transform("quantile", 0.25).abs().clip(0, 5)
    )
    old_tyre_next_cost = deg_slope + base_deg_slope
    undercut_benefit = old_tyre_next_cost - fresh_deg_expected - warmup_penalty_proxy

    out["fresh_deg_expected"] = fresh_deg_expected.astype("float32")
    out["warmup_penalty_proxy"] = warmup_penalty_proxy.astype("float32")
    out["undercut_benefit_proxy"] = undercut_benefit.astype("float32")
    out["undercut_benefit_per_pitloss"] = (
        undercut_benefit / pit_loss_proxy.clip(lower=eps)
    ).astype("float32")
    out["pit_economics_pressure"] = (
        (out["stay_out_cost_3lap"].astype(float) + undercut_benefit)
        / pit_loss_proxy.clip(lower=eps)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000991"],
        "research_usage_note": "Verified assigned hypothesis 000991.",
    }, sort_keys=True))
