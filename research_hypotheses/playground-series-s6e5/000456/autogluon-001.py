...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["lap_frac"] = out["LapNumber"] / out.groupby("Race_Year")[
        "LapNumber"
    ].transform("max").clip(lower=1)
    out["tyre_frac"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["stint_lap_ratio"] = out["TyreLife"] / out.groupby(["Race_Year", "Stint"])[
        "TyreLife"
    ].transform("max").clip(lower=1)
    ...
    for col in [
        ...
        grp = out.groupby("Race_Year")[col]
        mean = grp.transform("mean")
        std = grp.transform("std").replace(0, np.nan)
        out[col + "_race_z"] = ((out[col] - mean) / std).fillna(0)
    ...
    out["late_stint_pressure"] = out["RaceProgress"] * out["TyreLife"]
    out["compound_stint"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000456"],
        "research_usage_note": "Verified assigned hypothesis 000456.",
    }, sort_keys=True))
