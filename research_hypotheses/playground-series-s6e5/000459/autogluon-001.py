...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    driver_freq = out["Driver"].value_counts(dropna=False)
    race_freq = out["Race"].value_counts(dropna=False)
    race_year_freq = race_year.value_counts(dropna=False)
    driver_race_freq = (
        out["Driver"].astype(str) + "_" + out["Race"].astype(str)
    ).value_counts(dropna=False)

    out["Driver_Freq"] = out["Driver"].map(driver_freq).astype("float32")
    out["Race_Freq"] = out["Race"].map(race_freq).astype("float32")
    out["Race_Year_Freq"] = race_year.map(race_year_freq).astype("float32")
    out["Driver_Race_Freq"] = (
        (out["Driver"].astype(str) + "_" + out["Race"].astype(str))
        .map(driver_race_freq)
        .astype("float32")
    )

    out["Driver_Grouped"] = out["Driver"].where(
        out["Driver_Freq"] >= 8, "__RARE_DRIVER__"
    )
    out["Race_Grouped"] = out["Race"].where(out["Race_Freq"] >= 20, "__RARE_RACE__")
    out["Race_Year_Grouped"] = race_year.where(
        out["Race_Year_Freq"] >= 12, "__RARE_RACE_YEAR__"
    )

    lap_denom = out["LapNumber"].clip(lower=1)
    prog_denom = out["RaceProgress"].clip(lower=0.01)

    out["TyreLife_Per_Lap"] = (out["TyreLife"] / lap_denom).astype("float32")
    out["TyreLife_Per_Progress"] = (out["TyreLife"] / prog_denom).astype("float32")
    ...
    out["Stint_Lap_Interaction"] = (out["Stint"] * out["LapNumber"]).astype("float32")
    out["Position_X_Progress"] = (out["Position"] * out["RaceProgress"]).astype(
        "float32"
    )
    out["PositionChange_X_Progress"] = (
        out["Position_Change"] * out["RaceProgress"]
    ).astype("float32")
    out["FreshTyreFlag"] = (out["TyreLife"] <= 2).astype("int8")
    out["LateRaceFlag"] = (out["RaceProgress"] >= 0.8).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000459.",
    }, sort_keys=True))
