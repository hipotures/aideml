...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Stint"] = out["Compound"] + "__S" + out["Stint"].astype("string")
    out["Race_Compound_Stint"] = (
        out["Race"] + "__" + out["Compound"] + "__S" + out["Stint"].astype("string")
    )
    ...
    out["tyre_life_frac_lap"] = tyre / (lap + 1.0)
    out["tyre_life_x_progress"] = tyre * progress
    out["tyre_life_x_stint"] = tyre * stint
    out["stint_progress"] = stint * progress
    out["laps_remaining_est"] = lap * (1.0 - progress) / progress.clip(lower=0.01)
    out["degradation_per_tyre_lap"] = degr / (tyre + 1.0)
    out["degradation_per_lap"] = degr / (lap + 1.0)
    out["lap_delta_per_tyre_lap"] = delta / (tyre + 1.0)
    out["lap_time_per_lap"] = lap_time / (lap + 1.0)
    out["abs_lap_delta"] = delta.abs()
    out["abs_position_change"] = pos_chg.abs()
    out["position_pressure"] = (21.0 - pos) * progress
    out["lost_position_flag"] = (pos_chg > 0).astype("int8")
    out["gained_position_flag"] = (pos_chg < 0).astype("int8")
    out["old_tyre_flag"] = (tyre >= 20).astype("int8")
    out["very_old_tyre_flag"] = (tyre >= 35).astype("int8")
    out["late_race_flag"] = (progress >= 0.70).astype("int8")
    out["wet_or_inter_flag"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000767"],
        "research_usage_note": "Verified assigned hypothesis 000767.",
    }, sort_keys=True))
