...
def add_count_frequency_features(tr, te, key_defs):
    ...
    for name, keys in key_defs.items():
        ...
        tr[count_col] = tr_key.map(counts).fillna(0).astype(float)
        te[count_col] = te_key.map(counts).fillna(0).astype(float)
        tr[freq_col] = tr[count_col] / n_all
        te[freq_col] = te[count_col] / n_all
        generated.extend([count_col, freq_col])
    return generated


freq_cols = add_count_frequency_features(train, test, key_defs)
...
def fit_target_encoding(key_values, targets, smoothing=30.0, min_count=3):
    ...
    return enc.where(stats["count"] >= min_count, global_mean)
...
generated_cols = set(freq_cols)
raw_cols = [c for c in test.columns if c != id_col]
num_cols = [c for c in raw_cols if c not in set(cat_cols) and c not in generated_cols]
...
for cols_name, cols in [
    ("base_feature_cols", base_feature_cols),
    ("te_feature_cols", te_feature_cols),
]:
    dupes = [k for k, v in Counter(cols).items() if v > 1]
    if dupes:
        raise ValueError(f"{cols_name} contains duplicate feature names: {dupes}")
...
for fold, (tr_idx, va_idx) in enumerate(outer_cv.split(X_base, y, groups), 1):
    ...
    test_preds += (
        te_model.predict_proba(X_test_te[te_feature_cols])[:, 1] / outer_cv.n_splits
    )
