...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    est_total_laps = (lap / progress).clip(lower=lap)
    remaining_laps = (est_total_laps - lap).clip(lower=0)
    prev_remaining_laps = remaining_laps + 1.0

    out["est_total_laps"] = est_total_laps
    out["remaining_laps"] = remaining_laps

    slick_order = {"SOFT": 0, "MEDIUM": 1, "HARD": 2}
    current_compound = out["Compound"].astype(str).str.upper()

    # Conservative fresh-slick final-stint life thresholds in laps.
    thresholds = {"SOFT": 18.0, "MEDIUM": 28.0, "HARD": 38.0}

    diff_open_any = np.zeros(len(out), dtype=np.int8)
    softest_diff_open = np.full(len(out), -1, dtype=np.int8)
    best_diff_headroom = np.full(len(out), -999.0, dtype=float)

    for comp, limit in thresholds.items():
        newly_feasible = (remaining_laps <= limit) & (prev_remaining_laps > limit)
        different_compound = current_compound.ne(comp)
        diff_open = newly_feasible & different_compound

        out[f"fresh_{comp.lower()}_newly_feasible"] = newly_feasible.astype(np.int8)
        out[f"fresh_{comp.lower()}_headroom"] = limit - remaining_laps

        comp_rank = slick_order[comp]
        diff_open_any = np.maximum(diff_open_any, diff_open.astype(np.int8))
        softest_diff_open = np.where(
            diff_open & ((softest_diff_open == -1) | (comp_rank < softest_diff_open)),
            comp_rank,
            softest_diff_open,
        )
        best_diff_headroom = np.where(
            diff_open,
            np.maximum(best_diff_headroom, limit - remaining_laps),
            best_diff_headroom,
        )

    out["h000016_diff_compound_final_stop_open"] = diff_open_any
    out["h000016_softest_diff_newly_feasible_slick"] = softest_diff_open
    out["h000016_best_diff_compound_headroom"] = best_diff_headroom
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000016"],
        "research_usage_note": "Verified assigned hypothesis 000016.",
    }, sort_keys=True))
