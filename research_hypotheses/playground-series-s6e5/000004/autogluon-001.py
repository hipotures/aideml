...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp = out["Compound"].astype("string").str.upper().str.strip().fillna("UNKNOWN")
    out["Compound"] = comp.astype("object")

    tyre = pd.to_numeric(out["TyreLife"], errors="coerce").astype("float64")
    ...
    est_total = (
        (lap / progress).replace([np.inf, -np.inf], np.nan).clip(upper=120).fillna(lap)
    )
    est_total = np.maximum(est_total, lap)
    remaining = pd.Series(est_total - lap, index=out.index).clip(lower=0)

    out["est_total_laps_from_progress"] = pd.Series(est_total, index=out.index).astype(
        "float32"
    )
    out["laps_remaining_from_progress"] = remaining.astype("float32")

    train_tyre = tyre.iloc[:n_train]
    qs = [0.60, 0.75, 0.90]
    global_q = train_tyre.quantile(qs).to_dict()

    stat_df = pd.DataFrame(
        {
            "Compound": comp.iloc[:n_train].to_numpy(),
            "TyreLife": train_tyre.to_numpy(),
        }
    )
    grouped = stat_df.groupby("Compound")["TyreLife"]
    counts = grouped.count()
    qtab = grouped.quantile(qs).unstack()
    alpha = 75.0

    for q, label in [(0.60, "q60"), (0.75, "q75"), (0.90, "q90")]:
        gq = float(global_q[q])
        smoothed = ((qtab[q] * counts) + alpha * gq) / (counts + alpha)
        window = comp.map(smoothed.to_dict()).astype("float64").fillna(gq)
        out[f"compound_life_{label}"] = window.astype("float32")
        out[f"tyre_life_margin_{label}"] = (window - tyre).astype("float32")
        out[f"tyre_life_ratio_{label}"] = (
            (tyre / (window + 1e-6)).clip(0, 5).astype("float32")
        )
        out[f"current_finish_margin_{label}"] = (window - tyre - remaining).astype(
            "float32"
        )
        out[f"next_stop_finish_margin_{label}"] = (window - remaining).astype("float32")
        out[f"over_compound_life_{label}"] = (tyre > window).astype("int8")
        out[f"can_finish_current_{label}"] = ((window - tyre - remaining) >= 0).astype(
            "int8"
        )
        out[f"can_finish_after_next_stop_{label}"] = ((window - remaining) >= 0).astype(
            "int8"
        )
    ...
    fixed = comp.map(fixed_map).astype("float64").fillna(float(global_q[0.75]))
    out["fixed_compound_life"] = fixed.astype("float32")
    out["fixed_life_margin"] = (fixed - tyre).astype("float32")
    out["fixed_life_ratio"] = (tyre / (fixed + 1e-6)).clip(0, 5).astype("float32")
    out["fixed_current_finish_margin"] = (fixed - tyre - remaining).astype("float32")
    out["fixed_next_stop_finish_margin"] = (fixed - remaining).astype("float32")
    out["fixed_over_life"] = (tyre > fixed).astype("int8")
    out["learned_q75_minus_fixed_life"] = (
        pd.to_numeric(out["compound_life_q75"], errors="coerce") - fixed
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000004.",
    }, sort_keys=True))
