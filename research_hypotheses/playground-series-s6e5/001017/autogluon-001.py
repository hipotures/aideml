...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected_life = comp.map(compound_expected).fillna(22.0).astype(float)
    pressure_mult = comp.map(compound_pressure).fillna(1.0).astype(float)
    ...
    estimated_total_laps = (lap_num / race_progress).clip(1, 90)
    estimated_laps_remaining = (estimated_total_laps - lap_num).clip(lower=0)

    out["estimated_laps_remaining"] = estimated_laps_remaining
    out["tyre_age_vs_expected"] = (tyre_life / expected_life).clip(0, 4)
    out["finish_tyre_deficit"] = (
        (tyre_life + estimated_laps_remaining) - expected_life
    ).clip(-40, 80)
    out["lap_delta_loss"] = lap_delta.clip(lower=0, upper=60)
    out["lap_delta_gain"] = (-lap_delta).clip(lower=0, upper=60)
    out["compound_pit_window_pressure"] = (
        out["tyre_age_vs_expected"] * pressure_mult
    ).clip(0, 5)
    out["degradation_pressure"] = (
        cum_deg.clip(lower=0, upper=800) / expected_life
    ).clip(0, 80)
    out["late_race_tyre_pressure"] = out["tyre_age_vs_expected"] * race_progress
    out["stint_age_pressure"] = out["Stint"].astype(float) * out["tyre_age_vs_expected"]
    out["position_loss_pressure"] = (
        out["Position_Change"].astype(float).clip(lower=0, upper=18)
        * out["tyre_age_vs_expected"]
    )

    for col in ["Driver", "Race", "Compound"]:
        freq = (
            out[col].astype(str).map(out[col].astype(str).value_counts(normalize=True))
        )
        out[col + "_freq"] = freq.astype(float)
    ...
    out = out.drop(columns=["driver_race"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001017"],
        "research_usage_note": "Verified assigned hypothesis 001017.",
    }, sort_keys=True))
