...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_cols = [c for c in ["Driver", "Race", "Compound"] if c in out.columns]
    for c in cat_cols:
        out[c] = (
            out[c]
            .astype("string")
    ...
    if {"Driver", "Race"}.issubset(out.columns):
        out["Driver_Race"] = (
            out["Driver"].astype("string") + "__" + out["Race"].astype("string")
        ).astype("category")
    if {"Driver", "Compound"}.issubset(out.columns):
        out["Driver_Compound"] = (
            out["Driver"].astype("string") + "__" + out["Compound"].astype("string")
        ).astype("category")
    if {"Race", "Compound"}.issubset(out.columns):
        out["Race_Compound"] = (
            out["Race"].astype("string") + "__" + out["Compound"].astype("string")
        ).astype("category")

    eps = 1e-6
    if "TyreLife" in out.columns:
        tyre = out["TyreLife"].astype(float)
        out["TyreLife_log1p"] = np.log1p(np.clip(tyre, 0, None))
        out["TyreLife_sq"] = tyre * tyre
    if {"TyreLife", "LapNumber"}.issubset(out.columns):
        lap = out["LapNumber"].astype(float)
        tyre = out["TyreLife"].astype(float)
        out["TyreLife_Lap_Ratio"] = tyre / (lap + eps)
        out["Laps_Since_Stint_Start"] = np.maximum(tyre - 1.0, 0.0)
    if {"TyreLife", "RaceProgress"}.issubset(out.columns):
        out["TyreLife_x_RaceProgress"] = out["TyreLife"].astype(float) * out[
            "RaceProgress"
        ].astype(float)
    if {"Stint", "RaceProgress"}.issubset(out.columns):
        out["Stint_x_RaceProgress"] = out["Stint"].astype(float) * out[
            "RaceProgress"
        ].astype(float)
    if {"LapTime (s)", "LapTime_Delta"}.issubset(out.columns):
        out["LapTime_Delta_Ratio"] = out["LapTime_Delta"].astype(float) / (
            out["LapTime (s)"].astype(float).abs() + eps
        )
        out["Abs_LapTime_Delta"] = out["LapTime_Delta"].astype(float).abs()
    if {"Position", "Position_Change"}.issubset(out.columns):
        out["Position_After_Change"] = out["Position"].astype(float) + out[
            "Position_Change"
        ].astype(float)
        out["Abs_Position_Change"] = out["Position_Change"].astype(float).abs()
    if {"Cumulative_Degradation", "TyreLife"}.issubset(out.columns):
        out["Deg_Per_TyreLife"] = out["Cumulative_Degradation"].astype(float) / (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000522"],
        "research_usage_note": "Verified assigned hypothesis 000522.",
    }, sort_keys=True))
