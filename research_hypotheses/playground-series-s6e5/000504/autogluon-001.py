...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    plan_stop_count = pd.Series(1.0, index=out.index)
    first_lo = pd.Series(np.nan, index=out.index)
    first_hi = pd.Series(np.nan, index=out.index)
    second_lo = pd.Series(np.nan, index=out.index)
    second_hi = pd.Series(np.nan, index=out.index)
    preferred_next_code = pd.Series(0, index=out.index)

    priors = {
        ("Italian Grand Prix", 2025): (1, 18, 26, np.nan, np.nan, "HARD"),
        ("Emilia Romagna Grand Prix", 2025): (1, 22, 30, np.nan, np.nan, "HARD"),
        ("Mexico City Grand Prix", 2023): (1, 28, 36, np.nan, np.nan, "HARD"),
        ("Dutch Grand Prix", 2023): (2, 18, 28, 43, 53, "MEDIUM"),
        ("Dutch Grand Prix", 2024): (1, 21, 31, np.nan, np.nan, "HARD"),
        ("Hungarian Grand Prix", 2023): (2, 16, 24, 42, 52, "MEDIUM"),
        ("Hungarian Grand Prix", 2024): (2, 18, 26, 43, 53, "MEDIUM"),
        ("British Grand Prix", 2023): (1, 22, 32, np.nan, np.nan, "HARD"),
        ("British Grand Prix", 2024): (1, 19, 29, np.nan, np.nan, "HARD"),
        ("Bahrain Grand Prix", 2023): (2, 12, 20, 32, 42, "HARD"),
        ("Bahrain Grand Prix", 2024): (2, 13, 21, 34, 44, "HARD"),
        ("Saudi Arabian Grand Prix", 2023): (1, 16, 24, np.nan, np.nan, "HARD"),
        ("Saudi Arabian Grand Prix", 2024): (1, 16, 24, np.nan, np.nan, "HARD"),
        ("Australian Grand Prix", 2023): (1, 18, 28, np.nan, np.nan, "HARD"),
        ("Australian Grand Prix", 2024): (1, 16, 26, np.nan, np.nan, "HARD"),
        ("Japanese Grand Prix", 2023): (2, 14, 22, 32, 42, "HARD"),
        ("Japanese Grand Prix", 2024): (2, 15, 23, 35, 45, "HARD"),
        ("Monaco Grand Prix", 2023): (1, 25, 38, np.nan, np.nan, "HARD"),
        ("Monaco Grand Prix", 2024): (1, 25, 40, np.nan, np.nan, "HARD"),
        ("Canadian Grand Prix", 2023): (1, 18, 28, np.nan, np.nan, "HARD"),
        ("Canadian Grand Prix", 2024): (1, 18, 30, np.nan, np.nan, "HARD"),
        ("Spanish Grand Prix", 2023): (2, 13, 21, 35, 45, "MEDIUM"),
        ("Spanish Grand Prix", 2024): (2, 14, 22, 37, 47, "MEDIUM"),
        ("Austrian Grand Prix", 2023): (2, 14, 22, 36, 46, "HARD"),
        ("Austrian Grand Prix", 2024): (2, 15, 23, 38, 48, "HARD"),
        ("Belgian Grand Prix", 2023): (2, 10, 18, 27, 37, "MEDIUM"),
        ("Belgian Grand Prix", 2024): (1, 18, 28, np.nan, np.nan, "HARD"),
        ("Singapore Grand Prix", 2023): (1, 20, 30, np.nan, np.nan, "HARD"),
        ("Singapore Grand Prix", 2024): (1, 20, 32, np.nan, np.nan, "HARD"),
        ("Qatar Grand Prix", 2023): (3, 16, 20, 34, 40, "HARD"),
        ("Qatar Grand Prix", 2024): (2, 14, 22, 34, 44, "HARD"),
        ("United States Grand Prix", 2023): (2, 16, 24, 36, 46, "HARD"),
        ("United States Grand Prix", 2024): (1, 19, 29, np.nan, np.nan, "HARD"),
        ("Abu Dhabi Grand Prix", 2023): (1, 18, 28, np.nan, np.nan, "HARD"),
        ("Abu Dhabi Grand Prix", 2024): (1, 18, 30, np.nan, np.nan, "HARD"),
    }
    comp_code = {"SOFT": 1, "MEDIUM": 2, "HARD": 3, "INTERMEDIATE": 4, "WET": 5}

    for (r, y), vals in priors.items():
        m = (race == r) & (year == y)
        if m.any():
            sc, f1, f2, s1, s2, nxt = vals
            plan_stop_count.loc[m] = sc
            first_lo.loc[m] = f1
            first_hi.loc[m] = f2
            second_lo.loc[m] = s1
            second_hi.loc[m] = s2
            preferred_next_code.loc[m] = comp_code.get(nxt, 0)

    fallback_first_center = (0.36 * lap / progress.replace(0, np.nan)).clip(12, 32)
    first_lo = first_lo.fillna((fallback_first_center - 5).clip(1, 78))
    first_hi = first_hi.fillna((fallback_first_center + 5).clip(1, 78))
    second_lo = second_lo.fillna((first_hi + 14).clip(1, 78))
    second_hi = second_hi.fillna((first_hi + 24).clip(1, 78))

    d_first = np.where(
        lap < first_lo, first_lo - lap, np.where(lap > first_hi, lap - first_hi, 0.0)
    )
    d_second = np.where(
        lap < second_lo,
        second_lo - lap,
        np.where(lap > second_hi, lap - second_hi, 0.0),
    )
    use_second = plan_stop_count >= 2
    nearest = np.where(use_second, np.minimum(d_first, d_second), d_first)

    out["strategy_plan_stop_count"] = plan_stop_count.astype(float)
    out["strategy_first_window_start"] = first_lo.astype(float)
    out["strategy_first_window_end"] = first_hi.astype(float)
    out["strategy_second_window_start"] = second_lo.astype(float)
    out["strategy_second_window_end"] = second_hi.astype(float)
    out["distance_to_nearest_strategy_window"] = nearest.astype(float)
    out["inside_plan_a_window"] = ((lap >= first_lo) & (lap <= first_hi)).astype(int)
    out["inside_any_strategy_window"] = (
        ((lap >= first_lo) & (lap <= first_hi))
        | (use_second & (lap >= second_lo) & (lap <= second_hi))
    ).astype(int)
    out["plan_stop_count_pressure"] = (plan_stop_count - (stint - 1)).astype(float)
    out["strategy_window_tyrelife_gap"] = (tyre - first_lo).astype(float)
    out["preferred_next_compound_code"] = preferred_next_code.astype(int)
    out["current_is_preferred_strategy_compound"] = (
        compound.map(comp_code).fillna(0).eq(preferred_next_code).astype(int)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000504.",
    }, sort_keys=True))
