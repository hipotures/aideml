...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = out.groupby(gcols, sort=False, observed=True)

    out["race_lap_delta_median"] = (
        grp["LapTime_Delta"].transform("median").astype("float32")
    )
    out["race_lap_delta_std"] = (
        grp["LapTime_Delta"].transform("std").fillna(0).astype("float32")
    )
    out["race_lap_delta_iqr"] = (
        grp["LapTime_Delta"]
    ...
    out["_pos_delta"] = pos_delta
    out["_is_pos_delta"] = (delta > 0).astype("float32")
    out["_is_extreme_slow"] = (delta > 10).astype("float32")
    grp2 = out.groupby(gcols, sort=False, observed=True)

    out["race_lap_pos_delta_share"] = (
        grp2["_is_pos_delta"].transform("mean").astype("float32")
    )
    out["race_lap_extreme_slow_rate"] = (
        grp2["_is_extreme_slow"].transform("mean").astype("float32")
    )
    out["race_lap_pos_delta_q90"] = (
        grp2["_pos_delta"]
    ...
    out["delta_minus_race_lap_median"] = (delta - out["race_lap_delta_median"]).astype(
        "float32"
    )
    out["race_lap_shock_score"] = (
        out["race_lap_delta_median"].clip(lower=0)
        + out["race_lap_pos_delta_q90"]
        + 5.0 * out["race_lap_extreme_slow_rate"]
    ).astype("float32")

    compound = out["Compound"].astype(str).str.upper()
    soft_pressure = compound.map(
        {"SOFT": 1.0, "MEDIUM": 0.6, "HARD": 0.3, "INTERMEDIATE": 0.8, "WET": 0.9}
    ).fillna(0.5)
    tyre_pressure = np.log1p(out["TyreLife"].astype("float32")) * (
        1.0 + 0.15 * out["Stint"].astype("float32")
    )

    out["shock_x_tyre_pressure"] = (out["race_lap_shock_score"] * tyre_pressure).astype(
        "float32"
    )
    out["shock_x_compound_pressure"] = (
        out["race_lap_shock_score"] * soft_pressure.astype("float32")
    ).astype("float32")

    return out.drop(columns=["_pos_delta", "_is_pos_delta", "_is_extreme_slow"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000042"],
        "research_usage_note": "Verified assigned hypothesis 000042.",
    }, sort_keys=True))
