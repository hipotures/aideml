...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Stable categorical group identifier so tree models can learn lap-level context directly.
    X["LapGroup"] = (
        X["Year"].astype(str)
        + "_"
        + X["Race"].astype(str)
    ...
    # Small-cardinality, leakage-safe group size feature.
    X["LapFieldSize"] = grp["Driver"].transform("size").astype("int16")
    ...
    for col in base_cols:
        ...
        grange = (gmax - gmin).replace(0, 1.0)

        X[f"{col}_lap_rank_pct"] = (
            grp[col].rank(method="average", pct=True).astype("float32")
        )
        X[f"{col}_lap_centered"] = (X[col] - gmean).astype("float32")
        X[f"{col}_lap_z"] = ((X[col] - gmean) / (gstd + 1e-6)).astype("float32")
        X[f"{col}_lap_minmax"] = ((X[col] - gmin) / (grange + 1e-6)).astype("float32")

    # Compact interactions aligned with "who is most likely to pit this lap?"
    X["TyreDeg_x_Delta"] = (X["TyreLife"] * X["LapTime_Delta"]).astype("float32")
    X["Deg_x_PosLoss"] = (X["Cumulative_Degradation"] * (-X["Position_Change"])).astype(
        "float32"
    )
    X["Tyre_vs_Progress"] = (X["TyreLife"] / (X["RaceProgress"] + 1e-3)).astype(
        "float32"
    )
    X["Stint_vs_Progress"] = (X["Stint"] / (X["RaceProgress"] + 1e-3)).astype("float32")

    # Sidecar-inspired composite rank score: higher means more "pit-ready" relative to same-lap peers.
    X["PitPressureRank"] = (
        0.35 * X["TyreLife_lap_rank_pct"]
        + 0.30 * X["Cumulative_Degradation_lap_rank_pct"]
        + 0.20 * X["LapTime_Delta_lap_rank_pct"]
        + 0.10 * X["Position_lap_rank_pct"]
        + 0.05 * (1.0 - X["Position_Change_lap_rank_pct"])
    ).astype("float32")

    # A few bounded categorical-normalized combinations for non-linear learners.
    X["Compound_Stint"] = X["Compound"].astype(str) + "_" + X["Stint"].astype(str)
    X["Compound_LapPhase"] = (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000537.",
    }, sort_keys=True))
