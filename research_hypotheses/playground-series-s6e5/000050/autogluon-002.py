...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_agg["rc_lap_count"] = lap_agg["lap_count"].astype(float)
    lap_agg["rc_prior_count"] = lap_agg.groupby(key_rc, dropna=False)[
        "lap_count"
    ].transform(lambda s: s.cumsum().shift(1)).fillna(0.0)
    for feat in base_feats:
        sum_col = f"{feat}_sum"
        lap_agg[f"{feat}_rc_lap_mean"] = lap_agg[sum_col] / lap_agg[
            "lap_count"
        ].replace(0, np.nan)
        csum = lap_agg.groupby(key_rc, dropna=False)[sum_col].transform(
            lambda s: s.cumsum().shift(1)
        )
        ccnt = lap_agg["rc_prior_count"].replace(0.0, np.nan)
        lap_agg[f"{feat}_rc_prior_mean"] = csum / ccnt

    prior_cols = (
        key_rcl
        + [f"{f}_rc_lap_mean" for f in base_feats]
        + [f"{f}_rc_prior_mean" for f in base_feats]
        + ["rc_lap_count", "rc_prior_count"]
    )
    X = X.merge(lap_agg[prior_cols], on=key_rcl, how="left", sort=False)

    for feat in base_feats:
        g = X[f"{feat}_compound_global_mean"]
        rc_m = X[f"{feat}_rc_lap_mean"]
        pr_m = X[f"{feat}_rc_prior_mean"]

        X[f"{feat}_rc_smooth"] = (X["rc_lap_count"] * rc_m + shrink * g) / (
            X["rc_lap_count"] + shrink
        )
    ...
    X["rc_count_log"] = np.log1p(X["rc_lap_count"])
