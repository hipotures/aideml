...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key_rc = ["Race", "Compound"]
    key_rcl = ["Race", "Compound", "LapNumber"]
    base_feats = ["TyreLife", "Cumulative_Degradation", "LapTime_Delta"]
    shrink = 12.0
    ...
    global_compound = (
        X.groupby("Compound", dropna=False)[base_feats]
        .mean()
        .rename(columns={c: f"{c}_compound_global_mean" for c in base_feats})
        .reset_index()
    )
    X = X.merge(global_compound, on="Compound", how="left", sort=False)

    rc_mean = (
        X.groupby(key_rc, dropna=False)[base_feats]
        .mean()
        .rename(columns={c: f"{c}_rc_mean" for c in base_feats})
        .reset_index()
    )
    rc_count = X.groupby(key_rc, dropna=False).size().rename("rc_count").reset_index()
    X = X.merge(rc_mean, on=key_rc, how="left", sort=False)
    X = X.merge(rc_count, on=key_rc, how="left", sort=False)

    lap_agg = (
        X.groupby(key_rcl, dropna=False)
        .agg(
            TyreLife_sum=("TyreLife", "sum"),
            Cumulative_Degradation_sum=("Cumulative_Degradation", "sum"),
            LapTime_Delta_sum=("LapTime_Delta", "sum"),
            lap_count=("TyreLife", "size"),
        )
        .reset_index()
        .sort_values(key_rcl, kind="mergesort")
    )

    for feat in base_feats:
        sum_col = f"{feat}_sum"
        csum = lap_agg.groupby(key_rc, dropna=False)[sum_col].cumsum().shift(1)
        ccnt = lap_agg.groupby(key_rc, dropna=False)["lap_count"].cumsum().shift(1)
        lap_agg[f"{feat}_rc_prior_mean"] = csum / ccnt

    lap_agg["rc_prior_count"] = (
        lap_agg.groupby(key_rc, dropna=False)["lap_count"].cumsum().shift(1).fillna(0)
    )

    prior_cols = (
        key_rcl + [f"{f}_rc_prior_mean" for f in base_feats] + ["rc_prior_count"]
    )
    X = X.merge(lap_agg[prior_cols], on=key_rcl, how="left", sort=False)

    for feat in base_feats:
        g = X[f"{feat}_compound_global_mean"]
        rc_m = X[f"{feat}_rc_mean"]
        pr_m = X[f"{feat}_rc_prior_mean"]

        X[f"{feat}_rc_smooth"] = (X["rc_count"] * rc_m + shrink * g) / (
            X["rc_count"] + shrink
        )
        X[f"{feat}_rc_prior_smooth"] = (
            X["rc_prior_count"] * pr_m.fillna(g) + shrink * g
        ) / (X["rc_prior_count"] + shrink)

        X[f"{feat}_minus_rc_baseline"] = X[feat] - X[f"{feat}_rc_smooth"]
        X[f"{feat}_minus_rc_prior_baseline"] = X[feat] - X[f"{feat}_rc_prior_smooth"]

    X["rc_count_log"] = np.log1p(X["rc_count"])
    X["rc_prior_count_log"] = np.log1p(X["rc_prior_count"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000050"],
        "research_usage_note": "Verified assigned hypothesis 000050.",
    }, sort_keys=True))
