...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap = (
        ...
        .agg(
            rl_field_size=("LapNumber", "size"),
            rl_pitters=("PitStop", "sum"),
            rl_lap_delta_mean=("LapTime_Delta", "mean"),
            rl_lap_delta_std=("LapTime_Delta", "std"),
            rl_degradation_mean=("Cumulative_Degradation", "mean"),
            rl_degradation_std=("Cumulative_Degradation", "std"),
        )
        .reset_index()
    )

    lap["rl_lap_delta_std"] = lap["rl_lap_delta_std"].fillna(0.0)
    lap["rl_degradation_std"] = lap["rl_degradation_std"].fillna(0.0)
    lap["rl_pit_frac"] = lap["rl_pitters"] / lap["rl_field_size"].replace(0, np.nan)

    compounds = ["HARD", "MEDIUM", "SOFT", "INTERMEDIATE", "WET"]
    comp = out["Compound"].astype("string").str.upper()
    comp_frame = out[keys].copy()
    frac_cols = []
    for c in compounds:
        col = "rl_frac_" + c.lower()
        comp_frame[col] = (comp == c).astype("float32")
        frac_cols.append(col)

    comp_frac = comp_frame.groupby(keys, sort=False)[frac_cols].mean().reset_index()
    lap = lap.merge(comp_frac, on=keys, how="left", sort=False)
    lap[frac_cols] = lap[frac_cols].fillna(0.0)

    lap = lap.sort_values(keys).reset_index(drop=True)
    race_group = lap.groupby(["Year", "Race"], sort=False)

    for i in (1, 2, 3):
        lap[f"rl_pitters_prev{i}"] = race_group["rl_pitters"].shift(i).fillna(0.0)
        lap[f"rl_pit_frac_prev{i}"] = race_group["rl_pit_frac"].shift(i).fillna(0.0)

    lap["rl_pitters_prev3_sum"] = lap[
        ["rl_pitters_prev1", "rl_pitters_prev2", "rl_pitters_prev3"]
    ].sum(axis=1)
    lap["rl_pit_frac_prev3_mean"] = lap[
        ["rl_pit_frac_prev1", "rl_pit_frac_prev2", "rl_pit_frac_prev3"]
    ].mean(axis=1)
    ...
    for col in shift_cols:
        prev = race_group[col].shift(1)
        lap[f"{col}_shift1"] = (lap[col] - prev).fillna(0.0)
    ...
    latent_cols = (
        ...
        + frac_cols
        + [f"{c}_shift1" for c in shift_cols]
    )
    ...
    X = np.clip((X - mu) / sigma, -8.0, 8.0)

    n_states = 3 if len(lap) >= 3 else max(1, len(lap))
    if n_states == 1:
        post = np.ones((len(lap), 1), dtype="float64")
    else:
        try:
            gm = GaussianMixture(
            ...
            emission = np.clip(gm.fit(X).predict_proba(X), 1e-8, 1.0)
        except Exception:
            score = X[:, 0] + 0.5 * X[:, 1] + 0.25 * X[:, 4]
            ranks = pd.qcut(
                pd.Series(score).rank(method="first"),
                q=n_states,
                labels=False,
                duplicates="drop",
            ).to_numpy()
            emission = np.full((len(lap), n_states), 1e-3, dtype="float64")
            emission[np.arange(len(lap)), np.nan_to_num(ranks, nan=0).astype(int)] = 1.0

        stay = 0.88
        trans = np.full(
            (n_states, n_states), (1.0 - stay) / (n_states - 1), dtype="float64"
        )
        np.fill_diagonal(trans, stay)
        post = np.zeros_like(emission)

        for _, idx in lap.groupby(["Year", "Race"], sort=False).indices.items():
            idx = np.asarray(idx)
            e = emission[idx]
            t = len(idx)

            alpha = np.zeros((t, n_states), dtype="float64")
            beta = np.ones((t, n_states), dtype="float64")

            alpha[0] = e[0] / e[0].sum()
            for j in range(1, t):
                alpha[j] = (alpha[j - 1] @ trans) * e[j]
                alpha[j] = alpha[j] / max(alpha[j].sum(), 1e-12)

            for j in range(t - 2, -1, -1):
                beta[j] = trans @ (e[j + 1] * beta[j + 1])
                beta[j] = beta[j] / max(beta[j].sum(), 1e-12)

            seq_post = alpha * beta
            seq_post = seq_post / np.maximum(seq_post.sum(axis=1, keepdims=True), 1e-12)
            post[idx] = seq_post

    for s in range(n_states):
        lap[f"rl_latent_regime_prob_{s}"] = post[:, s]
    for s in range(n_states, 3):
        lap[f"rl_latent_regime_prob_{s}"] = 0.0

    feature_cols = [c for c in lap.columns if c not in keys]
    out = out.merge(lap[keys + feature_cols], on=keys, how="left", sort=False)
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000572"],
        "research_usage_note": "Verified assigned hypothesis 000572.",
    }, sort_keys=True))
