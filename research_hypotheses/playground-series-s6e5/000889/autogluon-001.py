...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Year", "Race", "Driver", "LapNumber", "Stint", "_row_id"]
    out = out.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)

    lap_col = "LapTime (s)"
    grp_seq = ["Year", "Race", "Driver"]
    grp_rc = ["Year", "Race", "Compound"]

    race_compound_median = out.groupby(grp_rc)[lap_col].transform("median")
    out["rc_laptime_resid"] = out[lap_col] - race_compound_median

    abs_resid = out["rc_laptime_resid"].abs()
    resid_scale = out.groupby(grp_rc)["rc_laptime_resid"].transform(
        lambda s: s.abs().median()
    )
    resid_scale = resid_scale.fillna(resid_scale.median()).replace(
        0,
        (
            resid_scale.median()
            if np.isfinite(resid_scale.median()) and resid_scale.median() != 0
            else 1.0
        ),
    )

    valid_update = (out["PitStop"] == 0) & (abs_resid <= (6.0 * resid_scale))
    out["degradation_update_valid"] = valid_update.astype("int8")

    prior_resid = (
        out["rc_laptime_resid"]
        .where(valid_update)
        .groupby([out[c] for c in grp_seq], sort=False)
        .shift(1)
    )
    out["latent_deg_ewm"] = (
        prior_resid.groupby([out[c] for c in grp_seq], sort=False)
        .transform(lambda s: s.ewm(alpha=0.35, adjust=False, ignore_na=True).mean())
        .fillna(0.0)
    )

    resid_lag1 = out.groupby(grp_seq, sort=False)["rc_laptime_resid"].shift(1)
    resid_lag2 = out.groupby(grp_seq, sort=False)["rc_laptime_resid"].shift(2)
    resid_lag3 = out.groupby(grp_seq, sort=False)["rc_laptime_resid"].shift(3)
    resid_lag4 = out.groupby(grp_seq, sort=False)["rc_laptime_resid"].shift(4)

    out["latent_deg_slope3"] = ((resid_lag1 - resid_lag3) / 2.0).fillna(0.0)
    out["latent_deg_slope5"] = ((resid_lag1 - resid_lag4) / 3.0).fillna(0.0)
    out["latent_deg_curvature3"] = (resid_lag1 - 2.0 * resid_lag2 + resid_lag3).fillna(
        0.0
    )

    stint_start_resid = out.groupby(grp_seq + ["Stint"], sort=False)[
        "rc_laptime_resid"
    ].transform("first")
    out["resid_since_stint_start"] = (
        out["rc_laptime_resid"] - stint_start_resid
    ).fillna(0.0)

    first_in_stint = out.groupby(grp_seq + ["Stint"], sort=False).cumcount() == 0
    prev_pit = (
        out.groupby(grp_seq, sort=False)["PitStop"].shift(1).fillna(0).astype("int8")
    )
    stint_changed = out.groupby(grp_seq, sort=False)["Stint"].diff().fillna(0).ne(0)
    out["pit_reset_flag"] = (first_in_stint | prev_pit.eq(1) | stint_changed).astype(
        "int8"
    )

    out["degradation_pressure"] = (
        0.6 * out["latent_deg_ewm"]
        + 0.25 * out["resid_since_stint_start"]
        + 0.10 * out["latent_deg_slope3"]
        + 0.05 * out["latent_deg_curvature3"]
    )

    out["tyrelife_x_pressure"] = out["TyreLife"] * out["degradation_pressure"]
    out["cumdeg_x_reset"] = out["Cumulative_Degradation"] * (
        1.0 - out["pit_reset_flag"]
    )
    out["progress_x_pressure"] = out["RaceProgress"] * out["degradation_pressure"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000889"],
        "research_usage_note": "Verified assigned hypothesis 000889.",
    }, sort_keys=True))
