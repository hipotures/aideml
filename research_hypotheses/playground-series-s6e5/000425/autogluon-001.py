...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Basic race/lap structure.
    max_lap = out.groupby(race_key, sort=False)["LapNumber"].transform("max")
    out["laps_remaining"] = (max_lap - out["LapNumber"]).astype("float32")
    out["race_lap_frac"] = (out["LapNumber"] / max_lap.replace(0, np.nan)).astype(
        "float32"
    )

    # Lap-aligned race-control proxies: field-wide slowdown and instability on the same lap.
    lap_med_time = out.groupby(lap_key, sort=False)["LapTime (s)"].transform("median")
    race_base_time = out.groupby(race_key, sort=False)["LapTime (s)"].transform(
        "median"
    )
    out["race_control_lap_slowdown"] = (
        lap_med_time / race_base_time.replace(0, np.nan) - 1.0
    ).astype("float32")

    lap_med_delta = out.groupby(lap_key, sort=False)["LapTime_Delta"].transform(
        "median"
    )
    lap_abs_delta = out.groupby(lap_key, sort=False)["LapTime_Delta"].transform(
        lambda s: s.abs().median()
    )
    out["race_control_delta_median"] = lap_med_delta.astype("float32")
    out["race_control_delta_abs_median"] = lap_abs_delta.astype("float32")

    lap_pit_rate = out.groupby(lap_key, sort=False)["PitStop"].transform("mean")
    out["race_control_lap_pit_rate"] = lap_pit_rate.astype("float32")

    # Thresholded SC/VSC-like status from field-level slowdowns; current-lap only, no future lookahead.
    slow = out["race_control_lap_slowdown"].fillna(0.0)
    out["rc_status_green"] = (slow < 0.10).astype("int8")
    out["rc_status_vsc_like"] = ((slow >= 0.10) & (slow < 0.35)).astype("int8")
    out["rc_status_sc_like"] = (slow >= 0.35).astype("int8")
    out["rc_any_caution"] = (
        (out["rc_status_vsc_like"] == 1) | (out["rc_status_sc_like"] == 1)
    ).astype("int8")

    # Previous-lap status and causal transition features.
    lap_status = (
        out[
            lap_key
            + [
                ...
                "rc_status_vsc_like",
                "rc_status_sc_like",
                "race_control_lap_slowdown",
            ]
        ]
        .drop_duplicates(lap_key)
        .sort_values(lap_key)
    )
    lap_status["rc_prev_any_caution"] = (
        lap_status.groupby(race_key, sort=False)["rc_any_caution"]
        .shift(1)
        .fillna(0)
        .astype("int8")
    )
    lap_status["rc_prev_slowdown"] = (
        lap_status.groupby(race_key, sort=False)["race_control_lap_slowdown"]
        .shift(1)
        .fillna(0)
        .astype("float32")
    )
    lap_status["rc_status_changed_lap"] = (
        lap_status["rc_any_caution"] != lap_status["rc_prev_any_caution"]
    ).astype("int8")
    lap_status["rc_caution_started_lap"] = (
        (lap_status["rc_any_caution"] == 1) & (lap_status["rc_prev_any_caution"] == 0)
    ).astype("int8")
    lap_status["rc_vsc_ending_flag"] = (
        (lap_status["rc_prev_any_caution"] == 1)
        & (lap_status["rc_any_caution"] == 1)
        & (
            lap_status["race_control_lap_slowdown"]
            < lap_status["rc_prev_slowdown"] - 0.08
        )
    ).astype("int8")

    lap_status["rc_segment"] = lap_status.groupby(race_key, sort=False)[
        "rc_caution_started_lap"
    ].cumsum()
    lap_status["rc_laps_since_caution_start"] = (
        (lap_status.groupby(race_key + ["rc_segment"], sort=False).cumcount() + 1)
        .where(lap_status["rc_any_caution"] == 1, 0)
        .astype("int16")
    )
    lap_status["rc_recent_events_3"] = (
        lap_status.groupby(race_key, sort=False)["rc_status_changed_lap"]
    ...
    out = out.merge(
        lap_status[
            ...
            + [
                "rc_prev_any_caution",
                "rc_status_changed_lap",
                "rc_caution_started_lap",
                "rc_vsc_ending_flag",
                "rc_laps_since_caution_start",
                "rc_recent_events_3",
            ]
        ],
        on=lap_key,
    ...
    # Interactions requested by hypothesis 000425.
    out["rc_caution_x_tyre_life"] = (out["rc_any_caution"] * out["TyreLife"]).astype(
        "float32"
    )
    out["rc_caution_x_laps_remaining"] = (
        out["rc_any_caution"] * out["laps_remaining"]
    ).astype("float32")
    out["rc_caution_x_stint"] = (out["rc_any_caution"] * out["Stint"]).astype("float32")
    out["rc_vsc_x_tyre_life"] = (out["rc_status_vsc_like"] * out["TyreLife"]).astype(
        "float32"
    )
    out["rc_sc_x_tyre_life"] = (out["rc_status_sc_like"] * out["TyreLife"]).astype(
        "float32"
    )

    compound_norm = out["Compound"].astype("string").str.upper()
    out["compound_is_soft"] = (compound_norm == "SOFT").astype("int8")
    out["compound_is_medium"] = (compound_norm == "MEDIUM").astype("int8")
    out["compound_is_hard"] = (compound_norm == "HARD").astype("int8")
    out["compound_is_inter"] = (compound_norm == "INTERMEDIATE").astype("int8")
    out["compound_is_wet"] = (compound_norm == "WET").astype("int8")
    out["rc_caution_soft"] = (out["rc_any_caution"] * out["compound_is_soft"]).astype(
        "int8"
    )
    out["rc_caution_medium"] = (
        out["rc_any_caution"] * out["compound_is_medium"]
    ).astype("int8")
    out["rc_caution_hard"] = (out["rc_any_caution"] * out["compound_is_hard"]).astype(
        "int8"
    )
    out["rc_caution_wet_or_inter"] = (
        out["rc_any_caution"]
        * ((out["compound_is_inter"] == 1) | (out["compound_is_wet"] == 1))
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000425.",
    }, sort_keys=True))
