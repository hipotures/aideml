...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    keys = ["Driver", "Race", "Year"]
    order_col = "__orig_order__"
    out[order_col] = np.arange(len(out), dtype=np.int64)

    s = out.sort_values(keys + ["LapNumber", order_col], kind="mergesort").copy()
    g = s.groupby(keys, sort=False)

    lap_time = s["LapTime (s)"].astype("float32")
    cum_best = g["LapTime (s)"].cummin()
    prior_best = cum_best.groupby([s[k] for k in keys], sort=False).shift(1)
    residual = lap_time - prior_best.fillna(lap_time).astype("float32")
    resid_clip = residual.clip(-30.0, 240.0).astype("float32")

    s["pace_resid_prior_best"] = residual.astype("float32")
    s["pace_resid_clipped"] = resid_clip

    ewm = (
        s.groupby(keys, sort=False)["pace_resid_clipped"]
        .ewm(alpha=0.35, adjust=False)
        .mean()
        .reset_index(level=list(range(len(keys))), drop=True)
    )
    s["pace_resid_ewm"] = ewm.astype("float32")
    s["pace_resid_ewm_slope"] = (
        s["pace_resid_ewm"]
        - s.groupby(keys, sort=False)["pace_resid_ewm"].shift(1).fillna(0.0)
    ).astype("float32")
    ...
    s["pace_resid_roll_var5"] = roll_var.fillna(0.0).astype("float32")

    pos_resid = s["pace_resid_clipped"].clip(lower=0.0)
    s["pace_pos_resid_cumsum"] = (
        pos_resid.groupby([s[k] for k in keys], sort=False).cumsum().astype("float32")
    )

    pit = s["PitStop"].astype("int8")
    stint_change = s.groupby(keys, sort=False)["Stint"].diff().fillna(0).ne(0)
    reset_flag = (pit.eq(1) | stint_change).astype("int8")
    s["latent_reset_flag"] = reset_flag
    s["__reset_segment__"] = (
        reset_flag.groupby([s[k] for k in keys], sort=False).cumsum().astype("int16")
    )

    seg_keys = keys + ["__reset_segment__"]
    wear_input = pos_resid.mask(reset_flag.astype(bool), 0.0).astype("float32")
    s["__wear_input__"] = wear_input

    s["latent_laps_since_reset"] = (
        s.groupby(seg_keys, sort=False).cumcount().astype("float32")
    )
    s["latent_wear_cumsum_reset"] = (
        s.groupby(seg_keys, sort=False)["__wear_input__"].cumsum().astype("float32")
    )

    wear_ewm = (
        s.groupby(seg_keys, sort=False)["__wear_input__"]
        .ewm(alpha=0.30, adjust=False)
        .mean()
        .reset_index(level=list(range(len(seg_keys))), drop=True)
    )
    s["latent_wear_ewm_reset"] = wear_ewm.astype("float32")
    s["latent_wear_rate_reset"] = (
        s["latent_wear_cumsum_reset"] / (s["latent_laps_since_reset"] + 1.0)
    ).astype("float32")

    s = s.sort_values(order_col, kind="mergesort")
    return s.drop(columns=[order_col, "__reset_segment__", "__wear_input__"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000138"],
        "research_usage_note": "Verified assigned hypothesis 000138.",
    }, sort_keys=True))
