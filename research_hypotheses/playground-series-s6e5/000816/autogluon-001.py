...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["race_lap_group_size"] = g["TyreLife"].transform("size").astype("float32")

    for col in num_cols:
        if col in out.columns:
            mean = g[col].transform("mean")
            std = g[col].transform("std").replace(0, np.nan)
            out[f"{col}_race_lap_diff"] = (out[col] - mean).astype("float32")
            out[f"{col}_race_lap_z"] = (
                ((out[col] - mean) / std).fillna(0).astype("float32")
            )
            out[f"{col}_race_lap_pct_rank"] = (
                g[col].rank(method="average", pct=True).astype("float32")
            )

    out["tyre_age_progress"] = (out["TyreLife"] * out["RaceProgress"]).astype("float32")
    out["stint_tyre_ratio"] = (
        (out["TyreLife"] / (out["Stint"].replace(0, np.nan)))
        .fillna(0)
        .astype("float32")
    )
    ...
    for col in ["Driver", "Race", "Compound", "race_lap_key"]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000816.",
    }, sort_keys=True))
