...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    est_total_laps = est_total_laps.fillna(lap)
    laps_remaining = (est_total_laps - lap).clip(lower=0)

    out["RaceProgress_sq"] = progress * progress
    out["RaceProgress_cube"] = progress * progress * progress
    out["RaceProgress_sqrt"] = np.sqrt(progress)
    out["Estimated_Total_Laps"] = est_total_laps
    out["Estimated_Laps_Remaining"] = laps_remaining
    out["Finish_Window_Pressure"] = (1.0 / (laps_remaining + 1.0)).clip(0, 1)

    out["Phase_Early"] = (progress <= 0.25).astype("int8")
    out["Phase_Mid"] = ((progress > 0.25) & (progress <= 0.65)).astype("int8")
    out["Phase_Late"] = ((progress > 0.65) & (progress <= 0.85)).astype("int8")
    out["Phase_Final_Quarter"] = (progress > 0.75).astype("int8")
    out["Phase_Final_Ten_Laps"] = (laps_remaining <= 10).astype("int8")

    out["TyreLife_x_Progress"] = tyre * progress
    out["TyreLife_x_Early"] = tyre * out["Phase_Early"]
    out["TyreLife_x_Mid"] = tyre * out["Phase_Mid"]
    out["TyreLife_x_Late"] = tyre * out["Phase_Late"]
    out["TyreLife_x_FinalQuarter"] = tyre * out["Phase_Final_Quarter"]
    out["TyreLife_x_FinalTen"] = tyre * out["Phase_Final_Ten_Laps"]
    out["TyreLife_x_FinishPressure"] = tyre * out["Finish_Window_Pressure"]
    out["Progress_x_FinishPressure"] = progress * out["Finish_Window_Pressure"]

    out.attrs["research_hypotheses_llm_claimed_used"] = ["000026"]
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000026"],
        "research_usage_note": "Verified assigned hypothesis 000026.",
    }, sort_keys=True))
