...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp = out["Compound"].astype("object")
    old_map = {
        "SOFT": 14.0,
        "MEDIUM": 20.0,
        "HARD": 27.0,
    ...
    old_thr = comp.map(old_map).fillna(20.0).astype("float32")
    very_old_thr = comp.map(very_old_map).fillna(32.0).astype("float32")

    out["FreshTyre"] = (tyre <= 2).astype("int8")
    out["VeryFreshTyre"] = (tyre <= 1).astype("int8")
    out["OldTyreRaw"] = (tyre >= 25).astype("int8")
    out["VeryOldTyreRaw"] = (tyre >= 40).astype("int8")
    out["OldForCompound"] = (tyre >= old_thr).astype("int8")
    out["VeryOldForCompound"] = (tyre >= very_old_thr).astype("int8")

    out["TyreLifeOverOldBand"] = (tyre - old_thr).astype("float32")
    out["TyreLifeOverVeryOldBand"] = (tyre - very_old_thr).astype("float32")
    out["TyreAgeCompoundRatio"] = (tyre / old_thr.clip(lower=1.0)).astype("float32")

    late = (progress >= 0.70).astype("int8")
    high_degr = (degr >= 0).astype("int8")

    out["FreshAfterPit"] = (out["FreshTyre"] * pit).astype("int8")
    out["OldLateRace"] = (out["OldForCompound"] * late).astype("int8")
    out["VeryOldLateRace"] = (out["VeryOldForCompound"] * late).astype("int8")
    out["OldWithHighDegradation"] = (out["OldForCompound"] * high_degr).astype("int8")
    out["VeryOldWithHighDegradation"] = (out["VeryOldForCompound"] * high_degr).astype(
        "int8"
    )
    out["OldPitState"] = (out["OldForCompound"] * pit).astype("int8")

    out["AgeDegradationPressure"] = (out["TyreAgeCompoundRatio"] * degr).astype(
        "float32"
    )
    out["LateAgePressure"] = (out["TyreAgeCompoundRatio"] * progress).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000028"],
        "research_usage_note": "Verified assigned hypothesis 000028.",
    }, sort_keys=True))
