...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["position_pressure_base"] = (21 - out["Position"]) / 20.0

    lap_keys = ["Year", "Race", "LapNumber"]
    race_keys = ["Year", "Race"]

    out["lap_delta_pct"] = (
        out.groupby(lap_keys, observed=True)["LapTime_Delta"]
        .rank(pct=True)
        .astype("float32")
    )
    out["deg_lap_pct"] = (
        out.groupby(lap_keys, observed=True)["Cumulative_Degradation"]
        .rank(pct=True)
        .astype("float32")
    )
    out["tyre_life_lap_pct"] = (
        out.groupby(lap_keys, observed=True)["TyreLife"]
        .rank(pct=True)
        .astype("float32")
    )

    s = out.sort_values(lap_keys + ["Position"]).copy()
    g = s.groupby(lap_keys, observed=True, sort=False)

    pressure = np.zeros(len(s), dtype="float32")
    older = np.zeros(len(s), dtype="float32")
    newer = np.zeros(len(s), dtype="float32")
    diff_comp = np.zeros(len(s), dtype="float32")

    for k, w in [(-2, 0.35), (-1, 0.65), (1, 1.0), (2, 0.45)]:
        n_delta = g["LapTime_Delta"].shift(k)
        n_tyre = g["TyreLife"].shift(k)
        n_comp = g["Compound"].shift(k)
        valid = n_delta.notna()
        delta_gap = (s["LapTime_Delta"] - n_delta).abs()
        pressure += (valid & (delta_gap <= 1.25)).astype("float32").to_numpy() * w
        older += (valid & (n_tyre > s["TyreLife"] + 2)).astype("float32").to_numpy() * w
        newer += (valid & (n_tyre + 2 < s["TyreLife"])).astype("float32").to_numpy() * w
        diff_comp += (
            valid & (n_comp.astype("object") != s["Compound"].astype("object"))
        ).astype("float32").to_numpy() * w

    s["nearby_pressure"] = pressure
    s["nearby_older_tyre_count"] = older
    s["nearby_newer_tyre_count"] = newer
    s["nearby_diff_compound_count"] = diff_comp
    ctx = s[
        [
            "nearby_pressure",
            "nearby_older_tyre_count",
            "nearby_newer_tyre_count",
            "nearby_diff_compound_count",
        ]
    ].sort_index()
    out = out.join(ctx)

    race_pos_chg = out.groupby(race_keys, observed=True)["Position_Change"].transform(
        lambda x: x.abs().median()
    )
    out["hard_to_overtake_proxy"] = (1.0 / (1.0 + race_pos_chg)).astype("float32")

    comp_year = ["Year", "Compound"]
    post_stop_like = out["PitStop"].eq(1)
    response_src = out["LapTime_Delta"].where(post_stop_like)
    out["compound_year_poststop_response"] = response_src.groupby(
        [out["Year"], out["Compound"]], observed=True
    ).transform("median")
    out["compound_year_poststop_response"] = (
        out["compound_year_poststop_response"]
        .fillna(response_src.median())
        .astype("float32")
    )

    deg_z = (
        out["Cumulative_Degradation"]
        - out.groupby(race_keys, observed=True)["Cumulative_Degradation"].transform(
            "median"
        )
    ) / (
        out.groupby(race_keys, observed=True)["Cumulative_Degradation"]
        .transform("std")
        .fillna(0)
        + eps
    )
    out["undercut_opportunity"] = (
        deg_z.clip(-3, 6)
        * (0.5 + out["hard_to_overtake_proxy"])
        * (1.0 + out["nearby_pressure"])
        * (0.5 + out["nearby_newer_tyre_count"])
        * out["RaceProgress"].clip(0.05, 0.98)
    ).astype("float32")

    out["overcut_opportunity"] = (
        (-out["compound_year_poststop_response"]).clip(-10, 10)
        * (0.5 + out["nearby_older_tyre_count"])
        * (1.0 - out["lap_delta_pct"].fillna(0.5))
        * out["RaceProgress"].between(0.15, 0.85).astype("float32")
    ).astype("float32")

    out["reactive_stop_pressure"] = (
        out["nearby_pressure"]
        + 0.5 * out["nearby_diff_compound_count"]
        + out["position_pressure_base"] * out["hard_to_overtake_proxy"]
        + out["deg_lap_pct"].fillna(0.5)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000149.",
    }, sort_keys=True))
