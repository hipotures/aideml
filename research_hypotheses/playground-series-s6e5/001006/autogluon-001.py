...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cat_cols:
        out[c] = out[c].astype("string").str.strip().str.upper().astype("category")
    ...
    stint = pd.to_numeric(out["Stint"], errors="coerce").astype("float32")
    ...
    lap_delta = pd.to_numeric(out["LapTime_Delta"], errors="coerce").astype("float32")
    degr = pd.to_numeric(out["Cumulative_Degradation"], errors="coerce").astype(
        "float32"
    )
    pit_now = pd.to_numeric(out["PitStop"], errors="coerce").astype("float32")
    year = pd.to_numeric(out["Year"], errors="coerce").astype("float32")
    ...
    laps_remaining = (est_total_laps - lap).clip(0, 120).astype("float32")
    tyre_safe = tyre.clip(1, 120)

    out["hazard_est_total_laps"] = est_total_laps
    out["hazard_laps_remaining"] = laps_remaining
    out["hazard_remaining_frac"] = (
        laps_remaining / est_total_laps.clip(1, 120)
    ).astype("float32")
    out["hazard_tyre_frac_of_race"] = (tyre_safe / est_total_laps.clip(1, 120)).astype(
        "float32"
    )
    out["hazard_tyre_to_remaining"] = (
        (tyre_safe / (laps_remaining + 1)).clip(0, 80).astype("float32")
    )
    out["hazard_lap_to_tyre"] = (lap / tyre_safe).clip(0, 120).astype("float32")

    out["hazard_log_tyre_life"] = np.log1p(tyre_safe).astype("float32")
    out["hazard_sqrt_tyre_life"] = np.sqrt(tyre_safe).astype("float32")
    out["hazard_tyre_life_sq"] = (tyre_safe**2).clip(0, 6000).astype("float32")
    out["hazard_progress_sq"] = (progress**2).astype("float32")
    out["hazard_laps_remaining_sq"] = (
        (laps_remaining**2).clip(0, 10000).astype("float32")
    )
    out["hazard_stint_x_tyre"] = (stint * tyre_safe).astype("float32")
    out["hazard_stint_x_progress"] = (stint * progress).astype("float32")
    out["hazard_tyre_x_progress"] = (tyre_safe * progress).astype("float32")

    out["hazard_tyre_age_bin"] = pd.cut(
        tyre_safe,
        bins=[0, 2, 4, 7, 10, 14, 18, 24, 32, 45, 80, 200],
        labels=False,
        include_lowest=True,
    ).astype("float32")
    out["hazard_lap_bin"] = pd.cut(
        lap,
        bins=[0, 3, 6, 10, 15, 20, 28, 36, 45, 55, 70, 130],
        labels=False,
        include_lowest=True,
    ).astype("float32")
    out["hazard_progress_bin"] = pd.cut(
        progress,
        bins=[0, 0.05, 0.10, 0.18, 0.28, 0.40, 0.55, 0.70, 0.82, 0.92, 1.01],
        labels=False,
        include_lowest=True,
    ).astype("float32")
    out["hazard_remaining_bin"] = pd.cut(
        laps_remaining,
        bins=[-1, 1, 3, 5, 8, 12, 18, 25, 35, 50, 130],
        labels=False,
        include_lowest=True,
    ).astype("float32")

    out["hazard_first_stint_lap"] = (tyre_safe <= 1).astype("int8")
    out["hazard_old_tyre_12"] = (tyre_safe >= 12).astype("int8")
    out["hazard_old_tyre_18"] = (tyre_safe >= 18).astype("int8")
    out["hazard_old_tyre_25"] = (tyre_safe >= 25).astype("int8")
    out["hazard_late_race"] = (progress >= 0.70).astype("int8")
    out["hazard_final_10_laps"] = (laps_remaining <= 10).astype("int8")
    out["hazard_recent_pit_current_lap"] = pit_now.astype("int8")

    out["hazard_degr_per_tyre_lap"] = (
        (degr / tyre_safe).replace([np.inf, -np.inf], 0).astype("float32")
    )
    out["hazard_abs_degr"] = degr.abs().astype("float32")
    out["hazard_lap_delta_abs"] = lap_delta.abs().clip(0, 3000).astype("float32")
    out["hazard_lap_delta_x_tyre"] = (
        (lap_delta * tyre_safe).clip(-10000, 10000).astype("float32")
    )
    out["hazard_degr_x_progress"] = (
        (degr * progress).clip(-2000, 5000).astype("float32")
    )
    out["hazard_position_norm"] = ((pos - 1) / 19).clip(0, 1).astype("float32")
    out["hazard_front_position"] = (21 - pos).clip(1, 20).astype("float32")
    out["hazard_lost_positions"] = (pos_chg > 0).astype("int8")
    out["hazard_gained_positions"] = (pos_chg < 0).astype("int8")
    out["hazard_abs_position_change"] = pos_chg.abs().astype("float32")
    out["hazard_tyre_x_position"] = (tyre_safe * pos).astype("float32")
    ...
    out["hazard_typical_remaining_to_stop"] = (
        (typical - tyre_safe).clip(-60, 60).astype("float32")
    )
    out["hazard_over_typical_stop_age"] = (
        (tyre_safe - typical).clip(-60, 60).astype("float32")
    )
    out["hazard_age_to_typical_stop"] = (
        (tyre_safe / typical.clip(1, 80)).clip(0, 8).astype("float32")
    )

    for name in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        mask = (comp == name).astype("int8")
        lname = name.lower()
        out[f"hazard_compound_{lname}"] = mask
        out[f"hazard_tyre_x_{lname}"] = (tyre_safe * mask).astype("float32")
        out[f"hazard_progress_x_{lname}"] = (progress * mask).astype("float32")

    race_year_key = out["Race"].astype("string") + "|" + out["Year"].astype("string")
    med_lap_time = (
        lap_time.groupby(race_year_key, observed=True)
        .transform("median")
        .astype("float32")
    )
    std_lap_time = (
    ...
    out["hazard_lap_time_vs_race_year"] = (
        (lap_time - med_lap_time).clip(-1000, 3000).astype("float32")
    )
    out["hazard_lap_time_race_year_z"] = (
        ((lap_time - med_lap_time) / std_lap_time).clip(-20, 20).astype("float32")
    )

    for c in ["Compound", "Driver", "Race", "Year", "Stint"]:
        s = out[c].astype("string")
        out[f"hazard_{c.lower()}_freq"] = s.map(s.value_counts(normalize=True)).astype(
            "float32"
        )
    ...
    for a, b in [
        ...
        key = out[a].astype("string") + "|" + out[b].astype("string")
        out[f"hazard_{a.lower()}_{b.lower()}_freq"] = key.map(
            key.value_counts(normalize=True)
        ).astype("float32")

    out["hazard_year_centered"] = (year - 2023.5).astype("float32")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001006"],
        "research_usage_note": "Verified assigned hypothesis 001006.",
    }, sort_keys=True))
