...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_time = out["LapTime (s)"].to_numpy(dtype=np.float64, copy=False)
    lap_num = out["LapNumber"].to_numpy(dtype=np.float64, copy=False)
    stint = out["Stint"].to_numpy(dtype=np.float64, copy=False)
    tyre_life = out["TyreLife"].to_numpy(dtype=np.float64, copy=False)
    pit_stop = out["PitStop"].to_numpy(dtype=np.float64, copy=False)

    year_code = pd.factorize(out["Year"], sort=True)[0].astype(np.int64, copy=False)
    race_code = pd.factorize(out["Race"], sort=True)[0].astype(np.int64, copy=False)
    driver_code = pd.factorize(out["Driver"], sort=True)[0].astype(np.int64, copy=False)
    compound_code = pd.factorize(out["Compound"], sort=True)[0].astype(
        np.int64, copy=False
    )

    order = np.lexsort((np.arange(n), lap_num, driver_code, race_code, year_code))

    latent_state = np.zeros(n, dtype=np.float32)
    latent_slope = np.zeros(n, dtype=np.float32)
    latent_next_state = np.zeros(n, dtype=np.float32)
    residual_expected = np.zeros(n, dtype=np.float32)
    reset_magnitude = np.zeros(n, dtype=np.float32)
    state_per_tyre_life = np.zeros(n, dtype=np.float32)

    prev_y = prev_r = prev_d = None
    prev_stint = prev_tyre = prev_compound = np.nan
    pace = state = slope = 0.0
    have_group = False
    ...
    for idx in order:
        ...
        rk = race_code[idx]
        dk = driver_code[idx]
        y = float(lap_time[idx])
        if not np.isfinite(y):
            y = pace if have_group else 0.0

        new_group = (not have_group) or yk != prev_y or rk != prev_r or dk != prev_d
        reset_value = 0.0
        ...
        if new_group:
            ...
            stint_reset = stint[idx] != prev_stint
            tyre_reset = tyre_life[idx] + 0.25 < prev_tyre
            compound_reset = compound_code[idx] != prev_compound
            if stint_reset or tyre_reset or compound_reset:
                reset_value = max(0.0, state)
                state = 0.0
                slope = 0.0
                if np.isfinite(pace) and abs(y - pace) <= 8.0:
                    pace = 0.75 * pace + 0.25 * y

        pred_state = max(0.0, state + slope)
        expected = pace + pred_state
        raw_resid = y - expected
        resid_feat = float(np.clip(raw_resid, -60.0, 60.0))
        innovation = float(np.clip(raw_resid, -8.0, 12.0))

        updated_state = max(0.0, pred_state + 0.35 * innovation)
        updated_slope = float(np.clip(0.75 * slope + 0.18 * innovation, -1.5, 2.5))

        if pit_stop[idx] >= 0.5:
            reset_value = max(reset_value, updated_state)
            updated_state = 0.0
            updated_slope = 0.0
        elif abs(raw_resid) <= 25.0:
            clean_pace_est = y - updated_state
            pace = 0.97 * pace + 0.03 * clean_pace_est

        next_state = max(0.0, updated_state + updated_slope)
        life_scale = max(1.0, np.sqrt(max(1.0, float(tyre_life[idx]))))

        latent_state[idx] = updated_state
        latent_slope[idx] = updated_slope
        latent_next_state[idx] = next_state
        residual_expected[idx] = resid_feat
        reset_magnitude[idx] = reset_value
        state_per_tyre_life[idx] = updated_state / life_scale

        state = updated_state
        slope = updated_slope
        prev_y, prev_r, prev_d = yk, rk, dk
        prev_stint = stint[idx]
        prev_tyre = tyre_life[idx]
        prev_compound = compound_code[idx]

    out["latent_tyre_state"] = latent_state
    out["latent_state_slope"] = latent_slope
    out["latent_expected_next_state"] = latent_next_state
    out["residual_vs_expected_laptime"] = residual_expected
    out["post_pit_reset_magnitude"] = reset_magnitude
    out["latent_state_per_tyre_life"] = state_per_tyre_life
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000535"],
        "research_usage_note": "Verified assigned hypothesis 000535.",
    }, sort_keys=True))
