...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Race", "Driver", "Compound"]:
        out[c] = out[c].astype("string").str.upper().str.strip().fillna("UNKNOWN")

    g = out.groupby(keys, sort=False, observed=True)
    n = g["TyreLife"].transform("count").astype(float).clip(lower=1.0)

    tyre_rank = g["TyreLife"].rank(method="average", pct=True)
    deg_rank = g["Cumulative_Degradation"].rank(method="average", pct=True)
    out["snapshot_tyrelife_pct"] = tyre_rank.astype("float32")
    out["snapshot_degradation_rank"] = deg_rank.astype("float32")
    out["snapshot_field_size"] = n.astype("float32")

    lap_mean = g["LapTime_Delta"].transform("mean")
    lap_std = g["LapTime_Delta"].transform("std").replace(0, np.nan)
    out["snapshot_laptime_delta_z"] = (
        ((out["LapTime_Delta"] - lap_mean) / lap_std).fillna(0).astype("float32")
    )
    out["snapshot_laptime_delta_vs_mean"] = (out["LapTime_Delta"] - lap_mean).astype(
        "float32"
    )

    tyre_mean = g["TyreLife"].transform("mean")
    out["snapshot_tyrelife_vs_mean"] = (out["TyreLife"] - tyre_mean).astype("float32")

    comp_counts = (
        out.groupby(keys + ["Compound"], sort=False, observed=True)
        .size()
        .rename("cnt")
        .reset_index()
    )
    comp_counts["p"] = comp_counts["cnt"] / comp_counts.groupby(
        keys, sort=False, observed=True
    )["cnt"].transform("sum")
    entropy = (
        comp_counts.assign(e=lambda x: -x["p"] * np.log(x["p"].clip(lower=1e-12)))
        .groupby(keys, sort=False, observed=True)["e"]
        .sum()
    )
    out = out.merge(
        entropy.rename("snapshot_compound_entropy"), on=keys, how="left", sort=False
    )

    stint_counts = (
        out.groupby(keys + ["Stint"], sort=True, observed=True)
        .size()
        .rename("cnt")
        .reset_index()
    )
    stint_counts["group_n"] = stint_counts.groupby(keys, sort=False, observed=True)[
        "cnt"
    ].transform("sum")
    stint_counts["cum_le"] = stint_counts.groupby(keys, sort=False, observed=True)[
        "cnt"
    ].cumsum()
    stint_counts["frac_later_stint_than_self"] = (
        stint_counts["group_n"] - stint_counts["cum_le"]
    ) / stint_counts["group_n"].clip(lower=1)
    out = out.merge(
        stint_counts[keys + ["Stint", "frac_later_stint_than_self"]],
        on=keys + ["Stint"],
        how="left",
        sort=False,
    )
    out["snapshot_frac_later_stint_than_self"] = (
        out["frac_later_stint_than_self"].fillna(0).astype("float32")
    )
    out = out.drop(columns=["frac_later_stint_than_self"])

    delta_q75 = g["LapTime_Delta"].transform(lambda s: s.quantile(0.75))
    out["_large_pos_delta"] = (out["LapTime_Delta"] > delta_q75).astype("int8")
    sorted_idx = out.sort_values(keys + ["Position"], kind="mergesort").index
    s = out.loc[sorted_idx, keys + ["TyreLife", "_large_pos_delta"]].copy()
    sg = s.groupby(keys, sort=False, observed=True)

    older_near = pd.Series(0, index=s.index, dtype="float32")
    large_near = pd.Series(0, index=s.index, dtype="float32")
    for k in (-2, -1, 1, 2):
        shifted_tyre = sg["TyreLife"].shift(k)
        shifted_large = sg["_large_pos_delta"].shift(k)
        older_near += (shifted_tyre > s["TyreLife"]).fillna(False).astype("float32")
        large_near += shifted_large.fillna(0).astype("float32")

    out.loc[sorted_idx, "nearby_older_tyre_count"] = older_near.values
    out.loc[sorted_idx, "nearby_large_delta_count"] = large_near.values
    out["nearby_pressure_score"] = (
        out["nearby_older_tyre_count"] + out["nearby_large_delta_count"]
    ).astype("float32")
    out = out.drop(columns=["_large_pos_delta"])

    for c in [
        "snapshot_compound_entropy",
        "nearby_older_tyre_count",
        "nearby_large_delta_count",
    ]:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000696.",
    }, sort_keys=True))
