...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["Stint"] = X["Stint"].astype("int64")
    X["PitStop"] = X["PitStop"].astype("float64").fillna(0.0)

    X["Race_Year"] = X["Race"] + "_" + X["Year"].astype(str)
    X["Driver_Compound"] = X["Driver"] + "_" + X["Compound"]
    X["Compound_Stint"] = X["Compound"] + "_" + X["Stint"].astype(str)

    global_mean = X["PitStop"].mean()
    global_logit = np.log((global_mean + 1e-6) / (1.0 - global_mean + 1e-6))
    X["pp_global_pitstop_mean"] = global_mean
    X["pp_global_pitstop_logit"] = global_logit

    def add_smoothed_mean_feature(
        frame, key_col, value_col, out_prefix, prior_mean, alpha
    ):
        stats = frame.groupby(key_col, dropna=False)[value_col].agg(["sum", "count"])
        post = (stats["sum"] + alpha * prior_mean) / (stats["count"] + alpha)
        cnt = stats["count"]
        frame[f"{out_prefix}_pp_mean"] = frame[key_col].map(post).astype("float64")
        frame[f"{out_prefix}_count"] = (
            frame[key_col].map(cnt).astype("float64").fillna(0.0)
        )
        p = frame[f"{out_prefix}_pp_mean"].clip(1e-6, 1 - 1e-6)
        frame[f"{out_prefix}_pp_logit"] = np.log(p / (1.0 - p))
        return frame

    X = add_smoothed_mean_feature(
        X, "Compound", "PitStop", "compound", global_mean, 20.0
    )
    X = add_smoothed_mean_feature(X, "Stint", "PitStop", "stint", global_mean, 20.0)
    X = add_smoothed_mean_feature(X, "Race", "PitStop", "race", global_mean, 40.0)

    race_prior = X["race_pp_mean"]
    stint_prior = X["stint_pp_mean"]
    compound_prior = X["compound_pp_mean"]

    X = add_smoothed_mean_feature(
        X, "Race_Year", "PitStop", "race_year", global_mean, 30.0
    )
    X = add_smoothed_mean_feature(X, "Driver", "PitStop", "driver", global_mean, 50.0)
    X = add_smoothed_mean_feature(
        X, "Driver_Compound", "PitStop", "driver_compound", global_mean, 30.0
    )
    X = add_smoothed_mean_feature(
        X, "Compound_Stint", "PitStop", "compound_stint", global_mean, 20.0
    )

    X["driver_vs_race_prior"] = X["driver_pp_mean"] - race_prior
    X["driver_vs_stint_prior"] = X["driver_pp_mean"] - stint_prior
    X["driver_compound_vs_compound_prior"] = (
        X["driver_compound_pp_mean"] - compound_prior
    )
    X["race_year_vs_race_prior"] = X["race_year_pp_mean"] - race_prior
    X["compound_stint_vs_compound_prior"] = X["compound_stint_pp_mean"] - compound_prior
    X["compound_stint_vs_stint_prior"] = X["compound_stint_pp_mean"] - stint_prior
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000842.",
    }, sort_keys=True))
