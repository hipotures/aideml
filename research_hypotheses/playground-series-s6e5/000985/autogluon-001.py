...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Year_Race"] = out["Year"].astype(str) + "_" + out["Race"].astype(str)
    out["Driver_Race"] = out["Driver"].astype(str) + "_" + out["Year_Race"].astype(str)
    ...
    out["LapNumber_sq"] = lap * lap
    out["RaceProgress_sq"] = progress * progress
    out["EstimatedRaceLaps"] = lap / progress
    out["LapsRemaining_est"] = out["EstimatedRaceLaps"] - lap

    out["TyreLife_per_progress"] = tyre / progress
    out["TyreLife_frac_est_race"] = tyre / out["EstimatedRaceLaps"].replace(0, np.nan)
    out["Stint_TyreLife"] = stint * tyre
    out["Stint_Progress"] = stint * progress
    out["Position_TyreLife"] = pos * tyre

    out["LapTime_log1p"] = np.log1p(lap_time.clip(lower=0))
    out["Abs_LapTime_Delta"] = lap_delta.abs()
    out["LapDelta_per_tyre"] = lap_delta / (tyre + 1.0)
    out["Degradation_per_tyre"] = degr / (tyre + 1.0)
    out["Degradation_per_lap"] = degr / (lap + 1.0)

    out["IsWetCompound"] = out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["IsDrySoft"] = (out["Compound"] == "SOFT").astype("int8")
    out["IsDryHard"] = (out["Compound"] == "HARD").astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000985"],
        "research_usage_note": "Verified assigned hypothesis 000985.",
    }, sort_keys=True))
