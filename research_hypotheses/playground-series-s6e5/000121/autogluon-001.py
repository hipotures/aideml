...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["TyreLife", "LapTime_Delta", "Cumulative_Degradation"]:
        ...
        out[f"{col}_lap_pct"] = g.rank(pct=True).fillna(0.5).astype("float32")

    lap_count = (
        out.groupby(lap_key, sort=False)["LapNumber"]
        .transform("size")
        .astype("float32")
    )
    for comp in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        is_comp = out["Compound"].astype(str).eq(comp).astype("float32")
        out[f"lap_share_{comp.lower()}"] = (
            is_comp.groupby(lap_key, sort=False).transform("sum") / lap_count
        ).astype("float32")

    out["lap_pit_count"] = (
        out.groupby(lap_key, sort=False)["PitStop"].transform("sum").astype("float32")
    )
    out["lap_pit_share"] = (out["lap_pit_count"] / lap_count).astype("float32")

    pos_bucket = pd.cut(
        out["Position"],
        bins=[0, 3, 10, np.inf],
        labels=["p1_3", "p4_10", "p11_plus"],
        right=True,
    ).astype(str)
    out["position_bucket"] = pos_bucket.astype("category")

    for bucket in ["p1_3", "p4_10", "p11_plus"]:
        in_bucket = pos_bucket.eq(bucket).astype("float32")
        out[f"lap_share_{bucket}"] = (
            in_bucket.groupby(lap_key, sort=False).transform("sum") / lap_count
        ).astype("float32")
        out[f"bucket_pit_share_{bucket}"] = (
            (
                (in_bucket * out["PitStop"])
                .groupby(lap_key, sort=False)
                .transform("sum")
                / in_bucket.groupby(lap_key, sort=False)
                .transform("sum")
                .replace(0, np.nan)
            )
            .fillna(0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000121"],
        "research_usage_note": "Verified assigned hypothesis 000121.",
    }, sort_keys=True))
