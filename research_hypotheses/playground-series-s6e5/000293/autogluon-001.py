...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    finish_margin = (est_total_laps - lap).clip(lower=0)
    tyre_to_finish = (tyre_life / (finish_margin + 1.0)).clip(0, 80)

    out["h000293_est_total_laps"] = est_total_laps.fillna(lap)
    out["h000293_finish_margin"] = finish_margin.fillna(0)
    out["h000293_tyre_life_to_finish_pressure"] = tyre_to_finish.fillna(0)
    out["h000293_degradation_per_life"] = (
        (degr / (tyre_life + 1.0)).replace([np.inf, -np.inf], 0).fillna(0)
    )
    out["h000293_positive_laptime_delta"] = lap_delta.clip(lower=0)
    out["h000293_laptime_drift_pressure"] = np.log1p(lap_delta.clip(lower=0))
    out["h000293_late_race_pressure"] = progress.pow(2)
    out["h000293_stop_debt"] = np.maximum(
        stint - out["PitStop"].astype(float) - 1.0, 0.0
    )
    out["h000293_mandatory_stop_debt"] = (
        (progress > 0.55) & (stint <= 1) & (out["PitStop"] == 0)
    ).astype(np.int8)
    out["h000293_fresh_tyre"] = (tyre_life <= 3).astype(np.int8)
    out["h000293_fresh_tyre_relief"] = (1.0 / (tyre_life + 1.0)).clip(0, 1)
    out["h000293_compound_life_pressure"] = tyre_to_finish * out["Compound"].map(
        {"SOFT": 1.25, "MEDIUM": 1.0, "HARD": 0.75, "INTERMEDIATE": 1.15, "WET": 1.2}
    ).fillna(1.0)

    for col in ["Driver", "Race", "Compound"]:
        freq = out[col].astype(str).value_counts(dropna=False)
        out[f"h000293_{col.lower()}_freq"] = out[col].astype(str).map(freq).astype(
            float
        ) / len(out)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000293"],
        "research_usage_note": "Verified assigned hypothesis 000293.",
    }, sort_keys=True))
