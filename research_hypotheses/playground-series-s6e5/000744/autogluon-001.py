...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    ctx = ["Year", "Race", "LapNumber"]

    g = out.groupby(ctx, sort=False, observed=True)
    out["field_size_race_lap"] = g["TyreLife"].transform("size").astype("float32")
    out["tyrelife_rank_race_lap"] = (
        g["TyreLife"].rank(method="average", pct=True).astype("float32")
    )
    out["lapdelta_rank_race_lap"] = (
        g["LapTime_Delta"].rank(method="average", pct=True).astype("float32")
    )
    out["position_rank_race_lap"] = (
        g["Position"].rank(method="average", pct=True).astype("float32")
    )

    tyre_med = g["TyreLife"].transform("median")
    delta_med = g["LapTime_Delta"].transform("median")
    out["tyrelife_vs_lap_median"] = (out["TyreLife"] - tyre_med).astype("float32")
    out["lapdelta_vs_lap_median"] = (out["LapTime_Delta"] - delta_med).astype("float32")

    gc = out.groupby(ctx + ["Compound"], sort=False, observed=True)
    comp_tyre_med = gc["TyreLife"].transform("median")
    comp_delta_med = gc["LapTime_Delta"].transform("median")
    out["tyrelife_vs_compound_lap_median"] = (out["TyreLife"] - comp_tyre_med).astype(
        "float32"
    )
    out["lapdelta_vs_compound_lap_median"] = (
        out["LapTime_Delta"] - comp_delta_med
    ).astype("float32")

    comp = out["Compound"].astype(str)
    out["_top5"] = out["Position"].le(5).astype("float32")
    out["_top10"] = out["Position"].le(10).astype("float32")
    den5 = (
        out.groupby(ctx, sort=False, observed=True)["_top5"]
        .transform("sum")
        .replace(0, np.nan)
    )
    den10 = (
        out.groupby(ctx, sort=False, observed=True)["_top10"]
        .transform("sum")
        .replace(0, np.nan)
    )
    ...
    for c in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        name = c.lower()
        out["_hit"] = (out["Position"].le(5) & comp.eq(c)).astype("float32")
        out[f"top5_frac_{name}"] = (
            (
                out.groupby(ctx, sort=False, observed=True)["_hit"].transform("sum")
                / den5
            )
            .fillna(0)
            .astype("float32")
        ...
        out["_hit"] = (out["Position"].le(10) & comp.eq(c)).astype("float32")
        out[f"top10_frac_{name}"] = (
            (
                out.groupby(ctx, sort=False, observed=True)["_hit"].transform("sum")
                / den10
            )
    ...
    pos_agg = (
        out.groupby(ctx + ["Position"], sort=False, observed=True)
        .agg(
            _n_tyre=("TyreLife", "mean"),
            _n_delta=("LapTime_Delta", "mean"),
            _n_pit=("PitStop", "mean"),
        )
        .reset_index()
    )

    idx = pd.MultiIndex.from_frame(pos_agg[ctx + ["Position"]])
    tyre_s = pd.Series(pos_agg["_n_tyre"].to_numpy(), index=idx)
    delta_s = pd.Series(pos_agg["_n_delta"].to_numpy(), index=idx)
    pit_s = pd.Series(pos_agg["_n_pit"].to_numpy(), index=idx)

    y = out["Year"].to_numpy()
    r = out["Race"].to_numpy()
    lap = out["LapNumber"].to_numpy()
    pos = out["Position"].to_numpy()
    cur_tyre = out["TyreLife"].to_numpy(dtype="float64")
    cur_delta = out["LapTime_Delta"].to_numpy(dtype="float64")
    n = len(out)

    ahead_cnt = np.zeros(n, dtype="float32")
    behind_cnt = np.zeros(n, dtype="float32")
    ahead_tyre_sum = np.zeros(n, dtype="float64")
    behind_tyre_sum = np.zeros(n, dtype="float64")
    ahead_delta_sum = np.zeros(n, dtype="float64")
    behind_delta_sum = np.zeros(n, dtype="float64")
    ahead_cur_older = np.zeros(n, dtype="float32")
    behind_cur_older = np.zeros(n, dtype="float32")
    ahead_neighbor_older = np.zeros(n, dtype="float32")
    behind_neighbor_older = np.zeros(n, dtype="float32")

    for d in [-3, -2, -1, 1, 2, 3]:
        k = pd.MultiIndex.from_arrays([y, r, lap, pos + d], names=ctx + ["Position"])
        nt = tyre_s.reindex(k).to_numpy()
        nd = delta_s.reindex(k).to_numpy()
        valid = ~np.isnan(nt)
        nt0 = np.nan_to_num(nt, nan=0.0)
        nd0 = np.nan_to_num(nd, nan=0.0)

        if d < 0:
            ahead_cnt += valid.astype("float32")
            ahead_tyre_sum += nt0
            ahead_delta_sum += nd0
            ahead_cur_older += (valid & (cur_tyre >= nt)).astype("float32")
            ahead_neighbor_older += (valid & (nt > cur_tyre)).astype("float32")
        else:
            behind_cnt += valid.astype("float32")
            behind_tyre_sum += nt0
            behind_delta_sum += nd0
            behind_cur_older += (valid & (cur_tyre >= nt)).astype("float32")
            behind_neighbor_older += (valid & (nt > cur_tyre)).astype("float32")

    ahead_den = np.maximum(ahead_cnt, 1.0)
    behind_den = np.maximum(behind_cnt, 1.0)
    out["ahead3_tyrelife_mean_diff"] = (cur_tyre - ahead_tyre_sum / ahead_den).astype(
        "float32"
    )
    out["behind3_tyrelife_mean_diff"] = (
        cur_tyre - behind_tyre_sum / behind_den
    ).astype("float32")
    out["ahead3_lapdelta_mean_diff"] = (cur_delta - ahead_delta_sum / ahead_den).astype(
        "float32"
    )
    out["behind3_lapdelta_mean_diff"] = (
        cur_delta - behind_delta_sum / behind_den
    ).astype("float32")
    out["ahead3_current_tyre_rank_frac"] = (ahead_cur_older / ahead_den).astype(
        "float32"
    )
    out["behind3_current_tyre_rank_frac"] = (behind_cur_older / behind_den).astype(
        "float32"
    )
    out["ahead3_neighbor_older_frac"] = (ahead_neighbor_older / ahead_den).astype(
        "float32"
    )
    out["behind3_neighbor_older_frac"] = (behind_neighbor_older / behind_den).astype(
        "float32"
    )

    prev_near = np.zeros(n, dtype="float64")
    prev_ahead = np.zeros(n, dtype="float64")
    prev_behind = np.zeros(n, dtype="float64")
    for d in [-2, -1, 0, 1, 2]:
        k = pd.MultiIndex.from_arrays(
            [y, r, lap - 1, pos + d], names=ctx + ["Position"]
        )
        pv = np.nan_to_num(pit_s.reindex(k).to_numpy(), nan=0.0)
        prev_near += pv
        if d < 0:
            prev_ahead += pv
        elif d > 0:
    ...
    out["nearby_prev_lap_pit_pressure"] = prev_near.astype("float32")
    out["ahead2_prev_lap_pit_pressure"] = prev_ahead.astype("float32")
    out["behind2_prev_lap_pit_pressure"] = prev_behind.astype("float32")
    out["nearby_prev_lap_pit_any"] = (prev_near > 0).astype("int8")

    out = out.drop(columns=["_top5", "_top10", "_hit"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000744"],
        "research_usage_note": "Verified assigned hypothesis 000744.",
    }, sort_keys=True))
