...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["lap_remaining"] = (1.0 - out["RaceProgress"]).astype("float32")
    out["race_progress_sq"] = (out["RaceProgress"] ** 2).astype("float32")
    out["tyre_life_sq"] = (out["TyreLife"] ** 2).astype("float32")
    out["tyre_per_lap"] = (out["TyreLife"] / (out["LapNumber"] + 1.0)).astype("float32")
    out["degradation_per_tyre_lap"] = (
        out["Cumulative_Degradation"] / (out["TyreLife"] + 1.0)
    ).astype("float32")
    out["stint_progress"] = (
        out["TyreLife"] / (out["TyreLife"] + out["lap_remaining"] * 80.0 + 1.0)
    ).astype("float32")

    out["late_race_old_tyre"] = (out["RaceProgress"] * out["TyreLife"]).astype(
        "float32"
    )
    out["stint_x_tyre"] = (out["Stint"] * out["TyreLife"]).astype("float32")
    out["position_x_change"] = (out["Position"] * out["Position_Change"]).astype(
        "float32"
    )
    out["pit_x_tyre"] = (out["PitStop"] * out["TyreLife"]).astype("float32")
    out["pit_x_progress"] = (out["PitStop"] * out["RaceProgress"]).astype("float32")

    out["abs_laptime_delta"] = out["LapTime_Delta"].abs().astype("float32")
    out["abs_position_change"] = out["Position_Change"].abs().astype("float32")
    out["is_early_lap"] = (out["LapNumber"] <= 3).astype("int8")
    out["is_late_race"] = (out["RaceProgress"] >= 0.75).astype("int8")
    ...
    for c in ["Race", "Driver", "Compound"]:
        out[c] = out[c].cat.add_categories(["__MISSING__"]).fillna("__MISSING__")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000867.",
    }, sort_keys=True))
