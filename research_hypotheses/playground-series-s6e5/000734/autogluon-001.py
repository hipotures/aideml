...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = out.assign(_orig_order=orig_index).sort_values(
    ...
    def robust_z(s):
        ...
        scale = 1.4826 * mad if mad and mad > 1e-9 else x.std()
        if not scale or not np.isfinite(scale) or scale < 1e-9:
            scale = 1.0
        return ((x - med) / scale).clip(-6, 6).fillna(0.0)

    z_life = robust_z(work["TyreLife"]) if "TyreLife" in work else 0.0
    z_deg = (
        robust_z(work["Cumulative_Degradation"])
        if "Cumulative_Degradation" in work
        else 0.0
    )
    z_delta = robust_z(work["LapTime_Delta"]) if "LapTime_Delta" in work else 0.0
    z_prog = robust_z(work["RaceProgress"]) if "RaceProgress" in work else 0.0
    obs = np.asarray(
        0.38 * z_life + 0.34 * z_deg + 0.22 * z_delta + 0.06 * z_prog, dtype="float64"
    )

    group_cols = [c for c in ["Year", "Race", "Driver", "Stint"] if c in work.columns]
    keys = (
        work[group_cols].astype(str).agg("|".join, axis=1).to_numpy()
        if group_cols
        else np.zeros(len(work), dtype=object)
    )
    pit = (
        pd.to_numeric(work["PitStop"], errors="coerce").fillna(0).to_numpy()
        if "PitStop" in work
        else np.zeros(len(work))
    )

    level = np.zeros(len(work), dtype="float64")
    slope = np.zeros(len(work), dtype="float64")
    innov = np.zeros(len(work), dtype="float64")
    uncert = np.zeros(len(work), dtype="float64")

    q, r = 0.045, 0.70
    state, prev_state, p = 0.0, 0.0, 1.0
    last_key = None

    for i, key in enumerate(keys):
        if key != last_key or pit[i] > 0:
            state, prev_state, p = 0.0, 0.0, 1.0
            last_key = key

        pred = state
        pred_p = p + q
        residual = float(np.clip(obs[i] - pred, -5.0, 5.0))
        k = pred_p / (pred_p + r)

        prev_state = state
        state = pred + k * residual
        p = (1.0 - k) * pred_p

        level[i] = state
        slope[i] = state - prev_state
        innov[i] = residual
        uncert[i] = p

    work["latent_wear_level"] = level
    work["latent_wear_slope"] = slope
    work["latent_wear_innovation"] = innov
    work["latent_wear_uncertainty"] = uncert
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000734"],
        "research_usage_note": "Verified assigned hypothesis 000734.",
    }, sort_keys=True))
