...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.sort_values(
        group_cols + ["LapNumber", "_row_order"], kind="mergesort"
    ).reset_index(drop=True)
    grp = out.groupby(group_cols, sort=False)

    stint_idx = grp.cumcount()
    stint_len = grp["LapNumber"].transform("size").astype("float32")

    out["StintLapIndex"] = (stint_idx + 1).astype("int16")
    out["StintLength"] = stint_len.astype("int16")
    out["StintProgressLocal"] = ((stint_idx + 1) / stint_len).astype("float32")
    out["JustPitted"] = (stint_idx == 0).astype("int8")
    out["WarmupLap2"] = (stint_idx <= 1).astype("int8")
    out["WarmupLap3"] = (stint_idx <= 2).astype("int8")
    out["WarmupDecay"] = (1.0 / (stint_idx + 1)).astype("float32")
    ...
    for col, prefix in metric_map.items():
        g = out.groupby(group_cols, sort=False)[col]
        lag1 = g.shift(1)
        lag2 = g.shift(2)
        diff1 = out[col] - lag1
        diff2 = out[col] - lag2
        accel = out[col] - 2.0 * lag1 + lag2
        ...
        out[f"{prefix}_Lag1"] = lag1.fillna(out[col]).astype("float32")
        ...
        out[f"{prefix}_EwmResid"] = (out[col] - ewm_mean).fillna(0.0).astype("float32")
        out[f"{prefix}_ChangeZ5"] = (
            ((out[col] - roll_mean3) / (roll_std5 + 1e-6))
            .fillna(0.0)
            .clip(-10, 10)
            .astype("float32")
        )
    ...
    out["LatentPaceGap"] = (out["LapTime_Delta"] - out["LatentPaceProxy"]).astype(
        "float32"
    )
    out["LatentPaceStep"] = grp["LatentPaceProxy"].shift(1)
    out["LatentPaceStep"] = (
        (out["LatentPaceProxy"] - out["LatentPaceStep"]).fillna(0.0).astype("float32")
    )
    ...
    out["TyreLife_x_Cliff"] = (
        out["TyreLife"].astype("float32") * out["TyreCliffScore"]
    ).astype("float32")
    out["RaceProgress_x_Cliff"] = (
        out["RaceProgress"].astype("float32") * out["TyreCliffScore"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000438"],
        "research_usage_note": "Verified assigned hypothesis 000438.",
    }, sort_keys=True))
