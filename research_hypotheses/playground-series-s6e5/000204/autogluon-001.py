...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_dry = comp.isin(dry)
    is_wet = comp.isin(["INTERMEDIATE", "WET"])
    ...
    tmp = pd.DataFrame(
        {
            ...
            "Compound": comp.to_numpy(),
            "TyreLife": out["TyreLife"].to_numpy(),
            "RaceProgress": out["RaceProgress"].to_numpy(),
            "_is_dry": is_dry.astype(np.int8).to_numpy(),
            "_is_wet": is_wet.astype(np.int8).to_numpy(),
        }
    ).sort_values(sort_cols, kind="mergesort")

    g = tmp.groupby(["Year", "Race", "Driver"], sort=False)

    for c in ["SOFT", "MEDIUM", "HARD"]:
        seen_before = (
            tmp["Compound"]
            .eq(c)
            .groupby([tmp["Year"], tmp["Race"], tmp["Driver"]])
            .cummax()
            .groupby([tmp["Year"], tmp["Race"], tmp["Driver"]])
            .shift(fill_value=False)
        )
        tmp[f"UsedBefore_{c}"] = seen_before.astype(np.int8)
    ...
    tmp["_wet_before"] = wet_before.astype(np.int8)

    dry_count_before = tmp[
        ["UsedBefore_SOFT", "UsedBefore_MEDIUM", "UsedBefore_HARD"]
    ].sum(axis=1)
    current_dry_seen_before = (
        (tmp["Compound"].eq("SOFT") & tmp["UsedBefore_SOFT"].astype(bool))
        | (tmp["Compound"].eq("MEDIUM") & tmp["UsedBefore_MEDIUM"].astype(bool))
        | (tmp["Compound"].eq("HARD") & tmp["UsedBefore_HARD"].astype(bool))
    )

    mandate_active = ((tmp["_wet_before"] == 0) & (tmp["_is_wet"] == 0)).astype(np.int8)
    satisfied_before = ((dry_count_before >= 2) & (mandate_active == 1)).astype(np.int8)
    distinct_dry_including_current = dry_count_before + (
        (tmp["_is_dry"] == 1) & (~current_dry_seen_before)
    ).astype(np.int8)
    satisfied_now = (
        (distinct_dry_including_current >= 2) & (mandate_active == 1)
    ).astype(np.int8)

    tmp["DryCompoundsUsedBefore"] = dry_count_before.astype(np.int8)
    tmp["MandatoryRuleAlreadySatisfied"] = satisfied_before
    tmp["MandatoryRuleSatisfiedIncludingCurrent"] = satisfied_now
    tmp["MustUseAnotherDryCompound"] = (
        (mandate_active == 1) & (satisfied_now == 0)
    ).astype(np.int8)
    tmp["WetIntermediateMandateExempt"] = (
        (tmp["_wet_before"] == 1) | (tmp["_is_wet"] == 1)
    ).astype(np.int8)
    tmp["StopNowCanSatisfyDryMandate"] = (
        (mandate_active == 1) & (satisfied_now == 0) & (tmp["_is_dry"] == 1)
    ).astype(np.int8)
    tmp["StretchingCurrentTyreStillIllegalFinish"] = (
        (mandate_active == 1) & (satisfied_now == 0) & (tmp["RaceProgress"] >= 0.70)
    ).astype(np.int8)
    tmp["MandatoryUrgency"] = tmp["MustUseAnotherDryCompound"] * tmp[
        "RaceProgress"
    ].astype(float)
    tmp["MandatoryUrgencyTyreLife"] = tmp["MustUseAnotherDryCompound"] * tmp[
        "TyreLife"
    ].astype(float)
    ...
    add_cols = [
        ...
        "UsedBefore_MEDIUM",
        "UsedBefore_HARD",
        "DryCompoundsUsedBefore",
        "MandatoryRuleAlreadySatisfied",
        "MandatoryRuleSatisfiedIncludingCurrent",
        "MustUseAnotherDryCompound",
        "WetIntermediateMandateExempt",
        "StopNowCanSatisfyDryMandate",
        "StretchingCurrentTyreStillIllegalFinish",
        "MandatoryUrgency",
        "MandatoryUrgencyTyreLife",
    ]
    tmp = tmp.sort_values("_idx", kind="mergesort")
    for c in add_cols:
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000204"],
        "research_usage_note": "Verified assigned hypothesis 000204.",
    }, sort_keys=True))
