...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year = out["Year"].astype(np.int16)

    dry = compound.isin(["SOFT", "MEDIUM", "HARD"])
    wet = compound.isin(["INTERMEDIATE", "WET"])
    monaco_2025 = (year == 2025) & race.str.contains("Monaco", case=False, regex=False)

    out["Is_Monaco_2025"] = monaco_2025.astype(np.int8)
    out["Is_Dry_Compound"] = dry.astype(np.int8)
    out["Is_Wet_Exempt_Compound"] = wet.astype(np.int8)

    max_lap = out.groupby(["Year", "Race"], sort=False)["LapNumber"].transform("max")
    laps_remaining = (max_lap - out["LapNumber"]).clip(lower=0).astype(np.float32)
    out["LapsRemaining_Est"] = laps_remaining
    out["RaceProgress_Remaining"] = (
        (1.0 - out["RaceProgress"]).clip(0, 1).astype(np.float32)
    )

    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    s = out[sort_cols].copy()
    s["_idx"] = np.arange(len(out))
    order = s.sort_values(sort_cols + ["_idx"], kind="mergesort")["_idx"].to_numpy()
    ...
    keys = [hist["Year"], hist["Race"], hist["Driver"]]

    pit = hist["PitStop"].fillna(0).astype(np.int8)
    prior_stops = (
        pit.groupby(keys, sort=False)
        .cumsum()
        .groupby(keys, sort=False)
        .shift(1, fill_value=0)
    )

    comp_hist = hist["Compound"].astype(str).str.upper()
    dry_hist = comp_hist.isin(["SOFT", "MEDIUM", "HARD"]).astype(np.int8)
    wet_hist = comp_hist.isin(["INTERMEDIATE", "WET"]).astype(np.int8)

    prior_wet = (
        wet_hist.groupby(keys, sort=False)
        .cummax()
        .groupby(keys, sort=False)
        .shift(1, fill_value=0)
    )
    ...
    prior_distinct_dry = prior_dry_soft + prior_dry_medium + prior_dry_hard
    current_adds_dry = (
        ((comp_hist == "SOFT") & (prior_dry_soft == 0))
        | ((comp_hist == "MEDIUM") & (prior_dry_medium == 0))
        | ((comp_hist == "HARD") & (prior_dry_hard == 0))
    ).astype(np.int8)
    dry_compounds_used_now = prior_distinct_dry + current_adds_dry

    sets_used_now = hist["Stint"].fillna(1).astype(np.float32)
    monaco_required_stops = pd.Series(
        np.where(
            (hist["Year"].to_numpy() == 2025)
            & hist["Race"]
            .astype(str)
            .str.contains("Monaco", case=False, regex=False)
            .to_numpy()
            & (prior_wet.to_numpy() == 0)
            & (wet_hist.to_numpy() == 0),
            2,
            1,
        ),
    ...
    remaining_mandatory_stops = (monaco_required_stops - prior_stops).clip(lower=0)
    dry_stop_debt = np.where(
        (prior_wet == 0) & (wet_hist == 0), remaining_mandatory_stops, 0
    )
    dry_compound_debt = np.where(
        (prior_wet == 0) & (wet_hist == 0),
        (2 - dry_compounds_used_now).clip(lower=0),
        0,
    )
    monaco_set_debt = np.where(
        monaco_required_stops.eq(2), (3 - sets_used_now).clip(lower=0), 0
    )

    inv = np.empty_like(order)
    inv[order] = np.arange(len(order))

    out["PriorPitStops_DriverRace"] = prior_stops.to_numpy()[inv].astype(np.float32)
    out["PriorWetExempt_DriverRace"] = prior_wet.to_numpy()[inv].astype(np.int8)
    out["DistinctDryCompounds_Used"] = dry_compounds_used_now.to_numpy()[inv].astype(
        np.float32
    )
    out["RemainingMandatoryStops_Rule"] = remaining_mandatory_stops.to_numpy()[
        inv
    ].astype(np.float32)
    out["DryStopDebt_Rule"] = np.asarray(dry_stop_debt)[inv].astype(np.float32)
    out["DryCompoundDebt_Rule"] = np.asarray(dry_compound_debt)[inv].astype(np.float32)
    out["MonacoSetDebt_Rule"] = np.asarray(monaco_set_debt)[inv].astype(np.float32)

    denom = (laps_remaining + 1.0).astype(np.float32)
    out["MandatoryStopPressure"] = (out["RemainingMandatoryStops_Rule"] / denom).astype(
        np.float32
    )
    out["DryDebtPressure"] = (
        (out["DryStopDebt_Rule"] + out["DryCompoundDebt_Rule"]) / denom
    ).astype(np.float32)
    out["MonacoRulePressure"] = (
        out["Is_Monaco_2025"]
        * (out["DryStopDebt_Rule"] + out["MonacoSetDebt_Rule"])
        / denom
    ).astype(np.float32)
    out["WetExemption_LapsRemaining"] = (
        out["Is_Wet_Exempt_Compound"] * laps_remaining
    ).astype(np.float32)
    out["DryCompoundDebt_CurrentDry"] = (
        out["DryCompoundDebt_Rule"] * out["Is_Dry_Compound"]
    ).astype(np.float32)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000379.",
    }, sort_keys=True))
