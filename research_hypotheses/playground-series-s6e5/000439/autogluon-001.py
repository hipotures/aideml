...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap = out.groupby(["Race", "LapNumber"], sort=False)
    race_lap_comp = out.groupby(["Race", "LapNumber", "Compound"], sort=False)
    ...
    for c in num_cols:
        ...
        out[f"{c}_compound_lap_pct"] = race_lap_comp[c].rank(pct=True).astype("float32")
        ...
        out[f"{c}_race_lap_z"] = ((out[c] - rl_mean) / (rl_std + eps)).astype("float32")
    ...
    stint_counts = race_lap["Stint"].transform("count")
    same_stint_counts = out.groupby(["Race", "LapNumber", "Stint"], sort=False)[
        "Driver"
    ].transform("count")
    out["alt_stint_rivals"] = (stint_counts - same_stint_counts).astype("int16")
    out["alt_stint_share"] = (
        out["alt_stint_rivals"] / (out["race_lap_cars"] - 1 + eps)
    ).astype("float32")

    out["_orig_order"] = np.arange(len(out))
    s = out.sort_values(["Race", "Driver", "LapNumber", "_orig_order"])
    gd = s.groupby(["Race", "Driver"], sort=False)
    ...
    recent = s[
        ["_orig_order", "_pit_prev1", "_pit_prev2", "_pit_prev3", "_pit_cum_before"]
    ].set_index("_orig_order")
    out = out.join(recent, on="_orig_order")
    ...
    out["go_long_weak_score"] = (
        ...
        * (1.0 - out["prior_stops"].rank(pct=True).astype("float32"))
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000439"],
        "research_usage_note": "Verified assigned hypothesis 000439.",
    }, sort_keys=True))
