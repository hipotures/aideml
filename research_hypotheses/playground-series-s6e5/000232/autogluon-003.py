...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tr = X[["RaceKey", "RaceProgress", "deg_per_tyre_lap", "is_wet_compound"]].copy()

    race_base = tr.groupby("RaceKey").agg(
        race_deg_median=("deg_per_tyre_lap", "median"),
        race_wet_share=("is_wet_compound", "mean"),
        race_stop_progress_median=("RaceProgress", "median"),
        race_progress_q25=("RaceProgress", lambda s: s.quantile(0.25)),
        race_progress_q75=("RaceProgress", lambda s: s.quantile(0.75)),
    )
    race_base["race_pit_window_width"] = (
        race_base["race_progress_q75"] - race_base["race_progress_q25"]
    )
    race_base["race_stop_rate"] = (
        1.0 / (1.0 + 10.0 * race_base["race_pit_window_width"].clip(lower=0.02))
    ).astype("float32")
