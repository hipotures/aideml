...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if n_total > n_train:
        y_domain = np.zeros(n_total, dtype=np.int8)
        y_domain[n_train:] = 1
        ...
        adv.fit(out, y_domain)
        p_test = adv.predict_proba(out)[:, 1].astype("float32")
        p_test = np.clip(p_test, 0.02, 0.98)

        prior_train = n_train / n_total
        prior_test = (n_total - n_train) / n_total
        density_ratio = (p_test / (1.0 - p_test)) * (prior_train / prior_test)

        out["adv_test_likeness_000765"] = p_test
        out["adv_density_ratio_000765"] = np.clip(density_ratio, 0.05, 20.0).astype(
            "float32"
        )
    else:
        out["adv_test_likeness_000765"] = np.float32(0.5)
        out["adv_density_ratio_000765"] = np.float32(1.0)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000765"],
        "research_usage_note": "Verified assigned hypothesis 000765.",
    }, sort_keys=True))
