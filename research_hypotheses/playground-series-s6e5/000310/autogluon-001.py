...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cat_cols:
        out[c] = out[c].astype("string").fillna("NA").str.upper()

    out["Year_cat"] = out["Year"].astype("int16").astype("string")
    out["Stint_cat"] = out["Stint"].astype("int16").astype("string")
    out["Driver_Race"] = out["Driver"] + "_" + out["Race"]
    out["Race_Year_Compound"] = (
        out["Race"] + "_" + out["Year_cat"] + "_" + out["Compound"]
    )
    out["Compound_Stint"] = out["Compound"] + "_" + out["Stint_cat"]
    out["Driver_Compound_Stint"] = (
        out["Driver"] + "_" + out["Compound"] + "_" + out["Stint_cat"]
    )
    ...
    out["TyreLife_per_Stint"] = tyre / (stint + 1.0)
    out["Degradation_per_TyreLife"] = degr / (tyre + 1.0)
    out["LapTime_per_Lap"] = laptime / (lap + 1.0)
    out["Abs_LapTime_Delta"] = delta.abs()
    out["Abs_Position_Change"] = pos_chg.abs()
    out["LateRace_OldTyre"] = progress * np.log1p(tyre)
    out["Position_x_Stint"] = pos * stint
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000310.",
    }, sort_keys=True))
