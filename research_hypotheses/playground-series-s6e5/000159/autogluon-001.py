...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    laps_remaining = (est_race_laps - lap).clip(lower=0)

    out["EstimatedRaceLaps"] = est_race_laps
    out["LapsRemaining"] = laps_remaining
    out["TyreLifeSq"] = tyre * tyre
    out["TyreLifeProgress"] = tyre * progress
    out["TyreLifeFracRace"] = tyre / (est_race_laps + eps)
    out["TyreLifeFracCurrentLap"] = tyre / (lap + eps)
    out["DegradationPerTyreLap"] = degr / (tyre + eps)
    out["DegradationProgress"] = degr * progress
    out["LapDeltaAbs"] = delta.abs()
    out["LapDeltaPerLapTime"] = delta / (laptime + eps)
    out["LapDeltaTyreLife"] = delta * tyre
    out["PositionProgress"] = pos * progress
    out["PositionChangeAbs"] = pos_chg.abs()
    out["LateRaceOldTyre"] = progress * tyre
    out["RemainingOverTyreLife"] = laps_remaining / (tyre + eps)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000159"],
        "research_usage_note": "Verified assigned hypothesis 000159.",
    }, sort_keys=True))
