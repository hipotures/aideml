...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Stint"] = df["Stint"].astype("int64").astype("string")

    # Raw categorical interactions for CatBoost-style handling
    df["Year_Race"] = df["Year"] + "_" + df["Race"]
    df["Driver_Race"] = df["Driver"] + "_" + df["Race"]
    df["Driver_Compound"] = df["Driver"] + "_" + df["Compound"]
    df["Race_Compound"] = df["Race"] + "_" + df["Compound"]
    df["Year_Race_Compound"] = df["Year_Race"] + "_" + df["Compound"]
    df["Stint_Compound"] = df["Stint"] + "_" + df["Compound"]

    # Simple causal numeric context
    lap_num = df["LapNumber"].astype("float32")
    tyre_life = df["TyreLife"].astype("float32")
    race_prog = df["RaceProgress"].astype("float32").clip(1e-6, 1.0)
    ...
    df["TyreLife_Ratio"] = (tyre_life / lap_num.clip(lower=1.0)).astype("float32")
    df["StintProgress"] = (tyre_life / (lap_num + 1.0)).astype("float32")
    df["LapRemainingFrac"] = (1.0 - race_prog).astype("float32")
    df["Degradation_Per_Lap"] = (degr / tyre_life.clip(lower=1.0)).astype("float32")
    df["Delta_x_TyreLife"] = (lap_delta * tyre_life).astype("float32")
    df["Delta_x_Degradation"] = (lap_delta * degr).astype("float32")
    df["LapTime_x_RaceProgress"] = (lap_time * race_prog).astype("float32")
    df["PositionPressure"] = ((21.0 - pos) * race_prog).astype("float32")
    df["PositionChangePressure"] = (pos_chg * race_prog).astype("float32")

    # In-lap style indicators from current-lap observables only
    df["SlowLapFlag"] = (lap_delta > 5.0).astype("int8")
    df["VerySlowLapFlag"] = (lap_delta > 15.0).astype("int8")
    df["HighWearFlag"] = (tyre_life >= 15).astype("int8")
    df["LateRaceFlag"] = (race_prog >= 0.7).astype("int8")
    df["EarlyStintFlag"] = (tyre_life <= 3).astype("int8")

    # Keep interaction columns as categorical/string
    new_cat_cols = [
        "Year",
        "Stint",
        "Year_Race",
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000901.",
    }, sort_keys=True))
