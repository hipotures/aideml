...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    compound = out["Compound"].astype("string").str.upper()
    out["is_wet_tyre"] = compound.isin(["INTERMEDIATE", "WET"]).astype("int8")
    out["is_slick_tyre"] = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    out["abs_position_change"] = out["Position_Change"].abs().astype("float32")

    lap_agg = (
        out.groupby(keys, sort=False, observed=True)
        .agg(
            rl_laptime_delta_median=("LapTime_Delta", "median"),
            rl_laptime_delta_std=("LapTime_Delta", "std"),
            rl_laptime_median=("LapTime (s)", "median"),
            rl_wet_share=("is_wet_tyre", "mean"),
            rl_pit_count=("PitStop", "sum"),
            rl_pit_share=("PitStop", "mean"),
            rl_position_churn_mean=("abs_position_change", "mean"),
            rl_position_churn_max=("abs_position_change", "max"),
            rl_field_size=("PitStop", "size"),
        )
        .reset_index()
        .sort_values(keys)
    ...
    lap_agg["rl_laptime_delta_std"] = lap_agg["rl_laptime_delta_std"].fillna(0.0)
    by_race = lap_agg.groupby(race_keys, sort=False, observed=True)
    lap_agg["rl_prev_laptime_delta_median"] = by_race["rl_laptime_delta_median"].shift(
        1
    )
    lap_agg["rl_prev_wet_share"] = by_race["rl_wet_share"].shift(1)
    lap_agg["rl_prev_position_churn_mean"] = by_race["rl_position_churn_mean"].shift(1)

    lap_agg["rl_wet_share_change"] = (
        lap_agg["rl_wet_share"] - lap_agg["rl_prev_wet_share"]
    ).fillna(0.0)

    pit_cluster = lap_agg["rl_pit_count"] >= np.maximum(
        2, np.ceil(lap_agg["rl_field_size"] * 0.10)
    )
    neutralized = (
        (lap_agg["rl_laptime_delta_median"] > 8.0)
        & (
            (lap_agg["rl_laptime_delta_std"] < 12.0)
            | pit_cluster
            | (lap_agg["rl_position_churn_mean"] <= 1.0)
        )
    ) | (lap_agg["rl_laptime_delta_median"] > 20.0)
    wet_transition = (lap_agg["rl_wet_share_change"].abs() >= 0.15) | (
        (lap_agg["rl_wet_share"] > 0.05) & (lap_agg["rl_wet_share"] < 0.95)
    )
    restart = (
        neutralized.shift(1).fillna(False)
        & (lap_agg[race_keys].eq(lap_agg[race_keys].shift(1)).all(axis=1))
        & (
            (
                lap_agg["rl_laptime_delta_median"]
                < lap_agg["rl_prev_laptime_delta_median"].fillna(
                    lap_agg["rl_laptime_delta_median"]
                )
                - 4.0
            )
            | (
                lap_agg["rl_position_churn_mean"]
                > lap_agg["rl_prev_position_churn_mean"].fillna(
                    lap_agg["rl_position_churn_mean"]
                )
                + 0.75
            )
    ...
    lap_agg["rl_is_neutralized"] = neutralized.astype("int8")
    lap_agg["rl_is_wet_transition"] = wet_transition.astype("int8")
    lap_agg["rl_is_restart"] = restart.astype("int8")
    lap_agg["rl_regime_code"] = np.select(
        [restart, wet_transition, neutralized], [3, 2, 1], default=0
    ).astype("int8")
    lap_agg["rl_regime"] = np.array(
        ["green", "neutralized", "wet_transition", "restart"], dtype=object
    )[lap_agg["rl_regime_code"]]

    merge_cols = keys + [
        "rl_laptime_delta_median",
        "rl_laptime_delta_std",
        "rl_laptime_median",
        "rl_wet_share",
        "rl_pit_count",
        "rl_pit_share",
        "rl_position_churn_mean",
        "rl_position_churn_max",
        "rl_field_size",
        "rl_wet_share_change",
        "rl_is_neutralized",
        "rl_is_wet_transition",
        "rl_is_restart",
        "rl_regime_code",
        "rl_regime",
    ]
    out = out.merge(lap_agg[merge_cols], on=keys, how="left", sort=False)

    tyre_life = out["TyreLife"].astype("float32")
    race_progress = out["RaceProgress"].clip(0, 1).astype("float32")
    degradation = out["Cumulative_Degradation"].astype("float32")
    stint = out["Stint"].astype("float32")

    out["tyre_age_pressure"] = (np.log1p(tyre_life) * (1.0 + race_progress)).astype(
        "float32"
    )
    out["degradation_per_tyre_lap"] = (degradation / np.maximum(tyre_life, 1.0)).astype(
        "float32"
    )
    out["stint_progress_pressure"] = (stint * race_progress).astype("float32")
    out["field_pit_share_x_tyre_age"] = (
        out["rl_pit_share"] * out["tyre_age_pressure"]
    ).astype("float32")
    out["field_pit_count_x_degradation"] = (out["rl_pit_count"] * degradation).astype(
        "float32"
    )
    out["wet_share_x_tyre_age"] = (
        out["rl_wet_share"] * out["tyre_age_pressure"]
    ).astype("float32")

    for state in ["neutralized", "wet_transition", "restart"]:
        flag = out[f"rl_is_{state}"].astype("float32")
        out[f"tyrelife_x_{state}"] = (tyre_life * flag).astype("float32")
        out[f"degradation_x_{state}"] = (degradation * flag).astype("float32")
        out[f"raceprogress_x_{state}"] = (race_progress * flag).astype("float32")
        out[f"stint_x_{state}"] = (stint * flag).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000434"],
        "research_usage_note": "Verified assigned hypothesis 000434.",
    }, sort_keys=True))
