...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race = out["Race"].astype("string").str.strip().fillna("NA")
    driver = out["Driver"].astype("string").str.strip().str.upper().fillna("NA")
    compound = out["Compound"].astype("string").str.strip().str.upper().fillna("NA")
    year = out["Year"].astype("string").fillna("NA")

    out["Race"] = race.astype("category")
    out["Driver"] = driver.astype("category")
    out["Compound"] = compound.astype("category")
    out["Race_Year"] = (race + "_" + year).astype("category")
    ...
    sort_idx = out.sort_values(
        ["Race_Year", "Driver", "LapNumber"], kind="mergesort"
    ).index
    seq = out.loc[sort_idx]
    grp = seq.groupby(["Race_Year", "Driver"], sort=False, observed=True)

    seq_pos = grp.cumcount()
    prev_pit = grp["PitStop"].shift(1)
    prev2_pit = grp["PitStop"].shift(2)
    stint_changed = grp["Stint"].diff().fillna(0).ne(0)
    purge_gap = seq_pos.eq(0) | prev_pit.eq(1) | prev2_pit.eq(1) | stint_changed

    out.loc[sort_idx, "seq_pos_in_race_driver"] = seq_pos.astype("float32")
    out.loc[sort_idx, "purge_gap_000188"] = purge_gap.astype("int8")
    ...
    lag_cols = list(safe.keys())
    for col in lag_cols:
        lag1 = grp[col].shift(1).mask(purge_gap)
        out.loc[sort_idx, f"{safe[col]}_lag1_purged"] = lag1.astype("float32")
    ...
    for col in [
        ...
        delta = (seq[col] - prev).mask(purge_gap)
        out.loc[sort_idx, f"{safe[col]}_delta_from_lag1_purged"] = delta.astype(
            "float32"
        )

    roll_keys = [seq["Race_Year"], seq["Driver"]]
    for col in ["LapTime_Delta", "Position_Change", "Cumulative_Degradation"]:
        shifted = grp[col].shift(1).mask(purge_gap)
        roll = (
            shifted.groupby(roll_keys, sort=False, observed=True)
            .rolling(3, min_periods=2)
            .mean()
            .reset_index(level=[0, 1], drop=True)
            .reindex(sort_idx)
        )
        out.loc[sort_idx, f"{safe[col]}_roll3_mean_purged"] = roll.astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000188"],
        "research_usage_note": "Verified assigned hypothesis 000188.",
    }, sort_keys=True))
