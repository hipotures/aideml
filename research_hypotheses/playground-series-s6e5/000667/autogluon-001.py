...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    g = out.groupby(["Race_Year", "LapNumber"], sort=False)
    for c in [
        "TyreLife",
        "LapTime_Delta",
        "Cumulative_Degradation",
        ...
        r = g[c].rank(method="average", pct=True)
        out[f"{c}_field_pct"] = r.astype("float32")
        med = g[c].transform("median")
        q25 = g[c].transform(lambda s: s.quantile(0.25))
        q75 = g[c].transform(lambda s: s.quantile(0.75))
        iqr = (q75 - q25).replace(0, np.nan)
        out[f"{c}_field_iqr_dist"] = (
            ((out[c] - med) / iqr)
    ...
    pace_loss = (
        out["LapTime_Delta"]
        + 0.03 * out["Cumulative_Degradation"]
        + 0.10 * out["TyreLife"]
    )
    out["PaceLossProxy"] = pace_loss.astype("float32")
    out["PaceLoss_field_pct"] = (
        g["PaceLossProxy"].rank(method="average", pct=True).astype("float32")
    )

    sort_cols = ["Race_Year", "LapNumber", "Position"]
    tmp = out[
        sort_cols
        + [
            "TyreLife",
    ...
    for c in [
        ...
        tmp[f"ahead_{c}"] = tmp.groupby(["Race_Year", "LapNumber"], sort=False)[
            c
        ].shift(1)
        tmp[f"behind_{c}"] = tmp.groupby(["Race_Year", "LapNumber"], sort=False)[
            c
        ].shift(-1)

    tmp["ahead_pit_risk"] = (
        0.45 * tmp["ahead_PaceLossProxy"].rank(pct=True)
        + 0.35 * tmp["ahead_TyreLife"].rank(pct=True)
        + 0.20 * tmp["ahead_Cumulative_Degradation"].rank(pct=True)
    )
    tmp["behind_pit_risk"] = (
        0.45 * tmp["behind_PaceLossProxy"].rank(pct=True)
        + 0.35 * tmp["behind_TyreLife"].rank(pct=True)
        + 0.20 * tmp["behind_Cumulative_Degradation"].rank(pct=True)
    )

    tmp["ahead_undercut_threat"] = (
        (tmp["ahead_TyreLife"].fillna(-1) >= tmp["TyreLife"].fillna(-1) - 2)
        & (tmp["ahead_PaceLossProxy"].fillna(-999) >= tmp["PaceLossProxy"].fillna(-999))
    ).astype("int8")
    tmp["behind_undercut_threat"] = (
        (tmp["behind_TyreLife"].fillna(-1) >= tmp["TyreLife"].fillna(-1) - 2)
        & (
            tmp["behind_PaceLossProxy"].fillna(-999)
            >= tmp["PaceLossProxy"].fillna(-999)
        )
    ).astype("int8")
    ...
    keep = [
        ...
        "ahead_Cumulative_Degradation",
        "behind_Cumulative_Degradation",
        "ahead_PaceLossProxy",
        "behind_PaceLossProxy",
        "ahead_PitStop",
        "behind_PitStop",
        "ahead_pit_risk",
        "behind_pit_risk",
        "ahead_undercut_threat",
        "behind_undercut_threat",
    ]
    tmp = tmp[keep].sort_values("_idx", kind="mergesort").drop(columns="_idx")
    for c in tmp.columns:
        out[c] = tmp[c].to_numpy()

    out["rival_tyrelife_gap_ahead"] = (
        (out["TyreLife"] - out["ahead_TyreLife"]).fillna(0).astype("float32")
    )
    out["rival_tyrelife_gap_behind"] = (
        (out["TyreLife"] - out["behind_TyreLife"]).fillna(0).astype("float32")
    )
    out["rival_pace_loss_gap_ahead"] = (
        (out["PaceLossProxy"] - out["ahead_PaceLossProxy"]).fillna(0).astype("float32")
    )
    out["rival_pace_loss_gap_behind"] = (
        (out["PaceLossProxy"] - out["behind_PaceLossProxy"]).fillna(0).astype("float32")
    )

    out["field_pit_count"] = g["PitStop"].transform("sum").astype("float32")
    out["field_pit_rate"] = g["PitStop"].transform("mean").astype("float32")
    out["nearby_recent_pit_signal"] = (
        out[["ahead_PitStop", "behind_PitStop"]]
        .fillna(0)
        .mean(axis=1)
        .astype("float32")
    )
    ...
    out["pit_window_core"] = (
        (out["TyreLife"] >= 12) & (out["RaceProgress"].between(0.18, 0.82))
    ).astype("int8")
    out["traffic_undercut_window"] = (
        (out["pit_window_core"].eq(1))
        & ((out["ahead_undercut_threat"].eq(1)) | (out["behind_undercut_threat"].eq(1)))
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000667"],
        "research_usage_note": "Verified assigned hypothesis 000667.",
    }, sort_keys=True))
