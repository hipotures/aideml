...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    eps = 1e-6
    out["lap_frac_in_race"] = out["LapNumber"] / out.groupby(
        ["Year", "Race"], observed=True
    )["LapNumber"].transform("max").clip(lower=1)
    out["tyrelife_frac_race"] = out["TyreLife"] / out.groupby(
        ["Year", "Race"], observed=True
    )["TyreLife"].transform("max").clip(lower=1)
    out["stint_x_tyre"] = out["Stint"] * out["TyreLife"]
    ...
    out["laptime_per_progress"] = out["LapTime (s)"] / (out["RaceProgress"] + eps)
    out["position_pressure"] = out["Position"] + out["Position_Change"]
    out["late_stint_proxy"] = np.log1p(out["TyreLife"]) * out["RaceProgress"]
    out["horizon_risk_proxy"] = (
        0.45 * np.log1p(out["TyreLife"])
        + 0.35 * out["RaceProgress"]
        + 0.20 * np.log1p(out["Stint"])
    )
    ...
    for col in [
        ...
        s[f"{col}_lag1"] = g[col].shift(1)
        s[f"{col}_lag2"] = g[col].shift(2)
        s[f"{col}_roll3_mean"] = (
            g[col]
            .shift(1)
    ...
    s["known_laps_since_pit"] = g["PitStop"].cumsum()
    s["known_laps_since_pit"] = g.cumcount() - g["known_laps_since_pit"].where(
        s["PitStop"].eq(1)
    ).groupby([s["Year"], s["Race"], s["Driver"]], observed=True).ffill().fillna(-1)
    s["known_laps_since_pit"] = s["known_laps_since_pit"].clip(lower=0)

    hist_cols = [c for c in s.columns if c not in out.columns]
    out.loc[s.index, hist_cols] = s[hist_cols]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000127"],
        "research_usage_note": "Verified assigned hypothesis 000127.",
    }, sort_keys=True))
