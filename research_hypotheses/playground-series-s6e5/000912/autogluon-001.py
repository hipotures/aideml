...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    slick_map = {"SOFT": 0, "MEDIUM": 1, "HARD": 2}
    d["CompoundCode"] = d["Compound"].map(compound_map).fillna(-1).astype("int8")
    d["SlickCode"] = d["Compound"].map(slick_map).fillna(-1).astype("int8")
    d["IsWetRaceTyre"] = d["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    d["IsSlickTyre"] = (d["IsWetRaceTyre"] == 0).astype("int8")

    grp = d.groupby(["Year", "Race", "Driver"], sort=False)
    d["DriverLapIdx"] = grp.cumcount().astype("int16")

    prev_lap_time = grp["LapTime (s)"].shift(1)
    prev_lap_delta = grp["LapTime_Delta"].shift(1)
    prev_tyre_life = grp["TyreLife"].shift(1)
    prev_deg = grp["Cumulative_Degradation"].shift(1)
    prev_pos = grp["Position"].shift(1)

    d["PrevLapTime"] = prev_lap_time
    d["PrevLapDelta"] = prev_lap_delta
    d["PrevTyreLife"] = prev_tyre_life
    d["PrevDeg"] = prev_deg
    d["PrevPosition"] = prev_pos

    d["LapTimeChange"] = d["LapTime (s)"] - prev_lap_time
    d["LapDeltaChange"] = d["LapTime_Delta"] - prev_lap_delta
    d["TyreLifeStep"] = d["TyreLife"] - prev_tyre_life
    d["DegStep"] = d["Cumulative_Degradation"] - prev_deg
    d["PositionStep"] = d["Position"] - prev_pos

    prior_count = d["DriverLapIdx"].replace(0, pd.NA)
    d["PastMeanLapDelta"] = (
        grp["LapTime_Delta"].cumsum() - d["LapTime_Delta"]
    ) / prior_count
    d["PastMeanLapTime"] = (
        grp["LapTime (s)"].cumsum() - d["LapTime (s)"]
    ) / prior_count
    d["PastMeanDeg"] = (
        grp["Cumulative_Degradation"].cumsum() - d["Cumulative_Degradation"]
    ) / prior_count

    race_progress = d["RaceProgress"].replace(0, pd.NA)
    d["EstimatedTotalLaps"] = (d["LapNumber"] / race_progress).clip(lower=1, upper=100)
    d["EstimatedLapsRemaining"] = (d["EstimatedTotalLaps"] - d["LapNumber"]).clip(
        lower=0, upper=80
    )

    d["DegPerTyreLap"] = d["Cumulative_Degradation"] / d["TyreLife"].clip(lower=1)
    d["TyreLifeShare"] = d["TyreLife"] / d["EstimatedTotalLaps"].clip(lower=1)
    d["StintPressure"] = d["TyreLife"] / (d["EstimatedLapsRemaining"] + 1.0)
    d["PaceFadeVsHistory"] = d["LapTime_Delta"] - d["PastMeanLapDelta"]
    d["RawStopPressure"] = (
        d["DegPerTyreLap"].fillna(0)
        + d["StintPressure"].fillna(0)
        + d["PaceFadeVsHistory"].clip(lower=0).fillna(0)
        + d["LapDeltaChange"].clip(lower=0).fillna(0) / 10.0
    )

    d["AuxLapsToStopProxy"] = (
        d["EstimatedLapsRemaining"] / (1.0 + d["RawStopPressure"].abs())
    ).clip(lower=0, upper=80)

    d["NextCompoundRegimeCode"] = (
        d["IsWetRaceTyre"] * 10
        + d["IsSlickTyre"] * d["SlickCode"].clip(lower=0)
        + (d["RawStopPressure"] > d["RawStopPressure"].median()).astype("int8")
    ).astype("int8")

    d["WetStopPressure"] = d["RawStopPressure"] * d["IsWetRaceTyre"]
    d["SlickStopPressure"] = d["RawStopPressure"] * d["IsSlickTyre"]
    d["CompoundPressureInteraction"] = d["CompoundCode"] * d["RawStopPressure"]
    d["StintCompoundInteraction"] = d["Stint"] * d["SlickCode"].clip(lower=0)
    d["TyreLifeCompoundInteraction"] = d["TyreLife"] * d["SlickCode"].clip(lower=0)
    d["DegCompoundInteraction"] = d["Cumulative_Degradation"] * d["SlickCode"].clip(
        lower=0
    )

    d["DriverFreq"] = (
        d.groupby("Driver", sort=False)["Driver"].transform("size").astype("int32")
    )
    d["RaceFreq"] = (
        d.groupby("Race", sort=False)["Race"].transform("size").astype("int32")
    )
    d["CompoundFreq"] = (
        d.groupby("Compound", sort=False)["Compound"].transform("size").astype("int32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000912.",
    }, sort_keys=True))
