...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["LapTime_Delta", "Cumulative_Degradation"]:
        lo = s.groupby("Compound")[col].transform(lambda x: x.quantile(0.01))
        hi = s.groupby("Compound")[col].transform(lambda x: x.quantile(0.99))
        s[col + "_win"] = s[col].clip(lo, hi)

    g = s.groupby(keys, sort=False)
    for col in ["LapTime_Delta_win", "Cumulative_Degradation_win"]:
        base = col.replace("_win", "")
        lag1 = g[col].shift(1)
        lag2 = g[col].shift(2)
        d1 = s[col] - lag1
        d2 = s[col] - 2 * lag1 + lag2

        s[base + "_causal_diff1"] = d1.fillna(0)
        s[base + "_causal_diff2"] = d2.fillna(0)

        for w in (3, 5, 8):
            s[base + f"_slope_r{w}"] = (
                d1.groupby([s[k] for k in keys], sort=False)
            ...
            s[base + f"_accel_r{w}"] = (
                d2.groupby([s[k] for k in keys], sort=False)
                .rolling(w, min_periods=2)
                .mean()
                .reset_index(level=list(range(len(keys))), drop=True)
                .fillna(0)
        ...
        early_masked = s[col].where(s["TyreLife"] <= 3)
        early_mean = early_masked.groupby([s[k] for k in keys], sort=False).transform(
            "mean"
        )
        early_std = early_masked.groupby([s[k] for k in keys], sort=False).transform(
            "std"
        )
        fallback_mean = s.groupby(["Race", "Year", "Compound"], sort=False)[
            col
        ].transform("median")
        fallback_std = s.groupby(["Race", "Year", "Compound"], sort=False)[
            col
        ].transform("std")

        mean = early_mean.fillna(fallback_mean).fillna(s[col].median())
        std = (
            early_std.fillna(fallback_std)
            .replace(0, np.nan)
            .fillna(s[col].std() + 1e-6)
        )
        z = (s[col] - mean) / (std + 1e-6)

        s[base + "_early_baseline"] = mean
        s[base + "_early_z"] = z.replace([np.inf, -np.inf], 0).fillna(0)
        s[base + "_cliff_flag"] = ((z > 2.0) & (s["TyreLife"] >= 5)).astype("int8")

    s["degradation_pace_accel_sum"] = (
        s["LapTime_Delta_accel_r5"] + s["Cumulative_Degradation_accel_r5"]
    )
    s["double_cliff_flag"] = (
        (s["LapTime_Delta_cliff_flag"] == 1)
        & (s["Cumulative_Degradation_cliff_flag"] == 1)
    ).astype("int8")

    return s.loc[out.index]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000519"],
        "research_usage_note": "Verified assigned hypothesis 000519.",
    }, sort_keys=True))
