...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap_base = (
        lap_time.groupby([X["Year"], X["Race"], X["LapNumber"]])
        .transform("median")
        .astype("float32")
    )
    resid0 = (lap_time - race_lap_base).astype("float32")
    compound_base = (
        resid0.groupby([X["Year"], X["Race"], X["Compound"]])
        .transform("median")
        .astype("float32")
    )
    resid = (resid0 - compound_base).clip(-300, 300).astype("float32")

    X["LapTime_Residual"] = resid
    X["LapTime_Residual_Abs"] = resid.abs().astype("float32")

    n = len(X)
    year = X["Year"].to_numpy()
    race_code = pd.factorize(X["Race"], sort=False)[0]
    driver_code = pd.factorize(X["Driver"], sort=False)[0]
    lap_num = X["LapNumber"].to_numpy()
    stint = X["Stint"].to_numpy()
    pit = X["PitStop"].to_numpy()
    lap_delta = X["LapTime_Delta"].to_numpy(dtype=np.float32)
    pos_change = X["Position_Change"].to_numpy(dtype=np.float32)
    obs = resid.to_numpy(dtype=np.float32)

    order = np.lexsort((lap_num, driver_code, race_code, year))

    level_out = np.zeros(n, dtype=np.float32)
    slope_out = np.zeros(n, dtype=np.float32)
    shock_out = np.zeros(n, dtype=np.float32)
    uncert_out = np.zeros(n, dtype=np.float32)
    conf_state_out = np.zeros(n, dtype=np.float32)

    prev_year = prev_race = prev_driver = None
    prev_stint = None
    level = 0.0
    slope = 0.0
    var = 9.0
    ...
    for i in order:
        new_group = (
            (year[i] != prev_year)
            or (race_code[i] != prev_race)
            or (driver_code[i] != prev_driver)
        )
        reset = (
            new_group
            or (prev_stint is not None and stint[i] != prev_stint)
            or (pit[i] == 1)
        )
        ...
        if pit[i] == 1:
            ...
            slope_out[i] = 0.0
            shock_out[i] = 0.0
            uncert_out[i] = np.float32(var)
            conf_state_out[i] = 0.0
        else:
            prior_level = level + 0.85 * slope
            prior_slope = 0.60 * slope
            prior_var = min(var + 2.5, 64.0)

            noise = (
                16.0 + 0.05 * abs(float(lap_delta[i])) + 1.5 * abs(float(pos_change[i]))
            )
            innovation = float(obs[i]) - prior_level
            gain = prior_var / (prior_var + noise)

            post_level = prior_level + gain * innovation
            post_slope = 0.70 * prior_slope + 0.30 * (post_level - level)
            post_var = (1.0 - gain) * prior_var + 0.15 * abs(innovation)
            shock = innovation / np.sqrt(prior_var + noise)

            level_out[i] = np.float32(post_level)
            slope_out[i] = np.float32(post_slope)
            shock_out[i] = np.float32(shock)
            uncert_out[i] = np.float32(post_var)
            conf_state_out[i] = np.float32(post_level / np.sqrt(post_var + 1.0))
        ...
        prev_year = year[i]
        prev_race = race_code[i]
        prev_driver = driver_code[i]
        prev_stint = stint[i]

    X["LatentDeg_Level"] = level_out
    X["LatentDeg_Slope"] = slope_out
    X["LatentDeg_Shock"] = shock_out
    X["LatentDeg_Uncertainty"] = uncert_out
    X["LatentDeg_ConfState"] = conf_state_out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000302"],
        "research_usage_note": "Verified assigned hypothesis 000302.",
    }, sort_keys=True))
