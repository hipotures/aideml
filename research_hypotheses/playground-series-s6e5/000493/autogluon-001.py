...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    neighbor = out[base_cols + neigh_cols].copy()

    ahead = neighbor.rename(
        columns={
            "Position": "ahead_join_position",
            "TyreLife": "ahead_tyre_life",
            "Compound": "ahead_compound",
            "Stint": "ahead_stint",
            "PitStop": "ahead_pitstop",
            "lap_laptime_resid": "ahead_laptime_resid",
            "Position_Change": "ahead_position_change",
            "Cumulative_Degradation": "ahead_cum_deg",
            "LapTime_Delta": "ahead_laptime_delta",
        }
    )
    out["ahead_join_position"] = out["Position"] - 1
    out = out.merge(ahead, on=key_cols + ["ahead_join_position"], how="left")

    behind = neighbor.rename(
        columns={
            "Position": "behind_join_position",
            "TyreLife": "behind_tyre_life",
            "Compound": "behind_compound",
            "Stint": "behind_stint",
            "PitStop": "behind_pitstop",
            "lap_laptime_resid": "behind_laptime_resid",
            "Position_Change": "behind_position_change",
            "Cumulative_Degradation": "behind_cum_deg",
            "LapTime_Delta": "behind_laptime_delta",
        }
    )
    out["behind_join_position"] = out["Position"] + 1
    out = out.merge(behind, on=key_cols + ["behind_join_position"], how="left")
    ...
    return out.drop(columns=["ahead_join_position", "behind_join_position"])
