...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key_cols = ["Race", "Year", "LapNumber"]

    grp = out.groupby(key_cols, sort=False)
    out["lap_laptime_mean"] = grp["LapTime (s)"].transform("mean")
    out["lap_laptime_resid"] = out["LapTime (s)"] - out["lap_laptime_mean"]
    ...
    neighbor = out[key_cols + ["Position"] + neigh_cols].copy()
    neighbor["_row_id"] = row_id
    asc = neighbor.sort_values(key_cols + ["Position", "_row_id"], kind="mergesort")
    asc_grp = asc.groupby(key_cols, sort=False)
    asc_rows = asc["_row_id"].to_numpy(dtype=np.int64)

    desc = neighbor.sort_values(
        key_cols + ["Position", "_row_id"],
        ascending=[True] * len(key_cols) + [False, True],
        kind="mergesort",
    )
    desc_grp = desc.groupby(key_cols, sort=False)
    desc_rows = desc["_row_id"].to_numpy(dtype=np.int64)

    neighbor_map = {
        "TyreLife": ("ahead_tyre_life", "behind_tyre_life"),
        "Stint": ("ahead_stint", "behind_stint"),
        "PitStop": ("ahead_pitstop", "behind_pitstop"),
        "lap_laptime_resid": ("ahead_laptime_resid", "behind_laptime_resid"),
        "Position_Change": ("ahead_position_change", "behind_position_change"),
        "Cumulative_Degradation": ("ahead_cum_deg", "behind_cum_deg"),
        "LapTime_Delta": ("ahead_laptime_delta", "behind_laptime_delta"),
    }
    for src, (ahead_name, behind_name) in neighbor_map.items():
        ahead = np.full(n, np.nan, dtype=float)
        behind = np.full(n, np.nan, dtype=float)
        ahead[asc_rows] = (
            pd.to_numeric(asc_grp[src].shift(1), errors="coerce")
            .to_numpy(dtype=float)
        )
        behind[desc_rows] = (
            pd.to_numeric(desc_grp[src].shift(1), errors="coerce")
            .to_numpy(dtype=float)
        )
        out[ahead_name] = ahead
        out[behind_name] = behind

    for src, (ahead_name, behind_name) in {
        "Compound": ("ahead_compound", "behind_compound")
    }.items():
        ahead = np.empty(n, dtype=object)
        behind = np.empty(n, dtype=object)
        ahead[:] = None
        behind[:] = None
        ahead[asc_rows] = asc_grp[src].shift(1).to_numpy(dtype=object)
        behind[desc_rows] = desc_grp[src].shift(1).to_numpy(dtype=object)
        out[ahead_name] = ahead
        out[behind_name] = behind

    out["older_than_car_ahead"] = out["TyreLife"] - out["ahead_tyre_life"]
    out["fresher_than_car_behind"] = out["behind_tyre_life"] - out["TyreLife"]
    out["ahead_same_compound"] = (out["Compound"] == out["ahead_compound"]).astype(
        "int8"
    )
    out["behind_same_compound"] = (out["Compound"] == out["behind_compound"]).astype(
        "int8"
    )
    out["ahead_neighbor_just_pitted"] = out["ahead_pitstop"].fillna(0).astype("int8")
    out["behind_neighbor_just_pitted"] = out["behind_pitstop"].fillna(0).astype("int8")
    out["neighbor_just_pitted"] = (
        (out["ahead_neighbor_just_pitted"] == 1)
        | (out["behind_neighbor_just_pitted"] == 1)
    ).astype("int8")
    out["pace_delta_to_car_ahead"] = (
        out["lap_laptime_resid"] - out["ahead_laptime_resid"]
    )
    out["pace_delta_to_car_behind"] = (
        out["lap_laptime_resid"] - out["behind_laptime_resid"]
    )
    out["pace_delta_to_adjacent_position"] = (
        out[["pace_delta_to_car_ahead", "pace_delta_to_car_behind"]].abs().min(axis=1)
    )
    out["stint_delta_to_ahead"] = out["Stint"] - out["ahead_stint"]
    out["stint_delta_to_behind"] = out["Stint"] - out["behind_stint"]
    out["deg_delta_to_ahead"] = out["Cumulative_Degradation"] - out["ahead_cum_deg"]
    out["deg_delta_to_behind"] = out["Cumulative_Degradation"] - out["behind_cum_deg"]
    out["position_change_pressure"] = out["ahead_position_change"].fillna(0) - out[
        "behind_position_change"
    ].fillna(0)
    out["adjacent_laptime_delta_gap"] = (
        out["ahead_laptime_delta"] - out["behind_laptime_delta"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000493"],
        "research_usage_note": "Verified assigned hypothesis 000493.",
    }, sort_keys=True))
