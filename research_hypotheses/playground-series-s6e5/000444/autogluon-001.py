...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["CompoundCode"] = compound_code
    df["IsWetTyre"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    df["IsSlickTyre"] = (1 - df["IsWetTyre"]).astype("int8")
    df["CompoundSoftness"] = (4 - compound_code).clip(lower=0).astype("int8")

    grp = df.groupby(["Year", "Race", "Driver"], sort=False)

    for lag in (1, 2, 3):
        df[f"LapTimeDelta_lag{lag}"] = grp["LapTime_Delta"].shift(lag).fillna(0.0)
        df[f"PosChange_lag{lag}"] = grp["Position_Change"].shift(lag).fillna(0.0)
        df[f"LapTimeSec_lag{lag}"] = (
            grp["LapTime (s)"].shift(lag).fillna(df["LapTime (s)"].median())
        )

    pos_deg = df["Cumulative_Degradation"].clip(lower=0)
    pos_delta = df["LapTime_Delta"].clip(lower=0)
    neg_pos_change = (-df["Position_Change"]).clip(lower=0)

    lag_delta_mean = (
        df["LapTimeDelta_lag1"].clip(lower=0)
        + df["LapTimeDelta_lag2"].clip(lower=0)
        + df["LapTimeDelta_lag3"].clip(lower=0)
    ) / 3.0
    lag_posloss_mean = (
        (-df["PosChange_lag1"]).clip(lower=0)
        + (-df["PosChange_lag2"]).clip(lower=0)
        + (-df["PosChange_lag3"]).clip(lower=0)
    ) / 3.0

    df["TyreLife_x_Degradation"] = df["TyreLife"] * np.log1p(pos_deg)
    df["TyreLife_x_RaceProgress"] = df["TyreLife"] * df["RaceProgress"]
    df["Degradation_x_RaceProgress"] = np.log1p(pos_deg) * df["RaceProgress"]
    df["PaceFadePressure"] = pos_delta + lag_delta_mean
    df["TrackPositionPressure"] = neg_pos_change + lag_posloss_mean
    df["LateRaceFlag"] = (df["RaceProgress"] >= 0.7).astype("int8")
    df["LongStintFlag"] = (df["TyreLife"] >= 15).astype("int8")
    df["VeryLongStintFlag"] = (df["TyreLife"] >= 25).astype("int8")

    base_hazard = (
        0.55 * np.log1p(df["TyreLife"].clip(lower=0))
        + 0.35 * np.log1p(pos_deg)
        + 0.20 * pos_delta
        + 0.15 * lag_delta_mean
        + 0.10 * neg_pos_change
        + 0.08 * lag_posloss_mean
        + 0.25 * df["RaceProgress"]
        + 0.08 * df["Stint"]
        - 0.12 * df["PitStop"]
        - 0.04 * compound_code
    )
    ...
    horizon_step = (
        0.30
        + 0.06 * np.log1p(pos_deg)
        + 0.05 * lag_delta_mean
        + 0.03 * df["RaceProgress"]
        + 0.02 * df["TyreLife"] / (df["LapNumber"] + 1.0)
    )

    df["CumHazard_1lap"] = base_hazard
    df["CumHazard_2lap"] = base_hazard + horizon_step
    df["CumHazard_3lap"] = base_hazard + 2.0 * horizon_step
    df["HazardGap_12"] = horizon_step
    df["HazardGap_23"] = horizon_step

    df["TyreLife_h1"] = df["TyreLife"] + 1.0
    df["TyreLife_h2"] = df["TyreLife"] + 2.0
    df["TyreLife_h3"] = df["TyreLife"] + 3.0
    df["DegPressure_h1"] = np.log1p(pos_deg + pos_delta)
    df["DegPressure_h2"] = np.log1p(pos_deg + 2.0 * pos_delta + lag_delta_mean)
    df["DegPressure_h3"] = np.log1p(pos_deg + 3.0 * pos_delta + 2.0 * lag_delta_mean)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000444"],
        "research_usage_note": "Verified assigned hypothesis 000444.",
    }, sort_keys=True))
