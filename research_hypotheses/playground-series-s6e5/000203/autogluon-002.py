...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Race-wide stop-wave pressure from previous laps using observable fresh-tyre resets.
    stop_proxy = ((out["TyreLife"].astype(float) <= 2) & (out["Stint"].astype(float) > 1)).astype(float)
    lap_pit = (
        stop_proxy.groupby([out[c] for c in keys], sort=False)
        .mean()
        .rename("pit_frac_lap")
        .reset_index()
    )
    ...
    for lag in (1, 2):
        shifted = lap_pit.copy()
        shifted["LapNumber"] = shifted["LapNumber"] + lag
        shifted = shifted.rename(columns={"pit_frac_lap": f"pit_frac_prev{lag}"})
        out = out.merge(shifted, on=keys, how="left", sort=False)
        out[f"pit_frac_prev{lag}"] = out[f"pit_frac_prev{lag}"].fillna(0.0)

    out["pit_wave_prev12"] = 0.65 * out["pit_frac_prev1"] + 0.35 * out["pit_frac_prev2"]
    out["pit_wave_accel"] = out["pit_frac_prev1"] - out["pit_frac_prev2"]

    # Same-lap field state percentiles and driver deviations from the field.
    state_cols = ["TyreLife", "Cumulative_Degradation", "LapTime_Delta"]
    for col in state_cols:
        grp = out.groupby(keys, sort=False)[col]
        q25 = grp.transform(lambda s: s.quantile(0.25))
        q50 = grp.transform("median")
        q75 = grp.transform(lambda s: s.quantile(0.75))
        iqr = (q75 - q25).replace(0, np.nan)

        base = col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        out[f"{base}_field_q25"] = q25
        out[f"{base}_field_q50"] = q50
        out[f"{base}_field_q75"] = q75
        out[f"{base}_dev_q50"] = out[col] - q50
        out[f"{base}_dev_q75"] = out[col] - q75
        out[f"{base}_field_pct"] = ((out[col] - q25) / iqr).clip(-3, 4).fillna(0.5)

    # Interactions with track position and compound.
    pos_pressure = (21 - out["Position"]).astype(float)
    out["position_pit_wave"] = pos_pressure * out["pit_wave_prev12"]
    out["position_wave_accel"] = pos_pressure * out["pit_wave_accel"]
    out["tyrelife_position_wave"] = out["TyreLife"] * out["position_pit_wave"]
    ...
    out["compound_pit_wave"] = compound_risk * out["pit_wave_prev12"]
    out["compound_tyre_dev"] = compound_risk * out["TyreLife_dev_q50"]
    out["compound_deg_dev"] = compound_risk * out["Cumulative_Degradation_dev_q50"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000203.",
    }, sort_keys=True))
