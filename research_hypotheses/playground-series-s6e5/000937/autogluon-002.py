...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pit = out["PitStop"].fillna(0).astype("float32")
    lap_count = (
        out.groupby(keys, observed=True)["PitStop"].transform("size").astype("float32")
    )
    lap_pits = (
        out.groupby(keys, observed=True)["PitStop"].transform("sum").astype("float32")
    )

    other_pits = lap_pits - pit
    other_cars = (lap_count - 1).clip(lower=0)
    out["pitwave_same_lap_other_count"] = other_pits.astype("float32")
    out["pitwave_same_lap_other_rate"] = (
        (other_pits / other_cars.replace(0, np.nan)).fillna(0).astype("float32")
    )
    ...
    def add_prev_lap_features(group_cols, prefix):
        lap_agg = (
            out.groupby(group_cols + ["LapNumber"], observed=True)["PitStop"]
            .agg(["sum", "size"])
            .rename(columns={"sum": "pit_sum", "size": "car_count"})
            .reset_index()
            .sort_values(group_cols + ["LapNumber"])
        )
        g = lap_agg.groupby(group_cols, observed=True)
        for w in (1, 2, 3):
            prev_sum = (
                g["pit_sum"]
            ...
            lap_agg[f"{prefix}_prev{w}_pit_count"] = prev_sum.fillna(0).astype(
                "float32"
            )
            lap_agg[f"{prefix}_prev{w}_pit_rate"] = (
                (prev_sum / prev_cars.replace(0, np.nan)).fillna(0).astype("float32")
            )

        keep = (
            group_cols
            + ["LapNumber"]
            + [c for c in lap_agg.columns if c.startswith(prefix)]
        )
        return lap_agg[keep]

    for group_cols, prefix in [
        (["Year", "Race"], "pitwave_race"),
        (["Year", "Race", "Compound"], "pitwave_compound"),
        (["Year", "Race", "PositionBand"], "pitwave_posband"),
    ]:
        feats = add_prev_lap_features(group_cols, prefix)
        out = out.merge(feats, on=group_cols + ["LapNumber"], how="left", sort=False)

    new_cols = [c for c in out.columns if c.startswith("pitwave_")]
    out[new_cols] = out[new_cols].fillna(0).astype("float32")
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000937"],
        "research_usage_note": "Verified assigned hypothesis 000937.",
    }, sort_keys=True))
