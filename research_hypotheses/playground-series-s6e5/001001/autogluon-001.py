...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["laps_remaining_est"] = laps_remaining.clip(lower=0, upper=90)
    ...
    expected_life = out["Compound"].map(compound_expected).fillna(24.0)

    tyre_age_vs_expected = out["TyreLife"] / expected_life
    finish_tyre_deficit = (
        out["TyreLife"] + out["laps_remaining_est"] - expected_life
    ) / expected_life

    lap_delta_loss = out["LapTime_Delta"].clip(lower=0)
    degradation_pressure = out["Cumulative_Degradation"].clip(lower=0) / expected_life
    old_tyre_late_race_pressure = tyre_age_vs_expected * out["RaceProgress"]

    out["tyre_age_vs_expected"] = tyre_age_vs_expected.clip(lower=0, upper=5)
    out["finish_tyre_deficit"] = finish_tyre_deficit.clip(lower=-2, upper=5)
    out["lap_delta_loss"] = lap_delta_loss.clip(upper=120)
    out["degradation_pressure"] = degradation_pressure.clip(lower=0, upper=120)
    out["old_tyre_late_race_pressure"] = old_tyre_late_race_pressure.clip(
        lower=0, upper=5
    )
    out["past_pit_count_proxy"] = (out["Stint"] - 1).clip(lower=0)
    out["laps_remaining_pressure"] = (-out["laps_remaining_est"]).clip(
        lower=-90, upper=0
    )

    out["pit_window_pressure"] = (
        0.35 * out["tyre_age_vs_expected"]
        + 0.25 * out["finish_tyre_deficit"].clip(lower=0)
        + 0.20 * (out["lap_delta_loss"] / 30.0).clip(upper=4)
        + 0.20 * out["old_tyre_late_race_pressure"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001001"],
        "research_usage_note": "Verified assigned hypothesis 001001.",
    }, sort_keys=True))
