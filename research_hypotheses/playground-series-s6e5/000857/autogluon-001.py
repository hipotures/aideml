...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["same_lap_field_size"] = g["Position"].transform("count").astype("float32")
    ...
    for c in core_cols:
        ...
        std = g[c].transform("std").fillna(0.0)
        mn = g[c].transform("min")
        mx = g[c].transform("max")
        out[c + "_same_lap_mean"] = mean.astype("float32")
        out[c + "_same_lap_std"] = std.astype("float32")
        out[c + "_same_lap_range"] = (mx - mn).astype("float32")
        out[c + "_vs_same_lap_mean"] = (out[c] - mean).astype("float32")
        out[c + "_same_lap_rank_pct"] = (
            g[c].rank(method="average", pct=True).astype("float32")
        )

    if "Compound" in out.columns:
        comp_codes = out["Compound"].cat.codes
        out["_compound_code_tmp"] = comp_codes
        for comp in out["Compound"].cat.categories:
            code = out["Compound"].cat.categories.get_loc(comp)
            flag = (out["_compound_code_tmp"] == code).astype("float32")
            out["same_lap_compound_prop_" + str(comp).lower()] = (
                flag.groupby([out[k] for k in key], observed=True, sort=False)
                .transform("mean")
                .astype("float32")
            )
        out = out.drop(columns=["_compound_code_tmp"])

    if "Position" in out.columns:
        s = out.sort_values(key + ["Position"], kind="mergesort")
        sg = s.groupby(key, observed=True, sort=False)
        ...
        for c in neighbor_cols:
            ahead = sg[c].shift(1)
            behind = sg[c].shift(-1)
            s[c + "_car_ahead_delta"] = (s[c] - ahead).fillna(0.0).astype("float32")
            s[c + "_car_behind_delta"] = (s[c] - behind).fillna(0.0).astype("float32")
            s[c + "_nearest_pair_mean"] = (
                pd.concat([ahead, behind], axis=1)
                .mean(axis=1)
                .fillna(s[c])
                .astype("float32")
            )
        ...
        if "TyreLife" in s.columns:
            s["older_than_car_ahead"] = (
                (s["TyreLife"] > sg["TyreLife"].shift(1)).fillna(False).astype("int8")
            )
            s["older_than_car_behind"] = (
                (s["TyreLife"] > sg["TyreLife"].shift(-1)).fillna(False).astype("int8")
            )

        new_cols = [c for c in s.columns if c not in out.columns]
        out = out.join(s[new_cols].sort_index())

    if {"TyreLife", "RaceProgress"}.issubset(out.columns):
        out["tyrelife_x_raceprogress"] = (out["TyreLife"] * out["RaceProgress"]).astype(
            "float32"
        )
        out["tyrelife_per_lap"] = (
            (out["TyreLife"] / (out["LapNumber"].replace(0, np.nan)))
            .fillna(0.0)
            .astype("float32")
        )

    if {"LapTime_Delta", "Cumulative_Degradation"}.issubset(out.columns):
        out["degradation_pressure"] = (
            out["LapTime_Delta"].clip(-30, 30)
            + 0.02 * out["Cumulative_Degradation"].clip(-500, 1500)
        ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000857"],
        "research_usage_note": "Verified assigned hypothesis 000857.",
    }, sort_keys=True))
