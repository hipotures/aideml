...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pos = out["Position"].astype(np.int16)
    lap = out["LapNumber"].astype(np.int16)

    out["_pos_ahead"] = pos - 1
    out["_pos_behind"] = pos + 1
    out["_prev_lap"] = lap - 1

    cur_keys = lap_cols + ["Position"]
    cur_ctx = out[
        cur_keys + ["PitStop", "TyreLife", "LapTime_Delta", "Compound"]
    ].copy()

    ahead = cur_ctx.rename(
        columns={
            "Position": "_pos_ahead",
            "PitStop": "ahead_pit_this_lap",
            "TyreLife": "ahead_tyre_life",
            "LapTime_Delta": "ahead_laptime_delta",
            "Compound": "ahead_compound",
        }
    )
    behind = cur_ctx.rename(
        columns={
            "Position": "_pos_behind",
            "PitStop": "behind_pit_this_lap",
            "TyreLife": "behind_tyre_life",
            "LapTime_Delta": "behind_laptime_delta",
            "Compound": "behind_compound",
        }
    )

    out = out.merge(ahead, on=event_cols + ["LapNumber", "_pos_ahead"], how="left")
    out = out.merge(behind, on=event_cols + ["LapNumber", "_pos_behind"], how="left")

    prev_ctx = cur_ctx.rename(columns={"LapNumber": "_prev_lap"})
    ahead_prev = prev_ctx.rename(
        columns={
            "Position": "_pos_ahead",
            "PitStop": "ahead_pit_last_lap",
            "TyreLife": "ahead_tyre_life_last",
            "LapTime_Delta": "ahead_laptime_delta_last",
            "Compound": "ahead_compound_last",
        }
    )
    behind_prev = prev_ctx.rename(
        columns={
            "Position": "_pos_behind",
            "PitStop": "behind_pit_last_lap",
            "TyreLife": "behind_tyre_life_last",
            "LapTime_Delta": "behind_laptime_delta_last",
            "Compound": "behind_compound_last",
        }
    )

    out = out.merge(ahead_prev, on=event_cols + ["_prev_lap", "_pos_ahead"], how="left")
    out = out.merge(
        behind_prev, on=event_cols + ["_prev_lap", "_pos_behind"], how="left"
    )
    ...
    for c in [
        "ahead_tyre_life",
        "behind_tyre_life",
        "ahead_tyre_life_last",
        "behind_tyre_life_last",
        "ahead_laptime_delta",
        "behind_laptime_delta",
        "ahead_laptime_delta_last",
        "behind_laptime_delta_last",
    ]:
        out[c] = out[c].astype("float32")

    out["nearby_rival_pit_this_lap"] = (
        out["ahead_pit_this_lap"] | out["behind_pit_this_lap"]
    ).astype(np.int8)
    out["nearby_rival_pit_last_lap"] = (
        out["ahead_pit_last_lap"] | out["behind_pit_last_lap"]
    ).astype(np.int8)
    out["adjacent_pit_pressure"] = (
        2 * out["ahead_pit_this_lap"]
        + out["behind_pit_this_lap"]
        + 2 * out["ahead_pit_last_lap"]
        + out["behind_pit_last_lap"]
    ).astype(np.int8)

    out["tyre_life_vs_ahead"] = (out["TyreLife"] - out["ahead_tyre_life"]).astype(
        "float32"
    )
    out["tyre_life_vs_behind"] = (out["TyreLife"] - out["behind_tyre_life"]).astype(
        "float32"
    )
    out["delta_vs_ahead"] = (out["LapTime_Delta"] - out["ahead_laptime_delta"]).astype(
        "float32"
    )
    out["delta_vs_behind"] = (
        out["LapTime_Delta"] - out["behind_laptime_delta"]
    ).astype("float32")

    out["local_position_cluster_density"] = (
        out.groupby(lap_cols)["Position"].transform(
            lambda s: s.between(pos.loc[s.index] - 2, pos.loc[s.index] + 2).sum()
        )
    ).astype(np.int8)

    older_than_rival = (
        (out["ahead_pit_this_lap"] | out["ahead_pit_last_lap"])
        & (out["tyre_life_vs_ahead"] > 0)
    ) | (
        (out["behind_pit_this_lap"] | out["behind_pit_last_lap"])
        & (out["tyre_life_vs_behind"] > 0)
    )
    worse_delta = (
        (out["ahead_pit_this_lap"] | out["ahead_pit_last_lap"])
        & (out["delta_vs_ahead"] > 0)
    ) | (
        (out["behind_pit_this_lap"] | out["behind_pit_last_lap"])
        & (out["delta_vs_behind"] > 0)
    )
    out["must_cover_undercut"] = (
        (out["nearby_rival_pit_this_lap"] | out["nearby_rival_pit_last_lap"])
        & (older_than_rival | worse_delta)
    ).astype(np.int8)
    ...
    for c in [
        ...
        "behind_compound",
        "ahead_compound_last",
        "behind_compound_last",
    ]:
        out[c] = out[c].fillna("NONE").astype("category")

    return out.drop(columns=["_pos_ahead", "_pos_behind", "_prev_lap"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000893"],
        "research_usage_note": "Verified assigned hypothesis 000893.",
    }, sort_keys=True))
