...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    eps = 1e-6
    tyrelife = out["TyreLife"].astype(float).clip(lower=1)
    lapnum = out["LapNumber"].astype(float).clip(lower=1)
    progress = out["RaceProgress"].astype(float).clip(eps, 1)
    remaining_laps = (lapnum / progress - lapnum).clip(lower=0)

    out["Race_Estimated_Total_Laps"] = (lapnum / progress).clip(1, 100)
    out["RemainingLaps"] = remaining_laps
    out["StintProgress"] = tyrelife / (tyrelife + remaining_laps + eps)

    deg_per_lap = (
        out["Cumulative_Degradation"].astype(float).clip(lower=0) / tyrelife
    ).replace([np.inf, -np.inf], np.nan)
    delta_abs = out["LapTime_Delta"].astype(float).abs().clip(0, 20)
    laptime_excess = (
        out["LapTime (s)"].astype(float)
        - out.groupby(["Race", "Year"])["LapTime (s)"].transform("median")
    ).clip(lower=0, upper=30)

    compound_d = out.groupby("Compound")["Cumulative_Degradation"].transform(
        "median"
    ).clip(lower=0) / (
        out.groupby("Compound")["TyreLife"].transform("median").clip(lower=1)
    )
    global_d = float(np.nanmedian(deg_per_lap))
    local_d = (
        0.55 * deg_per_lap.fillna(global_d)
        + 0.25 * compound_d.fillna(global_d)
        + 0.15 * delta_abs
        + 0.05 * laptime_excess
    ).clip(0.03, 25)
    out["Local_Degradation_Rate"] = local_d

    pit = out["PitStop"].astype(int)
    race_median = out.groupby(["Race", "Year"])["LapTime (s)"].transform("median")
    pit_loss_raw = (out["LapTime (s)"].astype(float) - race_median).where(pit.eq(1))
    global_pit_loss = float(np.nanmedian(pit_loss_raw))
    if not np.isfinite(global_pit_loss):
        global_pit_loss = 22.0
    global_pit_loss = float(np.clip(global_pit_loss, 12.0, 35.0))

    race_pit_loss = pit_loss_raw.groupby([out["Race"], out["Year"]]).transform("median")
    race_pit_count = (
        pit.groupby([out["Race"], out["Year"]]).transform("sum").astype(float)
    )
    shrink = race_pit_count / (race_pit_count + 25.0)
    pit_loss = (
        shrink * race_pit_loss.fillna(global_pit_loss)
        + (1.0 - shrink) * global_pit_loss
    ).clip(10, 40)
    out["PitLoss_Prior"] = pit_loss

    optimal = np.sqrt((2.0 * pit_loss) / (local_d + eps)).clip(3, 60)
    out["Optimal_Stint_Length"] = optimal
    out["TyreLife_minus_Optimal"] = tyrelife - optimal
    out["TyreLife_to_Optimal_Ratio"] = tyrelife / (optimal + eps)
    out["RemainingLaps_minus_OptimalFresh"] = remaining_laps - optimal

    fresh_recovery = (local_d * tyrelife * 0.65).clip(0, 35)
    old_next_loss = (local_d * (tyrelife + 1.0)).clip(0, 45)
    out["Undercut_Advantage"] = old_next_loss + fresh_recovery - pit_loss
    out["Undercut_Condition"] = (out["Undercut_Advantage"] > 0).astype("int8")

    out["Pit_Economics_Urgency"] = (
        out["TyreLife_minus_Optimal"]
        + 0.25 * out["Undercut_Advantage"]
        - 0.05 * remaining_laps
    )
    out["Can_Pit_With_Race_Left"] = (
        remaining_laps > pit_loss / (local_d + eps)
    ).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000768.",
    }, sort_keys=True))
