...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wet = compound.isin(["INTERMEDIATE", "WET"]).astype("float32")
    slick = (1.0 - wet).astype("float32")

    lap = out["LapNumber"].astype("float32")
    tyre = out["TyreLife"].astype("float32").clip(lower=0)
    progress = out["RaceProgress"].astype("float32").clip(eps, 1.0)
    stint = out["Stint"].astype("float32")
    pit_now = out["PitStop"].astype("float32")

    race_laps_est = (lap / progress).replace([np.inf, -np.inf], np.nan).fillna(lap)
    laps_left = (race_laps_est - lap).clip(lower=0)

    keys = ["Year", "Race", "Driver"]
    s = out[["LapNumber", "TyreLife", "PitStop"] + keys].copy()
    s["_row"] = np.arange(len(out))
    s = s.sort_values(keys + ["LapNumber", "_row"], kind="mergesort")
    g = s.groupby(keys, sort=False, observed=True)

    s["_pit_cum"] = g["PitStop"].cumsum()
    s["_pit_before"] = (s["_pit_cum"] - s["PitStop"]).clip(lower=0)
    s["_prev_pit"] = g["PitStop"].shift(1).fillna(0)
    s["_pit_lap"] = s["LapNumber"].where(s["PitStop"] > 0)
    s["_last_pit_lap"] = g["_pit_lap"].ffill()
    s["_laps_since_pit"] = (s["LapNumber"] - s["_last_pit_lap"]).fillna(s["TyreLife"])

    s = s.sort_values("_row")
    stops_before = s["_pit_before"].to_numpy(dtype="float32")
    prev_stop = s["_prev_pit"].to_numpy(dtype="float32")
    laps_since_stop = s["_laps_since_pit"].to_numpy(dtype="float32")
    ...
    degr_clip = degr.clip(degr.quantile(0.005), degr.quantile(0.995))
    ltd_clip = ltd.clip(ltd.quantile(0.005), ltd.quantile(0.995))

    nominal_life = compound.map(
        {
            "SOFT": 16.0,
            "MEDIUM": 22.0,
            "HARD": 28.0,
    ...
    ).fillna(22.0).astype("float32")
    nominal_service_gap = (nominal_life - laps_since_stop).clip(lower=0)
    log_service_gap = np.log1p(nominal_service_gap).astype("float32")
    age_ratio = (tyre / nominal_life.clip(lower=1.0)).clip(0, 3).astype("float32")
    left_frac = (laps_left / race_laps_est.clip(lower=1.0)).clip(0, 1).astype("float32")
    end_pressure = ((progress - 0.82) / 0.18).clip(0, 1).astype("float32")
    ...
    z = (
        ...
        + 0.065 * stint
        + 0.020 * position
        + 0.040 * pos_chg.clip(lower=0)
        - 1.25 * pit_now
        - 0.75 * prev_stop
        - 0.055 * nominal_service_gap
        + 0.40 * wet
        + 0.90 * end_pressure
    )
    clock_urgency = (1.0 / (1.0 + np.exp(-z.clip(-20, 20)))).astype("float32")

    out["pit_clock_race_laps_est"] = race_laps_est.astype("float32")
    out["pit_clock_laps_left"] = laps_left.astype("float32")
    out["pit_clock_left_frac"] = left_frac.astype("float32")
    out["pit_clock_stops_before"] = stops_before.astype("float32")
    out["pit_clock_prev_lap_stop"] = prev_stop.astype("float32")
    out["pit_clock_laps_since_stop"] = laps_since_stop.astype("float32")
    out["pit_clock_nominal_service_gap"] = nominal_service_gap.astype("float32")
    out["pit_clock_log_service_gap"] = log_service_gap.astype("float32")
    out["pit_clock_age_ratio"] = age_ratio.astype("float32")
    out["pit_clock_end_pressure"] = end_pressure.astype("float32")
    out["pit_clock_degradation_clipped"] = degr_clip.astype("float32")
    out["pit_clock_laptime_delta_clipped"] = ltd_clip.astype("float32")
    out["pit_clock_urgency_proxy"] = clock_urgency.astype("float32")
    out["pit_clock_urgency_slick"] = (clock_urgency * slick).astype("float32")
    out["pit_clock_urgency_wet"] = (clock_urgency * wet).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000802"],
        "research_usage_note": "Verified assigned hypothesis 000802.",
    }, sort_keys=True))
