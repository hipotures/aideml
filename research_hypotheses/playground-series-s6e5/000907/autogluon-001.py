...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_key = out["Year"].astype(str) + "_" + out["Race"].astype(str)
    race_codes, _ = pd.factorize(race_key, sort=True)
    driver_codes, _ = pd.factorize(out["Driver"].astype(str), sort=True)
    compound_codes, _ = pd.factorize(out["Compound"].astype(str), sort=True)

    out["year_race_code"] = race_codes.astype("int32")
    out["driver_code"] = driver_codes.astype("int32")
    out["compound_code"] = compound_codes.astype("int8")
    ...
    out["laps_remaining_est"] = (lap / progress - lap).clip(0, 100).astype("float32")
    out["tyre_age_progress"] = (tyre * progress).astype("float32")
    out["tyre_life_sq"] = (tyre * tyre).astype("float32")
    out["stint_tyre_ratio"] = (tyre / (stint + 1.0)).astype("float32")
    out["degradation_per_tyre_lap"] = (degr / (tyre + 1.0)).astype("float32")
    out["abs_laptime_delta"] = delta.abs().astype("float32")
    out["relative_laptime_delta"] = (delta / laptime.clip(lower=1.0)).astype("float32")
    out["position_pressure"] = ((21.0 - pos) * progress).astype("float32")
    out["position_change_abs"] = pos_chg.abs().astype("float32")

    out["is_wet_compound"] = (
        out["Compound"].isin(["INTERMEDIATE", "WET"]).astype("int8")
    )
    out["is_soft_compound"] = (out["Compound"] == "SOFT").astype("int8")
    out["late_race"] = (progress >= 0.75).astype("int8")
    out["early_stint"] = (tyre <= 3).astype("int8")
    out["long_stint"] = (tyre >= 20).astype("int8")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000907"],
        "research_usage_note": "Verified assigned hypothesis 000907.",
    }, sort_keys=True))
