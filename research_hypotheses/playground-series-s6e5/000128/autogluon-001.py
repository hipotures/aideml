...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    global_h = float((pit.sum() + 20.0 * pit.mean()) / (len(out) + 20.0))
    out["haz_global_prior"] = global_h

    def smooth_rate(keys, prior_col, strength):
        stats = (
            out.assign(_pit=pit)
            .groupby(keys, observed=True)["_pit"]
            .agg(["sum", "count"])
        )
        rate = (stats["sum"] + strength * global_h) / (stats["count"] + strength)
        mapped = out.set_index(keys).index.map(rate)
        return pd.Series(mapped, index=out.index, dtype="float64").fillna(
            out[prior_col] if prior_col in out else global_h
        )

    out["haz_race_compound"] = smooth_rate(
        ["Race", "Compound"], "haz_global_prior", 80.0
    )
    out["haz_year_compound"] = smooth_rate(
        ["Year", "Compound"], "haz_global_prior", 80.0
    )

    tyre_bin = np.minimum(
        out["TyreLife"].fillna(0).astype(float).clip(lower=0).astype(int), 80
    )
    out["_tyre_bin_hazard"] = tyre_bin
    out["haz_compound_stint_tyre"] = smooth_rate(
        ["Compound", "Stint", "_tyre_bin_hazard"], "haz_global_prior", 120.0
    )

    out["haz_hier_prior"] = (
        0.50 * out["haz_compound_stint_tyre"]
        + 0.25 * out["haz_race_compound"]
        + 0.20 * out["haz_year_compound"]
        + 0.05 * global_h
    ).clip(eps, 1 - eps)

    out["haz_logit_prior"] = np.log(
        out["haz_hier_prior"] / (1.0 - out["haz_hier_prior"])
    )
    out["survival_prior_to_finish"] = np.exp(
        -out["haz_hier_prior"] * (1.0 - out["RaceProgress"].fillna(0).clip(0, 1)) * 80.0
    )
    out["tyrelife_hazard_load"] = out["TyreLife"].fillna(0) * out["haz_hier_prior"]
    out["degradation_per_tyre_lap"] = out["Cumulative_Degradation"] / (
        out["TyreLife"].replace(0, np.nan) + eps
    )
    out["degradation_hazard_residual"] = out["degradation_per_tyre_lap"].fillna(
        0
    ) - out.groupby(["Compound", "Stint"], observed=True)[
        "degradation_per_tyre_lap"
    ].transform(
        "mean"
    ).fillna(
        out["degradation_per_tyre_lap"].mean()
    )
    out["laptime_delta_hazard_residual"] = out["LapTime_Delta"] - out.groupby(
        ["Race", "Compound"], observed=True
    )["LapTime_Delta"].transform("mean").fillna(out["LapTime_Delta"].mean())
    out["position_hazard_pressure"] = out["Position"].fillna(0) * out["haz_hier_prior"]
    out["progress_hazard_interaction"] = (
        out["RaceProgress"].fillna(0) * out["haz_hier_prior"]
    )

    out = out.drop(columns=["_tyre_bin_hazard"])
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000128.",
    }, sort_keys=True))
