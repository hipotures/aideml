...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_year = out["Race"].astype(str) + "_" + out["Year"].astype(str)
    out["Race_Year"] = race_year.astype("category")

    lap_grp = [race_year, out["LapNumber"]]

    lap_delta_med = out.groupby(lap_grp, observed=True)["LapTime_Delta"].transform(
        "median"
    )
    lap_delta_mean = out.groupby(lap_grp, observed=True)["LapTime_Delta"].transform(
        "mean"
    )
    lap_delta_std = (
        out.groupby(lap_grp, observed=True)["LapTime_Delta"].transform("std").fillna(0)
    )
    lap_time_med = out.groupby(lap_grp, observed=True)["LapTime (s)"].transform(
        "median"
    )
    lap_tyre_med = out.groupby(lap_grp, observed=True)["TyreLife"].transform("median")
    lap_tyre_max = (
        out.groupby(lap_grp, observed=True)["TyreLife"]
        .transform("max")
        .replace(0, np.nan)
    )
    lap_cars = out.groupby(lap_grp, observed=True)["LapNumber"].transform("size")

    out["field_delta_med"] = lap_delta_med
    out["field_delta_std"] = lap_delta_std
    out["delta_vs_field_med"] = out["LapTime_Delta"] - lap_delta_med
    out["laptime_vs_field_med"] = out["LapTime (s)"] - lap_time_med
    out["tyrelife_vs_field_med"] = out["TyreLife"] - lap_tyre_med
    out["tyrelife_field_pct"] = (out["TyreLife"] / lap_tyre_max).fillna(0)
    out["field_size_lap"] = lap_cars
    ...
    out["_fresh_stop_proxy"] = (
        (out["TyreLife"].astype(float) <= 2) & (out["Stint"].astype(float) > 1)
    ).astype("float32")

    band_grp = [race_year, out["LapNumber"], pos_band]
    band_tyre_med = out.groupby(band_grp, observed=True)["TyreLife"].transform("median")
    band_delta_med = out.groupby(band_grp, observed=True)["LapTime_Delta"].transform(
        "median"
    )
    band_old_count = (
        out["TyreLife"]
        .gt(band_tyre_med)
        .groupby(band_grp, observed=True)
        .transform("sum")
    )

    out["band_tyre_med"] = band_tyre_med
    out["band_delta_med"] = band_delta_med
    out["band_old_tyre_count"] = band_old_count
    out["tyrelife_vs_band_med"] = out["TyreLife"] - band_tyre_med
    out["delta_vs_band_med"] = out["LapTime_Delta"] - band_delta_med

    ordered = out.assign(_ry=race_year).sort_values(
        ["_ry", "LapNumber", "Position"], kind="mergesort"
    )
    g = ordered.groupby(["_ry", "LapNumber"], observed=True)

    ahead_delta = g["LapTime_Delta"].shift(1)
    behind_delta = g["LapTime_Delta"].shift(-1)
    ahead_tyre = g["TyreLife"].shift(1)
    behind_tyre = g["TyreLife"].shift(-1)
    ahead_laptime = g["LapTime (s)"].shift(1)
    behind_laptime = g["LapTime (s)"].shift(-1)

    ordered["_ahead_delta_gap"] = ordered["LapTime_Delta"] - ahead_delta
    ordered["_behind_delta_gap"] = behind_delta - ordered["LapTime_Delta"]
    ordered["_ahead_tyre_gap"] = ordered["TyreLife"] - ahead_tyre
    ordered["_behind_tyre_gap"] = behind_tyre - ordered["TyreLife"]
    ordered["_local_pace_spread"] = pd.concat(
        [ahead_laptime, ordered["LapTime (s)"], behind_laptime], axis=1
    ...
    nearby_old = ahead_tyre.gt(ordered["TyreLife"]).astype("int8") + behind_tyre.gt(
        ordered["TyreLife"]
    ).astype("int8")
    ordered["_nearby_older_rivals"] = nearby_old
    ...
    cols = [
        ...
        "_behind_tyre_gap",
        "_local_pace_spread",
        "_nearby_older_rivals",
    ]
    out.loc[ordered.index, [c[1:] for c in cols]] = ordered[cols].to_numpy()

    lap_state = (
        out.assign(_ry=race_year)
        .groupby(["_ry", "LapNumber"], observed=True)
        .agg(
            _lap_delta_mean=("LapTime_Delta", "mean"),
            _lap_time_mean=("LapTime (s)", "mean"),
            _lap_pit_rate=("_fresh_stop_proxy", "mean"),
            _lap_tyre_mean=("TyreLife", "mean"),
        )
        .reset_index()
        .sort_values(["_ry", "LapNumber"], kind="mergesort")
    )
    sg = lap_state.groupby("_ry", observed=True)
    lap_state["_field_slowdown_spike"] = lap_state["_lap_delta_mean"] - sg[
        "_lap_delta_mean"
    ].shift(1)
    lap_state["_field_time_spike"] = lap_state["_lap_time_mean"] - sg[
        "_lap_time_mean"
    ].shift(1)
    lap_state["_pit_wave_prev"] = sg["_lap_pit_rate"].shift(1)
    lap_state["_tyre_age_wave_prev"] = sg["_lap_tyre_mean"].shift(1)
    ...
    out = out.merge(
        lap_state[
            [
                "_ry",
                "LapNumber",
                "_field_slowdown_spike",
                "_field_time_spike",
                "_pit_wave_prev",
                "_tyre_age_wave_prev",
            ]
        ],
        left_on=[race_year, "LapNumber"],
        right_on=["_ry", "LapNumber"],
        how="left",
        sort=False,
    ).drop(columns=["_ry"])
    ...
    out["undercut_window_pressure"] = (
        out["tyrelife_vs_band_med"].clip(lower=0)
        * (out["Position"] <= 15).astype("int8")
        * (1 + out["nearby_older_rivals"])
    )
    out["clean_air_proxy"] = (
        out["ahead_delta_gap"].clip(lower=0)
        + out["behind_delta_gap"].clip(lower=0)
        + out["local_pace_spread"].clip(lower=0)
    )
    out["safety_car_like_state"] = (
        out["_field_slowdown_spike"].gt(out["field_delta_std"].fillna(0) * 1.5)
        | out["_field_time_spike"].gt(5)
    ).astype("int8")
    out["pit_wave_pressure"] = out["_pit_wave_prev"] * (1 + out["tyrelife_field_pct"])

    for c in ["Compound", "Driver", "Race"]:
        out[c] = out[c].astype("category")

    return out.drop(columns=["_fresh_stop_proxy"], errors="ignore")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000215"],
        "research_usage_note": "Verified assigned hypothesis 000215.",
    }, sort_keys=True))
