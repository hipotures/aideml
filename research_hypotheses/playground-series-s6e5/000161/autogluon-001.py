...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    comp = out["Compound"].astype("category")
    out["Compound"] = comp

    # Local same-lap strategic pressure: approximate nearby rivals by position band.
    pos = out["Position"].astype(float)
    lap_count = out.groupby(lap_keys)["Position"].transform("count").astype(float)
    out["same_lap_field_size"] = lap_count
    for w in (1, 2):
        lo = (pos - w).astype(float)
        hi = (pos + w).astype(float)
        lower_cnt = (
            out.assign(_p=pos).groupby(lap_keys)["_p"].transform(lambda s, lo=lo: 0)
        )
    # Vectorized count of rivals within +/-2 positions per race-lap.
    sorted_idx = out.sort_values(lap_keys + ["Position"]).index
    tmp = out.loc[sorted_idx, lap_keys + ["Position", "TyreLife", "PitStop"]].copy()
    g = tmp.groupby(lap_keys, sort=False)
    tmp["nearby_rank_rivals_2"] = (
        g["Position"].transform(
            lambda s: s.rolling(5, center=True, min_periods=1).count()
        )
        - 1
    )
    out.loc[sorted_idx, "nearby_rank_rivals_2"] = tmp["nearby_rank_rivals_2"].to_numpy()

    # Same-compound rivals on older tyres are likely undercut/cover targets.
    same_comp_keys = lap_keys + ["Compound"]
    comp_lap_n = (
        out.groupby(same_comp_keys)["TyreLife"].transform("count").astype(float)
    )
    comp_lap_sum_older = out.groupby(same_comp_keys)["TyreLife"].transform("sum")
    out["same_compound_field_count"] = comp_lap_n
    out["same_compound_avg_tyre_life_gap"] = (comp_lap_sum_older / comp_lap_n) - out[
        "TyreLife"
    ]
    out["same_compound_older_proxy"] = (
        out["same_compound_avg_tyre_life_gap"].clip(lower=0) * comp_lap_n
    )

    # Pit-wave intensity from cars already stopping on this same lap.
    lap_pits = out.groupby(lap_keys)["PitStop"].transform("sum").astype(float)
    out["same_lap_pit_count"] = lap_pits
    out["same_lap_pit_rate"] = lap_pits / lap_count.clip(lower=1)

    # Adjacent rivals who pitted last lap, using only previous-lap pit status by race/driver.
    prev = out.sort_values(race_keys + ["Driver", "LapNumber"])[
        race_keys + ["Driver", "LapNumber", "PitStop"]
    ].copy()
    prev["prev_lap_pit"] = (
        prev.groupby(race_keys + ["Driver"], sort=False)["PitStop"].shift(1).fillna(0)
    )
    out["prev_lap_pit_driver"] = 0.0
    out.loc[prev.index, "prev_lap_pit_driver"] = prev["prev_lap_pit"].to_numpy()

    adj = out.sort_values(lap_keys + ["Position"])[
        lap_keys + ["Position", "prev_lap_pit_driver"]
    ].copy()
    gg = adj.groupby(lap_keys, sort=False)["prev_lap_pit_driver"]
    adj_prev_pit = gg.shift(1).fillna(0) + gg.shift(-1).fillna(0)
    out.loc[adj.index, "adjacent_prev_lap_pit_count"] = adj_prev_pit.to_numpy()
    out["adjacent_prev_lap_pit_any"] = (out["adjacent_prev_lap_pit_count"] > 0).astype(
        np.int8
    )

    # Race-level degradation regime and compound warm-up favorability proxies.
    race_deg = out.groupby(race_keys)["Cumulative_Degradation"].transform("median")
    race_delta_abs = out.groupby(race_keys)["LapTime_Delta"].transform(
        lambda s: s.abs().median()
    )
    out["race_degradation_regime"] = race_deg
    out["race_laptime_volatility"] = race_delta_abs

    is_soft = (out["Compound"].astype(str) == "SOFT").astype(float)
    is_hard = (out["Compound"].astype(str) == "HARD").astype(float)
    is_wet = out["Compound"].astype(str).isin(["INTERMEDIATE", "WET"]).astype(float)
    prog = out["RaceProgress"].astype(float)
    tyre = out["TyreLife"].astype(float)

    out["soft_warmup_favorability"] = (
        is_soft * (race_deg.rank(pct=True) + prog) / (1.0 + np.log1p(tyre))
    )
    out["hard_warmup_penalty_proxy"] = (
        is_hard * race_delta_abs.rank(pct=True) * np.log1p(tyre)
    )
    out["wet_strategy_noise_proxy"] = (
        is_wet * out["same_lap_pit_rate"] * race_delta_abs.rank(pct=True)
    )

    out["pit_wave_x_adjacent"] = out["same_lap_pit_rate"] * (
        1.0 + out["adjacent_prev_lap_pit_count"]
    )
    out["undercut_pressure_proxy"] = (
        out["nearby_rank_rivals_2"].fillna(0)
        + out["same_compound_older_proxy"].fillna(0)
        + 3.0 * out["adjacent_prev_lap_pit_count"].fillna(0)
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000161"],
        "research_usage_note": "Verified assigned hypothesis 000161.",
    }, sort_keys=True))
