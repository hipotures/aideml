...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    laptime = pd.to_numeric(out["LapTime (s)"], errors="coerce")
    ...
    base = out.groupby(["Year", "Race", "Compound"], observed=True)[
        "LapTime (s)"
    ].transform("median")
    lap_resid = laptime - base
    deg_rate = degradation / np.maximum(pd.Series(tyre, index=out.index), 1.0)
    ...
    obs = 0.45 * _rz(lap_delta) + 0.35 * _rz(lap_resid) + 0.20 * _rz(deg_rate)
    obs = np.clip(obs, -8.0, 8.0)

    driver_code = pd.factorize(out["Driver"], sort=False)[0]
    race_code = pd.factorize(out["Race"], sort=False)[0]
    year = pd.to_numeric(out["Year"], errors="coerce").to_numpy(dtype=float)
    ...
    order = (
        ...
        .sort_values(
            ["year", "race", "driver", "stint", "lap", "_pos"], kind="mergesort"
        )["_pos"]
        .to_numpy()
    )
    ...
    accel_arr = np.zeros(n, dtype=np.float32)
    innov_arr = np.zeros(n, dtype=np.float32)
    var_arr = np.zeros(n, dtype=np.float32)
    ...
    for i in order:
        key = (year[i], race_code[i], driver_code[i], stint[i])
        new_group = key != last_key
        if new_group or pit[i] > 0:
            level, slope = 0.0, 0.0
            p00, p01, p11 = 1.0, 0.0, 0.25
            prev_lap = lap[i]
            last_key = key
        ...
        dt = 1.0 if new_group else float(np.clip(lap[i] - prev_lap, 1.0, 5.0))
        pred_level = level + slope * dt
        pred_slope = slope
        q_level = 0.035 * dt
        q_slope = 0.006 * dt
        p00p = p00 + 2.0 * dt * p01 + dt * dt * p11 + q_level
        p01p = p01 + dt * p11
        p11p = p11 + q_slope

        innovation = float(obs[i] - pred_level)
        s_var = p00p + 0.85
        k0 = p00p / s_var
        k1 = p01p / s_var
        old_slope = slope
        ...
        p00 = max((1.0 - k0) * p00p, 1e-6)
        p01 = (1.0 - k0) * p01p
        p11 = max(p11p - k1 * p01p, 1e-6)

        level_arr[i] = level
        slope_arr[i] = slope
        accel_arr[i] = (slope - old_slope) / dt
        innov_arr[i] = innovation
        var_arr[i] = p00
        prev_lap = lap[i]
        last_key = key

    out["kf_wear_level"] = level_arr
    out["kf_wear_slope"] = slope_arr
    out["kf_wear_accel"] = accel_arr
    out["kf_wear_innovation"] = innov_arr
    out["kf_wear_variance"] = var_arr
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000387"],
        "research_usage_note": "Verified assigned hypothesis 000387.",
    }, sort_keys=True))
