...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = grp + ["Position"]

    out["_orig_order"] = np.arange(len(out))
    s = out.sort_values(sort_cols + ["_orig_order"], kind="mergesort")
    ...
    s["_compound_code"] = s["Compound"].map(comp_map).astype("float32")

    neighbor_cols = ["TyreLife", "LapTime_Delta", "PitStop", "_compound_code"]
    for k in (1, 2, 3):
        ahead = s.groupby(grp, sort=False)[neighbor_cols].shift(k)
        behind = s.groupby(grp, sort=False)[neighbor_cols].shift(-k)

        for col in neighbor_cols:
            clean = "CompoundCode" if col == "_compound_code" else col
            s[f"ahead{k}_{clean}"] = ahead[col].astype("float32")
            s[f"behind{k}_{clean}"] = behind[col].astype("float32")

        s[f"ahead{k}_tyre_life_diff"] = (s["TyreLife"] - ahead["TyreLife"]).astype(
            "float32"
        )
        s[f"behind{k}_tyre_life_diff"] = (s["TyreLife"] - behind["TyreLife"]).astype(
            "float32"
        )
        s[f"ahead{k}_lap_delta_diff"] = (
            s["LapTime_Delta"] - ahead["LapTime_Delta"]
        ).astype("float32")
        s[f"behind{k}_lap_delta_diff"] = (
            s["LapTime_Delta"] - behind["LapTime_Delta"]
        ).astype("float32")
        s[f"ahead{k}_same_compound"] = (
            s["_compound_code"] == ahead["_compound_code"]
        ).astype("int8")
        s[f"behind{k}_same_compound"] = (
            s["_compound_code"] == behind["_compound_code"]
        ).astype("int8")

    ahead_pits = [f"ahead{k}_PitStop" for k in (1, 2, 3)]
    behind_pits = [f"behind{k}_PitStop" for k in (1, 2, 3)]
    ahead_life = [f"ahead{k}_TyreLife" for k in (1, 2, 3)]
    behind_life = [f"behind{k}_TyreLife" for k in (1, 2, 3)]
    ahead_delta = [f"ahead{k}_LapTime_Delta" for k in (1, 2, 3)]
    behind_delta = [f"behind{k}_LapTime_Delta" for k in (1, 2, 3)]

    s["neighbors_pitting_now_3"] = (
        s[ahead_pits + behind_pits].fillna(0).sum(axis=1).astype("int8")
    )
    s["ahead_pitting_now_3"] = s[ahead_pits].fillna(0).sum(axis=1).astype("int8")
    s["behind_pitting_now_3"] = s[behind_pits].fillna(0).sum(axis=1).astype("int8")

    s["avg_ahead_tyre_life_3"] = s[ahead_life].mean(axis=1).astype("float32")
    s["avg_behind_tyre_life_3"] = s[behind_life].mean(axis=1).astype("float32")
    s["max_ahead_tyre_life_3"] = s[ahead_life].max(axis=1).astype("float32")
    s["max_behind_tyre_life_3"] = s[behind_life].max(axis=1).astype("float32")
    s["avg_ahead_lap_delta_3"] = s[ahead_delta].mean(axis=1).astype("float32")
    s["avg_behind_lap_delta_3"] = s[behind_delta].mean(axis=1).astype("float32")

    s["older_than_ahead_3"] = (s["TyreLife"] - s["avg_ahead_tyre_life_3"]).astype(
        "float32"
    )
    s["older_than_behind_3"] = (s["TyreLife"] - s["avg_behind_tyre_life_3"]).astype(
        "float32"
    )
    s["local_old_tyre_pressure"] = (
        s["older_than_ahead_3"].fillna(0) + s["older_than_behind_3"].fillna(0)
    ).astype("float32")

    s["undercut_risk_by_trailing_car"] = (
        (s["behind1_TyreLife"].notna())
        & (s["behind1_TyreLife"] >= s["TyreLife"] - 2)
        & (s["behind1_LapTime_Delta"] <= s["LapTime_Delta"])
        & (s["behind1_PitStop"].fillna(0) == 0)
    ).astype("int8")

    s["clear_air_opportunity_if_pitting"] = (
        (s["ahead1_PitStop"].fillna(0) == 1)
        | (
            (s["ahead1_TyreLife"] > s["TyreLife"] + 5)
            & (s["ahead1_LapTime_Delta"] > s["LapTime_Delta"])
        )
    ).astype("int8")

    s["box_window_pressure"] = (
        (s["neighbors_pitting_now_3"] >= 1).astype("int8")
        + s["undercut_risk_by_trailing_car"]
        + (
            (
                s["TyreLife"] >= s["avg_ahead_tyre_life_3"].fillna(s["TyreLife"]) + 4
            ).astype("int8")
        )
    ).astype("int8")

    s = s.sort_values("_orig_order", kind="mergesort").drop(
        columns=["_orig_order", "_compound_code"]
    )
    return s
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000532"],
        "research_usage_note": "Verified assigned hypothesis 000532.",
    }, sort_keys=True))
