...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"Year", "Race", "LapNumber"}.issubset(out.columns):
        g_lap = [out["Year"], out["Race"], out["LapNumber"]]
        out["query_lap_size"] = (
            out.groupby(g_lap, observed=True)["LapNumber"]
            .transform("size")
            .astype("int16")
        )
        ...
        for c in rank_cols:
            out[f"{c}_lap_rank_pct"] = (
                out.groupby(g_lap, observed=True)[c]
                .rank(method="average", pct=True)
                .astype("float32")
            )
            med = out.groupby(g_lap, observed=True)[c].transform("median")
            out[f"{c}_lap_rel_med"] = (out[c] - med).astype("float32")

    if {"Year", "Race", "RaceProgress"}.issubset(out.columns):
        progress_bin = (
            out["RaceProgress"].astype(float).mul(100).round().astype("Int16")
        )
        g_prog = [out["Year"], out["Race"], progress_bin]
        out["query_progress_size"] = (
            out.groupby(g_prog, observed=True)["RaceProgress"]
            .transform("size")
            .astype("int16")
        )
        ...
        for c in [
            ...
            out[f"{c}_progress_rank_pct"] = (
                out.groupby(g_prog, observed=True)[c]
                .rank(method="average", pct=True)
                .astype("float32")
            )
    ...
    if {"TyreLife", "RaceProgress"}.issubset(out.columns):
        out["tyrelife_progress_ratio"] = (
            (out["TyreLife"] / out["RaceProgress"].replace(0, np.nan))
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000292"],
        "research_usage_note": "Verified assigned hypothesis 000292.",
    }, sort_keys=True))
