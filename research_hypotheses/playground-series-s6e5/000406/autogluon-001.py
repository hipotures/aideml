...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stint_key = [race_year, out["Driver"].astype(str), out["Stint"]]

    out["_orig_order_000406"] = np.arange(len(out))
    out["_race_year_000406"] = race_year
    out.sort_values(
        ["_race_year_000406", "Driver", "Stint", "LapNumber", "_orig_order_000406"],
        inplace=True,
    )

    g = out.groupby(["_race_year_000406", "Driver", "Stint"], sort=False)

    delta = out["LapTime_Delta"].astype(float)
    lap_time = out["LapTime (s)"].astype(float)

    prev_delta = g["LapTime_Delta"].shift(1)
    prev2_delta = g["LapTime_Delta"].shift(2)

    out["chg_delta_000406"] = delta - prev_delta
    out["accel_delta_000406"] = delta - 2.0 * prev_delta + prev2_delta

    for w in (3, 5, 8):
        roll_mean = g["LapTime_Delta"].transform(
            lambda s: s.shift(1).rolling(w, min_periods=2).mean()
        )
        roll_std = g["LapTime_Delta"].transform(
            lambda s: s.shift(1).rolling(w, min_periods=2).std()
        )
        out[f"roll_delta_mean_{w}_000406"] = roll_mean
        out[f"roll_delta_std_{w}_000406"] = roll_std.fillna(0.0)
        out[f"roll_delta_slope_{w}_000406"] = (delta - roll_mean) / (w + eps)

    out["var_jump_000406"] = out["roll_delta_std_3_000406"] / (
        out["roll_delta_std_8_000406"] + eps
    )

    deteriorating = (out["chg_delta_000406"] > 0).astype(np.int16)
    reset = (
        (deteriorating.eq(0))
        .groupby([out["_race_year_000406"], out["Driver"], out["Stint"]], sort=False)
        .cumsum()
    )
    out["deterioration_run_000406"] = deteriorating.groupby(
        [out["_race_year_000406"], out["Driver"], out["Stint"], reset], sort=False
    ).cumsum()

    peer_lap_med = out.groupby(["_race_year_000406", "LapNumber"], sort=False)[
        "LapTime (s)"
    ].transform("median")
    peer_lap_mad = (
        (lap_time - peer_lap_med)
        .abs()
        .groupby([out["_race_year_000406"], out["LapNumber"]], sort=False)
        .transform("median")
    )
    out["peer_resid_pace_000406"] = (lap_time - peer_lap_med) / (peer_lap_mad + 1.0)

    out["prev_peer_resid_000406"] = g["peer_resid_pace_000406"].shift(1)
    out["peer_resid_jump_000406"] = (
        out["peer_resid_pace_000406"] - out["prev_peer_resid_000406"]
    )

    baseline = g["LapTime_Delta"].transform(
        lambda s: s.shift(1).rolling(6, min_periods=3).median()
    )
    short_recent = g["LapTime_Delta"].transform(
        lambda s: s.shift(1).rolling(2, min_periods=1).mean()
    )
    collapse_raw = short_recent - baseline
    out["breakpoint_score_000406"] = collapse_raw.clip(lower=0.0).fillna(0.0)
    out["sustained_collapse_000406"] = (
        (
            out["breakpoint_score_000406"]
            > out["roll_delta_std_8_000406"].fillna(0.0) + 0.25
        )
        .astype(np.int16)
        .groupby([out["_race_year_000406"], out["Driver"], out["Stint"]], sort=False)
    ...
    out["tyrelife_progress_000406"] = out["TyreLife"].astype(float) / (
        out["LapNumber"].astype(float) + eps
    )
    out["degradation_per_lap_000406"] = out["Cumulative_Degradation"].astype(float) / (
        out["TyreLife"].astype(float) + eps
    )

    out.sort_values("_orig_order_000406", inplace=True)
    out.drop(columns=["_orig_order_000406", "_race_year_000406"], inplace=True)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000406"],
        "research_usage_note": "Verified assigned hypothesis 000406.",
    }, sort_keys=True))
