...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_key = (
        out["Race"].astype("string").fillna("NA")
        + "_"
        + out["Year"].astype("string").fillna("NA")
    )
    out["Race_Year"] = race_key.astype("category")

    num_cols = ["TyreLife", "Cumulative_Degradation", "LapTime_Delta", "Position"]
    lap_key = [race_key, out["LapNumber"]]
    comp_lap_key = [
        race_key,
        out["LapNumber"],
        out["Compound"].astype("string").fillna("NA"),
    ]

    for col in num_cols:
        s = out[col].astype("float32")

        g = out.groupby(lap_key, sort=False)[col]
        mean = g.transform("mean").astype("float32")
        std = g.transform("std").replace(0, np.nan).astype("float32")
        cnt = g.transform("count").astype("float32")
        rank = g.rank(method="average", pct=True).astype("float32")
        out[f"{col}_lap_pct"] = rank
        out[f"{col}_lap_z"] = (
            ((s - mean) / std).replace([np.inf, -np.inf], 0).fillna(0).astype("float32")
        )
        out[f"{col}_lap_center"] = (s - mean).astype("float32")
        out[f"{col}_lap_count"] = cnt

        cg = out.groupby(comp_lap_key, sort=False)[col]
        cmean = cg.transform("mean").astype("float32")
        cstd = cg.transform("std").replace(0, np.nan).astype("float32")
        ccnt = cg.transform("count").astype("float32")
        crank = cg.rank(method="average", pct=True).astype("float32")
        out[f"{col}_compound_lap_pct"] = crank
        out[f"{col}_compound_lap_z"] = (
            ((s - cmean) / cstd)
            .replace([np.inf, -np.inf], 0)
            .fillna(0)
            .astype("float32")
        )
        out[f"{col}_compound_lap_center"] = (s - cmean).astype("float32")
        out[f"{col}_compound_lap_count"] = ccnt

    lap_count = (
        out.groupby(lap_key, sort=False)["TyreLife"]
        .transform("count")
        .astype("float32")
    )
    tyre_rank_min = (
    ...
    older = (lap_count - tyre_rank_max).clip(lower=0)
    newer = (tyre_rank_min - 1).clip(lower=0)
    out["rivals_older_tyre_count"] = older.astype("float32")
    out["rivals_older_tyre_share"] = (
        (older / (lap_count - 1).replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["rivals_newer_tyre_count"] = newer.astype("float32")
    out["rivals_newer_tyre_share"] = (
        (newer / (lap_count - 1).replace(0, np.nan)).fillna(0).astype("float32")
    )

    comp_count = (
        out.groupby(comp_lap_key, sort=False)["TyreLife"]
        .transform("count")
        .astype("float32")
    )
    alt_count = (lap_count - comp_count).clip(lower=0)
    out["rivals_alt_compound_count"] = alt_count.astype("float32")
    out["rivals_alt_compound_share"] = (
        (alt_count / (lap_count - 1).replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["compound_field_share"] = (
        (comp_count / lap_count.replace(0, np.nan)).fillna(0).astype("float32")
    )

    tmp = pd.DataFrame(
        {
            "race_year": race_key,
            "lap": out["LapNumber"].astype("int16"),
            "compound": out["Compound"].astype("string").fillna("NA"),
            "stint_bucket": pd.cut(
                out["Stint"].astype("float32"),
                bins=[0, 1, 2, 3, 99],
                labels=["s1", "s2", "s3", "s4p"],
                include_lowest=True,
            ).astype("string"),
            "pit": (
                (out["TyreLife"].astype("float32") <= 2)
                & (out["Stint"].astype("float32") > 1)
            ).astype("float32"),
        }
    )

    lap_pits = (
        tmp.groupby(["race_year", "lap"], sort=False)["pit"]
        .sum()
        .rename("lag_pit_count")
        .reset_index()
    )
    lap_pits["lap"] = lap_pits["lap"] + 1
    out["lag1_lap_pit_count"] = (
        pd.MultiIndex.from_frame(tmp[["race_year", "lap"]])
        .map(lap_pits.set_index(["race_year", "lap"])["lag_pit_count"])
        .fillna(0)
        .astype("float32")
    )
    out["lag1_lap_pit_share"] = (
        (out["lag1_lap_pit_count"] / lap_count.replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )

    comp_pits = (
        tmp.groupby(["race_year", "lap", "compound"], sort=False)["pit"]
        .sum()
        .rename("lag_comp_pit_count")
        .reset_index()
    )
    comp_pits["lap"] = comp_pits["lap"] + 1
    out["lag1_compound_pit_count"] = (
        pd.MultiIndex.from_frame(tmp[["race_year", "lap", "compound"]])
        .map(comp_pits.set_index(["race_year", "lap", "compound"])["lag_comp_pit_count"])
        .fillna(0)
        .astype("float32")
    )
    out["lag1_compound_pit_share"] = (
        (out["lag1_compound_pit_count"] / comp_count.replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )

    stint_pits = (
        tmp.groupby(["race_year", "lap", "stint_bucket"], sort=False)["pit"]
        .sum()
        .rename("lag_stint_pit_count")
        .reset_index()
    )
    stint_pits["lap"] = stint_pits["lap"] + 1
    out["lag1_stint_bucket_pit_count"] = (
        pd.MultiIndex.from_frame(tmp[["race_year", "lap", "stint_bucket"]])
        .map(stint_pits.set_index(["race_year", "lap", "stint_bucket"])["lag_stint_pit_count"])
        .fillna(0)
        .astype("float32")
    )

    out["tyre_x_progress"] = (
        out["TyreLife"].astype("float32") * out["RaceProgress"].astype("float32")
    ).astype("float32")
    out["degradation_per_tyre_lap"] = (
        (
            out["Cumulative_Degradation"].astype("float32")
            / out["TyreLife"].replace(0, np.nan).astype("float32")
        )
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000235.",
    }, sort_keys=True))
