...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_lap = ["Race", "Year", "LapNumber"]

    for c in ["LapTime (s)", "LapTime_Delta", "Position_Change"]:
        med_col = f"rl_{c}_median"
        mad_col = f"rl_{c}_mad"
        jump_col = f"rl_{c}_median_jump"
        z_col = f"rl_{c}_shock_z"

        out[med_col] = out.groupby(race_lap, observed=True)[c].transform("median")
        abs_dev = (out[c] - out[med_col]).abs()
        out[mad_col] = abs_dev.groupby(
            [out["Race"], out["Year"], out["LapNumber"]]
        ).transform("median")

        lap_meds = (
            out[race_lap + [med_col]]
            .drop_duplicates(race_lap)
            .sort_values(["Race", "Year", "LapNumber"])
        )
        lap_meds[jump_col] = (
            lap_meds.groupby(["Race", "Year"], observed=True)[med_col]
            .diff()
            .fillna(0.0)
        )
        out = out.merge(
            lap_meds[race_lap + [jump_col]], on=race_lap, how="left", sort=False
        )

        scale = out.groupby(["Race", "Year"], observed=True)[jump_col].transform(
            lambda s: np.nanmedian(np.abs(s - np.nanmedian(s))) + 1e-6
        )
        out[z_col] = (out[jump_col].abs() / scale).clip(0, 20)

    out["rl_pitstop_rate"] = out.groupby(race_lap, observed=True)["PitStop"].transform(
        "mean"
    )
    pit_laps = (
        out[race_lap + ["rl_pitstop_rate"]]
        .drop_duplicates(race_lap)
        .sort_values(["Race", "Year", "LapNumber"])
    )
    pit_laps["rl_pitstop_rate_jump"] = (
        pit_laps.groupby(["Race", "Year"], observed=True)["rl_pitstop_rate"]
        .diff()
        .fillna(0.0)
    )
    out = out.merge(
        pit_laps[race_lap + ["rl_pitstop_rate_jump"]],
        on=race_lap,
        how="left",
        sort=False,
    )
    ...
    shock_parts = [
        "rl_LapTime (s)_shock_z",
        "rl_LapTime_Delta_shock_z",
        "rl_Position_Change_shock_z",
    ]
    out["neutralization_shock_score"] = (
        out[shock_parts].mean(axis=1)
        + 3.0 * out["rl_pitstop_rate"]
        + 2.0 * out["rl_pitstop_rate_jump"].clip(lower=0)
    ).clip(0, 25)

    out["tyre_life_sq"] = out["TyreLife"] ** 2
    out["late_stint"] = (out["TyreLife"] >= 18).astype("int8")
    out["very_late_stint"] = (out["TyreLife"] >= 28).astype("int8")
    out["race_window_mid"] = (
        (out["RaceProgress"] >= 0.20) & (out["RaceProgress"] <= 0.75)
    ).astype("int8")
    out["race_window_late"] = (out["RaceProgress"] > 0.75).astype("int8")
    out["pit_loss_proxy"] = (
        out["LapTime_Delta"].clip(-30, 120)
        + out["Cumulative_Degradation"].clip(-50, 300) / 20.0
    )
    out["shock_x_tyre_life"] = out["neutralization_shock_score"] * out["TyreLife"]
    out["shock_x_late_stint"] = out["neutralization_shock_score"] * out["late_stint"]
    out["shock_x_mid_window"] = (
        out["neutralization_shock_score"] * out["race_window_mid"]
    )
    out["shock_x_pit_loss_proxy"] = (
        out["neutralization_shock_score"] * out["pit_loss_proxy"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000848"],
        "research_usage_note": "Verified assigned hypothesis 000848.",
    }, sort_keys=True))
