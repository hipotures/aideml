...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pos_keys = ["Year", "Race", "LapNumber", "Position"]

    compound_order = {"SOFT": 0, "MEDIUM": 1, "HARD": 2, "INTERMEDIATE": 3, "WET": 4}
    out["CompoundCode"] = out["Compound"].map(compound_order).fillna(-1).astype("int16")
    out["DriverFreq"] = out["Driver"].map(out["Driver"].value_counts()).astype("int32")
    out["RaceFreq"] = out["Race"].map(out["Race"].value_counts()).astype("int32")

    max_lap = out.groupby(race_keys)["LapNumber"].transform("max").astype("float32")
    out["LapsRemaining"] = (max_lap - out["LapNumber"]).clip(lower=0).astype("float32")
    out["TyreLifeFrac"] = (
        (out["TyreLife"] / max_lap.replace(0, np.nan)).fillna(0).astype("float32")
    )
    out["DegPerTyreLap"] = (
    ...
    lap_pace_mean = out.groupby(lap_keys)["LapTime (s)"].transform("mean")
    lap_pace_std = out.groupby(lap_keys)["LapTime (s)"].transform("std").fillna(0)
    out["LapPaceRel"] = (out["LapTime (s)"] - lap_pace_mean).astype("float32")
    out["LapPaceZ"] = (
        ((out["LapTime (s)"] - lap_pace_mean) / (lap_pace_std + 1e-3))
        .clip(-20, 20)
        .astype("float32")
    )
    out["LapPaceDispersion"] = lap_pace_std.astype("float32")

    pos_lap_mean = out.groupby(pos_keys)["LapTime (s)"].transform("mean")
    out["PositionPaceRel"] = (out["LapTime (s)"] - pos_lap_mean).astype("float32")
    out["ClosePackProxy"] = (
        out.groupby(lap_keys)["Position"].transform("count").astype("float32") / 20.0
    )
    out["FrontTraffic"] = ((out["Position"] > 1) & (out["Position"] <= 10)).astype(
        "int8"
    )
    out["BackTraffic"] = ((out["Position"] >= 10) & (out["Position"] < 20)).astype(
        "int8"
    )
    out["CleanAirProxy"] = (out["Position"] <= 3).astype("float32") + (
        out["LapPaceZ"] < -0.5
    ).astype("float32")

    lap_pit_rate = out.groupby(lap_keys)["PitStop"].transform("mean").astype("float32")
    pit_wave = out[lap_keys].copy()
    pit_wave["lap_pit_rate"] = lap_pit_rate
    pit_wave = pit_wave.drop_duplicates(lap_keys).sort_values(lap_keys)
    pit_wave["PriorLapPitWave"] = (
        pit_wave.groupby(race_keys)["lap_pit_rate"].shift(1).fillna(0).astype("float32")
    )
    out = out.merge(
        pit_wave[lap_keys + ["PriorLapPitWave"]], on=lap_keys, how="left", sort=False
    )

    post_pit_mask = (out["PitStop"] == 1) | (out["TyreLife"] <= 2)
    warm_base = out["LapTime_Delta"].where(post_pit_mask)
    comp_warm = warm_base.groupby(out["Compound"]).transform("mean")
    race_comp_warm = warm_base.groupby(
        [out["Year"], out["Race"], out["Compound"]]
    ).transform("mean")
    global_warm = float(warm_base.mean()) if warm_base.notna().any() else 0.0
    out["CompoundWarmupPenalty"] = (
        race_comp_warm.fillna(comp_warm).fillna(global_warm).astype("float32")
    )

    out["UndercutOpportunity"] = (
        (
            out["DegPerTyreLap"]
            * (1.0 + out["ClosePackProxy"])
            * (1.0 + out["PriorLapPitWave"])
            * np.log1p(out["TyreLife"])
        )
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    out["OvercutOpportunity"] = (
        (
            out["CompoundWarmupPenalty"]
            * (1.0 + out["CleanAirProxy"])
            / (1.0 + out["LapsRemaining"])
        )
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
        .astype("float32")
    )
    out["StrategyOpportunityBalance"] = (
        out["UndercutOpportunity"] - out["OvercutOpportunity"]
    ).astype("float32")
    out["TrafficWearInteraction"] = (
        out["ClosePackProxy"] * out["TyreLife"] * out["DegPerTyreLap"]
    ).astype("float32")
    out["PitWaveAgeInteraction"] = (
        out["PriorLapPitWave"] * out["TyreLife"] * (1.0 + out["RaceProgress"])
    ).astype("float32")
    out["WarmupRemainingInteraction"] = (
        out["CompoundWarmupPenalty"] * out["LapsRemaining"] * (out["CompoundCode"] + 1)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000914"],
        "research_usage_note": "Verified assigned hypothesis 000914.",
    }, sort_keys=True))
