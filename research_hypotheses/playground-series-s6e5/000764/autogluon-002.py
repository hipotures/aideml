...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = ["Year", "Race", "LapNumber"]
    driver_race = ["Year", "Race", "Driver"]
    driver_stint = ["Year", "Race", "Driver", "Stint"]
    ...
    X["TyreLife_x_Degradation"] = (X["TyreLife"] * X["Cumulative_Degradation"]).astype(
        "float32"
    )
    X["TyreLife_per_Progress"] = (
        X["TyreLife"] / X["RaceProgress"].clip(lower=1e-3)
    ).astype("float32")
    X["StintProgress"] = (X["TyreLife"] / X["LapNumber"].clip(lower=1)).astype(
        "float32"
    )
    X["Degradation_per_TyreLife"] = (
        X["Cumulative_Degradation"] / X["TyreLife"].clip(lower=1)
    ).astype("float32")
    X["LapTime_x_TyreLife"] = (X["LapTime (s)"] * X["TyreLife"]).astype("float32")
    X["PositionPressure"] = (X["Position"] / 20.0).astype("float32")
    ...
    for col in [
        ...
        g = X.groupby(grp, observed=True)[col]
        mean = g.transform("mean")
        std = g.transform("std").fillna(0)
        minv = g.transform("min")
        maxv = g.transform("max")
        X[f"{col}_lap_mean"] = mean.astype("float32")
        X[f"{col}_lap_std"] = std.astype("float32")
        X[f"{col}_lap_min"] = minv.astype("float32")
        X[f"{col}_lap_max"] = maxv.astype("float32")
        X[f"{col}_vs_lap_mean"] = (X[col] - mean).astype("float32")
        X[f"{col}_lap_z"] = (
            ((X[col] - mean) / std.replace(0, np.nan)).fillna(0).astype("float32")
        )
        X[f"{col}_lap_range_pos"] = (
            ((X[col] - minv) / (maxv - minv).replace(0, np.nan))
            .fillna(0)
            .astype("float32")
        )
        X[f"{col}_lap_rank_pct"] = (
            X.groupby(grp, observed=True)[col].rank(method="average", pct=True)
        ).astype("float32")

    lap_size = (
        X.groupby(grp, observed=True)["Driver"].transform("size").astype("float32")
    )
    X["LapFieldSize"] = lap_size

    for col in ["Compound", "Stint", "PitStop"]:
        counts = X.groupby(grp + [col], observed=True)["Driver"].transform("size")
        X[f"{col}_lap_count"] = counts.astype("float32")
        X[f"{col}_lap_share"] = (counts / lap_size).astype("float32")

    # Relative traffic / neighbor context within the lap snapshot
    ordered = X.sort_values(grp + ["Position", "LapTime (s)", "Driver"]).copy()
    for col in [
        "Position",
        "LapTime (s)",
        "LapTime_Delta",
        ...
        prevv = ordered.groupby(grp, observed=True)[col].shift(1)
        nextv = ordered.groupby(grp, observed=True)[col].shift(-1)
        ordered[f"{col}_gap_prev_pos"] = (
            (ordered[col] - prevv).fillna(0).astype("float32")
        )
        ordered[f"{col}_gap_next_pos"] = (
            (nextv - ordered[col]).fillna(0).astype("float32")
        )
        ordered[f"{col}_local_span"] = (nextv - prevv).fillna(0).astype("float32")

    # Counts of cars in similar strategic windows on the same lap
    for base_col, qs in {
        "TyreLife": [0.5, 0.75, 0.9],
        "Cumulative_Degradation": [0.5, 0.75, 0.9],
        "LapTime_Delta": [0.1, 0.25, 0.75, 0.9],
    }.items():
        qvals = X.groupby(grp, observed=True)[base_col].transform(
            lambda s, q=qs: (
                pd.DataFrame({str(v): [s.quantile(v)] for v in q}).iloc[0, 0]
                if False
                else 0
            )
        )
        # compute quantiles separately to avoid row-wise callbacks
        for q in qs:
            qcol = (
                X.groupby(grp, observed=True)[base_col]
                .transform("quantile", q=q)
                .astype("float32")
            )
            flag = (X[base_col] >= qcol).astype("int8")
            X[f"{base_col}_ge_q{int(q*100)}"] = flag
            X[f"{base_col}_ge_q{int(q*100)}_lap_count"] = (
                X.groupby(grp, observed=True)[f"{base_col}_ge_q{int(q*100)}"]
                .transform("sum")
                .astype("float32")
            )
            X.drop(columns=[f"{base_col}_ge_q{int(q*100)}"], inplace=True)

    # Driver/race temporal state available up to the current lap
    neighbor_cols = [c for c in ordered.columns if c not in X.columns]
    X = X.join(ordered[neighbor_cols].sort_index())
    X["DriverRaceLapIdx"] = (
        X.groupby(driver_race, observed=True).cumcount() + 1
    ).astype("float32")
    X["WithinStintLapIdx"] = (
        X.groupby(driver_stint, observed=True).cumcount() + 1
    ).astype("float32")
    X["StintLapMismatch"] = (X["WithinStintLapIdx"] - X["TyreLife"]).astype("float32")
    ...
    for col in [
        ...
        prev1 = X.groupby(driver_race, observed=True)[col].shift(1)
        prev2 = X.groupby(driver_race, observed=True)[col].shift(2)
        X[f"{col}_lag1"] = prev1.fillna(0).astype("float32")
        X[f"{col}_chg1"] = (X[col] - prev1).fillna(0).astype("float32")
        X[f"{col}_chg2"] = (X[col] - prev2).fillna(0).astype("float32")

    # Interaction between own state and whole-field state
    X["TyreLife_x_lap_old_share"] = (
        X["TyreLife"] * X["TyreLife_ge_q75_lap_count"] / X["LapFieldSize"].clip(lower=1)
    ).astype("float32")
    X["Deg_x_lap_old_share"] = (
        X["Cumulative_Degradation"]
        * X["Cumulative_Degradation_ge_q75_lap_count"]
        / X["LapFieldSize"].clip(lower=1)
    ).astype("float32")
    X["Compound_pressure"] = (
        X["Compound_lap_share"] * X["TyreLife_lap_rank_pct"]
    ).astype("float32")
    X["TrafficRisk"] = (
        X["LapTime (s)_gap_prev_pos"].abs() + X["LapTime (s)_gap_next_pos"].abs()
    ).astype("float32")
    X["StrategyPressure"] = (
        X["TyreLife_lap_z"]
        + X["Cumulative_Degradation_lap_z"]
        + X["LapTime_Delta_lap_z"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000764"],
        "research_usage_note": "Verified assigned hypothesis 000764.",
    }, sort_keys=True))
