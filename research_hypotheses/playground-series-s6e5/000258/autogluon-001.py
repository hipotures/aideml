...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wet = comp.isin(["INTERMEDIATE", "WET"]).astype("int8")
    slick = comp.isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")

    grp_cols = ["Year", "Race", "LapNumber"]

    out["wet_tyre_lap_frac"] = (
        wet.groupby([out[c] for c in grp_cols], sort=False)
        .transform("mean")
        .astype("float32")
    )
    out["slick_tyre_lap_frac"] = (
        slick.groupby([out[c] for c in grp_cols], sort=False)
        .transform("mean")
        .astype("float32")
    )
    out["compound_lap_diversity"] = (
        comp.groupby([out[c] for c in grp_cols], sort=False)
        .transform("nunique")
        .astype("float32")
    )

    race_lap_mean = out.groupby(grp_cols, sort=False)["LapTime (s)"].transform("mean")
    race_driver_cols = ["Year", "Race", "Driver"]
    prev_lap_time = out.groupby(race_driver_cols, sort=False)["LapTime (s)"].shift(1)
    improvement = (prev_lap_time - out["LapTime (s)"]).replace(
        [np.inf, -np.inf], np.nan
    )
    peer_improve = (
        improvement.groupby([out[c] for c in grp_cols], sort=False)
        .transform("mean")
        .fillna(0)
    )
    out["peer_laptime_improvement_rate"] = peer_improve.astype("float32")
    out["rel_laptime_to_field"] = (out["LapTime (s)"] - race_lap_mean).astype("float32")

    mixed_or_wet_lap = (
        (out["wet_tyre_lap_frac"] > 0) & (out["wet_tyre_lap_frac"] < 1)
    ) | (out["compound_lap_diversity"] >= 3)
    out["wet_transition_lap"] = mixed_or_wet_lap.astype("int8")
    out["wet_crossover_pressure"] = (
        out["wet_tyre_lap_frac"]
        * out["slick_tyre_lap_frac"]
        * out["compound_lap_diversity"]
    ).astype("float32")
    out["wet_transition_progress"] = (
        out["wet_transition_lap"] * out["RaceProgress"]
    ).astype("float32")
    out["wet_peer_improve_progress"] = (
        out["peer_laptime_improvement_rate"] * out["RaceProgress"]
    ).astype("float32")

    lap_state = out[grp_cols + ["wet_tyre_lap_frac"]].drop_duplicates(grp_cols)
    lap_state = lap_state.sort_values(grp_cols)
    lap_state["prev_lap_wet_frac"] = (
        lap_state.groupby(["Year", "Race"], sort=False)["wet_tyre_lap_frac"]
        .shift(1)
        .fillna(0)
    )
    out = out.merge(
        lap_state[grp_cols + ["prev_lap_wet_frac"]], on=grp_cols, how="left", sort=False
    )

    out["dry_after_wet_transition"] = (
        (slick == 1) & (out["prev_lap_wet_frac"] > 0)
    ).astype("int8")
    out["dry_after_wet_progress"] = (
        out["dry_after_wet_transition"] * out["RaceProgress"]
    ).astype("float32")
    out["is_wet_compound"] = wet
    out["is_slick_compound"] = slick
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000258"],
        "research_usage_note": "Verified assigned hypothesis 000258.",
    }, sort_keys=True))
