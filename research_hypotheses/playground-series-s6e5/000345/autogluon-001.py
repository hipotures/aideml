...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_med = (
        out.groupby("Race")[lap_col].transform("median")
        if "Race" in out.columns
        else out[lap_col].median()
    )
    comp_med = (
        out.groupby("Compound")[lap_col].transform("median")
        if "Compound" in out.columns
        else out[lap_col].median()
    )
    global_med = float(out[lap_col].median())
    residual = (
        out[lap_col].astype(float)
        - 0.5 * race_med.astype(float)
        - 0.5 * comp_med.astype(float)
    )
    residual = residual.fillna(out[lap_col].astype(float) - global_med)

    out["lt_resid_race_comp"] = residual.astype("float32")
    out["tyrelife_progress"] = (
        out["TyreLife"].astype(float) / (out["LapNumber"].astype(float) + 1.0)
    ).astype("float32")
    out["degradation_per_lap"] = (
    ...
    wear_slope = np.zeros(n, dtype=np.float32)
    wear_accel = np.zeros(n, dtype=np.float32)
    forecast_error = np.zeros(n, dtype=np.float32)
    uncertainty = np.zeros(n, dtype=np.float32)

    r = residual.to_numpy(dtype=np.float64)
    pit = (
        out["PitStop"].to_numpy(dtype=np.int8)
        if "PitStop" in out.columns
        else np.zeros(n, dtype=np.int8)
    )

    if group_cols:
        groups = out.groupby(group_cols, sort=False).indices.values()
    else:
        groups = [np.arange(n)]

    q_level, q_slope, obs_var = 0.08, 0.015, 9.0

    for idx in groups:
        level = 0.0
        slope = 0.0
        p00, p01, p11 = 25.0, 0.0, 4.0
        prev_slope = 0.0
        ...
        for j in idx:
            ...
            level = pred_level + k0 * err
            slope = pred_slope + k1 * err
            p00 = (1.0 - k0) * pp00
            p01 = (1.0 - k0) * pp01
            p11 = pp11 - k1 * pp01
            ...
            wear_level[j] = level
            wear_slope[j] = slope
            wear_accel[j] = slope - prev_slope
            forecast_error[j] = err
            uncertainty[j] = np.sqrt(max(s_var, 0.0))
            prev_slope = slope

    out["latent_wear_level"] = wear_level
    out["latent_wear_slope"] = wear_slope
    out["latent_wear_accel"] = wear_accel
    out["latent_wear_forecast_error"] = forecast_error
    out["latent_wear_uncertainty"] = uncertainty
    out["latent_wear_risk"] = (
        wear_level + 2.0 * wear_slope + 0.5 * uncertainty
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000345"],
        "research_usage_note": "Verified assigned hypothesis 000345.",
    }, sort_keys=True))
