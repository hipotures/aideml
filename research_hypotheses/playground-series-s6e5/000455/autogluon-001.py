...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_key = race.str.lower()

    # Official-style circuit priors: approximate pit-lane total-loss seconds,
    # tyre severity bucket, and expected strategic stop count by Grand Prix.
    pit_loss = {
        "australian grand prix": 20.5,
        "bahrain grand prix": 23.0,
        "saudi arabian grand prix": 20.0,
        "miami grand prix": 19.5,
        "emilia romagna grand prix": 28.0,
        "monaco grand prix": 20.5,
        "spanish grand prix": 22.0,
        "canadian grand prix": 18.5,
        "austrian grand prix": 20.0,
        "british grand prix": 21.0,
        "hungarian grand prix": 21.5,
        "belgian grand prix": 19.0,
        "dutch grand prix": 21.5,
        "italian grand prix": 24.5,
        "azerbaijan grand prix": 20.5,
        "singapore grand prix": 27.5,
        "japanese grand prix": 22.5,
        "qatar grand prix": 24.5,
        "united states grand prix": 20.5,
        "mexico city grand prix": 22.0,
        "sao paulo grand prix": 20.5,
        "las vegas grand prix": 20.0,
        "abu dhabi grand prix": 21.5,
        "chinese grand prix": 21.0,
        "pre-season testing": 23.0,
    }
    severity = {
    ...
    stop_prior = {
        "australian grand prix": 1.4,
        "bahrain grand prix": 2.0,
        "saudi arabian grand prix": 1.2,
        "miami grand prix": 1.3,
        "emilia romagna grand prix": 1.1,
        "monaco grand prix": 1.0,
        "spanish grand prix": 2.0,
        "canadian grand prix": 1.3,
        "austrian grand prix": 1.6,
        "british grand prix": 1.8,
        "hungarian grand prix": 1.7,
        "belgian grand prix": 1.8,
        "dutch grand prix": 1.6,
        "italian grand prix": 1.2,
        "azerbaijan grand prix": 1.1,
        "singapore grand prix": 1.4,
        "japanese grand prix": 1.8,
        "qatar grand prix": 2.0,
        "united states grand prix": 1.8,
        "mexico city grand prix": 1.5,
        "sao paulo grand prix": 1.6,
        "las vegas grand prix": 1.2,
        "abu dhabi grand prix": 1.4,
        "chinese grand prix": 1.6,
        "pre-season testing": 1.5,
    }

    out["Circuit_PitLoss_Prior"] = race_key.map(pit_loss).fillna(22.0).astype("float32")
    out["Circuit_TyreSeverity_Prior"] = (
        race_key.map(severity).fillna(3).astype("float32")
    )
    out["Circuit_StopCount_Prior"] = (
        race_key.map(stop_prior).fillna(1.5).astype("float32")
    )

    progress = out["RaceProgress"].astype("float32").clip(0.001, 1.0)
    lap = out["LapNumber"].astype("float32")
    est_total_laps = (lap / progress).replace([np.inf, -np.inf], np.nan).fillna(lap)
    out["LapsRemaining_Est"] = (
        (est_total_laps - lap).clip(lower=0, upper=90).astype("float32")
    )

    compound = out["Compound"].astype(str).str.upper()
    dry = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype("float32")
    wet = compound.isin(["INTERMEDIATE", "WET"]).astype("float32")
    softness = (
        compound.map(
            {"HARD": 1.0, "MEDIUM": 2.0, "SOFT": 3.0, "INTERMEDIATE": 1.5, "WET": 1.2}
        )
        .fillna(2.0)
        .astype("float32")
    )
    ...
    prior_stops_done = (stint - 1).clip(lower=0, upper=8)

    out["Dry_TyreRule_Active_Prior"] = dry
    out["Wet_Compound_Flag"] = wet
    out["Compound_Softness_Prior"] = softness
    out["StopDebt_vs_CircuitPrior"] = (
        out["Circuit_StopCount_Prior"] - prior_stops_done
    ).astype("float32")
    out["PitPressure_CircuitPrior"] = (
        out["StopDebt_vs_CircuitPrior"]
        * out["Circuit_TyreSeverity_Prior"]
        * (tyre_life / (out["LapsRemaining_Est"] + 5.0))
        * dry
    ).astype("float32")
    out["PitLoss_Adjusted_StopDebt"] = (
        out["StopDebt_vs_CircuitPrior"] / (out["Circuit_PitLoss_Prior"] + 1.0)
    ).astype("float32")
    out["Severity_x_TyreLife"] = (out["Circuit_TyreSeverity_Prior"] * tyre_life).astype(
        "float32"
    )
    out["Severity_x_CompoundSoftness"] = (
        out["Circuit_TyreSeverity_Prior"] * softness
    ).astype("float32")
    out["LapsRemaining_x_StopPrior"] = (
        out["LapsRemaining_Est"] * out["Circuit_StopCount_Prior"]
    ).astype("float32")
    out["MandatoryDryWindowPressure"] = (
        dry
        * out["StopDebt_vs_CircuitPrior"].clip(lower=0)
        / (out["LapsRemaining_Est"] + 1.0)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000455"],
        "research_usage_note": "Verified assigned hypothesis 000455.",
    }, sort_keys=True))
