...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    nonpit = out["PitStop"].eq(0) if has_pit else pd.Series(True, index=out.index)
    pit = out["PitStop"].eq(1) if has_pit else pd.Series(False, index=out.index)

    base_global = (
        float(out.loc[nonpit, lap_col].median())
        if lap_col in out.columns and nonpit.any()
        else 0.0
    )
    pit_global = (
        float((out.loc[pit, lap_col].median() - base_global))
        if lap_col in out.columns and pit.any()
        else 22.0
    )

    if lap_col in out.columns and key_cols:
        base_by_race = (
            out.loc[nonpit].groupby(key_cols, observed=True)[lap_col].median()
        )
        pit_by_race = out.loc[pit].groupby(key_cols, observed=True)[lap_col].median()
        pit_loss_map = (
            (pit_by_race - base_by_race).replace([np.inf, -np.inf], np.nan).dropna()
        )
        mi = pd.MultiIndex.from_frame(out[key_cols])
        pit_loss = pd.Series(
            mi.map(pit_loss_map), index=out.index, dtype="float64"
        ).fillna(pit_global)
    else:
        pit_loss = pd.Series(pit_global, index=out.index, dtype="float64")

    pit_loss = pit_loss.clip(5.0, 45.0)
