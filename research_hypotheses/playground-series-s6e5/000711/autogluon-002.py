...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_col = "LapTime (s)"
    key_cols = [c for c in ["Race", "Year"] if c in out.columns]
    lap_values = (
        pd.to_numeric(out[lap_col], errors="coerce")
        if lap_col in out.columns
        else pd.Series(np.nan, index=out.index)
    )

    base_global = (
        float(lap_values.median())
        if lap_col in out.columns and lap_values.notna().any()
        else 0.0
    )
    if not np.isfinite(base_global):
        base_global = 0.0

    if "Race" in out.columns:
        race_lower = out["Race"].astype("string").str.lower().fillna("")
    else:
        race_lower = pd.Series("", index=out.index, dtype="string")
    if "Position" in out.columns:
        position = pd.to_numeric(out["Position"], errors="coerce").fillna(10.0)
    else:
        position = pd.Series(10.0, index=out.index)
    if "RaceProgress" in out.columns:
        progress = pd.to_numeric(out["RaceProgress"], errors="coerce").clip(0, 1)
    else:
        progress = pd.Series(0.5, index=out.index)
    ...
    pit_loss += np.where(race_lower.str.contains("jeddah|saudi|austria", regex=True), -1.5, 0.0)
    pit_loss += ((position.clip(1, 20) - 10.5) * 0.10).astype("float64")
    pit_loss += ((progress.fillna(0.5) - 0.5) * 1.5).astype("float64")
    pit_loss = pit_loss.clip(8.0, 45.0)
    out["cf_pit_loss_est"] = pit_loss.astype("float32")

    tyre = (
        out["TyreLife"].astype("float64")
        if "TyreLife" in out.columns
        else pd.Series(1.0, index=out.index)
    )
    lap_time = (
        out[lap_col].astype("float64")
        if lap_col in out.columns
        else pd.Series(base_global, index=out.index)
    )

    if "Compound" in out.columns:
        comp = out["Compound"].astype("string").fillna("UNK")
    else:
        comp = pd.Series("UNK", index=out.index, dtype="string")

    work = pd.DataFrame(
        {"Compound": comp, "TyreLife": tyre, "LapTime": lap_time, "nonpit": nonpit}
    )
    work_np = work.loc[work["nonpit"]].copy()
    work_np["x"] = work_np["TyreLife"].clip(1, 80)
    global_slope = 0.03
    slopes = {}
    intercepts = {}
    for name, g in work_np.groupby("Compound", observed=True):
        if len(g) >= 30 and g["x"].nunique() > 1:
            x = g["x"].to_numpy(dtype="float64")
            y = g["LapTime"].to_numpy(dtype="float64")
            vx = np.var(x)
            slope = (
                float(np.cov(x, y, bias=True)[0, 1] / vx) if vx > 0 else global_slope
            )
            slope = float(np.clip(slope, -0.25, 0.80))
            intercept = float(np.median(y - slope * x))
        else:
            slope = global_slope
            intercept = base_global
        slopes[str(name)] = slope
        intercepts[str(name)] = intercept

    slope_s = comp.map(slopes).astype("float64").fillna(global_slope).clip(-0.25, 0.80)
    intercept_s = comp.map(intercepts).astype("float64").fillna(base_global)
    current_next = intercept_s + slope_s * (tyre + 1.0)

    feasible = ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]
    fresh_times = []
    for fc in feasible:
        fs = slopes.get(fc, global_slope)
        fi = intercepts.get(fc, base_global)
        fresh_times.append(fi + fs * 1.0)
    best_fresh = float(np.nanmin(fresh_times)) if fresh_times else base_global
    worst_fresh = float(np.nanmax(fresh_times)) if fresh_times else base_global

    stop_now_best = pit_loss + best_fresh
    stop_now_worst = pit_loss + worst_fresh
    wait_then_pit = current_next + pit_loss + best_fresh

    out["cf_current_continue_next"] = current_next.astype("float32")
    out["cf_best_fresh_lap1"] = np.float32(best_fresh)
    out["cf_stop_now_best_cost"] = stop_now_best.astype("float32")
    out["cf_stop_now_advantage"] = (current_next - stop_now_best).astype("float32")
    out["cf_delay_penalty"] = (wait_then_pit - stop_now_best).astype("float32")
    out["cf_fresh_vs_current_gap"] = (current_next - best_fresh).astype("float32")
    out["cf_fresh_compound_spread"] = np.float32(worst_fresh - best_fresh)
    out["cf_decay_slope_current"] = slope_s.astype("float32")

    if "RaceProgress" in out.columns:
        progress = out["RaceProgress"].astype("float64").clip(0, 1)
        out["cf_delay_penalty_progress"] = (
            out["cf_delay_penalty"].astype("float64") * (0.5 + progress)
        ).astype("float32")
        out["cf_stop_window_pressure"] = (
            (out["cf_fresh_vs_current_gap"].astype("float64") - pit_loss / 10.0)
            * progress
        ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000711"],
        "research_usage_note": "Verified assigned hypothesis 000711.",
    }, sort_keys=True))
