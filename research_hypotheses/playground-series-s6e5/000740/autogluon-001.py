...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mu = np.nanmean(x_num, axis=0).astype(np.float32)
    sig = np.nanstd(x_num, axis=0).astype(np.float32)
    sig[sig < 1e-6] = 1.0
    x_num = (x_num - mu) / sig
    x_num = np.nan_to_num(x_num, nan=0.0, posinf=0.0, neginf=0.0)
    x_num = np.clip(x_num, -8.0, 8.0).astype(np.float32)

    compound = (
        out["Compound"].astype("string").fillna("__missing__")
        if "Compound" in out.columns
        else pd.Series(["__missing__"] * n)
    )
    comp_codes = pd.factorize(compound, sort=True)[0].astype(np.int64) + 1
    n_comp = int(comp_codes.max()) + 1

    key_cols = [c for c in ["Driver", "Race", "Stint"] if c in out.columns]
    sort_cols = key_cols + [
        c for c in ["Year", "LapNumber", "TyreLife"] if c in out.columns
    ]
    if sort_cols:
        sorter = out[sort_cols].copy()
        sorter["_pos_740"] = np.arange(n)
        order = sorter.sort_values(sort_cols, kind="mergesort", na_position="last")[
            "_pos_740"
    ...
    c_sorted = comp_codes[order]

    if key_cols:
        change = np.zeros(n, dtype=bool)
        change[0] = True
        for col in key_cols:
            vals = out[col].astype("string").fillna("__missing__").to_numpy()[order]
    ...
    for s, e in zip(starts, ends):
        for cs in range(int(s), int(e), seq_len):
            cl = min(seq_len, int(e) - cs)
            if cl > 0:
    ...
    class TinyStintEncoder(torch.nn.Module):
        def __init__(self, num_dim, n_compound, emb_dim=3, hidden_dim=16):
            super().__init__()
            self.comp_emb = torch.nn.Embedding(n_compound + 1, emb_dim, padding_idx=0)
            self.gru = torch.nn.GRU(num_dim + emb_dim, hidden_dim, batch_first=True)
            self.num_head = torch.nn.Linear(hidden_dim, num_dim)
            self.comp_head = torch.nn.Linear(hidden_dim, n_compound + 1)

        def forward(self, xb, cb):
            ce = self.comp_emb(cb.clamp(0, self.comp_emb.num_embeddings - 1))
            h, _ = self.gru(torch.cat([xb, ce], dim=-1))
            return h, self.num_head(h), self.comp_head(h)

    def make_batch(ids):
        max_l = int(chunk_lengths[ids].max())
        b = len(ids)
        xb = np.zeros((b, max_l, x_sorted.shape[1]), dtype=np.float32)
        cb = np.zeros((b, max_l), dtype=np.int64)
        mb = np.zeros((b, max_l), dtype=np.float32)
        for i, cid in enumerate(ids):
            s = int(chunk_starts[cid])
            l = int(chunk_lengths[cid])
            xb[i, :l] = x_sorted[s : s + l]
            cb[i, :l] = c_sorted[s : s + l]
            mb[i, :l] = 1.0
        return torch.from_numpy(xb), torch.from_numpy(cb), torch.from_numpy(mb)

    model = TinyStintEncoder(x_sorted.shape[1], n_comp, hidden_dim=16)
    opt = torch.optim.AdamW(model.parameters(), lr=0.01, weight_decay=1e-4)

    max_train_chunks = min(n_chunks, max(1, 180000 // seq_len))
    train_ids = (
        np.linspace(0, n_chunks - 1, max_train_chunks, dtype=np.int64)
        if n_chunks > max_train_chunks
        else np.arange(n_chunks, dtype=np.int64)
    )
    batch_size = 384

    model.train()
    for _ in range(2):
        for b0 in range(0, len(train_ids), batch_size):
            ids = train_ids[b0 : b0 + batch_size]
            xb, cb, mb = make_batch(ids)
            noise_num = (torch.rand_like(xb) < 0.15) & (mb.unsqueeze(-1) > 0)
            xb_in = xb.masked_fill(noise_num, 0.0)
            cb_in = cb.clone()
            noise_cat = (torch.rand_like(cb.float()) < 0.10) & (mb > 0)
            cb_in[noise_cat] = 0

            _, pred_num, pred_comp = model(xb_in, cb_in)
            denom = mb.sum().clamp_min(1.0)
            loss_num = (((pred_num - xb) ** 2) * mb.unsqueeze(-1)).sum() / (
                denom * max(1, x_sorted.shape[1])
            )
            loss_cat = torch.nn.functional.cross_entropy(
                pred_comp.reshape(-1, pred_comp.shape[-1]),
                cb.reshape(-1),
                reduction="none",
            ).reshape_as(mb)
            loss = loss_num + 0.10 * ((loss_cat * mb).sum() / denom)
    ...
    emb_sorted = np.zeros((n, 16), dtype=np.float32)
    model.eval()
    with torch.no_grad():
        all_ids = np.arange(n_chunks, dtype=np.int64)
        for b0 in range(0, n_chunks, batch_size):
            ids = all_ids[b0 : b0 + batch_size]
            xb, cb, _ = make_batch(ids)
            h, _, _ = model(xb, cb)
            h = h.cpu().numpy().astype(np.float32)
            for i, cid in enumerate(ids):
                s = int(chunk_starts[cid])
                l = int(chunk_lengths[cid])
                emb_sorted[s : s + l] = h[i, :l]

    emb = np.zeros_like(emb_sorted)
    emb[order] = emb_sorted
    for j in range(emb.shape[1]):
        out[f"ssl_stint_emb_{j:02d}"] = emb[:, j]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000740"],
        "research_usage_note": "Verified assigned hypothesis 000740.",
    }, sort_keys=True))
