...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prior_n = grp.cumcount().astype("float32")
    out["stint_lap_index"] = (prior_n + 1).astype("float32")

    for col, prefix in [("LapTime (s)", "lt"), ("LapTime_Delta", "ld")]:
        x = out[col].astype("float32")
        x_sq = (x * x).astype("float32")

        csum = grp[col].cumsum().astype("float32")
        csum_sq = (
            x_sq.groupby([out[c] for c in group_cols], sort=False)
            .cumsum()
            .astype("float32")
        )

        prior_sum = csum - x
        prior_sum_sq = csum_sq - x_sq

        prior_mean = prior_sum / prior_n.where(prior_n > 0)
        var_num = prior_sum_sq - (prior_sum * prior_sum) / prior_n.where(prior_n > 0)
        prior_var = var_num / (prior_n - 1).where(prior_n > 1)

        global_std = float(pd.Series(x).std())
        if not np.isfinite(global_std) or global_std <= 0:
            global_std = 1.0

        prior_std = np.sqrt(prior_var.clip(lower=1e-6))
        prior_mean_filled = prior_mean.fillna(x)
        prior_std_filled = prior_std.fillna(global_std).clip(lower=1e-3)

        innovation = (x - prior_mean_filled).astype("float32")
        innovation_z = (innovation / prior_std_filled).astype("float32")
        innovation_abs = innovation.abs().astype("float32")

        innov_csum = (
            innovation.groupby([out[c] for c in group_cols], sort=False)
            .cumsum()
            .astype("float32")
        )
        innov_abs_csum = (
            innovation_abs.groupby([out[c] for c in group_cols], sort=False)
            .cumsum()
            .astype("float32")
        )

        prior_innov_mean = (innov_csum - innovation) / prior_n.where(prior_n > 0)
        prior_innov_abs_mean = (innov_abs_csum - innovation_abs) / prior_n.where(
            prior_n > 0
        )

        out[f"{prefix}_state_pred"] = prior_mean_filled.astype("float32")
        out[f"{prefix}_state_uncertainty"] = prior_std_filled.astype("float32")
        out[f"{prefix}_innovation"] = innovation
        out[f"{prefix}_innovation_z"] = (
            innovation_z.replace([np.inf, -np.inf], 0).fillna(0).astype("float32")
        )
        out[f"{prefix}_innovation_mean_prior"] = prior_innov_mean.fillna(0).astype(
            "float32"
        )
        out[f"{prefix}_innovation_abs_mean_prior"] = prior_innov_abs_mean.fillna(
            0
        ).astype("float32")

    out["next_lap_pace_proxy"] = (
        out["lt_state_pred"] + out["ld_state_pred"].fillna(0)
    ).astype("float32")
    out["innovation_pressure"] = (
        out["lt_innovation_z"].clip(lower=0) + out["ld_innovation_z"].clip(lower=0)
    ).astype("float32")
    out["pace_uncertainty_ratio"] = (
        out["lt_state_uncertainty"] / out["lt_state_pred"].abs().clip(lower=1.0)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000296"],
        "research_usage_note": "Verified assigned hypothesis 000296.",
    }, sort_keys=True))
