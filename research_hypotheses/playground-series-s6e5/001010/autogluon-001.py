...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["Year", "Race", "LapNumber", "Position", "LapTime (s)", "Driver"]
    out = out.sort_values(sort_cols, kind="mergesort")

    lap_grp = ["Year", "Race", "LapNumber"]
    pos_grp = out.groupby(lap_grp, sort=False)
    ...
    for step in (1, 2):
        for side, shift_n in [("ahead", step), ("behind", -step)]:
            for src, short in base_cols.items():
                out[f"{side}{step}_{short}"] = pos_grp[src].shift(shift_n)

    for step in (1, 2):
        for side in ("ahead", "behind"):
            mult = 1.0 if side == "ahead" else -1.0
            out[f"{side}{step}_pos_gap"] = mult * (
                out["Position"] - out[f"{side}{step}_pos"]
            )
            out[f"{side}{step}_ltd_gap"] = (
                out["LapTime_Delta"] - out[f"{side}{step}_ltd"]
            )
            out[f"{side}{step}_lt_gap"] = out["LapTime (s)"] - out[f"{side}{step}_lt"]
            out[f"{side}{step}_tyre_gap"] = out["TyreLife"] - out[f"{side}{step}_tyre"]
            out[f"{side}{step}_stint_gap"] = out["Stint"] - out[f"{side}{step}_stint"]
            out[f"{side}{step}_same_compound"] = (
                out["Compound"] == out[f"{side}{step}_compound"]
            ).astype("int8")
            out[f"{side}{step}_same_compound_older_rival"] = (
                (out["Compound"] == out[f"{side}{step}_compound"])
                & (out[f"{side}{step}_tyre"] >= out["TyreLife"] + 5)
            ).astype("int8")
            out[f"{side}{step}_same_compound_fresher_rival"] = (
                (out["Compound"] == out[f"{side}{step}_compound"])
                & (out[f"{side}{step}_tyre"] <= out["TyreLife"] - 5)
            ).astype("int8")

    out["undercut_threat_ahead"] = (
        out["ahead1_same_compound"].fillna(0)
        * (out["ahead1_ltd_gap"] > 0).astype("int8")
        * (out["ahead1_tyre_gap"] > 0).astype("int8")
    )

    out["cover_risk_behind"] = (
        out["behind1_same_compound"].fillna(0)
        * (out["behind1_ltd_gap"] < 0).astype("int8")
        * (out["behind1_tyre_gap"] > 0).astype("int8")
    )

    out["overcut_setup_ahead"] = (
        out["ahead1_same_compound"].fillna(0)
        * (out["ahead1_ltd_gap"] < 0).astype("int8")
        * (out["ahead1_tyre_gap"] < 0).astype("int8")
    )

    out["rejoin_clean_air_proxy"] = (
        (out["behind1_ltd"] > out["LapTime_Delta"]).fillna(False).astype("int8")
        + (out["behind2_ltd"] > out["LapTime_Delta"]).fillna(False).astype("int8")
        + (out["behind1_tyre"] >= out["TyreLife"] + 3).fillna(False).astype("int8")
        + (out["behind2_tyre"] >= out["TyreLife"] + 3).fillna(False).astype("int8")
    ).astype("int8")

    out["traffic_ahead_proxy"] = (
        (out["ahead1_ltd"] < out["LapTime_Delta"]).fillna(False).astype("int8")
        + (out["ahead2_ltd"] < out["LapTime_Delta"]).fillna(False).astype("int8")
    ).astype("int8")

    abs_ahead = out["ahead1_ltd_gap"].abs()
    abs_behind = out["behind1_ltd_gap"].abs()
    choose_ahead = abs_ahead.le(abs_behind) | abs_behind.isna()

    out["nearest_rival_ltd_gap"] = np.where(
        choose_ahead, out["ahead1_ltd_gap"], out["behind1_ltd_gap"]
    )
    out["nearest_rival_tyre_gap"] = np.where(
        choose_ahead, out["ahead1_tyre_gap"], out["behind1_tyre_gap"]
    )
    out["nearest_rival_same_compound"] = np.where(
        choose_ahead, out["ahead1_same_compound"], out["behind1_same_compound"]
    ).astype("float32")
    out["nearest_rival_pos_side"] = np.where(choose_ahead, 1, -1).astype("int8")

    out["pit_window_urgency"] = (
        0.6 * out["cover_risk_behind"].fillna(0)
        + 0.4 * out["undercut_threat_ahead"].fillna(0)
        + 0.15 * out["traffic_ahead_proxy"].fillna(0)
        + 0.10 * out["rejoin_clean_air_proxy"].fillna(0)
    ).astype("float32")

    out["pit_counterfactual_gain_proxy"] = (
        (-out["behind1_ltd_gap"].fillna(0)).clip(-10, 10) * 0.35
        + (out["ahead1_ltd_gap"].fillna(0)).clip(-10, 10) * 0.25
        + (out["behind1_tyre_gap"].fillna(0)).clip(-20, 20) * 0.05
        + (-out["ahead1_tyre_gap"].fillna(0)).clip(-20, 20) * 0.05
        + out["rejoin_clean_air_proxy"].fillna(0) * 0.30
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["001010"],
        "research_usage_note": "Verified assigned hypothesis 001010.",
    }, sort_keys=True))
