...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap = out[lap_col].astype("float64")
    base_keys = ["Year", "Race", "Driver"]
    base_count = out.groupby(base_keys, sort=False, observed=True)[lap_col].transform(
        "size"
    )
    base_driver = (
        out.groupby(base_keys, sort=False, observed=True)[lap_col]
        .transform("median")
        .astype("float64")
    )
    base_compound = (
        out.groupby(["Year", "Race", "Compound"], sort=False, observed=True)[lap_col]
        .transform("median")
        .astype("float64")
    )
    base_race = (
        out.groupby(["Year", "Race"], sort=False, observed=True)[lap_col]
        .transform("median")
        .astype("float64")
    )
    baseline = (
        base_driver.where(base_count >= 4, base_compound)
        .fillna(base_race)
        .fillna(float(lap.median()))
    )
    ...
    work = pd.DataFrame(
        {
            ...
            "Driver": out["Driver"].to_numpy(),
            "Stint": out["Stint"].to_numpy(),
            "LapNumber": out["LapNumber"].to_numpy(),
            "Compound": out["Compound"].to_numpy(),
            "PitStop": out["PitStop"].to_numpy(),
            "TyreLife": out["TyreLife"].to_numpy(),
            "_resid": (lap - baseline).to_numpy(dtype="float64"),
        }
    ).sort_values(
        ["Year", "Race", "Driver", "Stint", "LapNumber", "_ord"], kind="mergesort"
    )

    keys = ["Year", "Race", "Driver", "Stint"]
    g = work.groupby(keys, sort=False, observed=True)
    obs = work["_resid"] - g["_resid"].transform("first")
    lo, hi = np.nanpercentile(obs.to_numpy(dtype="float64"), [1, 99])
    work["_obs"] = obs.clip(lo, hi).astype("float64")

    g = work.groupby(keys, sort=False, observed=True)
    lap_step = g["LapNumber"].diff()
    tyre_step = g["TyreLife"].diff()
    raw_diff = g["_obs"].diff()
    valid_diff = raw_diff.where(
        (lap_step > 0)
        & (lap_step <= 3)
        & (tyre_step >= 0)
        & (tyre_step <= 3)
    ...
    positive_diff = valid_diff.where(valid_diff > 0)
    global_rate = positive_diff.median()
    if not np.isfinite(global_rate):
        global_rate = 0.0
    rate_table = positive_diff.groupby(
        work["Compound"], sort=False, observed=True
    ).median()
    rate = (
        work["Compound"]
        .map(rate_table)
        .astype("float64")
        .fillna(float(global_rate))
        .clip(0.0, 5.0)
    )

    alpha = 0.35
    beta = 1.0 - alpha
    age = g.cumcount().astype("float64")
    pow_beta = np.power(beta, age.to_numpy(dtype="float64"))
    adj_obs = work["_obs"].to_numpy(dtype="float64") - rate.to_numpy(
        dtype="float64"
    ) * age.to_numpy(dtype="float64")

    work["_scaled_adj"] = adj_obs / pow_beta
    g = work.groupby(keys, sort=False, observed=True)
    cum_scaled = g["_scaled_adj"].cumsum()
    first_scaled = g["_scaled_adj"].transform("first")
    z_state = pow_beta * (
        first_scaled.to_numpy(dtype="float64")
        + alpha
        * (
            cum_scaled.to_numpy(dtype="float64")
            - first_scaled.to_numpy(dtype="float64")
        )
    )
    state = z_state + rate.to_numpy(dtype="float64") * age.to_numpy(dtype="float64")
    innovation = work["_obs"].to_numpy(dtype="float64") - state

    work["_scaled_abs_innov"] = np.abs(innovation) / pow_beta
    g = work.groupby(keys, sort=False, observed=True)
    cum_abs = g["_scaled_abs_innov"].cumsum()
    first_abs = g["_scaled_abs_innov"].transform("first")
    uncertainty = pow_beta * (
        first_abs.to_numpy(dtype="float64")
        + alpha
        * (cum_abs.to_numpy(dtype="float64") - first_abs.to_numpy(dtype="float64"))
    )

    order = work["_ord"].to_numpy(dtype=np.int64)
    latent_state = np.empty(n, dtype="float64")
    latent_next = np.empty(n, dtype="float64")
    latent_innov = np.empty(n, dtype="float64")
    latent_unc = np.empty(n, dtype="float64")
    latent_rate = np.empty(n, dtype="float64")

    latent_state[order] = state
    latent_next[order] = state + rate.to_numpy(dtype="float64")
    latent_innov[order] = innovation
    latent_unc[order] = uncertainty
    latent_rate[order] = rate.to_numpy(dtype="float64")

    out["latent_degradation_state"] = latent_state
    out["latent_degradation_next"] = latent_next
    out["latent_degradation_innovation"] = latent_innov
    out["latent_degradation_uncertainty"] = latent_unc
    out["latent_degradation_rate"] = latent_rate

    return out.drop(columns=["TyreLife", "LapTime_Delta", "Cumulative_Degradation"])
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000750"],
        "research_usage_note": "Verified assigned hypothesis 000750.",
    }, sort_keys=True))
