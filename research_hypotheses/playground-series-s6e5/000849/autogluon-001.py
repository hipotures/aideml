...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_Degradation_Severity"] = (
        out["Compound"].map(compound_severity).astype(float).fillna(1.0)
    )

    raw_deg_slope = (
        0.50 * (cum_deg.abs() / tyre)
        + 0.35 * (lap_delta.clip(lower=0.0) / np.sqrt(tyre))
        + 0.15
        * (
            lap_time.sub(
                lap_time.groupby([out["Year"], out["Race"]]).transform("median")
            ).clip(lower=0.0)
            / tyre
        )
    )
    raw_deg_slope = raw_deg_slope.replace([np.inf, -np.inf], np.nan).fillna(
        raw_deg_slope.median()
    )
    out["Analytic_DegSlope_Proxy"] = (
        raw_deg_slope.clip(lower=0.03, upper=12.0)
        * out["Compound_Degradation_Severity"]
    )

    global_pit_loss = (
        float(lap_delta[pit_now > 0.5].median()) if (pit_now > 0.5).any() else 22.0
    )
    if not np.isfinite(global_pit_loss) or global_pit_loss <= 0:
        global_pit_loss = 22.0

    pit_delta = lap_delta.where(pit_now > 0.5)
    race_pit = pit_delta.groupby([out["Year"], out["Race"]]).transform("median")
    out["PitLoss_Prior"] = race_pit.fillna(global_pit_loss).clip(lower=12.0, upper=45.0)

    n_star = np.sqrt(
        (2.0 * out["PitLoss_Prior"]) / out["Analytic_DegSlope_Proxy"].clip(lower=0.03)
    )
    out["Analytic_Nstar"] = n_star.clip(lower=4.0, upper=70.0)
    out["TyreLife_over_Nstar"] = tyre / (out["Analytic_Nstar"] + eps)
    out["Distance_to_Analytic_Stop"] = out["Analytic_Nstar"] - tyre
    out["Abs_Distance_to_Analytic_Stop"] = out["Distance_to_Analytic_Stop"].abs()
    out["Analytic_Stop_Window"] = (out["Abs_Distance_to_Analytic_Stop"] <= 2.0).astype(
        int
    )

    old_next_penalty = out["Analytic_DegSlope_Proxy"] * (tyre + 1.0)
    fresh_next_penalty = (
        out["Analytic_DegSlope_Proxy"] * out["Compound_Degradation_Severity"]
    )
    out["NextLap_Undercut_Gain"] = (
        old_next_penalty - fresh_next_penalty - out["PitLoss_Prior"] / 10.0
    )

    out["LateRace_Analytic_Pressure"] = out["TyreLife_over_Nstar"] * race_progress
    out["Stint_Adjusted_StopPressure"] = out["TyreLife_over_Nstar"] / np.sqrt(stint)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000849"],
        "research_usage_note": "Verified assigned hypothesis 000849.",
    }, sort_keys=True))
