...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["__orig_order__"] = np.arange(len(out), dtype=np.int64)
    work = out.sort_values(sort_cols + ["__orig_order__"], kind="mergesort")

    stop = work["PitStop"].fillna(0).astype("int8")
    group_keys = [work[c] for c in group_cols]

    prev_stop = stop.groupby(group_keys, sort=False).shift(1).fillna(0).astype("int8")
    last_stop_lap = (
        work["LapNumber"].where(stop.eq(1)).groupby(group_keys, sort=False).ffill()
    )
    laps_since = (
        (work["LapNumber"].astype("float32") - last_stop_lap.astype("float32"))
        .where(last_stop_lap.notna(), -1)
        .clip(-1, 80)
    )

    post_1 = laps_since.eq(1).astype("int8")
    post_2 = laps_since.eq(2).astype("int8")
    post_3 = laps_since.eq(3).astype("int8")
    fresh_3 = laps_since.between(1, 3).astype("int8")
    recent_5 = laps_since.between(0, 5).astype("int8")

    work["post_stop_current_observed"] = stop
    work["post_stop_prev_lap_observed"] = prev_stop
    work["post_stop_laps_since_last_observed"] = laps_since.astype("float32")
    work["post_stop_has_observed_stop"] = laps_since.ge(0).astype("int8")
    work["post_stop_lap1"] = post_1
    work["post_stop_lap2"] = post_2
    work["post_stop_lap3"] = post_3
    work["post_stop_fresh_3_laps"] = fresh_3
    work["post_stop_recent_5_laps"] = recent_5

    tyre_life = work["TyreLife"].astype("float32")
    race_progress = work["RaceProgress"].astype("float32")
    fresh_float = fresh_3.astype("float32")
    recent_float = recent_5.astype("float32")

    work["post_stop_fresh_x_tyrelife"] = fresh_float * tyre_life
    work["post_stop_recent_x_tyrelife"] = recent_float * tyre_life
    work["post_stop_fresh_x_raceprogress"] = fresh_float * race_progress
    work["post_stop_current_x_raceprogress"] = stop.astype("float32") * race_progress

    compound = work["Compound"].astype(str)
    for comp in ["SOFT", "MEDIUM", "HARD", "INTERMEDIATE", "WET"]:
        work[f"post_stop_fresh_{comp.lower()}"] = (
            (compound.eq(comp)) & fresh_3.eq(1)
        ).astype("int8")

    work["post_stop_compound_state"] = compound + "_post" + fresh_3.astype(str)

    work = work.sort_values("__orig_order__", kind="mergesort").drop(
        columns=["__orig_order__"]
    )
    return work
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000034"],
        "research_usage_note": "Verified assigned hypothesis 000034.",
    }, sort_keys=True))
