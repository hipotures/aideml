...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Compound_code"] = compound.cat.codes.astype("int16")
    out["Race_code"] = race.cat.codes.astype("int16")
    out["Driver_code"] = driver.cat.codes.astype("int16")
    ...
    out["is_intermediate"] = (out["Compound"] == "INTERMEDIATE").astype("int8")
    out["is_full_wet"] = (out["Compound"] == "WET").astype("int8")

    out["race_year_block"] = (
        (out["Year"].astype("int16").astype(str) + "_" + out["Race"].astype(str))
        .astype("category")
        .cat.codes.astype("int16")
    )

    out["lap_frac_remaining"] = (1.0 - out["RaceProgress"]).astype("float32")
    out["tyre_life_progress_ratio"] = (
        out["TyreLife"] / (out["LapNumber"].clip(lower=1))
    ).astype("float32")
    out["stint_lap_ratio"] = (out["TyreLife"] / (out["Stint"].clip(lower=1))).astype(
        "float32"
    )
    ...
    out["late_race_old_tyre"] = (out["RaceProgress"] * out["TyreLife"]).astype(
        "float32"
    )
    out["pit_pressure_score"] = (
        out["TyreLife"] * (1.0 + out["RaceProgress"]) + out["Stint"] * 3.0
    ).astype("float32")
    ...
    for col in [
        ...
        mean = out.groupby(grp, observed=True)[col].transform("mean")
        std = out.groupby(grp, observed=True)[col].transform("std").replace(0, np.nan)
        out[f"{col}_race_lap_centered"] = (out[col] - mean).astype("float32")
        out[f"{col}_race_lap_z"] = ((out[col] - mean) / std).fillna(0).astype("float32")

    comp_grp = ["Year", "Race", "Compound"]
    for col in ["TyreLife", "Cumulative_Degradation", "LapTime_Delta"]:
        mean = out.groupby(comp_grp, observed=True)[col].transform("mean")
        out[f"{col}_compound_block_centered"] = (out[col] - mean).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000599.",
    }, sort_keys=True))
