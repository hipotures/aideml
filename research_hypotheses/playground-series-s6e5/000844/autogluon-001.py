...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_max_lap = out.groupby(race_key)["LapNumber"].transform("max").astype("float32")
    out["laps_to_race_end"] = (race_max_lap - out["LapNumber"]).astype("float32")
    out["lap_frac_in_race"] = (
        (out["LapNumber"] / race_max_lap.replace(0, np.nan)).fillna(0).astype("float32")
    )
    ...
    race_lap_median = (
        out.groupby(race_key)["LapTime (s)"].transform("median").astype("float32")
    )
    out["laptime_vs_race_median"] = (out["LapTime (s)"] - race_lap_median).astype(
        "float32"
    )

    driver_race_max_stint = (
        out.groupby(driver_race_key)["Stint"].transform("max").astype("float32")
    )
    out["stint_frac_driver_race"] = (
        (out["Stint"] / driver_race_max_stint.replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000844.",
    }, sort_keys=True))
