...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["stint_lap_ratio"] = tyre / lap.clip(lower=1)
    out["laps_remaining_est"] = lap * (1 - progress) / progress.clip(lower=0.02)
    out["race_final_quarter"] = (progress >= 0.75).astype("int8")
    out["race_final_tenth"] = (progress >= 0.90).astype("int8")

    out["service_window_pressure"] = np.maximum(0, tyre - 12) * (
        1 + 0.35 * np.maximum(0, stint - 1)
    )
    out["cannot_finish_pressure"] = tyre / (out["laps_remaining_est"] + eps)
    out["late_old_tyre_pressure"] = tyre * progress
    out["stint_progress_pressure"] = stint * progress

    out["degradation_per_tyre_lap"] = degr / tyre.clip(lower=1)
    out["degradation_pressure"] = np.maximum(0, out["degradation_per_tyre_lap"])
    out["slow_lap_pressure"] = np.maximum(0, delta)
    out["abs_laptime_delta"] = np.abs(delta)
    out["lap_time_x_progress"] = laptime * progress
    out["degradation_x_tyre_life"] = degr * tyre

    out["position_loss_pressure"] = np.maximum(0, poschg)
    out["front_running"] = (pos <= 5).astype("int8")
    out["back_marker"] = (pos >= 16).astype("int8")

    out["wet_or_inter"] = (
        out["Compound"].astype(str).isin(["WET", "INTERMEDIATE"]).astype("int8")
    )
    out["dry_compound"] = (
        out["Compound"].astype(str).isin(["SOFT", "MEDIUM", "HARD"]).astype("int8")
    )
    out["compound_tyre_life"] = (
    ...
    for c in ["compound_tyre_life", "driver_race", "race_compound"]:
        out[c] = out[c].astype("category")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000735.",
    }, sort_keys=True))
