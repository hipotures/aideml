...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_test = np.zeros(n, dtype=np.int8)
    if n > n_test:
        is_test[n_train:] = 1
    else:
        is_test[:] = 1

    cat_cols = ["Race", "Year", "Driver", "Compound", "Stint", "PitStop"]
    num_cols = [
        "LapNumber",
        "LapTime (s)",
        "LapTime_Delta",
    ...
    for col in cat_cols:
        s = out[col].astype(str).fillna("__MISSING__")
        tr = s.iloc[:n_train] if n_train < n else s.iloc[:0]
        te = s.iloc[n_train:] if n_train < n else s
        tr_freq = tr.value_counts(normalize=True)
        te_freq = te.value_counts(normalize=True)
        cats = tr_freq.index.union(te_freq.index)
        trp = tr_freq.reindex(cats, fill_value=0.0)
        tep = te_freq.reindex(cats, fill_value=0.0)
        ratio = np.log((tep + 1e-4) / (trp + 1e-4)).clip(-3, 3)
        out[f"{col}_shift_logratio"] = s.map(ratio).astype(np.float32)
        logit += out[f"{col}_shift_logratio"].to_numpy(dtype=np.float32)

    for col in num_cols:
        x = pd.to_numeric(out[col], errors="coerce").astype(np.float32)
        ranks = x.rank(method="average", pct=True)
        bins = np.floor(ranks * 20).clip(0, 19).astype("Int64")
        tr = bins.iloc[:n_train] if n_train < n else bins.iloc[:0]
        te = bins.iloc[n_train:] if n_train < n else bins
        tr_freq = tr.value_counts(normalize=True, dropna=False)
        te_freq = te.value_counts(normalize=True, dropna=False)
        all_bins = tr_freq.index.union(te_freq.index)
        trp = tr_freq.reindex(all_bins, fill_value=0.0)
        tep = te_freq.reindex(all_bins, fill_value=0.0)
        ratio = np.log((tep + 1e-4) / (trp + 1e-4)).clip(-2, 2)
        mapped = bins.map(ratio).fillna(0.0).astype(np.float32)
        out[f"{col}_bin_shift_logratio"] = mapped
        logit += mapped.to_numpy(dtype=np.float32)

    preseason = (out["Race"].astype(str) == "Pre-Season Testing").astype(np.float32)
    out["is_preseason_testing"] = preseason
    out["preseason_downweight_proxy"] = (1.0 / (1.0 + 4.0 * preseason)).astype(
        np.float32
    )

    gp_like = (
        -preseason
        + 0.5 * (out["RaceProgress"].astype(np.float32) - 0.5)
        - 0.03 * out["PitStop"].astype(np.float32)
    )
    testing_like = (
        preseason
        + 0.02 * out["LapTime_Delta"].abs().astype(np.float32)
        + 0.001 * out["Cumulative_Degradation"].clip(lower=0).astype(np.float32)
    )
    out["gp_like_score"] = gp_like.astype(np.float32)
    out["testing_like_score"] = testing_like.astype(np.float32)

    logit = np.clip(logit, -6, 6)
    domain_prob = 1.0 / (1.0 + np.exp(-logit))
    weight = np.clip(domain_prob / np.clip(1.0 - domain_prob, 1e-3, None), 0.2, 5.0)

    out["domain_shift_logit"] = logit.astype(np.float32)
    out["domain_shift_prob"] = domain_prob.astype(np.float32)
    out["importance_weight_proxy"] = weight.astype(np.float32)
    out["effective_weight_proxy"] = np.sqrt(weight).astype(np.float32)
    out["expert_blend_proxy"] = (
        out["gp_like_score"] - out["testing_like_score"]
    ).astype(np.float32)

    for a, b, name in [
        ("TyreLife", "RaceProgress", "tyrelife_x_progress"),
        ("TyreLife", "Cumulative_Degradation", "tyrelife_x_deg"),
        ("LapNumber", "Stint", "lap_x_stint"),
        ("Position", "Position_Change", "position_x_delta"),
        ("importance_weight_proxy", "TyreLife", "weight_x_tyrelife"),
        ("importance_weight_proxy", "RaceProgress", "weight_x_progress"),
    ]:
        out[name] = pd.to_numeric(out[a], errors="coerce").astype(
            np.float32
        ) * pd.to_numeric(out[b], errors="coerce").astype(np.float32)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000377"],
        "research_usage_note": "Verified assigned hypothesis 000377.",
    }, sort_keys=True))
