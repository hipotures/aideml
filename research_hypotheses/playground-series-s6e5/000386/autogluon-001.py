...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_progress = df["RaceProgress"].replace(0, np.nan)
    total_laps_est = (df["LapNumber"] / race_progress).clip(lower=df["LapNumber"])
    laps_remaining_est = (total_laps_est - df["LapNumber"]).clip(lower=0)
    ...
    df["CompoundSoftness"] = df["Compound"].map(compound_softness).astype(float)
    df["IsWetLike"] = df["Compound"].isin(["INTERMEDIATE", "WET"]).astype(np.int8)
    df["IsSlick"] = df["Compound"].isin(["HARD", "MEDIUM", "SOFT"]).astype(np.int8)

    pos_deg = df["Cumulative_Degradation"].clip(lower=0)
    pos_delta = df["LapTime_Delta"].clip(lower=0)

    df["TotalLapsEst"] = total_laps_est.fillna(df["LapNumber"])
    df["LapsRemainingEst"] = laps_remaining_est.fillna(0)
    df["FinishMargin"] = (1.0 - df["RaceProgress"]).clip(lower=0, upper=1)
    df["TyreLifePerLapRemaining"] = df["TyreLife"] / (df["LapsRemainingEst"] + 1.0)
    df["DegradationPerTyreLap"] = pos_deg / (df["TyreLife"] + 1.0)
    df["WearPressure"] = np.log1p(df["TyreLife"]) * np.log1p(pos_deg)
    df["PaceLossPressure"] = np.log1p(pos_delta) * np.log1p(df["TyreLife"])
    df["StrategyPressure"] = df["WearPressure"] * (
        df["LapsRemainingEst"] / (df["TyreLife"] + 1.0)
    )
    df["LateStintPressure"] = df["TyreLife"] * df["RaceProgress"]

    stint_median = df.groupby(["Compound", "Stint"])["TyreLife"].transform("median")
    deg_median = df.groupby(["Compound", "Stint"])["Cumulative_Degradation"].transform(
        "median"
    )
    delta_median = df.groupby(["Compound", "Stint"])["LapTime_Delta"].transform(
        "median"
    )

    df["StopDebt"] = df["TyreLife"] - stint_median
    df["DegradationDebt"] = df["Cumulative_Degradation"] - deg_median
    df["PaceDebt"] = df["LapTime_Delta"] - delta_median

    race_compound_freq = df.groupby(["Race", "Compound"])["Driver"].transform("size")
    driver_race_freq = df.groupby(["Driver", "Race"])["LapNumber"].transform("size")
    df["RaceCompoundFreq"] = race_compound_freq.astype(float)
    df["DriverRaceHistory"] = driver_race_freq.astype(float)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000386.",
    }, sort_keys=True))
