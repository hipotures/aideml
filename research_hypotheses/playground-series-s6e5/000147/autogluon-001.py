...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    race_year = ["Race", "Year"]
    driver_key = ["Race", "Year", "Driver"]

    out["LapsRemaining_Est"] = (
        out["LapNumber"] / out["RaceProgress"].clip(lower=eps) - out["LapNumber"]
    ).clip(lower=0)
    out["StintProgress_Est"] = out["TyreLife"] / (
        out["TyreLife"] + out["LapsRemaining_Est"] + eps
    )

    out["StintMean_LapTime"] = out.groupby(race_year + ["Stint"])[
        "LapTime (s)"
    ].transform("mean")
    out["StintResidual_LapTime"] = out["LapTime (s)"] - out["StintMean_LapTime"]

    sort_cols = driver_key + ["LapNumber"]
    s = out.sort_values(sort_cols, kind="mergesort")

    for col, base in [
        ("LapTime_Delta", "LapDelta"),
        ("StintResidual_LapTime", "StintResid"),
        ("Position_Change", "PosChange"),
    ]:
        shifted = s.groupby(driver_key, sort=False)[col].shift(1)
        roll = shifted.groupby([s[c] for c in driver_key], sort=False).rolling(
            5, min_periods=2
        )
        s[f"{base}_RollStd5"] = roll.std().reset_index(level=[0, 1, 2], drop=True)
        med = (
            shifted.groupby([s[c] for c in driver_key], sort=False)
            .rolling(5, min_periods=2)
            .median()
            .reset_index(level=[0, 1, 2], drop=True)
        )
        abs_dev = (shifted - med).abs()
        s[f"{base}_RollMAD5"] = (
            abs_dev.groupby([s[c] for c in driver_key], sort=False)
            .rolling(5, min_periods=2)
            .median()
            .reset_index(level=[0, 1, 2], drop=True)
        )
    ...
    out["FieldPitRate_SameLap"] = out.groupby(lap_key)["PitStop"].transform("mean")
    out["FieldPaceStd_SameLap"] = (
        out.groupby(lap_key)["LapTime_Delta"].transform("std").fillna(0)
    )
    out["FieldPaceRank_SameLap"] = out.groupby(lap_key)["LapTime_Delta"].rank(
        method="average", pct=True
    )

    pos_bin = (out["Position"].astype(float) // 3).astype("int16")
    out["TrafficPaceMean_PosBand"] = out.groupby(
        [out["Race"], out["Year"], out["LapNumber"], pos_bin]
    )["LapTime_Delta"].transform("mean")
    out["TrafficPaceStd_PosBand"] = (
        out.groupby([out["Race"], out["Year"], out["LapNumber"], pos_bin])[
            "LapTime_Delta"
        ]
        .transform("std")
        .fillna(0)
    )
    out["TrafficPaceRel_PosBand"] = (
        out["LapTime_Delta"] - out["TrafficPaceMean_PosBand"]
    )
    ...
    vol_cols = [
        ...
        "PosChange_RollStd5",
        "PosChange_RollMAD5",
        "FieldPaceStd_SameLap",
        "TrafficPaceStd_PosBand",
    ]
    for c in vol_cols:
        out[c] = out[c].fillna(0)
        out[f"{c}_x_LapsRemaining"] = out[c] * out["LapsRemaining_Est"]

    comp_codes = pd.Categorical(out["Compound"]).codes.astype("int16")
    out["Compound_Code"] = comp_codes
    out["Compound_x_FieldPitRate"] = comp_codes * out["FieldPitRate_SameLap"]
    out["Compound_x_LapDeltaVol"] = comp_codes * out["LapDelta_RollStd5"]
    out["Compound_x_LapsRemaining"] = comp_codes * out["LapsRemaining_Est"]
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000147"],
        "research_usage_note": "Verified assigned hypothesis 000147.",
    }, sort_keys=True))
