...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    is_wet = compound.isin(["INTERMEDIATE", "WET"]).astype(np.int8)
    is_dry = compound.isin(["SOFT", "MEDIUM", "HARD"]).astype(np.int8)

    sort_cols = ["Year", "Race", "Driver", "LapNumber"]
    if "Stint" in out.columns:
        sort_cols.append("Stint")

    s = out.assign(
        _orig_order=np.arange(len(out)),
        _is_wet=is_wet,
        _soft=(compound == "SOFT").astype(np.int8),
        _medium=(compound == "MEDIUM").astype(np.int8),
        _hard=(compound == "HARD").astype(np.int8),
    ).sort_values(sort_cols + ["_orig_order"], kind="mergesort")

    g = s.groupby(["Year", "Race", "Driver"], sort=False)

    s["_wet_seen_current_or_prior"] = g["_is_wet"].cummax().astype(np.int8)
    s["_soft_seen"] = g["_soft"].cummax().astype(np.int8)
    s["_medium_seen"] = g["_medium"].cummax().astype(np.int8)
    s["_hard_seen"] = g["_hard"].cummax().astype(np.int8)

    dry_seen_count = s["_soft_seen"] + s["_medium_seen"] + s["_hard_seen"]
    dry_debt_raw = (2 - dry_seen_count).clip(lower=0).astype(np.int8)
    dry_debt_exempt = dry_debt_raw.mask(
        s["_wet_seen_current_or_prior"].eq(1), 0
    ).astype(np.int8)

    s["wet_weather_seen_driver_race"] = s["_wet_seen_current_or_prior"]
    s["dry_compound_debt_raw"] = dry_debt_raw
    s["dry_compound_debt_after_wet_exemption"] = dry_debt_exempt
    s["dry_debt_wet_exempted"] = (
        (s["_wet_seen_current_or_prior"].eq(1)) & (dry_debt_raw.gt(0))
    ).astype(np.int8)
    s["is_slick_compound"] = is_dry.reindex(s.index).astype(np.int8)

    add_cols = [
        "wet_weather_seen_driver_race",
        "dry_compound_debt_raw",
        "dry_compound_debt_after_wet_exemption",
        "dry_debt_wet_exempted",
        "is_slick_compound",
    ]

    s = s.sort_values("_orig_order", kind="mergesort")
    for col in add_cols:
        out[col] = s[col].to_numpy()
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000019.",
    }, sort_keys=True))
