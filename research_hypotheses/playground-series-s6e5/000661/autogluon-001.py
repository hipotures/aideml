...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected_rc_lap = out.groupby(["Race", "Compound", "LapNumber"])[
        "LapTime (s)"
    ].transform("median")
    expected_r_lap = out.groupby(["Race", "LapNumber"])["LapTime (s)"].transform(
        "median"
    )
    expected_pace = expected_rc_lap.fillna(expected_r_lap)

    out["latent_residual"] = (out["LapTime (s)"] - expected_pace).clip(-180, 180)

    rc_mean = out.groupby(["Race", "Compound"])["latent_residual"].transform("mean")
    rc_std = (
        out.groupby(["Race", "Compound"])["latent_residual"]
        .transform("std")
        .replace(0, np.nan)
    )
    out["latent_residual_z_rc"] = (
        ((out["latent_residual"] - rc_mean) / rc_std).fillna(0).clip(-8, 8)
    )

    out = out.sort_values(["Race", "Driver", "Stint", "LapNumber", "_row_order"]).copy()
    g = out.groupby(["Race", "Driver", "Stint"], sort=False)

    resid_prev = g["latent_residual"].shift(1)
    resid_prev3_mean = resid_prev.groupby(
        [out["Race"], out["Driver"], out["Stint"]]
    ).transform(lambda s: s.rolling(3, min_periods=1).mean())
    resid_prev5_mean = resid_prev.groupby(
        [out["Race"], out["Driver"], out["Stint"]]
    ).transform(lambda s: s.rolling(5, min_periods=1).mean())

    out["latent_residual_prev"] = resid_prev.fillna(0)
    out["latent_residual_slope3"] = (
        ((resid_prev - g["latent_residual"].shift(3)) / 3.0).fillna(0).clip(-60, 60)
    )
    out["latent_residual_slope5"] = (
        ((resid_prev - g["latent_residual"].shift(5)) / 5.0).fillna(0).clip(-60, 60)
    )
    out["latent_residual_accel"] = (
        g["latent_residual_slope3"].diff().fillna(0).clip(-40, 40)
    )
    out["latent_residual_roll3_mean"] = resid_prev3_mean.fillna(0)
    out["latent_residual_roll5_mean"] = resid_prev5_mean.fillna(0)
    out["latent_residual_vs_roll3"] = (
        (resid_prev - resid_prev3_mean).fillna(0).clip(-120, 120)
    )
    out["latent_residual_vs_roll5"] = (
        (resid_prev - resid_prev5_mean).fillna(0).clip(-120, 120)
    )

    pos_resid_prev = resid_prev.clip(lower=0).fillna(0)
    out["latent_cum_positive_residual"] = g[pos_resid_prev.name].transform(
        lambda s: 0
    )  # placeholder alignment
    out["latent_cum_positive_residual"] = (
        pos_resid_prev.groupby([out["Race"], out["Driver"], out["Stint"]])
        .cumsum()
        .clip(0, 500)
    )

    out["latent_residual_x_tyrelife"] = out["latent_residual_prev"] * out["TyreLife"]
    out["latent_slope3_x_tyrelife"] = out["latent_residual_slope3"] * out["TyreLife"]
    out["latent_residual_x_deg"] = (
        out["latent_residual_prev"] * out["Cumulative_Degradation"]
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000661"],
        "research_usage_note": "Verified assigned hypothesis 000661.",
    }, sort_keys=True))
