...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["IsDry"] = out["Compound"].isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")
    out["IsWetLike"] = out["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    ...
    for col in roll_cols:
        lag1 = g[col].shift(1)
        out[f"{col}_roll3_mean"] = lag1.groupby(
            [out[c] for c in group_cols], sort=False
        ).transform(lambda s: s.rolling(3, min_periods=1).mean())
        out[f"{col}_roll3_std"] = lag1.groupby(
            [out[c] for c in group_cols], sort=False
        ).transform(lambda s: s.rolling(3, min_periods=1).std())
        out[f"{col}_roll5_mean"] = lag1.groupby(
            [out[c] for c in group_cols], sort=False
        ).transform(lambda s: s.rolling(5, min_periods=1).mean())
        out[f"{col}_dev_roll3"] = out[col] - out[f"{col}_roll3_mean"]

    out["Deg_per_TyreLife"] = out["Cumulative_Degradation"] / out["TyreLife"].clip(
        lower=1.0
    )
    out["LapDelta_x_TyreLife"] = out["LapTime_Delta"] * out["TyreLife"]
    out["Deg_x_Compound"] = out["Cumulative_Degradation"] * out["CompoundOrdinal"]
    out["TyreLife_x_Compound"] = out["TyreLife"] * out["CompoundOrdinal"]
    out["PositionPressure"] = out["Position"] + out["Position_Change"]
    out["LateRaceOnOldTyres"] = out["RaceProgress"] * out["TyreLife"]
    out["DegAcceleration"] = out["Cumulative_Degradation_diff1"] - (
        out["Cumulative_Degradation_lag1"] - out["Cumulative_Degradation_lag2"]
    )
    out["LapDeltaAcceleration"] = out["LapTime_Delta_diff1"] - (
        out["LapTime_Delta_lag1"] - out["LapTime_Delta_lag2"]
    )

    out["JustPittedPrevLap"] = (out["PitStop_lag1"] == 1).astype("int8")
    out["StintChangedPrevLap"] = (out["Stint_diff1"] != 0).astype("int8")
    out["TyreResetFlag"] = (
        (out["TyreLife"] <= 2) | (out["TyreLife_diff1"] < 0)
    ).astype("int8")

    for col in ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]:
        std_col = f"{col}_roll3_std"
        mean_col = f"{col}_roll3_mean"
        out[f"{col}_z_roll3"] = (out[col] - out[mean_col]) / (
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000990"],
        "research_usage_note": "Verified assigned hypothesis 000990.",
    }, sort_keys=True))
