...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.copy()
    X["_rowid"] = np.arange(len(X), dtype=np.int64)

    race_keys = ["Year", "Race"]
    driver_race_keys = ["Year", "Race", "Driver"]

    X = X.sort_values(
        driver_race_keys + ["LapNumber", "_rowid"], kind="mergesort"
    ).reset_index(drop=True)

    compound = X["Compound"].astype(str).str.upper()
    is_wet = compound.isin(["WET", "INTERMEDIATE"]).astype("int8")
    is_dry = (1 - is_wet).astype("int8")
    is_inter = compound.eq("INTERMEDIATE").astype("int8")
    is_full_wet = compound.eq("WET").astype("int8")
    is_soft = compound.eq("SOFT").astype("int8")
    is_medium = compound.eq("MEDIUM").astype("int8")
    is_hard = compound.eq("HARD").astype("int8")

    X["compound_family"] = np.where(is_wet == 1, "wet", "dry")
    X["is_wet_compound"] = is_wet
    X["is_intermediate"] = is_inter
    X["is_full_wet"] = is_full_wet
    X["is_soft"] = is_soft
    X["is_medium"] = is_medium
    X["is_hard"] = is_hard

    lap_grp = X.groupby(race_keys + ["LapNumber"], sort=False)
    drv_grp = X.groupby(driver_race_keys, sort=False)

    X["field_size_lap"] = lap_grp["Driver"].transform("size").astype("float32")
    X["field_wet_share_lap"] = (
        lap_grp["is_wet_compound"].transform("mean").astype("float32")
    )
    X["field_inter_share_lap"] = (
        lap_grp["is_intermediate"].transform("mean").astype("float32")
    )
    X["field_full_wet_share_lap"] = (
        lap_grp["is_full_wet"].transform("mean").astype("float32")
    )
    X["field_soft_share_lap"] = lap_grp["is_soft"].transform("mean").astype("float32")
    X["field_medium_share_lap"] = (
        lap_grp["is_medium"].transform("mean").astype("float32")
    )
    X["field_hard_share_lap"] = lap_grp["is_hard"].transform("mean").astype("float32")
    X["field_pit_share_lap"] = lap_grp["PitStop"].transform("mean").astype("float32")
    X["field_laptime_median_lap"] = (
        lap_grp["LapTime (s)"].transform("median").astype("float32")
    )
    X["field_delta_median_lap"] = (
        lap_grp["LapTime_Delta"].transform("median").astype("float32")
    )
    X["field_poschg_median_lap"] = (
        lap_grp["Position_Change"].transform("median").astype("float32")
    )

    X["laptime_vs_field"] = (X["LapTime (s)"] - X["field_laptime_median_lap"]).astype(
        "float32"
    )
    X["delta_vs_field"] = (X["LapTime_Delta"] - X["field_delta_median_lap"]).astype(
        "float32"
    )
    X["poschg_vs_field"] = (X["Position_Change"] - X["field_poschg_median_lap"]).astype(
        "float32"
    )

    prev_compound = drv_grp["Compound"].shift(1).astype("string")
    prev_is_wet = drv_grp["is_wet_compound"].shift(1).fillna(0).astype("int8")
    prev_pit = drv_grp["PitStop"].shift(1).fillna(0).astype("int8")
    prev_delta = drv_grp["LapTime_Delta"].shift(1).fillna(0).astype("float32")
    prev_poschg = drv_grp["Position_Change"].shift(1).fillna(0).astype("float32")

    X["prev_is_wet_compound"] = prev_is_wet
    X["prev_pitstop"] = prev_pit
    X["compound_changed_prev_lap"] = (
        prev_compound.ne(X["Compound"].astype("string")).fillna(False).astype("int8")
    )
    X["entered_wet_prev_lap"] = ((prev_is_wet == 0) & (is_wet == 1)).astype("int8")
    X["entered_dry_prev_lap"] = ((prev_is_wet == 1) & (is_wet == 0)).astype("int8")
    X["delta_shock"] = (X["LapTime_Delta"] - prev_delta).astype("float32")
    X["poschg_shock"] = (X["Position_Change"] - prev_poschg).astype("float32")

    X["tyre_life_ratio"] = (X["TyreLife"] / X["LapNumber"].clip(lower=1)).astype(
        "float32"
    )
    X["stint_progress"] = (X["TyreLife"] / (X["TyreLife"] + 3.0)).astype("float32")
    X["deg_per_lap"] = (
        X["Cumulative_Degradation"] / X["TyreLife"].clip(lower=1)
    ).astype("float32")
    X["deg_x_age"] = (X["Cumulative_Degradation"] * X["TyreLife"]).astype("float32")
    X["late_race"] = (X["RaceProgress"] >= 0.7).astype("int8")
    X["very_late_race"] = (X["RaceProgress"] >= 0.85).astype("int8")

    soft_seen = drv_grp["is_soft"].cummax().astype("int8")
    medium_seen = drv_grp["is_medium"].cummax().astype("int8")
    hard_seen = drv_grp["is_hard"].cummax().astype("int8")
    wet_seen = drv_grp["is_wet_compound"].cummax().astype("int8")

    X["other_dry_used_before"] = (
        ((is_soft == 0) & (soft_seen == 1))
        | ((is_medium == 0) & (medium_seen == 1))
        | ((is_hard == 0) & (hard_seen == 1))
    ).astype("int8")
    X["multi_dry_compounds_seen"] = ((soft_seen + medium_seen + hard_seen) >= 2).astype(
        "int8"
    )
    X["mandatory_dry_pressure"] = (
        (is_dry == 1)
        * (wet_seen == 0)
        * (X["other_dry_used_before"] == 0)
        * X["late_race"]
    ).astype("int8")

    X["dry_strategic_gate"] = (
        (is_dry == 1) & (X["field_wet_share_lap"] <= 0.10)
    ).astype("int8")

    X["wet_to_dry_gate"] = (
        (is_wet == 1)
        & ((X["field_wet_share_lap"] < 0.85) | (X["field_inter_share_lap"] > 0.0))
    ).astype("int8")

    X["dry_to_wet_or_reactive_gate"] = (
        (is_dry == 1)
        & (
            (X["field_wet_share_lap"] > 0.10)
            | (X["delta_shock"] > 5.0)
            | (X["poschg_shock"] < -2.0)
        )
    ).astype("int8")

    X["dry_strategic_hazard_age"] = (X["dry_strategic_gate"] * X["TyreLife"]).astype(
        "float32"
    )
    X["dry_strategic_hazard_deg"] = (
        X["dry_strategic_gate"] * X["Cumulative_Degradation"]
    ).astype("float32")
    X["dry_strategic_hazard_rule"] = (
        X["dry_strategic_gate"]
        * X["mandatory_dry_pressure"]
        * (1.0 - X["multi_dry_compounds_seen"])
    ).astype("float32")

    X["wet_to_dry_hazard_crossover"] = (
        X["wet_to_dry_gate"] * (1.0 - X["field_wet_share_lap"]) * X["TyreLife"]
    ).astype("float32")
    X["wet_to_dry_hazard_pace"] = (
        X["wet_to_dry_gate"] * (-X["laptime_vs_field"])
    ).astype("float32")
    X["wet_to_dry_hazard_intermediate"] = (
        X["wet_to_dry_gate"] * X["is_intermediate"] * (1.0 - X["field_inter_share_lap"])
    ).astype("float32")

    X["dry_to_wet_hazard_transition"] = (
        X["dry_to_wet_or_reactive_gate"] * X["field_wet_share_lap"]
    ).astype("float32")
    X["dry_to_wet_hazard_shock"] = (
        X["dry_to_wet_or_reactive_gate"]
        * (X["delta_shock"].clip(lower=0) - X["poschg_shock"].clip(upper=0))
    ).astype("float32")
    X["dry_to_wet_hazard_reactive"] = (
        X["dry_to_wet_or_reactive_gate"]
        * (X["field_pit_share_lap"] + (X["Position"] / 20.0))
    ).astype("float32")

    X["gate_overlap_wet_dry"] = (
        (X["wet_to_dry_gate"] == 1) & (X["dry_to_wet_or_reactive_gate"] == 1)
    ).astype("int8")
    X["pit_after_pit_flag"] = ((X["prev_pitstop"] == 1) & (X["PitStop"] == 1)).astype(
        "int8"
    )
    X["fresh_tyre_flag"] = (X["TyreLife"] <= 2).astype("int8")
    X["stint_reset_flag"] = (
        (X["PitStop"] == 1) | (X["compound_changed_prev_lap"] == 1)
    ).astype("int8")

    X = (
        X.sort_values("_rowid", kind="mergesort")
        .drop(columns=["_rowid"])
        .reset_index(drop=True)
    )
    return X
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000752.",
    }, sort_keys=True))
