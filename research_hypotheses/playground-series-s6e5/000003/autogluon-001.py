...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    USE_FRESH_SLICK_FULL_BLOCK = False
    USE_FRESH_SLICK_COMPACT_BLOCK = True
    USE_FINISH_WINDOW_BASELINE = True

    lap = out["LapNumber"].astype(float)
    progress = out["RaceProgress"].astype(float).clip(1e-4, 1.0)
    tyre_life = out["TyreLife"].astype(float)
    compound = out["Compound"].astype(str).str.upper()

    est_race_laps = (lap / progress).clip(1, 90)
    remaining_laps = (est_race_laps - lap).clip(lower=0)

    slick_caps = {"SOFT": 22.0, "MEDIUM": 32.0, "HARD": 44.0}
    current_cap = compound.map(slick_caps).astype(float).fillna(0.0)

    if USE_FINISH_WINDOW_BASELINE:
        out["EstimatedRaceLaps"] = est_race_laps.astype("float32")
        out["RemainingLaps"] = remaining_laps.astype("float32")
        out["CurrentTyreFinishMargin"] = (
            current_cap - tyre_life - remaining_laps
        ).astype("float32")
        out["CurrentTyreCanFinish"] = (out["CurrentTyreFinishMargin"] >= 0).astype(
            "int8"
        )
        out["CurrentTyreNeedsStop"] = (out["CurrentTyreFinishMargin"] < 0).astype(
            "int8"
        )

    soft_margin = slick_caps["SOFT"] - remaining_laps
    medium_margin = slick_caps["MEDIUM"] - remaining_laps
    hard_margin = slick_caps["HARD"] - remaining_laps

    soft_ok = soft_margin >= 0
    medium_ok = medium_margin >= 0
    hard_ok = hard_margin >= 0

    diff_soft_ok = soft_ok & (compound != "SOFT")
    diff_medium_ok = medium_ok & (compound != "MEDIUM")
    diff_hard_ok = hard_ok & (compound != "HARD")

    option_count = (
        diff_soft_ok.astype("int8")
        + diff_medium_ok.astype("int8")
        + diff_hard_ok.astype("int8")
    )
    softest_viable = np.select(
        [diff_soft_ok, diff_medium_ok, diff_hard_ok], [1, 2, 3], default=0
    )
    different_can_finish = option_count > 0
    best_finish_margin = np.maximum.reduce(
        [
            np.where(compound != "SOFT", soft_margin, -999.0),
            np.where(compound != "MEDIUM", medium_margin, -999.0),
            np.where(compound != "HARD", hard_margin, -999.0),
        ]
    )

    if USE_FRESH_SLICK_COMPACT_BLOCK:
        out["FreshSlickOptionCount"] = option_count.astype("int8")
        out["FreshSlickSoftestViable"] = softest_viable.astype("int8")
        out["FreshSlickBestFinishMargin"] = best_finish_margin.astype("float32")
        out["DifferentSlickCanFinish"] = different_can_finish.astype("int8")
        out["NeedsStop_And_DifferentSlickCanFinish"] = (
            (current_cap - tyre_life - remaining_laps < 0) & different_can_finish
        ).astype("int8")
        out["FreshSoftFinishMargin"] = soft_margin.astype("float32")
        out["FreshMediumFinishMargin"] = medium_margin.astype("float32")
        out["FreshHardFinishMargin"] = hard_margin.astype("float32")

    if USE_FRESH_SLICK_FULL_BLOCK:
        out["FreshSlickMinPositiveFinishMargin"] = np.where(
            option_count > 0,
            np.minimum.reduce(
                [
                    np.where(diff_soft_ok, soft_margin, 999.0),
                    np.where(diff_medium_ok, medium_margin, 999.0),
                    np.where(diff_hard_ok, hard_margin, 999.0),
                ]
            ),
            -999.0,
        ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000003.",
    }, sort_keys=True))
