...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["TyreLife", "LapTime_Delta", "Cumulative_Degradation"]:
        ...
        out[f"{col}_lap_rank_pct"] = g.rank(method="average", pct=True).astype(
            "float32"
        )
        out[f"{col}_vs_lap_mean"] = (out[col] - mean).astype("float32")
        out[f"{col}_vs_lap_median"] = (out[col] - med).astype("float32")
        out[f"{col}_lap_z"] = ((out[col] - mean) / std).fillna(0).astype("float32")

    ordered = out.sort_values(key + ["Position"], kind="mergesort")
    grp = ordered.groupby(key, observed=True)

    for offset in [1, 2]:
        ordered[f"ahead{offset}_tyrelife"] = grp["TyreLife"].shift(offset)
        ordered[f"behind{offset}_tyrelife"] = grp["TyreLife"].shift(-offset)
        ordered[f"ahead{offset}_lapdelta"] = grp["LapTime_Delta"].shift(offset)
        ordered[f"behind{offset}_lapdelta"] = grp["LapTime_Delta"].shift(-offset)
        ordered[f"ahead{offset}_degradation"] = grp["Cumulative_Degradation"].shift(
            offset
        )
        ordered[f"behind{offset}_degradation"] = grp["Cumulative_Degradation"].shift(
            -offset
        )
        ordered[f"ahead{offset}_stint"] = grp["Stint"].shift(offset)
        ordered[f"behind{offset}_stint"] = grp["Stint"].shift(-offset)
        ordered[f"ahead{offset}_compound"] = grp["Compound"].shift(offset)
        ordered[f"behind{offset}_compound"] = grp["Compound"].shift(-offset)

    ordered["local_tyre_median_5"] = (
        ordered.groupby(key, observed=True)["TyreLife"]
        .transform(lambda s: s.rolling(5, center=True, min_periods=1).median())
        .astype("float32")
    )

    ordered["local_delta_median_5"] = (
        ordered.groupby(key, observed=True)["LapTime_Delta"]
        .transform(lambda s: s.rolling(5, center=True, min_periods=1).median())
        .astype("float32")
    )
    ...
    restore_cols = [
        ...
        if c.startswith(("ahead", "behind"))
        or c in ["local_tyre_median_5", "local_delta_median_5"]
    ]
    out[restore_cols] = ordered.sort_index()[restore_cols]

    for offset in [1, 2]:
        out[f"tyrelife_gap_ahead{offset}"] = (
            (out["TyreLife"] - out[f"ahead{offset}_tyrelife"])
            .fillna(0)
            .astype("float32")
        )
        out[f"tyrelife_gap_behind{offset}"] = (
            (out["TyreLife"] - out[f"behind{offset}_tyrelife"])
            .fillna(0)
            .astype("float32")
        )
        out[f"lapdelta_gap_ahead{offset}"] = (
            (out["LapTime_Delta"] - out[f"ahead{offset}_lapdelta"])
            .fillna(0)
            .astype("float32")
        )
        out[f"lapdelta_gap_behind{offset}"] = (
            (out["LapTime_Delta"] - out[f"behind{offset}_lapdelta"])
            .fillna(0)
            .astype("float32")
        )
        out[f"degradation_gap_ahead{offset}"] = (
            (out["Cumulative_Degradation"] - out[f"ahead{offset}_degradation"])
            .fillna(0)
            .astype("float32")
        )
        out[f"degradation_gap_behind{offset}"] = (
            (out["Cumulative_Degradation"] - out[f"behind{offset}_degradation"])
            .fillna(0)
            .astype("float32")
        )
        out[f"stint_mismatch_ahead{offset}"] = (
            out["Stint"] != out[f"ahead{offset}_stint"]
        ).astype("int8")
        out[f"stint_mismatch_behind{offset}"] = (
            out["Stint"] != out[f"behind{offset}_stint"]
        ).astype("int8")
        out[f"compound_mismatch_ahead{offset}"] = (
            out["Compound"].astype(str) != out[f"ahead{offset}_compound"].astype(str)
        ).astype("int8")
        out[f"compound_mismatch_behind{offset}"] = (
            out["Compound"].astype(str) != out[f"behind{offset}_compound"].astype(str)
        ).astype("int8")

    out["tyrelife_vs_local5"] = (
        (out["TyreLife"] - out["local_tyre_median_5"]).fillna(0).astype("float32")
    )
    out["lapdelta_vs_local5"] = (
        (out["LapTime_Delta"] - out["local_delta_median_5"]).fillna(0).astype("float32")
    )
    out["old_tyre_local_pressure"] = (
        out["tyrelife_vs_local5"]
        * out["RaceProgress"]
        * (1 + out["Position"].rsub(21) / 20.0)
    ).astype("float32")
    ...
    out = out.drop(columns=drop_neighbors)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000198"],
        "research_usage_note": "Verified assigned hypothesis 000198.",
    }, sort_keys=True))
