...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    progress = out["RaceProgress"].astype(float).clip(1e-6, 1.0)
    estimated_total_laps = (lap / progress).clip(lower=lap)
    laps_after_next_stop = (estimated_total_laps - lap - 1.0).clip(lower=0.0)

    slick_life = {"SOFT": 18.0, "MEDIUM": 28.0, "HARD": 40.0}
    margins = {}
    viable = {}

    for compound, max_life in slick_life.items():
        margin = max_life - laps_after_next_stop
        key = compound.lower()
        margins[key] = margin
        viable[key] = (margin >= 0).astype(np.int8)
        out[f"fresh_{key}_finish_margin"] = margin.astype(np.float32)
        out[f"fresh_{key}_can_finish"] = viable[key]

    viable_stack = np.vstack([viable["soft"], viable["medium"], viable["hard"]]).T
    margin_stack = np.vstack([margins["soft"], margins["medium"], margins["hard"]]).T

    out["fresh_slick_viable_count"] = viable_stack.sum(axis=1).astype(np.int8)

    softest = np.zeros(len(out), dtype=np.int8)
    softest[viable["hard"].to_numpy() == 1] = 1
    softest[viable["medium"].to_numpy() == 1] = 2
    softest[viable["soft"].to_numpy() == 1] = 3
    out["fresh_slick_softest_viable_ord"] = softest

    viable_margins = np.where(viable_stack == 1, margin_stack, np.nan)
    out["fresh_slick_best_viable_margin"] = np.nan_to_num(
        np.nanmax(viable_margins, axis=1), nan=-99.0
    ).astype(np.float32)
    out["fresh_slick_worst_viable_margin"] = np.nan_to_num(
        np.nanmin(viable_margins, axis=1), nan=-99.0
    ).astype(np.float32)

    current = out["Compound"].astype(str).str.upper()
    different_viable = (
        ((current != "SOFT") & (viable["soft"] == 1))
        | ((current != "MEDIUM") & (viable["medium"] == 1))
        | ((current != "HARD") & (viable["hard"] == 1))
    )
    out["fresh_slick_different_can_finish"] = different_viable.astype(np.int8)
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000014"],
        "research_usage_note": "Verified assigned hypothesis 000014.",
    }, sort_keys=True))
