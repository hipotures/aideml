...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lap_mean = out.groupby(group_cols)["LapTime (s)"].transform("mean")
    lap_std = out.groupby(group_cols)["LapTime (s)"].transform("std").replace(0, np.nan)
    out["traffic_laptime_z"] = (
        ((out["LapTime (s)"] - lap_mean) / lap_std).fillna(0).astype("float32")
    )
    out["traffic_laptime_pct_rank"] = (
        out.groupby(group_cols)["LapTime (s)"]
        .rank(pct=True, method="average")
        .astype("float32")
    )

    pos = out["Position"].astype("int16")
    lap_pos_counts = (
        out.groupby(group_cols + ["Position"])["Position"]
        .transform("size")
        .astype("float32")
    )
    out["traffic_same_position_count"] = lap_pos_counts

    count_table = (
        out.groupby(group_cols + ["Position"]).size().rename("n").reset_index()
    )
    keys = out[group_cols].copy()
    keys["_row_id"] = np.arange(len(out))
    keys["Position"] = pos.values

    nearby_total = np.zeros(len(out), dtype=np.float32)
    for offset in (-2, -1, 0, 1, 2):
        probe = keys[["_row_id"] + group_cols].copy()
        probe["Position"] = (keys["Position"] + offset).astype("int16")
        merged = probe.merge(
            count_table, on=group_cols + ["Position"], how="left", sort=False
        )
        nearby_total += (
            merged.sort_values("_row_id")["n"].fillna(0).to_numpy(dtype=np.float32)
        )
    out["traffic_nearby_position_count"] = nearby_total
    out["traffic_position_density"] = (nearby_total / 5.0).astype("float32")

    pos_lap_mean = out.groupby(group_cols + ["Position"])["LapTime (s)"].transform(
        "mean"
    )
    adj_table = (
        out.groupby(group_cols + ["Position"])["LapTime (s)"]
        .mean()
        .rename("adj_lap_mean")
        .reset_index()
    )
    adj_prev = keys[["_row_id"] + group_cols].copy()
    adj_prev["Position"] = (keys["Position"] - 1).astype("int16")
    adj_next = keys[["_row_id"] + group_cols].copy()
    adj_next["Position"] = (keys["Position"] + 1).astype("int16")

    prev_vals = adj_prev.merge(
        adj_table, on=group_cols + ["Position"], how="left", sort=False
    ).sort_values("_row_id")["adj_lap_mean"]
    next_vals = adj_next.merge(
        adj_table, on=group_cols + ["Position"], how="left", sort=False
    ).sort_values("_row_id")["adj_lap_mean"]
    adj_mean = pd.concat(
        [prev_vals.reset_index(drop=True), next_vals.reset_index(drop=True)], axis=1
    ).mean(axis=1)
    adj_mean = adj_mean.fillna(pos_lap_mean).to_numpy(dtype=np.float32)

    out["traffic_vs_adjacent_laptime"] = (
        out["LapTime (s)"].to_numpy(dtype=np.float32) - adj_mean
    ).astype("float32")
    out["traffic_faster_than_adjacent"] = (
        out["traffic_vs_adjacent_laptime"] < 0
    ).astype("int8")
    out["traffic_slower_than_adjacent"] = (
        out["traffic_vs_adjacent_laptime"] > 0
    ).astype("int8")

    out["traffic_front_runner"] = (out["Position"] <= 3).astype("int8")
    out["traffic_free_air_proxy"] = (
        ((out["Position"] <= 3) | (out["traffic_nearby_position_count"] <= 3))
        & (out["traffic_laptime_pct_rank"] <= 0.45)
    ).astype("int8")
    out["traffic_stuck_proxy"] = (
        (out["traffic_nearby_position_count"] >= 5)
        & (out["traffic_slower_than_adjacent"] == 1)
        & (out["Position_Change"] <= 0)
    ).astype("int8")
    out["traffic_extend_stint_proxy"] = (
        (out["traffic_free_air_proxy"] == 1)
        & (
            out["TyreLife"]
            >= out.groupby(["Compound", "Race", "Year"])["TyreLife"].transform("median")
        )
        & (out["RaceProgress"] < 0.85)
    ).astype("int8")

    compound_codes = out["Compound"].astype("category").cat.codes.astype("int16")
    out["traffic_compound_code"] = compound_codes
    out["traffic_density_x_tyre_life"] = (
        out["traffic_position_density"] * out["TyreLife"]
    ).astype("float32")
    out["traffic_density_x_progress"] = (
        out["traffic_position_density"] * out["RaceProgress"]
    ).astype("float32")
    out["traffic_free_air_x_tyre_life"] = (
        out["traffic_free_air_proxy"] * out["TyreLife"]
    ).astype("float32")
    out["traffic_stuck_x_tyre_life"] = (
        out["traffic_stuck_proxy"] * out["TyreLife"]
    ).astype("float32")
    out["traffic_free_air_x_progress"] = (
        out["traffic_free_air_proxy"] * out["RaceProgress"]
    ).astype("float32")
    out["traffic_stuck_x_progress"] = (
        out["traffic_stuck_proxy"] * out["RaceProgress"]
    ).astype("float32")
    out["traffic_compound_x_density"] = (
        compound_codes.to_numpy(dtype=np.float32) * out["traffic_position_density"]
    ).astype("float32")
    out["traffic_compound_x_free_air"] = (
        compound_codes.to_numpy(dtype=np.float32) * out["traffic_free_air_proxy"]
    ).astype("float32")
    out["traffic_compound_x_stuck"] = (
        compound_codes.to_numpy(dtype=np.float32) * out["traffic_stuck_proxy"]
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000646"],
        "research_usage_note": "Verified assigned hypothesis 000646.",
    }, sort_keys=True))
