...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    wet_flag = compound.isin(["WET", "INTERMEDIATE"]).astype("int8")
    dry_flag = (1 - wet_flag).astype("int8")

    rp = X["RaceProgress"].astype(float).clip(lower=1e-6)
    lap = X["LapNumber"].astype(float)
    tyre_life = X["TyreLife"].astype(float).clip(lower=1.0)
    cum_deg = X["Cumulative_Degradation"].astype(float)

    est_total_laps = (lap / rp).replace([np.inf, -np.inf], np.nan)
    est_total_laps = est_total_laps.fillna(est_total_laps.median()).clip(1, 100)

    race_keys = ["Year", "Race"]
    race_deg_median = X.groupby(race_keys, sort=False)[
        "Cumulative_Degradation"
    ].transform("median")
    dry_race_deg = race_deg_median.where(wet_flag == 0)
    global_dry_median = (
        float(dry_race_deg.median())
        if dry_race_deg.notna().any()
        else float(race_deg_median.median())
    )
    high_deg_flag = ((race_deg_median > global_dry_median) & (wet_flag == 0)).astype(
        "int8"
    )
    low_deg_flag = ((race_deg_median <= global_dry_median) & (wet_flag == 0)).astype(
        "int8"
    )

    laps_remaining = (est_total_laps - lap).clip(lower=0)
    finish_window = ((rp >= 0.85) | (laps_remaining <= 5)).astype("int8")

    regime = np.where(
        wet_flag == 1,
        "wet_intermediate",
        np.where(high_deg_flag == 1, "dry_high_deg", "dry_low_deg"),
    )
    regime = np.where(
        finish_window == 1, pd.Series(regime).astype(str) + "_late", regime
    )

    X["WetFlag"] = wet_flag
    X["DryFlag"] = dry_flag
    X["EstTotalLaps"] = est_total_laps
    X["LapsRemaining"] = laps_remaining
    X["RaceDegMedian"] = race_deg_median
    X["DegPerLap"] = cum_deg / tyre_life
    X["DegVsRaceMedian"] = cum_deg - race_deg_median
    X["HighDegFlag"] = high_deg_flag
    X["LowDegFlag"] = low_deg_flag
    X["FinishWindowFlag"] = finish_window
    X["Regime"] = pd.Series(regime, index=X.index).astype("category")
    X["GateKey"] = (
        pd.Series(regime, index=X.index).astype(str)
        + "|"
        + compound.astype(str)
        + "|"
        + X["Race"].astype(str)
    ...
    specialist_base = [c for c in specialist_base if c in X.columns]

    flags = {
        "wet": wet_flag,
        "dry_low": low_deg_flag,
        "dry_high": high_deg_flag,
        "late": finish_window,
    }
    for col in specialist_base:
        s = X[col].astype(float)
        for name, flag in flags.items():
            X[f"{col}__{name}"] = s * flag
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000697.",
    }, sort_keys=True))
