...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X["Compound_Code"] = X["Compound"].map(compound_map).astype("float32")
    X["Is_Wet_Tyre"] = X["Compound"].isin(["WET", "INTERMEDIATE"]).astype("int8")
    X["Is_Slick_Tyre"] = X["Compound"].isin(["HARD", "MEDIUM", "SOFT"]).astype("int8")
    X["Is_Soft"] = (X["Compound"] == "SOFT").astype("int8")

    rp = X["RaceProgress"].clip(0.01, 1.0)
    total_laps_est = (
        (X["LapNumber"] / rp).clip(lower=X["LapNumber"], upper=90).astype("float32")
    )
    laps_left_est = (total_laps_est - X["LapNumber"]).clip(lower=0).astype("float32")

    X["Est_Total_Laps"] = total_laps_est
    X["Est_Laps_Left"] = laps_left_est
    X["TyreLife_Ratio"] = (X["TyreLife"] / total_laps_est).astype("float32")
    X["Lap_Ratio"] = (X["LapNumber"] / total_laps_est).astype("float32")
    X["Stint_Race_Interaction"] = (X["Stint"] * X["RaceProgress"]).astype("float32")

    tyre_life_safe = X["TyreLife"].clip(lower=1.0)
    lap_safe = X["LapNumber"].clip(lower=1.0)
    pos_safe = X["Position"].clip(lower=1.0)

    X["Deg_Per_TyreLap"] = (X["Cumulative_Degradation"] / tyre_life_safe).astype(
        "float32"
    )
    X["Deg_Per_RaceLap"] = (X["Cumulative_Degradation"] / lap_safe).astype("float32")
    X["Delta_Per_TyreLap"] = (X["LapTime_Delta"] / tyre_life_safe).astype("float32")
    X["Pace_x_Deg"] = (X["LapTime_Delta"] * X["Cumulative_Degradation"]).astype(
        "float32"
    )
    X["Tyre_x_Deg"] = (X["TyreLife"] * X["Deg_Per_TyreLap"]).astype("float32")
    X["Position_Pressure"] = ((21 - pos_safe) / 20.0).astype("float32")
    X["Abs_Position_Change"] = X["Position_Change"].abs().astype("float32")

    race_driver = ["Year", "Race", "Driver"]
    X["Lap_In_Driver_Race"] = (
        X.groupby(race_driver, sort=False).cumcount() + 1
    ).astype("int16")
    X["Pit_Cumcount"] = (
        X.groupby(race_driver, sort=False)["PitStop"].cumsum().astype("int16")
    )
    X["Pit_Just_Happened"] = X["PitStop"].astype("int8")

    stint_keys = ["Year", "Race", "Driver", "Stint"]
    X["Lap_In_Stint_From_Stint"] = (
        X.groupby(stint_keys, sort=False).cumcount() + 1
    ).astype("int16")
    X["TyreLife_Minus_StintLap"] = (
        X["TyreLife"] - X["Lap_In_Stint_From_Stint"]
    ).astype("float32")

    for col in ["Driver", "Race", "Compound"]:
        freq = X[col].value_counts(normalize=True, dropna=False)
        X[f"{col}_Freq"] = X[col].map(freq).astype("float32")

    base = (
        0.085 * X["TyreLife"].astype("float32")
        + 0.020 * X["Cumulative_Degradation"].astype("float32")
        + 0.180 * X["Deg_Per_TyreLap"]
        + 0.120 * X["Delta_Per_TyreLap"]
        + 1.250 * X["RaceProgress"].astype("float32")
        + 0.220 * X["Stint"].astype("float32")
        + 0.180 * X["Compound_Code"]
        + 0.100 * X["Abs_Position_Change"]
        + 0.120 * X["Position_Pressure"]
        - 0.060 * laps_left_est
        - 1.750 * X["Pit_Just_Happened"].astype("float32")
        - 0.080 * X["Is_Wet_Tyre"].astype("float32")
    ).astype("float32")
    ...
    p1 = sigmoid(base - 5.2).astype("float32")
    p2 = sigmoid(base - 4.7).astype("float32")
    p3 = sigmoid(base - 4.3).astype("float32")
    p5 = sigmoid(base - 3.8).astype("float32")

    p2 = np.maximum(p2, p1).astype("float32")
    p3 = np.maximum(p3, p2).astype("float32")
    p5 = np.maximum(p5, p3).astype("float32")
    ...
    X["Hazard_1"] = p1
    X["Hazard_2_Only"] = np.clip(p2 - p1, 0.0, 1.0).astype("float32")
    X["Hazard_3_Only"] = np.clip(p3 - p2, 0.0, 1.0).astype("float32")
    X["Hazard_5_Only"] = np.clip(p5 - p3, 0.0, 1.0).astype("float32")
    X["Hazard_Spread_5v1"] = (p5 - p1).astype("float32")
    X["Hazard_Spread_3v1"] = (p3 - p1).astype("float32")
    X["Hazard_x_TyreLife"] = (p1 * X["TyreLife"]).astype("float32")
    X["Hazard_x_Degradation"] = (p1 * X["Cumulative_Degradation"]).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_usage_note": "Verified assigned hypothesis 000746.",
    }, sort_keys=True))
