...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["RaceYear"] = out["Race"].astype("string") + "_" + out["Year"].astype("string")
    out["RaceYear_freq"] = (
        out["RaceYear"]
        .map(out["RaceYear"].value_counts(dropna=False, normalize=True))
        .astype("float32")
    )
    ...
    out["RaceYearDriver_freq"] = seq_key.map(
        seq_key.value_counts(dropna=False, normalize=True)
    ).astype("float32")

    out["lap_frac_in_race"] = out["LapNumber"] / out.groupby(["Race", "Year"])[
        "LapNumber"
    ].transform("max").clip(lower=1)
    out["tyre_age_frac"] = out["TyreLife"] / out["LapNumber"].clip(lower=1)
    out["stint_lap_ratio"] = out["TyreLife"] / out.groupby(
    ...
    for c in ["LapTime (s)", "LapTime_Delta", "Cumulative_Degradation"]:
        ...
        out[c + "_clip"] = out[c].clip(lo, hi).astype("float32")
        out[c + "_abs"] = out[c].abs().astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000168"],
        "research_usage_note": "Verified assigned hypothesis 000168.",
    }, sort_keys=True))
