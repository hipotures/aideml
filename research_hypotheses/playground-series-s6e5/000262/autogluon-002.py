...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    row_id = "_h000262_row_id"
    out[row_id] = np.arange(len(out))
    race_year = out["Race"].astype(str) + "_" + out["Year"].astype(str)
    keys = [race_year, out["LapNumber"]]

    ordered = out.assign(_race_year=race_year).sort_values(
        ["_race_year", "LapNumber", "Position", row_id], kind="mergesort"
    )
    g = ordered.groupby(["_race_year", "LapNumber"], sort=False)
    for offset in (1, 2):
        ordered[f"near_ahead{offset}_tyre_gap"] = ordered["TyreLife"] - g["TyreLife"].shift(offset)
        ordered[f"near_behind{offset}_tyre_gap"] = g["TyreLife"].shift(-offset) - ordered["TyreLife"]
        ordered[f"near_ahead{offset}_delta_gap"] = ordered["LapTime_Delta"] - g["LapTime_Delta"].shift(offset)
        ordered[f"near_behind{offset}_delta_gap"] = g["LapTime_Delta"].shift(-offset) - ordered["LapTime_Delta"]
        ordered[f"near_ahead{offset}_same_compound"] = (ordered["Compound"].astype(str) == g["Compound"].shift(offset).astype(str)).astype("int8")
        ordered[f"near_behind{offset}_same_compound"] = (ordered["Compound"].astype(str) == g["Compound"].shift(-offset).astype(str)).astype("int8")

    fresh_proxy = ((ordered["TyreLife"].astype(float) <= 2) & (ordered["Stint"].astype(float) > 1)).astype("float32")
    ordered["local_fresh_proxy_pm2"] = (
        fresh_proxy.groupby([ordered["_race_year"], ordered["LapNumber"]], sort=False)
        .rolling(5, center=True, min_periods=1)
        .mean()
        .reset_index(level=[0, 1], drop=True)
        .astype("float32")
    )
    ordered["local_clean_air_proxy"] = (
        ordered["near_ahead1_delta_gap"].fillna(0).clip(lower=0)
        + ordered["near_behind1_delta_gap"].fillna(0).clip(lower=0)
    ).astype("float32")

    add_cols = [c for c in ordered.columns if c.startswith("near_") or c.startswith("local_")]
    restored = ordered.sort_values(row_id, kind="mergesort")
    out[add_cols] = restored[add_cols].to_numpy()
    out["traffic_window_pressure_000262"] = (
        out["local_fresh_proxy_pm2"].fillna(0)
        * (1.0 + out["local_clean_air_proxy"].fillna(0))
        * (1.0 + out["TyreLife"].astype(float) / 25.0)
    ).astype("float32")
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000262"],
        "research_usage_note": "Verified assigned hypothesis 000262.",
    }, sort_keys=True))
