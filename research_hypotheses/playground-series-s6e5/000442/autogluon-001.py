...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in metrics:
        ...
        grp = x.groupby(lap_key, sort=False)
        mean = grp.transform("mean")
        std = grp.transform("std").replace(0, np.nan)
        med = grp.transform("median")

        safe = col.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        out[f"{safe}_race_lap_rank"] = grp.rank(pct=True, method="average").astype(
            "float32"
        )
        out[f"{safe}_race_lap_z"] = ((x - mean) / std).fillna(0).astype("float32")
        out[f"{safe}_race_lap_med_delta"] = (x - med).astype("float32")

    pos = pd.to_numeric(out["Position"], errors="coerce")
    pos_bucket = (pos // 2).astype("Int64").astype("string")
    compound = out["Compound"].astype("string").fillna("NA")
    stint = (
        pd.to_numeric(out["Stint"], errors="coerce").astype("Int64").astype("string")
    )

    out["race_lap_field_size"] = (
        out.groupby(lap_key, sort=False)["Position"].transform("size").astype("int16")
    )
    out["race_lap_near_position_count"] = (
        out.groupby([race_key, out["LapNumber"], pos_bucket], sort=False)["Position"]
        .transform("size")
        .astype("int16")
    )
    out["race_lap_same_compound_count"] = (
        out.groupby([race_key, out["LapNumber"], compound], sort=False)["Position"]
        .transform("size")
        .astype("int16")
    )
    out["race_lap_same_stint_count"] = (
        out.groupby([race_key, out["LapNumber"], stint], sort=False)["Position"]
        .transform("size")
        .astype("int16")
    )

    out["near_position_share"] = (
        out["race_lap_near_position_count"] / out["race_lap_field_size"]
    ).astype("float32")
    out["same_compound_share"] = (
        out["race_lap_same_compound_count"] / out["race_lap_field_size"]
    ).astype("float32")
    out["same_stint_share"] = (
        out["race_lap_same_stint_count"] / out["race_lap_field_size"]
    ).astype("float32")

    out["position_from_front_frac"] = (
        ((pos - 1) / (out["race_lap_field_size"] - 1).replace(0, np.nan))
        .fillna(0)
        .astype("float32")
    )
    out["race_progress_bucket"] = (
        np.floor(pd.to_numeric(out["RaceProgress"], errors="coerce") * 20)
        .clip(0, 20)
        .astype("int8")
    )
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000442"],
        "research_usage_note": "Verified assigned hypothesis 000442.",
    }, sort_keys=True))
