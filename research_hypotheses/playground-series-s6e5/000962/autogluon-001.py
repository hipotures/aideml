...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out["_orig_order_"] = np.arange(len(out))

    cat_cols = ["Compound", "Driver", "Race"]
    for c in cat_cols:
        if c in out.columns:
            out[c] = out[c].astype(str).str.upper().fillna("UNKNOWN")

    group_cols = ["Year", "Race", "LapNumber"]
    sort_cols = group_cols + ["Position", "Driver"]
    out = out.sort_values(sort_cols, kind="mergesort")

    g = out.groupby(group_cols, sort=False)

    out["field_size"] = g["Position"].transform("size").astype("float32")
    out["pos_norm"] = (out["Position"] / out["field_size"]).astype("float32")

    out["tyrelife_pct_in_lap"] = g["TyreLife"].rank(pct=True).astype("float32")
    out["degradation_pct_in_lap"] = (
        g["Cumulative_Degradation"].rank(pct=True).astype("float32")
    )
    out["lapdelta_pct_in_lap"] = g["LapTime_Delta"].rank(pct=True).astype("float32")
    out["relative_wear_pct"] = (
        0.6 * out["tyrelife_pct_in_lap"] + 0.4 * out["degradation_pct_in_lap"]
    ).astype("float32")

    prev_map = {
        "Position": "ahead_position",
        "Compound": "ahead_compound",
        "TyreLife": "ahead_tyrelife",
        "Cumulative_Degradation": "ahead_degradation",
        "LapTime_Delta": "ahead_lapdelta",
        "PitStop": "ahead_same_lap_pit",
        "relative_wear_pct": "ahead_wear_pct",
    }
    next_map = {
        "Position": "behind_position",
        "Compound": "behind_compound",
        "TyreLife": "behind_tyrelife",
        "Cumulative_Degradation": "behind_degradation",
        "LapTime_Delta": "behind_lapdelta",
        "PitStop": "behind_same_lap_pit",
        "relative_wear_pct": "behind_wear_pct",
    }

    for src, dst in prev_map.items():
        out[dst] = g[src].shift(1)
    for src, dst in next_map.items():
        out[dst] = g[src].shift(-1)

    for side in ["ahead", "behind"]:
        out[f"{side}_gap_pos"] = (
            (out[f"{side}_position"] - out["Position"]).fillna(0).astype("float32")
        )
        out[f"{side}_tyrelife_diff"] = (
            (out[f"{side}_tyrelife"] - out["TyreLife"]).fillna(0).astype("float32")
        )
        out[f"{side}_degradation_diff"] = (
            (out[f"{side}_degradation"] - out["Cumulative_Degradation"])
            .fillna(0)
            .astype("float32")
        )
        out[f"{side}_lapdelta_diff"] = (
            (out[f"{side}_lapdelta"] - out["LapTime_Delta"]).fillna(0).astype("float32")
        )
        out[f"{side}_wear_pct_diff"] = (
            (out[f"{side}_wear_pct"] - out["relative_wear_pct"])
            .fillna(0)
            .astype("float32")
        )
        ratio = (out["TyreLife"] + 1.0) / (out[f"{side}_tyrelife"] + 1.0)
        out[f"{side}_tyrelife_ratio"] = (
            ratio.replace([np.inf, -np.inf], 0).fillna(0).astype("float32")
        )
        out[f"{side}_same_compound"] = (
            (out["Compound"] == out[f"{side}_compound"])
            & out[f"{side}_compound"].notna()
        ).astype("int8")
        out[f"{side}_same_lap_pit"] = (
            out[f"{side}_same_lap_pit"].fillna(0).astype("int8")
        )

    out["adjacent_same_compound_count"] = (
        out["ahead_same_compound"] + out["behind_same_compound"]
    ).astype("int8")

    out["field_lapdelta_median"] = (
        g["LapTime_Delta"].transform("median").astype("float32")
    )
    out["field_lapdelta_std"] = (
        g["LapTime_Delta"].transform("std").fillna(0).astype("float32")
    )
    out["lap_chaos_flag"] = (
        (out["LapNumber"] <= 3)
        | (out["field_lapdelta_std"] > out["field_lapdelta_std"].median())
    ).astype("int8")

    out["abnormal_delta_spike"] = (
        (out["LapTime_Delta"] - out["field_lapdelta_median"]).abs()
        > (2.5 * out["field_lapdelta_std"] + 3.0)
    ).astype("int8")

    out["undercut_threat_from_behind"] = (
        (out["behind_tyrelife_diff"] < 0)
        & (out["behind_degradation_diff"] < 0)
        & (out["behind_lapdelta_diff"] < 0)
        & (out["behind_gap_pos"] == 1)
    ).astype("int8")

    out["cover_car_ahead"] = (
        (
            (out["ahead_same_lap_pit"] == 1)
            | (
                (out["ahead_gap_pos"] == -1)
                & (out["ahead_tyrelife_diff"] > 0)
                & (out["ahead_degradation_diff"] > 0)
            )
        )
        & (out["Position"] > 1)
    ).astype("int8")

    out["same_compound_trap"] = (
        (out["adjacent_same_compound_count"] >= 1)
        & (out["relative_wear_pct"] > 0.6)
        & (out["Position"].between(2, 19))
    ).astype("int8")

    out["traffic_pocket_risk"] = (
        (out["ahead_gap_pos"] == -1)
        & (out["behind_gap_pos"] == 1)
        & (out["ahead_same_compound"] == 1)
        & (out["behind_same_compound"] == 1)
        & (out["ahead_lapdelta_diff"].abs() < 1.0)
        & (out["behind_lapdelta_diff"].abs() < 1.0)
    ).astype("int8")

    out["neighbor_pit_pressure"] = (
        out["ahead_same_lap_pit"] + out["behind_same_lap_pit"]
    ).astype("int8")

    out["adjacency_pressure_score"] = (
        1.5 * out["undercut_threat_from_behind"]
        + 1.2 * out["cover_car_ahead"]
        + 0.8 * out["same_compound_trap"]
        + 0.8 * out["traffic_pocket_risk"]
        + 0.5 * out["neighbor_pit_pressure"]
    ).astype("float32")

    calm_mask = (
        (out["lap_chaos_flag"] == 0) & (out["abnormal_delta_spike"] == 0)
    ).astype("float32")
    for c in [
        "undercut_threat_from_behind",
        "cover_car_ahead",
        "same_compound_trap",
        "traffic_pocket_risk",
        "neighbor_pit_pressure",
        "adjacency_pressure_score",
        "ahead_lapdelta_diff",
        "behind_lapdelta_diff",
        "ahead_tyrelife_diff",
        "behind_tyrelife_diff",
        "ahead_degradation_diff",
        "behind_degradation_diff",
        "ahead_wear_pct_diff",
        "behind_wear_pct_diff",
    ]:
        out[c + "_calm"] = (out[c].astype("float32") * calm_mask).astype("float32")

    out = out.sort_values("_orig_order_", kind="mergesort").drop(
        columns=["_orig_order_"]
    )
    return out
...
def main() -> None:
    ...
    print(RESULT_MARKER + " " + json.dumps({
        ...
        "research_hypotheses_llm_claimed_used": ["000962"],
        "research_usage_note": "Verified assigned hypothesis 000962.",
    }, sort_keys=True))
