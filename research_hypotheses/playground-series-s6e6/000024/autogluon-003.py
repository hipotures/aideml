...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Redshift handling + raw helper columns
    z_raw = out["redshift"].astype(float).to_numpy()
    z = np.clip(z_raw, -0.5, 6.0)
    out["redshift_clipped"] = z
    out["inv_1plus_z"] = 1.0 / (1.0 + z)
    out["log1p_redshift_pos"] = np.log1p(np.maximum(z, 0.0))
    out["redshift_abs"] = np.abs(z)

    # Categorical encodings (fixed columns)
    spectral_cats = ["M", "O/B", "G/K", "A/F"]
    galaxy_cats = ["Blue_Cloud", "Red_Sequence"]
    for cat in spectral_cats:
        out[f"spectral_type_{cat.replace('/', '_')}"] = (
            out["spectral_type"] == cat
        ).astype("uint8")
    for cat in galaxy_cats:
        out[f"galaxy_population_{cat.replace(' ', '_')}"] = (
            out["galaxy_population"] == cat
        ).astype("uint8")
    out = out.drop(columns=["spectral_type", "galaxy_population"])

    # Magnitude preprocessing
    bands = ["u", "g", "r", "i", "z"]
    mags = out[bands].to_numpy(dtype=float)
    mag_med = np.median(mags, axis=1, keepdims=True)
    mag_rel = mags - mag_med
    for j, b in enumerate(bands):
        out[f"{b}_rel"] = mag_rel[:, j]

    # SDSS effective wavelengths (A)
    wave_obs = np.array([3543.0, 4770.0, 6231.0, 7625.0, 8931.0], dtype=float)

    inv1pz = out["inv_1plus_z"].to_numpy(dtype=float)
    wave_rest = wave_obs[None, :] * inv1pz[:, None]
    y_rest = mag_rel

    # Interpolate centered magnitudes onto compact common rest-frame grid
    rest_grid = np.array(
        [1300.0, 1800.0, 2400.0, 3000.0, 4000.0, 5000.0, 6200.0, 7600.0, 9000.0],
        dtype=float,
    )
    raw_idx = np.vstack(
        [np.searchsorted(row_wave, rest_grid, side="right") - 1 for row_wave in wave_rest]
    )
    is_left = raw_idx < 0
    is_right = raw_idx >= (wave_rest.shape[1] - 1)
    left = np.clip(raw_idx, 0, wave_rest.shape[1] - 2)
    right = left + 1

    row_idx = np.arange(len(out))[:, None]
    xL = wave_rest[row_idx, left]
    xR = wave_rest[row_idx, right]
    yL = y_rest[row_idx, left]
    yR = y_rest[row_idx, right]
    denom = xR - xL
    frac = np.zeros_like(xL, dtype=float)
    np.divide(rest_grid[None, :] - xL, denom, out=frac, where=denom != 0.0)

    interp = yL + frac * (yR - yL)
    interp = np.where(is_left, y_rest[:, [0]], interp)
    interp = np.where(is_right, y_rest[:, [-1]], interp)

    for j, wl in enumerate(rest_grid.astype(int)):
        out[f"rest_mag_interp_{wl}"] = interp[:, j]

    # Coverage and extrapolation diagnostics
    covered = (rest_grid[None, :] >= wave_rest[:, [0]]) & (
        rest_grid[None, :] <= wave_rest[:, [-1]]
    )
    covered_count = covered.sum(axis=1).astype("uint8")
    out["rest_grid_covered_count"] = covered_count
    out["rest_grid_covered_frac"] = covered_count.astype(float) / rest_grid.size
    out["rest_grid_below_count"] = (
        (~covered & (rest_grid[None, :] < wave_rest[:, [0]]))
        .sum(axis=1)
        .astype("uint8")
    )
    out["rest_grid_above_count"] = (
        (~covered & (rest_grid[None, :] > wave_rest[:, [-1]]))
        .sum(axis=1)
        .astype("uint8")
    )

    for j, wl in enumerate(rest_grid.astype(int)):
        out[f"rest_grid_covered_{wl}"] = covered[:, j].astype("uint8")

    # Slope / curvature features in rest-frame interpolated SED
    dlam = np.diff(rest_grid)
    slope = np.divide(np.diff(interp, axis=1), dlam)
    for j in range(slope.shape[1]):
        out[f"rest_slope_{int(rest_grid[j])}_to_{int(rest_grid[j + 1])}"] = slope[:, j]

    dlam_mid = (dlam[:-1] + dlam[1:]) / 2.0
    curvature = np.divide(np.diff(slope, axis=1), dlam_mid)
    for j in range(curvature.shape[1]):
        out[f"rest_curv_{int(rest_grid[j + 1])}"] = curvature[:, j]

    # 4000A-style blue-red contrast proxies and profile shape summaries
    idx_4000 = np.where(rest_grid == 4000.0)[0][0]
    out["rest_4000_proxy_blue_red"] = interp[:, :idx_4000].mean(axis=1) - interp[
        :, idx_4000 + 1 :
    ].mean(axis=1)
    out["rest_4000_minus_5000"] = interp[:, idx_4000] - interp[:, idx_4000 + 1]
    out["rest_3200_minus_6200"] = interp[:, 3] - interp[:, 6]
    out["rest_profile_range_1_9000"] = interp[:, -1] - interp[:, 0]
    out["rest_profile_min"] = interp.min(axis=1)
    out["rest_profile_max"] = interp.max(axis=1)
    out["rest_profile_std"] = interp.std(axis=1)

    # Redshift-band crossing indicators around useful spectral anchor points
    pivots = np.array([2500.0, 3000.0, 4000.0, 5000.0, 7000.0], dtype=float)
    pair_names = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    for pv in pivots:
        below = (wave_rest < pv).sum(axis=1).astype(np.int16)
        out[f"rest_n_bands_below_{int(pv)}"] = below
        out[f"rest_pivot_in_span_{int(pv)}"] = ((below > 0) & (below < 5)).astype(
            "uint8"
        )
        for i, (a, b) in enumerate(pair_names):
            crossed = (wave_rest[:, i] < pv) & (wave_rest[:, i + 1] >= pv)
            out[f"rest_cross_{int(pv)}_{a}_{b}"] = crossed.astype("uint8")

    # Reuse raw numeric context without row reordering or row count changes
    return out
