...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    bands = ["u", "g", "r", "i", "z"]
    centers = np.array([3543., 4770., 6231., 7625., 9134.], dtype="float32")
    if all(b in out.columns for b in bands):
        z = np.maximum(_num(out, "redshift"), -0.95) if "redshift" in out.columns else pd.Series(0, index=out.index)
        rest = centers[None, :] / (1.0 + z.to_numpy()[:, None])
        mags = out[bands].to_numpy(dtype="float32")
        grid = [1500, 2200, 3000, 4000, 5200, 7000]
        for wave in grid:
            vals = [np.interp(wave, rest[i], mags[i], left=mags[i, 0], right=mags[i, -1]) for i in range(len(out))]
            out[f"rest_mag_{wave}"] = np.asarray(vals, dtype="float32")
        out["rest_wave_span"] = (rest.max(axis=1) - rest.min(axis=1)).astype("float32")
        out["rest_4000a_flux_break_proxy"] = (out["rest_mag_3000"] - out["rest_mag_5200"]).astype("float32")
    return out
