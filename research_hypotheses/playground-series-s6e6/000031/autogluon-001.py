...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(c in out.columns for c in ["u_g", "g_r"]):
        rules["uv_excess_qso"] = out["u_g"] - 0.6
        rules["stellar_blue_edge"] = 0.35 - out["g_r"]
    if all(c in out.columns for c in ["g_r", "r_i"]):
        rules["red_sequence"] = out["g_r"] - 1.4 * out["r_i"] - 0.1
    for name, margin in rules.items():
        out[f"{name}_margin"] = margin.astype("float32")
        out[f"{name}_flag"] = (margin > 0).astype("int8")
    flag_cols = [c for c in out.columns if c.endswith("_flag")]
    if flag_cols:
        out["rule_flag_count"] = out[flag_cols].sum(axis=1).astype("int8")
    return out
