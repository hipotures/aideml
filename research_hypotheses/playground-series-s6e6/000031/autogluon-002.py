...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    if cat_cols:
        cat_features = pd.get_dummies(X[cat_cols], prefix=cat_cols, dtype=np.uint8)
        X = pd.concat([X.drop(columns=cat_cols), cat_features], axis=1)
    ...
    ug = (u - g).astype("float32")
    gr = (g - r).astype("float32")
    ri = (r - i).astype("float32")
    iz = (i - z).astype("float32")
    ur = (u - r).astype("float32")
    gi = (g - i).astype("float32")
    ui = (u - i).astype("float32")
    rz = (r - z).astype("float32")
    ...
    # UVX-like quasar candidates (fixed photometric regimes)
    uvx1_flag = ug < 0.55
    uvx2_flag = (ug < 0.80) & (gr < 0.40)
    uvx3_flag = ur < 1.25
    uvx4_flag = (ug < 0.90) & (gr > -0.10) & (gr < 0.55)

    X["sdss_uvx_u_g_lt_055_flag"] = uvx1_flag.astype("uint8")
    X["sdss_uvx_u_g_lt_055_margin"] = (0.55 - ug).astype("float32")
    X["sdss_uvx_u_g_lt_080_g_r_lt_040_flag"] = uvx2_flag.astype("uint8")
    X["sdss_uvx_u_g_lt_080_g_r_lt_040_margin"] = np.minimum(
        0.80 - ug, 0.40 - gr
    ).astype("float32")
    X["sdss_uvx_u_r_lt_125_flag"] = uvx3_flag.astype("uint8")
    X["sdss_uvx_u_r_lt_125_margin"] = (1.25 - ur).astype("float32")
    X["sdss_uvx_u_g_lt_090_g_r_mid_flag"] = uvx4_flag.astype("uint8")
    X["sdss_uvx_u_g_lt_090_g_r_mid_margin"] = np.minimum.reduce(
        [0.90 - ug, gr + 0.10, 0.55 - gr]
    ).astype("float32")

    # High-redshift/dropout-like redder continua regimes
    hz1_flag = ug > 1.20
    hz2_flag = gr > 0.95
    hz3_flag = ri > 0.30
    hz4_flag = iz > 0.25
    hz5_flag = (ug > 1.20) & (gr > 0.55) & (ri > 0.20)

    X["sdss_hiz_u_g_gt_120_flag"] = hz1_flag.astype("uint8")
    X["sdss_hiz_u_g_gt_120_margin"] = (ug - 1.20).astype("float32")
    X["sdss_hiz_g_r_gt_095_flag"] = hz2_flag.astype("uint8")
    X["sdss_hiz_g_r_gt_095_margin"] = (gr - 0.95).astype("float32")
    X["sdss_hiz_r_i_gt_030_flag"] = hz3_flag.astype("uint8")
    X["sdss_hiz_r_i_gt_030_margin"] = (ri - 0.30).astype("float32")
    X["sdss_hiz_i_z_gt_025_flag"] = hz4_flag.astype("uint8")
    X["sdss_hiz_i_z_gt_025_margin"] = (iz - 0.25).astype("float32")
    X["sdss_hiz_chain_flag"] = hz5_flag.astype("uint8")
    X["sdss_hiz_chain_margin"] = np.minimum.reduce(
        [ug - 1.20, gr - 0.55, ri - 0.20]
    ).astype("float32")
    ...
    X["sdss_red_g_i_gt_160_flag"] = red1_flag.astype("uint8")
    X["sdss_red_g_i_gt_160_margin"] = (gi - 1.60).astype("float32")
    X["sdss_red_u_r_gt_250_flag"] = red2_flag.astype("uint8")
    X["sdss_red_u_r_gt_250_margin"] = (ur - 2.50).astype("float32")
    X["sdss_red_u_i_gt_240_flag"] = red3_flag.astype("uint8")
    X["sdss_red_u_i_gt_240_margin"] = (ui - 2.40).astype("float32")

    # Green-Pea-like emission-line color regime inequalities using u-r, r-i, r-z, g-r
    gp1_flag = (ur < 2.20) & (gr > 0.45) & (ri < 0.35)
    gp2_flag = (ur < 1.90) & (gr > 0.20) & (iz > 0.10) & (ri < 0.40)
    gp3_flag = (ur < 2.10) & (gr < 1.20) & (ri < 0.55) & (rz < 0.65)

    X["sdss_gp_u_r_lt_220_g_r_gt_045_r_i_lt_035_flag"] = gp1_flag.astype("uint8")
    X["sdss_gp_u_r_lt_220_g_r_gt_045_r_i_lt_035_margin"] = np.minimum.reduce(
        [2.20 - ur, gr - 0.45, 0.35 - ri]
    ).astype("float32")
    X["sdss_gp_u_r_lt_190_g_r_gt_020_i_z_gt_010_r_i_lt_040_flag"] = gp2_flag.astype(
        "uint8"
    )
    X["sdss_gp_u_r_lt_190_g_r_gt_020_i_z_gt_010_r_i_lt_040_margin"] = np.minimum.reduce(
        [1.90 - ur, gr - 0.20, iz - 0.10, 0.40 - ri]
    ).astype("float32")
    X["sdss_gp_u_r_lt_210_g_r_lt_120_r_i_lt_055_r_z_lt_065_flag"] = gp3_flag.astype(
        "uint8"
    )
    X["sdss_gp_u_r_lt_210_g_r_lt_120_r_i_lt_055_r_z_lt_065_margin"] = np.minimum.reduce(
        [2.10 - ur, 1.20 - gr, 0.55 - ri, 0.65 - rz]
    ).astype("float32")

    uvx_flag_cols = [
        "sdss_uvx_u_g_lt_055_flag",
        "sdss_uvx_u_g_lt_080_g_r_lt_040_flag",
        "sdss_uvx_u_r_lt_125_flag",
        "sdss_uvx_u_g_lt_090_g_r_mid_flag",
    ]
    uvx_margin_cols = [
        "sdss_uvx_u_g_lt_055_margin",
        "sdss_uvx_u_g_lt_080_g_r_lt_040_margin",
        "sdss_uvx_u_r_lt_125_margin",
        "sdss_uvx_u_g_lt_090_g_r_mid_margin",
    ]
    hz_flag_cols = [
        "sdss_hiz_u_g_gt_120_flag",
        "sdss_hiz_g_r_gt_095_flag",
        "sdss_hiz_r_i_gt_030_flag",
        "sdss_hiz_i_z_gt_025_flag",
        "sdss_hiz_chain_flag",
    ]
    hz_margin_cols = [
        "sdss_hiz_u_g_gt_120_margin",
        "sdss_hiz_g_r_gt_095_margin",
        "sdss_hiz_r_i_gt_030_margin",
        "sdss_hiz_i_z_gt_025_margin",
        "sdss_hiz_chain_margin",
    ]
    gp_flag_cols = [
        "sdss_gp_u_r_lt_220_g_r_gt_045_r_i_lt_035_flag",
        "sdss_gp_u_r_lt_190_g_r_gt_020_i_z_gt_010_r_i_lt_040_flag",
        "sdss_gp_u_r_lt_210_g_r_lt_120_r_i_lt_055_r_z_lt_065_flag",
    ]
    gp_margin_cols = [
        "sdss_gp_u_r_lt_220_g_r_gt_045_r_i_lt_035_margin",
        "sdss_gp_u_r_lt_190_g_r_gt_020_i_z_gt_010_r_i_lt_040_margin",
        "sdss_gp_u_r_lt_210_g_r_lt_120_r_i_lt_055_r_z_lt_065_margin",
    ]
    red_flag_cols = [
        "sdss_red_g_i_gt_160_flag",
        "sdss_red_u_r_gt_250_flag",
        "sdss_red_u_i_gt_240_flag",
    ]
    red_margin_cols = [
        "sdss_red_g_i_gt_160_margin",
        "sdss_red_u_r_gt_250_margin",
        "sdss_red_u_i_gt_240_margin",
    ]

    X["sdss_uvx_rule_count"] = X[uvx_flag_cols].sum(axis=1).astype("uint8")
    X["sdss_hiz_rule_count"] = X[hz_flag_cols].sum(axis=1).astype("uint8")
    X["sdss_greenpea_rule_count"] = X[gp_flag_cols].sum(axis=1).astype("uint8")
    X["sdss_redness_rule_count"] = X[red_flag_cols].sum(axis=1).astype("uint8")
    X["sdss_rule_count_total"] = (
        X["sdss_uvx_rule_count"]
        + X["sdss_hiz_rule_count"]
        + X["sdss_greenpea_rule_count"]
        + X["sdss_redness_rule_count"]
    ).astype("uint8")

    X["sdss_uvx_rule_max_margin"] = X[uvx_margin_cols].max(axis=1).astype("float32")
    X["sdss_uvx_rule_min_margin"] = X[uvx_margin_cols].min(axis=1).astype("float32")
    X["sdss_hiz_rule_max_margin"] = X[hz_margin_cols].max(axis=1).astype("float32")
    X["sdss_hiz_rule_min_margin"] = X[hz_margin_cols].min(axis=1).astype("float32")
    X["sdss_greenpea_rule_max_margin"] = X[gp_margin_cols].max(axis=1).astype("float32")
    X["sdss_greenpea_rule_min_margin"] = X[gp_margin_cols].min(axis=1).astype("float32")
    X["sdss_redness_rule_max_margin"] = X[red_margin_cols].max(axis=1).astype("float32")
    X["sdss_redness_rule_min_margin"] = X[red_margin_cols].min(axis=1).astype("float32")

    z_bands = {
        "z_000_020": redshift <= 0.20,
        "z_020_050": (redshift > 0.20) & (redshift <= 0.50),
        "z_050_100": (redshift > 0.50) & (redshift <= 1.00),
        "z_100_200": (redshift > 1.00) & (redshift <= 2.00),
        "z_200_350": (redshift > 2.00) & (redshift <= 3.50),
        "z_350_plus": redshift > 3.50,
    }

    family_counts = [
        "sdss_uvx_rule_count",
        "sdss_hiz_rule_count",
        "sdss_greenpea_rule_count",
        "sdss_redness_rule_count",
    ]
    for band_name, band_mask in z_bands.items():
        band = band_mask.astype("uint8")
        for family in family_counts:
            X[f"{family}_{band_name}"] = X[family] * band

    indicator_cols = [
        c
        for c in X.columns
        if c.startswith("spectral_type_") or c.startswith("galaxy_population_")
    ]
    for family in family_counts:
        for col in indicator_cols:
            X[f"{family}_x_{col}"] = X[family] * X[col].astype("uint8")
