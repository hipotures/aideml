...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    id_series = pd.to_numeric(out["id"], errors="coerce")
    id_filled = id_series.fillna(0.0)
    id_int = id_filled.astype("int64")

    out["id_is_missing"] = id_series.isna().astype("uint8")
    out["id_rank"] = id_int.rank(method="first").astype("int32")
    n_rows = len(out)
    out["id_rank_norm"] = out["id_rank"] / float(max(n_rows - 1, 1))
    out["id_block_1k"] = (id_int // 1000).astype("int64")
    out["id_block_100k"] = (id_int // 100000).astype("int64")
    out["id_mod_2"] = (id_int % 2).astype("int8")
    out["id_mod_11"] = (id_int % 11).astype("int16")
    out["id_mod_97"] = (id_int % 97).astype("int16")
    out["id_even"] = (id_int % 2 == 0).astype("uint8")
    out["id_log1p"] = np.log1p(np.maximum(id_int, 0)).astype("float64")

    sorted_idx = id_int.sort_values(kind="mergesort").index
    sorted_vals = id_int.loc[sorted_idx].to_numpy()
    gap_prev = np.zeros(n_rows, dtype="float64")
    gap_next = np.zeros(n_rows, dtype="float64")
    if n_rows > 1:
        gap_prev[1:] = np.diff(sorted_vals)
        gap_next[:-1] = np.diff(sorted_vals)
    out.loc[sorted_idx, "id_gap_prev"] = gap_prev
    out.loc[sorted_idx, "id_gap_next"] = gap_next
