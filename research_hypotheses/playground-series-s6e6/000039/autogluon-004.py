...
def preprocess(df: "pd.DataFrame", aux: "pd.DataFrame") -> "pd.DataFrame":
    ...
    for cname, (left, right) in color_defs.items():
        out[cname] = out[left].astype(np.float32) - out[right].astype(np.float32)
    ...
    if isinstance(aux, pd.DataFrame) and len(aux) > 0:
        for col in base_numeric:
            if col in aux.columns:
                s = pd.to_numeric(aux[col], errors="coerce").astype(np.float64)
                if col in {"u", "g", "z"}:
                    s = s.mask(s <= -9000.0)
                aux_fit[col] = s.to_numpy()

        for cname, (left, right) in color_defs.items():
            if left in aux_fit and right in aux_fit:
                aux_fit[cname] = aux_fit[left] - aux_fit[right]

    def _fit_spline_block(
        feature: str, n_knots: int, periodic: bool = False
    ) -> pd.DataFrame:
        train_vals = pd.to_numeric(out[feature], errors="coerce").to_numpy(
            dtype=np.float64
        )
        finite_train = np.isfinite(train_vals)
        if finite_train.any():
            train_fill = float(np.nanmedian(train_vals[finite_train]))
        else:
            train_fill = 0.0
        train_vals = np.where(finite_train, train_vals, train_fill)

        fit_vals = train_vals
        if feature in aux_fit and aux_fit[feature].size > 0:
            aux_vals = np.asarray(aux_fit[feature], dtype=np.float64)
            finite_aux = np.isfinite(aux_vals)
            if finite_aux.any():
                aux_fill = float(np.nanmedian(aux_vals[finite_aux]))
            else:
                aux_fill = train_fill
            aux_vals = np.where(finite_aux, aux_vals, aux_fill)
            fit_vals = np.concatenate([fit_vals, aux_vals], axis=0)
        ...
        if not finite_fit.any():
            return pd.DataFrame(
                {f"{feature}_spline_0": np.zeros(len(out), dtype=np.float32)},
                index=out.index,
            )

        fill_fit = float(np.nanmedian(fit_vals[finite_fit]))
        fit_vals = np.where(finite_fit, fit_vals, fill_fit)
        train_vals = np.where(np.isfinite(train_vals), train_vals, fill_fit)
        ...
        knot_target = int(max(2, min(n_knots, max(2, n_unique - 1))))
        attempts = [
            {
                "extrapolation": "periodic" if periodic else "constant",
                "n_knots": knot_target,
            },
            {"extrapolation": "constant", "n_knots": max(2, min(3, n_unique - 1))},
        ]
        ...
        for params in attempts:
            try:
                ...
                tr = trans.transform(train_vals.reshape(-1, 1))
                if hasattr(tr, "toarray"):
                    tr = tr.toarray()
                tr = np.asarray(tr, dtype=np.float32)

                keep = np.nanstd(tr, axis=0) > 1e-12
                tr = tr[:, keep]
                if tr.shape[1] == 0:
                    continue
                cols = [f"{feature}_spline_{i}" for i, k in enumerate(keep) if bool(k)]
                return pd.DataFrame(tr, index=out.index, columns=cols, dtype=np.float32)
            except Exception:
                continue

        return pd.DataFrame(
            {f"{feature}_spline_0": train_vals.astype(np.float32)},
            index=out.index,
        )
    ...
    spline_blocks = {}
    for feature, kn, is_periodic in spline_specs:
        block = _fit_spline_block(feature, kn, periodic=is_periodic)
        if block.shape[1] > 0:
            spline_blocks[feature] = block
    ...
    for left, right, prefix in tensor_pairs:
        if left in spline_blocks and right in spline_blocks:
            a = spline_blocks[left].to_numpy(dtype=np.float32)
            b = spline_blocks[right].to_numpy(dtype=np.float32)
            k = min(rank, a.shape[1], b.shape[1])
            if k <= 0:
                continue
            aa = a[:, :k]
            bb = b[:, :k]
    ...
    if cat_cols:
        ...
        out = pd.concat([out.drop(columns=cat_cols), cat_encoded], axis=1)
