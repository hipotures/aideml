...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    color_expr = {
        "color_u_g": out["u"] - out["g"],
        "color_g_r": out["g"] - out["r"],
        "color_r_i": out["r"] - out["i"],
        "color_i_z": out["i"] - out["z"],
        "color_u_r": out["u"] - out["r"],
        "color_g_i": out["g"] - out["i"],
        "color_r_z": out["r"] - out["z"],
        "color_u_z": out["u"] - out["z"],
    }
    for k, v in color_expr.items():
        out[k] = v.astype(np.float32)

    spline_features = list(numeric_cols) + list(color_expr.keys())
    knot_counts = {
        "alpha": 7,
        "delta": 5,
        "u": 5,
    ...
    def _sanitize_series(s: pd.Series) -> pd.Series:
        ...
        if "u" in str(s.name) or "g" in str(s.name) or str(s.name) == "z":
            ser = ser.mask(ser < -9000)
    ...
    if isinstance(aux, pd.DataFrame):
        for c in numeric_cols:
            if c in aux.columns:
                aux_fit[c] = _sanitize_series(aux[c])
        ...
        for name, expr in {
            ...
            if expr[0] in aux_fit and expr[1] in aux_fit:
                aux_fit[name] = aux_fit[expr[0]] - aux_fit[expr[1]]
    ...
    def _spline_block(
        ...
        train_vals = _to_float_array(train_series)
        fit_vals = _to_float_array(fit_series)

        if periodic:
            train_vals = np.mod(train_vals, 360.0)
            fit_vals = np.mod(fit_vals, 360.0)

        finite_fit = np.isfinite(fit_vals)
        if not finite_fit.any():
            return pd.DataFrame(index=train_series.index)

        fill = np.nanmedian(fit_vals[finite_fit])
        train_vals = np.where(np.isfinite(train_vals), train_vals, fill)
        fit_vals = np.where(np.isfinite(fit_vals), fit_vals, fill)

        n_unique = np.unique(fit_vals).size
        if n_unique <= 1:
            return pd.DataFrame(
                {f"{name}_spline_0": np.zeros(train_vals.shape[0], dtype=np.float32)},
                index=train_series.index,
            )

        n_kn = int(min(max(2, n_knots), max(2, n_unique)))
        attempts = [{}]
        if periodic:
            attempts = [{"extrapolation": "periodic"}, {"periodic": True}, {}]
        ...
        for extra in attempts:
            ...
            params.update(extra)
            try:
                trans = SplineTransformer(**params)
                trans.fit(fit_vals.reshape(-1, 1))
                Xtr = trans.transform(train_vals.reshape(-1, 1))
                if hasattr(Xtr, "toarray"):
                    Xtr = Xtr.toarray()
                Xtr = np.asarray(Xtr, dtype=np.float32)
                cols = [f"{name}_spline_{i}" for i in range(Xtr.shape[1])]
                return pd.DataFrame(
                    Xtr, index=train_series.index, columns=cols, dtype=np.float32
                )
            except Exception:
                Xtr = None
                continue

        # Fallback if spline construction fails for this feature.
        return pd.DataFrame(
            {f"{name}_fallback": train_vals.astype(np.float32)},
            index=train_series.index,
            dtype=np.float32,
        )
    ...
    for col in spline_features:
        fit_source = out[col]
        if isinstance(aux_fit, dict) and col in aux_fit:
            fit_source = pd.concat([_to_float_array(out[col]) for _ in []], axis=0)

        if isinstance(aux_fit, dict) and col in aux_fit:
            fit_vals = pd.concat(
                [pd.Series(_to_float_array(out[col]), dtype=np.float64), aux_fit[col]],
                ignore_index=True,
            )
        else:
            fit_vals = out[col].astype(np.float64)

        block = _spline_block(
            out[col], fit_vals, knot_counts[col], col, periodic=(col == "alpha")
        )
        if block.shape[1] == 0:
            continue
        spline_frames[col] = block
        out = pd.concat([out, block], axis=1)

    def _tensor_block(
        a: pd.DataFrame, b: pd.DataFrame, prefix: str, k: int = 3
    ) -> pd.DataFrame:
        a_arr = np.asarray(a, dtype=np.float32)
        b_arr = np.asarray(b, dtype=np.float32)
        r = min(k, a_arr.shape[1], b_arr.shape[1])
        data = {}
        for i in range(r):
            ai = a_arr[:, i]
            for j in range(r):
                data[f"{prefix}_{i}_{j}"] = ai * b_arr[:, j]
        return pd.DataFrame(data, index=a.index, dtype=np.float32)

    tensor_pairs = [
        ("redshift", "color_u_g", "tx_redshift_x_color_u_g"),
        ("redshift", "color_g_r", "tx_redshift_x_color_g_r"),
        ("redshift", "color_r_i", "tx_redshift_x_color_r_i"),
        ("color_g_r", "color_r_i", "tx_color_g_r_x_color_r_i"),
    ]
    for left, right, name in tensor_pairs:
        if left in spline_frames and right in spline_frames:
            out = pd.concat(
                [out, _tensor_block(spline_frames[left], spline_frames[right], name)],
                axis=1,
            )

    cat_cols = [c for c in ["spectral_type", "galaxy_population"] if c in out.columns]
    if cat_cols:
        cat_encoded = pd.get_dummies(
            out[cat_cols].astype("string"), prefix=cat_cols, dtype=np.float32
        )
        out = out.drop(columns=cat_cols)
        out = pd.concat([out, cat_encoded], axis=1)

    # Keep only informative, non-redundant engineered columns.
