...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _color_block(out)
    _sky_block(out)
    for c in ["u", "g", "r", "i", "z", "redshift", "u_g", "g_r", "r_i", "i_z"]:
        if c in out.columns:
            x = _num(out, c).rank(pct=True).clip(0, 1)
            for knot in [0.2, 0.4, 0.6, 0.8]:
                name = str(knot).replace(".", "p")
                out[f"{c}_hinge_{name}"] = np.maximum(0, x - knot).astype("float32")
    return out
