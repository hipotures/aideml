...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    u = out["u"].astype("float64")
    g = out["g"].astype("float64")
    r = out["r"].astype("float64")
    i = out["i"].astype("float64")
    z = out["z"].astype("float64")
    redshift = out["redshift"].astype("float64")
    alpha = out["alpha"].astype("float64")
    delta = out["delta"].astype("float64")
    ...
    out["color_ug"] = color_ug
    out["color_gr"] = color_gr
    out["color_ri"] = color_ri
    out["color_iz"] = color_iz
    out["color_ur"] = color_ur
    out["color_gi"] = color_gi
    out["color_rz"] = color_rz
    out["color_uz"] = color_uz

    out["color_ug_abs"] = color_ug.abs()
    out["color_gr_abs"] = color_gr.abs()
    out["color_ri_abs"] = color_ri.abs()
    out["color_iz_abs"] = color_iz.abs()
    out["color_ug_sq"] = color_ug**2
    out["color_gr_sq"] = color_gr**2
    out["color_ri_sq"] = color_ri**2
    out["color_uz_sq"] = color_uz**2

    out["color_mix_1"] = color_ug + color_gr
    out["color_mix_2"] = color_ug - color_ri
    out["color_mix_3"] = color_gr + color_iz
    out["color_mix_4"] = color_gi * color_rz
    out["color_mix_5"] = color_ug * color_rz

    z_pos = redshift.clip(lower=0.0)
    out["redshift_abs"] = z_pos
    out["redshift_sq"] = z_pos**2
    out["redshift_cb"] = z_pos**3
    out["redshift_sqrt"] = np.sqrt(z_pos)
    out["redshift_log1p"] = np.log1p(z_pos)
    out["redshift_inv"] = 1.0 / (1.0 + z_pos)

    out["redshift_bin_low"] = (z_pos < 0.15).astype("uint8")
    out["redshift_bin_mid"] = ((z_pos >= 0.15) & (z_pos < 1.0)).astype("uint8")
    out["redshift_bin_high"] = (z_pos >= 1.0).astype("uint8")

    out["stage1_gate_color"] = color_ug + color_gr - color_ri
    out["stage1_gate_color_abs"] = out["stage1_gate_color"].abs()
    out["stage1_gate1"] = out["stage1_gate_color"] * (1.0 - np.exp(-z_pos))
    out["stage1_gate2"] = (
        out["stage1_gate_color"] * (1.0 + out["redshift_sqrt"])
    ).astype("float64")
    out["stage2_gate_gal_qso"] = color_ug * z_pos
    out["stage2_gate_gal_qso_2"] = color_uz * (1.0 + z_pos**2)
    out["stage2_gate_gal_qso_3"] = (color_gr * color_rz) * (1.0 + out["redshift_sq"])
    ...
    out["sky_proj_x"] = out["delta_cos"] * out["alpha_cos"]
    out["sky_proj_y"] = out["delta_cos"] * out["alpha_sin"]
    out["sky_proj_z"] = out["delta_sin"]

    out["mag_sum"] = u + g + r + i + z
    out["mag_range"] = pd.concat([u, g, r, i, z], axis=1).max(axis=1) - pd.concat(
        [u, g, r, i, z], axis=1
    ).min(axis=1)
    out["color_dispersion"] = (
        (color_ug - color_ug.mean()) ** 2
        + (color_gr - color_gr.mean()) ** 2
        + (color_ri - color_ri.mean()) ** 2
    ) / 3.0
