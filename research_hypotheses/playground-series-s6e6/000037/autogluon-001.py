...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _sky_block(out)
    _color_block(out)
    if all(c in out.columns for c in ["sky_z", "u", "g", "r", "i", "z"]):
        dust = 1.0 / (np.abs(out["sky_z"]) + 0.05)
        out["dust_proxy"] = dust.astype("float32")
        coeff = {"u":1.57, "g":1.20, "r":0.87, "i":0.67, "z":0.49}
        for b, k in coeff.items():
            out[f"m0_{b}"] = (_num(out, b) - 0.05 * k * dust).astype("float32")
        out["reddening_projection_strength"] = sum(coeff[b] * _num(out, b) for b in coeff).astype("float32")
    return out
