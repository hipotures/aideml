...
def preprocess(df, aux):
    ...
    # Galactic coordinate conversion (J2000 equatorial -> Galactic).
    ra_rad = np.deg2rad(out["alpha"].astype(float).to_numpy())
    dec_rad = np.deg2rad(out["delta"].astype(float).to_numpy())
    ...
    x_eqyz = np.vstack((x_eq, y_eq, z_eq))
    x_gal = R[0, 0] * x_eqyz[0] + R[0, 1] * x_eqyz[1] + R[0, 2] * x_eqyz[2]
    y_gal = R[1, 0] * x_eqyz[0] + R[1, 1] * x_eqyz[1] + R[1, 2] * x_eqyz[2]
    z_gal = R[2, 0] * x_eqyz[0] + R[2, 1] * x_eqyz[1] + R[2, 2] * x_eqyz[2]

    lat_gal = np.degrees(np.arcsin(np.clip(z_gal, -1.0, 1.0)))
    lon_gal = (np.degrees(np.arctan2(y_gal, x_gal)) + 360.0) % 360.0

    out["galactic_latitude_deg"] = lat_gal
    out["galactic_longitude_deg"] = lon_gal

    b_abs = np.abs(lat_gal)
    dust_proxy = 1.0 / np.sin(np.deg2rad(np.clip(b_abs, 1.0, 89.0)))
    dust_proxy = np.clip(dust_proxy, 1.0, 30.0).astype(float)
    out["dust_proxy"] = dust_proxy
    out["dust_proxy_log1p"] = np.log1p(dust_proxy)
    out["dust_proxy_z"] = (dust_proxy - dust_proxy.mean()) / (
        dust_proxy.std(ddof=0) + 1e-9
    )
    out["low_latitude_flag"] = (b_abs < 10.0).astype("int8")
    out["very_low_latitude_flag"] = (b_abs < 5.0).astype("int8")
    out["lat_dust_taper"] = 1.0 - np.clip(b_abs / 30.0, 0.0, 1.0)

    # Fixed SDSS extinction coefficients (per E(B-V)).
    coeff = {"u": 5.155, "g": 3.793, "r": 2.751, "i": 2.086, "z": 1.479}
    for band in ["u", "g", "r", "i", "z"]:
        out[f"{band}_dered"] = out[band] - coeff[band] * dust_proxy

    # Adjacent colors.
    out["u_g_color"] = out["u"] - out["g"]
    out["g_r_color"] = out["g"] - out["r"]
    out["r_i_color"] = out["r"] - out["i"]
    out["i_z_color"] = out["i"] - out["z"]

    # Dereddened adjacent colors.
    out["u_g_color_dered"] = out["u_dered"] - out["g_dered"]
    out["g_r_color_dered"] = out["g_dered"] - out["r_dered"]
    out["r_i_color_dered"] = out["r_dered"] - out["i_dered"]
    out["i_z_color_dered"] = out["i_dered"] - out["z_dered"]

    # Broad colors.
    out["u_r_color_dered"] = out["u_dered"] - out["r_dered"]
    out["g_i_color_dered"] = out["g_dered"] - out["i_dered"]
    out["r_z_color_dered"] = out["r_dered"] - out["z_dered"]

    out["u_r_color"] = out["u"] - out["r"]
    out["g_i_color"] = out["g"] - out["i"]
    out["r_z_color"] = out["r"] - out["z"]

    # Reddening-vector projection and orthogonal residuals in color space.
    color_mat = np.column_stack(
        [
            out["u_g_color"].to_numpy(),
            out["g_r_color"].to_numpy(),
            out["r_i_color"].to_numpy(),
            out["i_z_color"].to_numpy(),
        ]
    )
    redd_vec = np.array(
        [
    ...
    k2 = float(np.dot(redd_vec, redd_vec))
    projection = (color_mat @ redd_vec) / k2
    residual = color_mat - np.outer(projection, redd_vec)

    out["reddening_projection_strength"] = projection
    out["reddening_projection_abs"] = np.abs(projection)
    out["reddening_orthogonal_norm"] = np.linalg.norm(residual, axis=1)
    out["reddening_orthogonal_l1"] = np.abs(residual).sum(axis=1)

    out["blue_color_dereddened_mean"] = (
        out["u_dered"] - out["g_dered"] + out["g_dered"] - out["r_dered"]
    ) / 2.0
    out["red_color_dereddened_mean"] = (
        out["r_dered"] - out["i_dered"] + out["i_dered"] - out["z_dered"]
    ) / 2.0

    # Categorical interactions (target-safe and deterministic).
    spectral_d = pd.get_dummies(
        out["spectral_type"], prefix="spectral_type", dtype=float
    )
    galaxy_d = pd.get_dummies(
        out["galaxy_population"], prefix="galaxy_population", dtype=float
    )

    out = pd.concat([out, spectral_d, galaxy_d], axis=1)

    for col in galaxy_d.columns:
        out[f"{col}_x_dust_proxy"] = out[col].to_numpy() * dust_proxy
        out[f"{col}_x_dust_proxy_redshift"] = (
            out[col].to_numpy() * dust_proxy * out["redshift"].to_numpy()
        )
        out[f"{col}_x_projection"] = out[col].to_numpy() * projection

    for col in spectral_d.columns:
        out[f"{col}_x_dust_proxy"] = out[col].to_numpy() * dust_proxy
        out[f"{col}_x_dust_proxy_redshift"] = (
            out[col].to_numpy() * dust_proxy * out["redshift"].to_numpy()
        )
