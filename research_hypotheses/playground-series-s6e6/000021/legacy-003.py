...
def build_features(train_df: pd.DataFrame, test_df: pd.DataFrame):
    ...
    # Covariate-only transductive features: train+test are concatenated with no target column.
    combined = pd.concat(
    ...
    for col in categorical_cols:
        combined[col] = (
            combined[col].astype("string").fillna("__MISSING__").astype("category")
        )
...
def prepare_model_frames(df: pd.DataFrame, categorical_cols):
    df_xgb_lgb = df.copy()
    df_cat = df.copy()

    for col in categorical_cols:
        df_xgb_lgb[col] = df_xgb_lgb[col].astype("category")
        df_cat[col] = df_cat[col].astype(str)

    for col in df_xgb_lgb.columns:
        if col not in categorical_cols:
            df_xgb_lgb[col] = df_xgb_lgb[col].astype(np.float32)
            df_cat[col] = df_cat[col].astype(np.float32)

    return df_cat, df_xgb_lgb


def prepare_histgb_frame(df: pd.DataFrame, categorical_cols):
    out = df.copy()
    for col in categorical_cols:
        out[col] = out[col].cat.codes.astype(np.int32)
...
def main():
    with aide_stage("build_features_stage"):
        ...
        train_cat, train_xgb_lgb = prepare_model_frames(train_feat, categorical_cols)
        test_cat, test_xgb_lgb = prepare_model_frames(test_feat, categorical_cols)
        train_hist = prepare_histgb_frame(train_feat, categorical_cols)
        test_hist = prepare_histgb_frame(test_feat, categorical_cols)
    ...
    with aide_stage("fit_predict_fold_stage"):
        for fold_idx, (train_idx, valid_idx) in enumerate(folds, start=1):
            ...
            X_tr_cat = train_cat.iloc[train_idx]
            X_va_cat = train_cat.iloc[valid_idx]

            X_tr_xgb_lgb = train_xgb_lgb.iloc[train_idx]
            X_va_xgb_lgb = train_xgb_lgb.iloc[valid_idx]

            X_tr_hist = train_hist.iloc[train_idx]
            X_va_hist = train_hist.iloc[valid_idx]

            log_stage(f"Fold {fold_idx}/{N_SPLITS} - CatBoost")
            cb_valid_proba, cb_test_proba = fit_catboost(
                X_tr_cat, y_tr, X_va_cat, y_va, test_cat, categorical_cols
            )
            ...
            xgb_valid_proba, xgb_test_proba = fit_xgboost(
                X_tr_xgb_lgb, y_tr, X_va_xgb_lgb, y_va, test_xgb_lgb
            )
            ...
            lgb_valid_proba, lgb_test_proba = fit_lightgbm(
                X_tr_xgb_lgb, y_tr, X_va_xgb_lgb, y_va, test_xgb_lgb, categorical_cols
            )
            ...
            hist_valid_proba, hist_test_proba = fit_histgb(
                X_tr_hist, y_tr, X_va_hist, y_va, test_hist
            )
