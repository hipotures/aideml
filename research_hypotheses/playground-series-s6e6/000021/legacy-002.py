...
"""
    ...
    return result.returncode == 0 and "LIGHTGBM_CUDA_OK" in result.stdout


def build_explicit_features(train_df, test_df):
    num_cols = ["alpha", "delta", "u", "g", "r", "i", "z", "redshift"]
    base_cat_cols = ["spectral_type", "galaxy_population"]
    use_cols = num_cols + base_cat_cols

    # Covariate-only transductive features are built on combined train+test rows with no target column.
    combined = pd.concat(
        [train_df[use_cols].copy(), test_df[use_cols].copy()],
        axis=0,
        ignore_index=True,
    )
    ...
    for col in base_cat_cols:
        combined[col] = combined[col].astype("string").fillna("__MISSING__").astype(str)
    ...
    band_labels = np.array(["u", "g", "r", "i", "z"])
    ...
    combined["mag_min_band"] = band_labels[min_idx]
    combined["mag_max_band"] = band_labels[max_idx]
    ...
    combined["alpha_rad"] = alpha_rad
    combined["delta_rad"] = delta_rad
    ...
    alpha_center = (sky_alpha_bin_24 + 0.5) * 15.0
    delta_center = (sky_delta_bin_18 + 0.5) * 10.0 - 90.0
    combined["sky_alpha_bin_center_deg"] = alpha_center
    combined["sky_alpha_bin_offset_deg"] = alpha_deg - alpha_center
    combined["sky_delta_bin_center_deg"] = delta_center
    combined["sky_delta_bin_offset_deg"] = delta_deg - delta_center

    nn = NearestNeighbors(
        n_neighbors=17, metric="haversine", algorithm="ball_tree", n_jobs=-1
    )
    sky_points = np.column_stack([delta_rad, alpha_rad])
    nn.fit(sky_points)
    nn_dist, _ = nn.kneighbors(sky_points)
    ...
    combined["redshift_bin_20"] = pd.qcut(
        combined["redshift"],
        q=20,
        labels=False,
        duplicates="drop",
    ...
    def cat_join(*parts):
        ...
        for part in parts[1:]:
            result = result + "__" + part.astype(str)
        return result

    combined["spectral_type__redshift_bin_20"] = cat_join(
        combined["spectral_type"], combined["redshift_bin_20"]
    )
    combined["galaxy_population__redshift_bin_20"] = cat_join(
        combined["galaxy_population"], combined["redshift_bin_20"]
    )
    combined["spectral_type__galactic_cell_bin"] = cat_join(
        combined["spectral_type"], combined["galactic_cell_bin"]
    )
    combined["galaxy_population__galactic_cell_bin"] = cat_join(
        combined["galaxy_population"], combined["galactic_cell_bin"]
    )

    combined["spectral_type__galaxy_population"] = cat_join(
        combined["spectral_type"], combined["galaxy_population"]
    )
    combined["spectral_type__band_min_band"] = cat_join(
        combined["spectral_type"], combined["band_min_band"]
    )
    combined["galaxy_population__band_max_band"] = cat_join(
        combined["galaxy_population"], combined["band_max_band"]
    )
    combined["spectral_type__sky_alpha_bin_24"] = cat_join(
        combined["spectral_type"], combined["sky_alpha_bin_24"]
    )
    combined["galaxy_population__sky_alpha_bin_24"] = cat_join(
        combined["galaxy_population"], combined["sky_alpha_bin_24"]
    )
    combined["spectral_type__sky_cell_bin"] = cat_join(
        combined["spectral_type"], combined["sky_cell_bin"]
    )
    combined["spectral_type__sky_alpha_bin_48"] = cat_join(
        combined["spectral_type"], combined["sky_alpha_bin_48"]
    )
    combined["galaxy_population__sky_alpha_bin_48"] = cat_join(
        combined["galaxy_population"], combined["sky_alpha_bin_48"]
    )
    combined["spectral_type__sky_cell_bin_48"] = cat_join(
        combined["spectral_type"], combined["sky_cell_bin_48"]
    )
    combined["spectral_type__galaxy_population__sky_delta_bin_18"] = cat_join(
    ...
    for col in categorical_cols:
        combined[col] = combined[col].astype("string").fillna("__MISSING__").astype(str)
        cat = pd.Categorical(combined[col])
        codebook[col] = np.arange(len(cat.categories), dtype=np.int32)
    ...
    train_features = combined.iloc[:n_train].reset_index(drop=True)
    test_features = combined.iloc[n_train:].reset_index(drop=True)
    return train_features, test_features, categorical_cols, codebook


def to_category_frame(df, categorical_cols):
    out = df.copy()
    for col in categorical_cols:
...
def fit_lightgbm(
    X_train, y_train, X_valid, y_valid, X_test, categorical_cols, use_cuda
):
    params = dict(
        objective="multiclass",
        num_class=len(CLASS_NAMES),
...
def main():
    with aide_stage("build_features_stage"):
        ...
        X_train_num, X_test_num, categorical_cols, codebook = build_explicit_features(
            train, test
        )
        X_train_cat = to_category_frame(X_train_num, categorical_cols)
        X_test_cat = to_category_frame(X_test_num, categorical_cols)

    with aide_stage("make_folds_stage"):
        skf = StratifiedKFold(
            n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE
        )
        folds = list(skf.split(X_train_num, y))
        lgbm_use_cuda = lightgbm_cuda_available()
        print(f"LightGBM CUDA available: {lgbm_use_cuda}", flush=True)
    ...
    with aide_stage("fit_predict_fold_stage"):
        for fold_idx, (train_idx, valid_idx) in enumerate(folds, start=1):
            ...
            X_va_num = X_train_num.iloc[valid_idx]
            X_tr_cat = X_train_cat.iloc[train_idx]
            X_va_cat = X_train_cat.iloc[valid_idx]
            y_tr = y[train_idx]
            y_va = y[valid_idx]
    ...
    with aide_stage("score_stage"):
        ...
        best_oof_pred = oof_proba[best_model].argmax(axis=1)
        best_test_pred = test_proba[best_model].argmax(axis=1)
        print(
        ...
        oof_df = pd.DataFrame(
            {
                ...
                "target": y_labels,
                "prediction": CLASS_NAMES[best_oof_pred],
            }
        )
        ...
        submission_df = pd.DataFrame(
            {
                ...
                "class": CLASS_NAMES[best_test_pred],
            }
        )
