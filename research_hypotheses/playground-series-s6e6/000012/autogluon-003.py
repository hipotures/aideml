...
def _find_aux_label_column(frame: pd.DataFrame) -> str | None:
    ...
    for col in frame.columns:
        ...
        if normalized.empty:
            continue
        if len(set(normalized.unique()) & expected) >= 2:
            return col
    return None
...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    _color_block(out)
    label_col = _find_aux_label_column(aux) if isinstance(aux, pd.DataFrame) and not aux.empty else None
    if label_col is not None:
        ...
        for cls, grp in aux2.groupby(aux2[label_col].astype(str).str.upper()):
