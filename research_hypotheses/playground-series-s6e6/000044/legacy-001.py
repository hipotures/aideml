# Hypothesis 000034: prune fine sky-bin categorical and frequency feature block
...
PRUNED_FINE_SKY_FEATURES = [
    ...
    "galaxy_population__sky_alpha_bin_48",
    "spectral_type__sky_cell_bin_48",
    "sky_alpha_bin_48_freq",
    "sky_delta_bin_36_freq",
    "sky_cell_bin_48_freq",
]
CATEGORICAL_COLS = [c for c in CATEGORICAL_COLS if c not in PRUNED_FINE_SKY_FEATURES]
...
def main():
    with aide_stage("build_features_stage"):
        ...
        # Controlled pruning: remove the high-cardinality fine sky-bin categorical family
        # and its covariate-only frequency columns before model fitting.
        drop_cols = [c for c in PRUNED_FINE_SKY_FEATURES if c in train_full.columns]
        if drop_cols:
            train_full = train_full.drop(columns=drop_cols)
            test = test.drop(columns=[c for c in drop_cols if c in test.columns])
            print(f"Pruned fine sky-bin feature columns: {len(drop_cols)}", flush=True)
