...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    # Keep preprocessing leak-safe and minimal: drop fine sky-bin family features only.
    PRUNED_FINE_SKY_FEATURES = [
        "sky_alpha_bin_48",
        "sky_delta_bin_36",
        "sky_cell_bin_48",
    ]
    PRUNED_FINE_SKY_PATTERNS = tuple(PRUNED_FINE_SKY_FEATURES)

    result = df.copy()

    # Drop exact matches plus interaction / frequency / frequency-normalized variants that contain these keys.
    to_drop = [
        col
        for col in result.columns
        if any(pattern in col for pattern in PRUNED_FINE_SKY_PATTERNS)
    ]
