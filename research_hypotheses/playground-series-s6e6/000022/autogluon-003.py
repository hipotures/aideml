...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    redshift = pd.to_numeric(out["redshift"], errors="coerce")
    redshift = redshift.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    redshift = redshift.clip(lower=-0.01)

    cleaned_mag = {}

    for col in mag_cols:
        mag = pd.to_numeric(out[col], errors="coerce")
        invalid = ~np.isfinite(mag) | (mag < -50) | (mag > 60)
        med = mag.where(~invalid).median()
        if not np.isfinite(med):
            med = 21.0
        mag = mag.where(~invalid, med).astype(float)
        mag = mag.clip(-1.0, 40.0)
        cleaned_mag[col] = mag
        out[f"{col}_mag_clean"] = mag
        out[f"{col}_flux"] = np.power(10.0, -0.4 * mag)
        out[f"{col}_log_flux"] = np.log1p(out[f"{col}_flux"])

    flux_cols = [f"{c}_flux" for c in mag_cols]
    flux = out[flux_cols]
    total_flux = flux.sum(axis=1)
    out["total_flux"] = total_flux
    out["log_total_flux"] = np.log1p(total_flux)

    total_safe = total_flux.replace(0, eps)
    for col in mag_cols:
        fc = f"{col}_flux_share"
        out[fc] = out[f"{col}_flux"] / total_safe
        out[f"{col}_log_flux_rs"] = out[f"{col}_log_flux"] + 2.0 * np.log1p(
            redshift.clip(lower=-0.999)
        )
        out[f"{col}_flux_scaled"] = (
            out[f"{col}_flux"] * (1.0 + redshift.clip(lower=-0.999)) ** 2
        )

    for a, b in [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]:
        out[f"flux_ratio_{a}_{b}"] = out[f"{a}_flux"] / (out[f"{b}_flux"] + eps)

    for a, b in [("u", "r"), ("g", "i"), ("u", "z"), ("g", "z")]:
        out[f"flux_ratio_broad_{a}_{b}"] = out[f"{a}_flux"] / (out[f"{b}_flux"] + eps)

    blue_flux = out["u_flux"] + out["g_flux"]
    red_flux = out["r_flux"] + out["i_flux"] + out["z_flux"]
    out["blue_flux_share"] = blue_flux / (out["total_flux"] + eps)
    out["red_flux_share"] = red_flux / (out["total_flux"] + eps)
    out["blue_to_red_balance"] = blue_flux / (red_flux + eps)
    out["flux_concentration"] = (out["u_flux"] + out["g_flux"]) / (
        out["i_flux"] + out["z_flux"] + eps
    )

    flux_arr = out[flux_cols].to_numpy(float)
    lam = np.array([355.0, 468.0, 616.0, 748.0, 893.0], dtype=float)
    share = flux_arr / (total_flux.to_numpy()[:, None] + eps)
    out["flux_weighted_mean_wavelength"] = np.dot(share, lam)

    mean_err = lam[None, :] - out["flux_weighted_mean_wavelength"].to_numpy()[:, None]
    var = np.sum(share * mean_err**2, axis=1)
    skew_num = np.sum(share * mean_err**3, axis=1)
    kurt_num = np.sum(share * mean_err**4, axis=1)
    out["flux_moment_var"] = var
    out["flux_moment_skew"] = skew_num / (var**1.5 + eps)
    out["flux_moment_kurt"] = kurt_num / (var**2 + eps)
    out["flux_entropy"] = -np.sum(share * np.log(share + eps), axis=1)

    out["total_flux_redshift_scaled"] = (
        total_flux * (1.0 + redshift.clip(lower=-0.999)) ** 2
    )
    out["log_total_flux_redshift_scaled"] = np.log1p(total_flux) + 2.0 * np.log1p(
        redshift.clip(lower=-0.999)
    )
