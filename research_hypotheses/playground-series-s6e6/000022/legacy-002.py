...
def make_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for cat in ["spectral_type", "galaxy_population"]:
        if cat in out.columns:
            out[cat] = out[cat].astype(str).fillna("missing")
...
def aligned_proba(model, X_part):
    raw = model.predict_proba(X_part)
    out = np.zeros((len(X_part), len(CLASSES)), dtype=float)
    for j, cls in enumerate(model.classes_):
        out[:, np.where(CLASSES == cls)[0][0]] = raw[:, j]
    return out
...
def main():
    ...
    with aide_stage("fit_predict_fold_stage"):
        for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
            ...
            oof_proba["linear"][va_idx] = aligned_proba(linear_model, X_va)
            test_proba["linear"] += aligned_proba(linear_model, X_test) / N_SPLITS
            fold_pred = CLASSES[np.argmax(oof_proba["linear"][va_idx], axis=1)]
            print(
                f"Fold {fold} linear balanced_accuracy: {balanced_accuracy_score(y_va, fold_pred):.6f}",
                flush=True,
            )
            ...
            oof_proba["extratrees"][va_idx] = aligned_proba(et_model, X_va)
            test_proba["extratrees"] += aligned_proba(et_model, X_test) / N_SPLITS
            fold_pred = CLASSES[np.argmax(oof_proba["extratrees"][va_idx], axis=1)]
            print(
                f"Fold {fold} extratrees balanced_accuracy: {balanced_accuracy_score(y_va, fold_pred):.6f}",
                flush=True,
            )
            ...
            cat_va = cat_model.predict_proba(va_pool)
            cat_te = cat_model.predict_proba(te_pool)
            cat_aligned_va = np.zeros((len(X_va), len(CLASSES)), dtype=float)
            cat_aligned_te = np.zeros((len(X_test), len(CLASSES)), dtype=float)
            for j, cls in enumerate(cat_model.classes_):
                dst = np.where(CLASSES == cls)[0][0]
                cat_aligned_va[:, dst] = cat_va[:, j]
                cat_aligned_te[:, dst] = cat_te[:, j]
            oof_proba["catboost"][va_idx] = cat_aligned_va
            test_proba["catboost"] += cat_aligned_te / N_SPLITS
            fold_pred = CLASSES[np.argmax(oof_proba["catboost"][va_idx], axis=1)]
            print(
                f"Fold {fold} catboost balanced_accuracy: {balanced_accuracy_score(y_va, fold_pred):.6f}",
                flush=True,
            )
