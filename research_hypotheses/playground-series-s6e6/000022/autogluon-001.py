...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if bands:
        fluxes = []
        for b in bands:
            flux = np.power(10.0, -0.4 * _num(out, b).clip(-50, 50))
            out[f"flux_{b}"] = flux.astype("float32")
            out[f"log_flux_{b}"] = np.log1p(flux).astype("float32")
            fluxes.append(flux)
        mat = np.column_stack(fluxes)
        total = mat.sum(axis=1) + 1e-12
        out["total_flux"] = total.astype("float32")
        out["log_total_flux"] = np.log1p(total).astype("float32")
        for i, b in enumerate(bands):
            out[f"flux_share_{b}"] = (mat[:, i] / total).astype("float32")
        out["flux_entropy"] = (-(mat / total[:, None]) * np.log((mat / total[:, None]) + 1e-12)).sum(axis=1).astype("float32")
        out["blue_red_flux_balance"] = ((mat[:, 0] + mat[:, 1]) / (mat[:, -2] + mat[:, -1] + 1e-12)).astype("float32")
    if "redshift" in out.columns and "total_flux" in out.columns:
        z = np.log1p(np.maximum(_num(out, "redshift"), 0))
        out["total_flux_z2"] = (out["total_flux"] * np.square(1 + z)).astype("float32")
    return out
