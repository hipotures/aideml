...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Deterministic sky tiling from covariates only
    n_ra_bins = 24
    n_dec_bins = 12
    ra_edges = np.linspace(0.0, 360.0, n_ra_bins + 1)
    dec_min = float(out["delta"].min())
    dec_max = float(out["delta"].max())
    if not np.isfinite(dec_min) or not np.isfinite(dec_max) or dec_min == dec_max:
        dec_min, dec_max = -90.0, 90.0
    dec_edges = np.linspace(dec_min, dec_max, n_dec_bins + 1)

    out["_ra_bin"] = pd.cut(
        out["alpha"], bins=ra_edges, labels=False, include_lowest=True, right=True
    ).astype("int16")
    out["_dec_bin"] = pd.cut(
        out["delta"], bins=dec_edges, labels=False, include_lowest=True, right=True
    ).astype("int16")

    cell_keys = ["_dec_bin", "_ra_bin"]
    key_series = pd.Series(list(zip(out["_dec_bin"], out["_ra_bin"])), index=out.index)

    # Global robust stats
    global_median = out[sky_cols].median()
    global_mad = out[sky_cols].apply(
        lambda s: np.median(np.abs(s.values - np.median(s.values))) + eps
    )

    cell_medians = out.groupby(cell_keys, sort=False)[sky_cols].transform("median")

    # Per-cell robust scale (MAD) and residual/offset features
    for col in sky_cols:
        out[f"{col}_cell_median"] = cell_medians[col]
        out[f"{col}_cell_resid"] = out[col] - cell_medians[col]
        out[f"{col}_cell_minus_global"] = cell_medians[col] - global_median[col]

        mad = out.groupby(cell_keys, sort=False)[col].transform(
            lambda x: np.median(np.abs(x.values - np.median(x.values))) + eps
        )
        out[f"{col}_cell_mad"] = mad
        out[f"{col}_mad_ratio"] = mad / (global_mad[col] + eps)

    # Sky-cell support features
    cell_counts = out.groupby(cell_keys, sort=False).size()
    out["sky_cell_count"] = key_series.map(cell_counts).astype("float64")
    out["sky_cell_log_count"] = np.log1p(out["sky_cell_count"])
    out["sky_cell_tiny"] = (out["sky_cell_count"] <= 3).astype("int8")

    # 3x3 neighbor-smoothed sky medians per feature
    n_dec = int(out["_dec_bin"].max()) + 1
    n_ra = int(out["_ra_bin"].max()) + 1
    rows = np.arange(n_dec, dtype=np.int16)
    cols = np.arange(n_ra, dtype=np.int16)
    full_idx = pd.MultiIndex.from_product([rows, cols])

    def _neighbor_mean_3x3(arr: np.ndarray) -> np.ndarray:
        padded = np.pad(arr, ((1, 1), (1, 1)), mode="constant", constant_values=np.nan)
        acc = np.zeros_like(arr, dtype=np.float64)
        cnt = np.zeros_like(arr, dtype=np.float64)
        for d_dec in (-1, 0, 1):
            for d_ra in (-1, 0, 1):
                neigh = padded[
                    1 + d_dec : 1 + d_dec + n_dec, 1 + d_ra : 1 + d_ra + n_ra
                ]
    ...
    for col in sky_cols:
        cell_table = (
            out.groupby(cell_keys, sort=False)[col]
            .median()
            .unstack(level=1, fill_value=np.nan)
            .reindex(index=rows, columns=cols)
        )
        arr = cell_table.to_numpy(dtype=np.float64)
        neigh_arr = _neighbor_mean_3x3(arr)
        neigh_map = pd.Series(neigh_arr.ravel(), index=full_idx).to_dict()
        out[f"{col}_sky3x3_median"] = key_series.map(
            lambda k: neigh_map.get((int(k[0]), int(k[1])), np.nan)
        )
        out[f"{col}_sky3x3_resid"] = out[col] - out[f"{col}_sky3x3_median"]
        out[f"{col}_sky3x3_minus_global"] = (
            out[f"{col}_sky3x3_median"] - global_median[col]
        )

    return out.drop(columns=["_ra_bin", "_dec_bin"])
