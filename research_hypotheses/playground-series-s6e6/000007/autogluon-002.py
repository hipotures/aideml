...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    redshift_filled = redshift.fillna(redshift.median())
    out["redshift"] = redshift_filled

    centers = (0.0, 0.05, 0.3, 0.7, 1.2, 2.0, 2.7, 3.5, 5.0)
    widths = (0.03, 0.05, 0.15, 0.25, 0.35, 0.45, 0.35, 0.55, 0.9)

    for center, width in zip(centers, widths):
        name = f"redshift_rbf_{str(center).replace('.', 'p')}"
        out[name] = np.exp(-0.5 * ((redshift_filled - center) / width) ** 2)

    out["redshift_near_zero_flag"] = (out["redshift_rbf_0p0"] > 0.2).astype(np.int8)
    out["redshift_qso_star_overlap_gate"] = out["redshift_rbf_2p7"]
    out["redshift_high_qso_gate"] = (redshift_filled > 1.6).astype(np.int8)
    out["redshift_extreme_qso_gate"] = (redshift_filled > 4.0).astype(np.int8)
    ...
    if all(c in out.columns for c in ("u", "g", "r", "i", "z")):
        ...
        overlap = out["redshift_qso_star_overlap_gate"]
        for col in ("color_u_g", "color_g_r", "color_r_i", "color_i_z", "mag_mean"):
            out[f"redshift_qso_star_overlap_gate_x_{col}"] = overlap * out[col]
