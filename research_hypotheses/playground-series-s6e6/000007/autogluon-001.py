...
AIDE_AG_CONFIG = {'class_balance': 'balanced',
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "redshift" in X.columns:
        z = pd.to_numeric(X["redshift"], errors="coerce")
        z_filled = z.fillna(z.median())
        X["redshift_filled"] = z_filled

        centers = [0.0, 0.05, 0.3, 0.7, 1.2, 2.0, 2.7, 3.5, 5.0]
        widths = [0.03, 0.05, 0.15, 0.25, 0.35, 0.45, 0.35, 0.55, 0.9]
        for c, w in zip(centers, widths):
            cname = str(c).replace(".", "p")
            X[f"redshift_rbf_{cname}"] = np.exp(-0.5 * ((z_filled - c) / w) ** 2)

        X["redshift_near_zero_flag"] = (z_filled.abs() <= 0.03).astype(np.int8)
        X["redshift_qso_star_overlap_gate"] = X["redshift_rbf_2p7"]
        X["redshift_high_qso_gate"] = (z_filled > 1.6).astype(np.int8)
        X["redshift_extreme_qso_gate"] = (z_filled > 4.0).astype(np.int8)

    band_cols = [c for c in ["u", "g", "r", "i", "z"] if c in X.columns]
    for c in band_cols:
        X[c] = pd.to_numeric(X[c], errors="coerce")

    if all(c in X.columns for c in ["u", "g"]):
        X["color_u_g"] = X["u"] - X["g"]
    if all(c in X.columns for c in ["g", "r"]):
        X["color_g_r"] = X["g"] - X["r"]
    if all(c in X.columns for c in ["r", "i"]):
        X["color_r_i"] = X["r"] - X["i"]
    if all(c in X.columns for c in ["i", "z"]):
        X["color_i_z"] = X["i"] - X["z"]

    if band_cols:
        X["mag_mean"] = X[band_cols].mean(axis=1)

    if "redshift_qso_star_overlap_gate" in X.columns:
        gate = X["redshift_qso_star_overlap_gate"]
        for feat in ["color_u_g", "color_g_r", "color_r_i", "color_i_z", "mag_mean"]:
            if feat in X.columns:
                X[f"overlap_gate_x_{feat}"] = gate * X[feat]
