...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(b in out.columns for b in bands):
        vals = out[bands].to_numpy()
        order = np.argsort(vals, axis=1)
        out["band_order_code"] = (order * np.array([1, 5, 25, 125, 625])).sum(axis=1).astype("int32")
        diffs = np.diff(vals, axis=1)
        out["color_sign_code"] = ((diffs > 0) * np.array([1, 2, 4, 8])).sum(axis=1).astype("int8")
        out["n_monotone_up_steps"] = (diffs > 0).sum(axis=1).astype("int8")
        out["n_local_turns"] = (np.diff(np.sign(diffs), axis=1) != 0).sum(axis=1).astype("int8")
    _categorical_block(out)
    return out
