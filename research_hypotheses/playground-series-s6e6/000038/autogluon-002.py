...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    bands = ["u", "g", "r", "i", "z"]
    bands_arr = np.array(bands, dtype=object)
    mag = data[bands].to_numpy(dtype=np.float64, copy=False)
    n_rows = len(data)

    # Basic adjacent colors
    color_cols = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    color_values = [
        data[a].to_numpy(dtype=np.float64) - data[b].to_numpy(dtype=np.float64)
        for a, b in color_cols
    ]
    data["color_u_g"] = color_values[0]
    data["color_g_r"] = color_values[1]
    data["color_r_i"] = color_values[2]
    data["color_i_z"] = color_values[3]

    # Brightness ordering and topology tokens
    order_idx = np.argsort(mag, axis=1)  # lower magnitude is brighter
    brightest_idx = order_idx[:, 0]
    faintest_idx = order_idx[:, -1]
    data["brightest_band"] = bands_arr[brightest_idx]
    data["faintest_band"] = bands_arr[faintest_idx]
    data["order_token"] = ["|".join(bands_arr[row]) for row in order_idx]

    # Adjacent color sign bitmask (4 bits)
    adj_sign = np.column_stack([c > 0 for c in color_values]).astype(np.int64)
    data["adjacent_sign_bitmask"] = (
        (adj_sign * np.array([1, 2, 4, 8], dtype=np.int64)).sum(axis=1).astype(np.int16)
    )

    # All pairwise brighter-than bits over ugriz (10 bits + explicit columns)
    pair_defs = [
        (0, 1, "u_gt_g"),
        (0, 2, "u_gt_r"),
        (0, 3, "u_gt_i"),
    ...
    pair_mask = np.zeros(n_rows, dtype=np.int16)
    for bit, (a, b, name) in enumerate(pair_defs):
        comp = (mag[:, a] < mag[:, b]).astype(np.int8)  # brighter-than in magnitudes
        data[f"pair_{name}"] = comp
        pair_mask |= comp.astype(np.int16) << bit
    data["pairwise_bright_bitmask"] = pair_mask

    # Inversion-style mismatch counts relative to blue->red and red->blue topologies
    upper = np.triu(np.ones((5, 5), dtype=bool), 1)
    gt_mat = (mag[:, None, :] > mag[:, :, None])[
        :, upper
    ]  # m_band_i > m_band_j for i<j
    lt_mat = (mag[:, None, :] < mag[:, :, None])[
        :, upper
    ]  # m_band_i < m_band_j for i<j
    data["inv_blue_to_red"] = gt_mat.sum(axis=1).astype(np.int8)
    data["inv_red_to_blue"] = lt_mat.sum(axis=1).astype(np.int8)

    # Local extrema flags/locations in the 5-point u-g-r-i-z sequence
    e1 = ((mag[:, 1] - mag[:, 0]) * (mag[:, 1] - mag[:, 2]) > 0).astype(np.int8)
    e2 = ((mag[:, 2] - mag[:, 1]) * (mag[:, 2] - mag[:, 3]) > 0).astype(np.int8)
    e3 = ((mag[:, 3] - mag[:, 2]) * (mag[:, 3] - mag[:, 4]) > 0).astype(np.int8)
    data["extrema_pos_ugr"] = e1
    data["extrema_pos_gri"] = e2
    data["extrema_pos_riz"] = e3
    local_mask = e1 + 2 * e2 + 4 * e3
    data["num_local_extrema"] = (e1 + e2 + e3).astype(np.int8)
    data["local_extrema_token"] = local_mask.astype(str)

    # Longest monotone run on consecutive signed differences
    d1 = np.diff(mag, axis=1)
    s1 = np.sign(d1).astype(np.int8)

    def _longest_run(sign_row: np.ndarray) -> int:
        best = 0
        cur = 0
        prev = 0
        for s in sign_row:
    ...
    data["longest_monotone_run"] = np.array(
        [_longest_run(row) for row in s1], dtype=np.int8
    )

    # Second-difference sign pattern + convex/concave segment counts
    d2 = np.diff(d1, axis=1)
    s2 = np.sign(d2).astype(np.int8)
    symbol = np.array(["N", "Z", "P"], dtype=object)
    data["second_diff_sign_token"] = np.array(
        ["".join(symbol[row + 1]) for row in s2], dtype=object
    )

    def _segment_count(sign_row: np.ndarray, target: int) -> int:
        count = 0
        active = False
        for s in sign_row:
            if s == target:
    ...
    data["convex_segments"] = np.array(
        [_segment_count(row, 1) for row in s2], dtype=np.int8
    )
    data["concave_segments"] = np.array(
        [_segment_count(row, -1) for row in s2], dtype=np.int8
    )

    # Redshift regime + coarse topology token
    redshift_regime = pd.qcut(data["redshift"], q=4, labels=False, duplicates="drop")
    data["redshift_regime"] = redshift_regime.astype("Int16")
    data["topology_token"] = (
        data["brightest_band"].astype(str)
        + "|"
        + data["faintest_band"].astype(str)
        + "|"
        + data["adjacent_sign_bitmask"].astype(str)
        + "|"
        + data["redshift_regime"].astype(str)
    )

    # Encode unsupervised topology tokens (low-cardinality one-hot, else frequency)
    token_cols = [
        "order_token",
        "adjacent_sign_bitmask",
        "local_extrema_token",
        "second_diff_sign_token",
        "topology_token",
    ]
    for col in token_cols:
        n_unique = data[col].nunique(dropna=False)
        if n_unique <= 24:
            dummies = pd.get_dummies(data[col], prefix=col, dtype="int8")
            data = data.drop(columns=[col]).join(dummies)
        else:
            freq = data[col].value_counts(dropna=False)
            data[f"{col}_freq"] = data[col].map(freq).astype(np.int32)
