...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    feature_cols = [c for c in ["u", "g", "r", "i", "z", "redshift"] if c in out.columns]
    if len(feature_cols) < 3 or not isinstance(aux, pd.DataFrame) or aux.empty:
        return out
    ...
    def numeric_frame(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for a, b in [
            ...
            if a in work.columns and b in work.columns:
                work[f"color_{a}_{b}"] = work[a] - work[b]
        if "redshift" in work.columns:
            work["redshift_tanh"] = np.tanh(work["redshift"])
        return work
    ...
    aux_feature_cols = [c for c in feature_cols if c in aux.columns]
    if len(aux_feature_cols) != len(feature_cols):
        return out
    label_col = find_aux_label_column(aux)
    if label_col is None:
        return out

    row_work = numeric_frame(out)
    aux_work = numeric_frame(aux)
    labels = aux[label_col].astype(str).str.upper().str.strip()
    dist_cols = list(row_work.columns)
    if not dist_cols:
        return out

    distance_features: dict[str, np.ndarray] = {}
    for label in ["GALAXY", "STAR", "QSO"]:
        ref = aux_work.loc[labels == label, dist_cols]
        if ref.empty:
            continue
        med = ref.median(axis=0)
        q75 = ref.quantile(0.75)
        q25 = ref.quantile(0.25)
        scale = (q75 - q25).replace(0, np.nan).fillna(ref.std(axis=0)).replace(0, 1.0)
        z = (row_work[dist_cols] - med) / scale
        dist = np.sqrt(np.nanmean(np.square(z.to_numpy(dtype="float64")), axis=1))
        dist = np.nan_to_num(dist, nan=float(np.nanmedian(dist)) if dist.size else 0.0)
        distance_features[f"aux_clsdist_{label}"] = dist.astype("float32")
        distance_features[f"aux_clsdist_{label}_log1p"] = np.log1p(dist).astype("float32")

    base_names = [name for name in ["GALAXY", "STAR", "QSO"] if f"aux_clsdist_{name}" in distance_features]
    for name, values in distance_features.items():
        out[name] = values
    if len(base_names) >= 2:
        stacked = np.column_stack([distance_features[f"aux_clsdist_{name}"] for name in base_names])
        order = np.argsort(stacked, axis=1)
        sorted_dist = np.take_along_axis(stacked, order, axis=1)
        out["aux_clsdist_best"] = sorted_dist[:, 0].astype("float32")
        out["aux_clsdist_margin"] = (sorted_dist[:, 1] - sorted_dist[:, 0]).astype("float32")
        inv = 1.0 / (1.0 + stacked)
        out["aux_clsdist_confidence"] = (inv.max(axis=1) / np.maximum(inv.sum(axis=1), 1e-6)).astype("float32")
        for left_index, left in enumerate(base_names):
            for right in base_names[left_index + 1 :]:
                out[f"aux_clsdist_margin_{left}_{right}"] = (
                    distance_features[f"aux_clsdist_{left}"]
                    - distance_features[f"aux_clsdist_{right}"]
                ).astype("float32")
    return out
