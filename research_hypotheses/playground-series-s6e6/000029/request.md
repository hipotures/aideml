...
### 11
Title: Fold-Safe Local Class Posterior Features
Summary: Represent each object by out-of-fold nearest-neighbor class posterior, margin, entropy, and class-distance features in standardized photometric space.
Rationale: Feature family: fold_safe_local_class_density_features Feature strategy: Build a compact base space from raw ugriz magnitudes, redshift, alpha/delta, one-hot spectral_type and galaxy_population, plus only the standard adjacent color indices needed to define a useful distance metric. Within each training fold, standardize numeric columns on the fold-train split, fit KNN-style neighbor models using fold-train labels, and transform fold-validation rows into distance-weighted class probabilities at several neighborhoo…
### 12
Title: Rest-Frame SED Warp Features
Summary: Convert observed ugriz magnitudes into approximate rest-frame spectral features by shifting filter wavelengths by redshift and interpolating pseudo-colors on a common wavelength grid.
Rationale: Feature family: rest_frame_sed_warp_features Feature strategy: Keep the existing raw magnitudes, redshift, alpha/delta, spectral_type, and galaxy_population, then add a redshift-warped representation of the photometric SED. Use fixed SDSS effective wavelengths for u,g,r,i,z, compute rest wavelengths as lambda_obs/(1+clipped_redshift), convert magnitudes to relative log-flux or centered magnitude profiles, and interpolate each object onto a small common rest-frame wavelength grid. Derive pseudo-rest-frame colors, c…
### 13
Title: Transductive Graph Label-Propagation Features
Summary: Add fold-safe semi-supervised class-probability features by propagating train labels across a KNN graph built on combined train/test photometric covariates.
Rationale: Feature family: semi_supervised_graph_label_features Feature strategy: Build a compact standardized space from ugriz magnitudes, redshift, adjacent/broad color indices, alpha/delta, and one-hot spectral_type and galaxy_population. For each CV fold, construct a sparse KNN similarity graph over fold-train rows, fold-validation rows with labels masked, and optionally test rows as unlabeled covariate-only anchors; run LabelSpreading or a lightweight equivalent label-propagation routine with validation labels set to -1…
### 14
...
### 16
Title: Class-Conditional Photometric Density Ratios
Summary: Fit fold-safe class-wise Gaussian density models in color-redshift space and use their log-likelihood ratios, posterior margins, and entropy as compact supervised statistical features.
Rationale: Feature family: fold_safe_parametric_class_density_features Feature strategy: Build a standardized compact space from ugriz magnitudes, adjacent and broad color indices, redshift, alpha/delta, and one-hot spectral_type/galaxy_population. Within each CV fold, fit small class-conditional GaussianMixture or regularized Gaussian density models separately for GALAXY, STAR, and QSO using only fold-training rows. Transform validation rows into per-class log likelihoods, log-likelihood ratios such as QSO-vs-STAR and GALAX…
