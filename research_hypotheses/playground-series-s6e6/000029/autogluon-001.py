...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if cols:
        x = ((out[cols] - out[cols].mean()) / (out[cols].std().replace(0, 1.0) + 1e-6)).to_numpy()
        for k in range(min(4, x.shape[1])):
            weights = np.cos(np.linspace(0, np.pi * (k + 1), x.shape[1]))
            out[f"manifold_axis_{k+1}"] = (x @ weights).astype("float32")
    return out
