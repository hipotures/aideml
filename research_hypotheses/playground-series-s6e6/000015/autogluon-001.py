...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _color_block(out)
    _categorical_block(out)
    for c in ["redshift", "u", "g", "r", "i", "z", "u_g", "g_r"]:
        if c in out.columns:
            out[f"{c}_bin10"] = pd.qcut(_num(out, c).rank(method="first"), 10, labels=False, duplicates="drop").fillna(0).astype("int16")
    if "spectral_type" in out.columns and "redshift_bin10" in out.columns:
        out["spectral_type_x_redshift_bin"] = out["spectral_type"].astype(str) + "_" + out["redshift_bin10"].astype(str)
    if "galaxy_population" in out.columns and "g_r_bin10" in out.columns:
        out["galaxy_population_x_gr_bin"] = out["galaxy_population"].astype(str) + "_" + out["g_r_bin10"].astype(str)
    return out
