...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Categorical frequency encodings for robust low-cardinality signals
    cat_cols = ["spectral_type", "galaxy_population"]
    n_rows = float(len(X))
    for col in cat_cols:
        vc = X[col].value_counts(dropna=False)
        X[f"{col}_freq"] = X[col].map(vc).astype("float32") / n_rows

    # Stable one-hot for categories
    X = X.join(pd.get_dummies(X["spectral_type"], prefix="spectral_type"))
    X = X.join(pd.get_dummies(X["galaxy_population"], prefix="galaxy_population"))

    # Numeric color features
    X["color_gr"] = X["g"] - X["r"]
    X["color_ug"] = X["u"] - X["g"]

    # Deterministic quantile binning helper
    def qbin(s: pd.Series, q: int) -> pd.Series:
        qn = int(min(max(2, q), max(2, s.nunique())))
        b = pd.qcut(s, q=qn, labels=False, duplicates="drop")
        return b.fillna(-1).astype("int16")

    # Coarse numeric bins
    X["redshift_bin"] = qbin(X["redshift"], 6)
    X["u_bin"] = qbin(X["u"], 6)
    X["g_bin"] = qbin(X["g"], 6)
    X["r_bin"] = qbin(X["r"], 6)
    X["i_bin"] = qbin(X["i"], 6)
    X["z_bin"] = qbin(X["z"], 6)
    X["color_gr_bin"] = qbin(X["color_gr"], 5)

    # Small interaction features
    X["spectral_type_x_redshift"] = (
        X["spectral_type"].astype(str) + "__" + X["redshift_bin"].astype(str)
    )
    X["galaxy_population_x_color"] = (
        X["galaxy_population"].astype(str) + "__" + X["color_gr_bin"].astype(str)
    )

    X = X.join(pd.get_dummies(X["spectral_type_x_redshift"], prefix="sp_x_zbin"))
    X = X.join(pd.get_dummies(X["galaxy_population_x_color"], prefix="gal_x_colorbin"))
