...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    z = np.maximum(_num(out, "redshift"), 1e-6)
    lum = 4282.75 * z * (1.0 + 0.5 * z)
    dm = 5.0 * np.log10(np.maximum(lum, 1e-6)) + 25.0
    out["luminosity_distance_proxy"] = lum.astype("float32")
    out["distance_modulus"] = dm.astype("float32")
    for b in _bands(out):
        out[f"M_{b}"] = (_num(out, b) - dm).astype("float32")
    _color_block(out)
    return out
