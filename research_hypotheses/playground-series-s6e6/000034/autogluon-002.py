...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Preserve raw numeric redshift and regime flags.
    z = out["redshift"].to_numpy(dtype=np.float64)
    out["redshift_nonpositive"] = (z <= 0).astype("uint8")
    out["near_zero_redshift"] = ((z > 0) & (z <= 1e-3)).astype("uint8")
    out["high_redshift"] = (z >= 1.5).astype("uint8")
    out["log1p_redshift"] = np.log1p(np.maximum(z, 0.0))

    # Flat LCDM approximation for luminosity distance: 299792.458/H0 * (1+z) * integral dz/E(z)
    z_for_distance = np.clip(z, 1e-6, None)
    z_max = float(np.nanmax(z_for_distance))
    if not np.isfinite(z_max):
        z_max = 0.0
    z_grid_max = max(z_max * 1.05, 2.0)
    z_grid = np.linspace(0.0, z_grid_max, 1024)
    hz = np.sqrt(0.3 * (1.0 + z_grid) ** 3 + 0.7)
    inv_e = 1.0 / hz
    dz = z_grid[1] - z_grid[0]
    comoving = np.empty_like(z_grid)
    comoving[0] = 0.0
    comoving[1:] = np.cumsum((inv_e[:-1] + inv_e[1:]) * 0.5 * dz)
    d_lum_mpc = (299792.458 / 70.0) * (1.0 + z_grid) * comoving

    luminosity_distance_proxy = np.interp(
        z_for_distance, z_grid, d_lum_mpc, left=d_lum_mpc[0], right=d_lum_mpc[-1]
    )
    out["luminosity_distance_proxy"] = luminosity_distance_proxy
    out["log_luminosity_distance"] = np.log(
        np.maximum(luminosity_distance_proxy, 1e-12)
    )
    out["distance_modulus"] = (
        5.0 * np.log10(np.maximum(luminosity_distance_proxy, 1e-12)) + 25.0
    )

    # Approximate absolute magnitudes.
    for band in ["u", "g", "r", "i", "z"]:
        out[f"M_{band}"] = out[band] - out["distance_modulus"]

    abs_mags = out[["M_u", "M_g", "M_r", "M_i", "M_z"]]
    out["M_abs_mean"] = abs_mags.mean(axis=1)
    out["M_abs_std"] = abs_mags.std(axis=1)
    out["M_abs_range"] = abs_mags.max(axis=1) - abs_mags.min(axis=1)

    # Color features + absolute-magnitude interactions.
    out["color_u_g"] = out["u"] - out["g"]
    out["color_g_r"] = out["g"] - out["r"]
    out["color_r_i"] = out["r"] - out["i"]
    out["color_i_z"] = out["i"] - out["z"]
    out["M_r_x_color_u_g"] = out["M_r"] * out["color_u_g"]
    out["M_r_x_color_g_r"] = out["M_r"] * out["color_g_r"]
    out["M_i_x_color_g_r"] = out["M_i"] * out["color_g_r"]

    # Deterministic one-hot for known categories (prevents category instability).
    spectral_categories = ["M", "O/B", "G/K", "A/F"]
    spectral = out["spectral_type"].astype(
        pd.CategoricalDtype(categories=spectral_categories)
    )
    spectral_d = pd.get_dummies(spectral, prefix="spectral_type", dtype="uint8")
    for c in [f"spectral_type_{v}" for v in spectral_categories]:
        if c not in spectral_d.columns:
            spectral_d[c] = 0
    out = pd.concat(
        [out, spectral_d[[f"spectral_type_{v}" for v in spectral_categories]]], axis=1
    )

    pop_categories = ["Blue_Cloud", "Red_Sequence"]
    galaxy = out["galaxy_population"].astype(
        pd.CategoricalDtype(categories=pop_categories)
    )
    galaxy_d = pd.get_dummies(galaxy, prefix="galaxy_population", dtype="uint8")
    for c in [f"galaxy_population_{v}" for v in pop_categories]:
        if c not in galaxy_d.columns:
            galaxy_d[c] = 0
    out = pd.concat(
        [out, galaxy_d[[f"galaxy_population_{v}" for v in pop_categories]]], axis=1
    )

    # Redshift-regime interactions with one-hot semantics.
    interaction_cols = [
        "spectral_type_M",
        "spectral_type_O/B",
        "spectral_type_G/K",
    ...
    for c in interaction_cols:
        out[f"{c}_x_redshift"] = out[c] * out["redshift"]
        out[f"{c}_x_high_redshift"] = out[c] * out["high_redshift"]
        out[f"{c}_x_near_zero_redshift"] = out[c] * out["near_zero_redshift"]
