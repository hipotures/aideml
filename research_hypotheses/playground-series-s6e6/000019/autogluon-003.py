...
def preprocess(df: "pd.DataFrame", aux: "pd.DataFrame") -> "pd.DataFrame":
    ...
    for a, b in color_pairs:
        if a in out.columns and b in out.columns:
            col = out[a].to_numpy(dtype=np.float64) - out[b].to_numpy(dtype=np.float64)
            out[f"{a}_{b}_color"] = col
            out[f"{a}_{b}_color_abs"] = np.abs(col)

    # Band-profile features on ugriz
    band_cols = [c for c in ["u", "g", "r", "i", "z"] if c in out.columns]
    if len(band_cols) == 5:
        mags = out[band_cols].to_numpy(dtype=np.float64, copy=False)
        idx = np.arange(5, dtype=np.float64)
        idx_mean = idx.mean()
        idx_var = np.sum((idx - idx_mean) ** 2.0)

        mag_mean = np.nanmean(mags, axis=1)
        mag_std = np.nanstd(mags, axis=1)
        mag_min = np.nanmin(mags, axis=1)
        mag_max = np.nanmax(mags, axis=1)
        ...
        slope = np.nansum((mags - mag_mean[:, None]) * (idx - idx_mean), axis=1) / (
            idx_var if idx_var else 1.0
        )
        intercept = mag_mean - slope * idx_mean
        pred = slope[:, None] * idx + intercept[:, None]
        residual = mags - pred

        out["mag_spectral_slope"] = slope
        out["mag_resid_mean_abs"] = np.nanmean(np.abs(residual), axis=1)
        out["mag_resid_std"] = np.nanstd(residual, axis=1)
        ...
        mag_labels = np.array(band_cols)
        argmin_idx = np.nanargmin(mags, axis=1)
        argmax_idx = np.nanargmax(mags, axis=1)
        out["mag_min_band"] = pd.Series(mag_labels[argmin_idx], index=out.index)
        out["mag_max_band"] = pd.Series(mag_labels[argmax_idx], index=out.index)
    ...
    if "redshift" in out.columns:
        ...
        z_abs = np.abs(z)
        z_sign = np.sign(z)
        log_abs = np.log1p(np.maximum(z_abs, 0.0))

        out["redshift_abs"] = z_abs
        out["redshift_sq"] = z * z
        out["redshift_log1p_abs"] = log_abs
        out["redshift_signed_log1p_abs"] = z_sign * log_abs

        # Transductive but covariate-only redshift binning
        z_ok = np.isfinite(z)
        if np.any(z_ok):
            edges = np.quantile(z[z_ok], np.linspace(0.0, 1.0, 11))
            edges = np.unique(edges)
            if len(edges) >= 3:
                out["redshift_bin"] = pd.Series(
                    np.digitize(z, bins=edges[1:-1], right=True),
                    index=out.index,
                    dtype="Int16",
                )
            else:
                out["redshift_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        else:
            out["redshift_bin"] = pd.Series(0, index=out.index, dtype="Int16")
    else:
        out["redshift_bin"] = pd.Series(0, index=out.index, dtype="Int16")

    # Categorical normalization and interactions
    if "spectral_type" in out.columns:
        spectral = out["spectral_type"].astype("string").fillna("NA")
    else:
        spectral = pd.Series(["NA"] * n, index=out.index, dtype="string")
    if "galaxy_population" in out.columns:
        galaxy_pop = out["galaxy_population"].astype("string").fillna("NA")
    else:
        galaxy_pop = pd.Series(["NA"] * n, index=out.index, dtype="string")

    out["spectral_type"] = spectral
    out["galaxy_population"] = galaxy_pop

    spec_code = spectral.map({"M": 0.0, "O/B": 1.0, "G/K": 2.0, "A/F": 3.0}).fillna(
        -1.0
    )
    pop_code = galaxy_pop.map({"Blue_Cloud": 0.0, "Red_Sequence": 1.0}).fillna(-1.0)
    out["spectral_type_code"] = spec_code.astype(np.float64)
    out["galaxy_population_code"] = pop_code.astype(np.float64)

    if "redshift" in out.columns:
        out["redshift_x_spec_code"] = (
            out["redshift"].to_numpy(dtype=np.float64) * out["spectral_type_code"]
        )
        out["redshift_x_pop_code"] = (
            out["redshift"].to_numpy(dtype=np.float64) * out["galaxy_population_code"]
        )
        ...
        if "redshift_signed_log1p_abs" in out.columns:
    ...
    # RA/Dec trig, unit vectors, harmonics, bins, centers, offsets, local density
    if {"alpha", "delta"}.issubset(out.columns):
        ...
        out["alpha_sin"] = alpha_sin
        out["alpha_cos"] = alpha_cos
        out["delta_sin"] = delta_sin
        out["delta_cos"] = delta_cos

        x = delta_cos * alpha_cos
        y = delta_cos * alpha_sin
        z = delta_sin
        out["sky_x"] = x
        out["sky_y"] = y
        out["sky_z"] = z
        ...
        a_coarse = np.clip((alpha / 15.0).astype(np.int64), 0, 23)
        d_coarse = np.clip(((delta + 90.0) / 10.0).astype(np.int64), 0, 17)
        a_fine = np.clip((alpha / 5.0).astype(np.int64), 0, 71)
        d_fine = np.clip(((delta + 90.0) / 2.0).astype(np.int64), 0, 89)
        ...
        out["sky_cell_offset_radius_coarse"] = np.sqrt(
            out["alpha_coarse_center_offset"] * out["alpha_coarse_center_offset"]
            + out["delta_coarse_center_offset"] * out["delta_coarse_center_offset"]
        )
        out["sky_cell_offset_radius_fine"] = np.sqrt(
            out["alpha_fine_center_offset"] * out["alpha_fine_center_offset"]
            + out["delta_fine_center_offset"] * out["delta_fine_center_offset"]
        )

        def _neighbor_counts(a_bin, d_bin, a_size, d_size):
            ...
            neighbor = np.zeros_like(self_count, dtype=np.float64)
            for da in (-1, 0, 1):
                for dd in (-1, 0, 1):
                    a2 = np.clip(a_i + da, 0, a_size - 1)
                    d2 = np.clip(d_i + dd, 0, d_size - 1)
                    neighbor += counts[a2 * d_size + d2]
            return self_count, neighbor

        coarse_self, coarse_neighbor = _neighbor_counts(a_coarse, d_coarse, 24, 18)
        fine_self, fine_neighbor = _neighbor_counts(a_fine, d_fine, 72, 90)

        out["sky_cell_count_coarse"] = coarse_self
        out["sky_cell_plus1_coarse"] = coarse_neighbor
        out["sky_cell_count_fine"] = fine_self
        out["sky_cell_plus1_fine"] = fine_neighbor
        out["sky_density_ratio_coarse"] = coarse_neighbor / np.maximum(coarse_self, 1.0)
        out["sky_density_ratio_fine"] = fine_neighbor / np.maximum(fine_self, 1.0)

        # Galactic transformation (approx) + binning + interactions
        v_x = x
        v_y = y
        v_z = z
        g_x = -0.0548755604 * v_x + -0.8734370902 * v_y + -0.4838350155 * v_z
        g_y = 0.4941094279 * v_x + -0.4448296300 * v_y + 0.7469822445 * v_z
        g_z = -0.8676661490 * v_x + -0.1980763734 * v_y + 0.4559837762 * v_z

        gal_l = (np.degrees(np.arctan2(g_y, g_x)) + 360.0) % 360.0
        gal_b = np.degrees(np.arcsin(np.clip(g_z, -1.0, 1.0)))
        ...
        out["gal_l_bin"] = pd.Series(
            np.clip((gal_l / 15.0).astype(np.int64), 0, 23),
            index=out.index,
            dtype="Int16",
        )
        out["gal_b_bin"] = pd.Series(
            np.clip(((gal_b + 90.0) / 10.0).astype(np.int64), 0, 17),
            index=out.index,
            dtype="Int16",
        )

        gal_l_center = (out["gal_l_bin"].to_numpy(dtype=np.float64) + 0.5) * 15.0
        gal_b_center = (out["gal_b_bin"].to_numpy(dtype=np.float64) + 0.5) * 10.0 - 90.0
        out["gal_l_center"] = gal_l_center
        out["gal_b_center"] = gal_b_center
        out["gal_l_offset"] = gal_l - gal_l_center
        out["gal_b_offset"] = gal_b - gal_b_center
        out["spec_x_pop"] = spectral + "|" + galaxy_pop
    else:
        out["alpha_sin"] = 0.0
        out["alpha_cos"] = 1.0
        out["delta_sin"] = 0.0
        ...
        out["gal_b_cos"] = 1.0
        out["gal_b_abs"] = 0.0
        out["gal_l_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        out["gal_b_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        out["gal_l_center"] = 0.0
        out["gal_b_center"] = 0.0
        out["gal_l_offset"] = 0.0
        out["gal_b_offset"] = 0.0
        out["spec_x_pop"] = spectral + "|" + galaxy_pop

    # Crossed categorical features
    out["spec_x_redshift_bin"] = (
        spectral + "|" + out["redshift_bin"].astype("string").fillna("0")
    )
    out["pop_x_redshift_bin"] = (
        galaxy_pop + "|" + out["redshift_bin"].astype("string").fillna("0")
    )
    sky_cell = (
        "a"
        + out["alpha_coarse_bin"].astype("string").fillna("0")
        + "_d"
        + out["delta_coarse_bin"].astype("string").fillna("0")
    )
    out["sky_cell_cat"] = sky_cell
    out["spec_x_sky_cell"] = spectral + "|" + sky_cell
    out["pop_x_sky_cell"] = galaxy_pop + "|" + sky_cell
    gal_cell = (
        "l"
        + out["gal_l_bin"].astype("string").fillna("0")
        + "_b"
        + out["gal_b_bin"].astype("string").fillna("0")
    )
    out["gal_cell_cat"] = gal_cell
    out["spec_x_gal_cell"] = spectral + "|" + gal_cell
    out["pop_x_gal_cell"] = galaxy_pop + "|" + gal_cell
    ...
    # Frequency features from covariates only
    freq_candidates = [
        "alpha_coarse_bin",
        "delta_coarse_bin",
        "alpha_fine_bin",
    ...
    for c in freq_candidates:
        if c in out.columns:
            ...
            out[f"{c}_freq"] = out[c].astype("string").map(freq).astype(np.float64)

    # Percentile-rank covariates
    for c in ["u", "g", "r", "i", "z", "redshift"]:
        if c in out.columns:
            vc = out[c]
            out[f"{c}_prank"] = vc.rank(pct=True, method="average")

    # Optional auxiliary anchored features (shared columns only)
    if isinstance(aux, pd.DataFrame) and not aux.empty:
        ...
        for c in shared_cols:
            if c not in out.columns:
                continue
            aux_v = pd.to_numeric(aux[c], errors="coerce").to_numpy(dtype=np.float64)
            if c in {"u", "g", "r", "i", "z"}:
                aux_v = aux_v[aux_v > -9000]
            aux_v = aux_v[np.isfinite(aux_v)]
            if aux_v.size == 0:
                continue
            aux_sorted = np.sort(aux_v)
            aux_mean = float(aux_v.mean())
            aux_std = float(aux_v.std()) or 1.0
            x = out[c].to_numpy(dtype=np.float64)
            out[f"{c}_aux_q"] = np.searchsorted(aux_sorted, x, side="right") / len(
                aux_sorted
            )
            out[f"{c}_aux_z"] = (x - aux_mean) / aux_std

        for a, b in color_pairs:
            if f"{a}_{b}_color" not in out.columns:
                continue
            av = pd.to_numeric(aux[a], errors="coerce").to_numpy(dtype=np.float64)
            bv = pd.to_numeric(aux[b], errors="coerce").to_numpy(dtype=np.float64)
            if a in {"u", "g", "r", "i", "z"}:
                m = (av > -9000) & (bv > -9000)
                av = av[m]
                bv = bv[m]
                ...
                bv = bv[m]
            color_aux = av - bv
            color_aux = color_aux[np.isfinite(color_aux)]
            if color_aux.size == 0:
                continue
            sorted_color = np.sort(color_aux)
            color_mean = float(color_aux.mean())
            color_std = float(color_aux.std()) or 1.0
            c_val = out[f"{a}_{b}_color"].to_numpy(dtype=np.float64)
            out[f"{a}_{b}_color_aux_q"] = np.searchsorted(
                sorted_color, c_val, side="right"
            ) / len(sorted_color)
            out[f"{a}_{b}_color_aux_z"] = (c_val - color_mean) / color_std
