...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    spectral = (
        out["spectral_type"].astype("string").fillna("UNKNOWN")
        if "spectral_type" in out.columns
        else pd.Series("UNKNOWN", index=out.index, dtype="string")
    )
    galaxy_pop = (
        out["galaxy_population"].astype("string").fillna("UNKNOWN")
    ...
    spec_map = {"M": 0.0, "O/B": 1.0, "G/K": 2.0, "A/F": 3.0}
    pop_map = {"Blue_Cloud": 0.0, "Red_Sequence": 1.0}
    out["spectral_type_code"] = spectral.map(spec_map).fillna(-1.0).astype(np.float64)
    out["galaxy_population_code"] = (
        galaxy_pop.map(pop_map).fillna(-1.0).astype(np.float64)
    )
    ...
    for a, b in color_pairs:
        if a in out.columns and b in out.columns:
            diff = out[a].to_numpy(dtype=np.float64) - out[b].to_numpy(dtype=np.float64)
            out[f"{a}_{b}_color"] = diff
            out[f"{a}_{b}_color_abs"] = np.abs(diff)

    band_cols = [c for c in ["u", "g", "r", "i", "z"] if c in out.columns]
    if len(band_cols) == 5:
        mags = out[band_cols].to_numpy(dtype=np.float64)
        ...
        idx_mean = idx.mean()
        idx_var = np.sum((idx - idx_mean) ** 2.0) or 1.0
        centered = mags - mag_mean[:, None]
        slope = np.nansum(centered * (idx - idx_mean), axis=1) / idx_var
        ...
        safe = np.where(np.isfinite(mags), mags, 1e12)
        band_labels = np.array(band_cols, dtype=object)
        argmin_idx = safe.argmin(axis=1)
        argmax_idx = safe.argmax(axis=1)
        finite_any = np.isfinite(mags).any(axis=1)
        out["mag_min_band"] = pd.Series(
            np.where(finite_any, band_labels[argmin_idx], np.nan),
            index=out.index,
            dtype="string",
        )
        out["mag_max_band"] = pd.Series(
            np.where(finite_any, band_labels[argmax_idx], np.nan),
            index=out.index,
            dtype="string",
        )
    ...
    if "redshift" in out.columns:
        ...
        out["redshift_sq"] = z * z
        out["redshift_log1p_abs"] = np.log1p(np.maximum(z_abs, 0.0))
        out["redshift_signed_log1p_abs"] = z_sign * out["redshift_log1p_abs"]

        z_ok = np.isfinite(z)
        if np.any(z_ok):
            edges = np.unique(np.quantile(z[z_ok], np.linspace(0.0, 1.0, 11)))
            if len(edges) >= 3:
                bins = edges[1:-1]
                z_bin = np.digitize(z, bins=bins, right=True)
            else:
                z_bin = np.full(len(out), -1, dtype=np.int16)
        else:
            z_bin = np.full(len(out), -1, dtype=np.int16)
        out["redshift_bin"] = pd.Series(z_bin, index=out.index, dtype="Int16")
    ...
    if {"alpha", "delta"}.issubset(out.columns):
        ...
        alpha_coarse = np.clip((alpha / 15.0).astype(np.int32), 0, 23)
        delta_coarse = np.clip(((delta + 90.0) / 10.0).astype(np.int32), 0, 17)
        alpha_fine = np.clip((alpha / 5.0).astype(np.int32), 0, 71)
        delta_fine = np.clip(((delta + 90.0) / 2.0).astype(np.int32), 0, 89)

        out["alpha_coarse_bin"] = pd.Series(
            alpha_coarse, index=out.index, dtype="Int16"
        )
        out["delta_coarse_bin"] = pd.Series(
            delta_coarse, index=out.index, dtype="Int16"
        )
        out["alpha_fine_bin"] = pd.Series(alpha_fine, index=out.index, dtype="Int16")
        out["delta_fine_bin"] = pd.Series(delta_fine, index=out.index, dtype="Int16")

        alpha_coarse_center = (alpha_coarse + 0.5) * 15.0
        delta_coarse_center = (delta_coarse + 0.5) * 10.0 - 90.0
        alpha_fine_center = (alpha_fine + 0.5) * 5.0
        delta_fine_center = (delta_fine + 0.5) * 2.0 - 90.0
        out["alpha_coarse_center"] = alpha_coarse_center
        out["delta_coarse_center"] = delta_coarse_center
        out["alpha_fine_center"] = alpha_fine_center
        out["delta_fine_center"] = delta_fine_center
        out["alpha_coarse_center_offset"] = alpha - alpha_coarse_center
        out["delta_coarse_center_offset"] = delta - delta_coarse_center
        out["alpha_fine_center_offset"] = alpha - alpha_fine_center
        out["delta_fine_center_offset"] = delta - delta_fine_center
        ...
        def _neighbor_counts(a_bin, d_bin, a_size, d_size):
            ...
            neigh = np.zeros_like(self_count, dtype=np.float64)
            for da in (-1, 0, 1):
                for dd in (-1, 0, 1):
                    a2 = np.clip(a_i + da, 0, a_size - 1)
                    d2 = np.clip(d_i + dd, 0, d_size - 1)
                    neigh += counts[a2 * d_size + d2]
            return self_count, neigh

        coarse_self, coarse_neigh = _neighbor_counts(alpha_coarse, delta_coarse, 24, 18)
        fine_self, fine_neigh = _neighbor_counts(alpha_fine, delta_fine, 72, 90)
        ...
        gal_l_rad = np.deg2rad(gal_l)
        gal_b_rad = np.deg2rad(gal_b)

        out["gal_l"] = gal_l
        out["gal_b"] = gal_b
        out["gal_l_sin"] = np.sin(gal_l_rad)
        out["gal_l_cos"] = np.cos(gal_l_rad)
        out["gal_b_sin"] = np.sin(gal_b_rad)
        out["gal_b_cos"] = np.cos(gal_b_rad)
        out["gal_b_abs"] = np.abs(gal_b)

        gal_l_bin = np.clip((gal_l / 15.0).astype(np.int32), 0, 23)
        gal_b_bin = np.clip(((gal_b + 90.0) / 10.0).astype(np.int32), 0, 17)
        ...
        out["sky_radius"] = 1.0
        for k in range(2, 9):
            out[f"alpha_sin_{k}"] = 0.0
            out[f"alpha_cos_{k}"] = 1.0
            out[f"delta_sin_{k}"] = 0.0
            out[f"delta_cos_{k}"] = 1.0
        out["alpha_coarse_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        out["delta_coarse_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        out["alpha_fine_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        out["delta_fine_bin"] = pd.Series(0, index=out.index, dtype="Int16")
        out["alpha_coarse_center"] = 0.0
        out["delta_coarse_center"] = 0.0
        out["alpha_fine_center"] = 0.0
        out["delta_fine_center"] = 0.0
        out["alpha_coarse_center_offset"] = 0.0
        out["delta_coarse_center_offset"] = 0.0
        out["alpha_fine_center_offset"] = 0.0
        out["delta_fine_center_offset"] = 0.0
        out["sky_cell_offset_radius_coarse"] = 0.0
        out["sky_cell_offset_radius_fine"] = 0.0
        out["sky_cell_count_coarse"] = 0.0
        out["sky_cell_plus1_coarse"] = 0.0
        out["sky_cell_count_fine"] = 0.0
        out["sky_cell_plus1_fine"] = 0.0
        out["sky_density_ratio_coarse"] = 0.0
        out["sky_density_ratio_fine"] = 0.0
        out["gal_l"] = 0.0
        out["gal_b"] = 0.0
        out["gal_l_sin"] = 0.0
        out["gal_l_cos"] = 1.0
    ...
    out["spec_x_redshift_bin"] = spectral + "|" + out["redshift_bin"].astype("string")
    out["pop_x_redshift_bin"] = galaxy_pop + "|" + out["redshift_bin"].astype("string")
    out["spec_x_sky_cell"] = spectral + "|" + out["sky_cell_cat"]
    out["pop_x_sky_cell"] = galaxy_pop + "|" + out["sky_cell_cat"]
    out["spec_x_gal_cell"] = spectral + "|" + out["gal_cell_cat"]
    out["pop_x_gal_cell"] = galaxy_pop + "|" + out["gal_cell_cat"]
    ...
    if isinstance(aux, pd.DataFrame) and not aux.empty:
        ...
        for c in shared:
            a = pd.to_numeric(aux[c], errors="coerce").to_numpy(dtype=np.float64)
            if c in {"u", "g", "r", "i", "z"}:
                a = a[a > -9000]
            a = a[np.isfinite(a)]
            if a.size == 0:
            ...
            aux_mean = float(a.mean())
            aux_std = float(a.std()) or 1.0
            out[f"{c}_aux_q"] = np.searchsorted(
                a_sorted, np.clip(x, a_sorted.min(), a_sorted.max()), side="right"
            ) / len(a_sorted)
        ...
        for a, b in color_pairs:
            ...
            if a in {"u", "g", "r", "i", "z"}:
                ...
                av = av[m]
                bv = bv[m]
            m2 = np.isfinite(av) & np.isfinite(bv)
            av = av[m2]
            bv = bv[m2]
            color_aux = av - bv
            if color_aux.size == 0:
            ...
            color_sorted = np.sort(color_aux)
            color_mean = float(color_aux.mean())
            color_std = float(color_aux.std()) or 1.0
            c_val = out[color_name].to_numpy(dtype=np.float64)
            out[f"{color_name}_aux_q"] = np.searchsorted(
                color_sorted,
                np.clip(c_val, color_sorted.min(), color_sorted.max()),
                side="right",
            ) / len(color_sorted)
            out[f"{color_name}_aux_z"] = (c_val - color_mean) / color_std
