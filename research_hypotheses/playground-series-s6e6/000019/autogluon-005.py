...
def preprocess(df: "pd.DataFrame", aux: "pd.DataFrame") -> "pd.DataFrame":
    ...
    def _quant_bin(values, n_bins=10):
    ...
    if len(band_cols) == 5:
        ...
        band_array = np.array(band_cols, dtype=object)
        argmin_idx = np.argmin(safe_min, axis=1)
        argmax_idx = np.argmax(safe_max, axis=1)
        out["mag_min_band"] = pd.Series(
            np.where(finite_any, band_array[argmin_idx], np.nan),
            index=out.index,
            dtype="string",
        )
        out["mag_max_band"] = pd.Series(
            np.where(finite_any, band_array[argmax_idx], np.nan),
            index=out.index,
            dtype="string",
        )

    # 4) Redshift transforms + bins + interactions
    if "redshift" in out.columns:
        ...
        out["redshift_bin"] = pd.Series(
            _quant_bin(z, 10), index=out.index, dtype="Int16"
        )
    ...
    # 5) Sky geometry + harmonics + cells + local density
    ...
    out["pop_x_redshift_bin"] = population + "|" + out["redshift_bin"].astype("string")
    ...
    # 7) Frequency-of-covariate stats (train+test only)
    for col in [
        "alpha_coarse_bin",
        "delta_coarse_bin",
        "alpha_fine_bin",
    ...
    # 8) Percentile-rank covariates
    for col in ["u", "g", "r", "i", "z", "redshift"]:
        if col in out.columns:
            out[f"{col}_prank"] = out[col].rank(pct=True, method="average")

    # 9) Optional aux-derived reference-domain features (no row merge, no target use)
    if isinstance(aux, pd.DataFrame) and not aux.empty:
        ...
        aux_numeric = {}
        for c in shared:
            s = pd.to_numeric(aux[c], errors="coerce")
            if c in {"u", "g", "r", "i", "z"}:
                s = s.where(s > -9000, np.nan)
            s = s.replace([np.inf, -np.inf], np.nan).dropna()
            if not s.empty:
                aux_numeric[c] = s.to_numpy(float)

        for c in shared:
            a = aux_numeric.get(c)
            if a is None or len(a) == 0:
                continue
            a_sorted = np.sort(a.astype(float))
            ...
            out[f"{c}_aux_z"] = (x - mean) / std

        for a, b in color_pairs:
            color_name = f"{a}_{b}_color"
            if color_name not in out.columns:
                continue
            av = pd.to_numeric(aux["a" if False else a], errors="coerce").to_numpy(
                float
            )
            bv = pd.to_numeric(aux[b], errors="coerce").to_numpy(float)
            av = pd.Series(av)
            bv = pd.Series(bv)
            mask = np.ones(len(av), dtype=bool)
            if a in {"u", "g", "r", "i", "z"}:
                mask &= (av.to_numpy() > -9000) & (bv.to_numpy() > -9000)
            color_aux = (
                (av.where(mask) - bv.where(mask))
            ...
            out[f"{color_name}_aux_q"] = np.searchsorted(
                color_aux, np.clip(cval, color_aux.min(), color_aux.max()), side="right"
            ) / len(color_aux)
            mean = float(color_aux.mean())
            std = float(color_aux.std()) or 1.0
            out[f"{color_name}_aux_z"] = (cval - mean) / std
