...
def preprocess(df: "pd.DataFrame", aux: "pd.DataFrame") -> "pd.DataFrame":
    ...
    if "redshift" in out.columns:
        ...
        redshift_bin = pd.Series(np.full(len(out), -1, dtype="Int16"), index=out.index)
        if vals.size >= 2:
            ...
            if edges.size >= 3:
                ...
                redshift_bin = pd.Series(
                    np.full(len(out), -1, dtype="Int16"), index=out.index
                )
                redshift_bin.loc[out.index[fin]] = bins
    ...
    if isinstance(aux, pd.DataFrame) and not aux.empty:
        ...
        for c in [x for x in aux_cols if x in out.columns and x in aux.columns]:
