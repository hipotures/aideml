...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    out["spectral_type_code"] = spectral.map(
        {"M": 0.0, "O/B": 1.0, "G/K": 2.0, "A/F": 3.0}
    ).fillna(-1.0)
    out["galaxy_population_code"] = population.map(
        {"Blue_Cloud": 0.0, "Red_Sequence": 1.0}
    ).fillna(-1.0)

    for a, b in color_pairs:
        if a in out.columns and b in out.columns:
            v = out[a].to_numpy(dtype=float) - out[b].to_numpy(dtype=float)
            out[f"{a}_{b}_color"] = v
            out[f"{a}_{b}_color_abs"] = np.abs(v)

    if all(c in out.columns for c in mag_cols):
        mags = out[mag_cols].to_numpy(dtype=float)
        finite = np.isfinite(mags)
        row_mean = np.nanmean(np.where(finite, mags, np.nan), axis=1)
        row_mean0 = np.where(np.isfinite(row_mean), row_mean, 0.0)
        mags_fill = np.where(finite, mags, row_mean0[:, None])

        out["mag_mean"] = np.nanmean(mags_fill, axis=1).astype(np.float32)
        out["mag_std"] = np.nanstd(mags_fill, axis=1).astype(np.float32)
        out["mag_range"] = (
            np.nanmax(mags_fill, axis=1) - np.nanmin(mags_fill, axis=1)
        ).astype(np.float32)
        out["mag_min"] = np.nanmin(mags_fill, axis=1).astype(np.float32)
        out["mag_max"] = np.nanmax(mags_fill, axis=1).astype(np.float32)

        x = np.arange(5.0, dtype=float)
        xbar = 2.0
        denom = np.sum((x - xbar) ** 2.0)
        centered = mags_fill - row_mean0[:, None]
        slope = np.sum(centered * (x - xbar), axis=1) / (denom if denom != 0 else 1.0)
        intercept = row_mean0 - slope * xbar
        pred = slope[:, None] * x[None, :] + intercept[:, None]
        resid = mags_fill - pred

        out["mag_spectral_slope"] = slope.astype(np.float32)
        out["mag_resid_mean_abs"] = np.nanmean(np.abs(resid), axis=1).astype(np.float32)
        out["mag_resid_std"] = np.nanstd(resid, axis=1).astype(np.float32)

        d1 = np.diff(mags_fill, axis=1)
        d2 = np.diff(d1, axis=1)
        out["mag_d1_mean"] = np.nanmean(d1, axis=1).astype(np.float32)
        out["mag_d1_std"] = np.nanstd(d1, axis=1).astype(np.float32)
        out["mag_d1_abs_mean"] = np.nanmean(np.abs(d1), axis=1).astype(np.float32)
        out["mag_d2_mean"] = np.nanmean(d2, axis=1).astype(np.float32)
        out["mag_d2_std"] = np.nanstd(d2, axis=1).astype(np.float32)
        out["mag_d2_abs_mean"] = np.nanmean(np.abs(d2), axis=1).astype(np.float32)

        safe_min = np.where(finite, mags, np.inf)
        safe_max = np.where(finite, mags, -np.inf)
        band_array = np.array(mag_cols, dtype=object)
        out["mag_min_band"] = pd.Series(
            band_array[np.argmin(safe_min, axis=1)], index=out.index, dtype="string"
        )
        out["mag_max_band"] = pd.Series(
            band_array[np.argmax(safe_max, axis=1)], index=out.index, dtype="string"
        )
    ...
    if "redshift" in out.columns:
        z = out["redshift"].to_numpy(dtype=float)
        z_abs = np.abs(z)
        z_abs_log = np.log1p(z_abs)
        z_signed_log = np.sign(z) * z_abs_log

        out["redshift_abs"] = z_abs.astype(np.float32)
        out["redshift_sq"] = (z * z).astype(np.float32)
        out["redshift_log1p_abs"] = z_abs_log.astype(np.float32)
        out["redshift_signed_log1p_abs"] = z_signed_log.astype(np.float32)

        z_finite = np.isfinite(z)
        redshift_bin = np.full(len(out), -1, dtype=np.int16)
        if z_finite.sum() >= 2:
            edges = np.quantile(z[z_finite], np.linspace(0.0, 1.0, 11))
            edges = np.unique(np.round(edges, 12))
            if edges.size >= 3:
                redshift_bin[z_finite] = np.searchsorted(
                    edges[1:-1], z[z_finite], side="right"
                ).astype(np.int16)
            else:
                lo = float(np.nanmin(z[z_finite]))
                hi = float(np.nanmax(z[z_finite]))
                if np.isfinite(lo) and np.isfinite(hi) and hi > lo:
                    bins = np.linspace(lo, hi, 12)
                    redshift_bin[z_finite] = np.searchsorted(
                        bins[1:-1], z[z_finite], side="right"
                    ).astype(np.int16)
        out["redshift_bin"] = redshift_bin.astype(np.int16)
        out["redshift_abs_x_spectral_type_code"] = (
            out["redshift_abs"] * out["spectral_type_code"]
        )
        out["redshift_abs_x_galaxy_population_code"] = (
            out["redshift_abs"] * out["galaxy_population_code"]
        )
        out["redshift_x_spectral_type_code"] = (
            out["redshift"] * out["spectral_type_code"]
        )
        out["redshift_x_galaxy_population_code"] = (
            out["redshift"] * out["galaxy_population_code"]
        )
        out["redshift_signed_log1p_abs_x_spectral_type_code"] = (
            out["redshift_signed_log1p_abs"] * out["spectral_type_code"]
        )
        out["redshift_signed_log1p_abs_x_galaxy_population_code"] = (
    ...
    if "alpha" in out.columns and "delta" in out.columns:
        ...
        out["alpha_sin"] = asn.astype(np.float32)
        out["alpha_cos"] = acos.astype(np.float32)
        out["delta_sin"] = dsn.astype(np.float32)
        out["delta_cos"] = dcos.astype(np.float32)

        sx = dcos * acos
        sy = dcos * asn
        sz = dsn

        out["sky_x"] = sx.astype(np.float32)
        out["sky_y"] = sy.astype(np.float32)
        out["sky_z"] = sz.astype(np.float32)
        out["sky_xy"] = (sx * sy).astype(np.float32)
        out["sky_xz"] = (sx * sz).astype(np.float32)
        out["sky_yz"] = (sy * sz).astype(np.float32)
        out["sky_rxy"] = np.hypot(sx, sy).astype(np.float32)
        out["sky_radius"] = np.sqrt((sx * sx + sy * sy + sz * sz)).astype(np.float32)

        for k in range(2, 9):
            out[f"alpha_sin_{k}"] = np.sin(k * ra).astype(np.float32)
            out[f"alpha_cos_{k}"] = np.cos(k * ra).astype(np.float32)
            out[f"delta_sin_{k}"] = np.sin(k * dc).astype(np.float32)
            out[f"delta_cos_{k}"] = np.cos(k * dc).astype(np.float32)
        ...
        def _cell_stats(a_bins, d_bins, a_size: int, d_size: int):
            a_i = np.asarray(a_bins, dtype=np.int64)
            d_i = np.asarray(d_bins, dtype=np.int64)
            flat = a_i * d_size + d_i
            counts = np.bincount(flat, minlength=a_size * d_size).astype(np.float32)
            self_count = counts[flat]
            neigh = np.zeros_like(self_count, dtype=np.float32)
            for da in (-1, 0, 1):
                aa = np.clip(a_i + da, 0, a_size - 1)
                base = aa * d_size
                for dd in (-1, 0, 1):
                    dd_i = np.clip(d_i + dd, 0, d_size - 1)
                    neigh += counts[base + dd_i]
            return self_count, neigh

        self_c, neigh_c = _cell_stats(a_coarse, d_coarse, 24, 18)
        self_f, neigh_f = _cell_stats(a_fine, d_fine, 72, 90)
        out["sky_cell_count_coarse"] = self_c
        out["sky_cell_plus1_coarse"] = neigh_c
        out["sky_cell_count_fine"] = self_f
        out["sky_cell_plus1_fine"] = neigh_f
        out["sky_density_ratio_coarse"] = (neigh_c / np.maximum(self_c, 1.0)).astype(
            np.float32
        )
        out["sky_density_ratio_fine"] = (neigh_f / np.maximum(self_f, 1.0)).astype(
            np.float32
        )

        gx = -0.0548755604 * sx + -0.8734370902 * sy + -0.4838350155 * sz
        gy = 0.4941094279 * sx + -0.4448296300 * sy + 0.7469822445 * sz
        gz = -0.8676661490 * sx + -0.1980763734 * sy + 0.4559837762 * sz

        gal_l = (np.degrees(np.arctan2(gy, gx)) + 360.0) % 360.0
        gal_b = np.degrees(np.arcsin(np.clip(gz, -1.0, 1.0)))
        out["gal_l"] = gal_l.astype(np.float32)
        out["gal_b"] = gal_b.astype(np.float32)
        out["gal_l_sin"] = np.sin(np.deg2rad(gal_l)).astype(np.float32)
        out["gal_l_cos"] = np.cos(np.deg2rad(gal_l)).astype(np.float32)
        out["gal_b_sin"] = np.sin(np.deg2rad(gal_b)).astype(np.float32)
        out["gal_b_cos"] = np.cos(np.deg2rad(gal_b)).astype(np.float32)
        out["gal_b_abs"] = np.abs(gal_b).astype(np.float32)

        gl_bin = np.clip((gal_l / 15.0).astype(np.int16), 0, 23)
        gb_bin = np.clip(((gal_b + 90.0) / 10.0).astype(np.int16), 0, 17)
        out["gal_l_bin"] = pd.Series(gl_bin, index=out.index, dtype="int16")
        out["gal_b_bin"] = pd.Series(gb_bin, index=out.index, dtype="int16")
        out["gal_l_center"] = (gl_bin + 0.5) * 15.0
        out["gal_b_center"] = (gb_bin + 0.5) * 10.0 - 90.0
        out["gal_l_offset"] = gal_l - out["gal_l_center"]
        out["gal_b_offset"] = gal_b - out["gal_b_center"]

    out["spec_x_pop"] = spectral.astype("string") + "|" + population.astype("string")
    if "alpha_coarse_bin" in out.columns:
        out["sky_cell_cat"] = (
            "a"
        ...
        out["sky_cell_cat"] = pd.Series("na", index=out.index, dtype="string")
    if "gal_l_bin" in out.columns and "gal_b_bin" in out.columns:
        ...
        out["gal_cell_cat"] = pd.Series("na", index=out.index, dtype="string")
    if "redshift_bin" in out.columns:
        out["spec_x_redshift_bin"] = (
            spectral.astype("string") + "_z" + out["redshift_bin"].astype("string")
        )
        out["pop_x_redshift_bin"] = (
            population.astype("string") + "_z" + out["redshift_bin"].astype("string")
        )
    else:
        out["spec_x_redshift_bin"] = pd.Series("na", index=out.index, dtype="string")
        out["pop_x_redshift_bin"] = pd.Series("na", index=out.index, dtype="string")

    out["spec_x_sky_cell"] = (
        spectral.astype("string") + "|" + out["sky_cell_cat"].astype("string")
    )
    out["pop_x_sky_cell"] = (
        population.astype("string") + "|" + out["sky_cell_cat"].astype("string")
    )
    out["spec_x_gal_cell"] = (
        spectral.astype("string") + "|" + out["gal_cell_cat"].astype("string")
    )
    out["pop_x_gal_cell"] = (
        population.astype("string") + "|" + out["gal_cell_cat"].astype("string")
    )
    ...
    for c in freq_cols:
        if c in out.columns:
            ...
            out[f"{c}_freq"] = s.map(freq).astype(np.float32)
    ...
    if isinstance(aux, pd.DataFrame) and len(aux) > 0:
        aux_cols = [
            c
            for c in ["alpha", "delta", "u", "g", "r", "i", "z", "redshift"]
            if c in aux.columns and c in out.columns
        ]
        aux_ref = {}
        ...
        for c in aux_cols:
            ...
            a = a.replace([np.inf, -np.inf], np.nan).dropna().to_numpy(dtype=float)
            if a.size == 0:
                continue
            a = np.sort(a)
            x = out[c].to_numpy(dtype=float)
            x_clip = np.clip(x, a[0], a[-1])
            out[f"{c}_aux_prank"] = np.searchsorted(a, x_clip, side="right") / float(
                a.size
            )
            ...
            if not np.isfinite(mad) or mad == 0.0:
                mad = 1.0
            out[f"{c}_aux_z"] = (x - med) / (1.4826 * mad)
        ...
        for a, b in color_pairs:
            ...
            if (
                color_name not in out.columns
                or a not in aux.columns
                or b not in aux.columns
            ):
                continue
            ...
            v = out[color_name].to_numpy(dtype=float)
            v_clip = np.clip(v, color_aux[0], color_aux[-1])
            out[f"{color_name}_aux_prank"] = np.searchsorted(
                color_aux, v_clip, side="right"
            ) / float(color_aux.size)
            med = float(np.nanmedian(color_aux))
            mad = float(np.nanmedian(np.abs(color_aux - med)))
            if not np.isfinite(mad) or mad == 0.0:
                mad = 1.0
            out[f"{color_name}_aux_z"] = (v - med) / (1.4826 * mad)

        if "mag_extremum_cat" in out.columns and "spectral_type" in out.columns:
            out["spec_x_extrema_mag"] = (
                spectral.astype("string")
                + "|"
                + out["mag_extremum_cat"].astype("string")
            )
        ...
        if len(loc_cols) >= 4:
            med = np.array([aux_ref[c][0] for c in loc_cols], dtype=float)
            mad = np.array(
                [aux_ref[c][1] if aux_ref[c][1] != 0 else 1.0 for c in loc_cols],
                dtype=float,
            )

            X_aux = aux[loc_cols].to_numpy(dtype=float)
            if np.isfinite(X_aux).any():
                keep = np.isfinite(X_aux).all(axis=1)
                X_aux = X_aux[keep]
                if X_aux.shape[0] >= 200:
                    X_aux = (X_aux - med) / mad
                    n_samples = X_aux.shape[0]
                    cap = 8000
                    if n_samples > cap:
                        idx = np.linspace(0, n_samples - 1, num=cap, dtype=int)
                        X_aux = X_aux[idx]

                    Xq = (out[loc_cols].to_numpy(dtype=float) - med) / mad
                    Xq = np.nan_to_num(Xq, nan=9e9, posinf=9e9, neginf=-9e9)
                    nn = NearestNeighbors(n_neighbors=1, algorithm="kd_tree", n_jobs=-1)
                    nn.fit(X_aux)
                    dist = np.empty(len(out), dtype=float)
                    qn = len(out)
                    step = 25000
                    for s in range(0, qn, step):
                        e = min(qn, s + step)
                        d, _ = nn.kneighbors(
                            Xq[s:e], return_distance=True, n_neighbors=1
                        )
                        dist[s:e] = d[:, 0]
                    dist = np.clip(dist, 0.0, 20.0)
                    out["aux_knn_dist"] = dist.astype(np.float32)
                    out["aux_knn_w"] = np.exp(-dist).astype(np.float32)
