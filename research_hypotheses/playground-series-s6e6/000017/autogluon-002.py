...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    out["color_g_z"] = (out["g"] - out["z"]).astype("float32")

    cat_df = (
        out[["galaxy_population", "spectral_type"]].fillna("Unknown").astype("string")
    )
    cat_onehot = pd.get_dummies(cat_df, dtype="float32")
    ...
    X = unsup_df.to_numpy(dtype=np.float32, copy=False)
    ...
    for i in range(pca_scores.shape[1]):
        out[f"unsup_pca_{i + 1}"] = pca_scores[:, i].astype("float32")

    n_clusters = int(max(8, min(24, max(3, n_rows // 35000))))
    km = MiniBatchKMeans(
    ...
    cluster_dist = km.fit_transform(X_scaled)

    nearest_cluster = np.argmin(cluster_dist, axis=1)
    nearest_dist = cluster_dist[np.arange(n_rows), nearest_cluster]
    out["unsup_cluster_id"] = nearest_cluster.astype("int16")
    out["unsup_cluster_dist_1"] = nearest_dist.astype("float32")

    if n_clusters > 1:
        top2 = np.partition(cluster_dist, 1, axis=1)
        second_dist = top2[:, 1]
    else:
        second_dist = nearest_dist.copy()
    out["unsup_cluster_dist_2"] = second_dist.astype("float32")
    out["unsup_cluster_margin_1_2"] = (second_dist - nearest_dist).astype("float32")

    cluster_counts = np.bincount(nearest_cluster, minlength=n_clusters).astype(
        "float32"
    )
    cluster_freq = cluster_counts / float(n_rows)
    out["unsup_cluster_train_test_freq"] = cluster_freq[nearest_cluster]

    ks = (10, 25, 50)
    max_k = max(ks)
    ref_size = min(20000, n_rows)
    rng = np.random.RandomState(0)
    ref_idx = rng.choice(n_rows, size=ref_size, replace=False)
    X_ref = X_scaled[ref_idx]
    k_nn = min(max_k, ref_size)
    nn = NearestNeighbors(n_neighbors=k_nn, metric="euclidean", n_jobs=-1)
    nn.fit(X_ref)
    dists, _ = nn.kneighbors(X_scaled, n_neighbors=k_nn, return_distance=True)
    dists = np.asarray(dists, dtype=np.float32)

    for k in ks:
        kk = min(k, k_nn)
        out[f"unsup_nn_mean_dist_k{kk}"] = dists[:, :kk].mean(axis=1)
        out[f"unsup_nn_median_dist_k{kk}"] = np.median(dists[:, :kk], axis=1).astype(
            "float32"
        )
        out[f"unsup_nn_inv_density_k{kk}"] = 1.0 / (
            1.0 + out[f"unsup_nn_mean_dist_k{kk}"]
        )

    out["unsup_nn_min_dist"] = dists[:, 0]
    return out
