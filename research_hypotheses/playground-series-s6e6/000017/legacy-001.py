...
def nearest_margin_features(distances: np.ndarray, prefix: str) -> pd.DataFrame:
    ...
    margin = smallest[:, 1] - smallest[:, 0]
    ratio = smallest[:, 0] / np.maximum(smallest[:, 1], 1e-6)
    return pd.DataFrame(
        {
            f"{prefix}_nearest": smallest[:, 0].astype(np.float32),
            f"{prefix}_second": smallest[:, 1].astype(np.float32),
            f"{prefix}_margin": margin.astype(np.float32),
            f"{prefix}_ratio": ratio.astype(np.float32),
        }
    )
...
def build_full_feature_matrix(train: pd.DataFrame, test: pd.DataFrame):
    ...
    cat_matrix = cat_df.to_numpy(dtype=np.float32, copy=False)
    unsup_input = np.concatenate([scaled_num, cat_matrix], axis=1).astype(
    ...
    km12.fit(pca_features)
    dist12 = km12.transform(pca_features).astype(np.float32)
    ...
    km24.fit(pca_features)
    dist24 = km24.transform(pca_features).astype(np.float32)
    ...
    blocks = [
        ...
        pd.DataFrame(aff12, columns=[f"km12_aff_{i}" for i in range(aff12.shape[1])]),
        pd.DataFrame(
            dist24, columns=[f"km24_dist_{i}" for i in range(dist24.shape[1])]
        ),
        margin12.reset_index(drop=True),
        margin24.reset_index(drop=True),
        density_df.reset_index(drop=True),
    ]
    feature_df = pd.concat(blocks, axis=1).astype(np.float32)
    ...
    del combined, num_df, cat_df, scaled_num, cat_matrix, unsup_input
    del (
        pca_features,
        dist12,
        aff12,
...
def summarize_predictions(y_true_labels, pred_labels, class_names, model_name):
    ...
    print(
        f"{model_name} balanced_accuracy={bal_acc:.6f}, {recall_text}",
        flush=True,
    )
    return bal_acc, recalls
...
def main():
    ...
    with aide_stage("fit_predict_fold_stage"):
        for model_name in model_names:
            ...
            score, recalls = summarize_predictions(
    ...
    with aide_stage("score_stage"):
        ...
        best_score, _ = summarize_predictions(
            y_true_labels=y_labels,
            pred_labels=best_oof_pred,
            class_names=class_names,
            model_name=f"selected_{best_model_name}",
        )
