...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pairwise color indices from ugriz magnitudes.
    color_pairs = [
        ("u", "g"),
        ("g", "r"),
        ("r", "i"),
    ...
    for left, right in color_pairs:
        out[f"color_{left}_{right}"] = out[left] - out[right]

    # Adjacent-band spectral slopes (Δmag / Δeffective wavelength) using SDSS-like effective wavelengths.
    wave = {"u": 355.1, "g": 468.6, "r": 616.5, "i": 748.9, "z": 893.1}
    slope_pairs = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    for lo, hi in slope_pairs:
        out[f"slope_{lo}_{hi}"] = (out[hi] - out[lo]) / (wave[hi] - wave[lo])

    # Brightness summaries across ugriz.
    mags = out[["u", "g", "r", "i", "z"]]
    out["ugriz_mean"] = mags.mean(axis=1)
    out["ugriz_std"] = mags.std(axis=1, ddof=0)
    out["ugriz_min"] = mags.min(axis=1)
    out["ugriz_max"] = mags.max(axis=1)
    out["ugriz_range"] = out["ugriz_max"] - out["ugriz_min"]

    # Nonlinear color-gap transforms.
    for c in ["color_u_g", "color_g_r", "color_r_i", "color_i_z"]:
        series = out[c]
        out[f"{c}_abs"] = series.abs()
        out[f"{c}_sq"] = series * series
        out[f"{c}_inv"] = 1.0 / (series.abs() + 1e-3)

    # Robust redshift nonlinear summaries.
    rz = out["redshift"].clip(lower=0.0)
    out["redshift_pos"] = rz
    out["redshift_log1p"] = np.log1p(rz)
    out["redshift_sq"] = rz * rz
    out["redshift_tanh"] = np.tanh(rz)

    # Deterministic z-score variants for numeric block features.
    scale_cols = [
        "u",
        "g",
        "r",
