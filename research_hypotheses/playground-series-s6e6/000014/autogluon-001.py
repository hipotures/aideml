...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["u_g", "g_r", "r_i", "i_z"]:
        if c in out.columns:
            ...
            out[f"{c}_sq"] = np.square(out[c]).astype("float32")
            out[f"{c}_blue_flag"] = (out[c] < 0).astype("int8")
            if "redshift" in out.columns:
                out[f"redshift_x_{c}"] = (_num(out, "redshift") * out[c]).astype("float32")
    if all(c in out.columns for c in ["u_g", "g_r"]):
        out["color_curve_blue"] = (out["u_g"] - out["g_r"]).astype("float32")
    if all(c in out.columns for c in ["r_i", "i_z"]):
        out["color_curve_red"] = (out["r_i"] - out["i_z"]).astype("float32")
    return out
