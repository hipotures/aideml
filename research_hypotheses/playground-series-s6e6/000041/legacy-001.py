# Hypothesis 000031: add a regularized second LightGBM expert and extend blend search to four models
...
def fit_lgbm_with_fallback(X_tr, y_tr, X_val, y_val, sample_weight, config=None, model_label="LightGBM"):
    if not HAS_LGBM:
        return None
    cfg = LGBM_CONFIG if config is None else config
    attempts = []
    ...
    for label, device_params in attempts:
        ...
        try:
            ...
            if label == "cuda":
                print(
                    f"{model_label} CUDA fit failed ({type(exc).__name__}); falling back to CPU.",
                    flush=True,
                )
            try:
...
def generate_simplex_candidates(n_models, step):
    ...
    def rec(prefix, remaining, slots_left):
        if slots_left == 1:
            candidates.append(tuple(float(x) / float(units) for x in prefix + [remaining]))
            return
...
def generate_local_simplex_candidates(
    ...
    if center.ndim != 1 or len(center) <= 1 or not np.isfinite(center).all():
        return []
    ...
    n_models = len(center)
    step = max(float(step), 1e-6)
    radius = max(float(radius), step)
    deltas = np.round(np.arange(-radius, radius + 1e-12, step), 10)
    candidates = set()

    def rec(dim, prefix):
        if dim == n_models - 1:
            first = center[: n_models - 1] + np.asarray(prefix, dtype=float)
            last = 1.0 - float(first.sum())
            weights = np.concatenate([first, np.asarray([last], dtype=float)])
            if np.all(weights >= -1e-12) and np.all(weights <= 1.0 + 1e-12):
                weights = np.clip(weights, 0.0, 1.0)
                weights = weights / weights.sum()
                candidates.add(tuple(float(np.round(w, 10)) for w in weights))
            return
        for d in deltas:
            rec(dim + 1, prefix + [float(d)])
...
def main():
    ...
    model_names = ["catboost", "xgboost", "lightgbm", "lightgbm_reg"]
    oof_probs = {
    ...
    model_available = {"catboost": True, "xgboost": HAS_XGB, "lightgbm": HAS_LGBM, "lightgbm_reg": HAS_LGBM}
    ...
    with aide_stage("fit_predict_fold_stage"):
        for fold_idx, (tr_pos, va_pos) in enumerate(folds, start=1):
            ...
            log_stage(
                f"event=progress|stage=fit_predict_fold_stage|fold={fold_idx}|model=lightgbm_reg"
            )
            start = time.perf_counter()
            if model_available["lightgbm_reg"]:
                lgb_reg_model = fit_lgbm_with_fallback(
                    X_tr_tree,
                    y_tr_code,
                    X_va_tree,
                    y_va_code,
                    tree_sw,
                    config=LGBM_REG_CONFIG,
                    model_label="LightGBM regularized",
                )
                if lgb_reg_model is None:
                    model_available["lightgbm_reg"] = False
                else:
                    lgb_reg_val_prob = align_numeric_proba(
                        lgb_reg_model.predict_proba(X_va_tree),
                        lgb_reg_model.classes_,
                        num_classes,
                    )
                    lgb_reg_test_prob = align_numeric_proba(
                        lgb_reg_model.predict_proba(test_tree),
                        lgb_reg_model.classes_,
                        num_classes,
                    )
                    oof_probs["lightgbm_reg"][main_va_idx] = lgb_reg_val_prob
                    fold_record["oof"]["lightgbm_reg"] = lgb_reg_val_prob
                    fold_record["test"]["lightgbm_reg"] = lgb_reg_test_prob
                    score = score_predictions(y_va, lgb_reg_val_prob, class_order)
                    elapsed = time.perf_counter() - start
                    fold_scores["lightgbm_reg"].append(score)
                    fold_times["lightgbm_reg"].append(elapsed)
                    print(
                        f"Fold {fold_idx} | lightgbm_reg | balanced_accuracy={score:.6f} | runtime_s={elapsed:.1f}",
                        flush=True,
                    )
            if not model_available["lightgbm_reg"]:
                print(
                    f"Fold {fold_idx} | lightgbm_reg | unavailable_after_failure | runtime_s={time.perf_counter() - start:.1f}",
                    flush=True,
                )
