...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for b, lim in limits.items():
        if b in out.columns:
            margin = lim - _num(out, b)
            out[f"{b}_depth_margin"] = margin.astype("float32")
            out[f"{b}_is_fainter_than_limit"] = (_num(out, b) > lim).astype("int8")
            margins.append(margin)
    if margins:
        mat = np.column_stack(margins)
        out["depth_margin_min"] = mat.min(axis=1).astype("float32")
        out["depth_margin_mean"] = mat.mean(axis=1).astype("float32")
        out["overlimit_count_all_bands"] = (mat < 0).sum(axis=1).astype("int8")
    _color_block(out)
    return out
