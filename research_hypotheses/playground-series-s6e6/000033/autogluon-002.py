...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    enable_reliability_block = True
    if enable_reliability_block:
        limits = {"u": 22.0, "g": 22.2, "r": 22.2, "i": 21.3, "z": 20.5}
        margin_scale = 2.5

        band_cols = []
        rel_cols = []
        for band, limit in limits.items():
            mag = pd.to_numeric(out[band], errors="coerce")
            margin_col = f"{band}_depth_margin"
            over_col = f"{band}_over_limit"
            rel_col = f"{band}_rel_score"

            out[margin_col] = limit - mag
            out[over_col] = (mag > limit).astype(np.int8)
            out[rel_col] = (out[margin_col] / margin_scale).clip(lower=0.0, upper=1.0)

            band_cols.append(margin_col)
            rel_cols.append(rel_col)

        # Bitmask-style and grouped over-limit summaries (blue= u,g / middle= r,i / red= z)
        out["ugriz_faint_bitmask"] = (
            out["u_over_limit"].astype(int)
            + 2 * out["g_over_limit"].astype(int)
            + 4 * out["r_over_limit"].astype(int)
            + 8 * out["i_over_limit"].astype(int)
            + 16 * out["z_over_limit"].astype(int)
        ).astype(np.int16)
        out["faint_blue_count"] = out["u_over_limit"].astype(int) + out[
            "g_over_limit"
        ].astype(int)
        out["faint_middle_count"] = out["r_over_limit"].astype(int) + out[
            "i_over_limit"
        ].astype(int)
        out["faint_red_count"] = out["z_over_limit"].astype(int)
        out["faint_band_count"] = (
            out["u_over_limit"]
            + out["g_over_limit"]
            + out["r_over_limit"]
            + out["i_over_limit"]
        ...
        out["blue_faint_mask"] = out["u_over_limit"].astype(int) + 2 * out[
            "g_over_limit"
        ].astype(int)
        out["middle_faint_mask"] = out["r_over_limit"].astype(int) + 2 * out[
            "i_over_limit"
        ].astype(int)
        out["red_faint_mask"] = out["z_over_limit"].astype(int)

        # Bright/saturation and mid-quality caution flags
        out["u_bright_flag"] = (pd.to_numeric(out["u"], errors="coerce") < 16.0).astype(
            np.int8
        )
        out["gri_bright_flag"] = (
            (pd.to_numeric(out["g"], errors="coerce") < 14.5)
            | (pd.to_numeric(out["r"], errors="coerce") < 14.5)
            | (pd.to_numeric(out["i"], errors="coerce") < 14.5)
        ).astype(np.int8)

        out["u_mid_quality_flag"] = (
            pd.to_numeric(out["u"], errors="coerce").between(19.0, 20.0)
        ).astype(np.int8)
        out["g_mid_quality_flag"] = (
            pd.to_numeric(out["g"], errors="coerce").between(19.0, 20.0)
        ).astype(np.int8)
        out["r_mid_quality_flag"] = (
            pd.to_numeric(out["r"], errors="coerce").between(19.0, 20.0)
        ).astype(np.int8)
        out["i_mid_quality_flag"] = (
            pd.to_numeric(out["i"], errors="coerce").between(19.0, 20.0)
        ).astype(np.int8)
        out["mid_quality_band_count"] = (
            out["u_mid_quality_flag"]
            + out["g_mid_quality_flag"]
            + out["r_mid_quality_flag"]
            + out["i_mid_quality_flag"]
        ...
        # Margin aggregate reliability summaries
        out["depth_margin_min"] = out[band_cols].min(axis=1)
        out["depth_margin_mean"] = out[band_cols].mean(axis=1)
        out["depth_margin_std"] = out[band_cols].std(axis=1, ddof=0)

        # Reliability-weighted colors
        color_pairs = [
            ("u", "g"),
            ("g", "r"),
            ("r", "i"),
        ...
        for left, right in color_pairs:
            color = pd.to_numeric(out[left], errors="coerce") - pd.to_numeric(
                out[right], errors="coerce"
            )
            reliability = np.minimum(
                out[f"{left}_rel_score"], out[f"{right}_rel_score"]
            )
            out[f"{left}_{right}_color"] = color
            out[f"{left}_{right}_color_reliable"] = color * reliability

        # Keep a compact overall reliability score for the object
        out["reliability_band_score"] = np.minimum.reduce(
            [
                out["u_rel_score"],
                out["g_rel_score"],
                out["r_rel_score"],
        ...
        out["faintness_penalty"] = (-out["depth_margin_min"]).clip(lower=0.0)

    cat_cols = [c for c in ["spectral_type", "galaxy_population"] if c in out.columns]
    if cat_cols:
        out = pd.get_dummies(out, columns=cat_cols, dtype="int8")
