...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame | None = None) -> pd.DataFrame:
    drop_cols = {
        "alpha_missing",
        "delta_missing",
        "redshift_missing",
        "spectral_type_missing",
        "galaxy_population_missing",
        "mag_sentinel_count",
        "mag_sentinel_any",
        "mag_sentinel_rate",
        "mag_missing_count",
        "mag_curvature_sign",
        "mag_quality_state_code",
        "mag_quality_state_code_all_clean",
        "mag_quality_state_code_nonzero",
        "aux_color_ref_valid",
        "sky_eqarea_valid",
        "sky_galactic_valid",
        "aux_gal_b_abs_pct_valid",
        "alpha_filled",
        "delta_filled",
        "u_filled",
        "g_filled",
    ...
    band_missing_like = {
        ...
        if (
            (
                c.startswith(("u_", "g_", "r_", "i_", "z_"))
                and ("missing" in c or "sentinel" in c)
            )
            or ("band" in c and ("missing" in c or "sentinel" in c))
        )
    }

    drop_cols.update(band_missing_like)
