...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(col in out.columns for col in required):
        return out
    if not isinstance(aux, pd.DataFrame) or aux.empty or not all(col in aux.columns for col in required):
        return out

    def find_aux_label_column(frame: pd.DataFrame) -> str | None:
        expected = {"GALAXY", "STAR", "QSO"}
        for col in frame.columns:
            ...
            if normalized.empty:
                continue
            hits = set(normalized.unique()) & expected
            if len(hits) >= 2:
                return col
        return None

    label_col = find_aux_label_column(aux)
    if label_col is None:
        return out

    def build_space(frame: pd.DataFrame) -> pd.DataFrame:
        work = pd.DataFrame(index=frame.index)
        for col in required:
            values = pd.to_numeric(frame[col], errors="coerce").mask(lambda s: s <= -9000)
            med = values.median()
            work[col] = values.fillna(0.0 if pd.isna(med) else med)
        work["blue"] = work["u"] - work["r"]
        work["red"] = work["r"] - work["z"]
        work["rz"] = np.log1p(np.maximum(work["redshift"], 0.0))
        return work[["blue", "red", "rz"]]

    aux_space = build_space(aux)
    row_space = build_space(out)
    labels = aux[label_col].astype(str).str.upper().str.strip()
    valid = labels.isin(["GALAXY", "STAR", "QSO"])
    aux_space = aux_space.loc[valid]
    labels = labels.loc[valid]
    if aux_space.empty:
        return out

    bin_counts = {"blue": 16, "red": 16, "rz": 12}
    edges: dict[str, np.ndarray] = {}
    for col, count in bin_counts.items():
        quantiles = np.linspace(0.0, 1.0, count + 1)
        edge = np.unique(np.nanquantile(aux_space[col].to_numpy(dtype="float64"), quantiles))
        if edge.size < 3:
            lo = float(np.nanmin(aux_space[col]))
            hi = float(np.nanmax(aux_space[col]))
            if not np.isfinite(lo) or not np.isfinite(hi) or lo == hi:
                lo, hi = -1.0, 1.0
            edge = np.linspace(lo, hi, count + 1)
        edges[col] = edge

    def encode(space: pd.DataFrame) -> pd.Series:
        parts = []
        for col in ["blue", "red", "rz"]:
            bins = np.searchsorted(edges[col], space[col].to_numpy(dtype="float64"), side="right") - 1
            bins = np.clip(bins, 0, len(edges[col]) - 2)
            parts.append(pd.Series(bins.astype("int16"), index=space.index).astype(str))
        return parts[0] + "_" + parts[1] + "_" + parts[2]

    aux_key = encode(aux_space)
    row_key = encode(row_space)
    counts = pd.crosstab(aux_key, labels).reindex(columns=["GALAXY", "STAR", "QSO"], fill_value=0)
    smoothed = counts.astype("float64") + 2.0
    priors = smoothed.div(smoothed.sum(axis=1), axis=0)
    support = counts.sum(axis=1)

    mapped = priors.reindex(row_key).reset_index(drop=True).fillna(1.0 / 3.0)
    for label in ["GALAXY", "STAR", "QSO"]:
        out[f"aux_grid_prior_{label}"] = mapped[label].astype("float32")
    mapped_support = support.reindex(row_key).reset_index(drop=True).fillna(0.0)
    out["aux_grid_prior_support_log1p"] = np.log1p(mapped_support).astype("float32")
    out["aux_grid_prior_qso_margin"] = (
        mapped["QSO"] - mapped[["GALAXY", "STAR"]].max(axis=1)
    ).astype("float32")
    out["aux_grid_prior_entropy"] = (
        -(mapped * np.log(np.maximum(mapped, 1e-9))).sum(axis=1)
    ).astype("float32")
    return out
