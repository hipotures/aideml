...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    mag = out[mag_cols]
    band_medians = mag.median(axis=0)

    faint = mag - band_medians
    faint_clipped = faint.clip(lower=-5.0, upper=5.0)
    noise_proxy = np.exp(0.4 * faint_clipped).clip(lower=0.05, upper=20.0)

    noise_cols = [f"phot_noise_proxy_{c}" for c in mag_cols]
    out.loc[:, noise_cols] = noise_proxy.values

    out["phot_noise_proxy_mean"] = noise_proxy.mean(axis=1)
    out["phot_noise_proxy_max"] = noise_proxy.max(axis=1)
    out["phot_noise_proxy_blue"] = (
        out["phot_noise_proxy_u"] + out["phot_noise_proxy_g"]
    ) / 2.0
    out["phot_noise_proxy_red"] = (
        out["phot_noise_proxy_i"] + out["phot_noise_proxy_z"]
    ) / 2.0
    out["blue_minus_red"] = out["phot_noise_proxy_blue"] - out["phot_noise_proxy_red"]

    color_pairs = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    for left, right in color_pairs:
        color = out[left] - out[right]
        denom = np.sqrt(
            out[f"phot_noise_proxy_{left}"] ** 2
            + out[f"phot_noise_proxy_{right}"] ** 2
            + 1e-6
        )
        out[f"color_{left}_{right}_snr_proxy"] = color / denom
