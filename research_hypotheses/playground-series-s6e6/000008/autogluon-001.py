...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mag_cols = ["u", "g", "r", "i", "z"]
    mag_matrix = out[mag_cols].astype("float32")

    band_medians = mag_matrix.median(axis=0)
    faint = mag_matrix.sub(band_medians, axis=1)
    faint_clipped = faint.clip(lower=-5.0, upper=5.0)
    noise_proxy = np.exp(0.4 * faint_clipped).clip(0.05, 20.0)

    noise_proxy.columns = [f"{c}_noise_proxy" for c in mag_cols]
    out[noise_proxy.columns] = noise_proxy.astype("float32")

    out["phot_noise_proxy_mean"] = noise_proxy.mean(axis=1).astype("float32")
    out["phot_noise_proxy_max"] = noise_proxy.max(axis=1).astype("float32")
    out["phot_noise_proxy_blue"] = noise_proxy["u_noise_proxy"].astype("float32")
    out["phot_noise_proxy_red"] = noise_proxy["z_noise_proxy"].astype("float32")
    out["phot_noise_proxy_blue_minus_red"] = (
        noise_proxy["u_noise_proxy"] - noise_proxy["z_noise_proxy"]
    ).astype("float32")

    adjacent_pairs = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    for a, b in adjacent_pairs:
        color = mag_matrix[a] - mag_matrix[b]
        denom = np.sqrt(
            noise_proxy[f"{a}_noise_proxy"] ** 2
            + noise_proxy[f"{b}_noise_proxy"] ** 2
            + 1e-6
        )
        out[f"color_{a}_{b}_snr_proxy"] = (color / denom).astype("float32")
