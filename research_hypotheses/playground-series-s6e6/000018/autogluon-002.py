...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    spectral_map = {"M": 0, "O/B": 1, "G/K": 2, "A/F": 3}
    galaxy_map = {"Blue_Cloud": 0, "Red_Sequence": 1}
    out["spectral_type_code"] = (
        out["spectral_type"].map(spectral_map).fillna(-1).astype(np.int8)
    )
    out["galaxy_population_code"] = (
        out["galaxy_population"].map(galaxy_map).fillna(-1).astype(np.int8)
    )

    # Redshift regressor inputs
    redshift_feats = [
        "alpha",
        "delta",
        "u",
        ...
        "g_i",
        "spectral_type_code",
        "galaxy_population_code",
    ]
    X = out[redshift_feats].astype(np.float32).to_numpy(copy=False)
    y = out["redshift"].astype(np.float32).to_numpy(copy=False)

    n_rows = len(out)
    train_n = min(n_rows, 577347)
    train_mask = np.arange(n_rows) < train_n

    predicted = np.zeros(n_rows, dtype=np.float32)
    oof_pred = np.full(train_n, np.nan, dtype=np.float32)

    if train_n > 1:
        n_folds = 5 if train_n >= 5 else train_n
        kf = KFold(n_splits=n_folds, shuffle=True, random_state=42)
        for tr_pos, va_pos in kf.split(np.arange(train_n)):
            tr_idx = tr_pos
            va_idx = va_pos
            ...
            oof_pred[va_idx] = reg.predict(X[va_idx])

        if np.isnan(oof_pred).any():
            reg = Ridge(alpha=1.0)
            reg.fit(X[:train_n], y[:train_n])
            oof_pred = reg.predict(X[:train_n])
        predicted[:train_n] = oof_pred

        reg_full = Ridge(alpha=1.0)
        reg_full.fit(X[:train_n], y[:train_n])
        full_pred = reg_full.predict(X)
        if train_n < n_rows:
            predicted[train_n:] = full_pred[train_n:]
    else:
        predicted[:] = y

    residual = out["redshift"].to_numpy(np.float32) - predicted

    out["predicted_redshift"] = predicted
    out["redshift_minus_predicted"] = residual
    out["abs_redshift_residual"] = np.abs(residual)
    out["squared_redshift_residual"] = residual**2
    out["residual_redshift_scale"] = residual / (
        np.abs(out["redshift"]).to_numpy(np.float32) + 1e-6
    )
    out["residual_sign"] = np.sign(residual).astype(np.float32)
    out["residual_sign_positive"] = (residual > 0).astype(np.int8)
    out["residual_sign_negative"] = (residual < 0).astype(np.int8)
    out["residual_abs_01"] = (out["abs_redshift_residual"] >= 0.1).astype(np.int8)
    out["residual_abs_05"] = (out["abs_redshift_residual"] >= 0.5).astype(np.int8)
    out["residual_abs_1"] = (out["abs_redshift_residual"] >= 1.0).astype(np.int8)
    train_abs = out.loc[train_mask, "abs_redshift_residual"].to_numpy()
    if train_abs.size > 0:
        q90 = np.nanquantile(train_abs, 0.9)
    else:
        q90 = 1.0
    out["residual_abs_top10pct"] = (out["abs_redshift_residual"] >= q90).astype(np.int8)
    return out
