...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _color_block(out)
    cols = [c for c in ["u_g", "g_r", "r_i", "i_z", "u_z", "mag_mean"] if c in out.columns]
    if cols and "redshift" in out.columns:
        score = sum(out[c].rank(pct=True) for c in cols) / float(len(cols))
        z_rank = _num(out, "redshift").rank(pct=True)
        out["photoz_rank_proxy"] = score.astype("float32")
        out["photoz_rank_residual"] = (z_rank - score).astype("float32")
        out["photoz_abs_residual"] = out["photoz_rank_residual"].abs().astype("float32")
        out["photoz_color_scatter"] = out[cols].std(axis=1).fillna(0).astype("float32")
    return out
