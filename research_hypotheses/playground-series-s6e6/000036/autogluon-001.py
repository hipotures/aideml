...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _color_block(out)
    cols = [c for c in ["u", "g", "r", "i", "z", "redshift", "u_g", "g_r", "r_i", "i_z"] if c in out.columns]
    tails = []
    for c in cols:
        pct = _num(out, c).rank(method="average", pct=True).clip(1e-4, 1 - 1e-4)
        out[f"{c}_pct_pool"] = pct.astype("float32")
        out[f"{c}_logit_pool"] = np.log(pct / (1 - pct)).astype("float32")
        tail = np.minimum(pct, 1 - pct)
        out[f"{c}_tail2_pool"] = tail.astype("float32")
        tails.append(tail)
    if tails:
        mat = np.column_stack(tails)
        out["tail_surprise_sum_pool"] = (-np.log(mat + 1e-6)).sum(axis=1).astype("float32")
        out["n_tail_le_5pct_pool"] = (mat <= 0.05).sum(axis=1).astype("int8")
    return out
