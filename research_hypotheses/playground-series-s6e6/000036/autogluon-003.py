...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep only columns available in preprocessing input
    out["spectral_type"] = out["spectral_type"].astype("category")
    out["galaxy_population"] = out["galaxy_population"].astype("category")

    raw_cont = ["alpha", "delta", "u", "g", "r", "i", "z", "redshift"]

    for col in raw_cont:
        lo = out[col].quantile(0.005)
        hi = out[col].quantile(0.995)
        out[f"{col}_winsor_0p5pct"] = out[col].clip(lo, hi).astype("float32")

    # Adjacent + broad color indices
    out["color_u_g"] = (out["u"] - out["g"]).astype("float32")
    out["color_g_r"] = (out["g"] - out["r"]).astype("float32")
    out["color_r_i"] = (out["r"] - out["i"]).astype("float32")
    out["color_i_z"] = (out["i"] - out["z"]).astype("float32")
    out["color_u_r"] = (out["u"] - out["r"]).astype("float32")
    out["color_r_z"] = (out["r"] - out["z"]).astype("float32")
    out["color_u_z"] = (out["u"] - out["z"]).astype("float32")
    ...
    pooled_pct_cols = []
    pooled_surprise_cols = []
    group_fallback_cols = []

    def add_ecdf_feature_block(data: pd.DataFrame, prefix: str, p: pd.Series) -> None:
        # p = percent rank in (0,1), deterministic on current fold-train+test pool
        p = pd.Series(p, index=data.index).astype("float64").clip(EPS, 1.0 - EPS)
        tail_low = p
        tail_high = 1.0 - p
        tail_prob = (2.0 * np.minimum(p, 1.0 - p)).clip(EPS, 1.0)

        data[f"{prefix}_ecdf_pct"] = p.astype("float32")
        data[f"{prefix}_ecdf_logit"] = np.log(p / (1.0 - p)).astype("float32")
        data[f"{prefix}_ecdf_norm"] = norm.ppf(p).astype("float32")
        data[f"{prefix}_ecdf_tail_low"] = tail_low.astype("float32")
        data[f"{prefix}_ecdf_tail_high"] = tail_high.astype("float32")
        data[f"{prefix}_ecdf_tail_prob"] = tail_prob.astype("float32")
        data[f"{prefix}_ecdf_neglog_tail"] = (-np.log(tail_prob)).astype("float32")
        data[f"{prefix}_ecdf_tail_rate_1pct_log"] = float(np.log1p((p <= 0.01).mean()))
        data[f"{prefix}_ecdf_tail_rate_2_5pct_log"] = float(
            np.log1p((p <= 0.025).mean())
        )
        data[f"{prefix}_ecdf_tail_rate_5pct_log"] = float(np.log1p((p <= 0.05).mean()))

    for col in anchor_features:
        base_pct = out[col].rank(pct=True, method="average")
        add_ecdf_feature_block(out, f"{col}_pooled", base_pct)
        pooled_pct_cols.append(f"{col}_pooled_ecdf_pct")
        pooled_surprise_cols.append(f"{col}_pooled_ecdf_neglog_tail")

        for gcol in ["spectral_type", "galaxy_population"]:
            group_sizes = out[gcol].value_counts(dropna=False)
            mapped_group_sizes = pd.to_numeric(
                out[gcol].map(group_sizes), errors="coerce"
            ).fillna(0)
            group_ok = mapped_group_sizes >= MIN_GROUP_SIZE
            grouped_pct = out.groupby(gcol, observed=True)[col].transform(
                lambda s: s.rank(pct=True, method="average")
            )
            pct_used = grouped_pct.where(group_ok, base_pct)
            add_ecdf_feature_block(out, f"{col}_{gcol}_group", pct_used)

            fallback_name = f"{col}_{gcol}_ecdf_fallback"
            out[fallback_name] = (~group_ok).astype("uint8")
            group_fallback_cols.append(fallback_name)

    # Pooled row summaries of ECDF tail behavior
    pooled_pct = out[pooled_pct_cols].to_numpy(dtype=np.float32)
    pooled_surprise = out[pooled_surprise_cols].to_numpy(dtype=np.float32)

    out["ecdf_pooled_tail_surprise_max"] = np.nanmax(pooled_surprise, axis=1).astype(
        "float32"
    )
    out["ecdf_pooled_tail_surprise_mean"] = np.nanmean(pooled_surprise, axis=1).astype(
        "float32"
    )
    out["ecdf_pooled_tail_surprise_sum"] = np.nansum(pooled_surprise, axis=1).astype(
        "float32"
    )
    out["ecdf_pooled_tail_surprise_std"] = np.nanstd(pooled_surprise, axis=1).astype(
        "float32"
    )

    out["ecdf_tail_count_1pct"] = (
        ((pooled_pct <= 0.01) | (pooled_pct >= 0.99)).sum(axis=1).astype("uint8")
    )
    out["ecdf_tail_count_2_5pct"] = (
        ((pooled_pct <= 0.025) | (pooled_pct >= 0.975)).sum(axis=1).astype("uint8")
    )
    out["ecdf_tail_count_5pct"] = (
        ((pooled_pct <= 0.05) | (pooled_pct >= 0.95)).sum(axis=1).astype("uint8")
    )

    # Blue-band vs red-band tail counts (pooled percentiles)
    blue_cols = [f"{c}_pooled_ecdf_pct" for c in ["u", "g"]]
    red_cols = [f"{c}_pooled_ecdf_pct" for c in ["r", "i", "z"]]
    blue_pct = out[blue_cols].to_numpy(dtype=np.float32)
    red_pct = out[red_cols].to_numpy(dtype=np.float32)

    out["ecdf_tail_count_blue_1pct"] = (
        ((blue_pct <= 0.01) | (blue_pct >= 0.99)).sum(axis=1).astype("uint8")
    )
    out["ecdf_tail_count_red_1pct"] = (
        ((red_pct <= 0.01) | (red_pct >= 0.99)).sum(axis=1).astype("uint8")
    )
    out["ecdf_tail_count_blue_minus_red_1pct"] = (
        out["ecdf_tail_count_blue_1pct"] - out["ecdf_tail_count_red_1pct"]
    ).astype("int16")

    # Group-ECDF stability diagnostics (for detecting fallback/no-op behavior)
    out["ecdf_group_fallback_count"] = (
        out[group_fallback_cols].sum(axis=1).astype("uint16")
    )
    out["ecdf_group_fallback_log1p"] = np.log1p(
        out["ecdf_group_fallback_count"]
    ).astype("float32")
    out["ecdf_group_fallback_ratio"] = (
        out["ecdf_group_fallback_count"].astype("float32") / (2 * len(anchor_features))
    ).astype("float32")

    # Rank-space geometry in color-redshift copula-like space
    rank_cols = [
        "color_u_g_pooled_ecdf_norm",
        "color_g_r_pooled_ecdf_norm",
        "color_r_i_pooled_ecdf_norm",
        "redshift_pooled_ecdf_norm",
    ]
    rank_matrix = out[rank_cols].to_numpy(dtype=np.float64)
    centered = rank_matrix - np.mean(rank_matrix, axis=0)
    color_cov = np.cov(centered, rowvar=False) + np.eye(centered.shape[1]) * 1e-6
    inv_cov = np.linalg.pinv(color_cov)
    out["ecdf_mahalanobis_color_redshift"] = np.sqrt(
        np.einsum("ij,jk,ik->i", centered, inv_cov, centered)
    ).astype("float32")
    out["ecdf_l2_color_redshift"] = np.linalg.norm(centered[:, :3], axis=1).astype(
        "float32"
    )
