...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Compact color features
    out["color_u_g"] = out["u"] - out["g"]
    out["color_g_r"] = out["g"] - out["r"]
    out["color_r_i"] = out["r"] - out["i"]
    out["color_i_z"] = out["i"] - out["z"]

    # Keep raw metadata one-hot features
    meta_dummies = pd.get_dummies(
        out[["spectral_type", "galaxy_population"]],
        dtype=np.uint8,
        drop_first=False,
    )
    out = pd.concat([out, meta_dummies], axis=1)

    # Train/test split is implicit in concatenated input by row order in this contest wrapper
    # (train first, test last)
    n_train = max(len(out) - 247435, 0)
    train_mask = np.zeros(len(out), dtype=bool)
    if n_train > 0:
    ...
    def _fit_metadata_meta(
        y_series: pd.Series, base_name: str
    ) -> tuple[np.ndarray, np.ndarray]:
        y = y_series.astype(str).to_numpy()
        ...
        k = len(classes)
        if k == 0:
            return np.zeros((n, 0), dtype=np.float64), np.zeros(n, dtype=np.float64)

        oof_probs = np.full((n, k), np.nan, dtype=np.float64)

        counts = pd.Series(y).value_counts(dropna=False)
        min_count = int(counts.min()) if len(counts) > 0 else 0
        n_splits = min(4, min_count) if min_count >= 2 else 1
        ...
        if n_splits >= 2 and k >= 2:
            ...
            for tr_idx, va_idx in skf.split(X, y):
                ...
                fold_probs = model.predict_proba(X[va_idx])

                class_to_pos = {c: j for j, c in enumerate(model.classes_)}
                for j, cls in enumerate(classes):
                    if cls in class_to_pos:
                        oof_probs[va_idx, j] = fold_probs[:, class_to_pos[cls]]

        full_model = LogisticRegression(**model_kwargs)
        full_model.fit(X, y)
        full_probs = np.full((n, k), np.nan, dtype=np.float64)
        full_raw = full_model.predict_proba(X)
        class_to_pos_full = {c: j for j, c in enumerate(full_model.classes_)}
        for j, cls in enumerate(classes):
            if cls in class_to_pos_full:
                full_probs[:, j] = full_raw[:, class_to_pos_full[cls]]

        probs = oof_probs.copy()
        not_filled = np.isnan(probs).any(axis=1)
        if not_filled.any():
            probs[not_filled] = full_probs[not_filled]
        # For synthetic test rows, explicitly use full-fit probabilities.
        if test_mask.any():
            probs[test_mask] = full_probs[test_mask]

        # Append probability-vector diagnostics
        for j, cls in enumerate(classes):
            out[f"{base_name}_prob_{_safe_name(cls)}"] = probs[:, j].astype(np.float64)

        max_prob = np.max(probs, axis=1)
        entropy = -np.sum(probs * np.log(np.clip(probs, 1e-15, 1.0)), axis=1)
        if k > 1:
            srt = np.sort(probs, axis=1)
            top2_margin = srt[:, -1] - srt[:, -2]
        else:
            top2_margin = np.zeros(n, dtype=np.float64)

        cat_codes = pd.Categorical(y, categories=classes).codes
        idx = np.arange(n)
        actual_prob = np.zeros(n, dtype=np.float64)
        valid = cat_codes >= 0
        if valid.any():
            actual_prob[valid] = probs[idx[valid], cat_codes[valid]]

        pred_idx = probs.argmax(axis=1)
        pred_label = np.array(classes, dtype=object)[pred_idx]
        pred_match = (pred_label == y).astype(np.uint8)
        mismatch = 1.0 - actual_prob

        out[f"{base_name}_max_prob"] = max_prob
        out[f"{base_name}_entropy"] = entropy
        out[f"{base_name}_top2_margin"] = top2_margin
        out[f"{base_name}_actual_prob"] = actual_prob
        out[f"{base_name}_match_flag"] = pred_match
        out[f"{base_name}_mismatch_residual"] = mismatch

        return probs, actual_prob

    _st_probs, st_actual_prob = _fit_metadata_meta(
        out["spectral_type"], "spectral_type"
    )
    _gp_probs, gp_actual_prob = _fit_metadata_meta(
        out["galaxy_population"], "galaxy_population"
    )

    # Cross-consistency signals
    out["metadata_actual_prob_interaction"] = st_actual_prob * gp_actual_prob
    out["metadata_entropy_delta"] = (
        out["spectral_type_entropy"] - out["galaxy_population_entropy"]
    )
    out["metadata_margin_delta"] = (
        out["spectral_type_top2_margin"] - out["galaxy_population_top2_margin"]
    )
