...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["spectral_type", "galaxy_population"]:
        out = pd.concat(
            [
                ...
                pd.get_dummies(out[col], prefix=col, dtype=np.float32),
            ],
            axis=1,
        )

    mags = out[bands].to_numpy(dtype=np.float64)
    with np.errstate(over="ignore", under="ignore", invalid="ignore"):
        flux = np.power(10.0, -0.4 * mags, dtype=np.float64)
    flux = np.where(np.isfinite(flux), flux, 0.0)
    row_sum = flux.sum(axis=1, keepdims=True)
    row_sum = np.where(row_sum > 0, row_sum, 1.0)
    flux_norm = flux / row_sum

    z = out["redshift"].to_numpy(dtype=np.float64)
    z_safe = np.clip(z, 0.0, 7.0)
    lam_obs = np.array([3551.0, 4686.0, 6165.0, 7481.0, 8931.0], dtype=np.float64)

    def _fit_blackbody(flux_norm_local, lam_local, temps):
        n = flux_norm_local.shape[0]
        best = {
            "rmse": np.full(n, np.inf, dtype=np.float64),
            "mae": np.full(n, np.inf, dtype=np.float64),
            "maxabs": np.full(n, np.inf, dtype=np.float64),
            "param": np.full(n, np.nan, dtype=np.float64),
            "amp": np.zeros(n, dtype=np.float64),
            "resid": np.zeros((n, len(bands)), dtype=np.float64),
        }

        for temp in temps:
            if lam_local.ndim == 1:
                lam_t = lam_local / temp
                lam_t = np.clip(lam_t, 1e-12, np.inf)
                bb = 1.0 / (lam_t**5 * np.expm1(np.minimum(1.438777e8 / lam_t, 700.0)))
                s = bb.sum()
                if not np.isfinite(s) or s <= 0:
                    continue
                bb = bb / s
                dot = flux_norm_local @ bb
                amp = dot / (bb @ bb)
                pred = amp[:, None] * bb
            else:
                lam_t = lam_local / temp
                lam_t = np.clip(lam_t, 1e-12, np.inf)
                bb = 1.0 / (lam_t**5 * np.expm1(np.minimum(1.438777e8 / lam_t, 700.0)))
                row_s = bb.sum(axis=1, keepdims=True)
                row_s = np.where(row_s > 0, row_s, 1.0)
                bb = bb / row_s
                dot = np.einsum("ij,ij->i", flux_norm_local, bb)
                denom = np.einsum("ij,ij->i", bb, bb)
                amp = np.divide(dot, denom, out=np.zeros_like(dot), where=denom > 0)
                pred = bb * amp[:, None]

            resid = flux_norm_local - pred
            rmse = np.sqrt(np.mean(resid * resid, axis=1))
            mae = np.mean(np.abs(resid), axis=1)
            maxabs = np.max(np.abs(resid), axis=1)

            better = rmse < best["rmse"]
            if np.any(better):
                idx = np.flatnonzero(better)
                best["rmse"][idx] = rmse[idx]
                best["mae"][idx] = mae[idx]
                best["maxabs"][idx] = maxabs[idx]
                best["param"][idx] = temp
                best["amp"][idx] = amp[idx]
                best["resid"][idx] = resid[idx]

        return best

    def _fit_powerlaw(flux_norm_local, lam_local, alphas):
        n = flux_norm_local.shape[0]
        best = {
            "rmse": np.full(n, np.inf, dtype=np.float64),
            "mae": np.full(n, np.inf, dtype=np.float64),
        ...
        lam_norm = lam_local / lam_local.mean()
        for alpha in alphas:
            pl = lam_norm ** (-alpha)
            s = pl.sum()
            if not np.isfinite(s) or s <= 0:
    ...
    bb_obs = _fit_blackbody(flux_norm, lam_obs, temps)
    bb_rest = _fit_blackbody(flux_norm, lam_obs / (1.0 + z_safe[:, None]), temps)
    pl_fit = _fit_powerlaw(flux_norm, lam_obs, alphas)

    out["bb_obs_best_temp"] = bb_obs["param"]
    out["bb_rest_best_temp"] = bb_rest["param"]
    out["pl_best_alpha"] = pl_fit["param"]

    out["bb_obs_best_amp"] = bb_obs["amp"]
    out["bb_rest_best_amp"] = bb_rest["amp"]
    out["pl_best_amp"] = pl_fit["amp"]

    out["bb_obs_rmse"] = bb_obs["rmse"]
    out["bb_rest_rmse"] = bb_rest["rmse"]
    out["pl_rmse"] = pl_fit["rmse"]

    out["bb_obs_mae"] = bb_obs["mae"]
    out["bb_rest_mae"] = bb_rest["mae"]
    out["pl_mae"] = pl_fit["mae"]

    out["bb_obs_maxabs"] = bb_obs["maxabs"]
    out["bb_rest_maxabs"] = bb_rest["maxabs"]
    out["pl_maxabs"] = pl_fit["maxabs"]

    out["bb_obs_rest_rmse_ratio"] = bb_obs["rmse"] / (bb_rest["rmse"] + 1e-12)
    out["bb_obs_minus_rest_rmse"] = bb_obs["rmse"] - bb_rest["rmse"]
    out["bb_obs_minus_rest_mae"] = bb_obs["mae"] - bb_rest["mae"]
    out["bb_temp_minus_rest_temp"] = bb_obs["param"] - bb_rest["param"]

    out["bb_vs_pl_rmse_ratio"] = bb_obs["rmse"] / (pl_fit["rmse"] + 1e-12)
    out["bb_vs_pl_mae_ratio"] = bb_obs["mae"] / (pl_fit["mae"] + 1e-12)
    out["bb_better_than_pl"] = (bb_obs["rmse"] <= pl_fit["rmse"]).astype(np.float32)

    bb_obs_resid = bb_obs["resid"]
    bb_rest_resid = bb_rest["resid"]
    pl_resid = pl_fit["resid"]

    for j, b in enumerate(bands):
        out[f"bb_obs_resid_{b}"] = bb_obs_resid[:, j]
        out[f"bb_rest_resid_{b}"] = bb_rest_resid[:, j]
        out[f"pl_resid_{b}"] = pl_resid[:, j]

    out["bb_obs_resid_std"] = np.std(bb_obs_resid, axis=1)
    out["bb_rest_resid_std"] = np.std(bb_rest_resid, axis=1)
    out["pl_resid_std"] = np.std(pl_resid, axis=1)

    out["bb_obs_resid_peak_to_trough"] = np.max(bb_obs_resid, axis=1) - np.min(
        bb_obs_resid, axis=1
    )
    out["bb_rest_resid_peak_to_trough"] = np.max(bb_rest_resid, axis=1) - np.min(
        bb_rest_resid, axis=1
    )
    out["pl_resid_peak_to_trough"] = np.max(pl_resid, axis=1) - np.min(pl_resid, axis=1)

    out["bb_obs_resid_curv_ugr"] = (
        bb_obs_resid[:, 0] - 2.0 * bb_obs_resid[:, 1] + bb_obs_resid[:, 2]
    )
    out["bb_obs_resid_curv_gri"] = (
        bb_obs_resid[:, 1] - 2.0 * bb_obs_resid[:, 2] + bb_obs_resid[:, 3]
    )
    out["bb_obs_resid_curv_riz"] = (
        bb_obs_resid[:, 2] - 2.0 * bb_obs_resid[:, 3] + bb_obs_resid[:, 4]
    )
    out["pl_resid_curv_ugr"] = pl_resid[:, 0] - 2.0 * pl_resid[:, 1] + pl_resid[:, 2]
    out["pl_resid_curv_gri"] = pl_resid[:, 1] - 2.0 * pl_resid[:, 2] + pl_resid[:, 3]
    out["pl_resid_curv_riz"] = pl_resid[:, 2] - 2.0 * pl_resid[:, 3] + pl_resid[:, 4]

    out["bb_obs_u_excess_over_gr"] = bb_obs_resid[:, 0] - bb_obs_resid[:, 1]
    out["bb_obs_i_excess_over_r"] = bb_obs_resid[:, 3] - bb_obs_resid[:, 2]
    out["pl_u_excess_over_gr"] = pl_resid[:, 0] - pl_resid[:, 1]
    out["pl_i_excess_over_r"] = pl_resid[:, 3] - pl_resid[:, 2]
