...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(b in out.columns for b in bands):
        flux = np.power(10.0, -0.4 * out[bands].to_numpy(dtype="float32"))
        norm = flux / (flux.sum(axis=1, keepdims=True) + 1e-12)
        templates = {
            "blue_powerlaw": np.array([.40, .28, .17, .10, .05]),
            "flat": np.array([.20, .20, .20, .20, .20]),
            "red_galaxy": np.array([.06, .10, .17, .27, .40]),
        }
        for name, tpl in templates.items():
            tpl = tpl / tpl.sum()
            res = norm - tpl[None, :]
            out[f"sed_template_{name}_l2"] = np.sqrt(np.square(res).sum(axis=1)).astype("float32")
            out[f"sed_template_{name}_dot"] = (norm @ tpl).astype("float32")
    return out
