...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Trigonometric / angular positional features.
    ...
    # Stable color and normalized morphology-like features.
    color_pairs = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    for a, b in color_pairs:
        if a in out.columns and b in out.columns:
            out[f"color_{a}_{b}"] = out[a] - out[b]
            den = out[b].replace(0, np.nan)
            out[f"color_ratio_{a}_{b}"] = out[a] / den

    if all(c in out.columns for c in ["u", "g", "r", "i", "z"]):
        out["color_index_ugri"] = (out["u"] - out["g"]) / (out["r"] - out["i"]).replace(
            0, np.nan
        )
        out["color_index_rizz"] = (out["r"] - out["i"]) / (out["i"] - out["z"]).replace(
            0, np.nan
        )
        out["color_u2gz"] = (out["u"] - out["g"]) - (out["g"] - out["z"])
        out["color_spread"] = (out["u"] + out["z"]) - (
            out["g"] + out["r"] + out["i"]
        ) / 3.0

    # Log-scaled photometry / redshift for robust nonlinearity.
    for col in ["u", "g", "r", "i", "z", "redshift"]:
        if col in out.columns:
            shifted = out[col].clip(lower=0.0).fillna(0.0)
            out[f"{col}_log1p"] = np.log1p(shifted)

    # Frequency encoding for categorical columns.
    for col in ["galaxy_population", "spectral_type"]:
        if col in out.columns:
            freq = out[col].value_counts(dropna=True, normalize=True)
            out[f"{col}_freq"] = out[col].map(freq).astype(float)
            out[f"{col}_code"] = out[col].astype("category").cat.codes

    # Keep values bounded and informative for a model that values continuous structure.
    if "redshift" in out.columns:
        out["redshift_clip"] = out["redshift"].clip(
            lower=out["redshift"].quantile(0.001), upper=out["redshift"].quantile(0.999)
        )
        out["redshift_abs"] = out["redshift"].abs()

    # Auxiliary-derived centroid-distance features (leakage-safe and row-preserving).
    if aux is not None and isinstance(aux, pd.DataFrame) and aux.shape[0] > 0:
        aux_df = aux.copy(deep=False)

        # Identify auxiliary label column without hardcoding its name.
        aux_meta = {
            "obj_ID",
            "run_ID",
            "rerun_ID",
        ...
        aux_common = set(numeric_features) | {"redshift"}
        candidate_label_cols = [
            c for c in aux_df.columns if c not in aux_meta and c not in aux_common
        ]
        if len(candidate_label_cols) == 0:
            aux_label_col = None
        elif len(candidate_label_cols) == 1:
            aux_label_col = candidate_label_cols[0]
        else:
            # Fallback for ambiguous structures: choose the non-numeric candidate not in metadata.
            object_candidates = [
                c for c in candidate_label_cols if aux_df[c].dtype == "object"
            ]
            aux_label_col = (
                object_candidates[0] if object_candidates else candidate_label_cols[0]
            )

        if aux_label_col is not None and aux_label_col in aux_df.columns:
            color_cols = [
                c
                for c in ["u", "g", "r", "i", "z", "redshift"]
                if c in aux_df.columns and c in out.columns
            ]
            if len(color_cols) > 0 and aux_label_col in aux_df.columns:
                aux_sub = aux_df[color_cols + [aux_label_col]].copy()
                for c in color_cols:
                    if np.issubdtype(aux_sub[c].dtype, np.number):
                        aux_sub[c] = pd.to_numeric(aux_sub[c], errors="coerce")
                    else:
                        aux_sub[c] = pd.to_numeric(aux_sub[c], errors="coerce")
                    aux_sub.loc[aux_sub[c] == -9999, c] = np.nan

                # Clean per-column sentinel/nan with median-imputation in auxiliary space.
                for c in color_cols:
                    col_median = aux_sub[c].median()
                    if pd.isna(col_median):
                        col_median = 0.0
                    aux_sub[c] = aux_sub[c].fillna(col_median)

                x = out[color_cols].astype(float)
                x_mat = x.to_numpy(dtype=float)
                out[f"aux_{'_'.join(color_cols)}_valid"] = (
                    np.isfinite(x_mat).astype(int).sum(axis=1)
                )
                ...
                dist_columns = []
                label_values = aux_sub[aux_label_col].dropna().unique()
                for idx, lab in enumerate(label_values):
                    mask = aux_sub[aux_label_col] == lab
                    group = aux_sub.loc[mask, color_cols]
                    if group.shape[0] < 5:
                        continue

                    center = group.median(axis=0).to_numpy(dtype=float)
                    mad = (
                        group.apply(_mad, axis=0)
                        .replace(0.0, np.nan)
                        .to_numpy(dtype=float)
                    )
                    mad[~np.isfinite(mad)] = 1.0

                    d = np.sqrt(np.nansum(((x_mat - center) / mad) ** 2, axis=1))
                    d = np.where(np.isfinite(d), d, np.nan)
                    cname = f"aux_dist_to_group_{idx}"
                    out[cname] = d
                    dist_columns.append(cname)

                if dist_columns:
                    dist_matrix = out[dist_columns].to_numpy(dtype=float)
                    out["aux_dist_min"] = np.nanmin(dist_matrix, axis=1)
                    out["aux_dist_max"] = np.nanmax(dist_matrix, axis=1)
                    if len(dist_columns) > 1:
                        sorted_idx = np.argsort(dist_matrix, axis=1)
                        row_idx = np.arange(dist_matrix.shape[0])
                        best1 = dist_matrix[row_idx, sorted_idx[:, 0]]
                        best2 = dist_matrix[row_idx, sorted_idx[:, 1]]
                        out["aux_dist_margin"] = best2 - best1
                        out["aux_dist_best_group"] = sorted_idx[:, 0]
                    else:
                        out["aux_dist_margin"] = np.nan
                        out["aux_dist_best_group"] = 0
                else:
                    out["aux_dist_min"] = np.nan
                    out["aux_dist_max"] = np.nan
                    out["aux_dist_margin"] = np.nan
                    out["aux_dist_best_group"] = np.nan
