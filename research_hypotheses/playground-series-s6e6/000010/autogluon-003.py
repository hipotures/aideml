...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"alpha", "delta"}.issubset(out.columns):
        a_rad = np.deg2rad(out["alpha"].to_numpy(dtype=float))
        d_rad = np.deg2rad(out["delta"].to_numpy(dtype=float))
    ...
    if {"u", "g", "r", "i", "z", "redshift"}.issubset(out.columns):
        ...
        out["redshift_log1p"] = np.log1p(
            np.clip(out["redshift"].to_numpy(dtype=float), 0.0, None)
        )
    ...
    for col in ["galaxy_population", "spectral_type"]:
        if col in out.columns:
            ...
            freq = ser.value_counts(dropna=False)
            out[f"{col}_freq"] = ser.map(freq).astype(float).div(len(out))
            out[f"{col}_code"] = ser.astype("category").cat.codes

    if isinstance(aux, pd.DataFrame):
        aux_clean = aux.copy(deep=True)

        for col in numeric_cols:
            if col in aux_clean.columns:
                aux_clean[col] = pd.to_numeric(aux_clean[col], errors="coerce")
                aux_clean.loc[aux_clean[col] == -9999, col] = np.nan
        ...
        overlap = set(aux_clean.columns).intersection(out.columns)
        aux_target = None
        for c in aux_clean.columns:
            if c not in overlap and not pd.api.types.is_numeric_dtype(aux_clean[c]):
                aux_target = c
                break
        if aux_target is None:
            for c in aux_clean.columns:
                if c not in overlap and aux_clean[c].dtype == object:
                    aux_target = c
                    break

        if aux_target is not None and aux_target in aux_clean.columns:
            aux_feature_cols = [
                c
                for c in [
                    "u",
            ...
            if len(aux_feature_cols) >= 2:
                aux_cols = aux_feature_cols + [aux_target]
                aux_work = (
                    aux_clean[aux_cols].replace([np.inf, -np.inf], np.nan).dropna()
                )
                if len(aux_work) > 0 and aux_work[aux_target].nunique(dropna=True) >= 2:
                    centers = aux_work.groupby(aux_target, dropna=True)[
                        aux_feature_cols
                    ].median()
                    if len(centers) >= 2:
                        robust_scale = (
                            aux_work[aux_feature_cols]
                            .mad(numeric_only=True)
                            .replace(0.0, np.nan)
                            .fillna(1.0)
                        )
                        scale = robust_scale.to_numpy(dtype=float)
                        x_ref = out[aux_feature_cols].to_numpy(dtype=float)
                        fallback = np.nanmedian(
                            aux_work[aux_feature_cols].to_numpy(dtype=float), axis=0
                        )
                        x_ref = np.where(
                            np.isfinite(x_ref), x_ref, fallback[np.newaxis, :]
                        )

                        dist_cols = []
                        for k, center in centers.iterrows():
                            c = center.to_numpy(dtype=float)
                            d = (x_ref - c) / scale
                            dist = np.sqrt(np.nansum(d * d, axis=1))
                            safe = re.sub(r"[^A-Za-z0-9]+", "_", str(k).lower()).strip(
                                "_"
                            )
                            if safe == "":
                                safe = "target"
                            name = f"aux_dist_to_{safe}"
                            out[name] = dist
                            dist_cols.append(name)

                        if dist_cols:
                            dist_mat = out[dist_cols].to_numpy(dtype=float)
                            out["aux_dist_min"] = np.nanmin(dist_mat, axis=1)
                            out["aux_dist_max"] = np.nanmax(dist_mat, axis=1)
                            out["aux_dist_margin"] = (
                                out["aux_dist_max"] - out["aux_dist_min"]
                            )
