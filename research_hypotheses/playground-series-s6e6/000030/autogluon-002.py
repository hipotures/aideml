...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame = None) -> pd.DataFrame:
    out = df.copy()
    n = len(out)

    feature_cols = ["alpha", "delta", "u", "g", "r", "i", "z", "redshift"]
    source = out[feature_cols].copy()

    if (
        aux is not None
        and not aux.empty
        and all(col in aux.columns for col in feature_cols)
    ):
        aux_source = aux[feature_cols].copy()
        for mag in ["u", "g", "r", "i", "z"]:
            aux_source[mag] = pd.to_numeric(aux_source[mag], errors="coerce").where(
                aux_source[mag] > -9000, np.nan
            )
        for col in ["alpha", "delta", "redshift"]:
            aux_source[col] = pd.to_numeric(aux_source[col], errors="coerce")
        source = pd.concat([source, aux_source], axis=0, ignore_index=True)

    for mag in ["u", "g", "r", "i", "z"]:
        source[mag] = pd.to_numeric(source[mag], errors="coerce").where(
            source[mag] > -9000, np.nan
        )
    for col in ["alpha", "delta", "redshift"]:
        source[col] = pd.to_numeric(source[col], errors="coerce")

    source["gi_coord"] = source["g"] - source["i"]
    valid_coord = source["gi_coord"].notna()

    locus_bin = pd.Series(pd.NA, index=source.index, dtype="Int64")
    if valid_coord.sum() >= 64:
        try:
            q = pd.qcut(source.loc[valid_coord, "gi_coord"], q=48, duplicates="drop")
            locus_bin.loc[valid_coord] = q.cat.codes.astype("Int64")
        except Exception:
            try:
                edges = np.quantile(
                    source.loc[valid_coord, "gi_coord"], q=np.linspace(0.0, 1.0, 49)
                )
                edges = np.unique(edges)
                if len(edges) >= 3:
                    q = pd.cut(
                        source.loc[valid_coord, "gi_coord"],
                        bins=edges,
                        include_lowest=True,
                    )
                    locus_bin.loc[valid_coord] = q.cat.codes.astype("Int64")
                ...
                pass
    if locus_bin.notna().sum() == 0:
        locus_bin = pd.Series(0, index=source.index, dtype="Int64")
    source["locus_bin"] = locus_bin
    ...
    for cname, (left, right) in color_pairs.items():
        col = source[left] - source[right]
        source[f"color_{cname}"] = col

        med_curve = source.groupby("locus_bin")[f"color_{cname}"].median()
        mad_vals = (
            col - source.groupby("locus_bin")[f"color_{cname}"].transform("median")
        ).abs()
        mad_curve = mad_vals.groupby(source["locus_bin"]).median()

        if len(med_curve) > 1:
            full_idx = np.arange(
                int(med_curve.index.min()), int(med_curve.index.max()) + 1
            )
            med_curve = (
                med_curve.reindex(full_idx)
                .interpolate(limit_direction="both")
                .rolling(7, center=True, min_periods=1)
                .median()
            )
            mad_curve = (
                mad_curve.reindex(full_idx)
                .interpolate(limit_direction="both")
                .rolling(7, center=True, min_periods=1)
                .median()
            )

        local_med = source["locus_bin"].map(med_curve)
        local_mad = source["locus_bin"].map(mad_curve)

        if med_curve.empty:
            global_med = col.median()
            global_mad = (col - global_med).abs().median()
        else:
            global_med = med_curve.median()
            global_mad = mad_curve.median()

        local_med = local_med.fillna(global_med)
        local_mad = local_mad.fillna(global_mad).replace(0, np.nan)
        local_mad = local_mad.fillna(global_mad if global_mad > 0 else 1e-6).clip(
            lower=1e-6
        )

        source[f"loc_resid_{cname}"] = col - local_med
        source[f"loc_abs_resid_{cname}"] = source[f"loc_resid_{cname}"].abs()
        source[f"loc_z_{cname}"] = source[f"loc_resid_{cname}"] / local_mad
        source[f"loc_abs_z_{cname}"] = source[f"loc_z_{cname}"].abs()

    locus_df = source.iloc[:n].copy()

    out["gi_coord"] = locus_df["gi_coord"]
    out["locus_bin"] = locus_df["locus_bin"]
    for cname in color_pairs:
        out[f"color_{cname}"] = locus_df[f"color_{cname}"]
        out[f"loc_resid_{cname}"] = locus_df[f"loc_resid_{cname}"]
        out[f"loc_abs_resid_{cname}"] = locus_df[f"loc_abs_resid_{cname}"]
        out[f"loc_z_{cname}"] = locus_df[f"loc_z_{cname}"]
        out[f"loc_abs_z_{cname}"] = locus_df[f"loc_abs_z_{cname}"]

    z_cols = [f"loc_z_{c}" for c in color_pairs]
    z_abs_cols = [f"loc_abs_z_{c}" for c in color_pairs]

    z_df = out[z_cols]
    out["locus_total_z"] = np.sqrt((z_df**2).sum(axis=1))
    out["locus_max_abs_z"] = out[z_abs_cols].abs().max(axis=1)
    out["locus_uv_excess"] = (-out["loc_z_u_g"]).clip(lower=0.0)
    out["locus_red_excess"] = np.maximum(0.0, out["loc_z_r_i"]) + np.maximum(
        0.0, out["loc_z_i_z"]
    )

    for zcol in z_cols:
        out[f"{zcol}_x_redshift"] = out[zcol] * out["redshift"]

    spectral_d = pd.get_dummies(out["spectral_type"], prefix="spectral", dtype="float")
    for sc in spectral_d.columns:
        out[f"{sc}_x_locus_uv_excess"] = spectral_d[sc] * out["locus_uv_excess"]
        out[f"{sc}_x_locus_z_u_g"] = spectral_d[sc] * out["loc_z_u_g"]
