...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    alpha = out["alpha"].astype(float).to_numpy()
    delta = out["delta"].astype(float).to_numpy()
    ...
    # Equal-area-ish sky occupancy: RA bins x sin(Dec) bins
    n_lon = 24
    n_lat = 12
    alpha_bin = np.floor_divide(np.floor(alpha * n_lon), 360.0).astype(int)
    alpha_bin = np.clip(alpha_bin, 0, n_lon - 1)

    sin_delta = np.sin(delta_rad)
    sin_delta_bins = np.linspace(-1.0, 1.0, n_lat + 1)
    delta_bin = np.digitize(sin_delta, sin_delta_bins, right=True) - 1
    delta_bin = np.clip(delta_bin, 0, n_lat - 1)

    cell_id = (delta_bin * n_lon + alpha_bin).astype(int)
    cell_counts = pd.Series(cell_id).map(pd.Series(cell_id).value_counts())
    out["sky_eqarea_count_log"] = np.log1p(cell_counts.to_numpy(dtype=float))

    # Auxiliary sky percentile features from aux sky proxy = abs(sin(delta))
    if isinstance(aux, pd.DataFrame) and {"delta"}.issubset(aux.columns):
        aux_delta = aux["delta"].astype(float).to_numpy()
    else:
        aux_delta = delta

    aux_b_abs = np.abs(np.sin(np.deg2rad(aux_delta)))
    aux_ref = np.sort(np.nan_to_num(aux_b_abs, nan=np.nanmedian(aux_b_abs)))
    if aux_ref.size == 0:
        aux_ref = np.array([0.0], dtype=float)

    row_b_abs = np.abs(sin_delta)
    row_pct = np.searchsorted(aux_ref, row_b_abs, side="right") / len(aux_ref)
    row_pct = np.clip(row_pct, 0.0, 1.0)
    out["aux_gal_b_abs_pct"] = row_pct
    out["aux_gal_b_abs_pct_centered"] = out["aux_gal_b_abs_pct"] - 0.5
    out["aux_gal_b_abs_pct_tail"] = out["aux_gal_b_abs_pct_centered"].abs()
    out["aux_gal_b_abs_pct_extreme"] = np.minimum(
        out["aux_gal_b_abs_pct"], 1.0 - out["aux_gal_b_abs_pct"]
    )

    mode = os.getenv("SKY_ABLATION", "minimal_sky").lower().replace("-", "_").strip()

    if mode in {"minimal_sky", "minimal"}:
        keep_sky = {
            "alpha",
            "delta",
            "alpha_sin",
            ...
            "delta_sin",
            "delta_cos",
            "sky_eqarea_count_log",
        }
        keep_aux = {
            "aux_gal_b_abs_pct",
            "aux_gal_b_abs_pct_centered",
            "aux_gal_b_abs_pct_tail",
        }

        drop_cols = [
            c
            for c in out.columns
            if (c.startswith("sky_") and c not in keep_sky)
            or (c.startswith("aux_gal_b_abs_pct") and c not in keep_aux)
        ]
        out = out.drop(columns=drop_cols, errors="ignore")

    elif mode in {"no_aux_sky", "noaux_sky"}:
        drop_cols = [c for c in out.columns if c.startswith("aux_gal_b_abs_pct")]
        out = out.drop(columns=drop_cols, errors="ignore")

    elif mode in {"no_raw_coordinates", "no_raw_coord", "noraw_coordinates"}:
        drop_cols = [
            c for c in ["alpha", "delta", "sky_x", "sky_y", "sky_z"] if c in out.columns
        ]
        out = out.drop(columns=drop_cols, errors="ignore")
