...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Resolve id column if present; fallback to positional index for determinism.
    id_col = None
    for cand in ("id", "ID", "Id", "object_id", "objectId"):
        if cand in df.columns:
            id_col = cand
            break

    if id_col is not None:
        id_vals = pd.to_numeric(df[id_col], errors="coerce")
        id_int = id_vals.fillna(pd.Series(np.arange(len(df)), index=df.index)).astype(
            "int64"
        )
    else:
        id_int = pd.Series(np.arange(len(df)), index=df.index, dtype="int64")

    block_sizes = [256, 1024, 4096, 16384]

    # Build color/magnitude base features from available photometric bands.
    mag_cols = [c for c in ["u", "g", "r", "i", "z"] if c in df.columns]
    derived_cols = []
    if "u" in df.columns and "g" in df.columns:
        df["color_u_g"] = df["u"] - df["g"]
        derived_cols.append("color_u_g")
    if "g" in df.columns and "r" in df.columns:
        df["color_g_r"] = df["g"] - df["r"]
        derived_cols.append("color_g_r")
    if "r" in df.columns and "i" in df.columns:
        df["color_r_i"] = df["r"] - df["i"]
        derived_cols.append("color_r_i")
    if "i" in df.columns and "z" in df.columns:
        df["color_i_z"] = df["i"] - df["z"]
        derived_cols.append("color_i_z")
    if len(mag_cols) >= 1:
        df["mag_mean"] = df[mag_cols].mean(axis=1)
        derived_cols.extend(["mag_mean"])
    if len(mag_cols) >= 2:
        df["mag_std"] = df[mag_cols].std(axis=1)
        derived_cols.append("mag_std")
    ...
    for block_size in block_sizes:
        block_key = (id_int // block_size).astype("int64")
        block_counts = block_key.value_counts()
        count_col = f"id_block_{block_size}_count"
        count_log_col = f"id_block_{block_size}_count_log1p"
        df[count_col] = block_key.map(block_counts).astype("int64")
        df[count_log_col] = np.log1p(df[count_col])

        for col in base_resid_cols:
            block_median = df.groupby(block_key)[col].transform("median")
            df[f"{col}_dev_idblock_{block_size}"] = df[col] - block_median
