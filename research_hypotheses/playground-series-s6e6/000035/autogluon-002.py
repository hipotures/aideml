...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    _ = aux  # auxiliary frame is available if future extensions are needed; not merged to preserve leakage safety

    # Core feature columns used by this runner
    numeric_cols = ["alpha", "delta", "u", "g", "r", "i", "z", "redshift"]
    for c in numeric_cols:
        out[c] = pd.to_numeric(out[c], errors="coerce")

    # Redshift handling (non-negative, bounded to dataset range)
    z = out["redshift"].clip(lower=0.0, upper=7.0)
    out["redshift_clipped"] = z
    out["redshift_log1p"] = np.log1p(z)

    # Explicit one-hot for categorical features (do not rely on downstream inference)
    spectral_order = ["M", "O/B", "G/K", "A/F"]
    galaxy_order = ["Blue_Cloud", "Red_Sequence"]
    spectral = pd.Categorical(out["spectral_type"], categories=spectral_order)
    galaxy = pd.Categorical(out["galaxy_population"], categories=galaxy_order)
    out = pd.concat(
        [
            out,
            pd.get_dummies(spectral, prefix="spectral_type", dtype="uint8"),
            pd.get_dummies(galaxy, prefix="galaxy_population", dtype="uint8"),
        ],
        axis=1,
    )

    # SDSS ugriz effective centers and FWHM-like band widths (Å), fixed constants
    line_wls = np.array(
        [
            1216.0,
            1549.0,
    ...
    band_centers = np.array([3551.0, 4686.0, 6166.0, 7480.0, 8931.0], dtype=np.float64)
    band_widths = np.array([575.0, 1380.0, 1370.0, 1528.0, 950.0], dtype=np.float64)

    z_term = (1.0 + z.to_numpy(dtype=np.float64))[None, :]
    observed = line_wls[:, None] * z_term  # (n_lines, n_rows)

    # Band geometry
    half_width = band_widths / 2.0
    band_min = band_centers - half_width
    band_max = band_centers + half_width

    dist = np.abs(
        observed[:, :, None] - band_centers[None, None, :]
    )  # (n_lines, n_rows, n_bands)
    dist_norm = dist / (band_widths[None, None, :] + 1e-9)
    sigma = band_widths[None, None, :] / 2.355
    affinity = np.exp(-0.5 * (dist / (sigma + 1e-9)) ** 2)

    nearest_band = np.argmin(dist, axis=2)  # (n_lines, n_rows)
    global_optical_min = band_min.min()
    global_optical_max = band_max.max()
    filter_edges = np.sort(np.concatenate([band_min, band_max]))

    # Line-level geometry features
    line_ints = line_wls.astype(int)
    for i, lw in enumerate(line_ints):
        line_obs = observed[i]
        d_norm_i = dist_norm[i]
        aff_i = affinity[i]
        near_i = nearest_band[i]

        for b, bname in enumerate(band_names):
            out[f"line_{lw}_to_{bname}_dist_norm"] = d_norm_i[:, b]
            out[f"line_{lw}_to_{bname}_aff"] = aff_i[:, b]
            out[f"line_{lw}_nearest_{bname}"] = (near_i == b).astype("uint8")

        out[f"line_{lw}_nearest_band_code"] = near_i.astype("uint8")
        out[f"line_{lw}_min_band_dist_norm"] = d_norm_i.min(axis=1)
        out[f"line_{lw}_visible_optical"] = (
            (line_obs >= global_optical_min) & (line_obs <= global_optical_max)
        ).astype("uint8")
        gap_distance = np.min(np.abs(line_obs[:, None] - filter_edges[None, :]), axis=1)
        out[f"line_{lw}_min_filter_gap_distance"] = gap_distance
        out[f"line_{lw}_min_filter_gap_distance_norm"] = gap_distance / 1000.0
        out[f"line_{lw}_obs_log"] = np.log1p(np.abs(line_obs))

    # Per-band and pairwise aggregate geometry features
    band_aff_sum = affinity.sum(axis=0)  # (n_rows, n_bands)
    mag_cols = ["u", "g", "r", "i", "z"]

    for b, bname in enumerate(band_names):
        band_aff_col = band_aff_sum[:, b]
        out[f"line_aff_{bname}_sum"] = band_aff_col.astype("float32")
        out[f"line_aff_{bname}_x_mag"] = band_aff_col * out[mag_cols[b]]

    # Blue/Red line separation, to separate UV-optical vs optical-red diagnostics
    blue_mask = line_wls < 4500.0
    blue_per_band = affinity[blue_mask].sum(axis=0)
    red_per_band = affinity[~blue_mask].sum(axis=0)

    out["line_aff_blue_minus_red_total"] = (
        blue_per_band.sum(axis=1) - red_per_band.sum(axis=1)
    ).astype("float32")
    out["line_aff_blue_red_ratio"] = (blue_per_band.sum(axis=1) + 1e-6) / (
        red_per_band.sum(axis=1) + 1e-6
    )

    for b, bname in enumerate(band_names):
        out[f"line_aff_{bname}_blue_minus_red"] = (
            blue_per_band[:, b] - red_per_band[:, b]
        ).astype("float32")

    # Adjacent color-affinity interactions
    color_pairs = [("u", "g"), ("g", "r"), ("r", "i"), ("i", "z")]
    color_idx = [(0, 1), (1, 2), (2, 3), (3, 4)]
    for (c1, c2), (i1, i2) in zip(color_pairs, color_idx):
        color = (out[c1] - out[c2]).astype("float32")
        out[f"line_aff_{c1}_{c2}_avg_color"] = (
            color * ((band_aff_sum[:, i1] + band_aff_sum[:, i2]) / 2.0)
        ).astype("float32")
        out[f"line_aff_{c1}_{c2}_diff_sum"] = (
            (affinity[:, :, i1] - affinity[:, :, i2]).sum(axis=0) / float(len(line_wls))
        ).astype("float32")

    out["line_aff_global_sum"] = band_aff_sum.sum(axis=1).astype("float32")
    return out
