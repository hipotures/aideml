...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    centers = {"u":3543., "g":4770., "r":6231., "i":7625., "z":9134.}
    lines = {"LyA":1216., "MgII":2798., "OII":3727., "Hbeta":4861., "OIII":5007., "Halpha":6563.}
    z = np.clip(_num(out, "redshift"), -0.5, 7.0)
    for lname, wl in lines.items():
        obs = wl * (1.0 + z)
        dists = []
        for b, cen in centers.items():
            d = np.abs(obs - cen) / cen
            out[f"line_{lname}_{b}_dist_norm"] = d.astype("float32")
            out[f"line_{lname}_{b}_affinity"] = np.exp(-0.5 * np.square(d / 0.15)).astype("float32")
            dists.append(d)
        out[f"line_{lname}_min_center_normdist"] = np.vstack(dists).min(axis=0).astype("float32")
        out[f"line_{lname}_visible"] = ((obs >= 3263) & (obs <= 9889)).astype("int8")
    _color_block(out)
    return out
