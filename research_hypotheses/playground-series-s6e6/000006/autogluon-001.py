...
def preprocess(df: pd.DataFrame, aux: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep a legacy reference and frequency encoding first
    out["spectral_type_code"] = out["spectral_type"].astype("category").cat.codes
    out["spectral_type_freq"] = out["spectral_type"].map(
        out["spectral_type"].value_counts(dropna=False)
    )

    out["galaxy_population_code"] = (
        out["galaxy_population"].astype("category").cat.codes
    )
    out["galaxy_population_freq"] = out["galaxy_population"].map(
        out["galaxy_population"].value_counts(dropna=False)
    )

    # Ordered, semantically motivated category coding
    spectral_order = {"O/B": 0, "A/F": 1, "G/K": 2, "M": 3}
    galaxy_order = {"Blue_Cloud": 0, "Red_Sequence": 1}

    out["spectral_type_temp_order"] = (
        out["spectral_type"].map(spectral_order).fillna(-1).astype(np.int8)
    )
    out["galaxy_population_red_order"] = (
        out["galaxy_population"].map(galaxy_order).fillna(-1).astype(np.int8)
    )
    out["spectral_type_temp_order_sq"] = out["spectral_type_temp_order"] ** 2

    # Core numeric features
    out["redshift_tanh"] = np.tanh(out["redshift"])
    ...
    out["mag_mean"] = (out["u"] + out["g"] + out["r"] + out["i"] + out["z"]) / 5.0
    out["mag_slope"] = (out["u"] + out["r"] - out["g"] - out["i"]) / 4.0
    out["mag_curvature"] = (
        out["color_u_g"] - out["color_g_r"] - out["color_r_i"] + out["color_i_z"]
    )
    ...
    for col in core_interactions:
        out[f"spectral_type_temp_x_{col}"] = out["spectral_type_temp_order"] * out[col]
        out[f"galaxy_population_red_x_{col}"] = (
            out["galaxy_population_red_order"] * out[col]
        )

    # Pruning variant: remove arbitrary codes/frequency columns after ordered interactions are built
    out = out.drop(
        columns=[
            "spectral_type_code",
