...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _color_block(out)
    cols = [c for c in ["u_g", "g_r", "r_i", "i_z", "u_z", "redshift"] if c in out.columns]
    if cols:
        z = (out[cols] - out[cols].median()) / (out[cols].std().replace(0, 1.0) + 1e-6)
        out["locus_norm"] = np.sqrt(np.square(z).sum(axis=1)).astype("float32")
        out["locus_l1"] = z.abs().sum(axis=1).astype("float32")
        out["locus_max_abs"] = z.abs().max(axis=1).astype("float32")
        out["locus_density_proxy"] = (1.0 / (1.0 + out["locus_norm"])).astype("float32")
        for c in cols:
            out[f"{c}_std_resid"] = z[c].astype("float32")
    return out
