...
def blend_categorical_columns(values: list[pd.Series], weights: tuple[float, ...]) -> pd.Series:
    rows = []
    for row_values in zip(*[series.astype(str).tolist() for series in values], strict=True):
        totals: dict[str, float] = {}
        for label, weight in zip(row_values, weights, strict=True):
            totals[label] = totals.get(label, 0.0) + float(weight)
        rows.append(sorted(totals.items(), key=lambda item: (-item[1], item[0]))[0][0])
...
def score_oof(target: pd.Series, prediction: pd.Series, metric_name: str) -> float:
    ...
    if is_numeric_series(target) and is_numeric_series(prediction):
...
def evaluate_blend_spec(
    ...
    *,
    ...
    test_frames = [read_prediction_file(record_test_or_submission_path(record)) for record in selected]
    ...
    if spec.blend_kind == "oof":
        oof_frames = [read_prediction_file(record_oof_path(record)) for record in selected]
...
def choose_modes(records: list[dict[str, Any]], *, target_col: str) -> list[str]:
    for record in records:
...
def candidate_specs(
    ...
    *,
    ...
    modes = choose_modes(pool, target_col=target_col)
    specs = []
    for size in (2, 3, 4, 5):
        if len(pool) < size:
            continue
...
def select_new_blends(
    ...
    *,
    ...
    for blend_kind in ("oof", "submission"):
        for spec in candidate_specs(ready, blend_kind=blend_kind, target_col=target_col, top_k=top_k):
            if spec.recipe_hash in seen_recipe:
                continue
            try:
                ...
                continue
            if result.submission_sha256 in seen_sha:
                continue
            evaluated[blend_kind].append(result)
        if blend_kind == "oof":
            ...
            evaluated[blend_kind].sort(key=lambda item: item.component_score, reverse=True)
    ...
    def take(kind: str, quota: int) -> None:
        ...
        for result in evaluated[kind]:
            if len([item for item in selected if item.spec.blend_kind == kind]) >= quota:
                return
            if result.spec.recipe_hash in seen_recipe or result.submission_sha256 in seen_sha:
                continue
