...
def numbered_panel_title(title: str, number: int, *, width: int = 100) -> Text:
    ...
    title_width = max(7, int(width))
    max_title_width = max(1, title_width - number_text.cell_len - 5)
    if title_text.cell_len > max_title_width:
        title_text.truncate(max_title_width, overflow="ellipsis")

    title_start = max(1, (title_width - title_text.cell_len) // 2)
    left_padding = title_start - 1
    number_start = title_width - number_text.cell_len - 1
    middle_padding = max(
        0,
        number_start - left_padding - title_text.cell_len - 3,
    )
    result = Text("─" * left_padding, no_wrap=True)
    result.append(" ")
    result.append_text(title_text)
    result.append(" ")
    result.append("─" * middle_padding)
    result.append(" ")
...
class _NumberedPanel(Panel):
    """Panel variant without Rich's automatic one-cell title padding."""
